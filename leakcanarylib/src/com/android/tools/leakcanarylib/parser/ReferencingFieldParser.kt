...
class ReferencingFieldParser {
  companion object {
     ...
     *
     * @param referenceLines List of lines from the heap analysis output representing the field reference. The first line typically contains
     *   the reference path, and the second line may indicate if the field is a prime suspect (underlined).
     * @param additionalLinesPrefix A prefix to remove from each line before parsing.This is usually used to strip away indentation or
     *   common prefixes in the output.
    ...
    fun parseReferencingField(referenceLines: List<String>, additionalLinesPrefix: String): ReferencingField {
       ...
       */
      val referencePathLine =
        referenceLines.joinToString(separator = "") { line -> line.removePrefix(additionalLinesPrefix).trim() }.filter { it != '~' }
    ...
    /**
     * Parses a single line representing a reference path.This method extracts the field reference type, owning class name, and reference
     * display name from the reference path line.
     *
     * @param referencePathLine A single line representing the reference path.
     * @return A list containing three elements, in the following order: [0]- The field reference type as a string (e.g., "STATIC_FIELD",
     *   "INSTANCE_FIELD"). [1]- The reference display name (e.g., "myField"). [2]- The simple name of the owning class.
     */
    private fun parseReferencePath(referencePathLine: String): List<String> {
      ...
      val referencePathLineTrimmed = referencePathLine.substring(referencePathLineStartingIndex).trim()
      ...
      when {
        ...
        referencePathLineTrimmed.contains("[") -> {
          ...
          val arrayEndIndex = if (endIndex == -1) referencePathLineTrimmed.length else endIndex
