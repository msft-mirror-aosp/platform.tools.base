...
class NodeParser {
  ...
  companion object {
    private val leakingStatusRegex = Regex("""Leaking: (YES|NO|UNKNOWN)(?: \((.*)\))?""")
    private val leakingStatusMultiLineRegex = Regex("""Leaking:\s*(YES|NO|UNKNOWN)\s*\((.*)""")
    private val retainingRegex = Regex("""Retaining (\d+(?:\.\d+)?)\s*(\w+)?B? in (\d+) objects""")

    /**
     * Parses a list of strings representing a leak trace node and its associated reference line (if present). This method first separates
     * the lines describing the leaking object itself from the reference lines, then parses each part separately and combines them into a
     * single `Node`.
     *
    ...
    fun parse(
      ...
      val firstReferenceLineIndex = lines.indexOfFirst { it.startsWith(referenceLinePrefix) }

      val (objectLines, referenceLines) =
        if (firstReferenceLineIndex == -1) {
          Pair(lines, emptyList())
        } else {
          Pair(lines.subList(0, firstReferenceLineIndex), lines.subList(firstReferenceLineIndex, lines.size))
        }

      val leakingNode = parseLeakNode(objectLines, firstLinePrefix, multiLinePrefix, additionalLinesPrefix)
      leakingNode.referencingField = ReferencingFieldParser.parseReferencingField(referenceLines, additionalLinesPrefix)
      return leakingNode
    }

    /**
     * Parses a list of strings representing the core information of a leak trace node (excluding reference lines). This method extracts
     * details like the class name, type, leaking status, retained memory information, and any additional notes.
     *
     * @param inputLines: The list of lines to parse.
     ...
     */
    fun parseLeakNode(inputLines: List<String>, firstLinePrefix: String, multiLinePrefix: String, additionalLinesPrefix: String): Node {
      ...
      for (line in inputLines) {
        when {
          ...
          leakingStatusRegex.matches(line.removePrefix(additionalLinesPrefix)) -> {
            val matchResult = leakingStatusRegex.find(line.removePrefix(additionalLinesPrefix))!!
          ...
          leakingStatusMultiLineRegex.matches(line.removePrefix(additionalLinesPrefix)) -> {
            val matchResult = leakingStatusMultiLineRegex.find(line.removePrefix(additionalLinesPrefix))!!
            leakingStatus = LeakingStatus.fromString(matchResult.groupValues[1])
          ...
          leakingMultiLine -> {
            leakingStatusReasonBuilder.append(" ").append(line.removePrefix(additionalLinesPrefix))

            leakingMultiLine = !line.endsWith(")")
            if (!leakingMultiLine) leakingStatusReason = leakingStatusReasonBuilder.toString().trimEnd().dropLast(1)
          }

          retainingRegex.matches(line.removePrefix(additionalLinesPrefix)) -> {
            val matchResult = retainingRegex.find(line.removePrefix(additionalLinesPrefix))!!
      ...
      return Node(type, className, leakingStatus, leakingStatusReason, retainedHeapSize, retainedObjectCount, notes, null)
    }
