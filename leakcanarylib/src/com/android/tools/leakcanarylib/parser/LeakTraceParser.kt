 ...
 *
 * This class provides functionality to extract structured information about GC roots,reference paths, and leaking objects from the raw text
 * output.
 */
class LeakTraceParser {
  ...
  companion object {
     ...
     */
    fun parseLeakTrace(lines: List<String>, prefixReferencePath: String, prefixLeakingObject: String): LeakTrace {
    ...
    /**
     * Parses multiple leak traces from a list of lines. This method is designed to handle cases where a leak analysis tool reports multiple
     * leaks traces with the same signature, consolidating them into a list of `LeakTrace` objects.
     *
    ...
    fun parseLeakTraces(leakLines: List<String>): Pair<List<LeakTrace>, Int> {
      ...
      val lineCount = leakLines.indices.find { leakLines[it].startsWith("Displaying only") }

      var leakTraceCount = if (lineCount == null) 1 else parseTraceCount(leakLines[lineCount])

      val lines = leakLines.subList(leakLines.indices.find { leakLines[it].startsWith("Signature:") } ?: 0, leakLines.size)

      if (lines.isEmpty()) return Pair(leakTraces, leakTraceCount)
     ...
     */
    private fun extractReferencePathLines(lines: List<String>, pathPrefix: String, objectPrefix: String): List<String> {
     ...
     */
    private fun extractLeakingObjectLines(lines: List<String>, objectPrefix: String): List<String> {
      ...
      return linesAfterReferences
        .takeWhile { line -> !line.trim().startsWith(REFERENCE_PREFIX) && !line.trim().startsWith(GC_ROOT_PREFIX) }
     ...
     */
    private fun parseReferenceNodes(lines: List<String>, prefix: String): List<Node> {
