...
class LeakParser {
  ...
  companion object {
    ...
    private fun extractBytesRetained(lines: List<String>): Int {
      return lines.firstOrNull { it.contains("bytes retained") }?.split(" ")?.firstOrNull()?.toIntOrNull() ?: 0
    }
    ...
    private fun extractSignature(lines: List<String>): String {
      return lines.firstOrNull { it.contains("Signature:") }?.replace("Signature: ", "")?.trim() ?: ""
    }
  }
}
