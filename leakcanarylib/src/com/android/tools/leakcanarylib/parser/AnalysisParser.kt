...
/**
 * A utility class for parsing the string representation of heap analysis results into corresponding `AnalysisSuccess` or `AnalysisFailure`
 * objects.
 */
  ...
  companion object {
    ...
    fun analysisFailureFromString(text: String): AnalysisFailure? {
      return try {
        ...
        AnalysisFailure(heapDumpFile, heapDumpTimestamp, heapDumpDuration, heapAnalysisDuration, exception)
      } catch (e: Exception) {
    ...
    private fun extractHeapDumpDuration(lines: List<String>): Long {
      ...
      return durationMillisLine?.substringAfter(": ")?.trim()?.removeSuffix(" ms")?.toLongOrNull() ?: -1
    }
    ...
    private fun extractHeapAnalysisDuration(lines: List<String>): Long {
      ...
      return durationLine?.substringAfter(": ")?.trim()?.removeSuffix(" ms")?.toLong() ?: 0
    }
    ...
    private fun extractSection(text: String, sectionName: String): String {
      ...
      val sectionSubList = section.take(sectionIndex).trim().trim().split("\n")
