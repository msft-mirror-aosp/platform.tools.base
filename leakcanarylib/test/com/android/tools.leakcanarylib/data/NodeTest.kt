...
class NodeTest {
  ...
  fun `test parse valid leak trace object from string`() {
    ...
    val expected =
      Node(
        ...
        leakingStatus = LeakingStatus.YES,
        leakingStatusReason =
          "ObjectWatcher was watching this because " +
    ...
    val toStringOfActual =
      actual.toString(firstLinePrefix = "╰→ ", additionalLinesPrefix = "$ZERO_WIDTH_SPACE     ", showLeakingStatus = true)
  ...
  fun `test leak trace object to string`() {
    ...
    val actual = input.toString(firstLinePrefix = "╰→ ", additionalLinesPrefix = "$ZERO_WIDTH_SPACE     ", showLeakingStatus = true)
