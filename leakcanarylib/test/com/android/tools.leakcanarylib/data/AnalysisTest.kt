...
class AnalysisTest {
  ...
  fun `heap analysis success - verify application and library leak`() {
    ...
    val expectedLeakString =
      heapAnalysisText.lines().filterNot { it.contains("Leak pattern:") || it.contains("Description:") }.joinToString("\n").trim()

    // Filter out the lines to ignore in the actual output
    val actualLeakString =
      actualToString.lines().filterNot { it.contains("Leak pattern:") || it.contains("Description:") }.joinToString("\n").trim()
  ...
  private fun compareLeaks(expected: String, actual: String) {
    // Filter out the lines to ignore in the expected output
    val expectedLibraryLeakString = expected.lines().filterNot { it.contains("RandomAccess[bytes=") }.joinToString("\n").trim()

    // Filter out the lines to ignore in the actual output
    val actualLibraryLeakString = actual.lines().filterNot { it.contains("RandomAccess[bytes=") }.joinToString("\n").trim()
