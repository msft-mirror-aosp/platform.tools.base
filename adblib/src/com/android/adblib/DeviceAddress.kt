...
/**
 * An abstraction over a device address, typically of the form `host:port`, similar to a [InetSocketAddress].
 *
 * To ensure future compatibility, converting from/to an [InetSocketAddress] is supported but not guaranteed by future implementations.
 * However, if the address format is irrelevant to the consumer, [DeviceAddress] allows round-tripping an arbitrary address from/to the ADB
 * server without relying on the specific format of the address.
 */
data class DeviceAddress(
   ...
   *
   * @throws IllegalArgumentException if this [DeviceAddress] is not compatible with an [InetSocketAddress] representation.
   */
