 ...
 *
 * Once a [AbbCommand] is configured with various `withXxx` methods, use the [execute] method to launch the abb command execution, returning
 * a [Flow&lt;T&gt;][Flow].
 *
...
interface AbbCommand<T> {
  ...
  /**
   * Applies a [ShellV2Collector] to transfer the raw binary abb command output. This changes the type of this [AbbCommand] from [T] to the
   * final target type [U].
   */
  fun <U> withCollector(collector: ShellV2Collector<U>): AbbCommand<U>

  /**
   * Applies a legacy [ShellCollector] to transfer the raw binary abb command output. This changes the type of this [AbbCommand] from [T] to
  ...
  /**
   * Applies a [timeout] that triggers [TimeoutException] exception if the abb command does not terminate within the specified [Duration].
   *
  ...
  /**
   * Allows [execute] to fall back to [AdbDeviceServices.abb_exec] if [AdbDeviceServices.abb] is not available or not allowed.
   *
   * The default value is `true`.
   */
  ...
  /**
   * When using ABB_EXEC, this method allows to specify whether the device channel output is shutdown after piping `stdinChannel`.
   *
   * The default value is `true`.
   */
  ...
  /**
   * Returns a [Flow] that executes the abb command on the device, according to the various customization rules set by the `withXxx`
   * methods.
   *
  ...
  /**
   * Execute the abb command on the device, assuming there is a single output emitted by [AbbCommand.withCollector]. The single output is
   * passed as an argument to [block] **while the shell command is still active**.
   *
