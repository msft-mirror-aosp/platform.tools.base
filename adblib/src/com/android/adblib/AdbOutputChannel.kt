 ...
 *
 * Note: This is the equivalent of [java.io.OutputStream] with suspending operations instead of blocking ones.
 */
interface AdbOutputChannel : AutoCloseable {

  /**
   * Closes this output channel and releases any system resources associated with it. A closed channel cannot perform output operations and
   * cannot be reopened. Any currently suspended [writeBuffer] operation is promptly cancelled and throws [AsynchronousCloseException]
   *
   ...
   * of bytes written.
   *
   * If a failure occurs, an [java.io.IOException] is thrown, and the [ByteBuffer] state is undefined (i.e. some bytes may have been
   * written, but not all).
   ...
   */
  suspend fun writeBuffer(buffer: ByteBuffer, timeout: Long = Long.MAX_VALUE, unit: TimeUnit = TimeUnit.MILLISECONDS)
...
/**
 * Writes up to [ByteBuffer.remaining] bytes from [buffer] to the underlying channel, updating [ByteBuffer.position] to match the number of
 * bytes written.
 *
 * Returns the number of bytes written on success. The return value is zero if and only if [ByteBuffer.remaining] is zero.
 *
 * If a failure occurs, an [java.io.IOException] is thrown, and the [ByteBuffer] state is undefined (i.e. some bytes may have been written,
 * but not all).
 *
 * Throws a [TimeoutException] in case the data cannot be written before the timeout expires.
 */
suspend inline fun AdbOutputChannel.write(buffer: ByteBuffer, timeout: Long = Long.MAX_VALUE, unit: TimeUnit = TimeUnit.MILLISECONDS): Int {
