...
/**
 * Supports customization of various aspects of the execution of a shell command on a device, including automatically falling back from
 * [AdbDeviceServices.shellV2] to legacy protocols on older devices.
 *
 * Once a [ShellCommand] is configured with various `withXxx` methods, use the [execute] method to launch the shell command execution,
 * returning a [Flow&lt;T&gt;][Flow].
 *
...
interface ShellCommand<T> {
  ...
  /**
   * Applies a [ShellV2Collector] to transfer the raw binary shell command output. This change the type of this [ShellCommand] from [T] to
   * the final target type [U].
   */
  fun <U> withCollector(collector: ShellV2Collector<U>): ShellCommand<U>

  /**
   * Applies a legacy [ShellCollector] to transfer the raw binary shell command output. This change the type of this [ShellCommand] from [T]
   * to the final target type [U].
   */
  ...
  /**
   * Applies a [timeout] that triggers [TimeoutException] exception if the shell command does not terminate within the specified [Duration].
   *
  ...
  /**
   * Applies a [timeout] that triggers a [TimeoutException] exception when the command does not generate any output (`stdout` or `stderr`)
  ...
  /**
   * Overrides the [Flow] of [ShellWindowSize] to send to the interactive shell on the device. This flow is typically used to notify the
   * interactive shell on the device that the terminal window size (on the host) has changed size.
   *
  ...
  /**
   * Allows [execute] to fall back to [AdbDeviceServices.exec] if [AdbDeviceServices.shellV2] is not available or not allowed.
   *
   * The default value is `false`, because exec is not as general purpose as [allowShellV2] and [allowLegacyExec] (see
   * [AdbDeviceServices.exec] for documentation).
   */
  fun allowLegacyExec(value: Boolean): ShellCommand<T>

  /**
   * Allows [execute] to fall back to [AdbDeviceServices.shell] if [AdbDeviceServices.shellV2] and [AdbDeviceServices.exec] are not
  ...
  /**
   * When [execute] falls back to using the `shell v1` or `exec` protocols, this method allows to specify whether the device channel output
  ...
  /**
   * When [execute] falls back to using the [AdbDeviceServices.shell] service, and when the device API <= 23, this option allows [execute]
  ...
  /**
   * Allows overriding the shell command to [execute] on the device just before execution starts, when the [Protocol] to be used is known.
   *
   * This can be useful, for example, for providing custom shell handling in case [AdbDeviceServices.shellV2] is not supported and [execute]
   * has to fall back to [AdbDeviceServices.exec].
   *
  ...
  /**
   * Returns a [Flow] that executes the shell command on the device, according to the various customization rules set by the `withXxx`
   * methods.
   *
   * If [withCollector] or [withLegacyCollector] was not invoked before [execute], an [IllegalArgumentException] is thrown, as a shell
   * collector is mandatory.
   *
  ...
  /**
   * Execute the shell command on the device, assuming there is a single output emitted by [ShellCommand.withCollector]. The single output
   * is passed as an argument to [block] **while the shell command is still active**.
   *
  ...
  companion object {
    ...
    /**
     * Pattern to escape filenames for shell command consumption. This pattern identifies any special characters that need to be escaped
     * with a backslash.
     */
 ...
 */
class TextShellCollector(bufferCapacity: Int = 256, charset: Charset = AdbProtocolUtils.ADB_CHARSET) :
  ...
  /** We store the lambda in a field to avoid allocating a new lambda instance for every invocation of [AdbBufferDecoder.decodeBuffer] */
...
/**
 * A [ShellV2Collector] implementation that concatenates the entire output (and `stderr`) of the command execution into a single
 * [ShellCommandOutput] instance
 *
 * Note: This should be used only if the output of a shell command is expected to be somewhat small and can easily fit into memory.
 */
class TextShellV2Collector(bufferCapacity: Int = 256) : ShellV2Collector<ShellCommandOutput>, ShellCollectorCapabilities {
  ...
  override suspend fun collectStdout(collector: FlowCollector<ShellCommandOutput>, stdout: ByteBuffer) {
    stdoutCollector.collect(stdoutFlowCollector, stdout)
  }

  override suspend fun collectStderr(collector: FlowCollector<ShellCommandOutput>, stderr: ByteBuffer) {
  ...
  override suspend fun end(collector: FlowCollector<ShellCommandOutput>, exitCode: Int) {
    ...
    val result = ShellCommandOutput(stdoutFlowCollector.value ?: "", stderrFlowCollector.value ?: "", exitCode)
...
class ShellCommandOutput(
  ...
  val stdout: String,
  /**
   * The shell command error output ("stderr") captured as a single string, only set if [ShellCommand.Protocol] is
   * [ShellCommand.Protocol.SHELL_V2].
   *
   * @see ShellCommand.Protocol
   */
  val stderr: String,
  /** The shell command exit code, only set if [ShellCommand.Protocol] is [ShellCommand.Protocol.SHELL_V2]. */
  val exitCode: Int,
)
...
/** A [ShellV2Collector] implementation that collects `stdout` and `stderr` as sequences of [text][String] lines */
class LineShellV2Collector(bufferCapacity: Int = 256) : ShellV2Collector<ShellCommandOutputElement> {
  ...
  private val stdoutFlowCollector = LineFlowCollector { line -> ShellCommandOutputElement.StdoutLine(line) }
  private val stderrFlowCollector = LineFlowCollector { line -> ShellCommandOutputElement.StderrLine(line) }
  ...
  override suspend fun collectStdout(collector: FlowCollector<ShellCommandOutputElement>, stdout: ByteBuffer) {
    stdoutFlowCollector.forwardingFlowCollector = collector
    stdoutCollector.collect(stdoutFlowCollector, stdout)
  }

  override suspend fun collectStderr(collector: FlowCollector<ShellCommandOutputElement>, stderr: ByteBuffer) {
    stderrFlowCollector.forwardingFlowCollector = collector
    stderrCollector.collect(stderrFlowCollector, stderr)
  }
  ...
  class LineFlowCollector(private val builder: (String) -> ShellCommandOutputElement) : FlowCollector<String> {
...
sealed class ShellCommandOutputElement {
  ...
  /** The exit code of the shell command. This is always the last entry of the [Flow] returned by [AdbDeviceServices.shellAsLines]. */
...
class LineBatchShellV2Collector(bufferCapacity: Int = 256) : ShellV2Collector<BatchShellCommandOutputElement> {
  ...
  private val stdoutFlowCollector = LineBatchFlowCollector { lines -> BatchShellCommandOutputElement.StdoutLine(lines) }
  private val stderrFlowCollector = LineBatchFlowCollector { lines -> BatchShellCommandOutputElement.StderrLine(lines) }
  ...
  override suspend fun collectStdout(collector: FlowCollector<BatchShellCommandOutputElement>, stdout: ByteBuffer) {
    stdoutFlowCollector.forwardingFlowCollector = collector
    stdoutCollector.collect(stdoutFlowCollector, stdout)
  }

  override suspend fun collectStderr(collector: FlowCollector<BatchShellCommandOutputElement>, stderr: ByteBuffer) {
    stderrFlowCollector.forwardingFlowCollector = collector
    stderrCollector.collect(stderrFlowCollector, stderr)
  }

  override suspend fun end(collector: FlowCollector<BatchShellCommandOutputElement>, exitCode: Int) {
    stdoutFlowCollector.forwardingFlowCollector = collector
    stdoutCollector.end(stdoutFlowCollector)
  ...
  class LineBatchFlowCollector(private val builder: (List<String>) -> BatchShellCommandOutputElement) : FlowCollector<List<String>> {
...
sealed class BatchShellCommandOutputElement {
  ...
  /** The exit code of the shell command. This is always the last entry of the [Flow] returned by [AdbDeviceServices.shellAsLineBatches]. */
...
/**
 * A [ShellV2Collector] that exposes the output of a [shellCommand] as a [InputChannelShellOutput], itself exposing `stdout`, `stderr` as
 * [AdbInputChannel] instances.
 */
class InputChannelShellCollector(session: AdbSession, bufferSize: Int = DEFAULT_BUFFER_SIZE) :
  ...
  override suspend fun collectStdout(collector: FlowCollector<InputChannelShellOutput>, stdout: ByteBuffer) {
  ...
  override suspend fun collectStderr(collector: FlowCollector<InputChannelShellOutput>, stderr: ByteBuffer) {
 ...
 */
class OutputStreamCollector(adbSession: AdbSession, stdoutStream: OutputStream, stderrStream: OutputStream) : ShellV2Collector<Int> {
