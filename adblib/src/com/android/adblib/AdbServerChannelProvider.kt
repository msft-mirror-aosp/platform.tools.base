 ...
 *
 * This abstraction is used to ensure various service implementations don't depend on concrete implementations of acquiring connections to
 * the ADB host.
 *
...
interface AdbServerChannelProvider {
  ...
  /**
   * Opens a new [AdbChannel] to communicate with an ADB Server. Implementations can decide to re-use an existing ADB Server or start a new
   * instance on-demand if needed. Callers are responsible for [closing][AutoCloseable.close] the returned [AdbChannel] instance when done.
   *
   ...
   */
  suspend fun createChannel(timeout: Long = Long.MAX_VALUE, unit: TimeUnit = TimeUnit.MILLISECONDS): AdbChannel

  companion object {

    fun createOpenLocalHost(host: AdbSessionHost, portSupplier: suspend () -> Int = { DEFAULT_ADB_HOST_PORT }): AdbServerChannelProvider {
    ...
    fun createConnectAddressesWithServerStartup(host: AdbSessionHost): AdbServerChannelProvider {
      return AdbChannelProviderWithServerStartup(host, AdbServerChannelConnectOptions(DEFAULT_ADB_HOST_PORT))
    }
  }
}
