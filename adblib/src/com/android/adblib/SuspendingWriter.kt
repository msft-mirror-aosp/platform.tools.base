...
/**
 * Abstract class for writing to character streams using `suspend` functions. This is the coroutine
 * friendly  equivalent of [java.io.Writer].
 *
 * The [shutdown] method must always be called to ensure the final bytes of the output
 * are "flushed" successfully. This is required because some encoding use "surrogate" characters
 * and the caller may want to detect incomplete sequences of characters.
 */
abstract class SuspendingWriter(
    private val bufferCapacity: Int = 256
) : AutoShutdown {

    /**
     * Temporary buffer of [bufferCapacity] length, used to hold writes of (short) strings and
     * single characters
     */
    ...
    /**
     * Encodes and writes the contents of a [CharBuffer].
     * On success, all characters are written, and [CharBuffer.remaining] is equal to zero.
     */
    ...
    private fun getWriteBuffer(len: Int): CharBuffer {
        return if (len <= bufferCapacity) {
            ...
            (tempWriteBuffer ?: CharBuffer.allocate(bufferCapacity).also { tempWriteBuffer = it }).also {
                it.clear()
            }
        } else {
