...
sealed class AdbFailResponseException(val service: String, val failMessage: String) : IOException() {
...
/** Exception thrown when a `FAIL` response is received from the ADB host when executing a "host" (i.e. not device related) [service] */
@OptIn(
  /**
...
class AdbHostFailResponseException(service: String, failMessage: String) :
  AdbFailResponseException(service, failMessage), CopyableThrowable<AdbHostFailResponseException> {

  constructor(service: String, buffer: ByteBuffer) : this(service, extractMessageFromBuffer(buffer))

  override val message: String
    get() = "'$failMessage' error executing ADB service '$service'"

  override fun createCopy(): AdbHostFailResponseException {
    return AdbHostFailResponseException(service, failMessage).also { it.initCause(this) }
  }
}

/** Exception thrown when a `FAIL` response is received from the ADB host when executing [service] for a given [device]. */
@OptIn(
  /**
   * Needed to support [CopyableThrowable], see
   * https://github.com/Kotlin/kotlinx.coroutines/blob/master/docs/topics/debugging.md#stacktrace-recovery
...
class AdbDeviceFailResponseException(val device: DeviceSelector, service: String, failMessage: String) :
  AdbFailResponseException(service, failMessage), CopyableThrowable<AdbDeviceFailResponseException> {

  constructor(
    deviceSerial: DeviceSelector,
    service: String,
    buffer: ByteBuffer,
  ) : this(deviceSerial, service, extractMessageFromBuffer(buffer))

  override val message: String
    get() = "'$failMessage' error on ${device.shortDescription} executing service " + "'${service.sanitizeForLogging()}'"

  override fun createCopy(): AdbDeviceFailResponseException {
    return AdbDeviceFailResponseException(device, service, failMessage).also { it.initCause(this) }
  }
}
