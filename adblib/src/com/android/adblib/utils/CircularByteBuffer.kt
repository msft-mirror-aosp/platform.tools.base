...
/**
 * A fixed size circular buffer of bytes, i.e. a wrapper over a fixed size [ByteArray] than allows [writing][write] bytes "to the end" and
 * [reading][read] bytes "from the start" efficiently (i.e. without moving bytes in the array).
 *
...
class CircularByteBuffer(size: Int = DEFAULT_BUFFER_SIZE) {
  ...
  /** Index of the first byte of free space in the buffer (also index of last byte of available data). */
  ...
  /** The number of unused bytes, ranging from `zero`, when the buffer is full, to [capacity], when buffer is empty. */
  val remaining: Int
  ...
  /**
   * Appends bytes from [sourceBuffer] to the end of this [CircularByteBuffer], returning the number of bytes copied.
   * * If there is not enough [remaining] capacity in this [CircularByteBuffer], only a subset of [sourceBuffer] is copied over. In
   *   particular, if [CircularByteBuffer] is full, this method returns `zero` and no bytes are copied from [sourceBuffer].
   */
  ...
  /**
   * Reads bytes from this [CircularByteBuffer] into [targetBuffer], returning the number of bytes read.
   * * If there is not enough room in [targetBuffer], only a subset of this [CircularByteBuffer] is copied over.
   * * If [CircularByteBuffer] is empty, this method returns `zero` and no bytes are copied to [targetBuffer].
   */
