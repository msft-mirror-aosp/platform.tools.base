...
/**
 * A wrapper around a [CharsetDecoder] to decode a sequence of bytes received from an ADB device.
 *
...
class AdbBufferDecoder(bufferCapacity: Int = 256, charset: Charset = AdbProtocolUtils.ADB_CHARSET) {
  ...
  /** The [CharBuffer] used to temporarily store decoded characters when calling [CharsetDecoder.decode] */
  private val charBuffer: CharBuffer = CharBuffer.allocate(bufferCapacity)

  /**
   * (Optional) [ByteBuffer] used to store leftover bytes from the previous [decodeBuffer] call, in case a multi-byte character sequence was
   * not fully terminated at the end of the processed buffer, e.g. we are processing an UTF-8 character of 3 bytes but only 1 or 2 were
   * present at the end of the [ByteBuffer] passed to [decodeBuffer].
   *
  ...
  /** Decode characters in a [ByteBuffer], invoking [processor] when a valid character sequence has been decoded. */
  fun decodeBuffer(buffer: ByteBuffer, processor: (CharBuffer) -> Unit) {
    leftoverBuffer?.apply { decodeLeftoverBuffer(this, buffer, processor) }
  ...
  private fun decodeLeftoverBuffer(leftovers: ByteBuffer, buffer: ByteBuffer, processor: (CharBuffer) -> Unit) {
