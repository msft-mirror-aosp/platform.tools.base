 ...
 *
 * This class determines *if* a warning should be logged by the caller, allowing for
 * periodic re-logging of ongoing issues and automatic cleanup of stale warnings.
 *
 * @param staleThreshold Time after which an inactive warning is purged. Must be positive.
 * @param repeatLogPeriod Time after which an identical warning should be logged again.
 * Must not be greater than `staleThreshold`.
 */
class WarningsTracker(
    ...
    init {
        require(staleThreshold > Duration.ZERO) {
            "staleThreshold must be positive."
        }
        repeatLogPeriod?.let {
     ...
     */
    fun getLogAction(key: String, failureReason: String): LogAction = synchronized(lock) {
        runCleanupIfNeeded()
        ...
        if (previousState?.failureReason != failureReason) {
            val skippedCount = if (previousState != null) {
        ...
        val shouldRepeat =
            repeatLogPeriod != null && Duration.between(
                previousState.lastLoggedTime,
                now
            ) > repeatLogPeriod
        if (shouldRepeat) {
        ...
        val newState = previousState.copy(
            lastSeenTime = now,
            consecutiveSeenCount = previousState.consecutiveSeenCount + 1
        )
        lastWarningStates[key] = newState
    ...
    /**
     * Notifies the tracker that an operation succeeded, checking if it was recovering
     * from a previously tracked warning.
     *
     ...
     */
    fun didRecover(key: String): RecoveryAction = synchronized(lock) {
        runCleanupIfNeeded()
    ...
    private fun cleanupStaleEntries() {
        ...
        lastWarningStates.entries.removeIf { entry ->
            Duration.between(entry.value.lastSeenTime, now) > staleThreshold
        }
    }
}

/**
 * Logs an info message according to the [WarningsTracker.LogAction], adding
 * a `[Skipped N]` prefix for repeated warnings.
 */
fun WarningsTracker.LogAction.logInfo(
  logger: AdbLogger,
 ...
 */
fun WarningsTracker.RecoveryAction.logInfo(
  logger: AdbLogger,
  messageProvider: () -> String
) {
    when (this) {
