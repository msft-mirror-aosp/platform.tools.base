...
object AdbProtocolUtils {
  ...
  /** The ADB protocol uses UTF-8 encoding when strings are serialized on the communication channel. */
  ...
  fun isStat(buffer: ByteBuffer): Boolean {
  ...
  /**
   * Encode an integer value into a 4 byte integer representing the hexadecimal string of that value.
   *
  ...
  /**
   * Create a "hex dump" string (for debugging/logging purpose) of up to [maxCount] characters of [status], leaving [ByteBuffer.position]
   * and [ByteBuffer.limit] untouched.
   */
