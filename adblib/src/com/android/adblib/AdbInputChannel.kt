 ...
 *
 * Note: This is the equivalent of [java.io.InputStream] with suspending operations instead of blocking ones.
 */
interface AdbInputChannel : AutoCloseable {

  /**
   * Closes this input channel and releases any system resources associated with it. A closed channel cannot perform input operations and
   * cannot be reopened. Any currently suspended [readBuffer] operation is promptly cancelled and throws [AsynchronousCloseException]
   *
   * Note: Implementations should be thread-safe to allow prompt cancellation of suspended operations.
   */
  override fun close()
   ...
   */
  suspend fun readBuffer(buffer: ByteBuffer, timeout: Long = Long.MAX_VALUE, unit: TimeUnit = TimeUnit.MILLISECONDS)
   ...
   *
   * If end of stream is reached before all bytes are read, throws an [java.io.EOFException] and the [ByteBuffer] state is undefined (i.e.
   * some bytes may have been read, but not all)
   * * Throws [ClosedChannelException] if [close] was previously called
   * * Throws [AsynchronousCloseException] if [close] is called __while this function is suspended__
   * * Throws [java.io.IOException] if an I/O occurs reading from the underlying resource
   * * Throws [TimeoutException] in case the data cannot be read before the timeout expires.
 ...
 *
 * Returns the number of bytes read (always at least `1`), or `-1` if end of stream is reached, or `0` if [ByteBuffer.remaining] is `0`.
 *
 * Throws an [java.io.IOException] in case of error, or a [TimeoutException] in case no data is available before [timeout] expires.
 */
suspend inline fun AdbInputChannel.read(buffer: ByteBuffer, timeout: Long = Long.MAX_VALUE, unit: TimeUnit = TimeUnit.MILLISECONDS): Int {
...
/**
 * Converts this [AdbInputChannel] to an [AdbChannelReader] for text reading operations. The returned [AdbChannelReader] should be
 * [closed][AutoCloseable] when done to ensure the [AdbInputChannel] is [closed][AutoCloseable].
 */
...
/**
 * Reads the contents of this [AdbInputChannel] in a concurrent coroutine of [scope] and returns a [ReceiveChannel] of [String] for each
 * line of text as lines are decoded using the provided [charset].
 *
 * When the [AdbInputChannel] is fully read, the returned [ReceiveChannel] is closed without a [cause][Throwable].
 *
 * If there is an exception reading or decoding the contents of [AdbInputChannel], the returned [ReceiveChannel] is closed with the
 * corresponding [Throwable] as the cause.
 */
