...
abstract class AdbLogger {
  ...
  /**
   * The current minimum logging [Level]. This [AdbLogger] instance logs messages only if they are of greater or equal severity than
   * [minLevel].
   *
   * For example, if [minLevel] is [Level.WARN], only [Level.WARN] and [Level.ERROR] messages will be logged, while [Level.INFO],
   * [Level.VERBOSE] and [Level.DEBUG] will be skipped.
   */
...
/** Wraps this [AdbLogger] instance as an [AdbLogger] instance that re-formats all messages with the given [prefix]. */
...
/**
 * Abstract implementation of [AdbLogger] that delegates to another [AdbLogger] instance after allowing derived classes to re-format the
 * message by implementing [formatMessage].
 */
...
/**
 * Factory of [AdbLogger] instances. See [AdbLoggerFactoryWithCache] for an implementation that uses a simple in-memory cache to minimize GC
 * allocations of [AdbLogger] instances.
 *
...
/** Implementation of [AdbLoggerFactory] that caches instances of [AdbLogger] created by the [createLogger] functions. */
abstract class AdbLoggerFactoryWithCache<T> : AdbLoggerFactory where T : AdbLogger {
   ...
   * * the use of the cache decreases GC allocation pressure, as adblib code tends to use non-static fields for [AdbLogger] instances.
   */
  ...
  /** Creates a [AdbLogger] instance of a given [Class] the first time the logger is requested through [createLogger]. */
  abstract fun createClassLogger(cls: Class<*>): T

  /** Creates an [AdbLogger] instance of a given [Category][String] the first time the logger is requested through [createLogger]. */
  abstract fun createCategoryLogger(category: String): T

  private fun synchronizedRootLogger(): T {
    return synchronized(this) { _rootLogger ?: createRootLogger().also { _rootLogger = it } }
  }

  private class LoggerCache<TKey : Any, T>(
    /**
     * Note: We use a [Function] lambda (as opposed to a `Kotlin` lambda) to ensure no allocation is performed when calling
