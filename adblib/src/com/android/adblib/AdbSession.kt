 ...
 * should be called when the session is not needed anymore, to ensure all pending operations and all optional state are released.
 *
...
interface AdbSession : AutoCloseable {
  ...
  /**
   * The parent [AdbSession] if this session is a child session, `null` otherwise. If there is a parent session, [scope] is a guaranteed by
   * be a child [CoroutineScope] of the parent session scope.
   */
  val parentSession: AdbSession?

  /**
   * The [AdbSessionHost] implementation provided by the hosting application for environment specific configuration.
   *
  ...
  /**
   * An [AdbChannelFactory] that can be used to create various implementations of [AdbChannel], [AdbInputChannel] and [AdbOutputChannel] for
   * files, streams, etc.
   *
   * @throws ClosedSessionException if this [AdbSession] has been [closed][close].
   */
   ...
   *
   * Useful when creating coroutines, e.g. with [CoroutineScope.launch], that need to be automatically cancelled when the session is
   * [closed][close].
   *
   * The scope uses a [SupervisorJob] so that child jobs don't cancel the scope if they fail with an exception.
   *
  ...
  /**
   * Thread safe in-memory cache to store objects for as long this [AdbSession] is active. Any value added to the cache that implements
   * [AutoCloseable] is [closed][AutoCloseable.close] when this [AdbSession] is closed.
   *
   * @throws ClosedSessionException if this [AdbSession] has been [closed][close].
   */
  ...
  companion object {
     ...
     *
     * @param host The [AdbSessionHost] implementation provided by the hosting application for environment specific configuration
    ...
    /**
     * Create a child [AdbSession] given its [parent session][parentSession]. The child session will use a child [CoroutineScope] of the
     * parent session, but can use a distinct [host] and [channelProvider] if necessary.
     */
    @JvmStatic
...
suspend fun <R> AdbSession.withErrorTimeout(timeout: Long, unit: TimeUnit, block: suspend CoroutineScope.() -> R): R {
 ...
 *   ``` val session: AdbSession session.scope.launch { session.trackDevices.flowOn(Dispatchers.Default).collect { // Collect until session
 *   scope is cancelled. } } ```
 * * Downstream flows may not be able to collect all values produced by the [StateFlow], because [StateFlow] use the
 *   [BufferOverflow.DROP_OLDEST] strategy. Downstream flows should keep track of [TrackedDeviceList.connectionId] to determine if
 *   [DeviceInfo.serialNumber] values can be compared against previously collected serial numbers.
...
fun AdbSession.trackDevices(retryDelay: Duration = property(TRACK_DEVICES_RETRY_DELAY)): StateFlow<TrackedDeviceList> {
  ...
  return this.cache.getOrPut(MyKey(retryDelay)) { SessionDeviceTracker(this, retryDelay) }.stateFlow
}

/** A [list][DeviceList] of [DeviceInfo] as collected by the [state flow][StateFlow] returned by [AdbSession.trackDevices]. */
class TrackedDeviceList(
  /**
   * Connection id, unique to each underlying [AdbHostServices.trackDevices] connection. [DeviceInfo.serialNumber] may not uniquely identify
   * a device if their connection ids are not the same.
   */
  val connectionId: Int,
  /** The [list][DeviceList] of [DeviceInfo], the list is always empty if the underlying connection to ADB failed. */
  devices: DeviceList,
  ...
  /** The last [Throwable] collected when the underlying ADB connection failed, or `null` if the underlying ADB connection is active. */
 ...
 * value.
 *
 * The flow terminates when the device is no longer connected or when the ADB connection is terminated.
 */
...
val AdbSession.connectedDevicesTracker: ConnectedDevicesTracker
  get() {
    return this.cache.getOrPut(ConnectedDevicesManagerKey) { ConnectedDevicesTrackerImpl(this) }
  }
...
val AdbSession.connectionStatusTracker: ConnectionStatusTracker
  get() {
    return this.cache.getOrPut(ConnectionStatusTrackerKey) { ConnectionStatusTrackerImpl(this) }
  }
...
val AdbSession.deviceCacheProvider: DeviceCacheProvider
  get() {
    return this.cache.getOrPut(DeviceCacheProviderKey) { ConnectedDevicesDeviceCacheProvider(this) }
  }
...
val AdbSession.activityManagerServices: AdbActivityManagerServices
  get() {
    return this.cache.getOrPut(AdbActivityManagerServicesKey) { AdbActivityManagerServicesImpl(this) }
  }
