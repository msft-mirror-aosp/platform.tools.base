...
/**
 * Utility class to keep track of how much time is left given an initial timeout. This is useful when a method receives a timeout parameter
 * and needs to perform multiple operations within that given timeout.
 *
 * Note: The implementation keeps track of time using [System.nanoTime] units, meaning this class is not suitable for timeouts longer than
 * ~290 years.
 */
internal class TimeoutTracker(
  ...
  /** Value of system nano time when the timer is created, meaning overflowing occurs every ~290+ years. */
  ...
  companion object {
    ...
    fun fromTimeout(unit: TimeUnit, timeout: Long): TimeoutTracker {
      return if (timeout == Long.MAX_VALUE) INFINITE else TimeoutTracker(timeout, unit)
    }
  }
}
...
fun remainingTimeoutToString(timeout: Long, unit: TimeUnit): String {
  return if (timeout == Long.MAX_VALUE) {
    ...
    String.format(Locale.ROOT, "%,.1f msec remaining", TimeUnit.MILLISECONDS.convert(timeout, unit).toDouble())
  }
}
