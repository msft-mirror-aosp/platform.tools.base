...
internal class AdbServiceRunner(val session: AdbSession, private val channelProvider: AdbServerChannelProvider) {
  ...
  /**
   * Opens an [AdbChannel] and invokes a service on the ADB host, then waits for an OKAY/FAIL response.
   *
   * In case of OKAY response, the returned [AdbChannel] is open and ready for the next steps of the communication protocol.
   *
   * In case of FAIL response, an [AdbFailResponseException] exception is thrown. The exception contains the error message included in the
   * FAIL response.
   */
  suspend fun startHostQuery(workBuffer: ResizableBuffer, service: String, timeout: TimeoutTracker): AdbChannel {
  ...
  suspend fun runHostDeviceQuery(device: DeviceSelector, query: String, timeout: TimeoutTracker): String {
  ...
  /** Executes a query on the ADB host that relates to a single [device][DeviceSelector] */
  suspend fun runHostDeviceQuery(device: DeviceSelector, query: String, timeout: TimeoutTracker, okayData: OkayDataExpectation): String? {
   ...
   * Executes a query on the ADB host that relates to a single [device][DeviceSelector].
   *
   * This method is similar to [runHostDeviceQuery], except the methods is for use with a variation of the ADB protocol where the ADB Host
   * sends back to 2 OKAY replies (one for "connect", one for "status") instead of only one.
   */
  suspend fun runHostDeviceQuery2(device: DeviceSelector, query: String, timeout: TimeoutTracker, okayData: OkayDataExpectation): String? {
    val workBuffer = newResizableBuffer()
    val service = device.hostPrefix + ":" + query
    return startHostQueryImpl(workBuffer, device, service, timeout).use { channel ->
  ...
  /** Executes a `<host-prefix>` query and returns the data after the `OKAY` response as a [ByteBuffer]. */
  ...
  /** Executes a `<host-prefix>` query and returns the data after the `OKAY` response as a [String]. */
  ...
  /** Executes a `<host-prefix>` query and returns the data after the `OKAY` response as a [String]. */
  suspend fun runHostQuery(service: String, timeout: TimeoutTracker, okayData: OkayDataExpectation): String? {
    val workBuffer = newResizableBuffer()
    return startHostQueryImpl(workBuffer, device = null, service, timeout).use { channel ->
      readOkayFailString(channel, workBuffer, service, timeout, okayData)
    }
  ...
  /**
   * Opens an [AdbChannel] and invokes a [service] on the ADB host, then waits for an OKAY/FAIL response. The [device] parameter is
   * optional, as some host services apply to the ADB host itself (e.g. [AdbHostServices.version]) or to a specific [DeviceSelector]
   * connected to the ADB host (e.g. [AdbHostServices.forward]).
   *
   * In case of OKAY response, the returned [AdbChannel] is open and ready for the next steps of the communication protocol.
   *
   * In case of FAIL response, an [AdbFailResponseException] exception is thrown. The exception contains the error message included in the
  ...
  suspend fun sendAdbServiceRequest(channel: AdbOutputChannel, workBuffer: ResizableBuffer, service: String, timeout: TimeoutTracker) {
    workBuffer.clear()
  ...
  suspend fun runDaemonQuery(device: DeviceSelector, query: String, timeout: TimeoutTracker): String {
    return runDaemonQuery(device, query, timeout, OkayDataExpectation.EXPECTED)
      ?: throw AdbProtocolErrorException("Data segment expected after OKAY response")
  }

  /** Executes a [query] on the ADB Daemon of the given [device][DeviceSelector] */
  suspend fun runDaemonQuery(device: DeviceSelector, query: String, timeout: TimeoutTracker, okayData: OkayDataExpectation): String? {
    val workBuffer = newResizableBuffer()
  ...
  /** Executes a [query] on the ADB Daemon of the given [device][DeviceSelector] */
  suspend fun runDaemonQuery2(device: DeviceSelector, query: String, timeout: TimeoutTracker, okayData: OkayDataExpectation): String? {
    val workBuffer = newResizableBuffer()
    val channel = switchToTransport(device, workBuffer, query, timeout)
    return channel.use {
      sendAdbServiceRequest(channel, workBuffer, query, timeout)
  ...
  /**
   * Starts execution of a [service] on the ADB Daemon of the given [device][DeviceSelector], and calls [block] when the ADB Daemon is ready
   * to send/receive data corresponding to the service invoked.
   */
  suspend fun runDaemonService(
    ...
    val workBuffer = newResizableBuffer()
    startDaemonService(device, service, timeout).use { channel -> block(channel, workBuffer) }
  }

  /**
   * Starts execution of a [service] on the ADB Daemon of the given [device][DeviceSelector], and returns a [AdbChannel] when the ADB Daemon
   * is ready to send/receive data corresponding to the service invoked.
   */
      ...
      AdbProtocolUtils.isFail(data) -> {
        ...
        val error =
          AdbProtocolErrorException(
            "Expected \"OKAY\" or \"FAIL\" response header, " + "got \"${AdbProtocolUtils.bufferToByteDumpString(data)}\" instead"
          )
        throw error
      }
    }
   ...
   * * If `FAIL`, consume it and the following message, then throws an [AdbProtocolErrorException] exception (with the error message)
   */
  ...
  /**
   * Reads length prefixed data from the [channel] into [workBuffer], as returns a [ByteBuffer] that contains the data read.
   *
   * Note: The returned [ByteBuffer] is the result of calling [ResizableBuffer.afterChannelRead] on [workBuffer], so the buffer is only
   * valid as long as [workBuffer] is not changed.
   */
  suspend fun readLengthPrefixedData(channel: AdbInputChannel, workBuffer: ResizableBuffer, timeout: TimeoutTracker): ByteBuffer {
    workBuffer.clear()
    ...
    try {
      ...
      throw AdbProtocolErrorException("Invalid format for ADB response length (\"$lengthString\")", e)
    }
  }
  ...
  private suspend fun consumeTransportId(channel: AdbInputChannel, workBuffer: ResizableBuffer, timeout: TimeoutTracker): Long {
