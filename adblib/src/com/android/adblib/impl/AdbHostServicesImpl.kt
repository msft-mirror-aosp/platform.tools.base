...
internal class AdbHostServicesImpl(
  ...
  override suspend fun version(): Int {
    ...
    try {
      ...
      val error = AdbProtocolErrorException("Invalid ADB response (expected 4 digit hex. number, got \"${versionString}\" instead)", e)
  ...
  override suspend fun kill() {
    ...
    try {
      ...
      logger.info { "Received EOF instead of OKAY response. This can happen, " + "as server was killed just after sending OKAY" }
    }
  }
  ...
  override suspend fun serverStatus(): ServerStatus {
    ...
    return ServerStatus(
      usbBackend,
      status.usbBackendForced,
      mdnsBackend,
      status.mdnsBackendForced,
      status.version,
      status.build,
      status.logAbsolutePath,
      status.executableAbsolutePath,
      status.os,
      status.traceLevel,
      status.burstMode,
      status.mdnsEnabled,
      status.keystorePath,
      status.knownHostsPath,
    )
  }
  ...
  override suspend fun forward(device: DeviceSelector, local: SocketSpec, remote: SocketSpec, rebind: Boolean): String? {
    ...
    val tracker = TimeoutTracker(host.timeProvider, timeout, unit)
    val service = "forward:" + (if (rebind) "" else "norebind:") + local.toQueryString() + ";" + remote.toQueryString()

    return serviceRunner.runHostDeviceQuery2(device, service, tracker, OkayDataExpectation.OPTIONAL)
  }
  ...
  override suspend fun killForward(device: DeviceSelector, local: SocketSpec) {
    ...
    serviceRunner.runHostDeviceQuery2(device, service, tracker, OkayDataExpectation.NOT_EXPECTED)
  }
