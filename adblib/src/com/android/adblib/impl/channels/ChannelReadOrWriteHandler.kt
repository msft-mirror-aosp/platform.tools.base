...
/**
 * Allows calling read/writes methods of an NIO [Channel] that require a [CompletionHandler] as Kotlin coroutines.
 *
 * Note: Do not instantiate or inherit from this class directly, instead use [ChannelReadHandler] or [ChannelWriteHandler] for read or write
 * operations.
 *
 ...
 */
internal abstract class ChannelReadOrWriteHandler protected constructor(private val host: AdbSessionHost, private val nioChannel: Channel) {

  private val logger = adbLogger(host)

  private val completionHandler =
    object : CompletionHandler<Int, CancellableContinuation<Unit>> {
      override fun completed(result: Int, continuation: CancellableContinuation<Unit>) {
        ...
        if (!continuation.isActive) {
          logger.debug {
            "CompletionHandler.completed() called for inactive continuation: 'continuation[${continuation.hashCode()}] isCompleted=${continuation.isCompleted}, isCancelled=${continuation.isCancelled}"
          }
          return
        }
       ...
       *
       * Note: [AsynchronousCloseException] is thrown when an async socket is closed during a pending async read or write operation. This
       * function wraps these exceptions as [CancellationException] to play "nicely" with coroutine cancellation processing.
       *
       * For example
       *
       ...
       * * which results in closing the `x` socket channel, because async socket operations close the socket when a pending I/O operation is
       *   cancelled,
       * * which results in the pending `read` operation failing,
       * * which results with an [AsynchronousCloseException] being sent to the async. read "completed" callback (i.e. this callback).
       * * which is caught by the `async` operation
       *
       * If we don't convert the [AsynchronousCloseException] into a [CancellationException], the `async` block throws a non-cancellation
       * exception ([AsynchronousCloseException]), which may end up taking precedence over the initial [CancellationException] because the
       * behavior is more or less undefined when a non cancellation exception is thrown in a scope already in the process of being
       * cancelled.
       *
       * By wrapping the [AsynchronousCloseException] into a [CancellationException], the `async` block throws a cancellation exception that
       * is ignored because the parent scope is already being cancelled (i.e. the coroutine runtime behavior seems to be more deterministic
       * dealing with "cancellation of cancellation").
       */
      ...
      override fun failed(e: Throwable, continuation: CancellableContinuation<Unit>) {
        if (!continuation.isActive) {
          logger.debug {
            "CompletionHandler.failed() called for inactive continuation: 'continuation[${continuation.hashCode()}] isCompleted=${continuation.isCompleted}, isCancelled=${continuation.isCancelled}"
          }
          return
        }
        if (e is IOException && e.message == "The specified network name is no longer available") {
  ...
  /**
   * The [ByteBuffer] used when [runExactly] is active. It's marked as @Volatile since it's accessed on a different thread from a completion
   * handler.
   */
  @Volatile private var bufferForRunExactly: ByteBuffer? = null

  /**
   * The [TimeoutTracker] used when [runExactly] is active. It's marked as @Volatile since it's accessed on a different thread from a
   * completion handler.
   */
  @Volatile private var timeoutTracker: TimeoutTracker? = null

  /**
   * `true` if the [asyncReadOrWrite] implementation natively supports timeouts, `false` if the timeout handling should be done by this base
   * class
   *
  ...
  /** Invoked when a single [asyncReadOrWrite] successfully completes, passing the number of bytes read/written as [byteCount] */
  ...
  private fun completionHandlerCompleted(result: Int, continuation: CancellableContinuation<Unit>) {
    try {
      ...
      logger.debug {
        "'continuation[${continuation.hashCode()}].resumeWithException($t)', isCompleted=${continuation.isCompleted}, isCancelled=${continuation.isCancelled}"
      }
  ...
  private fun logContinuationResume(continuation: CancellableContinuation<Unit>) {
    logger.debug {
      "'continuation[${continuation.hashCode()}].resume(Unit)', isCompleted=${continuation.isCompleted}, isCancelled=${continuation.isCancelled}, isRunExactly=${bufferForRunExactly != null}"
    }
  ...
  private fun addResumeStackTrace(continuation: CancellableContinuation<Unit>) {
    ...
    recentResumeCallStackTraces.offer(
      "continuation[${continuation.hashCode()}], timestamp[${System.currentTimeMillis()}], handler[${hashCode()}]:\n${Throwable().stackTraceToString()}"
    )
  }

  /**
   * This method is called by a completion handler when executing a `runExactly` operation. Returns `true` if the buffer has been completely
   * processed. Otherwise, triggers a further async read or write and returns `false`. Throws `EOFException` if EOF is reached during a read
   * operation.
   */
  private fun runExactlyCompleted(result: Int, continuation: CancellableContinuation<Unit>): Boolean {
    ...
    asyncReadOrWrite(tempBuffer, tempTimeoutTracker.remainingMills, TimeUnit.MILLISECONDS, continuation, completionHandler)
    return false
  }
...
/**
 * Provides services to [readBuffer] data from any NIO [Channel] that supports asynchronous reads, e.g. [AsynchronousFileChannel] or
 * [AsynchronousSocketChannel].
 */
internal abstract class ChannelReadHandler(host: AdbSessionHost, nioChannel: Channel) : ChannelReadOrWriteHandler(host, nioChannel) {
  ...
  /**
   * Reads exactly [ByteBuffer.remaining] bytes from the underlying channel, throwing an [EOFException] if the channel reaches EOF.
   *
...
/**
 * Provides services to [writeBuffer] data from any NIO [Channel] that supports asynchronous writes, e.g. [AsynchronousFileChannel] or
 * [AsynchronousSocketChannel].
 */
internal abstract class ChannelWriteHandler(host: AdbSessionHost, nioChannel: Channel) : ChannelReadOrWriteHandler(host, nioChannel) {

  /**
   * Writes up to [ByteBuffer.remaining] bytes to the underlying channel, returning the number of bytes successfully written.
   *
