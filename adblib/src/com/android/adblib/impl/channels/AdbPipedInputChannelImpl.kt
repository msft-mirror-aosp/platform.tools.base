...
internal class AdbPipedInputChannelImpl(
    ...
    /**
     * The circular buffer used to store bytes for [readBuffer] and
     * [AdbPipedOutputChannelImpl.writeBuffer] operations
     */
    private val circularBuffer = CircularByteBuffer(bufferSize)

    /**
     * [StateFlow] of [State] used to coordinate calls to [readBuffer],
     * [AdbPipedOutputChannelImpl.writeBuffer], [close], [AdbPipedOutputChannelImpl.close] and
     * [AdbPipedOutputChannelImpl.error]. The intent is to make sure any
     * state change will wake up a pending [readBuffer] or [AdbPipedOutputChannelImpl.writeBuffer]
     * waiting on the [StateFlow]. For example, a call to [close] should interrupt suspended
     * calls to [readBuffer].
     */
    ...
    /**
     * Wraps a read or write [operation] on [circularBuffer] and returns the number
     * of bytes processed.
     */
    ...
    private class AdbPipedOutputChannelImpl(
        private val pipeImpl: AdbPipedInputChannelImpl
