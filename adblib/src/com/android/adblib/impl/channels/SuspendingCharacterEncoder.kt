...
/**
 * Performs encoding of characters into bytes using a [byteProcessor] coroutine.
 * Call [encode] as many times as needed to encode characters, then [shutdown] when no
...
internal class SuspendingCharacterEncoder(
    ...
    /**
     * Internal buffer used to store the encoded characters.
     * Bytes already encoded are in the [0, pos] range
     * Available room for new encoded characters is in the [pos, limit=capcacity] range
     */
    private val byteBuffer = ByteBuffer.allocate(bufferCapacity)

    /**
     * Leftover characters from previous calls to [encode], in case of incomplete
     * multi character sequence. Note that leftover characters are always stored
     * in the `[0, position]` range.
     */
    ...
    private val leftoverBuffer: CharBuffer
        get() {
            return _leftoverBuffer ?: run {
                CharBuffer.allocate(byteBuffer.capacity()).limit(0).also {
                    _leftoverBuffer = it
                }
            }
        }
    ...
    override suspend fun shutdown() {
        ...
        if (_leftoverBuffer != null) {
            encodeOnce(leftoverBuffer, byteBuffer, endOfInput = true).also {
                assert(it == EncodeOnceResult.Done)
            }
    ...
    /**
     * Calls [CharsetEncoder.encode] once, using current [inputBuffer] and [outputBuffer].
     * Returns [EncodeOnceResult.Done] if no more calls are needed
     * Results [EncodeOnceResult.CallAgain] if another call is needed.
     */
    ...
    private fun prepareInputBuffer(inputBuffer: CharBuffer): CharBuffer {
        ...
        val inputCharBuffer = if (_leftoverBuffer !== null && leftoverBuffer.position() > 0) {
            ...
            val copyCount =
                min(leftoverBuffer.capacity() - leftoverBuffer.limit(), inputBuffer.remaining())
    ...
    companion object {
        ...
        private fun createEncoder(
            ...
            val errorAction = if (throwOnMalformed) {
            ...
            return charset.newEncoder()
                ...
                .onUnmappableCharacter(errorAction).also {
                    if (replacement != null) {
    ...
    /**
     *  This method transfers [length] chars into the [dst] buffer from
     *  the given [src] buffer, starting at the given [srcPosition] in the
