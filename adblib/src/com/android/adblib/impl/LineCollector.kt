...
/**
 * Accepts chunks of [CharBuffer] and splits them into lines. Partial lines at the end of a chunk are saved to be used with the next chunk.
 */
internal class LineCollector(private val newLine: String = AdbProtocolUtils.ADB_NEW_LINE) {
  ...
  /**
   * Consumes [charBuffer] characters (from [CharBuffer.position] to [CharBuffer.limit]), looking for sequences of characters terminated by
   * [newLine], and accumulating them as [strings][String] in [lines] (see [getLines]).
   *
   * Note: This method consumes all characters of [charBuffer], [CharBuffer.limit] is equal to [CharBuffer.position].
   */
  ...
  /**
   * Returns a non-empty string containing the last characters of the last call to [collectLines], or the empty [String] if there were no
   * leftover characters.
   */
