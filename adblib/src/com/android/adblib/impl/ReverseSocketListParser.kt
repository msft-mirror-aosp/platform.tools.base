 ...
 */
private const val REVERSE_LINE_PATTERN = "(${ALL_BUT_SPACE})${SPACE}(${ALL_BUT_SPACE})${SPACE}(${ALL_BUT_SPACE})"

/**
 * Parser for the list of reverse forward socket connections, as returned by the `reverse:forward-list` command.
 *
 * Input example
 *
 *  ```
...
internal class ReverseSocketListParser {
  ...
  private fun parseOneShortFormatLine(builder: ListWithErrors.Builder<ReverseSocketInfo>, lineIndex: Int, lineText: CharSequence) {
    ...
    if (matchResult == null) {
      builder.addError("Forward line format is not recognized", lineIndex, lineText.toString())
      return
    }
