...
internal abstract class ShellWithIdleMonitoring<T, TShellCollector, Command>(private val parameters: Parameters<TShellCollector, Command>) {
  ...
  abstract fun execute(parameters: Parameters<TShellCollector, Command>, forwardingCollector: TShellCollector): Flow<T>

  /**
   * A [ShellCollector] that forwards all messages to another [ShellCollector] while at the same time emits "heartbeats" on a [SendChannel]
   * when messages are forwarded.
   */
  ...
  /**
   * A [ShellV2Collector] that forwards all messages to another [ShellV2Collector] while at the same time emits "heartbeats" on a
   * [SendChannel] when messages are forwarded.
   */
  ...
  /** A coroutine helper class that ensures [heartbeat] is updated at least every [commandIdleTimeout] duration. */
  protected class HeartbeatDetector<Command>(
    ...
    private fun throwIfInactive() {
      ...
      if (nanosFromLastHeartbeat >= timeoutNanos) {
        throw TimeoutException("Command `$command` has been inactive for more than " + "${commandIdleTimeout.toSafeMillis()} millis")
      }
    }
  }
  ...
  class HeartbeatRecorder(private val timeProvider: SystemNanoTimeProvider) {
    ...
    private suspend fun waitWhile(predicate: (NanoTime) -> Boolean): NanoTime {
      lastHeartbeatFlow.takeWhile { predicate(lastRecorded) }.collect()
...
internal class ShellV2WithIdleMonitoring<T>(parameters: Parameters<ShellV2Collector<T>, String>) :
  ...
  override fun execute(parameters: Parameters<ShellV2Collector<T>, String>, forwardingCollector: ShellV2Collector<T>): Flow<T> {
...
internal class LegacyExecWithIdleMonitoring<T>(parameters: Parameters<ShellCollector<T>, String>) :
  ...
  override fun execute(parameters: Parameters<ShellCollector<T>, String>, forwardingCollector: ShellCollector<T>): Flow<T> {
...
internal class LegacyShellWithIdleMonitoring<T>(parameters: Parameters<ShellCollector<T>, String>) :
  ShellWithIdleMonitoring<T, ShellCollector<T>, String>(parameters) {
...
internal class AbbWithIdleMonitoring<T>(parameters: Parameters<ShellV2Collector<T>, List<String>>) :
  ...
  override fun execute(parameters: Parameters<ShellV2Collector<T>, List<String>>, forwardingCollector: ShellV2Collector<T>): Flow<T> {
...
internal class AbbExecWithIdleMonitoring<T>(parameters: Parameters<ShellCollector<T>, List<String>>) :
  ...
  override fun execute(parameters: Parameters<ShellCollector<T>, List<String>>, forwardingCollector: ShellCollector<T>): Flow<T> {
