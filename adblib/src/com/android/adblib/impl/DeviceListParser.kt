...
private val LONG_LINE_PATTERN =
  Pattern.compile("(" + SERIAL_PATTERN + ")" + "\\s+" + "(" + DEVICE_STATE_PATTERN + ")" + "\\s*" + "(" + ADDITIONAL_FIELDS_PATTERN + ")$")
...
internal class DeviceListTextParser(val format: DeviceInfoFormat) : DeviceListParser() {
  ...
  private fun parseOneShortFormatLine(result: ListWithErrors.Builder<DeviceInfo>, lineIndex: Int, lineText: CharSequence) {
  ...
  private fun parseOneLongFormatLine(result: ListWithErrors.Builder<DeviceInfo>, lineIndex: Int, lineText: CharSequence) {
    // Output format in ADB Host code:
    // https://cs.android.com/android/platform/superproject/+/790d619575aea7032a4fe5f097d412adedf6623b:packages/modules/adb/transport.cpp;l=1316
  ...
  internal fun interface OneLineParser {
    ...
    fun call(result: ListWithErrors.Builder<DeviceInfo>, lineIndex: Int, lineText: CharSequence)
  }
}
