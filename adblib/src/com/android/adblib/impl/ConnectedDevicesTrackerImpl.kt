...
internal class ConnectedDevicesTrackerImpl(override val session: AdbSession) : ConnectedDevicesTracker {
  ...
  private fun updateCache(activeDevices: Map<String, DeviceInfo>, flowStatus: StateFlowStatus) {
    ...
    val connectedDeviceList =
      synchronized(deviceMapLock) {
        ...
        updated.forEach { serial -> toUpdate.add(Pair(deviceMap.getValue(serial), activeDevices.getValue(serial))) }
