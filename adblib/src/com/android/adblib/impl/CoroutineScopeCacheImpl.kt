...
internal class CoroutineScopeCacheImpl(parentScope: CoroutineScope, val description: String) : CoroutineScopeCache() {
  ...
  init {
    ...
    job.invokeOnCompletion { throwable -> runCatching { close() }.onFailure { throwable?.addSuppressed(it) } }
  }
  ...
  override suspend fun <T> getOrPutSuspendingWorker(key: Key<T>, defaultValue: suspend CoroutineScope.() -> T): T {
    return suspendingMap.getOrPut(key, defaultValue)
  }

  override fun <T> getOrPutSuspendingWorker(key: Key<T>, fastDefaultValue: () -> T, defaultValue: suspend CoroutineScope.() -> T): T {
  ...
  private class SuspendingMap(private val scope: CoroutineScope) {
    ...
    fun <T> getOrPut(key: Key<T>, fastDefaultValue: () -> T, defaultValue: suspend CoroutineScope.() -> T): T {
    ...
    /**
     * Launches a new computation. This should be called only if the result was not previously computed or if the previous computation
     * failed.
     */
    private fun <T> launchComputeValue(currentResult: Result<*>?, key: Key<T>, defaultValue: suspend CoroutineScope.() -> T) {
      scope.ensureActive()
