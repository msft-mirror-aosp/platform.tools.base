...
class AdbActivityManagerServicesImpl(override val session: AdbSession) : AdbActivityManagerServices {
  ...
  override suspend fun capabilities(device: DeviceSelector): AmCapabilitiesResult {
    ...
    return AmCapabilitiesResult(
      ...
      vmInfo =
        if (protoCapabilities.vmInfo == AmCapabilitiesProto.VMInfo.getDefaultInstance()) {
          null
        } else {
          AmCapabilitiesResult.VmInfo(name = protoCapabilities.vmInfo.name, version = protoCapabilities.vmInfo.version)
        },
    )
  }

  /** Invokes the [amCommand] and check for error in the output */
  private suspend fun runAmCommand(device: DeviceSelector, amCommand: String): ByteArrayShellCollector.CommandResult {
    return deviceServices.shellCommand(device, amCommand).withCollector(ByteArrayShellCollector()).execute().first().also {
    ...
    when (api) {
      ...
      in 24..25 -> {
        ...
        if (result.stderr.isNotEmpty()) {
          throwAmCommandError(device = device, amCommand = amCommand, result = result, errorOutput = result.stderr)
        }
      }
        ...
        val errorMessage =
          result.stdout.toString(AdbProtocolUtils.ADB_CHARSET).split(AdbProtocolUtils.ADB_NEW_LINE).lastOrNull { line ->
            line.startsWith("Error:", ignoreCase = true)
          }
        if (errorMessage != null) {
          throwAmCommandError(device = device, amCommand = amCommand, result = result, errorOutput = errorMessage)
        }
      }
    }
  }
  ...
  private fun throwAmCommandError(
    ...
    val errorMessage = errorOutput.split(AdbProtocolUtils.ADB_NEW_LINE).takeLast(10).joinToString(AdbProtocolUtils.ADB_NEW_LINE)

    throw AdbActivityManagerException(device = device, command = amCommand, errorOutput = errorMessage)
  }
