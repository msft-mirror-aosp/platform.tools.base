...
internal class AdbDeviceServicesImpl(
  ...
  override suspend fun rawExec(device: DeviceSelector, command: String): AdbChannel {
    ...
    val service = getExecServiceString(ExecService.EXEC, serviceArguments = emptyList(), command = command)
    ...
    channel.closeOnException {
      ...
      serviceRunner.consumeOkayFailResponse(device, service, channel, workBuffer, timeout)
    }
    return channel
  }
    ...
    return channelFlow {
        ...
        serviceRunner.runDaemonService(device, service, tracker) { channel, workBuffer ->
          host.timeProvider.withErrorTimeout(commandTimeout) {
            ...
            stdinChannel?.let { launchCancellable { forwardStdInputV2Format(deviceChannelWithLock, stdinChannel, bufferSize) } }

            windowSizeFlow?.let { launchCancellable { forwardWindowSizeFlow(deviceChannelWithLock, windowSizeFlow) } }
            ...
            collectShellCommandOutputV2Format(channel, workBuffer, service, shellCollector, bufferSize, flowCollector)
          }
        }
      }
  ...
  override suspend fun reverseForward(device: DeviceSelector, remote: SocketSpec, local: SocketSpec, rebind: Boolean): String? {
    val tracker = TimeoutTracker(host.timeProvider, timeout, unit)
    val service = "reverse:forward:" + (if (rebind) "" else "norebind:") + remote.toQueryString() + ";" + local.toQueryString()
  ...
  override suspend fun reverseKillForward(device: DeviceSelector, remote: SocketSpec) {
    ...
    serviceRunner.runDaemonQuery2(device, service, tracker, OkayDataExpectation.NOT_EXPECTED)
  }
  ...
  private suspend fun rootImpl(device: DeviceSelector, service: String): RootResult {
    ...
    return serviceRunner
      .startDaemonService(device, service, tracker, workBuffer)
      .use { channel ->
      ...
      .also { logger.debug { "${device.shortDescription} - \"$service\": $it" } }
  }
  ...
  private suspend fun forwardStdInputV2Format(deviceChannel: AdbOutputChannelWithMutex, stdInput: AdbInputChannel, bufferSize: Int) {
    val workBuffer = serviceRunner.newResizableBuffer(bufferSize)
  ...
  private suspend fun forwardWindowSizeFlow(deviceChannel: AdbOutputChannelWithMutex, windowSizeFlow: Flow<ShellWindowSize>) {
    ...
    windowSizeFlow.collect { windowSize ->
      ...
      // See
      // https://cs.android.com/android/platform/superproject/main/+/4ed60ff3550cdd7f60b71721bb108b594f560a67:packages/modules/adb/client/commandline.cpp;l=472
      //    size_t l = snprintf(shell->data(), shell->data_capacity(), "%dx%d,%dx%d",
      //                        ws.ws_row, ws.ws_col, ws.ws_xpixel, ws.ws_ypixel);
      val windowSizeString =
        String.format(
          Locale.ROOT,
          "%dx%d,%dx%d",
          windowSize.rowCount,
          windowSize.columnCount,
          windowSize.xPixelCount,
          windowSize.yPixelCount,
        )
      ...
      assert(buffer.remaining() >= windowSizePayload.remaining()) {
        "Buffer should have room for ascii representation of WindowSize " + "(${buffer.remaining()} < ${windowSizePayload.remaining()})"
      }
    ...
    return channelFlow {
        ...
        serviceRunner.runDaemonService(device, service, tracker) { channel, workBuffer ->
          host.timeProvider.withErrorTimeout(commandTimeout) {
            // Forward `stdin` from channel to adb (in a new coroutine so that we
            // can also collect `stdout` concurrently)
            stdinChannel?.let { launchCancellable { forwardStdInput(channel, stdinChannel, bufferSize, shutdownOutput) } }

            // Wraps our `ProducerScope` as `FlowCollector` so that the `shellCollector`
            // is not aware of this implementation detail.
            val producerScope = this@channelFlow
            val flowCollector = FlowCollector<T> { value -> producerScope.send(value) }
            collectShellCommandOutput(channel, workBuffer, service, bufferSize, stripCrLf, shellCollector, flowCollector)
          }
        }
      }
      .flowOn(host.ioDispatcher)
  ...
  private fun getExecServiceString(service: ExecService, serviceArguments: List<String>, command: String): String {
    // Shell service string can look like: shell[,arg1,arg2,...]:[command].
    // See
    // https://cs.android.com/android/platform/superproject/+/fbe41e9a47a57f0d20887ace0fc4d0022afd2f5f:packages/modules/adb/client/commandline.cpp;l=594;drc=fbe41e9a47a57f0d20887ace0fc4d0022afd2f5f
    val args =
      if (serviceArguments.isEmpty()) {
        ""
      } else {
        check(serviceArguments.none { it.contains(',') }) { "Invalid ',' separator in one or more shell service argument(s)" }
        serviceArguments.joinToString(separator = ",", prefix = ",")
      }
    // We don't escape here, just like ssh(1). http://b/20564385.
    // See
    // https://cs.android.com/android/platform/superproject/+/fbe41e9a47a57f0d20887ace0fc4d0022afd2f5f:packages/modules/adb/client/commandline.cpp;l=776
    return when (service) {
