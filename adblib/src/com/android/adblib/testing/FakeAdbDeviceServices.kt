...
class FakeAdbDeviceServices(override val session: AdbSession) : AdbDeviceServices {
   ...
   * Simulate a timeout.
   *
   * The functions [shell] and [shellV2] will throw a [TimeoutException] this number of times before returning a valid result.
   */
   ...
   *
   * Note that this is implemented by configuring the no-arg "getprop" shell command as used by [DevicePropertiesImpl]; invocations of
   * "getprop" with arguments are not simulated.
   */
   ...
   */
  fun configureShellCommand(deviceSelector: DeviceSelector, command: String, stdout: String, stderr: String = "", exitCode: Int = 0) {
  ...
  fun configureShellV1Command(deviceSelector: DeviceSelector, command: String, result: String) {
    shellCommands.getOrPut(deviceSelector.transportPrefix) { ShellCommands() }.add(command, result)
  }
   ...
   */
  fun configureShellV2Command(deviceSelector: DeviceSelector, command: String, stdout: String, stderr: String = "", exitCode: Int = 0) {
    shellV2Commands.getOrPut(deviceSelector.transportPrefix) { ShellV2Commands() }.add(command, stdout, stderr, exitCode)
  }

  fun configureShellV2Command(deviceSelector: DeviceSelector, command: String, stdout: ByteBuffer, stderr: ByteBuffer, exitCode: Int = 0) {
    shellV2Commands.getOrPut(deviceSelector.transportPrefix) { ShellV2Commands() }.add(command, stdout, stderr, exitCode)
  }
  ...
  override fun <T> shell(
    ...
    if (shellNumTimeouts <= 0) {
      ...
      val output =
        shellCommands[device.transportPrefix]?.get(command) ?: throw IllegalStateException("""Command not setup for $device: "$command"""")
      return channelFlow {
  ...
  override fun <T> shellV2(
    ...
    if (shellNumTimeouts <= 0) {
      ...
      val output =
        shellV2Commands[device.transportPrefix]?.get(command)
...
/**
 * split the [ByteBuffer.remaining] bytes of this buffer into array chunks of [chunkSize] bytes. The buffer position and limit remain
 * unchanged on exit.
 */
