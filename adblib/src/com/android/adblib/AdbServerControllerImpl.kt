...
internal class AdbServerControllerImpl(private val host: AdbSessionHost, configurationFlow: StateFlow<AdbServerConfiguration>) :
  AdbServerController {
  ...
  /** The current [State] of this instance. The [start], [stop] and [restart] methods can change the [currentState] at any time. */
  private var currentState = State.initial(host, configurationFlow)

  private var currentJob: Deferred<Unit>? = null

  /**
   * Job [TransitionStatus] is volatile because it's updated from `invokeOnCompletion`. When the transition starts it is set to
   * `IN_PROGRESS` and when transition ends it's set to `COMPLETED_OK` or `COMPLETED_FAILURE`.
   */
  @Volatile private var currentJobTransitionStatus = TransitionStatus.COMPLETED_OK

  private var lastKnownRemoteAddressStateFlow = MutableStateFlow<InetSocketAddress?>(null)

  /**
   * * Returns `true` after the [start] method has successfully completed, i.e. after ADB server was successfully started.
   * * Returns `false` after the [stop] method has successfully completed, i.e. after ADB server was successfully stopped.
   *
   * Note: If ADB server is killed manually, e.g. by running adb `kill-server` this value still returns `true`
   */
  ...
  /**
   * Awaits for the deferred, but distinguishes between different sources of `CancellationException`s thrown from `await` and treats them
   * differently:
   * - If `CancellationException` is the result of the current Job being cancelled, then simply propagate this exception
   * - If `CancellationException` is thrown by await itself, e.g. when `AdbServerController`'s state transition job was interrupted by the
   *   controller's business logic, then throw `IOException` to indicate that a transition failed to complete.
   */
  private suspend fun <T> Deferred<T>.awaitOrThrow(): T {
    return try {
      ...
      throw IOException("`AdbServerController` operation was interrupted by another operation").apply { addSuppressed(e) }
    }
  }
  ...
  private suspend fun createChannelWithRetryOnFailure(
    ...
    host.timeProvider.withErrorTimeout(tracker.remainingMills) { currentState.waitIsStarted() }
    ...
    return try {
      ...
      host.timeProvider.withErrorTimeout(tracker.remainingMills) {
        try {
          ...
          throw IOException("`createChannel` failed to restart `AdbServerController` on failure", e)
        }
      }
      connectProvider.createChannel(tracker.remainingNanos, TimeUnit.NANOSECONDS)
    }
  ...
  /**
   * This channel provider relies on the `AdbServerControllerImpl` to start the ADB server. Essentially, some other part of the system needs
   * to call `AdbServerControllerImpl.start`. Once that happens, the ADB server should be running, allowing this provider to establish
   * connections (channels) to it.
   *
   * This server provider can also restart the adb server if the server process dies.
   *
   * If the controller has not yet been started `createChannel` will attempt to wait for it to get started before trying to establish the
   * connection, except for when the controller is in a stopped/stopping state in which case it immediately throws `IOException`.
   */
  private inner class AdbServerControllerProvider : AdbServerChannelProvider {
    ...
    private val connectProvider =
      AdbServerChannelProvider.createConnectAddresses(host) {
        val port = currentState.params.lastUsedConfig.value?.serverPort ?: throw IllegalStateException("lastUsedConfig serverPort is null")
    ...
    override suspend fun createChannel(timeout: Long, unit: TimeUnit): AdbChannel {
      return createChannelWithRetryOnFailure(connectProvider, timeout, unit).also { channel ->
        lastKnownRemoteAddressStateFlow.update { (channel as? AdbSocketChannel)?.remoteAddress }
      }
    }
  }
  ...
  private sealed class State(val params: StateParams) : AutoCloseable {
    ...
    /**
     * Waits for the adb server configuration to be set so that we could start adb server if needed, and so that the [channelProvider] could
    ...
    companion object {
      ...
      /** Returns the initial [State], corresponding to no calls made to [start], [stop] or [restart]. */
      fun initial(host: AdbSessionHost, configurationFlow: StateFlow<AdbServerConfiguration>): State {
  ...
  /**
   * The "starting" state, i.e. [State.start] has been called. Note that we don't have an explicit `StartedState`, as this can be
   * represented by this [StartingState] with a completed job.
   */
