...
/** Tracks devices that are currently [connected][ConnectedDevice] to the ADB server corresponding to a given [session]. */
@IsThreadSafe
...
/** Returns a [ConnectedDevice] instance for a given [serialNumber], or `null` if the device is not currently connected. */
...
/** Waits for a device with the given [serialNumber] to appear in the list of [ConnectedDevicesTracker.connectedDevices]. */
suspend fun ConnectedDevicesTracker.waitForDevice(serialNumber: String): ConnectedDevice {
  ...
  return connectedDevices.value.firstOrNull { it.serialNumber == serialNumber }
    ?: run {
      connectedDevices
        .transform { devices -> emit(devices.firstOrNull { device -> device.serialNumber == serialNumber }) }
        .filterNotNull()
        .first()
    }
}
