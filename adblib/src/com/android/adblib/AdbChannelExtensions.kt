...
/**
 * Forwards the contents of this [AdbInputChannel] to an [AdbOutputChannel] and returns the number of bytes read.
 *
 ...
 * @param bufferSize (optional) The maximum number of bytes to read when transferring bytes
 */
suspend fun AdbInputChannel.skipRemaining(workBuffer: ResizableBuffer = ResizableBuffer(), bufferSize: Int = DEFAULT_BUFFER_SIZE): Int {
  var totalCount = 0
  while (true) {
    workBuffer.clear()
    val count = read(workBuffer.forChannelRead(bufferSize))
...
/**
 * Appends all remaining bytes of this [AdbInputChannel] to a [ResizableBuffer], starting at [ResizableBuffer.position], and returns the
 * number of bytes read. [ResizableBuffer.position] is incremented by the numbers of bytes read.
 *
 ...
 */
suspend fun AdbInputChannel.readRemaining(workBuffer: ResizableBuffer, bufferSize: Int = DEFAULT_BUFFER_SIZE): Int {
  var totalCount = 0
  while (true) {
...
/**
 * Reads [count] bytes from this [AdbInputChannel] and appends them to [workBuffer], increasing the [ResizableBuffer] size as needed.
 *
 * Use [ResizableBuffer.afterChannelRead] to access the [ByteBuffer] where the [count] bytes are stored.
 */
