 ...
 *
 * Note: Implementations are guaranteed to be thread-safe, i.e. writing to [pipeSource]
 * can be done concurrently with [read] operations. However, concurrent writes to
 * [pipeSource] or concurrent [reads][read] are not allowed.
 */
interface AdbPipedInputChannel : AdbInputChannel {
     ...
     *
     * If the pipe is empty and [AdbPipedOutputChannel.shutdown] has been called, [buffer]
     * is unchanged (i.e. no bytes are read, signalling EOF).
     *
     ...
     * * Throws [Throwable] if [AdbPipedOutputChannel.error] was previously called and the
     * pipe is empty.
     */
...
interface AdbPipedOutputChannel : AdbBufferedOutputChannel {
    ...
    /**
     * Wait until all bytes that have been written so far are consumed by the corresponding
     * pipe reader.
     *
     * * Throws [ClosedChannelException] if [close] or [shutdown] were previously called
     * * Throws [AsynchronousCloseException] if [close] is called __while this function is
    ...
    /**
     * Similar to [flush] but also signal the corresponding pipe reader that no more data
     * will be coming, i.e. `readXxx` operations on the corresponding [AdbPipedInputChannel]
     * should start returning `-1` after all currently remaining data in the pipe has been read.
     *
     * * Throws [ClosedChannelException] if [close] or [shutdown] were previously called
     * * Throws [AsynchronousCloseException] if [close] is called __while this function is
     * suspended__
    ...
    /**
     * Close this [AdbPipedOutputChannel] synchronously or asynchronously, i.e. all future
     * operations throw [ClosedChannelException] and any pending operation throws
     * [AsynchronousCloseException].
     */
    override fun close()

    /**
     * Signal that `readXxx` operations on the corresponding [AdbPipedInputChannel]
     * should throw [throwable] after all currently remaining data in the pipe has been read.
     *
