...
class AdbDeviceServicesTest {
  ...
  fun testShellToLines(): Unit = runBlockingWithTimeout {
    ...
    val lines =
      deviceServices
        .shellAsLines(deviceSelector, "getprop", bufferSize = 10)
  ...
  fun testShellWithArguments(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val commandOutput = deviceServices.shell(deviceSelector, "cmd package install-create", TextShellCollector()).first()
  ...
  fun testShellWithTimeout() {
    ...
    /*val bytes = */ runBlocking {
      deviceServices.shell(deviceSelector, "write-no-stop", collector, commandTimeout = Duration.ofMillis(10)).first()
    }
  ...
  fun testShellWithStdin(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val commandOutput = deviceServices.shellAsText(deviceSelector, "cat", stdinChannel = input.asAdbInputChannel(deviceServices.session))
  ...
  fun testShellWithNoShutdownOutput(): Unit = runBlockingWithTimeout {
    ...
    stdinChannel.use {
      ...
      val flow = deviceServices.shell(deviceSelector, "cat", collector, stdinChannel = it, commandTimeout = timeout, shutdownOutput = false)
      exceptionRule.expect(TimeoutException::class.java)
      flow.first()
  ...
  fun testShellWithLargeInputAndSmallBufferSize(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.session.host.setTestLoggerMinLevel(deviceServices.session.host.logger.minLevel.coerceAtLeast(AdbLogger.Level.INFO))
    val deviceSelector = DeviceSelector.fromSerialNumber(fakeDevice.deviceId)
    val input =
      """
      This is some text with
    ...
    // Act
    val commandOutput =
      deviceServices.shellAsText(deviceSelector, "cat", input.asAdbInputChannel(deviceServices.session), INFINITE_DURATION, 15)
  ...
  fun testShellWithErroneousStdinMaintainsInitialException(): Unit = runBlockingWithTimeout {
    ...
    val fakeDevice = addFakeDevice(fakeAdb)
    val deviceSelector = DeviceSelector.fromSerialNumber(fakeDevice.deviceId)
    val errorInputChannel =
      object : AdbInputChannel {
    ...
    /*val ignored = */ deviceServices.shell(deviceSelector, "cat", TextShellCollector(), stdinChannel = errorInputChannel).first()
  ...
  fun testShellWithCancelledStdinMaintainsCancellation() {
    ...
    /*val ignored = */ runBlocking {
      deviceServices.shell(deviceSelector, "cat", TextShellCollector(), stdinChannel = errorInputChannel).first()
    }
  ...
  /**
   * Ensures the [Flow] returned by [AdbDeviceServices.shellAsLines] has the expected "streaming" behavior, i.e. that lines of output are
   * emitted to the [Flow] as soon as they are received from the shell output (`stdin` in this case).
   *
   * This test could fail as an infinite wait if the input channel was read "ahead" before the shell output is passed to the underlying
   * [ShellCollector] and [Flow]. This could easily happen if the [AdbDeviceServices.shell] implementation had the coroutines forwarding
   * stdin and stdout run inside coroutine scopes that wait on each other, e.g. parent/child scopes.
   */
  @Test
  fun testShellStdinIsForwardedConcurrentlyWithStdout(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val flow = deviceServices.shell(deviceSelector, "cat", LineShellCollector(), stdinChannel = testInputChannel)

    // Assert
    Assert.assertEquals("Input should be left alone before the flow is active", 0, testInputChannel.currentLineIndex)
    ...
    flow.collectIndexed { index, line ->
      if (index == 10) {
        ...
        Assert.assertEquals("Last line index should be 10", 10, testInputChannel.currentLineIndex)
        Assert.assertEquals("Last line should be empty", "", line)
      } else {
        Assert.assertEquals("Input channel should advance one line at a time", index + 1, testInputChannel.currentLineIndex)
  ...
  fun testShellWithMonitoringCanDetectInactiveCommand(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(TimeoutException::class.java)
    /*val ignored = */ deviceServices
      .shellCommand(deviceSelector, "cat")
  ...
  fun testShellCollectorServiceOutputIsNotBlockedOnEmit(): Unit = runBlockingWithTimeout {
    ...
    for (dispatcher in listOf(Dispatchers.IO, Dispatchers.Default)) {
      ...
      val collector =
        object : ShellCollector<ByteBuffer> {
      ...
      val job =
        launch(dispatcher) {
          deviceServices.shell(deviceSelector, "getprop", collector).collect {
  ...
  fun testShellV2CollectorServiceOutputIsNotBlockedOnEmit(): Unit = runBlockingWithTimeout {
    ...
    for (dispatcher in listOf(Dispatchers.IO, Dispatchers.Default)) {
      ...
      var shellCollectorHasCollected = false
      val collector =
        object : ShellV2Collector<ByteBuffer> {
          override suspend fun start(collector: FlowCollector<ByteBuffer>) {
            collector.emit(ByteBuffer.allocate(10))
          }

          override suspend fun collectStdout(collector: FlowCollector<ByteBuffer>, stdout: ByteBuffer) {
            shellCollectorHasCollected = true
          }

          override suspend fun collectStderr(collector: FlowCollector<ByteBuffer>, stderr: ByteBuffer) {
            shellCollectorHasCollected = true
          }
      ...
      val job =
        launch(dispatcher) {
          deviceServices.shellV2(deviceSelector, "getprop", collector).collect {
            // Collecting the emit called from `collector.start()`
            // This infinite delay shouldn't prevent
            delay(Long.MAX_VALUE)
          }
  ...
  fun testShellTerminalSession(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val commandOutput =
      deviceServices.shellTerminalSession(deviceSelector).withCollector(TextShellV2Collector()).withStdin(stdinPipe).execute().first()
  ...
  fun testShellTerminalSession_propagatesWindowSize(): Unit = runBlockingWithTimeout {
    ...
    val windowSizeFlow =
      flow<ShellWindowSize> {
        emit(ShellWindowSize(rowCount = 5, columnCount = 255, xPixelCount = 2000, yPixelCount = 200))
    ...
    val commandOutput =
      deviceServices
        .shellTerminalSession(deviceSelector)
  ...
  fun testShellTerminalSessionThrowsIfLegacyShellProtocolNotAllowedOnOldDevice(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.shellTerminalSession(deviceSelector).allowLegacyShell(false).withTextCollector().execute().collect()
  ...
  fun testRawExec(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.rawExec(deviceSelector, "getprop").use { channel ->
      ...
      Assert.assertEquals(expectedOutput, AdbProtocolUtils.ADB_CHARSET.decode(adbChannelOutput).toString())
    }
  }
  ...
  fun testShellV2Works(): Unit = runBlockingWithTimeout {
    ...
    val expectedOutput =
      ...
      """
        .trimIndent()
    Assert.assertEquals(expectedOutput, AdbProtocolUtils.byteBufferToString(shellV2Result.stdout))
  ...
  fun testShellV2SplitsShellPackets(): Unit = runBlockingWithTimeout {
    ...
    val shellV2Result =
      deviceServices
        .shellV2(deviceSelector, "shell-protocol-echo", collector, stdinChannel = input.asAdbInputChannel(deviceServices.session))
        .first()
  ...
  fun testShellV2WithErroneousStdinMaintainsInitialException(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(MyTestException::class.java)
    /*val ignored = */ deviceServices.shellV2(deviceSelector, "cat", TextShellV2Collector(), stdinChannel = errorInputChannel).first()
  ...
  fun testShellAsTextWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val commandOutput =
      deviceServices.shellAsText(deviceSelector, "shell-protocol-echo", stdinChannel = input.asAdbInputChannel(deviceServices.session))
  ...
  fun testShellV2AsLineWorks(): Unit = runBlockingWithTimeout {
    ...
    deviceServices
      .shellAsLines(deviceSelector, "shell-protocol-echo", stdinChannel = input.asAdbInputChannel(deviceServices.session))
      .collect { entry ->
        when (entry) {
  ...
  fun testShellInputChannelCollectorWorks(): Unit = runBlockingWithTimeout {
    ...
    var collectedExitCode = -1
    deviceServices
      .shellCommand(deviceSelector, "shell-protocol-echo")
    ...
    for (dispatcher in listOf(Dispatchers.IO, Dispatchers.Default)) {
      withContext(dispatcher) {
        ...
        deviceServices
          ...
          .withInputChannelCollector()
          .executeAsSingleOutput { inputChannelOutput ->
            AdbInputChannelReader(inputChannelOutput.stdout).use { reader -> collectedStdout = reader.readLine() ?: "" }
  ...
  fun testShellCommandExecuteSingleOutputIsTransparentToShellExceptions(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.shellCommand(deviceSelector, "shell-protocol-echo").withStdin(input).withInputChannelCollector().executeAsSingleOutput {
      inputChannelOutput ->
  ...
  fun testShellCommandExecuteSingleOutputIsTransparentToExceptions(): Unit = runBlockingWithTimeout {
    ...
    exceptionRule.expect(anyExceptionOf(MyTestException::class.java, ClosedChannelException::class.java))
    val collectedStdout = ArrayList<String>()
    val collectedStderr = ArrayList<String>()
    deviceServices
      .shellCommand(deviceSelector, "shell-protocol-echo")
  ...
  fun testShellCommandExecuteSingleOutputIsTransparentToExceptionsInStdoutReader(): Unit = runBlockingWithTimeout {
    ...
    coroutineScope {
      deviceServices
        ...
        .withInputChannelCollector()
        .executeAsSingleOutput { inputChannelOutput ->
          launchCancellable { AdbInputChannelReader(inputChannelOutput.stdout).use { throw MyTestException("My Message") } }

          launchCancellable { AdbInputChannelReader(inputChannelOutput.stderr).use {} }
        }
    }
  ...
  fun testShellCommandExecuteSingleOutputIsTransparentToExceptionsInStderrReader(): Unit = runBlockingWithTimeout {
    ...
    coroutineScope {
      deviceServices
        ...
        .withInputChannelCollector()
        .executeAsSingleOutput { inputChannelOutput ->
          launchCancellable { AdbInputChannelReader(inputChannelOutput.stdout).use {} }

          launchCancellable { AdbInputChannelReader(inputChannelOutput.stderr).use { throw MyTestException("My Message") } }
        }
    }
  ...
  fun testShellCommandExecuteSingleOutputAllowsCancellation(): Unit = runBlockingWithTimeout {
    ...
    exceptionRule.expect(anyExceptionOf(CancellationException::class.java, ClosedChannelException::class.java))
    val collectedStdout = ArrayList<String>()
    val collectedStderr = ArrayList<String>()
    coroutineScope {
      deviceServices
  ...
  fun testShellCommandExecuteSingleOutputAllowsCancellationInStdoutReader(): Unit = runBlockingWithTimeout {
    ...
    // stdout and stderr. It is unclear why we get this exception when the cause
    // of cancellation is always [MyCancellationException].
    exceptionRule.expect(anyExceptionOf(MyCancellationException::class.java, ClosedChannelException::class.java))
    coroutineScope {
      deviceServices
        .shellCommand(deviceSelector, "shell-protocol-echo")
        .withStdin(input.asAdbInputChannel(deviceServices.session))
        ...
        .executeAsSingleOutput { inputChannelOutput ->
          ...
          // We read data from stdin and stdout input channels until EOF
          // Then we get the exitCode
          launchCancellable { AdbInputChannelReader(inputChannelOutput.stdout).use { throw MyCancellationException("My Message") } }
    ...
    coroutineScope {
      deviceServices
        ...
        .executeAsSingleOutput { inputChannelOutput ->
          ...
          launchCancellable { AdbInputChannelReader(inputChannelOutput.stderr).use { throw MyCancellationException("My Message") } }
          delay(1_000)
        }
    }
  ...
  fun testShellCommandUsesLegacyExecIfSupported(): Unit = runBlockingWithTimeout {
    ...
    deviceServices
      .shellCommand(deviceSelector, "getprop")
      .allowLegacyExec(true)
      .withTextCollector()
  ...
  fun testShellCommandThrowsIfNoCollectorSet(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(IllegalArgumentException::class.java)
    deviceServices.shellCommand(deviceSelector, "getprop").execute().collect()
  ...
  fun testShellCommandThrowsIfLegacyShellProtocolNotAllowedOnOldDevice(): Unit = runBlockingWithTimeout {
    ...
    exceptionRule.expect(IllegalArgumentException::class.java)
    exceptionRule.expectMessage("No compatible shell protocol is supported or allowed")
    deviceServices.shellCommand(deviceSelector, "getprop").withTextCollector().allowLegacyShell(false).execute().collect()
  ...
  fun testShellCommandStripsCrLfOnOldDevices(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val output = deviceServices.shellCommand(deviceSelector, "getprop").withTextCollector().execute().first().stdout
  ...
  fun testShellCommandOutputElement(): Unit = runBlockingWithTimeout {
    Assert.assertEquals("contents", ShellCommandOutputElement.StdoutLine("contents").toString())
    Assert.assertEquals("contents", ShellCommandOutputElement.StderrLine("contents").toString())
    Assert.assertEquals("42", ShellCommandOutputElement.ExitCode(42).toString())
  }
  ...
  fun testDevicePropertiesAllReadonlyWorks() = runBlockingWithTimeout {
    ...
    yieldUntil { deviceServices.session.connectedDevicesTracker.connectedDevices.value.isNotEmpty() }
  ...
  fun testSyncRecvFileWorks(): Unit = runBlockingWithTimeout {
    ...
    val fileMode = RemoteFileMode.fromPosixPermissions(OWNER_READ, OWNER_WRITE)
    val fileDate = FileTime.from(1_000_000, TimeUnit.SECONDS)
    fakeDevice.createFile(DeviceFileState(filePath, fileMode.modeBits, (fileDate.toMillis() / 1_000).toInt(), fileBytes))
    val progress = TestSyncProgress()
    val outputStream = ByteArrayOutputStream()
    val outputChannel = AdbOutputStreamChannel(deviceServices.session, outputStream)

    // Act
    deviceServices.syncRecv(deviceSelector, filePath, outputChannel, progress, bufferSize = 1_024)
  ...
  fun testSyncStatFileWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val fileStat = deviceServices.syncStat(deviceSelector, filePath)
  ...
  @Test
  fun testSyncListWorks(): Unit = runBlockingWithTimeout {
    // Prepare
    val fakeDevice = addFakeDevice(fakeAdb)
    val deviceSelector = DeviceSelector.fromSerialNumber(fakeDevice.deviceId)
    ...
    // Act
    val entries = deviceServices.syncList(device = deviceSelector, remoteFilePath = "/sdcard/foo").toList()
  ...
  fun testReverseForward(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val port = deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(), SocketSpec.Tcp(4000))
  ...
  fun testReverseForwardNoRebind(): Unit = runBlockingWithTimeout {
    ...
    val port =
      deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(), SocketSpec.Tcp(4000))?.toIntOrNull()
        ?: throw AssertionError("Port should have been an integer")

    // Act
    exceptionRule.expect(AdbFailResponseException::class.java)
    runBlocking { deviceServices.reverseForward(DeviceSelector.any(), SocketSpec.Tcp(port), SocketSpec.Tcp(4000), rebind = false) }
  ...
  fun testReverseForwardRebind(): Unit = runBlockingWithTimeout {
    ...
    val port =
      runBlocking { deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(), SocketSpec.Tcp(4000)) }?.toIntOrNull()
        ?: throw AssertionError("Port should have been an integer")

    // Act
    val port2 = deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(port), SocketSpec.Tcp(4000), rebind = true)
  ...
  fun testReverseKillForward(): Unit = runBlockingWithTimeout {
    ...
    val port =
      runBlocking { deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(), SocketSpec.Tcp(4000)) }
    ...
    // Act
    deviceServices.reverseKillForward(deviceSelector, SocketSpec.Tcp(port.toInt()))
  ...
  fun testReverseKillForwardAll(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(), SocketSpec.Tcp(4000))
    Assert.assertEquals(1, fakeDevice.allReversePortForwarders.size)

    // Act
    runBlocking { deviceServices.reverseKillForwardAll(deviceSelector) }
  ...
  fun testReverseListForward(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.reverseForward(deviceSelector, SocketSpec.Tcp(1000), SocketSpec.Tcp(4000))
    Assert.assertEquals(1, fakeDevice.allReversePortForwarders.size)

    // Act
    val reverseList = runBlocking { deviceServices.reverseListForward(deviceSelector) }
  ...
  fun testAbbExec(): Unit = runBlockingWithTimeout {
    ...
    val flow = deviceServices.abb_exec(deviceSelector, listOf("package", "path", appId), TextShellCollector())
    flow.collect { resp.append(it) }
  ...
  fun testAbb(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val shellOutput = run {
      val flow = deviceServices.abb(deviceSelector, listOf("package", "path", appId), TextShellV2Collector())
  ...
  fun testAbbProvidesStderrAndExitCode(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val shellOutput = run {
      val flow = deviceServices.abb(deviceSelector, listOf("invalid_service_name"), TextShellV2Collector())
      // With TextShellV2Collector, there is always a single result
      flow.first()
    }
  ...
  fun testAbbWithStdin(): Unit = runBlockingWithTimeout {
    ...
    val sessionOutput = run {
      val flow = deviceServices.abb(deviceSelector, listOf("package", "install-create"), TextShellV2Collector())
      flow.first().stdout
    }
  ...
  fun testTrackJdwp(): Unit = runBlockingWithTimeout {
    ...
    val lists = run {
      ...
      flow.takeSome(changeCount) { index, _ -> addClient(fakeDevice, 100 + index * 2) }.toList()
    }
  ...
  fun testTrackAppFlowWorks(): Unit = runBlockingWithTimeout {
    ...
    val lists = run {
      ...
      flow.takeSome(changeCount) { index, _ -> addProfileableProcess(fakeDevice, 100 + index * 2) }.toList()
    }
  ...
  fun testTrackAppFlowThrowsIfInvalidDeviceSelector(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(AdbFailResponseException::class.java)
    deviceServices.trackApp(DeviceSelector.fromSerialNumber("1")).collect {}
  ...
  fun testTrackAppFlowIsTransparentToExceptions(): Unit = runBlockingWithTimeout {
    ...
    deviceServices.trackApp(deviceSelector).collect { throw IOException("MyMessage") }
  ...
  fun testTrackAppFlowIsTransparentToCancellation(): Unit = runBlockingWithTimeout {
    ...
    exceptionRule.expect(CancellationException::class.java)
    exceptionRule.expectMessage("MyMessage")
    deviceServices.trackApp(deviceSelector).collect { cancel("MyMessage") }
  ...
  fun testJdwpSessionOpens(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val sessionOpened = deviceServices.jdwp(deviceSelector, pid).use { true }
  ...
  @Test
  fun testShellCommandForceShellV2_throwsWhenDeviceDoesNotSupportShellV2(): Unit = runBlockingWithTimeout {
    // Prepare
    ...
    // Act
    exceptionRule.expect(IllegalArgumentException::class.java)
    deviceServices.shellCommand(deviceSelector, "getprop").forceShellV2().withTextCollector().execute().collect()
  ...
  @Test
  fun testShellCommandForceLegacyExec_throwsWhenDeviceDoesNotSupportExec(): Unit = runBlockingWithTimeout {
    // Prepare
    ...
    // Act
    exceptionRule.expect(IllegalArgumentException::class.java)
    deviceServices.shellCommand(deviceSelector, "getprop").forceLegacyExec().withTextCollector().execute().collect()
  ...
  fun testAbbCommandThrows_whenAbbIsNotSupported(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(IllegalArgumentException::class.java)
    deviceServices.abbCommand(deviceSelector, listOf("package", "path", appId)).withTextCollector().execute().first().stdout
  ...
  fun testAbbCommandWithStdin(): Unit = runBlockingWithTimeout {
    ...
    // Run a create session. It should create session 0 in the package manager
    val sessionOutput =
      deviceServices.abbCommand(deviceSelector, listOf("package", "install-create")).withTextCollector().execute().first().stdout
    val sessionID = extractSessionID(sessionOutput)

    // Act
    val shellOutput =
      deviceServices
        .abbCommand(deviceSelector, listOf("package", "install-write", "-S", dataToSend.size.toString(), sessionID, "-"))
        .withTextCollector()
        .withStdin(stdin)
        .execute()
        .first()
  ...
  fun testAbbCommandWithMonitoringCanDetectInactiveCommand(): Unit = runBlockingWithTimeout {
    ...
    exceptionRule.expect(TimeoutException::class.java)
    /*val ignored = */ deviceServices
      .abbCommand(deviceSelector, listOf("package", "-l"))
  ...
  private fun addFakeDevice(fakeAdb: FakeAdbServerProvider, sdk: Int = 30): DeviceState {
    val fakeDevice = fakeAdb.connectDevice("1234", "test1", "test2", "model", AndroidApiLevel(sdk), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    return fakeDevice
  }

  private fun addClient(fakeDevice: DeviceState, pid: Int): ClientState {
    return fakeDevice.startClient(pid, userId = pid * 2, processName = "package-$pid", packageName = "app-$pid", isWaiting = true)
  }
  ...
  class ShellV2ResultCollector : ShellV2Collector<ShellV2Result> {
    ...
    override suspend fun collectStdout(collector: FlowCollector<ShellV2Result>, stdout: ByteBuffer) {
      stdoutBuffer.appendBytes(stdout)
    }

    override suspend fun collectStderr(collector: FlowCollector<ShellV2Result>, stderr: ByteBuffer) {
      stderrBuffer.appendBytes(stderr)
    }

    override suspend fun end(collector: FlowCollector<ShellV2Result>, exitCode: Int) {
      val result = ShellV2Result(stdoutBuffer.forChannelWrite(), stderrBuffer.forChannelWrite(), exitCode)
      collector.emit(result)
    }
  }

  class ShellV2Result(val stdout: ByteBuffer, val stderr: ByteBuffer, val exitCode: Int)
