 ...
 *
 * In most cases, socket connections are established within a few milliseconds, but the time can dramatically increase under stress testing.
 * This behavior was observed on the Windows platform specifically.
 */
...
class AdbHostServicesTest {
  ...
  fun testConnect() {
    // Prepare
    fakeAdb.registerNetworkDevice("localhost:12345", "1234", "test1", "test2", "model", sdk = AndroidApiLevel(23))

    // Act
    runBlocking { hostServices.connect(DeviceAddress("localhost:12345")) }

    // Assert
    runBlocking { Assert.assertEquals(ONLINE, hostServices.getState(DeviceSelector.fromSerialNumber("1234"))) }
  }
  ...
  fun testDisconnect() {
    ...
    runBlocking {
      try {
        ...
        Assert.assertEquals("No device with serial: '1234' is connected.", expected.failMessage)
      }
    }
  }
  ...
  fun testTrackDevicesPropagatesExceptions() {
    ...
    try {
      /*val deviceList = */ runBlocking {
        ...
        flow.collect { throw IllegalStateException() }
      }
    } catch (t: Throwable) {
      exception = t
    }
  ...
  fun testMdnsServices() {
    ...
    fakeAdb.addMdnsService(
      MdnsService("foo-bar", "service", InetSocketAddress.createUnresolved("192.168.1.1", 10), serviceType = ServiceType.TLS)
    )
    fakeAdb.addMdnsService(
      MdnsService("foo-bar2", "service", InetSocketAddress.createUnresolved("192.168.1.1", 11), serviceType = ServiceType.TLS)
    )
  ...
  fun testPair() {
    // Prepare
    fakeAdb.addMdnsService(MdnsService("foo-bar2", "service", InetSocketAddress.createUnresolved("foo", 11), serviceType = ServiceType.TLS))

    // Act
    val result = runBlocking { hostServices.pair(DeviceAddress("foo:11"), "123456") }
  ...
  fun testTrackMdnsServicesWorks() {
    // Prepare
    fakeAdb.addMdnsService(
      MdnsService("foo-bar1", "service1", InetSocketAddress.createUnresolved("192.168.1.1", 10), serviceType = ServiceType.TLS)
    )
    fakeAdb.addMdnsService(
      MdnsService("foo-bar2", "service2", InetSocketAddress.createUnresolved("192.168.1.2", 11), serviceType = ServiceType.TCP)
    )
    fakeAdb.addMdnsService(
      MdnsService("foo-bar3", "service3", InetSocketAddress.createUnresolved("192.168.1.3", 12), serviceType = ServiceType.PAIRING)
    )
  ...
  fun testTrackMdnsServicesPropagatesExceptions() = runBlockingWithTimeout {
    // Prepare
    fakeAdb.addMdnsService(
      MdnsService("errorInstance", "_error._tcp", InetSocketAddress.createUnresolved("192.168.1.250", 1234), serviceType = ServiceType.TCP)
    )
  ...
  fun testPairFailsIfDeviceNotPresent() {
    ...
    // Act
    val result = runBlocking { hostServices.pair(DeviceAddress("foo:100"), "123456") }
  ...
  fun testGetState() {
    ...
    // Act
    val state = runBlocking { hostServices.getState(DeviceSelector.fromSerialNumber("1234")) }
  ...
  fun testGetSerialNo() {
    ...
    // Act
    val serialNumber = runBlocking { hostServices.getSerialNo(DeviceSelector.any()) }
  ...
  fun testGetSerialNoUsesKnownSerialNumber() {
    ...
    // Act
    val serialNumber = runBlocking { hostServices.getSerialNo(DeviceSelector.fromSerialNumber("1234")) }
  ...
  fun testGetSerialNoConnectsWhenForceRoundTripIsTrue() {
    ...
    // Act
    val serialNumber = runBlocking { hostServices.getSerialNo(DeviceSelector.fromSerialNumber("1234"), forceRoundTrip = true) }
  ...
  fun testGetSerialConnectsWhenNoSerialNumber() {
    ...
    // Act
    val serialNumber = runBlocking { hostServices.getSerialNo(DeviceSelector.usb()) }
  ...
  fun testGetDevPath() {
    ...
    // Act
    val devPath = runBlocking { hostServices.getDevPath(DeviceSelector.fromSerialNumber("1234")) }
  ...
  fun testFeatures() {
    ...
    // Act
    val featureList = runBlocking { hostServices.features(DeviceSelector.fromSerialNumber("1234")) }
  ...
  fun testForward() {
    ...
    // Act
    val port = runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(), SocketSpec.Tcp(4000)) }
  ...
  fun testForwardNoRebind() {
    ...
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val port =
      runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(), SocketSpec.Tcp(4000)) }?.toIntOrNull()
        ?: throw AssertionError("Port should have been an integer")

    // Act
    exceptionRule.expect(AdbFailResponseException::class.java)
    runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(port), SocketSpec.Tcp(4000), rebind = false) }
  ...
  fun testForwardRebind() {
    ...
    // Act
    val port2 = runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(port), SocketSpec.Tcp(4000), rebind = true) }
  ...
  fun testKillForward() {
    ...
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val port =
      runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(), SocketSpec.Tcp(4000)) }
        ?: throw Exception("`forward` command should have returned a port")
    Assert.assertEquals(1, fakeDevice.allPortForwarders.size)

    // Act
    runBlocking { hostServices.killForward(DeviceSelector.any(), SocketSpec.Tcp(port.toInt())) }
  ...
  fun testKillForwardAll() {
    ...
    // Act
    runBlocking { hostServices.killForwardAll(DeviceSelector.any()) }
  ...
  fun testListForward() {
    ...
    val fakeDevice = fakeAdb.connectDevice("1234", "test1", "test2", "model", sdk = AndroidApiLevel(23), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    runBlocking { hostServices.forward(DeviceSelector.any(), SocketSpec.Tcp(1000), SocketSpec.Tcp(4000)) }
    Assert.assertEquals(1, fakeDevice.allPortForwarders.size)

    // Act
    val forwardList = runBlocking { hostServices.listForward() }
  ...
  fun testWaitForOffline(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val job = launch { hostServices.waitFor(deviceSelector, WaitForState.DISCONNECT) }
  ...
  fun testWaitForOnline(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val job = launch { hostServices.waitFor(deviceSelector, WaitForState.ONLINE) }
    fakeAdb
      .connectDevice(
        deviceId = "1234",
        manufacturer = "test1",
