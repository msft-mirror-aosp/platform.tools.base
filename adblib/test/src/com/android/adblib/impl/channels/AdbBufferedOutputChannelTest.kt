...
class AdbBufferedOutputChannelTest {
  ...
  fun toStringIsWellDefined(): Unit = runBlockingWithTimeout {
    ...
    // Assert
    Assert.assertEquals(
      "AdbBufferedOutputChannel(bufferedBytes=10, bufferCapacity=1000, closed=false, isShutdown=false)",
      bufferedChannel.toString(),
    )
  }
  ...
  fun writeCalledMultipleTimesIsBufferedAndBatched(): Unit = runBlockingWithTimeout {
    ...
    repeat(chunkCount) {
      ...
      repeat(chunkSize) { buffer.put(byteData.next()) }
      buffer.position(0)
  ...
  private open class TestingOutputChannel(private val writeDelay: Duration? = null) : AdbOutputChannel {
