...
class AdbInputFileChannelTest {
  ...
  fun testChannelReaderWorks() {
    ...
    val lines = runBlocking {
      channelFactory.openFile(path).use { inputChannel -> inputChannel.toChannelReader().use { reader -> reader.toList() } }
    }
  ...
  fun testChannelReaderClosesInputChannel() {
    ...
    // Act
    val list = runBlocking { inputChannel.toChannelReader().use { reader -> reader.toList() } }
  ...
  fun testChannelReaderReportsExceptions() {
    ...
    runBlocking { inputChannel.toChannelReader().use { reader -> reader.toList() } }
  ...
  fun testReadLinesReportsScopeCancellation() {
    ...
    runBlocking {
      inputChannel.use {
        ...
        val job = launch { channel = it.readLines(this, Charsets.UTF_8, "\n", 256) }
        while (channel == null) {
          delay(5)
        }
