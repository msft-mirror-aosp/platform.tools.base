...
class SuspendingCharacterEncoderTest {
  ...
  @Suppress("DEPRECATION") @JvmField @Rule var exceptionRule: ExpectedException = ExpectedException.none()
  ...
  fun asciiEncodingWorks(): Unit = runBlockingWithTimeout {
    ...
    // Assert
    assertArrayEquals(
      byteArrayOf(ASCII_LOWER_F, ASCII_LOWER_O, ASCII_LOWER_O, ASCII_LOWER_B, ASCII_LOWER_A, ASCII_LOWER_R),
  ...
  fun asciiEncodingThrowsOnInvalidCharacter(): Unit = runBlockingWithTimeout {
    ...
    testEncoder.encoder.useShutdown { encoder -> encoder.encode(UTF16_SURROGATE_PAIR) }
  }

  @Test
  fun asciiEncodingUsesReplacementOnInvalidCharacter(): Unit = runBlockingWithTimeout {
    // Prepare
    val testEncoder = TestEncoder(Charsets.US_ASCII, bufferCapacity = 16, throwOnMalformed = false)
  ...
  fun asciiEncodingUsesCustomReplacementString(): Unit = runBlockingWithTimeout {
    // Prepare
    val testEncoder =
      TestEncoder(Charsets.US_ASCII, bufferCapacity = 16, throwOnMalformed = false, replacement = byteArrayOf('f'.code.toByte()))
  ...
  fun utf8EncodingThrowsOnInvalidSurrogate(): Unit = runBlockingWithTimeout {
    // Prepare
    val testEncoder = TestEncoder(Charsets.UTF_8, bufferCapacity = 16, throwOnMalformed = true)
  ...
  /**
   * Helper class to wrap a [SuspendingCharacterEncoder] and its accumulated [ByteBuffer] resulting from calling
   * [SuspendingCharacterEncoder.encode]
   */
