...
class TestingAdbLoggerFactory : AdbLoggerFactory {
  ...
  /** Whether to log with "delta time" from previous log entry. `true` by default, but can be disabled manually in test code. */
  @Suppress("MemberVisibilityCanBePrivate") var logDeltaTime: Boolean = true

  override val logger: AdbLogger by lazy { TestingAdbLogger(this) }
  ...
  private fun formatInstant(newInstant: Instant) = newInstant.toString().replace('T', ' ').dropLast(4)
}

open class TestingAdbLogger(private val factory: TestingAdbLoggerFactory) : AdbLogger() {
...
class TestingAdbLoggerWithPrefix(factory: TestingAdbLoggerFactory, override val prefix: String) : TestingAdbLogger(factory)
