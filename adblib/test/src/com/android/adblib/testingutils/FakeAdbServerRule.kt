 ...
 *
 * @param configure An optional lambda to apply additional customization to the [FakeAdbServer] after the default command handlers have been
 *   installed.
 */
open class FakeAdbServerRule(private val configure: (FakeAdbServer.Builder.() -> Unit)? = null) : ExternalResource(), FakeDeviceCreator {

  lateinit var adbServer: FakeAdbServer
    private set

  public override fun before() {
    adbServer = FakeAdbServer.Builder().installDefaultCommandHandlers().apply { configure?.invoke(this) }.build().also { it.start() }
  }
  ...
  override fun disconnectDevice(deviceId: String) {
    adbServer.disconnectDevice(deviceId).get(FAKE_ADB_SERVER_EXECUTOR_TIMEOUT_MS, TimeUnit.MILLISECONDS)
  }
}
