...
object CoroutineTestUtils {
  ...
  fun <T> runBlockingWithTimeout(timeout: Duration = Duration.ofSeconds(30), block: suspend CoroutineScope.() -> T): T {
    return runBlocking { runTestBlockWithTimeout(timeout, "runBlocking") { block(this) } }
  }

  suspend fun yieldUntil(timeout: Duration = Duration.ofSeconds(30), predicate: suspend () -> Boolean) {
  ...
  suspend fun <T> waitNonNull(timeout: Duration = Duration.ofSeconds(30), provider: suspend () -> T?): T {
    ...
    return runTestBlockWithTimeout(timeout, "waitNonNull") { loop(provider) }
  }
    ...
    return try {
      ...
      throw AssertionError(
        "A '$blockDescription' did not terminate within the specified timeout ($timeout), " +
          "there is a bug somewhere (in the test or in the tested code)",
        e,
      )
    }
  }
