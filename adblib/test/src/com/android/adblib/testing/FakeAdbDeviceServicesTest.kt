...
class FakeAdbDeviceServicesTest {
  ...
  fun shellAsText(): Unit = runBlocking {
    ...
    assertContentEquals(listOf(ShellRequest(deviceSelector.toString(), "command")), deviceServices.shellV2Requests)
  }
  ...
  fun shellAsText_withBufferSize(): Unit = runBlocking {
    ...
    assertEquals("", actual.stderr)
    assertEquals(0, actual.exitCode)
    assertContentEquals(listOf(ShellRequest(deviceSelector.toString(), "command", bufferSize = 4)), deviceServices.shellV2Requests)
  }
  ...
  fun shellAsText_runTwice(): Unit = runBlocking {
    ...
    assertEquals(0, actual.exitCode)
    assertContentEquals(
      listOf(ShellRequest(deviceSelector.toString(), "command1"), ShellRequest(deviceSelector.toString(), "command2")),
  ...
  fun shellAsLines(): Unit = runBlocking {
    ...
    val actual =
      deviceServices
        .shellAsLines(deviceSelector, "command")
        .mapNotNull {
          if (it is StdoutLine) {
            it.contents
          } else {
    ...
    assertContentEquals(listOf("line1", "line2", "line3"), actual)
    assertContentEquals(listOf(ShellRequest(deviceSelector.toString(), "command")), deviceServices.shellV2Requests)
  }

  @Test
  fun shellV2AsText(): Unit = runBlocking {
    deviceServices.configureShellV2Command(deviceSelector, "command", "output", "error", exitCode = 1)
  ...
  fun shellV2AsText_withBufferSize(): Unit = runBlocking {
    deviceServices.configureShellV2Command(deviceSelector, "command", "command output", "command error", exitCode = 1)
  ...
  fun shellV2AsText_runTwice(): Unit = runBlocking {
    ...
    assertEquals(1, actual.exitCode)
    assertContentEquals(
      listOf(ShellRequest(deviceSelector.toString(), "command"), ShellRequest(deviceSelector.toString(), "command")),
      deviceServices.shellV2Requests,
    )
  }
  ...
  fun shellV2AsLines(): Unit = runBlocking {
    ...
    val stderr =
      ...
      """
        .trimIndent()
    deviceServices.configureShellV2Command(deviceSelector, "command", stdout, stderr, exitCode = 1)

    val actual = deviceServices.shellAsLines(deviceSelector, "command").toList()

    assertContentEquals(listOf("line1", "line2"), actual.filterIsInstance<StdoutLine>().map { it.contents })
    assertContentEquals(listOf("error1", "error2"), actual.filterIsInstance<StderrLine>().map { it.contents })
    // There is only 1 exit code
    assertContentEquals(listOf(1), actual.filterIsInstance<ExitCode>().map { it.exitCode })
