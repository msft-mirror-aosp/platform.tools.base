...
class AdbDeviceSyncServicesTest {
  ...
  fun testSendFileWorks(): Unit = runBlockingWithTimeout {
    ...
    withSyncServices(deviceSelector) {
      it.send(AdbInputStreamChannel(adbSession.host, fileBytes.inputStream()), filePath, fileMode, fileDate, progress, bufferSize = 1_024)
    }
  ...
  fun testSendWithTimeoutWorks(): Unit = runBlockingWithTimeout {
    ...
    adbSession.host.timeProvider.withErrorTimeout(Duration.ofMillis(100)) {
      withSyncServices(deviceSelector) { it.send(slowInputChannel, filePath, fileMode, fileDate, progress, bufferSize = 1_024) }
    }
  ...
  fun testSendFileWithNoDateSetsModifiedDateToCurrentTime(): Unit = runBlockingWithTimeout {
    ...
    // Act
    withSyncServices(deviceSelector) {
      it.send(AdbInputStreamChannel(adbSession.host, fileBytes.inputStream()), filePath, fileMode, null, progress, bufferSize = 1_024)
    }
    ...
    Assert.assertNotNull(fakeDevice.getFile(filePath))
    fakeDevice.getFile(filePath)?.run { Assert.assertEquals(currentTime.epochSecond, modifiedDate.toLong()) }
  }
  ...
  fun testRecvFileWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    withSyncServices(deviceSelector) { it.recv(filePath, outputChannel, progress, bufferSize = 1_024) }
  ...
  fun testRecvTwoFileInSameSessionWorks(): Unit = runBlockingWithTimeout {
    ...
    val fileMode2 = RemoteFileMode.fromPosixPermissions(OWNER_READ, OWNER_WRITE)
    val fileDate2 = FileTime.from(2_000_000, TimeUnit.SECONDS)
    fakeDevice.createFile(DeviceFileState(filePath2, fileMode2.modeBits, (fileDate2.toMillis() / 1_000).toInt(), fileBytes2))
  ...
  fun testStatFileWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val fileStat = withSyncServices(deviceSelector) { it.stat(filePath) }
  ...
  fun testListWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val entries = withSyncServices(deviceSelector) { syncServices -> syncServices.list("/sdcard/foo").toList() }
  ...
  fun testListSkipsDotAndDotDot(): Unit = runBlockingWithTimeout {
    ...
    listOf(".", "..", ".foo", "bar.blah").map { fileName ->
      ...
      val fileBytes = createFileBytes(1_000)
      val fileMode = RemoteFileMode.fromPosixPermissions(OWNER_READ, OWNER_WRITE)
      DeviceFileState(filePath, fileMode.modeBits, (fileDate.toMillis() / 1_000).toInt(), fileBytes).also { fakeDevice.createFile(it) }
    }

    // Act
    val entries = withSyncServices(deviceSelector) { syncServices -> syncServices.list(remoteFilePath = "/sdcard/foo").toList() }
  ...
  fun testListAllowsListingDotAndDotDot(): Unit = runBlockingWithTimeout {
    ...
    val entries =
      withSyncServices(deviceSelector) { syncServices ->
        syncServices.list(remoteFilePath = "/sdcard/foo", options = ListOptions(skipDotEntries = false)).toList()
      }
  ...
  fun testListFlowThrowsAfterSyncServicesIsClosed(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val flow = withSyncServices(deviceSelector) { syncServices -> syncServices.list("/sdcard/foo") }
  ...
  fun testStatV2FileWorks(): Unit = runBlockingWithTimeout {
    ...
    val fileMode = RemoteFileMode.fromPosixPermissions(OWNER_READ, OWNER_WRITE)
    val fileDate = FileTime.from(1_000_000, TimeUnit.SECONDS)
    val fileState = DeviceFileState(filePath, fileMode.modeBits, (fileDate.toMillis() / 1_000).toInt(), fileBytes)
    fakeDevice.createFile(fileState)

    // Act
    val fileStat = withSyncServices(deviceSelector) { it.statV2(filePath) }
  ...
  fun testListV2Works(): Unit = runBlockingWithTimeout {
    ...
    val entries =
      withSyncServices(deviceSelector) { syncServices -> syncServices.listV2(remoteFilePath = "/sdcard/foo", options = options).toList() }
  ...
  fun testListV2FallsBackToListV1OnOlderDevice(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val entries = withSyncServices(deviceSelector) { syncServices -> syncServices.listV2(remoteFilePath = "/sdcard/foo").toList() }
  ...
  suspend inline fun <R> withSyncServices(device: DeviceSelector, block: (AdbDeviceSyncServices) -> R): R {
    return fakeAdbRule.adbSession.deviceServices.withSyncServices(device) { block(it) }
  }
