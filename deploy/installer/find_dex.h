...
class FindDexCommand : public AgentInteractionCommand {
 public:
  FindDexCommand(Workspace& workspace) : AgentInteractionCommand(workspace) {}
  virtual ~FindDexCommand() {}

  void ParseParameters(const proto::InstallerRequest& request) override;
  void Run(proto::InstallerResponse* response) override;

 private:
  proto::FindDexRequest request_;
};
