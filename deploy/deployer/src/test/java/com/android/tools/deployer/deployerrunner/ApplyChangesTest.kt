...
class ApplyChangesTest : DeployRunnerTestBase(){
    ...
    fun testBasicSwap() {
        ...
        if (device.getApi() < 26) {
            Assert.assertEquals(
                DeployerException.Error.CANNOT_SWAP_BEFORE_API_26.ordinal.toLong(),
                retcode.toLong()
            )
    ...
    fun testSwapWithAppNotRunning() {
        ...
        if (device.api < 26) {
            ...
            Assert.assertEquals(
                DeployerException.Error.DUMP_UNKNOWN_PROCESS.ordinal.toLong(),
                retcode.toLong()
            )
            assertHistory(
                device,
                ...
                ("/system/bin/cmd package "
                        + (if (device.api < 28) "dump" else "path")
                        + " com.example.simpleapp"),
                getInstallerInvocation(),  // deltapreinstall
                "/system/bin/cmd package install-create -t -r --dont-kill",
                "cmd package install-write -S \${size:com.example.simpleapp} 2 base.apk",
    ...
    fun testCodeSwapThatFails() {
        ...
        if (device.api < 26) {
            ...
            Assert.assertEquals(
                DeployerException.Error.CANNOT_SWAP_RESOURCE.ordinal.toLong(),
                retcode.toLong()
            )
            assertHistory(
                device,
    ...
    fun checkFailingMkDir() {
        ...
        if (device.api < 26) {
            ...
            Assert.assertEquals(
                DeployerException.Error.DUMP_FAILED.ordinal.toLong(),
                retcode.toLong()
            )
            assertHistory(
                device,
    ...
    fun testApkNotRecognized() {
        ...
        if (device.supportsJvmti()) {
            ...
            if (device.api < 30) {
                Assert.assertEquals(
                    DeployerException.Error.REMOTE_APK_NOT_FOUND_IN_DB.ordinal.toLong(),
                    retcode.toLong()
                )
            }
        } else {
