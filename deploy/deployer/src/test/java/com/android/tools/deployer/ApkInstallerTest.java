...
public class ApkInstallerTest {
    ...
    public void handleAndroidSManifestRestrictions() {
        ...
        Assert.assertEquals(
                "INSTALL_PARSE_FAILED_MANIFEST_MALFORMED: Failed parse during installPackageLI:"
                    + " /data/app/vmdl395250143.tmp/base.apk (at Binary XML file line #21):"
                    + " com.example.myapplication.MainActivity: Targeting S+ (version 10000 and"
    ...
    public void forceInstallOnNoDeltaShouldTurnOffInheritance() {
        boolean inherit = ApkInstaller.canInherit(15, new ArrayList<>(), InstallMode.DELTA_NO_SKIP);
        Assert.assertFalse(inherit);

        inherit = ApkInstaller.canInherit(15, new ArrayList<>(), InstallMode.DELTA);
