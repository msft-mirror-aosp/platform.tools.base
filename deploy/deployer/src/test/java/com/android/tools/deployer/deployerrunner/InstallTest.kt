...
class InstallTestTest : DeployRunnerTestBase() {
    ...
    fun testInstallOldVersion() {
        ...
        val retcode = runner.run(args)
        Assert.assertEquals(
            DeployerException.Error.INSTALL_FAILED.ordinal.toLong(),
            retcode.toLong()
        )
        Assert.assertEquals(1, device.apps.size.toLong())
        ...
        if (device.api == 19) {
            assertHistory(
                device,
                "getprop",
                "pm install -r -t \"/data/local/tmp/simple.apk\"" // ,"rm \"/data/local/tmp/simple.apk\"" TODO: ddmlib doesn't remove when installation fails
            )
            assertMetrics(
                runner.metrics,
                "DELTAINSTALL:API_NOT_SUPPORTED",
    ...
    fun testStartApp() {
        ...
        Assert.assertEquals(
            0,
            device.executeScript("am force-stop com.example.simpleapp.bar", byteArrayOf())
                .value.toLong()
        )
        processes = device.processes
        Assert.assertEquals(1, processes.size.toLong())
        ...
        Assert.assertNotEquals(
            0,
            device.executeScript("am force-stop", byteArrayOf()).value.toLong()
        )
        processes = device.processes
        Assert.assertEquals(1, processes.size.toLong())
        Assert.assertEquals("com.example.simpleapp", processes[0].application.packageName)
    ...
    fun testDump() {
        ...
        if (device.api < 24) {
            ...
            Assert.assertEquals(
                device.getAppPaths(packageName)!![0],
                response.getPackages(0).getApks(0).absolutePath
            )
        }
