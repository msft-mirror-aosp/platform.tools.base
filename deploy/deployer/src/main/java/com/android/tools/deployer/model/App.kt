...
class App(
    ...
    val allStrategies : List<DeploymentStrategy<Apk>> get() = apks
    ...
    companion object {
        ...
        fun fromPaths(
            appId: String?,
            paths: List<Path>,
            baselineProfiles: List<BaselineProfile>): App {
                ...
                if (path.toString().endsWith(".dm")) {
                    if (baselineProfiles.isEmpty()) {
                        baselineProfiles.add(
                            BaselineProfile(
                                Int.MIN_VALUE, Int.MAX_VALUE, ArrayList()
                            )
                        )
                    }
        ...
        fun fromStrategy(path: Path, logger: ILogger): App {
            ...
            val strategies = artifacts.elements.map { artifact ->
                val apkPath: Path = Paths.get(artifact.outputFile)
                val apk = ApkParser.parse(apkPath.toAbsolutePath().toString())
                val filters = artifact.filters.stream().collect(
                    Collectors.toMap(
                        GenericFilterConfiguration::filterType,
                        GenericFilterConfiguration::identifier
                    )
                )
