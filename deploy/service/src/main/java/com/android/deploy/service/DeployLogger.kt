...
/** A logger that captures values and stores them in the DeployLog proto. This is useful for debugging errors with the [DeployServer]. */
class DeployLogger(private val myLevel: Level) : ILogger {

  /**
   * Error level to capture. Levels are inclusive of levels defined before them. Eg Level.INFO will capture INFO, WARNING, ERROR logs but
   * will not capture VERBOSE.
   */
  enum class Level {
    ERROR,
    WARNING,
