 ...
 * @param rawTypes whether to generate parameterized java generics (Type<Param> vs Type) in the class inheritance.
 * Some classes (FunctionReference) already have raw types in their definition; java complains if raw and
 * parameterized types are mixed in this context.
 */
...
fun main(args: Array<String>) {
    val functionInterfaces =  ((0..22) + listOf("N")).map {
        Class.forName("kotlin.jvm.functions.Function$it")
    }
    ...
    val lambdas = listOf(
        ...
        functionInterfaces.map { ProxySpec(l.simpleName + it.simpleName.substringAfter("Function"), l, listOf(it)) }
    }
...
private fun generateProxies(proxies: List<ProxySpec>): JavaFile {
    ...
    val field =
        FieldSpec.builder(mapType, "proxies", Modifier.PUBLIC, Modifier.STATIC)
            .initializer(CodeBlock.of("new \$T<>()", java.util.HashMap::class.java)).build()
    ...
    val getter = MethodSpec.methodBuilder("getProxyInterface")
...
private fun TypeSpec.Builder.addConstructorsFrom(clazz: Class<*>) =
    this.addMethods(clazz.constructors.mapNotNull {
        val params =
            it.parameters.map { param -> ParameterSpec.builder(param.type, param.name).build() }
...
private fun TypeSpec.Builder.addMethodsFrom(clazz: Class<*>, implemented: MutableSet<Pair<String, String>>) =
    this.addMethods(clazz.declaredMethods.mapNotNull {
        ...
        val method =
            MethodSpec.methodBuilder(it.name)
                ...
                .addParameters(it.parameters.map { param ->
        ...
        val args = CodeBlock.of(
            it.parameters.joinToString(", ") { "\$N" },
            *it.parameters.map { param -> param.name }.toTypedArray()
        )

        val invoke = CodeBlock.of(
            "\$N.invokeMethod(\$N, \$S, \$S, new \$T[] { \$L })",
            "handler",
            "this",
            it.name,
            descriptor,
