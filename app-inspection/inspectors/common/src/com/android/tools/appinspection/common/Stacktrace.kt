 ...
 *
 * If [packagePrefix] is specified, starting from the top all lines containing the prefix will be ignored until encountering a line that
 * does not contain it. Then [offset] logic is applied.
 */
fun getStackTrace(offset: Int, packagePrefix: String? = APPINSPECTION_PACKAGE_PREFIX): String {
  return Throwable().stackTrace.let { stacks ->
    val trimmed = packagePrefix?.let { prefix -> stacks.dropWhile { element -> element.className.startsWith(prefix) } } ?: stacks.toList()
