 ...
 *
 * This class is similar to [java.util.Optional], except it may also contain an "error" with an associated text message.
 */
class OptionalValue<T : Any> private constructor(private val obj: T) {
  ...
  companion object {
    ...
    fun <T : Any> ofError(message: String): OptionalValue<T> {
      @Suppress("UNCHECKED_CAST") val obj = Error(message) as T
    ...
    /** Returns an [OptionalValue] a nullable [value], either [OptionalValue.empty] if [value] is `null` or [OptionalValue.of] otherwise. */
...
/**
 * Returns the most "precise" value of [this] and [other], where [OptionalValue.hasValue] is considered "more precise" than
 * [OptionalValue.isError], itself considered "more precise" than [OptionalValue.empty]
 */
