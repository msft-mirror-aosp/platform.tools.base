 ...
 *
 * See the [processesFlow] property for the list of [processes][JdwpProcess] exposed as a [StateFlow].
 */
interface JdwpProcessTracker {
  ...
  /**
   * A [CoroutineScope] tied to the lifecycle of this [JdwpProcessTracker], which is typically tied to the lifecycle of the corresponding
   * [device].
   */
   ...
   * [JdwpProcess] instances contained in emitted lists remain the same for processes that remain active.
   *
  ...
  companion object {
    ...
    /**
     * Creates a [JdwpProcessTracker] instance that actively tracks JDWP processes of a given [device]. Use the
     * [JdwpProcessTracker.processesFlow] property to access or collect the list of active [JdwpProcess].
     *
     * @param device The [ConnectedDevice] to track processes on.
     * @param useTrackAppIfAvailable If `true`, the implementation will use the `track-app` device service if available. This is generally
     *   more efficient, and allows reusing an existing `track-app` service connection if one is already active.
     */
    fun create(
      device: ConnectedDevice,
      useTrackAppIfAvailable: Boolean = device.session.property(JDWP_PROCESS_TRACKER_SHOULD_USE_TRACK_APP_IF_AVAILABLE),
    ): JdwpProcessTracker {
 ...
 */
class JdwpProcessList(list: List<JdwpProcess>, flowStatus: StateFlowStatus) : ListWithStateFlowStatus<JdwpProcess>(list, flowStatus)
...
/**
 * The default [JdwpProcessTracker] for this device, giving access to the list of [JdwpProcess] currently active on the device (through a
 * [StateFlow]).
 */
val ConnectedDevice.jdwpProcessTracker: JdwpProcessTracker
  get() = this.cache.getOrPut(jdwpProcessTrackerKey) { JdwpProcessTracker.create(this) }
