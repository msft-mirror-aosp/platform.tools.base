...
/**
 * Implementation of [JdwpProcessPropertiesCollector], responsible for collecting properties of a given [JdwpProcess] in a [StateFlow] of
 * [JdwpProcessProperties].
 */
internal class JdwpProcessPropertiesCollectorImpl(override val process: JdwpProcess) : JdwpProcessPropertiesCollector {
  ...
  override val stateFlow: StateFlow<JdwpProcessProperties> = propertiesAtomicStateFlow.asStateFlow()
    get() {
      lazyStartMonitoring
      return field
    }
  ...
  private val lazyStartMonitoring by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      ...
      processScope.launch(device.session.ioDispatcher) {
        runCatching { createFlowUpdater().collectUpdates(propertiesAtomicStateFlow) }
          .onFailure { throwable ->
      ...
      processScope.launch(device.session.ioDispatcher) {
        runCatching {
            ...
            device.session.externalJdwpProcessPropertiesCollectorFactoryList
              .mapNotNull { factory -> factory.create(process) }
              .forEach { externalCollector ->
                launch {
                  runCatching {
                      val handler = ExternalPropertiesCollectorHandler(externalCollector, propertiesAtomicStateFlow)
                      handler.execute()
                    }
            ...
            process.externalJdwpProcessCommandDispatcherList().forEach { externalDispatcher ->
              launch { runCatching { externalDispatcher.start() }.onFailure { throwable -> logger.logIOCompletionErrors(throwable) } }
            }
          }
          .onFailure { throwable -> logger.logIOCompletionErrors(throwable) }
      }
  ...
  companion object {
    /**
     * The process name (and package name) can be set to this value when the process is not yet fully initialized. We should ignore this
     * value to make sure we only return "valid" process/package name. Note that sometimes the process name (or package name) can also be
     * empty.
     */
    ...
    /**
     * Ensure all [OptionalValue] of this [JdwpProcessProperties] that are still [OptionalValue.empty] are updated to
     * [endOfCollectorErrorSingleton]
     */
    internal fun AtomicStateFlow<JdwpProcessProperties>.applyEndOfCollectorErrorIfEmpty() {
      fun <T : Any> OptionalValue<T>.applyEndOfCollectorErrorIfEmpty(): OptionalValue<T> {
        @Suppress("UNCHECKED_CAST") val error = endOfCollectorErrorSingleton as OptionalValue<T>
