...
internal class JdwpProxySocketServerImpl(override val process: JdwpProcess) : JdwpProxySocketServer {
  ...
  private val lazyStartMonitoring by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      logger.debug { "Start monitoring" }

      process.scope.launch(session.ioDispatcher) {
        runCatching { runSocketServer() }.onFailure { throwable -> logger.logIOCompletionErrors(throwable) }
      }
    }
  ...
  private suspend fun runSocketServer() {
    try {
      ...
      session.channelFactory.createServerSocket().use { serverSocket ->
        ...
        proxyStatusStateFlow.update { it.copy(socketAddress = OptionalValue.of(socketAddress)) }
      ...
      proxyStatusStateFlow.update { it.copy(socketAddress = serverTerminationErrorSingleton) }
    }
  }
  ...
  private suspend fun acceptOneJdwpConnection(serverSocket: AdbServerSocket) {
    serverSocket.accept().use { debuggerSocket ->
      ...
      proxyStatusStateFlow.update { it.copy(isExternalDebuggerAttached = true) }
      try {
        proxyJdwpSession(debuggerSocket)
      } finally {
        logger.debug { "Debugger proxy has ended proxy connection" }
        proxyStatusStateFlow.update { it.copy(isExternalDebuggerAttached = false) }
      }
    }
  }
    ...
    process.withJdwpSession {
      ...
      JdwpSession.wrapSocketChannel(device, debuggerSocket, pid, null).use { debuggerSession ->
        coroutineScope {
          ...
          val job1 = launch(session.host.ioDispatcher) { forwardDebuggerJdwpSession(debuggerPipeline, deviceSession, deferredStart) }

          // Forward packets from jdwp process on device to external debugger
          val job2 = launch(session.host.ioDispatcher) { forwardDeviceJdwpSession(deviceSession, debuggerPipeline, deferredStart) }
          ...
          job1.invokeOnCompletion { throwable -> cancelJobs(throwable, job1, job2) }
          job2.invokeOnCompletion { throwable -> cancelJobs(throwable, job1, job2) }
        }
      }
    }
  }
  ...
  private fun cancelJobs(throwable: Throwable?, job1: Job, job2: Job) {
    val cancellationException =
      (throwable as? CancellationException) ?: CancellationException("JDWP session proxy has completed", throwable)
  ...
  /** Forwards JDWP packets from the external debugger [JdwpSessionPipeline] to the device [SharedJdwpSession]. */
  ...
  /** Forwards JDWP packets from the device [SharedJdwpSession] to the external debugger [JdwpSession]. */
   ...
   * * Filtering out DDMS packets is not strictly required for JDWP compliant debuggers (i.e. The JDWP spec. mentions extension packets
   *   should be ignored by debuggers that don't support them), but the reference JDI implementation emits a `System.err` message when any
   *   unsupported packet is received leading to the assumption there is a serious issue. (see
   *   https://github.com/JetBrains/intellij-deps-jdi/blob/9d99ceeacefabae3e4c37b411e5f4b7637aba162/src/main/java/com/jetbrains/jdi/TargetVM.java#L244)
   * * Filtering out DDMS packets also slightly improve performances of this debugger proxy, as DDMS packets are not sent/received on the
   *   communication channel between the external debugger [JdwpSession] and the device [SharedJdwpSession].
   *
   * The main drawback of applying the DDMS filter is that, if needed in the future, it prevents any external debugger connecting to this
   * proxy from directly sending DDMS commands to an Android process.
   */
  ...
  companion object {
    val serverTerminationErrorSingleton = OptionalValue.ofError<InetSocketAddress>("Jdwp proxy server has terminated")
  }
}
