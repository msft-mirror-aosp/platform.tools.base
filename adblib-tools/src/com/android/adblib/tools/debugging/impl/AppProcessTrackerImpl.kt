...
internal class AppProcessTrackerImpl(override val device: ConnectedDevice) : AppProcessTracker {

  private val logger = adbLogger(device.session).withPrefix("${device.session} - $device - ")

  private val processesMutableFlow = MutableStateFlow(AppProcessList(emptyList(), StateFlowStatus.startOfFlow))
  ...
  private suspend fun trackProcesses() {
    try {
      ...
      processesMutableFlow.value = AppProcessList(emptyList(), StateFlowStatus.endOfFlow)
    }
  }

  private fun updateProcessMap(appProcessMap: ProcessMap<AppProcessImpl>, newAppProcessEntryList: List<AppProcessEntry>) {
    ...
    appProcessMap.updateAll(
      newAppProcessEntries.keys,
      valueFactory = { pid ->
  ...
  private fun <K, V> Map<K, V>.checkedGet(key: K): V {
    return this[key]
      ?: run {
        val internalErrorMessage = "Internal error: map should contain key=$key (map keys=${this.keys})"
