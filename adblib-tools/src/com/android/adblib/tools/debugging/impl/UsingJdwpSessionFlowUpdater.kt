...
internal class UsingJdwpSessionFlowUpdater(private val process: JdwpProcess) : JdwpProcessPropertiesFlowUpdater {
  ...
  /**
   * Collects [JdwpProcessProperties] for the [process] and emits them to [stateFlow], retrying as many times as necessary if there is
   * contention on acquiring JDWP sessions to the process.
   */
  ...
  private suspend fun collectUpdatesWorker(processScope: CoroutineScope, stateFlow: AtomicStateFlow<JdwpProcessProperties>) {
    ...
    while (true) {
      ...
      try {
        withTimeout(session.property(PROCESS_PROPERTIES_READ_TIMEOUT).toMillis()) { collect(processScope, collectState) }
        ...
        if (collectState.shouldRetryCollecting) {
          ...
          process.warningsTracker.getLogAction(key = this@UsingJdwpSessionFlowUpdater.toString(), throwable.message.orEmpty()).logInfo(
            logger,
            exceptionToRecord,
          ) {
          ...
          stateFlow.update { oldProps ->
            oldProps.copy(isWaitingForDebugger = oldProps.isWaitingForDebugger.orElse(optionalValueFactory.of(false)))
          }
          logUsage(
            isSuccess = true,
          ...
          process.warningsTracker.didRecover(key = this@UsingJdwpSessionFlowUpdater.toString()).logInfo(logger) {
    ...
    assert(stateFlow.value.areAllPropertiesInitialized()) { "Properties flow should have initialized all JDWP process properties" }
  }
    ...
    session.host.usageTracker.logUsage(
      AdbUsageTracker.Event(
        ...
        jdwpProcessPropertiesCollector =
          AdbUsageTracker.JdwpProcessPropertiesCollectorEvent(
            isSuccess = isSuccess,
  ...
  /** Collects [JdwpProcessProperties] from a new JDWP session to the [process] and emits them to [CollectState.propertiesFlow]. */
  ...
  private suspend fun collectWithSession(jdwpSession: SharedJdwpSession, collectState: CollectState) {
    // Send a few ddms commands to collect process information
    val commands = IntroCommands(heloCommand = createHeloPacket(jdwpSession), featCommand = createFeatPacket(jdwpSession))
  ...
  private suspend fun processHeloReply(collectState: CollectState, packet: JdwpPacketView, workBuffer: ResizableBuffer) {
    val heloChunk =
      processJdwpPacket(packet, "HELO") {
    ...
    collectState.propertiesFlow.update {
      ...
      it.copy(
        ...
        userId = optionalValueFactory.optionalOrErrorIfNull(heloChunk.userId) { userId -> OptionalValue.of(userId) }.orElse(it.userId),
        vmIdentifier =
          optionalValueFactory
            .optionalOrErrorIfNull(heloChunk.vmIdentifier) { vmIdentifier -> optionalValueFactory.ofVmIdentifier(vmIdentifier) }
            .orElse(it.vmIdentifier),
        packageNames =
          optionalValueFactory
            .optionalOrErrorIfNull(heloChunk.packageName) { packageName -> optionalValueFactory.ofFilteredFakeNames(listOf(packageName)) }
            .orElse(it.packageNames),
        instructionSet =
          optionalValueFactory
            .optionalOrErrorIfNull(heloChunk.abi) { abi ->
              val instructionSet = convertLegacyDescriptionToInstructionSet(abi)
              instructionSet?.let { optionalValueFactory.ofInstructionSet(instructionSet) } ?: OptionalValue.empty()
            }
            .orElse(it.instructionSet),
        jvmFlags =
          optionalValueFactory
            .optionalOrErrorIfNull(heloChunk.jvmFlags) { jvmFlags -> optionalValueFactory.ofJvmFlags(jvmFlags) }
            .orElse(it.jvmFlags),
        isNativeDebuggable =
          optionalValueFactory
            .optionalOrErrorIfNull(heloChunk.isNativeDebuggable) { isNativeDebuggable -> OptionalValue.of(isNativeDebuggable) }
  ...
  private suspend fun processFeatReply(collectState: CollectState, packet: JdwpPacketView, workBuffer: ResizableBuffer) {
    val featChunk =
      processJdwpPacket(packet, "FEAT") {
    ...
    collectState.propertiesFlow.update { it.copy(features = optionalValueFactory.ofFeatures(featChunk.features)) }
    logger.verbose { "Updated stateflow: ${collectState.propertiesFlow.value}" }
  }

  private suspend fun processWaitCommand(collectState: CollectState, chunkCopy: DdmsChunkView, workBuffer: ResizableBuffer) {
    val waitChunk = processDdmsChunk(chunkCopy, "WAIT") { DdmsWaitChunk.parse(chunkCopy, workBuffer) }
    logger.debug { "`WAIT` command: $waitChunk" }
    collectState.propertiesFlow.update { it.copy(isWaitingForDebugger = optionalValueFactory.of(true)) }
    logger.verbose { "Updated stateflow: ${collectState.propertiesFlow.value}" }
  }

  private suspend fun processApnmCommand(collectState: CollectState, chunkCopy: DdmsChunkView, workBuffer: ResizableBuffer) {
    val apnmChunk = processDdmsChunk(chunkCopy, "APNM") { DdmsApnmChunk.parse(chunkCopy, workBuffer) }
    ...
    collectState.propertiesFlow.update {
      it.copy(
        ...
        userId = optionalValueFactory.optionalOrErrorIfNull(apnmChunk.userId) { userId -> OptionalValue.of(userId) }.orElse(it.userId),
        packageNames =
          optionalValueFactory
            .optionalOrErrorIfNull(apnmChunk.packageName) { packageName -> optionalValueFactory.ofFilteredFakeNames(listOf(packageName)) }
            .orElse(it.packageNames),
      )
    }
    logger.verbose { "Updated stateflow: ${collectState.propertiesFlow.value}" }
  ...
  private suspend fun createHeloPacket(jdwpSession: SharedJdwpSession): JdwpPacketView {
    ...
    return createDdmsChunkPacket(jdwpSession.nextPacketId(), DdmsChunkType.HELO, payload.forChannelWrite())
  }
  ...
  private suspend fun createFeatPacket(jdwpSession: SharedJdwpSession): JdwpPacketView {
    ...
    // Return is as a packet
    return createDdmsChunkPacket(jdwpSession.nextPacketId(), DdmsChunkType.FEAT, payload.forChannelWrite())
  }

  private suspend fun createDdmsChunkPacket(packetId: Int, chunkType: DdmsChunkType, chunkData: ByteBuffer): JdwpPacketView {
    val chunk =
      EphemeralDdmsChunk(type = chunkType, length = chunkData.remaining(), payloadProvider = PayloadProvider.forByteBuffer(chunkData))
    val workBuffer = ResizableBuffer()
  ...
  private suspend fun launchJdwpSessionHolder(processScope: CoroutineScope, propertiesFlow: AtomicStateFlow<JdwpProcessProperties>) {
    ...
    processScope.launch {
      process.withJdwpSession {
        ...
        propertiesFlow.update { it.copy(isWaitingForDebugger = optionalValueFactory.of(false)) }
  ...
  private suspend fun <R> processJdwpPacket(packet: JdwpPacketView, packetName: String, block: suspend (JdwpPacketView) -> R): R {
    if (packet.isEmpty) {
  ...
  private suspend fun <R> processDdmsChunk(packet: DdmsChunkView, packetName: String, block: suspend (DdmsChunkView) -> R): R {
    return try {
      block(packet)
    } catch (t: Throwable) {
      t.rethrowCancellation()
  ...
  data class IntroCommands(val heloCommand: JdwpPacketView, val featCommand: JdwpPacketView)
  ...
  private class CollectState(val propertiesFlow: AtomicStateFlow<JdwpProcessProperties>) {
    ...
    private val isDone: Boolean
      get() {
        return heloReplyReceived && featReplyReceived && propertiesFlow.value.processName.hasValue
      }
  }
}
...
private fun Throwable.toAdbUsageTrackerFailureType(): AdbUsageTracker.JdwpProcessPropertiesCollectorFailureType {
  ...
  return when {
    this is TimeoutCancellationException -> AdbUsageTracker.JdwpProcessPropertiesCollectorFailureType.NO_RESPONSE

    this is ClosedChannelException -> AdbUsageTracker.JdwpProcessPropertiesCollectorFailureType.CLOSED_CHANNEL_EXCEPTION
    ...
    this is IOException -> AdbUsageTracker.JdwpProcessPropertiesCollectorFailureType.IO_EXCEPTION
