...
internal class SharedJdwpSessionImpl(
  ...
  /** The class that ensures [sendPacket] is thread-safe and also safe from coroutine cancellation. */
  ...
  /** [CompletableDeferred] used to store the [exception][Throwable] thrown by [jdwpSession] when it ended (normally or not). */
  private val jdwpSessionException = CompletableDeferred<Throwable>()

  /**
   * The [MutableSerializedSharedFlow] used to [MutableSerializedSharedFlow.emit] JDWP packets to all active [JdwpPacketReceiver] instances.
   *
   * Note: We use [Any] as the type parameter, but we only emit either [Throwable] or [JdwpPacketView] instances. We don't use [Result] to
   * avoid extra allocations.
   */
  private val jdwpPacketSharedFlow = MutableSerializedSharedFlow<Any>()

  /**
   * The [SharedJdwpSessionMonitor] to invoke when sending or receiving [JdwpPacketView] packets, or `null` if there are no active monitors
   * from [AdbSession.sharedJdwpSessionMonitorFactoryList].
   *
   * Note: We aggregate all monitors so that we can have an efficient `null` check in the common case where there are no active
   * [SharedJdwpSessionMonitor].
   */
  private val jdwpMonitor =
    createAggregateJdwpMonitor(session.sharedJdwpSessionMonitorFactoryList.mapNotNull { factory -> factory.create(this) })

  /**
   * The [SharedJdwpSessionFilterEngine] that forwards all [packets][JdwpPacketView] to all active [SharedJdwpSessionFilter] instances, so
   * that packets can be filtered for [JdwpPacketReceiver] instances that used [JdwpPacketReceiver.withFilter].
   */
  private val jdwpFilter = SharedJdwpSessionFilterEngine(this)

  /**
   * Opens the underlying JDWP session and perform the JDWP handshake "if needed", i.e. only the first time this method is called.
   *
        ...
        jdwpSession
          ?: run {
            jdwpSessionFactory(device).also {
              jdwpSession = it
              scope.launch { sendReceivedPackets() }
            }
          }
      }
      .also { it.waitForHandshake() }
  }
  ...
  override suspend fun shutdown() {
    jdwpSessionMutex.withLock { jdwpSession?.shutdown() }
  }
  ...
  private fun activeJdwpSession(): JdwpSession {
    return jdwpSession ?: run { throw IllegalStateException("${this::class.simpleName} is not open") }
  }

  /** Asynchronous long-running coroutine that forwards packets read from [activeJdwpSession] to active [jdwpPacketSharedFlow] */
  ...
  private suspend inline fun processSessionPacket(
    ...
    if (sessionPacket.isThreadSafeAndImmutable) {
      ...
      sessionPacket.withPayload { sessionPacketPayload ->
        ...
        val payloadProvider = sharedPayloadProviderFactory.create(sessionPacket, sessionPacketPayload)
  ...
  private class JdwpPacketReceiverImpl(private val jdwpSession: SharedJdwpSessionImpl) : JdwpPacketReceiver {
    ...
    override suspend fun receive(receiver: suspend (JdwpPacketView) -> Unit) {
      withContext(jdwpSession.session.ioDispatcher) {
        coroutineScope {
          ...
          val collectingFlow =
            jdwpSession.jdwpPacketSharedFlow.onSubscription {
    ...
    /**
     * Note: Wrt to concurrency: The session can end at any time when receiving packets (see [SharedJdwpSessionImpl.sendReceivedPackets]),
     * so [SharedJdwpSessionImpl.jdwpSessionException] may not be [completed][CompletableDeferred.isCompleted] right away when the session
     * ends.
     *
     * So, if the [JdwpSession] is ending (concurrently, with an exception), we have 2 possible code paths:
     * * If [SharedJdwpSessionImpl.jdwpSessionException] is already [completed][CompletableDeferred.isCompleted], then emitting the
     *   exception to the [FlowCollector] ensures it will be collected by the [JdwpPacketReceiverImpl.collectSharedFlow] method, and
     *   rethrown to the [JdwpPacketReceiver]. This case is taken care of by this method.
     * * If [SharedJdwpSessionImpl.jdwpSessionException] is **not** [completed][CompletableDeferred.isCompleted] yet, it is guaranteed the
     *   [exception][Throwable] will be collected from the [SharedJdwpSessionImpl.jdwpPacketSharedFlow] because
     *   [SharedJdwpSessionImpl.sendReceivedPackets] emits the exception **after** completing [SharedJdwpSessionImpl.jdwpSessionException].
     *   This case is taken care of by the [collectSharedFlow] method.
     */
    ...
    private suspend fun sendReplayPackets(receiveLogger: AdbLogger, receiver: suspend (JdwpPacketView) -> Unit) {
      receiveLogger.verbose { "Emitting ${jdwpSession.replayPackets.size} replay packet(s)" }
      jdwpSession.replayPackets.forEach { replyPacket -> callReceiverWithPacket(receiver, receiveLogger, replyPacket) }
    }

    /**
     * Launch the [activation] callback as a child of this [CoroutineScope] that runs concurrently with the [jdwpPacketSharedFlow] collector
     *
     * Note on exceptions: All exceptions, except for [EOFCancellationException], should be propagated to this [CoroutineScope].
     */
    ...
    /**
     * Collects [sharedFlow], emitting [JdwpPacketView] using [callReceiverWithPacket] to ensure serialized invocations of all active
     * collectors.
     */
  ...
  private class SharedPayloadProviderFactory(session: AdbSession, private val scope: CoroutineScope) : PayloadProviderFactory(session) {
  ...
  /**
   * A [PayloadProvider] that wraps a [payload][AdbInputChannel] using a [ScopedAdbRewindableInputChannel] so that cancellation of pending
   * read operations never close the initial [payload][AdbInputChannel].
   */
  private class ScopedPayloadProvider(
    /** The [CoroutineScope] used to asynchronously read from the [payload]. This scope should be active as long as [payload] is active. */
    ...
    /**
     * The [ScopedAdbRewindableInputChannel] wrapping the original [AdbInputChannel], to ensure cancellation of [AdbInputChannel.read]
     * operations don't close the [AdbInputChannel].
     */
    ...
    override suspend fun shutdown(workBuffer: ResizableBuffer) {
      closed = true
      mutex.withLock { scopedPayload.waitForPendingRead() }
    }

    override suspend fun toOffline(workBuffer: ResizableBuffer): PayloadProvider {
      return withPayload { PayloadProvider.forInputChannel(scopedPayload.toOffline(workBuffer)) }
    }
    ...
    /**
     * An [AdbRewindableInputChannel] that reads from an [AdbInputChannel] in a custom [CoroutineScope] so that cancellation does not affect
     * the source [AdbInputChannel].
     *
     * Note:
     * * The [close] method may invalidate the initial [AdbInputChannel] if there is a pending [read] operation. Caller should call
     *   [waitForPendingRead] to prevent this.
    ...
    private class ScopedAdbRewindableInputChannel(
      ...
      /** Ensure we have a [AdbRewindableInputChannel] so we support [AdbRewindableInputChannel.rewind] */
      private val rewindableInput =
        if (input is AdbRewindableInputChannel) {
          input
        } else {
      ...
      override suspend fun toOffline(workBuffer: ResizableBuffer): AdbRewindableInputChannel {
        return scopedRead { rewindableInput.toOffline(workBuffer) }
      }
      ...
      private suspend inline fun <R> scopedRead(crossinline reader: suspend () -> R): R {
        ...
        return scope.async { reader() }.also { job -> currentReadJob = job }.await().also { currentReadJob = null }
      }

      private fun throwIfPendingRead() {
        check(!(currentReadJob?.isActive ?: false)) { "Operation is not supported if there is a pending read operation" }
      }
    }
  }

  /**
   * Send [JdwpPacketView] to the underlying [activeJdwpSession] in such a way that cancellation of the [sendPacket] coroutine does not
   * cancel the underlying socket connection.
   */
  private class PacketSender(private val scope: CoroutineScope, private val sharedJdwpSession: SharedJdwpSessionImpl) {
      ...
      return if (jdwpMonitors.isEmpty()) {
        ...
        object : SharedJdwpSessionMonitor {
          override suspend fun onSendPacket(packet: JdwpPacketView) {
            jdwpMonitors.forEach { it.onSendPacket(packet) }
          }

          override suspend fun onReceivePacket(packet: JdwpPacketView) {
            jdwpMonitors.forEach { it.onReceivePacket(packet) }
          }

          override fun close() {
            jdwpMonitors.forEach { it.close() }
          }
        }
      }
    }
