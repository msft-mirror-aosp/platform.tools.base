...
internal object JdwpCommands {
  ...
  fun cmdSetToString(cmdSet: Int): String {
    return CmdSet.values().firstOrNull { it.cmdSet == cmdSet }?.name ?: String.format("SET_%02X", cmdSet)
  }

  fun cmdToString(cmdSet: Int, cmd: Int): String {
    return CmdSet.values().firstOrNull { it.cmdSet == cmdSet }?.let { cmdSetEnum -> cmdSetEnum.cmdList.firstOrNull { it.cmd == cmd }?.name }
