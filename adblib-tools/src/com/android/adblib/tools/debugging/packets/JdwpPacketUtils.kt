 ...
 * @throws IllegalArgumentException if [JdwpPacketView.withPayload] does not contain exactly [JdwpPacketView.length] minus
...
/**
 * Serializes the contents [JdwpPacketView] to [workBuffer], returning the [ByteBuffer] wrapped by [workBuffer] containing data from
 * [ByteBuffer.position] `0` to [ByteBuffer.limit] of the packet length.
 */
suspend fun JdwpPacketView.writeToBuffer(workBuffer: ResizableBuffer): ByteBuffer {
  ...
  val byteCount = withPayload { payload -> payload.readRemaining(workBuffer) }
  checkPacketLength(byteCount)
...
/**
 * Appends the byte representation of [JdwpPacketView] at the end of the given [ResizableBuffer], i.e. from [ResizableBuffer.position] to
 * [ResizableBuffer.position] + [JdwpPacketView.length].
 *
 * To access the data written to underlying [ByteBuffer], the caller should call [ResizableBuffer.afterChannelRead].
 */
suspend fun ResizableBuffer.appendJdwpPacket(packet: JdwpPacketView) {
  ...
  packet.withPayload { payload -> payload.readExactly(byteBuffer) }
}
...
internal fun JdwpPacketView.checkPacketLength(byteCount: Int) {
  ...
  if (byteCount != expectedByteCount) {
    throw IllegalArgumentException("JDWP packet should contain $expectedByteCount " + "bytes but contains $byteCount bytes instead")
  }
}

/**
 * Returns a [ByteBuffer] that contains a copy the contents of this [ByteBuffer], from [ByteBuffer.position] to [ByteBuffer.limit]. The
 * returned [ByteBuffer] has data from [position][ByteBuffer.position] 0 to [limit][ByteBuffer.limit].
 */
...
/** Writes the entire contents of [inputChannel] to this [AdbOutputChannel] and returns the number of bytes written. */
internal suspend fun AdbOutputChannel.write(inputChannel: AdbInputChannel, workBuffer: ResizableBuffer = ResizableBuffer()): Int {
