 ...
 * @throws IllegalArgumentException if [DdmsChunkView.withPayload] does not contain exactly [DdmsChunkView.length] bytes
 */
internal suspend fun DdmsChunkView.writeToChannel(channel: AdbOutputChannel, workBuffer: ResizableBuffer = ResizableBuffer()) {
  workBuffer.clear()
 ...
 * @throws IllegalArgumentException if [DdmsChunkView.withPayload] does not contain exactly [DdmsChunkView.length] bytes
 */
internal suspend fun DdmsChunkView.clone(workBuffer: ResizableBuffer = ResizableBuffer()): DdmsChunkView {
  ...
  return EphemeralDdmsChunk(type = type, length = length, payloadProvider = PayloadProvider.forByteBuffer(bufferCopy))
}

/**
 * Provides a view of a [JdwpPacketView] as a "DDMS" packet. A "DDMS" packet is a "JDWP" packet that contains one or more
 * [chunks][DdmsChunkView].
 *
 ...
 */
internal fun JdwpPacketView.ddmsChunks(workBuffer: ResizableBuffer = ResizableBuffer()): Flow<DdmsChunkView> {
...
private fun DdmsChunkView.checkChunkLength(byteCount: Int) {
  ...
  if (byteCount != expectedByteCount) {
    throw IllegalArgumentException("DDMS packet should contain $expectedByteCount " + "bytes but contains $byteCount bytes instead")
  }
}
...
internal suspend fun DdmsChunkView.createFailException(): DdmsFailException {
  try {
    // See
    // https://cs.android.com/android/platform/superproject/+/android13-release:libcore/dalvik/src/main/java/org/apache/harmony/dalvik/ddmc/ChunkHandler.java;l=102
    // 0-3: error code
    ...
    return DdmsFailException(-1, "Unknown error due to invalid FAIL packet format").also { it.addSuppressed(t) }
  }
}
