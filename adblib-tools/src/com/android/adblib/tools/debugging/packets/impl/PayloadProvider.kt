...
internal interface PayloadProvider : SupportsOffline<PayloadProvider>, AutoCloseable {
   ...
   *
   * @throws IllegalStateException if the `payload` of this [PayloadProvider] instance is not available anymore.
   */
   ...
   *
   * **Note to implementors**: Similar to [Mutex.unlock] (for example), this function is **not** a `suspend` function "by design", to ensure
   * that a `finally` block that calls [releasePayload] is executed even when a coroutine cancellation occurs in the `try` block
   * corresponding to the `finally` block.
   */
  fun releasePayload()
  ...
  /** Clone this [PayloadProvider] into a [PayloadProvider] that is guaranteed to always have a `payload` available. */
  ...
  companion object {
    ...
    /**
     * Creates a [PayloadProvider] wrapping the contents of the given [ByteBuffer], from [ByteBuffer.position] to [ByteBuffer.limit].
     *
     * **Note**
     *
     * The contents of [buffer] should not change after calling this method, to ensure the [withPayload] method returns a valid `payload`.
     */
    ...
    private class ForInputChannel(payload: AdbInputChannel) : PayloadProvider {
      private val rewindablePayload =
        if (payload is AdbRewindableInputChannel) {
          payload
        } else {
    ...
    /** A thread-safe, cancellation safe, lock-free implementation of [PayloadProvider], wrapping a [ByteBuffer]. */
    private class ForByteBuffer(private val payload: ByteBuffer) : PayloadProvider, ThreadSafetySupport {
...
/**
 * Invokes [block] with payload of this [PayloadProvider]. The payload is passed to [block] as an [AdbInputChannel] instance that is valid
...
/**
 * Returns whether this [PayloadProvider] is thread-safe and immutable, meaning it can be safely shared across threads and coroutines.
 *
