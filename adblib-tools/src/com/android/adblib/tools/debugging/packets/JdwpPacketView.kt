...
/**
 * Provides access to various elements of a JDWP packet. A JDWP packet always starts with an 11-byte header, followed by variable size
 * buffer of [length] minus 11 bytes.
 */
interface JdwpPacketView {
  ...
  /** The "command set" identifier (1 byte) if the packet is a [isCommand] packet, or throws [IllegalStateException] otherwise. */
  val cmdSet: Int

  /**
   * The "command" identifier (1 byte) within the [cmdSet] if the packet is a [isCommand] packet, or throws [IllegalStateException]
   * otherwise.
   */
  val cmd: Int

  /** The "error code" if the packet is a [isReply] packet, or throws [IllegalStateException] otherwise. */
  val errorCode: Int

  /**
   * **Note: Do NOT use directly, use [withPayload] instead**
   *
   * Returns the payload of this [JdwpPacketView] as an [AdbInputChannel] instance. [releasePayload] must be called when the returned
   * [AdbInputChannel] is not used anymore.
   *
   * @throws IllegalStateException if the `payload` of this [JdwpPacketView] instance is not available anymore.
  ...
  /**
   * Creates an "offline" version of this [JdwpPacketView] that is thread-safe, immutable and has an [withPayload] that is detached from any
   * underlying volatile data source (e.g. a network socket).
   */
  suspend fun toOffline(workBuffer: ResizableBuffer = ResizableBuffer()): JdwpPacketView

  /** Returns `true` is the packet is a "command" packet matching the given [cmdSet] and [cmd] values. */
  ...
  companion object {
    ...
    fun Command(id: Int, length: Int, cmdSet: Int, cmd: Int, payload: AdbInputChannel): JdwpPacketView {
      return EphemeralJdwpPacket.Command(id, length, cmdSet, cmd, PayloadProvider.forInputChannel(payload))
    }

    @Suppress("FunctionName") // constructor like syntax
    fun Reply(id: Int, length: Int, errorCode: Int, payload: AdbInputChannel): JdwpPacketView {
      return EphemeralJdwpPacket.Reply(id, length, errorCode, PayloadProvider.forInputChannel(payload))
    }

    fun fromPacket(source: JdwpPacketView, payload: AdbInputChannel): JdwpPacketView {
      return if (source.isCommand) {
        Command(id = source.id, length = source.length, cmdSet = source.cmdSet, cmd = source.cmd, payload)
      } else {
        Reply(id = source.id, length = source.length, errorCode = source.errorCode, payload)
      }
    }

    fun wrapByteBuffer(buffer: ByteBuffer): JdwpPacketView {
      return MutableJdwpPacket().also { it.wrapByteBuffer(buffer) }
    }

    /**
     * Inline value class that efficiently wraps the [JdwpPacketView.flags], [JdwpPacketView.cmdSet], [JdwpPacketView.cmd] and
     * [JdwpPacketView.errorCode] properties into a single 32-bit integer value.
     */
    @Suppress("NOTHING_TO_INLINE")
    @JvmInline
    internal value class FlagsAndWord
    private constructor(
      /**
      ...
      constructor(flags: Int, cmdSet: Int, cmd: Int, errorCode: Int) : this(buildRawValue(flags, cmdSet, cmd, errorCode))
      ...
      fun withCmd(cmd: Int): FlagsAndWord {
        ...
        return fromRawValues(rawFlags = rawFlags() and JdwpPacketConstants.REPLY_PACKET_FLAG.inv(), rawWord = (rawWord() and 0xff00) or cmd)
      }
      ...
      fun withErrorCode(errorCode: Int): FlagsAndWord {
        ...
        return fromRawValues(rawFlags = rawFlags() or JdwpPacketConstants.REPLY_PACKET_FLAG, rawWord = errorCode)
      }
...
/**
 * Invokes [block] with payload of this [JdwpPacketView]. The payload is passed to [block] as an [AdbInputChannel] instance that is valid
 * only during the [block] invocation.
 *
 * @throws IllegalStateException if the `payload` of this [JdwpPacketView] instance is not available anymore.
 */
...
/**
 * Returns whether this [JdwpPacketView] is thread-safe and immutable, meaning it can be safely shared across threads and coroutines.
 *
...
internal fun JdwpPacketView.toStringImpl(): String {
  return "%s(id=0x%X, length=%d, flags=0x%02X, %s)"
    .format(
      ...
      flags,
      if (isReply) {
        "isReply=true, errorCode=%s[%d]".format(JdwpErrorCode.errorName(errorCode), errorCode)
      } else {
        "isCommand=true, cmdSet=%s[%d], cmd=%s[%d]"
          .format(JdwpCommands.cmdSetToString(cmdSet), cmdSet, JdwpCommands.cmdToString(cmdSet, cmd), cmd)
      },
    )
}
