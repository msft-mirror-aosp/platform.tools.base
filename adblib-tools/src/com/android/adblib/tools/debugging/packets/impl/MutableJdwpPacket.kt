...
/**
 * A mutable version of [JdwpPacketView], to be used for creating JDWP packets or re-using the same instance for multiple views over time
...
internal class MutableJdwpPacket : JdwpPacketView {
    ...
    set(value) {
      if (value < JdwpPacketConstants.PACKET_HEADER_LENGTH) {
        throw IllegalArgumentException(
          "Packet length should always be greater or equal " + "to ${JdwpPacketConstants.PACKET_HEADER_LENGTH}"
        )
      }
      field = value
    }
  ...
  override suspend fun toOffline(workBuffer: ResizableBuffer): JdwpPacketView {
    ...
    return EphemeralJdwpPacket.fromPacket(this, payloadProvider).toOffline(workBuffer)
  }
  ...
  companion object {
    ...
    fun createCommandPacket(packetId: Int, cmdSet: Int, cmd: Int, payload: ByteBuffer): MutableJdwpPacket {
...
/**
 * Read the first 11 bytes of [buffer] into [MutableJdwpPacket.length], [MutableJdwpPacket.id] and so on. The
 * [MutableJdwpPacket.payloadProvider] field is not modified.
 */
