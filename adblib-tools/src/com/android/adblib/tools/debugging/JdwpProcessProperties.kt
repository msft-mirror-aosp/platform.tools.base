...
data class JdwpProcessProperties(
   ...
   * The process name
   *
   * A value of [OptionalValue.empty] indicates the underlying process discovery mechanism is still trying to retrieve the actual value.
   *
   * A value of [OptionalValue.isError], for example [unsupportedByOlderApiSingleton], indicates the underlying process discovery mechanism
   * could not retrieve the actual value for some reason.
   *
   * Note: The process name is often equal to [packageName], except when a `android:process` process name entry is specified in the
   * [AndroidManifest.xml](https://developer.android.com/guide/topics/manifest/application-element) file.
   */
  val processName: OptionalValue<String> = OptionalValue.empty(),

  /**
   * The list of package names of the package hosted by this process. For simple Android application, there is typically only one package
   * name (see [packageName]).
   *
   * A value of [OptionalValue.empty] indicates the underlying process discovery mechanism is still trying to retrieve the actual value.
   *
   * A value of [OptionalValue.isError], for example [unsupportedByOlderApiSingleton], indicates the underlying process discovery mechanism
  ...
  /**
   * A description of the [InstructionSet] (e.g. "64-bit (arm64)"), or [unsupportedByOlderApi] if the device does not support retrieving
   * this information.
   *
   * A value of [OptionalValue.empty] indicates the underlying process discovery mechanism is still trying to retrieve the actual value.
   *
  ...
  val instructionSetDescription: OptionalValue<String>
    get() {
      return _instructionSetDescription ?: run { computeInstructionSetDescription().also { _instructionSetDescription = it } }
    }

  /**
   * Shortcut for getting the first value [packageNames], useful for "regular" Android app that typically have a single package name.
   *
   * A value of [OptionalValue.empty] indicates the underlying process discovery mechanism is still trying to retrieve the actual value.
   *
   * A value of [OptionalValue.isError], for example [unsupportedByOlderApiSingleton], indicates the underlying process discovery mechanism
  ...
  val packageName: OptionalValue<String>
    get() {
      return _packageName ?: run { computePackageName().also { _packageName = it } }
    }
  ...
  private fun computePackageName(): OptionalValue<String> {
    return packageNames.let {
      when {
        it.hasValue && it.getOrThrow().isNotEmpty() -> {
          OptionalValue.of(it.getOrThrow()[0])
        }
        it.isError -> {
          OptionalValue.ofError(it.getErrorMessageOrThrow())
        }
        else -> {
          OptionalValue.empty()
        }
  ...
  companion object {
    ...
    /**
     * The [OptionalValue.isError] containing the error specific to a property of [JdwpProcessProperties] that cannot be retrieved because
     * the property is not supported on older Android APIs.
     */
...
/**
 * Convert this [InstructionSet] (typically `"arm64"` or `"arm"`) to the legacy representation used in [DdmsHeloChunk.abi] for backward
 * compatibility (e.g. `"64-bit (arm)"`).
 */
internal fun InstructionSet.toLegacyDescription(): String {
  // See
  // https://cs.android.com/android/_/android/platform/frameworks/base/+/eea3b0d26916f92184b48d8ba95a064db2ca884c:core/java/android/ddm/DdmHandleHello.java;l=128
  val instructionSetDescription =
    if (text.contains("64")) {
      "64-bit"
    } else {
      "32-bit"
...
/**
 * Convert a legacy instruction set description from the [DdmsHeloChunk.abi] field of [DdmsHeloChunk] (e.g. `"64-bit (arm)"`) into a valid
 * [InstructionSet].
 *
...
/**
 * Returns a new [JdwpProcessProperties] that contains all [OptionalValue] properties that are have [a value][OptionalValue.hasValue] in
 * [newer] or [this].
 */
internal fun JdwpProcessProperties.mergeWith(newer: JdwpProcessProperties): JdwpProcessProperties {
  ...
  return current.copy(
    ...
    isWaitingForDebugger = current.isWaitingForDebugger.mergeWithTerminalValue(newer.isWaitingForDebugger, terminalValue = false),
  )
}

/**
 * Merges two [OptionalValue]s, treating a specific [terminalValue] in the receiver as a final state that should not be overwritten by a
 * newer value.
 */
private fun <T : Any> OptionalValue<T>.mergeWithTerminalValue(newer: OptionalValue<T>, terminalValue: T): OptionalValue<T> {
