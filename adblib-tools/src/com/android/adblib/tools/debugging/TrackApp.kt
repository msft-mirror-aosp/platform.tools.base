...
/**
 * A thread-safe wrapper for [AdbDeviceServices.trackApp] that exposes a [StateFlow] of [AppProcessEntryList] for a given [ConnectedDevice].
 *
 * The implementation uses a single underlying [AdbDeviceServices.trackApp] invocation that is shared by all collectors of the flow.
 *
 * Use the [ConnectedDevice.trackApp] extension to access this component.
 *
 * Note: The caller is responsible for calling [ConnectedDevice.isTrackAppSupported] to make sure [AdbDeviceServices.trackApp] is supported
 * by the device, otherwise the [StateFlow] will only contains entries of [StateFlowStatus.isRetrying] status.
 */
interface TrackApp {
  ...
  /**
   * The [StateFlow] of [AppProcessEntryList], which contains both the list of [AppProcessEntry] entries as well as a
   * [AppProcessEntryList.status] describing the current state of the connection (see [StateFlowStatus])
   */
 ...
 */
class AppProcessEntryList(list: List<AppProcessEntry>, flowStatus: StateFlowStatus) :
...
val ConnectedDevice.trackApp: TrackApp
  get() = cache.getOrPutSynchronized(trackAppKey) { TrackAppImpl(this) }

private val trackAppKey = CoroutineScopeCache.Key<TrackApp>("TrackApp")

/**
 * Whether [AdbFeatures.TRACK_APP] is supported by this [ConnectedDevice]. If `false`, [TrackApp] will only emit errors to the
 * [TrackApp.stateFlow].
 */
...
/**
 * Whether [trackApp] **and** [AdbFeatures.APP_INFO] are supported, meaning [AppProcessEntry] instances will be populated with
 * [AppProcessEntry.processName], [AppProcessEntry.packageNames], etc. This should return `true` for API 36 and later.
 */
