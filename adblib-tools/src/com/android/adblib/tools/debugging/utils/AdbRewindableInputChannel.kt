...
internal interface AdbRewindableInputChannel : AdbInputChannel {
  ...
  /** Rewind this [AdbRewindableInputChannel] to the beginning, so that [read] operations can be executed again. */
  suspend fun rewind()
  ...
  companion object {
    ...
    /**
     * A [AdbRewindableInputChannel] that wraps a [ByteBuffer], from [ByteBuffer.position] to [ByteBuffer.limit]. [rewind] resets the
     * [ByteBuffer.position] to its original value.
     */
    fun forByteBuffer(buffer: ByteBuffer): AdbRewindableInputChannel {
      return ForByteBuffer(buffer)
    }

    /** A [AdbRewindableInputChannel] that wraps an [AdbInputChannel], keeping data read from the channel in-memory to support [rewind]. */
    fun forInputChannel(input: AdbInputChannel, bufferHolder: ByteBufferHolder = ByteBufferHolder()): AdbRewindableInputChannel {
    ...
    private class ForByteBuffer(private val sourceBuffer: ByteBuffer) :
    ...
    /** A [AdbRewindableInputChannel] that wraps an [AdbInputChannel] and buffers data in memory to support [rewinding][rewind]. */
    private class ForInputChannel(
      ...
      private fun throwIfClosed() {
        check(!closed) { throw IllegalStateException("${this::class.simpleName} has been closed") }
      }
...
internal suspend fun AdbRewindableInputChannel.toOffline(workBuffer: ResizableBuffer = ResizableBuffer()): AdbRewindableInputChannel {
  return toOfflineOrNull(workBuffer)
    ?: run {
