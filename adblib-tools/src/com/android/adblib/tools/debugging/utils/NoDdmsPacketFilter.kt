...
internal class NoDdmsPacketFilter(session: SharedJdwpSession) : SharedJdwpSessionFilter {
  ...
  private val logger = adbLogger(session.device.session).withProcessPrefix(session.device, session.pid)
  ...
  /** A packet is sent to the device, store its ID in our list of active requests, so we can skip it later */
