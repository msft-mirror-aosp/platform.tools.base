...
internal class SynchronizedChannelImpl<E> : SynchronizedChannel<E> {
  ...
  private suspend fun processReceivedElement(anyValue: Any?, block: suspend (E) -> Unit) {
    when (anyValue) {
      is SendToken<*> -> {
        ...
        try {
          @Suppress("UNCHECKED_CAST") block(anyValue.element as E)
        } finally {
        ...
        @Suppress("UNCHECKED_CAST") block(anyValue as E)
      }
    }
  }
  ...
  override fun close(cause: Throwable?): Boolean {
    ...
    return channel.close(cause).also {
      val exception = cause ?: ClosedSendChannelException("${this::class.simpleName} has been closed")
  ...
  override fun cancel(cause: CancellationException?) {
    ...
    channel.cancel(cause).also {
      val exception = cause ?: CancellationException("${this::class.simpleName} has been closed")
      cancelPendingSends(exception)
    }
  }
  ...
  private fun cancelPendingSends(exception: Throwable) {
    pendingSends.forEach { token -> token.deferred.completeExceptionally(exception) }
    pendingSends.clear()
  }
