...
internal class DeviceProcessCatalog(session: AdbSession, val deviceId: DeviceId) {
  ...
  private val logger = adbLogger(session).withPrefix("adbSessionId=${deviceId.adbSessionId}, " + "serialNumber=${deviceId.serialNumber} - ")
  ...
  /**
   * Returns a [Flow] of [ProcessInventoryServerProto.ProcessUpdates] that emits a new item everytime anything changes in the
   * [list of processes][ProcessList] tracked by this [DeviceProcessCatalog].
   */
  ...
  fun trackProcessCommands(): Flow<ProcessCommand> = flow { processCommandSharedFlow.collect { emit(it) } }

  fun trackProcessCommandReplies(): Flow<ProcessCommandReply> = flow { processCommandReplySharedFlow.collect { emit(it) } }
      ...
      processUpdates.processUpdateList.forEach { processUpdate ->
        when {
          ...
          processUpdate.hasProcessUpdated() -> {
            ...
            val currentJdwpProcessInfo = processInfoMap.computeIfAbsent(newJdwpProcessInfo.pid) { newJdwpProcessInfo }
            processInfoMap[newJdwpProcessInfo.pid] = currentJdwpProcessInfo.mergeWith(newJdwpProcessInfo)
          }
        }
      }

      // Return new process list
      ProcessList(processes = processInfoMap.values.sortedBy { it.pid })
    }
  }
  ...
  private suspend fun FlowCollector<ProcessUpdates>.emitInitialProcessList(processList: ProcessList) {
    val addProcesses =
      ProcessListUpdates(addedProcessInfo = processList.processes, removedProcessInfo = emptyList(), updatedProcessInfo = emptyList())
    emitProcessListUpdates(addProcesses)
  }

  private suspend fun FlowCollector<ProcessUpdates>.emitProcessListUpdates(updates: ProcessListUpdates) {
    logger.debug {
    ...
    val response =
      ProcessUpdates.newBuilder()
        .addAllProcessUpdate(
          updates.addedProcessInfo.map { processInfo -> ProcessUpdate.newBuilder().setProcessUpdated(processInfo).build() } +
            updates.removedProcessInfo.map { processInfo -> ProcessUpdate.newBuilder().setProcessTerminatedPid(processInfo.pid).build() } +
            updates.updatedProcessInfo.map { processInfo -> ProcessUpdate.newBuilder().setProcessUpdated(processInfo).build() }
        )
        .build()
  ...
  private class ProcessListUpdates(
    ...
    fun isEmpty(): Boolean {
      return addedProcessInfo.isEmpty() && updatedProcessInfo.isEmpty() && removedProcessInfo.isEmpty()
    }
  }

  private fun computeProcessListUpdates(currentList: ProcessList, newList: ProcessList): ProcessListUpdates {
    ...
    val updatedInfoPids =
      (oldInfoMap.keys intersect newInfoMap.keys).filter { pid ->
  ...
  private fun updateProcessListStateFlow(update: (ProcessList) -> ProcessList) {
    processListAtomicStateFlow.update { currentList ->
      update(currentList).also { newList ->
        logger.debug { "Updating process list from ${currentList.processes.size} element(s) " + "to ${newList.processes.size} element(s)" }
      }
    }
  }
  ...
  /** Returns a [JdwpProcessInfo] instances resulting from the merging of properties of this [JdwpProcessInfo] with [newer]. */
  private fun JdwpProcessInfo.mergeWith(newer: JdwpProcessInfo): JdwpProcessInfo {
    return JdwpProcessInfo.newBuilder(this)
      .also { proto ->
        ...
        // terminal value.
        proto.waitingForDebugger = proto.waitingForDebugger.mergeWithTerminalValue(newer.waitingForDebugger, terminalValue = false)
  ...
  /**
   * Merges two [OptionalBool]s, treating a specific [terminalValue] in the receiver as a final state that should not be overwritten by a
   * newer value.
   */
  private fun OptionalBool.mergeWithTerminalValue(newer: OptionalBool, terminalValue: Boolean): OptionalBool {
