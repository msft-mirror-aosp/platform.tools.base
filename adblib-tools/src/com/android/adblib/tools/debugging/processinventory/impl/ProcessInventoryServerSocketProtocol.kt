...
/**
 * Wrapper around the network socket protocol used to send [ProcessInventoryServerProto] messages. This can be used from both the server and
 * client side of the protocol.
 */
internal class ProcessInventoryServerSocketProtocol(private val session: AdbSession, private val socket: AdbChannel) {
  ...
  inner class ClientProtocol(private val clientDescription: String) {
    ...
    /**
     * Invokes the [ProcessInventoryServerProto.Request.TrackDeviceRequestPayload] service on the server a given device [deviceSerial],
     * returning a [Flow] of [ProcessInventoryServerProto.Response] each time there are updates to the inventory of
     * [ProcessInventoryServerProto.JdwpProcessInfo] of the corresponding device.
     */
    fun trackDeviceRequests(deviceSerial: String): Flow<ProcessInventoryServerProto.Response> = flow {
      val request =
        ProcessInventoryServerProto.Request.newBuilder()
          .setClientDescription(clientDescription)
          .setTrackDeviceRequestPayload(
            ProcessInventoryServerProto.Request.TrackDeviceRequestPayload.newBuilder().setDeviceId(deviceId(deviceSerial))
          )
          .build()
    ...
    /**
     * Invokes the [ProcessInventoryServerProto.Request.UpdateDeviceRequestPayload] service on the server given a device [deviceSerial],
     * returning a [ProcessInventoryServerProto.Response] with "ok" status.
     */
      ...
      val request =
        ProcessInventoryServerProto.Request.newBuilder()
          ...
          .setProcessCommandPayload(
            ProcessInventoryServerProto.Request.ProcessCommandPayload.newBuilder()
      ...
      val request =
        ProcessInventoryServerProto.Request.newBuilder()
          ...
          .setProcessCommandReplyPayload(
            ProcessInventoryServerProto.Request.ProcessCommandReplyPayload.newBuilder()
              .setDeviceId(deviceId(deviceSerial))
     ...
     * returning a [ProcessInventoryServerProto.Response] with "ok" status.
     */
    suspend fun sendDeviceProcessRemoval(deviceSerial: String, pid: Int): ProcessInventoryServerProto.Response {
      ...
      val request =
        ProcessInventoryServerProto.Request.newBuilder()
          ...
          .setUpdateDeviceRequestPayload(
            ProcessInventoryServerProto.Request.UpdateDeviceRequestPayload.newBuilder()
              .setDeviceId(deviceId(deviceSerial))
              .setProcessUpdates(
                ProcessInventoryServerProto.ProcessUpdates.newBuilder()
                  .addAllProcessUpdate(
                    processInfoUpdateList.map { ProcessInventoryServerProto.ProcessUpdate.newBuilder().setProcessUpdated(it).build() } +
                      removedProcessList.map { ProcessInventoryServerProto.ProcessUpdate.newBuilder().setProcessTerminatedPid(it).build() }
                  )
              )
          )
          .build()
  ...
  inner class ServerProtocol(private val serverDescription: String) {
    ...
    suspend fun readRequest(): ProcessInventoryServerProto.Request {
      return reader.readRequest().also { lastRequest = it }
    }
    ...
    fun buildOkResponse(block: (ProcessInventoryServerProto.Response.Builder) -> Unit): ProcessInventoryServerProto.Response {
  ...
  private fun deviceId(deviceSerial: String): ProcessInventoryServerProto.DeviceId {
    return ProcessInventoryServerProto.DeviceId.newBuilder()
  ...
  private class Reader(private val logger: AdbLogger, private val inputChannel: AdbInputChannel, private val workBuffer: ResizableBuffer) {
  ...
  companion object {
    ...
    /**
     * Generates a value for [ProcessInventoryServerProto.DeviceId.Builder.setAdbSessionId], currently it is a hard-coded value as we assume
     * devices serial numbers are unique amongst all running instances of the ADB server the process inventory server deals with.
     */
    private fun AdbSession.processInventorySessionId(): String {
      return cache.getOrPutSynchronized(sessionIdKey) { "<adblib-generic-session-id-v1>" }
    }
  }
}
