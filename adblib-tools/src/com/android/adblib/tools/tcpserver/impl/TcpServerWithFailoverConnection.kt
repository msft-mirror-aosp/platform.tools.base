...
internal class TcpServerWithFailoverConnection(
  ...
  override suspend fun <R> withClientSocket(block: suspend (newServerStarted: Boolean, socket: AdbChannel) -> R): R {
    ...
    while (true) {
      try {
        ...
        if (!retryDelayIterator.hasNext()) {
          throw IOException("Operation failed after $failureCount attempt(s)")
            .also { exceptions.forEach { throwable -> it.addSuppressed(throwable) } }
            .also { logger.verbose(it) { "Aborting after $failureCount failures" } }
        }
        val retryDelay = retryDelayIterator.next()
        logger.debug(t) { "Operation failed $failureCount time(s), retrying in " + "${retryDelay.toMillis()} millis" }
  ...
  private inner class ServerConnection(private val initialServerAddress: InetSocketAddress) : AutoCloseable {
    ...
    suspend fun connect(): ConnectionResult {
      return connectMutex.withLock {
        try {
          ...
          currentCoroutineContext().ensureActive()
          logger.info {
            "Cannot connect to server at address '$currentServerAddress', " +
    ...
    override fun toString(): String {
      return "${this::class.java.simpleName}(currentServerSocket=$currentServerSocket, " + "currentServerJob=$currentServerJob"
    }

    private suspend fun tryConnect(): AdbChannel {
      return session.channelFactory.connectSocket(currentServerAddress, connectTimeout.toMillis(), TimeUnit.MILLISECONDS)
    }
    ...
    private suspend fun tryStartServer() {
      session.channelFactory.createServerSocket().closeOnException { serverSocket ->
        ...
        logger.verbose { "Started server at '$currentServerAddress' with server " + "socket '$currentServerSocket'" }
      }
    }
    ...
    private suspend fun tryBind(serverSocket: AdbServerSocket): InetSocketAddress {
      ...
      return result.getOrNull() ?: run { throw IOException("Error starting server on socket", result.exceptionOrNull()) }
    }
  ...
  private class ConnectionResult(val newServerStarted: Boolean, val socketChannel: AdbChannel) : AutoCloseable {
