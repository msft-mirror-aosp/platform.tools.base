...
class EmulatorConsole(private val adbChannel: AdbChannel) : AutoCloseable {

  private val workBuffer = ResizableBuffer()
  private val channelReader = adbChannel.toChannelReader(EMULATOR_CONSOLE_CHARSET, EMULATOR_CONSOLE_NEWLINE)
  ...
  suspend fun avdName(): String =
    sendCommand("avd name").throwOnError().outputLines.firstOrNull() ?: throw EmulatorCommandException("No output from \"avd name\"")

  /**
   * Returns the absolute path to the virtual device in the file system. The path is operating system dependent; it will have / name
   * separators on Linux and \ separators on Windows.
   *
  ...
  suspend fun avdPath(): Path =
    Paths.get(
      sendCommand("avd path").throwOnError().outputLines.firstOrNull() ?: throw EmulatorCommandException("No output from \"avd path\"")
    )

  /** Checks if the virtual machine of Android emulator was stopped. */
  suspend fun isVmStopped(): Boolean {
    val status =
      (sendCommand("avd status").throwOnError().outputLines.firstOrNull()
  ...
  /** Starts recording the emulator screen to the given path. The path must end with .webm and have no whitespace. */
  ...
  internal suspend fun readResponse(): EmulatorCommandResult {
    ...
    while (true) {
      val line =
        try {
          channelReader.readLine() ?: return EmulatorCommandResult(outputLines, "Connection closed unexpectedly")
          ...
          if (e.nonCancellationCause() is AsynchronousCloseException) {
            return EmulatorCommandResult(outputLines, "Connection closed")
          }
          throw e
        }
  ...
  /**
   * The result of invoking an emulator command. Emulator responses contain zero or more lines of output, followed by "OK" on success, or
   * "KO: <error message>" on error.
   *
   * @property outputLines the output of the command prior to the success / error code, without line terminators.
...
/**
 * Attempts to connect to an emulator console at the supplied address, authenticating if required.
 *
 * @throws IOException if we're unable to read the auth token
...
suspend fun AdbSession.openEmulatorConsole(address: InetSocketAddress): EmulatorConsole {
  return EmulatorConsole(channelFactory.connectSocket(address)).closeOnException { console ->
    ...
    if (result.outputLines.any { it.contains(AUTH_REQUIRED) }) {
      ...
      // OK
      val authTokenPromptIdx = result.outputLines.indexOfFirst { it.contains("you can find your <auth_token> in") }
      if (authTokenPromptIdx >= 0 && authTokenPromptIdx < result.outputLines.size - 1) {
        ...
        val authToken = channelFactory.openFile(authTokenPath).use { it.toChannelReader().readLine()?.trim() ?: "" }
...
fun localConsoleAddress(port: Int) = InetSocketAddress(InetAddress.getLoopbackAddress(), port)

private fun String.trimQuotes() = substringAfter('\'').substringBeforeLast('\'')
