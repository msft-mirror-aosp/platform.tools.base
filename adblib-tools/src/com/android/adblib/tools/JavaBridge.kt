...
object JavaBridge {
  ...
  /**
   * Allows invoking any suspending function from a Java [block] using a [CancellableContinuation], returning a [JavaDeferred] that
   * completes when the suspending function completes (exceptionally or not).
   *
   * The returned [JavaDeferred] can be converted into a specific implementation of [Future], for example
   * [ListenableFuture](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-guava/kotlinx.coroutines.guava/as-listenable-future.html)
  ...
  fun <T> invokeAsync(
    ...
    val deferred =
      scope.async<T>(session.host.ioDispatcher) {
  ...
  /**
   * Allows invoking any [suspending function][block] from Java, blocking the calling thread until the coroutine completes.
   *
   * Throws an [IllegalStateException] if the current thread is not allowed to issue blocking calls (see
   * [AdbSessionHost.isEventDispatchThread]).
   *
   * **Note** Consider using [invokeAsync] as an alternative, as [runBlocking] blocks the calling thread and may lead to thread starvation
   * if used too liberally.
   *
   * Example:
   *
   * int version = JavaBridge.runBlocking(continuation -> session.getHostServices().version(continuation) ); System.out.println("ADB
   * internal version number is " + version);
   *
   * @see invokeAsync
...
class JavaDeferred<T>(val session: AdbSession, val scope: CoroutineScope, val deferred: Deferred<T>) {
  ...
  /** Adds a callback that is invoked (on a [AdbSessionHost.ioDispatcher] thread) when this [Deferred] completes. */
  ...
  fun awaitBlocking(timeoutMillis: Long = Long.MAX_VALUE): T {
    ...
    return runBlocking { withTimeout(timeoutMillis) { deferred.await() } }
  }
}
...
interface CheckedFunction<T, R> {
   ...
   */
  @Throws(Throwable::class) fun accept(t: T): R
}
...
interface CheckedBiConsumer<T, U> {
   ...
   * @throws Throwable if an error occurs
   */
  @Throws(Throwable::class) fun accept(t: T, u: U)
}
