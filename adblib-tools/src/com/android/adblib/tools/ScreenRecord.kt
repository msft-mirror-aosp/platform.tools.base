...
/**
 * Records the device's display to a .mp4 file on the device located at [remotePath] using the `screenrecord` command.
 *
 * The function runs as long as the recording is active, and stops when [stopRecordingSignal] is [Deferred.getCompleted].
 *
 * When the function exits successfully, the video file at [remotePath] is guaranteed to be a valid `mp4` file.
 *
 * Note: The caller is responsible for deleting [remotePath]
 *
 * Note: If [stopRecordingSignal] completes exceptionally, this function rethrows the exception (e.g. cancellation) immediately.
 *
...
data class ScreenRecordOptions(
  /**
   * Set the video size, e.g. "1280x720". Default is the device's main display resolution (if supported), 1280x720 if not. For best results,
