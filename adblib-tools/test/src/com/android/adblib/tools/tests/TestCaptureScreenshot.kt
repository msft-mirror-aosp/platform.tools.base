...
class TestCaptureScreenshot {
  ...
  fun testScreenCapAsBufferedImageSuccess() {
    deviceServices.configureShellV2Command(device, "screencap -p", ByteBuffer.wrap(screenshotBytes), emptyByteBuffer, 0)
  ...
  fun testScreenCapAsBufferedImage_correctPhysicalDisplayIdInCommand() {
    deviceServices.configureShellV2Command(device, "screencap -p -d $displayId", ByteBuffer.wrap(screenshotBytes), emptyByteBuffer, 0)

    runBlocking {
      val resultScreenshot = deviceServices.screenCapAsBufferedImage(device, physicalDisplayId = displayId)
      assertArrayEquals(screenshotBytes, resultScreenshot.toBytes())
    }
  }
  ...
  fun testScreenCapAsBufferedImageFailure() {
    deviceServices.configureShellV2Command(device, "screencap -p", ByteBuffer.wrap(screenshotBytes), emptyByteBuffer, 1)
