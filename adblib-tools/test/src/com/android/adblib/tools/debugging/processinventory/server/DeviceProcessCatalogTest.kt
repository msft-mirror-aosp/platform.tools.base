...
class DeviceProcessCatalogTest {
  ...
  private val deviceId = DeviceId.newBuilder().setAdbSessionId("test-session").setSerialNumber("test-device").build()
  private val catalog = DeviceProcessCatalog(fakeAdbSession, deviceId)

  @Test
  fun trackProcessUpdatesInitiallyEmitsEmptyUpdateList(): Unit = CoroutineTestUtils.runBlockingWithTimeout {
    val processUpdates = catalog.trackProcessUpdates().first()
    assertTrue(processUpdates.processUpdateList.isEmpty())
  }

  @Test
  fun trackProcessUpdatesEmitsUpdatedProcessValues(): Unit = CoroutineTestUtils.runBlockingWithTimeout {
  ...
  @Test
  fun newerValuesOverwriteOlderValues(): Unit = CoroutineTestUtils.runBlockingWithTimeout {
    // Prepare
    var collectedIndex = 0
    val process =
      createJdwpProcessInfo(
  ...
  @Test
  fun emptyProcessValuesDoNotOverwriteExistingValues(): Unit = CoroutineTestUtils.runBlockingWithTimeout {
    // Prepare
    var collectedIndex = 0
    val process =
      createJdwpProcessInfo(
  ...
  @Test
  fun testWaitingForDebuggerFalseIsTerminalState(): Unit = CoroutineTestUtils.runBlockingWithTimeout {
    // Prepare
    val pid = 100
  ...
  private fun createJdwpProcessInfo(
    ...
    builder.processName =
      processName?.let { OptionalString.newBuilder().setHasValue(true).setStringValue(it).build() } ?: OptionalString.getDefaultInstance()
    builder.packageNames =
      packageName?.let { OptionalStringList.newBuilder().setHasValue(true).addStringsValue(it).build() }
        ?: OptionalStringList.getDefaultInstance()
    builder.userId =
      userId?.let { OptionalInt32.newBuilder().setHasValue(true).setInt32Value(it).build() } ?: OptionalInt32.getDefaultInstance()
    builder.instructionSet =
      instructionSet?.let { OptionalString.newBuilder().setHasValue(true).setStringValue(it).build() }
        ?: OptionalString.getDefaultInstance()
    builder.vmIdentifier =
      vmIdentifier?.let { OptionalString.newBuilder().setHasValue(true).setStringValue(it).build() } ?: OptionalString.getDefaultInstance()
    builder.jvmFlags =
      jvmFlags?.let { OptionalString.newBuilder().setHasValue(true).setStringValue(it).build() } ?: OptionalString.getDefaultInstance()
    builder.nativeDebuggable =
      nativeDebuggable?.let { OptionalBool.newBuilder().setHasValue(true).setBoolValue(it).build() } ?: OptionalBool.getDefaultInstance()
    builder.waitingForDebugger =
      waitingForDebugger?.let { OptionalBool.newBuilder().setHasValue(true).setBoolValue(it).build() } ?: OptionalBool.getDefaultInstance()
    builder.features =
      feature?.let { OptionalStringList.newBuilder().setHasValue(true).addStringsValue(it).build() }
        ?: OptionalStringList.getDefaultInstance()
