...
class ProcessInventoryServerTest {
  ...
  private suspend fun connectToServer(session: FakeAdbSession, localServerSocket: AdbServerSocket): ProcessInventoryServerSocketProtocol {
  ...
  private suspend fun sendDeviceProcess(
    ...
    val protocol = connectToServer(session, localServerSocket)
    val deviceUpdateResponse =
      protocol
        .forClient("foo")
        .sendDeviceProcessInfo(
          deviceSerial,
          ProcessInventoryServerProto.JdwpProcessInfo.newBuilder()
            .setPid(pid)
            .also { proto ->
              processName?.also {
                proto.processName = ProcessInventoryServerProto.OptionalString.newBuilder().setHasValue(true).setStringValue(it).build()
              }
              packageName?.also {
                proto.packageNames =
                  ProcessInventoryServerProto.OptionalStringList.newBuilder().setHasValue(true).addAllStringsValue(listOf(it)).build()
              }
            }
            .build(),
        )
    assertTrue("Response is not ok: ${deviceUpdateResponse.errorMessage}", deviceUpdateResponse.ok)
  }
  ...
  private suspend fun sendDeviceProcessTermination(
    ...
    val protocol = connectToServer(session, localServerSocket)
    val deviceUpdateResponse = protocol.forClient("foo").sendDeviceProcessRemoval(deviceSerial, pid)
    assertTrue("Response is not ok: ${deviceUpdateResponse.errorMessage}", deviceUpdateResponse.ok)
  }
