...
class JdwpProcessTrackerTest {
  ...
  @Test
  fun testJdwpProcessTrackerSwitchesToTrackApp_whenTrackAppIsSupported(): Unit = runBlockingWithTimeout {
  ...
  @Test
  fun testJdwpProcessTrackerUsedTrackJdwp_whenTrackAppIsSupportedButUseTrackAppIfAvailableIsSetToFalse(): Unit = runBlockingWithTimeout {
    val deviceState = runJdwpProcessTrackerTest(createDeviceSupportingTrackApp = true, useTrackAppIfAvailable = false)
  ...
  private suspend fun CoroutineScope.runJdwpProcessTrackerTest(
    ...
    val fakeDevice =
      fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(apiLevel), DeviceState.HostConnectionType.USB)
    ...
    assertSame(listOfProcessList[0].first { it.pid == pid10 }, listOfProcessList[1].first { it.pid == pid10 })
  ...
  fun testJdwpProcessTrackerFlowIsExceptionTransparent(): Unit = runBlockingWithTimeout {
    ...
    val exception =
      assertThrows(Exception::class.java) {
        fakeDevice.startClient(pid10, 0, "a.b.c", false)
        val jdwpTracker = JdwpProcessTracker.create(connectedDevice)
        runBlocking { jdwpTracker.processesFlow.collect { throw Exception("My Test Exception") } }
      }
  ...
  fun testJdwpProcessTrackerFlowICanBeCancelled(): Unit = runBlockingWithTimeout {
    ...
    val exception =
      assertThrows(CancellationException::class.java) {
        fakeDevice.startClient(pid10, 0, "a.b.c", false)
        val jdwpTracker = JdwpProcessTracker.create(connectedDevice)
        runBlocking { jdwpTracker.processesFlow.collect { cancel("My Test Exception") } }
      }
  ...
  @Test
  fun testJdwpProcessTrackerExposesUnsupportedPropertiesForApi17AndBelow() = runBlockingWithTimeout {
    val deviceID = "1234"
    val fakeDevice = fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(17), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val connectedDevice = hostServices.session.waitForOnlineConnectedDevice(fakeDevice.deviceId)
    val pid10 = 10
    fakeDevice.startClient(pid10, 100, "a.b.c", false)
    ...
    // Act
    val process = jdwpTracker.processesFlow.filter { it.isNotEmpty() }.map { it.first() }.first()
    process.propertiesFlow
      .takeWhile { it.processName.isEmpty }
      .collect {
  ...
  @Test
  fun testJdwpProcessTrackerExposesUnsupportedPropertiesForApi18To20() = runBlockingWithTimeout {
    val deviceID = "1234"
    val fakeDevice = fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(18), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val connectedDevice = hostServices.session.waitForOnlineConnectedDevice(fakeDevice.deviceId)
    val pid10 = 10
    fakeDevice.startClient(pid10, 100, "a.b.c", false)
  ...
  @Test
  fun testJdwpProcessTrackerExposesUnsupportedPropertiesForApi21To23() = runBlockingWithTimeout {
    val deviceID = "1234"
    val fakeDevice = fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(21), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val connectedDevice = hostServices.session.waitForOnlineConnectedDevice(fakeDevice.deviceId)
    val pid10 = 10
    fakeDevice.startClient(pid10, 100, "a.b.c", false)
  ...
  @Test
  fun testJdwpProcessTrackerExposesUnsupportedPropertiesForApi24To29() = runBlockingWithTimeout {
    val deviceID = "1234"
    val fakeDevice = fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(24), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val connectedDevice = hostServices.session.waitForOnlineConnectedDevice(fakeDevice.deviceId)
    val pid10 = 10
    fakeDevice.startClient(pid10, 100, "a.b.c", false)
  ...
  @Test
  fun testJdwpProcessTrackerSetsAllPropertiesForApi30AndAbove() = runBlockingWithTimeout {
    val deviceID = "1234"
  ...
  @Test
  fun testJdwpProcessPropertiesCollectorSetsAllProperties_whenItUsesAppInfo() = runBlockingWithTimeout {
    val deviceID = "1234"
    val fakeDevice =
      fakeAdb.connectDevice(
        deviceID,
