...
class SharedJdwpSessionTest : AdbLibToolsTestBase() {
  ...
  fun sendPacketWorks() = runBlockingWithTimeout {
    ...
    val reply = jdwpSession.newPacketReceiver().withActivation { ready.complete(Unit) }.receiveFirst { it.id == packet.await().id }
  ...
  fun packetReceiverReceiveMethodWorksWithEmptyPayload(): Unit = runBlockingWithTimeout {
    packetReceiverReceiveMethodWorksWithPacketOfLength(payloadLength = 0, isThreadSafeAndImmutableExpected = true)
  }
  ...
  private suspend fun packetReceiverReceiveMethodWorksWithPacketOfLength(payloadLength: Int, isThreadSafeAndImmutableExpected: Boolean) {
    ...
    val expectedReplyData =
      ByteArray(payloadLength / Char.SIZE_BYTES)
        .also { bytes -> repeat(bytes.size) { index -> bytes[index] = ('a' + (index % 26)).code.toByte() } }
    ...
    val offlineReplyPacket =
      jdwpSession
        .newPacketReceiver()
        .withName("Unit Test Receiver")
        .withActivation { jdwpSession.sendPacket(commandPacket) }
  ...
  fun packetReceiverReceiveMethodIsTransparentToExceptions(): Unit = runBlockingWithTimeout {
    ...
    jdwpSession
      ...
      .receive { throw Exception("My Message") }
  ...
  fun packetReceiverReceiveMethodFromConcurrentReceiversAreNotSerialized(): Unit = runBlockingWithTimeout {
    ...
    val def =
      (0 until receiverCount).map { index ->
  ...
  fun packetReceiverActivationCancellationIsTransparentToCollector(): Unit = runBlockingWithTimeout {
    ...
    jdwpSession.newPacketReceiver().withActivation { throw CancellationException("My Cancellation") }.receive {}
  ...
  fun packetReceiverActivationIsNotExecutedIfNoReceiveMethodCall(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val activationCount = AtomicInteger(0)
    val def = (0 until receiverCount).map { async { jdwpSession.newPacketReceiver().withActivation { activationCount.incrementAndGet() } } }
  ...
  fun packetReceiverReceiveAndActivationDoNotDeadlock(): Unit = runBlockingWithTimeout {
    ...
    repeat(replayPacketCount) { jdwpSession.addReplayPacket(createHeloDdmsPacket(jdwpSession)) }
  ...
  fun packetReceiverConcurrentReceiveAndActivationDoNotDeadlock(): Unit = runBlockingWithTimeout {
    ...
    replayPacketsList.forEach { packets -> assertEquals(replayPacketCount, packets.size) }
  }
  ...
  fun packetReceiverReceiveFirstWithPredicateMethodWorks(): Unit = runBlockingWithTimeout {
    ...
    setHostPropertyValue(session.host, SHARED_JDWP_PACKET_IN_MEMORY_MAX_PAYLOAD_LENGTH, 0)
    val jdwpSession = openSharedJdwpSession(session, fakeDevice.deviceId, 10)
    val sendPacket = createHeloDdmsPacket(jdwpSession)
    ...
    val offlineReplyPacket =
      jdwpSession
        .newPacketReceiver()
        .withName("Unit Test Receiver")
        .withActivation { jdwpSession.sendPacket(sendPacket) }
        .receiveFirst { packet ->
  ...
  fun packetReceiverReceiveFirstMethodWorks(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val offlineReplyPacket =
      jdwpSession.newPacketReceiver().withName("Unit Test Receiver").withActivation { jdwpSession.sendPacket(sendPacket) }.receiveFirst()
  ...
  fun packetReceiverReceiveFirstMethodThrowsWhenNotFound(): Unit = runBlockingWithTimeout {
    ...
    // Act
    exceptionRule.expect(NoSuchElementException::class.java)
    jdwpSession.newPacketReceiver().withName("Unit Test Receiver").withActivation { fakeDevice.stopClient(10) }.receiveFirst()
  ...
  fun packetReceiverReceiveFirstOrNullMethodReturnsNullWhenNotFound(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val firstPacket =
      jdwpSession.newPacketReceiver().withName("Unit Test Receiver").withActivation { fakeDevice.stopClient(10) }.receiveFirstOrNull()
  ...
  fun packetReceiverFlowThrowsExceptionAfterClose() = runBlockingWithTimeout {
    ...
    exceptionRule.expect(Exception::class.java)
    jdwpSession.newPacketReceiver().withName("Test").flow().toList()
  ...
  fun packetReceiverFlowContainsReplayPackets() = runBlockingWithTimeout {
    ...
    val packets =
      jdwpSession.newPacketReceiver().withName("Test").withActivation { jdwpSession.sendPacket(sendPacket3) }.flow().take(3).toList()
  ...
  fun packetReceiverFlowEmitsOfflinePackets() = runBlockingWithTimeout {
    ...
    val payloads = packets.map { packet -> packet.withPayload { payload -> payload.toByteArray(packet.payloadLength) } }
  ...
  fun addReplayPacketStoresOfflineJdwpPacket() = runBlockingWithTimeout {
    ...
    jdwpSession.newPacketReceiver().receiveFirst { replayPacket ->
      replayPacketPayload = replayPacket.withPayload { it.toByteArray(replayPacket.payloadLength) }
      true
    }
  ...
  private suspend fun openSharedJdwpSession(session: AdbSession, deviceSerial: String, pid: Int): SharedJdwpSessionImpl {
    ...
    return registerCloseable(SharedJdwpSession.create(jdwpSession.device, pid) { jdwpSession }).also { it.openAndHandshakeIfNeeded() }
  }

  private suspend fun JdwpPacketView.isApnmCommand(): Boolean {
    return isDdmsCommand && ddmsChunks().firstOrNull { it.type == DdmsChunkType.APNM } != null
  }
  ...
  class TestJdwpSessionMonitorFactory : SharedJdwpSessionMonitorFactory {
    ...
    override fun create(session: SharedJdwpSession): SharedJdwpSessionMonitor {
      return TestJdwpSessionMonitor().also { createdMonitors.add(it) }
    }
