...
class JdwpProcessResumeProcessTest : AdbLibToolsJdwpTestBase() {
  ...
  @Test
  fun testJdwpProcessResumeWorksWithJdwpPropertiesCollectorWhenCalledOnProcessThatIsWaitingForJdwpConnection(): Unit =
  ...
  @Test
  fun testJdwpProcessResumeWorksWithJdwpPropertiesCollectorWhenCalledOnProcessThatIsHoldingTheJdwpConnection(): Unit =
    runBlockingWithTimeout {
      val processPicker: suspend (List<JdwpProcess>) -> JdwpProcess = { jdwpProcessList ->
  ...
  fun testJdwpProcessResumeWorksWithJdwpPropertiesCollectorWhenCalledOnDelegateProcess(): Unit = runBlockingWithTimeout {
    val processPicker: suspend (List<JdwpProcess>) -> JdwpProcess = { jdwpProcessList ->
      jdwpProcessList.first { (it is AbstractJdwpProcessDelegateProvider) }
    }
  ...
  fun testJdwpProcessResumeWorksWithAppInfo(): Unit = runBlockingWithTimeout {
    ...
    fakeAdbRule.host.setPropertyValue(AdbLibToolsProperties.PROCESS_PROPERTIES_COLLECTOR_USE_APP_INFO_IF_AVAILABLE, true)

    // Act: create 2 adb sessions with one device
    val jdwpProcessList =
      createDeviceAndProcessInAdbSessions(
    ...
    jdwpProcessList.forEach { jdwpProcess -> jdwpProcess.jdwpPropertiesCollector.stateFlow.first { it.isWaitingForDebugger.isValue(true) } }
  ...
  private suspend fun testJdwpProcessResumeWorksWithJdwpPropertiesCollector(
    ...
    fakeAdbRule.host.setPropertyValue(AdbLibToolsProperties.PROCESS_PROPERTIES_COLLECTOR_DELAY_DEFAULT, Duration.ofMillis(20))
    val adbSessions = adbSessionsProvider()

    // Act: create 2 adb sessions with one device, and call `resumeProcess`
    val jdwpProcessList =
      createDeviceAndProcessInAdbSessions(deviceApiLevel = 30 /* ensure fake device does not support app_info */, adbSessions = adbSessions)

    // All processes should reach `isWaitingForDebugger`==`true` (since the Process Inventory
    // server is active)
    combine(jdwpProcessList.map { it.jdwpPropertiesCollector.stateFlow }) { it.toList() }
      .first { it.all { jdwpProcessProperties -> jdwpProcessProperties.isWaitingForDebugger.isValue(true) } }
    ...
    // server is active)
    combine(jdwpProcessList.map { it.jdwpPropertiesCollector.stateFlow }) { it.toList() }
      .first { it.all { jdwpProcessProperties -> jdwpProcessProperties.isWaitingForDebugger.isValue(false) } }
  }
  ...
  private suspend fun createDeviceAndProcessInAdbSessions(deviceApiLevel: Int = 30, adbSessions: List<AdbSession>): List<JdwpProcess> {
    ...
    val jdwpProcessList =
      adbSessions.toList().map { session ->
        ...
        val process =
          connectedDevice.jdwpProcessTracker.processesFlow.mapNotNull { processList -> processList.firstOrNull { it.pid == pid } }.first()
        ...
        if (!connectedDevice.useAppInfoForProcessProperties()) {
          ...
          process.jdwpPropertiesCollector.stateFlow.first { it.packageName.hasValue }
        }
        process
      }
  ...
  private fun DeviceState.addSampleJdwpProcess(pid: Int, isWaitingForDebugger: Boolean = false): ClientState {
    return startClient(pid, userId = 0, packageName = "a.b.c", isWaiting = isWaitingForDebugger).also {
      ...
      fakeAdbRule.host.setPropertyValue(AdbLibToolsProperties.RESUME_PROCESS_DELAY_BEFORE_CLOSING_JDWP_SESSION, Duration.ofMillis(20))
    }
  }
  ...
  private fun AdbSession.setupForTest() {
    ...
    val server = ProcessInventoryServerConnection.create(this, config).also { registerCloseable(it) }
    this.installProcessInventoryJdwpProcessPropertiesCollectorFactory(server, enabled = { true })
