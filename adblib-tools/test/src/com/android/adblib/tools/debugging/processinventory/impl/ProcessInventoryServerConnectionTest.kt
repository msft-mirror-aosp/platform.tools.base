...
class ProcessInventoryServerConnectionTest : AdbLibToolsTestBase() {
  ...
  @Test
  fun testProcessListIsEmptyWhenStarting(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      ...
      // Act
      val processList = serverConnection.withConnectionForDevice(device) { processListStateFlow.first() }
  ...
  @Test
  fun testBlockIsCancelledWhenDeviceDisconnects(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      val job = async { serverConnection.withConnectionForDevice(device) { processListStateFlow.collect { started.complete(Unit) } } }
      started.await()
  ...
  @Test
  fun testProcessUpdateUpdatesAllProperties(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      val job = async { serverConnection.withConnectionForDevice(device) { processListStateFlow.collect { processListSnapshots.add(it) } } }
      ...
      serverConnection.withConnectionForDevice(device) { sendProcessProperties(localProperties) }
  ...
  @Test
  fun testProcessUpdatesAreAccumulatedToStateFlow(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      yieldUntil { processListSnapshots.isNotEmpty() && processListSnapshots.last().size == 3 }
      job.cancel()
  ...
  @Test
  fun testProcessTerminationResultsInProcessUpdate(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      serverConnection.withConnectionForDevice(device) { notifyProcessExit(localProperties.pid) }

      yieldUntil { processListSnapshots.run { isNotEmpty() && last().isEmpty() } }
      job.cancel()
  ...
  @Test
  fun testWithConnectionForDeviceIsTransparentToExceptions(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      serverConnection.withConnectionForDevice(device) { processListStateFlow.collect { throw Exception("Foo") } }
  ...
  @Test
  fun testWithConnectionForDeviceIsTransparentToCancellation(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      exceptionRule.expectMessage("Foo")
      serverConnection.withConnectionForDevice(device) { processListStateFlow.collect { cancel("Foo") } }
  ...
  @Test
  fun testProcessUpdatesAreForwardedToAllClients(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val clientCount = 10
      val sessions = listOf(session) + (2..clientCount).map { registerCloseable(fakeAdbRule.createTestAdbSession(session.host)) }
      val serverConnections = sessions.map { session -> createServerConnection(session) }
      val deviceState = addFakeDevice(fakeAdb, api = 32)
      val devices = sessions.map { session -> async { session.waitForOnlineConnectedDevice(deviceState.deviceId) } }.awaitAll()

      // Act: One connection sends updates, all connections should receive them
      val processListFlows = sessions.map { MutableStateFlow<List<JdwpProcessProperties>>(emptyList()) }
      val collectorJobs =
        List(sessions.size) { index ->
          async {
            serverConnections[index].withConnectionForDevice(devices[index]) {
              this.processListStateFlow.collect { processListFlows[index].value = it }
            }
          }
        }
      ...
      sendAndWaitForUpdate(serverConnections.first(), devices.first(), processListFlows, localProperties) { it.pid == 10 }
        .also {
          ...
          @Suppress("DEPRECATION") assertFalse(it.isNativeDebuggable.getOrDefault(false))
      ...
      sendAndWaitForUpdate(serverConnections.first(), devices.first(), processListFlows, localProperties) {
        ...
        .also {
          ...
          assertEquals(InstructionSet.X86, it.instructionSet.getOrNull())
          assertEquals("FooBar", it.jvmFlags.getOrNull())
          @Suppress("DEPRECATION") assertTrue(it.isNativeDebuggable.getOrDefault(false))
          assertFalse(it.isWaitingForDebugger.getOrDefault(false))
          assertTrue(it.features.getOrDefault(emptyList()).isEmpty())
          assertFalse(it.areAllPropertiesInitialized())
        }
  ...
  @Test
  fun testProcessCommandsAreDispatched(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      val deviceState = addFakeDevice(fakeAdb, api = 32)
      val device = session.waitForOnlineConnectedDevice(deviceState.deviceId)
      val processCommandHelper = ProcessCommandHelper(device, serverConnection).also { it.awaitStarted() }

      // Act: send a few commands
      serverConnection.withConnectionForDevice(device) {
        sendProcessCommand(
          ProcessCommand.newBuilder()
  ...
  @Test
  fun testProcessCommandRepliesAreDispatched(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      // Prepare
      setHostPropertyValue(session.host, AdbLibToolsProcessInventoryServerProperties.LOCAL_PORT_V1, findFreeTcpPort())
      val serverConnection = createServerConnection(session)
      ...
      serverConnection.withConnectionForDevice(device) {
        sendProcessCommandReply(
          ProcessCommandReply.newBuilder().setCommandUuid(session.generateUniqueUUID()).setPid(10).setCommandIgnored(true).build()
        )

        sendProcessCommandReply(
          ProcessCommandReply.newBuilder().setCommandUuid(session.generateUniqueUUID()).setPid(10).setCommandExecutedOk(true).build()
        )

        sendProcessCommandReply(
          ProcessCommandReply.newBuilder()
            .setCommandUuid(session.generateUniqueUUID())
            .setPid(10)
  ...
  fun testProcessUpdate_isWaitingForDebuggerIsTerminalWhenFalse(): Unit =
    CoroutineTestUtils.runBlockingWithTimeout {
      ...
      serverConnection.withConnectionForDevice(device) {
        sendProcessProperties(JdwpProcessProperties(pid, isWaitingForDebugger = OptionalValue.of(true)))
      }
      yieldUntil { processListSnapshots.lastOrNull()?.firstOrNull()?.isWaitingForDebugger?.getOrNull() == true }
      ...
      serverConnection.withConnectionForDevice(device) {
        sendProcessProperties(JdwpProcessProperties(pid, isWaitingForDebugger = OptionalValue.of(false)))
      }
      yieldUntil { processListSnapshots.lastOrNull()?.firstOrNull()?.isWaitingForDebugger?.getOrNull() == false }
      assertEquals(false, processListSnapshots.last().first().isWaitingForDebugger.getOrNull())
  ...
  private class ProcessCommandHelper(private val device: ConnectedDevice, private val serverConnection: ProcessInventoryServerConnection) {
    ...
    init {
      scope.launch(device.session.ioDispatcher) { startCommandCollector() }
      scope.launch(device.session.ioDispatcher) { startReplyCollector() }
    ...
    private suspend fun startCommandCollector() {
      serverConnection.withConnectionForDevice(device) {
        processCommandSharedFlow
          .onSubscription {
            delay(100)
            commandCollectorStarted.complete(Unit)
          }
          .collect { processCommands.add(it) }
      }
    }
    ...
    private suspend fun startReplyCollector() {
      serverConnection.withConnectionForDevice(device) {
        processCommandReplySharedFlow
          .onSubscription { commandReplyCollectorStarted.complete(Unit) }
          .collect { processCommandReplies.add(it) }
      }
    }
  }
  ...
  private suspend fun sendAndWaitForUpdate(
    ...
    return waitNonNull {
      coroutineScope {
        ...
        // matching `predicate`
        val asyncLists = processListFlows.map { flow -> async { flow.first { list -> list.any { predicate(it) } } } }
  ...
  private fun createServerConnection(session: AdbSession): ProcessInventoryServerConnection {
    val config =
      object : ProcessInventoryServerConfiguration {
