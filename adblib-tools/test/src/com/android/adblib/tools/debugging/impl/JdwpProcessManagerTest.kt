...
class JdwpProcessManagerTest : AdbLibToolsJdwpTestBase() {
  ...
  fun addProcessesEnsuresProcessInstancesAreCreated() = runBlockingWithTimeout {
    // Prepare
    setHostPropertyValue(session.host, AdbLibToolsProperties.JDWP_PROCESS_MANAGER_REFRESH_DELAY, Duration.ofSeconds(20))
  ...
  fun addProcessesIsThreadSafeAndCoherent() = runBlockingWithTimeout {
    ...
    // Act
    val maps1Async = async(Dispatchers.Default) { (1..100).map { async { processManager.addProcesses(processIds) } }.awaitAll() }
    ...
    maps.forEach { map ->
      ...
      processIds.forEach { pid -> assertNotNull(map[pid]) }
    }
  }
  ...
  fun addProcessesWithDelegatingSessionIsThreadSafeAndCoherent() = runBlockingWithTimeout {
    ...
    val maps2Async = async(Dispatchers.Default) { (1..100).map { async { delegatingProcessManager.addProcesses(processIds) } }.awaitAll() }
  ...
  fun addProcessesWithDelegatingSessionTracksProcessIds() = runBlockingWithTimeout {
    ...
    yieldUntil { !process.scope.isActive }
  ...
  fun delegatingProcessToAlternateAdbSessionIfPresent() = runBlockingWithTimeout {
    // Prepare
    setHostPropertyValue(session.host, AdbLibToolsProperties.PROCESS_PROPERTIES_READ_TIMEOUT, Duration.ofSeconds(1))
  ...
  fun delegatingProcessSharedJdwpSessionToAlternateAdbSessionIfPresent() = runBlockingWithTimeout {
    ...
    delegatingProcess.withJdwpSession { wasJdwpSessionRetained.await() }
    job.join()
  ...
  fun delegatingProcessWithManyJdwpSessionActivationIsThreadSafe() = runBlockingWithTimeout {
    ...
    val delegatingProcesses =
      (1..delegatingSessionCount).map {
        val delegatingSession = connectedJdwpProcess.device.session.createDelegatingChildSession(fakeAdbRule)
    ...
    launch {
      ...
      connectedJdwpProcess.jdwpSessionActivationCount.first { it == delegatingSessionCount * jdwpSessionActivationCount }
      allSessionsSeen.complete(Unit)
    }
    delegatingProcesses
      .map { process ->
        launch(Dispatchers.Default) {
          (1..jdwpSessionActivationCount)
            .map { launch(Dispatchers.Default) { process.withJdwpSession { allSessionsSeen.await() } } }
            .joinAll()
        }
      }
      .joinAll()
    ...
    yieldUntil {
      connectedJdwpProcess.properties.areAllPropertiesInitialized() && connectedJdwpProcess.jdwpSessionActivationCount.value == 0
    }
  }
