...
class ScreenRecordTest {
  ...
  fun test_screenRecord_worksWithStopSignal(): Unit = runBlockingWithTimeout {
    ...
    yieldUntil { ScreenRecordCommandHandler.UNFINISHED_RECORDING_CONTENTS.contentEquals(fakeDevice.getFile("/sdcard/foo/bar.mp4")?.bytes) }
  ...
  fun test_screenRecord_allowsCancellationThroughSignal(): Unit = runBlockingWithTimeout {
    ...
    assertSuspendingThrows(
      ...
      additionalAssertions = { e -> assertEquals("Cancellation from test", e.message) },
    ) {
      screenRecorderJob.await()
    }
    val file = fakeDevice.getFile("/sdcard/foo/bar.mp4")
  ...
  fun test_screenRecord_allowsExceptionThroughSignal(): Unit = runBlockingWithTimeout {
    ...
    assertSuspendingThrows(
      ...
      additionalAssertions = { e -> assertEquals("Exception from test", e.message) },
    ) {
      screenRecorderJob.await()
    }
    val file = fakeDevice.getFile("/sdcard/foo/bar.mp4")
      ...
      additionalAssertions = { e ->
        assertEquals(
          "'No device with serial: 'invalid-id' is connected.'" +
            ...
            " service 'host-serial:invalid-id:features'",
          e.message,
        )
      },
    ) {
  ...
  fun test_screenRecord_ThrowsOnMissingOutputFileName(): Unit = runBlockingWithTimeout {
    ...
    assertSuspendingThrows(
      ...
      additionalAssertions = { e ->
        assertEquals(
          "Screen recording terminated with the error " +
      ...
      deviceServices.screenRecord(connectedDevice.selector, remotePath = "", options = options, stopRecordingSignal = stopSignal)
    }
  }
  ...
  private suspend fun createTestDevice(sdkApi: Int = 31): Pair<DeviceState, ConnectedDevice> {
    val deviceID = "1234"
    val fakeDevice = fakeAdb.connectDevice(deviceID, "test1", "test2", "model", AndroidApiLevel(sdkApi), DeviceState.HostConnectionType.USB)
    fakeDevice.deviceStatus = DeviceState.DeviceStatus.ONLINE
    val connectedDevice = deviceServices.session.waitForOnlineConnectedDevice(fakeDevice.deviceId)
