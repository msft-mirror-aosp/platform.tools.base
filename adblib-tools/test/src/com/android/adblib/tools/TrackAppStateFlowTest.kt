...
class TrackAppStateFlowTest {
  ...
  fun testStateFlowWorks(): Unit = runBlockingWithTimeout {
    ...
    Assert.assertEquals(listOfProcessList[0].first { it.pid == pid10 }, listOfProcessList[1].first { it.pid == pid10 })
  ...
  fun testStateFlowRetriesWhenChannelIsClosed(): Unit = runBlockingWithTimeout {
    ...
    val pid10 = 10
    val pid11 = 11
    setHostPropertyValue(hostServices.session.host, AdbLibToolsProperties.TRACK_APP_RETRY_DELAY, Duration.ofMillis(100))
  ...
  fun testStateFlowReturnsErrorEntryWhenTrackAppNotSupported(): Unit = runBlockingWithTimeout {
    ...
    // Act
    val error = connectedDevice.trackApp.stateFlow.mapNotNull { it.flowStatus.currentError }.first()
  ...
  fun testStateFlowIsTransparentToException(): Unit = runBlockingWithTimeout {
    ...
    runBlocking { appTracker.collect { throw Exception("My Test Exception") } }
  ...
  fun testStateFlowCanBeCancelled(): Unit = runBlockingWithTimeout {
    ...
    fakeDevice.startClient(pid10, 0, "a.b.c", false)
    val appTracker = connectedDevice.trackApp.stateFlow
    runBlocking { appTracker.collect { cancel("My Test Exception") } }

    // Assert
    Assert.fail("Should not reach")
  }

  private fun <T : Any> setHostPropertyValue(host: AdbSessionHost, property: AdbSessionHost.Property<T>, value: T) {
