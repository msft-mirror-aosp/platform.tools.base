...
private val timeBenchmark =
  Benchmark.Builder("Multipreview ASM Search Time Benchmark")
...
private val memoryBenchmark =
  Benchmark.Builder("Multipreview ASM Search Memory Usage Benchmark")
    .setDescription("Base line for Multipreview ASM Search memory usage (mean) after $NUMBER_OF_SAMPLES samples.")
    .build()

internal fun computeAndRecordMetric(timeMetricName: String, memoryMetricName: String, computable: () -> MultipreviewMetric) {
  System.gc()
...
/**
 * Remove the largest time measurements as they are outliers. One cannot measure the time that it less than it actually took to run, however
