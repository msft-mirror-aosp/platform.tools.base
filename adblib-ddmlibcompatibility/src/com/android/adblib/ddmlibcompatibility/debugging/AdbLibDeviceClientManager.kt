...
internal class AdbLibDeviceClientManager(
  ...
  fun startDeviceTracking() {
    session.scope.launch {
      ...
      val connectedDevice =
        withTimeoutOrNull(session.property(DEVICE_TRACKER_WAIT_TIMEOUT).toMillis()) { waitForConnectedDevice(iDevice.serialNumber) }
          ?: run {
  ...
  inner class ProcessTrackerHostImpl(override val device: ConnectedDevice) : ProcessTrackerHost {
    ...
    override suspend fun clientsUpdated(list: List<Client>) {
      ...
      ddmlibEventQueue.post(device.scope, "processListUpdated") { listener.processListUpdated(bridge, this@AdbLibDeviceClientManager) }
    }

    override suspend fun postClientUpdated(clientWrapper: AdblibClientWrapper, updateKind: ClientUpdateKind): Deferred<Unit> {
