...
internal class AdblibClientWrapper(private val trackerHost: ProcessTrackerHost, val jdwpProcess: JdwpProcess) : Client, JdwpProcessHolder {
  ...
  fun startTracking() {
    ...
    jdwpProcess.scope.launch { trackJdwpProcessInfo() }
  }
  ...
  private suspend fun trackJdwpProcessInfo() {
    ...
    jdwpProcess.propertiesFlow
      .combine(jdwpProcess.jdwpProxySocketServer.proxyStatusFlow) { properties, proxyStatus -> Pair(properties, proxyStatus) }
  ...
  private suspend fun updateJdwpProcessInfo(
    ...
    with(previousProperties) {
      if (
        hasChanged(processName, newProperties.processName) ||
        ...
        trackerHost.postClientUpdated(this@AdblibClientWrapper, ProcessTrackerHost.ClientUpdateKind.NameOrProperties)
      }
    }
    ...
    if (previousDebuggerStatus != newDebuggerStatus) {
      ...
      @Suppress("DeferredResultUnused") trackerHost.postClientUpdated(this, ProcessTrackerHost.ClientUpdateKind.DebuggerConnectionStatus)
    }
  }
  ...
  private suspend fun updateClientWrapper(
    ...
    val names =
      ClientData.Names(newProperties.processName.getOrNull() ?: "", newProperties.userId.getOrNull(), newProperties.packageName.getOrNull())
    ...
    clientWrapper.clientData.debuggerConnectionStatus =
      when {
  ...
  private fun addFeatures(features: List<String>) {
    ...
    if (!featuresAdded) {
      synchronized(this) {
        if (!featuresAdded) {
          features.forEach { clientData.addFeature(it) }
          featuresAdded = true
        }
      }
    }
  ...
  /**
   * In ddmlib case, this method would return `true` when ddmlib had an active JDWP socket connection with the process on the device. Since
   * we use "on-demand" JDWP connection, we return `true` when 1) the process is still active and 2) we were able to retrieve all process
   * properties during the initial JDWP connection.
   */
  override fun isValid(): Boolean {
    return jdwpProcess.scope.isActive && jdwpProcess.properties.vmIdentifier.hasValue
  }

  /**
   * Returns the TCP port (on "localhost") that an "external" debugger (i.e. IntelliJ or Android Studio) can connect to open a JDWP session
   * with the process.
   */
  ...
  /**
   * Returns `true` if there is an "external" debugger (i.e. IntelliJ or Android Studio) currently attached to the process via a JDWP
   * session.
   */
  ...
  private suspend fun queryProfilerStatus() {
    ...
    @Suppress("DeferredResultUnused") trackerHost.postClientUpdated(this, ProcessTrackerHost.ClientUpdateKind.ProfilingStatus)
  }
  ...
  override fun startSamplingProfiler(samplingInterval: Int, timeUnit: TimeUnit) {
    ...
    runHalfBlockingLegacy("startSamplingProfiler") { progress ->
      jdwpProcess.profiler.startSampleProfiling(samplingInterval.toLong(), timeUnit, bufferSize, progress)
  ...
  /**
   * Requests allocation details (asynchronously) and invokes the [IClientChangeListener.clientChanged] callback, with
   * [Client.CHANGE_HEAP_ALLOCATIONS] as flags. The allocation data (an array of bytes) is available in [ClientData.getAllocationsData]
   * during the callback invocation.
   *
   * Note that allocation tracking should be enabled (see [enableAllocationTracker]) for this method to collect meaningful data.
   */
  ...
  override fun enableAllocationTracker(enabled: Boolean) {
    ...
    //    client.enableAllocationTracker(false)
    //    (ddmlib callback from requestAllocationDetails is invoked some time later)
    runHalfBlockingLegacy("enableAllocationTracker") { progress -> jdwpProcess.allocationTracker.enable(enabled, progress) }
  }

  /**
   * The [notifyVmMirrorExited] method was originally added due to an implementation defect of ddmlib: there was a race condition during
   * process disconnect where ddmlib would non-deterministically lose track of some client processes when the debugger ends its debugging
   * session. See [bug 37104675](https://issuetracker.google.com/issues/37104675) for more information.
   *
   * Adblib keeps track of client processes in a different way, so this method can be a "no-op".
   */
  ...
  override fun captureView(viewRoot: String, view: String, replyHandler: DebugViewDumpHandler) {
    launchLegacyWithJdwpSession("captureView($viewRoot, $view)") {
      jdwpProcess.viewHierarchy.captureView(viewRoot, view) { payload, payloadLength ->
        // Note: At this point, the ddms chunk payload points directly
        // to the socket of the underlying JDWP session.
        // We clone it into an in-memory ByteBuffer (which is wasteful)
        // only because the ddmlib API requires it.
  ...
  private fun <R> runBlockingLegacy(
    ...
    return runBlocking { session.withErrorTimeout(timeout) { block() } }
  }

  /**
   * A mix of [launchLegacy] and [runBlockingLegacy]: The [block] invoked by [launchLegacy] completes a [CompletableDeferred] to unblock a
   * wait inside [runBlockingLegacy].
   */
  private fun runHalfBlockingLegacy(
    ...
    val progress =
      object : JdwpCommandProgress {
        override suspend fun afterSend(packet: JdwpPacketView) {
          deferred.complete(Unit)
        }
      }
    launchLegacy(operation) { block(progress) }
    return runBlockingLegacy(timeout = timeout) { deferred.await() }
  }

  private fun launchLegacy(operation: String, block: suspend CoroutineScope.() -> Unit) {
    // Note: We use `async` here to make sure exceptions are not propagated to
    // the parent context.
    val deferred = legacyOperationsScope.async { block() }
    ...
    deferred.invokeOnCompletion { cause: Throwable? ->
      cause?.also {
        if (cause !is CancellationException) {
          logger.warn(cause, "A legacy ddmlib operation ('$operation') failed with an error ${cause.message}")
        }
      }
    }
  }
  ...
  private fun launchLegacyWithJdwpSession(operation: String, block: suspend SharedJdwpSession.() -> Unit) {
    launchLegacy(operation) { jdwpProcess.withJdwpSession { block() } }
  }

  private fun legacyNotImplemented(@Suppress("SameParameterValue") operation: String) {
    val message = "Operation '$operation' is not implemented because it is deprecated. It should never be called."
    logger.info { message }
