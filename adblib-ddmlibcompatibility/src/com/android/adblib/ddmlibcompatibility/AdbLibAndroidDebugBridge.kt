...
class AdbLibAndroidDebugBridge(
  ...
  /**
   * We need this because the AndroidDebugBridge class is mostly static, coming from ddmlib, and an instance is required for some APIs (e.g.
   * `getDevices`, `isConnected`, `restart`).
   */
  @Volatile private var currentAndroidDebugBridge: AndroidDebugBridge? = null

  /** Set to `true` after one of the `init` methods is called. Is reset to `false` when AndroidDebugBridge is `terminate`-ed. */
  @Volatile private var initialized: Boolean = false

  /**
   * We re-use the `IDeviceManager` implementation from this ddmlib compatibility module, and we create a new instance every time `start` or
   * `restart` is called.
   */
  @Volatile private var adblibCompatDeviceManager: IDeviceManager? = null
  ...
  override fun init(options: AdbInitOptions) {
    logUsage(AdbDelegateUsageTracker.Method.INIT_3) {
      withLock {
        Preconditions.checkState(!initialized, "AndroidDebugBridge.init() has already been called.")
        initialized = true
  ...
  override fun enableFakeAdbServerMode(port: Int) {
    logUsage(AdbDelegateUsageTracker.Method.ENABLE_FAKE_ADB_SERVER_MODE) {
      Preconditions.checkState(
        !initialized,
        "AndroidDebugBridge.init() has already been called or " + "terminate() has not been called yet.",
      )
      isUnitTestMode = true
      sAdbServerPort = port
    }
  ...
  override fun createBridge(): AndroidDebugBridge? {
    return logUsage(AdbDelegateUsageTracker.Method.CREATE_BRIDGE_1) { createBridge(Long.MAX_VALUE, TimeUnit.MILLISECONDS) }
  }
   ...
   *
   * @return a connected bridge, or null if there were errors while creating or connecting to the bridge
   */
   ...
   * @param forceNewBridge force creation of a new bridge even if one with the same location already exists.
   * @param timeout the maximum time to wait
   * @param unit the time unit of the `timeout` argument
   * @return a connected bridge, or null if there were errors while creating or connecting to the bridge
   */
  override fun createBridge(osLocation: String, forceNewBridge: Boolean, timeout: Long, unit: TimeUnit): AndroidDebugBridge? {
    return logUsage(AdbDelegateUsageTracker.Method.CREATE_BRIDGE_4) {
      ...
      val newBridgeInstance = withLock {
        ...
        if (!isUnitTestMode) {
          if (currentAndroidDebugBridge != null) {
            if (mAdbOsLocation != null && mAdbOsLocation.equals(osLocation) && !forceNewBridge) {
              // We return without notifying listeners, since there were no changes
              return@logUsage currentAndroidDebugBridge
            } else {
   ...
   */
  private fun start(bridgeInstance: AndroidDebugBridge, timeout: Long, unit: TimeUnit): Boolean {
    assert(lock.isHeldByCurrentThread)
  ...
  fun updateAdbServerConfiguration() {
    adbServerConfiguration.update {
      AdbServerConfiguration(mAdbOsLocation?.let { Path(it) }, sAdbServerPort, isUserManagedAdbMode, isUnitTestMode, adbEnvVars)
    }
  }
  ...
  /**
   * Disconnects the current debug bridge, and destroy the object. A new object will have to be created with [.createBridge].
   *
  ...
  override fun disconnectBridge() {
    logUsage(AdbDelegateUsageTracker.Method.DISCONNECT_BRIDGE_1) { disconnectBridge(Long.MAX_VALUE, TimeUnit.MILLISECONDS) }
  }
  ...
  override fun addDeviceChangeListener(listener: IDeviceChangeListener) {
    logUsage(AdbDelegateUsageTracker.Method.ADD_DEVICE_CHANGE_LISTENER) { adbChangeEvents.addDeviceChangeListener(listener) }
  }

  override fun removeDeviceChangeListener(listener: IDeviceChangeListener?) {
    logUsage(AdbDelegateUsageTracker.Method.REMOVE_DEVICE_CHANGE_LISTENER) { adbChangeEvents.removeDeviceChangeListener(listener!!) }
  }

  override fun getDeviceChangeListenerCount(): Int {
    return logUsage(AdbDelegateUsageTracker.Method.GET_DEVICE_CHANGE_LISTENER_COUNT) { adbChangeEvents.deviceChangeListenerCount() }
  }

  override fun addClientChangeListener(listener: IClientChangeListener?) {
    logUsage(AdbDelegateUsageTracker.Method.ADD_CLIENT_CHANGE_LISTENER) { adbChangeEvents.addClientChangeListener(listener!!) }
  }

  override fun removeClientChangeListener(listener: IClientChangeListener?) {
    logUsage(AdbDelegateUsageTracker.Method.REMOVE_CLIENT_CHANGE_LISTENER) { adbChangeEvents.removeClientChangeListener(listener!!) }
  }
  ...
  override fun deviceConnected(device: IDevice) {
    logUsage(AdbDelegateUsageTracker.Method.DEVICE_CONNECTED) { adbChangeEvents.notifyDeviceConnected(device) }
  }

  override fun deviceDisconnected(device: IDevice) {
    logUsage(AdbDelegateUsageTracker.Method.DEVICE_DISCONNECTED) { adbChangeEvents.notifyDeviceDisconnected(device) }
  }
   ...
   * 1. Defaults to [.DEFAULT_ADB_PORT] if neither the system property nor the env var are set.
   *
  ...
  private fun getAdbServerPort(): Int {
    ...
    if (prop != null) {
      try {
        ...
        val msg = String.format("Invalid value (%1\$s) for ANDROID_ADB_SERVER_PORT system property.", prop)
        Log.w(DDMS, msg)
      }
    }
    ...
    try {
      ...
      Log.w(
        DDMS,
        "No access to env variables allowed by current security manager. " + "If you've set ANDROID_ADB_SERVER_PORT: it's being ignored.",
      )
    } catch (e: java.lang.IllegalArgumentException) {
      val msg = String.format("Invalid value (%1\$s) for ANDROID_ADB_SERVER_PORT environment variable" + " (%2\$s).", prop, e.message)
      Log.w(DDMS, msg)
    }
   ...
   * @throws IllegalArgumentException when `adbServerPort` is not bigger than 0 or it is not a number at all
   */
  ...
  internal fun getAdbVersionForTesting(adbPath: Path): ListenableFuture<AdbVersion> {
    return withLock { getAdbVersion(adbPath) }
  }
    ...
    return session.scope
      .async {
        ...
        if (exitCode != 0) {
          ...
          if (exitCode == STATUS_DLL_NOT_FOUND && SdkConstants.currentPlatform() == SdkConstants.PLATFORM_WINDOWS) {
            errorMessage.append(
  ...
  private fun checkAdbVersion(adbVersion: AdbVersion?): Boolean {
    ...
    if (adbVersion == null) {
      ...
      val message = String.format("Required minimum version of adb: %1\$s." + "Current version is %2\$s", MIN_ADB_VERSION, adbVersion)
  ...
  private fun startIDeviceManager(bridgeInstance: AndroidDebugBridge) {
    ...
    adblibCompatDeviceManager = AdbLibIDeviceManagerFactory(session).createIDeviceManager(bridgeInstance)
  }

  private fun stopIDeviceManager(timeout: Long = INFINITE_TIMEOUT, unit: TimeUnit = TimeUnit.MILLISECONDS) {
    assert(lock.isHeldByCurrentThread)
  ...
  override fun getDevices(): Array<IDevice> {
    return logUsage(AdbDelegateUsageTracker.Method.GET_DEVICES) { adblibCompatDeviceManager?.devices?.toTypedArray() ?: emptyArray() }
  }
  ...
  override fun restart(): Boolean {
    return logUsage(AdbDelegateUsageTracker.Method.RESTART_1) { restart(Long.MAX_VALUE, TimeUnit.MILLISECONDS) }
  }
  ...
  override fun getVirtualDeviceId(service: ListeningExecutorService, adb: File, device: IDevice): ListenableFuture<String?>? {
    unsupportedMethod()
  }
  ...
  private inline fun <R> logUsage(method: AdbDelegateUsageTracker.Method, crossinline block: () -> R): R {
    return try {
      block().also { adbDelegateUsageTracker?.logUsage(method, /* isException= */ false) }
    } catch (t: Throwable) {
