...
class Main {
  ...
  private fun reformat(root: File, file: File) {
    ...
    if (smartQuotes && (file.name.endsWith(".md") || file.name.endsWith(".html") || file.name.endsWith(".txt"))) {
  ...
  private fun fixQuotes(root: File, file: File, formatted: String): String {
    ...
    fun warnUnterminated(message: String, startIndex: Int) {
      val text = formatted.substring(startIndex, min(formatted.length, startIndex + 40)).replace("\n", "\\n")
      val path = if (root.path != file.path) file.path.removePrefix(root.path).removePrefix(File.separator) else file.path
      println("WARNING: $message in $path; started at line ${lineNumber(startIndex)}, offset $startIndex: $text...")
    }
    ...
    fun Char.isQuoteChar(): Boolean {
      val c = this
      return c == '"' || c == '\u201C' || c == '\u201D' || c == '«' || c == '»' || c == QUOTE_START || c == QUOTE_END
    }
    ...
    while (i < formatted.length) {
      ...
      if (c == '`' && !formatted.startsWith("```", i) && !inPre) {
        ...
        if (inCode) {
          ...
          column == 0 &&
              (formatted.startsWith("<style", i) || formatted.startsWith("<meta", i) || formatted.startsWith("<!-- Markdeep:", i))
      ) {
        skipLine = true
      } else if ((c == '`' && formatted.startsWith("```", i) || c == '~' && formatted.startsWith("~~~", i)) && isLineStart(i)) {
        inPre = !inPre
        if (inPre) {
          preStart = i
        }
