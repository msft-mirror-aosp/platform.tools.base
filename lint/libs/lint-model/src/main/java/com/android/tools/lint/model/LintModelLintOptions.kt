 ...
 * sarifOutput file("$buildDir/reports/lint-report.sarif.json")
...
interface LintModelLintOptions {
  /**
   * Returns the set of issue id's to suppress. Callers are allowed to modify this collection. To suppress a given issue, add the lint issue
   * id to the returned set.
   */
  val disable: Set<String>

  /**
   * Returns the set of issue id's to enable. Callers are allowed to modify this collection. To enable a given issue, add the lint issue id
   * to the returned set.
   */
  val enable: Set<String>

  /**
   * Returns the exact set of issues to check, or null to run the issues that are enabled by default plus any issues enabled via
   * [.getEnable] and without issues disabled via [.getDisable]. If non-null, callers are allowed to modify this collection.
   */
  ...
  /**
   * Returns whether lint should run all checks on test sources, instead of just the lint checks that have been specifically written to
   * include tests (e.g. checks looking for specific test errors, or checks that need to consider testing code such as the unused resource
   * detector)
   *
  ...
  /**
   * Like [.isCheckTestSources], but always skips analyzing tests -- meaning that it also ignores checks that have explicitly asked to look
   * at test sources, such as the unused resource check.
   *
   * @since 3.2.0-alpha14
   */
  ...
  /**
   * Returns whether lint should include explanations for issue errors. (Note that HTML and XML reports intentionally do this
  ...
  /**
   * Returns whether lint should check for fatal errors during release builds. Default is true. If issues with severity "fatal" are found,
   * the release build is aborted.
   */
  val checkReleaseBuilds: Boolean

  /** Returns whether lint should check all dependencies too as part of its analysis. Default is false. */
  val checkDependencies: Boolean

  /**
   * Returns the baseline file to use, if any. The baseline file is an XML report previously created by lint, and any warnings and errors
