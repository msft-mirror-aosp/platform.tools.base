...
class PermissionDataGenerator {
  ...
  private fun computePermissions(skipHidden: Boolean = false): List<Permission>? {
    ...
    while (true) {
      ...
      if (document != null) {
        ...
        while (element != null) {
          processPermissionTag(element, apiLevel, nameToPermission, valueToFieldName, skipHidden, apiLookup)
  ...
  private fun isDangerousPermission(protectionLevels: List<String>, name: String, apiLevel: Int): Boolean {
  ...
  private fun isSignaturePermission(protectionLevels: List<String>): Boolean =
    (protectionLevels.contains("signature") || protectionLevels.contains("privileged") || protectionLevels.contains("signatureOrSystem")) &&
      !protectionLevels.contains("appop")
