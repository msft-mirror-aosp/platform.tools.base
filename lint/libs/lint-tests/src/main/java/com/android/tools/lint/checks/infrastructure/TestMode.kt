...
/**
 * Different types of test execution for lint. See [TestLintTask.testModes]. Similar to an enum, but left open to make it extendable by
 * other test types outside of lint's built in modes. For example, a third party lint check could be run multiple times with different flags
...
open class TestMode(
  ...
  /**
   * Optional hook to run before running lint on the given list of project directories. The hook can return a state object of some sort,
   * which will be passed back to the [after] hook.
   */
  open fun before(context: TestModeContext): Any? = null

  /** Optional hook to run after the test type has finished, which can perform any appropriate test cleanup. */
  open fun after(context: TestModeContext) {}

  /**
   * Optional hook to register to be notified of events triggered by lint. It will pass back the state object optionally created by the
   * [before] hook.
   */
  ...
  /**
   * Whether this test mode will modify the source files relative to what is in the test file descriptors. Adding or removing files (such as
   * .class files) is not considered a modification, only actually changing the source content of a described file.
   */
  ...
  /**
   * Method to check that the output for this test mode is as expected; normally this is just equality but some test modes may modify
   * conditions such that the actual output is different, and they'll want to take this into account before a difference is treated as a
   * failure
   */
  ...
  companion object {
    /**
     * Special marker state to return from [TestMode.before] to signal that upon reflection, this test mode does not need to be checked for
     ...
     * will first drop the minSdkVersion down to 1 and run lint in analysis-mode only; it will then revert the minSdkVersion and run it in
     * merge mode. This will not only find lint checks which are incorrectly deferring minSdkVersion calculations to the merge stage; it
     * will also cause any detectors which are calling illegal methods during analysis mode (such as getMainProject) to be caught. (Note
     * that this isn't limited to minSdkVersion, it will also modify targetSdkVersion, and potentially additional elements in the manifest,
     * such as moving all permissions into the main module, though that is not yet implemented.)
     *
     * For multi-module projects it will pick the "main" module (e.g. app), and then run analysis-only on all the modules, then merge on the
     * main module.
     */
  ...
  /** Allow the test mode to change the project layout by returning a different project layout than the current one which is passed in */
  open fun configureProjects(projects: List<ProjectDescription>): List<ProjectDescription> = projects

  /**
   * State passed to test modes. This is encapsulated in a separate object such that it can vary over time without breaking any third party
