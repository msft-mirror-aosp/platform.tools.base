 ...
 *
 * Issues and detectors are separate classes because a detector can discover multiple different issues as it's analyzing code, and we want
 * to be able to different severities for different issues, the ability to suppress one but not other issues from the same detector, and so
 * on.
 */
class Issue
private constructor(
  ...
  /**
   * Set of platforms where this issue applies. For example, if the analysis is being run on an Android project, lint will include all
   * checks that either don't specify any platform scopes, or includes the android scope.
   */
  var platforms: EnumSet<Platform> = platforms
    private set

  /**
   * Whether we're analyzing Android sources. Note that within an Android project there may be non-Android libraries, but this flag
  ...
  /**
   * Sets previous names for this issue; this is useful when you for various reasons have to rename or combine multiple issues into one; by
   * declaring it here, lint can more gracefully handle existing incidents listed in baselines etc.
   */
  ...
  /**
   * Describes the error found by this rule, e.g. "Buttons must define contentDescriptions". Preferably the explanation should also contain
   * a description of how the problem should be solved. Additional info can be provided via [moreInfo].
   *
  ...
  /**
   * Sorts the detectors alphabetically by id. This is intended to make it convenient to store settings for detectors in a fixed order. It
   * is not intended as the order to be shown to the user; for that, a tool embedding lint might consider the priorities, categories,
   * severities etc of the various detectors.
   *
  ...
  /**
   * The registry providing this issue, if known. Note that this is typically the original registry providing the issue; there may be other
   * issue registries providing this issue as well (for example, [CompositeIssueRegistry]), so it is not the case that each issue returned
   * by registry.getIssues() will point back to the same registry. (Note that lint will set this on its own after loading an issue
   * registry.)
   *
  ...
  /**
   * The vendor providing this fix. You do not need to set this for every issue; as long as the [IssueRegistry] associated with this issue
   * provides a vendor, lint will find and use it.
   */
  ...
  companion object {
     ...
     * @param briefDescription short summary (typically 5-6 words or less), typically describing the **problem** rather than the **fix**
     ...
     * @param priority the priority, a number from 1 to 10 with 10 being most important/severe
     * @param severity the default severity of the issue
     * @param androidSpecific true if this issue only applies to Android, false if it does not apply to Android at all, and null if not
     *   specified or should run on all platforms. Convenience for specifying platforms=[ANDROID].
  ...
  interface IgnoredIdProvider {
    /**
     * Returns a comma separated list of issue id's to be ignored. This string list can be specified in a number of places, such as in
     * `tools:ignore` XML attributes, in `//noinspection` comments in source files that use // as line comments, etc.
     */
