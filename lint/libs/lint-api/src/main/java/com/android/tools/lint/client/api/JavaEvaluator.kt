...
class JavaEvaluator {
  ...
  abstract fun implementsInterface(cls: PsiClass, interfaceName: String, strict: Boolean = false): Boolean

  open fun isMemberInSubClassOf(member: PsiMember, className: String, strict: Boolean = false): Boolean {
   ...
   */
  open fun methodMatches(method: PsiMethod, className: String?, allowInherit: Boolean, vararg argumentTypes: String): Boolean {
    val classMatches =
      when {
        className == null -> true
   ...
   */
  open fun getMethodDescription(method: PsiMethod, includeName: Boolean, includeReturn: Boolean): String? {
  ...
  fun getInternalDescription(method: PsiMethod, includeName: Boolean = false, includeReturn: Boolean = true): String? {
    val signature = StringBuilder()
  ...
  @Deprecated("Use getAnnotations() instead; consider providing a parent", replaceWith = ReplaceWith("getAnnotations(owner, inHierarchy)"))
  abstract fun getAllAnnotations(owner: PsiModifierListOwner, inHierarchy: Boolean): Array<PsiAnnotation>

  abstract fun getAnnotations(owner: PsiModifierListOwner?, inHierarchy: Boolean = false, parent: UElement? = null): List<UAnnotation>
  ...
  abstract fun findAnnotationInHierarchy(listOwner: PsiModifierListOwner, vararg annotationNames: String): PsiAnnotation?

  abstract fun getAnnotationInHierarchy(listOwner: PsiModifierListOwner, vararg annotationNames: String): UAnnotation?

  @Deprecated("Use getAnnotation returning a UAnnotation instead", replaceWith = ReplaceWith("getAnnotation(listOwner, *annotationNames)"))
  abstract fun findAnnotation(listOwner: PsiModifierListOwner?, vararg annotationNames: String): PsiAnnotation?

  abstract fun getAnnotation(listOwner: PsiModifierListOwner?, vararg annotationNames: String): UAnnotation?
  ...
  private fun findOwnerLibrary(dependencies: Collection<LintModelLibrary>, jarFile: String): LintModelLibrary? {
  ...
  private fun findOwnerLibrary(dependencies: Collection<LintModelLibrary>, pathPrefix: String, pathSuffix: String): LintModelLibrary? {
    for (library in dependencies.asSequence().filterIsInstance<LintModelExternalLibrary>()) {
      for (jar in library.jarFiles) {
        val path = jar.path
   ...
   * operator.
   */
  open fun computeArgumentMapping(call: UCallExpression, method: PsiMethod): Map<UExpression, PsiParameter> {
    return emptyMap()
  }
