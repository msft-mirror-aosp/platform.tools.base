...
internal class ConstantEvaluatorImpl(private val evaluator: ConstantEvaluator) {
  ...
  companion object {
    ...
    ): T? = with(ConstantEvaluator().apply { if (allowUnknown) allowUnknowns() }) { eval(expr) as? T }
  ...
  internal class LastAssignmentFinder(
    ...
    override fun visitVariable(node: UVariable): Boolean {
      if (variableLevel < 0) {
        if (node.javaPsi?.isEquivalentTo(variable) == true) {
...
private fun ArgList<Any?>.reduceAsInts(
    ...
    mapOrNull(::tryCoerceToNum)?.let {
      it.ifAny<Long>()?.reduceOn(Number::toLong, opLong) ?: it.ifAny<Int>()?.reduceOn(Number::toInt, opInt)
    }
