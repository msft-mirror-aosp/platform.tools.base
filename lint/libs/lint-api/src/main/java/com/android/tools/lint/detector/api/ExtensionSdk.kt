...
class ExtensionSdk(val name: String, val shortName: String?, val id: Int, val reference: String?) : Comparable<ExtensionSdk> {
  ...
  companion object {
    ...
    /** SDK id's 1 through 999999 are reserved for platform SDKs, where the id matches the corresponding API level */
    const val MAX_PLATFORM_SDK_ID = 999999
    /**
     * Special marker value identifying not just the Android SDK but that the corresponding version API level corresponds to a packed
     * integer containing both the major and minor parts. Deliberately not -1 since that's used as a "not found" return value.
     */
    ...
    fun getAndroidVersionField(api: Int, fullyQualified: Boolean): String = ApiLevel(api).toSourceReference(fullyQualified)

    /**
     * Given a major API level and a minor API level, returns the corresponding field name which encodes the version. If [fullyQualified] is
