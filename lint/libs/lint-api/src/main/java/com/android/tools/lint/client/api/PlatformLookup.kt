...
/** An interface for looking up Android target platforms, by API level, by code name, etc, as well as "give me the latest" */
interface PlatformLookup {
  /**
   * Returns the scanned platforms as a list of [IAndroidTarget] (though note that these [IAndroidTarget] only supports a small subset of
   * the API surface; unsupported methods will throw an exception or when applicable just return empty results. Already cached; okay to be
   * called repeatedly.
   */
  fun getTargets(includeAddOns: Boolean = false): List<IAndroidTarget>

  /**
   * Returns the latest installed platform. If [includePreviews] is false, it will return the most recent stable version. If [minApi] is
  ...
  /**
   * Returns the platform installed for the given [api], if found. If there is a stable version available it will prefer that version over
   * any previews for the same API level.
   */
  fun getTarget(api: Int): IAndroidTarget? {
    ...
    if (first.version.codename != null) {
      return getTargets().lastOrNull { it.version.apiLevel == api && it.version.codename == null } ?: first
    }
