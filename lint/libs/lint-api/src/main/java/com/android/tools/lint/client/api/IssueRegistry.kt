...
abstract class IssueRegistry protected constructor() {
  /**
   * The vendor providing this lint check. For all the built in checks, this will report the Android Open Source Project, but for checks
   * provided from libraries, this should point to the library author/vendor. Lint will include this in lint reports such that bug reports
   * (and compliments!) can be directed to the right place.
   *
   * (This is a val with a default null implementation instead of an abstract method for backwards compatibility; we don't want to cause
   * class load errors when loading older registries in newer lint.)
   */
  ...
  /** The Lint API version this issue registry's checks were compiled. You should return [CURRENT_API]. */
  open val api: Int = -1

  /**
   * The minimum API version this issue registry works with. Normally the same as [api], but if you have tested it with older version and it
  ...
  /**
   * The maximum API level that this lint check works for. This allows an issue registry to provide issues for older versions of lint than a
   * certain version (and then presumably provide a second issue registry with [minApi] set to the next API level with an alternative
   * implementation that works on newer versions).
   *
   * See [com.android.tools.lint.detector.api.describeApi] for API level to version mappings.
   */
  open val maxApi: Int
  ...
  /** The list of issues that can be found by all known detectors (including those that may be disabled!) */
  abstract val issues: List<Issue>

  /**
   * The issue id's from any issues that have been deleted from this registry. This is here such that when an issue no longer applies and is
   * no longer registered, any existing mentions of the issue id in baselines, lint.xml files etc are gracefully handled.
   */
  open val deletedIssues: List<String> = emptyList()

  /**
   * Whether this issue registry is up to date. Normally true but for example for custom rules loaded from disk, may return false if the
   * underlying file is updated or deleted.
   */
   ...
   * @param scope the scope for the analysis, to filter out detectors that require wider analysis than is currently being performed
   * @param platforms the platforms applying to this analysis
   * @param scopeToDetectors an optional map which (if not null) will be filled by this method to contain mappings from each scope to the
   *   applicable detectors for that scope
  ...
  /**
   * Whether this issue registry has state that is cacheable. Issue registries which include project specific state (such as custom checks
   * for example) are not.
   */
  ...
  companion object {
    ...
    private val EMPTY_IMPLEMENTATION = Implementation(Detector::class.java, EnumSet.noneOf(Scope::class.java))

    private var deletedIds = mutableSetOf<String>()
    private var renamedIds = mutableMapOf<String, String>()

    /** Returns true if the given [id] used to be a valid id in lint but has since been deleted or renamed. */
    fun isDeletedIssueId(id: String): Boolean = deletedIds.contains(id)

    /** If the given issue has been renamed (which involves deleting the previous id) return its new name. */
     ...
     * the user's code base. Similar to [LINT_WARNING] but intended for lower severity problems which may or may not be significant.
     */
    @JvmField // temporarily
    val LINT_WARNING =
        Issue.create(
