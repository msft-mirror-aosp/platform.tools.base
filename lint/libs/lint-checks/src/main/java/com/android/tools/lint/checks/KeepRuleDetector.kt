...
class KeepRuleDetector : Detector(), SourceCodeScanner {
  companion object {
    private val IMPLEMENTATION = Implementation(KeepRuleDetector::class.java, Scope.JAVA_FILE_SCOPE)
          ...
          "char.class" -> source.substringBefore(".")
            ...
            if (source.endsWith(".TYPE")) {
              when (source) {
                ...
                else -> getParameterType(context, getJavaClassType(context, argument), isKotlin)
              }
            } else {
    ...
    private fun getParameterType(context: JavaContext, type: PsiType?, isKotlin: Boolean): String? {
      type ?: return null
    ...
    private fun getCastType(argument: UExpression): Pair<PsiType, Boolean>? {
      if (
        argument is UQualifiedReferenceExpression &&
          ...
          (argument.selector as USimpleNameReferenceExpression).resolvedName == "getJavaClass"
      ) {
      ...
      container.accept(
        object : AbstractUastVisitor() {
          override fun visitCallExpression(node: UCallExpression): Boolean {
            ...
            if (REFLECTION_CALLS.contains(name) && name != LOAD_CLASS && name != FOR_NAME) {
              found = true
            }
      ...
      for (annotation in method.uAnnotations) {
        ...
        when (qualifiedName) {
          ...
          USES_REFLECTION_TO_ACCESS_METHOD_FQN -> {
            ...
            val paramsAttribute = annotation.findDeclaredAttributeValue("parameterTypes")
            if (paramsAttribute is UCallExpression) {
              ...
              val paramClassNames = annotation.findDeclaredAttributeValue("parameterTypeNames")
              if (paramClassNames is UCallExpression) {
                paramsAreStrings = true
                val list = mutableListOf<String>()
                for (argument in paramClassNames.valueArguments) {
                  ConstantEvaluator.evaluateString(null, argument, false)?.let(list::add)
                }
                params = list
              }
            }
            ...
            val classNameAttribute = annotation.findDeclaredAttributeValue("returnTypeName")
            if (classNameAttribute != null) {
              returnClass = classNameAttribute.evaluateString() ?: ""
            } else {
              val classConstantAttribute = annotation.findDeclaredAttributeValue("returnType")
          ...
          USES_REFLECTION_TO_ACCESS_FIELD_FQN -> {
            ...
            val classNameAttribute = annotation.findDeclaredAttributeValue("fieldTypeName")
            if (classNameAttribute != null) {
              fieldClass = classNameAttribute.evaluateString() ?: ""
            } else {
              val classConstantAttribute = annotation.findDeclaredAttributeValue("fieldType")
              if (classConstantAttribute is UClassLiteralExpression) {
                classConstantAttribute.type?.canonicalText?.let {
                  fieldClass = if (isKotlin) javaPrimitiveToKotlinPrimitive(it) ?: it else it
  ...
  private class FieldReflection(
    ...
    override fun contains(other: Reflection): Boolean {
      ...
      if (fieldType.isNotEmpty() && other.fieldType.isNotEmpty() && fieldType != other.fieldType) {
        return false
      }
    ...
    if (element is UExpression) {
      ...
      if (element is UQualifiedReferenceExpression) {
        ...
        if (selector.isJavaClassAccess()) {
          ...
          //    public void printFieldValues(PrintableFieldInterface objectWithFields) throws
          // Exception {
          //      for (Field field : objectWithFields.getClass().getDeclaredFields()) {
          // we don't know which potential subclass of PrintableFieldInterface we're going to get
          // from getClass,
  ...
  override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
    ...
    if (name == FOR_NAME || name == LOAD_CLASS) {
      if (hasMemberReflectionCalls(annotationTarget) || node.valueArguments.isEmpty()) {
        // We found a specific reflection call on a member (like getDeclaredMethod, getConstructors,
        // etc.); we'll handle that through a separate visitMethodCall callback.
        return
      }

      var className = ConstantEvaluator.evaluateString(context, node.valueArguments.first(), false)
      var classNameIsConstant = false
      if (className == null) {

        var p = skipParenthesizedExprUp(node.uastParent)
        if (p is UQualifiedReferenceExpression && p.selector.skipParenthesizedExprDown() == node) {
    ...
    // Run a data flow analysis and track what happens to the method *handles* -- whether they're
    // invoked
          ...
          if (type != null) {
            ...
            if (invokedMethodName != null) {
              ...
              while (curr != null) {
                ...
                if (parent is UQualifiedReferenceExpression || parent is UParenthesizedExpression) {
                  curr = parent
                } else {
                  if (parent is UBinaryExpressionWithType) {
                    typeClass =
                      (context.evaluator.erasure(parent.type)?.canonicalText ?: "").let {
                        if (isKotlin) javaPrimitiveToKotlinPrimitive(it) ?: it else it
                      }
            ...
            if (
              ...
              if (
                rhsValue is String &&
                  ...
                  resolved.containingClass?.qualifiedName == "java.lang.reflect.Method"
              ) {
                val access =
                  if (!isFieldLookup) {
                    createMethodReflection(
    ...
    if (reflections.isEmpty()) {
      ...
      when (name) {
        ...
        GET_FIELDS -> {
          ...
          reflections.add(createFieldReflection(node, classConstant, className ?: "", "*"))
        }
      }
    }
    ...
    for (reflection in reflections) {
      ...
      context.report(ISSUE, invocationNode, context.getNameLocation(invocationNode), message, fix)
    }
  }
        ...
        override fun receiver(call: UCallExpression) {
          ...
          if (type != null) {
            ...
            if (access is MethodReflection) {
              access.setParameterList(context, arguments.subList(1, arguments.size), isKotlin)
            }
          } else if (isConstructorNewInstance(methodName, call)) {
            access.node = call
            if (access is MethodReflection) {
