...
abstract class AbstractAnnotationDetector : Detector(), SourceCodeScanner {
  protected fun report(context: JavaContext, issue: Issue, scope: UElement?, location: Location, message: String) {
    report(context, issue, scope, location, message, null)
  }

  protected fun report(context: JavaContext, issue: Issue, scope: UElement?, location: Location, message: String, quickfixData: LintFix?) {
