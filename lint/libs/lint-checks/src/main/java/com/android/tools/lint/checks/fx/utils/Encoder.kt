 ...
 *
 * Users of [Encoder] should not rely on any other encoding details other than the promise that
 * "encoding then decoding gives back the same value",
 */
interface Encoder<T> {
  ...
  companion object {
    ...
    inline fun <reified E : Enum<E>> enum(): Encoder<E> {
      ...
      return when {
        entries.size < Byte.MAX_VALUE ->
          byte.adapt({ it.ordinal.toByte() }, { entries[it.toInt()] })
        else -> int.adapt(Enum<E>::ordinal, entries::get)
      }
    }

    /**
     * Return an encoder like [this], but if it appears as part of a product and has a common
     * [default] value, the encoding of containing product might be optimized.
     */
    ...
    fun <K, V> map(key: Encoder<K>, value: Encoder<V>): Encoder<PersistentMap<K, V>> =
      product(::Pair, key, value)
        .zeroOrMore()
        .adapt(
          to = PersistentMap<K, V>::toList,
          from = { it.fold(persistentMapOf(), PersistentMap<K, V>::plus) },
        )

    /** Encoder for [PersistentSet] of [T] */
    fun <T> set(elem: Encoder<T>): Encoder<PersistentSet<T>> =
      elem.zeroOrMore().adapt(PersistentSet<T>::toList, List<T>::toPersistentSet)
    ...
    inline fun <reified P : Any, T0, T1> product(
      constructor: KFunction2<T0, T1, P>,
      enc0: Encoder<T0>,
      enc1: Encoder<T1>,
    ): Encoder<P> = P::class.uncheckedProduct(constructor, enc0, enc1)
    ...
    fun <P : Any> KClass<P>.uncheckedProduct(
      constructor: KFunction<P>,
      vararg encoders: Encoder<*>,
    ): Encoder<P> = Product(this, constructor, encoders)

    /** Given an encoder for subtype [Ti] of [T], make a [Case] of encoder for [T] */
    inline fun <T, reified Ti : T & Any> case(
      encoder: Encoder<Ti>,
      name: String = Ti::class.java.simpleName,
    ): Case<T, Ti> = Case(name, encoder) { it as? Ti }
  }

  private class WithHintedDefault<T>(val encoder: Encoder<T>, val default: T) :
  ...
  private class Primitive<T>(
    private val read: (InputStream) -> T,
    private val write: (OutputStream, T) -> Unit,
  ) : Encoder<T> {
    override fun encodeTo(stream: OutputStream, state: EncodingState<*>, value: T) =
      write(stream, value)
  ...
  private class Interned<T>(val base: Encoder<T>) : Encoder<T> {
    override fun encodeTo(stream: OutputStream, state: EncodingState<*>, value: T) =
      stream.writeInt(state.intern(base, value))

    override fun decodeFrom(stream: InputStream, state: DecodingState<*>): T =
      state.valueAt(base, stream.readInt())
  }
  ...
  private class Adapter<S, T>(
    private val base: Encoder<T>,
    private val to: (S) -> T,
    private val from: (T) -> S,
  ) : Encoder<S> {
    override fun encodeTo(stream: OutputStream, state: EncodingState<*>, value: S) =
      base.encodeTo(stream, state, to(value))

    override fun decodeFrom(stream: InputStream, state: DecodingState<*>): S =
      from(base.decodeFrom(stream, state))
  }

  private class Product<P : Any>(
    klass: KClass<P>,
    private val constructor: KFunction<P>,
    encoders: Array<out Encoder<*>>,
  ) : Encoder<P> {
    init {
        ...
        val prop =
          klass.memberProperties.find { it.name == param.name }
            ?: throw IllegalArgumentException(
              "Class `${klass.simpleName}` has no field `${param.name}`"
            )
        fun <T> uncheckedField(prop: KProperty1<P, *>, enc: Encoder<T>) =
          Field(prop as KProperty1<P, T>, enc)
    ...
    private class Field<P, T>(val access: KProperty1<P, T>, val encoder: Encoder<T>) {
      fun decode(stream: InputStream, state: DecodingState<*>): T =
        encoder.decodeFrom(stream, state)

      fun encode(stream: OutputStream, state: EncodingState<*>, product: P) =
        encoder.encodeTo(stream, state, access(product))
    }
  }
  ...
  private class Sum<S>(private val cases: Array<out Case<S, *>>) : Encoder<S> {
    ...
    override fun encodeTo(stream: OutputStream, state: EncodingState<*>, value: S) {
      for ((i, case) in cases.withIndex()) {
        if (case.tryEncodeTo(stream, state, value, { indexEncoder.encodeTo(stream, state, i) }))
          return
      }
  ...
  private class Delay<T>(ref: () -> Encoder<T>) : Encoder<T> {
    ...
    override fun encodeTo(stream: OutputStream, state: EncodingState<*>, value: T) =
      delegate.encodeTo(stream, state, value)

    override fun decodeFrom(stream: InputStream, state: DecodingState<*>): T =
      delegate.decodeFrom(stream, state)
  }

  class Case<T, Ti : T & Any>(
    val name: String,
    private val encoder: Encoder<Ti>,
    private val tryCast: (T) -> Ti?,
  ) : Encoder<Ti> by encoder {
    override fun toString() = name

    internal fun tryEncodeTo(
      stream: OutputStream,
      state: EncodingState<*>,
      value: T,
      writeIndex: () -> Unit,
  ...
  class EncodingState<S : OutputStream>(interning: List<Pair<Encoder<*>, S>>) {
    ...
    /**
     * The index of the last intern stream flushed at the end. If flushing one stream results in
     * growing an already flushed intern stream, there'll be a runtime error. This can be avoided by
     * only allowing earlier intern streams to refer to later ones.
     */
    ...
    fun <T> intern(rawEncoder: Encoder<T>, value: T): Int =
      when (val recordIndex = records.indexOfFirst { it.rawEncoder == rawEncoder }) {
        !in records.indices ->
          throw IllegalArgumentException(
            "$rawEncoder not registered for interning in ${records.map { it.rawEncoder }}"
          )
...
private fun InputStream.readInt(): Int =
  (0 until Int.SIZE_BYTES).fold(0) { res, i -> res or (read() shl (i * Byte.SIZE_BITS)) }
 ...
 */
fun <T> Encoder<T>.encodeAsStrings(
  value: T,
  vararg internedEncoders: Encoder<*>,
...
fun <T> Encoder<T>.decodeFromStrings(
  mainEncoding: String,
  vararg interns: Pair<Encoder<*>, String>,
): T =
...
private fun <S : OutputStream, T> Encoder<T>.encodeAndFinalize(
  stream: S,
  state: Encoder.EncodingState<S>,
  value: T,
) {
  try {
