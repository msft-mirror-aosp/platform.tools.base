...
private constructor(
  ...
  override fun outputJson(writer: JsonWriter) {
    ...
    if (compareWithMainBranch) {
      ...
      checkNotNull(baselineMetric) { "baselineMetric must be set when not comparing with main branch" }
  ...
  companion object {
    fun forComparingWithMainBranch(runInfoQueryLimit: Int? = null, significance: Double? = null, relativeShiftValue: Double? = null) =
      UTestAnalyzer(
        baselineMetric = null,
