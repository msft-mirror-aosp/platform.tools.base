...
/*
 * Declares the main JVMTI controller (`JvmtiArtTooling`), which manages
 * bytecode transformations for method hook placement, registers the JVMTI
 * ClassFileLoadHook callback, and implements heap-walking to search for JVM
 * class instances on demand. The interface exposes only raw JVMTI primitives.
 */
...
class JvmtiArtTooling {
 ...
 private:
  ...
  // Returns the JVM descriptor of |clazz| (e.g. "Lcom/example/Foo;"), or an
  // empty string when JVMTI could not provide it.
  std::string ClassDescriptor(jclass clazz);
