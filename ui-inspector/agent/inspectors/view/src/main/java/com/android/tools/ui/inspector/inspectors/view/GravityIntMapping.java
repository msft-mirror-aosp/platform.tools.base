...
/**
 * A functional mapper ({@link IntFunction}) that decodes packed {@link Gravity} integer bitmasks
 * into sets of human-readable string tokens.
 */
public final class GravityIntMapping implements IntFunction<Set<String>> {
    /**
     * Framework standard mapping utility used to decode packed gravity integer bitmasks into sets
     * of human-readable string flags (e.g., "top", "start").
     */
    ...
    public Set<String> apply(int value) {
        Set<String> values = gravityIntFlagMapping.get(value);
        if ((value & Gravity.RELATIVE_LAYOUT_DIRECTION) != 0) {
            // Under the hood, the framework packs relative constants (start/end) with overlapping
            // bit patterns
            // as absolute constants (left/right). If the relative layout direction flag is set,
            // resolve absolute
