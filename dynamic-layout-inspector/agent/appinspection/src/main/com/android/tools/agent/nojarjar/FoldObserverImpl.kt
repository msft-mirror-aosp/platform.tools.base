 ...
 *
 * This class interacts directly with the library built into an app, and so has to be separately from the rest of the agent so it will be
 * able to pick up the kotlin and coroutines versions used by the app rather than the inspector. This also means that this class must be
 * invoked via reflection, as must the sendFoldStateEvent callback. Additionally, `sendFoldStateEvent`, the callback that the state has
 * changed, can't be declared explicitly as `() -> Unit`, and must itself be invoked by reflection as well.
 */
...
class FoldObserverImpl(private val sendFoldStateEvent: Any) : FoldObserver {
  ...
  private val windowRepositoryClass =
    try {
      // beta03 and later
  ...
  private val windowRepositoryGetter =
    try {
      // beta04 and later
    ...
    listeners.computeIfAbsent(rootView) { view ->
      ...
      scope.launch {
        windowInfo.collect { newLayoutInfo ->
          ...
          currentFoldFeature = features.firstOrNull { it != null && foldingFeatureClass.isAssignableFrom(it.javaClass) }
