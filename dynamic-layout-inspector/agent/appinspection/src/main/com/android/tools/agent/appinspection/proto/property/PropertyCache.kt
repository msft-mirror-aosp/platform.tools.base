 ...
 *
 * By calling [typeOf], properties will be cached for that type when encountered for the first time.
 *
 * @param T The parent type of classes to cache property types against. This type MUST either match OR be a subclass of [fqcn].
...
class PropertyCache<T : Any>(private val fqcn: String) {
  ...
  class Data<T : Any>(val properties: List<PropertyBuilder>, val companions: List<InspectionCompanion<T>>) {
