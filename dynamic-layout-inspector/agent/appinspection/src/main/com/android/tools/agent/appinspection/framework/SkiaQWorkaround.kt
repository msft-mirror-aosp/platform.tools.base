...
/**
 * In the initial public version of Q there was a bug in `ViewDebug#startRenderingCommandsCapture` that would sometimes crash apps. The code
 * here is a fixed version of that.
 *
 * This class is no longer necessary starting in T. ViewDebug should be used directly for T+ devices.
 */
object SkiaQWorkaround {

  private fun getFieldValue(o: Any, fieldName: String): Any? {
    val f: Field =
      generateSequence(o.javaClass) { it.superclass }
        .mapNotNull { c ->
          try {
  ...
  fun startRenderingCommandsCapture(tree: View, executor: Executor, callback: Callable<OutputStream?>): AutoCloseable? {
    try {
      VMDebug.allowHiddenApiReflectionFrom(SkiaQWorkaround::class.java)
    } catch (exception: SecurityException) {
      throw UnsupportedOperationException("Unable to enable reflection. " + "App must be built with debugging enabled.")
    }
    ...
    val renderer = getFieldValue(attachInfo, "mThreadedRenderer") ?: throw IllegalStateException("Unable to get ThreadedRenderer")
    val streamingPictureCallbackHandler = StreamingPictureCallbackHelper.createCallback(renderer, callback, executor) as AutoCloseable
    try {
      ...
      StreamingPictureCallbackHelper.setPictureCaptureCallback(renderer, streamingPictureCallbackHandler)
  ...
  private object StreamingPictureCallbackHelper {
    fun setPictureCaptureCallback(renderer: Any, callback: Any?) {
      try {
        ...
        val setPictureCaptureCallback =
          rendererClass.getDeclaredMethod("setPictureCaptureCallback", HardwareRenderer.PictureCapturedCallback::class.java)
    ...
    fun createCallback(renderer: Any, callback: Callable<OutputStream?>, executor: Executor): Any {
    ...
    private class StreamingPictureCallbackHandler(
      ...
      override fun onPictureCaptured(picture: Picture) {
        ...
        try {
          Picture::class.java.getDeclaredMethod("writeToStream", OutputStream::class.java).invoke(picture, mByteStream)
        } catch (e: Exception) {
          // shouldn't happen
        }
      ...
      override fun run() {
        ...
        if (Thread.currentThread() === mRenderThread) {
          close()
          throw IllegalStateException("ViewDebug#startRenderingCommandsCapture must be given an executor that " + "invokes asynchronously")
        }
        if (isStopped) {
          return
        }
