...
/**
 * Uses reflection to get the app's xr session, then uses the session to get all the existing
 * panels in the xr app and extracts the root view from them.
 *
 * This method exist to have backwards compatibility up to alpha03 of the jetpack xr libraries.
 * TODO(b/411291368): remove this code the transition phase from alpha03 is complete.
 */
internal fun getXrViewsUsingReflection(environment: InspectorEnvironment): List<View> {
    try {
        val xrSessions = getXrSessions(environment)
        return xrSessions
...
private fun getXrSessions(environment: InspectorEnvironment): List<Any> {
