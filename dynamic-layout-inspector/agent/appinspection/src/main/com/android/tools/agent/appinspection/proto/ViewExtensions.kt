...
private fun View.toNodeImpl(stringTable: StringTable, absOffset: Point): ViewNode.Builder {
  ...
  return ViewNode.newBuilder().apply {
    ...
    (view.layoutParams as? WindowManager.LayoutParams)?.let { params -> layoutFlags = params.flags }
    if (view is WebView) {
      flags = ViewNode.Flag.IS_WEBVIEW.number
    }

    view.getTextValue()?.let { text -> textValue = stringTable.put(text) }
    if (view is ViewGroup) {
      view.getChildren().forEach { child -> addChildren(child.toNodeImpl(stringTable, Point(absPos.x - scrollX, absPos.y - scrollY))) }
    }
  }
}

/** Search this view for a resource with matching [resourceId] and, if found, return its proto representation. */
...
fun View.getNamespace(attributeId: Int): String = if (attributeId != 0) resources.getResourcePackageName(attributeId) else ""
...
fun View.createAppContext(stringTable: StringTable): AppContext {
  ...
  return AppContext.newBuilder()
    .apply {
      createResource(stringTable, context.themeResId)?.let { themeResource -> theme = themeResource }
      addAllDisplayInfo(displayInfo)
    }
    .build()
}

fun View.createConfiguration(stringTable: StringTable) = context.resources.configuration.convert(stringTable)
...
private fun View.createPropertyGroupImpl(stringTable: StringTable): PropertyGroup {
  ...
  val viewReader = SimplePropertyReader(stringTable, this, viewProperties, SimplePropertyReader.PropertyCategory.VIEW)
  viewCache.readProperties(this, viewReader)
  val layoutReader = SimplePropertyReader(stringTable, this, layoutProperties, SimplePropertyReader.PropertyCategory.LAYOUT_PARAMS)
  layoutCache.readProperties(layoutParams, layoutReader)

  val view = this
  return PropertyGroup.newBuilder()
    .apply {
      this.viewId = view.uniqueDrawingId

      view.createResource(stringTable, sourceLayoutResId)?.let { layoutResource -> layout = layoutResource }
