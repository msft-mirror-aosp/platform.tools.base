...
@VisibleForTesting const val DOUBLE_TAP_TIMEOUT_MS = 300L

/**
 * View responsible for drawing Layout Inspector overlay on-top the app's ui. Each root view in the app contains a OverlayView that does the
 * rendering. Each [OverlayView] is controlled by the [OnDeviceRenderingViewModel].
 */
class OverlayView(
  ...
  private val selectedRectPaint = Paint().apply { style = Paint.Style.STROKE }
  private val hoveredRectPaint = Paint().apply { style = Paint.Style.STROKE }
  private val visibleRectPaint = Paint().apply { style = Paint.Style.STROKE }
  private val recomposingRectPaint = Paint().apply { style = Paint.Style.FILL }
  val textPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { color = Color.WHITE }
  ...
  /**
   * Temporary [Rect] used to convert a [DrawInstruction.rect] from screen to view coordinates, without allocating a new [Rect] instance
   * each time.
   */
  ...
  /** Rendering instruction for the visible rectangles, which include selected and hovered rectangles. */
  ...
  override fun onAttachedToWindow() {
    ...
    viewScope =
      CoroutineScope(scope.coroutineContext + SupervisorJob()).apply {
        ...
        launch { viewModel.interceptTouchEvents.collect { intercept -> interceptTouchEvents = intercept } }
  ...
  private fun DrawInstruction.paint(canvas: Canvas, paint: Paint) {
    ...
    if (label != null) {
      drawLabel(label = label, nodeBounds = bounds, backgroundPaint = paint, textPaint = textPaint, canvas = canvas)
    }
  }
  ...
  private fun List<OverlayViewInstruction>.mapToDrawInstructions(ownerRootId: Long): List<DrawInstruction> {
    return filter { it.rootId == ownerRootId }
      .map { DrawInstruction(rect = it.bounds, color = it.color, strokeThickness = it.strokeThickness, label = it.label) }
  }
}
