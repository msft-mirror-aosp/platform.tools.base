 ...
 */
data class OverlayViewInstruction(val rootId: Long, val bounds: Rect, val color: Int, val strokeThickness: Float, val label: Label?)
...
class OnDeviceRenderingViewModel(
  ...
  private suspend fun addOverlayView(inspectorView: InspectorView) =
    withContext(mainDispatcher) {
      ...
      if (view is ViewGroup) {
        if (view.getChildren().filterIsInstance<OverlayView>().isNotEmpty()) {
          ...
          try {
            val overlayView = OverlayView(root = view, scope = scope, viewModel = this@OnDeviceRenderingViewModel)
            view.addView(overlayView, ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT))
  ...
  private suspend fun removeOverlayView(inspectorView: InspectorView) =
    withContext(mainDispatcher) {
      val view = inspectorView.view
      if (view is ViewGroup) {
  ...
  /**
   * A root view can be missing its OverlayView if the root view children were all removed, but the root view itself was not. This can
   * happen in some cases, for example if the Activity is relaunched, see b/402729631.
   *
   * This is a utility method to make sure the OverlayView is actually present, each time LayoutInspector tries to interact with it.
   */
...
/** Sizes used for studio-side rendering look small in on-device rendering. This scale factor is used to adjust them. */
