...
class CaptureExecutor(
  ...
  private fun executeCapture(doCapture: Runnable, captureOutputStream: ByteArrayOutputStream, rootView: View, isXr: Boolean) {
    ...
    synchronized(state.lock) {
      ...
      if (snapshotRequest?.state?.compareAndSet(SnapshotRequest.State.NEW, SnapshotRequest.State.PROCESSING) != true) {
        snapshotRequest = null
      }
      screenshotSettings =
        when (snapshotRequest?.screenshotType) {
          LayoutInspectorViewProtocol.Screenshot.Type.SKP -> ScreenshotSettings(LayoutInspectorViewProtocol.Screenshot.Type.SKP)
  ...
  private fun sendLayoutEvent(
    ...
    if (snapshotResponse != null) {
      ...
      connection.sendEvent { layoutEvent = layout }
    }
  }
  ...
  private fun sendAllPropertiesEvent(
    ...
    val allViews = ThreadUtils.runOnMainThread { root.flatten().toList() }.get(10, TimeUnit.SECONDS) ?: throw TimeoutException()
    val stringTable = StringTable()
    val propertyGroups = allViews.map { it.createPropertyGroup(stringTable) }
    val properties =
      LayoutInspectorViewProtocol.PropertiesEvent.newBuilder()
        .apply {
    ...
    if (snapshotResponse != null) {
      ...
      connection.sendEvent { propertiesEvent = properties }
    }
  }
