...
/** A [PropertyReader] implementation which modifies a list of passed in [PropertyBuilder]s as it reads in properties. */
  ...
  private fun readAny(id: Int, value: Any?) {
    ...
    if (category == PropertyCategory.VIEW) {
      ...
      for (resourceId in view.getAttributeResolutionStack(metadata.attributeId)) {
        view.createResource(stringTable, resourceId)?.let { resource -> property.addResolutionResource(resource) }
      }
    }
  }

  private fun getResourceValueOfAttribute(attributeId: Int): Resource? {
    return resourceMap[attributeId]?.let { resourceId -> view.createResource(stringTable, resourceId) }
  }
}
