...
class ViewLayoutInspectorTestWithWorkaround : ViewLayoutInspectorTestBase() {
  @get:Rule val apiLevelRule = PropertySetterRule(31, Build.VERSION::SDK_INT)
}

class ViewLayoutInspectorTest : ViewLayoutInspectorTestBase() {
  @get:Rule val apiLevelRule = PropertySetterRule(33, Build.VERSION::SDK_INT)
}
...
abstract class ViewLayoutInspectorTestBase {
  ...
  @get:Rule val inspectorRule = InspectorRule()

  @get:Rule val frameworkRule = FrameworkStateRule()

  private val threadExceptions = mutableListOf<Throwable>()

  @Before
  fun trackThread() {
    ThreadUtils.registerThread = { it.uncaughtExceptionHandler = Thread.UncaughtExceptionHandler { _, ex -> threadExceptions.add(ex) } }
  }
  ...
  fun canStartAndStopInspector() = createViewInspector { viewInspector ->
    ...
    inspectorRule.commandCallback.replyListeners.add { bytes -> responseQueue.add(bytes) }

    val packageName = "view.inspector.test"
    val root =
      ViewGroup(Context(packageName, createResources(packageName))).apply {
        width = 100
        height = 200
  ...
  fun startOnDeviceRendering() = createViewInspector { viewInspector ->
    ...
    val enableOnDeviceRendering =
      Command.newBuilder()
        .apply {
          enableOnDeviceRenderingCommand =
            LayoutInspectorViewProtocol.EnableOnDeviceRenderingCommand.newBuilder().apply { enable = true }.build()
        }
        .build()
    viewInspector.onReceiveCommand(enableOnDeviceRendering.toByteArray(), inspectorRule.commandCallback)
    responseQueue.take()
  ...
  fun canStartWithNoWindowInitially() = createViewInspector { viewInspector ->
    ...
    val packageName = "view.inspector.test"
    val root =
      object : View(Context(packageName, createResources(packageName))) {
          override fun isAttachedToWindow(): Boolean {
            // For the first few tries say this window isn't attached and thus not a valid root
            return checkCount++ > 5
          }
        }
        .apply { setAttachInfo(View.AttachInfo()) }
    WindowManagerGlobal.getInstance().rootViews.addAll(listOf(root))
    val startFetchCommand = Command.newBuilder().apply { startFetchCommandBuilder.apply { continuous = false } }.build()
    viewInspector.onReceiveCommand(startFetchCommand.toByteArray(), inspectorRule.commandCallback)
    responseQueue.take().let { bytes ->
      val response = Response.parseFrom(bytes)
      assertThat(response.specializedCase).isEqualTo(Response.SpecializedCase.START_FETCH_RESPONSE)
  ...
  fun restartingInspectorResendsRootEvents() = createViewInspector { viewInspector ->
    ...
    run {
      ...
      checkNonProgressEvent(eventQueue) { event -> assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.LAYOUT_EVENT) }
      checkNonProgressEvent(eventQueue) { event -> assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.PROPERTIES_EVENT) }
    }
  ...
  fun canCaptureTreeInContinuousMode() = createViewInspector { viewInspector ->
    ...
    checkNonProgressEvent(eventQueue) { event ->
      assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.ROOTS_EVENT)
      assertThat(event.rootsEvent.idsList).containsExactly(tree1.uniqueDrawingId, tree2.uniqueDrawingId)
    }
    ...
    checkNonProgressEvent(eventQueue) { event ->
      assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.ROOTS_EVENT)
      assertThat(event.rootsEvent.idsList).containsExactly(tree1.uniqueDrawingId, tree2.uniqueDrawingId, tree3.uniqueDrawingId)
    }
    ...
    checkNonProgressEvent(eventQueue) { event ->
      assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.ROOTS_EVENT)
      assertThat(event.rootsEvent.idsList).containsExactly(tree1.uniqueDrawingId, tree3.uniqueDrawingId)
    }
  ...
  fun noEmptyRootsEventOnStopContinuousCapturing() = createViewInspector { viewInspector ->
    ...
    viewInspector.onReceiveCommand(
      Command.newBuilder().apply { startFetchCommandBuilder.apply { continuous = true } }.build().toByteArray(),
      inspectorRule.commandCallback,
    )
    // Don't need to get into the screenshot processing stuff
    viewInspector.onReceiveCommand(
      Command.newBuilder().apply { updateScreenshotTypeCommandBuilder.apply { type = Screenshot.Type.NONE } }.build().toByteArray(),
      inspectorRule.commandCallback,
    )
    ...
    checkNonProgressEvent(eventQueue) { event ->
      assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.ROOTS_EVENT)
      assertThat(event.rootsEvent.idsList).containsExactly(tree1.uniqueDrawingId)
    }
    ...
    // roots.
    viewInspector.onReceiveCommand(
      Command.newBuilder().apply { startFetchCommandBuilder.apply { continuous = false } }.build().toByteArray(),
      inspectorRule.commandCallback,
    )
  ...
  fun textPropertyCaptured() = createViewInspector { viewInspector ->
    ...
    val startFetchCommand = Command.newBuilder().apply { startFetchCommandBuilder.apply { continuous = true } }.build()
    val enableBitmapScreenshotCommand = Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = true } }.build()
    viewInspector.onReceiveCommand(startFetchCommand.toByteArray(), inspectorRule.commandCallback)
    viewInspector.onReceiveCommand(enableBitmapScreenshotCommand.toByteArray(), inspectorRule.commandCallback)
  ...
  fun correctBitmapTypesCaptured() = createViewInspector { viewInspector ->
    ...
    val check = { event: Event ->
      ...
      event.layoutEvent.screenshot.let { screenshot ->
        ...
        assertThat(decompressedBytes.size).isEqualTo(BITMAP_HEADER_SIZE + mainScreen.width * mainScreen.height)
        // Check the bitmap header
        assertThat(decompressedBytes.take(BITMAP_HEADER_SIZE))
          .isEqualTo(((mainScreen.width).toBytes() + (mainScreen.height).toBytes() + BitmapType.RGB_565.byteVal).asList())
        ...
        assertThat(decompressedBytes.drop(BITMAP_HEADER_SIZE).take(fakeBitmapHeader.size)).isEqualTo(fakeBitmapHeader.asList())
      }
    }
    checkNonProgressEvent(eventQueue, check)
    ...
    checkNonProgressEvent(eventQueue) { event ->
      ...
      event.layoutEvent.screenshot.let { screenshot ->
        ...
        // The full screenshot byte array is width * height
        assertThat(decompressedBytes.size).isEqualTo(BITMAP_HEADER_SIZE + floatingDialog.width * floatingDialog.height)
        // Check the bitmap header
        assertThat(decompressedBytes.take(BITMAP_HEADER_SIZE))
          .isEqualTo(((floatingDialog.width).toBytes() + (floatingDialog.height).toBytes() + BitmapType.ABGR_8888.byteVal).asList())
        // Check the first few bytes to make sure they match our header,
        // that's enough to know that all the data went through correctly.
        assertThat(decompressedBytes.drop(BITMAP_HEADER_SIZE).take(floatingFakeBitmapHeader.size))
          .isEqualTo(floatingFakeBitmapHeader.asList())
      }
    }
  }
  ...
  fun testNullViewRootImpl() = createViewInspector { viewInspector ->
    ...
    val bitmapCapturingCommand = Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = true } }.build()
    viewInspector.onReceiveCommand(bitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
  ...
  fun bitmapNotCapturedWhenDisabled() = createViewInspector { viewInspector ->
    ...
    val disableBitmapCapturingCommand = Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = false } }.build()
    viewInspector.onReceiveCommand(disableBitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
    ...
    val enableBitmapCapturingCommand = Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = true } }.build()
    viewInspector.onReceiveCommand(enableBitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
  ...
  fun disablingBitmapCapturingDoesNotAffectSkp() = createViewInspector { viewInspector ->
    ...
    run {
      ...
      val disableBitmapCapturingCommand =
        Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = false } }.build()
      viewInspector.onReceiveCommand(disableBitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
      responseQueue.take()
    ...
    run { // Search for properties for View
      ...
      val getPropertiesCommand =
        Command.newBuilder()
          ...
          .build()
      viewInspector.onReceiveCommand(getPropertiesCommand.toByteArray(), inspectorRule.commandCallback)
      ...
      responseQueue.take().let { bytes ->
        ...
        response.getPropertiesResponse.let { propertiesResponse ->
          ...
          assertThat(propertyGroup.propertyList.map { strings[it.name] }).containsExactly("visibility", "layout_width", "layout_height")
        }
      }
    }
    ...
    run { // Search for properties for TextView
      ...
      responseQueue.take().let { bytes ->
        ...
        response.getPropertiesResponse.let { propertiesResponse ->
          ...
          assertThat(propertyGroup.propertyList.map { strings[it.name] })
            .containsExactly("text", "visibility", "layout_width", "layout_height")
        }
      }
    }
    ...
    run { // Search for properties for AppCompatButton
      ...
      responseQueue.take().let { bytes ->
        ...
        response.getPropertiesResponse.let { propertiesResponse ->
          val strings = StringTable.fromStringEntries(propertiesResponse.stringsList)
          val propertyGroup = propertiesResponse.propertyGroup
          val props = propertyGroup.propertyList.map { "${strings[it.namespace]}.${strings[it.name]}" }
  ...
  fun flagSetForWebView() = createViewInspector { viewInspector ->
    ...
    checkNonProgressEvent(eventQueue) { event ->
      ...
      assertThat(flagsPerNode)
        .containsExactly(ViewNode.Flag.NONE_VALUE, ViewNode.Flag.IS_WEBVIEW_VALUE, ViewNode.Flag.IS_WEBVIEW_VALUE, ViewNode.Flag.NONE_VALUE)
        .inOrder()
    }
  }
  ...
  fun correctScreenshotTypeForConnectType() = createViewInspector { viewInspector ->
    ...
    run {
      ...
      val disableBitmapCapturingCommand =
        Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = true } }.build()
      viewInspector.onReceiveCommand(disableBitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
      responseQueue.take()
  ...
  fun settingScreenshotTypeAffectsCaptureOutput() = createViewInspector { viewInspector ->
    ...
    // enable capturing bitmap screenshots
    val disableBitmapCapturingCommand = Command.newBuilder().apply { enableBitmapScreenshotCommandBuilder.apply { enable = true } }.build()
    viewInspector.onReceiveCommand(disableBitmapCapturingCommand.toByteArray(), inspectorRule.commandCallback)
    responseQueue.take()
    ...
    run { // Start first by setting type to BITMAP
      ...
      val check = { event: Event ->
        ...
        event.layoutEvent.screenshot.let { screenshot ->
          ...
          // The full screenshot byte array is width * height
          assertThat(decompressedBytes.size).isEqualTo((BITMAP_HEADER_SIZE + root.width * scale * root.height * scale).toInt())
          // Check the bitmap header
          assertThat(decompressedBytes.take(BITMAP_HEADER_SIZE))
            .isEqualTo(
              ((root.width * scale).toInt().toBytes() + (root.height * scale).toInt().toBytes() + BitmapType.RGB_565.byteVal).asList()
            )
          // Check the first few bytes to make sure they match our header,
          // that's enough to know that all the data went through correctly.
          assertThat(decompressedBytes.drop(BITMAP_HEADER_SIZE).take(fakeBitmapHeader.size)).isEqualTo(fakeBitmapHeader.asList())
      ...
      val dontUpdateScreenshotCommand =
        Command.newBuilder().apply { updateScreenshotTypeCommandBuilder.apply { this.scale = scale2.toFloat() } }.build()
      viewInspector.onReceiveCommand(dontUpdateScreenshotCommand.toByteArray(), inspectorRule.commandCallback)
      ThreadUtils.runOnMainThread {}.get() // Wait for notifications to be reset
      ...
      val check2 = { event: Event ->
        event.layoutEvent.screenshot.let { screenshot ->
          ...
          assertThat(decompressedBytes.size).isEqualTo((BITMAP_HEADER_SIZE + root.width * scale2 * root.height * scale2).toInt())
        }
      }
      ...
      val dontUpdateScaleCommand =
        Command.newBuilder().apply { updateScreenshotTypeCommandBuilder.apply { type = Screenshot.Type.SKP } }.build()
      viewInspector.onReceiveCommand(dontUpdateScaleCommand.toByteArray(), inspectorRule.commandCallback)
    ...
    run { // Now set type back to SKP
      ...
      responseQueue.take()
      root.forcePictureCapture(fakePicture2)
      checkNonProgressEvent(eventQueue) { event -> assertThat(event.layoutEvent.screenshot.type).isEqualTo(Screenshot.Type.SKP) }

      // Send another event without the scale specified and verify it isn't changed
      val dontUpdateScaleCommand =
        Command.newBuilder().apply { updateScreenshotTypeCommandBuilder.apply { type = Screenshot.Type.BITMAP } }.build()
      viewInspector.onReceiveCommand(dontUpdateScaleCommand.toByteArray(), inspectorRule.commandCallback)
      ThreadUtils.runOnMainThread {}.get() // Wait for notifications to be reset
      ...
      checkNonProgressEvent(eventQueue) { event ->
        event.layoutEvent.screenshot.let { screenshot ->
          ...
          assertThat(decompressedBytes.take(BITMAP_HEADER_SIZE))
            .isEqualTo(
              ((root.width * scale2).toInt().toBytes() + (root.height * scale2).toInt().toBytes() + BitmapType.RGB_565.byteVal).asList()
            )
        }
      }
    }
  ...
  fun settingScreenshotTypeToExistingDoesntTriggerInvalidate() = createViewInspector { viewInspector ->
    ...
    ThreadUtils.runOnMainThread { assertThat(root.invalidateCount).isEqualTo(initialInvalidateCount + 1) }.get(10, TimeUnit.SECONDS)
      ?: throw TimeoutException()

    // Make another change and verify that in invalidation did happen
    val updateScreenshotTypeCommand2 =
      Command.newBuilder().apply { updateScreenshotTypeCommandBuilder.apply { type = Screenshot.Type.SKP } }.build()
    viewInspector.onReceiveCommand(updateScreenshotTypeCommand2.toByteArray(), inspectorRule.commandCallback)
    responseQueue.poll(10, TimeUnit.SECONDS) ?: throw TimeoutException()

    ThreadUtils.runOnMainThread { assertThat(root.invalidateCount).isEqualTo(initialInvalidateCount + 2) }.get(10, TimeUnit.SECONDS)
      ?: throw TimeoutException()
  }
  ...
  fun noHardwareAccelerationDuringStartReturnsError() = createViewInspector { viewInspector ->
    ...
    responseQueue.take().let { bytes ->
      val response = Response.parseFrom(bytes)
      assertThat(response.specializedCase).isEqualTo(Response.SpecializedCase.START_FETCH_RESPONSE)
      assertThat(response.startFetchResponse.error).isEqualTo("Activity must be hardware accelerated for live inspection")
      assertThat(response.startFetchResponse.code).isEqualTo(ErrorCode.NO_HARDWARE_ACCELERATION)
    }
  }
  ...
  fun noRootViewsSendsEmptyLayoutEvent() = createViewInspector { viewInspector ->
    ...
    responseQueue.take().let { bytes ->
      val response = Response.parseFrom(bytes)
      assertThat(response.specializedCase).isEqualTo(Response.SpecializedCase.START_FETCH_RESPONSE)
      assertThat(response.startFetchResponse.error).isEqualTo("")
      assertThat(response.startFetchResponse.code).isEqualTo(ErrorCode.UNKNOWN_ERROR_CODE)
    }
    checkNonProgressEvent(eventQueue) { event ->
      assertThat(event.specializedCase).isEqualTo(Event.SpecializedCase.LAYOUT_EVENT)
  ...
  fun takeSnapshot() = createViewInspector { viewInspector ->
    ...
    checkNonProgressEvents(eventQueue, Event.SpecializedCase.ROOTS_EVENT, Event.SpecializedCase.LAYOUT_EVENT)

    val captureSnapshotCommand =
      Command.newBuilder().apply { captureSnapshotCommandBuilder.apply { screenshotType = Screenshot.Type.SKP } }.build()
    viewInspector.onReceiveCommand(captureSnapshotCommand.toByteArray(), inspectorRule.commandCallback)
    ...
    checkNonProgressEvents(eventQueue, Event.SpecializedCase.LAYOUT_EVENT)
  }
  ...
  private fun checkNonProgressEvent(eventQueue: BlockingQueue<ByteArray>, block: (Event) -> Unit) {
  ...
  private fun checkNonProgressEvents(eventQueue: BlockingQueue<ByteArray>, vararg expectedEventTypes: Event.SpecializedCase) {
  ...
  private fun createViewInspector(block: (ViewLayoutInspector) -> Unit) {
    ...
    val viewInspector = factory.createInspector(inspectorRule.connection, inspectorRule.environment)
    ...
    try {
      ...
      Looper.getLoopers().keys.filter { !initialThreads.contains(it) }.forEach { thread -> thread.join(TimeUnit.SECONDS.toMillis(1)) }
    }
  }
