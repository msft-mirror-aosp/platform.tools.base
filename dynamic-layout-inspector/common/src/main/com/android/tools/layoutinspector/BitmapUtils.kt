...
/** The bitmap header contains the width and height as ints, and the type as a byte. See [BitmapType]. */
...
enum class BitmapType(val byteVal: Byte, val pixelSize: Int) {
  RGB_565(1, 2) {
    override fun createImage(bytes: ByteBuffer, width: Int, height: Int): BufferedImage {
      ...
      val colorModel = DirectColorModel(16, 0x1f.shl(11), 0x3f.shl(5), 0x1f)
  ...
  protected fun createImage8888(width: Int, height: Int, bytes: ByteBuffer, rgbaBytes: IntArray): BufferedImage {
  ...
  companion object {
    fun fromByteVal(byte: Byte) = values().find { it.byteVal == byte } ?: throw UnknownBitmapTypeException(byte)
  }
}
