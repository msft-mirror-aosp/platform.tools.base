...
object LayoutInspectorUtils {
  private val versionMapUnmarshaller = JAXBContext.newInstance(VersionMap::class.java).createUnmarshaller()
...
/** Convert `this` to bytes using little-endian order. Optionally will populate the given [bytes] at the given [offset]. */
...
/** A view as seen in a Skia image. We can only have children or an image, not both. */
...
class VersionMap {
  @XmlElement(name = "server") val servers: MutableList<ServerVersionSpec> = mutableListOf()
}
...
class ServerVersionSpec {
  ...
  @XmlAttribute(name = "version", required = true) val version: Int = 0

  /** The first `SkPicture` version supported by this server. */
  @XmlAttribute(name = "skpStart", required = true) val skpStart: Int = 0

  /** The last `SkPicture` version supported by this server. */
  @XmlAttribute(name = "skpEnd", required = false) val skpEnd: Int? = null
}
