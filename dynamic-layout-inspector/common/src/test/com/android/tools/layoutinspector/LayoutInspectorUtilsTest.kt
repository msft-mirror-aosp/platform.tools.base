...
class LayoutInspectorUtilsTest {
  ...
  fun testBuildTree() {
    ...
    val root =
      SkiaParser.InspectorView.newBuilder()
        .apply {
          id = 1
          width = 100
          height = 200
    ...
    val result =
      LayoutInspectorUtils.buildTree(
        root,
        mapOf(1 to node1Bytes, 2 to node2Bytes),
        { false },
        mapOf(
          1L to
    ...
    val expected =
      SkiaViewNode(
        ...
        listOf(
          ...
          SkiaViewNode(4, listOf(SkiaViewNode(3, listOf(SkiaViewNode(3, node3Image))))),
        ),
      )
  ...
  fun testGetSkpVersion() {
    val version = getSkpVersion("skiapict".toByteArray().plus(byteArrayOf(10, 0, 1, 0)).plus("blah".toByteArray()))
  ...
  private fun assertTreesEqual(actual: SkiaViewNode, expected: SkiaViewNode) {
    ...
    if (expected.image != null) {
      assertImageSimilar("image", expected.image as BufferedImage, actual.image as BufferedImage, 0.0)
    } else {
      assertThat(actual.image).isNull()
    }
    assertThat(actual.children.size).isEqualTo(expected.children.size)
    actual.children.zip(expected.children).forEach { (actual, expected) -> assertTreesEqual(actual, expected) }
  }
  ...
  private fun createImage(desiredColor: Color, width: Int, height: Int): Pair<BufferedImage, ByteString> {
    ...
    val model = PixelInterleavedSampleModel(DataBuffer.TYPE_BYTE, width, height, 4, width * 4, intArrayOf(2, 1, 0, 3))
    val raster = Raster.createWritableRaster(model, buffer, Point(0, 0))
    val colorModel =
      ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_sRGB), true, false, Transparency.TRANSLUCENT, DataBuffer.TYPE_BYTE)
    @Suppress("UndesirableClassUsage") val image = BufferedImage(colorModel, raster, false, null)
