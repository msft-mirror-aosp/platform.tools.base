...
Status TransportServiceImpl::ValidateAndGetCanonicalPath(
  ...
  *canonical_path = std::filesystem::canonical(file_path);
  if (!std::filesystem::exists(*canonical_path) ||
    ...
    Log::E(Log::Tag::TRANSPORT, "File not found: %s", canonical_path->c_str());
...
Status TransportServiceImpl::GetBytesInChunks(
  ...
  try {
    Log::I(Log::Tag::TRANSPORT, "Streaming file %s started, size %lu bytes",
           file_path.c_str(), std::filesystem::file_size(file_path));
