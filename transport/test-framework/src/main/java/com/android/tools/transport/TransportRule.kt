...
/** A JUnit rule which handles management of test ports, processes, and basic rpc calls that are shared between all transport tests. */
class TransportRule
@JvmOverloads
constructor(private val activityClass: String, val sdkLevel: SdkLevel, private val ruleConfig: Config = Config()) : ExternalResource() {
  ...
  /** A class which provides access to gRPC, allowing tests to send and receive protobuf messages between host and device device. */
  lateinit var grpc: Grpc
    private set

  /** The class which talks to the fake Android device and allows us to issue commands directly to it. */
  lateinit var androidDriver: FakeAndroidDriver
    private set

  val commonConfig: CommonConfig by lazy {
    CommonConfig.newBuilder().setServiceAddress("$LOCAL_HOST:$serverPort").apply { ruleConfig.initDaemonConfig(this) }.build()
  }

  val layoutInspectorConfig: LayoutInspectorConfig by lazy {
    LayoutInspectorConfig.newBuilder().apply { ruleConfig.initLayoutInspectorConfig(this) }.build()
  }
  ...
  init {
    DeviceProperties("", sdkLevel.toString(), sdkLevel.toString()).apply { writeFile() }
  }
   ...
   *
   * This function will wait until the framework has been loaded, at which point it will load the proper dex(es) onto the device and launch
  ...
  private fun start(activity: String) {
    ...
    System.getProperty(dexesProperty).split(':').forEach { path -> androidDriver.loadDex(ProcessRunner.getProcessPathRoot() + path) }
  ...
  /** Helper function to create and serialize AgentConfig for test to use, this is specific to each test. */
  private fun buildDaemonConfig(): File {
    val file = temporaryFolder.newFile()
    val outputStream = FileOutputStream(file)
    val config = DaemonConfig.newBuilder().setCommon(commonConfig).setLayoutInspectorConfig(layoutInspectorConfig).build()
   ...
   *
   * If the process does not bind to the given port, it is restarted on a new port. Because JVMTI will reuse the port from the agent config,
   * we cannot set zero in the config.
   */
  private fun startDaemon() {
    while (!::transportDaemon.isInitialized || transportDaemon.port == 0) {
      ...
      transportDaemon = TransportDaemonRunner(daemonConfig.absolutePath, "--file_system_root=${temporaryFolder.newFolder("daemon-root")}")
  ...
  private fun buildAgentConfig(): File {
    ...
    FileOutputStream(file).use { outputStream ->
      val agentConfig: AgentConfig = AgentConfig.newBuilder().apply { ruleConfig.initAgentConfig(this) }.setCommon(commonConfig).build()
