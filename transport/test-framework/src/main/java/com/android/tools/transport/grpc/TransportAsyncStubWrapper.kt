 ...
 *
 * This wrapper is useful for testing against transport APIs that stream, where you have to wait for multiple events to arrive. You may also
 * want to see [TransportStubWrapper] if you are testing blocking APIs.
 */
class TransportAsyncStubWrapper(val transportStub: TransportServiceStub, val timeoutTimeSeconds: Long = 30) {
  ...
  fun getEvents(
    ...
    onStarted: Runnable,
  ): Map<Long, MutableList<Common.Event>> {
    val events: MutableMap<Long, MutableList<Common.Event>> = LinkedHashMap()
    val stopLatch = CountDownLatch(1)
    val observer: StreamObserver<Common.Event> =
  ...
  /**
   * Like the main [getEvents] method but also uses a condition to stop after an expected number of matching events are received.
   *
   ...
   * See also: [Grpc.createTransportAsyncStub]
   */
  fun getEvents(expectedEventCount: Int, accept: Predicate<Common.Event>, onStarted: Runnable): Map<Long, MutableList<Common.Event>> {
