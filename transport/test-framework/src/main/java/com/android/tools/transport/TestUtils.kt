...
object TestUtils {
  ...
  /** Some Kotlin-specific utilities that would otherwise clash with the legacy Java APIs of the same name. */
  object Kt {
    /**
     * Run the loop and wait until condition is true or the retry limit is reached. Returns the result afterwards.
     *
  ...
  fun <T> waitForAndReturn(supplier: Supplier<T>, condition: Predicate<T>, retryLimit: Int = NO_LIMIT): T {
    return Kt.waitForAndReturn({ supplier.get() }, { value: T -> condition.test(value) }, retryLimit)
  }
