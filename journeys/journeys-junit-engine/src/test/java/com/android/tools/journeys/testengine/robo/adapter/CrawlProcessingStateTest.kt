...
class CrawlProcessingStateTest {
  ...
  fun testAddScreenshot_savesScreenshotToFile() {
    ...
    val displayState = DisplayState.newBuilder().setDisplayStateId(1).setScreenshot(ByteString.copyFrom(byteArrayOf(1, 2, 3))).build()
  ...
  fun testAddInteractionToCurrentTurn_addsInteractionToTurn() {
    ...
    val action =
      Action.newBuilder()
        ...
        .setDetails(ActionDetails.newBuilder().setLaunchAction(LaunchAction.newBuilder().setAppPackageName("com.example.app")))
        .build()
    ...
    val testCases =
      mapOf(
        Action.newBuilder()
          .setDisplayStateId(1)
          .setDetails(
            ActionDetails.newBuilder().setWaitAction(WaitAction.newBuilder().setWaitDuration(Duration.newBuilder().setSeconds(2)))
          )
          .build() to "Waited for 2 seconds",
        Action.newBuilder()
          .setDisplayStateId(1)
          .setDetails(ActionDetails.newBuilder().setBackAction(BackAction.getDefaultInstance()))
          .build() to "Pressed back",
        Action.newBuilder()
          .setDisplayStateId(1)
          .setDetails(ActionDetails.newBuilder().setHomeAction(HomeAction.getDefaultInstance()))
          .build() to "Pressed home",
        Action.newBuilder()
          .setDisplayStateId(1)
          .setDetails(
            ActionDetails.newBuilder()
              .setPointTapAction(PointTapAction.newBuilder().setCoordinate(Point.newBuilder().setXCoordinate(100).setYCoordinate(200)))
          )
          .build() to "Point tap at (100, 200)",
        Action.newBuilder()
          .setDisplayStateId(1)
          .setDetails(ActionDetails.newBuilder().setAdbCommandAction(AdbCommandAction.newBuilder().setCommand("test command")))
            ...
            ActionDetails.newBuilder()
              .setTwoPointGestureAction(
                TwoPointGestureAction.newBuilder()
                  .setFirstPointStart(Point.newBuilder().setXCoordinate(1).setYCoordinate(2))
                  .setFirstPointEnd(Point.newBuilder().setXCoordinate(3).setYCoordinate(4))
                  .setSecondPointStart(Point.newBuilder().setXCoordinate(5).setYCoordinate(6))
                  .setSecondPointEnd(Point.newBuilder().setXCoordinate(7).setYCoordinate(8))
              )
          )
  ...
  fun testMakeActionString_forClickTargetAction_returnsCorrectString() {
    ...
    val screenElement =
      ScreenElement.newBuilder()
        ...
        .build()
    val screenState = ScreenState.newBuilder().setScreenStateId(1).addRootElements(screenElement).build()
    state.addScreenState(screenState)

    val targetAction = TargetAction.newBuilder().setActionType(TargetActionType.CLICK).setScreenElementId("element-id").build()
    val action =
      Action.newBuilder()
        .setScreenStateId(1)
        .setDisplayStateId(1)
        .setDetails(ActionDetails.newBuilder().setTargetAction(targetAction))
        .build()
  ...
  fun testMakeActionString_forClickTargetAction_returnsCorrectString_whenScreenElementMissing() {
    ...
    val screenState = ScreenState.newBuilder().setScreenStateId(1).build()
    state.addScreenState(screenState)
  ...
  fun testMakeActionString_forEnterTextTargetAction_returnsCorrectString() {
    ...
    assertThat(interaction.command).isEqualTo("ENTER_TEXT \"Hello World\" in Some Text EditText (id: my_edit_text)")
  }
