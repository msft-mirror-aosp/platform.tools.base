...
class RoboResultAdapterTest {
  ...
  fun setUp() {
    ...
    mockConsumer = mock<JourneyRunEventConsumer> { on { onEvent(org.mockito.kotlin.any()) }.then { capturedEvents.add(it.getArgument(0)) } }
  }
  ...
  fun `reports correctly for proto with incorrect contextual roboscripts`() {
    ...
    val adapter = RoboResultAdapter(RoboResultAdapter.RoboResultAdapterConfig(mockConsumer, state, tempDir))
    ...
    val expectedEvents =
      listOf(
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1746708026).setNanos(39000000).build(), buildResult(Status.SUCCEEDED)),
        buildStepStartedEvent("Tap on the \'Compose\' topic", Timestamp.newBuilder().setSeconds(1746708026).setNanos(39000000).build()),
        buildTurnAddedEvent(
          "Tap on the \'Compose\' topic.",
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1746708030).setNanos(312000000).build(), buildResult(Status.SUCCEEDED)),
        buildStepStartedEvent("Save the first post", Timestamp.newBuilder().setSeconds(1746708030).setNanos(312000000).build()),
        buildTurnAddedEvent(
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1746708034).setNanos(536000000).build(), buildResult(Status.SUCCEEDED)),
        buildStepStartedEvent("Go to saved posts", Timestamp.newBuilder().setSeconds(1746708034).setNanos(536000000).build()),
        buildTurnAddedEvent(
    ...
    val expectedEvents =
      listOf(
        ...
        buildStepFinishedEvent(
          ...
          buildResult(Status.FAILED, "Could not successfully complete the action in max allowed attempts"),
        ),
      )
    verifyEvents(expectedEvents)
  }
  ...
  private fun loadCrawlFromResource(resourceName: String): Crawl {
    val roboResults = Resources.toString(Resources.getResource(resourceName), StandardCharsets.UTF_8)
  ...
  private fun verifyEvents(expectedEvents: List<JourneyRunEvent>) {
    assertEquals(expectedEvents.size, capturedEvents.size, "Number of events mismatch. The complete list of actual events: $capturedEvents")
    for (i in expectedEvents.indices) {
      assertEquals(expectedEvents[i], capturedEvents[i], "Event mismatch at index $i")
    }
  }
  ...
  private fun sendCrawlEvents(adapter: RoboResultAdapter, crawl: Crawl) {
    ...
    crawl.actionsList.forEachIndexed { index, action ->
      // Send display state.
      adapter.onCrawlReceived(Crawl.newBuilder().addDisplayStates(crawl.displayStatesList[index]).build())
      ...
      if (action.details?.detailsCase != ActionDetails.DetailsCase.TERMINATE_CRAWL_ACTION) {
        adapter.onCrawlReceived(
          Crawl.newBuilder().addActions(action.toBuilder().clearEndTime().clearResultDetails().clearExecutionResult()).build()
        )
      }
  ...
  private fun buildInteraction(type: CommandType, command: String, screenshotPath: String, result: Result): Interaction {
    return Interaction.newBuilder()
      .setType(type)
      .setCommand(command)
      .addArtifactsBefore(Artifact.newBuilder().setType(ArtifactType.SCREENSHOT).setUri(screenshotPath))
      .setResult(result)
      .build()
  }
  ...
  private fun buildTurnAddedEvent(
    ...
    return JourneyRunEvent.newBuilder().setJourneyRunId("journeyRunId").setTurnAdded(turnAdded).build()
  }
  ...
  private fun buildStepStartedEvent(promptText: String, startTimestamp: Timestamp): JourneyRunEvent {
    ...
    return JourneyRunEvent.newBuilder().setJourneyRunId("journeyRunId").setStepStarted(stepStarted).build()
  }

  private fun buildStepFinishedEvent(endTimestamp: Timestamp, result: Result): JourneyRunEvent {
    ...
    return JourneyRunEvent.newBuilder().setJourneyRunId("journeyRunId").setStepFinished(stepFinished).build()
  }
}
