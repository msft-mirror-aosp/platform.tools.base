...
class ProxyTest {
  ...
  fun setUp() {
    ...
    validJourneyPath.toFile().writeText("<journey name=\"test\"><actions><action>Some action</action></actions></journey>")
  ...
  fun testExecuteJourney_journeyReadFailed_missingFile() {
    ...
    assertThrows(JourneyExecutionException::class.java) { proxy.executeJourney("run-1", "device-123", journeyPath) { artifact -> } }
      .apply {
  ...
  fun testExecuteJourney_journeyReadFailed_invalidXml() {
    ...
    assertThrows(JourneyExecutionException::class.java) { proxy.executeJourney("run-1", "device-123", invalidJourneyPath) { artifact -> } }
      .apply {
        assertEquals(JourneyFailureReason.JOURNEY_READ_FAILED, reason)
        assertContains(
          message!!,
  ...
  fun testExecuteJourney_usesCorrectAdbCommands_api34() {
    ...
    `when`(mockAdb.runInstrumentation(anyString(), anyString(), anyString(), anyMap())).thenThrow(RuntimeException("Test failure"))
    ...
    verify(mockAdb)
      .install(eq("device-123"), argThat { contains("extracted_apk") && endsWith(".apk") }, installFlagsCaptor.capture(), anyLong())
    assertEquals(listOf("-r", "-d", "-g", "--bypass-low-target-sdk-block"), installFlagsCaptor.firstValue)

    verify(mockAdb).install(eq("device-123"), eq("app.apk"), installFlagsCaptor.capture(), anyLong())
  ...
  fun testExecuteJourney_adbInstallFailed() {
    `when`(mockAdb.getDeviceApiLevel(anyString())).thenReturn(30)
    `when`(mockAdb.install(anyString(), anyString(), anyList(), anyLong())).thenThrow(RuntimeException("Install failed"))

    assertThrows(JourneyExecutionException::class.java) { proxy.executeJourney("run-1", "device-123", validJourneyPath) { artifact -> } }
      .apply {
  ...
  fun testExecuteJourney_instrumentationFailed() {
    `when`(mockAdb.getDeviceApiLevel(anyString())).thenReturn(30)
    `when`(mockAdb.install(anyString(), anyString(), anyList(), anyLong())).thenAnswer {}
    `when`(mockAdb.runInstrumentation(anyString(), anyString(), anyString(), anyMap()))
  ...
  fun testExecuteJourney_verifyCorrectPortCaptured() {
    ...
    `when`(mockAdb.forward(anyString(), anyInt(), anyInt())).thenThrow(RuntimeException("Test failure"))
  ...
  fun testExecuteJourney_adbForwardingFailed() {
    ...
    `when`(mockAdb.runInstrumentation(anyString(), anyString(), anyString(), anyMap())).thenReturn(mockProcess)
    `when`(mockAdb.dumpsys(anyString(), anyString(), anyLong())).thenReturn("port_is_bound 12345")
    `when`(mockAdb.forward(anyString(), anyInt(), anyInt())).thenThrow(RuntimeException("Forwarding failed"))
  ...
  fun testExecuteJourney_authenticationFailed() {
    ...
    assertThrows(JourneyExecutionException::class.java) { proxyWithMissingToken.executeJourney("run-1", "device-123", validJourneyPath) {} }
      .apply {
  ...
  fun testExecuteJourney_cleanupCalledOnSuccess() {
    ...
    mockStatic(ImpersonatedCredentials::class.java).use { mockedImpersonatedCredentials ->
      ...
      mockStatic(NettyChannelBuilder::class.java).use { mockedNettyChannelBuilder ->
        ...
        mockedNettyChannelBuilder.`when`<Any> { NettyChannelBuilder.forTarget(anyString()) }.thenReturn(mockChannelBuilder)

        mockConstruction(GrpcClient::class.java) { mockGrpcClient, context ->
            val mockResult = mock(GrpcClient.Result::class.java)
            `when`(mockResult.outcome()).thenReturn(GrpcClient.Result.Outcome.SUCCESS)
            `when`(mockGrpcClient.startForward(any(), any(), any(), any(), any())).thenReturn(mockResult)
          }
          .use { proxyWithFakeToken.executeJourney("run-1", "device-123", validJourneyPath) {} }
      }
    }
