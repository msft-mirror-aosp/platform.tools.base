 ...
 */
class Adb(private val adbPath: String, private val executor: ProcessExecutor) {

  /** Secondary constructor that uses the [DefaultProcessExecutor]. */
  constructor(adbPath: String) : this(adbPath, DefaultProcessExecutor())
   ...
   */
  fun install(deviceId: String, apkFilePath: String, flags: List<String> = listOf(), timeoutSeconds: Long = 300) {
    execCmdSync(
      adbCmdParts =
        buildList {
          add(adbPath)
      ...
      hasFailed = { exitValue != 0 || !fullOut.contains("Success") },
    )
  }

  /**
   * Uninstalls an application from the device. Checks if the app is installed before attempting uninstallation.
   *
   * @param deviceId The ID of the target device.
  ...
  fun uninstall(deviceId: String, applicationId: String) {
    val isAppInstalled =
      execCmdSync(deviceId = deviceId, adbCmd = "shell pm path $applicationId", hasFailed = { false }).fullOut.contains("package:")
    if (isAppInstalled) {
  ...
  fun getDeviceApiLevel(deviceId: String): Int {
    val output =
      execCmdSync(deviceId = deviceId, adbCmd = "shell getprop ro.build.version.sdk", hasFailed = { stdout.trim().toIntOrNull() == null })
        .stdout
   ...
   *
   * Starts the `am instrument` command but returns the [Process] object immediately. The caller is responsible for managing this process,
   * including waiting for completion, handling streams (stdout/stderr), and destroying the process if necessary.
   *
   * @param deviceId The ID of the target device.
   ...
   */
  fun runInstrumentation(deviceId: String, testApplicationId: String, runner: String, args: Map<String, String> = mapOf()): Process {
  ...
  /**
   * Removes a specific port forwarding rule using `adb forward --remove`. Checks if the forwarding rule exists before attempting removal.
   *
   * @param deviceId The ID of the target device.
  ...
  fun dumpsys(deviceId: String, serviceIdentifier: String, timeoutSeconds: Long = 10): String {
    return execCmdSync(deviceId = deviceId, adbCmd = "shell dumpsys activity service $serviceIdentifier", timeoutSeconds = timeoutSeconds)
      .fullOut
  }
   ...
   * @param deviceId The ID of the target device.
   */
  fun setGlobalSettingsValue(deviceId: String, key: String, value: String, timeoutSeconds: Long = 10) {
    execCmdSync(deviceId = deviceId, adbCmd = "shell settings put global $key $value", timeoutSeconds = timeoutSeconds)
  }

  /**
   * Executes an ADB command synchronously, handling process execution, timeout, stream reading, and result validation.
   *
   * @param deviceId The ID of the target device.
   * @param adbCmd The specific ADB command.
   * @param timeoutSeconds The maximum time to wait for the process to finish.
   * @param hasFailed A lambda with [ProcessResult] as receiver to determine if the execution failed. Defaults to checking if the exit value
   *   is non-zero.
   * @return A [ProcessResult] containing exit code, stdout, and stderr.
   * @throws IllegalStateException if the command times out. or if the [hasFailed] predicate returns true.
   */
