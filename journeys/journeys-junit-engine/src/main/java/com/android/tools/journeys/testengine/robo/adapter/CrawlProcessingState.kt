...
/**
 * Holds the mutable state associated with processing Robo crawl results for a single journey file execution, to produce JourneyRunEvent.
 *
...
class CrawlProcessingState(private val journeyRunId: String, private val prompts: List<String>) {
  ...
  /**
   * Stores a map of screen state ID to screen state. This is useful for obtaining a human-readable name for screen elements reported in an
   * interaction.
   */
  ...
  /** Tracks the final journey result. Set to success by default and updated by the adapter if any prompt fails. */
  ...
  /**
   * Stores the screenshot data associated with a display state ID and saves the path to the local map.
   *
  ...
  fun addScreenshot(displayState: DisplayState, resultsDir: Path) {
    ...
    val targetFilePath = resultsDir.resolve("displayState$id.png")
    try {
  ...
  fun initNewTurn(displayStateId: Int, modelDetails: ModelDetails) {
    turnBuilder =
      Turn.newBuilder().apply {
        addArtifactsBefore(Artifact.newBuilder().setType(ArtifactType.SCREENSHOT).setUri(displayStateToScreenshotPath[displayStateId]))
        description = modelDetails.description
        reasoning =
          modelDetails.interactionsList?.getOrNull(0)?.response?.takeIf { it.isNotBlank() } ?: RoboConfigConstants.DEFAULT_MODEL_REASONING
      }
    ...
    turnBuilder.addInteractions(
      Interaction.newBuilder()
        .apply {
          ...
          addArtifactsBefore(
            Artifact.newBuilder().setType(ArtifactType.SCREENSHOT).setUri(displayStateToScreenshotPath[action.displayStateId])
          )
          setResult(
            when (action.executionResult) {
              Action.ExecutionResult.ACTION_SUCCESS -> Result.newBuilder().setStatus(Status.SUCCEEDED)

              else -> Result.newBuilder().setStatus(Status.FAILED).setErrorMessage("Action failed")
            }
          )
        }
        .build()
  ...
  private fun toCommandType(action: Action): CommandType {
    ...
    return when (details.detailsCase) {
      ...
      ActionDetails.DetailsCase.TARGET_ACTION ->
        when (action.details.targetAction.actionType) {
  ...
  private fun makeActionString(action: Action): String {
    ...
    return when (details.detailsCase) {
      ...
      ActionDetails.DetailsCase.TARGET_ACTION -> toString(details.targetAction, action.screenStateId)
  ...
  /** Returns the `ScreenElement` corresponding to the given `elementId` in the screen given by the `screenStateId` if it exists */
  private fun elementForId(screenState: ScreenState, elementId: String): ScreenElement? {
    var currentElement = screenState.rootElementsList.firstOrNull { elementId.startsWith(it.screenElementId) } ?: return null

    while (currentElement.screenElementId != elementId) {
      currentElement = currentElement.childElementsList.maxByOrNull { stringMatchLength(it.screenElementId, elementId) } ?: break
    }
    return currentElement
  }

  /** Returns the length of the longest common prefix of the given strings. */
  private fun stringMatchLength(s1: String, s2: String): Int = s1.zip(s2).takeWhile { (c1, c2) -> c1 == c2 }.count()
}
