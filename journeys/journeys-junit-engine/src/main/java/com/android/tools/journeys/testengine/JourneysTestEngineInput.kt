...
object JourneysTestEngineInput {
  ...
  val journeysFilter: List<String> = getStringListFromSystemPropertyOrEnvVar("journeysFilter", "JOURNEYS_FILTER")
  val resultsDir: File

  // Enabled by default. Can be disabled by setting to "false".
  val enableStdoutReport: Boolean = getSystemPropertyOrEnvVar("", "JOURNEYS_ENABLE_STDOUT_REPORT").ifBlank { "true" }.toBoolean()
  ...
  object ProxyInput {
    ...
    val appApkPath: File
    val adbPath: File
    val accessTokenPath: String = getSystemPropertyOrEnvVar("Proxy.accessTokenPath", "GEMINI_ACCESS_TOKEN_PATH")
  ...
  object RoboConverterInput {
    ...
    val enableExperimentalMode: Boolean = getSystemPropertyOrEnvVar("", "JOURNEYS_ENABLE_EXPERIMENTAL_MODE").toBoolean()
  }
}
...
private fun getStringListFromSystemPropertyOrEnvVar(propertyName: String, envVarName: String): List<String> {
