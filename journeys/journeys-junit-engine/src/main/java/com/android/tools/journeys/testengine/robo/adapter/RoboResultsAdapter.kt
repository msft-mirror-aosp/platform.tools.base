 ...
 */
class RoboResultAdapter(private val config: RoboResultAdapterConfig) : JourneysResultAdapter {
  ...
  /**
   * Processes a [Crawl] object. The raw artifact bytes are expected to be a serialized [Crawl] object.
   *
  ...
  /**
   * Main entry point to process a received [Crawl] object. Iterates through actions and associated RoboScript details, determines their
   * status, and triggers the appropriate event handlers.
   *
  ...
  fun onCrawlReceived(crawl: Crawl) {
    ...
    mapCrawlResultToException(crawl.crawlResult)?.let { throw it }
  }

  private fun mapCrawlResultToException(crawlResult: Crawl.CrawlResult): JourneyExecutionException? {
    return when (crawlResult) {
      Crawl.CrawlResult.TIMED_OUT ->
        JourneyExecutionException(
          message = "The crawler ran out of time before exploring all actions",
          reason = JourneyFailureReason.CRAWLER_TIMED_OUT,
        )

      Crawl.CrawlResult.APP_TERMINATED ->
        JourneyExecutionException(message = "The app terminated during the crawl", reason = JourneyFailureReason.APP_TERMINATED)

      Crawl.CrawlResult.CRAWLER_TERMINATED ->
        JourneyExecutionException(
      ...
      Crawl.CrawlResult.CANCELLED ->
        JourneyExecutionException(message = "Cancelled by user", reason = JourneyFailureReason.CRAWLER_TERMINATED)

      Crawl.CrawlResult.AGENT_TIMED_OUT ->
        JourneyExecutionException(message = "The AI agent directing this crawl timed out", reason = JourneyFailureReason.AGENT_TIMED_OUT)

      Crawl.CrawlResult.AGENT_RESOURCE_EXHAUSTED ->
        JourneyExecutionException(
  ...
  private fun preprocessCrawlResults(crawl: Crawl) {
    crawl.displayStatesList.forEach { displayState -> config.crawlProcessingState.addScreenshot(displayState, config.resultsDir) }
    crawl.screenStatesList.forEach { screenState -> config.crawlProcessingState.addScreenState(screenState) }
  }
  ...
  private fun processActions(crawl: Crawl) {
    for (action in crawl.actionsList) {
      if (
        action.details.detailsCase == ActionDetails.DetailsCase.LAUNCH_ACTION &&
          action.executionResult == Action.ExecutionResult.ACTION_FAILED
      ) {
        val errorMessage =
          if (action.resultDetails.hasLaunchResult()) {
            "Failed to launch app - ${action.resultDetails.launchResult.resultType}"
          } else {
            "Failed to launch app"
          }
        throw JourneyExecutionException(message = errorMessage, reason = JourneyFailureReason.LAUNCH_APP_FAILED)
      }
      ...
      action.roboScriptDetailsList.forEach { roboScript ->
        ...
        if (RoboConfigConstants.ALLOWED_BUILTIN_ROBOSCRIPT_IDS.contains(roboScript.activeRoboScriptId)) return@forEach

        handleRoboScriptEvents(roboScript, action, lastInActionListRoboScriptIndex)
      }
    }
  }
  ...
  private fun getLastRoboScriptIndexInAction(action: Action): Int {
    ...
    while (lastElementIndex > 0 && RoboConfigConstants.ALLOWED_BUILTIN_ROBOSCRIPT_IDS.contains(lastRoboScript?.activeRoboScriptId ?: -1)) {
      lastElementIndex--
   ...
   */
  private fun handleRoboScriptEvents(roboScript: RoboScriptDetails, action: Action, lastInActionListRoboScriptIndex: Int) {
    val roboIndex = roboScript.actionIndex
    val currentRoboScriptIndex = config.crawlProcessingState.currentRoboScriptIndex
    var lastCompletedIndex = config.crawlProcessingState.lastCompletedRoboScriptIndex
    ...
    if (processEvent && roboIndex == currentRoboScriptIndex + 1 && lastCompletedIndex == currentRoboScriptIndex - 1) {
      ...
      currentRoboScript?.let {
        onRoboScriptFinished(action.startTime, action.displayStateId, it)
        config.crawlProcessingState.updateLastCompletedRoboScriptIndex(currentRoboScriptIndex)
        lastCompletedIndex = currentRoboScriptIndex
      }
    }
    if (processEvent && roboIndex == lastCompletedIndex + 1 && roboIndex != currentRoboScriptIndex) {
      onRoboScriptStarted(action.startTime, action.displayStateId, roboScript)
    ...
    if ((processEvent && isComplete) || isTerminateCrawlAction(action)) {
      onRoboScriptFinished(action.startTime, action.displayStateId, roboScript)
  ...
  private fun onStepFinished(result: Result, endTimestamp: Timestamp) {
    config.consumer.onEvent(
      JourneyRunEvent.newBuilder()
        .setJourneyRunId(journeyRunId)
        .setStepFinished(
          StepFinished.newBuilder().setCompletion(Step.Completion.newBuilder().setResult(result).setEndTimestamp(endTimestamp))
        )
        .build()
    )
  }
  ...
  /**
   * Handles the addition (completion) of a turn. This function is invoked whenever a new turn is received. Receiving a new turn implies
  ...
  private fun onTurnAdded() {
    ...
    config.consumer.onEvent(
      JourneyRunEvent.newBuilder().setJourneyRunId(journeyRunId).setTurnAdded(TurnAdded.newBuilder().setTurn(currentTurn)).build()
    )
  }

  /**
   * Handles the start of a new RoboScript execution (corresponds to a Prompt). Reports the start event as a [StepStarted] proto.
   *
   ...
   */
  private fun onRoboScriptStarted(startTimestamp: Timestamp, displayStateId: Int, roboScript: RoboScriptDetails) {
  ...
  /**
   * Handles the completion of a RoboScript execution (corresponds to a Prompt). Reports the final status (success/failure) of the
   * corresponding prompt as a [StepFinished] proto.
   *
   * @param endTimestamp The timestamp when the prompt finished.
   * @param roboScript The details of the prompt that finished.
   */
  private fun onRoboScriptFinished(endTimestamp: Timestamp, displayStateId: Int, roboScript: RoboScriptDetails) {
    handleModelDetails(roboScript.modelDetails, displayStateId)
  ...
  /**
   * Handles an action that occurred while a RoboScript was ongoing. If the action is associated with a new turn, the current turn is
   * reported and a new turn is created. The action is then added as an Interaction to the current turn.
   *
   ...
   *
   * A "turn" represents a single cycle of the underlying model observing the screen, reasoning about the next action, and executing it. The
   * arrival of a new [ModelDetails] object, identified by a unique ID, signals the start of a new turn.
   *
   ...
   */
  private fun handleModelDetails(modelDetails: ModelDetails, displayStateId: Int) {
    ...
    config.crawlProcessingState.initNewTurn(displayStateId, modelDetails)
  }
  ...
  private fun terminationCauseToResult(roboScript: RoboScriptDetails): Result {
    return when (roboScript.terminationCause) {
      ...
      RoboScriptDetails.TerminationCause.END_OF_SCRIPT -> Result.newBuilder().setStatus(Status.SUCCEEDED).build()
   ...
   */
  private fun getErrorMessageFromTerminationCause(terminationCause: RoboScriptDetails.TerminationCause): String {
    return when (terminationCause) {
      ...
      RoboScriptDetails.TerminationCause.PROMPT_EVALUATION_LIMIT_REACHED ->
