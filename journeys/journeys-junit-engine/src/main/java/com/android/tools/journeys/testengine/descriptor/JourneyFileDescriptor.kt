...
class JourneyFileDescriptor(parentId: UniqueId, private val journeyFile: File, private val journeyFileRelativePath: String) :
  AbstractTestDescriptor(parentId.append(SEGMENT_TYPE, journeyFileRelativePath), journeyFileRelativePath), Node<JourneysExecutionContext> {
  ...
  private fun executeForRoboBackend(context: JourneysExecutionContext) {
    ...
    val reportEntryPublisher = { key: String, value: String ->
      if (value.isNotBlank()) {
        context.executionListener.reportingEntryPublished(this, ReportEntry.from(key, value))
      }
    }
    ...
    try {
      ...
      val errorResult = Result.newBuilder().setStatus(Status.ERROR).setErrorMessage(e.message).build()
    ...
    try {
      val adapterConfig = RoboResultAdapterConfig(compositeConsumer, crawlProcessingState, outputPath)
      ...
      context.proxy.executeJourney(context.journeyRunId, context.targetDeviceId, journeyFile.toPath(), artifactProcessor)
    } catch (e: Exception) {
      val errorResult = Result.newBuilder().setStatus(Status.ERROR).setErrorMessage(e.message).build()
      compositeConsumer.onEvent(createRunFinishedEvent(context, errorResult))
      throw e
    ...
    compositeConsumer.onEvent(createRunFinishedEvent(context, crawlProcessingState.journeyResult))
  ...
  private fun createRunStartedEvent(context: JourneysExecutionContext, prompts: List<String>): JourneyRunEvent {
    val initialization =
    ...
    context.targetDeviceName?.let { initialization.putMetadata("deviceName", it) }

    return JourneyRunEvent.newBuilder()
      .setJourneyRunId(context.journeyRunId)
      .setRunStarted(RunStarted.newBuilder().setInitialization(initialization.build()).build())
      .build()
  }

  private fun createRunFinishedEvent(context: JourneysExecutionContext, result: Result): JourneyRunEvent {
    return JourneyRunEvent.newBuilder()
      .setJourneyRunId(context.journeyRunId)
      .setRunFinished(
