...
/**
 * Executes a journey by installing necessary components on a device, establishing a connection with the crawler backend via gRPC, managing
 * ADB forwarding, and cleaning up the environment.
 *
...
class Proxy(
  ...
  /**
   * Executes a journey defined by the script at the given path. Manages instrumentation setup, crawl execution, and cleanup.
   *
   ...
   */
  fun executeJourney(journeyRunId: String, deviceId: String, journeyPath: Path, artifactProcessor: (Artifact) -> Unit) {
    var hostPort = 0
    try {
      ...
      val result = connectToCrawlerBackend(journeyRunId, hostPort, journeyScript, accessTokenPath, artifactProcessor)
  ...
  private fun readJourneyScript(journeyPath: Path): String {
    try {
      return journeyPath.inputStream().use { inputStream -> RoboConverter.convert(inputStream) }
    } catch (e: Exception) {
      throw JourneyExecutionException("Failed to read journey: ${e.message}", e, JourneyFailureReason.JOURNEY_READ_FAILED)
    }
  }
  ...
  private fun setupInstrumentation(deviceId: String): Process {
    try {
      ...
      if (customAppId.isBlank()) {
        ...
        val appApkPath =
          if (appApkLocation.isDirectory) {
            // hopefully, there is only one APK in the directory, we don't handle
            // multi APKs so far.
            File(
                appApkLocation,
                appApkLocation.list()?.single { it.endsWith(".apk") } ?: throw RuntimeException("no APK present in $appApkPath"),
              )
              .absolutePath
          } else appApkPath
      ...
      throw JourneyExecutionException("Installation failure: ${e.message}", e, JourneyFailureReason.ADB_INSTALL_FAILED)
    }
    ...
    try {
      val instrumentation =
        adb.runInstrumentation(
          ...
          args =
            mapOf(
              RoboConfigConstants.ROBO_V2_APP_PACKAGE_FLAG to (customAppId.takeIf { !it.isBlank() } ?: applicationId),
      ...
      throw JourneyExecutionException("Failed to run instrumentation: ${e.message}", e, JourneyFailureReason.INSTRUMENTATION_FAILED)
    }
  }

  /**
   * Provides a list of flags to use for installing the crawler app based on device API level.
   *
  ...
  /**
   * Provides a list of flags to use for installing the app based on device API level.
   *
   * @param deviceApiLevel The device API level to determine flags.
   * @return The list of install flags.
   */
   ...
   * @param destroyIfActiveBeforeExit Whether to destroy the process before checking exit value to exit gracefully. If crawl passed, we exit
   *   gracefully.
   */
  private fun stopInstrumentation(instrumentation: Process, destroyIfActiveBeforeExit: Boolean) {
    ...
    try {
      ...
      System.err.println("Unable to terminate instrumentation process. Instrumentation Output:\n" + "$fullOut ")
    }

    val failed =
      exitValue != 0 ||
        listOf("failures", "INSTRUMENTATION_FAILED", "INSTRUMENTATION_CODE: 0").any { fullOut.contains(it, ignoreCase = true) }
  ...
  /**
   * Sets up ADB forwarding by finding an available host port and forwarding it to the port used by the Robo service on the device. Retries
   * if necessary.
   *
   * @param deviceId The ID of the target device.
  ...
  private fun setupAdbForward(deviceId: String): Int {
    ...
    if (roboDevicePort == 0) {
      throw JourneyExecutionException("Could not determine Robo device port.", reason = JourneyFailureReason.ROBO_PORT_EXTRACTION_FAILED)
    }
  ...
  /**
   * Extracts the robo port number from the ADB dumpsys output for the proxy service. Retries if the port is not found.
   *
   * @param deviceId The ID of the target device.
  ...
  private fun extractRoboPortNumber(deviceId: String): Int {
    try {
      return retryIf(
        block = {
          val dumpsysOutput = adb.dumpsys(deviceId, "${RoboConfigConstants.CRAWLER_PACKAGE_ID}/${RoboConfigConstants.PROXY_SERVICE}")
          val matcher = RoboConfigConstants.ROBO_PROXY_PORT_IS_BOUND_PATTERN.matcher(dumpsysOutput)
          if (!matcher.find()) {
            0
          } else {
  ...
  private fun connectToCrawlerBackend(
    ...
    val crawlSetup =
      CrawlSetup.newBuilder()
        ...
        .setClientMetadata(ClientMetadata.newBuilder().setName("journeys-junit5-engine").build())
        .putAssets(RoboConfigConstants.ROBO_SCRIPT_CONFIG_NAME, ByteString.copyFromUtf8(journeyScript))
        .setTestTimeout(Duration.newBuilder().setSeconds(RoboConfigConstants.TEST_TIMEOUT_SECONDS).build())
        .setRoboConfig(RoboConfig.newBuilder().setUseNewExtensionHarness(true).setEnablePreActionOutput(true).build())
        .addCrawlParameters(CrawlParameter.newBuilder().setName(RoboConfigConstants.ROBO_ADB_FORWARD).setValue("true").build())
        .addCrawlParameters(CrawlParameter.newBuilder().setName(RoboConfigConstants.ROBO_PICK_OPEN_PORT).setValue("true").build())
        .build()
    ...
    try {
      ...
      try {
        val clientLogger = Logger.getLogger("${RoboConfigConstants.CRAWLER_PACKAGE_ID}.client.GrpcClient")
  ...
  /**
   * Retries executing the given function [block] until it meets the success condition (i.e., `!retryCondition(result)` is true) or exceeds
   * [maxRetries].
   *
  ...
  /**
   * A gRPC client interceptor that adds [CallCredentials] to outgoing calls. Combines provided credentials with any existing credentials in
   * [CallOptions].
   *
  ...
  class CallCredentialsInterceptor(private val credentials: CallCredentials) : ClientInterceptor {
    ...
    override fun <ReqT, RespT> interceptCall(
      ...
      callCredentials = if (callCredentials == null) credentials else CompositeCallCredentials(credentials, callOptions.credentials)
