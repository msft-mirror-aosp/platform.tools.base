...
open class RootObj @JvmOverloads constructor(val rootType: RootType, id: Long = 0, val thread: Int = 0, stack: StackTrace? = null) :
  Instance(id, stack) {

  fun getClassName(snapshot: Snapshot): String {
    val theClass: ClassObj? =
      when (rootType) {
  ...
  open val referredInstance: Instance?
    get() =
      when (rootType) {
