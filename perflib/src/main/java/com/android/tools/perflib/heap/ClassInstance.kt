...
open class ClassInstance(id: Long, stack: StackTrace?, private val valuesOffset: Long) : Instance(id, stack) {
  ...
  override val isSoftReference: Boolean
    get() = classObj!!.isSoftReference

  val isStringInstance: Boolean
    get() = "java.lang.String" == classObj?.className

  val asString: String?
    get() = getAsString(Int.MAX_VALUE)

  @VisibleForTesting fun getFields(name: String): List<FieldValue> = values.filter { it.field.name == name }
  ...
  override fun toString() = String.format(Locale.US, "%s@%d (0x%x)", classObj!!.className, uniqueId, uniqueId)
  ...
  fun getAsString(maxDecodeStringLength: Int): String? {
    ...
    fun <T> mkRaw(mk: (Int, Int) -> T) = mk(max(offset, 0), max(min(count, maxDecodeStringLength), 0))
    return when {
