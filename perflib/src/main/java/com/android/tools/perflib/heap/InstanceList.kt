...
/**
 * Internally, an InstanceList is either an instance (for length=1 case) or an array of instances. This class is used to reduce overhead
 * from indirection when there are lots of lists.
 *
 * This list only supports addition. Except for cases of lengths 0 and 1, operands to addition are not meant to be aliased afterwards. For
 * example, after `val list2 = list1 + instance`, `list1` is not meant to be referenced and used.
 *
 * Because Kotlin doesn't support ad-hoc (untagged) union at the moment, we emulate it by wrapping `Any` in a value type to avoid mis-uses,
 * and export `onCases` as a type-safe eliminator for the union.
 */
@JvmInline
value class InstanceList private constructor(val raw: Any /* Instance | Array<Instance?> */) {
  ...
  /** Returns a new list including the element. This instance is (conceptually) destroyed afterwards and should not be used elsewhere. */
  ...
  companion object {
    ...
    private fun Array<Instance?>.countInstances(): Int =
      when {
        isEmpty() -> 0
        else -> nextAvailableIndex()
      }

    private fun Array<Instance?>.instanceSequence(): Sequence<Instance> = asSequence().take(countInstances()) as Sequence<Instance>

    private fun Array<Instance?>.asInstanceList(): List<Instance> =
      when {
        isEmpty() -> listOf()
        else -> asList().subList(0, nextAvailableIndex()) as List<Instance>
      }

    private fun Array<Instance?>.plusElem(elem: Instance): Array<Instance?> =
      when {
        isEmpty() -> arrayOf(elem)
        last() != null ->
          arrayOfNulls<Instance>(size * 2).also { newElems ->
            copyInto(newElems)
