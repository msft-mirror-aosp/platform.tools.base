...
internal constructor(
  ...
  open val uniqueId: Long
    get() = id and heap!!.mSnapshot.idSizeMask
  ...
  val hardForwardReferences: Sequence<Instance>
    get() = _hardFwdRefs.asInstanceSequence()

  //  List of all objects that hold a live reference to this object
  internal var _hardRevRefs: InstanceList = InstanceList.Empty
  open val hardReverseReferences: Sequence<Instance>
    get() = _hardRevRefs.asInstanceSequence()
  ...
  open val softReverseReferences: Sequence<Instance>
    get() = _softRevRefs.asInstanceSequence()

  open val classObj: ClassObj?
    get() = heap!!.mSnapshot.findClass(classId)
  ...
  /**
   * Determines if this instance is reachable via hard references from any GC root. The results are only valid after ShortestDistanceVisitor
   * has been run.
   */
  val isReachable
    get() = distanceToGcRoot != Int.MAX_VALUE
   ...
   */
  open val isSoftReference: Boolean
    get() = false

  protected val buffer: DataBuffer
    get() = heap!!.mSnapshot.buffer
  ...
  val totalRetainedSize: Long
    get() = retainedSizes?.sum() ?: 0
  ...
  protected fun readValue(type: Type): Any? =
    when (type) {
