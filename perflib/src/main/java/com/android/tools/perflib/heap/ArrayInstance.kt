...
open class ArrayInstance(id: Long, stack: StackTrace?, val arrayType: Type, val length: Int, private val valuesOffset: Long) :
  Instance(id, stack) {
  ...
  fun asCharArray(offset: Int, length: Int): CharArray {
    ...
    val charBuffer = ByteBuffer.wrap(asRawByteArray(offset, length)).order(DataBuffer.HPROF_BYTE_ORDER).asCharBuffer()
