...
/**
 * Computes dominators based on the union-find data structure with path compression and linking by size. Using description found in:
 * http://adambuchsbaum.com/papers/dom-toplas.pdf which is based on a copy of the paper available at:
 * http://www.cc.gatech.edu/~harrold/6340/cs6340_fall2009/Readings/lengauer91jul.pdf
 */
object LinkEvalDominators {

  /** @return 2 parallel lists of each node and its immediate dominator, with `null` being the auxiliary root. */
  fun <T : Any> computeDominators(roots: Set<T>, next: (T) -> Stream<T>): Result<T> {
    ...
    for (currentNode in instances.size - 1 downTo 1) {
      ...
      for (i in 0 until buckets[parents[currentNode]].size) {
        ...
        doms[node] = if (semis[nodeEvaled] < semis[node]) nodeEvaled else parents[currentNode]
