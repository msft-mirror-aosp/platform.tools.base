...
class LinkEvalDominatorsTest : TestCase() {
  ...
  fun testSimpleGraph() =
    with(compute(Graph.Builder<Int>().addEdges(1, 2, 3).addEdges(2, 4, 6).addEdges(3, 4, 5).addEdges(4, 6).addRoots(1))) {
  ...
  fun testCyclicGraph() =
    with(compute(Graph.Builder<Int>().addEdges(1, 2, 3, 4).addEdges(2, 3).addEdges(3, 4).addEdges(4, 2).addRoots(1))) {
  ...
  fun testMultipleRoots() =
    with(compute(Graph.Builder<Int>().addEdges(1, 3).addEdges(2, 4).addEdges(3, 5).addEdges(4, 5).addEdges(5, 6).addRoots(1, 2))) {
      assertEquals(6, countReachable())
      assertImmDominates(1, 3)
  ...
  private inline fun <reified T : Any> compute(g: Graph.Builder<T>) = with(g.build()) { LinkEvalDominators.computeDominators(roots, next) }
}
...
class Graph<T>(val roots: Set<T>, val next: (T) -> Stream<T>) {
  class Builder<T> {
    ...
    fun addEdges(from: T, vararg to: T) = this.also { next.getOrPut(from, ::mutableSetOf).addAll(to) }
...
private fun <T> LinkEvalDominators.Result<T>.assertImmDominates(er: T, ee: T) = assertEquals(er, getImmDom(ee))
