 ...
 *
 * Deploys a runs a process`-tracker` binary on a device and converts it output to a [Flow] of [ProcessEvent].
 *
...
internal class AgentProcessTracker(
  ...
  private suspend fun pushAgent(deviceSelector: DeviceSelector, deviceAbi: String): Boolean {
    ...
    adbSession.deviceServices.shellAsLines(deviceSelector, command).collect {
      when {
        it is StderrLine && it.contents.isNotBlank() -> logger.warn("Unable to create $AGENT_DIR dir: ${it.contents.take(200)}")

        it is ExitCode && it.exitCode != 0 -> logger.warn("Unable to create $AGENT_DIR dir: ${it.exitCode}")
      }
    }
  ...
  companion object {
    ...
    @VisibleForTesting internal const val AGENT_DIR = "/data/local/tmp/.studio"

    @VisibleForTesting internal const val AGENT_PATH = "$AGENT_DIR/$AGENT_NAME"
  }
}
