...
class ProcessNameMonitorImplTest {
  ...
  private val fakeProcessTrackerFactory =
    object : FakeProcessTrackerFactory<String>() {
  ...
  fun devicesOnline(): Unit = runTest {
    FakeDeviceTracker().use { deviceTracker ->
      val monitor = processNameMonitor(deviceTracker, trackerFactory = fakeProcessTrackerFactory)
      ...
      assertThat(monitor.getProcessNames("device1", 1)).isEqualTo(ProcessNames("package1", "process1"))
      assertThat(monitor.getProcessNames("device2", 2)).isEqualTo(ProcessNames("package2", "process2"))
    }
  }
  ...
  fun propagatesMaxProcessRetention(): Unit = runTest {
    FakeDeviceTracker().use { deviceTracker ->
      ...
      assertThat(monitor.getProcessNames("device1", 1)).isNull()
      assertThat(monitor.getProcessNames("device1", 2)).isEqualTo(ProcessNames("package2", "process2"))
      assertThat(monitor.getProcessNames("device1", 3)).isEqualTo(ProcessNames("package3", "process3"))
    }
  }
  ...
  fun trackDeviceProcesses_multipleTrackers_sharesFlow(): Unit = runTest {
    ...
    val processTrackerFactory =
      ProcessTrackerFactory<String> {
        ProcessTracker {
          flow {
            listOf(0, 1).forEach {
    ...
    FakeDeviceTracker().use { deviceTracker ->
      ...
      assertThat(emitted).containsExactly(ProcessRemoved(0), ProcessRemoved(1)).inOrder()
    }
  }
  ...
  private fun CoroutineScope.processNameMonitor(
    ...
    return ProcessNameMonitorImpl(this, deviceTracker, trackerFactory, maxProcessRetention, FakeAdbLoggerFactory().logger).apply { start() }
  }
}
