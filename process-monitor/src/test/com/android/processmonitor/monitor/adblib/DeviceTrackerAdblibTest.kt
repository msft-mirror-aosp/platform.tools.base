...
class DeviceTrackerAdblibTest {
  ...
  private val fakeAdb
    get() = deviceProvisionerRule.fakeAdb
  ...
  @Test
  fun trackDevices_preexistingDevices(): Unit =
    runBlockingWithTimeout(Duration.ofSeconds(5)) {
      ...
      assertThat(tracker.trackDevices().take(2).toList()).containsExactly(DeviceOnline(connectedDevice1), DeviceOnline(connectedDevice2))
    }

  @Test
  fun trackDevices_noPreexistingDevices(): Unit =
    runBlockingWithTimeout(Duration.ofSeconds(5)) {
      val tracker = DeviceTrackerAdblib(deviceProvisioner, logger)
  ...
  @Test
  fun trackDevices_disconnects(): Unit =
    runBlockingWithTimeout(Duration.ofSeconds(5)) {
      fakeAdb.connectDevice("device1", ONLINE)
      fakeAdb.connectDevice("device2", ONLINE)
      val connectedDevice1 = deviceProvisioner.waitForDevice("device1")
      ...
      tracker.trackDevices().toChannel(this).use { channel ->
        assertThat(channel.take(2)).containsExactly(DeviceOnline(connectedDevice1), DeviceOnline(connectedDevice2))

        fakeAdb.disconnectDevice("device1")
        fakeAdb.disconnectDevice("device2")

        assertThat(channel.take(2))
          .containsExactly(DeviceDisconnected<DeviceState.Connected>("device1"), DeviceDisconnected<DeviceState.Connected>("device2"))
  ...
  private fun FakeAdbServerProvider.connectDevice(serialNumber: String, status: DeviceStatus = ONLINE) {
    connectDevice(serialNumber, "", "", "13", AndroidApiLevel(33), USB).apply { deviceStatus = status }
  }
  ...
  private suspend fun DeviceProvisioner.waitForDevice(serialNumber: String): DeviceState.Connected {
    ...
    return withTimeoutOrNull(5_000) { handle?.stateFlow?.first { it.isOnline() } as DeviceState.Connected }
