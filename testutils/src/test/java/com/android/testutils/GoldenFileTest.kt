...
class GoldenFileTest {
  ...
  fun assertUpToDateFailWithAutomaticUpdater() {
    ...
    val failure = assertFailsWith<AssertionError> { underTest.assertUpToDate(FakeUpdater::class.java) }
    assertThat(failure)
      .hasMessageThat()
      .isEqualTo(
        """
  ...
  fun assertUpToDateFailWithoutHelper() {
    ...
    val failureWithoutAutomaticUpdater = assertFailsWith<AssertionError> { underTest.assertUpToDate() }
  ...
  fun testUpdateNoOp() {
    ...
    assertThat(output.toString())
      .isEqualTo("No diff to apply to tools/base/testutils/src/test/resources/com/android/testutils/GoldenFileExample.txt\n")
  }
  ...
  fun testUpdateChanged() {
    Jimfs.newFileSystem(Configuration.unix()).use { fs ->
      ...
      val exampleFile = workspaceRoot.resolve("tools/base/testutils/src/test/resources/com/android/testutils/GoldenFileExample.txt")
      Files.createDirectories(exampleFile.parent)
      Files.write(exampleFile, listOf("Expected golden file content", "on multiple", "lines"))
      val output = StringBuilder()
      underTest.update(print = { string: String -> output.append(string).append('\n') }, getWorkspaceRoot = { workspaceRoot })
  ...
  fun testUpdateInconsistent() {
    Jimfs.newFileSystem(Configuration.unix()).use { fs ->
      ...
      val e = assertFailsWith<IOException> { underTest.update(getWorkspaceRoot = { workspaceRoot }) }
      assertThat(e)
        .hasMessageThat()
        .isEqualTo(
          "Workspace file tools/base/testutils/src/test/resources/com/android/testutils/GoldenFileExample.txt content different from corresponding resource, aborting update"
        )
    }
  }
}
