...
/**
 * Represents the contents of a zip file, with utility methods to make reading/writing zip files easier.
 *
 * Note:
 * - This class should be used in tests only as it is designed for convenience of use rather than performance (e.g., it loads all entries in
 *   memory at once). For production code, to improve performance/memory usage, use other utilities such as zipflinger's ZipArchive.
...
class ZipContents(
   ...
   *
   * Directory entries have names ending with "/" and empty byte array contents, and are often optional (e.g., a zip file containing a
   * `foo/bar.txt` file entry is not required to also contain a `foo` directory entry).
   */
  ...
  companion object {
    ...
    /** Reads [ZipContents] from the given [byteArray], with an optional [filter] to read only the [ZipEntry]s of interest. */
    fun fromByteArray(byteArray: ByteArray, filter: ((ZipEntry) -> Boolean)? = null): ZipContents {
      return read(byteArray.inputStream(), filter)
    }

    /** Reads [ZipContents] from the given [zipFile], with an optional [filter] to read only the [ZipEntry]s of interest. */
