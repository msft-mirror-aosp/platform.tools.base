...
class VersionRangeTest {
  ...
  fun testParseMavenRange() {
    for (open in listOf('[', ']', '(')) {
      for (close in listOf(']', '[', ')')) {
        assertThat(VersionRange.parse("${open}1.0,${close}").toString()).isEqualTo("${open}1.0,${close}")
        assertThat(VersionRange.parse("${open}1.0,${close}").toIdentifier()).isEqualTo("${open}1.0,${close}")
        assertThat(VersionRange.parse("${open},2.0${close}").toString()).isEqualTo("${open},2.0$close")
        assertThat(VersionRange.parse("${open},2.0${close}").toIdentifier()).isEqualTo("${open},2.0$close")
        assertThat(VersionRange.parse("${open}1.0,2.0${close}").toString()).isEqualTo("${open}1.0,2.0$close")
        assertThat(VersionRange.parse("${open}1.0,2.0${close}").toIdentifier()).isEqualTo("${open}1.0,2.0$close")
        // test also the edge case where the top is the next prefix (1.0 -> 1.1)
        assertThat(VersionRange.parse("${open}1.0,1.1${close}").toString()).isEqualTo("${open}1.0,1.1$close")
        assertThat(VersionRange.parse("${open}1.0,1.1${close}").toIdentifier()).isEqualTo("${open}1.0,1.1$close")
        // test an edge case with acceptable metacharacters
        assertThat(VersionRange.parse("${open}[]()+,1[]()${close}").toString()).isEqualTo("$open[]()+,1[]()$close")
        assertThat(VersionRange.parse("${open}[]()+,1[]()${close}").toIdentifier()).isEqualTo("$open[]()+,1[]()$close")
      }
    }
  }
  ...
  fun testUnrepresentableIdentifiers() {
    ...
    assertThat(VersionRange(Range.open(Version.prefixInfimum("1.0"), Version.prefixInfimum("2.0"))).toString())
    ...
    assertThat(VersionRange(Range.closed(Version.prefixInfimum("1.0"), Version.prefixInfimum("2.0"))).toString())
      .matches("^VersionRange\\(.*\\)$")
