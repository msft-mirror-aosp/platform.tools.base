...
class ResourceResolverNamespacesTest {
  ...
  private fun StyleResourceValueImpl.withItems(resolver: Resolver, vararg items: StyleItemResourceValueImpl): StyleResourceValue {
    for (item in items) {
  ...
  fun createRes() {
    ...
    val libRes =
      listOf(
        ...
        ResourceValueImpl(localLib, STRING, "header_explicit", "@com.example.common:string/company_name"),
        StyleResourceValueImpl(ResourceReference(localLib, STYLE, "Theme.Base"), "android:Theme", null)
          .withItems(Resolver.EMPTY_RESOLVER, StyleItemResourceValueImpl(app, "com.android.support:actionBarColor", "#00ff00", null)),
      )

    val appPrefixes = Resolver.fromBiMap(ImmutableBiMap.of("common", localLib.xmlNamespaceUri, "support", supportLib.xmlNamespaceUri))
    ...
    val appRes =
      listOf(
        ...
        ResourceValueImpl(app, STRING, "title_text_explicit", "@com.example.common:string/header_explicit"),
        StyleResourceValueImpl(ResourceReference(app, STYLE, "AppTheme"), "common:Theme.Base", null)
          .withItems(
            appPrefixes,
    ...
    val allRes = sequenceOf(androidRes, appRes, supportRes, libRes).flatMap { it.asSequence() }.asIterable()

    resolver = ResourceResolver.withValues(allRes, ResourceReference(app, STYLE, "AppTheme"))
  }

  private fun check(namespace: ResourceNamespace, type: ResourceType, name: String, resolvesTo: String) {
