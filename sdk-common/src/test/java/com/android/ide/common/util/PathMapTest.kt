...
class PathMapTest(val mapType: PathMapType) {
  ...
  @Test
  fun testValues() {
    val paths = arrayOf("", "/", "C:", "C:\\", "D:", "relativePath", "/foo/bar", "/foo/baz").map { PathString(it) }
  ...
  fun testGet() {
    ...
    paths.forEach { Truth.assertWithMessage("map[${it}] = ${it}").that(map[it]).isEqualTo(it) }
  ...
  fun testMostSpecificPath() {
    val inputTree = arrayOf("/foo" to "foo", "/foo/bar" to "bar", "/foo/bar/baz/bing" to "bing", "foo" to "relativePath")
    ...
    expected.forEach {
      Truth.assertWithMessage("map.findMostSpecific(${it.first}) == ${it.second}")
        .that(map.findMostSpecific(PathString(it.first)))
        .isEqualTo(it.second)
    }
  }
  ...
  fun testEntriesStartingWith() {
    val inputTree = arrayOf("/a", "/a/1", "/a/1/x", "/a/1/y", "/a/2", "/a/3", "/b", "/c/1/2/3/4/5", "/c/1/x")
    ...
    expected.forEach {
      ...
      Truth.assertWithMessage("map.findAllStartingWith(${it.first}) == ${it.second}").that(results).isEqualTo(expectedResults)
      Truth.assertWithMessage("map.containsKeyStartingWith(${it.first}) == ${containsAny}")
        .that(map.containsKeyStartingWith(PathString(it.first)))
        .isEqualTo(containsAny)
    }
  }
