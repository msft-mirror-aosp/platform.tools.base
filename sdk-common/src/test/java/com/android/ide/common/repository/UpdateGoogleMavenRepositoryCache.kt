 ...
 *
 * This class can be run using IJ run configurations or from bazel: `bazel run //tools/base/sdk-common:update_google_maven_repository_cache`
 *
 * In both cases, the path to the repo root directory (the one with `.repo` in it) can optionally be passed as the only argument.
 *
 * Running this may cause new files to be added to the sdk-common jar, which means `JarContentsTest` gradle integration test will need to be
 * updated. See that class for instructions on how to update it.
 */
fun main(args: Array<String>) {

  val workspace: Path =
    args.singleOrNull()?.let { Paths.get(it) }
      ?: try {
  ...
  while (current != null) {
    ...
    val groupIndex = readUrlDataAsString("${GMAVEN_BASE_URL}$relative/group-index.xml")
    ...
    groupIndex.lines().forEach { line ->
      ...
      if (start != -1) {
        ...
        sub.splitToSequence(",").forEach {
          ...
          if (!v.isPreview) {
            v.major?.let { major -> maxStablePerMajor.compute(major) { _, old -> if (old == null) v else maxOf(old, v) } }
              ?: error("Attempted to add a Version with no major: $v")
          }
        }
        if (max != null) {
          sb.append(line.substring(0, start))
          maxStablePerMajor.values.take(2).plus(max!!).toSortedSet().joinTo(sb, separator = ",")
  ...
  println(
    "Updated indices. NOTE: You may need to update the JarContentsTest gradle " +
...
private fun readUrlDataAsString(url: String): String = Resources.asCharSource(URL(url), Charsets.UTF_8).read()
...
private fun findWorkspaceInBazelRun(): Path? {
  ...
  while (currDir != null) {
    val doNotBuildHere: Path = currDir.resolve(DO_NOT_BUILD_HERE)
