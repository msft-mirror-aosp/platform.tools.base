...
class GenericBuiltArtifactsSplitOutputMatcherTest {
  ...
  private fun computeBestOutput(outputs: List<GenericBuiltArtifact>, vararg deviceAbis: String): List<File> {
  ...
  fun testAbiOnlyWithMultiMatch() {
    ...
    val result = computeBestOutput(list, "bar", "foo")
    TestCase.assertEquals(1, result.size)
    TestCase.assertEquals(File(match.outputFile), result[0])
  }
  ...
  fun testAbiOnlyWithMultiMatch2() {
    ...
    list.add(getAbiOutput("bar", 3).also { match = it })
    // bar is preferred over foo
    val result = computeBestOutput(list, "foo", "bar")
    TestCase.assertEquals(1, result.size)
    TestCase.assertEquals(File(match.outputFile), result[0])
  }
  ...
  fun testVariantLevelAbiFilter() {
    ...
    val list: MutableList<GenericBuiltArtifact> = ArrayList()
    list.add(getUniversalOutput(1).also { match = it })
    val result = computeBestOutput(list, Sets.newHashSet("bar", "foo"), "foo", "zzz")
    TestCase.assertEquals(1, result.size)
    TestCase.assertEquals(File(match.outputFile), result[0])
  }
  ...
  fun testWrongVariantLevelAbiFilter() {
    val list: MutableList<GenericBuiltArtifact> = ArrayList()
    list.add(getUniversalOutput(1))
    val result = computeBestOutput(list, Sets.newHashSet("bar", "foo"), "zzz")
    TestCase.assertEquals(0, result.size)
  }

  private fun getUniversalOutput(versionCode: Int): GenericBuiltArtifact {
    return GenericBuiltArtifact(outputType = "UNIVERSAL", outputFile = File("null").absolutePath, versionCode = versionCode)
  }

  private fun getAbiOutput(filter: String, versionCode: Int): GenericBuiltArtifact {
    return GenericBuiltArtifact(
      outputType = "ONE_OF_MANY",
      outputFile = File(filter).absolutePath,
      versionCode = versionCode,
      filters = listOf(GenericFilterConfiguration("ABI", filter)),
    )
  }

  private fun getAbiOutput(file: String): GenericBuiltArtifact {
    return GenericBuiltArtifact(
      outputType = "ONE_OF_MANY",
      outputFile = File(file).absolutePath,
      versionCode = 1,
      filters = listOf(GenericFilterConfiguration("ABI", "bar")),
    )
  }

  private fun getOutput(abiFilter: String, versionCode: Int): GenericBuiltArtifact {
    return GenericBuiltArtifact(
      outputType = "ONE_OF_MANY",
      outputFile = File(abiFilter).absolutePath,
      versionCode = versionCode,
      filters = listOf(GenericFilterConfiguration("ABI", abiFilter)),
    )
  }
}
