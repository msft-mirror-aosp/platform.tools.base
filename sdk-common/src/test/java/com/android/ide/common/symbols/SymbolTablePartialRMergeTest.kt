...
class SymbolTablePartialRMergeTest {
  ...
  @get:Rule val mTemporaryFolder = TemporaryFolder()
  ...
  fun testValuesOverrides() {
    ...
    Files.write(f1.toPath(), list("default int string s1", "default int string s2", "public int string s3"))

    val f2 = mTemporaryFolder.newFile("values-en_strings-R.txt")
    Files.write(f2.toPath(), list("default int string s1", "private int string s2", "default int string s3"))

    val result = SymbolTable.mergePartialTables(list(f1, f2), "com.foo.bar")

    assertThat(result.symbols.values()).hasSize(3)
    assertThat(result.symbols.values())
      .containsExactly(
        Symbol.normalSymbol(ResourceType.STRING, "s1", 0, ResourceVisibility.PRIVATE_XML_ONLY),
        Symbol.normalSymbol(ResourceType.STRING, "s2", 0, ResourceVisibility.PRIVATE),
        Symbol.normalSymbol(ResourceType.STRING, "s3", 0, ResourceVisibility.PUBLIC),
      )
  }
  ...
  fun testIncorrectValuesOverrides() {
    ...
    try {
      ...
      assertThat(e.message).contains("An error occurred during merging of the partial R files")
      assertThat(e.cause!!.message).contains("Symbol with resource type string and name s1 defined both as PRIVATE and " + "PUBLIC.")
    }
    ...
    try {
      ...
      assertThat(e.message).contains("An error occurred during merging of the partial R files")
      assertThat(e.cause!!.message).contains("Symbol with resource type string and name s1 defined both as PUBLIC and " + "PRIVATE.")
    }
  }
  ...
  fun testLayoutOverrides() {
    ...
    assertThat(result.symbols.values())
      .containsExactly(
        Symbol.normalSymbol(ResourceType.ID, "idB", 0, ResourceVisibility.PRIVATE_XML_ONLY),
        Symbol.normalSymbol(ResourceType.LAYOUT, "layout", 0, ResourceVisibility.PRIVATE_XML_ONLY),
      )
    ...
    assertThat(result.symbols.values())
      .containsExactly(
        Symbol.normalSymbol(ResourceType.ID, "idA", 0, ResourceVisibility.PRIVATE_XML_ONLY),
        Symbol.normalSymbol(ResourceType.LAYOUT, "layout", 0, ResourceVisibility.PRIVATE_XML_ONLY),
      )
  }
  ...
  fun testMultipleSourceSets() {
    ...
    Files.write(nonXmlA.toPath(), list("default int drawable image", "default int drawable picture"))
    files.add(nonXmlA)
    ...
    Files.write(stringsA.toPath(), list("default int string foo", "default int string bar", "default int string beep"))
    files.add(stringsA)
    ...
    Files.write(stylesA.toPath(), list("default int[] styleable dsA", "default int styleable dsA_attr1", "default int styleable dsA_attr2"))
    files.add(stylesA)

    // all should be kept as 'public'
    val publicA = File(sourceSetA, "values_public-R.txt")
    Files.write(publicA.toPath(), list("public int string foo", "public int layout layout1", "public int id id1"))
    files.add(publicA)
    ...
    Files.write(layout2B.toPath(), list("default int id id1", "default int id idB", "default int layout layout2"))
    files.add(layout2B)
    ...
    Files.write(styleablesC.toPath(), list("default int[] styleable dsA", "default int styleable dsA_attr3"))
    ...
    assertThat(result).isEqualTo(SymbolIo().readFromPartialRFile(generatedPartialRFile, "com.boop.beep"))
  }
  ...
  fun testMultipleStyleables() {
    ...
    Files.write(stylesC.toPath(), list("public int[] styleable s1", "default int styleable s1_android_name")) // should merge all children

    // Files given in order of A files, B files, C files.
    val result = SymbolTable.mergePartialTables(listOf(stylesA, stylesB, stylesC), "com.boop.beep")
    ...
    val expected =
      SymbolTable.builder()
        ...
        .add(
          Symbol.styleableSymbol("s1", ImmutableList.of(), ImmutableList.of("a1", "a2", "a3", "android:name"), ResourceVisibility.PUBLIC)
        )
        .build()
    assertThat(result).isEqualTo(expected)
  }
  ...
  fun styleablesWithConflictingStyleableChildrenVisibilities() {
    ...
    // public.
    Files.write(stylesA.toPath(), list("public int attr a1", "public int[] styleable s1", "default int styleable s1_a1"))

    val sourceSetB = mTemporaryFolder.newFolder("B")
    val stylesB = File(sourceSetB, "values_styles-R.txt")
    // Should conflict with A for "a1". The attribute the child is referring to is marked as
    // private.
    Files.write(stylesB.toPath(), list("private int attr a1", "public int[] styleable s1", "default int styleable s1_a1"))
    ...
    try {
      ...
      assertThat(Throwables.getRootCause(e).message)
        .isEqualTo("Symbol with resource type attr and name a1 defined both as PUBLIC and " + "PRIVATE.")
    }
  }
  ...
  fun styleablesWithConflictingStyleableVisibilities() {
    val sourceSetA = mTemporaryFolder.newFolder("A")
    val stylesA = File(sourceSetA, "values_styles-R.txt")
    Files.write(stylesA.toPath(), list("public int[] styleable s1"))

    val sourceSetB = mTemporaryFolder.newFolder("B")
    val stylesB = File(sourceSetB, "values_styles-R.txt")
    // All styleables are marked as public by  AAPT2, but let's tests this just in case.
    Files.write(stylesB.toPath(), list("private int[] styleable s1"))
    ...
    try {
      ...
      assertThat(Throwables.getRootCause(e).message)
        .isEqualTo("Symbol with resource type styleable and name s1 defined both as PUBLIC and " + "PRIVATE.")
    }
  }
}
