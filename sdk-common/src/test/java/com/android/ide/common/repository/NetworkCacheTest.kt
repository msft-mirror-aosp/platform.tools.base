...
class NetworkCacheTest {
  ...
  fun testNetworkDisabled() {
    val cache =
      object : TestCache(networkEnabled = false) {
  ...
  fun testParallelCounter() {
    val cache =
      object : TestCache(networkEnabled = true) {
  ...
  fun testNetworkEnabled() {
    var networkCalls = 0
    val networkEnabledCache =
      object : TestCache(networkEnabled = true) {
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) = ReadUrlDataResult(null, true).also { networkCalls++ }
      }
  ...
  fun testCacheMissNotModifiedSince() {
    ...
    val networkEnabledCacheWithHit =
      object : TestCache(networkEnabled = true) {
        val artifactPath = cacheDir!!.resolve("artifact.xml")
        ...
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) =
          ReadUrlDataResult(null, false).also { lastModifiedRequest = lastModified }
      }
  ...
  fun testCacheMiss() {
    ...
    val networkEnabledCacheWithHit =
      object : TestCache(networkEnabled = true) {
        ...
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) =
          ReadUrlDataResult(ByteArray(12) { i -> i.toByte() }, true).also { lastModifiedRequest = lastModified }
      }
  ...
  fun testCacheMissDeprecatedOverride() {
    val networkEnabledCacheWithMiss =
      object : TestCache(networkEnabled = true) {
        val artifactPath = cacheDir!!.resolve("artifact.xml")
        ...
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) = ReadUrlDataResult(ByteArray(5) { i -> i.toByte() }, true)
      }
  ...
  fun testKeyPrefixOfAnotherKey() {
    val servedContent =
      mapOf("" to """root content""", "directory/" to """directory content""", "directory/file" to """file in directory content""")
    val networkEnabledCache =
      object : TestCache(networkEnabled = true) {
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) =
  ...
  fun testPathologicalUrls() {
    val justNull = "\u0000"
    val windows =
      "con/prn/aux/nul/com0/com1/com2/com3/com4/com5/com6/com7/com8/com9/com¹/com²/com³/lpt0/lpt1/lpt2/lpt3/lpt4/lpt5/lpt6/lpt7/lpt8/lpt9/lpt¹/lpt²/lpt³/CON/PRN/AUX/NUL/COM0/COM1/COM2/COM3/COM4/COM5/COM6/COM7/COM8/COM9/COM¹/COM²/COM³/LPT0/LPT1/LPT2/LPT3/LPT4/LPT5/LPT6/LPT7/LPT8/LPT9/LPT¹/LPT²/LPT³/a"
    val servedContent = mapOf(justNull to """null content""", windows to """windows""")
    val networkEnabledCache =
      object : TestCache(networkEnabled = true) {
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) =
          ReadUrlDataResult(servedContent[url]?.toByteArray(Charsets.UTF_8), true)
  ...
  fun testFullUrlsSentAsRelativePath() {
    ...
    val servedContent = mapOf(url to "Example release notes")
    val networkEnabledCache =
      object : TestCache(networkEnabled = true) {
        override fun readUrlData(url: String, timeout: Int, lastModified: Long) =
          ReadUrlDataResult(servedContent[url]?.toByteArray(Charsets.UTF_8), true)
  ...
  fun testGetRelativePath() {
    ...
    assertThat(cache.getRelativePath("path1/path2:param1=value1;param2=value2"))
      .isEqualTo("path1/path2-3Aparam1-3Dvalue1-3Bparam2-3Dvalue2")
  ...
  private abstract class TestCache(cacheDir: Path = Files.createTempDirectory(""), networkEnabled: Boolean) :
    NetworkCache("", cacheDir, networkEnabled = networkEnabled) {
    fun loadArtifact() = findData("artifact.xml")

    fun getContentAsString(relative: String, treatAsDirectory: Boolean = false): String? =
      findData(relative, treatAsDirectory)?.use { it.readBytes().toString(Charsets.UTF_8) }

    override fun readDefaultData(relative: String): InputStream? = null

    override fun error(throwable: Throwable, message: String?) = fail("No error calls expected")
  }
}
