...
class DeviceUtilsTest {
  ...
  fun test() {
    ...
    Truth.assertThat(device.getLanguages(Duration.ZERO)).containsExactly("en", "fr", "es", "it", "de")
  }
  ...
  fun testIsSameAsDeviceWithSerialNumberIsMdnsAutoConnectTls() {
    ...
    // Act
    val sameAsDeviceWith = device.isSameAsDeviceWith("adb-serialNumber2-cYuns7._adb-tls-connect._tcp")
  ...
  fun testIsSameAsDeviceWithKeyIsMdnsAutoConnectTls() {
    ...
    // Act
    val sameAsDeviceWith = device.isSameAsDeviceWith("adb-serialNumber1-cYuns7._adb-tls-connect._tcp")
  ...
  private fun setUpDeviceSerialNumber(serialNumber: String) {
    whenever(device.serialNumber).thenAnswer { serialNumber }
  }

  private fun setUpDeviceOutput(vararg configs: String) {
    whenever(device.executeShellCommand(Mockito.anyString(), Mockito.any(), Mockito.anyLong(), Mockito.any())).thenAnswer {
