...
class ResourceDirectoryParserTest {
  ...
  /**
   * Creates a file in a directory with specific data. The file is created in the path `path` resolved against `directory`. For example, if
   * `path` is `a/b`, the file to create is named `b` and it is placed inside subdirectory `a` of `directory`. The subdirectory is created
   * if needed.
   *
   * @param data the data to write in the file
   * @param directory the directory where the file is created.
   * @param path the path for the file, relative to `directory` using forward slashes as separators
   */
  ...
  /**
   * Same as [make], but writing random data in the file instead of receiving specific data to write.
   *
   * @param directory the directory where the file is created.
   * @param path the path for the file, relative to `directory` using forward slashes as separators
   */
  ...
  fun parseEmptyResourceDirectory() {
    ...
    val parsed = parseResourceSourceSetDirectory(directory, IdProvider.sequential(), platformTable, logger = MockLog())
  ...
  fun checkLayoutXmlTrailingContentEmitsWarning() {
    ...
    val platformTable = SymbolTable.builder().tablePackage("android").build()
    val logger = MockLog()
    parseResourceSourceSetDirectory(directory, IdProvider.sequential(), platformTable, logger = logger)

    // Only layout/layoutWithTrailingContent.xml should trigger a warning.
    assertThat(logger.messages).hasSize(1)
    assertThat(logger.messages.single())
      .contains(
    ...
    val parsed =
      SymbolTable.builder()
        .apply {
          ...
          assertParsesWithError("res/bad/a.xml")
            .hasMessageThat()
            .isEqualTo("Error parsing 'res/bad/a.xml': Invalid resource directory name 'bad'")
          assertParsesWithError("res/drawable/%.png")
            .hasMessageThat()
            .isEqualTo("Error parsing 'res/drawable/%.png': The resource name must start with a letter")
        }
        .build()
    ...
    val parsed =
      SymbolTable.builder()
        .apply {
          ...
          assertParsesWithError("res/values/corrupt.xml", "<other></other>")
            .hasMessageThat()
            .isEqualTo("Error parsing 'res/values/corrupt.xml'")
        }
        .build()
  ...
  fun failWithException() {
    ...
    try {
      ...
      parseResourceSourceSetDirectory(directory, IdProvider.sequential(), platformTable, logger = MockLog())
      fail()
  ...
  fun reportPathForFailedNonValuesXmlFile() {
    ...
    try {
      ...
      assertThat(e.cause!!.message).contains("Validation of a resource with name 'android:name' and type 'id' failed.")
      assertThat(e.cause!!.cause!!.message).contains("Error: ':' is not a valid resource name character")
    }
  }
  ...
  private fun SymbolTable.Builder.assertParses(name: String) {
    getParseZipEntryError(name)?.let { throw AssertionError("expected to parse without error", it) }
  }

  private fun SymbolTable.Builder.assertParses(name: String, content: String) {
    getParseZipEntryError(name, content)?.let { throw AssertionError("expected to parse without error", it) }
  }

  private fun SymbolTable.Builder.assertParsesWithError(name: String): ThrowableSubject {
    return assertThat(getParseZipEntryError(name) ?: throw AssertionError("expected to have error parsing"))
  }

  private fun SymbolTable.Builder.assertParsesWithError(name: String, content: String): ThrowableSubject {
    return assertThat(getParseZipEntryError(name, content) ?: throw AssertionError("expected to have error parsing"))
  }
  ...
  private fun SymbolTable.Builder.getParseZipEntryError(name: String, content: () -> String): Exception? {
    var error: Exception? = null
    this.parseAarZipEntry(documentBuilder, { error = it }, name, { content().byteInputStream() })
    return error
  }
}
