...
class LocaleGenerationTest {
  ...
  fun `Test that manifest attribute is correctly generated`() {
    Truth.assertThat(generateLocaleConfigManifestAttribute("_gradle_res_locale_config")).isEqualTo("@xml/_gradle_res_locale_config")
  }

  @Test
  fun `Test that locale string is correctly generated`() {
    Truth.assertThat(generateLocaleString(FolderConfiguration.getConfig("values-en-rUS".split("-"))!!.localeQualifier)).isEqualTo("en-US")
    Truth.assertThat(generateLocaleString(FolderConfiguration.getConfig("values-b+zh+Hans+SG".split("-"))!!.localeQualifier))
  ...
  fun `Test that locale config xml is correctly generated`() {
    ...
    writeLocaleConfig(output = outfile, locales = setOf("en-US", "ru-RU", "es-ES", "pt-BR", "zh-Hans-SG", "en-GB"), defaultLocale = "en-US")
    val expectedLocales =
      listOf(
    ...
    writeLocaleConfig(output = outfile, locales = setOf("en-US", "ru-RU", "es-ES", "pt-BR", "zh-Hans-SG", "en-GB"))
    Truth.assertThat(
  ...
  fun `Test read supported locales`() {
    ...
    Truth.assertThat(supportedLocalesObject.folderLocales).containsExactly("zh", "es-rES", "pt-rBR", "b+zh+Hant", "b+zh+Hant+TW")
  }
}
