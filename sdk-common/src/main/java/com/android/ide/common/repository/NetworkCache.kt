...
abstract class NetworkCache(
  ...
  /**
   * If false, this repository won't make any network requests - Make sure you make another call, maybe in a background thread, with enabled
   * network if you want the cache to update properly.
   */
  private val networkEnabled: Boolean = true,
) {
  @Deprecated(
    "The cacheDir parameter is unused, call primary constructor instead",
  ...
  private val RESERVED_WINDOWS_FILE_NAMES =
    setOf(
      "CON",
      "PRN",
      "AUX",
  ...
  /**
   * Reads the given query URL in, with the given time out, and returns the bytes found. The second element of the pair is false if the
   * remote source reported an unmodified resource and true otherwise.
   */
  @Slow protected abstract fun readUrlData(url: String, timeout: Int, lastModified: Long): ReadUrlDataResult
  ...
  protected inline fun <T> withLock(file: Path, action: () -> T): T {
    ...
    try {
      return lock.withLock { action.invoke() }
    } finally {
      synchronized(this) {
  ...
  /**
   * Reads the given data relative to the base URL. Method is safe to execute in parallel.
   *
   * @param relative The relative
   * @param treatAsDirectory store the cache content in a directory, i.e. as if the URL ended in / This is useful if a queried URL will be a
   *   prefix of another queried URL, which can't be represented directly on the filesystem.
   */
  @Slow
  ...
  private fun encode(it: String): String {
    ...
    return if (
      SdkConstants.CURRENT_PLATFORM == SdkConstants.PLATFORM_WINDOWS &&
        RESERVED_WINDOWS_FILE_NAMES.contains(encoded.substringBefore(".").uppercase(Locale.US))
    )
      "($encoded)"
    else encoded
  }
