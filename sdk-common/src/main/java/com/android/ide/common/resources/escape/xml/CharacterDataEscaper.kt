...
/** Static singleton responsible for escaping character data in XML. See the [escape] method for more details. */
object CharacterDataEscaper {
  ...
  private val ESCAPED_HEXADECIMAL_REFERENCE = Pattern.compile("""$HEXADECIMAL_ESCAPE(\p{XDigit}+);""")
  private val ESCAPED_UNICODE_REFERENCE = Pattern.compile("""$UNICODE_ESCAPE(\p{XDigit}{4})""")
  private val saxParserFactory =
    XmlUtils.configureSaxFactory(SAXParserFactory.newInstance(), /* namespaceAware= */ false, /* checkDtd= */ false)

  /** Map from characters that need unescaping to what we replace them with when we unescape them, which may be the same character. */
  private val escapedCharReplacements = mapOf(' ' to ' ', '"' to '"', '\'' to '\'', '\\' to '\\', 'n' to '\n', 't' to '\t')
   ...
   * [this Android Cookbook recipe](https://androidcookbook.com/Recipe.seam?recipeId=2219)
   *
   * The argument is expected to be valid XML. Character data outside of CDATA sections is unescaped as follows:
  ...
  /** Returns a [ContentHandler] that appends the escaped content to the end of the provided [StringBuilder]. */
  @Suppress("UnstableApiUsage")
  private fun createEscapingContentHandler(stringBuilder: StringBuilder, escapeApostrophes: Boolean): ContentHandler {
  ...
  /**
   * Returns a [ContentHandler] that appends the content to the given [StringBuilder] after stripping escaped quotes and unescaping each
   * character.
   */
  private fun createUnescapingContentHandler(stringBuilder: StringBuilder): ContentHandler =
    StringResourceContentHandler(stringBuilder) { stringBuilder.append(it.stripUnescapedQuotes().unescapeChars()) }

  /** Returns the replacement character for this character per [escapedCharReplacements]. */
  private fun Char.getReplacement(): Char = escapedCharReplacements[this] ?: throw IllegalArgumentException(toString())
  ...
  private fun String.shouldUnescapeCharAt(index: Int): Boolean = get(index) in escapedCharReplacements && containsEscapedCharAt(index)

  /**
   * Determines if the character at index [index] is escaped.
   *
   * A character is considered escaped if the previous character is an *unescaped* backslash. The function calls itself (tail) recursively
   * if it needs to determine whether the previous character is itself escaped.
   */
  ...
  /** Returns a copy of this [String] with all escaped decimal and hexadecimal character references unescaped. */
  ...
  /**
   * Builds a [javax.xml.parsers.SAXParser], sets `this` [ContentHandler] as its handler and then uses the parser to parse the [String].
   *
