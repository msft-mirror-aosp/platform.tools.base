 ...
 *
 * A simple parser that accept both the GMS core v11 and v12 syntax of a downloadable font query.
 */
class QueryParser {
  ...
  companion object {
    @JvmStatic
    fun parseDownloadableFont(authority: String, query: String): DownloadableParseResult {
  ...
  private fun parseFontStyle() {
    when (symbol) {
      ...
      else -> throw FontQueryParserError("Unexpected keyword: " + scanner.last + valid("wght", "wdth", "ital", "bold", "exact", "nearest"))
    }
    symbol = scanner.next()
  }
  ...
  private fun parseV11(authority: String): DownloadableParseResult {
    ...
    while (symbol == Symbol.AND) {
      ...
      when (key) {
        ...
        else -> throw FontQueryParserError("Unexpected keyword: " + keyName + valid("width", "weight", "italic", "besteffort"))
      }
      symbol = scanner.next()
    }
    expect(Symbol.EOQ)
