 ...
 *
 * The absolute path to the module source set is identified by the source set ordering of a module. Format of the returned String is
 * `<package name - source set module order>:<path to source set>`.
 */
fun getRelativeSourceSetPath(resourceFile: File, moduleSourceSets: Map<String, String>): String {
  ...
  throw IllegalArgumentException("Unable to locate resourceFile ($absoluteResFilePath) in source-sets.")
}

/**
 * Converts a source set identified relative resource path to an absolute path.
 *
 * The source set identifier before the separator is replaced with the absolute source set path and then concatenated with the path after
 * the separator.
 */
...
fun relativeResourcePathToAbsolutePath(
  ...
  return { relativePath: String ->
    ...
    val resourcePathFromSourceSet = relativePath.substring(separatorIndex + separator.lastIndex, relativePath.length)
    val systemRelativePath =
      if ("/" != fileSystem.separator) {
        resourcePathFromSourceSet.replace("/", fileSystem.separator)
      } else {
        resourcePathFromSourceSet
      }
    val absolutePath =
      sourceSetPathMap[sourceSetPrefix]
...
/** Parses identifier and file path into a map from a file in the format 'packageName.projectName-sortedOrderPosition absolutePath'. */
fun readFromSourceSetPathsFile(artifactFile: File): Map<String, String> {
  ...
  return artifactFile.bufferedReader().use { bufferedReader ->
    bufferedReader.lineSequence().associate { it.substringBefore(" ") to it.substringAfter(" ") }
  }
}

/**
 * Writes a file containing a mapping of resource source-set absolute paths to a unique identifier in the format of
 * 'packageName.projectName-sortedOrderPosition absolutePath'.
 */
fun writeIdentifiedSourceSetsFile(resourceSourceSets: List<File>, namespace: String, projectPath: String, output: File) {
  output.bufferedWriter().use { bw ->
    getIdentifiedSourceSetMap(resourceSourceSets, namespace, projectPath).forEach { bw.write("${it.key} ${it.value}\n") }
  }
}

/**
 * Using a list of files following the format produced by writeIdentifiedSourceSetsFile, contents of each file are added to a single table
 * which maps the source set identifier to the absolute path of the source set.
 */
fun mergeIdentifiedSourceSetFiles(sourceSetFiles: List<File>): Map<String, String> {
  return mutableMapOf<String, String>().also { identifiedSourceMap ->
    sourceSetFiles.map { readFromSourceSetPathsFile(it) }.forEach { identifiedSourceMap.putAll(it) }
  }
}

fun getIdentifiedSourceSetMap(resourceSourceSets: List<File>, namespace: String, projectPath: String): Map<String, String> {
  ...
  return resourceSourceSets
    ...
    .associate { sourceSet ->
      ...
      val appendProjectName = if (namespace.endsWith(projectName)) "" else ".$projectName"
...
/**
 * Verifies if a string is relative resource sourceset filepath. This is for cases where it is not possible to determine if relative
 * resource filepaths are enabled by default.
 */
