...
/**
 * An implementation of a [Path]-like data structure that can represent unix or Windows-style path names. Unlike [Path], this is a
 * lightweight data structure with no dependencies so it can be instantiated and manipulated even if there is no matching nio filesystem
 * implementation installed. Unlike [File], this can be used to represent paths on filesystems other than file:// - provided the filesystem
 * uses compatible path names.
 *
 * This class remembers its scheme. It also remembers the exact string used to construct it. For this reason, conversions of [Path] ->
 * [PathString] -> [Path] are lossless. So are conversions of [File] -> [PathString] -> [File]. If the string held a valid pathname,
 * conversions of [String] -> [PathString] -> [String] would also be lossless, even if the string made inconsistent use of separators and
 * case.
 */
class PathString
private constructor(
  /**
   * Holds the URI for the filesystem root. For example, this would be `file:///` for local filesystem paths on Unix and `file:///X:\` or
   * `file:///X:/` on Windows.
   */
  val filesystemUri: URI,
  /**
   * String which contains the platform-independent path string for this [PathString]. The used portion of the string is indicated by
   * [startIndex] and [suffixEndIndex], below. Note that we store strings in this way to avoid copying the string in the methods that return
   * a sub-path.
   */
  ...
  /**
   * Exclusive index into "path" containing the last character of the prefix identifier. This would be something like "C:/" on Windows or
   * "/" on Linux.
   */
  private val prefixEndIndex: Int = 0,
  /**
   * Separator to be used when interpreting the [path] string, or [NUL_CHAR] if the [path] string doesn't contain any separators. This might
   * be different from the system separator when manipulating paths intended for use on other filesystems.
   */
  ...
  /**
   * Creates a [PathString] given a filesystem [protocol] and [path]. This constructor is applicable only to the local filesystem and the
   * filesystems uniquely defined by their protocol, e.g. "jar" and "apk" filesystems. For other filesystems use alternative constructors.
   *
   ...
   *
   * @param filesystemUri the URI used to identify the filesystem. It is intended to contain the shortest path such that
   *   [FileSystems.getFileSystem] will return the correct filesystem. For example, this would be "file:///" for local filesystem paths on
   *   Unix.
   * @param path the path within the filesystem
   */
  ...
  override fun toString(): String {
    ...
    if (schemeString.startsWith("file:///") && schemeString.endsWith(":/") && schemeString.length == "file:///".length + ":/".length + 1) {
  ...
  /** If this path uses the file:// scheme, this method returns the associated [File] object. Returns null if it uses any other scheme. */
  fun toFile(): File? {
  ...
  /**
   * If this [PathString] can be converted to a [Path], returns the [Path]. Returns null if the [filesystemUri] does not point to a known
   * nio filesystem.
   *
   * Throws [SecurityException] if the filesystem exists but permission is denied to it. Throws [java.nio.file.InvalidPathException] if the
   * filesystem exists but the path string cannot be converted.
   */
  fun toPath(): Path? {
    return try {
  ...
  /**
   * Returns the file name that is farthest from the root component. Returns an empty string if this path is empty besides the root
   * component.
   */
  val fileName: String
    get() {
  ...
  fun segment(index: Int): String {
    ...
    var subRangeStart =
      startIndex +
        startIndex.until(endIndex).countUntil {
    ...
    val subRangeEnd =
      subRangeStart +
        subRangeStart.until(endIndex).countUntil {
          if (isSeparator(path[it])) {
            lengthCount++
          }
          lengthCount >= 1
  ...
  /** Returns the name of the given segment of the [PathString] as a [PathString]. [index] must be between 0 and [nameCount]-1 */
  ...
  /**
   * Returns a sub-sequence of name elements from this path between, as a relative path. [beginIndex] is inclusive, [endIndex] is exclusive.
   * Requires that 0 <= [beginIndex] <= [endIndex] <= [nameCount].
   */
  fun subpath(beginIndex: Int, endIndex: Int): PathString = subRange(beginIndex, endIndex - beginIndex)

  /**
   * Returns true iff this [PathString] starts with the given prefix. That is, it returns true if this path has the same filesystem and
   * root, and begins with all of the segments in the possible prefix. This is different from checking for a string prefix. For example,
  ...
  val parent: PathString?
    get() {
      ...
      val result = PathString(filesystemUri, path, startIndex, newEnd, prefixEndIndex, separator)
  ...
  /** Returns the file name of the parent path, or null if this is a root path. This is a convenience method similar to [File.getParent]. */
  ...
  val nameCount: Int
    get() {
      if (endIndex == prefixEndIndex) return 0
      return startIndex.until(endIndex).count { isSeparator(path[it]) } + 1
    }
  ...
  /**
   * Returns a [PathString] that is the same as this one, but without a trailing separator. Returns this if it doesn't have a trailing
   * separator.
   */
  ...
  /**
   * Constructs a path which, when resolved against this path, will point to the same location as the original. Returns the original path if
   * there is no simpler way to express a relative path between this path and the other.
   */
  fun relativize(other: PathString): PathString {
    ...
    if (root != otherRoot) {
      ...
      if (otherRootString != convertedRootString) {
        ...
        if (afterDriveSeparator > 0) {
          val firstDifference = 0.until(min(afterDriveSeparator, other.prefixEndIndex)).countUntil { path[it] != other.path[it] }
          if (firstDifference >= afterDriveSeparator) {
            rootString = other.path.substring(afterDriveSeparator, other.prefixEndIndex)
          }
        }
        return PathString(filesystemUri, rootString + other.path.substring(other.startIndex, other.suffixEndIndex), rootString.length)
      }
    }
    ...
    val commonPrefixLength = 0.until(min(segments.count(), otherSegments.count())).countUntil { segments[it] != otherSegments[it] }

    val newSegments =
      (commonPrefixLength.until(segments.size)).map { PARENT } + (commonPrefixLength.until(otherSegments.size)).map { otherSegments[it] }
    ...
    return PathString(filesystemUri, finalString, 0, finalString.length, 0, separator)
  }
  ...
  /**
   * Resolves the given path against this one, given a path on the same filesystem. Resolving a path is a lot like concatenation for
   * relative paths. Absolute paths point to the same place after being resolved. This is best explained by analogy. Imagine you're in a
   * command prompt and this path is your current directory. You type "cd [other]". The resolved path is the directory you'd end up in.
   *
   * @param other a path string on the same filesystem as this. Both relative and absolute paths are supported.
   */
  fun resolve(other: String): PathString = resolve(PathString(filesystemUri, other))

  /**
   * Resolves the given path against this one. Resolving a path is a lot like concatenation for relative paths. Absolute paths point to the
   * same place after being resolved. This is best explained by analogy. Imagine you're in a command prompt and this path is your current
   * directory. You type "cd [other]". The resolved path is the directory you'd end up in.
   */
  ...
  private fun sameRootAs(other: PathString): Boolean {
    ...
    for (idx in 0 until prefixEndIndex) {
      if (path[idx] != other.path[idx] && !(isSeparator(path[idx]) && isSeparator(other.path[idx]))) {
        return false
      }
    }
    return true
  ...
  private fun driveName(rawPath: String): String = rawPath.substring(0, rawPath.countUntil(':')).uppercase()

  private fun subRangeOrNull(index: Int, length: Int = 1): PathString? {
    if (index < 0 || length < 0) {
      return null
    }
    ...
    return PathString(filesystemUri, path, subRangeStart, subRangeEnd, 0, separator)
  }

  private fun subRange(index: Int, length: Int = 1): PathString {
    return subRangeOrNull(index, length)
      ?: throw IllegalArgumentException("beginIndex $index and suffixEndIndex ${index + length} are out of range for path ${toString()}")
  }

  /** The end of the path string which does not include a trailing separator. */
  private val endIndex: Int
    get() = if (hasTrailingSeparator) suffixEndIndex - 1 else suffixEndIndex

  private fun computeNameStart(end: Int): Int = end - startIndex.until(end).reversed().countUntil { isSeparator(path[it]) }
}
...
private fun String.withSeparator(sep: Char): String = if (sep == NUL_CHAR) this else replace('/', sep).replace('\\', sep)

private fun chooseSeparator(c1: Char, c2: Char) =
  if (c1 == NUL_CHAR) {
    if (c2 == NUL_CHAR) File.separatorChar else c2
  } else c1
