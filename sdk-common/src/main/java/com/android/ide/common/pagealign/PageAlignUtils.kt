...
data class ProgramHeader(
     ...
     */
    val programHeaderType: String get() = when(type) {
        ELF_PT_LOAD -> "PT_LOAD"
...
/**
 * Represents a simplified ELF Section Header.
 * Used to verify strict containment within Segments.
 */
...
private fun verifyElfSectionAlignment(context: ElfContext): List<AlignmentProblem> {
    ...
    for (relro in relroHeaders) {
        ...
        val loadIndex = sortedLoadHeaders.indexOfLast { load ->
            load.vaddr <= relro.vaddr
        }
 ...
 *
 * It calculates the list of Sections contained within the RELRO segment
 * and the list of Sections contained within the LOAD segment.
 * If the lists are identical, RELRO effectively covers the entire LOAD segment logic.
 */
private fun relroIsEntireLoadSection(
    relro: ProgramHeader,
    load: ProgramHeader,
    ...
    fun getContainedSections(segment: ProgramHeader): List<SectionHeader> {
        return sections.filter { sh ->
            sh.addr >= segment.vaddr && (sh.addr + sh.size) <= segment.endVaddr
...
/**
 * Skip n bytes, even if the underlying stream provides only partial reads.
 * The caller is required to check the result because that's the way to know the
...
/**
 * Read the stream into the given byte array even if the underlying stream
 * provides only partial reads.
 */
    ...
    while (entry != null) {
        ...
        if (hasElfMagicNumber(input)) {
            ...
            if (elfProblems != null) {
                ...
                elfProblems.forEach {
                    alignmentProblems.computeIfAbsent(entry.name) { mutableListOf() }
                        .add(it)
                }

                if (entry.method == ZipEntry.STORED && !currentEntryAligned) {
                    alignmentProblems.computeIfAbsent(entry.name) { mutableListOf() }.add(
                        ZipEntryNotAligned(input.bytesRead)
                    )
                }
            }
        }
...
fun findElfFile16kAlignmentInfo(file: File): PageAlignmentInfo {
    return FileInputStream(file).use { fis ->
        ZipArchiveInputStream(fis).use { findElfFile16kAlignmentInfo(it) }
    }
}
