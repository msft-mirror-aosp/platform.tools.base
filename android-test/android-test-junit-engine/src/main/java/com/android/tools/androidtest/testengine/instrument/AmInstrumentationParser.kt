 ...
 *
 * The parser works for the output of `am instrument -w -r ...`. It handles individual test results, as well as custom instrumentations.
 *
 * Instances of this class are mutable. To use them concurrently, clients must surround each method invocation (or invocation sequence) with
 * external synchronization of the clients choosing.
 *
 * ## Reporting test results
 *
 * For individual test results, the parser expects a series of status key/value pairs, followed by a status code. Status codes identify the
 * individual test phase: started(1), pass(0), error(-1), fail(-2), ignored(-3), or assumption failure(-4).
 *
 * ## Reporting (custom) instrumentation results
 *
 * At the end, the parser expects an optional series of result key/value pairs, followed by an instrumentation code. Instrumentation codes
 * identify the end state: ok(-1), cancelled(0). These codes map to `Activity.RESULT_*` constants.
 *
 * ## Sample output
 *
 * ```
...
class AmInstrumentationParser(
  ...
  /**
   * Handles the reporting when an individual test result has been parsed. This can be the result of a started test case, or a completed
   * test case.
   */
  ...
  private fun handleInstrumentationEnded(errorCause: String? = null) {
    ...
    val result = InstrumentationResult(code, bundle.toMutableMap().also { knownStatus.forEach(it::remove) })
    this.result = result
  ...
  private fun reportInstrumentationFailed(message: String) = listeners.forEach { it.instrumentationFailed(message) }
  ...
  companion object {
    ...
    private const val STREAM_INSTRUMENTATION_PROCESS_CRASHED = "Test instrumentation process crashed"
