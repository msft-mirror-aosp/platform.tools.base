 ...
 *
 * This class encapsulates common operations like installing and uninstalling APKs. Its logic is designed to mirror the UTP (Unified Test
...
class AdbApkInstaller(
   ...
   *
   * @property grantPermissions Grants all runtime permissions automatically on installation (`-g` flag). Requires API 23+.
   * @property forceQueryable Makes the app queryable by any other app on the device (`--force-queryable` flag). Requires API 30+.
   * @property forceReinstall Reinstalls an existing app, allowing version code downgrades (`-r` and `-d` flags).
   * @property forceCompilation Specifies an AOT compilation mode to run after installation. Requires API 24+.
  ...
  /**
   * Lazily fetches the current user ID, for devices that support multiple users (API 24+). Returns `null` if the device doesn't support
   * multiple users or if the ID cannot be determined.
   */
  private val userId: String? by lazy {
    ...
    return@lazy if (userId.toIntOrNull() != null) userId
    else {
   ...
   *
   * This may include disabling the adb installation verifier and setting the debug app, depending on the device API level.
   *
   ...
   * @throws RuntimeException if installation fails or an option is not supported by the device's API level.
   */
  fun installApk(apk: File, options: InstallOptions) {
    ...
    if (
      deviceApiLevel >= MinFeatureApiLevel.FORCE_COMPILATION.apiLevel && options.forceCompilation != ForceCompilation.NO_FORCE_COMPILATION
    ) {
   ...
   *
   * The `adb install-multiple` command is used, which requires API 21+. **Note:** This method cannot be used to install multiple, separate
   * applications at once. All APKs in the list must belong to the same package.
   *
  ...
  fun installSplitApk(apks: List<File>, options: InstallOptions) {
    ...
    if (deviceApiLevel < MinFeatureApiLevel.SPLIT_APK.apiLevel) {
      throw RuntimeException(
        "Split APK installation requires API level ${MinFeatureApiLevel.SPLIT_APK.apiLevel}, " + "but device is API level $deviceApiLevel."
      )
    }
   ...
   *
   * This method first parses the APK to determine its package name, then executes `adb uninstall`. If the package name cannot be
   * determined, a warning is logged and no action is taken.
   *
   * @param apk The APK file used to identify the package to uninstall.
   */
  fun uninstallApk(apk: File) {
    getPackageNameFromApk(apk.absolutePath)?.let { packageName -> uninstallPackage(packageName) }
  ...
  private fun forceCompilationApk(apk: File, mode: ForceCompilation) {
    getPackageNameFromApk(apk.absolutePath)?.let { packageName -> forceCompilationPackage(packageName, mode) }
