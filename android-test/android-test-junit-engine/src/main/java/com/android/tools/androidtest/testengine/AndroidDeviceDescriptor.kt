...
class AndroidDeviceDescriptor(uniqueId: UniqueId, val deviceSerial: String) :
  ...
  override fun execute(context: AndroidTestExecutionContext, dynamicTestExecutor: Node.DynamicTestExecutor): AndroidTestExecutionContext {
    ...
    val listener = Listener(dynamicTestExecutor)
   ...
   */
  inner class Listener(private val dynamicTestExecutor: Node.DynamicTestExecutor) : AmInstrumentationListener {
    ...
    override fun testStarted(testIdentifier: TestIdentifier) {
      ...
      val packageDescriptor =
        packageDescriptors.computeIfAbsent(packageName) { name ->
          val packageUniqueId = this@AndroidDeviceDescriptor.uniqueId.append("package", name)
          val newPackageDescriptor = AndroidPackageTestDescriptor(packageUniqueId, name)
          newPackageDescriptor.setParent(this@AndroidDeviceDescriptor)
