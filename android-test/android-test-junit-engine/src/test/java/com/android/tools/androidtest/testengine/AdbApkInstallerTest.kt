...
class AdbApkInstallerTest {
  ...
  private fun getCommandKey(command: List<String>): String {
    ...
    return when {
      ...
      else -> throw IllegalArgumentException("Unknown command to generate key for: $commandString")
    }
  }

  /** Queues a mock process to be returned when a command with the given key is executed. */
  private fun mockCommand(commandKey: String, exitCode: Int, output: String = "", error: String = "") {
    val process =
      mock<Process> {
  ...
  fun `installApk failure throws RuntimeException`() {
    ...
    val exception = assertFailsWith<RuntimeException> { helper.installApk(apk1, AdbApkInstaller.InstallOptions()) }
  ...
  fun `installApk with all options on API 34`() {
    ...
    val options =
      AdbApkInstaller.InstallOptions(
  ...
  fun `installApk options on unsupported API are ignored`() {
    ...
    val options = AdbApkInstaller.InstallOptions(grantPermissions = true, forceQueryable = true)
    helper.installApk(apk1, options)
  ...
  fun `installApk with full compilation triggers compile command`() {
    ...
    val options = AdbApkInstaller.InstallOptions(forceCompilation = AdbApkInstaller.ForceCompilation.FULL_COMPILATION)
    helper.installApk(apk1, options)
  ...
  fun `installSplitApk api too low throws exception`() {
    ...
    val exception = assertFailsWith<RuntimeException> { helper.installSplitApk(listOf(apk1), AdbApkInstaller.InstallOptions()) }
