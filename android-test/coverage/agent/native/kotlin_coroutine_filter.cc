...
// Traces backward in the flat instruction stream to determine if a virtual
// register was loaded with CoroutineSingletons.COROUTINE_SUSPENDED.
bool TraceRegisterToCoroutineSuspended(lir::Instruction* branch_instr,
                                       int target_reg) {
      ...
      if (bytecode->operands.size() > 0) {
        if (auto* dest_vreg = dynamic_cast<lir::VReg*>(bytecode->operands[0])) {
          if (dest_vreg->reg == target_reg) {
                ...
                if (auto* field_operand = dynamic_cast<lir::Field*>(operand)) {
                  if (field_operand->ir_field != nullptr &&
                    ...
                    const char* parent_class =
                        field_operand->ir_field->parent->descriptor->c_str();
                    const char* field_name =
                        (field_operand->ir_field->name != nullptr)
                            ? field_operand->ir_field->name->c_str()
                            : "";
                    if (strcmp(field_name, "COROUTINE_SUSPENDED") == 0 &&
                        strcmp(parent_class,
                               "Lkotlin/coroutines/intrinsics/"
                               "CoroutineSingletons;") == 0) {
                      return true;
                    }
                  }
                }
            ...
            // Case 2: Static getter method read (MOVE_RESULT_OBJECT of
            // getCOROUTINE_SUSPENDED)
            else if (op == dex::OP_MOVE_RESULT_OBJECT) {
              // Find the invoking instruction directly preceding the
              // move-result-object
              if (instr->prev != nullptr) {
                if (auto* prev_bytecode =
                        dynamic_cast<lir::Bytecode*>(instr->prev)) {
                        ...
                        if (method_operand->ir_method != nullptr &&
                          ...
                          const char* p_method_name =
                              (method_operand->ir_method->name != nullptr)
                                  ...
                                  : "";
                          if ((strcmp(parent_class,
                                      "Lkotlin/coroutines/intrinsics/"
                                      "IntrinsicsKt;") == 0 ||
                               strcmp(parent_class,
                                      "Lkotlin/coroutines/intrinsics/"
                                      "IntrinsicsKt__IntrinsicsJvmKt;") == 0) &&
                              strcmp(p_method_name, "getCOROUTINE_SUSPENDED") ==
                                  0) {
                            return true;
                          }
                        }
            ...
            // If the register was written to by any other instruction, its
            // value has been redefined, meaning it no longer holds the
            // coroutine singleton. Terminate trace.
            return false;
          }
        }
      }
...
// Scans backward in the flat instruction stream to detect synthetic coroutine
// suspension checks
bool IsSyntheticInstructionFlow(lir::Instruction* branch_instr) {
  ...
  // Check if either of the register operands evaluated in the branch traces
  // back to COROUTINE_SUSPENDED
  for (auto* operand : bytecode->operands) {
...
// Detects if a branch in invokeSuspend is part of the coroutine label
// state-machine setup
          ...
          if (auto* field_operand = dynamic_cast<lir::Field*>(operand)) {
            if (field_operand->ir_field != nullptr &&
              ...
              const char* field_type =
                  field_operand->ir_field->type->descriptor->c_str();
              if (strcmp(field_name, "label") == 0 &&
                  strcmp(field_type, "I") == 0) {
                return true;
              }
            }
          }
...
bool KotlinCoroutineFilter::FilterBranch(ir::EncodedMethod* ir_method,
  ...
  if (ir_method->decl != nullptr && ir_method->decl->parent != nullptr &&
    ...
    if (strcmp(method_name, "invokeSuspend") == 0 &&
      ...
      if (IsCoroutineSetupBranch(branch_instr)) {
        return true;
      }
    }
  }
  ...
  if (IsSyntheticInstructionFlow(branch_instr)) {
    return true;
  }
