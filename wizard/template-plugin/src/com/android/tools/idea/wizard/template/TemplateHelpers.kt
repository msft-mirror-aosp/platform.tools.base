...
fun activityToLayout(activityName: String, layoutName: String? = null): String =
  if (activityName.isNotEmpty()) AssetNameConverter(Type.ACTIVITY, activityName).overrideLayoutPrefix(layoutName).getValue(Type.LAYOUT)
  else ""
...
fun fragmentToLayout(fragmentName: String, layoutName: String? = null): String =
  if (fragmentName.isNotEmpty())
    AssetNameConverter(Type.FRAGMENT, fragmentName).overrideLayoutPrefix(layoutName ?: "fragment").getValue(Type.LAYOUT)
  else ""

/** Similar to [camelCaseToUnderlines], but strips off common class suffixes such as "Activity", "Fragment", etc. */
fun classToResource(name: String): String = if (name.isNotEmpty()) AssetNameConverter(Type.CLASS_NAME, name).getValue(Type.RESOURCE) else ""

/**
 * Converts a String to Camel Case. The return will not contain any two consecutive upper case characters For example: MyCLASsName to
 * MyClassName my_class_name to MyClassName URL to Url
 */
...
fun underscoreToCamelCase(string: String): String = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, string)

fun underscoreToLowerCamelCase(string: String): String = CaseFormat.LOWER_UNDERSCORE.to(CaseFormat.LOWER_CAMEL, string)

fun escapeKotlinIdentifier(identifier: String): String =
  identifier.split(".").joinToString(".") { if (it in kotlinKeywords) "`$it`" else it }

/**
 * Creates a Java class name out of the given string, if possible. For example, "My Project" becomes "MyProject", "hello" becomes "Hello",
 * "Java's" becomes "Javas", and so on.
 *
...
/**
 * Finds a resource given a file path and a context class. The implementation takes cares of the details of different file path separators
...
/**
 * Creates a domain name from the form of package name (e.g. from "com.example.www" to "www.example.com"). Returns an example domain if the
 * [packageName] is empty.
 */
fun packageNameToDomain(packageName: String): String {
  val domain = packageName.split(".").map { it.trim() }.filter { it.isNotEmpty() }.reversed().joinToString(".")
