...
def _get_nightly_ci_evals_tests(build_env: bazel.BuildEnv) -> List[str]:
  """Queries bazel for nightly ci evals tests."""
  query = [f'attr(tags, "nightly_ci_eval", "//...")']
...
def studio_evals(build_env: bazel.BuildEnv):
  ...
  target_tests = _get_nightly_ci_evals_tests(build_env)
