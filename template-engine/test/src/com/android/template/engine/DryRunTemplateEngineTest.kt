...
class DryRunTemplateEngineTest(private val fileSystemId: FileSystemId) {
  ...
  fun `verifies dry run captures expression errors on small in memory template`() {
    ...
    val engine = factory.createDryRunEngine(messageSink, destinationPathProvider = { getTestRootPath() })
