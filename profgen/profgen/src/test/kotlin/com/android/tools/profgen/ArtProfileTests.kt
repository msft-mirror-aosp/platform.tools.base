...
class ArtProfileTests {
  ...
  fun testTranscodeFromPtoS() {
    ...
    outP.use { prof.save(outP, ArtProfileSerializer.V0_1_0_P) }
    val outM2 = ByteArrayOutputStream()
    outM2.use { prof.save(outM2, ArtProfileSerializer.METADATA_0_0_2) }
    // Reconstruct
    ...
    outS.use { merged.save(outS, ArtProfileSerializer.V0_1_5_S) }
    ...
    val emptyTypeIds = baselineS.profileData.any { (file, _) -> file.header.typeIds == Span.Empty }
    assertFalse(emptyTypeIds)
    assertTrue { golden.contentEquals(baselineContents) }
  }
  ...
  private fun validateZipContentEquals(expected: ZipFile, observed: ZipFile) {
    fun ZipFile.getContents(): Map<String, ByteArray> = entries().asSequence().associate { it.name to getInputStream(it).readBytes() }
    val exp = expected.getContents()
    val obs = observed.getContents()
    assertEquals(exp.keys.toSet(), obs.keys.toSet())
    exp.entries
      .sortedBy { it.key }
      .zip(obs.entries.sortedBy { it.key })
      .forEach {
  ...
  fun testExtractProfileAsDm() {
    ...
    validateZipContentEquals(expected = ZipFile(golden), observed = ZipFile(output.toFile()))
  }
  ...
  fun testDexMetadataGeneration() {
    ...
    ZipFile(apkFile).use { zipFile ->
      ...
      val map =
        buildDexMetadata(apkFile.nameWithoutExtension, merged, outputDir, listOf(SerializerInfo(ArtProfileSerializer.V0_1_5_S, 31..34)))
      assertTrue(map.isNotEmpty())
      val dmFile = requireNotNull(map[31]) // 31 is Android S
      validateZipContentEquals(expected = ZipFile(golden), observed = ZipFile(dmFile))
    }
  }
  ...
  fun testCompareAgainstProfmanProfile() {
    ...
    val hrp = HumanReadableProfile(testData("com.example.countdown-hrp.txt"), strictDiagnostics)!!
    val profgen = ArtProfile(hrp, ObfuscationMap.Empty, Apk(testData("app-release.apk")))
    ...
    assertThat(goldenDexFile.dexChecksum).isEqualTo(profgenDexFile.dexChecksum)
    assertThat(goldenDexFile.header.methodIds.size).isEqualTo(profgenDexFile.header.methodIds.size)
  }
  ...
  fun testDumpProfiles_withInlineCaches_V015S_strictMode() {
    ...
    input.use {
      val prof = ArtProfile(input)!!
      val builder = StringBuilder()
      assertFailsWith(IllegalStateException::class) { dumpProfile(builder, prof, apk, obf, strict = true) }
    }
  }
  ...
  private val ArtProfile.typeIdCount: Int
    get() = profileData.values.sumOf { it.typeIndexes.size }

  private val ArtProfile.classIdCount: Int
    get() = profileData.values.sumOf { it.classIndexes.size }

  private val ArtProfile.methodCount: Int
    get() = profileData.values.sumOf { it.methods.values.size }
}

private fun strictHumanReadableProfile(path: String) = HumanReadableProfile(testData(path), strictDiagnostics)!!
