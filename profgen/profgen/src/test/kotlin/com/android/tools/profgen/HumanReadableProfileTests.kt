...
class HumanReadableProfileTests {
  @Test
  fun testLineMatchesItself() = assertMatchesItself("Lcom/google/Foo;->method(II)I", "Lcom/google/Foo;->method-name-with-hyphens(II)I")

  @Test
  fun testSinglePartWildcardInPackageSegment() =
    forLine("HLcom/*/Foo;->method(II)I") {
      assertMatches(
  ...
  @Test
  fun testComments() {
    val hrp = HumanReadableProfile("# Test Comment", "# Test Comment", "Lcom/anything/can/go/here;", "# Test Comment", "# Test Comment")
  ...
  @Test
  fun testMergedRules() {
    val hrp = HumanReadableProfile("HLa/B;->**(**)**", "SLa/B;->foo*(II)**", "PLa/B;->fooExact(II)V", "PLa/*;->foo*Inexact(II)Z")
...
internal fun HumanReadableProfile.assertStartupClass(type: String) = assertThat(match(type)).isEqualTo(STARTUP)
...
private fun parseRule(line: String) = parseRule(line, { _, message -> fail(message) }, RuleFragmentParser(line.length))!!
...
private fun HumanReadableProfile(vararg strings: String): HumanReadableProfile {
  return HumanReadableProfile(*strings) { _, _, message -> fail(message) }!!
}
