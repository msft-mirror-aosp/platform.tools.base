...
enum class ArtProfileSerializer(
   ...
   * =============================================
   *
   * The file starts with a header and section information: FileHeader FileSectionInfo[] The first FileSectionInfo must be for the DexFiles
   * section.
   *
   * The rest of the file is allowed to contain different sections in any order, at arbitrary offsets, with any gaps between them and each
   * section can be either plaintext or separately zipped. However, we're writing sections without any gaps with the following order and
   * compression: DexFiles - mandatory, plaintext ExtraDescriptors - optional, zipped Classes - optional, zipped Methods - optional, zipped
   * AggregationCounts - optional, zipped, server-side
   *
   * DexFiles: number_of_dex_files (checksum,num_type_ids,num_method_ids,profile_key)[number_of_dex_files] where `profile_key` is a
   * length-prefixed string, the length is `uint16_t`.
   *
   * ExtraDescriptors: number_of_extra_descriptors (extra_descriptor)[number_of_extra_descriptors] where `extra_descriptor` is a
   * length-prefixed string, the length is `uint16_t`.
   *
   * Classes section contains records for any number of dex files, each consisting of: profile_index // Index of the dex file in DexFiles
   * section. number_of_classes type_index_diff[number_of_classes] where instead of storing plain sorted type indexes, we store their
   * differences as smaller numbers are likely to compress better.
   *
   * Methods section contains records for any number of dex files, each consisting of: profile_index // Index of the dex file in DexFiles
   * section. following_data_size // For easy skipping of remaining data when dex file is filtered out. method_flags bitmap_data
   * method_encoding[] // Until the size indicated by `following_data_size`. where `method_flags` is a union of flags recorded for methods
   * in the referenced dex file, `bitmap_data` contains `num_method_ids` bits for each bit set in `method_flags` other than "hot" (the size
   * of `bitmap_data` is rounded up to whole bytes) and `method_encoding[]` contains data for hot methods. The `method_encoding` is:
   * method_index_diff number_of_inline_caches inline_cache_encoding[number_of_inline_caches] where differences in method indexes are used
   * for better compression, and the `inline_cache_encoding` is dex_pc (M|dex_map_size) type_index_diff[dex_map_size] where `M` stands for
   * special encodings indicating missing types (kIsMissingTypesEncoding) or memamorphic call (kIsMegamorphicEncoding) which both imply
   * `dex_map_size == 0`.
   */
  V0_1_5_S(byteArrayOf('0', '1', '5', '\u0000')) {
    ...
    override fun read(src: InputStream): Map<DexFile, DexFileData> {
      ...
      src.use {
        ...
        require(dexFileSection != null) { "Missing DEX_FILES section." }
        // DexFile section
        val dexFileDataArray = readDexFileSection(dexFileSection.readCompressed(profile).inputStream())
        // Classes section
        val classesSection = sections[FileSectionType.CLASSES]
        if (classesSection != null) {
          readClassesSection(classesSection.readCompressed(profile).inputStream(), dexFileDataArray)
        }
        // Methods Section
        val methodsSection = sections[FileSectionType.METHODS]
        if (methodsSection != null) {
          readMethodsSection(methodsSection.readCompressed(profile).inputStream(), dexFileDataArray)
        }

        return dexFileDataArray.associate { it.dexFile to it.asDexFileData() }
      }
    }
    ...
    private fun readFileSections(src: InputStream): Map<FileSectionType, ReadableFileSection> {
      src.use {
        ...
        for (i in 0 until fileSectionCount) {
          ...
          require(offset < Int.MAX_VALUE) { "Offset $offset must be <= ${Int.MAX_VALUE}" }
          val size = src.readUInt32()
          require(size < Int.MAX_VALUE) { "Size $size must be <= ${Int.MAX_VALUE}" }
          val inflateSize = src.readUInt32()
          require(inflateSize < Int.MAX_VALUE) { "Inflate Size $inflateSize must be <= ${Int.MAX_VALUE}" }
          val fileSection =
            ReadableFileSection(type = type, span = Span(size = size.toInt(), offset = offset.toInt()), inflateSize = inflateSize.toInt())
    ...
    private fun readDexFileSection(src: InputStream): Array<MutableDexFileData> {
      src.use {
        ...
        require(dexFileCount < MAX_NUM_DEX_FILES) { "Found $dexFileCount dex files. Maximum number allowed are $MAX_NUM_DEX_FILES" }
        val dexFileData =
          Array(dexFileCount) {
            val dexChecksum = src.readUInt32()
            val typeIdSetSize = src.readUInt32()
            require(typeIdSetSize <= MAX_NUM_CLASS_IDS) { "Invalid typeIdSetSize $typeIdSetSize. Max allowed is $MAX_NUM_CLASS_IDS" }
            val numMethodIds = src.readUInt32()
            require(numMethodIds <= MAX_NUM_METHOD_IDS) { "Invalid numMethodIds $numMethodIds. Max allowed is $MAX_NUM_METHOD_IDS" }
            val profileKeySize = src.readUInt16()
            require(profileKeySize <= MAX_PROFILE_KEY_SIZE) {
              """"Invalid profileKeySize $profileKeySize.
                            | Max allowed is $MAX_PROFILE_KEY_SIZE"""
                .trimMargin()
            }
    ...
    private fun writeProfileSections(os: OutputStream, profileData: List<Map.Entry<DexFile, DexFileData>>, apkName: String) {
      os.use {
    ...
    private fun writeDexFileSection(profileData: List<Map.Entry<DexFile, DexFileData>>, apkName: String): WritableFileSection {
      ...
      require(contents.size == expectedSize) { "Bytes saved (${contents.size}) do not match the expected size ($expectedSize)." }
    ...
    private fun readClassesSection(src: InputStream, dexFileDataArray: Array<MutableDexFileData>) {
      src.use {
    ...
    private fun createCompressibleClassSection(profileData: List<Map.Entry<DexFile, DexFileData>>): WritableFileSection {
      // Compute expected size of the header
      var expectedSize = 0
      val out = ByteArrayOutputStream()
      out.use {
    ...
    private fun readMethodsSection(src: InputStream, dexFileDataArray: Array<MutableDexFileData>) {
      src.use {
        while (src.available() > 0) {
          val dexProfileIdx = src.readUInt16()
          require(dexProfileIdx <= MAX_NUM_DEX_FILES) {
          ...
          require(followingDataSize <= src.available()) {
            """Following data size $followingDataSize.
                            | Max available bytes (${src.available()}"""
              .trimMargin()
          }
          ...
          val dexFileData = dexFileDataArray[dexProfileIdx]
          val methodBitMapSize = getMethodBitmapStorageSizeV015S(methodFlags, dexFileData.numMethodIds)
          src.readMethodBitmapV015S(methodFlags, dexFileData)
          // followingDataSize includes the method flag which is a UINT_16
          dexFileData.hotMethodRegionSize = followingDataSize.toInt() - (UINT_16_SIZE + methodBitMapSize)
          src.readHotMethodRegion(dexFileData, this@V0_1_5_S::skipInlineCaches)
        }
      }
    }

    private fun createCompressibleMethodsSection(profileData: List<Map.Entry<DexFile, DexFileData>>): WritableFileSection {
      var expectedSize = 0
      val out = ByteArrayOutputStream()
      out.use {
        profileData.onEachIndexed { index, entry ->
          ...
          val followingDataSize = UINT_16_SIZE + bitmapContents.size + methodRegionContents.size
          expectedSize += UINT_32_SIZE
    ...
    private fun createMethodBitmapRegion(methodFlags: Int, entry: Map.Entry<DexFile, DexFileData>): ByteArray {
      val dexFile = entry.key
      val dexFileData = entry.value
      val out = ByteArrayOutputStream()
      out.use { out.writeMethodBitmapV015S(methodFlags, dexFile, dexFileData) }
      return out.toByteArray()
    }

    private fun createMethodsWithInlineCaches(entry: Map.Entry<DexFile, DexFileData>): ByteArray {
      val dexFileData = entry.value
      val out = ByteArrayOutputStream()
      out.use { out.writeMethodsWithInlineCaches(dexFileData) }
      return out.toByteArray()
    }

    private fun computeMethodFlags(entry: Map.Entry<DexFile, DexFileData>): Int {
      val dexFileData = entry.value
      var methodFlags = 0
   ...
   * =============================================
   *
   * [profile_header, zipped[[dex_data_header1, dex_data_header2...],[dex_data1, dex_data2...], global_aggregation_count]] profile_header:
   * magic,version,number_of_dex_files,uncompressed_size_of_zipped_data,compressed_data_size dex_data_header: dex_location,number_of_classes
   * dex_data: class_id1,class_id2...
   */
  METADATA_FOR_N(MetadataVersion.V_001.versionBytes, byteArrayOf('p', 'r', 'm', '\u0000'), MetadataVersion.V_001) {
    ...
    /**
     * Serializes the profile data in a byte array. This methods only serializes the actual profile content and not the necessary headers.
     */
    private fun createCompressibleBody(profileData: List<Map.Entry<DexFile, DexFileData>>, apkName: String): ByteArray {
      ...
      for ((_, data) in profileData) {
        dataBos.writeIntsAsDeltas(data.classIndexes)
      }

      check(dataBos.size() == requiredCapacity) {
        ("The bytes saved do not match expectation. actual=" + dataBos.size() + " expected=" + requiredCapacity)
      }
    ...
    override fun read(src: InputStream): Map<DexFile, DexFileData> =
      with(src) {
        ...
        val uncompressedData = readCompressed(compressedDataSize.toInt(), uncompressedDataSize.toInt())
    ...
    private fun InputStream.readUncompressedBody(numberOfDexFiles: Int): Map<DexFile, DexFileData> {
      ...
      return dexFileData.map { it.dexFile to it.asDexFileData() }.toMap()
    }
  },
   ...
   * ==================================================
   * profile_header: magic,version,number_of_dex_files,uncompressed_size_of_zipped_data,compressed_data_size profile_data: profile_index,
   * profile_key_size, profile_key, type_id_size, class_index_size, class_index_deltas
   */
  METADATA_0_0_2(MetadataVersion.V_002.versionBytes, byteArrayOf('p', 'r', 'm', '\u0000'), MetadataVersion.V_002) {
    ...
    private fun writeMetadataSection(os: OutputStream, profileData: List<Map.Entry<DexFile, DexFileData>>, apkName: String) {
      val out = ByteArrayOutputStream()
      var expectedSize = 0
      out.use {
        profileData.onEachIndexed { index, entry ->
    ...
    override fun read(src: InputStream): Map<DexFile, DexFileData> {
      src.use {
        ...
        val contents = src.readCompressed(compressed.toInt(), uncompressed.toInt())
        contents.inputStream().use { input ->
          val indexedDexFileData: Array<MutableDexFileData> =
            Array(dexFileCount) {
              ...
              input.readIntsAsDeltas(dexFileData.classIdSetSize, dexFileData.classIdSet)
              dexFileData
            }

          return indexedDexFileData.associate { data -> data.dexFile to data.asDexFileData() }
        }
      }
    }
   ...
   *
   * [profile_header, zipped[[dex_data_header1, dex_data_header2...],[dex_data1, dex_data2...], global_aggregation_count]] profile_header:
   * magic,version,number_of_dex_files,uncompressed_size_of_zipped_data,compressed_data_size dex_data_header:
   * dex_location,number_of_classes,methods_region_size,dex_location_checksum,num_method_ids dex_data:
   * method_encoding_1,method_encoding_2...,class_id1,class_id2...,startup/post startup bitmap, aggregation_counters_for_classes,
   * aggregation_counters_for_methods. The method_encoding is: method_id,number_of_inline_caches,inline_cache1,inline_cache2... The
   * inline_cache is: dex_pc,[M|dex_map_size], dex_profile_index,class_id1,class_id2...,dex_profile_index2,... dex_map_size os the number of
   * dex_indices that follows. Classes are grouped per their dex files and the line
   * `dex_profile_index,class_id1,class_id2...,dex_profile_index2,...` encodes the mapping from `dex_profile_index` to the set of classes
   * `type_id1,type_id2...` M stands for megamorphic or missing types and it's encoded as either the byte kIsMegamorphicEncoding or
   * kIsMissingTypesEncoding. When present, there will be no class ids following. The aggregation_counters_for_classes is stored only for
   * 5.0.0 version and its format is: num_classes,count_for_class1,count_for_class2.... The aggregation_counters_for_methods is stored only
   * for 5.0.0 version and its format is: num_methods,count_for_method1,count_for_method2.... The aggregation counters are sorted based on
   * the index of the class/method.
   */
  V0_1_0_P(byteArrayOf('0', '1', '0', '\u0000')) {
    ...
    private fun createCompressibleBody(profileData: List<Map.Entry<DexFile, DexFileData>>, apkName: String): ByteArray {
      ...
      // Write dex file line headers.
      for ((dexFile, dexFileData) in profileData) {
        dataBos.writeLineHeader(dexFile, apkName, dexFileData, dexFileToHotMethodRegionSize[dexFile]!!)
      }

      // Write dex file data.
      for ((dexFile, dexFileData) in profileData) {
        dataBos.writeLineData(dexFile, dexFileData)
      }
   ...
   * =============================================
   *
   * Serialization format: magic,version,number_of_dex_files,uncompressed_size_of_zipped_data,compressed_data_size,
   * zipped[dex_location1,number_of_classes1,methods_region_size,dex_location_checksum1 num_method_ids,
   * method_encoding_11,method_encoding_12...,type_id1,type_id2... startup/post startup bitmap,
   * dex_location2,number_of_classes2,methods_region_size,dex_location_checksum2, num_method_ids,
   * method_encoding_21,method_encoding_22...,,type_id1,type_id2... startup/post startup bitmap, .....] The method_encoding is:
   * method_id,number_of_inline_caches,inline_cache1,inline_cache2... The inline_cache is: dex_pc,[M|dex_map_size],
   * dex_profile_index,class_id1,class_id2...,dex_profile_index2,... dex_map_size is the number of dex_indices that follows. Classes are
   * grouped per their dex files and the line `dex_profile_index,class_id1,class_id2...,dex_profile_index2,...` encodes the mapping from
   * `dex_profile_index` to the set of classes `class_id1,class_id2...` M stands for megamorphic or missing types and it's encoded as either
   * the byte kIsMegamorphicEncoding or kIsMissingTypesEncoding. When present, there will be no class ids following.
   */
  V0_0_9_OMR1(byteArrayOf('0', '0', '9', '\u0000')) {
    ...
    private fun InputStream.readUncompressedBody(numberOfDexFiles: Int): Map<DexFile, DexFileData> {
      ...
      return dexFileData.associate { it.dexFile to it.asDexFileData() }
    }
  },
  /**
   ...
   * =============================================
   *
   * magic,version,number_of_dex_files dex_location1,number_of_classes1,methods_region_size,dex_location_checksum1, \
   ...
   * method_encoding_21,method_encoding_22...,,type_id1,type_id2... ..... The method_encoding is:
   * method_id,number_of_inline_caches,inline_cache1,inline_cache2... The inline_cache is: dex_pc,[M|dex_map_size],
   * dex_profile_index,class_id1,class_id2...,dex_profile_index2,... dex_map_size is the number of dex_indices that follows. Classes are
   * grouped per their dex files and the line `dex_profile_index,class_id1,class_id2...,dex_profile_index2,...` encodes the mapping from
   * `dex_profile_index` to the set of classes `class_id1,class_id2...` M stands for megamorphic or missing types and it's encoded as either
  ...
  V0_0_5_O(byteArrayOf('0', '0', '5', '\u0000')) {
    ...
    override fun read(src: InputStream): Map<DexFile, DexFileData> =
      with(src) {
        ...
        // Load data for each discovered dex file.
        dexFileData.map { it.dexFile to it.asDexFileData() }.toMap()
      }
   ...
   * =============================================
   *
   * magic,version,number_of_lines dex_location1,number_of_methods1,number_of_classes1,dex_location_checksum1, \
   * method_id11,method_id12...,class_id1,class_id2... dex_location2,number_of_methods2,number_of_classes2,dex_location_checksum2, \
      ...
      with(os) {
        ...
        for ((dex, data) in profileData.toSortedMap(DexFile)) {
          ...
          if (data.classIndexes.isEmpty() && data.typeIndexes.isNotEmpty()) {
            error("Attempting to write a 0.0.1 profile with type ids and no class ids." + " This is likely an error.")
          }
   ...
   *
   * Note that we allow dex files without any methods or classes, so that inline caches can refer to valid dex files.
   *
   * @param dexFile the dex file to which the data belongs
   * @param dexFileData the dex data that should be serialized
   */
  internal fun OutputStream.writeLineData(dexFile: DexFile, dexFileData: DexFileData) {
   ...
   */
  internal fun OutputStream.writeMethodsWithInlineCaches(dexFileData: DexFileData) {
   ...
   * @param dexFileData the dex data that should be serialized
   */
  internal fun OutputStream.writeMethodBitmap(dexFile: DexFile, dexFileData: DexFileData) {
   ...
   * @param dexFileData the dex data that should be serialized
   */
  internal fun OutputStream.writeMethodBitmapV015S(methodFlags: Int, dexFile: DexFile, dexFileData: DexFileData) {
    val lastFlag = MethodFlags.LAST_FLAG_REGULAR
    val numMethodIds = dexFile.header.methodIds.size
    val methodBitmapStorageSize = getMethodBitmapStorageSizeV015S(methodFlags, numMethodIds)
  ...
  /** Returns the size needed for the method bitmap storage of the given dex file for Android S+ profile formats. */
   ...
   */
  private fun setMethodBitmapBit(bitmap: ByteArray, flag: Int, methodIndex: Int, dexFile: DexFile) {
  ...
  private fun isValidMethodFlag(methodFlag: Int): Boolean {
    return (isPowerOfTwo(methodFlag) && methodFlag >= MethodFlags.FIRST_FLAG && methodFlag <= MethodFlags.LAST_FLAG_REGULAR)
  }
  ...
  internal fun InputStream.readHotMethodRegion(data: MutableDexFileData, skipInlineCaches: (src: InputStream) -> Unit) {
    val expectedBytesAvailableAfterRead = available() - data.hotMethodRegionSize
    var lastMethodIndex = 0
  ...
  internal fun InputStream.readMethodBitmapV015S(methodFlags: Int, data: MutableDexFileData) {
    val methodBitmapStorageSize = getMethodBitmapStorageSizeV015S(methodFlags, data.numMethodIds)
    val methodBitmap = read(methodBitmapStorageSize)
    val bs = BitSet.valueOf(methodBitmap)
    val lastFlag = MethodFlags.LAST_FLAG_REGULAR
    var offset = 0
  ...
  private fun BitSet.readFlagsFromBitmap(methodIndex: Int, numMethodIds: Int): Int {
    var result = 0
    val lastFlag = MethodFlags.LAST_FLAG_REGULAR
    var flag = MethodFlags.FIRST_FLAG
    while (flag <= lastFlag) {
  ...
  private fun BitSet.readFlagsFromBitmapV015S(methodFlags: Int, methodIndex: Int, numMethodIds: Int): Int {
    var result = 0
    val lastFlag = MethodFlags.LAST_FLAG_REGULAR
    var flag = MethodFlags.FIRST_FLAG
    while (flag <= lastFlag) {
