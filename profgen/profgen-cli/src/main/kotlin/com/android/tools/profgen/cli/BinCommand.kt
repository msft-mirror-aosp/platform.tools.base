...
class BinCommand : Subcommand("bin", "Generate Binary Profile") {
  ...
  val artProfileFormat by
    option(ArgType.Choice<ArtProfileFormat>(), "profile-format", "pf", "The ART profile format version").default(ArtProfileFormat.V0_1_0_P)
  ...
  override fun execute() {
    ...
    if (metaFile != null) {
      require(metaFile.parentFile.exists()) { "Directory does not exist: ${metaFile.parent}" }
    }
...
class ExtractProfileCommand : Subcommand("extractProfile", "Extract Binary Profile as versioned dex metadata") {
  ...
  private val artProfileFormat by
    option(ArgType.Choice<ArtProfileFormat>(), "profile-format", "pf", "The ART profile format version").default(ArtProfileFormat.V0_1_0_P)
