...
class NetworkStubWrapper(val networkStub: NetworkServiceGrpc.NetworkServiceBlockingStub) {
  ...
  /** Fetch an http range that have at least one element in it. This method will block until a non-empty range is returned. */
  fun getNonEmptyHttpRange(session: Common.Session): NetworkProfiler.HttpRangeResponse {
    return TestUtils.waitForAndReturn(createHttpRangeSupplier(session), Predicate { result -> result.dataList.size > 0 })
  }

  fun createHttpRangeSupplier(session: Common.Session): Supplier<NetworkProfiler.HttpRangeResponse> {
    return Supplier {
      networkStub.getHttpRange(
        NetworkProfiler.HttpRangeRequest.newBuilder().setSession(session).setStartTimestamp(0L).setEndTimestamp(Long.MAX_VALUE).build()
      )
    }
  }

  fun getHttpDetails(connId: Long, type: NetworkProfiler.HttpDetailsRequest.Type): NetworkProfiler.HttpDetailsResponse {
    return networkStub.getHttpDetails(NetworkProfiler.HttpDetailsRequest.newBuilder().setConnId(connId).setType(type).build())
  }

  /**
   * Returns the payload id for the requested [connId].
   *
   * This operation may block, because it takes some time after connections are first tracked before a payload ID is generated.
   */
