 ...
 *
 * An [AgpTestSuite] can run against a single or multiple variants. Users should use [com.android.build.api.dsl.AgpTestSuite.targetVariants]
 * to identify the final list of variants that will be tested.
 *
 * Although this is not strictly necessary, if the test suite has source code, it will be recompiled for each variant it targets. This is to
 * ensure the compatibility of the test suite with each variant individually.
 *
 * Defines how different types of source files are organized and declared for a test suite within the Android Gradle Plugin. Test suites can
 * incorporate various kinds of sources, each serving a distinct purpose and following specific conventions.
 *
 * All sources for a given test suite (e.g., one named `myTestSuite`) are typically organized under the `src/myTestSuite/` directory,
 * relative to the module's root. Within this directory, specific source types will reside in conventional subdirectories.
 *
 * Three primary types of sources can be associated with a test suite using the methods on this interface:
 ...
 *   sources.
 * - Currently, it is not possible to declare the same type of source (e.g., two `hostTest` blocks) for a single test suite.
 * - Future versions of this API may introduce more flexibility, such as allowing multiple instances of the same source type or providing
 *   ways to customize the default directory names.
 *
 * Test execution engines can retrieve the locations or outputs of these configured sources through properties defined in
 * `com.android.build.api.variant.AgpTestSuiteInputParameters` (e.g., `STATIC_FILES`, `TEST_CLASSES`, `TESTED_APKS`). *
 */
...
interface AgpTestSuite : TestSuite {
  ...
  /**
   * Spec to identify the test engine that will be used to run this test suite. Do not call this method if the test suite should use a
   * dedicated test task. Calling this method will direct AGP to create a [org.gradle.api.tasks.testing.Test] task and configure it using
   * the returned [JUnitEngineSpec]
   */
  @get:Incubating val useJunitEngine: JUnitEngineSpec

  /** Specifies properties for the JUnit test engines to run in this test suite */
  @Incubating fun useJunitEngine(action: JUnitEngineSpec.() -> Unit)
   ...
   */
  @Incubating fun assets(action: TestSuiteAssetsSpec.() -> Unit)
   ...
   *
   * The configuration block can use the [action]'s context parameter to disambiguate between each [Test] task instance.
   *
   * Do not make assumption about how AGP decides to allocate [Test] task instances per device, as each AGP version can potentially change
   * it in future release, always use the context object to determine what the [Test] task applies to.
   *
   ...
   */
  @get:Incubating @set:Incubating var codeCoverage: Boolean
}
