...
interface VariantOutput : VariantOutputConfiguration {
   ...
   *
   * This will be initialized with the variant's merged flavor value, or it will be read from the manifest source file if it's not set via
   * the DSL, or it will be null if it's also not set in the manifest.
   *
