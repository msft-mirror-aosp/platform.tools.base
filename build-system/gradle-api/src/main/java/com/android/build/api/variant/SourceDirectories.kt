 ...
 *
 * [SourceDirectories] and subtypes are available for the variant instances through the [Sources]
 * interface. They are also used in [TestSuite] through the [TestSuiteSourceSet] interface.
 *
 * Adding directories to instances returned by the [Sources] will alter the underlying component.
 * Similarly, adding directories to instances returned by the [TestSuiteSourceSet] will change the
 * underlying test suite. However, since a test suite can apply to multiple variants through the
 * [com.android.build.api.dsl.AgpTestSuite.targetVariants] dsl element, modifying the test suite
 * affects all variants that the test suite targets.
 *
...
interface SourceDirectories: Named {
     ...
     * manager.
     *
     * The [Directory] is added last to the receiver's list of source directories. In case there is
     ...
     *
     * The directory will be added last in the receiver's list of source folders. In case there
    ...
    /**
     * Represents all the source folders for a source type.
     *
