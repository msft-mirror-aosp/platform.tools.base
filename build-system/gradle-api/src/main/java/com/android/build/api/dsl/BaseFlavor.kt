...
interface BaseFlavor : VariantDimension {
  ...
  /**
   * The minimum SDK version. Setting this it will override previous calls of [minSdk] and [minSdkPreview] setters. Only one of [minSdk] and
   * [minSdkPreview] should be set.
   *
  ...
  @Deprecated("Will be removed in AGP 10.0, replaced with the minSdk block") fun minSdkVersion(minSdkVersion: String?)
  ...
  /**
   * Whether the renderscript code should be compiled to generate C/C++ bindings. True for C/C++ generation, false for Java, null if not
   * specified.
   */
   ...
   *
   * This is a fully qualified class name of the runner, e.g. `androidx.test.runner.AndroidJUnitRunner`
   *
  ...
  @Deprecated("Replaced by testInstrumentationRunnerArguments property") fun testInstrumentationRunnerArguments(args: Map<String, String>)
   ...
   * to keep.
   *
   * For example, if you are using a library that includes language resources (such as AppCompat or Google Play Services), then your APK
   * includes all translated language strings for the messages in those libraries whether the rest of your app is translated to the same
   * languages or not. If you'd like to keep only the languages that your app officially supports, you can specify those languages using the
   * `resourceConfigurations` property, as shown in the sample below. Any resources for languages not specified are removed.
   *
   * ````
   * android {
   *     defaultConfig {
   ...
   * Gradle packages all string resources your app and its dependencies provide.
   *
