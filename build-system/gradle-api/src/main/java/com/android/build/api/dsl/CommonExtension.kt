...
interface CommonExtension : ExtensionAware {
   ...
   */
  @Suppress("DEPRECATION") @Deprecated("Replaced by installation", replaceWith = ReplaceWith("installation")) val adbOptions: AdbOptions
   ...
   *
   * Unlike using [ProductFlavor] to create different versions of your project that you expect to co-exist on a single device, build types
   * determine how Gradle builds and packages each version of your project. Developers typically use them to configure projects for various
   * stages of a development lifecycle. For example, when creating a new project from Android Studio, the Android plugin configures a
   * 'debug' and 'release' build type for you. By default, the 'debug' build type enables debugging options and signs your APK with a
   * generic debug keystore. Conversely, The 'release' build type strips out debug symbols and requires you to
   ...
   */
  @Deprecated("Renamed to packaging", replaceWith = ReplaceWith("packaging")) val packagingOptions: Packaging
   ...
   *
   * When configuring product flavors with Android plugin 3.0.0 and higher, you must specify at least one flavor dimension, using the
   * [`flavorDimensions`][flavorDimensions] property, and then assign each flavor to a dimension. Otherwise, you will get the following
   * build error:
   * ```
   ...
   * ```
   *
   * By default, when you specify only one dimension, all flavors you configure automatically belong to that dimension. If you specify more
   * than one dimension, you need to manually assign each flavor to a dimension, as shown in the sample below.
   *
   * Flavor dimensions allow you to create groups of product flavors that you can combine with flavors from other flavor dimensions. For
   * example, you can have one dimension that includes a 'free' and 'paid' version of your app, and another dimension for flavors that
   * support different API levels, such as 'minApi21' and 'minApi24'. The Android plugin can then combine flavors from these
   * dimensions—including their settings, code, and resources—to create variants such as 'debugFreeMinApi21' and 'releasePaidMinApi24', and
   * so on. The sample below shows you how to specify flavor dimensions and add product flavors to them.
   *
   * ```
   * android {
   *     ...
   ...
   * ```
   *
   * To learn more, read [Combine multiple flavors](https://developer.android.com/studio/build/build-variants.html#flavor-dimensions).
   */
   ...
   *
   * Use this to specify a fixed NDK version. Without this, each new version of the Android Gradle Plugin will choose a specific version of
   * NDK to use, so upgrading the plugin also means upgrading the NDK. Locking to a specific version can increase repeatability of the
   * build.
   *
   * ```
   * android {
   ...
   * ```
   *
   * If this field is specified then android.ndkPath may not be specified. If this field is not specified, a default value will be provided
   * depending on the AGP version
   *
   * The required format of the version is <code>major.minor.build</code>. It's not legal to specify less precision. If `ndk.dir` is
   * specified in `local.properties` file then the NDK that it points to must match the `android.ndkVersion`.
   *
   * Prior to Android Gradle Plugin version 3.5, the highest installed version of NDK will be used. In Android Gradle Plugin 3.4, specifying
   * `android.ndkVersion` was not an error, but the value would be ignored. Prior to Android Gradle Plugin version 3.4, it was illegal to
   * specify `android.ndkVersion`.
   *
   ...
   * ```
   *
   * If this field is specified, then the version it refers to need to match the one specified in ndkVersion. If ndkVersion is not
   * specified, ndkPath need to match the default ndkVersion
   *
   * For additional information about NDK installation see
   * [Install and configure the NDK](https://developer.android.com/studio/projects/install-ndk).
   *
  ...
  @Deprecated("Replaced by buildToolsVersion property") fun buildToolsVersion(buildToolsVersion: String)

  /**
   * Includes the specified library to the classpath.
   *
   * You typically use this property to support optional platform libraries that ship with the Android SDK. The following sample adds the
   * Apache HTTP API library to the project classpath:
   * ```
   * android {
   ...
   * ```
   *
   * To include libraries that do not ship with the SDK, such as local library modules or binaries from remote repositories,
   * [add the libraries as dependencies](https://developer.android.com/studio/build/dependencies.html) in the `dependencies` block. Note
   * that Android plugin 3.0.0 and later introduce
   * [new dependency configurations](https://developer.android.com/studio/build/gradle-plugin-3-0-0-migration.html#new_configurations). To
   * learn more about Gradle dependencies, read
   ...
   * To include libraries that do not ship with the SDK, such as local library modules or binaries from remote repositories,
   * [add the libraries as dependencies]("https://developer.android.com/studio/build/dependencies.html) in the `dependencies` block. Note
   * that Android plugin 3.0.0 and later introduce [new dependency configurations](new dependency configurations). To learn more about
   * Gradle dependencies, read [Dependency Management Basics](https://docs.gradle.org/current/userguide/artifact_dependencies_tutorial.html)
   *
   * @param name the name of the library.
   * @param required if using the library requires a manifest entry, the entry will indicate that the library is not required.
   */
  ...
  @Deprecated("Will be removed in AGP 10.0, replaced by the compileSdk block") fun compileSdkVersion(version: String)
