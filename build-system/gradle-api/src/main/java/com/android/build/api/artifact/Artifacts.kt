 ...
 *
 * Artifacts are temporary or final files or directories that are produced by the Android Gradle plugin during the build. Depending on its
 * configuration, each [com.android.build.api.variant.VariantBuilder] produces different versions of some of the output artifacts.
 *
 * An example of temporary artifacts are .class files obtained from compiling source files that will eventually get transformed further into
 * dex files. Final artifacts are APKs and bundle files that are not transformed further.
 *
 * Artifacts are uniquely defined by their [Artifact] type and public artifact types that can be accessed from third-party plugins or build
 * script are defined in [SingleArtifact]
 */
interface Artifacts {
  ...
  fun <FileTypeT : FileSystemLocation> add(type: MultipleArtifact<FileTypeT>, artifact: FileTypeT)

  /**
   * Add existing directory of type [MultipleArtifactT] to be combined with project artifact. Do not use this for generated files, use the
   * [use] API instead.
   *
   * @param type type of the multiple artifact.
   * @param inputLocation is an existing static file
   */
  fun <MultipleArtifactT> addStaticDirectory(type: MultipleArtifactT, inputLocation: Directory)
    where MultipleArtifactT : MultipleArtifact<Directory>, MultipleArtifactT : Artifact.Appendable

  /**
   * Access [Task] based operations.
   *
   * @param taskProvider The [TaskProvider] for the [TaskT] that will be producing and/or consuming artifact types.
  ...
  /**
   * Some artifacts do not have a single origin (like compiled from source code). Some artifacts can be obtained from a combination of
   * [Task]s running or incoming dependencies. For example, classes used for dexing can come from compilation related tasks as well as .aar
   * or .jar files expressed as a project dependency.
   *
   * Therefore, these artifacts values can have a scope like [ScopedArtifacts.Scope.PROJECT] for values directly produced by this module (as
   * a [Task] output most likely). Alternatively, the [ScopedArtifacts.Scope.ALL] adds all incoming dependencies (including transitive ones)
