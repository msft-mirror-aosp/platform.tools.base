...
interface OutOperationRequest<FileTypeT : FileSystemLocation> {
   ...
   *
   * This name will only be used when [toAppendTo] or [toCreate] is invoked. Although you can use this method on [Directory], it will most
   * likely not influence how the files contained in the resulting [Directory] are packaged in the APK or Bundle. This method is usually
   * only useful for [RegularFile]
   *
   * Some [Artifact] have a [Artifact.name] which cannot be changed. When invoking [toAppendTo] or [toCreate] after this method will throw
   * an exception if trying to set such a file or directory name when the target's [Artifact] [Artifact.name] is not null.
   *
   ...
   *
   * A creation request does not care about the existing producer, since it replaces the existing producer. Therefore, the existing producer
   * task will not execute (unless it produces other outputs). Please note that when such replace requests are made, the [Task] will replace
   * initial AGP providers.
   *
   * You cannot replace the [Artifact.Multiple] artifact type; therefore, you must instead combine it using the
   * [TaskBasedOperation.wiredWith] API.
   *
   ...
   * ```
   */
  fun <ArtifactTypeT> toCreate(type: ArtifactTypeT) where ArtifactTypeT : Artifact.Single<FileTypeT>, ArtifactTypeT : Artifact.Replaceable

  /**
   * Initiates a reactive request where the [Task] used in the [Artifacts.use] method will be executed once the final version of the [type]
   * artifact has been produced.
   *
   * The final version artifact will be injected in the [Task].
   *
   * Remember that an artifact is not always produced, for instance, an artifact related to minification will not be produced unless
   ...
   * ```kotlin
   *
   * val taskProvider= projects.tasks.register(MyTask::class.java, "verifyManifestTask") artifacts.use(taskProvider)
   * .wiredWith(MyTask::mergedManifestFile) .toListenTo(SingleArtifact.MERGED_MANIFEST)
   *
   * ```
   *
   ...
   */
  fun <ArtifactTypeT> toListenTo(type: ArtifactTypeT) where ArtifactTypeT : Artifact.Single<FileTypeT>
}
...
interface MultipleArtifactTypeOutOperationRequest<FileTypeT : FileSystemLocation> {
   ...
   * ```kotlin
   *
   * val taskProvider= projects.tasks.register(MyTask::class.java, "verifyKeepProguardTask") artifacts.use(taskProvider)
   * .wiredWithMultiple(MyTask::proguardFiles) .toListenTo(MultipleArtifact.MULTIDEX_KEEP_PROGUARD)
   *
   * ```
   *
   * [Task] will be registered as a finalizer task for the final producer of the [type]
   ...
   */
  fun <ArtifactTypeT> toListenTo(type: ArtifactTypeT) where ArtifactTypeT : Artifact.Multiple<FileTypeT>
}
...
interface InAndOutFileOperationRequest {
   ...
   *
   * @param type The [Artifact] identifying the artifact to transform. The [Artifact]'s [Artifact.kind] must be [Artifact.FILE].
   *
...
interface CombiningOperationRequest<FileTypeT : FileSystemLocation> {
   ...
   *
   * The implementation of the task must combine all the inputs into a single output. Chained transforms will get a [ListProperty]
   * containing the single output from the upstream transform.
   *
   * If some [append] calls are made on the same artifact type, the first transform will always get the complete list of artifacts
   * irrespective of the timing of the calls.
   *
...
interface InAndOutDirectoryOperationRequest<TaskT : Task> {
   ...
   * Initiates a transform request to a single [Artifact.Transformable] artifact type.
   *
   * @param type The [Artifact] identifying the artifact to transform. The [Artifact]'s [Artifact.kind] must be [Artifact.DIRECTORY]
   *
   * The artifact type must be [Artifact.Single] and [Artifact.Transformable].
   *
