...
sealed class SingleArtifact<T : FileSystemLocation>(
  ...
  /**
   * Directory where APK files will be located. Some builds can be optimized for testing when invoked from Android Studio. In such cases,
   * the APKs are not suitable for deployment to Play Store.
   */
  object APK : SingleArtifact<Directory>(DIRECTORY), ContainsMany, Replaceable, Transformable
  ...
  object OBFUSCATION_MAPPING_FILE : SingleArtifact<RegularFile>(FILE, Category.OUTPUTS, "mapping.txt"), Replaceable, Transformable {
  ...
  object BUNDLE : SingleArtifact<RegularFile>(FILE, Category.OUTPUTS), Transformable

  /** The final AAR file as it would be published. */
  object AAR : SingleArtifact<RegularFile>(FILE, Category.OUTPUTS), Transformable
   ...
   */
  @Incubating object METADATA_LIBRARY_DEPENDENCIES_REPORT : SingleArtifact<RegularFile>(FILE), Replaceable, Transformable
   ...
   * APKs. Build creates a bundle file first and then generates Universal APK from it.
   *
   * It's <i>not efficient</i> to use [APK_FROM_BUNDLE] because of size and because it creates a Bundle (.aab) file first and finally
   * extracts the APK from it. These steps will slow your build flow. Thus, unless your intent is to check the universal APK as produced
   * from a .aab file, prefer [APK].
   */
  @Incubating object APK_FROM_BUNDLE : SingleArtifact<RegularFile>(FILE, Category.OUTPUTS)

  /**
   * The directory containing all the native library (.so) files that will be packaged in the APK, AAR, or Bundle.
   *
   * The native libraries in this directory have not yet been stripped during the current build (though some of them might already be
   * stripped if they are from remote dependencies).
   */
  ...
  @Incubating object RUNTIME_SYMBOL_LIST : SingleArtifact<RegularFile>(FILE)
