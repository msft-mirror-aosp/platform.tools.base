 ...
 *
 * For example `AndroidPluginVersion(7, 0)` refers to Android Gradle Plugin version 7.0.0, and `AndroidPluginVersion(7, 0, 1)` refers to
 ...
 *
 * Also supports preview versions through methods [alpha], [beta] and [rc] that return the more specific version. For example
 * `AndroidPluginVersion(7, 0).alpha(5)` refers to Android Gradle Plugin version 7.0.0-alpha05. This is for use when developing using
 * incubating APIs that have changed between preview releases of the Android Gradle Plugin. Once those APIs are stable in a stable version
 * of Android Gradle Plugin, it's recommended to drop support for the preview versions. For example, if a new API was introduced in
 * 7.0.0-alpha05, you can test for that using
 *
 ...
 *
 * Take care with maximum version checks to avoid excluding point releases. For example, if a behavior is only applicable for AGP 8.7
 * series, and you have a different code path for the following version, don't check for 8.7.0 as the maximum, as your plugin will break on
 * point releases of 8.7.0, instead compare with an exclusive bound with the first alpha of the subsequent major-minor version. (This
 * applies even if the alpha is never released as it turned out to be a major version)
 *
...
private constructor(
      ...
      .apply {
        ...
        when (_previewType) {
          ...
          PreviewType.DEV -> append("-dev")
            ...
            val isTwoDigitPreviewFormat =
              major > 3 || major == 3 && minor > 1 || major == 3 && minor == 1 && micro == 0 && _previewType != PreviewType.BETA
  ...
  companion object {
    ...
    fun getCurrent(): AndroidPluginVersion {
      val className = "com.android.build.api.extension.impl.CurrentAndroidGradlePluginVersion"
      try {
