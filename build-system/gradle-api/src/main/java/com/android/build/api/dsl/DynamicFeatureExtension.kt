...
interface DynamicFeatureExtension : CommonExtension, ApkExtension, TestedExtension {
   ...
   *
   * Unlike using [DynamicFeatureProductFlavor] to create different versions of your project that you expect to co-exist on a single device,
   ...
   * configures a 'debug' and 'release' build type for you. You can then combine build types with product flavors to
   ...
   *
   * When you configure product flavors, the Android plugin automatically combines them with your [DynamicFeatureBuildType] configurations
   * to [create build variants](https://developer.android.com/studio/build/build-variants.html). If the plugin creates certain build
   * variants that you don't want, you can
   ...
   * Encapsulates all product flavors configurations for this project.
   *
   * For more information about the properties you can configure in this block, see [DynamicFeatureProductFlavor]
   */
