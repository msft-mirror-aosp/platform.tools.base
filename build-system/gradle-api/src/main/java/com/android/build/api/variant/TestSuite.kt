...
interface TestSuite : Named {
  ...
  /**
   * Return the list of sourcesets associated with this test suite. Sourcesets are added through the DSL using one of the following methods:
   ...
   *
   * The returned [Collection] of [TestSuiteSourceSet] is not specific to this variant but to the related [TestSuite] instance, which may
   * apply to multiple variants as specified through the [com.android.build.api.dsl.AgpTestSuite.targetVariants] API.
   */
  @get:Incubating val sources: Collection<TestSuiteSourceSet>

  /**
   * Configures the test tasks for this test target.
   *
   * There can be one to many instances of [org.gradle.api.tasks.testing.Test] tasks for a particular test suite target. For instance, if
   * the test suite targets more than one device, AGP may decide to create one [org.gradle.api.tasks.testing.Test] instance per device.
   *
   * The configuration block can use the [action]'s context parameter to disambiguate between each [org.gradle.api.tasks.testing.Test] task
   * instance.
   *
   * Do not make assumption about how AGP decides to allocate [org.gradle.api.tasks.testing.Test] task instances per device, as each AGP
   * version can potentially change it in future release, always use the context object to determine what the
   * [org.gradle.api.tasks.testing.Test] task applies to.
   *
   ...
   */
  @Incubating fun instrumentationRunner(source: TestSuiteSourceSet.TestApk): Provider<String>
}
