...
interface BuildType : Named, VariantDimension, ExtensionAware {
   ...
   * ```
   *
   * isTestCoverageEnabled is deprecated. Use enableUnitTestCoverage and enableAndroidTestCoverage instead which allow you to enable
   * coverage separately for unit and instrumentation tests.
   */
  @Deprecated("Replaced with: enableUnitTestCoverage and or enableAndroidTestCoverage.") var isTestCoverageEnabled: Boolean
   ...
   *
   * If enabled, prepares module class files for code coverage collection such as instrumenting dependent library classes and module
   * classes. This allows for code coverage reports to be generated.
   */
   ...
   *
   * A pseudolocale is a locale that simulates characteristics of languages that cause UI, layout, and other translation-related problems
   * when an app is localized. Pseudolocales can aid your development workflow because you can test and make adjustments to your UI before
   * you finalize text for translation.
   *
   * When you set this property to `true` as shown below, the plugin generates resources for the following pseudo locales and makes them
   * available in your connected device's language preferences: `en-XA` and `ar-XB`.
   *
   * ```
   * android {
   *     buildTypes {
   ...
   * ```
   *
   * When you build your app, the plugin includes the pseudolocale resources in your APK. If you notice that your APK does not include those
   * locale resources, make sure your build configuration isn't limiting which locale resources are packaged with your APK, such as using
   ...
   *
   * To learn more, read [Test Your App with Pseudolocales](https://d.android.com/guide/topics/resources/pseudolocales.html).
   */
  ...
  /**
   * Optimization level to use by the renderscript compiler. RenderScript APIs are deprecated starting in Android 12. For more information
   * about how to migrate, see [https://developer.android.com/guide/topics/renderscript/migrate]
   */
   ...
   * not possible.
   *
   * Android plugin 3.0.0 and higher try to match each variant of your module with the same one from its dependencies. For example, when you
   * build a "freeDebug" version of your app, the plugin tries to match it with "freeDebug" versions of the local library modules the app
   * depends on.
   *
   * However, there may be situations in which **your app includes build types that a dependency does not**. For example, consider if your
   * app includes a "stage" build type, but a dependency includes only a "debug" and "release" build type. When the plugin tries to build
   * the "stage" version of your app, it won't know which version of the dependency to use, and you'll see an error message similar to the
   * following:
   * ```
   ...
   * ```
   *
   * Note that there is no issue when a library dependency includes a build type that your app does not. That's because the plugin simply
   * never requests that build type from the dependency.
   *
  ...
  @Deprecated("Replaced with property matchingFallbacks") fun setMatchingFallbacks(vararg fallbacks: String)

  @Deprecated("Replaced with property matchingFallbacks") fun setMatchingFallbacks(fallbacks: List<String>)
  ...
  @get:Incubating val vcsInfo: VcsInfo

  @Incubating fun vcsInfo(action: VcsInfo.() -> Unit)
}
