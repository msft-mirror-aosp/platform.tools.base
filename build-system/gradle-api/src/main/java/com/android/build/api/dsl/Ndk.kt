...
interface Ndk {
  /** The module name */
  @Deprecated("Specify module name in CMakeLists.txt or Application.mk") var moduleName: String?
   ...
   * of creating one large APK with all versions of your native libraries, Gradle creates a separate APK for each ABI you want to support
   * and only packages the files each ABI needs.
   */
  val abiFilters: MutableSet<String>

  /** The APP_STL value */
  @Deprecated("Specify stl name in CMakeLists.txt or Application.mk") var stl: String?

  /** Number of parallel threads to spawn. */
  @Deprecated("Use Cmake or NdkBuild DSL instead") var jobs: Int?

  /**
   * The type of debug metadata which will be packaged in the app bundle.
   *
   * <p>Supported values are 'none' (default, no native debug metadata will be packaged), 'symbol_table' (only the symbol tables will be
   * packaged), and 'full' (the debug info and symbol tables will be packaged).
   *
   * <p>Example usage:
   * <pre>
   * android {
