 ...
 * ```
 *
 * Note that depending on the actual implementation of [AndroidComponentsExtension], the object received may be of a child type. For
 * instance [ApplicationAndroidComponentsExtension.onVariants] will pass [ApplicationVariant] to the lambda.
 *
...
interface Variant : Component, HasAndroidResources {
  ...
  /**
   * Variant's cmake [ExternalNativeBuild], initialized by merging the product flavor values or null if no cmake external build is
  ...
  /**
   * Returns an extension object registered via the [VariantBuilder.registerExtension] API or null if none were registered under the passed
   * [type].
   *
   ...
   *
   * This list will be initialized from [com.android.build.api.dsl.VariantDimension#proguardFile] for non test related variants and from
   ...
   */
  @get:Incubating val experimentalProperties: MapProperty<String, Any>
   ...
   * * [AndroidTest] component if this variant [HasDeviceTests] and android tests for this variant are enabled,
   * * [TestFixtures] component if this variant [HasTestFixtures] and test fixtures for this variant are enabled.
   *
   * Use this list to do operations on all nested components of this variant without having to manually check whether the variant has each
   * component.
   *
   * Example:
   * ```kotlin
   ...
   *  ```
   */
  @get:Incubating val nestedComponents: List<Component>
   ...
   *  ```
   */
  @get:Incubating val components: List<Component>
   ...
   * @see [com.android.build.api.dsl.BaseFlavor.missingDimensionStrategy] To learn more, read
   *   [Select default flavors for missing dimensions](d.android.com//build/build-variants).
   */
  @Incubating fun missingDimensionStrategy(dimension: String, vararg requestedValues: String)
}
