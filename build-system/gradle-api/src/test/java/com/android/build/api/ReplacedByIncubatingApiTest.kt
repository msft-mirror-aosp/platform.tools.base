...
class ReplacedByIncubatingApiTest {
  ...
  /**
   * When an incubating API is replacing an existing API, the existing API is not deprecated until the incubating API becomes stable
   * (otherwise, users have to choose between an incubating and a deprecated API).
   *
   * Such replaced APIs are tracked with @ReplacedByIncubating to ensure they become deprecated the day the incubating API graduates.
   *
   * Failing this test usually means that the [ReplacedByIncubating] annotation should be replaced by a [Deprecated] and deprecated-api.txt
   * should be updated.
   */
  @Test
  fun `ReplacedByIncubating references a bug which is present in incubating-api`() {
    val incubatingApis = InputStreamReader(snapshotFileUrl.openStream()).use { it.readText() }
    ...
    classes.forEach { classInfo ->
      ...
      apiTester.getApiElements(apiClass).forEach { apiElement ->
        methods
          .firstOrNull { ApiTester.getApiElement(it) == apiElement }
          ?.let { method ->
  ...
  private fun checkAnnotation(replacedByIncubating: ReplacedByIncubating, incubatingApis: String, memberDescription: String) {
