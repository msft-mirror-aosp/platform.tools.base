...
class AndroidPluginVersionTest {
  ...
  fun `test getVersion`() {
    ...
    assertThat(AndroidPluginVersion(3, 0).alpha(1).version).isEqualTo("3.0.0-alpha1")
    assertThat(AndroidPluginVersion(3, 0).beta(1).version).isEqualTo("3.0.0-beta1")
    assertThat(AndroidPluginVersion(3, 0).rc(1).version).isEqualTo("3.0.0-rc1")
    /** 3.1.0 has zero-padding on alpha and rc but not beta preview versions */
    assertThat(AndroidPluginVersion(3, 1).alpha(1).version).isEqualTo("3.1.0-alpha01")
    assertThat(AndroidPluginVersion(3, 1).beta(1).version).isEqualTo("3.1.0-beta1")
    assertThat(AndroidPluginVersion(3, 1).rc(1).version).isEqualTo("3.1.0-rc01")
    /** 3.2.0 has zero-padding on all preview versions */
    assertThat(AndroidPluginVersion(3, 2).alpha(1).version).isEqualTo("3.2.0-alpha01")
    assertThat(AndroidPluginVersion(3, 2).beta(1).version).isEqualTo("3.2.0-beta01")
    assertThat(AndroidPluginVersion(3, 2).rc(1).version).isEqualTo("3.2.0-rc01")
    /** dev versions */
    assertThat(AndroidPluginVersion(3, 0).dev().version).isEqualTo("3.0.0-dev")
    assertThat(AndroidPluginVersion(3, 1).dev().version).isEqualTo("3.1.0-dev")
  }
  ...
  fun `check stable version validation`() {
    assertThat(assertFailsWith<IllegalArgumentException> { AndroidPluginVersion(-1, 2) })
      .hasMessageThat()
      .isEqualTo("Versions of the Android Gradle Plugin must not be negative")
  ...
  fun `check misleading version use check`() {
    ...
    assertThat(assertFailsWith<IllegalArgumentException> { previewVersion.alpha(1) })
      .hasMessageThat()
      .isEqualTo("alpha(int) only expected to be called on final versions")
    assertThat(assertFailsWith<IllegalArgumentException> { previewVersion.beta(1) })
      .hasMessageThat()
      .isEqualTo("beta(int) only expected to be called on final versions")
    assertThat(assertFailsWith<IllegalArgumentException> { previewVersion.rc(1) })
      .hasMessageThat()
      .isEqualTo("rc(int) only expected to be called on final versions")
  }
}
