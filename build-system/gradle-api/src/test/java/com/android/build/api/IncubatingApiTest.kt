...
class IncubatingApiTest {
  ...
  fun `all elements of incubating classes should be marked as incubating`() {
    val classes =
      ClassPath.from(CommonExtension::class.java.classLoader)
    ...
    val nonIncubatingMembersThatReferenceIncubatingClasses =
      stableClasses
        .flatMap { clazz ->
          clazz.declaredMethods
            .filter { method -> !method.isIncubating && method.referencesTypes.any { incubatingClasses.contains(it) } }
            .map { "${clazz.name}#${it.name}" }
        }
        .toSet()
    expect
      .that(nonIncubatingMembersThatReferenceIncubatingClasses)
      .named("Non incubating members that reference incubating classes")
      .isEmpty()
  }

  @Suppress("unused") // Used by reflection below
  fun exampleMethod(nestedList: List<Map<String, IntArray>>, nestedAction: (Long.() -> Boolean) -> Unit): Set<Int> {
    throw NotImplementedError()
  }

  @Test
  fun `test references types test util method`() {
    val method = IncubatingApiTest::class.java.declaredMethods.single { it.name == "exampleMethod" }
    assertThat(method.referencesTypes)
      .containsExactly(
        List::class.java,
        Map::class.java,
        String::class.java,
  ...
  fun `file does not contain do not submit text`() {
    InputStreamReader(snapshotFileUrl.openStream()).use { assertThat(it.readText()).doesNotContain(DO_NOT_SUBMIT) }
  }
  ...
  companion object {
    ...
    private val snapshotFileUrl = Resources.getResource(DeprecatedApiTest::class.java, "incubating-api.txt")

    internal fun getApiTester(currentKey: String? = null): ApiTester {
      val classes =
        ClassPath.from(Lint::class.java.classLoader).getTopLevelClassesRecursive("com.android.build.api").filter(::filterNonApiClasses)
      return ApiTester(
    ...
    private fun loadClass(name: String): Class<*> = IncubatingApiTest::class.java.classLoader.loadClass(name)

    private val AnnotatedElement.isIncubating
      get() = annotations.any { it.annotationClass == Incubating::class }

    private val Method.referencesTypes: Set<Class<*>>
      get() = setOf(returnType) + genericParameterTypes.flatMap { it.referencesTypes }
    ...
    private val Type.referencesTypes: Set<Class<*>>
      get() =
        when (this) {
          ...
          is WildcardType -> {
            upperBounds.flatMap { it.referencesTypes }.toSet() + lowerBounds.flatMap { it.referencesTypes }
          }
    ...
    // TODO: clean these up
    private val NON_INCUBATING_METHODS_TO_CLEAN_UP =
      listOf(
