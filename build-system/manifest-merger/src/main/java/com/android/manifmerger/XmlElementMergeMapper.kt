...
/**
 * Pairs XML elements from lower-priority and higher-priority manifest XMLs based on their type, their unique key and feature flag
 * attribute.
 *
 * This function facilitates the merging of manifest XMLs by aligning elements from two sources for potential subsequent merging operations.
 * Normally, the manifest XML have elements with unique keys but there are two scenarios where there could elements with identical keys in a
 * manifest: When node type allows multiple declaration or when feature flag is used.
 *
 ...
 * @return A set of pairs, where each pair consists of a lower-priority XML element and an `Optional` containing either the matching
 *   higher-priority element or `Optional.empty()` if there's no match.
 */
...
/**
 * Pairs a given lower-priority xml element's child with children of higher priority xml element with matching node type and key by also
 * considering the feature flag attributes on these xml elements.
 *
