 ...
 * @property sourceFilePosition the source file position of the deep link element in the navigation xml file.
...
data class DeepLink(
  ...
  /**
   * A class representing an RFC 2396 compliant URI, following the same rules as java.net.URI, except also allowing the following deviations
   * in the input uri string:
  ...
  data class DeepLinkUri(
    ...
    companion object {
      ...
      fun fromUri(uri: String): DeepLinkUri {
        ...
        val encodedUri =
          uri
            .replace(DOLLAR_SIGN, dollarSignEncoder)
            ...
            .let {
              if (!Pattern.compile("^[^/]*:/").matcher(it).find() && !it.startsWith("/")) {
                "//" + it
              } else {
                it
              }
        ...
        URI(
          uri
            .replace(MANIFEST_PLACEHOLDER, dollarSignEncoder)
        ...
        val schemes =
          when {
            ...
            PATH_WILDCARD.containsMatchIn(decodedScheme) || wildcardEncoder in decodedScheme || hostWildcardEncoder in decodedScheme ->
              throw DeepLinkException("Improper use of wildcards and/or placeholders in deeplink URI scheme")
      ...
      /**
       * Returns a string which can be used as an encoder in the input uri string; i.e., the encoder can be inserted or substituted anywhere
       * in the uri string and is guaranteed to be unique in the resulting string, allowing accurate decoding.
       *
       * For example, chooseEncoder("www.example.com", 'w', 'x') returns "wwwwx", which, if inserted or substituted in "www.example.com",
       * will remain the only instance of itself in the resulting string. So, e.g., we could encode the "."'s with "wwwwx"'s to yield
       * "wwwwwwwxexamplewwwwxcom", which can be unambiguously decoded back to the original string.
       *
       * @param uri the String which the encoder will be designed to be inserted or substituted into.
       * @param char1 any char
       * @param char2 any char other than char1
       * @return a string which can be used as an encoder in uri. The returned value consists of repeated char1's followed by a single char2
       *   that's not present in the original uri string.
       */
