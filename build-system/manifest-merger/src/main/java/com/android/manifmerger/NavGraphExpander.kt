...
/**
 * Singleton with [expandNavGraphs] method, which returns an XmlDocument with the <nav-graph> elements converted into the corresponding
 * <intent-filter> elements
 */
object NavGraphExpander {

  /**
   * Return an [XmlDocument] with the <nav-graph> elements converted into the corresponding <intent-filter> elements
   *
   * @param xmlDocument the input XmlDocument whose <nav-graph> elements will be converted into <intent-filter> elements in the output
   *   XmlDocument. This method assumes that this XmlDocument matches the underlying XmlDocument.xml; e.g., if <nav-graph> elements have
   *   been added to the DOM document since this XmlDocument was created, the new <nav-graph> elements will not be converted.
   * @param loadedNavigationMap the map of navigationId Strings to NavigationXmlDocuments, which determine the set of DeepLinks used to
   *   generate the <intent-filter> element for each <nav-graph> element
   * @param mergingReportBuilder the MergingReport.Builder used for the entire merge
   * @return a new XmlDocument similar to the input XmlDocument, but with any <nav-graph> elements replaced with corresponding
   *   <intent-filter> elements
   */
  fun expandNavGraphs(
  ...
  /**
   * Helper method for [expandNavGraphs]. This method checks if the given xmlElement is an <activity> element with <nav-graph> children, and
   * if so, it calls the [expandNavGraph] helper method to generate the <intent-filter> elements, and then it finally removes the original
   * <nav-graph> elements.
   */
  private fun expandNavGraphs(
    ...
    for (navGraph in navGraphs) {
      val graphValue: String? = navGraph.getAttributeValue(ANDROID_URI, SdkConstants.ATTR_VALUE)
      val navigationXmlId =
        if (graphValue?.startsWith(SdkConstants.NAVIGATION_PREFIX) == true) graphValue.substring(SdkConstants.NAVIGATION_PREFIX.length)
        else null
      if (navigationXmlId == null) {
        val nsUriPrefix = navGraph.lookupNamespacePrefix(ANDROID_URI, false)
    ...
    for (navGraph in navGraphs) {
      ...
      mergingReportBuilder.actionRecorder.recordNodeAction(navGraph, Actions.ActionType.CONVERTED)
    }
  }

  /**
   * Helper method for [expandNavGraphs]. This method calls [findDeepLinks] to get all the [DeepLink]s associated with a given navigationId,
   * then generates the corresponding <intent-filter> elements.
   */
  private fun expandNavGraph(
    ...
    val sourceFilePosition = SourceFilePosition(xmlElement.document.sourceFile, xmlElement.position)
    val deepLinks =
      try {
    ...
    for (deepLinkGroup in deepLinkGroups.values) {
      ...
      val intentFilterXmlElement = xmlElement.addChildElement(SdkConstants.TAG_INTENT_FILTER)
      ...
      if (deepLink.action.isNotBlank()) {
        childElementDataList.add(ChildElementData(TAG_ACTION, ATTR_NAME, deepLink.action))
      }
      childElementDataList.add(ChildElementData(TAG_CATEGORY, ATTR_NAME, "android.intent.category.DEFAULT"))
      childElementDataList.add(ChildElementData(TAG_CATEGORY, ATTR_NAME, "android.intent.category.BROWSABLE"))
      ...
      if (deepLink.port != -1) {
        childElementDataList.add(ChildElementData(TAG_DATA, ATTR_PORT, deepLink.port.toString()))
      }
      val path = deepLink.path
      when {
        path.substringBefore(".*").length == path.length - 2 ->
          childElementDataList.add(ChildElementData(TAG_DATA, ATTR_PATH_PREFIX, path.substringBefore(".*")))
        path.contains(".*") -> childElementDataList.add(ChildElementData(TAG_DATA, ATTR_PATH_PATTERN, path))
      ...
      childElementDataList.forEach {
        intentFilterXmlElement.addChildElementWithSingleAttribute(it.tagName, ANDROID_URI, it.attrName, it.attrValue)
      }
      // finally record all added elements and attributes
      for (dl in deepLinkGroup) {
        recordXmlElementAddition(intentFilterXmlElement, dl.sourceFilePosition, actionRecorder)
      }
    }
  }
  ...
  fun findDeepLinks(
    ...
    findDeepLinks(navigationXmlId, loadedNavigationMap, deepLinkList, mergingReportBuilder, sourceFilePosition)
    return ImmutableList.copyOf(deepLinkList)
  }

  /**
   * This class is used to hold uri, action, and mimeType of a deep link and is used in [findDeepLinks] below to check for duplicate
   * elements.
   */
  ...
  private fun getDeepLinkUriBody(deepLink: DeepLink, includeQuery: Boolean, includeFragment: Boolean): String {
   ...
   *
   * Not guaranteed to return valid URI's (e.g., if a host is not specified, but a port is), but that's okay because we're only using the
   * returned URI's to check for duplicate [DeepLink]s
   */
  ...
  private fun recordXmlElementAddition(xmlElement: XmlElement, sourceFilePosition: SourceFilePosition, actionRecorder: ActionRecorder) {
    val nodeRecord = Actions.NodeRecord(Actions.ActionType.ADDED, sourceFilePosition, xmlElement.id, null, xmlElement.operationType)
  ...
  private fun recordXmlAttributeAddition(
    ...
    val attributeRecord = Actions.AttributeRecord(Actions.ActionType.ADDED, sourceFilePosition, xmlAttribute.id, null, null)
  ...
  private data class ChildElementData(val tagName: String, val attrName: String, val attrValue: String)
