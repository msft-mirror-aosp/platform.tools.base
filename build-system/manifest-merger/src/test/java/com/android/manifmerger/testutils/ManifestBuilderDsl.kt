...
class Manifest(attr_package: String, includeXmlnsTools: Boolean) : Tag("manifest") {
  ...
  fun application(android_label: String?, android_name: String? = null, init: Application.() -> Unit) =
    initChild(Application(android_label = android_label, android_name = android_name), init)

  fun usesSdk(android_minSdkVersion: String?, android_targetSdkVersion: String?, init: UsesSdk.() -> Unit) =
    initChild(UsesSdk(android_minSdkVersion = android_minSdkVersion, android_targetSdkVersion = android_targetSdkVersion), init)
}
...
class UsesSdk(android_minSdkVersion: String?, android_targetSdkVersion: String?) : Tag("uses-sdk") {
  init {
    ...
    android_targetSdkVersion?.let { attributes["android:minSdkVersion"] = android_targetSdkVersion }
  }
}
...
class Application(android_label: String?, android_name: String?) : Tag("application") {
  ...
  fun activity(android_name: String?, android_exported: Boolean? = null, init: Activity.() -> Unit) =
...
class IntentFilter : Tag("intent-filter") {
  ...
  fun action(android_name: String, init: Action.() -> Unit) = initChild(Action(android_name), init)

  fun category(android_name: String, init: Category.() -> Unit) = initChild(Category(android_name), init)
