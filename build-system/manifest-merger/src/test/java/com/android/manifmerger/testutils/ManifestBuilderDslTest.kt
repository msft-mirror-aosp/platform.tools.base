...
class ManifestBuilderDslTest {
    ...
    val manifest =
      manifest("com.example.test") {
        application(android_label = "Blerg") {
          ...
          activity(android_name = ".MyActivity") { intentFilter { action(android_name = "android.intent.action.MAIN") {} } }
        }
      }
  ...
  fun testAndroidExportedAttributeWithIntentFilterInActivity() {
    ...
    try {
      ...
      assertThat(loggingRecordsString)
        .contains("android:exported needs to be explicitly specified for element <activity#com.example.myapplication.MainActivity>.")
  ...
  fun testToolsAnnotationRemoval() {
    ...
    try {
      ...
      val replace = applications.item(0).attributes.getNamedItemNS(SdkConstants.TOOLS_URI, "replace")
  ...
  fun testNoFqcnsExtraction() {
    ...
    val xmlDocument = parse(mergingReport.getMergedDocument(MergedManifestKind.MERGED))
    assertThat(xmlDocument.getElementsByTagName("activity").asSequence().map { it.getNamedAttributeAndroidNS("name").nodeValue }.toList())
      .containsExactly("com.foo.example.activityOne", "com.foo.bar.example.activityTwo", "com.foo.example.activityThree")
      .inOrder()
    val applicationNode = xmlDocument.getElementsByTagName("application").item(0)
    assertThat(applicationNode.getNamedAttributeAndroidNS("name").nodeValue).isEqualTo("com.foo.example.applicationOne")
    assertThat(applicationNode.getNamedAttributeAndroidNS("backupAgent").nodeValue).isEqualTo("com.foo.example.myBackupAgent")
  }
}
