...
class LocaleConfigTagTest {
  ...
  fun `Value from lib does not override value from app`() {
    val mockLog = MockLog()
    val appManifest =
      manifest("com.example.test.locales") { application(android_label = "Blerg") { attr_android_localeConfig("@xml/locales_config") } }
    val libManifest =
      manifest("com.example.test.locales.lib") {
        application(android_label = "Blerg") { attr_android_localeConfig("@xml/locales_config_from_lib") }
      }
    ...
    try {
      val mergingReport =
        ManifestMerger2.newMerger(appFile, mockLog, ManifestMerger2.MergeType.APPLICATION).addLibraryManifest(libFile).merge()
      assertThat(mergingReport.result).isEqualTo(MergingReport.Result.SUCCESS)
      val document = parse(mergingReport.getMergedDocument(MergingReport.MergedManifestKind.MERGED))
      ...
      assertThat(
          document
            ...
            .item(0)
            .attributes
            .getNamedItemNS("http://schemas.android.com/apk/res/android", "localeConfig")
            .nodeValue
        )
  ...
  fun `Value from lib is never included in merge result`() {
    ...
    try {
      ...
      assertThat(document.getElementsByTagName("application")[0].getNamedAttributeAndroidNS("localeConfig")).isNull()
    } finally {
      assertThat(appFile.delete()).named("appFile was deleted").isTrue()
      assertThat(libFile.delete()).named("libFile was deleted").isTrue()
    }
  ...
  fun `Value from overlay must match value from app`() {
    ...
    val overlayManifest =
      manifest("com.example.test.locales") {
        application(android_label = "Blerg") { attr_android_localeConfig("@xml/locales_config_from_overlay") }
      }
    val appFile = TestUtils.inputAsFile("appFile", appManifest.toString())
  ...
  fun `Value from overlay can override app value via tools_replace`() {
    ...
    try {
      ...
      assertThat(document.getElementsByTagName("application")[0].getNamedAttributeAndroidNS("localeConfig").nodeValue)
