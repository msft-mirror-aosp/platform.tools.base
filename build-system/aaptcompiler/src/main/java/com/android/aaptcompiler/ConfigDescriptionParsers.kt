...
fun parseLayoutDirection(part: String, config: ConfigDescription): Boolean {
  ...
  config.screenLayout = maskAndApply(config.screenLayout, ResTableConfig.SCREEN_LAYOUT.DIR_MASK, value)
  return true
}
...
fun parseScreenLayoutSize(part: String, config: ConfigDescription): Boolean {
  ...
  config.screenLayout = maskAndApply(config.screenLayout, ResTableConfig.SCREEN_LAYOUT.SIZE_MASK, value)
  return true
}
...
fun parseScreenLayoutLong(part: String, config: ConfigDescription): Boolean {
  ...
  // AND existing config with the mask and then OR with the found layout size.
  config.screenLayout = maskAndApply(config.screenLayout, ResTableConfig.SCREEN_LAYOUT.SCREENLONG_MASK, value)
  return true
}
...
fun parseScreenRound(part: String, config: ConfigDescription): Boolean {
  ...
  config.screenLayout2 = maskAndApply(config.screenLayout2, ResTableConfig.SCREEN_LAYOUT2.SCREENROUND_MASK, value)
  return true
}
...
fun parseWideColorGamut(part: String, config: ConfigDescription): Boolean {
  ...
  config.colorMode = maskAndApply(config.colorMode, ResTableConfig.COLOR_MODE.WIDE_GAMUT_MASK, value)
  return true
}
...
fun parseKeysHidden(part: String, config: ConfigDescription): Boolean {
  ...
  config.inputFlags = maskAndApply(config.inputFlags, ResTableConfig.INPUT_FLAGS.KEYSHIDDEN_MASK, value)
  return true
}
...
fun parseNavHidden(part: String, config: ConfigDescription): Boolean {
  ...
  config.inputFlags = maskAndApply(config.inputFlags, ResTableConfig.INPUT_FLAGS.NAVHIDDEN_MASK, value)
  return true
}
...
fun parseVersion(part: String, config: ConfigDescription): Boolean {
  ...
  val sdkVersion = (if (dot == -1) part.substring(1) else part.substring(1, dot)).toIntOrNull()?.toShort() ?: return false
...
fun maskAndApply(current: Byte, mask: Int, value: Byte) = maskAndApply(current.toInt(), mask, value.toInt()).toByte()
