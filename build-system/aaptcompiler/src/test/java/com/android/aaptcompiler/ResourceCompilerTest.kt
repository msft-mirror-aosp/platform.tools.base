...
class ResourceCompilerTest {
  ...
  private fun testRawFile(input: String, extension: String, config: String = ""): File {
    ...
    compileResource(file, outputDir, ResourceCompilerOptions(), getMockBlameLogger(BlameLoggerTest.MockLogger()))
    val filePath = extractPathData(file)
    return File(outputDir, filePath.getIntermediateContainerFilename())
  }
  ...
  fun parseArrayTypes() {
    ...
    Truth.assertThat((stringsArray2.elements[0] as Reference).name).isEqualTo(parseNameOrFail("string/s0"))
    Truth.assertThat((stringsArray2.elements[1] as Reference).name).isEqualTo(parseNameOrFail("string/s1"))
    Truth.assertThat((stringsArray2.elements[2] as Reference).name).isEqualTo(parseNameOrFail("string/s2"))
    ...
    Truth.assertThat((colorsArray.elements[0] as? BinaryPrimitive)?.resValue?.data).isEqualTo(0xffff0000.toInt())
    Truth.assertThat((colorsArray.elements[1] as? BinaryPrimitive)?.resValue?.data).isEqualTo(0xff00ff00.toInt())
    Truth.assertThat((colorsArray.elements[2] as? BinaryPrimitive)?.resValue?.data).isEqualTo(0xff0000ff.toInt())
  }
  ...
  fun testCompileRawFileSupported() {
    ...
    Truth.assertThat(deserializeConfigFromPb(header.config, null).toString()).isEqualTo("DEFAULT")
  ...
  fun testCompilePatch9ProcessingNotSupported() {
    ...
    try {
      ...
      Truth.assertThat(e.cause!!.message).contains("Patch 9 PNG processing is not supported with the JVM Android resource compiler.")
      // expected
    }
  }
  ...
  fun testCompilePngCrunchingNotSupported() {
    ...
    try {
      testPngFile(AaptResourceType.LAYOUT, input, options = ResourceCompilerOptions(requirePngCrunching = true))
      fail()
    } catch (e: ResourceCompilationException) {
      Truth.assertThat(e.cause!!.message).contains("PNG crunching is not supported with the JVM Android resource compiler.")
      // expected
    }
  }
  ...
  fun testCompileFileWithCustomSourcePath() {
    ...
    val result = ContainerReader(testXmlFile(type = AaptResourceType.LAYOUT, input = input, options = options))
    Truth.assertThat(result.numEntries).isEqualTo(1)
