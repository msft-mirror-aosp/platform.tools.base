...
class PseudolocaleGeneratorTest {
  ...
  fun testPseudolocalizeBasicString() {
    val pool = StringPool()
    val original = BasicString(pool.makeRef("Hello world!"), listOf(UntranslatableSection(1, 4)))
    ...
    Truth.assertThat(newString3.ref.value())
      .isEqualTo("${bidiWordStart}H${bidiWordEnd}ell${bidiWordStart}o$bidiWordEnd " + "${bidiWordStart}world!$bidiWordEnd")
  }
  ...
  fun testPseudolocalizeNestedAndAdjacentTags() {
    val pool = StringPool()
    val originalSpans = listOf(Span("b", 16, 19), Span("em", 29, 47), Span("i", 38, 40), Span("b", 44, 47))
    ...
    Truth.assertThat(newString.ref.value()).isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ îš åţ åļļ. one two three four five six]")
    Truth.assertThat(newString.spans().size).isEqualTo(original.spans().size)
    ...
    Truth.assertThat(spans[1].lastChar).isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ îš åţ åļ".length)
    ...
    Truth.assertThat(spans[2].lastChar).isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ î".length)

    Truth.assertThat(spans[3].name.value()).isEqualTo("b")
    Truth.assertThat(spans[3].firstChar).isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ îš åţ".length)
    Truth.assertThat(spans[3].lastChar).isEqualTo("[Ţĥîš šéñţéñçé îš ñöţ ŵĥåţ ýöû ţĥîñķ îţ îš åţ åļ".length)
  }
  ...
  fun testPseudolocalizePartsOfStyledString() {
    ...
    val original = StyledString(pool.makeRef(StyleString(originalText, originalSpans)), untranslatables)

    val newString = pseudolocalizeStyledString(original, Pseudolocalizer.Method.ACCENT, pool)

    Truth.assertThat(newString.ref.value()).isEqualTo("[Ţĥîš šĥöûļð NOT ɓé þšéûðöļöçåļîžéð. one two three four]")
    Truth.assertThat(newString.spans().size).isEqualTo(original.spans().size)
  ...
  fun testPseudolocalizeOnlyDefaultConfigs() {
    ...
    table.addResource(parseResourceName("android:string/one")!!.resourceName, ConfigDescription(), "", string1)

    val string2 = BasicString(table.stringPool.makeRef("two"))
    table.addResource(parseResourceName("android:string/two")!!.resourceName, parse("en"), "", string2)

    val string3Default = BasicString(table.stringPool.makeRef("three"))
    table.addResource(parseResourceName("android:string/three")!!.resourceName, ConfigDescription(), "", string3Default)

    val string3Override = BasicString(table.stringPool.makeRef("three next"))
    table.addResource(parseResourceName("android:string/three")!!.resourceName, parse("en-rXA"), "", string3Override)

    val string4 = BasicString(table.stringPool.makeRef("four"))
    table.addResource(parseResourceName("android:string/four")!!.resourceName, ConfigDescription(), "", string4)
  ...
  fun testPluralsArePseudolocalized() {
    ...
    table.addResource(parseResourceName("com.pkg:plurals/foo")!!.resourceName, ConfigDescription(), "", plural)
  ...
  fun testUntranslatableSections() {
    ...
    val originalString = BasicString(table.stringPool.makeRef(originalText), listOf(UntranslatableSection(6, 11)))

    table.addResource(parseResourceName("android:string/foo")!!.resourceName, ConfigDescription(), "", originalStyle)
    table.addResource(parseResourceName("android:string/bar")!!.resourceName, ConfigDescription(), "", originalString)
