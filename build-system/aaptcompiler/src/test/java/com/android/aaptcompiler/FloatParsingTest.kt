...
class FloatParsingTest {
   ...
   *
   * This tests goes over ~2 billion different float values, and takes ~1 hour for full computation. As such this test is set to be ignored,
   * but should be enabled if float parsing is modified significantly.
   */
  @Test
  @Ignore("b/303076368")
  fun roundTrip() {
  ...
  private fun testParseDecSupported(text: String, expected: Float) {
    ...
    Truth.assertThat(parsedFloat!!.toRawBits().toString(16)).isEqualTo(expected.toRawBits().toString(16))
  ...
  /**
   * Tests a parseHex value that is expected to succeed. This must have the same result as the String.toFloat() method, with the possible
   * addition of a "p0" to the text to make it compatible with Kotlin's toFloat method.
   *
   * Specifically, the hex parsing functionality works for all "normal" floating point numbers. (i.e.) floating point values that do not
   * have a special value in the exponent to force
   */
  ...
  /**
   * Tests a parseHex value with a subnormal (denormal) or special values. parseFloat() should fail in these cases, but the text (with the
   * possible addition of "p0") should succeed when parsing normally in Kotlin's String.toFloat() method.
   */
