...
internal val bidiWordStart = String(byteArrayOf(0xe2.toByte(), 0x80.toByte(), 0x8f.toByte(), 0xe2.toByte(), 0x80.toByte(), 0xae.toByte()))
internal val bidiWordEnd = String(byteArrayOf(0xe2.toByte(), 0x80.toByte(), 0xac.toByte(), 0xe2.toByte(), 0x80.toByte(), 0x8f.toByte()))
...
class PseudolocalizerTest() {
  ...
  fun testSimpleICU() {
    ...
    simpleTest("Copy from {path1} to {path2}", "[Çöþý ƒŕöḿ »{path1}« ţö »{path2}« one two three]", Pseudolocalizer.Method.ACCENT)
    simpleTest("Today is {1,date} {1,time}", "[Ţöðåý îš »{1,date}« »{1,time}« one two]", Pseudolocalizer.Method.ACCENT)

    // Multi-fragment messages.
    compoundTest("[»{USER}« îš öƒƒļîñé one two]", Pseudolocalizer.Method.ACCENT, "{USER}", " ", "is offline")
    compoundTest(
  ...
  fun testICUBidi() {
    // Single-fragment messages.
    simpleTest("{placeholder}", "$bidiWordStart{placeholder}$bidiWordEnd", Pseudolocalizer.Method.BIDI)
    simpleTest(
      "{COUNT, plural, one {one} other {other}}",
      "{COUNT, plural, one {${bidiWordStart}one$bidiWordEnd} " + "other {${bidiWordStart}other$bidiWordEnd}}",
      Pseudolocalizer.Method.BIDI,
    )
  }
  ...
  fun testEscaping() {
    // Single-fragment messages.
    simpleTest("'{USER'} is offline", "['{ÛŠÉŔ'} îš öƒƒļîñé one two three]", Pseudolocalizer.Method.ACCENT)

    // Multi-fragment messages.
    compoundTest("['{ÛŠÉŔ} ''îš öƒƒļîñé one two three]", Pseudolocalizer.Method.ACCENT, "'{USER}", " ", "''is offline")
  }
  ...
  fun testPluralsAndSelects() {
    ...
    simpleTest(
      ...
      "[{1, select, female {»{1}« åððéð ýöû one two} male {»{1}« åððéð ýöû one two} " + "other {»{1}« åððéð ýöû one two}}]",
      Pseudolocalizer.Method.ACCENT,
    )
  ...
  @Test
  fun testNonHtmlTags() {
    simpleTest("Started &lt; 1 minute ago", "[Šţåŕţéð &ļţ; 1 ḿîñûţé åĝö one two three]", Pseudolocalizer.Method.ACCENT)

    simpleTest("Started < 1 minute ago", "[Šţåŕţéð < 1 ḿîñûţé åĝö one two three]", Pseudolocalizer.Method.ACCENT)
  ...
  fun testHtmlTags() {
    ...
    simpleTest("hello <i>happy</i> world", "[ĥéļļö <i>ĥåþþý</i> ŵöŕļð one two three]", Pseudolocalizer.Method.ACCENT)
    ...
    // should be.
    simpleTest("hello <a href=\"idk\"/>world</a>", "[ĥéļļö <a href=\"idk\"/>ŵöŕļð</a> one two]", Pseudolocalizer.Method.ACCENT)
