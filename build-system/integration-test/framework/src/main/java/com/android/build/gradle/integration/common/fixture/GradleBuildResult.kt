...
/**
 * The result from running a build. See [GradleTestProject.executor] and [GradleTaskExecutor].
 *
...
class GradleBuildResult(
  ...
  @JvmOverloads fun assertTask(name: String, withInfo: String? = null): GradleTaskSubject = taskStateList.assertTask(name, withInfo)
  ...
  /**
   * Returns the short (single-line) message that Gradle would print out in the console, without `--stacktrace`. If the build succeeded,
   * returns null.
   */
  ...
  /**
   * Returns the task info given the task name, or null if the task is not found (if it is not in the task execution plan).
   *
   * @see getTask
   */
  ...
  /**
   * Returns the task info given the task name. The task must exist (it must be in the task execution plan).
   *
   * @see findTask
   */
  ...
  private fun isPlaceholderEx(throwableType: String) =
    throwableType == PlaceholderException::class.java.name || throwableType == ContextualPlaceholderException::class.java.name

  @Suppress("DEPRECATION")
  fun assertOutputContains(text: String) {
    stdout.use { ScannerSubject.assertThat(it, name = "stdout").contains(text) }
  }

  @Suppress("DEPRECATION")
  fun assertErrorContains(text: String) {
    stderr.use { ScannerSubject.assertThat(it, name = "stderr").contains(text) }
  }

  @Suppress("DEPRECATION")
  fun assertOutputDoesNotContain(text: String) {
    stdout.use { ScannerSubject.assertThat(it, name = "stdout").doesNotContain(text) }
  }

  @Suppress("DEPRECATION")
  fun assertErrorDoesNotContain(text: String) {
    stderr.use { ScannerSubject.assertThat(it, name = "stderr").doesNotContain(text) }
  }
   ...
   *
   * If any of the mentioned files are HTML reports, this method will also perform simple text matching to find linked local CSS,
   * JavaScript, and other HTML files, copying them as well to preserve the report's structure and functionality. Remote files are ignored.
   * - For files located under the parent of [outputDir], the path relative to that parent is preserved within [outputDir].
   * - For all other files, their full absolute path (minus the root) is recreated inside [outputDir].
   *
  ...
  fun copyMentionedFilesTo(outputDir: File, mode: CopyFilesMode = CopyFilesMode.TEXT_ONLY) {
    ...
    filesToCopy.forEach { sourceFile ->
      ...
      val destinationFile =
        if (sourcePath.startsWith(outputDirPath.parent)) {
  ...
  /**
   * Most tests don't examine the state of the build's tasks and [TaskStateList] is relatively expensive to initialize, so this is done
   * lazily.
   */
  private val taskStateList: TaskStateList by lazy { TaskStateList(taskEvents, this.stdout) }
}
