...
internal open class JarBuilderImpl : JarBuilder {
  ...
  override fun addTextFiles(entries: List<Pair<String, String>>): JarBuilder {
    entries.forEach { addTextFile(it.first, it.second) }
    return this
  }
 ...
 */
internal class JarWithDependenciesBuilderImpl(private val mavenCoordinate: String) : JarBuilderImpl(), JarWithDependenciesBuilder {

  protected val dependencies = mutableListOf<String>()

  internal fun toLibrary(): Library = Library(mavenCoordinate, "jar", getContent(), *dependencies.toTypedArray())
...
internal class JarContentBuilder {
  ...
  internal fun addClassWithEmptyMethods(binaryClassName: String, vararg namesAndDescriptors: String) {
    if (closed) throw RuntimeException("cannot call addClassWithEmptyMethods after getContent")
    zip.putNextEntry(ZipEntry("$binaryClassName.class"))
    zip.write(
      createClass(binaryClassName) {
  ...
  internal fun addClass(theClass: Class<*>) {
    ...
    val content =
      theClass.getClassLoader().getResourceAsStream(classPath)?.use { it -> it.readAllBytes() }
  ...
  private fun createClass(binaryClassName: String, action: (ClassWriter.() -> Unit)? = null): ByteArray {
