...
/**
 * Takes 2 list of snapshot items and remove the identical content. This also changes the names of [NamedSnapshotItem], and possibly some
 * values to indicate whether they are ADDED/REMOVED.
 */
 ...
 *
 * The 2 containers must have the same named entries. This compares them and return a container with the entries having different values, or
 * null if they are all the same.
 */
private fun SnapshotContainer.subtractObjectProperties(reference: SnapshotContainer): SnapshotContainer? {
  val name = this.name
  ...
  return if (this.items == null && reference.items == null) {
    ...
    for (item in originalItems) {
      val referenceItem = referenceMap[item.name] ?: throw RuntimeException("Failed to find reference item for ${item.name}")
...
/**
 * Subtract a named item with another named item and returns a new item if they are different, or null if they are the same.
 *
...
private fun NamedSnapshotItem.computeDifference(reference: NamedSnapshotItem): SnapshotItem? =
  when (this) {
    is KeyValueItem -> {
      ...
      if (this.value == (reference as KeyValueItem).value) {
        null
      } else {
        this
      }
...
private fun SnapshotContainer.subtractObjectList(reference: SnapshotContainer): SnapshotContainer? {
  ...
  return if (referenceItems == null) {
    ...
    SnapshotContainerImpl(name, originalItems.map { it.cloneWithNewName("{ADDED}") }, SnapshotContainer.ContentType.OBJECT_LIST)
  } else {
...
private fun SnapshotItem.cloneWithNewName(suffix: String, stripContent: Boolean = false): SnapshotItem =
  when (this) {
    is KeyValueItem -> KeyValueItem(name + suffix, if (stripContent) null else value)
    is SnapshotContainer -> SnapshotContainerImpl(name + suffix, if (stripContent) null else items, contentType)
...
private fun SnapshotContainer.subtractValueList(reference: SnapshotContainer): SnapshotContainer? {
  ...
  return if (referenceItems == null) {
    ...
    SnapshotContainerImpl(name, originalItems.map { ValueOnlyItem("${it.value}{ADDED}") }, SnapshotContainer.ContentType.VALUE_LIST)
  } else {
    if (originalItems != null) {
      // both are non null, compare the two lists
      ...
      if (referenceItems.isEmpty()) {
        ...
        SnapshotContainerImpl(name, referenceItems.map { ValueOnlyItem("${it.value}{REMOVED}") }, SnapshotContainer.ContentType.VALUE_LIST)
      }
    }
  }
}
