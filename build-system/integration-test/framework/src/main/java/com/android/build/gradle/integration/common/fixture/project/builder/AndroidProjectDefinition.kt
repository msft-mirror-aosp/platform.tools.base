...
internal abstract class AndroidProjectDefinitionImpl<ExtensionT>(path: String) :
  ...
  internal open val namespace: String
    get() {
      ...
      throw RuntimeException(
        "Unsupported android extension type. Override namespace getter in the specific AndroidProjectDefinition implementation!"
      )
    }
  ...
  protected open fun initDefaultValues(extension: ExtensionT) {
    if (extension is CommonExtension) {
      ...
      extension.compileSdk { version = release(GradleBuildDefinition.DEFAULT_COMPILE_SDK_VERSION) }
    } else {
      throw RuntimeException(
        "Unsupported android extension type. Override initDefaultValues() in the specific AndroidProjectDefinition implementation!"
      )
    }
  }
  ...
  override fun writeExtension(writer: BuildWriter, location: Path) {
    writer.apply {
      ...
      if (kotlinActionRan) {
        emptyLine()
        block("kotlin") { kotlinDslRecorder.writeContent(this) }
      }
