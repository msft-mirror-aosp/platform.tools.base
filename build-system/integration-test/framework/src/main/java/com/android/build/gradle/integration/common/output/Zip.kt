 ...
 */
abstract class Zip(val name: String) {
  enum class Status {
    EXISTS,
    DIRECTORY,
    DOES_NOT_EXIST,
 ...
 */
class SimpleZip(val archivePath: Path?, name: String = archivePath?.fileName?.toString() ?: "missing zip path") : Zip(name), AutoCloseable {
  ...
  init {
    when {
      ...
      archivePath.fileSystem != FileSystems.getDefault() -> {
        throw IllegalArgumentException("Cannot create zip from non default fs, use getEntryAsZip() instead")
      }
  ...
  override fun getZipEntry(path: String): ZipEntry? {
    ...
    return ZipFile(archivePath.toFile()).use<ZipFile, ZipEntry> { it.getEntry(path) }
  }

  override fun getEntries(filter: ((String) -> Boolean)?): List<String> {
    return filter?.let { f -> allEntries.filter(f) } ?: allEntries
  }
  ...
  private val allEntries: List<String> by
    lazy(LazyThreadSafetyMode.NONE) {
      val zip = this.zip ?: return@lazy listOf()

      Files.walk(zip.getPath("/")).filter { it.isRegularFile() }.map { it.toString().substring(1) }.collect(Collectors.toList())
    }
}
 ...
 *
 * This does not care about duplicate objects across archives. When searching for a file in the archives, the first match is returned.
 *
 * This is meant to work with [ZipSubject] and [AarSubject] (and possibly others in the future), and meant to only wrap the result of
 * [Zip.innerZip]. Because of this, there is no need to close this object as the zip it wraps will be closed automatically when the
 * enclosing zip is itself closed
 */
internal class MultiZipView(private val zips: List<Zip>, name: String) : Zip(name) {
  ...
  override fun getEntry(path: String): Path? = findInZips(path) { getEntry(it) }
  ...
  override fun getEntries(filter: ((String) -> Boolean)?): List<String> = filter?.let { f -> allEntries.filter(f) } ?: allEntries

  override fun innerZip(path: String): Zip? = findInZips(path) { innerZip(it) }
 ...
 *
 * The filtering is only to give access to a specific sub folder inside the original zip, making all the paths relative to that sub-folder.
 *
 * This is meant to work with [ZipSubject] and [AarSubject] (and possibly others in the future), and meant to only wrap a [SimpleZip] or the
 * result of [Zip.innerZip]. Because of this, there is no need to close this object as the zip is the one that should be closed.
 */
internal class ZipFolderView(private val zip: Zip, folderName: String) : Zip("${zip.name}/$folderName") {
  ...
  private val allEntries: List<String> by
    lazy(LazyThreadSafetyMode.NONE) { zip.getEntries().mapNotNull { if (it.startsWith(prefix)) it.substring(prefix.length) else null } }
}
