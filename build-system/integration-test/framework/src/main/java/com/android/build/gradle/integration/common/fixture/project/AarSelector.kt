...
sealed interface AarSelector : OutputSelector {
  ...
  /** returns a new instance of the selector, targeting the test fixture output. This is the same as using withSuffix("testFixtures") */
  fun forTestFixtures(): AarSelector

  companion object {
    @JvmField val DEBUG = of(buildType = "debug")

    @JvmField val RELEASE = of(buildType = "release")

    @JvmField val NO_BUILD_TYPE = of(buildType = null)

    @JvmStatic
    fun of(buildType: String?): AarSelector {
      return AarSelectorImp(buildType = buildType, flavors = listOf())
    }
  }
}
...
internal data class AarSelectorImp(
  ...
  override fun withName(name: String): AarSelector = AarSelectorImp(name, buildType, flavors, filter, suffix)

  override fun withFlavor(name: String): AarSelector = AarSelectorImp(this.name, buildType, flavors + name, filter, suffix)

  override fun withFilter(newFilter: String): AarSelector = AarSelectorImp(this.name, buildType, flavors, newFilter, suffix)

  override fun withSuffix(newSuffix: String): AarSelector = AarSelectorImp(this.name, buildType, flavors, filter, newSuffix)
