...
sealed class PluginType(
  ...
  // Kotlin plugins
  object KOTLIN_JVM :
    PluginTypeWithExtension<KotlinExtension>(
  ...
  object KAPT : PluginType(id = "org.jetbrains.kotlin.kapt", version = TestUtils.KOTLIN_VERSION_FOR_TESTS)

  object KSP : PluginType(id = "com.google.devtools.ksp", version = TestUtils.KSP_VERSION_FOR_TESTS)
  ...
  object COMPOSE_COMPILER_PLUGIN :
    PluginType(id = com.android.build.gradle.internal.utils.COMPOSE_COMPILER_PLUGIN_ID, version = TestUtils.KOTLIN_VERSION_FOR_TESTS)
  ...
  ) : PluginType(id, version = version, artifact = artifact, hasMarker = hasMarker)

  /** A plugin that is associated with an extension which can be configured in [GradleProjectDefinition.applyPlugin] */
  ...
  abstract class AgpPlugin(id: String, isAndroid: Boolean = true) :
    PluginType(id = id, isAndroid = isAndroid, artifact = "com.android.tools.build:gradle", version = Version.ANDROID_GRADLE_PLUGIN_VERSION)
}
