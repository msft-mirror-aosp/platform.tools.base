...
object AsmApiApiTestUtils {
  private const val instrumentedAnnotationDescriptor = "Lcom/example/instrumentationlib/instrumentation/InstrumentedAnnotation;"
  private const val instrumentedInterfaceDescriptor = "Lcom/example/instrumentationlib/instrumentation/InstrumentedInterface;"
  ...
  val libClasses = listOf("R", "I", "InterfaceExtendsI")

  val featureClasses = listOf("R", "ClassExtendsAnAppClass")
  ...
  fun configureExtensionForAnnotationAddingVisitor(
    ...
    interfacesOfClassesToInstrument: List<String> = listOf("com.example.lib.I", "com.example.lib.InterfaceExtendsI"),
  ...
  fun checkClassesAreInstrumented(
    ...
    val filteredClasses =
      apk.allDexes
        .flatMap { it.classes.entries }
        .associate { it.key to it.value }
        .filterKeys { it.startsWith(classesDescriptorPackagePrefix) }
    Truth.assertThat(filteredClasses.filterNot { it.key.endsWith("\$Condy0;") }.keys)
      .containsExactlyElementsIn(expectedClasses.map { name -> "$classesDescriptorPackagePrefix$name;" })
  ...
  private fun assertExactlyTheGivenMethodsAreAnnotated(
    ...
    classesMap.forEach { (name, clazz) ->
      clazz.methods.forEach { method ->
        if (expectedAnnotatedMethods[name]?.contains(method.name) == true) {
          ...
          Truth.assertThat(method.annotations.first().type).isEqualTo(instrumentedAnnotationDescriptor)
    ...
    Truth.assertThat(annotatedMethodsCount).isEqualTo(expectedAnnotatedMethods.values.sumBy { it.size })
  }
