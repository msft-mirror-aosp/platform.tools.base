...
internal class GradleBuildDefinitionImpl(
  ...
  /**
   * Whether creation of new sub-project can include project content. This is disabled if the build definition is attached to an existing
   * on-disk test project.
   */
  ...
  override fun includedBuild(name: String, action: GradleBuildDefinition.() -> Unit): GradleBuildDefinition {
    val build =
      includedBuilds.computeIfAbsent(name) {
  ...
  override fun genericProject(path: String, action: GenericProjectDefinition.() -> Unit): GenericProjectDefinition {
    ...
    project as? GenericProjectDefinition ?: errorOnWrongType(project, path, "Generic Project")
  ...
  override fun androidApplication(
    ...
    val project =
      subProjects.computeIfAbsent(path) {
    ...
    project as? AndroidApplicationDefinitionImpl ?: errorOnWrongType(project, path, "Android Application")
  ...
  override fun androidJavaApplication(
    ...
    return androidApplication(path, createMinimumProject = true, action).also { HelloWorldAndroid.setupJava(it.files) }
  }
  ...
  override fun androidKotlinApplication(
    ...
    val app = androidApplication(path, createMinimumProject = true) { applyPlugin(PluginType.ANDROID_BUILT_IN_KOTLIN) }
    action(app)
  ...
  override fun androidLibrary(
    ...
    val project = subProjects.computeIfAbsent(path) { AndroidLibraryDefinitionImpl(it, createMinimumProject) }

    project as? AndroidLibraryDefinitionImpl ?: errorOnWrongType(project, path, "Android Library")
  ...
  override fun androidFeature(
    ...
    project as? AndroidDynamicFeatureDefinitionImpl ?: errorOnWrongType(project, path, "Android Dynamic Feature")
  ...
  override fun androidTest(
    ...
    project as? AndroidTestDefinitionImpl ?: errorOnWrongType(project, path, "Android Test")
  ...
  override fun aiPack(path: String, action: AiPackDefinition.() -> Unit): AiPackDefinition {
    if (path == ":") throw RuntimeException("root project cannot be an AI pack")
    if (!path.startsWith(":")) throw RuntimeException("Project paths must start with ':' (value: $path)")

    val project = subProjects.computeIfAbsent(path) { AiPackDefinitionImpl(it) }

    project as? AiPackDefinition ?: errorOnWrongType(project, path, "Android AI Pack")
  ...
  override fun assetPack(path: String, action: AssetPackDefinition.() -> Unit): AssetPackDefinition {
    if (path == ":") throw RuntimeException("root project cannot be an asset pack")
    if (!path.startsWith(":")) throw RuntimeException("Project paths must start with ':' (value: $path)")

    val project = subProjects.computeIfAbsent(path) { AssetPackDefinitionImpl(it) }

    project as? AssetPackDefinition ?: errorOnWrongType(project, path, "Asset Pack")
  ...
  override fun assetPackBundle(
    ...
    val project = subProjects.computeIfAbsent(path) { AssetPackBundleDefinitionImpl(it, createMinimumProject) }

    project as? AssetPackBundleDefinition ?: errorOnWrongType(project, path, "Asset Pack Bundle")
  ...
  override fun fusedLibrary(
    ...
    val project = subProjects.computeIfAbsent(path) { FusedLibraryDefinitionImpl(it, createMinimumProject) }

    project as? FusedLibraryDefinition ?: errorOnWrongType(project, path, "Fused Library")
  ...
  override fun javaLibrary(path: String, action: JavaLibraryProjectDefinition.() -> Unit): JavaLibraryProjectDefinition {
    if (path == ":") throw RuntimeException("root project cannot be a java library")
    if (!path.startsWith(":")) throw RuntimeException("Project paths must start with ':' (value: $path)")

    val project = subProjects.computeIfAbsent(path) { JavaLibraryProjectDefinitionImpl(it) }

    project as? JavaLibraryProjectDefinition ?: errorOnWrongType(project, path, "java-library")
  ...
  override fun kotlinMultiplatformLibrary(path: String, action: KotlinMultiplatformDefinition.() -> Unit): KotlinMultiplatformDefinition =
    kotlinMultiplatformLibrary(path, plugins = listOf(), createMinimumProject = false, action)
  ...
  override fun androidKotlinMultiplatformLibrary(
    ...
    return kotlinMultiplatformLibrary(path, plugins = listOf(PluginType.ANDROID_KMP_LIBRARY), createMinimumProject, action)
  }
  ...
  private fun kotlinMultiplatformLibrary(
    ...
    project as? KotlinMultiplatformDefinition ?: errorOnWrongType(project, path, "Android Kotlin Multiplatform Library")
  ...
  override fun configure(path: String, action: GradleProjectDefinition.() -> Unit) {
    if (!path.startsWith(":")) throw RuntimeException("Project paths must start with ':' (value: $path)")

    val project = subProjects[path] ?: throw RuntimeException("Project with path $path does not exist")
  ...
  private fun errorOnWrongType(project: GradleProjectDefinition, path: String, expectedType: String): Nothing {
    val wrongType =
      when (project) {
    ...
    throw RuntimeException(
      "Attempting to create a module with path '$path' of type '$expectedType', but a module of type '$wrongType' already exists."
    )
  }
  ...
  internal fun write(
    ...
    checkAgp9Behavior(disableUnnecessaryBuiltInKotlinOptOutChecks, disableUnnecessaryNewDslOptOutChecks)
    ...
    val projectRepositories =
      if (useOldPluginStyleForSeparateClassloaders || locallyDefinedRepo) globalDefinitionState.repositories else listOf()
    ...
    includedBuilds.values.forEach {
      ...
      val globalState =
        GlobalDefinitionStateImpl(
          globalDefinitionState.additionalProperties,
          globalDefinitionState.repositories,
          it.handleCustomBuildLogic(newLocation),
        )

      it.write(newLocation, globalState, disableUnnecessaryBuiltInKotlinOptOutChecks, disableUnnecessaryNewDslOptOutChecks)
    }
  }
  ...
  internal fun createAncillaryBuildFiles(parentFolder: Path, writeAction: (Path) -> Unit) {
    ...
    includedBuilds.values.forEach { it.createAncillaryBuildFiles(buildFolder, writeAction) }
  }
   ...
   *
   * This allows each build file to know how the plugins must be applied. For instance if 2 projects have the same plugin in different
   * versions then the version is specified in the project build files. Otherwise, it's specified in the root project (with apply false).
   */
  internal fun computeAllPluginMap(): Map<PluginType, Set<String>> {
    ...
    (subProjects.values + rootProject).forEach { project ->
      project.plugins.forEach { entry ->
        val set = allPlugins.computeIfAbsent(entry.plugin) { mutableSetOf() } as MutableSet<String>
  ...
  internal fun gatherInlineLibraries(): List<MavenRepoGenerator.Library> =
    includedBuilds.values.flatMap { it.gatherInlineLibraries() } + subProjects.values.flatMap { it.dependencies.externalLibraries }
  ...
  internal fun handleCustomBuildLogic(location: Path): Map<String, Set<String>> {
    ...
    val callbackMap =
      subProjects.values
        .asSequence()
        .flatMap { definition -> definition.pluginCallbacks.map { it to definition.path } }
    ...
    handler.use {
      ...
      for ((callbackClass, paths) in callbackMap) {
        ...
        // by each subproject
        paths.forEach { path ->
          val nameList = pluginClassMap.computeIfAbsent(path) { mutableSetOf() } as MutableSet<String>
   ...
   */
  private fun checkAgp9Behavior(disableUnnecessaryBuiltInKotlinOptOutChecks: Boolean, disableUnnecessaryNewDslOptOutChecks: Boolean) {
      ...
      .forEach {
        ...
        if (requiresOldVariantApi) {
          ...
          if (newDsl != false && newDslAsString != "false") {
            throw RuntimeException("Usage of Legacy Variant API requires to set ${USE_NEW_DSL.propertyName}=false")
          }
        }
    ...
    val allPlugins = subProjectList.flatMap { it.plugins.map { appliedPlugin -> appliedPlugin.plugin } }.toSet()
    ...
    if (!foundLegacy && propertiesDelegate.mutableBooleans[USE_NEW_DSL] == false && !foundNewCallbackForOldDsl && !hasLegacyKotlin) {
      if (!disableUnnecessaryNewDslOptOutChecks) {
        throw RuntimeException("Usage of ${USE_NEW_DSL.propertyName}=false is not necessary without any legacy plugins")
      }
    }
    ...
    if (hasLegacyKotlin && !optedOutBuiltInKotlin) {
      ...
      if (!disableUnnecessaryBuiltInKotlinOptOutChecks) {
        throw RuntimeException("Usage of ${BUILT_IN_KOTLIN.propertyName}=false is not necessary without usage of Legacy kotlin plugin")
      }
