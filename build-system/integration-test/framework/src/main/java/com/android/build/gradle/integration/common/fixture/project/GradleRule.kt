...
interface GradleRule : TestRule {
  companion object : GradleRuleEntryPoint {
     ...
     */
    override fun from(folderName: String, logicalName: String?, configAction: GradleBuildDefinition.() -> Unit): GradleRule =
      fromImplementation(null, folderName, logicalName, specConfigAction = null, configAction)
    ...
    ): GradleRule = fromImplementation(testProjectName, folderName, logicalName, specConfigAction = null, configAction)
   ...
   *
   * This can safely be queried before a call to [build]. This is the same value as [GradleBuild.directory].
   *
   * Calls to [GradleBuildDefinition.name] or [GradleBuildDefinition.rootFolderName] will impact the returned value
   */
   ...
   *
   * Once this is called the project is written on disk and it's not possible to change its structure.
   *
   * It is possible after the fact to add more source files can be added via [GenericProject.files] and it's possible to amend the build
   * file with [AndroidProject.reconfigure]
   */
   ...
   *
   * The gradle build must be queried at the end with either versions of [LocalRuleOptionBuilder.build]
   *
   * Once this is called the project is written on disk and it's not possible to change its structure.
   *
   * It is possible after the fact to add more source files can be added via [GenericProject.files] and it's possible to amend the build
