...
interface GradlePropertiesBuilder {
   ...
   *
   * If the key is a [BooleanOption] or a [StringOption], use the more specific [add] methods below.
   */
...
internal class GradlePropertiesDelegate : GradlePropertiesBuilder {
  ...
  internal val properties: List<String>
    get() {
      ...
      return mutableBooleans.map { (option, value) -> "${option.propertyName}=$value" } +
        mutableStrings.map { (option, value) -> "${option.propertyName}=$value" } +
        mutableProperties.map { (key, value) -> "$key=$value" }
    }
}
