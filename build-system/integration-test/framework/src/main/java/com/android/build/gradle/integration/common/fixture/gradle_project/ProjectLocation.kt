...
fun initializeProjectLocation(testClass: Class<*>, methodName: String?, projectName: String?): ProjectLocation {
  ...
  return ProjectLocation(testDir, testLocation)
}

private fun computeTestDir(testOutDir: File, testClass: Class<*>, methodName: String?, projectName: String?): File {
  var testDir = testOutDir
  if (SdkConstants.CURRENT_PLATFORM == SdkConstants.PLATFORM_WINDOWS && System.getenv("BUILDBOT_BUILDERNAME") == null) {
  ...
  if (SdkConstants.CURRENT_PLATFORM == SdkConstants.PLATFORM_WINDOWS) {
    ...
    if (totalLen > GradleTestProject.MAX_TEST_NAME_DIR_WINDOWS) {
      val hash = Hashing.sha1().hashString(classDir + methodDir, Charsets.US_ASCII).toString()
      ...
      classDir =
        (testIdentifier.substring(0, Math.min(testIdentifier.length, 10)) +
          hash.substring(0, Math.min(hash.length, GradleTestProject.MAX_TEST_NAME_DIR_WINDOWS - 10)))
      methodDir = null
    }
  }
