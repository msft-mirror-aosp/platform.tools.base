...
/**
 * a Build Action that returns all the [AndroidProject]s and all [ProjectSyncIssues] for all the sub-projects, via the tooling API.
 *
...
class GetAndroidModelV2Action(
  ...
  override fun execute(buildController: BuildController): ModelContainerV2 {
    ...
    for ((buildId, modelInfoMap) in modelMap) {
      val name =
        reverseBuildIdMap[buildId.rootDir] ?: throw RuntimeException("Failed to find name for ${buildId.rootDir}\nbuildIdMap = $buildIdMap")
      projectInfoMaps[name] = modelInfoMap.filter { it.value.isAndroid() }

      buildInfoMap[name] = BuildInfo(name, buildId.rootDir, modelInfoMap.map { it.key to it.value.projectDir })
    }
  ...
  private fun getAndroidProjectMap(
    ...
    for ((buildId, project) in projects) {
      ...
      if (modelVersions != null) {
        ...
        val projectGraph =
          if (variantName != null) {
            buildController.findModel(project, ProjectGraph::class.java, ModelBuilderParameter::class.java) {
        ...
        val variantDependencies =
          if (variantName != null) {
            buildController.findModel(project, VariantDependencies::class.java, ModelBuilderParameter::class.java) {
              it.variantName = variantName
              parameterMutator(it)
            }
          } else null
        ...
        val nativeModule =
          if (nativeParams != null) {
            buildController.findModel(project, NativeModule::class.java, NativeModelBuilderParameter::class.java) {
