...
/**
 * A Custom plugin to be used with [ApplicationComponentCallback] in projects created by [GradleRule].
 *
 * Do not extend this. Instead, implement [ApplicationComponentCallback] and register the implementation class to
...
abstract class ApplicationCallbackPlugin : Plugin<Project> {
  ...
  abstract fun handleExtension(project: Project, componentsExtension: ApplicationAndroidComponentsExtension)
}
...
interface ApplicationComponentCallback : PluginCallback {
  fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension)
}
