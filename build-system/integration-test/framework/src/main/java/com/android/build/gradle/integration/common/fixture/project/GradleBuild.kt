 ...
 *
 * It is also possible to query for subprojects and includedbuilds and add (non-build) files and query for the content of their output
 * folder
 */
interface GradleBuild {
  ...
  /** Queries for an application project via its gradle path. The project must exist and be an Android Application project */
  fun androidApplication(path: String = DEFAULT_APP_PATH): AndroidApplicationProject

  /** Queries for a library project via its gradle path. The project must exist and be an Android Library project */
  fun androidLibrary(path: String = DEFAULT_LIB_PATH): AndroidLibraryProject

  /** Queries for a feature project via its gradle path. The project must exist and be an Android Dynamic Feature project */
  fun androidFeature(path: String = DEFAULT_FEATURE_PATH): AndroidDynamicFeatureProject

  /** Queries for an android test project via its gradle path. The project must exist and be an Android Test project */
  fun androidTest(path: String = DEFAULT_TEST_PATH): AndroidTestProject

  /** Queries for a privacy sandbox sdk via its gradle path. The project must exist and be a Privacy Sandbox SDK. */

  /** Queries for an AI pack project via its gradle path. The project must exist and be an AI Pack project */
  fun aiPack(path: String): AiPackProject

  /** Queries for an Asset pack project via its gradle path. The project must exist and be an Asset Pack project */
  fun assetPack(path: String): AssetPackProject

  /** Queries for an Asset pack bundle project via its gradle path. The project must exist and be an Asset Pack project */
  fun assetPackBundle(path: String): AssetPackBundleProject

  /** Queries for a Fused Library project via its gradle path. The project must exist and be a Fused Library project */
  fun fusedLibrary(path: String): FusedLibraryProject

  /** Queries for a Java Library project via its gradle path. THe project must exist and be a Java Library project. */
  fun javaLibrary(path: String): JavaLibraryProject

  /** Queries for a Kotlin multiplatform project via its gradle path. The project must exist and be a Kotlin multiplatform project. */
   ...
   *
   * Any modifications to the project (using [GenericProject.files]) made from within the action, using the provided instance of
   * [GradleBuild], will be reverted after the action is run.
   */
...
/**
 * Basic implementation of some features of [GradleBuild] to be shared across the 2 different implementations ([GradleBuildImpl] and
 * [ReversibleGradleBuild]
 */
internal abstract class BaseGradleBuildImpl : GradleBuild {

  /**
   * a more complete list of projects to validate project types. This is separate for the case of [ReversibleGradleBuild] where the
   * subProject list is a clone of the parent build.
   */
...
internal class GradleBuildImpl(
  ...
  internal fun computeAllPluginMap(): Map<PluginType, Set<String>> = definition.computeAllPluginMap()

  internal fun getGlobalDefinitionState(): GlobalDefinitionState = definition.globalDefinitionState

  internal val useOldPluginStyle: Boolean
    get() = definition.useOldPluginStyleForSeparateClassloaders

  /**
   * Runs the provided action with this build. At the end of the action, all file changes made via [GradleProjectFiles] are reversed so that
   * the build is the same as before this method
   */
  override fun withReversibleModifications(action: (GradleBuild) -> Unit) {
    FileChangeController().use { controller -> action(ReversibleGradleBuild(this, controller)) }
  }
