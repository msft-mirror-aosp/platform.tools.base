...
internal constructor(
  ...
  override val build: GradleBuild by lazy {
    ...
    val profileDirectory = if (enableProfileOutput) locations.testFiles.resolve(GradleTestProjectBuilder.DEFAULT_PROFILE_DIR) else null
  ...
  override fun configure(): LocalRuleOptionBuilder = LocalRuleOptionBuilder(this, this.ruleOptionBuilder)
  ...
  private fun doWriteBuild() {
    ...
    val globalState =
      GlobalDefinitionStateImpl(getDefaultProperties(), localRepositories, buildDefinition.handleCustomBuildLogic(rootBuildPath))
  ...
  private fun copyProjectWithoutBuildFiles(source: Path, destination: Path) {
    source.walk(PathWalkOption.INCLUDE_DIRECTORIES).forEach { sourceFile ->
      val name = sourceFile.name
      if (name.endsWith(".gradle") || name.endsWith(".gradle.kts")) return@forEach
  ...
  private fun computeMavenRepoLocation(): Path = locations.testFiles.resolve(buildDefinition.rootFolderName).resolve("_maven_repo")

  /**
   * compute a [GradleBuild].
   *
   * For included builds, the location is the modified [locations] so that `projectDir` is updated to be the included directory.
   */
  private fun computeGradleBuild(
    ...
    val includedBuilds =
      buildDefinition.includedBuilds.values.associate {
        it.name to computeGradleBuild(it, buildPath, testSupportLocations, profileDirectory)
      }

    val subProjects =
      buildDefinition.subProjects.values.associate { definition ->
        val subProjectLocation = computeSubProjectPath(buildPath, definition.path)

        definition.path to
          when (definition) {
            is AndroidApplicationDefinitionImpl -> AndroidApplicationImpl(subProjectLocation, definition, definition.namespace)

            is AndroidLibraryDefinitionImpl -> AndroidLibraryImpl(subProjectLocation, definition, definition.namespace)

            is AndroidDynamicFeatureDefinitionImpl -> AndroidFeatureImpl(subProjectLocation, definition, definition.namespace)

            is AndroidTestDefinitionImpl -> AndroidTestImpl(subProjectLocation, definition, definition.namespace)

            is AssetPackDefinitionImpl -> AssetPackImpl(subProjectLocation, definition)

            is AssetPackBundleDefinitionImpl -> AssetPackBundleImpl(subProjectLocation, definition)

            is AiPackDefinitionImpl -> AiPackImpl(subProjectLocation, definition)

            is FusedLibraryDefinitionImpl -> FusedLibraryImpl(subProjectLocation, definition)

            is JavaLibraryProjectDefinition -> JavaLibraryProjectImpl(subProjectLocation, definition)

            is KotlinMultiplatformDefinitionImpl -> KotlinMultiplatformProjectImpl(subProjectLocation, definition)

            is GenericProjectDefinitionImpl -> GenericProjectImpl(subProjectLocation, definition)

            else -> throw RuntimeException("Unsupported GradleProjectDefinition type: ${definition::class.java}")
          }
      } + mapOf(":" to GenericProjectImpl(computeSubProjectPath(buildPath, ":"), buildDefinition.rootProject))
    ...
    return GradleBuildImpl(
      ...
      .also { build -> subProjects.values.forEach { it.build = build } }
  }
  ...
  private fun instantiateExecutor(): GradleTaskExecutor =
    GradleTaskExecutor(locations.toProjectLocation(buildDefinition), getTestInfo(), ruleOptionBuilder.gradleOptions, projectConnection) {
      lastBuildResult = it
    }

  private fun instantiateModelBuilder(): ModelBuilderV2 =
    ModelBuilderV2(locations.toProjectLocation(buildDefinition), getTestInfo(), ruleOptionBuilder.gradleOptions, projectConnection) {
        lastBuildResult = it
      }
      .withPerTestPrefsRoot(true)

  private fun getTestInfo(): GradleTestInfo =
    object : GradleTestInfo {
  ...
  private fun createAncillaryBuildFiles() {
    buildDefinition.createAncillaryBuildFiles(locations.testFiles) { path -> createLocalProp(path) }
  }

  private fun createLocalProp(destinationDir: Path) {
    val localProp = ProjectPropertiesWorkingCopy.create(destinationDir.toString(), ProjectPropertiesWorkingCopy.PropertyType.LOCAL)

    ruleOptionBuilder.sdkConfiguration.sdkDir?.let { localProp.setProperty(ProjectProperties.PROPERTY_SDK, it.toString()) }
  ...
  override fun apply(base: Statement, description: Description): Statement {
    ...
    return object : Statement() {
      override fun evaluate() {
        ...
        // used this mechanism)
        if (description.methodName == null) throw RuntimeException("Class level rule application is not supported.")
  ...
  private val projectConnection: ProjectConnection by lazy {
    ...
    (connector as DefaultGradleConnector).daemonMaxIdleTime(GradleTestProject.GRADLE_DEAMON_IDLE_TIME_IN_SECONDS, TimeUnit.SECONDS)
    ...
    connector.connect().also { connection -> openConnections.add(connection) }
  }

  private fun checkConfigurationCache() {
    val checker = ConfigurationCacheReportChecker()
    (build as GradleBuildImpl)
      .subProject(":")
