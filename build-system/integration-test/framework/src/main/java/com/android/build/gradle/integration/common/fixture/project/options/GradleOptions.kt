...
data class GradleOptions(val memoryRequirement: MemoryRequirement, val configurationCaching: ConfigurationCaching) {
...
data class MemoryRequirement(val heap: String = DEFAULT_HEAL, val metaspace: String = DEFAULT_METASPACE) {
  companion object {
    const val DEFAULT_HEAL: String = "1G"
    const val DEFAULT_METASPACE: String = "1G"

    /** use default heap size for gradle. */
    @JvmStatic fun useDefault(): MemoryRequirement = use(null, null)
    ...
    fun use(heap: String?, metaspace: String?): MemoryRequirement {
      return MemoryRequirement(heap ?: DEFAULT_HEAL, metaspace ?: DEFAULT_METASPACE)
    }
  }
...
internal class GradleOptionsDelegate(defaultValues: GradleOptions? = null) :
  ...
  val asGradleOptions: GradleOptions
    get() = GradleOptions(MemoryRequirement.use(heap, metaspace), configurationCaching)

  override fun mergeWith(other: GradleOptionsDelegate) {
    other.heap?.let { heap = it }

    other.metaspace?.let { metaspace = it }
