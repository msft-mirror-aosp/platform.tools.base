...
/**
 * Represents a bundle (AAB) location, based on parameters like build type, flavors, whether this is a test, or whether the APK is signed.
 *
...
sealed interface AabSelector : OutputSelector {
  ...
  companion object {
    ...
    @JvmField val NO_BUILD_TYPE = of(buildType = null, isSigned = false)

    @JvmField val NO_BUILD_TYPE_SIGNED = of(buildType = null, isSigned = true)

    @JvmField val ANDROIDTEST_DEBUG = of(buildType = "debug", testName = "androidTest", isSigned = true)

    @JvmStatic
    fun of(buildType: String?, isSigned: Boolean): AabSelector {
      return AabSelectorImp(buildType = buildType, testSuite = null, flavors = listOf(), isSigned = isSigned)
    }

    @JvmStatic
    fun of(buildType: String?, testName: String?, isSigned: Boolean): AabSelector {
      return AabSelectorImp(buildType = buildType, testSuite = testName, flavors = listOf(), isSigned = isSigned)
    }
  }
}
...
internal data class AabSelectorImp(
  ...
  override fun withName(name: String) = AabSelectorImp(name, buildType, testSuite, flavors, isSigned, filter, suffix, fromIntermediates)
  ...
  override fun fromIntermediates(): AabSelector =
    AabSelectorImp(this.name, buildType, testSuite, flavors, isSigned, filter, suffix, fromIntermediates = true)
