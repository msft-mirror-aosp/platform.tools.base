...
data class CompileCommandsJsonBinEntry(val sourceFile: String, val compiler: String, val workingDir: String, val flags: List<String>)
...
fun File.dumpCompileCommandsJsonBin(normalizer: FileNormalizer): String =
  readCompileCommandsJsonBin(normalizer)
    ...
    .joinToString("\n\n") { (sourceFile: String, compiler: String, workingDir: String, rawFlags: List<String>) ->
      val flags = rawFlags.filter { flag -> flag.contains("-target") || flag.contains("none-linux-android") }
      """
...
private fun CxxAbiModel.goldenConfigurationFlags(): String {
  ...
  return metadataGenerationCommandFile
    .readText()
    .slash()
...
fun findConfigurationSegment(file: File): String? {
  ...
  return string.substring(string.lastIndexOf(cxx) + cxx.length + 1, string.lastIndexOf(abi) - 1).replace("/meta", "")
}

/**
 * Discover and build a map (list of pair) from output file subsegment to a human readable name that can be used in tests. For example,
 *
 *      cmake/debug ==> {DEBUG}
...
private fun ModelBuilderV2.FetchResult<ModelContainerV2>.hashEquivalents(): List<Pair<String, String>> {
  ...
  val intermediatesCxxAliases = segments.map { (segment, alias) -> "/cxx/$segment" to "/$alias" }
  val intermediatesAliases = segments.map { (segment, alias) -> "/intermediates/$segment" to "/intermediates/$alias" }
  return configurationAliases + intermediatesCxxAliases + intermediatesAliases
}

/** Recover existing CxxAbiModels that were written to disk during the configure phase. No new configures should be done. */
...
fun recoverExistingCxxAbiModels(rootSearchFolder: File): List<CxxAbiModel> {
  ...
  val models = modelFiles.filter { it.isFile }.map { createCxxAbiModelFromJson(it.readText()) }.distinct()
  if (models.isEmpty()) {
...
fun GradleTestProject.getSoFolderFor(abi: Abi) = recoverExistingCxxAbiModels().singleOrNull { it.name == abi.tag }?.soFolder
...
/**
 * Build a translator that will convert from ".cxx/[some stuff in related to "debug" variant]/x86" to .cxx/{DEBUG}/x86 so that baselines can
 * be compared even with "stuff" changes (as it is for configuration folding).
 */
fun ModelBuilderV2.FetchResult<ModelContainerV2>.cxxFileVariantSegmentTranslator() = hashEquivalents().toTranslateFunction()

/** Sort [NativeModule] with variant names in alphabetic order and ABI names in ordinal order from [Abi]. */
fun NativeModule.sorted(): NativeModule {
  val module = this
  fun sorted(variant: NativeVariant) =
    object : NativeVariant by variant {
      override val abis = variant.abis.sortedBy { abi -> Abi.getByName(abi.name)?.ordinal ?: Int.MAX_VALUE }
    }
  return object : NativeModule by module {
    override val variants = module.variants.map(::sorted).sortedBy { variant -> variant.name }
  }
}
 ...
 * ```
 *
 * where `...` is the common model dumper [NativeModule.writeToBuilder]. The dumper normalizes any file paths relative to well-known
 * directories, for example, project root, SDK root. This makes test easier to write since these directories generally change across
 * platforms and test runs. The normalization is done by [FileNormalizer].
 */
...
fun ModelBuilderV2.FetchResult<ModelContainerV2>.withCxxFileNormalizer(): ModelBuilderV2.FetchResult<ModelContainerV2> {
  val cxxSegmentTranslator = cxxFileVariantSegmentTranslator()
  val normalizer =
    object : FileNormalizer by normalizer {
      override fun normalize(file: File) = cxxSegmentTranslator(normalizer.normalize(file))
    }
...
fun cartesianOf(c1: Array<*>, c2: Array<*>): Array<Array<*>> = c1.flatMap { e1 -> c2.map { e2 -> arrayOf(e1, e2) } }.toTypedArray()

/** Produce all combinations of elements individual arrays in [members]. */
fun cartesianOf(vararg members: Array<*>) =
  cartesianOfIndices(members.map { it.size })
    .map { it.mapIndexed { index, ordinal -> members[index][ordinal] } }
    .map { it.toTypedArray() }
    .toTypedArray()

private fun cartesianOfIndices(sizes: List<Int>): List<List<Int>> =
  if (sizes.size == 1) (0 until sizes[0]).map { listOf(it) }
  else (0 until sizes[0]).flatMap { ordinal -> cartesianOfIndices(sizes.drop(1)).map { inner -> listOf(ordinal) + inner } }

/**
 * Produces all pairs, triples, etc up to size [maxTupleSize] from [row]. Tuples elements are in ascending order according to their place in
 * [row]. The result key is the index of the field in [row]. The result value is the value of the field at index in [row].
 */
private fun tuplesOfRow(
  row: Array<Any?>,
  maxTupleSize: Int,
  startIndex: Int = 0,
  outer: Map<Int, Any?> = mapOf(),
  result: MutableSet<Map<Int, Any?>> = mutableSetOf(),
): Set<Map<Int, Any?>> {
...
/**
 * Find a small set of rows from [this] that covers all pairs, triples, etc of field values. The purposes is to select a smaller but still
 * representative set of tests so that we don't have to run all combinations. If [maxTupleSize] is set to the size of rows in [this] then
...
fun Array<Array<Any?>>.minimizeUsingTupleCoverage(maxTupleSize: Int): Array<Array<Any?>> {
  ...
  while (remainingTuples.isNotEmpty()) {
    ...
    val bestRow = indices.filter { !result.contains(it) }.minByOrNull { (remainingTuples - tuplesOfRow(this[it], maxTupleSize)).size }!!
...
/** Given a [GradleTestProject], read structured log records of a particular type (the type returned by [decode] function). */
inline fun <reified Encoded, Decoded> GradleTestProject.readStructuredLogs(
  crossinline decode: (Encoded, StringDecoder) -> Decoded
): List<Decoded> {
...
val GradleTestProject.lastConfigureInvalidationState: ConfigureInvalidationState
  get() {
...
private fun makePathsShorter(state: ConfigureInvalidationState): ConfigureInvalidationState {
  ...
  return state
    .toBuilder()
    .clearInputFiles()
    .addAllInputFiles(state.inputFilesList.map(::shorten))
    .clearRequiredOutputFiles()
    .addAllRequiredOutputFiles(state.requiredOutputFilesList.map(::shorten))
    .clearOptionalOutputFiles()
    .addAllOptionalOutputFiles(state.optionalOutputFilesList.map(::shorten))
    .clearHardConfigureFiles()
    .addAllHardConfigureFiles(state.hardConfigureFilesList.map(::shorten))
    .clearSoftConfigureReasons()
    .addAllSoftConfigureReasons(state.softConfigureReasonsList.map { it.toBuilder().setFileName(shorten(it.fileName)).build() })
    .clearHardConfigureReasons()
    .addAllHardConfigureReasons(state.hardConfigureReasonsList.map { it.toBuilder().setFileName(shorten(it.fileName)).build() })
    .clearAddedSinceFingerPrintsFiles()
    .addAllAddedSinceFingerPrintsFiles(state.addedSinceFingerPrintsFilesList.map(::shorten))
    .clearRemovedSinceFingerPrintsFiles()
    .addAllRemovedSinceFingerPrintsFiles(state.removedSinceFingerPrintsFilesList.map(::shorten))
    .build()
}

/** Return a total count of processes executed. */
val GradleTestProject.totalProcessExecuted: Int
  get() = readStructuredLogs(::decodeExecuteProcess).size
...
/**
 * Wraps a perfgate build benchmark project with convenience functions for setup and execution. It has setup specific to native projects and
 * isn't necessarily suitable for non-native projects.
 */
class NativeBuildBenchmarkProject(
  ...
  /**
   * Add a repo called [repo] to this project. If [repo] is a zip, then it is unzipped into the project's 'repo' directory. If [repo] is a
   * directory, then it is symlinked into the project's 'repo' directory.
   */
  ...
  fun applyDiff(diff: String) {
    UnifiedDiff(prebuilts.resolve(diff)).apply(src, 3)
  }

  /** Enabled C/C++ structured logging. If there was a previous structured log folder then it is deleted. */
  ...
  /** Read structured log records that match the types in [decode]. Other records are ignored. */
  inline fun <reified Encoded, Decoded> readStructuredLogs(crossinline decode: (Encoded, StringDecoder) -> Decoded): List<Decoded> {
    val structuredLogFolder = getCxxStructuredLogFolder(buildRoot)
  ...
  private fun locate(path: String): File {
    ...
    do {
      ...
      current = current.parentFile ?: error("Could not locate $path")
    } while (true)
  }
  ...
  private fun getLocalRepositories(): List<Path> {
    return when {
      ...
      System.getenv("CUSTOM_REPO") != null -> {
        ...
        Splitter.on(File.pathSeparatorChar).split(customRepo).map { Paths.get(it) }
      }
  ...
  init {
    ...
    getLocalRepositories().forEach { addRepo(it.toFile()) }
    buildDir.mkdirs()
    androidDir.mkdirs()
...
fun assertEqualsMultiline(actual: String, expected: String) {
  ...
  for ((actualLine, expectedLine) in actualLines zip expectedLines) {
    assertThat(actualLine).named("Difference on line $line").isEqualTo(expectedLine)
    ++line
  }
