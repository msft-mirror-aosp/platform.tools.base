 ...
 */
class ModelContainerV2(val infoMaps: Map<String, Map<String, ModelInfo>>, val buildMap: Map<String, BuildInfo>) : Serializable {
  ...
  @JvmOverloads
  fun getProject(projectPath: String? = null, buildName: String = ROOT_BUILD_ID): ModelInfo {
    val info: Map<String, ModelInfo> = infoMaps[buildName] ?: throw RuntimeException("Could not find projects for build '$buildName'")
    ...
    return if (projectPath == null) {
      ...
      info[projectPath] ?: throw RuntimeException("Failed to find model info for $projectPath in build '$buildName'")
    }
  }

  /**
   * Returns the only [NativeModule] when there is no composite builds and a single sub-project setup for native builds
   *
   * (there could be more than one Android sub-project, as long as only one sets up the native build)
   */
  val singleNativeModule: NativeModule
    get() = infoMaps.values.flatMap { it.values }.mapNotNull { it.nativeModule }.single()

  /** Returns the single [ModelInfo] when there is no composite builds and a single Android sub-project. */
