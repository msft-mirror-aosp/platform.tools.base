...
/**
 * Compare models to golden files. Also allows recreating the golden files when passing a special properties
 *
...
open class ModelComparator : BaseModelComparator {
  ...
  open fun with(
    ...
    referenceResult: ModelBuilderV2.FetchResult<ModelContainerV2>? = null,
  ): Comparator {
    return Comparator(this, result, referenceResult)
  }
}

abstract class BasicComparator(private val testClass: BaseModelComparator) {
  /**
   * Runs the comparison
   *
   ...
   */
  protected fun runComparison(name: String, actualContent: String, goldenFile: String) {
    if (System.getenv("GENERATE_MODEL_GOLDEN_FILES").isNullOrEmpty()) {
      ...
      if (TestUtils.runningFromBazel()) {
        ...
        val snapshotFileToIncludeInBazelOutputs =
          TestUtils.getTestOutputDir()
            .toFile()
            .resolve(System.getenv("TEST_BINARY"))
            .parentFile
      ...
      Truth.assertWithMessage("Dumped $name (full version in stdout)").that(actualText).isEqualTo(expectedText.trimEnd())
    } else {
  ...
  protected fun generateStdoutHeader(normalizer: FileNormalizer) {
    println("--------------------------------------------------")
  ...
  private fun findGoldenFileLocation(name: String): File {
    ...
    return File(fullPath).also { FileUtils.mkdirs(it.parentFile) }
  }
  ...
  private fun loadGoldenFile(name: String): String {
    ...
    return Resources.toString(Resources.getResource(testClass.javaClass, resourceName), Charsets.UTF_8)
  }

  private fun getResourceName(name: String) =
    if (name.isNotBlank()) {
...
class Comparator(
  ...
  fun compareVersions(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
  ...
  fun compareBasicAndroidProject(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
  ...
  fun ensureBasicAndroidProjectIsEmpty(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }) {
    ensureModelIsEmpty(
      projectAction = projectAction,
      modelName = "BasicAndroidProject",
      modelAction = { basicAndroidProject },
  ...
  fun compareAndroidProject(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
  ...
  fun ensureAndroidProjectIsEmpty(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }) {
    ensureModelIsEmpty(
      projectAction = projectAction,
      modelName = "AndroidProject",
      modelAction = { androidProject },
  ...
  fun compareAndroidDsl(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
  ...
  fun ensureAndroidDslIsEmpty(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }) {
    ensureModelIsEmpty(
      projectAction = projectAction,
      modelName = "AndroidDsl",
      modelAction = { androidDsl },
  ...
  fun compareVariantDependencies(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
  ...
  fun compareProjectGraph(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
  ...
  fun ensureVariantDependenciesIsEmpty(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }) {
    ensureModelIsEmpty(
      projectAction = projectAction,
      modelName = "VariantDependencies",
      modelAction = { variantDependencies },
  ...
  fun compareNativeModule(projectAction: ModelContainerV2.() -> ModelContainerV2.ModelInfo = { getProject() }, goldenFile: String) {
    compareModel(
      projectAction = projectAction,
