 ...
 *
 * The proxy records all calls to the class and stores them (in [DslRecorder]) so that they can be rewritten in build files, with a choice
 * of format (groovy, kts, declarative).
 *
 ...
 */
class DslProxy private constructor(private val theInterface: Class<*>, internal val dslRecorder: DslRecorder) : InvocationHandler {
  ...
  companion object {
    ...
    fun <T> createProxy(theClass: Class<T>, dslRecorder: DslRecorder): T {
      return when (theClass) {
        ...
        else -> Proxy.newProxyInstance(DslProxy::class.java.classLoader, arrayOf(theClass), DslProxy(theClass, dslRecorder)) as T
      }
    }
  }

  /**
   * Special handling for the namespace property. Because we need the value to finish creating projects (to set the value in the manifest
   * and/or to generate java code in the right folder), we need to not just record calls setting the values, but we need to keep track of
   * the current value, and allow doing a get (which we do not allow for other values).
   */
  ...
  private fun checkSetter(method: Method, args: Array<out Any?>): Boolean {
    ...
    val isNotation =
      if (param.type == Boolean::class.java) {
  ...
  data class MethodReturn(val validResult: Boolean, val value: Any?) {
  ...
  /**
   * Intercepts calls to the DSL for compile/min/max/target Sdk as the pattern is to call a method that returns an item to pass to a setter.
   *
      ...
      CompileSdkSpec::class.java -> {
        when (method.name) {
          ...
          "addon" -> {
            ...
            MethodReturn(
              CompileSdkVersionImpl(vendorName = args.first() as String, addonName = args[1] as String, apiLevel = args[2] as Int)
            )
          }
  ...
  private fun checkGetter(method: Method): MethodReturn {
    ...
    val propName = matcher.groups[1]?.value?.replaceFirstChar { c -> c.lowercaseChar() } ?: throw Error("Expected prop name but null")
    ...
    val returnValue =
      when (method.returnType) {
        ...
        PolymorphicDomainObjectContainer::class.java ->
          ExtensiblePolymorphicDomainObjectContainerProxy(
            extractResolvedTypeParamFromReturn(method),
            dslRecorder.createChainedRecorder(propName),
          )
        DependencyCollector::class.java -> DependencyCollectorProxy(dslRecorder.createChainedRecorder(propName))
        // the rest
        else -> {
          ...
          if (
            method.returnType.name.startsWith("com.android.build.api") ||
  ...
  fun Method.getChainedProxyForReturn(propName: String): Any =
    try {
      val genericType = genericReturnType
      val actualReturnClass =
      ...
      throw RuntimeException("Failed to load ${returnType.name} for ${theInterface.name}.$propName", e)
    }
  ...
  private fun checkNestedBlock(method: Method, args: Array<out Any?>): Boolean {
    ...
    // parameter.
    val blockTypeClass =
      if (blockTypeValue is TypeVariable<*>) {
        // search in the class that defined the method for the index of the type param for
        // the type used in the function.
        val ownerClass = method.declaringClass
    ...
    if (args.size == 1 && blockTypeValue.typeName.startsWith("org.gradle.api.NamedDomainObjectContainer<")) {
      when (blockTypeClass.name) {
        ...
        "com.android.build.api.dsl.TestBuildType" -> {
          ...
          dslRecorder.buildTypes(blockTypeClass as Class<BuildType>) {
            ...
            @Suppress("UNCHECKED_CAST") (args[0] as Function1<Any, *>).invoke(this)
          }
        }
      ...
      dslRecorder.runNestedBlock(
        ...
        @Suppress("UNCHECKED_CAST") (args.last() as Function1<Any, *>).invoke(this)
      }
    }
  ...
  private fun handleMethodCall(method: Method, args: Array<out Any?>?): MethodReturn {
    ...
    if (method.name == "getDefaultProguardFile" && method.declaringClass == CommonExtension::class.java) {
      return MethodReturn(true, MethodReturnedFile(methodName = "getDefaultProguardFile", parameter = args!!.first() as String))
    }
  ...
  private fun <T : BuildType> DslRecorder.buildTypes(theInterface: Class<T>, action: NamedDomainObjectContainerProxy<T>.() -> Unit) {
    runNestedBlock(
      name = "buildTypes",
      parameters = listOf(),
      instanceProvider = { NamedDomainObjectContainerProxy(theInterface, it) },
      action = action,
    )
  }
...
internal class MethodReturnedFile(val methodName: String, val parameter: String) : File(parameter), CustomObjectInstance {
  override fun toString(handler: StringHandler): String = "${methodName}(${handler.quoteString(parameter)})"
}

/** Simple impl of [CompileSdkReleaseSpec] to run the action provided to [CompileSdkSpec.release]. */
...
/**
 * Implementation of [TargetSdkVersion] so that we have an object that is returned by the call to [TargetSdkSpec.release] and other similar
 * methods
 */
private data class TargetSdkVersionImpl(override val apiLevel: Int? = null, override val codeName: String? = null) :
...
/**
 * Implementation of [MinSdkVersion] so that we have an object that is returned by the call to [MinSdkSpec.release] and other similar
 * methods
 */
private data class MinSdkVersionImpl(override val apiLevel: Int? = null, override val codeName: String? = null) :
...
/**
 * Implementation of [MaxSdkVersion] so that we have an object that is returned by the call to [MaxSdkSpec.release] and other similar
 * methods
 */
private data class MaxSdkVersionImpl(override val apiLevel: Int) : MaxSdkVersion, CustomObjectInstance {
  override fun toString(handler: StringHandler): String = """release($apiLevel)"""
}
