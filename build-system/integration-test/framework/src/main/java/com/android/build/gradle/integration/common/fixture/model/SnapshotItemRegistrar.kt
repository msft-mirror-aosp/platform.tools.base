...
data class KeyValueItem(override val name: String, val value: String?, val separator: String = "=") : NamedSnapshotItem

data class ValueOnlyItem(val value: String?) : SnapshotItem
...
interface SnapshotItemRegistrar : SnapshotContainer {
   ...
   */
  fun <T> valueList(name: String, list: Collection<T>?, formatAction: (T.() -> String)? = null)
   ...
   * @param formatAction an optional action to format the item
   */
  fun <T> objectList(name: String, list: Collection<T>?, action: SnapshotItemRegistrar.(Collection<T>) -> Unit)

  /**
   * Adds the content of an object.
   *
   * The object parameters is not used directly. It is passed back to the lambda which must be used to write each property into the new
   ...
   */
  fun <T> dataObject(name: String, obj: T?, action: T.(SnapshotItemRegistrar) -> Unit)
}
