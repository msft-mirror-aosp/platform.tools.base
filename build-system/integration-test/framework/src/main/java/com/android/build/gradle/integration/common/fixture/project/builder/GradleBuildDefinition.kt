...
interface GradleBuildDefinition {
  ...
  /** The name of the build. This is the value set in the settings.gradle files as the logical name of the build in gradle */
  val name: String
   ...
   *
   * This should only be used when trying to create situation where different projects have different classloaders for AGP.
   *
   * Note that this is not enough to get different classloaders. Each project should also have a different classpath. One can use local jars
   * on each project using [GradleProjectDefinition.buildscript] to achieve this.
   */
  var useOldPluginStyleForSeparateClassloaders: Boolean

  /**
   * Whether to modify the buildscript classpath to upgrade KGP from the version that AGP depends on to the latest version
   * ([com.android.testutils.TestUtils.KOTLIN_VERSION_FOR_TESTS]).
   *
  ...
  fun aiPack(path: String, action: AiPackDefinition.() -> Unit): AiPackDefinition

  /** Configures a subProject with the Android Asset Pack plugin, creating it if needed. */
  fun assetPack(path: String, action: AssetPackDefinition.() -> Unit): AssetPackDefinition
  ...
  fun fusedLibrary(path: String, createMinimumProject: Boolean = true, action: FusedLibraryDefinition.() -> Unit): FusedLibraryDefinition

  /** Configures a subproject with the Java Library plugin, creating it if needed. */
  fun javaLibrary(path: String, action: JavaLibraryProjectDefinition.() -> Unit): JavaLibraryProjectDefinition

  /** Configures a subProject with the Kotlin multiplatform plugin, configured with the Android plugin, creating it if needed. */
  ...
  fun kotlinMultiplatformLibrary(path: String, action: KotlinMultiplatformDefinition.() -> Unit): KotlinMultiplatformDefinition

  /**
   * Configures an existing project in a generic way.
   *
   * The project definition *must* have been created beforehand. The project can be of any type, but the action is generic.
   */
  fun configure(path: String, action: GradleProjectDefinition.() -> Unit)
