...
class SnapshotItemWriter {
  ...
  private fun write(item: SnapshotItem, sb: StringBuilder, indent: String, keySpacing: Int) {
    when (item) {
  ...
  private fun KeyValueItem.write(sb: StringBuilder, indent: String, keySpacing: Int) {
    sb.append(indent).append("- ").appendNameAndSpacing(name, keySpacing).append(separator).append(' ').append(value).append("\n")
  }
  ...
  private fun SnapshotContainer.write(sb: StringBuilder, indent: String = "", parentKeySpacing: Int = 0) {
    ...
    items?.forEach { item -> this@SnapshotItemWriter.write(item, sb, newIndent, keySpacing) }
  ...
  private fun SnapshotContainer.writeHeader(sb: StringBuilder, indent: String, keySpacing: Int, withFooter: Boolean) {
  ...
  private fun sizeOf(container: SnapshotContainer): Int =
    sizeOfHolderMap[container] ?: throw RuntimeException("Failed to find size for holder ${container.name}")
}
