...
interface ClassesSubject : FileArchiveSubject {
   ...
   *
   * The format of the class names is using the binary format. For example: com/example/Foo$InnerClass
   *
   * The possible format of the items in the provided list includes
   * - normal binary class names
   * - packages/folders (ending with /), in which case it will match against any classes in the archive that are in that package.
  ...
  /** creates a [ClassDefinitionSubject] for the class with the given binary name, and configures it with the given action */
  fun classDefinition(binaryName: String, action: ClassDefinitionSubject.() -> Unit) {
    action(classDefinition(binaryName))
  }

  /** Returns a [BinarySubject] with the binary content of class with the given binary name. */
...
abstract internal class BaseDexSubject<S : Subject<S, T>, T>(metadata: FailureMetadata, actual: T) :
  Subject<S, T>(metadata, actual), ClassesSubject {

  override fun containsExactly(items: Collection<String>) {
    check("entries()").about(ArchiveEntriesSubject.classes()).that(allClasses.keys).containsExactly(items)
  }
  ...
  override fun containsAtLeast(items: Collection<String>) {
    check("entries()").about(ArchiveEntriesSubject.classes()).that(allClasses.keys).containsAtLeast(items)
  }
  ...
  override fun classDefinition(binaryName: String): ClassDefinitionSubject {
    ...
    return check("classDefinition($binaryName)").about(ClassDefinitionSubject.classes()).that(ClassDefinitionFromDex(classDef))
  }
...
internal class DexSubject(
  ...
  companion object {
    ...
    internal fun dexFiles(rootPackage: String = ""): Factory<DexSubject, List<Dex>> {
      return Factory<DexSubject, List<Dex>> { metadata, actual -> DexSubject(metadata, actual, rootPackage) }
    }
  }
  ...
  override fun subPackage(name: String): ClassesSubject {
    ...
    // from them
    val name = name.replace('.', '/').run { if (endsWith('/')) this else "$this/" }
   ...
   * The names have been sanitized to not contain the L; wrapper.
   */
  override val allClasses: Map<String, DexBackedClassDef> by
...
internal class DexClassesFromApkSubject(
  ...
  companion object {
    ...
    internal fun apk(rootPackage: String = ""): Factory<DexClassesFromApkSubject, Zip> {
      return Factory<DexClassesFromApkSubject, Zip> { metadata, actual -> DexClassesFromApkSubject(metadata, actual, rootPackage) }
    }
  }
...
internal class JarWithClassesSubject(metadata: FailureMetadata, actual: Zip) :
  ...
  companion object {
    ...
    internal fun jars(): Factory<JarWithClassesSubject, Zip> {
      return Factory<JarWithClassesSubject, Zip> { metadata, actual -> JarWithClassesSubject(metadata, actual) }
    }
  }

  override fun containsExactly(items: Collection<String>) {
    check("entries()").about(ArchiveEntriesSubject.classes()).that(this.classNames).containsExactly(items)
  }
  ...
  override fun containsAtLeast(items: Collection<String>) {
    check("entries()").about(ArchiveEntriesSubject.classes()).that(this.classNames).containsAtLeast(items)
  }
  ...
  override fun classDefinition(binaryName: String): ClassDefinitionSubject {
    ...
    actual().binaryFile(binaryName.toPath())?.let { ClassReader(it).accept(classNode, 0) }

    return check("classDefinition($binaryName)").about(ClassDefinitionSubject.classes()).that(ClassDefinitionFromAsm(classNode))
  }

  override fun classAsBytes(binaryName: String): BinarySubject {
    check("classes()").that(classNames).contains(binaryName)
    val content = actual().binaryFile(binaryName.toPath())
    return check("classAsBytes($binaryName)").about(BinarySubject.bytes()).that(content)
  }
   ...
   */
  private val classNames by lazy(LazyThreadSafetyMode.NONE) { actual().getEntries(PATTERN_CLASS_FILE).map { it.removeSuffix(".class") } }
}
