...
class ExtensiblePolymorphicDomainObjectContainerProxy<T : Any>(
    ...
    override fun <U : T> create(
        ...
        dslRecorder.runNestedBlock(
            name = "create",
            parameters = listOf(name, type),
            instanceProvider = { DslProxy.createProxy(type, it) }
        ) {
    ...
    override fun <U : T?> registerFactory(
        type: Class<U?>,
        factory: Closure<out U?>
    ) {
        throw RuntimeException("Not Supported")
    }

    override fun <U : T?> registerBinding(
        type: Class<U?>,
        implementationType: Class<out U?>
    ) {
        throw RuntimeException("Not Supported")
    }
