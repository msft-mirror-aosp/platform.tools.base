 ...
 *
 * Each instance only handles the direct properties of the provided model. For nested models, new instances are created with their own
 * matching reference model (if applicable)
 *
...
internal fun <ModelT> snapshotModel(
  ...
  val projectSnapshotContainer = getSnapshotContainer(modelName, modelAction, project, action)

  val finalContainer =
    if (referenceProject != null) {
      val reference = getSnapshotContainer(modelName, modelAction, referenceProject, action)
...
private fun <ModelT> getSnapshotContainer(
  ...
  val registrar = SnapshotItemRegistrarImpl(modelName, SnapshotContainer.ContentType.OBJECT_PROPERTIES)
...
internal fun <ModelT> checkEmptyDelta(
  ...
  val referenceSnapshotContainer = getSnapshotContainer(modelName, modelAction, referenceProject, action)
 ...
 *
 * The instance is used only for the provided [model] object. Each new nested object will use its own instance.
 */
class ModelSnapshotter<ModelT>(
  ...
  fun <PropertyT> item(name: String, propertyAction: ModelT.() -> PropertyT?, modifyAction: (PropertyT?) -> Any? = { it }) {
    basicProperty(propertyAction, modifyAction) { registrar.item(name, it) }
  }

  fun value(propertyAction: ModelT.() -> Any?) {
    basicProperty(propertyAction) { registrar.value(it) }
  }
  ...
  fun artifactKey(propertyAction: ModelT.() -> String): String =
    normalizeArtifactAddress(propertyAction(model)).also { registrar.item("key", it) }

  fun normalizeArtifactAddress(propertyAction: ModelT.() -> String): String = normalizeArtifactAddress(propertyAction(model))
  ...
  internal fun normalizeArtifactAddress(address: String): String {
    ...
    return if (address.startsWith(LOCAL_JAR_PREFIX)) {
      ...
      if (address.startsWith(LOCAL_ASAR_PREFIX)) {
  ...
  fun <PropertyT> list(
    ...
    val valueObject =
      propertyAction(model)?.format(formatAction)?.map { it.toValueString(normalizer) }?.let { if (sorted) it.sorted() else it }

    registrar.item(name, valueObject?.toString() ?: NULL_STRING)
  }

  fun <PropertyT> dataObject(name: String, propertyAction: (ModelT) -> PropertyT?, action: ModelSnapshotter<PropertyT>.() -> Unit) {
    registrar.dataObject(name, propertyAction(model)) { action(ModelSnapshotter(registrar = it, model = this, normalizer = normalizer)) }
  }
  ...
  fun <PropertyT> valueList(
    ...
    registrar.valueList(name, sortAction(propertyAction(model)).format(formatAction)?.map { it.toNormalizedStrings(normalizer) })
  }
    ...
    registrar.objectList(name, objects) { itemList ->
      for (item in itemList) {
        ...
        dataObject(nameAction(item, this@ModelSnapshotter), item) { itemHolder ->
          action(ModelSnapshotter(registrar = itemHolder, model = this, normalizer = normalizer))
        }
      }
    }
  }
 ...
 */
fun Any?.toNormalizedStrings(normalizer: FileNormalizer): Any =
  when (this) {
    null -> NULL_STRING
    is File -> normalizer.normalize(this)
...
/**
 * Turns version string like "8.1" to a regex matching precisely that, and not 0.8.1 or 8.10. It will not falsely match what is followed by
 * a full stop and a digit (1.2 will not replace part of 1.2.3), but will match when followed by a full stop and a letter (1.2.jar will be
 * accepted).
 */
