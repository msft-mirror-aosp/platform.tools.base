...
class BinarySubject internal constructor(metadata: FailureMetadata, actual: ByteArray?) :
  ...
  companion object {
    ...
    internal fun bytes(): Factory<BinarySubject, ByteArray?> {
      return Factory<BinarySubject, ByteArray?> { metadata, actual -> BinarySubject(metadata, actual) }
    }
  }
  ...
  fun isEqualTo(content: ByteArray) {
    if (!array().contentEquals(content)) {
      failWithActual(Fact.fact("expected", content.contentToString()), Fact.fact("but was", actual().contentToString()))
    }
  }
  ...
  fun contains(content: ByteArray) {
    ...
    if (index == -1) {
      failWithActual(Fact.simpleFact("Did not find byte sequence"))
    }
  }
  ...
  fun doesNotContain(content: ByteArray) {
    val index: Int = Bytes.indexOf(array(), content)
    if (index != -1) {
      failWithActual(Fact.simpleFact("Found byte sequence ${content.contentToString()} at index $index"))
    }
  }
