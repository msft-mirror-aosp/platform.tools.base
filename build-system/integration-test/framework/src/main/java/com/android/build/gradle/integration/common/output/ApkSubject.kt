...
@SubjectDsl
class ApkSubject(metadata: FailureMetadata, actual: Zip) : AbstractAndroidArchiveSubject<ApkSubject, Zip>(metadata, actual) {

  companion object {
    /** Runs the provided action on an [ApkSubject] that is created for the zip at the provided path. */
    fun assertThat(path: Path, action: ApkSubject.() -> Unit) {
      SimpleZip(path).use { action(assertAbout(apks()).that(it)) }
    }
    ...
    fun assertThat(path: Path, action: Consumer<ApkSubject>) {
      SimpleZip(path).use { action.accept(assertAbout(apks()).that(it)) }
    }
    ...
    internal fun apks(): Factory<ApkSubject, Zip> {
      return Factory<ApkSubject, Zip> { metadata, actual -> ApkSubject(metadata, actual) }
    }
  }
  ...
  override fun manifest(): StringSubject {
    ...
    val manifestContent =
      (actual() as? SimpleZip)?.archivePath?.let { zip ->
        val processExecutor: ProcessExecutor = DefaultProcessExecutor(StdLogger(StdLogger.Level.ERROR))
  ...
  fun manifestAsNodes(): NodeSubject {
    ...
    val manifestContent: List<String>? =
      (actual() as? SimpleZip)?.archivePath?.let { zip ->
        val processExecutor: ProcessExecutor = DefaultProcessExecutor(StdLogger(StdLogger.Level.ERROR))
        val parser = ApkInfoParser(TestUtils.getAapt2().toFile(), processExecutor)
        val manifestLines = parser.getManifestContent(zip.toFile())
    ...
    if (manifestContent != null) {
      ...
      return check("manifestAsNodes()").about(NodeSubject.nodes()).that(parseManifestToNodes(manifestContent))
    }
   ...
   */
  private val codeJarSubject: ClassesSubject by lazy { check("classes()").about(DexClassesFromApkSubject.apk()).that(actual()) }
  ...
  fun apkMetadata(): PropertiesSubject {
    ...
    path?.let { it.bufferedReader(Charsets.UTF_8, it.fileSize().toInt()).use { reader -> properties.load(reader) } }
  ...
  private fun checkApkSigningBlock(): Boolean {
    ...
    if (zipFile == null) {
      failWithActual(Fact.simpleFact("expected to be a direct Zip"), Fact.simpleFact("but was ${actual().javaClass.name}"))
  ...
  private fun checkAlignment(checkName: String, pageSize: String? = null) {
   ...
   *
   * Computing entry list by filtering the content of the whole APK can be costly so it's better cached.
   */
  private val javaResourcesSubject by lazy { check("javaResources()").about(ApkWithJavaResourcesSubject.javaResources()).that(actual()) }

  private val apkInfo: ApkInfoParser.ApkInfo by
  ...
  private val secondaryDexesList: List<Dex> by
    lazy(LazyThreadSafetyMode.NONE) {
      buildList {
        var index = 2
...
private val APK_SIG_BLOCK_MAGIC =
