...
class ComparatorSubject internal constructor(
    ...
    private fun checkContains(
        ...
        val expectedInner = if (mode == Mode.CLASSES) {
            expected
                .filter { it.endsWith('$') }
                .map {
        ...
        val facts = buildList {
            ...
            if (actual().size > 1 || expected.size > 1) {
                if (missingItems.isNotEmpty()) {
                    add(
                        missingItems.toFact(
                            singleLabel = "missing",
                            multipleLabelAction = { "missing ($size)" }
                        )
                    )
                }
                ...
                if (unexpectedValues.isNotEmpty()) {
                    add(
                        unexpectedValues.toFact(
                            singleLabel = "unexpected",
                            multipleLabelAction = { "unexpected ($size)" }
                        )
                    )
                }
            }
            ...
            if (exactMatch) {
                add(
                    expected.sorted().toFact(
                        singleLabel = "expected",
                        multipleLabel = "expected"
                    )
                )
            } else {
                add(
                    expected.sorted().toFact(
                        singleLabel = "expected to contain",
                        multipleLabel = "expected at least"
                    )
                )
            }
