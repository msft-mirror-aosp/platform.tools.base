 ...
 */
abstract class BaseGradleExecutor<T : BaseGradleExecutor<T>>
internal constructor(
  ...
  init {
    gradleTestInfo.profileDirectory?.let { with(StringOption.PROFILE_OUTPUT_DIR, it.toString()) }
  }
  ...
  fun withArgument(argument: String): T {
    require(!(argument.startsWith("-Pandroid") && !argument.contains("testInstrumentationRunnerArguments"))) {
  ...
  /**
   * Sets whether to run Gradle with a per-test preference root. (The preference root outside of test is normally ~/.android.)
  ...
  /**
   * API option 1 This allows writing
   *
   * ```
  ...
  /**
   * API option 2 This allows writing
   *
   * ```
  ...
  private fun ifAppliesKotlinPlugin(testProject: GradleTestProject): Boolean {
    ...
    for (buildFile: File in FileUtils.find(rootProject.projectDir, Pattern.compile("build\\.gradle"))) {
      if (
        buildFile.readLines(Charsets.UTF_8).stream().anyMatch { s: String ->
          s.contains("apply plugin: 'kotlin'") || s.contains("apply plugin: 'kotlin-android'")
        }
      ) {
        return true
      }
  ...
  protected fun setJvmArguments(launcher: LongRunningOperation) {
    val jvmArguments: MutableList<String> = ArrayList(gradleOptionsDelegate.asGradleOptions.memoryRequirement.asJvmArgs)
  ...
  protected fun maybePrintJvmLogs(failure: GradleConnectionException) {
    ...
    if (
      stacktrace.contains("org.gradle.launcher.daemon.client.DaemonDisappearedException") ||
  ...
  enum class ConfigurationCaching {
    ...
    /**
     * Disables configuration cache (i.e., pass `--no-configuration-cache` to the build's arguments).
     *
  ...
  companion object {
    ...
    protected fun <LauncherT : ConfigurableLauncher<LauncherT>, ResultT> runBuild(
      launcher: LauncherT,
      runAction: RunAction<LauncherT, ResultT>,
    ): ResultT {
    ...
    private fun printThreadDumps() {
      if (SdkConstants.currentPlatform() != SdkConstants.PLATFORM_LINUX && SdkConstants.currentPlatform() != SdkConstants.PLATFORM_DARWIN) {
      ...
      val lines: Array<String> = processes.split(System.lineSeparator().toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
