 ...
 *
 * This rule create a gradle project in a temporary directory. It can be use with the @Rule or @ClassRule annotations. Using this class
 * with @Rule will create a gradle project in separate directories for each unit test, whereas using it with @ClassRule creates a single
 * gradle project.
 *
 * The test directory is always deleted if it already exists at the start of the test to ensure a clean environment.
 */
@Deprecated("Use GradleRule instead")
open class GradleTestProject
@JvmOverloads
constructor(
  ...
  companion object {
    ...
    @JvmField val DEFAULT_BUILD_TOOL_VERSION: String
    ...
    /**
     * List of Apk file reference that should be closed and deleted once the TestRule is done. This is useful on Windows when Apk will lock
     * the underlying file and most test code do not use try-with-resources nor explicitly call close().
     */
    ...
    private fun computeLatestGradleCheckedIn(): String? {
      ...
      val revisionsCmp: Comparator<Pair<String, String>> =
        Comparator.nullsFirst(
          Comparator.comparing { it: Pair<String, String> -> GradleVersion.version(it.first) }
      ...
      gradleDir.listFiles()?.forEach { f ->
        ...
        if (matcher.matches()) {
          val current = Pair.of(matcher.group(1), Strings.nullToEmpty(matcher.group(2)))
...
""",
    ...
    /**
     * Returns the prebuilts CMake folder for the requested version of CMake. Note: This function returns a path within the Android SDK
     * which is expected to be used in cmake.dir.
     */
    @JvmStatic
    fun getCmakeVersionFolder(cmakeVersion: String): File {
      val cmakeVersionFolderInSdk = TestUtils.getSdk().resolve(String.format("cmake/%s", cmakeVersion))
      if (!Files.isDirectory(cmakeVersionFolderInSdk)) {
        throw RuntimeException(String.format("Could not find CMake in %s", cmakeVersionFolderInSdk))
      }
      return cmakeVersionFolderInSdk.toFile()
    }

    /** The ninja in 3.6 cmake folder does not support long file paths. This function returns the version that does handle them. */
    ...
    init {
      try {
        ...
        DEFAULT_BUILD_TOOL_VERSION =
          MoreObjects.firstNonNull(envBuildToolVersion, ToolsRevisionUtils.DEFAULT_BUILD_TOOLS_REVISION.toString())
        val envVersion = Strings.emptyToNull(System.getenv("CUSTOM_PLUGIN_VERSION"))
        ANDROID_GRADLE_PLUGIN_VERSION = MoreObjects.firstNonNull(envVersion, Version.ANDROID_GRADLE_PLUGIN_VERSION)
    ...
    internal val allowedTests by lazy {
      val contentUrl =
        GradleTestProject::class.java.getResource("/allow-listed-test-classes.txt")
  ...
  private val ndkSymlinkPath: File? by lazy { relativeNdkSymlinkPath?.let { location.testLocation.buildDir.resolve(it).canonicalFile } }
  ...
  private val projectConnection: ProjectConnection by lazy {
    ...
    (connector as DefaultGradleConnector).daemonMaxIdleTime(GRADLE_DEAMON_IDLE_TIME_IN_SECONDS, TimeUnit.SECONDS)

    connector.useGradleUserHomeDir(location.testLocation.gradleUserHome.toFile()).forProjectDirectory(location.projectDir)

    if (targetGradleInstallation != null) {
      connector.useInstallation(targetGradleInstallation)
    } else {
      val distributionName = String.format("gradle-%s-bin.zip", targetGradleVersion ?: GRADLE_TEST_VERSION)
    ...
    connector.connect().also { connection -> rootProject.openConnections?.add(connection) }
  }
  ...
  constructor(
    ...
    Assert.assertTrue("No subproject dir at $projectDir", projectDir.isDirectory)
  }
      ...
      object : Statement() {
        override fun evaluate() {
          if (mutableProjectLocation == null) {
            mutableProjectLocation = initializeProjectLocation(description.testClass, description.methodName, name)
          }
          ...
          try {
            ...
            for (tmpApkFile in tmpApkFiles) {
              try {
                ...
                System.err.println("Error while closing APK file : " + e.message)
              }
              val tmpFile = tmpApkFile.file.toFile()
              if (tmpFile.exists() && !tmpFile.delete()) {
                System.err.println("Cannot delete temporary file " + tmpApkFile.file)
              }
            }
  ...
  private fun checkConfigurationCache() {
    ...
    File(buildDir, "reports").walk().filter { it.isFile }.filter { it.name != "configuration-cache.html" }.forEach(checker::checkReport)
  }
  ...
  private fun populateTestDirectory() {
    ...
    if (testProject != null) {
      testProject.write(projectDir, if (testProject.containsFullBuildScript()) "" else computeGradleBuildscript(), projectRepoScript)
    } else {
  ...
  val booleanOptions: Map<BooleanOption, Boolean>
    get() {
      val builder = ImmutableMap.builder<BooleanOption, Boolean>()
      builder.put(BooleanOption.DISALLOW_DEPENDENCY_RESOLUTION_AT_CONFIGURATION, withDependencyChecker)
  ...
  internal fun getAdditionalMavenRepo(): Path? {
    ...
    if (_additionalMavenRepoDir == null) {
      val moreMavenRepoDir = projectDir.toPath().parent.resolve("additional_maven_repo")
...
""",
  ...
  val intermediatesDir: File
    get() = FileUtils.join(projectDir, "build", SdkConstants.FD_INTERMEDIATES)
  ...
  /**
   * Returns the directory in which profiles will be generated. A null value indicates that profiles may not be generated, though setting
   * [ ][com.android.build.gradle.options.StringOption.PROFILE_OUTPUT_DIR] in gradle.properties will induce profile generation without
   * affecting this return value
   */
  ...
  private val apkDelegate: GeneratesApk by lazy(LazyThreadSafetyMode.NONE) { GeneratesApkDelegate(gradlePath, projectDir.toPath()) }

  override fun assertApk(apkSelector: ApkSelector, action: ApkSubject.() -> Unit) {
    apkDelegate.assertApk(apkSelector, action)
  }

  fun assertApk(apkSelector: ApkSelector, action: Consumer<ApkSubject>) {
    apkDelegate.assertApk(apkSelector) { action.accept(this) }
  }

  override fun getApkLocationForCopy(apkSelector: ApkSelector): Path = apkDelegate.getApkLocationForCopy(apkSelector)

  private val aarDelegate: GeneratesAar by lazy(LazyThreadSafetyMode.NONE) { GeneratesAarDelegate(gradlePath, projectDir.toPath()) }

  override fun assertAar(aarSelector: AarSelector, action: AarSubject.() -> Unit) {
    aarDelegate.assertAar(aarSelector, action)
  }

  fun assertAar(aarSelector: AarSelector, action: Consumer<AarSubject>) {
    aarDelegate.assertAar(aarSelector) { action.accept(this) }
  }
   ...
   *
   * Expected dimensions orders are: - product flavors - build type - other modifiers (e.g. "unsigned", "aligned")
   */
  @Deprecated(
  ...
  fun getApk(vararg dimensions: String?): Apk {
    val dimensionList: MutableList<String?> = Lists.newArrayListWithExpectedSize(1 + dimensions.size)
    ...
    val flavorDimensionList =
      Arrays.stream(dimensions).filter { dimension: String? -> dimension != "unsigned" }.collect(Collectors.toList())
    val apkFile =
      getOutputFile(
        "apk" +
          File.separatorChar +
  ...
  /**
   * Internal Apk construction facility that will copy the file first on Windows to avoid locking the underlying file.
   *
  ...
  public interface ApkType {
    ...
    companion object {
      @JvmStatic
      fun of(name: String, isSigned: Boolean): ApkType {
        return object : ApkType {
      ...
      @JvmStatic
      fun of(name: String, testName: String?, isSigned: Boolean): ApkType {
        return object : ApkType {
          override val buildType: String
            get() = name
      ...
      @JvmField val DEBUG = of("debug", true)
      @JvmField val RELEASE = of("release", false)
      @JvmField val RELEASE_SIGNED = of("release", true)
      @JvmField val ANDROIDTEST_DEBUG = of("debug", "androidTest", true)
      @JvmField val ANDROIDTEST_RELEASE = of("release", "androidTest", true)
      @JvmField val MIN_SIZE_REL = of("minSizeRel", false)
    }
  }
   ...
   */
  @JvmOverloads
  fun getApkAsFile(apk: ApkType, apkLocation: ApkLocation = ApkLocation.Output, vararg dimensions: String): File {
    return getApkAsFile(apkLocation = apkLocation, filterName = null /* filterName */, apkType = apk, dimensions = *dimensions)
  }
  ...
  fun getApk(apk: ApkType, vararg dimensions: String): Apk {
    return getApk(filterName = null, apkType = apk, dimensions = *dimensions, apkLocation = ApkLocation.Output)
  }

  fun getApk(apk: ApkType, apkLocation: ApkLocation, vararg dimensions: String): Apk {
    return getApk(filterName = null, apkType = apk, dimensions = *dimensions, apkLocation = apkLocation)
  }

  /**
   * Return the bundle universal output apk File from the application plugin for the given dimension.
   *
   * Expected dimensions orders are: - product flavors -
   */
  fun getBundleUniversalApk(apk: ApkType): Apk {
    return getOutputApk(ApkLocation.Output, "apk_from_bundle", null, apk, ImmutableList.of(), "universal")
  }

  /**
   * Return the output full split apk File from the application plugin for the given dimension as a File.
   *
   * Expected dimensions orders are: - product flavors -
   */
  @JvmOverloads
  fun getApkAsFile(filterName: String?, apkType: ApkType, apkLocation: ApkLocation = ApkLocation.Output, vararg dimensions: String): File {
   ...
   */
  @JvmOverloads
  fun getApk(filterName: String?, apkType: ApkType, apkLocation: ApkLocation = ApkLocation.Output, vararg dimensions: String): Apk {
    return getOutputApk(apkLocation, "apk", filterName, apkType, ImmutableList.copyOf(dimensions), null)
  }
  ...
  private fun getOutputApkFile(
    ...
    return getOutputFile(
      ...
      pathPrefix +
        ...
        if (apkType.isSigned) SdkConstants.DOT_ANDROID_PACKAGE else "-unsigned" + SdkConstants.DOT_ANDROID_PACKAGE,
    )
  }
  ...
  private fun getOutputApk(
    ...
    return _getApk(getOutputApkFile(apkLocation, pathPrefix, filterName, apkType, dimensions, suffix))
  }
  ...
  fun getBundle(type: ApkType): Aab {
    val bundles = outputDir.resolve("bundle/${type.buildType}/").walk().filter { it.extension == SdkConstants.EXT_APP_BUNDLE }.toList()
    if (bundles.size > 1) {
  ...
  private fun mangleApkName(apkType: ApkType, filterName: String?, dimensions: List<String?>, suffix: String?): String {
    val dimensionList: MutableList<String?> = Lists.newArrayListWithExpectedSize(1 + dimensions.size)
    dimensionList.add(name)
  ...
  fun executor(): GradleTaskExecutor {
    return applyOptions(GradleTaskExecutor(location, this, gradleOptions, projectConnection) { it -> setLastBuildResult(it) })
  }

  /** Fluent method to get the model. */
  fun modelV2(): ModelBuilderV2 {
    return applyOptions(ModelBuilderV2(location, this, gradleOptions, projectConnection) { setLastBuildResult(it) })
      .withPerTestPrefsRoot(true)
  }

  /** Returns [SyncIssue]s after fetching the model. */
  fun getSyncIssues(projectPath: String? = null, buildName: String = ROOT_BUILD_ID): Collection<SyncIssue> {
    return modelV2().ignoreSyncIssues().fetchModels().container.getProject(projectPath, buildName).issues?.syncIssues.orEmpty()
  }

  fun locateBundleFileViaModel(modelV2: ModelBuilderV2, variantName: String, projectPath: String?): File {
    val bundleFile =
      modelV2.fetchModels().container.getProject(projectPath).androidProject?.getVariantByName(variantName)?.getBundleLocation()

    return bundleFile ?: throw RuntimeException("Failed to get bundle file for $projectPath module")
  }
  ...
  fun locateApkFolderViaModel(variantName: String, projectPath: String?): File {
    val apkFiles =
      modelV2().fetchModels().container.getProject(projectPath).androidProject?.getVariantByName(variantName)?.getApkLocations()
  ...
  fun getApkFromBundleTaskName(variantName: String, projectPath: String?): String {
    val appModel =
      modelV2().fetchModels().container.getProject(projectPath).androidProject
  ...
  fun getBundleTaskName(variantName: String, projectPath: String?): String {
    ...
    val variantMainArtifact = appModel.getVariantByName(variantName).mainArtifact
    return variantMainArtifact.bundleInfo?.bundleTaskName ?: throw RuntimeException("Module $projectPath does not have bundle task name")
  }
  ...
  private fun createLocalProp(destDir: File): File {
    val localProp = ProjectPropertiesWorkingCopy.create(destDir.absolutePath, ProjectPropertiesWorkingCopy.PropertyType.LOCAL)
    if (withSdk) {
      val androidSdkDir = this.androidSdkDir ?: throw RuntimeException("androidHome is null while withSdk is true")
    ...
    if (withCmakeDirInLocalProp && cmakeVersion != null && cmakeVersion.isNotEmpty()) {
      localProp.setProperty(ProjectProperties.PROPERTY_CMAKE, getCmakeVersionFolder(cmakeVersion).absolutePath)
    }
    ndkSymlinkPath?.let { localProp.setProperty(ProjectProperties.PROPERTY_NDK_SYMLINKDIR, it.absolutePath) }
  ...
  /**
   * Creates a single settings script, either settings.gradle or settings.gradle.kts, depending on whether settings.gradle originally exists
   * or not
   */
...
apply from: "$versionCatalogPath"
    ...
    if (gradleBuildCacheDirectory != null) {
      val absoluteFile: File =
        if (gradleBuildCacheDirectory.isAbsolute) gradleBuildCacheDirectory else File(projectDir, gradleBuildCacheDirectory.path)
      settingsContent +=
        """
buildCache {
    local {
...
"""
  ...
  /**
   * Adds an adb timeout to the root project build file and applies it to all subprojects, so that tests using adb will fail fast when there
   * is no response.
   */
  @JvmOverloads
  fun addAdbTimeout(timeout: Duration = Duration.ofSeconds(30)) {
    if (settingsFile.name.endsWith(".kts")) {
      throw Exception("Adding adb timeout in kts file is not supported, " + "please configure it in each project using kotlin dsl")
    } else {
  ...
  fun setIncludedProjects(vararg projects: String) {
    ...
    try {
      TestFileUtils.searchAndReplace(settingsFile, "include '", "//include '")
    } catch (e: Throwable) {}
...
/** Collects some info to debug [GradleConnectionException]/[NoUsableDaemonFoundException] (b/353867542), then rethrows the exception. */
internal fun debugGradleConnectionExceptionThenRethrow(gradleConnectionException: GradleConnectionException, testOutputDir: File): Nothing {
