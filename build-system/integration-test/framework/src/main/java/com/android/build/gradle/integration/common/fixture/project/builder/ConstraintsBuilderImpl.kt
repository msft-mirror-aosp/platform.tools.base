...
internal class ConstraintsBuilderImpl : DependenciesConfigurationsBuilder by DependencyConfigurationsBuilderImpl(), ConstraintsBuilder {

  override fun runtime(dependency: Any, action: (DependencyBuilder.() -> Unit)?) {
          ...
          when (dependencyConstraint) {
            ...
            is ExternalDependencyBuilder -> {
              if (dependencyConstraint.testFixtures) {
                dependency(scope, rawMethod("testFixtures", dependencyConstraint.coordinate), capability)
              } else {
            ...
            is PlatformDependency -> {
              val path =
                (dependencyConstraint.path as? ProjectDependencyBuilder)?.let { getDependencyNotationForProject(it) }
                  ?: dependencyConstraint.path
              error("$path not allowed. platform dependencies can not be used as a constraint.")
            }

            is MavenRepoGenerator.Library -> dependency(scope, dependencyConstraint.mavenCoordinate.toString(), capability)
