 ...
 */
internal abstract class BaseBuildWriter(indentLevel: Int) : IndentHandler(indentLevel), BuildWriter, StringHandler {
  ...
  protected fun Any?.toFormattedString(isVarArg: Boolean = false): String {
    ...
    return when (this) {
      ...
      is Array<*> -> {
        val allItems = joinToString(separator = ", ") { it -> it.toFormattedString() }
  ...
  override fun method(name: String, singleParam: Any) {
    indent().put(name).put('(').put(singleParam.toFormattedString()).put(')').endLine()
  }

  override fun method(name: String, params: List<Any?>, isVarArg: Boolean) {
    indent().put(name).put('(').put(params.joinToString(separator = ", ") { it.toFormattedString(isVarArg) }).put(')').endLine()
  }

  override fun method(name: String, params: List<Pair<String, Any>>) {
    val namedParams = toNamedParams(params)
    indent().put(name).put('(').put(namedParams.joinToString(separator = ", ")).put(')').endLine()
  }

  override fun pluginId(id: String, version: String?, apply: Boolean) {
    val writer = indent().put("id(").put(quoteString(id)).put(')')

    version?.let { writer.put(" version ").put(quoteString(it)) }
  ...
  override fun dependency(scope: String, value: Any, capability: String?) {
    if (capability != null) {
      block(scope, listOf(value), capability) { block("capabilities") { method("requireCapability", capability) } }
    } else {
      method(scope, value)
    }
  }
  ...
  override fun writeCollectionAddAll(name: String, items: Collection<*>) {
    ...
    val itemStr = items.joinToString(separator = ", ") { it -> it.toFormattedString() }
  ...
  override fun writeMapPut(name: String, key: Any, value: Any?) {
    indent().put(name).put("[").put(key.toFormattedString()).put("] = ").put(value.toFormattedString()).endLine()
  }
  ...
  private fun concatMethodParams(vararg params: Any): String = params.joinToString(separator = ", ") { it -> it.toFormattedString() }

  abstract fun namedParam(name: String): String

  private fun toNamedParams(params: List<Pair<String, Any>>): List<String> {
    return params.map { it -> namedParam(it.first) + it.second.toFormattedString() }
  }

  override fun <T> block(name: String, parameters: List<Any?>, item: T, action: BuildWriter.(T) -> Unit): BuildWriter {
    val blockBuilder = newBuilder(indentLevel + 2)

    val writer = indent().put(name)

    if (parameters.isNotEmpty()) {
      writer.put('(').put(parameters.joinToString(separator = ", ") { it.toFormattedString(false) }).put(')')
    }
  ...
  override fun <T> block(name: String, item: T, action: BuildWriter.(T) -> Unit): BuildWriter {
    return block(name, listOf(), item, action)
  }

  override fun block(name: String, action: BuildWriter.() -> Unit): BuildWriter {
    val blockBuilder = newBuilder(indentLevel + 2)
  ...
  protected fun coordinates(coordinate: Substitution) =
    when (coordinate) {
...
internal class KtsBuildWriter(indentLevel: Int = 0) : BaseBuildWriter(indentLevel) {
  ...
  override fun applyPluginFromClass(pluginClass: String) {
    val updatedClassString =
      if (pluginClass.contains("\$")) pluginClass.substringBeforeLast(".") + ".`" + pluginClass.substringAfterLast(".") + "`"
      else pluginClass
  ...
  override fun mapOf(value: Map<*, *>): String {
    val mapDeclarationContent =
      value.entries.joinToString(separator = ", ") { (key, value) -> "${key.toFormattedString()} to ${value.toFormattedString()}" }
...
internal class DeclarativeBuildWriter(indentLevel: Int = 0) : BaseBuildWriter(indentLevel) {
  ...
  override fun dependencySubstitution(substitute: Substitution, using: Substitution) = error("Not supported yet")
...
internal class GroovyBuildWriter(indentLevel: Int = 0) : BaseBuildWriter(indentLevel) {
  ...
  /**
   * This is actually a map notation. This is used only(?) for the project() call in dependencies where it's a named param in KTS and a map
   * in groovy.
   */
  override fun namedParam(name: String): String = "$name: "
  ...
  override fun mapOf(value: Map<*, *>): String {
    val mapDeclarationContent =
      value.entries.joinToString(separator = ", ") { (key, value) -> "${key.toFormattedString()}: ${value.toFormattedString()}" }
