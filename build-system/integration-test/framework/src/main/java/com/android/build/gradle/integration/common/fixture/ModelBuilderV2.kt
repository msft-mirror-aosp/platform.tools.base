...
internal constructor(
  ...
  data class FetchResult<T>(val container: T, val normalizer: FileNormalizer)
  ...
  /** Java interop friendly signature, as default parameters are not respected. */
  fun fetchModels(variantName: String?, nativeParams: NativeModuleParams?) =
    fetchModels(variantName, parameterMutator = { it.buildAllRuntimeClasspaths() }, nativeParams)

  /**
   * Fetches the [AndroidProject], [VariantDependencies] and [ProjectSyncIssues] for each project and return them as a [ModelContainerV2]
   *
   * @param variantName the name of the variant for which to return [VariantDependencies]
   */
  ...
  /**
   * Fetches the [AndroidProject], [NativeModule] and [ProjectSyncIssues] for each project and return them as a [ModelContainerV2]
   *
   * @param nativeParams the [NativeModuleParams] to configure the native model query
  ...
  private fun fetchGradleProject(): GradleProject = projectConnection.model(GradleProject::class.java).withArguments(getArguments()).get()
  ...
  private fun <T> buildModel(executor: BuildActionExecuter<T>): Pair<T, GradleBuildResult> {
    ...
    return try {
      val model: T =
        BufferedOutputStream(FileOutputStream(stdOutFile)).use { stdout ->
          BufferedOutputStream(FileOutputStream(stdErrFile)).use { stderr ->
            ...
            runBuild(executor) { executor: BuildActionExecuter<T>, resultHandler: ResultHandler<T> -> executor.run(resultHandler) }
          }
        }

      val buildResult =
        GradleBuildResult(stdOutFile, stdErrFile, tasksProgressListener.getEvents(), problemsProgressListener.getEvents(), null)
      ...
      val buildResult =
        GradleBuildResult(stdOutFile, stdErrFile, tasksProgressListener.getEvents(), problemsProgressListener.getEvents(), e)
      lastBuildResultConsumer.accept(buildResult)
      maybePrintJvmLogs(e)
      throw e
    }
  ...
  private fun checkSyncIssues(containerAndResult: Pair<ModelContainerV2, GradleBuildResult>): ModelContainerV2 {
    val (container, buildResult) = containerAndResult
    assertGeneratedProblemsMatchIssues(
      buildResult.problemEvents,
      container.infoMaps.mapValues { it.value.mapValues { it.value.issues?.syncIssues?.toList() ?: emptyList() } },
    )
  ...
  private fun assertGeneratedProblemsMatchIssues(
    ...
    val problemsAsIssues =
      allSingleProblems
        ...
        .map {
          val severity =
            when (it.definition.severity) {
              Severity.WARNING -> SyncIssue.SEVERITY_WARNING
              Severity.ERROR -> SyncIssue.SEVERITY_ERROR
              else -> 0
            }
          val type = it.definition.id.name.toInt()
          val additionalData = (it.additionalData as? CustomAdditionalData)?.get(SyncIssueDataView::class.java)?.data
          """
severity: $severity
type: $type
data: ${additionalData}
...
${it.multiLineMessage}
  ...
  private fun assertNoUnexpectedSyncIssues(container: ModelContainerV2) {
    val allowedOptions: Set<String> = Sets.union(explicitlyAllowedOptions, optionPropertyNames)
    val errors =
      container.infoMaps.entries
        .asSequence()
        .flatMap { buildEntry: Map.Entry<String, Map<String, ModelInfo>> ->
          buildEntry.value.entries.asSequence().map { projectEntry: Map.Entry<String, ModelInfo> ->
            "${buildEntry.key}@@${projectEntry.key}" to removeAllowedIssues(projectEntry.value.issues?.syncIssues, allowedOptions)
          }
        }
        .filter { it.second.isNotEmpty() }
        .map { "project ${it.first} has Sync Issues: ${it.second.joinToString(separator = ", ", prefix = "[", postfix = "]")} " }
        .toList()
  ...
  private fun removeAllowedIssues(issues: Collection<SyncIssue>?, allowedOptions: Set<String>): List<SyncIssue> {
    if (issues == null) {
      return listOf()
    }
    return issues
...
class FileNormalizerImpl(
  ...
  private data class RootData(val root: File, val varName: String, val stringModifier: ((String) -> String)? = null)
  ...
  init {
    ...
    for ((buildName, buildInfo) in buildMap) {
      for ((projectPath, projectDir) in buildInfo.projects) {
        mutableList.add(
          RootData(File(File(projectDir, "build"), ".transforms"), "BUILD_FOLDER($buildName|${projectPath})") {
    ...
    additionalMavenRepo?.let { mutableList.add(RootData(it.toFile(), "ADDITIONAL_MAVEN_REPO")) }

    // then the root build (in case the included ones are inside the root build.
    val rootBuildInfo = buildMap[ModelContainerV2.ROOT_BUILD_ID] ?: throw RuntimeException("Unable to find BuildInfo for root build")
    ...
    androidNdkSxSRoot?.resolve(defaultNdkSideBySideVersion)?.let {
      ...
      mutableList.add(RootData(it.resolve("source.properties").canonicalFile.parentFile, "ANDROID_NDK"))
    }

    val defaultPlatformLocation = "platforms/android-${DEFAULT_COMPILE_SDK_VERSION}/"
    androidSdkDir?.let {
      mutableList.add(
        RootData(it, "ANDROID_SDK") { string -> string.replace(defaultPlatformLocation, "platforms/android-{COMPILE_SDK_VERSION}/") }
      )
    }
    androidPrefsDir?.let { mutableList.add(RootData(it, "ANDROID_PREFS")) }
    ...
    localRepos
      ...
      .forEach { mutableList.add(RootData(it, "LOCAL_REPO")) }
  ...
  override fun normalize(file: File): String {
    val filePath =
      rootDataList.firstNotNullOfOrNull { file.relativeToOrNull(it.root, it.varName, it.stringModifier) } ?: file.invariantSeparatorsPath
  ...
  override fun normalize(value: JsonElement): JsonElement =
    when (value) {
      is JsonNull -> value
      is JsonPrimitive ->
        when {
      ...
      is JsonArray -> JsonArray().apply { value.map(::normalize).forEach(::add) }
      is JsonObject -> JsonObject().apply { value.entrySet().forEach { (key, value) -> add(key, normalize(value)) } }
  ...
  private fun normalize(string: String): String {
    ...
    for ((root, varName, modifier) in rootDataList) {
      ...
      if (s.contains(normalizedRootPath)) {
        ...
        val modifiedStringAfter = modifier?.let { modifier(stringAfter) } ?: stringAfter

        s = "$stringBefore{$varName}/$modifiedStringAfter"
      }
    }

    extraNormalizer?.let { s = it(s) }
    return s
  }
}

fun File.relativeToOrNull(root: File, varName: String, action: ((String) -> String)? = null): String? {
