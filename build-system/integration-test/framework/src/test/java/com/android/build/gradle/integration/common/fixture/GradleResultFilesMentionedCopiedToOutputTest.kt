...
class GradleResultFilesMentionedCopiedToOutputTest {
    ...
    fun copyMentionedFilesTo_allFileTypes() {
        ...
        Truth.assertThat(relativePaths).containsExactly(
            "c.zip",
            "test_results/folder1/a.html"
        )
        Truth.assertThat(filesContent).containsExactly(
            "content of c.zip",
            "content of a.html"
        )
    }
    ...
    fun copyMentionedFilesTo_copiesLocallyReferencedWebFiles() {
        ...
        """.trimIndent()
        val htmlFile = File(testResultsFolder, "a.html").apply {
            writeText(mainHtmlContent)
        }
        File(testResultsFolder, "details.html").apply {
            writeText("content of details.html")
        }
        File(cssFolder, "style.css").apply {
            writeText("body { color: blue; }")
        }
        File(jsFolder, "script.js").apply {
            writeText("console.log('hello');")
        }
        val zipFile = temp.newFile("c.zip").apply { writeText("content of c.zip") }
        val stdOut = getStdOutWithFilePaths(htmlFile, zipFile)
        val result = createGradleBuildResult(stdOut)
        ...
        Truth.assertThat(relativePaths.toSet()).containsExactly(
            "c.zip",
            "test_results/folder1/a.html",
    ...
    private fun getNormalizedRelativePathsOfCopiedFiles(): Pair<List<String>, List<File>> {
        ...
        val relativePaths = filesCopied.map {
            it.relativeTo(outputDestination).path.replace(File.separatorChar, '/')
        }
