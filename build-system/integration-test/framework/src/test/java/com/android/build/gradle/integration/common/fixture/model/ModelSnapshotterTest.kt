...
class ModelSnapshotterTest {
  ...
  private val normalizer =
    FakeFileNormalizer(mapOf(File("/some/important/path") to "IMPORTANT_PATH1", File("/some/other/important/path") to "IMPORTANT_PATH2"))

  private val notImportantPath =
  ...
  fun `many types + normalizer`() {
    ...
    val snapshot = snapshot(largeObject) { snapshotLargeObject() }
  ...
  fun `pathAsAString with a valid unix based path`() {
    ...
    val snapshot = snapshot(smallObject) { pathAsAString("property1", SmallObject::property1) }
  ...
  fun artifactKey() {
    ...
    val snapshot = snapshot(smallObject) { artifactKey(StringObject::string) }
  ...
  fun `valueList with formatter`() {
    ...
    val snapshot = snapshot(smallObject) { valueList("property2", SmallObject::property2, formatAction = { "item($this)" }) }
  ...
  fun `dataObject with null object`() {
    val mainObject = EnclosingObject(property1 = SmallObject("a", listOf()), property2 = null)
  ...
  fun convertedObjectList() {
    val mainObject = ObjectWithMap(mapOf("1" to SmallObject("1", listOf("a")), "2" to SmallObject("2", listOf("b"))))
  ...
  private fun <ModelT> snapshot(model: ModelT, action: ModelSnapshotter<ModelT>.() -> Unit): String {
    val registrar = SnapshotItemRegistrarImpl(model!!::class.java.simpleName, SnapshotContainer.ContentType.OBJECT_PROPERTIES)
...
private fun ModelSnapshotter<EnclosingObject>.snapshotEnclosingObject() {
  dataObject("property1", EnclosingObject::property1) { snapshotSmallObject() }
  dataObject("property2", EnclosingObject::property2) { snapshotLargeObject() }
}
...
data class ObjectWithMap(val map: Map<String, SmallObject>?)
