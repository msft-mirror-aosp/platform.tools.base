...
class AarSubjectTest : BaseZipSubjectTest() {
  ...
  fun classes() {
    createAarWithCodeAndJavaResources().use { aar ->
      assertThat(aar) {
        ...
        mainJar { classes().containsExactly("com/example/SomeClass", "com/example/SomeOtherClass") }
      }

      // test negative results
      expectFailure { it.that(aar).classes().hasSize(5) }
        .assert {
      ...
      expectFailure { it.that(aar).classes().isEmpty() }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
      ...
      expectFailure { it.that(aar).classes().containsExactly("foo/bar") }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was")
          factValue("value of").isEqualTo("aar.classes().entries()")
    ...
    // check on empty aars
    createJar("empty.aar") {}.use { emptyAar -> assertThat(emptyAar) { this.classes().isEmpty() } }
  }
    ...
    createAarWithCodeAndJavaResources().use { aar ->
      assertThat(aar) {
        classes {
          ...
          classDefinition("com/example/SomeClass") { methods().containsExactly("<init>", "foo") }
          classDefinition("com/foo/SomeClass") { methods().containsExactly("<init>", "foo") }
          classDefinition("com/bar/SomeClass") { methods().containsExactly("<init>", "foo") }
        }

        // test that you can call the same inner zip (possibly via a different API)
        // multiple times
        // (must actually read some content from the zip and not rely on cache.)
        mainJar { classes().classDefinition("com/example/SomeClass") { methods().containsExactly("<init>", "foo") } }
      }

      // test negative results
      expectFailure { it.that(aar).classes().classDefinition("com/missing/MissingClass") }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was")
  ...
  fun secondaryJars() {
    createAarWithCodeAndJavaResources().use { aar ->
      assertThat(aar) {
        secondaryJars {
          classes {
            containsExactly("com/foo/SomeClass", "com/foo/SomeOtherClass", "com/bar/SomeClass", "com/bar/SomeOtherClass")
            classDefinition("com/bar/SomeClass") { methods().containsExactly("<init>", "foo") }
          }
          resources {
            containsExactly("foo/file.txt", "bar/file.data")
      ...
      // test negative results
      expectFailure { it.that(aar).secondaryJars().classes().hasSize(5) }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was")
      ...
      expectFailure { it.that(aar).secondaryJars().classes().classDefinition("com/missing/MissingClass") }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was")
  ...
  @Test
  fun mainJar() {
    testJar(methodName = "mainJar", jarName = "classes.jar", jarConfigAction = { withMainJar(it) }, jarSubjectProvider = { mainJar() })
  }
  ...
  fun apiJar() {
    createAar("valid.aar") {
      ...
      .use { aar ->
        assertThat(aar) { apiJar().hasSize(2) }

        // test negative results
        expectFailure { it.that(aar).apiJar().hasSize(5) }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
    ...
    createJar("empty.aar") {}
      .use { emptyAar ->
        expectFailure { it.that(emptyAar).apiJar() }
          .assert {
  ...
  fun javaResources() {
    createAarWithCodeAndJavaResources().use { aar ->
      assertThat(aar) {
        javaResources().apply {
          containsExactly("somefile.txt", "somefile.data", "foo/file.txt", "bar/file.data")
          resourceAsText("foo/file.txt").isEqualTo("foo")
          resourceAsBytes("bar/file.data").isEqualTo("bar".toByteArray())
        }
        ...
        // multiple times
        // (must actually read some content from the zip and not rely on cache.)
        mainJar { resources().resourceAsText("somefile.txt").isEqualTo("foo") }
      }

      // test negative results
      expectFailure { it.that(aar).javaResources().hasSize(5) }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was")
      ...
      expectFailure { it.that(aar).javaResources().isEmpty() }
        .assert {
          // we don't care about testing the 'expected' and 'but was' facts
          factKeys().containsAtLeast("value of", "aar was", "expected to be empty")
    ...
    // check on empty aars
    createJar("empty.aar") {}.use { emptyAar -> assertThat(emptyAar) { javaResources().isEmpty() } }
  }

  @Test
  fun manifest() {
    createAar("valid.aar") { withManifest("foo") }
      .use { aar ->
        assertThat(aar) { manifest().isEqualTo("foo") }

        // test negative results.
        expectFailure { it.that(aar).manifest().contains("bar") }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
    ...
    createJar("empty.aar") {}
      .use { emptyAar ->
        expectFailure { it.that(emptyAar).manifest() }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
  ...
  fun androidResources() {
    createAar("valid.aar") { addResource("values/values.xml", "foo") }
      .use { aar ->
        assertThat(aar) { androidResources().containsExactly("values/values.xml") }

        // test negative results
        expectFailure { it.that(aar).androidResources().hasSize(5) }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
  ...
  fun androidResource_resourceAsText() {
    createAar("valid.aar") {
      ...
      .use { aar ->
        assertThat(aar) { androidResources().resourceAsText("values/values.xml").isEqualTo("foo") }

        // test negative results.
        expectFailure { it.that(aar).androidResources().resourceAsText("values/values.xml").contains("bar") }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
        ...
        expectFailure { it.that(aar).androidResources().resourceAsText("values/missing.xml") }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
  ...
  fun androidResource_resourceAsBytes() {
    createAar("valid.aar") {
      ...
      .use { aar ->
        assertThat(aar) { androidResources().resourceAsBytes("drawable/foo.png").isEqualTo(FAKE_CLASS) }

        // test negative results.
        expectFailure { it.that(aar).androidResources().resourceAsBytes("drawable/foo.png").isEmpty() }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
        ...
        // check querying missing file has right error.
        expectFailure { it.that(aar).androidResources().resourceAsBytes("drawable/missing.png") }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
  ...
  fun assets() {
    createAar("valid.aar") {
      ...
      .use { aar ->
        assertThat(aar) { assets().containsExactly("foo.txt", "bar.txt", "bar.data") }

        // test negative results
        expectFailure { it.that(aar).assets().hasSize(5) }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
  ...
  fun textSymbolFile() {
    ...
    // JarBuilder
    createJar("valid.aar") { addTextFile("R.txt", "foo") }
      .use { aar ->
        assertThat(aar) { textSymbolFile().isEqualTo("foo") }

        // test negative results.
        expectFailure { it.that(aar).textSymbolFile().contains("bar") }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
    ...
    createJar("empty.aar") {}
      .use { emptyAar ->
        expectFailure { it.that(emptyAar).textSymbolFile() }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
  ...
  @Test
  fun lintJar() {
    testJar(methodName = "lintJar", jarName = "lint.jar", jarConfigAction = { withLintJar(it) }, jarSubjectProvider = { lintJar() })
  }
  ...
  fun aarMetadata() {
    ...
    createJar("valid.aar") {
      ...
      .use { aar ->
        ...
        // test negative results.
        expectFailure { it.that(aar).aarMetadata().formatVersion().isEqualTo("2.0") }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
    ...
    createAar("empty.aar") {}
      .use { emptyAar ->
        expectFailure { it.that(emptyAar).aarMetadata() }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
  ...
  private fun testJar(
    ...
    createAar("valid.aar") {
      ...
      .use { aar ->
        assertThat(aar) { jarSubjectProvider().classes().hasSize(2) }

        // test negative results
        expectFailure { it.that(aar).jarSubjectProvider().classes().hasSize(5) }
          .assert {
            // we don't care about testing the 'expected' and 'but was' facts
            factKeys().containsAtLeast("value of", "aar was")
    ...
    createJar("empty.aar") {}
      .use { emptyAar ->
        expectFailure { it.that(emptyAar).jarSubjectProvider() }
          .assert {
            // we want to check for a specific expected/but was here as we want to validate
            // which error is thrown
            factKeys().containsAtLeast("value of", "aar was", "expected to contain", "but was")
