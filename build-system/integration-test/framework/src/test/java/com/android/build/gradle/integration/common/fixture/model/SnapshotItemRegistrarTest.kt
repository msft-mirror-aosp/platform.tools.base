...
internal class SnapshotItemRegistrarTest {
  ...
  fun `test dataObject`() {
    ...
    val snapshot =
      snapshot(mainObject) { enclosingObject ->
  ...
  @Test
  fun `test objectList`() {
    val mainObject = ObjectWithList(listOf(SmallObject("a", listOf("b")), SmallObject("c", null)))
  ...
  private fun <ModelT> snapshot(model: ModelT, action: SnapshotItemRegistrarImpl.(ModelT) -> Unit): String {
...
class FakeFileNormalizer(private val map: Map<File, String> = mapOf()) : FileNormalizer {
  override fun normalize(file: File): String {
    for (entry in map.entries) {
      val result = file.relativeToOrNull(entry.key, entry.value)
      if (result != null) {
        return result
      }
    }
...
data class SmallObject(val property1: String?, val property2: List<String>? = null) : AndroidModel {
  constructor(values: List<String>?) : this(null, values)

  constructor() : this(null, null)
}

data class StringObject(val string: String) : AndroidModel
...
data class EnclosingObject(val property1: SmallObject, val property2: LargeObject?) : AndroidModel

data class ObjectWithList(val list: List<SmallObject>?)
