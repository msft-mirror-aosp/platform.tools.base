...
class DslRecorderTest {
  ...
  @Test
  fun simple() {
    dslRecorder.runNestedBlock(name = "address", parameters = listOf(), instanceProvider = { AddressImpl(it) }) {
      street = "foo"
      city = "bar"
      zipCode = 42
    }
  ...
  @Test
  fun nested() {
    dslRecorder.runNestedBlock("person", parameters = listOf(), instanceProvider = { PersonImpl(it) }) {
      name = "bob"
      surname = null
      age = 42
      address {
  ...
  fun skipNullAndBlock() {
    dslRecorder.runNestedBlock("person", parameters = listOf(), instanceProvider = { PersonImpl(it) }) { name = "bob" }
  ...
  @Test
  fun mapPut() {
    dslRecorder.runNestedBlock("address", parameters = listOf(), instanceProvider = { AddressImpl(it) }) {
      properties["foo"] = "bar"
...
/** Manual implementation of the [Person] interface using the [DslRecorder]. This simulates the code generated by [DslProxy] */
class PersonImpl(private val dslRecorder: DslRecorder) : Person {
  ...
  override fun address(action: Address.() -> Unit) {
    (dslRecorder as DefaultDslRecorder).runNestedBlock("address", parameters = listOf(), { AddressImpl(it) }) { action() }
  }
