...
class NodeSubjectTest {
  ...
  fun namespaces() {
    Truth.assertAbout(NodeSubject.nodes())
      ...
      .containsExactlyNamespaces("android=http://schemas.android.com/apk/res/android", "dist=http://schemas.android.com/apk/distribution")
  }

  @Test
  fun mainNodes() {
    Truth.assertAbout(NodeSubject.nodes()).that(parseManifestToNodes(MANIFEST_EXAMPLE_1.split("\n"))).containsExactlyNodes("manifest")
  }

  @Test
  fun rootNode() {
    val node = Truth.assertAbout(NodeSubject.nodes()).that(parseManifestToNodes(MANIFEST_EXAMPLE_1.split("\n"))).node("manifest")
  ...
  fun chainedNodes() {
    Truth.assertAbout(NodeSubject.nodes())
      ...
      .node("application")
      .node("profileable")
      .containsAttributeAndValue("http://schemas.android.com/apk/res/android:enabled", "true")
  }
  ...
  fun nodeByAttribute() {
    ...
    expectFailure { it.that(manifestContentAsNodes).node("manifest").node("application").nodeByNameAndAttribute("uses-sdk-library", "foo") }
      .assert {
        factKeys().containsExactly("value of", "expected to contain", "but was", "node was").inOrder()
        factValue("value of").isEqualTo("node.node(manifest).node(application).nodeByNameAndAttribute(uses-sdk-library, foo)")
