...
class GradleTaskSubjectTest {
  ...
  fun wasUpToDate() {
    ...
    expectFailureWhenTestingThat(":didWorkTask") { wasUpToDate() }
      .assert {
  ...
  fun wasFromCache() {
    ...
    expectFailureWhenTestingThat(":didWorkTask") { wasFromCache() }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  fun didWork() {
    ...
    expectFailureWhenTestingThat(":upToDateTask") { didWork() }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  fun wasSkipped() {
    ...
    expectFailureWhenTestingThat(":didWorkTask") { wasSkipped() }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  fun failed() {
    ...
    expectFailureWhenTestingThat(":didWorkTask") { failed() }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  fun ranBefore() {
    ...
    expectFailureWhenTestingThat(":didWorkTask") { ranBefore(":upToDateTask") }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  fun ranAfter() {
    ...
    expectFailureWhenTestingThat(":upToDateTask") { ranAfter(":didWorkTask") }
      .assert {
        factKeys().containsExactly("gradleTask was", "expected to be true", "value of")
  ...
  private fun expectFailureWhenTestingThat(taskName: String, action: GradleTaskSubject.() -> Unit): AssertionError {
    return ExpectFailure.expectFailureAbout(GradleTaskSubject.executionStates()) { action(it.that(taskStateList.findTask(taskName))) }
  }

  private class FakeTaskOperationDescriptor(private val name: String) : TaskOperationDescriptor, OperationDescriptor {
