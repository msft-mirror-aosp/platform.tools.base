...
class FileNormalizerTest {
  ...
  private val normalizer =
    FileNormalizerImpl(
      buildMap = mapOf(":" to ModelContainerV2.BuildInfo(":", File("/path/to/Project"), listOf())),
  ...
  fun `Test Outside Paths`() {
    Truth.assertThat(normalizer.normalize(File("/wrong/path"))).isEqualTo("/wrong/path{!}")
  }

  @Test
  fun `Test Project Paths`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/Project/foo"))).isEqualTo("{PROJECT}/foo{!}")
  ...
  fun `Test Gradle User Home`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/Gradle/foo"))).isEqualTo("{GRADLE}/foo{!}")
  }

  @Test
  fun `Test Gradle Transform Cache`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/Gradle/caches/transforms-3/12345678901234567890123456789012/transformed/foo")))
      .isEqualTo("{GRADLE_CACHE}/{CHECKSUM}/transformed/foo{!}")
  }

  @Test
  fun `Test local repos`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/localRepo1/blah"))).isEqualTo("{LOCAL_REPO}/blah{!}")
    Truth.assertThat(normalizer.normalize(File("/path/to/localRepo2/blah"))).isEqualTo("{LOCAL_REPO}/blah{!}")
  }

  @Test
  fun `Test android Sdk`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/Sdk/foo"))).isEqualTo("{ANDROID_SDK}/foo{!}")
  }

  @Test
  fun `Test android Home`() {
    Truth.assertThat(normalizer.normalize(File("/path/to/Home/foo"))).isEqualTo("{ANDROID_PREFS}/foo{!}")
  }
  ...
  fun `Test unscrupulouslyReplace(JsonElement) - windows`() {
    ...
    val normalizer =
      FileNormalizerImpl(
        buildMap = mapOf(":" to ModelContainerV2.BuildInfo(":", File("C:\\path\\to\\Project"), listOf())),
        ...
        localRepos = listOf(Paths.get("C:\\path\\to\\localRepo1"), Paths.get("C:\\path\\to\\localRepo2")),
