 ...
 * other code generators are in the picture.
 */
    ...
    val project = GradleRule.configure().from {
        androidApplication {
            ...
            files {
                ...
                add("src/main/java/com/foo/bar/app/SomeUtil.java",
                    """
                    package com.foo.bar.app;
        ...
        override fun handleExtension(
            ...
            extension.applicationVariants.all { variant ->
                ...
                val outputDir = project.layout.buildDirectory.dir(
                    "build/generated/source/kapt/${variant.name}"
                )
                val kaptProvider = project.tasks.register(
                    "generate${variant.name}KAPTSources",
                    FakeKAPTTask::class.java
                ) { task ->
                    ...
                    task.sources.set(
                        variant.getSourceFolders(SourceKind.JAVA)
                            .filterNot { it.dir.absolutePath == outputDir.get().asFile.absolutePath }
                    )
                }
                variant.registerExternalAptJavaOutput(
                    project.fileTree(outputDir).builtBy(kaptProvider)
                )
            }
        }
    }
    class MyAppCallback: ApplicationComponentCallback {
        override fun handleExtension(
            ...
            androidComponents.onVariants { variant ->
                ...
                val kspTaskProvider = project.tasks.register(
                    "generate${variant.name}KspSources",
                    ...
                    task.sourceFiles.addAll(
                        (variant.sources.java!! as FlatSourceDirectoriesForJavaImpl).allButKspAndKaptGenerators()
                    )
                }
                ...
                project.tasks.register(
                    "lint${variant.name}Sources",
                    KotlinSourceLinter::class.java
                ) { task ->
                    task.outputDir.set(
                        project.layout.buildDirectory.dir(
                            "build/intermediates/linter/${variant.name}"
                        )
                    )
...
/**
 * This is the fake KSP task that gets injected with all the sources including
 * generated ones, but it will NOT see KAPT and itself's output.
 *
 * KSP can see folder project/app/src/main/kotlin
 * KSP can see folder project/app/src/main/java
 * KSP can see folder project/app/src/debug/kotlin
 * KSP can see folder project/app/src/debug/java
 * KSP can see folder project/app/build/generated/kotlin/generatedebugKotlinSources
 * KSP can see folder project/app/src/main/java
 * KSP can see folder project/app/src/debug/java
 * KSP can see folder project/app/build/generated/java/generatedebugJavaSources
 *
 */
abstract class FakeKspTask: DefaultTask() {
    ...
    fun generate() {
        ...
        sourceFiles.get().forEach { directory ->
            System.err.println("KSP can see folder ${directory.asFile.absolutePath}")
        }
...
/**
 * This is random Kotlin code generator that only uses the static sources as inputs.
 * such a task is meant to act like databinding or build configs. It will not see any java or kotlin
 * source code generating task
 *
 * Kotlin Generator can see project/app/src/main/kotlin
 * Kotlin Generator can see project/app/src/main/java
 * Kotlin Generator can see project/app/src/debug/kotlin
...
abstract class KotlinSourceGeneratingTask: DefaultTask() {
    ...
    fun generate() {
        sourceFiles.get().forEach { directory ->
            System.err.println("Kotlin Generator can see ${directory.asFile.absolutePath}")
        }
 ...
 *
 * Kotlin/Java Linter can see project/app/src/main/kotlin
 * Kotlin/Java Linter can see project/app/src/main/java
 * Kotlin/Java Linter can see project/app/src/debug/kotlin
 * Kotlin/Java Linter can see project/app/src/debug/java
 * Kotlin/Java Linter can see project/app/build/generated/kotlin/generatedebugKotlinSources
 * Kotlin/Java Linter can see project/app/src/main/java
 * Kotlin/Java Linter can see project/app/src/debug/java
...
/**
 * This is random Java code generator that only uses the static sources as inputs.
 * such a task is meant to act like databinding or build configs.
 *
 * Java Generator can see app/src/main/java
 * Java Generator can see app/src/debug/java
 */
abstract class JavaSourceGeneratingTask: DefaultTask() {
    ...
    fun generate() {
        sourceFiles.get().forEach { directory ->
            System.err.println("Java Generator can see ${directory.asFile.absolutePath}")
        }
...
/**
 * This is the fake KAPT task which gets injected with all the source including the generated
 * sources
 *
...
abstract class FakeKAPTTask: DefaultTask() {
    ...
    abstract val outputDir: DirectoryProperty
    @get:InputFiles
    abstract val sources: ListProperty<FileTree>
    @TaskAction
    fun generate() {
        System.err.println("KAPT running")
        sources.get().forEach { fileTree ->
            fileTree.files.forEach { file ->
                System.err.println("KAPT can see ${file.absolutePath}")
            }
        }
        val outputFolder = File(outputDir.get().asFile, packageName.get().replace('.', File.separatorChar))
        outputFolder.mkdirs()
        File(outputFolder, "MyKaptClass.java").writeText(
            """
                package ${packageName.get()};
