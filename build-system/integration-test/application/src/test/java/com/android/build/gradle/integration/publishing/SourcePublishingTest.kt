...
class SourcePublishingTest {
  ...
  fun setUp() {
    ...
    val kotlinSourceWithAnnotation = library.mainSrcDir.resolve("com/example/android/java/SomeService.kt")
    FileUtils.createFile(
      kotlinSourceWithAnnotation,
      """
      package com.example.android.java
  ...
  fun testPublishSourceWithSingleVariant() {
    ...
    val sourceJar = project.projectDir.resolve("$SOURCE_JAR_DIR/library-1.1-sources.jar")
  ...
  fun testPublishSourceWithMultipleVariants() {
    ...
    val sourceJar = project.projectDir.resolve("$SOURCE_JAR_DIR/library-1.1-release-sources.jar")
    checkSourceJarContent(sourceJar)
  }
