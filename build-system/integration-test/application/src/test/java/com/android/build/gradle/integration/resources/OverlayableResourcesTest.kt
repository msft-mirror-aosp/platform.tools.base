...
/**
 * Tests support for RROs (Resource Runtime Overlays) tags handling at compile time. These tests make sure we allow the "overlay" tag in the
 * manifest, and the "overlayable" tag in values resources (it makes sure they are not removed by the resource merger, that they are parsed
...
class OverlayableResourcesTest {
  ...
  fun testOverlayableTagIsAllowedAndPassedCorrectly() {
    val appManifest = File(project.getSubproject("app").mainSrcDir.parentFile, "AndroidManifest.xml")
    ...
    val mergedLibValues = libFiles.getIntermediateFile("packaged_res", "debug", "packageDebugResources", "values", "values.xml")

    // Make sure the merged values in lib kept both the overlayable tag and its content.
    assertThat(mergedLibValues).containsAllOf("overlayable name=\"LibraryResources\"", "item name=\"foo_lib\" type=\"string\"")

    val libRdef = libFiles.getIntermediateFile("local_only_symbol_list", "debug", "parseDebugLocalResources", "R-def.txt")
    ...
    val mergedAppValues = appFiles.getIntermediateFile("incremental", "debug", "mergeDebugResources", "merged.dir", "values", "values.xml")

    // Make sure the merged values in app (big merge) contain both overlayable from app and lib,
    // as well as their content.
    assertThat(mergedAppValues)
      .containsAllOf(
        "overlayable name=\"AppResources\"",
    ...
    val appRTxt = appFiles.getIntermediateFile("runtime_symbol_list", "debug", "processDebugResources", "R.txt")
