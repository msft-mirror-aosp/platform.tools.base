...
class AsmTransformApiInstrumentationTest {
  ...
  private fun assertClassesAreInstrumentedInDebugVariant() {
    ...
    checkClassesAreInstrumented(
      ...
      expectedInstrumentedClasses = listOf("ClassWithNoInterfacesOrSuperclasses", "ClassExtendsOneClassAndImplementsTwoInterfaces"),
    )
  ...
  fun localAarAndJarsAreInstrumented() {
    configureExtensionForInterfaceAddingVisitor(
      project,
      classesToInstrument = listOf("com.example.myaarlib.MyAARlib", "com.example.myjar.MyClass"),
    )
  ...
  fun classesInExcludesAreNotInstrumented() {
    ...
    checkClassesAreInstrumented(
      ...
      classesDescriptorPackagePrefix = appClassesDescriptorPrefix,
      expectedClasses = projectClasses,
      expectedAnnotatedMethods = mapOf("ClassExtendsAClassThatExtendsAnotherClassAndImplementsTwoInterfaces" to listOf("f4")),
      expectedInstrumentedClasses = listOf("ClassWithNoInterfacesOrSuperclasses"),
    )
  ...
  fun featureClassesAreInstrumentedInDebugVariant() {
    configureExtensionForAnnotationAddingVisitor(project = project, subProject = ":feature", methodsToAnnotate = listOf("f3"))
    configureExtensionForInterfaceAddingVisitor(
      project = project,
      subProject = ":feature",
    ...
    checkClassesAreInstrumented(
      ...
      expectedAnnotatedMethods = mapOf("ClassExtendsAnAppClass" to listOf("f3")),
      expectedInstrumentedClasses = listOf("ClassExtendsAnAppClass"),
    )
  }

  @Test
  fun unitTestClassesAreInstrumentedInDebugVariant() {
    project.getSubproject(":app").file("src/test/java/com/example/unittest/InstrumentTest.kt").apply {
      parentFile.mkdirs()
      writeText(
        // language=kotlin
        """
  ...
  fun unitTestClassesConsumeInstrumentedAppClasses() {
    project.getSubproject(":app").file("src/test/java/com/example/unittest/AppInstrumentationTest.kt").apply {
      parentFile.mkdirs()
      writeText(
        // language=kotlin
        """
  ...
  fun onlyAnnotationVisitorShouldInstrumentClassesInReleaseVariant() {
    ...
    executor().with(BooleanOption.INCLUDE_DEPENDENCY_INFO_IN_APKS, false).run(":app:assembleRelease")
