...
class CheckMultiApkLibrariesTaskTest {
  ...
  fun `test library collision yields error`() {
    ...
    assertThat(result.failureMessage).contains("Multiple APKs packaging the same library can cause runtime errors.")
  }

  private fun createFeatureSplit(packageName: String) =
    MinimalSubProject.dynamicFeature(packageName)
      .appendToBuild(
        """
