...
class DynamicAppClassesAccessTest(val minified: Boolean) {
  ...
  companion object {
    @JvmStatic @Parameterized.Parameters(name = "{0}") fun minification() = listOf(true, false)
  }
  ...
  fun `ensure tasks can be transformed with PROJECT and ALL scopes simultaneously, minification=`() {
    ...
    TruthHelper.assertThatApk(apk).containsClass("Lcom/example/app/Dummy;")
    TruthHelper.assertThatApk(apk).containsClass("Lcom/android/api/tests/PROJECTInterface;")
    TruthHelper.assertThatApk(apk).containsClass("Lcom/android/api/tests/ALLInterface;")

    // check we have the dependencies.
    TruthHelper.assertThatApk(apk).containsClass("Ljavassist/ClassPool;")

    // check that the dynamic feature classes did not get merged into the main module.
    TruthHelper.assertThatApk(apk).doesNotContainClass("Lcom/android/api/tests/feature1Interface;")
    TruthHelper.assertThatApk(apk).doesNotContainClass("Lcom/android/api/tests/feature2Interface;")

    // but stayed in their respective APKs.
    listOf("feature1", "feature2").forEach { featureName ->
      project.getSubproject(featureName).getApk(GradleTestProject.ApkType.DEBUG).also { apk ->
        TruthHelper.assertThatApk(apk).containsClass("Lcom/android/api/tests/${featureName}Interface;")
      }
    }
  }

  private fun registerCustomizationTask(forScope: ScopedArtifacts.Scope, classNameToGenerate: String = "${forScope.name}Interface") =
    """
