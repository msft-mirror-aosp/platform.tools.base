...
class DynamicAppLegacyMultidexTest {
  ...
  @get:Rule val project: GradleTestProject = GradleTestProject.builder().fromTestProject("multiDex").create()

  private val forcedPrimaryDexClasses =
    listOf(
  ...
  fun before() {
    ...
    project.file("multiDex.txt").writeText(forcedPrimaryDexClasses.joinToString(separator = System.lineSeparator()) { "$it.class" })
  ...
  fun testPrimaryDexContainsAllNecessaryClasses() {
    project.executor().with(BooleanOption.R8_MAIN_DEX_LIST_DISALLOWED, false).with(BooleanOption.USE_ANDROID_X, false).run("bundleIcsR8")

    project.getBundle(GradleTestProject.ApkType.of("icsR8", false)).use { aab ->
      // http://b/163114632
      multiDexSupportLibClasses
        .filter { it != "Landroid/support/multidex/MultiDex\$V4;" }
        .forEach { className -> assertThat(aab).containsMainClass("base", className) }
      forcedPrimaryDexClasses.map { "L$it;" }.forEach { className -> assertThat(aab).containsMainClass("base", className) }
    }
  }
}
