...
/** Test that combines having a POST_COMPILATION_CLASSES generator with registered ASM visitors both the PROJECT and ALL scope. */
@RunWith(Parameterized::class)
class AsmTransformWithPostCompilationTest(val instrumentationScope: InstrumentationScope) {
  companion object {
    @Parameterized.Parameters(name = "scope_{0}")
    @JvmStatic
    fun params() = listOf(arrayOf(InstrumentationScope.ALL), arrayOf(InstrumentationScope.PROJECT))
  }
  ...
  open class AddPostCompilationCallback(val instrumentationScope: InstrumentationScope) : GenericCallback {
    ...
    override fun handleProject(project: Project) {
      ...
      extension.onVariants(extension.selector().withBuildType("debug")) { variant ->
        ...
        variant.instrumentation.transformClassesWith(InterfaceAddingClassVisitorFactory::class.java, instrumentationScope) { params ->
          // these are the types to instrument, note that when the scope if PROJECT,
          // the SomeLibClass will not be visited.
          params.classesToInstrument.setDisallowChanges(listOf("com.android.test.SomeAppClass", "com.android.test.utils.GeneratorUtils"))
          // add this interface to the 2 types above.
          params.interfaceInternalName.setDisallowChanges("com/android/tools/test/InstrumentedInterface")
        }
  ...
  fun testPostCompilationGeneratorSeesTransformedClasses() {
    ...
    project.androidApplication().assertApk(ApkSelector.DEBUG) {
      ...
      classes()
        .classDefinition("com/android/test/SomeAppClass")
        .interfaces()
        .containsExactly("Lcom/android/tools/test/InstrumentedInterface;")
    }
  }
}
...
abstract class InterfaceAddingClassVisitorFactory : AsmClassVisitorFactory<InterfaceAddingClassVisitorFactory.Params> {
  ...
  override fun isInstrumentable(classData: ClassData): Boolean {
    ...
    return parameters.get().classesToInstrument.get().contains(classData.className)
  }

  interface Params : InstrumentationParameters {
    @get:Input val classesToInstrument: ListProperty<String>

    @get:Input val interfaceInternalName: Property<String>
  }
}

class InterfaceAddingClassVisitor(private val interfaceInternalName: String, apiVersion: Int, cv: ClassVisitor) :
  ...
  override fun visit(version: Int, access: Int, name: String?, signature: String?, superName: String?, interfaces: Array<out String>?) {
    super.visit(
      ...
      signature,
      superName,
      if (interfaces == null) arrayOf(interfaceInternalName) else arrayOf(*interfaces, interfaceInternalName),
    )
  }
}
