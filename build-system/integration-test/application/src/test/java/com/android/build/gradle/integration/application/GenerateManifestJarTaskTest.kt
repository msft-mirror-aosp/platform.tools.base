...
class GenerateManifestJarTaskTest(private val enableManifestClass: Boolean) {
  ...
  val project =
    GradleTestProject.builder()
      .addGradleProperties("${BooleanOption.GENERATE_MANIFEST_CLASS.propertyName}=$enableManifestClass")
  ...
  fun `test Manifest Class is generated if there is at least one permission`() {
    val androidManifest = FileUtils.join(project.projectDir, "src", "main", SdkConstants.ANDROID_MANIFEST_XML)
    ...
    FileUtils.writeToFile(
      FileUtils.join(project.projectDir, "src", "main", "java", "com", "example", "helloworld", "ManifestClassTest.java"),
      // language=java
      """
    ...
    FileUtils.writeToFile(
      FileUtils.join(project.projectDir, "src", "test", "java", "com", "example", "helloworld", "ManifestClassUnitTest.java"),
      // language=java
      """
      package com.example.helloworld;
    ...
    if (enableManifestClass) {
      project.execute("assembleDebug", "assembleRelease", "testDebugUnitTest")
    } else {
      project.executeExpectingFailure("assembleDebug", "assembleRelease")
    }

    val manifestJarExists = getManifestJarArtifact().walkBottomUp().filter(File::isFile).map(File::getName).toList()
    if (enableManifestClass) {
  ...
  fun `test builds with no manifest custom permissions`() {
    project.executor().with(BooleanOption.GENERATE_MANIFEST_CLASS, enableManifestClass).run("clean", "assembleDebug")

    // Check Manifest JAR has been created, but contains no entries
    val intermediateCompiledManifest = getManifestJarArtifact().walkBottomUp().filter(File::isFile).map { it.toPath() }.toList()
    assertThat(intermediateCompiledManifest).hasSize(if (enableManifestClass) 1 else 0)
    intermediateCompiledManifest.forEach { manifestJar -> ZipSubject.assertThat(manifestJar) { isEmpty() } }
  }

  private fun getManifestJarArtifact() = project.getIntermediateFile(InternalArtifactType.COMPILE_MANIFEST_JAR.getFolderName())

  companion object {
    @Parameterized.Parameters(name = "enableManifestClass_{0}") @JvmStatic fun params() = listOf(true, false)
  }
}
