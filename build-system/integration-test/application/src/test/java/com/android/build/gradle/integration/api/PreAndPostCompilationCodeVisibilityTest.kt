...
class PreAndPostCompilationCodeVisibilityTest {
        ...
        override fun handleProject(project: Project) {
            ...
            extension
                .onVariants(extension.selector().withBuildType("debug")) { variant ->
                    val preGeneratorTask = project.tasks
                        .register(
                    ...
                    variant.artifacts.forScope(Scope.PROJECT)
                        ...
                        .toAppend(
                            ...
                            AddPreAndPostCompilationCodeGeneratorTask::outputDir
                        )
                }
        }
    }
        ...
        override fun handleExtension(
            ...
            androidComponents
                .onVariants { variant ->
                    project.tasks
                        .register(
                            "verifyVisibility${variant.name.capitalizeFirstChar()}Task",
                            CheckVisibilityTask::class.java
                        ) { task ->
    ...
    fun testVisibility() {
        ...
        val appClasses = build.subProject(":app")
        ...
        JarSubject.assertThat(appClasses) {
            classes().containsAtLeast(
                "com/android/test/post/GeneratorUtils",
                "com/android/test/pre/GeneratorUtils"
            )
        }

        val libClasses = build.subProject(":lib")
        ...
        JarSubject.assertThat(libClasses) {
            classes().containsAtLeast(
                "com/android/test/lib/post/GeneratorUtils",
                "com/android/test/lib/pre/GeneratorUtils"
            )
        }
    }
}
...
abstract class CheckVisibilityTask: DefaultTask() {
    ...
    @get:InputFiles
    @get:Classpath
    abstract val newApiCompileClasspath: ConfigurableFileCollection

    @get:InputFiles
    @get:Classpath
    abstract val newApiRuntimeConfiguration: ConfigurableFileCollection

    @get:InputFiles
    @get:Classpath
    abstract val oldApiRuntimeConfiguration: ConfigurableFileCollection
    ...
    fun verify() {
        ...
        println("New API ")
        newApiRuntimeConfiguration.files.forEach {
            println(it.absolutePath)
        }
        println("End of New API")

        println("Old API ")
        oldApiRuntimeConfiguration.files.forEach {
            println(it.absolutePath)
        }
        println("End of Old API")

        if (oldApiRuntimeConfiguration.files.map { it.absolutePath }
            == newApiRuntimeConfiguration.files.map { it.absolutePath }) {
        ...
        val appClasses = compileClasspath.firstOrNull() {
            it.absolutePath.contains(InternalArtifactType.COMPILE_APP_CLASSES_JAR.getFolderName())
        }
        if (appClasses == null) {
