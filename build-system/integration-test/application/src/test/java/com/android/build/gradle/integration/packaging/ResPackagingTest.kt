...
class ResPackagingTest {
    ...
    fun testAppProjectWithModifiedResFile() {
        ...
        val appProject = build.androidApplication(":app")
        appProject.files.update("src/main/res/raw/file.txt")
    ...
    fun testAppProjectWithNewDebugResFileOverridingMain() {
        ...
        build.androidApplication(":app").checkApkResources(
            "file.txt".withContent("app:abcd"),
            "filelib2.txt",
            "filelib.txt"
        )
    ...
    fun testAppProjectWithModifiedResInDependency() {
        ...
        appProject.files.update("src/main/res/raw/filelib.txt")
            .replaceWith("new content")
        build.executor.run("app:assembleDebug")
        appProject.checkApkResources(
            "filelib.txt".withContent("new content"),
    ...
    fun testAppProjectTestWithModifiedResFile() {
        ...
        val appProject = build.androidApplication(":app")
        appProject.files.update("src/androidTest/res/raw/filetest.txt")
            .replaceWith("new content")

        build.executor.run("app:assembleAT")

        appProject.checkTestApkResources(
            "filetest.txt".withContent("new content")
        )
    }
    ...
    fun testLibProjectWithNewResFileInDebugSourceSet() {
        ...
        // file's been removed, checking in the other direction.
        build.executor.run("library:assembleDebug")
        build.androidLibrary(":library").checkAarResources(
            "filelib.txt".withContent("library:abcd")
        )
    }
    ...
    fun testLibProjectTestWithNewResFileOverridingTestedLib() {
        ...
        build.executor.run("library:assembleAT")
        build.androidLibrary(":library").checkTestApkResources(
            "filelib.txt".withContent("library:abcd"),
            "filelibtest.txt",
            "filelib2.txt"
        )
    ...
    companion object {
        private fun AndroidProjectFiles.createOriginalResFile(
            dimension: String,
            filename: String,
            content: String
        ) {
         ...
         */
        private fun GeneratesApk.checkApkResources(
            vararg itemList: Any
        ) {
         ...
         * presence and content.
         */
        private fun GeneratesApk.checkTestApkResources(
            vararg itemList: Any
        ) {
