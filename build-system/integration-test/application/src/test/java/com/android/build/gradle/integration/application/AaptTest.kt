...
class AaptTest {
  ...
  fun testAaptOptionsFlagsWithAapt2() {
    ...
    build.androidApplication().reconfigure { android.androidResources.additionalParameters.clear() }
  ...
  class TraceFolderCallback : ApplicationComponentCallback {
    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      ...
      androidComponents.onVariants { variant -> variant.androidResources.aaptAdditionalParameters.addAll(listOf("--trace-folder", path)) }
    }
  }
  ...
  fun testIgnoreAssetsPatterns_dsl() {
    val build = rule.build { androidApplication { android.androidResources.ignoreAssetsPattern = "ignored" } }

    build.executor.run("clean", "assembleDebug")

    build.androidApplication().assertApk(ApkSelector.DEBUG) { assets { containsExactly("kept") } }
  }

  class IgnorePatternCallback : ApplicationComponentCallback {

    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      androidComponents.onVariants { variant -> variant.androidResources.ignoreAssetsPatterns.add("ignored") }
    }
  }

  @Test
  fun testIgnoreAssetsPatterns_variantApi() {
    val build = rule.build { androidApplication { pluginCallbacks += IgnorePatternCallback::class.java } }
  ...
  fun testTasksRunAfterAaptOptionsChanges_bundleDebug() {
    testTasksRunAfterAaptOptionsChanges(
      ...
      expectedTasksThatDidWorkOnANoCompressChange =
        listOf(":app:bundleDebugResources", ":app:mergeDebugJavaResource", ":app:packageDebugBundle", ":app:processDebugResources"),
      expectedTasksThatDidWorkOnANoIgnoreAssetsChange = listOf(":app:mergeDebugAssets"),
    )
  }
  ...
  fun testTasksRunAfterAaptOptionsChanges_assembleDebug() {
    testTasksRunAfterAaptOptionsChanges(
      ...
      expectedTasksThatDidWorkOnANoCompressChange =
        listOf(":app:mergeDebugJavaResource", ":app:packageDebug", ":app:processDebugResources"),
      expectedTasksThatDidWorkOnANoIgnoreAssetsChange = listOf(":app:mergeDebugAssets"),
    )
  }
  ...
  class IgnorePatternCallback2 : ApplicationComponentCallback {
    ...
    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      ...
      if (value != null) {
        androidComponents.onVariants { variant -> variant.androidResources.ignoreAssetsPatterns.set(listOf(".ignoreAssetsPatternApi2")) }
      }
    }
  }
  ...
  private fun testTasksRunAfterAaptOptionsChanges(
    ...
    build.executor.run(assembleTask).let { result ->
      Truth.assertThat(result.didWorkTasks).containsAtLeastElementsIn(expectedTasksThatDidWorkOnANoCompressChange)
    }

    app.reconfigure { android.androidResources { ignoreAssetsPattern = ".ignoreAssetsPatternDsl2" } }

    build.executor.run(assembleTask).let { result ->
      Truth.assertThat(result.didWorkTasks).containsAtLeastElementsIn(expectedTasksThatDidWorkOnANoIgnoreAssetsChange)
    }

    // test that tasks run when aapt options changed via the variant API
    build.executor.withArgument("-DnewIgnorePattern=true").run(assembleTask).let { result ->
      Truth.assertThat(result.didWorkTasks).containsAtLeastElementsIn(expectedTasksThatDidWorkOnANoIgnoreAssetsChange)
    }
  }
}
