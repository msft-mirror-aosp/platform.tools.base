...
class KotlinJvmToolchainTest {
  ...
  class KmpVariantApiCallback : AndroidKotlinMultiplatformLibraryComponentCallback {
    override fun handleExtension(project: Project, extension: KotlinMultiplatformAndroidComponentsExtension) {
      extension.onVariants { variant -> variant.configureJavaCompileTask { task -> task.targetCompatibility = "17" } }
    }
  }
    ...
    override fun handleExtension(project: Project, extension: KotlinMultiplatformExtension) {
      extension.apply {
        (this as ExtensionAware).extensions.findByType(KotlinMultiplatformAndroidLibraryTarget::class.java)!!.apply {
          compilations.all { it.compileTaskProvider.configure { compilerOptions.jvmTarget.set(JvmTarget.JVM_11) } }
        }
      }
    }
  }
