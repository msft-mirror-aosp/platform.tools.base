...
class FeatureOnFeatureDependencyTest {
  ...
  private val app = MinimalSubProject.app("com.example.app").appendToBuild("android.defaultConfig.minSdkVersion 14")
  ...
  fun includeUsesSplitInFeatureManifestForMiddleFeature() {
    ...
    assertThat(manifestFile).containsAllOf("<uses-split android:name=\"feature1\" />")

    // Ensure we're not somehow including the wrong dependencies
    assertThat(manifestFile).doesNotContain("<uses-split android:name=\"leafFeature\" />")
    assertThat(manifestFile).doesNotContain("<uses-split android:name=\"baseModule\" />")
  }
  ...
  fun includeUsesSplitInFeatureManifestForLeafFeature() {
    ...
    assertThat(manifestFile).exists()
    assertThat(manifestFile).containsAllOf("<uses-split android:name=\"feature1\" />", "<uses-split android:name=\"middleFeature\" />")
  }
  ...
  fun includeUsesSplitInBundleManifest() {
    ...
    val manifestFile =
      project
        .getSubproject(":leafFeature")
        .getIntermediateFile("packaged_manifests", "debug", "processDebugManifestForPackage", "AndroidManifest.xml")
  ...
  private fun getMergedManifestFile(project: GradleTestProject): File {
    return project.getIntermediateFile("packaged_manifests", "debug", "processDebugManifestForPackage", "AndroidManifest.xml")
  }
}
