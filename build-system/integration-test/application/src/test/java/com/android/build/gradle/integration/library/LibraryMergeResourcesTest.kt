...
class LibraryMergeResourcesTest {
  ...
  @get:Rule val project: GradleRule = GradleRule.from { androidLibrary {} }

  /** Regression test for b/355397971 */
  @Test
  fun `merge res task not executed when includeAndroidResources disabled`() {
    val build = project.build { androidLibrary { android { testOptions.unitTests.isIncludeAndroidResources = false } } }
  ...
  fun `merge res task executed when includeAndroidResources enabled`() {
    val build = project.build { androidLibrary { android { testOptions.unitTests.isIncludeAndroidResources = true } } }
  ...
  fun `test trailing text in xml`() {
    ...
    build.executor.run("clean", ":lib:parseDebugLocalResources").also {
      ...
      it.assertOutputContains("trailing_content_layout.xml contains trailing content. Trailing is stripped during XML parsing.")
