...
class KotlinTestCompilationTest {
  @Rule @JvmField var project = builder().fromTestProject("kotlinApp").create()
  ...
  fun testAccessingInternalMembersForApp() {
    ...
    project.getSubproject("app").file("src/androidTest/java").resolve("com/app/DataTest.kt").also {
      it.parentFile.mkdirs()
      it.writeText(
        """
        package com.app
    ...
    project.getSubproject("app").file("src/test/java").resolve("com/app/DataUnitTest.kt").also {
      it.parentFile.mkdirs()
      it.writeText(
        """
        package com.app
  ...
  fun testAccessingInternalMembersForLibrary() {
    ...
    project.getSubproject("library").file("src/androidTest/java").resolve("com/app/DataTest.kt").also {
      it.parentFile.mkdirs()
      it.writeText(
        """
        package com.app
    ...
    project.getSubproject("library").file("src/test/java").resolve("com/app/DataUnitTest.kt").also {
      it.parentFile.mkdirs()
      it.writeText(
        """
        package com.app
   ...
   *
   * The fasterxml dependency cannot parse the file successfully. With the fix, adding it to the classpath here should not trigger any
   * exception.
   */
  @Test
