...
class AssetPackBundleTest {
  ...
  fun `should build asset pack bundle successfully`() {
    ...
    build.assetPackBundle(":assetPackBundle").assertAab(AabSelector.NO_BUILD_TYPE) {
      ...
      containsFileWithContent("onDemandAiPack/assets/customModel.tflite", onDemandAiPackContent)
    ...
    build.assetPackBundle(":assetPackBundle").withAppBundle(AabSelector.NO_BUILD_TYPE) {
      assertThat(bundleConfig.type).isEqualTo(Config.BundleConfig.BundleType.ASSET_ONLY)
      ...
      assertThat(bundleConfig.optimizations.splitsConfig).isEqualTo(splitsConfigBuilder.build())

      assertThat(bundleConfig.assetModulesConfig)
        .isEqualTo(
          Config.AssetModulesConfig.newBuilder().setAssetVersionTag(VERSION_TAG).addAllAppVersion(VERSION_CODES.map { it.toLong() }).build()
        )

      val moduleNames = assetModules.keys.map { it.name }
      assertThat(moduleNames).containsExactly("assetPackOne", "assetPackTwo", "onDemandAiPack")

      val assetPackOneManifest = assetModules[BundleModuleName.create("assetPackOne")]!!.androidManifest
      assertThat(assetPackOneManifest.moduleType).isEqualTo(BundleModule.ModuleType.ASSET_MODULE)
      assertThat(assetPackOneManifest.packageName).isEqualTo(APP_ID)
      assertThat(assetPackOneManifest.manifestDeliveryElement.get().hasOnDemandElement()).isTrue()

      val assetPackTwoManifest = assetModules[BundleModuleName.create("assetPackTwo")]!!.androidManifest
      assertThat(assetPackTwoManifest.moduleType).isEqualTo(BundleModule.ModuleType.ASSET_MODULE)
      assertThat(assetPackTwoManifest.packageName).isEqualTo(APP_ID)
      assertThat(assetPackTwoManifest.manifestDeliveryElement.get().hasFastFollowElement()).isTrue()

      val onDemandAiPackManifest = assetModules[BundleModuleName.create("onDemandAiPack")]!!.androidManifest
      assertThat(onDemandAiPackManifest.moduleType).isEqualTo(BundleModule.ModuleType.ASSET_MODULE)
      assertThat(onDemandAiPackManifest.optionalModuleTypeAttributeValue).isEqualTo(Optional.of(MODULE_TYPE_AI_VALUE))
      assertThat(onDemandAiPackManifest.packageName).isEqualTo(APP_ID)
      assertThat(onDemandAiPackManifest.manifestDeliveryElement.get().hasOnDemandElement()).isTrue()
    }
  }
  ...
  fun `should fail if requested asset pack is not available in project`() {
    val build = rule.build { assetPackBundle(":assetPackBundle") { bundle { assetPacks += ":notAvailable" } } }

    val failure = build.executor.expectFailure().run(":assetPackBundle:bundle")
    failure.stderr.use { assertThat(it).contains("Unable to find matching projects for Asset Packs: [:notAvailable]") }
  }

  @Test
  fun `should fail if requested compileSdk is not available`() {
    val build = rule.build { assetPackBundle(":assetPackBundle") { bundle { compileSdk = 128 } } }

    val failure = build.executor.expectFailure().run(":assetPackBundle:bundle")
    failure.stderr.use {
      assertThat(it).contains("Could not determine the dependencies of task " + "':assetPackBundle:linkManifestForAssetPacks'")
  ...
  fun `should fail if install-time asset pack is included in asset pack bundle`() {
    val build = rule.build { assetPack(":assetPackTwo") { assetPack { dynamicDelivery.deliveryType.set("install-time") } } }

    val failure = build.executor.expectFailure().run(":assetPackBundle:bundle")
    failure.stderr.use { assertThat(it).contains("bundle contains an install-time asset module 'assetPackTwo'") }
  }
  ...
  fun `should fail if keystore file is signing config is invalid`() {
    ...
    val failure = build.executor.expectFailure().run(":assetPackBundle:bundle")
    failure.stderr.use { assertThat(it).contains("Keystore file '${keystore}' not found") }
  }
}
