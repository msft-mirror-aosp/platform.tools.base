...
class BuiltInKaptTest {
  ...
  fun testAnnotationProcessing() {
    project.executor().run("app:assembleDebug", "app:assembleDebugAndroidTest", "app:compileDebugUnitTestJavaWithJavac")
    val app = project.getSubproject(":app")
    ...
    val kaptGeneratedTestDir = app.buildDir.resolve("generated/source/kapt/test/debug/com/example")
  ...
  fun testGeneratedSourcesModel() {
    val appModel = project.modelV2().ignoreSyncIssues(SyncIssue.SEVERITY_WARNING).fetchModels().container.getProject(":app")
    appModel.androidProject!!.variants.forEach { variant ->
      ...
      val kaptGeneratedClasses =
        InternalArtifactType.BUILT_IN_KAPT_CLASSES_DIR.getOutputDir(project.getSubproject(":app").buildDir)
          .resolve("${variant.name}/$kaptTaskName")
      val kaptGeneratedJava = project.getSubproject(":app").buildDir.resolve("generated/source/kapt/${variant.name}")
      val kaptGeneratedKotlin = project.getSubproject(":app").buildDir.resolve("generated/source/kaptKotlin/${variant.name}")

      assertThat(variant.mainArtifact.classesFolders).contains(kaptGeneratedClasses)
      assertThat(variant.mainArtifact.generatedClassPaths.values).contains(kaptGeneratedClasses)
      assertThat(variant.mainArtifact.generatedSourceFolders).containsAtLeast(kaptGeneratedJava, kaptGeneratedKotlin)
  ...
  fun `fail when AGP is applied before kotlin-kapt plugin`() {
    ...
    result.assertErrorContains("The 'org.jetbrains.kotlin.kapt' plugin is not compatible with built-in Kotlin support.")
  }
