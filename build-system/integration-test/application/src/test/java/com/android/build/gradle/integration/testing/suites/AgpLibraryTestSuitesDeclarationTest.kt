...
class AgpLibraryTestSuitesDeclarationTest {
  @get:Rule
  val rule =
  ...
  @Test
  fun testLibraryTestSuites() {
    rule.build.executor.run(":lib:verifyCallbackInvoked")
  }
}
...
class MyLibraryCallback : LibraryComponentCallback {
  override fun handleExtension(project: Project, androidComponents: LibraryAndroidComponentsExtension) {
    ...
    androidComponents.onVariants(androidComponents.selector().withBuildType("debug")) { variant ->
      ...
      val firstTestSuite =
        libraryVariant.suites["first"]
          ?: throw RuntimeException("Cannot find first test suite in test suites : " + libraryVariant.suites.keys.joinToString(", "))
      ...
      project.tasks.register("verifyCallbackInvoked") { it.doLast { println("Callback was indeed invoked for $variantName") } }
    }

    androidComponents.onVariants(androidComponents.selector().withBuildType("release")) { variant ->
      val libraryVariant = variant as LibraryVariant
      Truth.assertThat(libraryVariant.suites).isEmpty()
    }
  }
}
