...
class ProguardAndroidOptimizeTest(val proguardAndroidTxtDisallowed: Boolean) {
    companion object {
        @JvmStatic
        @Parameterized.Parameters(name = "proguardAndroidTxtDisallowed={0}")
    ...
    val rule = GradleRule.from {
        gradleProperties {
            add(BooleanOption.R8_PROGUARD_ANDROID_TXT_DISALLOWED, proguardAndroidTxtDisallowed)
        }
        androidApplication {
    ...
    fun `test proguard-android-txt disallowed with flag`() {
        ...
        if (proguardAndroidTxtDisallowed) {
            ...
            assertThat(Throwables.getRootCause(result.exception).message).contains(
    ...
    fun `test proguard-android-optimize-txt always allowed`() {
        ...
        assertThat(build.getReleaseConfigurationTxtFile())
            .doesNotContain("-dontoptimize")
    }
