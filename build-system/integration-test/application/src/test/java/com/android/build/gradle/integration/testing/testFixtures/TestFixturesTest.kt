...
class TestFixturesTest {
  ...
  @get:Rule val project: GradleTestProject = GradleTestProject.builder().fromTestProject("testFixturesApp").create()

  private fun setUpProject(publishJavaLib: Boolean, publishAndroidLib: Boolean) {
    if (publishJavaLib) {
      TestFileUtils.searchAndReplace(project.getSubproject(":app").buildFile, "project(\":javaLib\")", "'com.example.javaLib:javaLib:1.0'")
    ...
    if (publishAndroidLib) {
      TestFileUtils.searchAndReplace(project.getSubproject(":app").buildFile, "project(\":lib\")", "'com.example.lib:lib:1.0'")

      TestFileUtils.searchAndReplace(project.getSubproject(":lib2").buildFile, "project(\":lib\")", "'com.example.lib:lib:1.0'")

      TestFileUtils.searchAndReplace(project.getSubproject(":appTests").buildFile, "project(\":lib\")", "'com.example.lib:lib:1.0'")
    }

    if (publishJavaLib) {
      TestFileUtils.appendToFile(
        project.getSubproject(":javaLib").buildFile,
        """
        publishing {
            repositories {
                maven {
    ...
    if (publishJavaLib) {
      executor().run(":javaLib:publish")
    }

    if (publishAndroidLib) {
      executor().run(":lib:publish")
    }
  }
  ...
  fun `test plugin excludes main lib classes but includes test fixtures`() {
    ...
    TestFileUtils.searchAndReplace(project.getSubproject(":app").buildFile, "compileOnly", "implementation")
  ...
  private fun testExclusionInTestApk(
    ...
    testApk.use {
      it.mainDexFile.get().classes.keys.let { classes ->
        ...
        listOf("Lcom/example/javalib/JavaLibInterface;", "Lcom/example/lib/LibInterface;").forEach { clazz ->
        ...
        // app classes
        listOf("Lcom/example/app/AppInterface;").forEach { clazz ->
  ...
  private fun setUpProjectForLint(ignoreTestFixturesSourcesInApp: Boolean) {
    ...
    TestFileUtils.searchAndReplace(
      project.getSubproject(":app").file("src/testFixtures/java/com/example/app/testFixtures/AppInterfaceTester.java"),
    ...
    TestFileUtils.searchAndReplace(
      project.getSubproject(":lib").file("src/testFixtures/java/com/example/lib/testFixtures/LibResourcesTester.java"),
      "public void test() {",
      """
