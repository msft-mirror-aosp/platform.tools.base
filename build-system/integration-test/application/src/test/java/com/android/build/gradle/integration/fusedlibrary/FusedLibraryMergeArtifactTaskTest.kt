...
internal class FusedLibraryMergeArtifactsTest {
  ...
  val rule =
    GradleRule.configure()
      .withMavenRepository {
        ...
        aar("com.remoteaar", "missing-min-compile-sdk")
          .withAarMetadataProperties(mapOf("aarFormatVersion" to "1.0", "aarMetadataVersion" to "1.0"))
      ...
      .from {
        ...
        androidLibrary(":libraryWithLint") {
          ...
          dependencies { lintPublish(project(":lintPublish1")) }
        }
        javaLibrary(":lintPublish1") { setUpLint(AddMyIssueRegistryManifestAttribute::class.java as Class<GenericCallback>) }
        androidLibrary(":libraryWithLint2") {
          ...
          dependencies { lintPublish(project(":lintPublish2")) }
        }
        ...
        androidLibrary(":libraryWithLint3") {
          ...
          dependencies { lintPublish(project(":lintPublish3")) }
        }
        javaLibrary(":lintPublish3") { setUpLint(AddMyIssueRegistryManifest2Attribute::class.java as Class<GenericCallback>) }
  ...
  class AddMyIssueRegistryManifestAttribute : GenericCallback {
    override fun handleProject(project: Project) {
      ...
      jar.configure {
        it.manifest { it.attributes(mutableMapOf<String, String>("Lint-Registry-v2" to "com.example.google.lint.MyIssueRegistry")) }
      }
    }
  }
  ...
  class AddMyIssueRegistryManifest2Attribute : GenericCallback {
    override fun handleProject(project: Project) {
      val jar = project.tasks.named("jar", Jar::class.java)
      jar.configure {
        it.manifest { it.attributes(mutableMapOf<String, String>("Lint-Registry-v2" to "com.example.google.lint.MyIssueRegistry2")) }
      }
    }
  }
  ...
  fun testAarMetadataMerging() {
    ...
    build.androidLibrary(":androidLib2").reconfigure {
      ...
      dependencies { coreLibraryDesugaring("com.android.tools:desugar_jdk_libs_nio:$DESUGAR_NIO_DEPENDENCY_VERSION") }
    }
    build.executor.run(":fusedLib1:assemble")
  ...
  fun testAssetsMergingWithDuplicateAssets() {
    ...
    fusedLib1.assertAar(AarSelector.NO_BUILD_TYPE) {
      assets {
        ...
        containsExactly("android_lib_one_asset.txt", "android_lib_two_asset.txt", "subdir/android_lib_one_asset_in_subdir.txt")
      }
    }
  }
  ...
  fun testJavaResourcesMerge() {
    ...
    fusedLib1.assertAar(AarSelector.NO_BUILD_TYPE) { javaResources().containsExactly("my_java_resource.txt") }

    val androidLib2 = build.androidLibrary(":androidLib2")
    androidLib2.files.add("src/main/resources/my_java_resource.txt", "content")

    build.executor.expectFailure().run(":fusedLib1:assemble").assertErrorContains("2 files found with path 'my_java_resource.txt'")
  }
  ...
  fun testLintJarIsPackaged() {
    ...
    build.fusedLibrary(":fusedLib1").reconfigure { dependencies { include(project(":libraryWithLint2")) } }
    build.executor.run(":fusedLib1:assemble")
    fusedLib.assertAar(AarSelector.NO_BUILD_TYPE) {
      // Fused AARs should contain the lint jars from their dependencies.
      contains("lint.jar")
    ...
    build.executor
      .expectFailure()
      .run(":fusedLib1:assemble")
      .assertErrorContains("com/example/google/lintpublish/MyIssueRegistry.class is present in multiple jars with different contents")
    ...
    build.executor.expectFailure().run(":fusedLib1:assemble").assertErrorContains("META-INF/MANIFEST.MF is present in multiple jars")
  }
