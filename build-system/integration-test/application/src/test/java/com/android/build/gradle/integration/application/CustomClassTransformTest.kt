...
class CustomClassTransformTest {
  ...
  @Rule @JvmField val project = GradleTestProject.builder().fromTestApp(HelloWorldApp.forPlugin("com.android.application")).create()

  private fun assertProjectClassWasTransformed() {
    val transformedHelloWorldClass =
      FileUtils.join(
  ...
  fun testCustomClassTransform() {
    ...
    project.executor().with(StringOption.IDE_ANDROID_CUSTOM_CLASS_TRANSFORMS, jarFile.absolutePath).run("assembleDebug")
    assertProjectClassWasTransformed()
    val apk = project.getApk(GradleTestProject.ApkType.DEBUG)
    assertThatApk(apk).containsClass("Lcom/android/build/gradle/integration/application/testData/TestDependency;")
  ...
  fun testBundle() {
    val jarFile = createProfilersJar()
    project.executor().with(StringOption.IDE_ANDROID_CUSTOM_CLASS_TRANSFORMS, jarFile.absolutePath).run("bundleDebug")
    project.getBundle(GradleTestProject.ApkType.DEBUG).use {
      assertThat(it).containsClass("base", "Lcom/android/build/gradle/integration/application/testData/TestDependency;")
  ...
  private fun createProfilersJar(): File {
    ...
    TestInputsGenerator.pathWithClasses(dependencyJarFile.toPath(), listOf(TestDependency::class.java))
    ...
    ZipOutputStream(FileOutputStream(jarFile)).use { zip ->
      ...
      zip.putNextEntry(e)
      url.openStream().use { ByteStreams.copy(it, zip) }
      zip.closeEntry()
      ...
      zip.putNextEntry(e)
      BufferedInputStream(FileInputStream(dependencyJarFile)).use { ByteStreams.copy(it, zip) }
      zip.closeEntry()

      e = ZipEntry("dependencies/fake_native_dependency.jar")
      zip.putNextEntry(e)
      BufferedInputStream(FileInputStream(nativeDependencyJarFile)).use { ByteStreams.copy(it, zip) }
      zip.closeEntry()
    }
    return jarFile
  }
