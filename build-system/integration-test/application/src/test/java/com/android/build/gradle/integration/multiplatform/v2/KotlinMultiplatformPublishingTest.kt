...
class KotlinMultiplatformPublishingTest {
  ...
  /**
   * Here we have a consumer with jvm + common targets consuming an artifact with android + common targets. And the androidApiElements
   * variant is successfully resolved which is wrong. jvmMain cannot consume androidMain
   */
  @Test
  fun `test kmp consumer without android target`() {
    ...
    ScannerSubject.assertThat(buildResult.stdout).contains("No matching variant of com.example.producer:producer:1.0 was found")
  }
  ...
  /**
   * This update the metadata file changes the org.jetbrains.kotlin.platform.type attribute in all publication configurations from
   * androidJvm to jvm in order to simulate consuming AGP-KMP library published with "jvm" value
   */
  fun simulateDifferentProducerArtifact(build: GradleBuild) {
    val gradleModuleFile = build.directory.resolve("repo/com/example/producer/producer/1.0/producer-1.0.module")
...
class AndroidDependencyCallback : KotlinMultiplatformCallback {
  override fun handleExtension(project: Project, extension: KotlinMultiplatformExtension) {
    extension.apply { sourceSets.androidMain.dependencies { implementation("com.example.producer:producer:1.0") } }
  }
}

class CommonDependencyCallback : KotlinMultiplatformCallback {
  override fun handleExtension(project: Project, extension: KotlinMultiplatformExtension) {
    extension.apply { sourceSets.commonMain.dependencies { implementation("com.example.producer:producer:1.0") } }
  }
}
...
class PublisherCallback : GenericCallback {
  override fun handleProject(project: Project) {
    ...
    publishing.apply { repositories { it.maven { it.url = project.uri(project.projectDir.parentFile.resolve("repo")) } } }
  }
}
