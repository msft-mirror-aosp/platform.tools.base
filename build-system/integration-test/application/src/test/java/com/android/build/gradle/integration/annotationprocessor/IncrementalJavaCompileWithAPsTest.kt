...
class IncrementalJavaCompileWithAPsTest(
  ...
  private fun setUpApp(): MinimalSubProject {
    ...
    app.withFile(
      ...
      with(BuildFileBuilder()) {
        ...
        minSdkVersion = "23"
        namespace = NAMESPACE
        addDependency(dependency = "'androidx.appcompat:appcompat:1.6.1'")
        addDependency(dependency = "'com.android.support.constraint:constraint-layout:" + "$SUPPORT_LIB_CONSTRAINT_LAYOUT_VERSION'")
  ...
  /**
   * Adds an aggregating annotation processor that generates a registry of all the classes annotated with the given annotation.
   *
   * See https://docs.gradle.org/current/userguide/java_plugin.html #sec:incremental_annotation_processing
   */
  private fun addAggregatingProcessorClass(builder: JavaSourceFileBuilder, @Suppress("SameParameterValue") processorName: String) {
    builder.addImports(
      "java.io.IOException",
  ...
  fun setUp() {
    ...
    val generatedSourceDir = if (withKapt) GENERATED_SOURCE_KAPT_DIR else GENERATED_SOURCE_DIR

    val mainActivityPackagePath = NAMESPACE.replace('.', '/')
    val annotatedPackagePath = ANNOTATED_PACKAGE.replace('.', '/')
    val generatedPackagePath = GENERATED_PACKAGE.replace('.', '/')

    annotation1Class1JavaFile = File("$appDir/$SOURCE_DIR/$annotatedPackagePath/$ANNOTATION_1_CLASS_1.java")

    annotation1GeneratedJavaFile = File("$appDir/$generatedSourceDir/$generatedPackagePath/$ANNOTATION_1_GENERATED_CLASS.java")
    annotation2GeneratedJavaFile = File("$appDir/$generatedSourceDir/$generatedPackagePath/$ANNOTATION_2_GENERATED_CLASS.java")

    mainActivityClass = File("$appDir/$COMPILED_CLASSES_DIR/$mainActivityPackagePath/$MAIN_ACTIVITY.class")
    annotation1Class1 = File("$appDir/$COMPILED_CLASSES_DIR/$annotatedPackagePath/$ANNOTATION_1_CLASS_1.class")
    annotation1Class2 = File("$appDir/$COMPILED_CLASSES_DIR/$annotatedPackagePath/$ANNOTATION_1_CLASS_2.class")
    annotation2Class1 = File("$appDir/$COMPILED_CLASSES_DIR/$annotatedPackagePath/$ANNOTATION_2_CLASS_1.class")
    annotation2Class2 = File("$appDir/$COMPILED_CLASSES_DIR/$annotatedPackagePath/$ANNOTATION_2_CLASS_2.class")
    annotation1GeneratedClass = File("$appDir/$COMPILED_CLASSES_DIR/$generatedPackagePath/$ANNOTATION_1_GENERATED_CLASS.class")
    annotation2GeneratedClass = File("$appDir/$COMPILED_CLASSES_DIR/$generatedPackagePath/$ANNOTATION_2_GENERATED_CLASS.class")
    ...
    if (withKapt) {
      ...
      if (withBuiltInKotlin) {
        TestFileUtils.appendToFile(appBuildFile, "kotlin.compilerOptions.jvmTarget = org.jetbrains.kotlin.gradle.dsl.JvmTarget.JVM_11\n")
      } else {
