...
class OptimizeResourcesTaskTest {
  ...
  @Rule @JvmField val rule = GradleRule.from { androidApplication { HelloWorldAndroid.setupKotlin(files) } }
  ...
  fun `test OptimizeResourcesTask works with resource shrinker`() {
    ...
    with(build.androidApplication()) {
      assertApk(ApkSelector.RELEASE) { androidResources().contains("01.xml") }

      buildDir.resolve("reports/${InternalArtifactType.RESOURCES_CONFIG_MAP_FILE.getFolderName()}/release/resources.cfg").apply {
