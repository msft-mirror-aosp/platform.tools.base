...
class SourceSetsTest {
  ...
  class MyAppCallback : ApplicationComponentCallback {
    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      androidComponents.onVariants(androidComponents.selector().withBuildType("debug")) { variant ->
        val reproTask = project.tasks.register("${variant.name}ReproTask", ReproducerTask::class.java) { println("ReproTask configured.") }
  ...
  fun sourceRegistrationShouldBeSuccessful() {
    ...
    Truth.assertThat(result.stdout.findAll("ReproducerTask called !").count()).isEqualTo(1)
  }
    ...
    override fun handleExtension(project: Project, extension: BaseAppModuleExtension) {
      extension.applicationVariants.all {
        extension.sourceSets
          .getByName(it.name)
          .assets
          .srcDir(project.layout.projectDirectory.dir(java.nio.file.Paths.get("extra-assets", it.name).toString()))
      }
    }
  }
  ...
  class RegisterTaskViaOldApi : LegacyApplicationCallback {
    override fun handleExtension(project: Project, extension: BaseAppModuleExtension) {
      extension.applicationVariants.all { variant ->
        val reproTask =
          project.tasks.register("${variant.name}ReproTask", ReproducerTask::class.java) { task ->
  ...
  /**
   * Test that create tasks very early and ensures that a directory registered using the old variant API is part of the sources for the
   * project.
   */
  @Test
  fun testOldVariantApiWithEarlyTaskRealization() {
    ...
    val content =
      build
        .androidApplication()
        .resolve(InternalArtifactType.ANDROID_RES_SOURCE_SET_PATH_MAP)
  ...
  fun testSettingCustomOutputPath() {
    ...
    Truth.assertThat(build.androidApplication(":app").buildDir.resolve("my/custom/path/some-res.bin").toFile().exists()).isTrue()
...
class SettingOutputPathCallback : ApplicationComponentCallback {
  ...
  override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
    androidComponents.onVariants(androidComponents.selector().withBuildType("debug")) { variant ->
      val reproTask =
        project.tasks.register("${variant.name}ReproTask", ReproducerTask::class.java) {
          it.outputDirectory.set(project.layout.buildDirectory.dir("my/custom/path"))
        }
