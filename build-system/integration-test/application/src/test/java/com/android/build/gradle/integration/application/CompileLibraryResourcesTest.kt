...
class CompileLibraryResourcesTest {
  ...
  private val library =
    MinimalSubProject.lib("com.example.library")
      ...
      .withFile("src/main/res/raw/me.raw", "test")
  ...
  private val testApp = MultiModuleTestProject.builder().subproject(":library", library).subproject(":app", app).build()

  @get:Rule val project = GradleTestProject.builder().fromTestApp(testApp).withName("project1").create()
  ...
  @get:Rule val projectCopy = GradleTestProject.builder().fromTestApp(testApp).withName("project2").create()
  ...
  fun testResourcesAreCompiledAndProcessed() {
    ...
    checkCompiledLibraryResourcesDir(
      setOf(
        Aapt2RenamingConventions.compilationRename(File(File("layout"), "layout_random_name.xml")),
        Aapt2RenamingConventions.compilationRename(File(File("drawable-anydpi-v26"), "button.xml")),
        Aapt2RenamingConventions.compilationRename(File(File("drawable-v26"), "ic_launcher_background.xml")),
        Aapt2RenamingConventions.compilationRename(File(File("raw"), "me.raw")),
      )
    )
  ...
  fun testIncrementalResourceChange() {
    ...
    // change me.raw
    FileUtils.writeToFile(FileUtils.join(project.getSubproject(":library").projectDir, "src", "main", "res", "raw", "me.raw"), "test2")
    ...
    FileUtils.deleteIfExists(
      FileUtils.join(project.getSubproject(":library").projectDir, "src", "main", "res", "drawable-v26", "ic_launcher_background.xml")
    )

    FileUtils.writeToFile(
      FileUtils.join(project.getSubproject(":app").projectDir, "src", "main", "res", "mipmap-anydpi-v26", "ic_launcher.xml"),
      """
      <?xml version="1.0" encoding="utf-8"?>
                              <adaptive-icon xmlns:android="http://schemas.android.com/apk/res/android">
                              </adaptive-icon>
  ...
  fun testIntegrationWithResourceShrinker() {
    ...
    val compressed =
      InternalArtifactType.SHRUNK_RESOURCES_PROTO_FORMAT.getOutputDir(project.getSubproject(":app").buildDir)
    ...
    ZFile.openReadOnly(compressed).use {
      ...
      assertThat(it.get("res/drawable-anydpi-v26/button.xml")!!.read()).isNotEqualTo(DummyContent.TINY_BINARY_XML)
      assertThat(it.get("res/drawable-v26/ic_launcher_background.xml")!!.read()).isNotEqualTo(DummyContent.TINY_BINARY_XML)
    }
  }
  ...
  fun testCompiledResourceContentsDoNotChangeOnProjectRelocation() {
    ...
    val originalCompiledLibResourcesDir =
      project.getSubproject("library").intermediatesDir.resolve("compiled_local_resources/debug/compileDebugLibraryResources/out/")

    val originalDirectoryCompiledResources = originalCompiledLibResourcesDir.listFiles().associate { it.name to it.readBytes().toList() }
    projectCopy.executor().run("assembleDebug")
    val newCompiledLibResourcesDir =
      projectCopy.getSubproject("library").intermediatesDir.resolve("compiled_local_resources/debug/compileDebugLibraryResources/out/")
    val newDirectoryCompiledResources = newCompiledLibResourcesDir.listFiles().associate { it.name to it.readBytes().toList() }
    assertThat(newDirectoryCompiledResources.keys).containsExactlyElementsIn(originalDirectoryCompiledResources.keys)
    assertThat(newDirectoryCompiledResources.values).containsExactlyElementsIn(originalDirectoryCompiledResources.values)
  }
  ...
  private fun checkCompiledLibraryResourcesDir(expected: Set<String>) {
    val compiledLibraryResourcesDir =
      FileUtils.join(
        project.getSubproject(":library").intermediatesDir,
        "compiled_local_resources",
        "debug",
    ...
    assertThat(compiledLibraryResourcesDir.listFiles()!!.map { it.name }.toSet()).containsExactlyElementsIn(expected)
  }

  private fun checkOnlyValuesWasMerged() {
    val mergedResDir =
      File(MERGED_RES.getOutputDir(project.getSubproject(":app").buildDir), "debug" + File.separatorChar + "mergeDebugResources")

    assertThat(mergedResDir.listFiles()!!.map { it.name }.toSet())
      .containsExactlyElementsIn(
        setOf(
          Aapt2RenamingConventions.compilationRename(File(File("values-v28"), "values-v28.xml")),
          Aapt2RenamingConventions.compilationRename(File(File("values"), "values.xml")),
          Aapt2RenamingConventions.compilationRename(File(File("mipmap-anydpi-v26"), "ic_launcher.xml")),
        )
      )
  }
}
