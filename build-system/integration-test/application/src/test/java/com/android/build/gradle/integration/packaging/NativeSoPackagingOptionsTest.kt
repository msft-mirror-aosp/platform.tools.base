...
class NativeSoPackagingOptionsTest {
  ...
  private val multiModuleTestProject =
    MultiModuleTestProject.builder().subproject(":lib", lib).subproject(":app", app).dependency(app, lib).build()
  ...
  fun testPackagingOptions() {
    ...
    appSubProject.assertApk(ApkSelector.DEBUG) {
      jniLibs {
        abi("x86") {
          ...
          containsExactly("appKeep.so", "libKeep.so", "dslPickFirst.so", "releaseExclude.so", "variantPickFirst.so")
      ...
      manifestAsNodes()
        .node("manifest")
        .node("application")
        .containsAttributeAndValue("http://schemas.android.com/apk/res/android:extractNativeLibs", "true")
    }
    ...
    appSubProject.assertApk(ApkSelector.RELEASE) {
      jniLibs {
        abi("x86") {
          // before checking content, ensure we only have that's expected, and nothing else
          containsExactly("appKeep.so", "libKeep.so", "dslPickFirst.so", "debugExclude.so", "variantPickFirst.so")
          bytesOf("appKeep.so").isEqualTo(LIB_CONTENT_FOO)
          bytesOf("dslPickFirst.so").isEqualTo(LIB_CONTENT_FOO)
      ...
      manifestAsNodes()
        .node("manifest")
        .node("application")
        .containsAttributeAndValue("http://schemas.android.com/apk/res/android:extractNativeLibs", "false")
    }
