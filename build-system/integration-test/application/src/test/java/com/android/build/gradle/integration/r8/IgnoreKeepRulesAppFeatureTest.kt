...
class IgnoreKeepRulesAppFeatureTest {
  ...
  @get:Rule
  val project: GradleTestProject = GradleTestProject.builder().withAdditionalMavenRepo(mavenRepo).fromTestProject("dynamicApp").create()
  ...
  fun testKeepRuleLibrariesVersionMismatch() {
    ...
    result.stdout.use { ScannerSubject.assertThat(it).contains("Keep rules from [$MISS_MATCHED_LIB_FOO_ID] are specified to be ignored") }
  }
  ...
  private fun ignoreProguardArtifact(project: GradleTestProject, variantDimensions: String, variantDimension: String, id: String) {
    TestFileUtils.appendToFile(
      project.buildFile,
      """
                android {
  ...
  private val mavenRepo: MavenRepoGenerator
    get() =
      MavenRepoGenerator(
        listOf(
          MavenRepoGenerator.Library(LIB_FOO_ID, TestInputsGenerator.jarWithTextEntries(PROGUARD_PATH to LIB_FOO_RULE)),
          MavenRepoGenerator.Library(LIB_BAR_ID, TestInputsGenerator.jarWithTextEntries(PROGUARD_PATH to LIB_BAR_RULE)),
        )
      )
