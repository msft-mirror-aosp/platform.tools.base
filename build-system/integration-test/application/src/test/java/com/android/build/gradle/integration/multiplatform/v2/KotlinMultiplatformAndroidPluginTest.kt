...
class KotlinMultiplatformAndroidPluginTest(private val publishLibs: Boolean) {
  ...
  companion object {
    ...
    @JvmStatic @Parameterized.Parameters(name = "publishLibs={0}") fun getOptions() = listOf(false, true)
  }
  ...
  fun testKmpLibraryTestApkContentsWithBuildTypeSelection() {
    ...
    executor().run(":kmpFirstLib:mergeAndroidDeviceTestJavaResource")

    val androidTestMergedRes =
      project
        .getSubproject("kmpFirstLib")
        .getIntermediateFile(
          InternalArtifactType.MERGED_JAVA_RES.getFolderName() +
            "/androidDeviceTest/mergeAndroidDeviceTestJavaResource/feature-kmpFirstLib.jar"
        )

    ZipSubject.assertThat(androidTestMergedRes) { textFile("android_lib_resource.txt").isEqualTo("android lib debug resource") }
  }
  ...
  fun testRunningUnitTests() {
    ...
    val reportDir = FileUtils.join(project.getSubproject("kmpFirstLib").buildDir, "reports", "tests", "testAndroidHostTest")
    val indexFilesText = reportDir.walkTopDown().filter { it.name == "index.html" }.joinToString("\n") { it.readText() }

    assertWithMessage("Running kmp unit tests should run common tests as well")
      .that(indexFilesText)
      .contains("com.example.kmpfirstlib.KmpAndroidFirstLibClassTest")
    assertWithMessage("Running kmp unit tests should run common tests as well")
      .that(indexFilesText)
      .contains("com.example.kmpfirstlib.KmpCommonFirstLibClassTest")
    ...
    assertThat(coveragePackageFolder.listFiles()!!.map { it.name })
      .containsExactly(
        "index.html",
        "index.source.html",
  ...
  fun testKmpLibraryAarContents() {
    ...
    if (publishLibs) {
      val file =
        FileUtils.join(project.projectDir, "testRepo", "com", "example", "kmpFirstLib-android", "1.0", "kmpFirstLib-android-1.0.aar")
    ...
    if (publishLibs) {
      val zipPath =
        FileUtils.join(
            ...
            "kmpFirstLib-android",
            "1.0",
            "kmpFirstLib-android-1.0-sources.jar",
          )
          .toPath()
