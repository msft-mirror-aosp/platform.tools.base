...
class LibraryClassVerificationTest {
    ...
    GradleRule.from {
      androidLibrary(":lib") {
        ...
        dependencies {
          ...
          implementation(MavenRepoGenerator.Library("com.example.base:base:0.1", jarWithClasses(listOf(BaseClass::class.java))))
      ...
      gradleProperties { add(BooleanOption.VERIFY_AAR_CLASSES, true) }
    }
  ...
  fun checkInvalidClasses() {
    ...
    androidLibrary.files.update("src/main/java/com/example/lib/Use.kt").searchAndReplace("EmbeddedJarClass", "CompileOnlyJarClass")
    ...
    assertThat(result.stderr)
      .contains(
        "Error: Missing class com.example.CompileOnlyJarClass (referenced from: void com.example.lib.Use.<init>() and 1 other context)"
      )

    // override to disable in that particular project
    androidLibrary.reconfigure { android { experimentalProperties["android.experimental.verifyLibraryClasses"] = false } }
    build.executor.run(":lib:assembleRelease")
  }
}
