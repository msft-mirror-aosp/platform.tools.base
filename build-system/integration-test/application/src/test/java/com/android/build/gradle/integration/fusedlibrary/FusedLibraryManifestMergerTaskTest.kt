...
internal class FusedLibraryManifestMergerTaskTest {
  ...
  fun checkFusedLibraryManifest() {
    ...
    val mergedManifestFile = fusedLib1Project.resolve(MERGED_MANIFEST).resolve("single/mergeManifest/AndroidManifest.xml").toFile()
    ...
    assertThat(mergedManifestContents).contains("<uses-permission android:name=\"android.permission.SEND_SMS\" />")
    checkManifestBlameLogIsCreated(fusedLib1Project)
    assertThat(mergedManifestContents).contains("android:name=\"com.externaldep.permission.REMOTE_PERMISSION\"")
  }

  @Test
  fun failWhenLibraryMinSdkVersionConflictWithFusedLibrary() {
    val build = rule.build { androidLibrary(":androidLib3") { android { defaultConfig.minSdk = 20 } } }

    build.executor
      .expectFailure()
      .run(":fusedLib1:mergeManifest")
      .assertErrorContains("uses-sdk:minSdkVersion 19 cannot be smaller than version 20 declared in library [:androidLib3]")
  }

  @Test
  fun failWhenFusedLibraryMinSdkVersionConflictWithApp() {
    val build = rule.build { fusedLibrary(":fusedLib1") { androidFusedLibrary { minSdk { version = release(20) } } } }
    ...
    build.androidApplication().reconfigure { dependencies { implementation(files(aarFile)) } }

    build.executor
      .expectFailure()
      .run(":app:processDebugMainManifest")
      .assertErrorContains("uses-sdk:minSdkVersion 19 cannot be smaller than version 20 declared in library [fusedLib1.aar]")
  }
  ...
  fun testAppManifestMergesFusedLibraryManifest() {
    ...
    val mergedManifest =
      build.androidApplication().intermediatesDir.resolve("merged_manifest/debug/processDebugMainManifest/AndroidManifest.xml").toFile()
    ...
    assertThat(parsedManifest.targetSdkVersion?.apiLevel).isEqualTo(GradleBuildDefinition.DEFAULT_COMPILE_SDK_VERSION)
  }
