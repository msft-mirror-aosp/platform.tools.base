...
private data class LocaleFolderToStandard(val folderName: String, val standardName: String)
...
class LocaleConfigGenerationTest {
  ...
  private fun addLocaleForVariantDimension(
    ...
    val file =
      if (locale == DEFAULT) {
        ...
        project.file("src/${variantDimension}/res/values-${locale.folderName}/strings.xml")
      }
  ...
  private fun GradleTestProject.withLocales(
    ...
    appLocales.forEach { addLocaleForVariantDimension(this.getSubproject("app"), it) }
    lib1Locales.forEach { addLocaleForVariantDimension(this.getSubproject("lib1"), it) }
    lib2Locales.forEach { addLocaleForVariantDimension(this.getSubproject("lib2"), it) }
    return this
  }
  ...
  private fun buildDsl(
    ...
    if (addResourcesProperties) {
      ...
      project.getSubproject("app").file("src/main/res/resources.properties").writeText(fileText)
    }
  }
  ...
  private fun localeListFromXml(xml: File): List<String> {
    ...
    xml.readLines().forEach { if (it.trim().startsWith("<locale android:name")) locales.add(it.split("\"")[1]) }
    return locales
  }
    ...
    return Truth.assertThat(
      localeListFromFile(
        project
          ...
          .file("$localeListPath/$variant/extract" + variant.replaceFirstChar { it.uppercase() } + "SupportedLocales/supported_locales.txt")
      )
    )
  }
  ...
  private fun assertLocaleConfig(variant: String): IterableSubject {
    return Truth.assertThat(
      localeListFromXml(project.getSubproject("app").file("$localeConfigPath/$variant/xml/$LOCALE_CONFIG_FILE_NAME.xml"))
    )
  }
  ...
  private fun validateLocalesInLocaleConfigAndApk(
    ...
    val apkType = if (isRelease) GradleTestProject.ApkType.RELEASE else GradleTestProject.ApkType.DEBUG
    TruthHelper.assertThat(project.getSubproject("app").getApk(apkType)).locales().containsExactlyElementsIn(apkLocaleList)
  }

  private fun getExpectedLocaleList(nonDefaultLocales: List<String>, defaultLocale: String): List<String> {
  ...
  fun `Test with default behavior locale config is not generated`() {
    ...
    val result =
      project
        .withLocales(appLocales = listOf(DEFAULT, ES, ZH_HANT_TW, EN_GB), lib1Locales = listOf(DEFAULT, ES, RU), lib2Locales = listOf(PT))
        .executor()
        .run("assembleDebug")

    Truth.assertThat(result.didWorkTasks).doesNotContain(":app:generateDebugLocaleConfig")

    PathSubject.assertThat(project.getSubproject("app").file(localeConfigPath)).doesNotExist()
    PathSubject.assertThat(project.getSubproject("app").file(localeListPath)).doesNotExist()
    PathSubject.assertThat(project.getSubproject("lib1").file(localeListPath)).doesNotExist()
    PathSubject.assertThat(project.getSubproject("lib2").file(localeListPath)).doesNotExist()
    PathSubject.assertThat(project.getSubproject("app").file("$mergedManifestPath/debug/processDebugMainManifest/AndroidManifest.xml"))
  ...
  fun `Test with feature off user locale config is not changed and locale config is not generated`() {
    ...
    val result =
      project
        .withLocales(appLocales = listOf(DEFAULT, ES, ZH_HANT_TW, EN_GB), lib1Locales = listOf(DEFAULT, ES, RU), lib2Locales = listOf())
        .withUserLocaleConfig()
        .executor()
        .run("assembleDebug")
    ...
    PathSubject.assertThat(project.getSubproject("lib1").file(localeListPath)).doesNotExist()
    PathSubject.assertThat(project.getSubproject("lib2").file(localeListPath)).doesNotExist()
    val mergedManifest = project.getSubproject("app").file("$mergedManifestPath/debug/processDebugMainManifest/AndroidManifest.xml")
    PathSubject.assertThat(mergedManifest).contains("""android:localeConfig="@xml/user_locale_config"""")
    PathSubject.assertThat(mergedManifest).doesNotContain(LOCALE_CONFIG_FILE_NAME)
  }
  ...
  fun `Test task runs and xml and locale lists are generated`() {
    ...
    val mergedManifest = project.getSubproject("app").file("$mergedManifestPath/debug/processDebugMainManifest/AndroidManifest.xml")
    PathSubject.assertThat(mergedManifest).contains("""android:localeConfig="@xml/$LOCALE_CONFIG_FILE_NAME""")
    PathSubject.assertThat(mergedManifest).doesNotContain("""android:localeConfig="@xml/user_locale_config""")

    // Test that xml is not generated in libs
    PathSubject.assertThat(project.getSubproject("lib1").file("build/generated/res/localeConfig")).doesNotExist()
    PathSubject.assertThat(project.getSubproject("lib2").file("build/generated/res/localeConfig")).doesNotExist()
  }
  ...
  fun `Test locales contain all supported locales`() {
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT, ES, ZH_HANT_TW, EN_GB), lib1Locales = listOf(RU), lib2Locales = listOf(PT))
      .execute("assembleDebug")

    assertLocaleList("app", "debug").isEqualTo(getExpectedLocaleList(listOf("b+zh+Hant+TW", "en-rGB", "es-rES"), "de-DE"))
    ...
    assertLocaleConfig("debug").isEqualTo(listOf("de-DE", "zh-Hant-TW", "en-GB", "es-ES", "ru-RU", "pt-BR"))
  }
  ...
  fun `Test locales are not duplicated`() {
    ...
    assertLocaleList("app", "debug").isEqualTo(getExpectedLocaleList(listOf("b+zh+Hant+TW", "en-rGB", "es-rES"), "en-US"))
    assertLocaleList("lib1", "debug").isEqualTo(listOf("es-rES", "ru-rRU"))
    assertLocaleList("lib2", "debug").isEqualTo(listOf("en-rGB", "pt-rBR"))
    assertLocaleConfig("debug").isEqualTo(listOf("en-US", "zh-Hant-TW", "en-GB", "es-ES", "ru-RU", "pt-BR"))
  }
  ...
  fun `Test user locale config produces conflict`() {
    ...
    ScannerSubject.assertThat(result.stderr)
      .contains("Locale config generation was requested but user locale config is present in manifest")
  }
  ...
  fun `Test invalid default locale`() {
    ...
    val result =
      project
        .withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(), lib2Locales = listOf())
        .executor()
        .expectFailure()
        .run("assembleDebug")
  ...
  fun `Test multiple resources properties files error`() {
    ...
    val newResourcesPropertiesFile = project.getSubproject("app").file("src/main/res1/resources.properties")

    FileUtils.createFile(
      newResourcesPropertiesFile,
      """
    ...
    val result =
      project
        .withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(DEFAULT), lib2Locales = listOf(DEFAULT))
        .executor()
        .expectFailure()
        .run("assembleDebug")

    ScannerSubject.assertThat(result.stderr)
      .contains("Multiple resources.properties files found with different unqualifiedResLocale values.")
  }
  ...
  fun `Test locales filtered by resource configurations`() {
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT, ES, ZH_HANT_TW, EN_GB, DE), lib1Locales = listOf(RU), lib2Locales = listOf(PT))
      .execute("assembleDebug")

    validateLocalesInLocaleConfigAndApk(listOf("en-US", "zh-Hant-TW", "de", "en-GB", "es-ES", "ru-RU", "pt-BR"))
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT, ES, ZH_HANT_TW, EN_GB, DE), lib1Locales = listOf(RU, DEFAULT), lib2Locales = listOf(PT))
      .execute("assembleDebug")
    ...
    TestFileUtils.searchAndReplace(
      project.getSubproject("app").buildFile,
      "android.defaultConfig.resourceConfigurations += [\"fr\"]",
      "android.defaultConfig.resourceConfigurations += " + "[\"mdpi\", \"de\", \"en\", \"b+zh+Hant\", \"es-rES\", \"b+chr\", \"b+es+419\"]",
    )
    ...
    validateLocalesInLocaleConfigAndApk(listOf("en-US", "de", "es-ES", "chr", "es-419", "zh-Hant"))
  }
  ...
  fun `Test resource configuration filtering with different default locale`() {
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT, ZH_HANT, ZH_HANT_TW), lib1Locales = listOf(EN_GB, DE_DE), lib2Locales = listOf(ES_419, DE))
      .execute("assembleDebug")
    ...
    validateLocalesInLocaleConfigAndApk(listOf("ru-RU", "zh-Hant", "zh-Hant-TW", "de-DE", "de"), defaultLocale = RU.standardName)
  }
  ...
  fun `Test that locale config xml is in apk res`() {
    ...
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(DEFAULT), lib2Locales = listOf(DEFAULT)).execute("assembleDebug")
  ...
  fun `Test that locale config xml is in bundle res`() {
    ...
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(DEFAULT), lib2Locales = listOf(DEFAULT)).execute("bundleDebug")

    ZipSubject.assertThat(project.getSubproject("app").file("build/outputs/bundle/debug/app-debug.aab").toPath()) {
  ...
  fun `Test build flavor res are included`() {
    ...
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(ES), lib2Locales = listOf(PT)).execute("assembleDebug")

    // london flavor res should not be included
    assertLocaleList("app", "cherokeeDebug").isEqualTo(getExpectedLocaleList(listOf("b+chr+US"), "en-US"))
  ...
  fun `Test emptyResFoldersAreIgnored`() {
    ...
    assertLocaleList("app", "debug").isEqualTo(getExpectedLocaleList(emptyList(), "en-US"))
  ...
  fun `Test AAR dependency locales not present in app locale config`() {
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(ES), lib2Locales = listOf(PT))
      .executor()
      .run(":lib1:assembleDebug")
  ...
  fun `Test pseudolocales are present`() {
    ...
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(), lib2Locales = listOf()).execute("assembleDebug")

    assertLocaleList("app", "debug").isEqualTo(getExpectedLocaleList(listOf(SdkConstants.EN_XA, SdkConstants.AR_XB), "en-US"))
  ...
  fun `Test pseudolocales are present with locale filters specified`() {
    ...
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(DE), lib2Locales = listOf()).execute("assembleDebug")
  ...
  fun `Test values folders that are not locale specific`() {
    buildDsl(generateLocaleConfig = true)
    val themesFile = project.getSubproject("app").file("src/main/res/values-night/themes.xml")
  ...
  fun `Test default locale for API level`() {
    buildDsl(generateLocaleConfig = true)
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(), lib2Locales = listOf()).execute("assembleDebug")
    val localeConfig = project.getSubproject("app").file("$localeConfigPath/debug/xml/$LOCALE_CONFIG_FILE_NAME.xml")
    ...
    val localeConfigV35 = project.getSubproject("app").file("$localeConfigPath/debug/xml-v35/$LOCALE_CONFIG_FILE_NAME.xml")

    listOf("app", "lib1", "lib2").forEach {
      TestFileUtils.searchAndReplace(project.getSubproject(it).buildFile, "compileSdkVersion 34", "compileSdkVersion 35")
    }
  ...
  fun `Test API 35 specific locale config is only included when necessary`() {
    buildDsl(generateLocaleConfig = true)
    project.withLocales(appLocales = listOf(DEFAULT), lib1Locales = listOf(), lib2Locales = listOf())
    ...
    TestFileUtils.searchAndReplace(project.getSubproject("app").buildFile, "minSdkVersion ${DEFAULT_MIN_SDK_VERSION}", "minSdkVersion 36")
  ...
  fun `Test localeFilters`() {
    ...
    TestFileUtils.searchAndReplace(project.getSubproject("app").buildFile, "de-rDE", "long")
  ...
  fun `Test localeFilters variant API`() {
    ...
    project
      .withLocales(appLocales = listOf(DEFAULT, ZH_HANT), lib1Locales = listOf(EN_GB), lib2Locales = listOf(ES_419))
