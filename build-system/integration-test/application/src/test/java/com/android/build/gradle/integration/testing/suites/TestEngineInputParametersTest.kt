    ...
    GradleRule.configure()
      .withMavenRepository {
        ...
        jar("com.test:toy-junit-engine:1.0")
          ...
          .addTextFile("META-INF/services/org.junit.platform.engine.TestEngine", ToyJunitEngineForTestingInputProperties::class.java.name)
      }
      .from {
  ...
  private fun checkOutputFolder(testEngineInputProperty: String): Boolean {
    ...
    val success =
      if (outFolderPresent) {
        ...
        outFolder.listFiles().forEach { logger.info("contains ${it.absolutePath}") }
        false
      } else {
        outFolder.mkdirs()
        true
