...
class StableResourceIDsTest {
  ...
  fun resourceIDsAreKept() {
    project.executor().with(BooleanOption.ENABLE_STABLE_IDS, true).run(":app:assembleDebug", ":app:bundleDebugResources")

    val appProject = project.getSubproject(":app")

    val stableIdsTxt = appProject.getIntermediateFile("stable_resource_ids_file", "debug", "processDebugResources", "stableIds.txt")
    ...
    val rDotTxt = appProject.getIntermediateFile("runtime_symbol_list", "debug", "processDebugResources", "R.txt")
    ...
    val valuesXml = FileUtils.join(appProject.mainSrcDir.parentFile, "res", "values", "colours.xml")
    TestFileUtils.searchAndReplace(valuesXml, "my_color_b", "my_color_bb")
    val styleablesXml = FileUtils.join(appProject.mainSrcDir.parentFile, "res", "values", "styleables.xml")
  ...
  fun disabledStableIds() {
    project.executor().with(BooleanOption.ENABLE_STABLE_IDS, false).run(":app:assembleDebug", ":app:bundleDebugResources")
  ...
  private fun dumpProtoApkRes(): List<String> {
    val bundleRes =
      InternalArtifactType.LINKED_RESOURCES_FOR_BUNDLE_PROTO_FORMAT.getOutputDir(project.getSubproject(":app").buildDir)
