...
class InjectedAbiTest {
  ...
  fun testInjectedAbiChange_WithSplits() {
    val build = rule.build { enableSplits(listOf("x86", "armeabi-v7a", "x86_64", "arm64-v8a")) }
    ...
    build.executor.with(StringOption.IDE_BUILD_TARGET_ABI, "x86").with(BooleanOption.ENABLE_LEGACY_API, true).run("assembleDebug").apply {
    ...
    project.assertApk(x86Selection.fromIntermediates()) { jniLibs().containsExactly("${Abi.X86.tag}/libapp.so") }
    ...
    build.executor
      ...
      .run("assembleDebug")
      .apply { assertTask(":app:packageDebug").didWork() }
    ...
    val armeabiV7aLastModifiedTime =
      java.nio.file.Files.getLastModifiedTime(project.getApkLocationForCopy(armV7aSelection.fromIntermediates()))

    // Run the third build without any target ABI, check that the APKs for all ABIs are
    // generated (or regenerated)
    build.executor.run("assembleDebug").apply { assertTask(":app:packageDebug").didWork() }
    ...
    PathSubject.assertThat(project.getApkLocationForCopy(armV7aSelection)).isNewerThan(armeabiV7aLastModifiedTime)

    val x86LastModifiedTime = java.nio.file.Files.getLastModifiedTime(project.getApkLocationForCopy(x86Selection))
    ...
    PathSubject.assertThat(project.getApkLocationForCopy(x86Selection.fromIntermediates())).isNewerThan(x86LastModifiedTime)
  }
  ...
  fun testInjectedAbiChange_WithoutSplits() {
    ...
    project.assertApk(DEBUG.fromIntermediates()) { jniLibs().containsExactly("${Abi.X86.tag}/libapp.so") }

    val apkLastModifiedTime = java.nio.file.Files.getLastModifiedTime(project.getApkLocationForCopy(DEBUG.fromIntermediates()))
    ...
    PathSubject.assertThat(project.getApkLocationForCopy(DEBUG.fromIntermediates())).isNewerThan(apkLastModifiedTime)
  }
    ...
    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      androidComponents.apply {
        onVariants(selector().withBuildType("release")) {
          it.experimentalProperties.put(BooleanWithDefault.BUILD_ALL_ABIS_IGNORING_IDE_OPTIMIZATIONS.key, true)
        }
      }
    }
  }
  ...
  fun testBuildAllAbisFlag() {
    val build = rule.build { androidApplication { pluginCallbacks += BuildAllAbisCallback::class.java } }
    val project = build.androidApplication()

    build.executor.with(StringOption.IDE_BUILD_TARGET_ABI, "x86").with(BooleanOption.ENABLE_LEGACY_API, true).run("assemble")
  ...
  fun testDefaultFilter() {
    ...
    build.executor.run("clean", "assembleDebug").apply { assertTask(":app:packageDebug").didWork() }

    project.assertApk(DEBUG) {
      jniLibs().containsExactly("x86_64/libapp.so", "x86/libapp.so", "arm64-v8a/libapp.so", "armeabi-v7a/libapp.so")
    }
  }
  ...
  fun testMissingSoFiles_WithoutSplits() {
    ...
    val executor = build.executor.with(StringOption.IDE_BUILD_TARGET_ABI, "x86_64,x86").with(BooleanOption.ENABLE_LEGACY_API, true)
    ...
    project.assertCorrectApk(DEBUG.fromIntermediates()) { jniLibs().containsExactly("${Abi.X86_64.tag}/libapp.so") }
    result1.stdout.use { scanner -> ScannerSubject.assertThat(scanner).doesNotContain("There are no .so files available") }
    ...
    project.assertCorrectApk(DEBUG.fromIntermediates()) { jniLibs().isEmpty() }
    result2.stdout.use { scanner ->
      ScannerSubject.assertThat(scanner).contains("There are no .so files available to package in the APK for x86_64.")
    }

    // Add explicit abiFilters and remove x86 .so files
    project.reconfigure { android { defaultConfig { ndk { abiFilters += listOf("x86_64", "x86", "arm64-v8a") } } } }
    ...
    project.assertCorrectApk(DEBUG.fromIntermediates()) { jniLibs().containsExactly("arm64-v8a/") }
    result3.stdout.use { scanner ->
      ScannerSubject.assertThat(scanner).contains("There are no .so files available to package in the APK for x86, x86_64.")
    }
  }
  ...
  fun testMissingSoFiles_WithSplits() {
    ...
    project.assertCorrectApk(x86_64Selection.fromIntermediates()) { jniLibs().containsExactly("${Abi.X86_64.tag}/libapp.so") }
    project.assertDoesNotExist(DEBUG.fromIntermediates())
    project.assertDoesNotExist(x86Selection.fromIntermediates())
    result1.stdout.use { scanner -> ScannerSubject.assertThat(scanner).doesNotContain("There are no .so files available") }
    ...
    // files.
    val apk2 = project.assertCorrectApk(x86_64Selection.fromIntermediates()) { jniLibs().isEmpty() }
    project.assertDoesNotExist(DEBUG.fromIntermediates())
    project.assertDoesNotExist(x86Selection.fromIntermediates())
    result2.stdout.use { scanner ->
      ScannerSubject.assertThat(scanner).contains("There are no .so files available to package in the APK for x86_64.")
  ...
  private fun AndroidApplicationProject.assertCorrectApk(apkSelector: ApkSelector, action: (ApkSubject.() -> Unit)? = null) {
    ZipSubject.assertThat(getApkLocationForCopy(apkSelector)) {
      entries().containsAtLeast("META-INF/MANIFEST.MF", "res/layout/main.xml", "AndroidManifest.xml", "classes.dex", "resources.arsc")
    }
    assertApk(apkSelector) { action?.invoke(this) }
  }

  private fun AndroidApplicationProject.assertDoesNotExist(apkSelector: ApkSelector) {
    assertApk(apkSelector) { doesNotExist() }
  }

  private fun GradleProjectFiles.createOriginalSoFile(abi: String, filename: String, content: String) {
