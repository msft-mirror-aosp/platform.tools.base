...
class KotlinMultiplatformResourcesTest {
  ...
  fun setUp() {
    ...
    FileUtils.writeToFile(
      project.getSubproject("kmpFirstLib").file("src/androidMain/kotlin/com/example/kmpfirstlib/UseR.kt"),
      """
  ...
  fun testKmpLibraryAarContents() {
    ...
    project.getSubproject("kmpFirstLib").assertAar(AarSelector.NO_BUILD_TYPE) {
      ...
      androidResources().containsExactly(listOf("drawable-nodpi-v4/image.png", "values/values.xml"))
      androidResources()
  ...
  fun testRclassPackagedInCompileClassesJar() {
    ...
    val classesJar =
      project
        ...
        .getIntermediateFile(
          InternalArtifactType.COMPILE_LIBRARY_CLASSES_JAR.getFolderName() + "/androidMain/bundleAndroidMainClassesToCompileJar/classes.jar"
        )
  ...
  fun testAppConsumeKmpResource() {
    FileUtils.writeToFile(
      project.getSubproject("app").file("src/main/java/com/example/app/UseKmpResource.java"),
      """
    ...
    project.getSubproject("app").assertApk(ApkSelector.DEBUG) { androidResources().containsExactly("drawable-nodpi-v4/image.png") }

    Truth.assertThat(result.didWorkTasks).containsAtLeastElementsIn(listOf(":kmpFirstLib:compileAndroidMainLibraryResources"))
  }
