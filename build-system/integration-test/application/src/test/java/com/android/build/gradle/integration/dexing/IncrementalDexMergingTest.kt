...
class IncrementalDexMergingTest {
  ...
  fun setUp() {
    ...
    // Add Java files
    addJavaFiles(app, COM_EXAMPLE_MYAPPLICATION) { classes, changedClass, changedJavaFile ->
      classesInApp = classes
      changedClassInApp = changedClass
      changedJavaFileInApp = changedJavaFile
    }
    addJavaFiles(lib, COM_EXAMPLE_LIB) { classes, changedClass, changedJavaFile ->
    ...
    incrementalTestHelper =
      IncrementalTestHelper(
          ...
          filesOrDirsToTrackChanges = setOf(mergedDexDirForApp, mergedDexDirForLib, mergedDexDirForExtLibs),
        )
        .updateExecutor { it.with(IntegerOption.DEXING_NUMBER_OF_BUCKETS, NUMBER_OF_BUCKETS) }
  }
  ...
  private fun addJavaFiles(
    ...
    val getClassFullName: (packageName: String, className: String) -> String = { packageName, className ->
      "${packageName.replace('.', '/')}/$className"
    }
    val getJavaFile: (classFullName: String) -> File = { File("${subproject.mainSrcDir}/$it.java") }
  ...
  fun `change Java file`() {
    val testHelper =
      incrementalTestHelper
        .runFullBuild()
        .applyChange { listOf(changedJavaFileInApp, changedJavaFileInLib).forEach { searchAndReplace(it, "Change me!", "I'm changed!") } }
        .runIncrementalBuild()
    ...
    val checkSubproject = { changedClass: String, mergedDexDir: File ->
      ...
      val bucketWithChangedClass = DexMergingTaskDelegate.getBucketNumber("$changedClass.dex", NUMBER_OF_BUCKETS)
      val dexFiles = FileUtils.getAllFiles(mergedDexDir)
      val dexFileWithChangedClass = mergedDexDir.resolve(bucketWithChangedClass.toString()).resolve("classes.dex")
      val dexFilesWithoutChangedClass = dexFiles.filter { it != dexFileWithChangedClass }
      testHelper.assertFileChanges(mapOf(dexFileWithChangedClass to CHANGED) + dexFilesWithoutChangedClass.map { it to UNCHANGED })
      ...
      val rClassesPath = COMPILE_AND_RUNTIME_R_CLASS_JAR.getOutputDir(app.buildDir).resolve("debug/processDebugResources/R.jar")
      val rClasses =
        Zip(rClassesPath).entries.map {
      ...
      for (dexFile in dexFiles) {
        Dex(dexFile).classes.keys.forEach { fullClassName ->
          ...
          assertWithMessage("Package $packageName is found in both $previousDexFile and $dexFile")
            .that(previousDexFile == null || previousDexFile == dexFile)
            .isTrue()
        }
      }
    }
    ...
    testHelper.assertFileChanges(mapOf(FileUtils.getAllFiles(mergedDexDirForExtLibs).single() to UNCHANGED))

    // Check contents of the APK
    app.getApk(DEBUG).use { apk -> (classesInApp + classesInLib).forEach { assertThat(apk).containsClass("L$it;") } }
  }
}
