...
class TestSuiteWithAddedSourcesViaVariantAPITest {
  ...
  fun testModel() {
    ...
    val basicTestSuites =
      models.basicAndroidProject?.testSuites ?: throw AssertionFailedError("no test suites defined in the BasicAndroidProject")
    ...
    Truth.assertThat(basicFirstSources.type).isEqualTo(SourceType.HOST_JAR)
    ...
    val testSuites = models.androidProject?.testSuites ?: throw AssertionFailedError("no test suites defined in the AndroidProject")
    ...
    Truth.assertThat(firstSources.type).isEqualTo(SourceType.HOST_JAR)
    val firstTestSuiteResources = firstSources.resources

    Truth.assertThat(firstTestSuiteResources)
      .containsExactly(project.subProject(":app").resolve("build/generated/first/firstTestSuiteGeneratorTask").toFile())
    ...
    Truth.assertThat(basicSecondSources.type).isEqualTo(SourceType.TEST_APK)
    ...
    Truth.assertThat(secondSources.type).isEqualTo(SourceType.TEST_APK)
    val secondTestSuiteResources = secondSources.sourceProvider.resourcesDirectories

    Truth.assertThat(secondTestSuiteResources)
      .containsExactly(project.subProject(":app").resolve("build/generated/second/secondTestSuiteGeneratorTask").toFile())
  }
...
abstract class AddGeneratedSourceSetTaskProducer : DefaultTask() {
  ...
  @get:InputFile abstract val configuration: RegularFileProperty

  @TaskAction
  fun taskAction() {
    outputDir.get().asFile.mkdirs()
    val numberOfFilesToGenerate = configuration.get().asFile.readText().toInt()
    repeat(numberOfFilesToGenerate) { iteration -> outputDir.get().file("random_text_$iteration.txt").asFile.writeText("random text") }
  }
}
...
open class AddGeneratedSourceSetToTestSuiteCallback : ApplicationComponentCallback {
  ...
  override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
    val firstTaskProducer =
      project.tasks.register<AddGeneratedSourceSetTaskProducer>(
        "firstTestSuiteGeneratorTask",
        AddGeneratedSourceSetTaskProducer::class.java,
      ) { task ->
        task.configuration.set(project.layout.projectDirectory.file("src/configuration"))
      }

    val secondTaskProducer =
      project.tasks.register<AddGeneratedSourceSetTaskProducer>(
      ...
      variant.suites.forEach { (suiteName, suite) ->
        suite.sources.forEach { sourceSet ->
          if (sourceSet is TestSuiteSourceSet.HostJar) {
            sourceSet.resources.addGeneratedSourceDirectory(firstTaskProducer, AddGeneratedSourceSetTaskProducer::outputDir)
          }
          if (sourceSet is TestSuiteSourceSet.TestApk) {
            sourceSet.resources.addGeneratedSourceDirectory(secondTaskProducer, AddGeneratedSourceSetTaskProducer::outputDir)
          }
        }
      }
    }
