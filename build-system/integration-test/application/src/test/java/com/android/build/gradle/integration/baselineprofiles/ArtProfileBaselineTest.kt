...
class ArtProfileBaselineTest {
  ...
  fun testStartupProfileTaskNotRunForDebug() {
    ...
    FileUtils.createFile(project.file("app/src/main/baselineProfiles/startup-prof.txt"), mainStartupProfileFileContent)
    FileUtils.createFile(project.file("app/src/debug/baselineProfiles/startup-prof.txt"), debugStartupProfileFileContent)
  ...
  private fun testMultipleLibrariesAndOptionalApplicationArtProfileMerging(addOldBaselineProfile: Boolean) {
    ...
    FileUtils.createFile(project.file("app/src/main/baselineProfiles/file.txt"), mainBaselineProfileFileContent)
    FileUtils.createFile(project.file("app/src/release/baselineProfiles/file.txt"), releaseBaselineProfileFileContent)
    ...
    if (addOldBaselineProfile) {
      ...
      File(appMainFolder, SdkConstants.FN_ART_PROFILE).writeText(oldBaselineProfileFileContent)
    }
    ...
    Truth.assertThat(HumanReadableProfile(mergedFile) { Assert.fail(it) }).isNotNull()
