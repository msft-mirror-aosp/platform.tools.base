...
class DeterministicTaskOutputsTest {
  ...
  companion object {
    ...
    /**
     * List of task outputs are currently either not deterministic or not relocatable (e.g., containing absolute paths).
     *
     * The goal is to reduce the size of list (tracked at bug 134654165).
     */
    private val INCONSISTENT_TASK_OUTPUTS =
      setOf(
  ...
  @get:Rule var project1 = setUpTestProject("project1")

  @get:Rule var project2 = setUpTestProject("project2")
  ...
  fun `check consistent outputs after building two identical projects`() {
    ...
    val snapshot1 = FileSnapshot.snapshot(fileToSnapshot = project1.getSubproject("app").buildDir, baseDir = project1.projectDir)

    // Build the second project
    project2.executor().run("clean", "assembleDebug", "testDebugUnitTest")
    val snapshot2 = FileSnapshot.snapshot(fileToSnapshot = project2.getSubproject("app").buildDir, baseDir = project2.projectDir)

    // Check that they have consistent outputs
    val filterInconsistentSubDirs = { dirs: Set<File> -> dirs.filter { dir -> INCONSISTENT_TASK_OUTPUTS.none { dir.startsWith(it) } } }
    expect
      .that(filterInconsistentSubDirs(snapshot1.directorySet))
      .containsExactlyElementsIn(filterInconsistentSubDirs(snapshot2.directorySet))
    for ((file, contents) in snapshot1.regularFileContentsMap.plus(snapshot2.regularFileContentsMap)) {
