...
class JavaCompileWithToolChainTest {
  ...
  @get:Rule
  val project =
    GradleTestProject.builder().fromTestApp(HelloWorldApp.forPlugin("com.android.application")).withKotlinGradlePlugin(true).create()
  ...
  fun basicTest() {
    ...
    result.stdout.use { ScannerSubject.assertThat(it).contains("Compiling with toolchain '${jdk8LocationFromStdout}'") }
    ...
    result.stdout.use { ScannerSubject.assertThat(it).contains("Compiling with toolchain '${latestJdkLocationFromStdout}'") }
  }
  ...
  fun `test source and target compatibility versions when toolchain is configured with built-in kotlin`() {
    ...
    val androidProject = project.modelV2().fetchModels(variantName = "debug").container.getProject().androidProject!!
  ...
  fun `test source and target compatibility versions when toolchain is configured with jetbrains kotlin`() {
    setUpKotlin(false)
    // Compiling should not throw an error (regression test for bug 260059413)
    project.executor().disableBuiltInKotlin().with(BooleanOption.USE_NEW_DSL, false).run("compileDebugJavaWithJavac")
  ...
  fun testDeprecatedSourceAndTarget() {
    ...
    TestFileUtils.searchAndReplace(project.buildFile, "JavaVersion.VERSION_1_7", "JavaVersion.VERSION_1_8")
    ...
    val resultWithNoWarning =
      project.executor().with(BooleanOption.JAVA_COMPILE_SUPPRESS_SOURCE_TARGET_DEPRECATION_WARNING, true).run("compileDebugJavaWithJavac")
