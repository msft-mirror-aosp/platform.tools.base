...
class R8TaskTest {
  ...
  fun testCheckDuplicateClassesTaskDidWork() {
    ...
    build.executor.run(":app:minifyReleaseWithR8").apply { assertTask(":app:checkReleaseDuplicateClasses").didWork() }
  }
  ...
  fun testTestedClassesPassedAsClasspathToR8() {
    ...
    val buildResult = build.executor.withLoggingLevel(LoggingLevel.DEBUG).run(":app:assembleReleaseAndroidTest")
    val appClasses =
      app.resolve(InternalArtifactType.COMPILE_APP_CLASSES_JAR).resolve("release/bundleReleaseClassesToCompileJar/classes.jar")
  ...
  fun testOutputMainDexList() {
    val build = rule.build { androidApplication { enableMultiDex() } }
    val app = build.androidApplication()

    build.executor.run(":app:assembleRelease")
    val mainDexListFile =
      app.resolve(InternalArtifactType.LEGACY_MULTIDEX_MAIN_DEX_LIST).resolve("release/minifyReleaseWithR8/mainDexList.txt")
  ...
  fun testInjectedDeviceApi() {
    ...
    app.assertApk(ApkSelector.RELEASE.fromIntermediates()) {
      classes().containsExactly("example/MyInterface", "example/MyInterfaceUser", "pkg/name/app/HelloWorld", "pkg/name/app/R\$layout")
    }
  ...
  fun testDefaultProguardRules() {
    ...
    build.executor.run(":app:assembleRelease").apply { assertTask(":app:minifyReleaseWithR8").didWork() }
  }
  ...
  fun `test system properties are passed to forked process`() {
    ...
    val result =
      build.executor
        .withArgument("-Dcom.android.tools.r8.synthesis.restrictrenaming=invalid_value")
        .expectFailure()
        .run(":app:minifyReleaseWithR8")
    TruthHelper.assertThat(result.failureMessage)
      .contains("Expected value of com.android.tools.r8.synthesis.restrictrenaming to be a boolean, but was: invalid_value")
  }
}
