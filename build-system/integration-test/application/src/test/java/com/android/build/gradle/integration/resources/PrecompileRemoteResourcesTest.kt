...
class PrecompileRemoteResourcesTest {
  ...
  @get:Rule val project = GradleTestProject.builder().fromTestApp(testApp).withAdditionalMavenRepo(mavenRepo).create()
  ...
  fun testLinkingErrorFromPrecompiledResource() {
    ...
    project.executor().with(BooleanOption.PRECOMPILE_DEPENDENCIES_RESOURCES, true).run(":publishedLib:assembleRelease")
    ...
    val result = project.executor().expectFailure().run(":localLib:assembleRelease")
    result.assertErrorContains(listOf("transformed", "lib-1.0", "res", "layout", "layout.xml").joinToString(separator = File.separator))
  }
  ...
  private fun checkAarResourcesCompilerTransformOutput() {
    ...
    assertThat(outputDir.listFiles()!!.map { file -> file.name }.toSortedSet())
  ...
  private fun checkValuesResourcedAreMerged() {
    ...
    assertThat(mergedResDir.listFiles()!!.map { file -> file.name }.toSortedSet())
      .containsExactlyElementsIn(
        listOf(
