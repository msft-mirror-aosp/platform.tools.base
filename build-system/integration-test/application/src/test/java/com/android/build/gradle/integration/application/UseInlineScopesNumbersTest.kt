...
class UseInlineScopesNumbersTest {
  ...
  fun testFlagAddedForDebuggableApks() {
    ...
    ScannerSubject.assertThat(result.stdout).contains(getLog(":app:compileDebugAndroidTestKotlin"))
    ScannerSubject.assertThat(result.stdout).contains(getLog(":lib:compileDebugAndroidTestKotlin"))
  ...
  fun testOptOut() {
    ...
    val result = build.executor.with(BooleanOption.DISABLE_INLINE_SCOPES_NUMBERS, true).withArgument("--info").run("tasks")

    ScannerSubject.assertThat(result.stdout).doesNotContain("-Xuse-inline-scopes-numbers")
  }
}

private fun getLog(taskPath: String) = "Adding -Xuse-inline-scopes-numbers Kotlin compiler flag for task $taskPath"
