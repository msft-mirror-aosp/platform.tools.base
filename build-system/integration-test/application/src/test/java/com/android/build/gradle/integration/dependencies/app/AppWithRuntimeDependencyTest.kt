...
/**
 * Test for runtime only dependencies. Test project structure: app -> library (implementation) ---- library -> library2 (implementation)
 * ---- library -> [guava (implementation) and example aar]
 *
 * The test verifies that the dependency model of app module contains library2, guava and the example aar as runtime only dependencies.
 */
class AppWithRuntimeDependencyTest {
  ...
  fun checkRuntimeClasspath() {
    ...
    val runtimeOnlyLibraries = (runtimeDeps.toSet() - compileDeps.toSet()).mapNotNull { it.toDependency() }

    assertThat(runtimeOnlyLibraries).containsExactly("com.google.guava:guava:19.0", "com.example:aar:1").inOrder()
  }
}
...
private fun List<GraphItem>.traverse(libraries: Map<String, Library>): List<Library> {
  ...
  //
  return seen.reversed().map { libraries[it]!! }
}

private fun Library.toDependency() = libraryInfo?.let { listOf(it.group, it.name, it.version).joinToString(":") }
