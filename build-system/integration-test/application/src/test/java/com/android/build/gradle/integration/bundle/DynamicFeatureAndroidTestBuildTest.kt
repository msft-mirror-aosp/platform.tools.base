...
/** Test to make sure we subtract dependency artifacts for android test for dynamic features correctly. */
class DynamicFeatureAndroidTestBuildTest {
   ...
   *
   * DF has a feature-on-feature dep on Middle DF, both depend on Base, Android Test DF is an android test variant for DF.
   *
   * Base depends on appLib and sharedLib. Middle DF depends on midLib and sharedMidLib. DF depends on featureLib. DF also has an android
   * test dependency on testFeatureLib, shared and sharedMidLib.
   */
  private val sharedLib = MinimalSubProject.lib("com.example.sharedLib").withFile("src/main/res/raw/shared_lib_file.txt", "shared lib file")

  private val appLib = MinimalSubProject.lib("com.example.appLib").withFile("src/main/res/raw/app_lib_file.txt", "app lib file")

  private val featureLib =
    MinimalSubProject.lib("com.example.featureLib").withFile("src/main/res/raw/main_feature_lib_file.txt", "feature lib file")

  private val middleFeatureLib =
    MinimalSubProject.lib("com.example.middleFeatureLib").withFile("src/main/res/raw/middle_main_feature_lib_file.txt", "feature lib file")
  ...
  private val testFeatureLib =
    MinimalSubProject.lib("com.example.testFeatureLib").withFile("src/main/res/raw/test_feature_lib_file.txt", "test feature lib file")
  ...
  private val middleFeature =
    MinimalSubProject.dynamicFeature("com.example.middleFeature").withFile("src/main/res/raw/middle_feature_file.txt", "mid feature file")
  ...
  fun testAndroidTestBuildCorrectArtifacts() {
    ...
    val androidTestFeatureRJar =
      project
        .getSubproject("feature")
        .getIntermediateFile("compile_and_runtime_r_class_jar", "debugAndroidTest", "processDebugAndroidTestResources", "R.jar")
        .toPath()
