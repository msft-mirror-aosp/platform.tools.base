...
class MlGeneratedClassTest {
  @JvmField @Rule val project: GradleTestProject = builder().fromTestApp(app("com.android.app")).create()
  ...
  fun setUp() {
    ...
    TestFileUtils.searchAndReplace(buildFile, "defaultConfig.minSdkVersion 14", "defaultConfig.minSdkVersion 19")
  ...
  fun testMobileNetModelClass() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(
        createOutputGetterMethodReference("getProbabilityAsCategoryList", LIST),
        createOutputGetterMethodReference("getProbabilityAsTensorBuffer", TENSOR_BUFFER),
      )
  ...
  fun testMobileNetModelClass_subFolder() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(
        createGetterMethodReference(MODEL_OUTPUT_SUB, "getProbabilityAsCategoryList", LIST),
        createGetterMethodReference(MODEL_OUTPUT_SUB, "getProbabilityAsTensorBuffer", TENSOR_BUFFER),
      )
    verifyModelClass("mobilenet_quant_metadata.tflite", modelMethods, outputsMethods, null, true)
  }
  ...
  fun testMobileNetModelClassWithoutMetadata() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(createOutputGetterMethodReference("getOutputFeature0AsTensorBuffer", TENSOR_BUFFER))
  ...
  fun testMobileNetModelClassWithV2Metadata() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(createOutputGetterMethodReference("getProbabilityAsTensorBuffer", TENSOR_BUFFER))
  ...
  fun testDeepLabModelClass() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(createOutputGetterMethodReference("getSegmentationMasksAsTensorBuffer", TENSOR_BUFFER))
  ...
  fun testSsdOdtModelClass() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(
        createOutputGetterMethodReference("getClassesAsTensorBuffer", TENSOR_BUFFER),
        createOutputGetterMethodReference("getLocationsAsTensorBuffer", TENSOR_BUFFER),
        createOutputGetterMethodReference("getNumberOfDetectionsAsTensorBuffer", TENSOR_BUFFER),
        createOutputGetterMethodReference("getScoresAsTensorBuffer", TENSOR_BUFFER),
      )
  ...
  fun testSsdOdtModelClass_v2() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(
        ...
        createOutputGetterMethodReference("getNumberOfDetectionsAsTensorBuffer", TENSOR_BUFFER),
        createOutputGetterMethodReference("getScoresAsTensorBuffer", TENSOR_BUFFER),
        createOutputGetterMethodReference("getDetectionResultList", LIST),
      )
    val groupClassMethods: Set<MethodReference> =
      ImmutableSet.of(
        createGetterMethodReference(MODEL_DETECTION_RESULT, "getClassesAsString", STRING),
        createGetterMethodReference(MODEL_DETECTION_RESULT, "getLocationsAsRectF", RECT_F),
        createGetterMethodReference(MODEL_DETECTION_RESULT, "getScoresAsFloat", "F"),
      )
    verifyModelClass(
  ...
  fun testStyleTransferModelClass() {
    ...
    val outputsMethods: Set<MethodReference> =
      ImmutableSet.of(
        createOutputGetterMethodReference("getStyledImageAsTensorImage", TENSOR_IMAGE),
        createOutputGetterMethodReference("getStyledImageAsTensorBuffer", TENSOR_BUFFER),
      )
  ...
  private fun verifyModelClass(
    ...
    FileUtils.copyFile(TestUtils.resolveWorkspacePath("prebuilts/tools/common/mlkit/testData/models/$modelFileName").toFile(), modelFile)
    project.executor().run(":assembleDebug")

    val apk = project.getApk(GradleTestProject.ApkType.DEBUG)
    assertThat(apk.getClass(modelClass)!!.methods).containsAtLeastElementsIn(expectedModelMethods)
    assertThat(apk.getClass(modelOutputClass)!!.methods).containsAtLeastElementsIn(expectedOutputMethods)

    if (groupClassInfo != null) {
      assertThat(apk.getClass(groupClassInfo.className)!!.methods).containsAtLeastElementsIn(groupClassInfo.methods)
    }
  ...
  companion object {
    ...
    private const val TENSOR_BUFFER = "Lorg/tensorflow/lite/support/tensorbuffer/TensorBuffer;"
    ...
    private fun createCloseMethodReference(definingClass: String): MethodReference {
      return ImmutableMethod(definingClass, "close", null, "V", 0, null, null, null)
    }

    private fun createOutputGetterMethodReference(name: String, returnType: String): MethodReference {
      return createGetterMethodReference(MODEL_OUTPUT, name, returnType)
    }

    private fun createGetterMethodReference(definingClass: String, name: String, returnType: String): MethodReference {
      return ImmutableMethod(definingClass, name, ParamUtil.parseParamString(""), returnType, 0, null, null, null)
    }
  }
}
