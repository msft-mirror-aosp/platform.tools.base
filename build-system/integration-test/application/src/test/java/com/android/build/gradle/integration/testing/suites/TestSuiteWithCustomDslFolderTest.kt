...
class TestSuiteWithCustomDslFolderTest(val testType: TestType) {
    ...
    val rule = GradleRule.configure()
        .withMavenRepository {
            ...
            androidApplication {
                ...
                pluginCallbacks += if (testType == TestType.HOST_JAR)
                    AddStaticFolderToHostJarTestSuiteCallback::class.java
    ...
    fun upToDateCheck() {
        ...
        Truth.assertThat(javaRes.resolve("some/random").listFiles().map { it.name })
            .containsExactly("file.txt", "res.txt")
    ...
    fun fileAddedCheck() {
        ...
        Truth.assertThat(result.didWorkTasks).contains(":app:processFirstDebugJavaRes")
        val javaRes = getJavaRes(project)
            .resolve("some${File.separatorChar}random")
        Truth.assertThat(javaRes.exists()).isTrue()
        Truth.assertThat(javaRes.listFiles().map { it.name }).containsExactly("file.txt", "res.txt")

        project.withReversibleModifications { build ->
            build.subProject(":app").files.run {
                add("src/shared/resources/some/random/third.txt", "yet another one")
            }
    ...
    fun testModel() {
        ...
        val firstTestSuiteResources = if (testType == TestType.HOST_JAR) {
            val sources = firstTestSuite.hostJars.single()
            Truth.assertThat(sources.type).isEqualTo(
                SourceType.HOST_JAR
            )
            sources.resources
        ...
        androidComponents.finalizeDsl { android ->
            android.testOptions.suites.getByName("first") { first ->
                first.hostJar {
                    ...
                    javaClass.getMethod("addStaticSourceSet", String::class.java )
                        .invoke(this, "src/shared")
                }
            }
