...
/**
 * Test that uses the POST_COMPILATION_CLASSES artifact to generate a class "GeneratorUtils". That class will use one of the type added by
 * this library module of the test project.
 *
...
class AddPostCompileCodeGeneratorTest(private val jacocoSupport: Boolean, private val minifyEnabled: Boolean) {
  ...
  fun testBuildCorrectness() {
    ...
    if (jacocoSupport) {
      ...
      PathSubject.assertThat(generatedFiles).containsFile("GeneratorUtils.class")
      val originalFiles =
        gradleBuild
          .androidApplication()
          .resolve(ScopedArtifact.CLASSES)
          ...
          .resolve("com/android/test/utils")
      PathSubject.assertThat(originalFiles).containsFile("GeneratorUtils.class")
      Truth.assertThat(originalFiles.resolve("GeneratorUtils.class").toFile().length())
        .isNotEqualTo(generatedFiles.resolve("GeneratorUtils.class").toFile().length())
    }
  }
}
