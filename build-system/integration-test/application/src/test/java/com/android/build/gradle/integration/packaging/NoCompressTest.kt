...
class NoCompressTest {
  ...
  fun noCompressIsAccepted() {
    ...
    val unitTestApk =
      FileUtils.join(
        project.getIntermediateFile(InternalArtifactType.APK_FOR_LOCAL_TEST.getFolderName()),
        "debugUnitTest",
  ...
  fun noCompressWithEmptyString() {
    TestFileUtils.searchAndReplace(project.buildFile, "noCompress = [", "noCompress = ['', ")
    project.execute(":assembleDebug")
    val apk = project.getApk(GradleTestProject.ApkType.DEBUG)
    assertThat(apk).exists()
    val uncompressedPaths =
      apk.entries
        ...
        .filter { it.startsWith("jres.") || it.startsWith("assets/") || it.startsWith("res/") }
    Truth.assertThat(uncompressedPaths.size).isEqualTo(36)
    ZFile.openReadOnly(apk.file.toFile()).use {
      uncompressedPaths.forEach { path -> it.expectCompressionMethodOf(path).isEqualTo(CompressionMethod.STORE) }
    }
  }
  ...
  fun bundleNoCompressTest() {
    ...
    FileUtils.createZipFilesystem(
        project.getIntermediateFile("apks_from_bundle", "debug", "makeApkFromBundleForDebug", "bundle.apks").toPath()
      )
      .use { apks -> extracted.outputStream().buffered().use { Files.copy(apks.getPath("splits/base-master.apk"), it) } }
  ...
  private fun ZFile.expectCompressionMethodOf(path: String) =
    expect.that(get(path)?.centralDirectoryHeader?.compressionInfoWithWait?.method).named("Compression method of $path")
}
