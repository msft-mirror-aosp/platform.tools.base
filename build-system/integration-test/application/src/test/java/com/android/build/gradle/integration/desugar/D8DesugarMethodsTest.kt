...
class D8DesugarMethodsTest {
  ...
  fun testModelFetching() {
    ...
    val d8BackportedMethods = model!!.variants.first().desugaredMethods.find { it.name.contains("D8BackportedDesugaredMethods.txt") }
    assertTrue { d8BackportedMethods!!.readLines().contains(expectedMethods) }
  }

  @Test
  fun testMethodListAffectedByMinSdk() {
    TestFileUtils.appendToFile(project.buildFile, "android.defaultConfig.minSdk = 24")
    ...
    assertTrue { desugaredMethodsForDebug!!.readLines().contains(requireNonNullElseGetMethod) }
    assertFalse { desugaredMethodsForRelease!!.readLines().contains(requireNonNullElseGetMethod) }
  }

  private fun getDesugaredMethods(model: AndroidProject?, variantName: String): File? {
    check(model != null) { "model should not be null" }
    return model.variants.first { it.name == variantName }.desugaredMethods.find { it.name.contains("D8BackportedDesugaredMethods.txt") }
  }
}
