...
/**
 * Tests that dexing with desugaring is incremental (only a minimal number of class files are re-dexed).
 *
 * Note that this test is focused on the incrementality aspect. The correctness aspect should be tested by other tests.
 */
@RunWith(FilterableParameterized::class)
class IncrementalDexingWithDesugaringTest(private val scenario: Scenario, private val withMinSdk24Plus: Boolean) {

  companion object {

    @Parameterized.Parameters(name = "scenario_{0}_minSdk24Plus_{1}")
    @JvmStatic
    ...
    private const val CLASS_USING_INTERFACE_WITH_DEFAULT_METHOD = "ClassUsingInterfaceWithDefaultMethod"
  ...
  var project =
    EmptyActivityProjectBuilder()
      ...
      .addAndroidLibrary(addImplementationDependencyFromApp = scenario in setOf(ANDROID_LIB, ANDROID_LIB_WITH_POST_JAVAC_CLASSES))
      .addJavaLibrary(addImplementationDependencyFromApp = scenario == JAVA_LIB)
      .build()
  ...
  private fun getSubproject() =
    when (scenario) {
      APP -> app
      ANDROID_LIB,
      ANDROID_LIB_WITH_POST_JAVAC_CLASSES -> androidLib
      JAVA_LIB -> javaLib
    }
  ...
  private fun getPackageName() =
    when (scenario) {
      ...
      ANDROID_LIB,
      ANDROID_LIB_WITH_POST_JAVAC_CLASSES -> COM_EXAMPLE_LIB
  ...
  fun setUp() {
    ...
    val classUsingInterfaceWithDefaultMethodFullName = "$packagePath/$CLASS_USING_INTERFACE_WITH_DEFAULT_METHOD"
    val standAloneClassFullName = "$packagePath/$STAND_ALONE_CLASS"

    // Java source files
    val javaFile: (classFullName: String) -> File = { File("${subproject.mainSrcDir}/$it.java") }
    interfaceWithDefaultMethodJavaFile = javaFile(interfaceWithDefaultMethodFullName)
    classUsingInterfaceWithDefaultMethodJavaFile = javaFile(classUsingInterfaceWithDefaultMethodFullName)
    ...
    val dexDir =
      when (scenario) {
        ...
        ANDROID_LIB,
        ANDROID_LIB_WITH_POST_JAVAC_CLASSES -> androidLib.buildDir.resolve(".transforms")
    ...
    incrementalTestHelper =
      IncrementalTestHelper(
          ...
          filesOrDirsToTrackChanges =
            setOf(interfaceWithDefaultMethodClassFile, classUsingInterfaceWithDefaultMethodClassFile, standAloneClassClassFile) +
   ...
   *
   * We can't locate them before the build because they may reside in a `<project>/build/.transforms/<hash>` directory where `<hash>` is not
   * known in advance.
   */
   ...
   *
   * We can't locate them before the build because they take the form of `<project>/build/.transforms/<hash>` where `<hash>` is not known in
   * advance.
   *
   * For example, if the given project contains
   * `<project>/build/.transforms/16e674a220d3c8cee4f318266cc44626/transformed/classes/classes.dex`, this method returns
   * `<project>/build/.transforms/16e674a220d3c8cee4f318266cc44626`.
   */
  private fun findDexTransformDir(project: GradleTestProject): File {
    val dexDirs =
      project.buildDir.resolve(".transforms").listFiles()!!.filter {
        it.isDirectory && it.walk().any { file -> file.extension.equals("dex", ignoreCase = true) }
      }
  ...
  enum class Scenario {
    ...
    /** Apply a change in an Android library, where the library has additional post-Javac classes. */
    ANDROID_LIB_WITH_POST_JAVAC_CLASSES,
