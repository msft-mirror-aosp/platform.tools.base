...
class AutoServiceTest(private val pluginName: String) {
  @get:Rule val project: GradleTestProject = GradleTestProject.builder().fromTestApp(HelloWorldApp.forPlugin(pluginName)).create()
  ...
  fun addAutoService() {
    ...
    Files.write(
      ...
      listOf("package com.example.helloworld;", "", "public interface MyService {", "    void doSomething();", "}", ""),
  ...
  fun checkAutoServiceResourceIncluded() {
    ...
    if (pluginName == "com.android.application") {
      assertThat(project.getApk(GradleTestProject.ApkType.DEBUG)).containsJavaResource("META-INF/services/com.example.helloworld.MyService")
    } else {

      project.assertAar(AarSelector.DEBUG) { javaResources().containsExactly("META-INF/services/com.example.helloworld.MyService") }
    }
  }
