...
class DependenciesReportTest {
  ...
  val app = MinimalSubProject.app("com.example.app").withFile("local.jar", TestInputsGenerator.jarWithClasses(listOf()))
    ...
    ZipFile(bundle).use {
      ...
      val mavenLib =
        libraryList
          ...
          .filter { library -> library.mavenLibrary.groupId.equals("androidx.core") && library.mavenLibrary.artifactId.equals("core") }
          .collect(toImmutableList())
      val fileLib = libraryList.stream().filter { library -> !library.hasMavenLibrary() }.collect(toImmutableList())
