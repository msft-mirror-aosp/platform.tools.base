...
class NoManifestTest {
  ...
  fun noManifestInLibraryModuleTest() {
    ...
    val fileOutput =
      FileUtils.join(
        SingleArtifact.MERGED_MANIFEST.getOutputDir(project.getSubproject(":lib").buildDir),
        "debug",
    ...
    val manifestFile = FileUtils.join(project.getSubproject(":lib").projectDir, "src", "main", "AndroidManifest.xml")
