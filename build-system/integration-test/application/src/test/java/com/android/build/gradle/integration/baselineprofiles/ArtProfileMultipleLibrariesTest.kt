...
class ArtProfileMultipleLibrariesTest(private val withArtProfileR8Rewriting: Boolean, private val minifyEnabled: Boolean) {
  ...
  companion object {
    ...
    fun checkAndroidArtifact(tempFolder: TemporaryFolder, target: Zip, entryName: String, expected: (ByteArray) -> Unit) {
      target.getEntry(entryName)?.let {
        val tempFile = tempFolder.newFile()
        Files.newInputStream(it).use { inputStream -> Files.copy(inputStream, tempFile.toPath(), StandardCopyOption.REPLACE_EXISTING) }
  ...
  private val lib1 = HelloWorldApp.forPluginWithNamespace("com.android.library", "com.example.lib1")
  private val lib2 = HelloWorldApp.forPluginWithNamespace("com.android.library", "com.example.lib2")
  private val lib3 = HelloWorldApp.forPluginWithNamespace("com.android.library", "com.example.lib3")
  ...
  private fun testMultipleLibrariesAndOptionalApplicationArtProfileMerging(addApplicationProfile: Boolean) {
    ...
    if (addApplicationProfile) {
      ...
      File(appAndroidAssets, SdkConstants.FN_ART_PROFILE).writeText(applicationBaselineProfContent)
    }
    ...
    for (i in 1..3) {
      val library =
        project.getSubproject(":lib$i").also {
          it.buildFile.appendText(
            """
            android {
                defaultConfig {
      ...
      File(library.mainSrcDir, "com/example/lib$i/Foo.java")
        .writeText(
          """
      ...
      File(library.mainSrcDir, "com/example/lib$i/Bar.java")
        .writeText(
          """
                        package com.example.lib$i;
                        public class Bar {
      ...
      File(library.projectDir, "consumer-rules.pro")
        .writeText(
          """
    ...
    val result =
      project
        .executor()
        .run(":lib1:bundleReleaseAar", ":lib2:bundleReleaseAar", ":lib3:bundleReleaseAar", ":app:assembleRelease", ":app:bundleRelease")
    ...
    for (i in 1..3) {
      ...
      // check packaging.
      project.getSubproject(":lib$i").assertAar(AarSelector.RELEASE) { textFile(aarEntryName).isEqualTo(expectedBaselineProfContent) }
    }
    ...
    Truth.assertThat(ArtProfile(ByteArrayInputStream(binaryProfile.readBytes()))).isNotNull()

    val binaryProfileMetadata =
      FileUtils.join(
        project.getSubproject(":app").buildDir,
        SdkConstants.FD_INTERMEDIATES,
    ...
    project.getSubproject(":app").getApk(GradleTestProject.ApkType.RELEASE).also {
      ...
      JarFile(it.file.toFile()).use { jarFile ->
        val artProfileEntry = jarFile.getEntry("${SdkConstants.FN_BINART_ART_PROFILE_FOLDER_IN_APK}/${SdkConstants.FN_BINARY_ART_PROFILE}")
        Truth.assertThat(artProfileEntry.method).isEqualTo(ZipEntry.STORED)

        val artProfileMetadataEntry =
          jarFile.getEntry("${SdkConstants.FN_BINART_ART_PROFILE_FOLDER_IN_APK}/${SdkConstants.FN_BINARY_ART_PROFILE_METADATA}")
    ...
    project.getSubproject(":app").getBundle(GradleTestProject.ApkType.RELEASE).also {
      checkAndroidArtifact(tempFolder, it, aabEntryName) { fileContent ->
        Truth.assertThat(ArtProfile(ByteArrayInputStream(fileContent))).isNotNull()
      }
    }
  }
