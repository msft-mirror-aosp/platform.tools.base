...
class SettingsSdkTest {
  ...
  fun checkDslSetUpInModel(build: GradleBuild, compileSdkVersion: String) {
    ...
    assertThat(androidDsl.compileTarget).named("androidDsl.compileTarget").isEqualTo("android-$compileSdkVersion")
    ...
    assertThat(androidDsl.lintOptions?.targetSdk?.apiLevel).named("androidDsl.lintOptions.targetSdk.apiLevel").isEqualTo(null)
    ...
    assertThat(libAndroidDsl.compileTarget).named("libAndroidDsl.compileTarget").isEqualTo("android-$compileSdkVersion")
    assertThat(libAndroidDsl.defaultConfig.targetSdkVersion).named("libAndroidDsl.defaultConfig.targetSdkVersion").isEqualTo(null)
  ...
  class LibSharedCheck : LibraryComponentCallback {
    override fun handleExtension(project: Project, androidComponents: LibraryAndroidComponentsExtension) {
      androidComponents.finalizeDsl { extension ->
        check(extension.compileSdk == COMPILE_SDK_VERSION) { "compileSdk should be ${DEFAULT_COMPILE_SDK_VERSION}" }
        check(extension.defaultConfig.minSdk == MIN_SDK_VERSION) { "minSdk should be ${MIN_SDK_VERSION}" }
        check(extension.testOptions.targetSdk == TARGET_SDK_VERSION) { "targetSdk should be ${TARGET_SDK_VERSION}" }
      }
    }
  }
  ...
  class LibMinorVersionCheck : LibraryComponentCallback {
    override fun handleExtension(project: Project, androidComponents: LibraryAndroidComponentsExtension) {
      androidComponents.finalizeDsl { extension ->
        check(extension.compileSdkMinor == COMPILE_SDK_MINOR_VERSION) { "compileSdkMinor should be ${COMPILE_SDK_MINOR_VERSION}" }
      }
    }
  }
  ...
  class KmpLibVersionCheck : AndroidKotlinMultiplatformLibraryComponentCallback {
    override fun handleExtension(project: Project, extension: KotlinMultiplatformAndroidComponentsExtension) {
      extension.finalizeDsl { extension ->
        check(extension.compileSdk == COMPILE_SDK_VERSION) { "compileSdk should be $COMPILE_SDK_VERSION" }
        check(extension.minSdk == MIN_SDK_VERSION) { "compileSdkExtension should be $MIN_SDK_VERSION" }
        extension.compileSdk {
          check(version?.minorApiLevel == COMPILE_SDK_MINOR_VERSION) { "compileSdkMinor should be ${COMPILE_SDK_MINOR_VERSION}" }
        }
      }
    }
  }
