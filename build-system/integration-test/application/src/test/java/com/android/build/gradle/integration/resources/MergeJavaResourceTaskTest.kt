...
class MergeJavaResourceTaskTest {
  ...
  val rule =
    GradleRule.configure()
      .withMavenRepository {
        jar("com.example:lib1:0.1").addTextFile("conflict_res", "a")
        jar("com.example:lib2:0.1").addTextFile("conflict_res", "b")
        jar("com.example:libWithAdditionalArtifact:0.1").addTextFile("content1", "a").addTextFile("content2", "b")
      }
      .from {
        androidApplication {
          android {
  ...
  fun ensureJavacDependencyIfAnnotationProcessorAddedViaDefaultDependencies() {
    ...
    if (build.androidApplication().resolve(InternalArtifactType.COMPILE_BUILD_CONFIG_JAR.getFolderName()).exists()) {
  ...
  fun ensureJavaResIsNotRunningWhenOnlyClassesChange() {
    ...
    build
      .androidApplication()
      .files
      .update("src/main/com/android/tests/basic/NewSourceFile.java")
      .replaceWith(
        """
        package com.android.tests.basic;
