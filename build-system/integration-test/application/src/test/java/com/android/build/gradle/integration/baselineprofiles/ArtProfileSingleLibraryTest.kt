...
class ArtProfileSingleLibraryTest {
  ...
  fun testSingleLibraryWithGeneratedSourceAndApplicationArtProfiles() {
    testSingleLibraryWithOptionalApplicationArtProfileMerging(addApplicationProfile = true, useGeneratedSource = true)
  }
  ...
  private fun testSingleLibraryWithOptionalApplicationArtProfileMerging(
    ...
    if (addApplicationProfile) {
      ...
      File(appAndroidAssets, SdkConstants.FN_ART_PROFILE).writeText(applicationFileContent)
    }
    ...
    if (useGeneratedSource) {
      ...
      FileUtils.createFile(library.file("src/release/generated/baselineProfiles/baseline-prof.txt"), libraryFolderFileContent1)
      FileUtils.createFile(library.file("src/release/generated/baselineProfiles/myCustomProfName.txt"), libraryFolderFileContent2)
    }

    File(androidAssets, SdkConstants.FN_ART_PROFILE).writeText(libraryBaselineProfileContent)

    val result = project.executor().run(":lib:bundleReleaseAar", ":app:assembleRelease", ":app:makeApkFromBundleForRelease")
    ...
    val parts =
      if (useGeneratedSource) {
        listOf(libraryBaselineProfileContent, libraryFolderFileContent1, libraryFolderFileContent2)
      } else {
    ...
    // check packaging.
    project.getSubproject(":lib").assertAar(AarSelector.RELEASE) { textFile(aarEntryName).checkContains(parts) }
    ...
    Truth.assertThat(dexMetadataProperties.readText())
      .isEqualTo(
        """
        31=0/.dm
        2147483647=0/.dm
    ...
    val bundleApks =
      project
        .getSubproject(":app")
        .getIntermediateFile("apks_from_bundle", "release", "makeApkFromBundleForRelease", "bundle.apks")
        .toPath()
