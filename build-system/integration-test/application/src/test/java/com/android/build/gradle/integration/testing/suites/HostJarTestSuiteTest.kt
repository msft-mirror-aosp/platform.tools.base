...
class HostJarTestSuiteJavaResProcessingTest {
    ...
    fun upToDateCheck() {
        ...
        PathSubject.assertThat(javaRes.resolve("some/random/file.txt"))
            .contains("some random text")
    ...
    fun fileRemovedCheck() {
        ...
        project.withReversibleModifications { build ->
            build.subProject(":app").files.run {
                remove("src/first/resources/some/random/res.txt")
            }
    ...
    fun fileAddedCheck() {
        ...
        project.withReversibleModifications { build ->
            build.subProject(":app").files.run {
                add("src/first/resources/some/random/third.txt", "yet another one")
            }
    ...
    fun fileChangedCheck() {
        ...
        project.withReversibleModifications { build ->
            build.subProject(":app")
                .files
                .update("src/first/resources/some/random/file.txt") {
                    replaceWith("some update")
                }
            ...
            Truth.assertThat(result.didWorkTasks).contains(":app:processFirstRedDebugJavaRes")
            PathSubject.assertThat(javaRes.resolve("some/random/file.txt"))
                .contains("some update")
        }
    }

    private fun getJavaRes(project: GradleBuild ) =
        project.subProject(":app").resolve(InternalArtifactType.JAVA_RES)
