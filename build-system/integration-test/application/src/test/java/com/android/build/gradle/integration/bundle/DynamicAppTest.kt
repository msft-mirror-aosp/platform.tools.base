...
internal val multiDexSupportLibClasses =
  listOf(
...
class DynamicAppTest {
  ...
  private val debugUnsignedContent: Array<String> =
    bundleContent.plus(
      arrayOf(
  ...
  private val releaseUnsignedContent: Array<String> =
    bundleContent
      .toList()
      .plus(
        listOf(
  ...
  private val mainDexClasses: List<String> = multiDexSupportLibClasses.plus("Lcom/example/app/AppClassNeededInMainDexList;")

  private val mainDexListClassesInBundle: List<String> = mainDexClasses.plus("Lcom/example/feature1/Feature1ClassNeededInMainDexList;")
  ...
  fun `test model contains feature information`() {
    ...
    val appProject = modelContainer.getProject(":app").androidProject
    val feature1Project = modelContainer.getProject(":feature1").androidProject

    assertThat(appProject).isNotNull()
    assertThat(appProject?.dynamicFeatures).containsExactly(":feature1", ":feature2")
  ...
  fun `test bundleMinSdkDifference task`() {
    TestFileUtils.searchAndReplace(project.getSubproject(":feature1").buildFile, "minSdkVersion 18", "minSdkVersion 21")
    ...
    val manifestFile =
      FileUtils.join(
        project.getSubproject("feature1").buildDir,
        "intermediates",
        "packaged_manifests",
        "debug",
  ...
  fun `test bundleDebug task`() {
    ...
    val originalManifestFile = FileUtils.join(project.getSubproject("feature1").getMainSrcDir(""), "AndroidManifest.xml")
  ...
  fun `test unsigned bundleRelease task with r8 dontminify`() {
    ...
    project.getSubproject("app").projectDir.resolve("proguard-rules.pro").writeText("-dontobfuscate\n-dontoptimize")
    val bundleTaskName = getBundleTaskName("release", ":app")
    project.executor().run("app:$bundleTaskName")
    ...
    Zip(bundleFile).use {
      ...
      val expectedMainDexList =
        multiDexSupportLibClasses.map { it.substring(1, it.length - 1) + ".class" }.minus("android/support/multidex/MultiDex\$V4.class")
  ...
  fun `test packagingOptions`() {
    ...
    Zip(bundleFile).use { assertThat(it.entries.map { it.toString() }).containsExactly(*debugUnsignedContent) }
  }
  ...
  fun `test abiFilters when building bundle`() {
    ...
    val bundleContentWithAbis =
      debugUnsignedContent.plus(
        listOf(
          "/base/native.pb",
    ...
    Zip(bundleFile).use { assertThat(it.entries.map { it.toString() }).containsExactly(*bundleContentWithAbis) }
  }
  ...
  fun `test warning if abiFilters specified in dynamic feature`() {
    ...
    val modelContainer = project.modelV2().ignoreSyncIssues().fetchModels().container
    val issue = modelContainer.getProject(":feature1").issues?.syncIssues?.first()
  ...
  fun `test making APKs from bundle`() {
    ...
    project.executor().with(StringOption.IDE_APK_SELECT_CONFIG, jsonFile.toString()).run("app:$apkFromBundleTaskName")
    ...
    assertThat(apkFileArray.toList()).named("APK List for API 27").containsExactly("base-master.apk")

    val baseApk = File(apkFolder, "base-master.apk")
    Zip(baseApk).use { assertThat(it.entries.map { it.toString() }).containsAllOf("/META-INF/BNDLTOOL.RSA", "/META-INF/BNDLTOOL.SF") }
    ...
    fun checkApkFromBundleModelFile(postApkFromBundleTaskModelFile: String) {
      ...
      val apkFromBundleModel = BuiltArtifactsLoaderImpl.loadFromFile(File(postApkFromBundleTaskModelFile))
    ...
    variantsBuildInformation?.forEach { buildInformation ->
      ...
      if (buildInformation.name == "debug") {
        checkApkFromBundleModelFile(buildInformation.mainArtifact.bundleInfo?.apkFromBundleTaskOutputListingFile?.path!!)
      }
    }
    val debugVariantModule = appModel.androidProject?.variants?.first { it.name == "debug" }
    val postApkFromBundleTaskModelFile = debugVariantModule?.mainArtifact?.bundleInfo?.apkFromBundleTaskOutputListingFile
    assertThat(postApkFromBundleTaskModelFile).isNotNull()
    ...
    assertThat(apkFileArray.toList()).named("APK List for API 18").containsExactly("standalone.apk")

    // Check universal APK generation too.
    project.executor().run(":app:packageDebugUniversalApk")

    project.getSubproject("app").getBundleUniversalApk(GradleTestProject.ApkType.DEBUG).use {
      assertThat(it.entries.map { it.toString() }).containsAllOf("/META-INF/BNDLTOOL.RSA", "/META-INF/BNDLTOOL.SF")
    }

    val result = project.executor().run(":app:packageDebugUniversalApk")
  ...
  fun `test extracting instant APKs from bundle`() {
    ...
    var apkFileArray = apkFolder.list() ?: fail("No Files at $apkFolder")
    assertThat(apkFileArray.toList())
      .named("APK List when extract instant is false")
      .containsExactly("base-master.apk", "feature2-master.apk")
    ...
    apkFileArray = apkFolder.list() ?: fail("No Files at $apkFolder")
    assertThat(apkFileArray.toList())
      .named("APK List when extract instant is true")
      .containsExactly("instant-base-master.apk", "instant-feature2-master.apk")
  }
  ...
  fun `test extracting instant APKs from bundle with IDE hint`() {
    ...
    assertThat(apkFileArray.toList())
      .named("APK List when extract instant is true")
      .containsExactly("instant-base-master.apk", "instant-feature1-master.apk", "instant-feature2-master.apk")
    ...
    assertThat(apkFileArray.toList())
      .named("APK List when extract instant is true")
      .containsExactly("instant-base-master.apk", "instant-feature1-master.apk")
  }
  ...
  fun `test overriding bundle output location`() {
    ...
    project.executor().with(StringOption.IDE_APK_LOCATION, "out/test/my-bundle").run("app:bundle")

    for (flavor in listOf("red", "blue")) {
      val bundleFile = project.locateBundleFileViaModel("${flavor}Debug", ":app")
      assertThat(FileUtils.join(project.getSubproject(":app").projectDir, "out", "test", "my-bundle", flavor, "debug", bundleFile.name))
        .exists()
    }

    // redo the test with an absolute output path this time.
    val absolutePath = tmpFile.newFolder("my-bundle").absolutePath
    project.executor().with(StringOption.IDE_APK_LOCATION, absolutePath).run("app:bundle")

    for (flavor in listOf("red", "blue")) {
      val bundleFile = project.locateBundleFileViaModel("${flavor}Debug", ":app")
      assertThat(FileUtils.join(File(absolutePath), flavor, "debug", bundleFile.name)).exists()
    }
  }
  ...
  fun `test invalid debuggable combination`() {
    ...
    // use a relative path to the project build dir.
    val failure = project.executor().expectFailure().run("app:$apkFromBundleTaskName")
    ...
    assertThat(exception).hasMessageThat().contains("set android.buildTypes.debug.debuggable = true")
  }
...
<vector xmlns:android="http://schemas.android.com/apk/res/android"
  ...
  fun `test bundleRelease is signed correctly`() {
    ...
    KeystoreHelper.createNewStore("jks", keyStoreFile, unicodeStorePass, unicodeKeyPass, keyAlias, "CN=Bundle signing test", 100)
    ...
    Assert.assertThrows(InvalidBundleException::class.java) {
      CheckTransparencyCommand.builder().setMode(CheckTransparencyCommand.Mode.BUNDLE).setBundlePath(bundleFile.toPath()).build().execute()
    }
  }

  @Test
  fun `test bundleRelease is signed correctly with Code Transparency via DSL`() {
    testBundleReleaseIsSignedCorrectlyWithCodeTransparency { keyAlias, keyPass, keyStoreFile, storePass ->
      """
  ...
  private fun testBundleReleaseIsSignedCorrectlyWithCodeTransparency(
    ...
    val keyStoreFile = tmpFile.root.resolve("keystore")
    KeystoreHelper.createNewStore("jks", keyStoreFile, storePass, keyPass, keyAlias, "CN=Bundle signing test", 100, 3072)

    val appProject = project.getSubproject(":app")
    TestFileUtils.appendToFile(appProject.buildFile, buildFileCustomizer(keyAlias, keyPass, keyStoreFile, storePass))
    ...
    ByteArrayOutputStream().use { outputStream ->
      ...
      assertThat(outputStream.toString()).contains("Code transparency signature is valid.")
    }
  ...
  fun `test excluding sources for release`() {
    ...
    project.executor().with(BooleanOption.EXCLUDE_RES_SOURCES_FOR_RELEASE_BUNDLES, false).run("app:$bundleTaskName")
    ...
    Zip(bundleFile).use { bundle ->
      bundle.entries
        .filter { it.toString().endsWith("resources.pb") }
        .forEach {
    ...
    project.executor().with(BooleanOption.EXCLUDE_RES_SOURCES_FOR_RELEASE_BUNDLES, true).run("app:$bundleTaskName")
  ...
  fun `test flag local_testing and extracting _ALL_ apks`() {
    ...
    ApkSubject.assertThat(baseApk) {
      manifestAsNodes().node("manifest").node("application").nodeByNameAndAttribute("meta-data", "local_testing_dir")
    }
  }
  ...
  fun `generate correct output-metadata`() {
    ...
    val outputMetadataFile =
      FileUtils.join(
        project.getSubproject("app").buildDir,
        "intermediates",
        InternalArtifactType.APK_FROM_BUNDLE_IDE_MODEL.getFolderName(),
        "debug",
  ...
  fun `test installing AndroidTest variant apk`() {
    val result = project.executor().expectFailure().run(":app:installDebugAndroidTest")
  ...
  /**
   * Modified implementation of [GradleTestProject.getBundleTaskName] so we can suppress warnings about
   * `[BooleanOption.USE_ANDROID_X]=false`
   */
  @Deprecated("Migrate this test to android.useAndroidX=true and remove this method")
  fun getBundleTaskName(variantName: String, projectPath: String?): String {
    val appModel =
      project.modelV2().suppressOptionWarning(BooleanOption.USE_ANDROID_X).fetchModels().container.getProject(projectPath).androidProject
  ...
  /**
   * Modified implementation of [GradleTestProject.locateBundleFileViaModel] so we can suppress warnings about
   * `[BooleanOption.USE_ANDROID_X]=false`
   */
  @Deprecated("Migrate this test to android.useAndroidX=true and remove this method")
