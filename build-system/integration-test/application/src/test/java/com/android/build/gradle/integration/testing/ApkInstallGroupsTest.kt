    ...
    override fun handleExtension(
        ...
        androidComponents.onVariants(
            ...
            val task = project.tasks.register(
                ...
                ApksOutputCustomTask::class.java
            )
            variant.outputProviders.provideApkOutputToTask(
                task,
                ApksOutputCustomTask::apkOutput,
                DeviceSpec.Builder().setApiLevel(21).build()
            )
        }
    }
}
...
abstract class ApksOutputCustomTask : DefaultTask() {
    ...
    fun run() {
        apkOutput.get().apkInstallGroups.forEach { installGroup ->
            println(
                installGroup.description +
                        ":[${installGroup.apks.joinToString(",") { it.asFile.name }}]"
            )
        }
    }
}
