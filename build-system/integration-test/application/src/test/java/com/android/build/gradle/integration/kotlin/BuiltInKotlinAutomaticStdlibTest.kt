...
class BuiltInKotlinAutomaticStdlibTest {
  ...
  fun `test kotlin-stdlib automatically added`() {
    val result = rule.build.executor.run(":app:dependencies", "--configuration", "debugCompileClasspath")
  ...
  fun `test kotlin-stdlib added by user`() {
    val build = rule.build { androidApplication { dependencies { implementation("org.jetbrains.kotlin:kotlin-stdlib-jdk8:1.9.24") } } }
  ...
  fun `test kotlin-stdlib automatically added and appears in Gradle module metadata`() {
    ...
    fun JsonArray.getVariant(variantName: String): JsonObject = single { it.asJsonObject["name"].asString == variantName } as JsonObject

    fun JsonObject.assertContainsKotlinStdlib() {
      val kotlinStdlibDep =
        getAsJsonArray("dependencies").find { it is JsonObject && it.asJsonObject.get("module").asString == "kotlin-stdlib" }
  ...
  fun `test kotlin_stdlib_default_dependency=false`() {
    val build = rule.build { gradleProperties { add("kotlin.stdlib.default.dependency", "false") } }
    val result = build.executor.run(":app:dependencies", "--configuration", "debugCompileClasspath")
