...
class StripDebugSymbolsTaskTest {
  ...
  fun testNoNdkLogsIfNoInputs() {
    project.executor().run("stripDebugDebugSymbols").stdout.use { scanner -> assertThat(scanner).doesNotContain("ANDROID_NDK_HOME") }
    doTest(project) {
      it.addFile("src/main/jniLibs/x86/foo.so", "foo")
      project.executor().run("stripDebugDebugSymbols").stdout.use { scanner -> assertThat(scanner).contains("ANDROID_NDK_HOME") }
    }
  }
  ...
  fun testSingleStripDebugSymbolsWarning() {
    val expectedWarning =
      "Unable to strip the following libraries, packaging them as they are: bar.so, foo.so." + " Run with --info option to learn more."
    doTest(project) {
      it.addFile("src/main/jniLibs/x86/foo.so", "foo")
  ...
  fun testStripDebugSymbolsVerboseLogs() {
    ...
    doTest(project) {
      it.addFile("src/main/jniLibs/x86/foo.so", "foo")
      project.executor().withArgument("--info").run("stripDebugDebugSymbols").stdout.use { scanner ->
        assertThat(scanner).contains(expectedWarning)
      }
    }
  }
  ...
  fun testTaskSkippedWhenNoNativeLibs() {
    ...
    assertThat(result1.skippedTasks).containsAtLeastElementsIn(listOf(":stripDebugDebugSymbols"))
    // then test that the task does work if we add a native library.
    doTest(project) {
      it.addFile("src/main/jniLibs/x86/foo.so", "foo")
      val result2 = project.executor().run(":stripDebugDebugSymbols")
      assertThat(result2.didWorkTasks).containsAtLeastElementsIn(listOf(":stripDebugDebugSymbols"))
      // then test that the task is up-to-date if nothing changes.
      val result3 = project.executor().run(":stripDebugDebugSymbols")
      assertThat(result3.upToDateTasks).containsAtLeastElementsIn(listOf(":stripDebugDebugSymbols"))
    }
    ...
    assertThat(result4.didWorkTasks).containsAtLeastElementsIn(listOf(":stripDebugDebugSymbols"))
    ...
    assertThat(result5.skippedTasks).containsAtLeastElementsIn(listOf(":stripDebugDebugSymbols"))
  }
}
