...
/**
 * This class provides test coverage for using custom values for namespace, testNamespace, applicationId, testApplicationId, and various
...
class NamespaceAndApplicationIdTest {
  ...
  fun testDefault() {
    ...
    app.assertApk(DEBUG) { applicationId().isEqualTo("com.example.app") }
    app.assertApk(ANDROIDTEST_DEBUG) { applicationId().isEqualTo("com.example.app.test") }
    assertThat(
        app.resolve(JAVAC).resolve("debugAndroidTest/compileDebugAndroidTestJavaWithJavac/classes/com/example/app/test/BuildConfig.class")
      )
      .isFile()
  }
  ...
  fun testDefaultForTestModule() {
    ...
    build.androidTest().assertApk(DEBUG) { applicationId().isEqualTo("com.example.test") }
  }

  @Test
  fun testCustomApplicationId() {
    val build = rule.build { androidApplication { android.defaultConfig.applicationId = "com.example.applicationId" } }
    val app = build.androidApplication()

    build.executor.run(":app:assembleDebug", ":app:assembleAndroidTest")

    app.assertApk(DEBUG) { applicationId().isEqualTo("com.example.applicationId") }
    app.assertApk(ANDROIDTEST_DEBUG) { applicationId().isEqualTo("com.example.applicationId.test") }
    assertThat(
        app.resolve(JAVAC).resolve("debugAndroidTest/compileDebugAndroidTestJavaWithJavac/classes/com/example/app/test/BuildConfig.class")
      )
      .isFile()
  ...
  fun testCustomTestApplicationId() {
    val build = rule.build { androidApplication { android.defaultConfig.testApplicationId = "com.example.testApplicationId" } }
    val app = build.androidApplication()

    build.executor.run(":app:assembleDebug", ":app:assembleAndroidTest")

    app.assertApk(DEBUG) { applicationId().isEqualTo("com.example.app") }
    app.assertApk(ANDROIDTEST_DEBUG) { applicationId().isEqualTo("com.example.testApplicationId") }
    assertThat(
        app.resolve(JAVAC).resolve("debugAndroidTest/compileDebugAndroidTestJavaWithJavac/classes/com/example/app/test/BuildConfig.class")
      )
      .isFile()
  ...
  fun testCustomNamespace() {
    ...
    app.assertApk(DEBUG) { applicationId().isEqualTo("com.example.namespace") }
    app.assertApk(ANDROIDTEST_DEBUG) { applicationId().isEqualTo("com.example.namespace.test") }
    assertThat(
        app
          .resolve(JAVAC)
  ...
  fun testErrorWhenTestNamespaceEqualsNamespace() {
    ...
    ScannerSubject.assertThat(result.stderr)
      .contains("namespace and testNamespace have the same value (\"com.example.app\"), which is not allowed.")
  }
}
