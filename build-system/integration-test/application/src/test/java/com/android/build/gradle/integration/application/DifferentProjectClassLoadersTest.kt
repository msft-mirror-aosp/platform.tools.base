...
class DifferentProjectClassLoadersTest {
  ...
  /** Test lint because we register the [LintParallelBuildService] once instead of registering one per classloader. */
  @Test
  fun testLint() {
  ...
  fun testAttributionFile() {
    fun setUpDummyTask(taskName: String): String =
      """
                ...
                """
        .trimIndent()
    TestFileUtils.appendToFile(project.getSubproject("androidLib1").buildFile, setUpDummyTask("firstLibTask"))
    TestFileUtils.appendToFile(project.getSubproject("androidLib2").buildFile, setUpDummyTask("secondLibTask"))

    val attributionDir = temporaryFolder.newFolder()
    val result = executor().with(StringOption.IDE_ATTRIBUTION_FILE_LOCATION, attributionDir.absolutePath).run("assembleDebug")
    verifyClassLoaderSetup(result)
  ...
  private fun verifyClassLoaderSetup(result: GradleBuildResult) {
    ...
    result.stdout.forEachLine { if (it.startsWith("Class loader for AGP API = ")) taskLogs.add(it) }
    assertThat(taskLogs).named("Log lines that should contain different class loader hashes").hasSize(2)
  }
