...
class VariantApiWithNameTest {
  ...
  fun testToTransformFilesWithName() {
    ...
    Truth.assertThat(File(project.buildDir, "outputs/bundle/debug").listFiles().map(File::getName)).containsExactly("my-fancy-name.aab")
  }
  ...
  fun testToAppendFileWithName() {
    ...
    Truth.assertThat(
        File(project.buildDir, "sources/multidex_keep_proguard/release/createreleaseManifest/my-fancy-proguard-rules.txt").exists()
      )
      .isTrue()
  }
}
