...
/**
 * Integration test for detection of different issues relating to different combinations of the [BooleanOption.USE_ANDROID_X] and
 * [BooleanOption.ENABLE_JETIFIER] properties.
 */
class AndroidXJetifierMatrixTest {

  @get:Rule val project = GradleTestProject.builder().fromTestApp(MinimalSubProject.app("com.example.application")).create()
  ...
  fun `AndroidX=false, Jetifier=false, AndroidX dependencies present, expect sync issue`() {
    ...
    expectSyncIssue(
      ...
      message =
        "Configuration `:debugAndroidTestCompileClasspath` contains AndroidX dependencies, but the `android.useAndroidX` property is not enabled, which may cause runtime issues.\n" +
