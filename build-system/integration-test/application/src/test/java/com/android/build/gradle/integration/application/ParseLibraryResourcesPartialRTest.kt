...
class ParseLibraryResourcesPartialRTest {
  ...
  val project =
    GradleTestProject.builder()
      ...
      .addGradleProperties("${BooleanOption.ENABLE_PARTIAL_R_INCREMENTAL_BUILDS.propertyName}=true")
      .create()
  ...
  fun `test incremental builds do not modify unnecessary partial r files`() {
    ...
    val partialRIntermediateDir =
      FileUtils.join(
        project.projectDir,
        "lib",
        "build",
        "intermediates",
        InternalArtifactType.LOCAL_ONLY_PARTIAL_SYMBOL_DIRECTORY.getFolderName(),
        "debug",
        "parseDebugLocalResources",
        "partial-r",
      )
    val partialRFiles = partialRIntermediateDir.listFiles() ?: error("No partial-r files generated.")
  ...
  fun `test incremental builds modify partial r files when resource is modified`() {
    ...
    val libLayoutPartialR = FileUtils.join(partialRIntermediateDir, "layout_main.xml.flat-R.txt")
    val libLayoutFile = FileUtils.join(project.projectDir, "lib", "src", "main", "res", "layout", "main.xml")
    val libLayoutPartialROriginalTimestamp = libLayoutPartialR.lastModified()
    val modifiedLibLayout = libLayoutFile.readText(Charset.defaultCharset()).replace("@+id/text", "@+id/changed_text")
    FileUtils.writeToFile(libLayoutFile, modifiedLibLayout)

    // Values (non-modified resource)
    val libValuesPartialR = FileUtils.join(partialRIntermediateDir, "values_values.arsc.flat-R.txt")
    ...
    assertThat(libLayoutPartialR.lastModified()).isGreaterThan(libLayoutPartialROriginalTimestamp)
  ...
  private fun checkLibPartialRFiles(files: List<File>) =
    assertThat(files.map { it.name }).containsExactly("layout_main.xml.flat-R.txt", "values_values.arsc.flat-R.txt")
}
