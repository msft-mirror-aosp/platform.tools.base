...
class IncrementalGlobalSyntheticsTest {
  ...
  val project =
    GradleTestProject.builder()
      .fromTestApp(MultiModuleTestProject.builder().subproject("app", MinimalSubProject.app("com.example.app")).build())
      .create()
  ...
  fun setUp() {
    ...
    changedGlobalInApp =
      app.buildDir.resolve(
        "intermediates/global_synthetics_project/debug/dexBuilderDebug/out/com/example/app/IllformedLocaleExceptionUsage.globals"
      )
    changedDexInApp =
      app.buildDir.resolve("intermediates/project_dex_archive/debug/dexBuilderDebug/out/com/example/app/IllformedLocaleExceptionUsage.dex")
  ...
  fun testModifyDexFileInTask() {
    ...
    TestFileUtils.searchAndReplace(changedSourceInApp, "throw new android.icu.util.IllformedLocaleException();", "//comment out")
    executor().run("assembleDebug")
    ...
    TestFileUtils.searchAndReplace(changedSourceInApp, "//comment out", "throw new android.icu.util.IllformedLocaleException();")
    executor().run("assembleDebug")
