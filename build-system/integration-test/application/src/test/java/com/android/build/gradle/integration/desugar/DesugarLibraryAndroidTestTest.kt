...
class DesugarLibraryAndroidTestTest {
  ...
  val lib =
    MinimalSubProject.lib("com.example.lib").apply {
      appendToBuild(
        """
      ...
      addFile(TestSourceFile("libs/interface.jar", TestInputsGenerator.jarWithClasses(listOf(InterfaceWithDefaultMethod::class.java))))
    }
  ...
  fun testUsingJava8() {
    ...
    project.getApk(GradleTestProject.ApkType.ANDROIDTEST_DEBUG).use { apk ->
      ...
      assertThat(classes.filter { it.startsWith("Lcom/example/lib/Java8Use") }).hasSize(3)
      assertThat(classes.filter { it.startsWith("Lcom/example/lib/test/Java8UseTest") }).hasSize(3)
