...
/**
 * A test that generates projects with all different dsl values applicable for a plugin and makes sure that configuration is successful.
 *
 * The point of this test is to make sure dsl invocation (even with placeholder values) shouldn't break configuration.
 */
...
class AllDslMethodsTest(
  ...
  @Before
  fun prepareProject() {
    val generator = DslScriptGenerator(buildTypeType, defaultConfigType)
    generator.visit(extensionClass, null, extensionAccessor)

    for (index in 1..generator.numberOfScriptsGenerated) {
      val projectName = "project$index"
      allProjects.add(projectName)
      TestFileUtils.appendToFile(project.settingsFile, "include '$projectName'")

      val buildFile = FileUtils.join(project.projectDir, projectName, "build.gradle.kts")
  ...
  fun configureAllProjects() {
    project
      ...
      .run(allProjects.map { projectName -> ":$projectName:clean" })
  }
}

private class DslScriptGenerator(private val buildTypeType: Class<*>? = null, private val defaultConfigType: Class<*>? = null) {
  ...
  private fun visitClosure(callChain: String, closureName: String, closureType: Class<*>) {
    ...
    nestedGenerator.visit(closureType, null, "this")
  ...
  fun visit(dslType: Class<*>, genericType: Type?, callChain: String) {
    ...
    if (mapTypes.any { it.isAssignableFrom(dslType) }) {
      allDslValues.add(
        getPossibleValues((genericType as ParameterizedType).actualTypeArguments.first() as Class<*>).flatMap { key ->
          val valueType = genericType.actualTypeArguments[1]

          if (valueType is ParameterizedType) {
            if (Collection::class.java.isAssignableFrom(valueType.rawType as Class<*>)) {
              getPossibleValues(valueType.actualTypeArguments.first() as Class<*>).map { value -> "$callChain.put($key, listOf($value))" }
            } else {
              throw RuntimeException(valueType.typeName)
            }
          } else {
            getPossibleValues(genericType.actualTypeArguments[1] as Class<*>).map { value -> "$callChain.put($key, $value)" }
          }
        }
      )
      return
    ...
    if (listTypes.any { it.isAssignableFrom(dslType) }) {
      ...
      if (nestedType !in typesToIgnore) {
        allDslValues.add(getPossibleValues(nestedType).map { "$callChain.add($it)" })
      }
      return
    }
    ...
    dslType.methods
      .sortedBy { it.name }
      .forEach { method ->
        if (method.toString() in methodsToIgnore || method.annotations.any { it.annotationClass == Deprecated::class }) {
          return@forEach
        }
        ...
        if (method.returnType.name == "void") {
          if (method.name.startsWith("set")) {
            ...
            getAllValues(method.toString(), method.parameterTypes.first() as Class<*>, "$callChain.${mapSetterToProperty(method.name)} =")
          } else if (method.parameters.size == 1 && method.parameterTypes.first() == Function1::class.java) {
            closures.add(method)
            return@forEach
          }
        } else if (method.parameters.isEmpty()) {
          Truth.assertWithMessage("Unknown $currentCallChain").that(method.name.startsWith("get") || method.name.startsWith("is")).isTrue()
    ...
    closures.forEach { closure ->
      if (!getters.any { getter -> closure.name == getter.name.removePrefix("get").usLocaleDecapitalize() }) {
        if (closure.name in closuresToManuallyVisit) {
          visitClosure(callChain, closure.name, closuresToManuallyVisit[closure.name]!!)
        } else {
          throw RuntimeException("Found a closure ($closure) without a backing getter.")
        }
      }
    }
  }

  private fun getPossibleValues(valueType: Class<*>): List<String> {
    return when (valueType) {
      Any::class.java,
      String::class.java -> listOf("\"${nextString()}\"")
      Int::class.java,
      Integer::class.java -> {
        listOf(nextInt().toString())
      }
      Boolean::class.java,
      ...
      File::class.java -> {
        listOf("File(\"${FileUtils.escapeSystemDependentCharsIfNecessary(nextFile().absolutePath)}\")")
      }
      JavaVersion::class.java -> {
        listOf(JavaVersion::class.java.name + ".VERSION_11")
      }
      ...
      CompileSdkVersion::class.java -> {
        listOf("null")
      } // implementation class not visible for testing
      else -> throw RuntimeException(valueType.name)
    }
  }

  private fun getAllValues(methodName: String, valueType: Class<*>, callChain: String) {
    if (methodsWithReturnValue[methodName] != null) {
      allDslValues.add(listOf("$callChain \"${methodsWithReturnValue[methodName]}\""))
      return
    }

    allDslValues.add(getPossibleValues(valueType).map { "$callChain $it" })
  }
  ...
  companion object {
    ...
    private val closuresToManuallyVisit =
      mapOf(
    ...
    private val mapTypes = setOf(Map::class.java, MapProperty::class.java, ListMultimap::class.java)

    private val listTypes = setOf(MutableCollection::class.java, ListProperty::class.java, SetProperty::class.java)

    private val typesToIgnore =
      setOf(
    ...
    private val methodsWithReturnValue =
      mapOf("public abstract void com.android.build.api.dsl.CommonExtension.setNamespace(java.lang.String)" to "com.example.foo")
