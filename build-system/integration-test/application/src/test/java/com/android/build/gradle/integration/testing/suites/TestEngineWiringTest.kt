...
class TestEngineWiringTest(
  ...
  configuration:
    (buildDefinition: GradleBuildDefinition, modulePath: String, action: AndroidProjectDefinition<out CommonExtension>.() -> Unit) -> Unit,
) {
  companion object {
      ...
      .from {
        ...
        androidApplication(":baseApp") {
          android {
            ...
            defaultConfig { applicationId = "com.example.baseApp" }
  ...
  fun testBasicModel() {
    ...
    Truth.assertThat(firstTestSuiteSources.type).isEqualTo(SourceType.HOST_JAR)
    Truth.assertThat(firstTestSuiteSources.kotlin).containsExactly(project.subProject(modulePath).resolve("src/first/kotlin").toFile())
    Truth.assertThat(firstTestSuiteSources.resources)
      .containsExactly(project.subProject(modulePath).resolve("src/first/resources").toFile())
    Truth.assertThat(firstTestSuiteSources.java).containsExactly(project.subProject(modulePath).resolve("src/first/java").toFile())

    // verify the variant specific model
    val variantTestSuites = getVariant(models.basicAndroidProject, "debug").testSuiteArtifacts
  ...
  fun testDependenciesModel() {
    ...
    val resolvedLibraries =
      models.variantDependencies
        ?.testSuiteArtifacts["first"]
        ?.sourcesDependencies
        ?.single()
        ?.artifactDependencies
        ?.compileDependencies
        ?.map { graphItem -> libraries!![graphItem.key] }
    ...
    Truth.assertThat(resolvedLibraries.filter { it!!.libraryInfo != null }.map { it!!.libraryInfo!!.name })
  ...
  private fun getVariant(basicAndroidProject: BasicAndroidProject?, variantName: String): BasicVariant {
    if (basicAndroidProject == null) throw AssertionFailedError("BasicAndroidProject is null")

    return basicAndroidProject.variants.first { variant -> variant.name == "debug" }
  }
}
