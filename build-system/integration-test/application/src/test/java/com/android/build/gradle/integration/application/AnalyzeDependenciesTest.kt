...
class AnalyzeDependenciesTest {
  ...
  private val usedClassAar =
    generateAarWithContent(
      "com.analyzedependenciesTest.usedClassAar",
      Resources.toByteArray(Resources.getResource(AnalyzeDependenciesTest::class.java, "AnalyzeDependenciesTest/used-jar.jar")),
    )

  private val usedResourceAar =
    generateAarWithContent(
      "com.analyzedependenciesTest.usedResAar",
      resources =
        mapOf(
  ...
  private val unUsedResourceAar =
    generateAarWithContent(
      "com.analyzedependenciesTest.unUsedResAar",
      resources =
        mapOf(
          "values/strings.xml" to
            // language=XML
  ...
  private val usedClassLocalLib =
    MinimalSubProject.lib("com.example.usedClasslocallib")
      .withFile(
        "src/main/java/com/example/usedclasslocallib/UsedClassLocalLib.java",
        // language=JAVA
        """
  ...
  private val unUsedLocalLib =
    MinimalSubProject.lib("com.example.unusedlocallib")
      .withFile(
        "src/main/java/com/example/usedlocallib/UnUsedLocalLib.java",
        // language=JAVA
        """
  ...
  private val mavenRepo =
    MavenRepoGenerator(
      listOf(
        MavenRepoGenerator.Library("com.analyzedependenciesTest:emptyAar:1", "aar", emptyAar),
        MavenRepoGenerator.Library("com.analyzedependenciesTest:usedClassAar:1", "aar", usedClassAar),
        MavenRepoGenerator.Library("com.analyzedependenciesTest:usedResAar:1", "aar", usedResourceAar),
        MavenRepoGenerator.Library("com.analyzedependenciesTest:unUsedResAar:1", "aar", unUsedResourceAar),
  ...
  @get:Rule val project = GradleTestProject.builder().withAdditionalMavenRepo(mavenRepo).fromTestApp(testApp).create()
  ...
  fun `Verify correct dependencies report is produced, only considering class and resource references`() {
    ...
    val parsedJson = Gson().fromJson(dependencyReportJson, DependenciesUsageReport::class.java)

    assertThat(parsedJson.remove)
      .containsExactly(
