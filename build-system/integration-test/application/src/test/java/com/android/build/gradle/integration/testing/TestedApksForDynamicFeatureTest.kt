...
class TestedApksForDynamicFeatureTest {
  ...
  fun checkAllApks() {
    ...
    val featureApkDir = FileUtils.join(rule.build.androidFeature().buildDir.pathString, "outputs", "apk", "debug")
...
class MyApkTestCallback : TestComponentCallback {
  override fun handleExtension(project: Project, androidComponents: TestAndroidComponentsExtension) {
    androidComponents.onVariants(androidComponents.selector().withBuildType("debug")) { variant ->
      project.tasks.register("${variant.name}ApkOutputCustomTask", TestedApksOutputCustomTask::class.java) {
...
abstract class TestedApksOutputCustomTask : DefaultTask() {
  ...
  @get:InputFiles abstract val apkDirectories: ConfigurableFileCollection

  @TaskAction
  fun run() {
    apkDirectories.files.forEach { println("APK: " + it.path) }
  }
}
