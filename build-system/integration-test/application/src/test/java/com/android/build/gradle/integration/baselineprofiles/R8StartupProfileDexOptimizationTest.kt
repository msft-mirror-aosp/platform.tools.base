...
class R8StartupProfileDexOptimizationTest(
  ...
  companion object {
    @Parameterized.Parameters(name = "withStartupProfileDexOptimization_{0}_withStartupProfile_{1}")
    @JvmStatic
    fun setups() = listOf(arrayOf(true, true), arrayOf(false, true), arrayOf(true, false), arrayOf(false, false))
  }
  ...
  fun testStartupProfile() {
    ...
    // 4 classes.
    assertThat(apk.mainDexFile.get().classes).hasSize(if (startupProfileDexOptimization) 1 else 4)
  }
  ...
  fun testVariantStartupProfile() {
    ...
    FileUtils.createFile(
      project.getSubproject("app").file("src/release/baselineProfiles/startup-prof.txt"),
      """
      Lcom/example/app/Foo;->foo()V
      """
        .trimIndent(),
  ...
  fun testGeneratedStartupProfile() {
    ...
    FileUtils.createFile(
      project.getSubproject("app").file("src/release/generated/baselineProfiles/startup-prof.txt"),
      """
      Lcom/example/app/Foo;->foo()V
      """
        .trimIndent(),
