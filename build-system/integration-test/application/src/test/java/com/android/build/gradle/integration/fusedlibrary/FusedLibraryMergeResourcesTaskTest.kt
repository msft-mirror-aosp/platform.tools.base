...
class FusedLibraryMergeResourcesTaskTest(private val publicationOnlyMode: Boolean) {
    ...
    GradleRule.configure()
      ...
      .from {
        ...
        androidLibrary(":androidLib3") {
          android { namespace = "com.example.androidLib3" }
          files {
            add(
              "src/main/res/values/strings.xml",
              // language=xml
  ...
  fun testMerge() {
    ...
    build.fusedLibrary(":fusedLib1").assertAar(AarSelector.NO_BUILD_TYPE) {
      androidResources {
        resourceAsText("values/values.xml")
          .isEqualTo(
            // language=xml
            """
            <?xml version="1.0" encoding="utf-8"?>
        ...
        resourceAsText("layout/layout.xml")
          .isEqualTo(
            // language=xml
            """
            <?xml version="1.0" encoding="utf-8"?>
  ...
  fun testAppResourceMergingWithFusedLib() {
    ...
    app.reconfigure { dependencies { implementation(files(aarFile)) } }

    build.executor.run(":app:assembleDebug")
    app.assertApk(ApkSelector.DEBUG) { androidResources().containsExactly("layout/layout.xml") }

    val incrementalMergedResDir =
      app.resolve(InternalArtifactType.MERGED_RES_INCREMENTAL_FOLDER).resolve("debug/mergeDebugResources/merged.dir")
    assertThat(incrementalMergedResDir.resolve("values/values.xml").readLines().map(String::trim))
      .containsExactly(
        "<?xml version=\"1.0\" encoding=\"utf-8\"?>",
        "<resources>",
        "<attr format=\"reference\" name=\"editTextStyle\"/>",
  ...
  fun testFusedLibraryResourcesAccessibleFromApp() {
    val build = rule.build { androidApplication { dependencies { implementation(project(":fusedLib1")) } } }
    ...
    build.androidApplication().assertApk(ApkSelector.DEBUG) {
      classes().apply {
        classDefinition("com/example/fusedLib1/R\$string")
          .fields()
          .containsExactly("string_from_android_lib_2", "string_from_android_lib_3", "string_overridden")
      }
    }
  }
  ...
  fun testPublishedFusedLibraryAarResourcesAccessibleFromApp() {
    val build = rule.build { androidApplication { dependencies { implementation("company:my-fused-library:1.0") } } }
    build.executor.run("publishReleasePublicationToMyrepoRepository")
    build.reconfigureSettings { addRepository("repo") }
    ...
    override fun handleProject(project: Project) {
      ...
      publishing.apply {
        publications.create("release", MavenPublication::class.java) {
          ...
          it.version = "1.0"
          it.from(project.components.getByName(FusedLibraryConstants.FUSED_LIBRARY_PUBLICATION_COMPONENT_NAME))
        }
        repositories {
          it.maven {
            it.name = "myrepo"
  ...
  companion object {
    @JvmStatic @Parameterized.Parameters fun publicationOnlyMode() = listOf(true, false)
  }
}
