...
class ArtifactsLocationsReportTest {
  ...
  val rule =
    GradleRule.from {
      gradleProperties { add(BooleanOption.DUMP_ARTIFACTS_LOCATIONS.propertyName, "true") }
  ...
  fun testDumping() {
    ...
    val artifactsLocations = gradleBuild.androidApplication().intermediatesDir.resolve("agp-debug/debug/dump-artifacts-locations.json")
    Truth.assertThat(artifactsLocations.toFile().exists()).isTrue()
    val data: Map<String, List<Map<String, String>>> =
      loadJsonFromFile(artifactsLocations.toFile()) ?: throw RuntimeException("Cannot load the artifacts locations json file")
    Truth.assertThat(data).isNotEmpty()

    // extract one record and verify that the location points to an existing file.
    val artifactData = data["single"]?.single { it["artifact"] == "COMPILE_AND_RUNTIME_R_CLASS_JAR" }
