...
class CompileRClassTest2 {
  ...
  fun checkBuilds(nonTransitiveRClass: Boolean) {
    ...
    Truth.assertThat(build.didWorkTasks)
    ...
    val compileR = app.getIntermediateFile("compile_r_class_jar", "debug", "generateDebugRFile", "R.jar")
    ...
    if (nonTransitiveRClass) {
      ...
      assertThat(compileRTxt)
        .containsAllOf("int string app_string 0x0", "int string lib_string 0x0", "int string abc_action_bar_home_description 0x0")
    }
    ...
    if (nonTransitiveRClass) {
      ...
      Truth.assertThat(runtimeRStrings.printFields()).contains("public static I app_string")
      Truth.assertThat(runtimeRStrings.printFields()).doesNotContain("public static I lib_string")
      Truth.assertThat(runtimeRStrings.printFields()).doesNotContain("public static I abc_action_bar_home_description")
    } else {
      Truth.assertThat(runtimeRStrings.printFields())
        .containsAtLeastElementsIn(
          listOf(
            ...
            "public static I abc_action_bar_home_description",
          )
        ) // string from transitive dep
    }
  ...
  private val modifierToString = mapOf(Opcodes.ACC_PUBLIC to "public", Opcodes.ACC_STATIC to "static", Opcodes.ACC_FINAL to "final")

  private fun modifiers(accessFlags: Int): String {
    val modifiers = ArrayList<String>()
    var runningFlags = accessFlags
    modifierToString.forEach { value, string ->
  ...
  private fun DexBackedClassDef.printFields(): List<String> = this.fields.map { modifiers(it.accessFlags) + " " + it.type + " " + it.name }

  private fun DexBackedClassDef.getValues(): List<Int> = this.fields.map { (it.initialValue!! as IntEncodedValue).value }
}
