...
class JavaResPackagingLibraryTest {
  ...
  fun testLibProjectWithNewResFile() {
    ...
    TemporaryProjectModification.doTest(libProject) { project ->
      ...
      execute("library:assembleDebug")
      libProject.checkAar("newlibrary.txt".withContent("newfile content"), "library.txt", "localjar.txt")
    }
  }
  ...
  fun testLibProjectWithModifiedResFile() {
    ...
    TemporaryProjectModification.doTest(libProject) { project ->
      ...
      execute("library:assembleDebug")
      libProject.checkAar("library.txt".withContent("new content"), "localjar.txt")
    }
  }
  ...
  fun testLibProjectWithNewResFileInDebugSourceSet() {
    ...
    execute("library:assembleDebug")
    libProject.checkAar("library.txt".withContent("library:abcd"), "localjar.txt")
  }
  ...
  fun testLibProjectTestWithModifiedResFile() {
    ...
    TemporaryProjectModification.doTest(libProject) { project ->
      ...
      execute("library:assembleAT")
      libProject.checkTestApk("librarytest.txt".withContent("new content"), "library.txt", "library2.txt", "localjar.txt")
    }
  }
  ...
  fun testLibProjectTestWithNewResFileOverridingTestedLib() {
    ...
    TemporaryProjectModification.doTest(libProject) { project ->
      ...
      libProject.checkTestApk("library.txt".withContent("new content"), "librarytest.txt", "library2.txt", "localjar.txt")
    }

    // file's been removed, checking in the other direction.
    execute("library:assembleAT")
    libProject.checkTestApk("library.txt".withContent("library:abcd"), "librarytest.txt", "library2.txt", "localjar.txt")
  }
  ...
  fun testLibProjectTestWithNewResFileOverridingDependency() {
    ...
    TemporaryProjectModification.doTest(libProject) { project ->
      ...
      val result = execute("library:assembleAT")
      result.stdout.use { stdout ->
        ScannerSubject.assertThat(stdout).contains("More than one file was found with OS independent path 'com/foo/library2.txt'.")
      }
      libProject.checkTestApk("library2.txt".withContent("new content"), "librarytest.txt", "library.txt", "localjar.txt")
    }

    // file's been removed, checking in the other direction.
    execute("library:assembleAT")
    libProject.checkTestApk("library2.txt".withContent("library2:abcd"), "librarytest.txt", "library.txt", "localjar.txt")
  }
