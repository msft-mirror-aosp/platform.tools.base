...
class ExtractVersionControlInfoTest {
  ...
  fun gitHeadContainsRef() {
    ...
    val apkFile = project.getSubproject("app").getApk(GradleTestProject.ApkType.DEBUG).file.toFile()
  ...
  fun gitNotInitialized() {
    ...
    val gitMissingMessage =
      "When VCS tagging is enabled (which is by default in " +
    ...
    result = project.executor().withLoggingLevel(LoggingLevel.DEBUG).run(":app:assembleRelease", ":app:makeApkFromBundleForRelease")
    ...
    val apkFile = project.getSubproject("app").getApk(GradleTestProject.ApkType.RELEASE).file.toFile()
  ...
  fun testDefaultBehavior() {
    TestFileUtils.searchAndReplace(project.getSubproject("app").buildFile, includeVcsDsl, "")

    val sha = "40cde1bb54e7895b717a55931e4421cbab41234e"
    FileUtils.createFile(project.file(".git/HEAD"), sha)

    // When DSL is not specified, debug build should not enable feature
    var result = project.executor().run(":app:assembleDebug", ":app:makeApkFromBundleForDebug")
    Truth.assertThat(result.didWorkTasks).doesNotContain(":app:extractDebugVersionControlInfo")

    result = project.executor().run(":app:assembleRelease", ":app:makeApkFromBundleForRelease")
    Truth.assertThat(result.didWorkTasks).contains(":app:extractReleaseVersionControlInfo")
    val apkFile = project.getSubproject("app").getApk(GradleTestProject.ApkType.RELEASE).file.toFile()
    validateApkFileContents(apkFile, sha)
  ...
  fun testFalseBehavior() {
    ...
    val result = project.executor().run(":app:assembleRelease", ":app:makeApkFromBundleForRelease")
    Truth.assertThat(result.didWorkTasks).doesNotContain(":app:extractReleaseVersionControlInfo")
  }
  ...
  fun testKotlinDsl() {
    ...
    val result = project.executor().run(":app:assembleDebug", ":app:makeApkFromBundleForDebug")
  ...
  fun validateOldFlagIsDeprecated() {
    val result = project.executor().with(BooleanOption.ENABLE_VCS_INFO, true).expectFailure().run(":app:assembleDebug")

    ScannerSubject.assertThat(result.stderr).contains("The option 'android.enableVcsInfo' is deprecated.")
  }

  private fun validateBundleApkFileContents(sha: String, variant: String = "debug") {
    val vcInfo = getBundleApkFileContents(variant)

    Truth.assertThat(vcInfo.decodeToString())
      .isEqualTo(
        """
                repositories {
                  system: GIT
  ...
  private fun getBundleApkFileContents(variant: String = "debug"): ByteArray {
    val bundleTaskName = if (variant == "debug") "makeApkFromBundleForDebug" else "makeApkFromBundleForRelease"
    val bundleApksFile = project.getSubproject("app").getIntermediateFile("apks_from_bundle", variant, bundleTaskName, "bundle.apks")
