...
class CoreLibraryDesugarConversionTest(val minSdkVersion: Int) {
      ...
      dex.classes
        ...
        .find {
          it.implementation!!
            ...
            .any {
              it.definingClass.equals(definingClass) &&
                it.name.equals(name) &&
  ...
  private fun getMethodReference(reference: MethodReference): String {
    return getMethodReference(reference.definingClass, reference.name, reference.returnType, reference.parameterTypes)
  }

  private fun getMethodReference(definingClass: String, name: String, returnType: String, parameterTypes: List<out CharSequence>): String {
  ...
  private fun getInvokedMethod(instruction: Instruction): MethodReference? {
    ...
    val isInvoke =
      opcode == INVOKE_VIRTUAL || opcode == INVOKE_SUPER || opcode == INVOKE_DIRECT || opcode == INVOKE_STATIC || opcode == INVOKE_INTERFACE
    val isInvokeRange =
