...
class DslTest {
  @get:Rule val rule = GradleRule.configure().disableBrokenNewDslOptOutChecks().from { androidApplication {} }
  ...
  fun versionNameSuffix() {
    rule.build {
      androidApplication {
        android {
          ...
          buildTypes { named("debug") { it.versionNameSuffix = "-suffix" } }
        }
      }
    }
    ...
    val path =
      rule.build.androidApplication().buildDir.resolve("intermediates/merged_manifests/debug/processDebugManifest/AndroidManifest.xml")
  ...
  @Test
  fun extraPropTest() {
    rule.build { gradleProperties { add(BooleanOption.USE_NEW_DSL, false) } }
  ...
  fun buildConfigEncoding() {
    rule.build {
      androidApplication {
        android {
          ...
          defaultConfig { buildConfigField("String", "test2", "\\u0105") }
        }
      }
    }
    ...
    val path = rule.build.androidApplication(":app").buildDir.resolve("generated/source/buildConfig/debug/pkg/name/app/BuildConfig.java")
    assertAbout(PathSubject.paths()).that(path).exists()
    val expected =
      """
      /**
  ...
  fun validateVersionCodeDefaultConfig() {
    rule.build { androidApplication { android { defaultConfig { versionCode = 0 } } } }
  ...
  fun projectConfigurationWithEmptyFlavorDimension() {
    rule.build { androidApplication { android { flavorDimensions += "foo" } } }
