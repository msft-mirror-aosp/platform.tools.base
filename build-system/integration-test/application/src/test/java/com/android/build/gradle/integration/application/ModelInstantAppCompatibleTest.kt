...
class ModelInstantAppCompatibleTest {
  ...
  @get:Rule val flavorProject = GradleTestProject.builder().fromTestProject("basicMultiFlavors").create()

  @get:Rule val buildTypeProject = GradleTestProject.builder().fromTestProject("unitTestingBuildTypes").create()
  ...
  fun testBuildModelForInstantAppWithDynamicFeatures() {
    ...
    val feature1 = project.modelV2().ignoreSyncIssues().fetchModels().container.getProject(":feature1").androidProject
    val feature2 = project.modelV2().ignoreSyncIssues().fetchModels().container.getProject(":feature2").androidProject
    val app = project.modelV2().ignoreSyncIssues().fetchModels().container.getProject(":feature2").androidProject
  ...
  fun testBuildModelForInstantAppWithManifestWithInvalidCharacters() {
    ...
    Truth.assertThat(issues.single().message)
  ...
  fun testBuildModelForInstantAppWithDynamicFeaturesWithNonstandardNamespace() {
    ...
    TestFileUtils.searchAndReplace(File(project.getSubproject(":feature1").mainSrcDir.parent, "/AndroidManifest.xml"), "dist:", "apkd:")
    TestFileUtils.searchAndReplace(
      File(project.getSubproject(":feature2").mainSrcDir.parent, "/AndroidManifest.xml"),
      "xmlns:dist",
      "xmlns:apkd",
    )
    TestFileUtils.searchAndReplace(File(project.getSubproject(":feature2").mainSrcDir.parent, "/AndroidManifest.xml"), "dist:", "apkd:")
    val models = project.modelV2().ignoreSyncIssues().fetchModels().container.rootInfoMap
    for ((modelName, model) in models) {
      for (variant in model.androidProject?.variants!!) {
        if (modelName == ":feature1") {
