...
class FusedLibraryTest {
    ...
    GradleRule.configure()
      ...
      .from {
        ...
        fusedLibrary(":fusedLib1") {
          androidFusedLibrary { minSdk { version = release(DEFAULT_MIN_SDK_VERSION) } }
  ...
  fun checkAarNoPublishing() {
    ...
    build.fusedLibrary(":fusedLib1").assertAar(AarSelector.NO_BUILD_TYPE) { exists() }
  }
  ...
  fun checkAarPublishing() {
    ...
    fusedLibrary.buildDir.resolve("publications/maven").also { publicationDir ->
    ...
    fusedLibrary.buildDir.resolve(FUSED_LIBRARY_REPO_NAME).also { repoPath ->
      val publishedLibRepoDir = repoPath.resolve("$FUSED_LIBRARY_GROUP/${FUSED_LIBRARY_ARTIFACT_NAME}/$FUSED_LIBRARY_VERSION")
      assertThat(publishedLibRepoDir.resolve("$FUSED_LIBRARY_ARTIFACT_NAME-${FUSED_LIBRARY_VERSION}.aar").isRegularFile()).isTrue()

      assertExpectedPomDependencies(
        publishedLibRepoDir.resolve("$FUSED_LIBRARY_ARTIFACT_NAME-${FUSED_LIBRARY_VERSION}.pom"),
        listOf(
          "junit:junit:4.12 scope:runtime",
          "org.hamcrest:hamcrest-core:1.3 scope:runtime",
          "fusedlib:androidLib3:1.0.0 scope:runtime",
      ...
      val publishedSourcesJar = publishedLibRepoDir.resolve("$FUSED_LIBRARY_ARTIFACT_NAME-${FUSED_LIBRARY_VERSION}-sources.jar")
      JarSubject.assertThat(publishedSourcesJar) {
        resources()
          .containsExactly("com/example/androidLib1/ClassFromAndroidLib1.java", "com/example/androidLib2/ClassFromAndroidLib2.java")
        classes().isEmpty()
      }
    }
  }
  ...
  fun checkContentsOfEmptyFusedLibrary() {
    ...
    ZipFile(buildDir.toFile()).use {
      assertThat(it.entries().asSequence().map(ZipEntry::getName).toList())
        .containsExactly(
          "META-INF/",
