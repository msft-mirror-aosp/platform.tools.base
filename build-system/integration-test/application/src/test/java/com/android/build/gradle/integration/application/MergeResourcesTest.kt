...
class MergeResourcesTest {
    ...
    fun executorEnforceUniquePkg() = project.executor()
    ...
    fun mergesRawWithLibraryWithOverride() {
        ...
        executorEnforceUniquePkg().run(":app:assembleDebug")
    ...
    fun addResourceBetweenBuildsWithProductFlavor() {
        ...
        FileUtils.createFile(FileUtils.join(flavor1, "values", "strings.xml"),
            "<resources>\n" +
                    ...
                    "</resources>")
        executorEnforceUniquePkg().run("clean", ":app:assembleDebug")
    ...
    fun injectedMinSdk() {
        ...
        executorEnforceUniquePkg().with(IntegerOption.IDE_TARGET_DEVICE_API, 23).run(":app:assembleDebug")
    }
    ...
    fun testIncrementalBuildWithShrinkResources() {
        ...
        var result = executorEnforceUniquePkg().run(":app:clean", ":app:assembleDebug")
        ...
        result = executorEnforceUniquePkg().run(":app:assembleDebug")
        assertThat(result.getTask(":app:mergeDebugResources")).didWork()
    ...
    fun testIncrementalResourceChangeOfMergedNotCompiledResource() {
        ...
        FileUtils.createFile(noCompileLayout,
                "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n" +
                        "<merge/>")
        executorEnforceUniquePkg().run(":app:mergeReleaseResources")
