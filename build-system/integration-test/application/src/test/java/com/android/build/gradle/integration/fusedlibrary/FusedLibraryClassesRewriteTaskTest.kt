...
internal class FusedLibraryClassesRewriteTaskTest {
  ...
  fun rewritesUnderFusedRClass() {
    ...
    val rewrittenClasses = fusedLibrary.resolve(CLASSES_WITH_REWRITTEN_R_CLASS_REFS).resolve("single/rewriteClasses")
    val fusedLibraryRjar = fusedLibrary.resolve(FUSED_R_CLASS).resolve("single/rewriteClasses/R.jar")
    val dependencyLib3RJar = dependencyLib3.resolve(InternalArtifactType.COMPILE_R_CLASS_JAR).resolve("release/generateReleaseRFile/R.jar")

    URLClassLoader(arrayOf(rewrittenClasses.toUri().toURL(), fusedLibraryRjar.toUri().toURL(), dependencyLib3RJar.toUri().toURL()), null)
      .use { classLoader ->
        // Check fused library R class generated and contains fields.
        val fusedLibraryRStringsClass = classLoader.loadClass("com.example.fusedLib1.R\$string")
        val fusedLibraryRClassStringFieldNames =
          (fusedLibraryRStringsClass.declaredFields)
            .map { it.name }
        ...
        val dependencyLib3RStringsClass = classLoader.loadClass("com.example.dependencyLib3.R\$string")
        val dependencyLib3RClassStringFieldNames =
          (dependencyLib3RStringsClass.declaredFields)
            .map { it.name }
            // Ignore Jacoco instrumentation injected by studio-coverage.
            .minus("\$jacocoData")
