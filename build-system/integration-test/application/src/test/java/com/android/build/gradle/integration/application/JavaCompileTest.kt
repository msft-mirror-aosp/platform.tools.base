...
class JavaCompileTest {
  ...
  fun `check DSL arguments are added to JavaCompile options when using Room`() {
    ...
    project.getSubproject((":app")).let {
      TestFileUtils.appendToFile(
        it.mainSrcDir.resolve("com/example/User.java"),
        """
      ...
      TestFileUtils.appendToFile(
        it.mainSrcDir.resolve("com/example/AppDatabase.java"),
        """
  ...
  private fun buildAndCheckPresenceOfMethodParamsInByteCode(presenceExpected: Boolean) {
    ...
    val fooClassFile =
      JAVAC.getOutputDir(project.getSubproject("app").buildDir).resolve("debug/compileDebugJavaWithJavac/classes/com/example/Foo.class")
    val fooClass = ClassNode(ASM_API_VERSION).also { ClassReader(fooClassFile.readBytes()).accept(it, 0) }
