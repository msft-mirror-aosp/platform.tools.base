...
/** Tests to verify the classes that are packaged within the AAR are correct or cause an expected build time failure when invalid. */
...
class FusedLibraryClassesVerificationTest(private val publicationOnlyMode: Boolean) {
  ...
  val rule =
    GradleRule.configure()
      .withMavenRepository {
        ...
        jar("com.externaldep:depwithdep:1.0").withDependencies("com.externaldep.externalaar:externalaar:1.0")

        jar("this.dependency:has-a-dependency-that-does-not-exist:1.0").withDependencies("this.dependency:doesnotexist:1.0")
      ...
      .from {
        ...
        */
        androidLibrary(":androidLib2") {
          ...
          dependencies { implementation(project(":androidLib1")) }
        }
        /*
               androidLib1
                    ▲
        ...
        */
        androidLibrary(":$ANDROID_LIB_MANY_TRANSITIVE_DEPS") {
          ...
          dependencies { implementation(project(":androidLib2")) }
        }
        /*
        ...
        */
        androidLibrary(":$ANDROID_LIB_WITH_EXTERNAL_LIB_DEPENDENCY") {
          ...
          dependencies { implementation("com.externaldep.externalaar:externalaar:1.0") }
        }
        /*
               com.externaldep:externalaar:1
                            ▲
        ...
        */
        androidLibrary(":$ANDROID_LIB_WITH_EXTERNAL_LIB_WITH_CIRCULAR_DEP") {
          ...
          dependencies { implementation("com.externaldep:depwithdep:1.0") }
        }
        ...
        */
        androidApplication {
          ...
          dependencies { implementation(project(":$FUSED_LIBRARY_PROJECT_NAME")) }
        }
        gradleProperties { add(BooleanOption.FUSED_LIBRARY_PUBLICATION_ONLY_MODE, publicationOnlyMode) }
      }
  ...
  fun checkNotIncludedProjectDependenciesAddedAsDependencies() {
    val build = rule.build { fusedLibrary(":$FUSED_LIBRARY_PROJECT_NAME") { dependencies { include(project(":androidLib2")) } } }
    ...
    build.checkFusedLibReportContents(
      ...
      dependencies =
        listOf("org.jetbrains.kotlin:kotlin-stdlib:<version>", "org.jetbrains:annotations:<version>", "project.:androidLib1:unspecified"),
    )
  }
  ...
  fun checkFusedLibraryAarForClassesFromLocalJarDependencies() {
    ...
    fusedLibrary.assertAar(AarSelector.NO_BUILD_TYPE) { hasSecondaryJars("testClass.jar") }

    fusedLibrary.files.add(
      "src/main/java/com/example/myapp/AppClass.kt",
      // language=kotlin
      """
    ...
    build.checkFusedLibReportContents(include = listOf("testClass.jar"), dependencies = listOf())
  }
  ...
  fun `validationFailsForDependencyIncludedButParentNotIncluded-ExternalDependency`() {
    ...
    build.fusedLibrary(":$FUSED_LIBRARY_PROJECT_NAME").reconfigure {
      androidFusedLibrary { experimentalProperties[ModulePropertyKey.BooleanWithDefault.FUSED_LIBRARY_VALIDATE_DEPENDENCIES.key] = false }
    }
    build.executor.run(":$FUSED_LIBRARY_PROJECT_NAME:assemble")
  }
  ...
  fun checkPlatformBomDependenciesInLibraryDependencies() {
    ...
    assertExpectedPomDependencies(
      build.fusedLibrary(":$FUSED_LIBRARY_PROJECT_NAME").buildDir.resolve("publications/maven/pom-default.xml"),
      listOf(
  ...
  fun checkExternalLibraryBomDependenciesInLibraryDependencies() {
    ...
    build.checkFusedLibReportContents(
      include = listOf("project :lib"),
      dependencies = listOf("bom:external-lib-bom:1.0", "com.externaldep:depwithdep:1.0"),
    )
    ...
    assertExpectedPomDependencies(
      build.fusedLibrary(":$FUSED_LIBRARY_PROJECT_NAME").buildDir.resolve("publications/maven/pom-default.xml"),
      listOf("bom:external-lib-bom:1.0 scope:runtime", "com.externaldep:depwithdep:1.0"),
    )
  }
  ...
  private fun GradleBuild.checkFusedLibReportContents(include: List<String>, dependencies: List<String>) {
    executor.run(":$FUSED_LIBRARY_PROJECT_NAME:report")

    val reportFile =
      fusedLibrary(":$FUSED_LIBRARY_PROJECT_NAME")
        .buildDir
        .resolve("reports/${FusedLibraryInternalArtifactType.FUSED_LIBRARY_REPORT.getFolderName()}/single/report.json")
    ...
    val idWithoutVersionPlaceholder = { str: String -> str.substringBeforeLast(":<version>") }
  ...
  private fun GradleBuild.assertFusedLibAarContainsExpectedClasses(classesFromDirectDependencies: List<String>) {
    ...
    fusedLib1Project.assertAar(AarSelector.NO_BUILD_TYPE) { mainJar().classes { containsExactly(classesFromDirectDependencies) } }
  }
  ...
  private fun AarBuilder.generateExternalAarContent() {
    ...
    withMainJar { addEmptyClasses("com/externaldep/externaljar/ExternalClass") }
  }
  ...
  companion object {
    ...
    @JvmStatic @Parameterized.Parameters(name = "{0}") fun publicationOnlyMode() = listOf(true, false)
  }
}
