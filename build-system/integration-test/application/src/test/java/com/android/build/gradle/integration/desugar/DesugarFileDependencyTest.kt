...
class DesugarFileDependencyTest(var tool: Tool) {
  ...
  companion object {
    @JvmStatic @Parameterized.Parameters(name = "{0}") fun getParams(): Array<Tool> = Tool.values()
  }

  private val mainProject =
  ...
  @get:Rule val project = GradleTestProject.builder().fromTestApp(mainProject).create()
  ...
  fun checkMinApi21WithInjectedDevice() {
    ...
    project.getApk(GradleTestProject.ApkType.DEBUG, GradleTestProject.ApkLocation.Intermediates).use { apk ->
      assertThat(apk)
        .hasClass("Lcom/android/build/gradle/integration/desugar/resources/ImplOfInterfaceWithDefaultMethod;")
        .that()
        .doesNotHaveMethod("myDefaultMethod")
  ...
  private fun addJars() {
    ...
    TestInputsGenerator.pathWithClasses(interfaceLib, listOf(InterfaceWithDefaultMethod::class.java))
    TestInputsGenerator.pathWithClasses(implLib, listOf(ImplOfInterfaceWithDefaultMethod::class.java))
  }
