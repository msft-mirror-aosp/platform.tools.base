...
class KotlinMultiplatformAndroidLintTest(private val lintAnalysisPerComponent: Boolean) {
  ...
  fun `test lint reports error when calling desugared apis in androidMain sourceset`() {
    ...
    val reportFile = File(project.getSubproject("kmpFirstLib").buildDir, "reports/lint-results-androidMain.txt")
  ...
  fun `test running lint on kmpJvmOnly with java`() {
    ...
    val javaFile = FileUtils.join(project.getSubproject("kmpJvmOnly").projectDir, "src", "jvmMain", "java", "com", "example", "Foo.java")
  ...
  fun `test running lint on project with jvm and android targets`() {
    ...
    PathSubject.assertThat(jvmReportFile).contains("Found byte-order-mark in the middle of a file [ByteOrderMark]")

    val androidrReportFile = File(project.getSubproject("kmpFirstLib").buildDir, "reports/lint-results-androidMain.txt")
    PathSubject.assertThat(androidrReportFile).exists()
    PathSubject.assertThat(androidrReportFile)
      .containsAllOf(
        "Error: Call requires API level 26, or core library desugaring (current min is 22): java.time.LocalDate#getMonth [NewApi]",
        "Error: Call requires API level 26, or core library desugaring (current min is 22): java.time.LocalDate#now [NewApi]",
      )
  ...
  fun `test updating lint baselines on project with jvm and android targets`() {
    ...
    val jvmBaselineFile = File(project.getSubproject("kmpFirstLib").projectDir, "jvm-lint-baseline.xml")
    val androidBaselineFile = File(project.getSubproject("kmpFirstLib").projectDir, "android-lint-baseline.xml")
  ...
  fun `test running lint on app with KMP dependency with jvm and android targets`() {
    ...
    PathSubject.assertThat(reportFile).doesNotContain("Found byte-order-mark in the middle of a file [ByteOrderMark]")
  }
  ...
  fun `test no lint tasks if lint plugin not applied`() {
    ...
    TestFileUtils.searchAndReplace(project.getSubproject("kmpFirstLib").ktsBuildFile, "id(\"com.android.lint\")", "")
  ...
  fun `test lint on KMP project with android target but no jvm target`() {
    ...
    val reportFile = File(project.getSubproject("kmpFirstLib").buildDir, "reports/lint-results.txt")
  ...
  fun `test lint on KMP project with no android or jvm targets`() {
    Assume.assumeTrue(lintAnalysisPerComponent)
    TestFileUtils.searchAndReplace(project.getSubproject("kmpJvmOnly").ktsBuildFile, "jvm()", "js()")
