...
class TestFixturesProjectLintModelIntegrationTest(private val lintAnalysisPerComponent: Boolean) {
  ...
  private fun setUpProject(publishJavaLib: Boolean, publishAndroidLib: Boolean) {
    ...
    if (publishJavaLib) {
      project.executor().run(":javaLib:publish")
    }

    if (publishAndroidLib) {
      project.executor().run(":lib:publish")
    }
  }
  ...
  fun `check lint model for project with local and library module testFixtures`() {
    ...
    if (lintAnalysisPerComponent) {
      checkLintModels(
        project = project,
        lintModelDir = project.getSubproject("app").intermediatesDir.toPath().resolve("incremental/lintAnalyzeReleaseTestFixtures"),
      ...
      checkLintModels(
        project = project,
        lintModelDir =
          project.getSubproject("app").intermediatesDir.toPath().resolve("lint_report_lint_model/release/generateReleaseLintReportModel"),
