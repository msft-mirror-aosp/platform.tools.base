...
public class LintBaselineTest {
    ...
    public void checkBaselineRecreation() throws Exception {
        ...
        if (lintBaselinesContinue) {
            ...
            result.assertTask(":app:lintDebug").failed();
        }
        ...
        result2.assertTask(":app:lintReportDebug").didWork();
        if (lintBaselinesContinue) {
            result2.assertTask(":app:lintDebug").didWork();
        } else {
            result2.assertTask(":app:lintDebug").failed();
        }
    ...
    public void checkMerging() throws Exception {
        ...
        assertThat(baseline)
                .contains(
                        "    <issue\n"
                            + "        id=\"UselessLeaf\"\n"
                            + "        message=\"This `LinearLayout` view is unnecessary (no"
                            + " children, no `background`, no `id`, no `style`)\"\n"
        ...
        assertThat(lintResults)
                .doesNotContain(
                        "errors/warnings were listed in the baseline file (lint-baseline.xml) but"
        ...
        upToDateRunResult.assertTask(":app:lintReportDebug").wasUpToDate();
        upToDateRunResult.assertTask(":app:lintAnalyzeDebug").wasUpToDate();
