...
class UpdateLintBaselineTest {
  ...
  @get:Rule val project = builder().fromTestProject("lintBaseline").disableBuiltInKotlin().create()

  @get:Rule
  val projectWithoutIssues =
    builder().fromTestApp(MinimalSubProject.app("com.example.app")).withName("projectWithoutIssues").disableBuiltInKotlin().create()
  ...
  fun checkUpdateLintBaseline() {
    ...
    TestFileUtils.searchAndReplace(project.getSubproject("app").buildFile, "baseline = file('lint-baseline.xml')", "")
    project.executor().run(":app:updateLintBaseline").apply {
      assertTask(":app:lintAnalyzeDebug").didWork()
      assertTask(":app:updateLintBaselineDebug").didWork()
  ...
  fun testMissingBaselineIsEmptyBaseline() {
    ...
    // deleted.
    projectWithoutIssues.executor().with(BooleanOption.MISSING_LINT_BASELINE_IS_EMPTY_BASELINE, true).run("updateLintBaseline")
    PathSubject.assertThat(baselineFile).doesNotExist()
    ...
    projectWithoutIssues.executor().with(BooleanOption.MISSING_LINT_BASELINE_IS_EMPTY_BASELINE, true).run("lint").apply {
    ...
    val result =
      projectWithoutIssues.executor().with(BooleanOption.MISSING_LINT_BASELINE_IS_EMPTY_BASELINE, true).expectFailure().run("lint")
  ...
  fun testOmitLineNumbers() {
    ...
    project.executor().with(BooleanOption.LINT_BASELINE_OMIT_LINE_NUMBERS, true).run(":app:updateLintBaseline")
    PathSubject.assertThat(baselineFile).exists()
