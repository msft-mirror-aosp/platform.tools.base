...
class LintTextOutputTest(private val lintReportAggregation: Boolean) {
  ...
  fun testStdoutAndStderr() {
    ...
    executor.run(lintTaskNames).apply {
  ...
  fun testNoIssuesFound() {
    ...
    executor.expectFailure().run(lintTaskNames)
