  ...
  private fun GradleBuildDefinition.addLanguageVersionsToProject(
    ...
    sourceSetsLanguageVersion?.let { version ->
      fun KotlinExtension.configureKotlinSourceSets() {
        sourceSets.all { it.languageSettings { languageVersion = version } }
      }

      val kotlinSubProjectPaths = listOf(":app", ":lib", ":feature", ":kotlin-lib", ":kmp-android-lib", ":kmp-jvm-lib")
        ...
        configure(subprojectPath) {
          when (this) {
            ...
            is GenericProjectDefinition -> {
              reconfigurePlugin(PluginType.KOTLIN_JVM) { configureKotlinSourceSets() }
            }
      ...
      if (useBuiltInKotlinSupport) {
        gradleProperties { add(SourceSetsLanguageVersion.KEY, version) }
      }
    }
    ...
    fun applyLintCheckCallback(
      ...
      configure(path) { pluginCallbacks += callbackClass }
      gradleProperties {
        add(useK2UastKey, useK2UastExpected.toString())
        languageVersionExpected?.let { add(languageVersionKey, it) }
      }
    }
...
class SourceSetsLanguageVersion : GenericCallback {
  ...
  override fun handleProject(project: Project) {
    val version =
      project.providers.gradleProperty(KEY).orNull ?: throw RuntimeException("Missing key for SourceSetsLanguageVersion callback")
...
/**
 * Generic call back to validate projects. This is not to be used directly. Instead, each project extends this to provide the right task
 * name, and key values
 */
abstract class LintAnalyzeCheck : GenericCallback {
  ...
  private fun Project.expectedUseK2Uast(): Boolean =
    providers.gradleProperty(useK2UastKey).orNull?.toBoolean() ?: throw RuntimeException("Missing value for $useK2UastKey")
