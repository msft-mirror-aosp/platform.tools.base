...
/** This test checks that the [AndroidLintAnalysisTask]'s outputs are not affected by properties which aren't annotated as inputs. */
class AndroidLintAnalysisTaskCacheabilityTest {

  @get:Rule val project1: GradleTestProject = createGradleTestProject("project1")

  @get:Rule val project2: GradleTestProject = createGradleTestProject("project2")

  @get:Rule val tmp = TemporaryFolder()

  private val extraSourceDirMap by lazy {
    mapOf(":app" to tmp.newFolder(), ":feature" to tmp.newFolder(), ":lib" to tmp.newFolder(), ":java-lib" to tmp.newFolder())
  }
  ...
  /**
   * Add source directories that are outside the project directories. Add debug source directories to the Android modules, and add an extra
   * java source directory to the java library. Add an extra test java source directory to all the modules.
   */
  @Before
  fun before() {
    listOf(":app", ":feature", ":lib", ":java-lib").forEach { moduleName ->
      val extraSourceDir = extraSourceDirMap[moduleName]!!
      FileUtils.copyDirectory(FileUtils.join(project1.getSubproject(moduleName).projectDir, "src", "main"), extraSourceDir)
      ...
      val extraTestJavaFile = FileUtils.join(extraTestSourceDir, "java", "com", "example", "Test.java")
      ...
      listOf(project1, project2).forEach { project ->
        val extraSourceDirPath = FileUtils.toSystemIndependentPath(extraSourceDir.absolutePath)
        val extraTestSourceDirPath = FileUtils.toSystemIndependentPath(extraTestSourceDir.absolutePath)
        if (moduleName == ":java-lib") {
  ...
  fun testDifferentProjectsProduceSameAnalysisOutputs() {
    ...
    listOf(":app", ":feature", ":lib", ":java-lib").forEach { moduleName ->
      listOf(lintReport1, lintReport2).forEach { lintReport ->
        assertThat(lintReport).contains("${extraSourceDirMap[moduleName]!!.name}${slash}java${slash}com${slash}example${slash}Extra.java")
        assertThat(lintReport)
          .contains("${extraTestSourceDirMap[moduleName]!!.name}${slash}java${slash}com${slash}example${slash}Test.java")
        if (moduleName != ":java-lib") {
          assertThat(lintReport).contains("${extraSourceDirMap[moduleName]!!.name}${slash}res${slash}values${slash}strings.xml")
          assertThat(lintReport).contains("${extraSourceDirMap[moduleName]!!.name}${slash}AndroidManifest.xml")
        }
      }
    }
    ...
    val moduleNameToArtifactTypeMap: Map<String, List<InternalArtifactType<Directory>>> =
      mapOf(
        ...
        ":feature" to listOf(ANDROID_TEST_LINT_PARTIAL_RESULTS, LINT_PARTIAL_RESULTS, UNIT_TEST_LINT_PARTIAL_RESULTS),
        ":lib" to
          listOf(
            ANDROID_TEST_LINT_PARTIAL_RESULTS,
            LINT_PARTIAL_RESULTS,
        ...
        ":java-lib" to listOf(LINT_PARTIAL_RESULTS, UNIT_TEST_LINT_PARTIAL_RESULTS),
      )
    moduleNameToArtifactTypeMap.forEach { moduleName, artifactTypes ->
      artifactTypes.forEach { artifactType ->
        ...
        val partialResultsDir1 =
          if (moduleName == ":java-lib") {
            FileUtils.join(
              project1.getSubproject(moduleName).getIntermediateFile(artifactType.getFolderName()),
              "global",
            ...
            FileUtils.join(project1.getSubproject(moduleName).getIntermediateFile(artifactType.getFolderName()), "debug", taskFolder, "out")
          }
        val partialResultsDir2 =
          if (moduleName == ":java-lib") {
            FileUtils.join(
              project2.getSubproject(moduleName).getIntermediateFile(artifactType.getFolderName()),
              "global",
              if (artifactType == LINT_PARTIAL_RESULTS) {
                "lintAnalyzeJvmMain"
              } else {
            ...
            FileUtils.join(project2.getSubproject(moduleName).getIntermediateFile(artifactType.getFolderName()), "debug", taskFolder, "out")
          }
        ...
        listOf(partialResultsDir1, partialResultsDir2).forEach {
          val expectedFileList =
            if (moduleName != ":java-lib" && artifactType == LINT_PARTIAL_RESULTS) {
              ...
              listOf(File(it, lintDefiniteFileName), File(it, lintPartialFileName), File(it, lintThreadDetectorDirName))
            }
          assertThat(it.listFiles()?.asList()).containsExactlyElementsIn(expectedFileList)
        }
        ...
        if (moduleName == ":java-lib") {
          if (artifactType == LINT_PARTIAL_RESULTS) {
            assertThat(File(partialResultsDir1, lintDefiniteFileName).readText()).contains("{$moduleName*projectDir}")
            assertThat(File(partialResultsDir1, lintDefiniteFileName).readText())
          ...
          when (artifactType) {
            ...
            LINT_PARTIAL_RESULTS -> {
              assertThat(File(partialResultsDir1, lintDefiniteFileName).readText()).contains("{$moduleName*projectDir}")
              assertThat(File(partialResultsDir1, lintDefiniteFileName).readText()).contains("{$moduleName*buildDir}")
              assertThat(File(partialResultsDir1, lintDefiniteFileName).readText())
        ...
        assertThat(File(partialResultsDir1, lintDefiniteFileName).readText()).doesNotContain(extraSourceDirMap[moduleName]!!.name)
        assertThat(File(partialResultsDir1, lintDefiniteFileName).readText()).doesNotContain(extraTestSourceDirMap[moduleName]!!.name)
        assertThat(File(partialResultsDir1, lintPartialFileName).readText()).doesNotContain(extraSourceDirMap[moduleName]!!.name)
        assertThat(File(partialResultsDir1, lintPartialFileName).readText()).doesNotContain(extraTestSourceDirMap[moduleName]!!.name)
      }
    }
  }

  /**
   * Lint's encoding and decoding of source set paths depends on all the lint tasks having the correct source set information. This test
   * checks that the expected lint tasks are not UP-TO-DATE after modifying the library module's source directories.
   */
  @Test
  fun testLintNotUpToDateAfterModifyingSourceSets() {
    ...
    listOf("foo1", "foo2", "bar").forEach { srcDirName ->
      val srcDir = FileUtils.join(project1.getSubproject(":lib").projectDir, "src", srcDirName)
      val assetFile = FileUtils.join(srcDir, "assets", if (srcDirName == "bar") "bar.txt" else "foo.txt")
