...
class CustomAndroidSdkRule : ExternalResource() {
  ...
  private val customSdkRepo = File(testTmpDir, FileUtils.toSystemDependentPath("SysImgSource/dl.google.com/android/repository"))
  ...
  private val systemImageFiles by lazy { Splitter.on(' ').split(System.getProperty("sdk.repo.sysimage.files")).map { File(it) }.toList() }
  ...
  companion object {
    fun GradleRuleBuilder.withCustomSdkDir(rule: CustomAndroidSdkRule): GradleRuleBuilder {
      withSdk { sdkDir(rule.customSdkDir.toPath()) }
      return this
    }
     ...
     *
     * Running managed devices require a custom location for the avds to be created. Needs to be able to download the system-image from a
     * local repo dir. Runs on the canary channel. Runs the emulator in software-rendering mode.
     */
    fun GradleTaskExecutor.withCustomAndroidSdk(rule: CustomAndroidSdkRule): GradleTaskExecutor {
      ...
      withEnvironmentVariables(
        mapOf("HOME" to rule.customUserHomeDir.absolutePath, "ANDROID_USER_HOME" to rule.customAndroidPrefDir.absolutePath)
      )
      withoutOfflineFlag()
      withSdkAutoDownload()
  ...
  /** Sets the given file as the sdkDir for the project, as well as sets up the directory to be ready for download. */
  ...
  private fun setupLicenses() {
    ...
    val previewLicenseHash = String.format("84831b9409646a918e30573bab4c9c91346d8abd")

    Files.write(licenseFile.toPath(), licensesHash.toByteArray(StandardCharsets.UTF_8))
    Files.write(previewLicenseFile.toPath(), previewLicenseHash.toByteArray(StandardCharsets.UTF_8))
  }

  /** Sets up a facsimile of the online android repository for the purpose of downloading the system image for managed devices locally. */
  ...
  /**
   * Sets up the toplevel repository directory, which contains the dependencies of system images, most notably the emulator.
   *
   * This is grouped into two steps.
   * 1. The repository XMLs, which specify the versions, urls, and channel of sdk components needed by the system image.
   * 2. The actual tools located at these relative urls.
   *
   * The repository XMLs are set up manually to ensure that the dependency versions are stable. As the real XML file may update the
   * available versions for download (and therefore require the underlying bazel rules to change what target is available).
   */
  private fun setupTopLevelRepository() {
    ...
    val repositoryXmlContents =
      ...
      """
        .trimIndent()
    Files.write(repositoryXml.toPath(), repositoryXmlContents.toByteArray(StandardCharsets.UTF_8))
    ...
    val repositoryV2XmlContents =
      ...
      """
        .trimIndent()
    Files.write(repositoryV2Xml.toPath(), repositoryV2XmlContents.toByteArray(StandardCharsets.UTF_8))
   ...
   *
   * For downloading system-images, we only need the sys-img XML for AOSP images at present. We do not want to download the full addons list
   * from online with a bazel rule. There is no need to include other XMLs, as this drowns the gradle output with useless XML parsing
   * errors.
   */
   ...
   *
   * The repository XML is set up manually to ensure that the revision # of the system image does not change, because the system image that
   * is downloaded by bazel is contingent on the version.
   */
  private fun setupAOSPImageRepository() {
    ...
    val sysImgCommonPathLength = systemImageFiles.map(File::getAbsolutePath).reduce(String::commonPrefixWith).length
    ...
    ZipOutputStream(BufferedOutputStream(sysImgZipFile.outputStream())).use { output ->
      systemImageFiles.forEach { file ->
        ...
        BufferedInputStream(file.inputStream()).use { content -> content.copyTo(output, bufferSize = 1024) }
      }
    }

    val sysImgZipSha256 =
      Hashing.sha256().newHasher().let {
        BufferedInputStream(sysImgZipFile.inputStream()).use { content -> ByteStreams.copy(content, Funnels.asOutputStream(it)) }
        it.hash().toString()
      }
