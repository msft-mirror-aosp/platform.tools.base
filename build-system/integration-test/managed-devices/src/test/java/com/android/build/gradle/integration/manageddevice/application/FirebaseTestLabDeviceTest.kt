...
class FirebaseTestLabDeviceTest {
  ...
  class RemoveFTLDevice3Callback : GenericCallback {
    override fun handleProject(project: Project) {
      project.plugins.withType(AppPlugin::class.java) {
        val componentsExtension = project.extensions.getByType(ApplicationAndroidComponentsExtension::class.java)
        val ftlExtension = project.extensions.getByType(TestLabGradlePluginExtension::class.java)
        componentsExtension.finalizeDsl { ftlExtension.managedDevices.remove(ftlExtension.managedDevices.getByName("myFtlDevice3")) }
      }
    }
  }
  ...
  fun dsl() {
    ...
    result.stdout.use {
      ...
      assertThat(it)
        .contains("extraDeviceFiles = {/sdcard/Android/data/com.example.myapplication/myAdditionalText.txt=app/myAdditionalText.txt}")
  ...
  fun managedDevicesRemovesAllDevices() {
    ...
    // that does not exist.
    result.stdout.use { assertThat(it).doesNotContain("myFtlDevice3Check") }
    result.stdout.use { assertThat(it).doesNotContain("myFtlDevice3DebugAndroidTest") }
    result.stdout.use {
      assertThat(it).contains("myFtlDevice1Check")
      assertThat(it).contains("myFtlDevice1DebugAndroidTest")
      assertThat(it).contains("myFtlDevice2Check")
