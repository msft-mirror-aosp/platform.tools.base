...
class DataBindingDeterminismTest {
  ...
  companion object {
    ...
    private val INCONSISTENT_NON_TASK_OUTPUTS = setOf("build/reports/configuration-cache/")
  }
  ...
  fun `check consistent outputs after building two identical projects`() {
    ...
    val snapshot1 = FileSnapshot.snapshot(fileToSnapshot = project1.buildDir, baseDir = project1.projectDir)

    // Build the second project
    project2.executor().run("clean", "compileDebugJavaWithJavac")
    val snapshot2 = FileSnapshot.snapshot(fileToSnapshot = project2.buildDir, baseDir = project2.projectDir)
  ...
  private fun taskOutputDirs(snapshot: FileSnapshot): List<File> {
    return snapshot.directorySet.filterNot { dir -> INCONSISTENT_NON_TASK_OUTPUTS.any { dir.startsWith(it) } }
  }
}
