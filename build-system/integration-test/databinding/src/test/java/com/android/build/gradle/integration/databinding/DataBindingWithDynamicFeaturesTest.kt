...
class DataBindingWithDynamicFeaturesTest(private val useAndroidX: Boolean) {
  ...
  fun checkApkContainsDataBindingClasses() {
    ...
    val aApk: Apk = project.getSubproject("featureA").getApk(GradleTestProject.ApkType.DEBUG)
    val bApk: Apk = project.getSubproject("featureB").getApk(GradleTestProject.ApkType.DEBUG)

    assertThat(aApk).exists()
    assertThat(bApk).exists()

    val baseApk: Apk = project.getSubproject("app").getApk(GradleTestProject.ApkType.DEBUG)
    ...
    val featureBClasses =
      listOf(bindingClass(FEATURE_B, FEATURE_B_ACTIVITY), bindingClass(FEATURE_B, FEATURE_B_ACTIVITY_IMPL), brClass(FEATURE_B))
