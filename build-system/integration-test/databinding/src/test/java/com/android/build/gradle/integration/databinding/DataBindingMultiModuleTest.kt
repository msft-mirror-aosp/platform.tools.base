...
class DataBindingMultiModuleTest(useAndroidX: Boolean) {
  ...
  fun checkBRClasses() {
    ...
    getApk().use { assertThat(it).exists() }
    assertBRs("_all", "inheritedInput", "input", "appInput")
    // add a bindable to the inherited
    val newBindable = project.file("inherited/src/main/java/android/databinding/multimodule/inherited/NewBindable.java")
    newBindable.writeText(
      """
    ...
    TestFileUtils.searchAndReplace(project.file("inherited/src/main/res/layout/inherited_main.xml"), "inheritedInput", "inheritedInput2")
    executor().run("assembleRelease")
  ...
  fun checkAdaptersAreNotInherited() {
    ...
    appLayout.mkdirs()
    TestFileUtils.searchAndReplace(appLayout, "android:text=\"@{appInput}\"", "app:customSetText=\"@{appInput}\"")
    val result2 = executor().expectFailure().run("assembleRelease")
    MatcherAssert.assertThat(
      result2.failureMessage ?: "",
      CoreMatchers.containsString(
        "Cannot find a setter for <android.widget.TextView app:customSetText> that accepts parameter type 'java.lang.String'"
      ),
    )
  }
  ...
  fun checkCompilationAvoidanceOnLayoutChange() {
    ...
    executor().run(APP_COMPILE_JAVA_TASK).apply { assertTask(APP_COMPILE_JAVA_TASK).wasUpToDate() }
  }
  ...
  fun checkCompilationAvoidanceOnDummyAdapterChange() {
    ...
    executor().run(APP_COMPILE_JAVA_TASK)
    TestFileUtils.searchAndReplace(adapterFile, "public class NewAdapter {", "public class //dummy comment\nNewAdapter {")
    executor().run(APP_COMPILE_JAVA_TASK).apply {
  ...
  private fun createAdapterInInherited(attributeName: String): File {
    val newAdapter = project.file("inherited/src/main/java/android/databinding/multimodule/inherited/NewAdapter.java")
    newAdapter.writeText(
      """
                    package android.databinding.multimodule.inherited;
  ...
  private fun assertBRs(vararg fields: String) {
    getApk().use { apk ->
      assertThat(apk).hasMainDexFile().that().run {
        BR_CLASSES.forEach { brClass -> containsClass(brClass).that().hasExactFields(fields.toSet()) }
      }
    }
  }

  private fun getApk(): Apk {
    return project.getSubproject("app").getApk(GradleTestProject.ApkType.RELEASE)
  }
