...
/** Integration test to ensure cacheability when data binding is used (https://issuetracker.google.com/69243050). */
@RunWith(FilterableParameterized::class)
class DataBindingCachingTest(private val withKotlin: Boolean, private val withBuiltInKotlin: Boolean) {
   ...
   * location.
   */
  private val expectedTaskStates: Map<String, TaskStateList.ExecutionState> =
    mapOf(
        ...
        ":dataBindingMergeDependencyArtifactsDebug" to DID_WORK,
        /**
         * Intentionally not cacheable. See [com.android.build.gradle.internal.tasks.databinding.DataBindingMergeDependencyArtifactsTask]
         */
        ":dataBindingTriggerDebug" to DID_WORK,
        /** Intentionally not cacheable. See [com.android.build.gradle.internal.tasks.databinding.DataBindingTriggerTask] */
      ...
      .plus(
        if (withKotlin) {
          mapOf(":compileDebugKotlin" to SKIPPED, ":kaptDebugKotlin" to FROM_CACHE, ":kaptGenerateStubsDebugKotlin" to FROM_CACHE)
            .plus(
  ...
  val project =
    GradleTestProject.builder()
      ...
      .apply { if (!withBuiltInKotlin) disableBuiltInKotlin() }
  ...
  fun `test different package names generate different DataBindingInfo classes`() {
    projectCopy.file("src/main/AndroidManifest.xml").let {
      ...
      it.writeText(manifest.replace("package=\"android.databinding.testapp\"", "package=\"android.databinding.testapp2\""))
    }
