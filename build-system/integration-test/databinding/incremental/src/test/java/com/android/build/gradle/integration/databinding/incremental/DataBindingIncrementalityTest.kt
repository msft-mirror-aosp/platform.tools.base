 ...
 *
 * The test project has the following structure (including generated source files and compiled classes):
 *
 * project
 * - app
 *     - src/main/java
 ...
 *
 * The layout1.xml and layout2.xml layout files use expressions defined in Data1.java and Data2.java, whereas Loner1.java and Loner2.java
 * are source files that do not have any relationships with other source files and layout files.
 *
 * The following tests will change each of the files marked with [*** changed ***] above to trigger incremental builds and verify the set of
 * re-generated source files and recompiled classes.
 */
class DataBindingIncrementalityTest {
  ...
  companion object {
    ...
    private const val COMPILED_CLASSES_DIR = "build/intermediates/javac/debug/compileDebugJavaWithJavac/classes"
    ...
    private const val APP_GENERATED_DATA_BINDER_MAPPER_CLASS = "com.example.app.DataBinderMapperImpl"
    private const val APP_GENERATED_ANDROIDX_DATA_BINDER_MAPPER_CLASS = "androidx.databinding.DataBinderMapperImpl"

    // Generated source files in lib/build/generated/ap_generated_sources
    private const val LIB_GENERATED_DATA_BINDER_MAPPER_CLASS = "com.example.lib.DataBinderMapperImpl"
    private const val LIB_GENERATED_LAYOUT_1_BINDING_CLASS = "com.example.lib.databinding.Layout1BindingImpl"
    private const val LIB_GENERATED_LAYOUT_2_BINDING_CLASS = "com.example.lib.databinding.Layout2BindingImpl"
    private const val LIB_GENERATED_ON_CLICK_LISTENER_CLASS = "com.example.lib.generated.callback.OnClickListener"
    ...
    private const val GENERATED_BASE_ADAPTERS_BR_CLASS = "androidx.databinding.library.baseAdapters.BR"
    private const val GENERATED_DATA_BINDING_COMPONENT_CLASS = "androidx.databinding.DataBindingComponent"
  ...
  private fun setUpTestProject(): TestProject {
    return MultiModuleTestProject.builder().subproject(APP_MODULE, setUpApp()).subproject(LIB_MODULE, setUpLib()).build()
  }
  ...
  private fun setUpApp(): MinimalSubProject {
    ...
    app.withFile(
      ...
      with(BuildFileBuilder()) {
        ...
        addDependency(dependency = "'androidx.appcompat:appcompat:$ANDROIDX_VERSION'")
  ...
  private fun setUpLib(): MinimalSubProject {
    ...
    lib.withFile(
      ...
      with(BuildFileBuilder()) {
        ...
        addDependency(dependency = "'androidx.constraintlayout:constraintlayout:" + "$ANDROIDX_CONSTRAINT_LAYOUT_VERSION'")
        build()
      },
    )

    // 2. Create AndroidManifest.xml file
    lib.withFile("src/main/AndroidManifest.xml", with(ManifestFileBuilder()) { build() })
    ...
    lib.withFile(
      ...
      with(LayoutFileBuilder()) {
        ...
        addTextView(
          "text1",
          "@{data1.data1Field1}",
          listOf("android:onClick=\"@{(view) -> data1.clicked(view)}\"", "android:customProperty1=\"@{data1.data1Field2}\""),
        )
        build()
      },
    )
    lib.withFile(
      ...
      with(LayoutFileBuilder()) {
        ...
        addTextView(
          "text1",
          "@{data2.data2Field1}",
          listOf("android:onClick=\"@{(view) -> data2.clicked(view)}\"", "android:customProperty2=\"@{data2.data2Field2}\""),
        )
        build()
      },
    )
  ...
  fun setUp() {
    ...
    appGeneratedAndroidXDataBinderMapperJavaFile =
      File("$appDir/$GENERATED_SOURCE_DIR/${getPath(APP_GENERATED_ANDROIDX_DATA_BINDER_MAPPER_CLASS)}.java")
    ...
    appGeneratedAndroidXDataBinderMapperClass =
      File("$appDir/$COMPILED_CLASSES_DIR/${getPath(APP_GENERATED_ANDROIDX_DATA_BINDER_MAPPER_CLASS)}.class")
  ...
  private fun recordChangedFiles() {
    changedFiles = fileToTimestampMap.filter { (file, previousTimestamp) -> file.lastModified() != previousTimestamp }.keys
  }
  ...
  fun `change original source file with a change related to data binding`() {
    ...
    expect.that(changedFiles).named("Recompiled classes").containsNoneOf(libLoner2Class, libLoner1Class)
  ...
  fun `change original source file with a change not related to data binding`() {
    ...
    // The irrelevant (original or generated) source files should not be recompiled
    expect.that(changedFiles).named("Recompiled classes").containsNoneOf(libLoner1Class, libLoner2Class)
  ...
  fun `change original source file that is not related to data binding`() {
    ...
    searchAndReplace(libLoner1JavaFile, "public class Loner1", "public class Loner1 /* dummy comment to trigger change */")
    ...
    // The irrelevant (original or generated) source files should not be recompiled
    expect.that(changedFiles).named("Recompiled classes").doesNotContain(libLoner2Class)
  ...
  fun `change original layout file with a change related to data binding`() {
    ...
    searchAndReplace(libLayout1LayoutFile, "android:text=\"@{data1.data1Field1}\"", "android:text=\"@{data1.data1Field2}\"")
  ...
  fun `change original layout file with a change not related to data binding`() {
    ...
    searchAndReplace(libLayout1LayoutFile, "<data>", "<data> <!-- dummy comment to trigger change -->")
