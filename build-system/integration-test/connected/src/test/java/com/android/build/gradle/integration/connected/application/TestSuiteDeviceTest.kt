...
class TestSuiteDeviceTest {
    ...
    class MyTestEngine : HierarchicalTestEngine<MyTestEngineContext>() {
        ...
        override fun discover(
            ...
            uniqueId: UniqueId
        ): TestDescriptor = MyTestEngineDescriptor(uniqueId).apply {
            addChild(MyTestDescriptor(uniqueId, "myTestCase"))
        }

        override fun createExecutionContext(request: ExecutionRequest) = MyTestEngineContext()
    }

    class MyTestEngineDescriptor(uniqueId: UniqueId) :
        EngineDescriptor(uniqueId, "MyTestEngine")

    class MyTestEngineContext : EngineExecutionContext

    class MyTestDescriptor(parentId: UniqueId, testName: String) :
        AbstractTestDescriptor(
            parentId.append("my-test-segment", testName),
            testName), Node<MyTestEngineContext> {
        override fun getType() = TestDescriptor.Type.TEST
        override fun execute(
            context: MyTestEngineContext,
            dynamicTestExecutor: Node.DynamicTestExecutor
        ): MyTestEngineContext {
            val client = TestSuiteExecutionClient.default()
            val serialIds =
                client.getInputParameter(TestEngineInputProperty.SERIAL_IDS).split(',').sorted()
    ...
    fun allOnlineDeviceShouldBePassedByDefault() {
        executor
            ...
            .assertOutputContains("Serial IDs = emulator-5554,emulator-5556")
    }
