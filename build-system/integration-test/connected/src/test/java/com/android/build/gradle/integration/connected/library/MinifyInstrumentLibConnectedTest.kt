...
class MinifyInstrumentLibConnectedTest {
  ...
  @get:Rule val project: GradleTestProject = GradleTestProject.builder().fromTestProject("libMinifyLibDep").create()
  ...
  fun testMinify() {
    ...
    TestFileUtils.addMethod(
      libProject.projectDir.resolve("src/androidTest/java/com/android/tests/basic/StringGetterTest.java"),
      """
      @Test
    ...
    TestFileUtils.appendToFile(libProject.buildFile, "android.buildTypes.debug.testProguardFiles('$testProguardFileName')")
    project.executor().run("lib:connectedAndroidTest")
    // aapt generated keep rules from android test manifest are consumed by R8, regression test
    // for b/328649293
    val androidTestManifest =
      InternalArtifactType.PACKAGED_MANIFESTS.getOutputDir(libProject.buildDir)
        .resolve("debugAndroidTest/processDebugAndroidTestManifest/AndroidManifest.xml")
    assertThat(libProject.projectDir.resolve(androidTestManifest).readText())
      .contains(
        """
    ...
    val aaptRule =
      InternalArtifactType.AAPT_PROGUARD_FILE.getOutputDir(libProject.buildDir)
        .resolve("debugAndroidTest/processDebugAndroidTestResources/aapt_rules.txt")
    assertThat(libProject.projectDir.resolve(aaptRule).readText())
      .contains(
        """
