...
class AndroidTestEngineConnectedTest {
    companion object {
        @ClassRule
        @JvmField
        val emulatorRule = if (TestUtils.runningFromBazel()) {
            Emulator(
            ...
            androidComponents.onVariants(androidComponents.selector().withName("debug")) { variant ->
                ...
                project.tasks.withType(org.gradle.api.tasks.testing.Test::class.java).configureEach { task ->
                    if (task.name.contains(variant.name, ignoreCase = true)) {
                        ...
                        task.doFirst {
                            val apks = appApkLocation.list()
                                .filter { it.endsWith(".apk") }
                                .joinToString(",") { File(appApkLocation, it).absolutePath }
                        ...
                        task.doFirst {
                            val apks = testApkLocation.list()
                                .filter { it.endsWith(".apk") }
                                .joinToString(",") { File(testApkLocation, it).absolutePath }
