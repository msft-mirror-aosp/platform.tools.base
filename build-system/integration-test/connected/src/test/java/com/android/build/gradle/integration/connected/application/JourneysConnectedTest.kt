...
class JourneysConnectedTest {
  ...
  private val executor: GradleTaskExecutor
    get() = rule.build.executor.withConfigurationCaching(BaseGradleExecutor.ConfigurationCaching.ON).withLoggingLevel(LoggingLevel.INFO)
  ...
  class FakeCrawlerServiceSetupCallback : ApplicationComponentCallback {
    ...
    override fun handleExtension(project: Project, androidComponents: ApplicationAndroidComponentsExtension) {
      ...
      project.tasks.withType(org.gradle.api.tasks.testing.Test::class.java).configureEach { testTask ->
        val path = project.providers.systemProperty("roboResultsPath").orNull ?: ""
        val shouldInduceServerError = project.providers.systemProperty("shouldInduceServerError").orNull ?: "false"
  ...
  fun `expect auth failure when using prod backend`() {
    val build = rule.build { androidApplication { pluginCallbacks -= FakeCrawlerServiceSetupCallback::class.java } }
    ...
    assertJourneyEvents(
      ...
      "",
      listOf(
        buildRunStartedEvent(prompts = listOf("Action 1"), metadata = mapOf("deviceId" to DEVICE_SERIAL, "deviceName" to DEVICE_NAME)),
  ...
  fun `expect journey filter to select specified journeys`() {
    ...
    val getExpectedEventsForJourney = { outputDir: Path ->
      listOf(
        ...
        buildStepStartedEvent("Action 1", Timestamp.newBuilder().setSeconds(1750247037).setNanos(165000000).build()),
        buildTurnAddedEvent(
          "Perform action 1",
          "Performing action 1",
          "$outputDir/displayState1.png",
          listOf(
            buildInteraction(CommandType.ADB_COMMAND, "ADB command: input tap 75 217", "$outputDir/displayState1.png", Status.SUCCEEDED)
          ),
        ),
        buildTurnAddedEvent("Goal Complete", "The goal for action 1 is complete.", "$outputDir/displayState2.png", listOf()),
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Action 2", Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build()),
        buildTurnAddedEvent(
          "Perform action 2",
          "Performing action 2",
          "$outputDir/displayState2.png",
          listOf(
            buildInteraction(CommandType.ADB_COMMAND, "ADB command: input tap 540 919", "$outputDir/displayState2.png", Status.SUCCEEDED)
          ),
        ),
        buildTurnAddedEvent("Goal Complete", "The goal for action 2 is complete.", "$outputDir/displayState3.png", listOf()),
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247096).setNanos(665000000).build(), Status.SUCCEEDED),
        buildRunFinishedEvent(Status.SUCCEEDED),
      )
    }
    assertJourneyEvents(
      result,
  ...
  fun `expect journey filter to select nested journeys`() {
    ...
    appProject.files.add("src/journeysTest/auth/newUsers/signup.journey.xml", journeyFileContent)
    appProject.files.add("src/journeysTest/auth/newUsers/file with spaces & special characters {}[]!@+().journey.xml", journeyFileContent)
    appProject.files.add("src/journeysTest/file with spaces & special characters {}[]!@+().journey.xml", journeyFileContent)
    ...
    appProject.files.add("src/journeysTest/profile/info.journey.xml", journeyFileContent)
  ...
  fun `expect error with malformed journey`() {
    ...
    assertJourneyEvents(
      result,
      "$DEVICE_SERIAL > malformed.journey.xml",
      "",
      listOf(
        buildRunStartedEvent(prompts = listOf(), metadata = mapOf("deviceId" to DEVICE_SERIAL, "deviceName" to DEVICE_NAME)),
        buildRunFinishedEvent(Status.ERROR, "The <journey> element must have exactly one <actions> element, but found 0."),
      ),
    )
  }
  ...
  fun `run successful journey`() {
    ...
    val expectedEvents =
      listOf(
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Tap on the 'Compose' topic", Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build()),
        buildTurnAddedEvent(
          "Tap on the 'Compose' topic",
          ...
          "$outputDir/displayState5.png",
          listOf(
            buildInteraction(CommandType.ADB_COMMAND, "ADB command: input tap 540 919", "$outputDir/displayState5.png", Status.SUCCEEDED)
          ),
        ),
        buildTurnAddedEvent(
          "Goal Complete",
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247084).setNanos(116000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Save the first post", Timestamp.newBuilder().setSeconds(1750247084).setNanos(116000000).build()),
        buildTurnAddedEvent(
          ...
          "$outputDir/displayState6.png",
          listOf(
            buildInteraction(CommandType.ADB_COMMAND, "ADB command: input tap 913 1922", "$outputDir/displayState6.png", Status.SUCCEEDED)
          ),
        ),
        buildTurnAddedEvent(
          "Goal Complete",
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247090).setNanos(48000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Go to saved posts", Timestamp.newBuilder().setSeconds(1750247090).setNanos(48000000).build()),
        buildTurnAddedEvent(
          ...
          "$outputDir/displayState7.png",
          listOf(
            buildInteraction(CommandType.ADB_COMMAND, "ADB command: input tap 540 2232", "$outputDir/displayState7.png", Status.SUCCEEDED)
          ),
        ),
        buildTurnAddedEvent(
          "Goal Complete",
    ...
    val expectedEvents =
      listOf(
        ...
        buildTurnAddedEvent(
          ...
          "The current goal is to tap on the search icon and enter 'Compose'. The search icon is available on the screen. So, I will tap on it.",
          "$outputDir/displayState1.png",
          listOf(buildInteraction(CommandType.CLICK, "CLICK on element with ID 1", "$outputDir/displayState1.png", Status.SUCCEEDED)),
        ),
        buildTurnAddedEvent(
          "Enter 'Compose' into the search field",
          "The current goal is to tap on the search icon and enter 'Compose'. The search icon has already been tapped in the previous step. Now I need to enter 'Compose' into the search field.",
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247072).setNanos(407000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Tap on the 'Compose' topic", Timestamp.newBuilder().setSeconds(1750247072).setNanos(407000000).build()),
        buildTurnAddedEvent(
          "Tap on the 'Compose' topic",
          "The current goal is to tap on the 'Compose' topic. The element with the text 'Compose' is tappable and is the correct element to tap.",
          "$outputDir/displayState4.png",
          listOf(buildInteraction(CommandType.CLICK, "CLICK on element with ID 4", "$outputDir/displayState4.png", Status.SUCCEEDED)),
        ),
        buildTurnAddedEvent(
          "Goal Complete",
          "The previous action was to tap on the 'Compose' topic, which fulfills the current goal.",
        ...
        buildStepFinishedEvent(Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build(), Status.SUCCEEDED),
        buildStepStartedEvent("Save the first post", Timestamp.newBuilder().setSeconds(1750247076).setNanos(824000000).build()),
        buildTurnAddedEvent(
          "Save the first post by tapping on the bookmark icon.",
          "Reasoning not available",
          "$outputDir/displayState5.png",
          listOf(buildInteraction(CommandType.CLICK, "CLICK on element with ID 5", "$outputDir/displayState5.png", Status.SUCCEEDED)),
        ),
        buildTurnAddedEvent(
          "Goal Failed",
  ...
  fun `run journey that fails to launch the app`() {
    ...
    val expectedEvents =
      listOf(
        ...
        buildRunFinishedEvent(Status.ERROR, "Failed to launch app - LAUNCH_INTENT_MISSING [Reason=LAUNCH_APP_FAILED]"),
      )
    assertJourneyEvents(result, "$DEVICE_SERIAL > simple.journey.xml", "", expectedEvents)
  }

  private fun createRoboResults(roboResultsResourceName: String, roboResultsPath: Path) {
    val roboResultsBytes = Resources.toByteArray(Resources.getResource(JourneysConnectedTest::class.java, roboResultsResourceName))
  ...
  private fun buildResult(status: Status, errorMessage: String = ""): Result =
    Result.newBuilder().setStatus(status).setErrorMessage(errorMessage).build()
  ...
  private fun buildInteraction(
    ...
    return Interaction.newBuilder()
      .setType(type)
      .setCommand(command)
      .addArtifactsBefore(Artifact.newBuilder().setType(ArtifactType.SCREENSHOT).setUri(screenshotPath).build())
      .setResult(result)
      .build()
  }
  ...
  private fun buildTurnAddedEvent(
    ...
    val turn =
      Turn.newBuilder()
        ...
        .addArtifactsBefore(Artifact.newBuilder().setType(ArtifactType.SCREENSHOT).setUri(turnScreenshotPath))
  ...
  private fun buildStepStartedEvent(promptText: String, startTimestamp: Timestamp): JourneyRunEvent {
    val init = Step.Initialization.newBuilder().setPromptText(promptText).setStartTimestamp(startTimestamp).build()
  ...
  private fun buildStepFinishedEvent(endTimestamp: Timestamp, status: Status, errorMessage: String = ""): JourneyRunEvent {
    val result = buildResult(status, errorMessage)
    val completion = Step.Completion.newBuilder().setEndTimestamp(endTimestamp).setResult(result).build()
  ...
  private fun buildRunStartedEvent(prompts: List<String>, metadata: Map<String, String> = emptyMap()): JourneyRunEvent {
    val init =
  ...
  private fun buildRunFinishedEvent(status: Status, errorMessage: String = ""): JourneyRunEvent {
    val result = buildResult(status, errorMessage)
    val completion = JourneyRunResult.Completion.newBuilder().setResult(result).build()
  ...
  private fun assertJourneyEvents(
    ...
    actualEvents.forEachIndexed { index, (description, actualEvent) ->
      ...
      assertEquals(expectedEvent.eventPayloadCase.name, description, "Event description mismatch at index $index")

      assertEquals(expectedEvent.eventPayloadCase, actualEvent.eventPayloadCase, "Event type mismatch at index $index")
      ...
      assertEquals(expectedEvent, actualEventForCompare, "Event payload mismatch at index $index")
    }
  }

  /**
   * Scans stdout and extracts journey artifact lines that are located between two boundary lines, identified by prefixes.
   *
   * The function starts scanning from the beginning of stdout. It ignores all lines until it finds a line that starts with
   * [startLinePrefix]. After that, it starts processing lines.
   *
   * For each line, it checks if the line matches against a journey artifact regex. If the regex matches, the key and value of the
   * corresponding artifact is obtained via first and second match groups respectively.
   *
   * The processing stops when a line starting with [endLinePrefix] is found, or the end of stdout is reached (if [endLinePrefix] is blank).
   * The line with [endLinePrefix] is not processed.
   *
   ...
   * @param endLinePrefix The prefix of the line that marks the end of the section to process. If blank, the processing goes until the end
   *   of the output.
