...
/**
 * This test builds mlModelBinding project and runs Android test inside to verify it returns valid model output through generated code.
 *
...
class MlModelBindingConnectedTest {
  ...
  @Rule @JvmField var project = builder().fromTestProject("mlModelBinding").create()
  ...
  fun setup() {
    ...
    FileUtils.copyFile(
      TestUtils.resolveWorkspacePath("prebuilts/tools/common/mlkit/testData/models/style_predict_quant_metadata.tflite").toFile(),
      stylePredictModelFile,
    )

    val styleTransferModelFile = project.file("src/main/ml/style_transfer_quant_metadata.tflite")
    FileUtils.copyFile(
      TestUtils.resolveWorkspacePath("prebuilts/tools/common/mlkit/testData/models/style_transfer_quant_metadata.tflite").toFile(),
      styleTransferModelFile,
    )

    val objectDetectionModelFile = project.file("src/main/ml/ssd_mobilenet_odt_metadata_v1.2.tflite")
    FileUtils.copyFile(
      TestUtils.resolveWorkspacePath("prebuilts/tools/common/mlkit/testData/models/ssd_mobilenet_odt_metadata_v1.2.tflite").toFile(),
  ...
  fun verifyModelInferenceWithValidOutput() {
    ...
    val htmlResults = project.file("build/reports/androidTests/connected/debug/com.example.app.ModelTest.html")
