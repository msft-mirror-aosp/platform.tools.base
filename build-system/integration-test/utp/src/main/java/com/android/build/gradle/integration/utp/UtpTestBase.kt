...
abstract class UtpTestBase(val runWithBuiltInPlatform: Boolean) {
    ...
    fun additionalTestOutputWithTestStorageService() {
        ...
        rule.build.androidApplication().reconfigure {
            ...
            files {
                add("src/androidTest/java/com/example/helloworld/TestStorageServiceExampleTest.kt",
                    //language=kotlin
                    """
        ...
        assertThat(project.resolve("${testAdditionalOutputPath}/myTestStorageOutputFile1"))
            .contains("output message1")
        assertThat(project.resolve("${testAdditionalOutputPath}/myTestStorageOutputFile2.txt"))
            .contains("output message2")
        assertThat(project.resolve("${testAdditionalOutputPath}/subdir/myTestStorageOutputFile3"))
            .contains("output message3")
        assertThat(project.resolve("${testAdditionalOutputPath}/subdir/nested/myTestStorageOutputFile4"))
            .contains("output message4")
        assertThat(project.resolve("${testAdditionalOutputPath}/subdir/white space/myTestStorageOutputFile5"))
    ...
    fun additionalTestOutputWithoutTestStorageService() {
        ...
        rule.build.androidApplication().reconfigure {
            files {
                add("src/androidTest/java/com/example/helloworld/AdditionalTestOutputExampleTest.kt",
                    //language=kotlin
                    """
                    package com.example.helloworld
    ...
    fun additionalTestOutputWithBenchmarkFiles() {
        ...
        assertThat(project.resolve("${testAdditionalOutputPath}/" +
    ...
    fun androidTestWithOrchestratorAndCodeCoverageWithDynamicFeature() {
        ...
        assertThat(project.resolve(testResultPbPath)).exists()
        assertThat(project.resolve(testCoverageXmlPath)).contains(
                """<method name="stubfeatureFuncForTestingCodeCoverage" desc="()V" line="9">"""
        )
        assertThat(project.resolve(testCoverageXmlPath)).contains(
                """<counter type="INSTRUCTION" missed="3" covered="5"/>"""
        )
