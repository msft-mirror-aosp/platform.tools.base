 ...
 *
 * The project set up is equivalent to File/Create New Project except that there is a new file called extra-header.hpp and this file is
 * referenced by CMakeLists.txt.
 *
 * Even though this is valid CMake syntax we shouldn't send the header-file to Android Studio because it won't have flags and Android Studio
 * will reject it. The header file is recorded in android_gradle_build.json but is not in the model sent to Android Studio.
 */
  ...
  fun testThatHeaderFileIsExcluded() {
    ...
    for (variant in nativeModule.variants) {
      for (abi in variant.abis) {
        assertThat(abi.sourceFlagsFile.readCompileCommandsJsonBin(nativeModules.normalizer)).hasSize(1)
      }
    }
  }
}
