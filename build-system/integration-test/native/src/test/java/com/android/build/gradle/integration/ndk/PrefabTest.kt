...
class PrefabTest(private val buildSystem: NativeBuildSystem, val cmakeVersion: String) {
  ...
  @Rule
  @JvmField
  val project = GradleTestProject.builder().fromTestProject("prefabApp").setSideBySideNdkVersion(DEFAULT_NDK_SIDE_BY_SIDE_VERSION).create()

  @Rule
  @JvmField
  val prefabNoDepsProject =
    GradleTestProject.builder().fromTestProject("prefabNoDeps").setSideBySideNdkVersion(DEFAULT_NDK_SIDE_BY_SIDE_VERSION).create()

  companion object {
    @Parameterized.Parameters(name = "build system = {0}, cmake = {1}")
    @JvmStatic
    fun data() =
      CMakeVersion.FOR_TESTING.map { arrayOf(NativeBuildSystem.CMAKE, it.version) } + listOf(arrayOf(NativeBuildSystem.NDK_BUILD, "N/A"))
  }
  ...
  private fun verifyCMakePackage(pkg: String, abi: CxxAbiModel) {
    ...
    assertThat(abi.prefabFolder.resolve("prefab/lib/$triple/cmake/$pkg/${pkg}Config.cmake")).exists()
  }
