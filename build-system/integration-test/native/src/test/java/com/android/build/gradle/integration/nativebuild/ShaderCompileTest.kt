...
class ShaderCompileTest {
  ...
  fun assembleGlslcDebugWrongName() {
    ...
    project.file("local.properties").appendText("glslc.dir=${FileUtils.toSystemIndependentPath(emptyDirectory.absolutePath)}")
    val result = project.executor().expectFailure().run("assembleDebug")
