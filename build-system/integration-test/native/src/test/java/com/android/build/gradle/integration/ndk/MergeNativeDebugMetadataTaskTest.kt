...
class MergeNativeDebugMetadataTaskTest(private val debugSymbolLevel: DebugSymbolLevel?) {
  ...
  companion object {
    @JvmStatic @Parameterized.Parameters(name = "debugSymbolLevel_{0}") fun params() = listOf(null, NONE, SYMBOL_TABLE, FULL)
  }

  @get:Rule
  val project: GradleTestProject =
    GradleTestProject.builder().fromTestProject("dynamicApp").setSideBySideNdkVersion(DEFAULT_NDK_SIDE_BY_SIDE_VERSION).create()
  ...
  data class TestSetup(val app: GradleTestProject, val expectedFullEntries: List<String>, val expectedSymbolTableEntries: List<String>)
  ...
  private fun setupExternalNativeDebugSymbolTest(): TestSetup {
    ...
    val expectedSymbolTableEntries =
      listOf(
  ...
  fun testBundleExternalNativeDebugSymbolsOutput() {
    ...
    val expectedSymbolTableEntries = testSetup.expectedSymbolTableEntries.map { "$bundleEntryPrefix$it" }
    Zip(bundleFile).use { zip ->
      when (debugSymbolLevel) {
        SYMBOL_TABLE -> {
          assertThat(zip.entries.map { it.toString() }).containsNoneIn(expectedFullEntries)
          assertThat(zip.entries.map { it.toString() }).containsAtLeastElementsIn(expectedSymbolTableEntries)
        }
        FULL -> {
          assertThat(zip.entries.map { it.toString() }).containsAtLeastElementsIn(expectedFullEntries)
          assertThat(zip.entries.map { it.toString() }).containsNoneIn(expectedSymbolTableEntries)
        }
  ...
  private fun setupNativeDebugSymbolTestForDirectory(): TestSetup {
    ...
    val expectedFullEntries = listOf("/$ABI_ARMEABI_V7A/app.so.dbg")

    val expectedSymbolTableEntries = listOf("/$ABI_ARMEABI_V7A/app.so.sym")
    return TestSetup(app, expectedFullEntries, expectedSymbolTableEntries)
  }
  ...
  fun testExternalNativeDebugSymbolsOutput() {
    ...
    Zip(output).use { verifyAllZipEntries(it, testSetup.expectedFullEntries, testSetup.expectedSymbolTableEntries) }
  }

  private fun verifyAllZipEntries(zip: Zip, expectedFullEntries: List<String>, expectedSymbolTableEntries: List<String>) {
    when (debugSymbolLevel) {
      SYMBOL_TABLE -> {
        assertThat(zip.entries.map { it.toString() }).containsNoneIn(expectedFullEntries)
        assertThat(zip.entries.map { it.toString() }).containsExactlyElementsIn(expectedSymbolTableEntries)
      }
      FULL -> {
        assertThat(zip.entries.map { it.toString() }).containsExactlyElementsIn(expectedFullEntries)
        assertThat(zip.entries.map { it.toString() }).containsNoneIn(expectedSymbolTableEntries)
      }
      else -> fail("Test should return early if not SYMBOL_TABLE or FULL")
    }
  ...
  fun testTaskRunsWhenNativeLibNameChanges() {
    ...
    Zip(output).use { zip ->
      when (debugSymbolLevel) {
        SYMBOL_TABLE -> {
          assertThat(zip.entries.map { it.toString() }).containsExactly("/$ABI_ARMEABI_V7A/foo.so.sym")
        }
        FULL -> {
          assertThat(zip.entries.map { it.toString() }).containsExactly("/$ABI_ARMEABI_V7A/foo.so.dbg")
        }
    ...
    FileUtils.renameTo(
      File(project.getSubproject(":feature1").getMainSrcDir("jniLibs"), "$ABI_ARMEABI_V7A/foo.so"),
      File(project.getSubproject(":feature1").getMainSrcDir("jniLibs"), "$ABI_ARMEABI_V7A/bar.so"),
    )
    val result2 = project.executor().run("app:assembleDebug")
    assertThat(output).exists()
    assertThat(result2.didWorkTasks).contains(":app:$taskName")
    Zip(output).use { zip ->
      when (debugSymbolLevel) {
        SYMBOL_TABLE -> {
          assertThat(zip.entries.map { it.toString() }).containsExactly("/$ABI_ARMEABI_V7A/bar.so.sym")
        }
        FULL -> {
          assertThat(zip.entries.map { it.toString() }).containsExactly("/$ABI_ARMEABI_V7A/bar.so.dbg")
        }
        else -> fail("Test assumes debugSymbolLevel is SYMBOL_TABLE or FULL")
      }
    }
  ...
  private fun getNativeDebugSymbolsOutput(): File {
    return File(project.getSubproject("app").buildDir, "/outputs/native-debug-symbols/debug/native-debug-symbols.zip")
  }

  private fun createDebugMetadataFile(project: GradleTestProject, abiName: String, full: Boolean) {
    ...
    MergeNativeDebugMetadataTaskTest::class.java.getResourceAsStream("/nativeDebugSymbols/$symFolder/$abiName/extra.so.$extension").use {
      inputStream ->
      File(abiFolder, "app-extra.so.$extension").outputStream().use { outputStream -> inputStream!!.copyTo(outputStream) }
    }
  }

  private fun createUnstrippedAbiFile(project: GradleTestProject, abiName: String, libName: String) {
    val abiFolder = File(project.getMainSrcDir("jniLibs"), abiName)
    FileUtils.mkdirs(abiFolder)
    MergeNativeDebugMetadataTaskTest::class.java.getResourceAsStream("/nativeLibs/unstripped.so").use { inputStream ->
      File(abiFolder, libName).outputStream().use { outputStream -> inputStream!!.copyTo(outputStream) }
    }
  }

  private fun createStrippedAbiFile(project: GradleTestProject, abiName: String, libName: String) {
    val abiFolder = File(project.getMainSrcDir("jniLibs"), abiName)
    FileUtils.mkdirs(abiFolder)
    MergeNativeDebugMetadataTaskTest::class.java.getResourceAsStream("/nativeLibs/libhello-jni.so").use { inputStream ->
      File(abiFolder, libName).outputStream().use { outputStream -> inputStream!!.copyTo(outputStream) }
    }
  }
}
