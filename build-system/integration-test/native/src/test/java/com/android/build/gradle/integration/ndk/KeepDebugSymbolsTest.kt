...
class KeepDebugSymbolsTest {
  ...
  fun testKeepDebugSymbolsForApp() {
    ...
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/appStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/appDslDoNotStrip1.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/appDslDoNotStrip2.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/appDebugDoNotStrip.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/appReleaseDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/libStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/libDslDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/libDebugDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(debugApk, "lib/x86/libReleaseDoNotStrip.so")).isStripped()
    ...
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/appStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/appDslDoNotStrip1.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/appDslDoNotStrip2.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/appDebugDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/appReleaseDoNotStrip.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/libStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/libDslDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/libDebugDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(releaseApk, "lib/x86/libReleaseDoNotStrip.so")).isStripped()

    val appAndroidTestDebugApk = appSubproject.getApk(GradleTestProject.ApkType.ANDROIDTEST_DEBUG)
    ...
    assertThatNativeLib(ZipHelper.extractFile(appAndroidTestDebugApk, "lib/x86/androidTestStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(appAndroidTestDebugApk, "lib/x86/androidTestDoNotStrip.so")).isNotStripped()
  }
  ...
  fun testKeepDebugSymbolsForLib() {
    ...
    libSubproject.assertAar(AarSelector.DEBUG) {
      jniLibs().apply {
        containsExactly("x86/libStrip.so", "x86/libDslDoNotStrip.so", "x86/libDebugDoNotStrip.so", "x86/libReleaseDoNotStrip.so")
    ...
    val libAndroidTestDebugApk = libSubproject.getApk(GradleTestProject.ApkType.ANDROIDTEST_DEBUG)
    ...
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/libStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/libDslDoNotStrip.so")).isNotStripped()
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/libDebugDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/libReleaseDoNotStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/androidTestStrip.so")).isStripped()
    assertThatNativeLib(ZipHelper.extractFile(libAndroidTestDebugApk, "lib/x86/androidTestDoNotStrip.so")).isNotStripped()
  }
  ...
  private fun createAbiFile(gradleTestProject: GradleTestProject, srcDirName: String, libName: String) {
    ...
    KeepDebugSymbolsTest::class.java.getResourceAsStream("/nativeLibs/unstripped.so").use { inputStream ->
