...
class ModuleToModuleDepsTest(
  ...
  /** When ndk-build is configure to produce .a but has shared STL it will silently produce no .a file. */
  ...
  fun `app second configure should be nop`() {
    ...
    executor.run(":app:configure${appBuildSystem.build}Debug[arm64-v8a]")
    val secondConfigure =
      getTestProject().readStructuredLogs(::decodeConfigureInvalidationState).filter {
        it.inputFilesList.any { it.contains("prefab_publication.json") }
      }
  ...
  fun `check single STL violation CXX1211`() {
    ...
    executor.run(":app:configure${appBuildSystem.build}Debug[arm64-v8a]")
    val errors = getTestProject().readStructuredLogs(::decodeLoggingMessage).filter { it.level == LoggingMessage.LoggingLevel.ERROR }
    val error = errors.map { it.diagnosticCode }.single()
    assertThat(error).named(errors.map { it.text() }.single()).isEqualTo(1211)
  }
  ...
  fun `check single STL violation CXX1212`() {
    ...
    val errors = getTestProject().readStructuredLogs(::decodeLoggingMessage).filter { it.level == LoggingMessage.LoggingLevel.ERROR }
    val error = errors.map { it.diagnosticCode }.single()
    assertThat(error).named(errors.map { it.text() }.single()).isEqualTo(1212)
  }
  ...
  fun `changing a cpp file causes a rebuild`() {
    ...
    executor.run(buildTask).apply { assertTask(":lib:prefabDebugPackage").wasUpToDate() }
