...
/**
 * Execute Prefab for different configurations. Check for expected errors. Also, there is a fuzz tester that was used to generate most of
 * these cases.
 */
class PrefabErrorReportingTest {
  ...
  /** If a message in stderr is not recognized then it should still be reported. It just won't have an error code. */
  @Test
  ...
  /**
   * The reportErrors(...) function tries (and always succeeds in known cases) to find a library name that is related to an error that comes
   * later. This test fabricates a STDERR such that relevantLibrary isn't found. When this happens, there should still be a reasonable error
   * message.
   */
  @Test
  fun `Ensure reasonable error message even when relevantLibrary not found`() {
    ...
    PassThroughRecordingLoggingEnvironment().use { logger ->
      reportErrors(stderr)
      assertThat(logger.errors).containsExactly("[CXX1214] User has minSdkVersion 10 but library was built for 11")
    }
  }

  @Test
  fun `CXX1210-No compatible library found for package`() {
    baseline.copy(moduleLibraryAbi = "armeabi-v7a").assertHasError("[CXX1210] No compatible library found [//pkg1/lib1]")
  }

  @Test
  fun `CXX1211-Library is a shared library with a statically linked STL and cannot be used with any library using the STL`() {
    baseline
      .copy(stl = "gnustl_shared", moduleLibraryAbiStl = "gnustl_static")
      .assertHasError(
  ...
  fun `CXX1212-User is using a static STL but library requires a shared STL`() {
    baseline
      .copy(stl = "c++_static", moduleLibraryAbiStl = "c++_shared")
      .assertHasError("[CXX1212] User is using a static STL but library requires a shared STL [//pkg1/lib1]")
  }

  @Test
  fun `CXX1213-User requested libstdc++ but library requires libc++`() {
    baseline
      .copy(stl = "gnustl_static", moduleLibraryAbiStl = "c++_static")
      .assertHasError("[CXX1213] User requested libstdc++ but library requires libc++ [//pkg1/lib1]")
  }

  @Test
  fun `CXX1214-User has minSdkVersion 7 but library was built for 16`() {
    baseline
      .copy(abi = "x86", osVersion = 7, moduleLibraryAbi = "x86", moduleLibraryAbiMinSdkVersion = 16)
      .assertHasError("[CXX1214] User has minSdkVersion 7 but library was built for 16 [//pkg1/lib1]")
  }
  ...
  fun `Only first error is reported`() {
    ...
    baseline
      .copy(stl = "gnustl_shared", moduleLibraryAbiStl = "gnustl_static", hasLibrary2 = true, moduleLibraryAbiStl2 = "stlport_shared")
      .assertHasError(
        "[CXX1211] Library is a shared library with a statically linked STL and cannot be used with any library using the STL [//pkg1/lib1]"
      )
  }
  ...
  fun `CXX1216-package contains artifacts for an unsupported platform windows`() {
    baseline
      .copy(moduleLibraryTargetPlatform = "windows")
  ...
  fun `CXX1217-Only schema_version 1 is supported, pkg1 uses version 2`() {
    ...
    baseline.copy(prefabVersion = "1.1.3").assertHasError("[CXX1217] Only schema_version 1 is supported. pkg1 uses version 2.")

    baseline.copy(prefabSchemaVersion = 100).assertHasError("[CXX1217] schema_version must be between 1 and 2. Package uses version 100.")
  }
  ...
  fun `CXX1218-Unexpected JSON token at offset 87, Encountered an unknown key 'static'`() {
    Assume.assumeFalse(runningFromBazel()) // Prefab 1.1.3 isn't available
    baseline
      .copy(prefabVersion = "1.1.3", prefabSchemaVersion = 1, moduleLibraryAbiStatic = true)
      .assertHasError(Regex(".*CXX1218.* Unexpected JSON token at offset .*: Encountered an unknown key 'static'."))
  }
  ...
  fun `CXX1219-Prebuilt directory does not contain`() {
    Assume.assumeFalse(runningFromBazel()) // Prefab 1.1.3 isn't available
    baseline
      .copy(prefabVersion = "1.1.3", prefabSchemaVersion = 1)
      .assertHasError(Regex(".*CXX1219.* Prebuilt directory does not contain.*"))
  }

  @Test
  fun `CXX1220-Miscellaneous Fatal Errors`() {
    baseline.copy(stl = "trash-stl").assertHasError(Regex(".*CXX1220.* invalid value for .*"))

    baseline.copy(moduleLibraryAbiStl = "trash-stl").assertHasError("[CXX1220] Unknown STL: trash-stl")

    baseline.copy(abi = "trash-abi").assertHasError(Regex(".*CXX1220.* Unknown ABI: trash-abi.*"))

    baseline.copy(moduleLibraryAbi = "trash-abi").assertHasError("[CXX1220] Unknown ABI: trash-abi")

    baseline.copy(prefabPackageVersion = "trash-version").assertHasError("[CXX1220] version must be compatible with CMake, if present")
  }
  ...
  fun `Lines after first error are logged as lifecycle`() {
    ...
    PassThroughRecordingLoggingEnvironment().use { logger ->
      reportErrors(stderr)
      assertThat(logger.errors).containsExactly("[CXX1214] User has minSdkVersion 10 but library was built for 11")
      assertThat(logger.lifecycles).containsExactly("C/C++: ", "C/C++: Line above is intentionally blank")
    }
  }
  ...
  fun fuzz() {
    ...
    randomPermutations.take(20).forEach {
  ...
  private val testRootFolder by lazy { tempFolder.newFolder().parentFile.parentFile.resolve("prefab-error") }
  ...
  private fun Permutation.runPrefab() {
    ...
    val proc = ProcessBuilder(args).directory(workingDir).redirectOutput(stdout).redirectError(stderr).start()
  ...
  private fun Permutation.writePrefabPackage() {
    if (hasLibrary) {
      ...
      moduleJson.writeText(jsonStringOf(ModuleMetadataV1(exportLibraries = emptyList(), libraryName = moduleLibraryName)))
    ...
    if (hasLibrary2) {
      ...
      moduleJson2.writeText(jsonStringOf(ModuleMetadataV1(exportLibraries = emptyList(), libraryName = moduleLibraryName2)))
      abiJson2.writeText(
        jsonStringOf(
          AndroidAbiMetadata(
  ...
  private fun Permutation.assertNoError() {
    ...
    PassThroughRecordingLoggingEnvironment().use { logger ->
      reportErrors(stderr)
      assertThat(logger.errorMessages.map { it.diagnosticCode }).isEmpty()
    }
  }
  ...
  private fun Permutation.assertHasError(vararg errors: String) {
    ...
    PassThroughRecordingLoggingEnvironment().use { logger ->
      reportErrors(stderr)
      assertThat(logger.errorMessages.map { it.text() }).containsExactlyElementsIn(errors)
    }
  }
