...
class ExtractNativeDebugMetadataTaskTest(private val debugSymbolLevel: DebugSymbolLevel?) {
  ...
  private val app = MinimalSubProject.app("com.example.app").appendToBuild("\n\nandroid.dynamicFeatures = [':feature']\n\n")
  private val feature =
  ...
  val project: GradleTestProject =
    GradleTestProject.builder().fromTestApp(multiModuleTestProject).setSideBySideNdkVersion(DEFAULT_NDK_SIDE_BY_SIDE_VERSION).create()
  ...
  fun testNativeDebugMetadataInBundle() {
    ...
    project.executor().with(BooleanOption.PACKAGE_NATIVE_DEBUG_METADATA_IN_APP_BUNDLE, false).run("app:$bundleTaskName")
  ...
  fun testNativeDebugMetadataInBundleWithAbiFilters() {
    ...
    val expectedExcludedFullEntries =
      listOf(
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/app.so.dbg",
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/feature.so.dbg",
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/appLib.so.dbg",
    ...
    val expectedExcludedSymbolTableEntries =
      listOf(
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/app.so.sym",
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/feature.so.sym",
        "$bundleEntryPrefix/$ABI_ARMEABI_V7A/appLib.so.sym",
    ...
    val expectedExcludedNativeLibEntries =
      listOf(
        "base/lib/$ABI_ARMEABI_V7A/app.so",
        "feature/lib/$ABI_ARMEABI_V7A/feature.so",
        "base/lib/$ABI_ARMEABI_V7A/appLib.so",
  ...
  fun testErrorIfCollidingNativeLibs() {
    ...
    try {
      ...
      assertThat(Throwables.getRootCause(e).message).startsWith("Multiple entries with same key")
      return
    }
  ...
  fun testTaskSkippedWhenNoNativeLibs() {
    ...
    val taskName =
      if (debugSymbolLevel == FULL) {
        "extractReleaseNativeDebugMetadata"
      } else {
        "extractReleaseNativeSymbolTables"
      }
    assertThat(result1.skippedTasks).containsAtLeastElementsIn(listOf(":app:$taskName", ":feature:$taskName"))
    ...
    assertThat(result2.skippedTasks).containsAtLeastElementsIn(listOf(":app:$taskName"))
    assertThat(result2.didWorkTasks).containsAtLeastElementsIn(listOf(":feature:$taskName"))
  }
  ...
  fun testErrorWhenInvalidString() {
    ...
    try {
      ...
      assertThat(Throwables.getRootCause(e).message)
        .startsWith("Unknown DebugSymbolLevel value 'INVALID'. Possible values are 'full', " + "'symbol_table', 'none'.")
      return
    }
  ...
  fun testNativeDebugMetadataInDefaultDebugBuild() {
    ...
    Zip(bundleFile).use { zip ->
      assertThat(zip.entries.map { it.toString() })
        .containsNoneIn(listOf("$bundleEntryPrefix/$ABI_ARMEABI_V7A/feature.so.dbg", "$bundleEntryPrefix/$ABI_ARMEABI_V7A/feature.so.sym"))
    }
  ...
  private fun createUnstrippedAbiFile(project: GradleTestProject, abiName: String, libName: String) {
    ...
    ExtractNativeDebugMetadataTaskTest::class.java.getResourceAsStream("/nativeLibs/unstripped.so").use { inputStream ->
  ...
  private fun createStrippedAbiFile(project: GradleTestProject, abiName: String, libName: String) {
    val abiFolder = File(project.getMainSrcDir("jniLibs"), abiName)
    FileUtils.mkdirs(abiFolder)
    ExtractNativeDebugMetadataTaskTest::class.java.getResourceAsStream("/nativeLibs/libhello-jni.so").use { inputStream ->
      File(abiFolder, libName).outputStream().use { outputStream -> inputStream.copyTo(outputStream) }
    }
  }
}
