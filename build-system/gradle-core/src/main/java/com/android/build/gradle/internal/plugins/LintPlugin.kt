...
abstract class LintPlugin : Plugin<Project> {
  ...
  @get:Inject abstract val listenerRegistry: BuildEventsListenerRegistry
  ...
  private fun registerTasks(
    ...
    val lintFixTask =
      project.tasks.register("lintFix", AndroidLintGlobalTask::class.java) {
        it.group = JavaBasePlugin.VERIFICATION_GROUP
        it.description = "Generates the lint report for project `${project.name}` and applies any safe suggestions to the source code."
      }
    ...
    project.afterEvaluate {
      ...
      val kmpJvmTargetNames: List<String>? =
        kotlinExtensionWrapper
          ?.kotlinExtension
          ?.targets
          ?.filter { it.platformType == KotlinPlatformType.jvm && it.name != KotlinMultiplatformAndroidPlugin.ANDROID_TARGET_NAME }
          ?.map { it.name }
          ?.sorted()
        ...
        project.tasks.register("updateLintBaselineJvm", AndroidLintTask::class.java) { task ->
          ...
          task.configureForStandalone(
            ...
            if (isPerComponentLintAnalysis && lintOptions!!.ignoreTestSources.not()) {
              artifacts.getAll(InternalMultipleArtifactType.UNIT_TEST_LINT_PARTIAL_RESULTS)
            } else {
              null
            },
            if (isPerComponentLintAnalysis && lintOptions!!.ignoreTestSources.not()) {
      ...
      project.tasks
        .register("lintReportJvm", AndroidLintTask::class.java) { task ->
      ...
      val lintVitalJvmTask =
        project.tasks.register("lintVitalJvm", AndroidLintTextOutputTask::class.java) { task ->
          task.configureForStandalone(taskCreationServices, artifacts, lintOptions!!, fatalOnly = true)
        }
      lintVitalTask.dependsOn(lintVitalJvmTask)
      project.tasks
        .register("lintVitalReportJvm", AndroidLintTask::class.java) { task ->
          task.description = "Generates the JVM lint report for just the fatal issues for project `${project.name}`"
          task.configureForStandalone(
            taskCreationServices,
            javaExtension,
            customLintChecks,
      ...
      if (isPerComponentLintAnalysis) {
        for (jvmTargetName in jvmTargetNames) {
          val lintAnalysisMainTask =
            project.tasks.register("lintAnalyze${jvmTargetName.usLocaleCapitalize()}Main", AndroidLintAnalysisTask::class.java) { task ->
              task.description = "Runs JVM lint analysis for main component of project `${project.name}`"
              task.configureForStandalone(
                taskCreationServices,
                javaExtension,
                kotlinExtensionWrapper,
          ...
          val lintModelWriterMainTask =
            project.tasks.register("generate${jvmTargetName.usLocaleCapitalize()}MainLintModel", LintModelWriterTask::class.java) { task ->
              task.configureForStandalone(
                taskCreationServices,
                javaExtension,
                kotlinExtensionWrapper,
          ...
          LintModelWriterTask.registerOutputArtifacts(lintModelWriterMainTask, LINT_REPORT_LINT_MODEL, artifacts)
          publishLintArtifact(
            project,
          ...
          if (lintOptions!!.ignoreTestSources.not()) {
            ...
            val standardJvmEnvironment = project.objects.named(TargetJvmEnvironment::class.java, TargetJvmEnvironment.STANDARD_JVM)
            val testCompileClasspath =
              project.configurations.create("${jvmTargetName}TestCompileClasspathForLint").apply {
                isCanBeConsumed = false
                isCanBeResolved = true
                attributes.attribute(Category.CATEGORY_ATTRIBUTE, libraryCategory)
            ...
            val testRuntimeClasspath =
              project.configurations.create("${jvmTargetName}TestRuntimeClasspathForLint").apply {
                isCanBeConsumed = false
                isCanBeResolved = true
                attributes.attribute(Category.CATEGORY_ATTRIBUTE, libraryCategory)
            ...
            val lintAnalysisTestTask =
              project.tasks.register("lintAnalyze${jvmTargetName.usLocaleCapitalize()}Test", AndroidLintAnalysisTask::class.java) { task ->
                task.description = "Runs JVM lint analysis for test component of project `${project.name}`"
                task.configureForStandalone(
                  taskCreationServices,
                  javaExtension,
                  kotlinExtensionWrapper,
            ...
            val lintModelWriterTestTask =
              project.tasks.register("generate${jvmTargetName.usLocaleCapitalize()}TestLintModel", LintModelWriterTask::class.java) { task
                ->
                task.configureForStandalone(
                  taskCreationServices,
                  javaExtension,
          ...
          val lintVitalAnalysisMainTask =
            project.tasks.register("lintVitalAnalyze${jvmTargetName.usLocaleCapitalize()}Main", AndroidLintAnalysisTask::class.java) { task
              ->
              task.description = "Runs JVM lint analysis on just the fatal issues for main component of project `${project.name}`"
              task.configureForStandalone(
                taskCreationServices,
                javaExtension,
                kotlinExtensionWrapper,
          ...
          LintModelWriterTask.registerOutputArtifacts(lintVitalModelWriterMainTask, LINT_VITAL_REPORT_LINT_MODEL, artifacts)
          publishLintArtifact(
            project,
        ...
        AndroidLintAnalysisTask.registerOutputArtifacts(lintAnalysisTask, InternalMultipleArtifactType.LINT_PARTIAL_RESULTS, artifacts)
        publishLintArtifact(
          project,
        ...
        val lintVitalAnalysisTask =
          project.tasks.register("lintVitalAnalyzeJvm", AndroidLintAnalysisTask::class.java) { task ->
            task.description = "Runs JVM lint analysis on just the fatal issues for project `${project.name}`"
            task.configureForStandalone(
              taskCreationServices,
              javaExtension,
              kotlinExtensionWrapper = null,
        ...
        val lintModelWriterTask =
          project.tasks.register("generateJvmLintModel", LintModelWriterTask::class.java) { task ->
            task.configureForStandalone(
              taskCreationServices,
              javaExtension,
              kotlinExtensionWrapper = null,
              lintOptions!!,
              artifacts.getOutputPath(InternalMultipleArtifactType.LINT_PARTIAL_RESULTS, "lintAnalyzeJvm"),
              lintModelArtifactType = null,
              fatalOnly = false,
              jvmTargetName = null,
            )
          }
        LintModelWriterTask.registerOutputArtifacts(lintModelWriterTask, LINT_REPORT_LINT_MODEL, artifacts)
        publishLintArtifact(
          project,
        ...
        val lintVitalModelWriterTask =
          project.tasks.register("generateLintVitalJvmLintModel", LintModelWriterTask::class.java) { task ->
            task.configureForStandalone(
              taskCreationServices,
              javaExtension,
              kotlinExtensionWrapper = null,
              lintOptions!!,
              artifacts.getOutputPath(InternalMultipleArtifactType.LINT_VITAL_PARTIAL_RESULTS, "lintVitalAnalyzeJvm"),
              lintModelArtifactType = null,
              fatalOnly = true,
              jvmTargetName = null,
            )
          }
        LintModelWriterTask.registerOutputArtifacts(lintVitalModelWriterTask, LINT_VITAL_REPORT_LINT_MODEL, artifacts)
        publishLintArtifact(
          project,
      ...
      val lintModelMetadataWriterTask =
        project.tasks.register("writeJvmLintModelMetadata", LintModelMetadataTask::class.java) { task ->
      ...
      if (LintTaskManager.needsCopyReportTask(lintOptions!!)) {
        val copyLintReportsTask =
          project.tasks.register("copyJvmLintReports", AndroidLintCopyReportTask::class.java) { task ->
      ...
      if (kotlinExtensionWrapper == null) {
        javaExtension.sourceSets.getByName(SourceSet.MAIN_SOURCE_SET_NAME) { mainSourceSet ->
          listOf(mainSourceSet.runtimeElementsConfigurationName, mainSourceSet.apiElementsConfigurationName).forEach { configurationName ->
              ...
              project.components.configureEach { component: SoftwareComponent ->
                if (component.name == "java" && component is AdhocComponentWithVariants) {
                  component.withVariantsFromConfiguration(configuration) { variant: ConfigurationVariantDetails ->
                    val category = variant.configurationVariant.attributes.getAttribute(Category.CATEGORY_ATTRIBUTE)
  ...
  private fun publishLintArtifact(
    ...
    val configurationNames: List<String> =
      if (kotlinExtensionWrapper == null) {
        ...
        listOf(mainSourceSet.runtimeElementsConfigurationName, mainSourceSet.apiElementsConfigurationName)
      } else {
        val jvmTarget = jvmTargetName?.let { kotlinExtensionWrapper.kotlinExtension.targets.findByName(it) }
        val jvmMainCompilation = jvmTarget?.compilations?.findByName("main")
        if (jvmMainCompilation != null) {
          listOf(jvmTarget.apiElementsConfigurationName, jvmTarget.runtimeElementsConfigurationName)
        } else {
          emptyList()
        }
      }
    configurationNames.forEach { configurationName ->
      project.configurations.getByName(configurationName) { configuration ->
        publishArtifactToConfiguration(
          ...
          provider,
          artifactType,
          artifactType.getAttributes { type, name -> projectServices.objectFactory.named(type, name) },
        )
      }
    }
  }

  private fun createExtension(project: Project, dslServices: DslServicesImpl): DslLifecycleComponentsOperationsRegistrar<Lint> {
    ...
    val decoratedLintOptionsClass = androidPluginDslDecorator.decorate(LintOptions::class.java)
    ...
    project.extensions.create("lintLifecycle", LintLifecycleExtensionImpl::class.java, dslOperationsRegistrar)
  ...
  private fun getJavaPluginExtension(project: Project): JavaPluginExtension? {
    ...
    if (javaExtension == null) {
      project.logger.warn("Cannot apply lint if the java or kotlin Gradle plugins " + "have not also been applied")
    }
  ...
  private fun createProjectServices(project: Project) {
    ...
    val deprecationReporter = DeprecationReporterImpl(syncIssueReporter, projectOptions, projectPath)
    ...
    projectOptions.allOptions.forEach { (option: Option<*>, value: Any) ->
  ...
  private fun registerBuildServices(project: Project) {
    ...
    val androidProblemReporterProvider = AndroidProblemReporterProvider.RegistrationAction(project, projectOptions).execute()
