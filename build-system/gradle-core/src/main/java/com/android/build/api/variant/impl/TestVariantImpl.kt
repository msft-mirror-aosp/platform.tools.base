...
import com.android.build.gradle.internal.utils.TestApkOutput
...
constructor(
  ...
  override val applicationId: Property<String> = internalServices.propertyOf(String::class.java, dslInfo.applicationId)
  ...
  override val packaging: ApkPackaging by lazy { ApkPackagingImpl(dslInfo.packaging, internalServices, minSdk.apiLevel) }

  override val renderscript: Renderscript? by lazy { renderscriptCreationConfig?.renderscript }
  override val testedApks: Provider<Directory> by lazy { getTestedModuleDirectoryArtifact(AndroidArtifacts.ArtifactType.APK) }
  ...
  private fun getTestedModuleAllDirectoryArtifacts(artifactType: AndroidArtifacts.ArtifactType): Provider<List<Directory>> {
    val projectDirectory = services.projectInfo.projectDirectory
    return variantDependencies
      .getArtifactFileCollection(AndroidArtifacts.ConsumedConfigType.PROVIDED_CLASSPATH, AndroidArtifacts.ArtifactScope.ALL, artifactType)
      .elements
  ...
  override val targetSdk: AndroidVersion by
    lazy(LazyThreadSafetyMode.NONE) { global.androidTestOptions.targetSdkVersion ?: variantBuilder.targetSdkVersion }
  ...
  override val signingConfig: SigningConfigImpl? by lazy {
    dslInfo.signingConfigResolver?.let {
      SigningConfigImpl(it.resolveConfig(profileable = false, debuggable), internalServices, minSdk.apiLevel, global.targetDeployApiFromIDE)
    }
  }
  ...
  private fun calculateTestedApplicationId(variantDependencies: VariantDependencies): Provider<String> {
    return variantDependencies
      ...
      .map {
        if (it.isEmpty()) {
          throw RuntimeException(
            "The tested application ID could not be computed. " +
  ...
  override fun <T : Component> createUserVisibleVariantObject(stats: GradleBuildVariant.Builder?): T =
    if (stats == null) {
      ...
      services.newInstance(AnalyticsEnabledTestVariant::class.java, this, stats) as T
    }
    ...
    get() =
      object : ApkOutputProviders {
        override fun <TaskT : Task> provideApkOutputToTask(
          ...
          val apkOutput = TestApkOutput(this@TestVariantImpl, deviceSpec)
          taskProvider.configure { task ->
            apkOutput.setTaskInputs(task.inputs)
