...
import com.android.build.gradle.internal.coverage.renderer.xmlparser.utils.ATTR_VALUE
 ...
 *
 * This object is designed to parse multiple XML files, accumulating the results into the
 * collections provided to the [transform] method.
 */
object XMLTransformer {
    ...
    fun transform(
        ...
        val moduleReportBuilder = coverageBuilder.moduleReportBuilders.getOrPut(context.moduleName) {
            ModuleReportBuilder(context.moduleName)
        }
        ...
        for (packageNode in rootElement.getElementsByTagName(TAG_PACKAGE).elements) {
            parsePackage(packageNode, context, projectBaseDir, moduleReportBuilder, testSuiteBuilder, sourceFileReportsBuilder, sourceFileLocations)
        }
    }
    ...
    private fun updateAggregatedCoverages(
        ...
        val projectCoverage = aggregatedVariantCoverages.getOrPut(context.variantName) {
        ...
        aggregatedVariantCoverages[context.variantName] = projectCoverage.copy(
            instruction = CoverageInfo(calculatePercent(newCovered, newTotal), newCovered, newTotal)
        )

        if (context.testSuiteName == VALUE_AGGREGATED) {
            moduleReportBuilder.variantCoverages.add(
                VariantCoverage(context.variantName, overallCoverage.instruction, overallCoverage.branch)
            )
        }
    }
    ...
    private fun getOrCreateTestSuiteBuilder(
        ...
        return moduleReportBuilder.testSuites.getOrPut(context.testSuiteName) {
            TestSuiteReportBuilder(context.testSuiteName, context.moduleName)
        }.also {
            it.variantCoverages.add(
                VariantCoverage(context.variantName, overallCoverage.instruction, overallCoverage.branch)
            )
        }
    }
    ...
    private fun parsePackage(
        ...
        val packageName = packageNode.getAttribute(ATTR_NAME).replace('/', '.')
            .takeIf { it.isNotEmpty() } ?: VALUE_DEFAULT
        val packageCounters = parseCoverageCounters(packageNode)
        val packageVariantCoverage = VariantCoverage(
            context.variantName, packageCounters.instruction, packageCounters.branch
        )
        ...
        val packageReportBuilder = if (testSuiteBuilder != null) {
            testSuiteBuilder.packages.getOrPut(packageName) {
                PackageReportBuilder(packageName, context.moduleName, context.testSuiteName)
            }
        } else {
            moduleReportBuilder.packages.getOrPut(packageName) {
                PackageReportBuilder(packageName, context.moduleName, context.testSuiteName)
            }
        }
    ...
    private fun parseClass(
        ...
        val className = classNode.getAttribute(ATTR_NAME).substringAfterLast('/')
            .takeIf { it.isNotEmpty() } ?: VALUE_DEFAULT
        val sourceFileName = classNode.getAttribute(ATTR_SOURCE_FILENAME)
        val classCounters = parseCoverageCounters(classNode)
        val classVariantCoverage = VariantCoverage(
            context.variantName, classCounters.instruction, classCounters.branch
        )

        val classReportBuilder = packageReportBuilder.classes.getOrPut(className) {
            ClassReportBuilder(className, packageName, sourceFileName, testSuiteName)
    ...
    private fun parseMethod(
        methodNode: Element,
        variantName: String,
        classReportBuilder: ClassReportBuilder
    ) {
        val methodName = (methodNode.getAttribute(ATTR_NAME) + methodNode.getAttribute(ATTR_DESC))
            .takeIf { it.isNotEmpty() } ?: VALUE_DEFAULT
        val methodLine = methodNode.getAttribute(ATTR_LINE).toIntOrNull() ?: 0

        classReportBuilder.methods.getOrPut(methodName) {
            MethodReportBuilder(methodName)
        }.variantLineNumbers.add(VariantLineNumber(variantName, methodLine))
    }
    ...
    private fun parseSourceFile(
        ...
        val pathPrefixToPackage = if (packagePath.isNotEmpty()) {
        ...
        val sourceFileBuilder = sourceFileReportsBuilder.sourceFileBuilders.getOrPut(relativePath) {
            SourceFileReportBuilder(packageFlattenedPath)
        }
        val variantBuilder = sourceFileBuilder.variantFileCoverageBuilders.getOrPut(context.variantName) {
            VariantFileCoverageBuilder()
        }

        val sourceFileCounters = parseCoverageCounters(sourceFileNode)
        val testSuiteBuilder = variantBuilder.testSuiteFileCoverageBuilders.getOrPut(context.testSuiteName) {
        ...
        sourceFileNode.getElementsByTagName(TAG_LINE).elements.forEach { lineNode ->
            ...
            testSuiteBuilder.lineCoverageBuilders[lineNumber] =
                LineCoverageBuilder(
                    instructionCoverageInfo = CoverageInfo(
                        percent = calculatePercent(
                            coveredInstructions,
                            coveredInstructions + missedInstructions
                        ),
                        covered = coveredInstructions,
                        total = coveredInstructions + missedInstructions
                    ),
                    branchCoverageInfo = CoverageInfo(
                        percent = calculatePercent(
                            coveredBranches,
    ...
    private fun updateSourceFilePathsInClassBuilders(
        ...
        packageReportBuilder.classes.values.forEach { classBuilder ->
            ...
            if (path != null) {
                classBuilder.variantSourceFilePaths.add(
                    VariantSourceFilePath(variantName, path)
                )
            }
        }
    }
    ...
    private fun parseSourceFileLocations(rootElement: Element): List<String> {
        ...
        return sourcesNode?.getElementsByTagName(TAG_FILE)?.elements?.map { it.getAttribute(
            ATTR_PATH
        ) } ?: emptyList()
    }

    private fun locateSourceFile(
        sourceRoots: List<String>,
        projectBaseDir: File,
        packagePath: String,
