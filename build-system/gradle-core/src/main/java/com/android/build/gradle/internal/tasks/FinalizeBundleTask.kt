...
abstract class FinalizeBundleTask : NonIncrementalTask(), SigningInputs {
  ...
  @get:OutputFile abstract val finalBundleFile: RegularFileProperty
  ...
  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(BundleToolRunnable::class.java) {
      ...
      signingConfigData?.convertToParams()?.let { signing -> it.signingConfig.set(signing) }
  ...
  abstract class BundleToolRunnable : ProfileAwareWorkAction<Params>() {
    ...
    private fun addCodeTransparencySigning(inputFile: File, outputFile: File, codeSigning: SigningConfigData) {
    ...
    override fun run() {
      ...
      var cleanup = {}
      try {
        parameters.codeTransparencySigningConfig.orNull?.resolve()?.let { codeSigningConfig ->
          val outputFile = FileUtils.join(parameters.tmpDir.get().asFile, inputFile.name)
        ...
        parameters.signingConfig.orNull?.resolve()?.let {
          ...
          val certificateInfo =
            KeystoreHelper.getCertificateInfo(it.storeType, it.storeFile!!, it.storePassword!!, it.keyPassword!!, keyAlias)
          AabFlinger(
            ...
            .use { aabFlinger -> aabFlinger.writeZip(inputFile, Deflater.DEFAULT_COMPRESSION) }
        } ?: run { compressBundle(inputFile, parameters.finalBundleFile.asFile.get()) }
      } finally {
        cleanup()
      }
    }
  ...
  class CreationForAssetPackBundleAction(
    ...
    override fun handleProvider(taskProvider: TaskProvider<FinalizeBundleTask>) {
      super.handleProvider(taskProvider)

      val bundleNameProvider = projectServices.projectInfo.getProjectBaseName().map { "$it${if (!isSigningReady) "-unsigned" else ""}.aab" }
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<FinalizeBundleTask, ApkCreationConfig>(creationConfig) {
    ...
    override fun configure(task: FinalizeBundleTask) {
      ...
      creationConfig.bundleConfig?.codeTransparency?.signingConfiguration?.let { codeSigning ->
        if (codeSigning.storeFile != null && codeSigning.keyAlias != null) {
          task.codeTransparencySigningConfigData = SigningConfigData.fromDslSigningConfig(codeSigning)
        }
      }

      task.tmpDir.setDisallowChanges(creationConfig.paths.intermediatesDir("tmp", "FinalizeBundle", creationConfig.dirName))
    }
  }
}
