...
/**
 * Build service used to (1) cache lint classloader jar hashes during a build to avoid rehashing the jars repeatedly, and (2) remove locks
 * from jar files from lint's [URLClassLoader] after all lint tasks have finished executing.
 */
abstract class LintClassLoaderBuildService : BuildService<BuildServiceParameters.None>, AutoCloseable {

  // Whether or not to call AndroidLintWorkAction.dispose() in close() method.
  @get:Synchronized @set:Synchronized var shouldDispose: Boolean = false
  ...
  internal fun hashJars(classpath: Iterable<FileSystemLocation>): String {
    ...
    val hashCode =
      jarsToHashCode.getOrPut(uris) { Hashing.combineOrdered(classpath.map { Files.asByteSource(it.asFile).hash(Hashing.murmur3_128()) }) }
  ...
  class RegistrationAction(project: Project) :
    ServiceRegistrationAction<LintClassLoaderBuildService, BuildServiceParameters.None>(project, LintClassLoaderBuildService::class.java) {
