...
open class DefaultSourcesProviderImpl(val component: ComponentCreationConfig, val variantSources: VariantSources) : DefaultSourcesProvider {
  ...
  override fun getShaders(lateAdditionsDelegate: LayeredSourceDirectoriesImpl): List<DirectoryEntries>? =
    if (component.buildFeatures.shaders) getSourceList(lateAdditionsDelegate) { sourceProvider -> sourceProvider.shaders } else null
  ...
  /**
   * Computes the default java sources: source sets and generated sources. For access to the final list of java sources, use
  ...
  private fun ComponentCreationConfig.defaultJavaSources(lateAdditionsDelegate: SourceDirectoriesImpl): List<DirectoryEntry> {
    ...
    sourceSets.addAll(flattenSourceProviders(lateAdditionsDelegate) { sourceSet -> sourceSet.java })

    // for the other, there's no duplicate so no issue.
    if (buildConfigCreationConfig?.buildConfigType == BuildConfigType.JAVA_SOURCE && this !is HostTestCreationConfig) {
      sourceSets.add(
        TaskProviderBasedDirectoryEntryImpl("generated_build_config", artifacts.get(InternalArtifactType.GENERATED_BUILD_CONFIG_JAVA))
      )
    }
    if (this is ConsumableCreationConfig && buildFeatures.aidl) {
      sourceSets.add(TaskProviderBasedDirectoryEntryImpl("generated_aidl", artifacts.get(InternalArtifactType.AIDL_SOURCE_OUTPUT_DIR)))
    }
  ...
  private fun ComponentCreationConfig.defaultResSources(lateAdditionsDelegate: SourceDirectoriesImpl): List<DirectoryEntries> {
    ...
    sourceDirectories.addAll(getSourceList(lateAdditionsDelegate) { sourceProvider -> sourceProvider.res })
    ...
    if (buildFeatures.resValues) {
      generatedFolders.add(
        TaskProviderBasedDirectoryEntryImpl(name = "generated_res", directoryProvider = artifacts.get(InternalArtifactType.GENERATED_RES))
      )
    }
