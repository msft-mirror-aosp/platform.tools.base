 ...
 *
 * This interface provides various ways to specify the Android API level that the project will be compiled against, including release
 * versions, preview versions, and versions with extensions or addons.
 *
 * This interface, along with its implementation, serves as a delegate to share common property implementations between the standard Android
 * Gradle Plugin extensions and the Kotlin Multiplatform (KMP) Android target extension.
 */
...
internal class CompileSdkDelegate(
  ...
  override fun compileSdk(action: CompileSdkSpec.() -> Unit) {
    createCompileSdkSpec().also {
      action.invoke(it)
      updateIfChanged(getCompileSdk(), it.version) { setCompileSdk(it) }
    }
  }
  ...
  private fun createCompileSdkSpec(): CompileSdkSpecImpl {
    return dslServices.newDecoratedInstance(CompileSdkSpecImpl::class.java, dslServices).also { it.version = getCompileSdk() }
  }
 ...
 *
 * This interface defines the minimum API level that the module's code is compatible with. It provides methods to set this value using
 * either a stable API level or a preview codename.
 *
 * This interface, along with its implementation, serves as a delegate to share common property implementations between the standard Android
 * Gradle Plugin extensions and the Kotlin Multiplatform (KMP) Android target extension.
 */
...
class MinSdkDelegate(
  ...
  override fun minSdk(action: MinSdkSpec.() -> Unit) {
    createMinSdkSpec().also {
      action.invoke(it)
      updateIfChanged(getMinSdk(), it.version) { setMinSdk(it) }
    }
  }
  ...
  private fun createMinSdkSpec(): MinSdkSpecImpl {
    return dslServices.newDecoratedInstance(MinSdkSpecImpl::class.java, dslServices).also { it.version = getMinSdk() }
  }
}
