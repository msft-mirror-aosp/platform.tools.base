...
data class ExecuteProcessResult(val stdout: File, val stderr: File)

/**
 * Execute a process of type [ExecuteProcessType] in the context of this [CxxAbiModel].
 *
 * This is a wrapper over [ExecuteProcessCommand::executeProcess] that is specific to the C\C++ build system such as:
 ...
 * - The process is timed and the result is stored in a location that is per-ABI and per-[ExecuteProcessType].
 *
 * [processType] defines what sort of CXX build system process it is. This is mainly to control the names of log files and whether output
 * should be elevated to 'lifecycle' due to gradle command-line flags.
 *
 * [command] defines executable, arguments, etc. of the process to launch.
 *
 * [logFileSuffix] if present, will be added to the end of the log file name. This is to designate a subcategory. For example, when building
 * C++, the suffix may be the name of the individual target that is being built.
 *
 * [processStderr] and [processStdout] are optional post-process functions. When present, output will not be logged to gradle build output.
 * Instead, the function ([processStderr] or [processStdout]) will be called with a file containing the output. These functions will not be
 * called if a build process exception was thrown. Instead, they will be appended to the [ProcessException] message text.
 */
 ...
 *
 * [classPath] is the list of jars or other entities that will be passed to java.exe via the --class-path flag.
 *
 * [main] is the name of the main entry point to use.
 *
 * [javaExe] is the path to java.exe to use. If not specified then the current JVM's java.exe will be used.
 */
 ...
 *
 * [commandFile] path to a shell script (like command.bat) that the command will be written to. If [useScript] is true then this is also the
 ...
 *
 * [verbose] if true, then STDOUT will be logged to lifecycle which is visible even if the user didn't pass ---info flag.
 *
 * [useScript] if true, then a shell script is written containing the command and the shell script is executed. This will enable
 * shell-specific behaviors such as 'my-script' matching 'my-script.bat'.
 */
...
private fun ExecuteProcessCommand.logResult(exitValue: Int) {
  logStructured { encoder ->
    with(encoder) {
      EncodedExecuteProcess.newBuilder()
        .apply {
...
/**
 * Construct a [ProcessException] error message. Weird indent is because [stdout] and [stderr] expand to lines without indent, so we can't
 * use [String::trimIndent].
 */
private val ExecuteProcessCommand.processErrorMessage
  get() =
    """
${stdout.readText()}
