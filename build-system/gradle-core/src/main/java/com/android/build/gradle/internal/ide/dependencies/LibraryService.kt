 ...
 *
 * This is used to share instances of string as much as possible (similar to [String.intern] which has performance issues.
 */
 ...
 *
 * Provided a [File] to an exploded AAR, this goes through the File system to find local jars in the folder and cache the result so that
 * subsequent calls for the same folder does not have to do IO again.
 */
...
class LibraryServiceImpl(private val libraryCache: LibraryCache) : LibraryService {
  ...
  override fun getLibrary(artifact: ResolvedArtifact, additionalArtifacts: AdditionalArtifacts) =
    libraryCache.getLibrary(artifact, additionalArtifacts).also { libraries.putIfAbsent(it.key, it) }
}

class LibraryCacheImpl(private val stringCache: StringCache, private val localJarCache: LocalJarCache) : LibraryCache {
  ...
  override fun getLibrary(artifact: ResolvedArtifact, additionalArtifacts: AdditionalArtifacts): Library =
    synchronized(libraryCache) { libraryCache.computeIfAbsent(artifact) { createLibrary(it, additionalArtifacts) } }
  ...
  private fun getProjectInfo(variant: ResolvedVariantResult): ProjectInfoImpl =
    projectInfoCache.computeIfAbsent(variant) {
      ...
      ProjectInfoImpl(
        ...
        it.capabilities.any { capability -> capability.isProjectTestFixturesCapability(component.projectName) },
      )
    }
      ...
      is ModuleComponentIdentifier -> {
        ...
        libraryInfoCache.computeIfAbsent(artifact.variant) {
          ...
          LibraryInfoImpl(
            ...
            it.capabilities.any { capability -> capability.isLibraryTestFixturesCapability(libraryName = component.module) },
          )
        }
      }
   ...
   * Handles an artifact.
   *
   * This optionally returns the model item that represents the artifact in case something needs use the return
   */
  private fun createLibrary(artifact: ResolvedArtifact, additionalArtifacts: AdditionalArtifacts): Library {
    ...
    return if (id !is ProjectComponentIdentifier || artifact.isWrappedModule) {
      ...
      when (artifact.dependencyType) {
        ResolvedArtifact.DependencyType.ANDROID -> {
          ...
          val runtimeJar = FileUtils.join(folder, SdkConstants.FD_JARS, SdkConstants.FN_CLASSES_JAR)
        ...
        ResolvedArtifact.DependencyType.RELOCATED_ARTIFACT -> {
          LibraryImpl.createRelocatedLibrary(stringCache.cacheString(libraryInfo.computeKey()), libraryInfo)
        }
      ...
      LibraryImpl.createNoArtifactProjectLibrary(stringCache.cacheString(projectInfo.computeKey()), projectInfo)
    } else {
      ...
      val projectInfo = getProjectInfo(artifact.variant)
      val artifactFile = artifact.artifactFile?.takeIf { it.extension == "jar" }
  ...
  private fun getBuildType(variant: ResolvedVariantResult): String? =
    variant.attributes
      ...
      ?.let { buildType -> variant.attributes.getAttribute(buildType)?.toString()?.let { stringCache.cacheString(it) } }
