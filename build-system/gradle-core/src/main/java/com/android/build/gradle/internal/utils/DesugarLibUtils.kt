...
val ATTR_ENABLE_CORE_LIBRARY_DESUGARING: Attribute<String> = Attribute.of("enable-core-library-desugaring", String::class.java)
...
/**
 * Returns the dependency graph resolved for [getDesugarLibConfiguration] which contains the metadata(group/name/version) of desugar lib.
 */
...
fun getDesugaredDesugarLib(creationConfig: ApkCreationConfig): FileCollection {
  ...
  return getDesugarLibConfiguration(creationConfig.services)
    .incoming
    .artifactView {
      it.componentFilter { id -> !id.displayName.contains(DESUGAR_LIB_COMPONENT_NAME) }
      L8DesugarLibTransformRegistration.getOutputArtifactAttributes(creationConfig).addAttributesToContainer(it.attributes)
    }
    .artifacts
    .artifactFiles
}
...
/**
 * Returns a provider which represents the content of desugar.json file extracted from desugar lib configuration jars
 *
 * IMPORTANT: DO NOT USE this method to set up a transform's input as this method uses [ValueSource] which might cause deadlock when used as
 * a transform input (see bug 329346760); use [getDesugarLibConfigFiles] instead.
 *
 * This method can still be used to set up a task's input (the deadlock mentioned above seems to happen only to [ValueSource] being used
 * transform inputs, not task inputs).
 */
...
/** Returns a [FileCollection] which contains files with desugared methods supported by D8 and core library desugaring. */
fun getDesugaredMethods(
  ...
  desugaredMethodsFiles.fromDisallowChanges(getD8DesugarMethodFileFromTransform(global.fakeDependency, coreLibDesugar, minSdk))
  return desugaredMethodsFiles
}

/**
 * Returns the configuration of core library to be desugared and throws runtime exception if the user didn't add any dependency to that
 * configuration.
 *
...
private fun getDesugarLibConfigFromTransform(configuration: Configuration): FileCollection {
  return configuration.incoming
    .artifactView { configuration -> configuration.attributes { it.attribute(ARTIFACT_TYPE_ATTRIBUTE, DESUGAR_LIB_CONFIG) } }
    .artifacts
    .artifactFiles
}
...
private fun getArtifactCollection(configuration: Configuration): FileCollection =
  configuration.incoming
    .artifactView { config -> config.attributes { it.attribute(ARTIFACT_TYPE_ATTRIBUTE, ArtifactTypeDefinition.JAR_TYPE) } }
    .artifacts
    .artifactFiles
...
private fun getDesugarLibLintFromTransform(configuration: Configuration, minSdkVersion: Int, compileSdkVersion: Int): FileCollection {
  return configuration.incoming
    .artifactView { configuration ->
      configuration.attributes {
        it.attribute(ARTIFACT_TYPE_ATTRIBUTE, DESUGAR_LIB_LINT)
...
private fun getD8DesugarMethodFileFromTransform(configuration: Configuration, coreLibDesugar: Boolean, minSdkVersion: Int): FileCollection {
  return configuration.incoming
    .artifactView { configuration ->
      configuration.attributes {
...
fun parseDesugarJdkVariant(variant: String): DesugarJdkVariant {
  return when (variant) {
    ...
    DESUGAR_JDK_LIBS_NIO -> DesugarJdkVariant.NIO
      ...
      throw RuntimeException(
        "Failed to parse desugar_jdk_libs variant from" + " desugarJdkLib property of AAR metadata: unknown variant '$variant' "
      )
  }
}
...
abstract class DesugarLibConfigExtractor : TransformAction<TransformParameters.None> {
  ...
  @get:PathSensitive(PathSensitivity.NONE) @get:InputArtifact abstract val inputArtifact: Provider<FileSystemLocation>
    ...
    ZipInputStream(inputFile.inputStream().buffered()).use { zipInputStream ->
      while (true) {
        ...
        if (entry.name.endsWith(DESUGAR_LIB_CONFIG_FILE)) {
          ...
          Files.newOutputStream(outputFile.toPath()).buffered().use { output -> ByteStreams.copy(zipInputStream, output) }
          break
        }
      }
    }
...
/** Extract the specific lint file with desugared APIs based on minSdkVersion & compileSdkVersion from desugar lib configuration jar. */
@CacheableTransform
abstract class DesugarLibLintExtractor : TransformAction<DesugarLibLintExtractor.Parameters> {
  ...
  interface Parameters : GenericTransformParameters {
    ...
    @get:Input val compileSdkVersion: Property<Int>
  }

  @get:InputArtifact @get:PathSensitive(PathSensitivity.NONE) abstract val inputArtifact: Provider<FileSystemLocation>
...
/** Generate a file of D8 backported desugared methods by invoking D8 API instead of processing input artifact. */
@CacheableTransform
abstract class D8BackportedMethodsGenerator : TransformAction<D8BackportedMethodsGenerator.Parameters> {

  interface Parameters : GenericTransformParameters {

    @get:Input val d8Version: Property<String>

    @get:Input val minSdkVersion: Property<Int>

    @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) val desugarLibConfigFiles: ConfigurableFileCollection

    @get:CompileClasspath @get:Optional val bootclasspath: ConfigurableFileCollection
  }
  ...
  override fun transform(outputs: TransformOutputs) {
    ...
    outputFile.printWriter().use {
      D8DesugaredMethodsGenerator.generate(
        ...
        .forEach { method -> it.println(method) }
    }
  }
}
