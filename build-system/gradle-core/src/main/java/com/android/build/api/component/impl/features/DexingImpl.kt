...
class DexingImpl(
  ...
  override fun finalizeAndLock() {
    if (multiDexKeepProguard.isPresent) {
      component.artifacts.getArtifactContainer(MultipleArtifact.MULTIDEX_KEEP_PROGUARD).addInitialProvider(multiDexKeepProguard)
    }
  }

  override val multiDexKeepProguard: RegularFileProperty =
    internalServices.regularFileProperty().also { regularFileProperty ->
      multiDexProguardFile?.let { regularFileProperty.set(internalServices.projectInfo.projectDirectory.file(it.absolutePath)) }
    }

  override val multiDexKeepFile: RegularFileProperty =
    internalServices.regularFileProperty().also { regularFileProperty ->
      multiDexKeepFile?.let { regularFileProperty.set(internalServices.projectInfo.projectDirectory.file(it.absolutePath)) }
    }

  override val needsMainDexListForBundle: Boolean
    get() = component.componentType.isBaseModule && component.global.hasDynamicFeatures && dexingType.isLegacyMultiDex

  /**
   * Package desugar_lib DEX for base feature androidTest only if the base packages shrunk desugar_lib. This should be fixed properly by
   * analyzing the test code when generating L8 keep rules, and thus packaging desugar_lib dex in the tested APK only which contains
  ...
  override val isCoreLibraryDesugaringEnabled: Boolean
    get() {
      if (
        component is DeviceTestCreationConfig && component.services.projectOptions.get(BooleanOption.ENABLE_INSTRUMENTATION_TEST_DESUGARING)
      ) {
        return true
      }
  ...
  /**
   * Returns the minimum SDK version which we want to use for dexing. In most cases this will be equal the minSdkVersion, but when the IDE
   * is deploying to:
   ...
   * - there is no device, the min sdk version for dexing is minSdkVersion It is used to enable some optimizations to build the APK faster.
   *
