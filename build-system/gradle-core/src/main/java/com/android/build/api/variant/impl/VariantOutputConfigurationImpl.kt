...
data class VariantOutputConfigurationImpl(
  @get:Input val isUniversal: Boolean = false,
  @get:Nested override val filters: Collection<FilterConfigurationImpl> = listOf(),
  ...
  override val outputType: OutputType
    get() {
      ...
      return if (filters.isEmpty()) OutputType.SINGLE else OutputType.ONE_OF_MANY
    }
}
...
fun VariantOutputConfiguration.baseName(component: ComponentCreationConfig): String =
  when (this.outputType) {
    ...
    OutputType.UNIVERSAL -> component.paths.computeBaseNameWithSplits(OutputType.UNIVERSAL.name.lowercase(Locale.US))
    OutputType.ONE_OF_MANY -> component.paths.computeBaseNameWithSplits(this.filters.getFilterName())
  }
...
fun VariantOutputConfiguration.fullName(component: ComponentCreationConfig): String {
  return when (this.outputType) {
    OutputType.UNIVERSAL -> component.paths.computeFullNameWithSplits(OutputType.UNIVERSAL.name.lowercase(Locale.US))
    OutputType.SINGLE -> component.name
...
fun Collection<FilterConfiguration>.joinToString() = this.joinToString { filter -> filter.identifier }
