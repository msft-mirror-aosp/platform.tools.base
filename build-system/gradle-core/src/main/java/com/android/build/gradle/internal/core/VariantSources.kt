...
internal constructor(
   ...
   *
   * Note: Avoid calling this method at configuration time because the final path to the manifest file may change during that time.
   */
  val mainManifestFilePath: File
    get() = defaultSourceProvider.manifestFile

  /**
   * Returns the path to the main manifest file if it exists, or `null` otherwise (e.g., the main manifest file is not required to exist for
   * a test variant or a test project).
   *
   * Note: Avoid calling this method at configuration time because (1) the final path to the manifest file may change during that time, and
   * (2) this method performs I/O.
   */
  ...
  val manifestOverlayFiles: List<File>
    get() =
      listOfNotNull(variantSourceProvider?.manifestFile, buildTypeSourceProvider?.manifestFile, multiFlavorSourceProvider?.manifestFile) +
  ...
  /**
   * Returns a list of sorted SourceProvider in ascending order of importance. This means that items toward the end of the list take
   * precedence over those toward the start of the list.
   *
   * @return a list of source provider
   */
  fun getSortedSourceProviders(addVariantSources: Boolean = true): List<SourceProvider> {
    val providers: MutableList<SourceProvider> = Lists.newArrayListWithExpectedSize(flavorSourceProviders.size + 4)
  ...
  /**
   * Returns a map af all customs source directories registered. Key is the source set name as registered by the user. Value is also a map
   * of source set name to list of folders registered for this source set.
   */
  val customSourceList: Map<String, Collection<CustomSourceDirectory>>
    get() {
      return immutableMapBuilder<String, Collection<CustomSourceDirectory>> {
          getSortedSourceProviders().forEach { this.put(it.name, it.customDirectories) }
        }
        .toMap()
    }
}
