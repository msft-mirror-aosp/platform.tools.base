...
class AdbHelper(
   ...
   *
   * Checks the boot_completed system property and the bootcomplete device property to check whether the given device has successfully
   * booted. If the device does not exist or is not connected to adb, this method returns false.
   *
  ...
  fun isManagedDevice(serial: String, logger: ILogger): Boolean {
    ...
    logger.verbose("$serial with id $id is${if(!result) " not" else ""} a Gradle Managed Device")
    return result
  }
   ...
   *
   * At least one serial is expected to have the given [idValue] associated with it. An exception is thrown if the serial cannot be found.
   *
   * @param idValue the id value for the emulator to look for. This is the value passed in with the "-id" flag to the emulator command, or
   *   the name of the avd launched if no value is passed in.
   * @return The serial of the emulator that can be used for subsequent adb commands. See [isBootCompleted], [killDevice].
   */
  ...
  fun killDevice(serial: String) {
    val killProcess = processFactory(listOf(adbExecutable.absolutePath, "-s", serial, "emu", "kill")).start()
    killProcess.waitFor()
  }

  private fun allSerials(logger: ILogger? = null): List<String> {
    val serials = mutableListOf<String>()
    val listDevicesProcess = processFactory(listOf(adbExecutable.absolutePath, "devices")).start()
  ...
  private fun getIdForSerial(serial: String): String? {
    var id: String? = null
    val idDetectionProcess = processFactory(listOf(adbExecutable.absolutePath, "-s", serial, "emu", "avd", "id")).start()
  ...
  private fun getDeviceProperty(propertyName: String, emulatorSerial: String, logger: ILogger, stdoutTextProcessor: (String) -> Unit) {
