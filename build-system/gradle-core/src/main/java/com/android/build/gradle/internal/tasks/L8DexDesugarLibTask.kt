...
/** A task using L8 to dex and shrink(if needed) desugar library with keep rules computed with trace reference tool. */
@CacheableTask
@BuildAnalyzer(primaryTaskCategory = TaskCategory.DEXING)
abstract class L8DexDesugarLibTask : NonIncrementalTask() {

  @get:Input abstract val libConfiguration: Property<String>

  @get:Input abstract val minSdkVersion: Property<Int>

  @get:Classpath abstract val desugarLibJar: ConfigurableFileCollection
  ...
  @get:Input @get:Optional abstract val keepRulesConfigurations: ListProperty<String>

  /**
   * The pre-processed(desugared) desugar lib jar in classfile format which is used by trace reference tool to generate keep rules for
   * shrinking desugar lib jar into dex format.
   */
  @get:Classpath @get:Optional abstract val desugaredDesugarLibJar: ConfigurableFileCollection

  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.NONE) abstract val dexFiles: ConfigurableFileCollection

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val inputMappingFile: RegularFileProperty

  @get:[InputFiles Optional PathSensitive(PathSensitivity.NAME_ONLY)]
  abstract val inputArtProfile: RegularFileProperty

  @get:OutputDirectory abstract val desugarLibDex: DirectoryProperty

  @get:OutputFile @get:Optional abstract val keepRules: RegularFileProperty

  @get:OutputFile @get:Optional abstract val outputArtProfile: RegularFileProperty
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<L8DexDesugarLibTask, ApkCreationConfig>(creationConfig),
    ...
    override fun handleProvider(taskProvider: TaskProvider<L8DexDesugarLibTask>) {
      ...
      creationConfig.artifacts
        .use(taskProvider)
        .wiredWithFiles(L8DexDesugarLibTask::inputArtProfile, L8DexDesugarLibTask::outputArtProfile)
    ...
    override fun configure(task: L8DexDesugarLibTask) {
      ...
      if (dexingCreationConfig.needsShrinkDesugarLibrary) {
        ...
        if (!creationConfig.componentType.isForTesting && creationConfig.global.hasDynamicFeatures) {
          task.dexFiles.from(
...
abstract class L8DexWorkAction : ProfileAwareWorkAction<L8DexWorkAction.Params>() {
  ...
  override fun run() {
    ...
    val keepRulesConfig = KeepRulesConfig(keepRuleFiles, parameters.keepRulesConfigurations.orNull ?: emptyList())
    runL8(
