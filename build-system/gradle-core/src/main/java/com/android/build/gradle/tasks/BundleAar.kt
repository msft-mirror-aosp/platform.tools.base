...
abstract class BundleAar : Zip(), VariantTask {
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val localAarDeps: ConfigurableFileCollection
  ...
  /**
   * AbstractArchiveTask has main output "archiveFile" as a Provider. Two properties destinationDirectory and archiveFileName need to be set
   * to define output. SingleInitialProviderRequestImpl has mechanism that manipulates names and paths and uses one path property to fuze
   * task into graph. To adapt two properties of AbstractArchiveTask we introduced mappedOutput that publishes value to
   * destinationDirectory, archiveFileName and archiveFile
   */
   ...
   *                  |                               |
   *
   * @Output <-- + ---- mappedOutput archiveFile <-- | | | +---- archiveFileName <---
   */
  init {
  ...
  /** Whether this task generates an AAR specifically for lint, in which case local AAR dependencies are allowed. */
  @get:Input abstract val forLint: Property<Boolean>
  ...
  @get:InputDirectory @get:PathSensitive(PathSensitivity.NONE) abstract val aarMetadataCheck: DirectoryProperty

  abstract class BaseCreationAction<T : ComponentCreationConfig>(creationConfig: T) :
    VariantTaskCreationAction<BundleAar, T>(creationConfig) {
    ...
    override fun configure(task: BundleAar) {
      ...
      if (buildFeatures.dataBinding && buildFeatures.androidResources) {
        task.from(
          task.project.provider { creationConfig.artifacts.get(InternalArtifactType.DATA_BINDING_ARTIFACT) },
          prependToCopyPath(DataBindingBuilder.DATA_BINDING_ROOT_FOLDER_IN_AAR),
        )
        task.from(
          creationConfig.artifacts.get(InternalArtifactType.DATA_BINDING_BASE_CLASS_LOG_ARTIFACT),
          prependToCopyPath(DataBindingBuilder.DATA_BINDING_CLASS_LOG_ROOT_FOLDER_IN_AAR),
        )
      }

      task.from(artifacts.get(InternalArtifactType.COMPILE_SYMBOL_LIST))
      task.from(artifacts.get(InternalArtifactType.PACKAGED_RES), prependToCopyPath(SdkConstants.FD_RES))
      task.from(artifacts.get(InternalArtifactType.UPDATED_NAVIGATION_XML), prependToCopyPath(SdkConstants.FD_RES))
      ...
      if (buildFeatures.androidResources) {
        ...
        task.from(Callable { artifacts.get(InternalArtifactType.NAVIGATION_JSON_FOR_AAR).takeIf { it.isPresent } })
      }

      task.from(artifacts.get(InternalArtifactType.AAR_MAIN_JAR))
      task.from(artifacts.get(InternalArtifactType.AAR_LIBS_DIRECTORY), prependToCopyPath(SdkConstants.LIBS_FOLDER))
      task.from(artifacts.get(InternalArtifactType.ANNOTATIONS_ZIP))

      if (creationConfig !is KmpCreationConfig) {
        task.from(creationConfig.artifacts.get(SingleArtifact.ASSETS), prependToCopyPath(SdkConstants.FD_ASSETS))
      }

      task.from(artifacts.get(InternalArtifactType.AAR_METADATA)) {
        it.rename(AarMetadataTask.AAR_METADATA_FILE_NAME, AarMetadataTask.AAR_METADATA_ENTRY_PATH)
      }
      task.localAarDeps.from(creationConfig.computeLocalFileDependencies { it.name.lowercase(Locale.US).endsWith(SdkConstants.DOT_AAR) })

      task.aarMetadataCheck.setDisallowChanges(artifacts.get(InternalArtifactType.AAR_METADATA_CHECK))
  ...
  /** Package artifacts similar to [LibraryCreationAction] without aidl, merged proguard, renderscript, jni, lint jar, prefab package. */
  class TestFixturesCreationAction(creationConfig: TestFixturesCreationConfig) :
  ...
  class TestFixturesLocalLintCreationAction(creationConfig: TestFixturesCreationConfig) :
    BaseCreationAction<TestFixturesCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<BundleAar>) {
      ...
      val outputFile = InternalArtifactType.LOCAL_AAR_FOR_LINT.getOutputPath(creationConfig.artifacts.buildDirectory, creationConfig.name)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, BundleAar::mappedOutput, BundleAar::getArchiveFile)
    ...
    override fun configure(task: BundleAar) {
      ...
      if (creationConfig.buildFeatures.androidResources) {
        task.from(creationConfig.artifacts.get(InternalArtifactType.COMPILE_R_CLASS_JAR), prependToCopyPath(SdkConstants.LIBS_FOLDER))
      }
  ...
  class LibraryCreationAction(creationConfig: LibraryCreationConfig) : AbstractLibraryCreationAction(creationConfig) {
  ...
  abstract class AbstractLibraryCreationAction(creationConfig: LibraryCreationConfig) :
    ...
    override fun configure(task: BundleAar) {
      ...
      if (buildFeatures.aidl) {
        task.from(creationConfig.artifacts.get(InternalArtifactType.AIDL_PARCELABLE), prependToCopyPath(SdkConstants.FD_AIDL))
      }

      task.from(artifacts.get(InternalArtifactType.MERGED_CONSUMER_PROGUARD_FILE))
      ...
      if (buildFeatures.prefabPublishing) {
        task.from(artifacts.get(InternalArtifactType.PREFAB_PACKAGE), prependToCopyPath(SdkConstants.FD_PREFAB_PACKAGE))
      }
  ...
  class KotlinMultiplatformCreationAction(creationConfig: KmpCreationConfig) : BaseCreationAction<KmpCreationConfig>(creationConfig) {
    ...
    override fun configure(task: BundleAar) {
      ...
      task.from(creationConfig.artifacts.get(LIBRARY_AND_LOCAL_JARS_JNI), prependToCopyPath(SdkConstants.FD_JNI))
    }
  }

  class KotlinMultiplatformLocalLintCreationAction(creationConfig: KmpCreationConfig) :
  ...
  companion object {
    ...
    private fun prependToCopyPath(pathSegment: String, includeEmptyDirs: Boolean = false) = Action { copySpec: CopySpec ->
      copySpec.includeEmptyDirs = includeEmptyDirs
      copySpec.eachFile { fileCopyDetails: FileCopyDetails ->
        fileCopyDetails.relativePath = fileCopyDetails.relativePath.prepend(pathSegment)
      }
    }
  }
}
