...
/** Task that takes the linked android resources zip and android assets and puts them into a zip file without code or signing. */
...
abstract class PackageForHostTest : NonIncrementalTask() {
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val resApk: DirectoryProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val mergedAssetsDirectory: DirectoryProperty

  @get:Input @get:Optional abstract val noCompress: ListProperty<String>

  @get:OutputFile abstract val apkForHostTest: RegularFileProperty
  ...
  override fun doTaskAction() {
    ...
    val creationData =
      ApkCreatorFactory.CreationData.builder()
        ...
        .setNoCompressPredicate(PackagingUtils.getNoCompressPredicateForJavaRes(noCompress.get())::test)
        .build()
      ...
      Files.walkFileTree(
        ...
        object : SimpleFileVisitor<Path>() {
          ...
          override fun visitFile(path: Path, basicFileAttributes: BasicFileAttributes): FileVisitResult {
            val relativePath = PathUtils.toSystemIndependentPath(mergedAssetsPath.relativize(path))
            val destPath = PathUtils.toSystemIndependentPath(apkAssetsPath.resolve(relativePath))
  ...
  internal fun apkFrom(compiledResourcesZip: Provider<Directory>): File {
    val builtArtifacts =
      BuiltArtifactsLoaderImpl().load(compiledResourcesZip) ?: throw RuntimeException("Cannot load resources from $compiledResourcesZip")
    ...
    builtArtifacts.elements.forEach { builtArtifact ->
      ...
      if (builtArtifact.filters.size == 1 && builtArtifact.getFilter(FilterConfiguration.FilterType.ABI) != null) {
    ...
    builtArtifacts.elements.forEach { sb.append("BuildOutput: ${Joiner.on(',').join(it.filters)}\n") }
  ...
  class CreationAction(creationConfig: HostTestCreationConfig) :
    VariantTaskCreationAction<PackageForHostTest, HostTestCreationConfig>(creationConfig) {

    override val name = computeTaskName("package", "For${creationConfig.componentType.suffix}")
