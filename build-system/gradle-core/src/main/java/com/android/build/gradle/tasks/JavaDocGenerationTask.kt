...
abstract class JavaDocGenerationTask : NonIncrementalTask() {
  ...
  @get:Input abstract override val projectPath: Property<String>
  ...
  @get:Classpath abstract val dokkaPlugins: ConfigurableFileCollection

  @get:Classpath abstract val dokkaRuntimeClasspath: ConfigurableFileCollection

  @get:Classpath abstract val dokkaCoreClasspath: ConfigurableFileCollection

  override fun doTaskAction() {
    workerExecutor
      .classLoaderIsolation { it.classpath.from(dokkaCoreClasspath) }
      .submit(DokkaWorkAction::class.java) {
  ...
  abstract class DokkaWorkAction : WorkAction<DokkaWorkAction.Params> {
    ...
    /**
     * Dokka needs a separate classpath because it not only uses classes bundled from the kotlin compiler, but also classes from intellij
     * that would heavily interfere with other plugins when loaded directly.
     *
     * Given Gradle leaks kotlin-stdlib to classpath of workers running in classloader isolation and process isolation mode, we shouldn't
     * launch Dokka via Gradle workers. Instead, we need to load Dokka runtime dependencies manually and launch it using reflection.
     */
    private fun getRuntimeClassLoader(runtime: ConfigurableFileCollection): URLClassLoader {
      val runtimeJars = runtime.files
      return URLClassLoader(runtimeJars.map { it.toURI().toURL() }.toTypedArray(), ClassLoader.getSystemClassLoader().parent)
    }
  ...
  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<JavaDocGenerationTask, ComponentCreationConfig>(creationConfig) {
    ...
    override fun configure(task: JavaDocGenerationTask) {
      ...
      val dokkaCore = task.project.configurations.detachedConfiguration(task.project.dependencies.create(DOKKA_CORE))
      ...
      task.classpath.fromDisallowChanges(creationConfig.global.bootClasspath, creationConfig.compileClasspath)
    }
  }
