...
data class AbiConfiguration(val allAbis: Set<String>, val validAbis: Set<String>)

/** This class is responsible for determining which ABIs are needed for the build based on the relevant contents of build.gradle DSL. */
class AbiConfigurator(private val key: AbiConfigurationKey) {
  private val configuration: AbiConfiguration
  val allAbis: Set<String>
    get() = configuration.allAbis

  val validAbis: Set<String>
    get() = configuration.validAbis
  ...
  init {
    with(key) {
      ...
      val userChosenAbis = externalNativeBuildAbiFilters union splitsFilterAbis union ndkConfigAbiFilters
      val userMistakes = userChosenAbis subtract ndkHandlerSupportedAbis
      ...
      if (userChosenAbis.isEmpty()) {
        ...
        val selectedAbis: Set<String> =
          sequenceOf(externalNativeBuildAbiFilters, ndkConfigAbiFilters, splitsFilterAbis)
        ...
        validAbis =
          if (injectedLegalAbis.isEmpty()) {
            ...
            if (invalidAbis.isNotEmpty()) {
              val (toleratedInvalid, nontoleratedInvalid) = invalidAbis.partition { toleratedAbiNames.contains(it) }
