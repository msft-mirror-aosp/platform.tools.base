...
abstract class AndroidLintAnalysisTask : NonIncrementalTask() {
  ...
  @get:Internal abstract val lintModelDirectory: DirectoryProperty

  @get:OutputDirectory abstract val partialResultsDirectory: DirectoryProperty
  ...
  @get:Nested abstract val variantInputs: VariantInputs
  ...
  private fun writeLintModelFile() {
    ...
    val variant = variantInputs.toLintModel(module, partialResultsDirectory.get().asFile, desugaredMethodsFiles.files)
  ...
  /** Creates the lintAnalyzeVariant Task. Linting a variant also includes looking at the tests for that variant. */
  class SingleVariantCreationAction(variant: VariantWithTests) : VariantCreationAction(variant) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<AndroidLintAnalysisTask>) {
      registerOutputArtifacts(taskProvider, LINT_PARTIAL_RESULTS, creationConfig.artifacts)
    }
  }
  ...
  class LintVitalCreationAction(variant: VariantCreationConfig) :
    ...
    override val description = "Run lint analysis with only the fatal issues enabled on the ${creationConfig.name} variant"

    override fun handleProvider(taskProvider: TaskProvider<AndroidLintAnalysisTask>) {
      registerOutputArtifacts(taskProvider, LINT_VITAL_PARTIAL_RESULTS, creationConfig.artifacts)
    }
  }

  abstract class VariantCreationAction(val variant: VariantWithTests) :
    VariantTaskCreationAction<AndroidLintAnalysisTask, ComponentCreationConfig>(variant.main) {
    ...
    final override fun configure(task: AndroidLintAnalysisTask) {
      ...
      task.checkOnly.set(creationConfig.services.provider { creationConfig.global.lintOptions.checkOnly })
  ...
  class PerComponentCreationAction(creationConfig: ComponentCreationConfig, private val fatalOnly: Boolean) :
    VariantTaskCreationAction<AndroidLintAnalysisTask, ComponentCreationConfig>(creationConfig) {
    ...
    private val description =
      "Run lint analysis " + if (fatalOnly) "with only the fatal issues enabled " else "" + "on the ${creationConfig.name} component"
    ...
    override fun configure(task: AndroidLintAnalysisTask) {
      ...
      task.lintRuleJars.from(
        mainVariant.variantDependencies.getArtifactFileCollection(
          AndroidArtifacts.ConsumedConfigType.LINT_CHECKS_CLASSPATH,
          AndroidArtifacts.ArtifactScope.ALL,
          AndroidArtifacts.ArtifactType.LINT,
        )
      ...
      task.lintRuleJars.disallowChanges()
      task.fatalOnly.setDisallowChanges(fatalOnly)
      task.checkOnly.set(mainVariant.services.provider { creationConfig.global.lintOptions.checkOnly })
      ...
      task.desugaredMethodsFiles.from(
        getDesugaredMethods(
          ...
          (mainVariant as? ConsumableCreationConfig)?.isCoreLibraryDesugaringEnabledLintCheck ?: false,
          mainVariant.minSdk,
          mainVariant.global,
        )
      )
  ...
  /**
   * If [lintModelArtifactType] is not null, only the corresponding artifact is initialized; if it's null, both the main and test artifacts
   * are initialized.
   *
   * If [testCompileClasspath] is null, the standard "testCompileClasspath" is used; otherwise, [testCompileClasspath] is used in order to
   * properly include the main jar.
   *
   * If [testRuntimeClasspath] is null, the standard "testRuntimeClasspath" is used; otherwise, [testRuntimeClasspath] is used in order to
   * properly include the main jar.
   */
  fun configureForStandalone(
    ...
    this.lintModelDirectory.setDisallowChanges(project.layout.buildDirectory.dir("intermediates/${this.name}/android-lint-model"))
  ...
  companion object {
    ...
    fun registerOutputArtifacts(
      ...
      artifacts.use(taskProvider).wiredWith(AndroidLintAnalysisTask::partialResultsDirectory).toAppendTo(internalArtifactType)
    }
  }
}
