...
class AndroidTestTaskManager(project: Project, globalConfig: GlobalTaskCreationConfig) : TaskManager(project, globalConfig) {
  ...
  fun createTopLevelTasks() {
    ...
    if (globalConfig.productFlavorCount > 0) {
      connectedAndroidTestTask =
        taskFactory.register(AndroidReportTask.CreationAction(globalConfig, AndroidReportTask.CreationAction.TaskKind.CONNECTED))
      reportTasks.add(connectedAndroidTestTask.name)
    } else {
      connectedAndroidTestTask =
        taskFactory.register(CONNECTED_ANDROID_TEST) { connectedTask: Task ->
          connectedTask.group = JavaBasePlugin.VERIFICATION_GROUP
          connectedTask.description = ("Installs and runs instrumentation tests " + "for all flavors on connected devices.")
        }
    }
    taskFactory.configure(globalConfig.taskNames.connectedCheck) { it.dependsOn(connectedAndroidTestTask.name) }
    ...
    if (providers.size > 1 || globalConfig.productFlavorCount > 0) {
      deviceAndroidTestTask =
        taskFactory.register(AndroidReportTask.CreationAction(globalConfig, AndroidReportTask.CreationAction.TaskKind.DEVICE_PROVIDER))
      reportTasks.add(deviceAndroidTestTask.name)
    } else {
      deviceAndroidTestTask =
        taskFactory.register(DEVICE_ANDROID_TEST) { providerTask: Task ->
          providerTask.group = JavaBasePlugin.VERIFICATION_GROUP
          providerTask.description = ("Installs and runs instrumentation tests " + "using all Device Providers.")
        }
    }
    taskFactory.configure(globalConfig.taskNames.deviceCheck) { it.dependsOn(deviceAndroidTestTask.name) }
    ...
    if (reportTasks.isNotEmpty() && project.gradle.startParameter.isContinueOnFailure) {
      project.gradle.taskGraph.whenReady { taskGraph: TaskExecutionGraph ->
        for (reportTask in reportTasks) {
          if (taskGraph.hasTask(getTaskPath(project, reportTask))) {
            taskFactory.configure(reportTask) { task: Task -> (task as AndroidReportTask).setWillRun() }
          }
        }
      }
    }
  ...
  fun createTasks(
    ...
    val isKmpPerComponentLintAnalysis = androidTestProperties is KmpComponentCreationConfig && project.plugins.hasPlugin(LINT_PLUGIN_ID)
    val isNonKmpPerComponentLintAnalysis =
      androidTestProperties !is KmpComponentCreationConfig && androidTestProperties.services.projectOptions.get(LINT_ANALYSIS_PER_COMPONENT)
    val isPerComponentLintAnalysis = isKmpPerComponentLintAnalysis || isNonKmpPerComponentLintAnalysis
    if (globalConfig.avoidTaskRegistration.not() && isPerComponentLintAnalysis && globalConfig.lintOptions.ignoreTestSources.not()) {
      taskFactory.register(AndroidLintAnalysisTask.PerComponentCreationAction(androidTestProperties, fatalOnly = false))
  ...
  private fun createPackageResourcesTask(creationConfig: DeviceTestCreationConfig) {
    ...
    if (isTestApkCompileRClassEnabled(appCompileRClass, creationConfig.componentType)) {
      ...
      basicCreateMergeResourcesTask(creationConfig, MergeType.PACKAGE, false, false, false, ImmutableSet.of(), null)
    }
  }
  ...
  private fun createConnectedTestForVariant(
    ...
    val testData: AbstractTestDataImpl =
      if (testedVariant.componentType.isDynamicFeature) {
        BundleTestDataImpl(
          ...
          testedVariant.variantDependencies.getArtifactFileCollection(
    ...
    for (testServer in servers) {
      val serverTask = taskFactory.register(TestServerTask.TestServerTaskCreationAction(androidTestProperties, testServer))
      serverTask.dependsOn<Task>(androidTestProperties.taskContainer.assembleTask)
      taskFactory.configure(globalConfig.taskNames.deviceCheck) { it.dependsOn(serverTask) }
    }

    createTestDevicesForVariant(androidTestProperties, testData, androidTestProperties.mainVariant.name)
  }
  ...
  private fun createTestDevicesTasks() {
    ...
    val cleanTask =
      taskFactory.register(
        ManagedDeviceCleanTask.CreationAction("cleanManagedDevices", globalConfig, managedDevices.filterIsInstance<ManagedVirtualDevice>())
      )
    val allDevices =
      taskFactory.register(globalConfig.taskNames.allDevicesCheck) { allDevicesCheckTask: Task ->
        allDevicesCheckTask.description = "Runs all device checks on all managed devices defined in the TestOptions dsl."
    ...
    for (device in managedDevices) {
      ...
      val setupTask =
        when {
          device is ManagedVirtualDevice ->
            taskFactory.register(ManagedDeviceInstrumentationTestSetupTask.CreationAction(setupTaskName(device), device, globalConfig))
          registration != null -> {
            if (registration.hasSetupActions) {
              taskFactory.register(
                ManagedDeviceSetupTask.CreationAction(
                  project.layout.buildDirectory.dir(FileUtils.join(FD_MANAGED_DEVICE_SETUP_RESULTS, device.name)),
      ...
      setupTask.configure { it.mustRunAfter(cleanTask) }

      val deviceAllVariantsTask =
        taskFactory.register(managedDeviceAllVariantsTaskName(device)) { deviceVariantTask: Task ->
          deviceVariantTask.description = "Runs all device checks on the managed device ${device.name}."
    ...
    for (group in getDeviceGroups()) {
      taskFactory.register(managedDeviceGroupAllVariantsTaskName(group)) { deviceGroupTask: Task ->
        deviceGroupTask.description = "Runs all device checks on all devices defined in group ${group.name}."
  ...
  private fun createMockableJarTask() {
    ...
    taskFactory.register(globalConfig.taskNames.createMockableJar) { task: Task -> task.dependsOn(globalConfig.mockableJarArtifact) }
  }
  ...
  override fun createVariantPreBuildTask(creationConfig: ComponentCreationConfig) {
    if (
      creationConfig is DeviceTestCreationConfig && // should always be true
        creationConfig.mainVariant.componentType.isApk &&
        !creationConfig.mainVariant.componentType.isForTesting
    ) {

      val disableAllConstraints = creationConfig.services.projectOptions[BooleanOption.DISABLE_ALL_CONSTRAINTS]

      val enableTask = creationConfig.services.projectOptions[BooleanOption.ENABLE_CLASSPATH_CHECK_TASKS]

      // Always register the prebuild task, regardless of whether it does anything as there
      // are other places it's assumed to exist.
      val testPreBuildTask = taskFactory.register(TestPreBuildTask.CreationAction(creationConfig))
      if (disableAllConstraints && enableTask) {
        val classpathCheck = taskFactory.register(AppClasspathCheckTask.CreationAction(creationConfig))
