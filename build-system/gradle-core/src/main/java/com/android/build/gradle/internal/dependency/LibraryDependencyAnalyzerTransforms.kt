...
/**
 * This transform outputs a directory containing a file listing the contents of this AAR's resource_symbols.txt which is in r-def form
 * [SymbolIo.readRDef]).
 */
@CacheableTransform
abstract class CollectResourceSymbolsTransform : TransformAction<GenericTransformParameters> {

  @get:InputArtifact @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputAndroidResArtifact: Provider<FileSystemLocation>
...
/**
 * This transform outputs a directory containing a file called 'classes.txt' listing all classes in the inputArtifact JAR. Each line in
 * 'classes.txt' is a java class name of the form 'com/example/MyClass.class'.
 */
@CacheableTransform
abstract class CollectClassesTransform : TransformAction<GenericTransformParameters> {

  @get:Classpath @get:InputArtifact abstract val inputJarArtifact: Provider<FileSystemLocation>

  override fun transform(transformOutputs: TransformOutputs) {
    writeListToFile(
      transformOutputs.file(SdkConstants.FN_CLASS_LIST),
      getSortedRelativePathsInJar(inputJarArtifact.get().asFile) { path -> path.endsWith(SdkConstants.DOT_CLASS) },
    )
  }
}
