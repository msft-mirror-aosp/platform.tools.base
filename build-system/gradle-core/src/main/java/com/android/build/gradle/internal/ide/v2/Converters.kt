...
internal fun DslDefaultConfig.convert(features: BuildFeatureValues) =
  ProductFlavorImpl(
    name = name,
    dimension = dimension,
...
internal fun DslProductFlavor.convert(features: BuildFeatureValues) =
  ProductFlavorImpl(
    name = name,
    dimension = dimension,
    applicationId = applicationId,
...
internal fun DslBuildType.convert(features: BuildFeatureValues) =
  BuildTypeImpl(
    name = name,
...
internal fun DslSigningConfig.convert() =
  SigningConfigImpl(
    name = name,
    storeFile = storeFile,
...
private fun Map<String, DslClassField>.convertBuildConfig(features: BuildFeatureValues): Map<String, ClassField>? =
  if (features.buildConfig) asSequence().map { it.key to it.value.convert() }.toMap() else null

private fun Map<String, DslClassField>.convertResValues(features: BuildFeatureValues): Map<String, ClassField>? =
  if (features.resValues) asSequence().map { it.key to it.value.convert() }.toMap() else null

private fun DslClassField.convert() =
  ClassFieldImpl(type = type, name = name, value = value, documentation = documentation, annotations = annotations)

private fun DslVectorDrawablesOptions.convert() =
  VectorDrawableOptionsImpl(generatedDensities = generatedDensities?.toSet(), useSupportLibrary = useSupportLibrary)

internal fun DslApiVersion.convert() = ApiVersionImpl(apiLevel = apiLevel, codename = codename)
...
internal fun DefaultAndroidSourceSet.convert(
  ...
  return SourceProviderImpl(
    ...
    baselineProfileDirectories =
      ((baselineProfiles as DefaultAndroidSourceDirectorySet).srcDirs +
          variantSourcesForList(mixin.mapNotNull { it.sources.baselineProfiles }))
        .toSet(),
...
internal fun DefaultAndroidSourceSet.convert(sources: InternalSources) =
  SourceProviderImpl(
    name = name,
    manifestFile = manifestFile,
...
/**
 * Returns source dirs that should be added to the model, but avoids getting the generated ones as they are handled separately in the model.
 */
...
internal fun CodeShrinkerV1.convert(): CodeShrinker =
  when (this) {
    CodeShrinkerV1.PROGUARD -> CodeShrinker.PROGUARD
    CodeShrinkerV1.R8 -> CodeShrinker.R8
  }

internal fun TestOptions.Execution.convert(): TestInfo.Execution =
  when (this) {
...
internal fun String.convertToExecution(): TestInfo.Execution? =
  when (this) {
    // The string coming from the old DSL (which is the enum converted) can be lowercase, so we
    // need to handle both
    "host",
    "HOST" -> TestInfo.Execution.HOST
    "android_test_orchestrator",
    "ANDROID_TEST_ORCHESTRATOR" -> TestInfo.Execution.ANDROID_TEST_ORCHESTRATOR
    "androidx_test_orchestrator",
    "ANDROIDX_TEST_ORCHESTRATOR" -> TestInfo.Execution.ANDROIDX_TEST_ORCHESTRATOR
    else -> null
  }
