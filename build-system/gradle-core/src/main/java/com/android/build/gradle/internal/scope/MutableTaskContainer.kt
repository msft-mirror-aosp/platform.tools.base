 ...
 * - the tasks needed by the variant API. The goal here is to revamp the API to get rid of the need to expose the tasks.
 * - tasks for internal usage and wiring. This should not be needed, except in rare cases (anchors). The goal is to get rid of this as much
 *   as possible, progressively; and to use buildable artifact exclusively to wire tasks.
 *
 * DO NOT ADD NEW TASKS TO THIS CLASS.
 */
class MutableTaskContainer : TaskContainer {
  ...
  override var connectedTestTask: TaskProvider<out DefaultTask>? = null
