...
/** [AnalyticsResourceManager] manages all build profile data for [AnalyticsConfiguratorService] and [AnalyticsService]. */
...
constructor(
  ...
  @VisibleForTesting val executionSpans = ConcurrentLinkedQueue<GradleBuildProfileSpan>()

  private var lastRecordId: AtomicLong? = null
  private val taskRecords = ConcurrentHashMap<String, TaskProfilingRecord>()
  private val otherEvents = Collections.synchronizedList<AndroidStudioEvent.Builder>(mutableListOf())
  private val threadRecorder: Recorder = ThreadRecorder()
  private val analyticsWriter = AnalyticsProfileWriter()

  fun writeAndFinish() {
    analyticsWriter.writeAndFinish(getFinalProfile(), otherEvents, profileDir, enableProfileJson)
  }
  ...
  fun getVariantBuilder(projectPath: String, variantName: String): GradleBuildVariant.Builder {
    val projectData: ProjectData =
      projects.computeIfAbsent(projectPath) {
        val projectBuilder = GradleBuildProject.newBuilder()
        projectBuilder.id = nameAnonymizer.anonymizeProjectPath(it)
        ProjectData(projectBuilder)
      }
  ...
  fun getTaskRecord(taskPath: String): TaskProfilingRecord? {
    if (!taskRecords.containsKey(taskPath)) {
      val builder =
        GradleBuildProfileSpan.newBuilder().apply {
      ...
      taskRecords[taskPath] = TaskProfilingRecord(this, builder, taskPath, projectPath, variantName)
    }
  ...
  fun recordTaskExecutionSpan(finishEvent: FinishEvent?) {
    ...
    taskBuilder.apply {
      when (taskState) {
        ...
        DID_WORK_NON_INCREMENTAL -> didWork = true
  ...
  /** Records the time elapsed while executing a void block at configuration phase and saves the resulting [GradleBuildProfileSpan]. */
  fun recordBlockAtConfiguration(
    ...
    val span =
      threadRecorder.record(executionType, getProjectId(projectPath), getVariantId(projectPath, variant), allocateRecordId(), block)
    configurationSpans.add(span)
  }

  /** Records the time elapsed while executing a void block at execution phase and saves the resulting [GradleBuildProfileSpan]. */
  ...
  fun writeRecord(
    ...
    if (variantName == null) {
      ...
      getVariantId(projectPath, variantName).let { executionRecord.variant = it }
    }
    ...
    if (taskExecutionPhases.isNotEmpty()) {
      ...
      executionSpans.add(
        GradleBuildProfileSpan.newBuilder()
          ...
          .setDurationInMs(firstPhase.startTimeInMs - executionRecord.startTimeInMs)
          .build()
      )
    }
  ...
  fun recordGlobalProperties(project: Project, configurationCacheActive: Boolean, projectIsolationActive: Boolean) {
    val projectOptions = getBuildService(project.gradle.sharedServices, ProjectOptionService::class.java).get().projectOptions
  ...
  fun collectTaskMetadata(graph: TaskExecutionGraph) {
    for (task in graph.allTasks) {
      val variantName = if (task is VariantTask) task.variantName else task.extensions.findByName(PROPERTY_VARIANT_NAME_KEY) as String?

      taskMetadata[task.path] =
        TaskMetadata(task.project.path, variantName, AnalyticsUtil.getPotentialTaskExecutionTypeName(task.javaClass))
    }
  }
  ...
  fun recordApplicationId(applicationId: Provider<String>) {
    synchronized(this) { applicationIds?.add(applicationId) }
  }
  ...
  private fun recordPlugins(project: Project) {
    project.gradle.allprojects {
      ...
      it.plugins.forEach { plugin ->
        ...
        maybeGetJarName(plugin.javaClass)?.let { jarName -> pluginData.setJarName(jarName) }
  ...
  private fun maybeGetJarName(pluginClass: Class<*>): String? =
    pluginJarNamesMap.getOrPut(pluginClass.name) {
      ...
      // if possible
      val entryUrl = pluginClass.simpleName.substringBefore('$', pluginClass.simpleName).let { pluginClass.getResource("$it.class") }

      entryUrl?.let { it.path.substringBefore(".jar!", "").substringAfterLast('/', "").ifEmpty { null } }
    }
  ...
  private fun getFinalProfile(): GradleBuildProfile {
    ...
    projects.forEach {
      ...
      it.value.variantBuilders.forEach { entry -> projectBuilder.addVariant(entry.value) }
    ...
    applicationIds?.let { profileBuilder.addAllRawProjectId(it.get().sorted()) }
  ...
  private fun createAndRecordMemorySample(): GradleBuildMemorySample {
    ...
    synchronized(profileBuilder) { profileBuilder.addMemorySample(stats) }
    return stats
  }
  ...
  private fun getProfileDir(projectOptions: ProjectOptions, project: Project): Path? {
    ...
    return when {
      ...
      enableJsonProfile -> {
        ...
        val profileDir = project.isolated.rootProject.projectDirectory.dir("build/${PROFILE_DIRECTORY}").asFile.toPath()
...
/**
 * A wrapper class mainly to associate [GradleBuildProject.Builder] of a project with [GradleBuildVariant.Builder]s of variants of that
 * project
 */
class ProjectData(var projectBuilder: GradleBuildProject.Builder) : Serializable {
  ...
  private fun writeObject(objectOutputStream: ObjectOutputStream) {
    objectOutputStream.writeObject(Base64.getEncoder().encodeToString(projectBuilder.build().toByteArray()))
    val serializableVariantBuilders = variantBuilders.mapValues { Base64.getEncoder().encodeToString(it.value.build().toByteArray()) }
  ...
  private fun readObject(objectInputStream: ObjectInputStream) {
    ...
    projectBuilder = GradleBuildProject.parseFrom(Base64.getDecoder().decode(projectBuilderSerialized)).toBuilder()
    val variantBuildersSerialized = objectInputStream.readObject() as Map<String, String>
    variantBuilders =
      variantBuildersSerialized.mapValues { GradleBuildVariant.parseFrom(Base64.getDecoder().decode(it.value)).toBuilder() }.toMutableMap()
  }
}

/** A wrapper class of metadata associated with a task path */
data class TaskMetadata(val projectPath: String, val variantName: String?, val typeName: String) : Serializable
