...
/** Build service used to load SDK components. All build operations requiring access to the SDK components should declare it as input. */
@Suppress("UnstableApiUsage")
abstract class SdkComponentsBuildService
@Inject
constructor(private val objectFactory: ObjectFactory, private val providerFactory: ProviderFactory) :
  ...
  private val sdkSourceSet: SdkLocationSourceSet by lazy { SdkLocationSourceSet(parameters.projectRootDir.get().asFile, providerFactory) }

  class Environment : SdkLibDataFactory.Environment() {
    private val systemProperties = SystemProperty.values().associate { it to System.getProperty(it.key) }
  ...
  /**
   * map of all created [SdkLoadingStrategy] keyed by compileSdkVersion-buildToolsRevision to a [SoftReference] of [SdkLoadingStrategy] that
   * will allow caching and proper release at the end of the build.
   */
  private val sdkLoadingStrategies: MutableMap<String, SdkLoadingStrategy> = mutableMapOf()

  /**
   * Lightweight class that cannot be cached since its parameters are not known at construction time (provided as Provider). However, once
   * the [SdkLoadingStrategy] is initialized lazily, those instances are cached and closed at the end of the build.
   *
   * So creating as many instances of VersionedSdkLoader as necessary is fine but instances of [SdkLoadingStrategy] should be allocated
   * wisely and closed once finished.
   *
   * Do not create instances of [VersionedSdkLoader] to store in [org.gradle.api.Task]'s input parameters or
   * [org.gradle.workers.WorkParameters] as it is not serializable. Instead inject the [SdkComponentsBuildService] along with
   * compileSdkVersion and buildToolsRevision for the module and call [SdkComponentsBuildService.sdkLoader] at execution time.
   */
  class VersionedSdkLoader(
    ...
    val sdkLoadStrategy: SdkLoadingStrategy by lazy {
      ...
      synchronized(sdkLoadingStrategies) {
        sdkLoadingStrategies.computeIfAbsent(key) {
          val fullScanLoadingStrategy =
            SdkFullLoadingStrategy(sdkHandler, compileSdkVersion.orNull, buildToolsRevision.orNull, parameters.useAndroidX.get())
          val directLoadingStrategy =
            SdkDirectLoadingStrategy(
              sdkSourceSet,
          ...
          SdkLoadingStrategy(directLoadingStrategy, fullScanLoadingStrategy)
        }
      }
    }
    ...
    val targetAndroidVersionProvider: Provider<AndroidVersion> = providerFactory.provider { sdkLoadStrategy.getTargetPlatformVersion() }
    ...
    val buildToolsRevisionProvider: Provider<Revision> = providerFactory.provider { sdkLoadStrategy.getBuildToolsRevision() }

    val buildToolInfoProvider: Provider<BuildToolInfo> = providerFactory.provider { sdkLoadStrategy.getBuildToolsInfo() }

    val adbExecutableProvider: Provider<RegularFile> =
      objectFactory.fileProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getAdbExecutable() })

    val renderScriptSupportJarProvider: Provider<File> = providerFactory.provider { sdkLoadStrategy.getRenderScriptSupportJar() }

    val sdkDirectoryProvider: Provider<Directory> =
      objectFactory
        .directoryProperty()
        .fileProvider(
          providerFactory.provider { getSdkDir(parameters.projectRootDir.get().asFile, parameters.issueReporter.get(), providerFactory) }
        )

    val androidJarProvider: Provider<File> = providerFactory.provider { sdkLoadStrategy.getAndroidJar() }

    val annotationsJarProvider: Provider<File> = providerFactory.provider { sdkLoadStrategy.getAnnotationsJar() }

    val additionalLibrariesProvider: Provider<List<OptionalLibrary>> = providerFactory.provider { sdkLoadStrategy.getAdditionalLibraries() }

    val adbHelper: Provider<AdbHelper> = providerFactory.provider { AdbHelper(providerFactory.provider { this }) }

    val coreLambdaStubsProvider: Provider<RegularFile> =
      objectFactory.fileProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getCoreLambaStubs() })

    val optionalLibrariesProvider: Provider<List<OptionalLibrary>> = providerFactory.provider { sdkLoadStrategy.getOptionalLibraries() }

    val aidlExecutableProvider: Provider<RegularFile> =
      objectFactory.fileProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getAidlExecutable() })
     ...
     *
     * Historically this was distributed in platform-tools. It has been moved to platforms, so it is versioned now. (There was some overlap,
     * so this is available in platforms since platform api 26, and was removed in the platform-tools several years later in 31.x)
     *
     * This will not be present if the compile-sdk version is less than 26 (a fallback to platform-tools would not help for users that
     * update their SDK, as it is removed in recent platform-tools)
     */
    val apiVersionsFile: Provider<RegularFile> =
      objectFactory.fileProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getApiVersionsFile() })

    fun sdkImageDirectoryProvider(imageHash: String): Provider<Directory> =
      objectFactory.directoryProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getSystemImageLibFolder(imageHash) })
    ...
    val emulatorDirectoryProvider: Provider<Directory> =
      objectFactory.directoryProperty().fileProvider(providerFactory.provider { sdkLoadStrategy.getEmulatorLibFolder() })

    val coreForSystemModulesProvider: Provider<File> = providerFactory.provider { sdkLoadStrategy.getCoreForSystemModulesJar() }
  }

  class VersionedNdkHandler(ndkLocator: NdkLocator, objectFactory: ObjectFactory, providerFactory: ProviderFactory) :
  ...
  fun sdkLoader(compileSdkVersion: Provider<String>, buildToolsRevision: Provider<Revision>): VersionedSdkLoader =
    VersionedSdkLoader(
      providerFactory,
      objectFactory,
  ...
  private val localProperties = gradleLocalProperties(parameters.projectRootDir.asFile.get(), providerFactory)
  ...
  fun versionedNdkHandler(ndkVersion: String?, ndkPathFromDsl: String?): VersionedNdkHandler =
    VersionedNdkHandler(ndkLoader(ndkVersion, ndkPathFromDsl, ndkDirFromProperties), objectFactory, providerFactory)

  fun versionedNdkHandler(input: NdkHandlerInput) =
    VersionedNdkHandler(
      ndkLoader(input.ndkVersion.orNull, input.ndkPathFromDsl.orNull, ndkDirFromProperties),
      objectFactory,
      providerFactory,
    )
  ...
  class RegistrationAction(project: Project, private val projectOptions: ProjectOptions) :
    ServiceRegistrationAction<SdkComponentsBuildService, Parameters>(project, SdkComponentsBuildService::class.java) {
...
/**
 * This avoids invoking [SdkComponentsBuildService.sdkDirectoryProvider] in order to get the SDK location. This is needed when AGP handles
 * missing [com.android.build.gradle.BaseExtension.compileSdkVersion] and it tries to add highest installed API when invoked from the IDE.
 */
...
abstract class AndroidJarInput : UsesSdkComponentsBuildService {
  ...
  @get:Internal abstract val compileSdkVersion: Property<String>
...
abstract class BuildToolsExecutableInput : UsesSdkComponentsBuildService {
  ...
  fun aidlExecutableProvider(): Provider<File> =
    sdkLoader().map {
      it.sdkLoadStrategy.getAidlExecutable() ?: throw RuntimeException("Cannot find aidl compiler from build-tools $buildToolsRevision")
    }

  fun aidlFrameworkProvider(): Provider<File> =
    sdkLoader().map {
      it.sdkLoadStrategy.getAidlFramework() ?: throw RuntimeException("Cannot find aidl framework from build-tools $buildToolsRevision")
    }
}
...
abstract class NdkHandlerInput {
  ...
  @get:Input @get:Optional abstract val ndkVersion: Property<String>

  @get:Input @get:Optional abstract val ndkPathFromDsl: Property<String>
}
