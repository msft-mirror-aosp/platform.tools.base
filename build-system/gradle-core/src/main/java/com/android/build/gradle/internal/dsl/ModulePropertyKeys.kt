...
sealed interface ModulePropertyKey<OutputT> {
  ...
  enum class BooleanWithDefault(override val key: String, private val default: Boolean) : ModulePropertyKey<Boolean> {
    /**
     * If false - the test APK instruments the target project APK, and the classes are provided. If true - the test APK targets itself (e.g.
    ...
    /** If false - Screenshot tests support will not be enabled If true - Screenshot test support will be enabled */
    SCREENSHOT_TEST("android.experimental.enableScreenshotTest", false),

    /**
     * This option only affects to running instrumented tests on devices API level 24 and above. If false - Does not run AOT compile
     * forcibly If true - Run AOT compile forcibly after installation before running the app
     */
    FORCE_AOT_COMPILATION("android.experimental.force-aot-compilation", false),

    /** If false - D8 will not attempt to optimize startup dex If true - D8 will optimize first dex for optimal startup performance. */
    D8_DEX_STARTUP_OPTIMIZATION("android.experimental.d8.dex-startup-optimization", false),

    /** If false - Android resources will be disabled in kmp If true - Android resources will be enabled in kmp */
    KMP_ANDROID_RESOURCES_ENABLED("android.experimental.kmp.enableAndroidResources", false),

    /**
     * If false - [FusedLibraryDependencyValidationTask] allows all dependencies included in the fused library if
     * true - [FusedLibraryDependencyValidationTask] fails build if a dependency doesn't meet validation checks
     */
    FUSED_LIBRARY_VALIDATE_DEPENDENCIES("android.experimental.fusedlibrary.validateDependencies", true),

    /** If false - when present, the injected ABI will be used if true - the injected ABI will be ignored and all ABIs will be built */
