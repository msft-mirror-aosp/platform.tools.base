...
constructor(
  ...
  override val applicationId: Provider<String> = internalServices.providerOf(String::class.java, dslInfo.applicationId)

  /**
   * Return the default runner as with host tests, there is no dexing. However, aapt2 requires the instrumentation tag to be present in the
   * merged manifest to process android resources.
   */
  ...
  override val manifestPlaceholdersCreationConfig: ManifestPlaceholdersCreationConfig by
    lazy(LazyThreadSafetyMode.NONE) {
      createManifestPlaceholdersCreationConfig(dslInfo.mainVariantDslInfo.manifestPlaceholdersDslInfo?.placeholders)
    }

  override val buildConfigCreationConfig: BuildConfigCreationConfig? by
    lazy(LazyThreadSafetyMode.NONE) {
      if (buildFeatures.buildConfig) {
        dslInfo.mainVariantDslInfo.buildConfigDslInfo?.let { BuildConfigCreationConfigImpl(this, it, internalServices) }
      } else {
        null
      }
    }
  ...
  /**
   * In unit tests, we don't produce an apk. However, we still need to set the target sdk version in the test manifest as robolectric
   * depends on it.
   */
    ...
    get() =
      when (dslInfo.componentType) {
        ComponentTypeImpl.UNIT_TEST -> {
          /**
           * In unit tests, we don't produce an apk. However, we still need to set the target sdk version in the test manifest as
           * robolectric depends on it.
           */
    ...
    when (dslInfo.componentType) {
      ...
      ComponentTypeImpl.SCREENSHOT_TEST -> {
        ...
        builtInKotlinSupportMode = run {
          ...
          if (support is BuiltInKotlinSupportMode.NotSupported && internalServices.projectInfo.hasPlugin(KOTLIN_ANDROID_PLUGIN_ID)) {
            BuiltInKotlinSupportMode.SupportedForTestFixturesAndScreenshotTest.ScreenshotTestAndKgpApplied
          } else {
            support
          }
        }
        builtInKaptSupportMode = run {
          ...
          if (support is BuiltInKaptSupportMode.NotSupported && internalServices.projectInfo.hasPlugin(KOTLIN_KAPT_PLUGIN_ID)) {
            BuiltInKaptSupportMode.SupportedForTestFixturesAndScreenshotTest.ScreenshotTestAndKaptApplied
          } else {
            support
          }
        }
        ...
        throw IllegalStateException("Expected a test component type, but ${componentIdentity.name} has type " + "$dslInfo.componentType")
    }
  }
}
