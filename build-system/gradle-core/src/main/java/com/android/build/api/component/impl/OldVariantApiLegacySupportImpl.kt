...
class OldVariantApiLegacySupportImpl(
  ...
  override val dslSigningConfig: com.android.build.gradle.internal.dsl.SigningConfig? =
    (dslInfo as? ApkProducingComponentDslInfo)?.signingConfigResolver?.resolveConfig(profileable = false, debuggable = false)
  ...
  /**
   * The old variant API runs after the new variant API, yet we need to make sure that whatever method used by the users (old variant API or
   * new variant API), we end up with the same storage so the information is visible to both old and new variant API users.
   *
   * The new Variant API does not allow for reading (at least not without doing an explicit [org.gradle.api.provider.Provider.get] call.
   * However, the old variant API was providing reading access.
   *
   * In order to use the same storage, an implementation of the old variant objects (List and Map) need to be proxied to the new Variant API
   * storage (ListProperty and MapProperty). It is not possible to do the reverse proxy'ing since the storage must be able to store
   * [org.gradle.api.provider.Provider<T>] which a plain java List cannot do.
   *
   * When the user reads information using the old variant API, there is no choice but doing a [org.gradle.api.provider.Provider.get] call
   * which can fail during old variant API execution since some of these providers can be obtained from a Task execution.
   *
   * Therefore, only allow access to the old variant API when the compatibility flag is set.
   */
  class JavaCompileOptionsForOldVariantAPI(private val services: BaseServices, private val annotationProcessor: AnnotationProcessor) :
    JavaCompileOptions {
    // Initialize the wrapper instance that will be returned on each call.
    private val _annotationProcessorOptions =
      object : AnnotationProcessorOptions {
        private val _classNames =
          MutableListBackedUpWithListProperty(annotationProcessor.classNames, "AnnotationProcessorOptions.classNames")
        private val _arguments = MutableMapBackedUpWithMapProperty(annotationProcessor.arguments, "AnnotationProcessorOptions.arguments")

        override fun getClassNames(): MutableList<String> = _classNames

        override fun getArguments(): MutableMap<String, String> = _arguments

        override fun getCompilerArgumentProviders(): MutableList<CommandLineArgumentProvider> = annotationProcessor.argumentProviders
      }
  ...
  override val oldVariantApiJavaCompileOptions: JavaCompileOptions =
    JavaCompileOptionsForOldVariantAPI(component.services, component.javaCompilation.annotationProcessor)

  override val outputs: VariantOutputList by
    lazy(LazyThreadSafetyMode.NONE) {
      ...
      val versionCodeProperty =
        if (component is DynamicFeatureCreationConfig) {
          ...
          internalServices.propertyOf(Int::class.java, -1).also { it.disallowChanges() }
        }
      ...
      val versionNameProperty =
        if (component is DynamicFeatureCreationConfig) {
          ...
          internalServices.propertyOf(String::class.java, "").also { it.disallowChanges() }
        }
        ...
        outputFileName
          ?: internalServices.newPropertyBackingDeprecatedApi(
            ...
            internalServices.projectInfo.getProjectBaseName().map {
              component.paths.getOutputFileName(dslSigningConfig != null, it, variantOutputConfiguration.baseName(component))
            },
          ),
      )
    )
  ...
  override fun getJavaClasspathArtifacts(
    ...
    val mainCollection = component.variantDependencies.getArtifactCollection(configType, AndroidArtifacts.ArtifactScope.ALL, classesType)
    val extraArtifact = component.services.provider { variantData.getGeneratedBytecode(generatedBytecodeKey) }
    val combinedCollection = component.services.fileCollection(mainCollection.artifactFiles, extraArtifact)
    val extraCollection =
      ArtifactCollectionWithExtraArtifact.makeExtraCollection(
        mainCollection,
        combinedCollection,
        extraArtifact,
    ...
    return (component as? TestComponentCreationConfig)?.onTestedVariant { testedVariant ->
      ...
      val internalArtifactType =
        getVariantPublishingSpec(testedVariant.componentType).getSpec(classesType, configType.publishedTo)!!.outputType

      @Suppress("USELESS_CAST") // Explicit cast needed here.
      val testedAllClasses: Provider<FileCollection> =
        component.services.provider {
          component.services.fileCollection(testedVariant.artifacts.get(internalArtifactType)) as FileCollection
        }
      val combinedCollectionForTest = component.services.fileCollection(combinedCollection, testedAllClasses, testedAllClasses)
  ...
  override fun getAllRawAndroidResources(component: ComponentCreationConfig): FileCollection {
    ...
    allRawAndroidResources =
      component.services.fileCollection().also { fileCollection ->
        fileCollection.from(
        ...
        component.sources.res { resSources ->
          resSources.getVariantSources().forEach { directoryEntries ->
            directoryEntries.directoryEntries.forEach { it.addTo(component.services.projectInfo.projectDirectory, fileCollection) }
          }
        }
      }
  ...
  override fun addBuildConfigField(type: String, key: String, value: Serializable, comment: String?) {
    component.buildConfigCreationConfig?.buildConfigFields?.put(key, BuildConfigField(type, value, comment))
  }

  override fun handleMissingDimensionStrategy(dimension: String, alternatedValues: List<String>) {
