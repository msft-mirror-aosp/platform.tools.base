...
abstract class PackageApplication : PackageAndroidArtifact() {
  ...
  @get:Optional @get:PathSensitive(PathSensitivity.RELATIVE) @get:InputFiles abstract val dexMetadataDirectory: DirectoryProperty

  /** Minimum SDK version for dexing, which may be different from [minSdkVersion] (see [DexingCreationConfig.getMinSdkVersionForDexing]). */
  @get:Input abstract val minSdkVersionForDexing: Property<Int>

  @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:InputFiles abstract val lintVital: RegularFileProperty

  // ----- CreationAction -----
  /** Configures the task to perform the "standard" packaging, including all files that should end up in the APK. */
  class CreationAction(creationConfig: ApkCreationConfig, private val outputDirectory: File) :
    PackageAndroidArtifact.CreationAction<PackageApplication>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<PackageApplication>) {
      ...
      val useResourcesShrinker = creationConfig.androidResourcesCreationConfig?.useResourceShrinker == true
      val operationRequest =
        creationConfig.artifacts
          .use(taskProvider)
          .wiredWithDirectories(PackageAndroidArtifact::resourceFiles, PackageApplication::outputDirectory)
      ...
      transformationRequest?.let { task?.let { t -> t._transformationRequest = it } }
      creationConfig.artifacts
    ...
    override fun configure(task: PackageApplication) {
      ...
      task.dexMetadataDirectory.setDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.DEX_METADATA_DIRECTORY))
      task.minSdkVersionForDexing.setDisallowChanges(creationConfig.dexing.minSdkVersionForDexing)
      if (shouldRegisterLintVitalTasks(creationConfig)) {
        task.lintVital.setDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.LINT_VITAL_OUTPUT))
      }
    }
    ...
    override fun finalConfigure(task: PackageApplication) {
      ...
      this.task = task
      transformationRequest?.let { task._transformationRequest = it }
    }
  }
  ...
  companion object {
    ...
    private fun customizeBuiltArtifacts(task: PackageApplication, input: BuiltArtifactsImpl): BuiltArtifactsImpl {
      ...
      return input.copy(
        baselineProfiles =
          if (task.dexMetadataDirectory.isPresent) {
            val dexMetadataPropertiesFile = File(task.dexMetadataDirectory.get().asFile, SdkConstants.FN_DEX_METADATA_PROP)
            if (dexMetadataPropertiesFile.exists()) {
              baselineProfileDataForJson(input.elements, dexMetadataPropertiesFile, task.outputDirectory.get().asFile)
            } else {
              null
            }
          } else null,
    ...
    private fun baselineProfileDataForJson(
      ...
      val apkNames = mappedElements.map { File(it.outputFile).nameWithoutExtension }
      ...
      baselineProfilesMapping.forEach { entry ->
        ...
        apkNames.forEach {
          val renamedDmFile = FileUtils.join(apkDirectory, SdkConstants.FN_OUTPUT_BASELINE_PROFILES, fileIndex, "$it.dm")
          renamedDmFile.parentFile.mkdirs()
          FileUtils.copyFile(dmFile, renamedDmFile)
          baselineProfileFiles.add(renamedDmFile)
        }
        baselineProfileData.add(BaselineProfileDetails(supportedApis.min(), supportedApis.max(), baselineProfileFiles))
      }
    ...
    @JvmStatic
    fun recordMetrics(projectPath: String?, apkOutputFile: File?, resourcesApFile: File?, analyticsService: AnalyticsService) {
      ...
      val resourcesApSize = getSize(resourcesApFile)
