...
constructor(
  ...
  override val packaging: ApkPackaging by lazy { ApkPackagingImpl(dslInfo.mainVariantDslInfo.packaging, variantServices, minSdk.apiLevel) }
  ...
  override val requiresJacocoTransformation: Boolean
    get() = codeCoverageEnabled && mainVariant.componentType.isAar
  ...
  override val buildConfigCreationConfig: BuildConfigCreationConfig? by
    lazy(LazyThreadSafetyMode.NONE) {
      if (buildFeatures.buildConfig) {
        dslInfo.buildConfigDslInfo?.let { BuildConfigCreationConfigImpl(this, it, internalServices) }
      } else {
        null
      }
    }
  ...
  final override val optimizationCreationConfig: OptimizationCreationConfig by
  ...
  override val testedApplicationId: Provider<String>
    get() =
      if (mainVariant.componentType.isAar) {
