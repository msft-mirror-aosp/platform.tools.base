...
abstract class BaseFlavor(name: String, private val dslServices: DslServices) :
  AbstractProductFlavor(name), CoreProductFlavor, ApplicationBaseFlavor, DynamicFeatureBaseFlavor, LibraryBaseFlavor, TestBaseFlavor {

  /** Encapsulates per-variant configurations for the NDK, such as ABI filters. */
  override val ndk: NdkOptions = dslServices.newInstance(NdkOptions::class.java)

  private val disallowProviderInAndroidSourceSet = dslServices.projectOptions.get(BooleanOption.DISALLOW_PROVIDER_IN_ANDROID_SOURCE_SET)
  ...
  @Deprecated("To be removed after Gradle drops space assignment support", ReplaceWith("minSdk {}"))
  ...
  @Deprecated("To be removed after Gradle drops space assignment support", ReplaceWith("targetSdk {}"))
  ...
  override fun maxSdk(action: MaxSdkSpec.() -> Unit) {
    createMaxSdkSpec().also {
      action.invoke(it)
      updateIfChanged(_maxSdk, it.version) { _maxSdk = it }
    }
  }
  ...
  @Deprecated("To be removed after Gradle drops space assignment support", ReplaceWith("maxSdk {}"))
  ...
  override fun buildConfigField(type: String, name: String, value: String) {
    ...
    if (alreadyPresent != null) {
      ...
      if (BuilderConstants.MAIN == flavorName) {
        dslServices.logger.info("DefaultConfig: buildConfigField '{}' value is being replaced.", name)
      } else {
        dslServices.logger.info("ProductFlavor({}): buildConfigField '{}' " + "value is being replaced.", flavorName, name)
      }
    }
  ...
  override fun resValue(type: String, name: String, value: String) {
    ...
    if (alreadyPresent != null) {
      val flavorName = getName()
      if (BuilderConstants.MAIN == flavorName) {
        dslServices.logger.info("DefaultConfig: resValue '{}' value is being replaced.", resValueKey.toString())
      } else {
        dslServices.logger.info("ProductFlavor({}): resValue '{}' value is being replaced.", flavorName, resValueKey.toString())
      }
    }
   ...
   *
   * To learn more, see [Add C and C++ Code to Your Project](http://developer.android.com/studio/projects/add-native-code.html#).
   */
   ...
   * `resConfigs` property, as shown in the sample below. Any resources for languages not specified are removed.
   *
   * ````
   * android {
   *     defaultConfig {
  ...
  private var _vectorDrawables: VectorDrawablesOptions = dslServices.newInstance(VectorDrawablesOptions::class.java)
  ...
  private fun createMaxSdkSpec(): MaxSdkSpecImpl {
    return dslServices.newDecoratedInstance(MaxSdkSpecImpl::class.java, dslServices).also { it.version = _maxSdk }
  }

  private fun createMinSdkSpec(): MinSdkSpecImpl {
    return dslServices.newDecoratedInstance(MinSdkSpecImpl::class.java, dslServices).also { it.version = _minSdk }
  }
