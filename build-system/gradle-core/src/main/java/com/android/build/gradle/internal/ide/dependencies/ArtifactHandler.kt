...
/**
 * Abstract class to handle [ResolvedArtifact] and create model class that can be used by AGP, Lint or the IDe.
 *
 * The entry point is [handleArtifact], and the logic figures out whether this is a sub-project, or an external library (android or java).
 * This then calls the abstract method to handle each case: [handleAndroidLibrary], [handleAndroidModule], [handleJavaLibrary] and
 * [handleJavaModule].
 */
abstract class ArtifactHandler<DependencyItemT>
protected constructor(
   ...
   */
  fun handleArtifact(artifact: ResolvedArtifact, isProvided: Boolean, lintJarMap: Map<ComponentIdentifier, File>?): DependencyItemT {
    val id = artifact.componentIdentifier

    val coordinatesSupplier = {
      mavenCoordinatesCache.cache.get(artifact) ?: throw RuntimeException("unable to compute coordinates for $artifact")
    }
    ...
    return if (id !is ProjectComponentIdentifier || artifact.isWrappedModule) {
      if (artifact.dependencyType === ResolvedArtifact.DependencyType.ANDROID) {
        ...
        val extractedFolder = artifact.extractedFolder ?: throw RuntimeException("Null extracted folder for artifact: $artifact")
        ...
        handleJavaLibrary(artifact.artifactFile!!, isProvided, coordinatesSupplier, modelAddressSupplier)
      }
    } else {
      if (artifact.dependencyType === ResolvedArtifact.DependencyType.ANDROID) {
