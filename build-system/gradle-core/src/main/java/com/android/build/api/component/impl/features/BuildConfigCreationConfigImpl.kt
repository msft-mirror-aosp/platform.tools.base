...
class BuildConfigCreationConfigImpl(
  ...
  override val compiledBuildConfig: FileCollection
    get() =
      if (buildConfigType == BuildConfigType.JAR) {
        ...
        if (component is TestComponentCreationConfig) {
          files.add(component.mainVariant.artifacts.get(InternalArtifactType.COMPILE_BUILD_CONFIG_JAR))
        }
  ...
  override val buildConfigType: BuildConfigType
    get() =
      if (
        internalServices.projectOptions[BooleanOption.ENABLE_BUILD_CONFIG_AS_BYTECODE]
