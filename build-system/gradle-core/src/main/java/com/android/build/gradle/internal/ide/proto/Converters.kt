...
internal fun <T, R> R.setIfNotNull(value: T?, setter: R.(T) -> Unit): R {
  if (value != null) {
    setter(value)
  }
  return this
...
internal fun File.convert() = com.android.builder.model.proto.ide.File.newBuilder().setAbsolutePath(absolutePath).build()
...
internal fun com.android.builder.model.v2.ide.AndroidGradlePluginProjectFlags.convert() =
  AndroidGradlePluginProjectFlags.newBuilder()
    .addAllBooleanFlagValues(
      com.android.builder.model.v2.ide.AndroidGradlePluginProjectFlags.BooleanFlag.values().mapNotNull { flag ->
        flag.convert()?.let { newFlag ->
          val value = flag.getValue(this)
          AndroidGradlePluginProjectFlags.BooleanFlagValue.newBuilder().setFlag(newFlag).setValue(value).build()
        }
      }
    )
    .build()
...
internal fun com.android.build.api.variant.AndroidVersion.convert() =
  AndroidVersion.newBuilder().setApiLevel(apiLevel).setIfNotNull(codename, AndroidVersion.Builder::setCodename)
...
private fun ComponentInfo.convertComponentInfo() =
  ComponentInfoProto.newBuilder()
    .setIfNotNull(buildType, ComponentInfoProto.Builder::setBuildType)
...
private fun ProjectInfo.convert() =
  ProjectInfoProto.newBuilder().setBuildId(buildId).setProjectPath(projectPath).setComponentInfo(convertComponentInfo())

private fun LibraryInfo.convert() =
  LibraryInfoProto.newBuilder().setComponentInfo(convertComponentInfo()).setGroup(group).setName(name).setVersion(version)
...
internal fun Library.convert() =
  LibraryProto.newBuilder()
    ...
    .setIfNotNull(projectInfo?.convert(), LibraryProto.Builder::setProjectInfo)
    .setIfNotNull(libraryInfo?.convert(), LibraryProto.Builder::setLibraryInfo)
    .setIfNotNull(artifact?.convert(), LibraryProto.Builder::setArtifact)
    .setIfNotNull(lintJar?.convert(), LibraryProto.Builder::setLintJar)
    .setIfNotNull(srcJar?.convert(), LibraryProto.Builder::setSrcJar)
    .setIfNotNull(srcJars.map { it.convert() }, LibraryProto.Builder::addAllSrcJars)
    .setIfNotNull(docJar?.convert(), LibraryProto.Builder::setDocJar)
    .setIfNotNull(samplesJar?.convert(), LibraryProto.Builder::setSamplesJar)
    .setIfNotNull(androidLibraryData?.convert(), LibraryProto.Builder::setAndroidLibraryData)
