...
/**
 * Processes a Jar containing the system module classes into a JDK Image, for use via the --system option of javac and other Java-related
 * tools.
 *
 * As per the discussion in b/154357088, the input for this process is, properly, the core-for-system-modules.jar included starting in
 * SDK 30. The steps taken to transform the Jar are discussed in b/63986449.
 */
class JdkImageTransformDelegate(val systemModulesJar: File, val workDir: File, val outDir: File, val jdkTools: JdkTools) {
  /**
   * Links a JDK Image from [systemModulesJar] in [outDir], using [workDir] for the output of any intermediate operations. This is done via
   * the "jlink" tool as exposed by [jdkTools].
   */
  fun run() {
    ...
    jdkTools.linkJmodsIntoJdkImage(jmodDir, "java.base", outDir)
  ...
  fun makeModuleDescriptorJava(): File {
    val moduleInfoJavaContent = generateModuleDescriptor("java.base", listOf(systemModulesJar))
  ...
  internal fun makeModuleInfoClass(): File {
    ...
    jdkTools.compileModuleDescriptor(moduleInfoJava, systemModulesJar, workDir)

    val moduleInfoClass = workDir.resolve("module-info.class")
    Preconditions.checkState(moduleInfoClass.exists(), "Expected compiled module descriptor file to be created at %s", moduleInfoClass)
  ...
  internal fun makeJmodFile(): File {
    ...
    jdkTools.createJmodFromModularJar(jmodFile, jlinkVersion, moduleJar)

    return jmodFile
  }
}

class JdkTools(val javaHome: File, val processExecutor: ProcessExecutor, val logger: ILogger) {
  ...
  val jlinkVersion: String by lazy {
    ...
    Preconditions.checkArgument(jlinkExecutable.exists(), "jlink executable %s does not exist.", jlinkExecutable)

    val pib =
      ProcessInfoBuilder().apply {
        setExecutable(jlinkExecutable)
        addArgs("--version")
      }

    val processOutputHandler = CachedProcessOutputHandler()
    processExecutor.execute(pib.createProcess(), processOutputHandler).rethrowFailure().assertNormalExitValue()
  ...
  fun compileModuleDescriptor(moduleInfoJava: File, systemModulesJar: File, outDir: File) {
    ...
    Preconditions.checkArgument(javacExecutable.exists(), "javac executable %s does not exist.", javacExecutable)
  ...
  fun createJmodFromModularJar(jmodFile: File, jlinkVersion: String, moduleJar: File) {
    ...
    Preconditions.checkArgument(jmodExecutable.exists(), "jmod executable %s does not exist.", jmodExecutable)
    val pib =
      ProcessInfoBuilder().apply {
...
/** Modeled after the Android platform build method for parsing module-info.java out of a list of Jars See b/63986449 for more info */
@VisibleForTesting
...
private fun copyJrtFsJar(outDir: File, jdkTools: JdkTools) {
  ...
  JarFlinger(destination.toPath()) { !it.equals("meta-inf/manifest.mf", ignoreCase = true) }.use { it.addJar(source.toPath()) }
}
