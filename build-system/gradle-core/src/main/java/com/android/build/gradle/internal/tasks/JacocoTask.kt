...
abstract class JacocoTask : NewIncrementalTask() {
  ...
  @get:Classpath abstract val jacocoAntTaskConfiguration: ConfigurableFileCollection

  @get:Nested abstract val jarsWithIdentity: JarsClasspathInputsWithIdentity

  @get:Classpath @get:Incremental abstract val classesDir: ConfigurableFileCollection

  @get:Input abstract val forceOutOfProcess: Property<Boolean>

  @get:OutputDirectory abstract val outputForDirs: DirectoryProperty

  @get:OutputDirectory abstract val outputForJars: DirectoryProperty
  ...
  private fun processDirectories(inputChanges: InputChanges) {
    ...
    val toProcess: MutableMap<Action, MutableList<SerializableChange>> = EnumMap(Action::class.java)
    ...
    workQueue.submit(InstrumentDirAction::class.java) { params: InstrumentDirAction.Parameters ->
  ...
  private fun processJars(inputChanges: InputChanges) {
    ...
    for ((key, fileInfo) in mappingState.jarsInfo) {
      if (fileInfo.hasChanged) {
        val instrumentedJar = getCorrespondingInstrumentedJar(outputForJars.get().asFile, Objects.requireNonNull(fileInfo.identity))
        try {
        ...
        val workQueue = workQueue
        workQueue.submit(InstrumentJarAction::class.java) { params: InstrumentJarAction.Parameters ->
          params.root.set(key)
  ...
  private fun removeFile(fileToRemove: SerializableChange) {
    ...
    val outputPath = outputForDirs.get().asFile.toPath().resolve(fileToRemove.normalizedPath)
    try {
  ...
  private val workQueue: WorkQueue
    get() =
      if (forceOutOfProcess.get()) {
        workerExecutor.processIsolation { spec: ProcessWorkerSpec -> spec.classpath.from(jacocoAntTaskConfiguration) }
      } else {
        workerExecutor.classLoaderIsolation { spec: ClassLoaderWorkerSpec -> spec.classpath.from(jacocoAntTaskConfiguration) }
      }
  ...
  enum class Action(vararg patterns: Pattern) {
    ...
    /** @param patterns Patterns are compared to files' relative paths to determine if they undergo the corresponding action. */
    init {
  ...
  /** This action is not a [ProfileAwareWorkAction] as it is submitted in isolation mode which is not supported by [AnalyticsService]. */
  abstract class InstrumentDirAction : WorkAction<InstrumentDirAction.Parameters> {
      ...
      for (toInstrument in inputs.getOrDefault(Action.INSTRUMENT, ImmutableList.of())) {
        ...
        try {
          Files.asByteSource(toInstrument.file).openBufferedStream().use { inputStream ->
            val instrumented = instrumenter.instrument(inputStream, toInstrument.toString())
            val outputFile = File(outputDir, toInstrument.normalizedPath)
            Files.createParentDirs(outputFile)
            Files.write(instrumented, outputFile)
          }
        } catch (e: IOException) {
          throw UncheckedIOException("Unable to instrument file with Jacoco: " + toInstrument.file, e)
        }
      }
      for ((file, _, normalizedPath) in inputs.getOrDefault(Action.COPY, ImmutableList.of())) {
    ...
    companion object {
      private val logger = LoggerWrapper.getLogger(InstrumentDirAction::class.java)
    }
  }
  ...
  abstract class InstrumentJarAction : WorkAction<InstrumentJarAction.Parameters> {
    ...
    override fun execute() {
      ...
      val instrumenter = Instrumenter(OfflineInstrumentationAccessGenerator())
      try {
        ZipOutputStream(BufferedOutputStream(FileOutputStream(instrumentedJar))).use { outputZip ->
    ...
    companion object {
      private val logger = LoggerWrapper.getLogger(InstrumentJarAction::class.java)
    }
  }
  ...
  companion object {
    ...
    private val JUNIT_RUNLISTENER_PATTERN = Pattern.compile("^org/junit/runner/notification/RunListener.*")
    ...
    fun calculateAction(inputRelativePath: String): Action {
      return when {
        ...
        Action.INSTRUMENT.patterns.any { it.matcher(inputRelativePath).matches() } -> Action.INSTRUMENT
        else -> Action.IGNORE
      }
    }

    private fun getCorrespondingInstrumentedJar(outputFolder: File, identity: String): File {
