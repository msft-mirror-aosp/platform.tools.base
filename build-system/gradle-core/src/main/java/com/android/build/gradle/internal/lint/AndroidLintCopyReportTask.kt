 ...
 *
 * Does not declare its inputs because doing so would cause this task to not run when the [AndroidLintTask] fails
 */
@DisableCachingByDefault(because = COPY_TASK)
@BuildAnalyzer(primaryTaskCategory = TaskCategory.LINT)
abstract class AndroidLintCopyReportTask :
  UnsafeOutputsTask(
    "The lintOptions DSL has configured potentially multiple lint tasks to write to the same location, but task should not have overlapping outputs."
  ) {

  @get:Internal abstract val textReportInput: RegularFileProperty

  @get:Internal abstract val htmlReportInput: RegularFileProperty

  @get:Internal abstract val xmlReportInput: RegularFileProperty

  @Suppress("SpellCheckingInspection") @get:Internal abstract val sarifReportInput: RegularFileProperty

  @get:Internal abstract val textReportOutput: RegularFileProperty

  @get:Internal abstract val htmlReportOutput: RegularFileProperty

  @get:Internal abstract val xmlReportOutput: RegularFileProperty

  @Suppress("SpellCheckingInspection") @get:Internal abstract val sarifReportOutput: RegularFileProperty
  ...
  override fun doTaskAction() {
    ...
    @Suppress("SpellCheckingInspection") doCopy(sarifReportInput, sarifReportOutput, displayName = "SARIF")
  }
  ...
  class CreationAction(variant: ComponentCreationConfig) :
    VariantTaskCreationAction<AndroidLintCopyReportTask, ComponentCreationConfig>(variant) {
    override val name: String = computeTaskName("copy", "LintReports")
    override val type: Class<AndroidLintCopyReportTask>
      get() = AndroidLintCopyReportTask::class.java
