 ...
 *
 * This is meant to be used only by the variant properties api objects. Other stages of the plugin will use different services objects.
 */
interface VariantServices : BaseServices {
   ...
   *
   * This should be used for properties used in the new API. If the property is backing an old API that returns T, use
   ...
   * the value on read).
   *
   * The property will be marked as [Property.finalizeValueOnRead], and will be locked with [Property.disallowChanges] after the variant
   * API(s) have run.
   */
   ...
   *
   * During configuration the property will be marked as [Property.disallowUnsafeRead] if [disallowUnsafeRead] is set to true. If false, the
   * property value access will be allowed.
   *
   * [value] will be used to initialize the property value and further changes will be disallowed unless [disallowChanges] is set to true.
   *
   * The [ListProperty] will be marked as [Property.finalizeValueOnRead], and will be locked with [Property.disallowChanges] after the
   * variant API(s) have run.
   *
   * [disallowUnsafeRead] should be set to true always, unless we need this for backward compatibility. Do not set it to false for new code.
   */
  fun <T : Any> listPropertyOf(type: Class<T>, value: Collection<T>, disallowUnsafeRead: Boolean = true): ListProperty<T>
   ...
   * variant API(s) have run.
   */
  fun <T : Any> listPropertyOf(type: Class<T>, fillAction: (ListProperty<T>) -> Unit): ListProperty<T>
   ...
   * the value on read).
   *
   * The [SetProperty] will be marked as [Property.finalizeValueOnRead], and will be locked with [Property.disallowChanges] after the
   * variant API(s) have run.
   */
   ...
   * [disallowUnsafeRead] should be set to true always, unless we need this for backward compatibility. Do not set it to false for new code.
   */
  fun <T : Any> setPropertyOf(type: Class<T>, value: Collection<T>, disallowUnsafeRead: Boolean = true): SetProperty<T>
   ...
   * the value on read).
   *
   * The [MapProperty] will be marked as [Property.finalizeValueOnRead], and will be locked with [Property.disallowChanges] after the
   * variant API(s) have run.
   */
  fun <K : Any, V : Any> mapPropertyOf(
    keyType: Class<K>,
    valueType: Class<V>,
    value: Map<K, V>,
   ...
   *
   * By default, this property is memoized with [Property.finalizeValueOnRead] but access to the old API getter will require disabling
   * memoization
   *
   * During configuration the property will be marked as [Property.disallowUnsafeRead] to disallow unsafe reads (which will also finalize
   * the value on read).
   *
   * The property will be locked with [Property.disallowChanges] after the variant API(s) have run.
   */
   ...
   * [disallowUnsafeRead] should be set to true always, unless we need this for backward compatibility. Do not set it to false for new code.
   */
  fun <T : Any> providerOf(type: Class<T>, value: Provider<T>, id: String = "", disallowUnsafeRead: Boolean = true): Provider<T>
  ...
  /** Creates a new [ListProperty] of [T] which is not protected and will not be automatically locked. */
