...
abstract class VariantImpl<DslInfoT : VariantDslInfo>(
  ...
  override val description: String
    get() =
      if (componentIdentity.productFlavors.isNotEmpty()) {
  ...
  override fun <T> getExtension(type: Class<T>): T? = type.cast(externalExtensions?.get(type))
   ...
   * Do Not Use
   *
   * This API is for backward compatibility, if you need to do work with host tests, iterate over all of them using the [hostTests] API. If
   * you only need to have access to unit tests, use [hostTests] using [HasHostTestsBuilder.UNIT_TEST_TYPE]
   */
  @Deprecated(
    "Use hostTests or getByTestComponentType APIs",
    ReplaceWith("hostTests[HasHostTestsBuilder.UNIT_TEST_TYPE) as? UnitTestImpl", "com.android.build.api.component.impl.UnitTestImpl"),
  )
  @Suppress("DEPRECATION")
  override val unitTest: UnitTest?
    get() = hostTests().firstOrNull() { it.componentType == ComponentTypeImpl.UNIT_TEST } as? UnitTest

  // ---------------------------------------------------------------------------------------------
  // INTERNAL API
  // ---------------------------------------------------------------------------------------------

  override val buildConfigCreationConfig: BuildConfigCreationConfig? by
    lazy(LazyThreadSafetyMode.NONE) {
      if (buildFeatures.buildConfig) {
        BuildConfigCreationConfigImpl(this, dslInfo.buildConfigDslInfo!!, internalServices)
      } else {
        null
      }
    }

  override val renderscriptCreationConfig: RenderscriptCreationConfig? by
    lazy(LazyThreadSafetyMode.NONE) {
  ...
  override val shadersCreationConfig: ShadersCreationConfig by
    lazy(LazyThreadSafetyMode.NONE) { ShadersCreationConfigImpl(dslInfo.shadersDslInfo!!) }
  ...
  override val nativeBuildCreationConfig: NativeBuildCreationConfig by
    lazy(LazyThreadSafetyMode.NONE) { NativeBuildCreationConfigImpl(this, dslInfo.nativeBuildDslInfo!!, internalServices) }

  private val externalExtensions: Map<Class<*>, Any>? by lazy { variantBuilder.getRegisteredExtensions() }
  ...
  override val nestedComponents: List<ComponentImpl<*>>
    get() =
      mutableListOf<ComponentImpl<*>>().also { list ->
        (this as? HasTestFixtures)?.testFixtures?.let { list.add(it) }
  ...
  override val manifestPlaceholdersCreationConfig: ManifestPlaceholdersCreationConfig by
    lazy(LazyThreadSafetyMode.NONE) { createManifestPlaceholdersCreationConfig(dslInfo.manifestPlaceholdersDslInfo?.placeholders) }
  ...
  private fun deviceTests(): List<ComponentImpl<*>> =
    (this as? HasDeviceTests)?.deviceTests?.values?.map { it as ComponentImpl<*> } ?: listOf()

  private fun hostTests(): List<ComponentImpl<*>> =
    (this as? HasHostTestsCreationConfig)?.hostTests?.values?.map { it as ComponentImpl<*> } ?: listOf()
}
