...
class KaptCreationAction(
  ...
  override fun handleProvider(task: TaskProvider<out Kapt>) {
    ...
    artifacts
      .setInitialProvider(task) { it.destinationDir }
      .atLocation(creationConfig.kaptSourceOutputDir)
      .on(InternalArtifactType.BUILT_IN_KAPT_GENERATED_JAVA_SOURCES)
    artifacts
      .setInitialProvider(task) { it.kotlinSourcesDestinationDir }
      .atLocation(creationConfig.kaptKotlinSourceOutputDir)
      .on(InternalArtifactType.BUILT_IN_KAPT_GENERATED_KOTLIN_SOURCES)
    artifacts.setInitialProvider(task) { it.incAptCache }.on(InternalArtifactType.BUILT_IN_KAPT_INCREMENTAL_CACHE)
    artifacts.setInitialProvider(task) { it.classesDir }.on(InternalArtifactType.BUILT_IN_KAPT_CLASSES_DIR)
  }

  override fun configureTask(task: Kapt) {
    task.sourceSetName.set(creationConfig.name)

    creationConfig.java { task.source.from(it.all) }
    ...
    task.defaultJavaSourceCompatibility.set(creationConfig.sourceCompatibility.toString())
