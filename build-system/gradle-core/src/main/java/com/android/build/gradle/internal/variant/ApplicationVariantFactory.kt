...
class ApplicationVariantFactory(dslServices: DslServices) :
  AbstractAppVariantFactory<ApplicationVariantBuilder, ApplicationVariantDslInfo, ApplicationCreationConfig>(dslServices) {
  ...
  override fun createVariant(
    ...
    restrictRenderScriptOnRiscv(dslServices, appVariant, buildFeatures, globalConfig)

    return appVariant
  }

  override fun createBuildFeatureValues(buildFeatures: BuildFeatures, projectServices: ProjectServices): BuildFeatureValues {
    buildFeatures as? ApplicationBuildFeatures ?: throw RuntimeException("buildFeatures not of type ApplicationBuildFeatures")
  ...
  override fun createTestFixturesBuildFeatureValues(
    ...
    return TestFixturesBuildFeaturesValuesImpl(buildFeatures, projectServices, androidResourcesEnabled)
  }
  ...
  private fun computeOutputs(
    ...
    val variantOutputs = populateMultiApkOutputs(abis, globalConfig)
    variantOutputs.forEach { appVariant.addVariantOutput(it) }
    restrictEnabledOutputs(nativeBuildCreationConfig, appVariant.outputs, globalConfig)
  }

  private fun populateMultiApkOutputs(abis: Set<String>, globalConfig: GlobalTaskCreationConfig): List<VariantOutputConfigurationImpl> {
    ...
    val universalApkForAbi = (globalConfig.splits.abi.isEnable && globalConfig.splits.abi.isUniversalApk)
    if (universalApkForAbi) {
      variantOutputs.add(VariantOutputConfigurationImpl(isUniversal = true))
    } else {
      if (abis.isEmpty()) {
        variantOutputs.add(VariantOutputConfigurationImpl(isUniversal = true))
      }
        ...
        Consumer { abi: String ->
          variantOutputs.add(
            VariantOutputConfigurationImpl(
              filters = listOf(FilterConfigurationImpl(filterType = FilterConfiguration.FilterType.ABI, identifier = abi))
            )
          )
        }
      )
  ...
  private fun restrictEnabledOutputs(
    ...
    val buildTargetAbi =
      (if (projectOptions[BooleanOption.BUILD_ONLY_TARGET_ABI] || globalConfig.splits.abi.isEnable)
        projectOptions[StringOption.IDE_BUILD_TARGET_ABI]
      else null) ?: return
    val supportedAbis: Set<String> =
      component.userDefinedAbis.takeIf { it.isNotEmpty() }
        ?: if (Strings.nullToEmpty(buildTargetAbi).isEmpty()) component.supportedAbis else setOf()

    val genericBuiltArtifacts =
      variantOutputs
        .map { variantOutput ->
        ...
        .toMap()
    val computedBestArtifact =
      GenericBuiltArtifactsSplitOutputMatcher.computeBestArtifact(
        genericBuiltArtifacts.keys,
        supportedAbis,
        Arrays.asList(*Strings.nullToEmpty(buildTargetAbi).split(",".toRegex()).toTypedArray()),
      )
    if (computedBestArtifact == null) {
      ...
      dslServices.issueReporter.reportWarning(
        ...
        String.format(
          ...
          buildTargetAbi,
          if (supportedAbis.isEmpty()) Joiner.on(", ").join(splits) else Joiner.on(", ").join(supportedAbis),
        ),
      )
