...
private fun isArm(architecture: CpuArchitecture) =
  when (architecture) {
    CpuArchitecture.ARM,
    CpuArchitecture.X86_ON_ARM -> true
    else -> false
  }

/**
 * Suggests DSL changes to a [ManagedVirtualDevice] in order for it to refer to a valid device. This is broken into different steps, and the
 * number of suggestions is capped by [MAX_SUGGESTIONS]. (See [message])
 *
 * @property architecture The architecture of the device that the [ManagedVirtualDevice] will be run on. Affects which suggestions are made.
...
class ManagedDeviceImageSuggestionGenerator(
  ...
  /**
   * The suggestion message generated from the given device information. This is broken into different steps.
   *
   * First, we need to determine whether the device is invalid only for this architecture or _all_ architectures. This is important as the
   * [ManagedVirtualDevice] may be perfectly valid, but not intended to be run on the given device architecture. We need the ability to
   * clarify between this case, and the case where the DSL cannot refer to a valid image in it's present state.
   *
   * Once we have framed the suggestion based on the scenario, we then take the first few suggestions that can specify a valid image. The
   * suggestions are ordered roughly by how much the images are different from the desired image specified by the DSL.
   * 1. Recommend a compatible non-ATD (Automated Test Device) image. (Only for aosp-atd and google-atd images).
   * 2. Recommend a compatible image from a different image source. (google-apis -> aosp, etc.) 3a. Recommend the next available API level
   *    for the device. 3b. If there is not available higher API level, recommend the highest available api level
   * 4. Recommend not requiring a 64 bit image, as this may change what tests are run in native code this is the last recommendation given.
   *
   * If no suggestion could be made, this likely means the system image source is invalid, and another value in the dsl needs to be changed.
   * In this case, we recommend the image source be changed to one of the other available sources, in order to make suggestions on the next
   * run.
   */
  val message: String by lazy {
    ...
    if (recommendations.isNotEmpty()) {
      recommendations.forEachIndexed { index, suggestion -> suggestionMessage.appendLine().append("${index + 1}. $suggestion") }
    } else {
      ...
      // future.
      suggestValidSource()?.also { suggestionMessage.appendLine().append(it) }
    }
  ...
  private fun computeHash(
    ...
    val abi = computeAbiFromArchitecture(otherRequire64Bit, otherSdkVersion, otherImageSource, otherArch)
  ...
  private fun checkForExtensionSuggestion(): String? {
    ...
    val highestExtension = allImages.filter { parseApiFromHash(it) == sdkVersion }.maxOfOrNull { parseExtensionFromHash(it) ?: 0 } ?: 0
    ...
    return if (latestAvailableExtension != null) {
      ...
      "No explicit extension levels exist for SDK version $sdkVersion. Either unset " + "sdkExtensionVersion or try a different sdkVersion."
    }
  }
  ...
  private fun checkAllSourcesSuggestion(): String? {
    ...
    val allRelevantSources =
      when (systemImageSource) {
        "aosp-atd",
        "aosp_atd" -> listOf("google-atd", "google")
        "google-atd",
        "google_atd" -> listOf("aosp-atd", "aosp")
        "aosp",
        "default" -> listOf("aosp-atd", "google", "google-atd")
        "google",
  ...
  private fun checkForOtherSdkVersionSuggestion(): String? {
    ...
    val highestApi = allImages.maxOfOrNull { parseApiFromHash(it) ?: 0 } ?: 0
  ...
  private fun suggestValidSource(): String? {
    val sources =
      allImages
        ...
        .mapNotNull { parseSystemImageSourceFromHash(it) }
        .filterNot { isTvOrAutoSource(it) }
        .toMutableSet()
    if (sources.isEmpty()) {
      return null
    }
    sources.addAll(listOf("aosp", "aosp-atd", "google", "google-atd"))
