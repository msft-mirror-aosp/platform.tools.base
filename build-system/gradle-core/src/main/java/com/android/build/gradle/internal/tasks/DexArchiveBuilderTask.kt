...
/**
 * Task that converts CLASS files to dex archives, [com.android.builder.dexing.DexArchive]. This will process class files, and for each of
 * the input scopes (project, subprojects, external Maven libraries, mixed-scope classes), corresponding dex archive will be produced.
 *
 * This task is incremental, only changed classes will be converted again. If only a single class file is changed, only that file will be
 * dex'ed. Additionally, if a jar changes, only classes in that jar will be dex'ed.
 */
@CacheableTask
@BuildAnalyzer(primaryTaskCategory = TaskCategory.DEXING)
abstract class DexArchiveBuilderTask : NewIncrementalTask() {

  @get:Incremental @get:Classpath abstract val projectClasses: ConfigurableFileCollection

  @get:Incremental @get:Classpath abstract val subProjectClasses: ConfigurableFileCollection

  @get:Incremental @get:Classpath abstract val externalLibClasses: ConfigurableFileCollection

  /**
   * These are classes that contain multiple transform API scopes. E.g. if there is a transform running before this task that outputs
   * classes with both project and subProject scopes, this input will contain them.
   */
  @get:Incremental @get:Classpath abstract val mixedScopeClasses: ConfigurableFileCollection
  ...
  @get:Nested abstract val externalLibsFromArtifactTransformsOutputs: DexingOutputs

  @get:Nested abstract val mixedScopeOutputs: DexingOutputs

  @get:Nested abstract val dexParams: DexParameterInputs

  @get:LocalState @get:Optional abstract val desugarGraphDir: DirectoryProperty

  @get:Input abstract val projectVariant: Property<String>

  @get:LocalState abstract val inputJarHashesFile: RegularFileProperty

  /**
   * This property is annotated with [Internal] in order to allow cache hits across build that use different number of buckets. Changing
   * this property will not re-run the task, but that is fine. See [canRunIncrementally] for details how this impacts incremental builds.
   */
  @get:Internal abstract val numberOfBuckets: Property<Int>

  @get:LocalState abstract val previousRunNumberOfBucketsFile: RegularFileProperty

  @get:Incremental @get:PathSensitive(PathSensitivity.NONE) @get:InputFiles abstract val externalLibDexFiles: ConfigurableFileCollection

  /**
   * Task runs incrementally if input changes allow that and if the number of buckets is the same as in the previous run. This is necessary
   * in order to have correct incremental builds as the output location for an input file is computed using the number of buckets.
   *
   * The following scenarios are handled:
   * - changing the number of buckets between runs will cause non-incremental run
   * - cache hit will not restore [previousRunNumberOfBucketsFile], and it will cause next run to be non-incremental
   * - build in which the number of buckets is the same as in the [previousRunNumberOfBucketsFile] can be incremental
   */
  private fun canRunIncrementally(inputChanges: InputChanges): Boolean {
    val canRunIncrementally =
      if (!inputChanges.isIncremental) false
        ...
        with(previousRunNumberOfBucketsFile.asFile.get()) { if (!isFile) false else readText() == numberOfBuckets.get().toString() }
      }
  ...
  override fun doTaskAction(inputChanges: InputChanges) {
    ...
    if ((!externalLibDexFiles.isEmpty && !isIncremental) || getChanged(isIncremental, inputChanges, externalLibDexFiles).isNotEmpty()) {
    ...
    DexArchiveBuilderTaskDelegate(
        ...
        desugarClasspathChangedClasses = getChanged(isIncremental, inputChanges, dexParams.desugarClasspath),
  ...
  class CreationAction(creationConfig: ApkCreationConfig, classesClasspathUtils: ClassesClasspathUtils) :
    VariantTaskCreationAction<DexArchiveBuilderTask, ApkCreationConfig>(creationConfig),
    ...
    override fun handleProvider(taskProvider: TaskProvider<DexArchiveBuilderTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider) { it.projectOutputs.dex }
        .withName("out")
        .withName("out")
        .on(InternalArtifactType.PROJECT_DEX_ARCHIVE)
      creationConfig.artifacts
        .setInitialProvider(taskProvider) { it.subProjectOutputs.dex }
        .withName("out")
        .withName("out")
        .on(InternalArtifactType.SUB_PROJECT_DEX_ARCHIVE)
      creationConfig.artifacts
        .setInitialProvider(taskProvider) { it.externalLibsOutputs.dex }
        .withName("out")
        .on(InternalArtifactType.EXTERNAL_LIBS_DEX_ARCHIVE)
      creationConfig.artifacts
        .setInitialProvider(taskProvider) { it.externalLibsFromArtifactTransformsOutputs.dex }
        .withName("out")
        .on(InternalArtifactType.EXTERNAL_LIBS_DEX_ARCHIVE_WITH_ARTIFACT_TRANSFORMS)
      creationConfig.artifacts
        .setInitialProvider(taskProvider) { it.mixedScopeOutputs.dex }
        .withName("out")
        .on(InternalArtifactType.MIXED_SCOPE_DEX_ARCHIVE)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DexArchiveBuilderTask::inputJarHashesFile)
        .withName("out")
        .on(InternalArtifactType.DEX_ARCHIVE_INPUT_JAR_HASHES)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DexArchiveBuilderTask::desugarGraphDir)
        .withName("out")
        .on(InternalArtifactType.DESUGAR_GRAPH)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, DexArchiveBuilderTask::previousRunNumberOfBucketsFile)
        .withName("out")
        .on(InternalArtifactType.DEX_NUMBER_OF_BUCKETS_FILE)
      if (creationConfig.enableGlobalSynthetics) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider) { it.projectOutputs.globalSynthetics }
          .withName("out")
          .on(InternalArtifactType.GLOBAL_SYNTHETICS_PROJECT)
        creationConfig.artifacts
          .setInitialProvider(taskProvider) { it.subProjectOutputs.globalSynthetics }
          .withName("out")
          .on(InternalArtifactType.GLOBAL_SYNTHETICS_SUBPROJECT)
        creationConfig.artifacts
          .setInitialProvider(taskProvider) { it.externalLibsOutputs.globalSynthetics }
          .withName("out")
          .on(InternalArtifactType.GLOBAL_SYNTHETICS_EXTERNAL_LIB)
        creationConfig.artifacts
          .setInitialProvider(taskProvider) { it.mixedScopeOutputs.globalSynthetics }
          .withName("out")
          .on(InternalArtifactType.GLOBAL_SYNTHETICS_MIXED_SCOPE)
        creationConfig.artifacts
          .setInitialProvider(taskProvider) { it.externalLibsFromArtifactTransformsOutputs.globalSynthetics }
          .withName("out")
          .on(InternalArtifactType.GLOBAL_SYNTHETICS_EXTERNAL_LIBS_ARTIFACT_TRANSFORM)
      }
    }
    ...
    override fun configure(task: DexArchiveBuilderTask) {
      ...
      val languageDesugaring = dexingCreationConfig.java8LangSupportType == Java8LangSupport.D8
      ...
      if (languageDesugaringNeeded || libraryDesugaring) {
        task.dexParams.desugarBootclasspath.from(creationConfig.global.filteredBootClasspath)
      }

      task.dexParams.errorFormatMode.set(SyncOptions.getErrorFormatMode(projectOptions))
      task.dexParams.debuggable.setDisallowChanges(creationConfig.debuggable)
      task.projectVariant.set("${task.project.name}:${creationConfig.name}")
      task.numberOfBuckets.set(
        task.project.providers.provider { projectOptions.getProvider(IntegerOption.DEXING_NUMBER_OF_BUCKETS).orNull ?: DEFAULT_NUM_BUCKETS }
      )
      if (libraryDesugaring) {
        task.dexParams.desugarLibConfigFiles.setFrom(getDesugarLibConfigFiles(creationConfig.services))
      }
    ...
    private fun getDexForExternalLibs(task: DexArchiveBuilderTask, inputType: String): FileCollection {
      ...
      services.dependencies.registerTransform(DexingExternalLibArtifactTransform::class.java) {
        it.parameters.run {
      ...
      return detachedExtConf.incoming
        .artifactView { it.attributes.attribute(Attribute.of("artifactType", String::class.java), "ext-dex-$name") }
        .files
    }
  }
  ...
  abstract class DexingOutputs {
    ...
    @get:OutputDirectory abstract val dex: DirectoryProperty

    @get:Optional @get:OutputDirectory abstract val globalSynthetics: DirectoryProperty
  }

  companion object {
    /**
     * Some files will be reported as both added and removed, as order of inputs may shift and we are using @Classpath on inputs. For those,
     * ignore the removed change, and just handle them as added. For non-incremental builds return an empty set as dexing pipeline traverses
     * directories and we'd like to avoid serializing this information to the worker action.
     */
    fun getChanged(canRunIncrementally: Boolean, inputChanges: InputChanges, input: FileCollection): Set<FileChange> {
      if (!canRunIncrementally) {
        return emptySet()
      }
...
abstract class DexParameterInputs {
  ...
  @get:Input abstract val withDesugaring: Property<Boolean>

  @get:CompileClasspath abstract val desugarBootclasspath: ConfigurableFileCollection

  @get:Incremental @get:CompileClasspath abstract val desugarClasspath: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val desugarLibConfigFiles: ConfigurableFileCollection

  @get:Input abstract val enableGlobalSynthetics: Property<Boolean>
...
/**
 * Ad-hoc artifact transform used to desugar and dex external libraries, that is using Gradle built-in caching. Every external library is
 * desugared against classpath that consists of all external libraries.
 */
@CacheableTransform
abstract class DexingExternalLibArtifactTransform : BaseDexingTransform<DexingExternalLibArtifactTransform.Parameters>() {
  interface Parameters : BaseDexingTransform.Parameters {
    @get:CompileClasspath val desugaringClasspath: ConfigurableFileCollection
  }
...
/**
 * Implementation of the worker action that copies dex files and global synthetics to the final output locations. Originating files are
 * output of [DexingExternalLibArtifactTransform] transform.
 */
abstract class CopyDexOutput : ProfileAwareWorkAction<CopyDexOutput.Params>() {
  ...
  override fun run() {
    ...
    parameters.inputDirs.files.forEach { inputDir ->
      inputDir
        .walk()
        .filter { it.extension == SdkConstants.EXT_DEX }
        .forEach { dexFile -> dexFile.copyTo(parameters.outputDexDir.asFile.get().resolve("classes_ext_${dexId++}.dex")) }
      parameters.outputGlobalSynthetics.asFile.orNull?.let {
        inputDir.resolve(computeGlobalSyntheticsDirName(inputDir)).let { dir ->
          dir.walk().forEach { file -> file.copyTo(it.resolve("global_synthetics_${syntheticsId++}")) }
        }
      }
    }
  }
