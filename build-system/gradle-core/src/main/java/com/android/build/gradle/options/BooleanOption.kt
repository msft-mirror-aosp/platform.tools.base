...
enum class BooleanOption(
  ...
  BUILD_FEATURE_SHADERS("android.defaults.buildfeatures.shaders", false, ApiStage.Stable),
  ...
  /**
   * Setting this field to false indicates that in the current project, all the APKs installed during test will be uninstalled after test
   * finishes. Setting it to true means that the AGP will leave the test APKs untouched after test.
   *
   * Default is false
   */
  ...
  /**
   * When this option is enabled, dexing transforms will use the full classpath (if desugaring requires a classpath). This classpath
   * consists of all external artifacts ([com.android.build.gradle.internal.publishing.AndroidArtifacts.ArtifactScope.EXTERNAL]) in addition
   * to the input artifact's dependencies provided by Gradle through [org.gradle.api.artifacts.transform.InputArtifactDependencies].
   *
   * This option is useful when some dependencies are missing from the input artifact's dependencies (see bug 230454566), so the full
   * classpath is needed.
   *
   * If dexing transforms do not require a classpath, this option is not used.
   */
  USE_FULL_CLASSPATH_FOR_DEXING_TRANSFORM("android.useFullClasspathForDexingTransform", false, ApiStage.Stable),
  PRINT_LINT_STACK_TRACE("android.lint.printStackTrace", false, ApiStage.Stable),
  PACKAGE_NATIVE_DEBUG_METADATA_IN_APP_BUNDLE("android.bundle.includeNativeDebugMetadata", true, ApiStage.Stable),
  ...
  /** This creates a sync issue when library constraints are applied, because disabling them would result in a performance boost. */
  GENERATE_SYNC_ISSUE_WHEN_LIBRARY_CONSTRAINTS_ARE_ENABLED(
    "android.generateSyncIssueWhenLibraryConstraintsAreEnabled",
    true,
    ApiStage.Stable,
  ),
  ...
  R8_STRICT_FULL_MODE_FOR_KEEP_RULES(
    ...
    defaultValue = true,
    FeatureStage.Supported,
    FutureStage(Version.VERSION_10_0, true, FeatureStage.SoftlyEnforced(VERSION_11_0)),
  ),
  ...
  /**
   * Gradle Managed devices officially supports Android devices on API level 27 and higher because using the API level 26 and lower devices
   * increase instability. When a user tries to use those old API devices, GMD task throws an exception and task fails by default.
   *
   * When this flag is enabled, it allows a user to use any old API level devices regardless of its instability.
   */
   ...
   *
   * Intended use is to keep backwards compatibility for users who previously set android.dependency.useConstraints=false by turning this
   * on.
   */
  ...
  MISSING_LINT_BASELINE_IS_EMPTY_BASELINE("android.experimental.lint.missingBaselineIsEmptyBaseline", false, FeatureStage.Experimental),
  LEGACY_TRANSFORM_TASK_FORCE_NON_INCREMENTAL("android.experimental.legacyTransform.forceNonIncremental", false, FeatureStage.Experimental),
   ...
   * - jar -> processed-jar
   *
   * These should not be needed in most scenarios now, but keeping the option for backwards compatibility.
   */
  ...
  PRIVACY_SANDBOX_SDK_PLUGIN_SUPPORT("android.experimental.privacysandboxsdk.plugin.enable", false, FeatureStage.Experimental),
  ...
  FUSED_LIBRARY_PUBLICATION_ONLY_MODE("android.experimental.fusedLibrarySupport.publicationOnly", true, FeatureStage.Experimental),
  ...
  /** Suppresses the warning shown when package attribute is present in the main manifest, and is equal to the component's namespace. */
  SUPPRESS_MANIFEST_PACKAGE_WARNING("android.experimental.suppressManifestPackageWarning", false, FeatureStage.Experimental),

  /** Whether to disable AGP's addition of the -Xuse-inline-scopes-numbers flag for KotlinCompile tasks. */
  DISABLE_INLINE_SCOPES_NUMBERS("android.kotlin.disableInlineScopesNumbers", false, FeatureStage.Experimental),

  /** Whether to enable the deviceTargetingConfig option in app bundles. */
  ENABLE_DEVICE_TARGETING_CONFIG_API("android.experimental.enableDeviceTargetingConfigApi", false, FeatureStage.Experimental),

  /** Dump all artifacts locations in a json file in the variant build output folder. */
  DUMP_ARTIFACTS_LOCATIONS("android.debug.dumpArtifactsLocations", defaultValue = false, FeatureStage.Experimental),
  ...
  TEST_SIMULATE_AGP_VERSION_BEHAVIOR(
    ...
    false,
    FeatureStage.Experimental,
    FutureStage(Version.VERSION_10_0, true, FeatureStage.Experimental),
  ),
  TEST_SUITE_SUPPORT(
  ...
  /**
   * When enabled, lint will use the default baseline file (lint-baseline.xml, or target-specific in KMP projects) if a baseline file is not
  ...
  CUSTOM_SHADER_PATH_REQUIRED("android.custom.shader.path.required", true, FeatureStage.SoftlyEnforced(VERSION_9_0)),
  ENABLE_RESOURCE_OPTIMIZATIONS("android.enableResourceOptimizations", true, FeatureStage.SoftlyEnforced(VERSION_9_0)),
  ENABLE_EMULATOR_CONTROL("android.experimental.androidTest.enableEmulatorControl", true, FeatureStage.SoftlyEnforced(VERSION_9_0)),
  ...
  /**
   * Whether to do lint analysis per component (instead of analysing the main variant and the test components in the same lint invocation).
   */
  LINT_ANALYSIS_PER_COMPONENT("android.experimental.lint.analysisPerComponent", true, FeatureStage.SoftlyEnforced(VERSION_9_0)),
  PRIVACY_SANDBOX_SDK_ENABLE_LINT("android.experimental.privacysandboxsdk.enableLint", true, FeatureStage.SoftlyEnforced(VERSION_9_0)),

  /**
   * When enabled, the <uses-sdk> tag in AndroidManifest.xml will generate build errors if it is used to declare either minSdkVersion or
   * targetSdkVersion. The only allowed use case will be tools:overrideLibrary.
   */
  DISALLOW_USES_SDK_IN_MANIFEST("android.usesSdkInManifest.disallowed", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),
  ...
  /**
   * Enables R8 full mode (https://r8.googlesource.com/r8/+/refs/heads/8.8/compatibility-faq.md#r8-full-mode).
   *
   * Note that to help users migrate to R8 full mode, we provide 2 types of R8 full mode:
   * - Legacy full mode for keep rules ([R8_STRICT_FULL_MODE_FOR_KEEP_RULES] = false): In this mode, the default constructor is implicitly
   *   kept when a class is kept (i.e., `-keep class A` is the same as `-keep class A { <init>(); }`)
   * - Strict full mode for keep rules ([R8_STRICT_FULL_MODE_FOR_KEEP_RULES] = true): In this mode, the default constructor is not
   *   implicitly kept when a class is kept (i.e., `-keep class A` is different from `-keep class A { <init>(); }`).
   *
   * When migrating from legacy full mode to strict full mode, if the user's app or a library that the app uses contains a keep rule such as
   * `-keep class A`, then the app/library's author will need to manually update the rule to `-keep class A { <init>(); }` if they want to
   * keep the default constructor. If they don't want to keep the default constructor, then they can keep the rule as-is.
   */
  FULL_R8("android.enableR8.fullMode", defaultValue = true, FeatureStage.SoftlyEnforced(VERSION_10_0)),
  FAIL_ON_MISSING_PROGUARD_FILES("android.proguard.failOnMissingFiles", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),
  DEFAULT_MIN_COMPILE_SDK_IN_AAR_METADATA(
    "android.aar.metadata.defaultMinCompileSdkToCompileSdk",
    true,
    FeatureStage.SoftlyEnforced(VERSION_10_0),
  ),
  ENABLE_APP_COMPILE_TIME_R_CLASS("android.enableAppCompileTimeRClass", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),

  /**
   * Whether to enable built-in Kotlin support (https://issuetracker.google.com/259523353).
   *
   * When this property is enabled, AGP provides Kotlin support for all [Project]s without requiring users to apply the
   * `org.jetbrains.kotlin.android` plugin.
   * - If the user applies the `org.jetbrains.kotlin.android` plugin, the build will fail as AGP already provides Kotlin support.
   * - If the user applies the `com.android.built-in-kotlin` plugin, the build doesn't fail, but it also doesn't have any further effect.
   *
   * When this property is disabled, the users will need to apply either the `com.android.built-in-kotlin` plugin or the
   * `org.jetbrains.kotlin.android` plugin to have Kotlin support.
   * - If the user applies the `com.android.built-in-kotlin` plugin (recommended), AGP will provide Kotlin support for the current [Project]
   *   that the plugin is applied to.
   * - If the user applies the `org.jetbrains.kotlin.android` plugin (legacy behavior), that plugin will provide Kotlin support for the
   *   current [Project] that the plugin is applied to.
   * - If the user applies both plugins, the build will fail.
   */
  BUILT_IN_KOTLIN(propertyName = "android.builtInKotlin", defaultValue = true, stage = FeatureStage.SoftlyEnforced(VERSION_10_0)),
   ...
   *
   * The legacy implementation classes also includes the legacy variant API, so setting this subsumes setting
   * `android.enableLegacyVariantApi`
   */
  USE_NEW_DSL("android.newDsl", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),

  /**
   * Disallows users to pass Provider<*> instances to the Android sources set APIs like srcDir and srcDirs.
   *
   * It is not supported by default because the old variant API keeps on resolving it too early, and it is not possible for Android Studio
   * to know if the directory contains generated or static files which is important to make the files read-only or not.
   */
  ...
  R8_PROGUARD_ANDROID_TXT_DISALLOWED("android.r8.proguardAndroidTxt.disallowed", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),
   ...
   */
  R8_OPTIMIZED_RESOURCE_SHRINKING("android.r8.optimizedResourceShrinking", true, FeatureStage.SoftlyEnforced(VERSION_10_0)),
  ...
  BUILD_FEATURE_RESVALUES("android.defaults.buildfeatures.resvalues", false, ApiStage.Deprecated(VERSION_10_0)),

  // Flag used to indicate a "deploy as instant" run configuration.
  @Suppress("unused") IDE_DEPLOY_AS_INSTANT_APP(PROPERTY_DEPLOY_AS_INSTANT_APP, false, ApiStage.Deprecated(VERSION_9_0)),
  ...
  /**
   * This flag triggers some different behavior in the old Variant API for specific APIs that need to break the laziness of the new API.
   ...
   * ENFORCED FEATURES
   */
  @Suppress("unused") PREFER_CMAKE_FILE_API("android.preferCmakeFileApi", true, FeatureStage.Enforced(VERSION_7_0)),
  @Suppress("unused")
  ENABLE_NATIVE_CONFIGURATION_FOLDING("android.enableNativeConfigurationFolding", true, FeatureStage.Enforced(VERSION_7_0)),
  @Suppress("unused")
  ENABLE_SIDE_BY_SIDE_NDK(
    ...
    FeatureStage.Enforced(
      VERSION_4_1,
      "The android.enableSideBySideNdk property does not have any effect. " + "Side-by-side NDK is always enabled.",
    ),
  ),
  @Suppress("unused")
  ...
  DISABLE_RES_MERGE_IN_LIBRARY(
    ...
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "Resources from dependencies are never merged in libraries."),
  ),
  @Suppress("unused")
  ...
  ENABLE_INCREMENTAL_DESUGARING(
    ...
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "This property has no effect, incremental desugaring is always enabled."),
  ),
  @Suppress("unused")
  ENABLE_CORE_LAMBDA_STUBS(
    "android.enableCoreLambdaStubs",
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "This property has no effect, core-lambda-stubs.jar is always in the bootclasspath."),
  ),
  @Suppress("unused")
  ENABLE_DEX_ARCHIVE(
    "android.useDexArchive",
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "This property has no effect, incremental dexing is always used."),
  ),
  @Suppress("unused")
  ENABLE_AAPT2(
    "android.enableAapt2",
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "This property has no effect, AAPT2 is now always used."),
  ),
  @Suppress("unused")
  ...
  ENABLE_D8_MAIN_DEX_LIST(
    ...
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "This property has no effect, D8 is always used to compute the main dex list."),
  ),
  @Suppress("unused")
  ENABLE_DATA_BINDING_V2("android.databinding.enableV2", true, FeatureStage.Enforced(VERSION_BEFORE_4_0, "Databinding v1 is removed.")),
  ENABLE_SEPARATE_R_CLASS_COMPILATION(
    "android.enableSeparateRClassCompilation",
    true,
    FeatureStage.Enforced(VERSION_BEFORE_4_0, "Separate R class compilation has been enabled and can no longer be disabled."),
  ),
  @Suppress("unused")
  ENABLE_GRADLE_WORKERS("android.enableGradleWorkers", true, FeatureStage.Enforced(VERSION_4_2, "Gradle workers are always used.")),
  @Suppress("unused")
  ENABLE_D8(
    "android.enableD8",
    true,
    FeatureStage.Enforced(VERSION_7_0, "For more details, see https://d.android.com/r/studio-ui/d8-overview.html."),
  ),
  @Suppress("unused")
  ...
  NON_TRANSITIVE_APP_R_CLASS(
    ...
    FeatureStage.Enforced(
      VERSION_7_0,
      "Non-transitive R classes are now enabled in app modules with the " + "${NON_TRANSITIVE_R_CLASS.propertyName} option.",
    ),
  ),
  ...
  ENABLE_AAPT2_WORKER_ACTIONS(
    ...
    true,
    FeatureStage.Enforced(VERSION_7_3, "AAPT2 worker actions have been used unconditionally since Android Gradle Plugin 3.3"),
  ),
  ENABLE_JACOCO_TRANSFORM_INSTRUMENTATION("android.enableJacocoTransformInstrumentation", true, FeatureStage.Enforced(VERSION_7_3)),
  @Suppress("unused") ENABLE_SOURCE_SET_PATHS_MAP("android.enableSourceSetPathsMap", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  @Suppress("unused") RELATIVE_COMPILE_LIB_RESOURCES("android.cacheCompileLibResources", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  @Suppress("unused") R8_FAIL_ON_MISSING_CLASSES("android.r8.failOnMissingClasses", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  ...
  ENABLE_ART_PROFILES("android.enableArtProfiles", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  USE_RELATIVE_PATH_IN_TEST_CONFIG("android.testConfig.useRelativePath", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  INCLUDE_REPOSITORIES_IN_DEPENDENCY_REPORT(
    "android.bundletool.includeRepositoriesInDependencyReport",
    true,
    FeatureStage.Enforced(Version.VERSION_8_0),
  ),
  ENABLE_INCREMENTAL_DATA_BINDING("android.databinding.incremental", true, FeatureStage.Enforced(Version.VERSION_8_1)),
  ENABLE_NEW_RESOURCE_SHRINKER("android.enableNewResourceShrinker", true, FeatureStage.Enforced(Version.VERSION_8_0)),
  @Suppress("unused")
  ENABLE_R_TXT_RESOURCE_SHRINKING("android.enableRTxtResourceShrinking", true, FeatureStage.Enforced(Version.VERSION_8_1)),
  @Suppress("unused")
  COMPILE_CLASSPATH_LIBRARY_R_CLASSES("android.useCompileClasspathLibraryRClasses", true, FeatureStage.Enforced(Version.VERSION_8_1)),
  ...
  ENABLE_GLOBAL_SYNTHETICS("android.enableGlobalSyntheticsGeneration", true, FeatureStage.Enforced(Version.VERSION_8_1)),
  ...
  R8_INTEGRATED_RESOURCE_SHRINKING(
    ...
    FeatureStage.Enforced(
      Version.VERSION_9_0,
      additionalMessage =
        "The android.r8.integratedResourceShrinking property does not have any effect. " +
  ...
  /**
   * When enabled, Gradle Managed Device allows a custom managed device type that can be provided by a plugin by implementing
   * ManagedDeviceTestRunner APIs.
   */
  ...
  @Suppress("unused") FUSED_LIBRARY_SUPPORT("android.fusedLibrarySupport", true, FeatureStage.Enforced(Version.VERSION_9_0)),
  ...
  ENABLE_RESOURCE_NAMESPACING_DEFAULT(
    ...
    false,
    ApiStage.Removed(Version.VERSION_9_0, "The android.enableResourceNamespacingDefault property has no effect"),
  ),
  ...
  CONVERT_NON_NAMESPACED_DEPENDENCIES(
    ...
    false,
    ApiStage.Removed(Version.VERSION_9_0, "The android.convertNonNamespacedDependencies property has no effect"),
  ),
  BUILD_FEATURE_RENDERSCRIPT(
    "android.defaults.buildfeatures.renderscript",
    false,
    ApiStage.Removed(Version.VERSION_9_0, "The buildfeatures.renderscript property has no effect"),
  ),
  BUILD_FEATURE_AIDL(
    "android.defaults.buildfeatures.aidl",
    false,
    ApiStage.Removed(Version.VERSION_9_0, "The buildfeatures.aidl property has no effect"),
  ),
  ...
  @Suppress("unused")
  ENABLE_DEPRECATED_NDK("android.useDeprecatedNdk", false, FeatureStage.Removed(VERSION_BEFORE_4_0, "NdkCompile is no longer supported")),
  @Suppress("unused")
  INJECT_SDK_MAVEN_REPOS(
    "android.injectSdkMavenRepos",
    false,
    FeatureStage.Removed(VERSION_3_5, "The ability to inject the Android SDK maven repos is removed in AGP 3.5"),
  ),
  @Suppress("unused")
  ENABLE_UNIT_TEST_BINARY_RESOURCES(
    "android.enableUnitTestBinaryResources",
    false,
    FeatureStage.Removed(VERSION_BEFORE_4_0, "The raw resource for unit test functionality is removed."),
  ),
  @Suppress("unused")
  ENABLE_EXPERIMENTAL_FEATURE_DATABINDING(
    "android.enableExperimentalFeatureDatabinding",
    false,
    FeatureStage.Removed(VERSION_4_0, "This property has no effect. The features plugin was removed in AGP 4.0."),
  ),
  @Suppress("unused")
  ...
  KEEP_TIMESTAMPS_IN_APK(
    ...
    false,
    FeatureStage.Removed(VERSION_3_6, "The ability to keep timestamps in the APK is removed in AGP 3.6"),
  ),
  @Suppress("unused")
  ...
  @Suppress("unused")
  ENABLE_DESUGAR("android.enableDesugar", false, FeatureStage.Removed(VERSION_7_0, "Desugar tool has been removed from AGP.")),
  @Suppress("unused")
  ...
  @Suppress("unused")
  ENABLE_VCS_INFO(
    "android.enableVcsInfo",
    false,
    FeatureStage.Removed(Version.VERSION_8_3, "This feature is now enabled in the DSL per build type with \"vcsInfo.include = true\"."),
  ),
  @Suppress("unused")
  ADDITIONAL_ARTIFACTS_IN_MODEL(
    "android.experimental.additionalArtifactsInModel",
    false,
    FeatureStage.Removed(Version.VERSION_8_11, "Android Studio is responsible for managing multi-variant Javadoc/source support."),
  ),

  /** Note: Use [R8_OPTIMIZED_RESOURCE_SHRINKING] instead. */
  R8_OPTIMIZED_SHRINKING("android.r8.optimizedShrinking", false, FeatureStage.Removed(Version.VERSION_8_11)),

  /**
   * When enabled, Gradle Managed Device test results will be included in the mergeAndroidReports task from the android-reporting plugin.
   *
   * This will cause all managed devices to run for all variants in all subprojects when the mergeAndroidReports task is executed.
   */
  @Suppress("unused")
  GRADLE_MANAGED_DEVICE_INCLUDE_MANAGED_DEVICES_IN_REPORTING(
    ...
    false,
    FeatureStage.Removed(Version.VERSION_9_0, "The managedDevices.includeInMergedReport property has no effect"),
  ); // end of enums

  override val status = stage.status

  override val futureStage: FutureStage? =
    when (stage) {
      ...
      is FeatureStage.Enforced,
      is FeatureStage.Removed,
      is ApiStage.Removed -> {
        check(futureStage == null) {
