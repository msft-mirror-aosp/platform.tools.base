...
>(
  ...
  private val variants: MutableList<ComponentInfo<VariantBuilderT, VariantT>> = Lists.newArrayList()
  private val lazyManifestParserMap: MutableMap<File, LazyManifestParser> = Maps.newHashMapWithExpectedSize(3)
   ...
   * @see .createVariants
   */
  val testComponents: MutableList<TestComponentCreationConfig> = Lists.newArrayList()
  ...
  private fun getFlavorSelection(variantDslInfo: ComponentDslInfo): Map<Attribute<ProductFlavorAttr>, ProductFlavorAttr> {
    val factory = project.objects
    return variantDslInfo.missingDimensionStrategies.entries
      .stream()
      .collect(
        Collectors.toMap({ entry: Map.Entry<String, DimensionRequest> -> ProductFlavorAttr.of(entry.key) }) {
          entry: Map.Entry<String, DimensionRequest> ->
          factory.named(ProductFlavorAttr::class.java, entry.value.requested)
        }
      )
  }
  ...
  private fun computeVariants() {
    ...
    for (variant in variants) {
      val block = createVariantsFromCombination(variant, testBuildTypeData, globalConfig)
    ...
    get() {
      ...
      if (dslExtension is TestedExtension) {
        ...
        if (testBuildTypeData == null) {
          throw RuntimeException(String.format("Test Build Type '%1\$s' does not" + " exist.", dslExtension.testBuildType))
        }
      }
  ...
  private fun createVariant(
    ...
    val variantDslInfoBuilder =
      getBuilder<CommonExtensionT, VariantDslInfoT>(
        ...
        signingOverride,
        getLazyManifestParser(defaultConfigSourceProvider.manifestFile, componentType.requiresManifest),
        variantServices,
        dslExtension,
    ...
    for (productFlavorData in productFlavorDataList) {
      variantDslInfoBuilder.addProductFlavor(productFlavorData.productFlavor, productFlavorData.sourceSet)
    }
    ...
    val variantBuilder = variantFactory.createVariantBuilder(globalConfig, componentIdentity, variantDslInfo, variantBuilderServices)
    postVariantBuilderCreation(variantBuilder, buildTypeData)

    // now that we have the variant, create the analytics object,
    val configuratorService = getBuildService(project.gradle.sharedServices, AnalyticsConfiguratorService::class.java).get()
    val profileEnabledVariantBuilder = configuratorService.getVariantBuilder(project.path, variantBuilder.name)
    ...
    val variantSourceSets: MutableList<DefaultAndroidSourceSet?> = Lists.newArrayListWithExpectedSize(productFlavorDataList.size + 4)
    ...
    val variantData = variantFactory.createVariantData(componentIdentity, artifacts, variantServices)

    // then the new Variant which will contain the 2 old objects.
    val variantApiObject =
  ...
  private fun <DslInfoT : ComponentDslInfo> createCompoundSourceSets(
    ...
    if (productFlavorList.isNotEmpty() /* && !variantConfig.getType().isSingleBuildType()*/) {
      val variantSourceSet =
        variantInputModel.sourceSetManager.setUpSourceSet(computeSourceSetName(dslInfoBuilder.name, componentType), componentType).get()
    ...
    if (productFlavorList.size > 1) {
      val multiFlavorSourceSet =
        variantInputModel.sourceSetManager
          .setUpSourceSet(computeSourceSetName(dslInfoBuilder.flavorName, componentType), componentType)
          .get()
  ...
  private fun addExtraSourceSets(sourceSet: DefaultAndroidSourceSet) {
    variantApiOperationsRegistrar.onEachSourceSetExtensions { name: String -> sourceSet.extras.create(name) }
  }
  ...
  private fun createTestFixturesComponent(
    ...
    val testFixturesSourceSet = variantInputModel.defaultConfigData.getSourceSet(ComponentTypeImpl.TEST_FIXTURES)!!
    val variantDslInfoBuilder =
      getBuilder<CommonExtensionT, TestFixturesComponentDslInfo>(
        dimensionCombination,
        ...
        signingOverride,
        getLazyManifestParser(testFixturesSourceSet.manifestFile, testFixturesComponentType.requiresManifest),
        variantServices,
    ...
    variantDslInfoBuilder.productionVariant = mainComponentInfo.variantDslInfo as TestedVariantDslInfo
    ...
    for (productFlavor in productFlavorList) {
      productFlavors[productFlavor.name]?.let {
        variantDslInfoBuilder.addProductFlavor(it.productFlavor, it.getSourceSet(ComponentTypeImpl.TEST_FIXTURES)!!)
      }
    }
    ...
    buildTypeConfigurationProvider?.let { testFixturesVariantSourceSets.add(it) }
    ...
    testFixturesVariantSourceSets.add(variantInputModel.defaultConfigData.getSourceSet(ComponentTypeImpl.TEST_FIXTURES))
    ...
    val taskContainer = MutableTaskContainer()
    val testFixturesBuildFeatureValues =
      variantFactory.createTestFixturesBuildFeatureValues(
  ...
  fun <TestDslInfoT : TestComponentDslInfo> createTestComponents(
    ...
    val variantDslInfoBuilder =
      getBuilder<CommonExtensionT, TestDslInfoT>(
        dimensionCombination,
        componentType,
        variantInputModel.defaultConfigData.defaultConfig,
        testSourceSet!!,
        ...
        signingOverride,
        getLazyManifestParser(testSourceSet.manifestFile, componentType.requiresManifest),
        variantServices,
        extension = dslExtension,
        buildDirectory = project.layout.buildDirectory,
        dslServices = dslServices,
      )
    variantDslInfoBuilder.productionVariant = testedComponentInfo.variantDslInfo as TestedVariantDslInfo
    ...
    val productFlavors = variantInputModel.productFlavors
    for (productFlavor in productFlavorList) {
      productFlavors[productFlavor.name]?.let { variantDslInfoBuilder.addProductFlavor(it.productFlavor, it.getSourceSet(componentType)!!) }
    }

    val testComponentDslInfo =
      when (testBuilder) {
    ...
    // variant-specific if the full combo of flavors+build type. Does not exist if no flavors.
    // multi-flavor is the combination of all flavor dimensions. Does not exist if <2 dimension.
    val testVariantSourceSets: MutableList<DefaultAndroidSourceSet?> = Lists.newArrayListWithExpectedSize(4 + productFlavorList.size)
    ...
    buildTypeConfigurationProvider?.let { testVariantSourceSets.add(it) }
    ...
    // 4. the flavors.
    for (productFlavor in productFlavorList) {
      variantInputModel.productFlavors[productFlavor.name]?.let { testVariantSourceSets.add(it.getSourceSet(componentType)) }
    }

    // now add the default config
    testVariantSourceSets.add(variantInputModel.defaultConfigData.getSourceSet(componentType))
    ...
    val testVariantData = TestVariantData(componentIdentity, artifacts, variantServices, taskContainer)
      ...
      when (componentType) {
        ...
        ComponentTypeImpl.ANDROID_TEST ->
          variantFactory.createAndroidTest(
            ...
            variantFactory.createAndroidTestBuildFeatureValues(dslExtension.buildFeatures, dslExtension.dataBinding, projectServices),
  ...
  private fun createVariantsFromCombination(
    ...
    val productFlavorDataList: List<ProductFlavorData<ProductFlavor>> =
      dimensionCombination.productFlavors.mapNotNull { (_, second) -> variantInputModel.productFlavors[second] }
    val productFlavorList: List<ProductFlavor> = productFlavorDataList.map { it.productFlavor }
    var ignore = false
    oldExtension.variantFilter?.let {
      variantFilter.reset(dimensionCombination, defaultConfig, buildType, componentType, productFlavorList)
      try {
    ...
    val variantInfo: VariantComponentInfo<VariantBuilderT, VariantDslInfoT, VariantT> =
      createVariant(dimensionCombination, buildTypeData, productFlavorDataList, componentType, globalConfig) ?: return {}
    ...
    if (buildTypeData.buildType.isDebuggable && buildTypeData.buildType.isMinifyEnabled) {
      val warningMsg =
        """BuildType '${buildType.name}' is both debuggable and has 'isMinifyEnabled' set to true.
    ...
    val testFixturesEnabledForVariant =
      variantBuilder is HasTestFixturesBuilder && (variantBuilder as HasTestFixturesBuilder).enableTestFixtures

    if (testFixturesEnabledForVariant) {
      val testFixtures = createTestFixturesComponent(dimensionCombination, buildTypeData, productFlavorDataList, variantInfo)
    ...
    if (variantFactory.componentType.hasTestComponents) {
      (variantBuilder as? HasDeviceTestsBuilder)?.deviceTests?.forEach { (name, deviceTestBuilder) ->
        if (deviceTestBuilder.enable) {
          val deviceTest =
            createTestComponents<AndroidTestComponentDslInfo>(
              dimensionCombination,
              buildTypeData,
          ...
          (variant as HasDeviceTestsCreationConfig).addDeviceTest(DeviceTestBuilder.ANDROID_TEST_TYPE, deviceTest as DeviceTestImpl)
          if (name == DeviceTestBuilder.ANDROID_TEST_TYPE) {
            usedAndroidTestSourceSets.addAll(getComponentSourceSets(ComponentTypeImpl.ANDROID_TEST, productFlavorDataList, buildTypeData))
          }
        } else {
          if (name == DeviceTestBuilder.ANDROID_TEST_TYPE) {
            ignoredAndroidTestCheckAction.add {
              checkIgnoredDependencies(ComponentTypeImpl.ANDROID_TEST, productFlavorDataList, buildTypeData)
            }
          }
        }
      }
      ...
      (variantBuilder as? HasHostTestsBuilder)
        ...
        ?.forEach { (_, hostTestBuilder) ->
          val testComponent =
            createTestComponents<HostTestComponentDslInfo>(
              dimensionCombination,
              buildTypeData,
              productFlavorDataList,
              variantInfo,
          ...
          (variant as HasHostTestsCreationConfig).addTestComponent(hostTestBuilder.type, testComponent as HostTestCreationConfig)
        }

      (variantBuilder as? HasTestSuitesBuilder)?.suites?.forEach { (_, testSuiteBuilder: TestSuiteBuilder) ->
    ...
    val userVisibleVariant = variant.createUserVisibleVariantObject<Variant>(variantInfo.stats)
    ...
    val variantExtensionConfig =
      object : VariantExtensionConfig<Variant> {
        ...
        override fun <T> projectExtension(extensionType: Class<T>): T {
          val ext: ExtensionAware =
            if (projectServices.projectOptions[BooleanOption.USE_NEW_DSL]) {
              ...
              oldExtension as ExtensionAware
            }
    ...
    variantApiOperationsRegistrar.dslExtensions.forEach { registeredExtension ->
      registeredExtension.configurator.invoke(variantExtensionConfig).let {
        variantBuilder.registerExtension(if (it is GeneratedSubclass) it.publicType() else it.javaClass, it)
      }
    }
    ...
    return {
      ...
      variantAnalytics?.let {
        it
          ...
          .setHasUnitTest((variant as? HasHostTests)?.hostTests?.containsKey(HostTestBuilder.UNIT_TEST_TYPE) ?: false)
        ...
        if (variant is ApkCreationConfig) {
          ...
          if (supportType != Java8LangSupport.INVALID && supportType != Java8LangSupport.UNUSED) {
            variantAnalytics.java8LangSupport = AnalyticsUtil.toProto(supportType)
          }
          variantAnalytics.targetSdkVersion = AnalyticsUtil.toProto(variant.targetSdk)
        ...
        variant.maxSdk?.let { version -> variantAnalytics.setMaxSdkVersion(ApiVersion.newBuilder().setApiLevel(version.toLong())) }
      }
    }
  }
  ...
  private fun getLazyManifestParser(file: File, isManifestFileRequired: Boolean): LazyManifestParser {
    return lazyManifestParserMap.computeIfAbsent(file) { f: File? ->
      LazyManifestParser(
  ...
  private val canParseManifest =
    projectServices.objectFactory.property(Boolean::class.java).also {
  ...
  fun postVariantBuilderCreation(variantBuilder: VariantBuilderT, buildTypeData: BuildTypeData<BuildType>) {
    ...
    (variantBuilder as? HasHostTestsBuilder)?.hostTests[HostTestBuilder.UNIT_TEST_TYPE]?.let { unitTest ->
      unitTest.enable =
        !variantBuilderServices.projectOptions[BooleanOption.ENABLE_NEW_TEST_DSL] &&
          (!variantBuilderServices.projectOptions[BooleanOption.ONLY_ENABLE_UNIT_TEST_BY_DEFAULT_FOR_THE_TESTED_BUILD_TYPE] ||
            testBuildTypeData == null ||
            testBuildTypeData == buildTypeData)
    }
  }
  ...
  fun finalizeAllVariants() {
    finalizeAllComponents(variants.map { it.variant } + testComponents + testFixturesComponents)
  ...
  /** Returns part of the component sourcesets, notably not taking into account the variant specific sourcesets */
  private fun getComponentSourceSets(
    ...
    // Flavors
    productFlavorDataList.forEach { flavorData -> flavorData.getSourceSet(componentType)?.let { sourceSets.add(it) } }
    return sourceSets
  }
  ...
  private fun checkIgnoredDependencies(
    ...
    val sourceSets = getComponentSourceSets(componentType, productFlavorDataList, buildTypeData)
