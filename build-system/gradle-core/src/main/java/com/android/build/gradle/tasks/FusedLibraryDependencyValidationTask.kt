...
/** Intended to prevent building a fused library in an invalid state due to misconfiguration of included dependencies. */
@CacheableTask
...
abstract class FusedLibraryDependencyValidationTask : NonIncrementalGlobalTask() {
  ...
  @get:Input abstract val resolvedIncludeDependencies: Property<ResolvedComponentResult>

  /** Controls whether this task causes a build failure if there is a check failure. */
  @get:Input abstract val ignoreFailures: Property<Boolean>

  /** As sources may or may not exist, a file collection avoids configuration cache issues if there is not a source directory. */
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val srcDir: ConfigurableFileCollection
   ...
   */
  @get:OutputDirectory @get:Optional abstract val outputDirectory: DirectoryProperty
  ...
  /** Runs a series of test cases on each dependency of the fused library to prevent misconfigurations. */
  private fun checkDependencies(includeDependencies: ResolvedComponentResult) {
    val mergedDependencies: Set<ComponentIdentifier?> =
    ...
    val checks: List<Pair<String, (DependencyResultWithParentId) -> ValidationCheck.Result>> =
      listOf(
        "Unresolved Dependencies" to
          {
            when (it.dependency) {
              ...
              is ResolvedDependencyResult -> {
                ...
                ValidationCheck.Result.DidNotComplete("${it.dependency.javaClass} is not supported by this check.")
              }
            }
          },
        "Databinding is not supported by Fused Library modules" to
          {
            ...
            when (it.dependency) {
              is ResolvedDependencyResult -> {
                ...
                if (id is ModuleComponentIdentifier && id.group in databindingGroups) {
                  ValidationCheck.Result.Invalid("${id.moduleIdentifier} is not a permitted dependency.")
                } else {
                  ValidationCheck.Result.Valid
                }
              }
              is UnresolvedDependencyResult -> {
                if (
                  it.dependency.requested is ModuleComponentSelector &&
                    (it.dependency.requested as ModuleComponentSelector).group in databindingGroups
                ) {
                  ValidationCheck.Result.Invalid("${it.dependency.requested.displayName} is not a permitted dependency.")
                } else {
                  ValidationCheck.Result.DidNotComplete("${it.dependency.javaClass} is not supported by this check.")
                }
              }
        ...
        "Require transitive dependency inclusion" to
          {
            when (it.dependency) {
              is ResolvedDependencyResult -> {
                ...
                val id = it.dependency.selected.id
                if (id in mergedDependencies && parent !in mergedDependencies && !it.dependency.isConstraint) {
                  ValidationCheck.Result.Invalid(
    ...
    doChecks(includeDependencies, checks.map { ValidationCheck(it.first, it.second) })
  }

  private fun doChecks(includeDependencies: ResolvedComponentResult, resolvableDependencyChecks: List<ValidationCheck>) {
    val next =
      includeDependencies.dependencies
        .map {
          if (it is ResolvedDependencyResult) {
    ...
    while (next.any()) {
        ...
        .forEach { (checkName, result) ->
          when (result) {
            is ValidationCheck.Result.Invalid -> {
              failedChecks[checkName] = (notSupportedChecks[checkName] ?: emptySet()) + result.message
            }
            is ValidationCheck.Result.DidNotComplete -> {
              notSupportedChecks[checkName] = (notSupportedChecks[checkName] ?: emptySet()) + result.reason
            }
      ...
      if (dependency is ResolvedDependencyResult) {
        next.addAll(
          dependency.selected.dependencies.map { DependencyResultWithParentId(it, dependency.selected.id) }.filterNot { it in seen }
        )
      }
      ...
      "Validation failed due to ${failedChecks.size} issue(s) with ${projectPath.get()} dependencies:\n" +
        ...
        if (notSupportedChecks.any())
          "The following checks did not finish:\n" +
            notSupportedChecks.keys.joinToString(separator = "") {
              " [$it]:\n${
  ...
  private data class DependencyResultWithParentId(val dependency: DependencyResult, val parentId: ComponentIdentifier?)

  private class ValidationCheck(val name: String, val check: (dependency: DependencyResultWithParentId) -> Result) {
  ...
  class CreationAction(val creationConfig: FusedLibraryGlobalScope) : GlobalTaskCreationAction<FusedLibraryDependencyValidationTask>() {
    ...
    override fun handleProvider(taskProvider: TaskProvider<FusedLibraryDependencyValidationTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, FusedLibraryDependencyValidationTask::outputDirectory)
    ...
    override fun configure(task: FusedLibraryDependencyValidationTask) {
      ...
      task.resolvedIncludeDependencies.setDisallowChanges(includeConfiguration.incoming.resolutionResult.rootComponent)
      task.ignoreFailures.setDisallowChanges(
        creationConfig.experimentalProperties.map {
          !ModulePropertyKey.BooleanWithDefault.FUSED_LIBRARY_VALIDATE_DEPENDENCIES.getValue(creationConfig.experimentalProperties.get())
        }
      )
