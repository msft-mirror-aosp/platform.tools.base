...
/**
 * Delegate for the [DexArchiveBuilderTask]. This is where the actual processing happens. Using the inputs in the task, the delegate
 * instance is configured. Main processing happens in [doProcess].
 */
class DexArchiveBuilderTaskDelegate(
  ...
  /** Directory containing dependency graph(s) for desugaring, not `null` iff desugaring is enabled. */
  ...
  /** Classpath resources provider is shared between invocations, and this key uniquely identifies it. */
  data class ClasspathServiceKey(private val id: Long) : WorkerActionServiceRegistry.ServiceKey<ClassFileProviderFactory> {
  ...
  private fun deletePreviousOutputsFromDirs(inputFileChanges: Set<FileChange>, outputDir: File, globalSyntheticsDir: File?) {
      ...
      .forEach {
        when {
          it.fileType == FileType.DIRECTORY -> {
            ...
            globalSyntheticsDir?.let { globalSyntheticsDir -> globalSyntheticsDir.resolve(it.normalizedPath) }
          }
          ClassFileInput.CLASS_MATCHER.test(it.normalizedPath) -> {
            // We are in DexFilePerClassFile output mode
            DexFilePerClassFile.getDexOutputRelativePath(classFileRelativePath = it.normalizedPath).let { outputRelativePath ->
              FileUtils.deleteIfExists(outputDir.resolve(outputRelativePath))
            }
            globalSyntheticsDir?.let { globalSyntheticsDir ->
              DexFilePerClassFile.getGlobalSyntheticOutputRelativePath(it.normalizedPath).let { outputRelativePath ->
  ...
  private fun removeChangedJarOutputs(changes: Set<FileChange>, dexOutput: File, globalSyntheticsDir: File?) {
    changes
      .filter { it.file.extension == SdkConstants.EXT_JAR }
      .forEach {
        outputMapping.getPreviousOutputsForJar(it.file, dexOutput).forEach { FileUtils.deleteIfExists(it) }
        globalSyntheticsDir?.let { globalSyntheticsDir ->
          outputMapping.getPreviousOutputsForJar(it.file, globalSyntheticsDir).forEach { FileUtils.deleteIfExists(it) }
        }
      }
  }
  ...
  class DexingOutputs(val dex: File, val globalSynthetics: File?) {
    constructor(outputs: DexArchiveBuilderTask.DexingOutputs) : this(outputs.dex.asFile.get(), outputs.globalSynthetics.asFile.orNull)
  }
    ...
    for (bucketId in 0 until numberOfBuckets) {
      ...
      fun computeOutputPath(outputDir: File, dexOutput: Boolean): File {
        return when (inputs) {
          ...
          is JarBucketGroup -> {
            outputMapping.getOutputForJar(inputs.jarFile, outputDir, bucketId, dexOutput).also { FileUtils.mkdirs(it.parentFile) }
          }
        }
      }
      ...
      workerExecutor.noIsolation().submit(DexWorkAction::class.java) { params ->
        ...
        params.dexSpec.set(
          IncrementalDexSpec(
            ...
            dexParams = dexParams.toDexParametersForWorkers(inputs is DirectoryBucketGroup, bootClasspath, classpath),
            isIncremental = isIncremental,
            changedFiles = changedFiles,
            desugarGraphFile = desugarGraphDir?.let { getDesugarGraphFile(it, classBucket) },
          )
        )
      }
    }
  ...
  private fun getClasspath(withDesugaring: Boolean): List<Path> {
    ...
    return ArrayList<Path>(
        projectClasses.size + subProjectClasses.size + externalLibClasses.size + mixedScopeClasses.size + dexParams.desugarClasspath.size
      )
      .also { list ->
  ...
  private fun getBootClasspath(androidJarClasspath: List<File>, withDesugaring: Boolean): List<Path> {
    if (!withDesugaring) {
      return emptyList()
    }
  ...
  private fun getDesugarGraphFile(desugarGraphDir: File, classBucket: ClassBucket): File {
    return when (classBucket.bucketGroup) {
      is DirectoryBucketGroup -> File(desugarGraphDir, "dirs_bucket_${classBucket.bucketNumber}/graph.bin")
      is JarBucketGroup -> {
        ...
        File(desugarGraphDir, "jar_${Hashing.sha256().hashUnencodedChars(jarFilePath)}_" + "bucket_${classBucket.bucketNumber}/graph.bin")
      }
    }
  }

  /**
   * We are using file hashes to determine the output location for input jars. If the file containing mapping from absolute paths to hashes
   * exists, we will load it, and re-use its content for all unchanged files. For changed jar files, we will recompute the hash.
   *
   * The mapping also specifies if this run can be incremental, see [canProcessIncrementally]. This is possible only if the list of files
   * previously recorded is the same as the current list of input files (from all input scopes). E.g. in case the file is removed, mapping
   * will report non-incremental run, but this is ok as most of the incremental builds are changing the content of the jars, not their path.
   * Also, this avoids bugs like b/154712997.
   */
  private inner class OutputMapping(isAbleToRunIncrementally: Boolean) {
    ...
    init {
      val (fileHashes, isPreviousLoaded) =
        if (!inputJarHashesFile.exists() || !isAbleToRunIncrementally) {
          Pair(mutableMapOf(), false)
        } else {
          BufferedInputStream(inputJarHashesFile.inputStream()).use { input ->
            try {
              ObjectInputStream(input).use {
                @Suppress("UNCHECKED_CAST") val previousState = it.readObject() as MutableMap<File, String>
              ...
              loggerWrapper.warning("Reading jar hashes from $inputJarHashesFile failed. Exception: ${e.message}")
      ...
      fun getFileHash(file: File): String = file.inputStream().buffered().use { Hashing.sha256().hashBytes(it.readBytes()).toString() }

      if (isPreviousLoaded) {
        // Update hashes of changed files.
        sequenceOf(projectChangedClasses, subProjectChangedClasses, externalLibChangedClasses, mixedScopeChangedClasses)
          .flatten()
          .filter { it.file.extension == SdkConstants.EXT_JAR }
          .forEach {
      ...
      ObjectOutputStream(inputJarHashesFile.outputStream().buffered()).use { it.writeObject(fileHashes) }
    ...
    /**
     * Computes the output path without using the jar absolute path. This method will use the hash of the file content to determine the
     * final output path, and this makes sure the task is relocatable.
     */
    fun getOutputForJar(input: File, outputDir: File, bucketId: Int, withJarExtension: Boolean = true): File {
    ...
    fun getPreviousOutputsForJar(input: File, outputDir: File): List<File> {
      ...
      return Lists.newArrayListWithCapacity<File>(numberOfBuckets + 1).also {
        ...
        (0 until numberOfBuckets).forEach { bucketId -> it.add(computeOutputPath(outputDir, hash, bucketId)) }
      }
    }

    /** Check if the previous mapping contains exactly all files currently being processed. If not, return false. */
    ...
    private fun getAllFilesToProcess() =
      sequenceOf(projectClasses, subProjectClasses, externalLibClasses, mixedScopeClasses).flatten().filter {
        it.extension == SdkConstants.EXT_JAR
      }

    private fun computeOutputPath(outputDir: File, hash: String, bucketId: Int?, withJarExtension: Boolean = true): File {
