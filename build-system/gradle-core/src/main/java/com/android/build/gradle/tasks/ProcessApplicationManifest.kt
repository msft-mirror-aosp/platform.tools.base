...
abstract class ProcessApplicationManifest : ManifestProcessorTask() {
  ...
  @get:OutputFile abstract val mergedManifest: RegularFileProperty
  ...
  @get:Optional @get:Input abstract val baseModuleDebuggable: Property<Boolean>

  @get:Optional @get:Input abstract val packageOverride: Property<String>

  @get:Input abstract val warningsAsErrors: Property<Boolean>

  @get:Input abstract val namespace: Property<String>

  @get:Input abstract val profileable: Property<Boolean>

  @get:Input abstract val testOnly: Property<Boolean>
  ...
  @get:Input abstract val addLocaleConfigAttribute: Property<Boolean>
  ...
  override fun doTaskAction() {
    if (baseModuleDebuggable.isPresent) {
      val isDebuggable = optionalFeatures.get().contains(Invoker.Feature.DEBUGGABLE)
    ...
    val mergingReport =
      mergeManifests(
        ...
        generatedLocaleConfigAttribute =
          if (addLocaleConfigAttribute.get()) {
  ...
  @get:PathSensitive(PathSensitivity.RELATIVE) @get:InputFile abstract val mainManifest: Property<File>

  /**
   * Compute the final list of providers based on the manifest file collection and the other providers.
   *
  ...
  private fun computeFullProviderList(): List<ManifestProvider> {
    ...
    for (artifact in artifacts) {
      providers.add(ManifestProviderImpl(artifact.file, getArtifactName(artifact)))
    }
  ...
  @get:Optional @get:Input abstract val minSdkVersion: Property<String>

  @get:Optional @get:Input abstract val targetSdkVersion: Property<String>

  @get:Optional @get:Input abstract val maxSdkVersion: Property<Int>

  @get:Input abstract val optionalFeatures: SetProperty<Invoker.Feature>
  ...
  @get:Optional @get:Input abstract val featureName: Property<String>

  @get:Internal("only for task execution") abstract val projectBuildFile: RegularFileProperty

  @get:Input @get:Optional abstract val versionCode: Property<Int>

  @get:Input @get:Optional abstract val versionName: Property<String>
  ...
  class CreationAction(creationConfig: ApkCreationConfig) :
    ...
    override fun preConfigure(taskName: String) {
      ...
      artifacts.republish(InternalArtifactType.PACKAGED_MANIFESTS, InternalArtifactType.MANIFEST_METADATA)
    }

    override fun handleProvider(taskProvider: TaskProvider<ProcessApplicationManifest>) {
      super.handleProvider(taskProvider)
      val artifacts = creationConfig.artifacts
      artifacts.setInitialProvider(taskProvider, ProcessApplicationManifest::mergedManifest).on(SingleArtifact.MERGED_MANIFEST)

      artifacts
        .setInitialProvider(taskProvider, ManifestProcessorTask::mergeBlameFile)
        .withName("manifest-merger-blame-" + creationConfig.baseName + "-report.txt")
        .on(InternalArtifactType.MANIFEST_MERGE_BLAME_FILE)
      artifacts
        .setInitialProvider(taskProvider, ProcessApplicationManifest::reportFile)
        .atLocation(creationConfig.services.projectInfo.getOutputsDir().map { it.dir("logs") })
        .withName("manifest-merger-" + creationConfig.baseName + "-report.txt")
        .on(MANIFEST_MERGE_REPORT)
    }

    override fun configure(task: ProcessApplicationManifest) {
      ...
      task.compileSdk.setDisallowChanges(parseTargetHash(creationConfig.global.compileSdkHashString).apiLevel)
      task.minSdkVersion.setDisallowChanges(creationConfig.minSdk.getApiString())
      task.targetSdkVersion.setDisallowChanges(if (creationConfig.targetSdk.apiLevel < 1) null else creationConfig.targetSdk.getApiString())
      task.maxSdkVersion.setDisallowChanges((creationConfig as VariantCreationConfig).maxSdk)
      task.optionalFeatures.setDisallowChanges(creationConfig.services.provider { getOptionalFeatures(creationConfig) })
      task.jniLibsUseLegacyPackaging.setDisallowChanges(creationConfig.packaging.jniLibs.useLegacyPackaging)
      ...
      if (componentType.isBaseModule) {
        ...
        val dfCreationConfig = creationConfig as DynamicFeatureCreationConfig
      ...
      task.profileable.setDisallowChanges((creationConfig as? ApplicationCreationConfig)?.profileable ?: false)
      task.testOnly.setDisallowChanges(
        ProfilingMode.getProfilingModeType(creationConfig.services.projectOptions[StringOption.PROFILING_MODE]) != ProfilingMode.UNDEFINED
      )
      ...
      task.addLocaleConfigAttribute.setDisallowChanges(
        (creationConfig is ApplicationCreationConfig) && (creationConfig.androidResources.generateLocaleConfig == true)
      )
  ...
  companion object {
    private fun computeProviders(artifacts: Set<ResolvedArtifactResult>): List<ManifestProvider> {
    ...
    private fun getOptionalFeatures(creationConfig: ApkCreationConfig): EnumSet<Invoker.Feature> {
      val features: MutableList<Invoker.Feature> = ArrayList()
      val componentType = creationConfig.componentType
      ...
      return if (features.isEmpty()) EnumSet.noneOf(Invoker.Feature::class.java) else EnumSet.copyOf(features)
    }
  }
}
