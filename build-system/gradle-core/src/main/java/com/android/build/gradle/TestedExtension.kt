...
/**
 * An intermediate implementation class of the previous `android` extension for plugins with separate test components, such as the
 * application, library and dynamic-feature plugins.
 *
...
abstract class TestedExtension(
  ...
  BaseExtension(dslServices, bootClasspathConfig, buildOutputs, sourceSetManager, isBaseModule, stats),
  TestedAndroidConfig,
  com.android.build.api.dsl.TestedExtension {

  private val testVariantList: DomainObjectSet<TestVariant> = dslServices.domainObjectSet(TestVariant::class.java)

  private val unitTestVariantList: DomainObjectSet<UnitTestVariant> = dslServices.domainObjectSet(UnitTestVariant::class.java)

  /**
   * A collection of Android test [build variants](https://developer.android.com/studio/build/build-variants.html)
   *
   * To process elements in this collection, you should use
   * [`all`](https://docs.gradle.org/current/javadoc/org/gradle/api/DomainObjectCollection.html#all-org.gradle.api.Action-). That's because
   * the plugin populates this collection only after the project is evaluated. Unlike the `each` iterator, using `all` processes future
   * elements as the plugin creates them.
   *
   * To learn more about testing Android projects, read [Test your app](https://developer.android.com/studio/test/index.html)
   */
  ...
  /**
   * Returns a collection of Android unit test [build variants](https://developer.android.com/studio/build/build-variants.html).
   *
   * To process elements in this collection, you should use
   * [`all`](https://docs.gradle.org/current/javadoc/org/gradle/api/DomainObjectCollection.html#all-org.gradle.api.Action-). That's because
   * the plugin populates this collection only after the project is evaluated. Unlike the `each` iterator, using `all` processes future
