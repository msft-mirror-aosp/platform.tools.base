...
/**
 * A singleton that is responsible for registering services that runs during the kotlin model building phase to resolve dependencies,
 * serialize and deserialize extras.
 */
@OptIn(ExternalKotlinTargetApi::class)
internal object KotlinIdeImportConfigurator {

  /**
   * Configures IDE import hooks. Since this is called on creating the target which happens on applying the plugin, we pass in lazy values
   * that will not be available until afterEvaluate.
   *
   * @param sourceSetToCreationConfigMap the default android sourceSets that map to a single compilation that maps then to an android
   *   component.
   * @param extraSourceSetsToIncludeInResolution this includes sourceSets that only exist in android compilation, such as a sourceSet that
   *   was added to an android compilation or the common sourceSet in an android-only kmp module. Those sourceSets we treat as fully android
   *   in order to have full IDE android support for them.
   */
  fun configure(
    project: Project,
    ...
    registerExtrasSerializers(service)
  }
  ...
  private fun registerDependencyResolvers(
    ...
    val libraryResolver =
      LibraryResolver(
        project = project,
        libraryService =
          getBuildService(project.gradle.sharedServices, GlobalSyncService::class.java).get().let { globalLibraryBuildService ->
            LibraryServiceImpl(LibraryCacheImpl(globalLibraryBuildService.stringCache, globalLibraryBuildService.localJarCache))
          },
        sourceSetToCreationConfigMap = sourceSetToCreationConfigMap,
      )

    val resolutionPhase = IdeMultiplatformImport.DependencyResolutionPhase.BinaryDependencyResolution
        ...
        IdeBinaryDependencyResolver(
          artifactResolutionStrategy =
            IdeBinaryDependencyResolver.ArtifactResolutionStrategy.ResolvableConfiguration(
              configurationSelector = { sourceSet -> sourceSetToCreationConfigMap.value[sourceSet]?.variantDependencies?.compileClasspath },
              setupArtifactViewAttributes = { attribute(AndroidArtifacts.ARTIFACT_TYPE, AndroidArtifacts.ArtifactType.CLASSES_JAR.type) },
    ...
    if (disableRuntimeClasspath.not()) {
      ...
      service.registerAdditionalArtifactResolver(
        resolver =
          AndroidLibraryDependencyResolver(
            libraryResolver = libraryResolver,
            sourceSetToCreationConfigMap = sourceSetToCreationConfigMap,
            configType = AndroidArtifacts.ConsumedConfigType.RUNTIME_CLASSPATH,
          ),
    ...
    service.registerAdditionalArtifactResolver(
      resolver =
        KotlinModelBuildingHook(
          project = project,
          mainVariant =
            lazy(LazyThreadSafetyMode.NONE) { sourceSetToCreationConfigMap.value.values.filterIsInstance<KmpVariantImpl>().first() },
  ...
  private fun registerExtrasSerializers(service: IdeMultiplatformImport) {
    service.registerExtrasSerializationExtension(AndroidExtrasSerializationExtension())
  }
}
