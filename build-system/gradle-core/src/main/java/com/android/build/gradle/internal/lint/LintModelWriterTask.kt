 ...
 *
 * This serialized [LintModelModule] file is read by Lint in consuming projects to get all the information about this variant in project.
 *
...
abstract class LintModelWriterTask : NonIncrementalTask() {
  ...
  /**
   * We care only about this directory's location, not its contents. Gradle's recommendation is to annotate this with @Internal and add a
   ...
   */
  @get:Internal abstract val partialResultsDir: DirectoryProperty

  @get:Input @get:Optional val partialResultsDirPath: Provider<String> = partialResultsDir.locationOnly.map { it.asFile.absolutePath }

  @get:OutputDirectory abstract val outputDirectory: DirectoryProperty

  override fun doTaskAction() {
    val module = projectInputs.convertToLintModelModule()
    val variant = variantInputs.toLintModel(module, partialResultsDir.orNull?.asFile, desugaredMethodsFiles = listOf())
  ...
  /** [isForLocalReportTask] should be true only if the lint model is being written for the lint report task in the same module. */
  class CreationAction(
    ...
    override fun handleProvider(taskProvider: TaskProvider<LintModelWriterTask>) {
      ...
      if (isForLocalReportTask) {
        ...
        registerOutputArtifacts(taskProvider, if (fatalOnly) LINT_VITAL_LINT_MODEL else LINT_MODEL, creationConfig.artifacts)
      }
    }
    ...
    override fun configure(task: LintModelWriterTask) {
      ...
      val warnIfProjectTreatedAsExternalDependency = isForLocalReportTask && creationConfig.global.lintOptions.checkDependencies
  ...
  /**
   * [isMainModelForLocalReportTask] should be true only if (1) creationConfig is not a nested creation config and (2) the lint model is
   * being written for the lint report task in the same module.
   */
  class PerComponentCreationAction(
    ...
    override fun configure(task: LintModelWriterTask) {
      ...
      val warnIfProjectTreatedAsExternalDependency = isMainModelForLocalReportTask && creationConfig.global.lintOptions.checkDependencies
      task.variantInputs.initialize(
        task,
        mainVariant,
  ...
  companion object {
    fun registerOutputArtifacts(
      ...
      artifacts.setInitialProvider(taskProvider, LintModelWriterTask::outputDirectory).on(internalArtifactType)
    }
    ...
    fun registerOutputArtifacts(
      ...
      artifacts.use(taskProvider).wiredWith(LintModelWriterTask::outputDirectory).toAppendTo(internalArtifactType)
    }
  }
}
