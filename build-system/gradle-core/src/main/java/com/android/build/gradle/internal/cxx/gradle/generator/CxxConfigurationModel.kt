...
/**
 * The createCxxMetadataGenerator(...) function is meant to be use at task action time and specifically not during config-time. Any config-
 * time data needed for C/C++ build should come from [CxxConfigurationModel].
 *
 * Change to 'true' to check that createCxxMetadataGenerator(...) is not called at configuration time. But don't check it in set to 'true'.
 ...
 *
 * This enum is meant to enable different logging subsystems rather than to supplement or replace Gradle's existing verbosity levels.
 *
 ...
 *
 * If user specific externalNativeBuild.cmake.buildStagingFolder = 'xyz' then that folder will be used instead of the default of
 * moduleRoot/.cxx.
 *
 * If the resulting build output folder would be inside of moduleRoot/build then issue an error because moduleRoot/build will be deleted
 * when the user does clean and that will lead to undefined behavior.
 */
private fun findCxxFolder(buildSystem: NativeBuildSystem, moduleRootFolder: File, buildStagingDirectory: File?, buildFolder: File): File {
...
/**
 * Try to create C/C++ model configuration parameters [CxxConfigurationParameters]. Return null when there is no C/C++ in the user's project
 * or if there is some other kind of error. In the latter case, an error will have been reported.
 */
fun tryCreateConfigurationParameters(projectOptions: ProjectOptions, variant: VariantCreationConfig): CxxConfigurationParameters? {
  ...
  val cxxFolder =
    findCxxFolder(buildSystem, projectInfo.projectDirectory.asFile, buildStagingFolder, projectInfo.buildDirectory.asFile.get())
  ...
  /**
   * Construct an [NdkHandler] and attempt to auto-download an NDK. If auto-download fails then allow valid [errorln]s to pass or throw
   * exception that will trigger download hyperlinks in Android Studio
   */
  ...
  val chromeTraceJsonFolder =
    if (enableProfileJson) {
      val profileDir =
        option(PROFILE_OUTPUT_DIR)?.let { variant.services.file(it) }
  ...
  /** Prefab settings. */
  val prefabPackageDirectoryList =
    if (variant.buildFeatures.prefab) {
  ...
  val prefabPackageConfigurationList =
    if (variant.buildFeatures.prefab) {
      variant.variantDependencies
        .getArtifactCollection(
          AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH,
  ...
  val outputOptions =
    (option(NATIVE_BUILD_OUTPUT_LEVEL) ?: "")
      .uppercase(Locale.US)
      .split(",")
      .map { it.trim() }
      ...
      .mapNotNull { level ->
        ...
        if (result == null) {
          errorln(BUILD_OUTPUT_LEVEL_NOT_SUPPORTED, "$NATIVE_BUILD_OUTPUT_LEVEL contains unrecognized $level")
        }
        result
      }
      .toSet()
...
fun externalNativeBuildIsActive(creationConfig: ConsumableCreationConfig): Boolean {
  return creationConfig.nativeBuildCreationConfig?.let { nativeBuildCreationConfig ->
    getProjectPath(nativeBuildCreationConfig, creationConfig.global.externalNativeBuild)
  } != null
}
 ...
 * - If there are more than 1, then that is an error. The user has specified both cmake and ndkBuild in the same project.
 */
private fun getProjectPath(component: NativeBuildCreationConfig, config: ExternalNativeBuild): NativeProjectPath? {
  val externalProjectPaths =
    listOfNotNull(
  ...
  return when {
    externalProjectPaths.size > 1 -> {
      errorln(INVALID_EXTERNAL_NATIVE_BUILD_CONFIG, "More than one externalNativeBuild path specified")
      null
    }
...
/**
 * This function is used at task execution time to construct a [CxxMetadataGenerator] to do the task's work. It should only have parameters
 * that can be obtained after task graph deserialization.
 */
fun createCxxMetadataGenerator(abi: CxxAbiModel, analyticsService: AnalyticsService): CxxMetadataGenerator {
  if (ENABLE_CHECK_CONFIG_TIME_CONSTRUCTION) {
    check(!isGradleConfiguration()) { "Should not call createCxxMetadataGenerator(...) at configuration time" }
  }

  val variant = abi.variant

  val variantBuilder = analyticsService.getVariantBuilder(variant.module.gradleModulePathName, variant.variantName)

  return when (variant.module.buildSystem) {
    NINJA -> NinjaMetadataGenerator(abi, variantBuilder)
    NDK_BUILD -> NdkBuildExternalNativeJsonGenerator(abi, variantBuilder)
    CMAKE -> {
      ...
      if (cmakePath == null || cmakePath == "") {
        ...
        if (cmakeRevision.major < 3 || cmakeRevision.major == 3 && cmakeRevision.minor <= 6) {
...
private fun isGradleConfiguration(): Boolean {
  return Thread.currentThread().stackTrace.any { it.toString().contains("BasePlugin.createAndroidTasks") }
}
