...
abstract class MergeJavaResWorkAction : ProfileAwareWorkAction<MergeJavaResWorkAction.Params>() {
  ...
  private fun run(isIncremental: Boolean, changedInputs: SerializableInputChanges?) {
    ...
    inputDataList.addAll(
      parameters.projectJavaRes.map {
        InputData(it, determinePriority(ScopedArtifacts.Scope.PROJECT), "project(\"${parameters.projectPath.get()}\") - This project")
      }
    )

    inputDataList.addAll(
      parameters.subProjectJavaRes.get().map {
        InputData(it.input, determinePriority(InternalScopedArtifacts.InternalScope.SUB_PROJECTS), it.source)
      }
    )
    inputDataList.addAll(
      parameters.externalLibJavaRes.get().map {
        InputData(it.input, determinePriority(InternalScopedArtifacts.InternalScope.EXTERNAL_LIBS), it.source)
      }
    )
    inputDataList.addAll(
      parameters.featureJavaRes.get().map {
        InputData(it.input, determinePriority(InternalScopedArtifacts.InternalScope.FEATURES), it.source)
      }
    )

    val inputs =
      try {
        toInputs(inputDataList, changedInputs, zipCache, cacheUpdates, !isIncremental)
      } catch (e: IllegalStateException) {
        if (isIncremental) {
          Logging.getLogger(MergeJavaResWorkAction::class.java).debug("Incremental loading failed, fall back to non-incremental ", e)
    ...
    val mergeJavaResDelegate =
      MergeJavaResourcesDelegate(
        inputs,
        outputFile,
        ParsedPackagingOptions(parameters.excludes.get(), parameters.pickFirsts.get(), parameters.merges.get()),
  ...
  data class SourcedInput(val input: File, val source: String) : Serializable
