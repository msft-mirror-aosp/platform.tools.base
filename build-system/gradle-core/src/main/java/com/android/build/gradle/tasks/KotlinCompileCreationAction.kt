...
class KotlinCompileCreationAction(creationConfig: BuiltInKotlinCreationConfig, private val kotlinServices: BuiltInKotlinServices) :
  ...
  override fun handleProvider(task: TaskProvider<out KotlinJvmCompile>) {
    ...
    artifacts.setInitialProvider(task) { it.destinationDirectory }.withName("classes").on(InternalArtifactType.BUILT_IN_KOTLINC)
  }
  ...
  override fun configureTask(task: KotlinJvmCompile) {
    ...
    creationConfig.getBuiltInKaptArtifact(BUILT_IN_KAPT_GENERATED_KOTLIN_SOURCES)?.let { task.source(it) }
    creationConfig.getBuiltInKaptArtifact(BUILT_IN_KAPT_CLASSES_DIR)?.let { task.libraries.from(it) }
...
abstract class KotlinTaskCreationAction<TASK : Task>(protected val creationConfig: BuiltInKotlinCreationConfig) {
...
internal fun KotlinJvmCompile.configureKotlinJvmCompile(creationConfig: BuiltInKotlinCreationConfig) {
  creationConfig.kotlin { source(it.getAsFileTrees()) }
  creationConfig.java { source(it.getAsFileTrees()) }
