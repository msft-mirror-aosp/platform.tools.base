...
abstract class TaskManager(@JvmField protected val project: Project, @JvmField protected val globalConfig: GlobalTaskCreationConfig) {
  protected val logger: Logger = Logging.getLogger(this.javaClass)
  @JvmField protected val taskFactory: TaskFactory = TaskFactoryImpl(project.tasks)
  ...
  protected fun createVerifyLibraryResTask(component: ComponentCreationConfig) {
    ...
    component.taskContainer.assembleTask.configure { task: Task ->
      task.dependsOn(component.artifacts.get(InternalArtifactType.VERIFIED_LIBRARY_RESOURCES))
    }
  }
  ...
  protected fun createVerifyLibraryClassesTask(component: VariantCreationConfig) {
    ...
    component.taskContainer.assembleTask.configure { task: Task ->
      task.dependsOn(component.artifacts.get(InternalArtifactType.VERIFIED_LIBRARY_CLASSES))
    }
  }
  ...
  protected fun registerLibraryRClassTransformStream(component: ComponentCreationConfig) {
    ...
    component.artifacts
      .forScope(InternalScopedArtifacts.InternalScope.COMPILE_ONLY)
      .setInitialContent(ScopedArtifact.CLASSES, component.artifacts, InternalArtifactType.COMPILE_R_CLASS_JAR)
  }
  ...
  open class ConfigAttrTask : DefaultTask() {
    @get:Internal var consumable = false
    @get:Internal var resolvable = false

    @TaskAction
    fun run() {
      for (config in project.configurations) {
        val attributes = config.attributes
        if (consumable && config.isCanBeConsumed || resolvable && config.isCanBeResolved) {
          println(config.name)
          println("\tcanBeResolved: " + config.isCanBeResolved)
          println("\tcanBeConsumed: " + config.isCanBeConsumed)
          for (attr in attributes.keySet()) {
            println("\t" + attr.name + ": " + attributes.getAttribute(attr))
          }
  ...
  protected open fun createDependencyStreams(creationConfig: ComponentCreationConfig) {
    ...
    creationConfig.artifacts
      ...
      .setInitialContent(ScopedArtifact.CLASSES, getFinalRuntimeClassesJarsFromComponent(creationConfig, ArtifactScope.EXTERNAL))
    ...
    creationConfig.artifacts
      ...
      .setInitialContent(ScopedArtifact.CLASSES, getFinalRuntimeClassesJarsFromComponent(creationConfig, ArtifactScope.PROJECT))
      ...
      creationConfig.artifacts
        ...
        .setInitialContent(
          ...
          creationConfig.variantDependencies
            .getArtifactCollection(ConsumedConfigType.RUNTIME_CLASSPATH, ArtifactScope.PROJECT, AndroidArtifacts.ArtifactType.JAVA_RES)
            .artifactFiles,
        )
    }
    ...
    if (
      ...
      creationConfig.artifacts
        .forScope(InternalScopedArtifacts.InternalScope.FEATURES)
        .setInitialContent(
    ...
    creationConfig.artifacts
      .forScope(InternalScopedArtifacts.InternalScope.COMPILE_ONLY)
      .setInitialContent(ScopedArtifact.CLASSES, creationConfig.providedOnlyClasspath)
    (creationConfig as? TestComponentCreationConfig)?.onTestedVariant { testedVariant ->
      creationConfig.artifacts
        .forScope(InternalScopedArtifacts.InternalScope.TESTED_CODE)
        .setInitialContent(ScopedArtifact.CLASSES, getFinalRuntimeClassesJarsFromComponent(testedVariant, ArtifactScope.ALL))
    }
  }
  ...
  protected fun createRenderscriptTask(creationConfig: ConsumableCreationConfig) {
    creationConfig.renderscriptCreationConfig?.let { renderscriptCreationConfig ->
      ...
      val rsTask =
        taskFactory.register(
          RenderscriptCompile.CreationAction(creationConfig, ndkConfig = creationConfig.nativeBuildCreationConfig!!.ndkConfig)
        )
        ...
        creationConfig.sources.java {
          it.addSource(
            TaskProviderBasedDirectoryEntryImpl(
              ...
              directoryProvider = creationConfig.artifacts.get(InternalArtifactType.RENDERSCRIPT_SOURCE_OUTPUT_DIR),
            )
          )
        }
      }
  ...
  fun createMergeResourcesTask(creationConfig: ComponentCreationConfig, processResources: Boolean, flags: Set<MergeResources.Flag>) {
    if (!creationConfig.buildFeatures.androidResources && creationConfig !is DeviceTestCreationConfig) {
      return
    }
    val alsoOutputNotCompiledResources =
    ...
    taskFactory.register(MapSourceSetPathsTask.CreateAction(creationConfig))
  }
  ...
  fun createBuildConfigTask(creationConfig: ConsumableCreationConfig) {
    creationConfig.buildConfigCreationConfig?.let { buildConfigCreationConfig ->
      val generateBuildConfigTask = taskFactory.register(GenerateBuildConfig.CreationAction(creationConfig))
      val isBuildConfigBytecodeEnabled = creationConfig.services.projectOptions[BooleanOption.ENABLE_BUILD_CONFIG_AS_BYTECODE]
      if (
        !isBuildConfigBytecodeEnabled
  ...
  protected fun createApkProcessResTask(creationConfig: ComponentCreationConfig, packageOutputType: Single<Directory>?) {
    ...
    createProcessResTask(creationConfig, packageOutputType, MergeType.MERGE, projectInfo.getProjectBaseName())
    ...
    if (creationConfig.componentType.isForTesting && !isTestApkCompileRClassEnabled(enableAppCompileRClass, creationConfig.componentType)) {
      return
    }
    ...
    if (enableAppCompileRClass || nonTransitiveR) {
      ...
      createProcessResTask(creationConfig, packageOutputType, MergeType.PACKAGE, projectInfo.getProjectBaseName())
    }
  }

  private fun isTestApkCompileRClassEnabled(compileRClassFlag: Boolean, componentType: ComponentType): Boolean {
  ...
  fun createProcessResTask(
    ...
    creationConfig.oldVariantApiLegacySupport?.variantData?.calculateFilters(creationConfig.global.splits)
    ...
    val useAaptToGenerateLegacyMultidexMainDexProguardRules =
      (creationConfig is ApkCreationConfig && creationConfig.dexing.dexingType.isLegacyMultiDex)
  ...
  private fun createNonNamespacedResourceTasks(
    ...
    when (mergeType) {
      MergeType.PACKAGE -> {
        ...
        if (generatesProguardOutputFile(creationConfig) && creationConfig.componentType.isAar) {
          taskFactory.register(GenerateLibraryProguardRulesTask.CreationAction(creationConfig))
        }
        ...
        if (!nonTransitiveRClassInApp || compileTimeRClassInApp || creationConfig.componentType.isAar) {
          taskFactory.register(GenerateLibraryRFileTask.CreationAction(creationConfig))
        }
      }
      MergeType.MERGE -> {
        ...
        if (!creationConfig.componentType.isForTesting) {
          check(creationConfig is ApkCreationConfig) {
            "Expected a component that produces an apk, instead found " + "${creationConfig.name} of type ${creationConfig::class.java}."
          }
          taskFactory.register(LinkAndroidResForBundleTask.CreationAction(creationConfig))
        }

        artifacts
          .forScope(ScopedArtifacts.Scope.PROJECT)
          .setInitialContent(ScopedArtifact.CLASSES, artifacts, COMPILE_AND_RUNTIME_R_CLASS_JAR)
   ...
   *
   * [Sync] task configured with [ProcessJavaResTask.CreationAction] will sync all source folders into a single folder identified by
  ...
  protected fun createProcessJavaResTask(creationConfig: ComponentCreationConfig) {
    ...
    val taskConfig =
      object : ProcessJavaResCreationConfig, TaskCreationConfig by creationConfig {
  ...
  protected open fun postJavacCreation(creationConfig: ComponentCreationConfig) {
    ...
    creationConfig.oldVariantApiLegacySupport?.variantData?.let { variantData ->
      creationConfig.artifacts
        .forScope(ScopedArtifacts.Scope.PROJECT)
        .setInitialContent(ScopedArtifact.POST_COMPILATION_CLASSES, variantData.allPreJavacGeneratedBytecode)
  ...
  protected fun createJavacTask(creationConfig: ComponentCreationConfig): TaskProvider<out JavaCompile> {
    ...
    val javaCompileConfig = createJavaCompileConfig(creationConfig, usingKapt)
    val javacTask: TaskProvider<out JavaCompile> = taskFactory.register(JavaCompileCreationAction(javaCompileConfig))
  ...
  /**
   * If the Jetbrains KAPT plugin is applied, copy the properties from its kapt extension to the kotlinServices.factory.kaptExtension.
   *
   * TODO(b/341765853) - remove this after [MINIMUM_BUILT_IN_KOTLIN_VERSION] >= 2.1.0-Beta2 because the kapt extension is passed to the task
   *   registration functions starting with Kotlin 2.1.0-Beta2
   */
  private fun copyKaptExtensionProperties(kotlinServices: BuiltInKotlinServices) {
    project.pluginManager.withPlugin(KOTLIN_KAPT_PLUGIN_ID) {
      project.afterEvaluate {
        val jetbrainsKaptExtension = project.extensions.findByName("kapt") as? KaptExtensionConfig ?: return@afterEvaluate
        kotlinServices.kotlinBaseApiPlugin.kaptExtension.also {
          ...
          it.dumpDefaultParameterValues = jetbrainsKaptExtension.dumpDefaultParameterValues
          it.includeCompileClasspath = jetbrainsKaptExtension.includeCompileClasspath
  ...
  private fun addSubpluginOptionsForBuiltInKotlin(
    ...
    fun Project.addMavenDependency(configuration: String, artifact: SubpluginArtifact) {
      ...
      val mavenCoordinate = "${artifact.groupId}:${artifact.artifactId}${artifactVersion?.let { ":$it" }.orEmpty()}"
    ...
    val targetId = kotlinCompilation.target.disambiguationClassifier?.replaceFirstChar { it.uppercase() } ?: ""
    ...
    if (appliedSubplugins.isNotEmpty()) {
      val pluginConfiguration =
        project.configurations.maybeCreate(pluginConfigurationName).also {
          it.isCanBeResolved = true
          it.isCanBeConsumed = false
        }
      kotlinCompilation.compileTaskProvider.configure { (it as? AbstractKotlinCompile<*>)?.pluginClasspath?.from(pluginConfiguration) }
  ...
  /**
   * Create a "kotlin-extension" configuration that holds the androidx compose kotlin compiler extension, if the configuration doesn't
   * already exist.
   *
   * Returns the "kotlin-extension" configuration.
   */
  protected fun maybeCreateKotlinExtensionConfiguration(): Configuration {
    project.configurations.findByName("kotlin-extension")?.also {
      return it
    }
    val kotlinExtension =
      project.configurations.create("kotlin-extension") {
    ...
    val kotlinCompilerExtensionVersion =
      globalConfig.composeOptions.kotlinCompilerExtensionVersion ?: COMPOSE_KOTLIN_COMPILER_EXTENSION_VERSION
    project.dependencies.add(kotlinExtension.name, "androidx.compose.compiler:compiler:$kotlinCompilerExtensionVersion")
   ...
   * @param variantName name of the variant under test. This can be different from the testing variant.
   * @param testTaskSuffix the suffix to be applied to the individual task names. This should be used if the test config's name does not
   *   include a test suffix.
   */
  protected fun createTestDevicesForVariant(
    ...
    val allDevicesVariantTask =
      taskFactory.register(creationConfig.computeTaskNameInternal("allDevices", testTaskSuffix)) { allDevicesVariant: Task ->
        allDevicesVariant.description = "Runs the tests for $variantName on all managed devices in the dsl."
        allDevicesVariant.group = JavaBasePlugin.VERIFICATION_GROUP
      }
    taskFactory.configure(globalConfig.taskNames.allDevicesCheck) { allDevices: Task -> allDevices.dependsOn(allDevicesVariantTask) }

    val resultsRootDir =
      if (globalConfig.androidTestOptions.resultsDir.isNullOrEmpty()) {
        creationConfig.paths.outputDir(BuilderConstants.FD_ANDROID_RESULTS).get().asFile
      } else {
        File(requireNotNull(globalConfig.androidTestOptions.resultsDir))
      }
    val reportRootDir =
      if (globalConfig.androidTestOptions.resultsDir.isNullOrEmpty()) {
        creationConfig.paths.reportsDir(BuilderConstants.FD_ANDROID_TESTS).get().asFile
      } else {
        File(requireNotNull(globalConfig.androidTestOptions.reportDir))
      }
    val additionalOutputRootDir =
      creationConfig.paths.outputDir(InternalArtifactType.MANAGED_DEVICE_ANDROID_TEST_ADDITIONAL_OUTPUT.getFolderName()).get().asFile
    val coverageOutputRootDir =
      creationConfig.paths.outputDir(InternalArtifactType.MANAGED_DEVICE_CODE_COVERAGE.getFolderName()).get().asFile

    val flavorDir = if (flavor.isNullOrEmpty()) "" else "${BuilderConstants.FD_FLAVORS}/$flavor"
    val resultsDir = File(resultsRootDir, "${BuilderConstants.MANAGED_DEVICE}/$buildTarget/$flavorDir")
    val reportDir = File(reportRootDir, "${BuilderConstants.MANAGED_DEVICE}/$buildTarget/$flavorDir")
    ...
    for (managedDevice in managedDevices) {
          ...
          registration != null -> {
            val setupResult: Provider<Directory>? =
              if (registration.hasSetupActions) {
                taskFactory.named(setupTaskName(managedDevice)).flatMap { task -> (task as ManagedDeviceSetupTask).setupResultDir }
              } else {
                null
              }
            taskFactory.register(
      ...
      taskFactory.configure(managedDeviceAllVariantsTaskName(managedDevice)) { managedDeviceTests: Task ->
    ...
    for (managedDevice in managedDevices) {
      taskFactory.configure(managedDeviceAllVariantsTaskName(managedDevice)) { managedDeviceCheck ->
        managedDeviceCheck.dependsOn(testResultAggregationTask)
      }
    }
    deviceToProvider.values.forEach { managedDeviceTestTask ->
      testResultAggregationTask.configure { it.mustRunAfter(managedDeviceTestTask) }
      // Run test result aggregation task even after test failures.
      managedDeviceTestTask.configure { it.finalizedBy(testResultAggregationTask) }
    }
    ...
    if (creationConfig is TestComponentCreationConfig && creationConfig.codeCoverageEnabled) {
      ...
      deviceToProvider.values.forEach { managedDeviceTestTask ->
        ...
        reportTask.configure { it.mustRunAfter(managedDeviceTestTask) }
      }
      taskFactory.configure(globalConfig.taskNames.allDevicesCheck) { allDevices: Task -> allDevices.dependsOn(reportTask) }
    }

    // Lastly the Device Group Tasks.
    for (group in getDeviceGroups()) {
      val variantDeviceGroupTask =
        taskFactory.register(managedDeviceGroupSingleVariantTaskName(creationConfig, group).appendCapitalized(testTaskSuffix)) {
          deviceGroupVariant: Task ->
          deviceGroupVariant.description = "Runs the tests for $variantName on all devices defined in ${group.name}."
      ...
      taskFactory.configure(managedDeviceGroupAllVariantsTaskName(group)) { deviceGroupTask: Task ->
  ...
  protected fun createPostCompilationTasks(creationConfig: ApkCreationConfig) {
    ...
    if (creationConfig.requiresJacocoTransformation && !creationConfig.componentType.isForTesting) {
      ...
      enumValues<ScopedArtifacts.Scope>().forEach {
        creationConfig.artifacts.forScope(it).republish(ScopedArtifact.CLASSES, InternalScopedArtifact.FINAL_TRANSFORMED_CLASSES)
      }
    }
    ...
    if (
      creationConfig.componentType.isDynamicFeature || (creationConfig as? ApplicationCreationConfig)?.shrinkingWithDynamicFeatures == true
    ) {
  ...
  /** initialize the [ScopedArtifacts.Scope.ALL] scope from the project, sub-projects and external libraries. */
  internal fun initializeAllScope(artifacts: ArtifactsImpl) {
    ...
    artifacts
      ...
      .run {
        ...
        from(artifacts.forScope(InternalScopedArtifacts.InternalScope.SUB_PROJECTS).getFinalArtifacts(ScopedArtifact.CLASSES))
        from(artifacts.forScope(InternalScopedArtifacts.InternalScope.EXTERNAL_LIBS).getFinalArtifacts(ScopedArtifact.CLASSES))
      }

    // initialize the all scoped classes
    artifacts.forScope(ScopedArtifacts.Scope.ALL).getScopedArtifactsContainer(ScopedArtifact.CLASSES).initialScopedContent.run {
      from(artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES))
      from(artifacts.forScope(InternalScopedArtifacts.InternalScope.SUB_PROJECTS).getFinalArtifacts(ScopedArtifact.CLASSES))
      from(artifacts.forScope(InternalScopedArtifacts.InternalScope.EXTERNAL_LIBS).getFinalArtifacts(ScopedArtifact.CLASSES))
    }

    artifacts.forScope(ScopedArtifacts.Scope.ALL).getScopedArtifactsContainer(ScopedArtifact.JAVA_RES).initialScopedContent.run {
      from(artifacts.forScope(InternalScopedArtifacts.InternalScope.SUB_PROJECTS).getFinalArtifacts(ScopedArtifact.JAVA_RES))
      from(artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.JAVA_RES))
      from(artifacts.forScope(InternalScopedArtifacts.InternalScope.EXTERNAL_LIBS).getFinalArtifacts(ScopedArtifact.JAVA_RES))
    }
  }
  ...
  /**
   * Creates tasks used for DEX generation. This will use an incremental pipeline that uses dex archives in order to enable incremental
   * dexing support.
   */
  private fun createDexTasks(creationConfig: ApkCreationConfig, dexingType: DexingType) {
    ...
    val separateFileDependenciesDexingTask =
      (creationConfig.dexing.java8LangSupportType == Java8LangSupport.D8 && classpathUtils.enableDexingArtifactTransform)
  ...
  protected fun getClassPathUtils(creationConfig: ApkCreationConfig): ClassesClasspathUtils {
    ...
    val supportsDesugaringViaArtifactTransform = (java8LangSupport == Java8LangSupport.UNUSED || java8LangSupport == Java8LangSupport.D8)

    val classesAlteredThroughVariantAPI =
      creationConfig.artifacts
        .forScope(ScopedArtifacts.Scope.ALL)
    ...
    val enableDexingArtifactTransform = supportsDesugaringViaArtifactTransform && !classesAlteredThroughVariantAPI

    return ClassesClasspathUtils(creationConfig, enableDexingArtifactTransform, classesAlteredThroughVariantAPI)
  }

  /**
   * Set up dex merging tasks when artifact transforms are used.
   *
   * External libraries are merged in mono-dex and native multidex modes. In case of a native multidex debuggable variant these dex files
   * get packaged. In mono-dex case, we will re-merge these files. Because this task will be almost always up-to-date, having a second
   * merger run over the external libraries will not cause a performance regression. In addition to that, second dex merger will perform
   * less I/O compared to reading all external library dex files individually. For legacy multidex, we must merge all dex files in a single
   * invocation in order to generate correct primary dex file in presence of desugaring. See b/120039166.
   *
   * When merging native multidex, debuggable variant, project's dex files are merged independently. Also, the library projects' dex files
   * are merged independently.
   *
   * For all other variants (release, mono-dex, legacy-multidex), we merge all dex files in a single invocation. This means that external
   * libraries, library projects and project dex files will be merged in a single task.
   */
  private fun createDexMergingTasks(
    ...
    when (dexingType) {
      DexingType.MONO_DEX -> {
        ...
        taskFactory.register(
          DexMergingTask.CreationAction(
            createDexMergingCreationConfig(creationConfig, DexMergingAction.MERGE_ALL, dexingType, dexingUsingArtifactTransforms)
          )
        )
      }
      ...
      DexingType.NATIVE_MULTIDEX -> {
        ...
        if (creationConfig.debuggable) {
          ...
          taskFactory.register(
            DexMergingTask.CreationAction(
              createDexMergingCreationConfig(creationConfig, DexMergingAction.MERGE_PROJECT, dexingType, dexingUsingArtifactTransforms)
            )
          )
          taskFactory.register(
            DexMergingTask.CreationAction(
  ...
  private fun handleJacocoDependencies(creationConfig: ComponentCreationConfig) {
    if (creationConfig is ApkCreationConfig && creationConfig.packageJacocoRuntime) {
      ...
      creationConfig.variantDependencies.runtimeClasspath.resolutionStrategy { r: ResolutionStrategy ->
  ...
  protected open fun createJacocoTask(creationConfig: ComponentCreationConfig) {
    ...
    val classesAlteredThroughVariantAPI =
      creationConfig.artifacts
        ...
        .artifactsAltered
        .get()
    val scope = if (classesAlteredThroughVariantAPI) ScopedArtifacts.Scope.ALL else ScopedArtifacts.Scope.PROJECT
  ...
  protected fun createDataBindingTasksIfNecessary(creationConfig: ComponentCreationConfig) {
    ...
    taskFactory.register(DataBindingMergeDependencyArtifactsTask.CreationAction(creationConfig))
      ...
      creationConfig.sources.java {
        it.addSource(
          TaskProviderBasedDirectoryEntryImpl(
            ...
            directoryProvider = creationConfig.artifacts.get(InternalArtifactType.DATA_BINDING_TRIGGER),
          )
        )
      }
  ...
  private fun setDataBindingAnnotationProcessorParams(creationConfig: ComponentCreationConfig) {
    val processorOptions = creationConfig.javaCompilation.annotationProcessor

    val dataBindingArgs = createArguments(creationConfig, logger.isDebugEnabled, DataBindingBuilder.getPrintMachineReadableOutput())
  ...
  protected fun createPackagingTask(creationConfig: ApkCreationConfig) {
    ...
    taskFactory.register(
      PackageApplication.CreationAction(creationConfig, creationConfig.paths.apkLocation),
      null,
    ...
    taskFactory.configure(creationConfig.global.taskNames.uninstallAll) { uninstallAll: Task -> uninstallAll.dependsOn(uninstallTask) }
  }
  ...
  protected fun createValidateSigningTask(creationConfig: ApkCreationConfig) {
    ...
    val service: Provider<AndroidLocationsBuildService> =
      getBuildService(creationConfig.services.buildServiceRegistry, AndroidLocationsBuildService::class.java)

    // FIXME create one per signing config instead of one per variant.
    taskFactory.register(ValidateSigningTask.CreationAction(creationConfig, service.get().getDefaultDebugKeystoreLocation()))
  }
    ...
    taskFactory.register(
      ...
      object : TaskConfigAction<Task> {
        override fun configure(task: Task) {
          task.description = "Assembles main output for variant " + component.name
        }
      },
  ...
  /** Actually creates the minify transform, using the given mapping configuration. The mapping is only used by test-only modules. */
  protected fun doCreateJavaCodeShrinkerTask(creationConfig: ConsumableCreationConfig, isTestApplication: Boolean = false) {
  ...
  private fun createR8Task(
    ...
    if (creationConfig is ApplicationCreationConfig) {
      publishArtifactsToDynamicFeatures(creationConfig, FEATURE_DEX, AndroidArtifacts.ArtifactType.FEATURE_DEX, null)
  ...
  /**
   * We have a separate method for publishing artifacts back to the features (instead of using the typical PublishingSpecs pipeline) because
   * multiple artifacts are published with different attributes for the given ArtifactType in this case.
   *
   * <p>This method will publish each of the children of the directory corresponding to the given internalArtifactType. The children files'
   * names must match the names of the corresponding feature modules + the given fileExtension.
   *
   * @param creationConfig the ApplicationCreationConfig
   * @param internalArtifactType the InternalArtifactType of the directory whose children will be published to the features
   * @param artifactType the ArtifactType used when publishing to the features
   * @param fileExtension the fileExtension of the directory's children files, or null if the children files are directories
   */
  private fun publishArtifactsToDynamicFeatures(
    ...
    val configuration = creationConfig.variantDependencies.getElements(PublishedConfigSpec(PublishedConfigType.RUNTIME_ELEMENTS))
    Preconditions.checkNotNull(
      configuration,
      "Publishing to Runtime Element with no Runtime Elements configuration object. " + "componentType: " + creationConfig.componentType,
    )
    ...
    for (modulePath in modulePaths) {
      ...
      publishArtifactToConfiguration(
        ...
        file,
        artifactType,
        AndroidAttributes(AndroidArtifacts.MODULE_PATH to project.absoluteProjectPath(modulePath)),
      )
    }
  }

  /** If resource shrinker is enabled, set-up and register the appropriate tasks. */
  private fun maybeCreateResourcesShrinkerTasks(creationConfig: ApkCreationConfig) {
    if ((creationConfig as? ApplicationCreationConfig)?.runResourceShrinking() == true && !creationConfig.runResourceShrinking()) {
      // For the APK
  ...
  protected fun createAnchorTasks(creationConfig: ComponentCreationConfig) {
    ...
    creationConfig.taskContainer.sourceGenTask =
      taskFactory.register(creationConfig.computeTaskNameInternal("generate", "Sources")) { task: Task ->
        ...
        listOfNotNull(creationConfig.sources.res, creationConfig.sources.java, creationConfig.sources.kotlin).forEach { code ->
          code.forAllSources { directoryEntry ->
            if (directoryEntry.isUserAdded && directoryEntry.isGenerated) {
              directoryEntry.makeDependentOf(task)
            }
          }
        }
      }
    creationConfig.taskContainer.resourceGenTask = taskFactory.register(ValidateResourcesTask.CreateAction(creationConfig))
    creationConfig.taskContainer.assetGenTask = taskFactory.register(creationConfig.computeTaskNameInternal("generate", "Assets"))
    ...
    if (creationConfig is VariantCreationConfig && creationConfig.requiresJacocoTransformation) {
      creationConfig.taskContainer.coverageReportTask =
        taskFactory.register(creationConfig.computeTaskNameInternal("create", "CoverageReport")) { task: Task ->
          task.group = JavaBasePlugin.VERIFICATION_GROUP
          task.description = String.format("Creates instrumentation test coverage reports for the %s variant.", creationConfig.name)
        }
    }
  ...
  protected open fun createDebugTasks(creationConfig: ComponentCreationConfig) {
    if (creationConfig.services.projectOptions[BooleanOption.DUMP_ARTIFACTS_LOCATIONS]) {
      val artifactsLocationsReportTask = taskFactory.register(ArtifactsLocationsReportTask.CreationAction(creationConfig))
      creationConfig.taskContainer.preBuildTask.configure { preBuild -> preBuild.finalizedBy(artifactsLocationsReportTask) }
    }
  }
  ...
  private fun createCompileAnchorTask(creationConfig: ComponentCreationConfig) {
    ...
    taskContainer.compileTask =
      taskFactory.register(creationConfig.computeTaskNameInternal("compile", "Sources")) { task: Task -> task.group = BUILD_GROUP }
  }

  protected fun configureTestData(creationConfig: InstrumentedTestCreationConfig, testData: AbstractTestDataImpl) {
    testData.animationsDisabled = creationConfig.services.provider(globalConfig.androidTestOptions::animationsDisabled)
  }

  private fun maybeCreateCheckDuplicateClassesTask(creationConfig: ComponentCreationConfig) {
  ...
  private fun maybeCreateDesugarLibTask(apkCreationConfig: ApkCreationConfig) {
    if (apkCreationConfig.dexing.shouldPackageDesugarLibDex) {
      ...
      if (apkCreationConfig.dexing.needsShrinkDesugarLibrary) {
        taskFactory.register(ExpandArtProfileWildcardsTask.ExpandL8ArtProfileCreationAction(apkCreationConfig))
      }

      taskFactory.register(L8DexDesugarLibTask.CreationAction(apkCreationConfig))
    }
  }

  protected fun getManagedDevices(): List<Device> {
    return globalConfig.androidTestOptions.managedDevices.allDevices.toList()
  }

  protected fun getDeviceGroups(): Collection<DeviceGroup> = globalConfig.androidTestOptions.managedDevices.groups

  protected fun maybeCreateTransformClassesWithAsmTask(creationConfig: ComponentCreationConfig) {
    ...
    if (instrumentationCreationConfig.projectClassesAreInstrumented) {
      val transformTask = taskFactory.register(TransformClassesWithAsmTask.CreationAction(creationConfig))
      creationConfig.artifacts
        .forScope(ScopedArtifacts.Scope.PROJECT)
        .use(transformTask)
        .toTransform(
      ...
      if (instrumentationCreationConfig.asmFramesComputationMode == FramesComputationMode.COMPUTE_FRAMES_FOR_ALL_CLASSES) {
  ...
  companion object {
    ...
    const val CONNECTED_ANDROID_TEST = BuilderConstants.CONNECTED + ComponentType.ANDROID_TEST_SUFFIX
    ...
    fun createTasksBeforeEvaluate(
      ...
      taskFactory.register(globalConfig.taskNames.uninstallAll) { uninstallAllTask: Task ->
        uninstallAllTask.description = "Uninstall all applications."
        uninstallAllTask.group = INSTALL_GROUP
      }
      taskFactory.register(globalConfig.taskNames.deviceCheck) { deviceCheckTask: Task ->
        deviceCheckTask.description = "Runs all device checks using Device Providers and Test Servers."
        deviceCheckTask.group = JavaBasePlugin.VERIFICATION_GROUP
      }
      taskFactory.register(globalConfig.taskNames.connectedCheck, DeviceSerialTestTask::class.java) {
        connectedCheckTask: DeviceSerialTestTask ->
        connectedCheckTask.description = "Runs all device checks on currently connected devices."
      ...
      taskFactory.register(ASSEMBLE_ANDROID_TEST) { assembleAndroidTestTask: Task ->
      ...
      taskFactory.register(ASSEMBLE_UNIT_TEST) { assembleUnitTestTask: Task ->
      ...
      if (globalConfig.services.projectOptions[BooleanOption.ENABLE_SCREENSHOT_TEST]) {
        taskFactory.register(ASSEMBLE_SCREENSHOT_TEST) { assembleScreenshotTestTask: Task ->
      ...
      // for testing only.
      taskFactory.register("resolveConfigAttr", ConfigAttrTask::class.java) { task: ConfigAttrTask -> task.resolvable = true }
      taskFactory.register("consumeConfigAttr", ConfigAttrTask::class.java) { task: ConfigAttrTask -> task.consumable = true }
    ...
    private fun generatesProguardOutputFile(creationConfig: ComponentCreationConfig): Boolean {
      return ((creationConfig is ConsumableCreationConfig && creationConfig.optimizationCreationConfig.minifiedEnabled) ||
        creationConfig.componentType.isDynamicFeature)
    }

    /** Makes the given task the one used by top-level "compile" task. */
    @JvmStatic
    fun setJavaCompilerTask(javaCompilerTask: TaskProvider<out JavaCompile>, creationConfig: ComponentCreationConfig) {
     ...
     * @param fileExtension the desired file extension (e.g., ".jar"), or null if no file extension (e.g., for a folder)
     * @return name of file
     */
    fun getFeatureFileName(modulePath: String, fileExtension: String?): String {
    ...
    @JvmStatic
    fun getTaskPath(project: Project, taskName: String) = if (project.rootProject === project) ":$taskName" else "${project.path}:$taskName"
  }
}
