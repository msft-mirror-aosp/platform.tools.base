...
class MutableMapBackedUpWithMapProperty<K : Any, V : Any>(private val mapProperty: MapProperty<K, V>, private val propertyName: String) :
  ...
  override val entries: MutableSet<MutableMap.MutableEntry<K, V>>
    get() =
      ReadOnlyMutableSet(
        _get().entries,
        "Cannot add elements using the $propertyName entries() returned Set," + " use the original collection",
      )

  override val keys: MutableSet<K>
    get() =
      ReadOnlyMutableSet(_get().keys, "Cannot add elements using the $propertyName keys() returned Set," + " use the original collection")

  override val values: MutableCollection<V>
    get() =
      ReadOnlyMutableCollection(
        _get().values,
        "Cannot add elements using the $propertyName values() returned collection, " + "use the original collection",
      )
  ...
  override fun remove(key: K): V? {
    throw NotImplementedError(
      "Cannot remove elements from the $propertyName map, first do a clear() " + "and add back selectively the elements you want to keep"
    )
  }
