...
abstract class TestOptions @Inject constructor(private val dslServices: DslServices) : com.android.build.api.dsl.TestOptions {
  ...
  override val unitTests: UnitTestOptions = dslServices.newInstance(UnitTestOptions::class.java, dslServices)
  ...
  override var execution: String
    get() = Verify.verifyNotNull(executionConverter.reverse().convert(_execution), "No string representation for enum.")
    set(value) {
      @Suppress("NULLABILITY_MISMATCH_BASED_ON_JAVA_ANNOTATIONS")
      _execution = Preconditions.checkNotNull(executionConverter.convert(value), "The value of `execution` cannot be null.")
    }
  ...
  abstract class UnitTestOptions @Inject constructor(dslServices: DslServices) : com.android.build.api.dsl.UnitTestOptions {
    ...
    /**
     * Configures a given test task. The configuration closures that were passed to {@link #all(Closure)} will be applied to it.
     *
     * <p>Not meant to be called from build scripts. The reason it exists is that tasks are created after the build scripts are evaluated,
     * so users have to "register" their configuration closures first and we can only apply them later.
     *
     * @since 1.2.0
     */
  ...
  private fun createTargetSdkSpec(): TargetSdkSpecImpl {
    return dslServices.newDecoratedInstance(TargetSdkSpecImpl::class.java, dslServices).also { it.version = targetSdkVersion }
  }
