...
abstract class LintTool {
   ...
   *
   * The lint cache is neither an input nor an output to the lint tasks, so it needs some manual handling to avoid lint trying to load cache
   * items written by a different version of lint.
   *
   * A marker file of lint-cache-version is used, for published versions it will include the version of lint, e.g. `30.2.0-alpha05`
   *
   * And for -dev versions, also a hash of the jars, the same as the classloader hash 30.2.0-dev_920ff9cabfbb40d0318735f9fe403b9
   *
  ...
  internal fun deriveVersionKey(
    ...
    val lintVersion = getLintMavenArtifactVersion(taskCreationServices.projectOptions[StringOption.LINT_VERSION_OVERRIDE]?.trim(), null)
  ...
  fun submit(workerExecutor: WorkerExecutor, mainClass: String, arguments: List<String>, lintMode: LintMode, useK2Uast: Boolean) {
    submit(
      workerExecutor,
      mainClass,
      arguments,
...
abstract class ProjectInputs {
  ...
  @get:Internal abstract val projectDirectoryPath: Property<String>
  ...
  @get:Input @get:Optional abstract val projectDirectoryPathInput: Property<String>

  @get:Input abstract val projectGradlePath: Property<String>

  @get:Input abstract val projectType: Property<LintModelModuleType>
  ...
  @get:Internal abstract val buildDirectoryPath: Property<String>
  ...
  @get:Input @get:Optional abstract val buildDirectoryPathInput: Property<String>

  @get:Nested abstract val lintOptions: LintOptionsInput

  @get:Input @get:Optional abstract val resourcePrefix: Property<String>

  @get:Input abstract val dynamicFeatures: ListProperty<String>
  ...
  @get:Input abstract val compileTarget: Property<String>

  /**
   * True if none of the build types used by this module have enabled shrinking, or false if at least one variant's build type is known to
   * use shrinking.
   */
  @get:Input abstract val neverShrinking: Property<Boolean>

  /**
   * [lintConfigFiles] contains all possible lint.xml files in the module's directory or any parent directories up to the root project
   * directory.
   */
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val lintConfigFiles: ConfigurableFileCollection
  ...
  internal fun initializeForStandalone(
    ...
    if (javaCompileTask != null) {
      bootClasspath.from(javaCompileTask.map { it.options.bootstrapClasspath ?: project.files() })
    }
  ...
  internal fun convertToLintModelModule(): LintModelModule {
    return DefaultLintModelModule(
      ...
      mavenName = DefaultLintModelMavenName(mavenGroupId.get(), mavenArtifactId.get(), mavenVersion.get()),
  ...
  /**
   * Initialize [lintConfigFiles] with all possible lint.xml files in the module's directory or any parent directories up to the root
   * project directory.
   */
...
abstract class LintOptionsInput {
  @get:Input abstract val disable: SetProperty<String>
  @get:Input abstract val enable: SetProperty<String>
  @get:Input abstract val checkOnly: SetProperty<String>
  @get:Input abstract val abortOnError: Property<Boolean>
  @get:Input abstract val absolutePaths: Property<Boolean>
  @get:Input abstract val noLines: Property<Boolean>
  @get:Input abstract val quiet: Property<Boolean>
  @get:Input abstract val checkAllWarnings: Property<Boolean>
  @get:Input abstract val ignoreWarnings: Property<Boolean>
  @get:Input abstract val warningsAsErrors: Property<Boolean>
  @get:Input abstract val checkTestSources: Property<Boolean>
  @get:Input abstract val checkGeneratedSources: Property<Boolean>
  @get:Input abstract val explainIssues: Property<Boolean>
  @get:Input abstract val showAll: Property<Boolean>
...
abstract class SystemPropertyInputs {
  ...
  @get:Input @get:Optional abstract val androidLintLogJarProblems: Property<String>

  // Use @get:Internal because javaVersion acts as proxy input for javaHome
  @get:Internal abstract val javaHome: Property<String>

  @get:Input @get:Optional abstract val javaVersion: Property<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val lintApiDatabase: RegularFileProperty

  @get:Input @get:Optional abstract val lintAutofix: Property<String>

  @get:Input @get:Optional abstract val lintBaselinesContinue: Property<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val lintConfigurationOverride: RegularFileProperty

  @get:Input @get:Optional abstract val lintHtmlPrefs: Property<String>

  @get:Input @get:Optional abstract val lintNullnessIgnoreDeprecated: Property<String>

  @get:Input @get:Optional abstract val lintUnusedResourcesExcludeTests: Property<String>

  @get:Input @get:Optional abstract val lintUnusedResourcesIncludeTests: Property<String>
  ...
  fun initialize(providerFactory: ProviderFactory, lintMode: LintMode) {
    if (lintMode == LintMode.ANALYSIS) {
      ...
      lintBaselinesContinue.setDisallowChanges(providerFactory.systemProperty("lint.baselines.continue"))
    ...
    lintConfigurationOverride.fileProvider(
      providerFactory.systemProperty("lint.configuration.override").map { File(it) }.filter { it.isFile }
    )
    lintConfigurationOverride.disallowChanges()
    lintNullnessIgnoreDeprecated.setDisallowChanges(providerFactory.systemProperty("lint.nullness.ignore-deprecated"))
    lintUnusedResourcesExcludeTests.setDisallowChanges(providerFactory.systemProperty("lint.unused-resources.exclude-tests"))
    lintUnusedResourcesIncludeTests.setDisallowChanges(providerFactory.systemProperty("lint.unused-resources.include-tests"))
  }
}
...
abstract class EnvironmentVariableInputs {
  ...
  @get:Input @get:Optional abstract val androidLintIncludeLdpi: Property<String>

  @get:Input @get:Optional abstract val androidLintMaxDepth: Property<String>

  @get:Input @get:Optional abstract val androidLintMaxViewCount: Property<String>

  @get:Input @get:Optional abstract val androidLintNullnessIgnoreDeprecated: Property<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val lintApiDatabase: RegularFileProperty

  @get:Input @get:Optional abstract val lintHtmlPrefs: Property<String>

  @get:Input @get:Optional abstract val lintXmlRoot: Property<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val lintOverrideConfiguration: RegularFileProperty
  ...
  fun initialize(providerFactory: ProviderFactory, lintMode: LintMode) {
    ...
    androidLintIncludeLdpi.setDisallowChanges(providerFactory.environmentVariable("ANDROID_LINT_INCLUDE_LDPI"))
    androidLintMaxDepth.setDisallowChanges(providerFactory.environmentVariable("ANDROID_LINT_MAX_DEPTH"))
    androidLintMaxViewCount.setDisallowChanges(providerFactory.environmentVariable("ANDROID_LINT_MAX_VIEW_COUNT"))
    androidLintNullnessIgnoreDeprecated.setDisallowChanges(providerFactory.environmentVariable("ANDROID_LINT_NULLNESS_IGNORE_DEPRECATED"))
    lintApiDatabase.fileProvider(providerFactory.environmentVariable("LINT_API_DATABASE").map { File(it) }.filter { it.isFile })
    lintApiDatabase.disallowChanges()
    lintOverrideConfiguration.fileProvider(
      providerFactory.environmentVariable("LINT_OVERRIDE_CONFIGURATION").map { File(it) }.filter { it.isFile }
    )
...
abstract class VariantInputs : UsesLibraryDependencyCacheBuildService {
  ...
  /** Whether the module dependencies should be modeled as module dependencies (instead of modeled as external libraries). */
  @get:Input abstract val useModuleDependencyLintModels: Property<Boolean>

  @get:Nested @get:Optional abstract val mainArtifact: Property<AndroidArtifactInput>

  @get:Nested @get:Optional abstract val testArtifact: Property<JavaArtifactInput>

  @get:Nested @get:Optional abstract val androidTestArtifact: Property<AndroidArtifactInput>

  @get:Nested @get:Optional abstract val testFixturesArtifact: Property<AndroidArtifactInput>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) @get:Optional abstract val mergedManifest: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) @get:Optional abstract val manifestMergeReport: RegularFileProperty

  @get:Input abstract val namespace: Property<String>

  @get:Nested @get:Optional abstract val minSdkVersion: SdkVersionInput

  @get:Nested @get:Optional abstract val targetSdkVersion: SdkVersionInput

  @get:Input abstract val resValues: MapProperty<ResValue.Key, ResValue>

  @get:Input abstract val manifestPlaceholders: MapProperty<String, String>

  @get:Input abstract val resourceConfigurations: ListProperty<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val proguardFiles: ListProperty<RegularFile>
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val extractedProguardFiles: DirectoryProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val consumerProguardFiles: ListProperty<RegularFile>

  @get:Nested @get:Optional abstract val mainSourceProvider: Property<SourceProviderInput>

  /**
   * Androidx compose plugin uses this field to add kotlin multiplatform sources sets as there is no public API to customize lint sources.
   */
  @get:Internal abstract val sourceProviders: ListProperty<SourceProviderInput>

  @get:Nested @get:Optional abstract val hostTestSourceProvider: Property<SourceProviderInput>

  @get:Nested @get:Optional abstract val androidTestSourceProvider: Property<SourceProviderInput>

  @get:Nested @get:Optional abstract val testFixturesSourceProvider: Property<SourceProviderInput>

  @get:Input abstract val debuggable: Property<Boolean>

  @get:Input abstract val shrinkable: Property<Boolean>

  @get:Input abstract val useSupportLibraryVectorDrawables: Property<Boolean>

  @get:Nested abstract val buildFeatures: BuildFeaturesInput

  @get:Internal abstract val mavenCoordinatesCache: Property<MavenCoordinatesCacheBuildService>

  /**
   * Initializes the variant inputs
   *
   * @param variantWithTests the [VariantWithTests].
   * @param useModuleDependencyLintModels whether the module dependencies should be modeled as module dependencies (instead of modeled as
   *   external libraries).
   * @param warnIfProjectTreatedAsExternalDependency whether to warn the user if the standalone plugin is not applied to a java module
   *   dependency when useModuleDependencyLintModels is true.
   * @param lintMode the [LintMode] for which this [VariantInputs] is being used.
   * @param addBaseModuleLintModel whether the base app module should be modeled as a module dependency if useModuleDependencyLintModels is
   *   false. This Boolean only affects dynamic feature modules, and it has no effect if useModuleDependencyLintModels is true.
  ...
  fun initialize(
    ...
    mergedManifest.setDisallowChanges(variantCreationConfig.artifacts.get(SingleArtifact.MERGED_MANIFEST))
    // The manifest merge report contains absolute paths, so it's not compatible with the lint
    // analysis task being cacheable.
    if (lintMode != LintMode.ANALYSIS) {
      manifestMergeReport.set(variantCreationConfig.artifacts.get(InternalArtifactType.MANIFEST_MERGE_REPORT))
    }
    ...
    when (variantCreationConfig) {
      is ApkCreationConfig -> targetSdkVersion.initialize(variantCreationConfig.targetSdk)
    ...
    resValues.setDisallowChanges(variantCreationConfig.resValuesCreationConfig?.resValues, handleNullable = { empty() })
    ...
    resourceConfigurations.setDisallowChanges(variantCreationConfig.androidResourcesCreationConfig?.resourceConfigurations ?: emptyList())
    ...
    if (includeMainArtifact) {
      ...
      extractedProguardFiles.set(variantCreationConfig.global.globalArtifacts.get(InternalArtifactType.DEFAULT_PROGUARD_FILES))
      consumerProguardFiles.set(variantCreationConfig.optimizationCreationConfig.consumerProguardFiles)
    }
    ...
    shrinkable.setDisallowChanges(variantCreationConfig.optimizationCreationConfig.minifiedEnabled)
    useSupportLibraryVectorDrawables.setDisallowChanges(
      variantCreationConfig.androidResourcesCreationConfig?.vectorDrawables?.useSupportLibrary ?: false
    )
  ...
  private fun initializeForStandalone(
    ...
    if (lintModelArtifactType == MAIN || lintModelArtifactType == null) {
      mainArtifact.set(
        project.objects
          .newInstance(AndroidArtifactInput::class.java)
          .initializeForStandalone(project, projectOptions, mainSourceSet, lintMode, useModuleDependencyLintModels, fatalOnly)
      )
      mainSourceProvider.set(
        project.objects.newInstance(SourceProviderInput::class.java).initializeForStandalone(mainSourceSet, lintMode, unitTestOnly = false)
      )
    }
    mainArtifact.disallowChanges()
    ...
    if (lintModelArtifactType == UNIT_TEST || lintModelArtifactType == null) {
      ...
      if (!fatalOnly) {
        hostTestSourceProvider.set(
          project.objects.newInstance(SourceProviderInput::class.java).initializeForStandalone(testSourceSet, lintMode, unitTestOnly = true)
        )
      }
    }
  ...
  private fun initializeForStandaloneWithKotlinMultiplatform(
    ...
    if (jvmMainCompilation != null && lintModelArtifactType == MAIN) {
      ...
      mainSourceProvider.set(
        project.objects
          .newInstance(SourceProviderInput::class.java)
          .initializeForStandaloneWithKotlinMultiplatform(sourceDirectories, lintMode, unitTestOnly = false)
      )
    }
    mainArtifact.disallowChanges()
    mainSourceProvider.disallowChanges()
    if (jvmTestCompilation != null && lintModelArtifactType == UNIT_TEST) {
      ...
      hostTestSourceProvider.set(
        project.objects
          .newInstance(SourceProviderInput::class.java)
          .initializeForStandaloneWithKotlinMultiplatform(sourceDirectories, lintMode, unitTestOnly = true)
      )
    }
    testArtifact.disallowChanges()
    hostTestSourceProvider.disallowChanges()
  ...
  fun toLintModel(module: LintModelModule, partialResultsDir: File? = null, desugaredMethodsFiles: Collection<File>): LintModelVariant {
    val dependencyCaches = DependencyCaches(libraryDependencyCacheBuildService.get().localJarCache, mavenCoordinatesCache.get())
    ...
    return DefaultLintModelVariant(
      ...
      androidTestArtifact.orNull?.toLintModel(dependencyCaches, LintModelArtifactType.INSTRUMENTATION_TEST),
      testFixturesArtifact.orNull?.toLintModel(dependencyCaches, LintModelArtifactType.TEST_FIXTURES),
      ...
      resValues = resValues.get().map { DefaultLintModelResourceField(it.key.type, it.key.name, it.value.value) }.associateBy { it.name },
      ...
      testSourceProviders =
        listOfNotNull(hostTestSourceProvider.orNull?.toLintModels(), androidTestSourceProvider.orNull?.toLintModels()).flatten(),
...
abstract class BuildFeaturesInput {
  @get:Input abstract val viewBinding: Property<Boolean>
  ...
  fun initialize(creationConfig: ComponentCreationConfig) {
    ...
    coreLibraryDesugaringEnabled.setDisallowChanges(
      (creationConfig as? ConsumableCreationConfig)?.isCoreLibraryDesugaringEnabledLintCheck ?: false
    )
  }
  ...
  fun toLintModel(): LintModelBuildFeatures {
    return DefaultLintModelBuildFeatures(viewBinding.get(), coreLibraryDesugaringEnabled.get())
  }
}
...
abstract class SourceProviderInput {
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val javaDirectories: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val resDirectories: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val assetsDirectories: ConfigurableFileCollection
  ...
  // javaDirectoryPaths.
  @get:Classpath @get:Optional abstract val javaDirectoriesClasspath: ConfigurableFileCollection

  // See comment for javaDirectoriesClasspath
  @get:Classpath @get:Optional abstract val resDirectoriesClasspath: ConfigurableFileCollection

  // See comment for javaDirectoriesClasspath
  @get:Classpath @get:Optional abstract val assetsDirectoriesClasspath: ConfigurableFileCollection

  @get:Input // Note: The files may not exist
  @get:Optional
  abstract val manifestAbsoluteFilePaths: ListProperty<String>

  @get:Input @get:Optional abstract val javaDirectoryPaths: ListProperty<String>

  @get:Input @get:Optional abstract val resDirectoryPaths: ListProperty<String>

  @get:Input @get:Optional abstract val assetsDirectoryPaths: ListProperty<String>

  @get:Input abstract val debugOnly: Property<Boolean>

  @get:Input abstract val unitTestOnly: Property<Boolean>

  @get:Input abstract val instrumentationTestOnly: Property<Boolean>

  @get:Input abstract val testFixtureOnly: Property<Boolean>
  ...
  internal fun initialize(
    ...
    fun FlatSourceDirectoriesImpl.getFilteredSourceProviders(into: ConfigurableFileCollection) {
      return getVariantSources().filter { dir -> !dir.isGenerated }.forEach { it.addTo(projectDir.get(), into) }
    }
    ...
    fun LayeredSourceDirectoriesImpl.getFilteredSourceProviders(into: ConfigurableFileCollection) {
      return getVariantSources().forEach { dirs ->
        dirs.directoryEntries.filter { dir -> !dir.isGenerated }.forEach { it.addTo(projectDir.get(), into) }
      }
    }
    ...
    if (lintMode == LintMode.ANALYSIS) {
      ...
      this.javaDirectoryPaths.set(javaDirectories.elements.map { elements -> elements.map { it.asFile.absolutePath } })
      this.resDirectoryPaths.set(resDirectories.elements.map { elements -> elements.map { it.asFile.absolutePath } })
      this.assetsDirectoryPaths.set(assetsDirectories.elements.map { elements -> elements.map { it.asFile.absolutePath } })
    }
  ...
  internal fun initializeForStandalone(sourceSet: SourceSet, lintMode: LintMode, unitTestOnly: Boolean): SourceProviderInput {
    this.manifestFilePath.disallowChanges()
...
abstract class SdkVersionInput {
  ...
  @get:Input @get:Optional abstract val codeName: Property<String>
...
/** Inputs for an Android Artifact. This is used by [VariantInputs] for the main and AndroidTest artifacts. */
abstract class AndroidArtifactInput : ArtifactInput() {

  @get:Input abstract val applicationId: Property<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val generatedSourceFolders: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val generatedResourceFolders: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) @get:Optional abstract val desugaredMethodsFiles: ConfigurableFileCollection
  ...
  fun initialize(
    ...
    if (includeGeneratedSourceFolders) {
      generatedSourceFolders.from(getGeneratedSourceFoldersFileCollection(creationConfig))
    }
    ...
    if (includeClassesOutputDirectories) {
      if (creationConfig is KmpComponentCreationConfig) {
        classesOutputDirectories.from(
          creationConfig.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES)
        )
      } else {
        ...
        creationConfig.getBuiltInKaptArtifact(InternalArtifactType.BUILT_IN_KAPT_CLASSES_DIR)?.let { classesOutputDirectories.from(it) }
      }
      creationConfig.oldVariantApiLegacySupport?.variantData?.let {
        classesOutputDirectories.from(it.allPreJavacGeneratedBytecode)
        classesOutputDirectories.from(it.allPostJavacGeneratedBytecode)
      }
      creationConfig.androidResourcesCreationConfig?.let {
        classesOutputDirectories.from(it.getCompiledRClasses(AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH))
      }
    }
    ...
    val coreLibDesugaring = (creationConfig as? ConsumableCreationConfig)?.isCoreLibraryDesugaringEnabledLintCheck ?: false
    desugaredMethodsFiles
      .from(getDesugaredMethods(creationConfig.services, coreLibDesugaring, creationConfig.minSdk, creationConfig.global))
      .disallowChanges()
  ...
  internal fun toLintModel(dependencyCaches: DependencyCaches, type: LintModelArtifactType): LintModelAndroidArtifact {
...
abstract class JavaArtifactInput : ArtifactInput() {
  ...
  fun initializeForStandalone(
    ...
    compileClasspath?.run {
      ...
      project.dependencies.add(name, project.files(Callable { mainJarTask.flatMap { it.archiveFile } }))
    }
  ...
  fun initializeForStandaloneWithKotlinMultiplatform(
    ...
    val compileClasspathForLint: Configuration =
      compileClasspath?.apply {
        this.extendsFrom(project.configurations.getByName(compilation.compileDependencyConfigurationName))
        project.dependencies.add(this.name, project.files(Callable { mainJarTask.flatMap { it.archiveFile } }))
      } ?: project.configurations.getByName(compilation.compileDependencyConfigurationName)
    ...
    val variantDependencies =
      VariantDependencies(
        ...
        lintChecksClasspath = maybeCreateLintChecksClasspath(project, compilation.name, compileClasspathForLint, runtimeClasspathForLint),
        sourceSetRuntimeConfigurations = listOf(),
        sourceSetImplementationConfigurations = listOf(),
        elements = mapOf(),
        providedClasspath = project.configurations.getByName(compilation.compileOnlyConfigurationName),
  ...
  internal fun toLintModel(dependencyCaches: DependencyCaches, type: LintModelArtifactType): LintModelJavaArtifact {
    return DefaultLintModelJavaArtifact(computeDependencies(dependencyCaches), classesOutputDirectories.files.toList(), type)
  }
}
...
abstract class ArtifactInput {
  ...
  @get:Classpath abstract val classesOutputDirectories: ConfigurableFileCollection

  @get:Nested abstract val artifactCollectionsInputs: Property<ArtifactCollectionsInputs>
  ...
  @get:Internal var projectRuntimeExplodedAars: ArtifactCollection? = null
  ...
  @get:Internal var projectCompileExplodedAars: ArtifactCollection? = null
  ...
  @get:Internal abstract val projectRuntimeLintModels: Property<ArtifactCollection>
  ...
  @get:Internal abstract val projectCompileLintModels: Property<ArtifactCollection>
  ...
  @get:Internal abstract val baseModuleLintModel: Property<ArtifactCollection>
  ...
  @get:Internal abstract val runtimeLintModelMetadata: Property<ArtifactCollection>
  ...
  @get:Internal abstract val compileLintModelMetadata: Property<ArtifactCollection>
  ...
  @get:Internal abstract val runtimeLintPartialResults: Property<ArtifactCollection>
  ...
  @get:Internal abstract val compileLintPartialResults: Property<ArtifactCollection>

  @get:Internal abstract val warnIfProjectTreatedAsExternalDependency: Property<Boolean>

  /**
   * Whether to ignore unexpected artifact types when resolving dependencies. This should be true when running lint via the standalone
   * plugin (b/198048896).
   */
  @get:Internal abstract val ignoreUnexpectedArtifactTypes: Property<Boolean>
  ...
  protected fun initializeProjectDependencyLintArtifacts(
    ...
    if (useModuleDependencyLintModels) {
      ...
      runtimeLintModelMetadataFileCollection.from(runtimeLintModelMetadataArtifacts.artifactFiles)
      ...
      compileLintModelMetadataFileCollection.from(compileLintModelMetadataArtifacts.artifactFiles)
      if (isMainArtifact && lintMode == LintMode.ANALYSIS && isPerComponentLintAnalysis) {
        ...
        runtimeLintPartialResultsFileCollection.from(runtimeLintPartialResultsArtifacts.artifactFiles)
        ...
        compileLintPartialResultsFileCollection.from(compileLintPartialResultsArtifacts.artifactFiles)
      }
    }
...
abstract class UastInputs {
  ...
  /**
   * The default kotlin language version used by the corresponding [KotlinCompile] task, which is used if the language version is not set on
   * [KotlinCompile.compilerOptions].
   */
  @get:Input @get:Optional abstract val defaultKotlinLanguageVersion: Property<String>
  ...
  fun initialize(project: Project, variant: VariantCreationConfig) {
    ...
    initializeFromKotlinCompileTask(kotlinCompileTaskName, project, variant.useBuiltInKotlinSupport)
  }
  ...
  /**
   * This function makes an effort to set the kotlin language version inputs based on the task named [kotlinCompileTaskName], but it's not
   * always possible (e.g., if KGP is not applied in the same class loader and [useBuiltInKotlinSupport] is false). This function catches
   * any exceptions that might be thrown because of unexpected behavior from KGP.
   */
  private fun initializeFromKotlinCompileTask(kotlinCompileTaskName: String, project: Project, useBuiltInKotlinSupport: Boolean = false) {
    ...
    this.compilerOptionsKotlinLanguageVersion.set(
      kotlinCompileTaskProvider.flatMap { kotlinCompileTask ->
        runCatching { kotlinCompileTask.compilerOptions.languageVersion.map { it.version } }.getOrNull() ?: project.provider { null }
      }
    )
    this.compilerOptionsKotlinLanguageVersion.disallowChanges()
    this.defaultKotlinLanguageVersion.setDisallowChanges(runCatching { DEFAULT.version }.getOrNull())
  }
}
...
/**
 * The lint binary uses the same version numbers as AGP (see LintCliClient#getClientRevision() which is called when you run lint --version,
 * as well as in release notes, etc etc).
 *
 * However, for historical reasons, the maven artifacts for its various libraries used in AGP are using the older tools-base version
 * numbers, which are 23 higher, so lint 7.0.0 is published at com.android.tools.lint:lint-gradle:30.0.0
 *
 * This function maps from the user-oriented lint version specified by the user to the maven lint library version number for the artifact to
 * load.
 *
 * Returns the actual lint version to use, the given [versionOverride] if valid, otherwise the default, reporting any issues as a side
 * effect.
 */
...
/**
 * A class to wrap [KotlinMultiplatformExtension]. If there are method parameters with type [KotlinMultiplatformExtension] in task or task
 * input classes, Gradle will fail at runtime for projects without the Kotlin Gradle plugin applied. Using this wrapper class works around
 * that constraint.
 *
...
/**
 * A class to wrap [KotlinCompilation]. If there are method parameters with type [KotlinCompilation] in task or task input classes, Gradle
 * will fail at runtime for projects without the Kotlin Gradle plugin applied. Using this wrapper class works around that constraint.
 *
 * When using this class, perform a runtime check that the Kotlin Gradle plugin is applied.
 */
...
internal fun maybeCreateLintChecksClasspath(
  ...
  lintChecksClasspath.resolutionStrategy.sortArtifacts(ResolutionStrategy.SortOrder.CONSUMER_FIRST)
  for (attributeKey in runtimeClasspath.attributes.keySet()) {
    copyAttribute(attributeKey as Attribute<*>, runtimeClasspath.attributes, lintChecksClasspath.attributes)
  }
