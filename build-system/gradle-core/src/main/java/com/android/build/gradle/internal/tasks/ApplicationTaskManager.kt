...
class ApplicationTaskManager(
  ...
  override fun doCreateTasksForVariant(variantInfo: ComponentInfo<ApplicationVariantBuilder, ApplicationCreationConfig>) {
    ...
    taskFactory.register(AppMetadataTask.CreationAction(variant)).dependsOn(variant.taskContainer.preBuildTask)
    ...
    val r8DexStartupOptimizationEnabled =
      variant.experimentalProperties.map { ModulePropertyKey.BooleanWithDefault.R8_DEX_STARTUP_OPTIMIZATION.getValue(it) }
    val d8DexStartupOptimizationEnabled =
      variant.experimentalProperties.map { ModulePropertyKey.BooleanWithDefault.D8_DEX_STARTUP_OPTIMIZATION.getValue(it) }
    ...
    if (variant.buildFeatures.dataBinding && globalConfig.hasDynamicFeatures) {
    ...
    if ((variant.includeVcsInfo == null && !variantInfo.variant.debuggable) || variant.includeVcsInfo == true) {
    ...
    for (component in publishInfo.components) {
      val configType =
        if (component.type == AbstractPublishing.Type.APK) {
  ...
  private fun createAssetPackTasks(appVariant: ApplicationCreationConfig) {
    ...
    if (assetPacks.isNotEmpty()) {
      val assetPackManifest = assetPackManifestConfiguration.incoming.artifacts
      val assetFiles = assetPackFilesConfiguration.incoming.files

      taskFactory.register(ProcessAssetPackManifestTask.CreationAction(appVariant, assetPackManifest))
      taskFactory.register(LinkManifestForAssetPackTask.CreationAction(appVariant))
      taskFactory.register(AssetPackPreBundleTask.CreationAction(appVariant, assetFiles))
    }
  }
  ...
  private fun createDynamicBundleTask(variantInfo: ComponentInfo<ApplicationVariantBuilder, ApplicationCreationConfig>) {
    ...
    if (variant.componentType.isBaseModule) {
      ...
      if (
        variant.global.bundleOptions.deviceTargetingConfig.isPresent &&
          variant.services.projectOptions[BooleanOption.ENABLE_DEVICE_TARGETING_CONFIG_API]
      ) {
      ...
      if (!debuggable) {
        ...
        if (includeSdkInfoInApk && variant.services.projectOptions[BooleanOption.INCLUDE_DEPENDENCY_INFO_IN_APKS]) {
      ...
      taskFactory.register(AnchorTaskNames.getExtractApksAnchorTaskName(variant)) {
        it.dependsOn(variant.artifacts.get(InternalArtifactType.EXTRACTED_APKS))
      }
  ...
  private fun createSoftwareComponent(appVariant: ApplicationCreationConfig, componentName: String, publication: PublishedConfigType) {
  ...
  override fun createInstallTask(creationConfig: ApkCreationConfig) {
    if (!globalConfig.hasDynamicFeatures || creationConfig is DeviceTestCreationConfig) {
