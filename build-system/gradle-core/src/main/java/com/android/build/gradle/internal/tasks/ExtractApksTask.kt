...
/**
 * Task that extract APKs from the apk zip (created with [BundleToApkTask] into a folder. a Device info file indicate which APKs to extract.
 * Only APKs for that particular device are extracted.
 */
...
abstract class ExtractApksTask : NonIncrementalTask() {
  ...
  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val apkSetArchive: RegularFileProperty

  @get:InputFile @get:Optional @get:PathSensitive(PathSensitivity.NONE) abstract val targetDeviceSpec: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) abstract val multipleDeviceConfigs: ConfigurableFileCollection

  @get:OutputDirectory abstract val outputDir: DirectoryProperty

  @get:OutputFile abstract val deviceSpecToApksLocationMappingFile: RegularFileProperty

  @get:OutputFile abstract val apksFromBundleIdeModel: RegularFileProperty

  @get:Input abstract val applicationId: Property<String>

  @get:Input @get:Optional abstract val dynamicModulesToInstall: ListProperty<String>

  @get:Input
  var extractInstant = false
    private set

  @get:Input abstract val setIncludeMetadata: Property<Boolean>
  ...
  abstract class BundleToolRunnable : ProfileAwareWorkAction<Params>() {
    ...
    override fun run() {
      ...
      val deviceSpecFiles: List<File> =
        when {
          targetDeviceSpec == null -> throw RuntimeException("Calling ExtractApk with no device config")

          multipleDeviceSpecs.isNotEmpty() -> multipleDeviceSpecs.toList()

          else -> listOf(targetDeviceSpec)
        }

      val specsToFiles: Map<DeviceSpec, List<DeviceSpecInfo>> =
        deviceSpecFiles
          .map { file ->
            val spec =
              DeviceSpec.newBuilder()
                .apply { Files.newBufferedReader(file.toPath(), Charsets.UTF_8).use { reader -> JsonFormat.parser().merge(reader, this) } }
                .build()
          ...
          ExtractApksCommand.builder()
            ...
            .also {
              if (parameters.optionalListOfDynamicModulesToInstall.get().isNotEmpty()) {
                it.setModules(parameters.optionalListOfDynamicModulesToInstall.get().toImmutableSet())
      ...
      parameters.deviceSpecToApksLocationMappingFile
        .get()
        .asFile
        .writeText(
          specsToFiles.entries.withIndex().joinToString(separator = "\n") { (i, specToFiles) ->
  ...
  class CreationAction(creationConfig: ApkCreationConfig) : VariantTaskCreationAction<ExtractApksTask, ApkCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ExtractApksTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts.setInitialProvider(taskProvider, ExtractApksTask::outputDir).on(InternalArtifactType.EXTRACTED_APKS)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ExtractApksTask::apksFromBundleIdeModel)
        .withName(BuiltArtifactsImpl.METADATA_FILE_NAME)
        .on(InternalArtifactType.APK_FROM_BUNDLE_IDE_MODEL)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ExtractApksTask::deviceSpecToApksLocationMappingFile)
    ...
    override fun configure(task: ExtractApksTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.APKS_FROM_BUNDLE, task.apkSetArchive)

      val targetDeviceSpecPath = creationConfig.services.projectOptions.get(StringOption.IDE_APK_SELECT_CONFIG)
      targetDeviceSpecPath?.let {
        task.targetDeviceSpec.setDisallowChanges(creationConfig.services.fileProvider(creationConfig.services.provider { File(it) }))
      }
      task.targetDeviceSpec.disallowChanges()

      val multipleDeviceSpec = creationConfig.services.projectOptions.get(StringOption.IDE_APK_SELECT_MULTIPLE_DEVICE_SPECS)
      val deviceSpecsPaths = multipleDeviceSpec?.split(",")?.map(String::trim) ?: emptyList()
      task.multipleDeviceConfigs.fromDisallowChanges(deviceSpecsPaths.map(::File))

      task.extractInstant = creationConfig.services.projectOptions.get(BooleanOption.IDE_EXTRACT_INSTANT)
      task.applicationId.setDisallowChanges(creationConfig.applicationId)
      val optionalListOfDynamicModulesToInstall = creationConfig.services.projectOptions.get(StringOption.IDE_INSTALL_DYNAMIC_MODULES_LIST)
      if (!optionalListOfDynamicModulesToInstall.isNullOrEmpty()) {
        task.dynamicModulesToInstall.addAll(optionalListOfDynamicModulesToInstall.split(','))
      }
