...
>(
  ...
  private val buildOutputs by lazy {
    withProject("buildOutputs") { it.container(com.android.build.gradle.api.BaseVariantOutput::class.java) }
  }

  private val extensionData by lazy { createExtension(dslServices, variantInputModel, buildOutputs, versionedSdkLoaderService) }

  // TODO: BaseExtension should be changed into AndroidT
  @Deprecated("use newExtension") val extension: BaseExtension by lazy { extensionData.oldExtension }
  ...
  private val variantApiOperations by lazy {
    VariantApiOperationsRegistrar<AndroidT, VariantBuilderT, VariantT>(extensionData.newExtension)
  }

  val lintDslLifecycleRegistrar by lazy { DslLifecycleComponentsOperationsRegistrar(extensionData.newExtension.lint) }

  val managedDeviceRegistry: ManagedDeviceRegistry by
    lazy(LazyThreadSafetyMode.NONE) { ManagedDeviceRegistry(DeviceTestOptionsDslInfoImpl((newExtension as CommonExtensionImpl<*, *, *>))) }
  ...
  open val variantInputModel: LegacyVariantInputManager by lazy {
    withProject("LegacyVariantInputManager") { project ->
      LegacyVariantInputManager(
        ...
        SourceSetManager(project, isPackagePublished(), dslServices, DelayedActionsExecutor()),
      )
    }
  }
  ...
  protected val dslServices: DslServicesImpl by lazy {
    DslServicesImpl(projectServices, sdkComponentsBuildService, getProjectTypeV2()) { versionedSdkLoaderService }
  }

  private val taskManagerConfig: TaskManagerConfig by lazy { TaskManagerConfigImpl(dslServices, componentFactory) }
  ...
  protected val versionedSdkLoaderService: VersionedSdkLoaderService by lazy {
    withProject("versionedSdkLoaderService") { project ->
      VersionedSdkLoaderService(
        dslServices,
        project,
        { @Suppress("DEPRECATION") extension.compileSdkVersion },
        { @Suppress("DEPRECATION") extension.buildToolsRevision },
      )
    }
  }

  private val bootClasspathConfig: BootClasspathConfigImpl by lazy { extensionData.bootClasspathConfig }

  private val variantFactory: VariantFactory<VariantBuilderT, VariantDslInfoT, CreationConfigT> by lazy { createVariantFactory() }
  ...
  protected abstract fun createExtension(
    ...
    @Suppress("DEPRECATION") buildOutputs: NamedDomainObjectContainer<com.android.build.gradle.api.BaseVariantOutput>,
  ...
  override fun configureProject(project: Project) {
    ...
    val locationsProvider = getBuildService(project.gradle.sharedServices, AndroidLocationsBuildService::class.java)

    AvdComponentsBuildService.RegistrationAction(
        project,
        projectOptions,
        getManagedDeviceAvdFolder(project.objects, project.providers, locationsProvider.get()),
        sdkComponentsBuildService,
        project.providers.provider { @Suppress("DEPRECATION") extension.compileSdkVersion },
        project.providers.provider { @Suppress("DEPRECATION") extension.buildToolsRevision },
      )
      .execute()
    ...
    projectOptions.allOptions.forEach(projectServices.deprecationReporter::reportOptionIssuesIfAny)
  ...
  companion object {
    fun createAndroidJdkImageConfiguration(project: Project, versionedSdkLoaderService: VersionedSdkLoaderService) {
      ...
      project.dependencies.add(
        ...
        project.files(versionedSdkLoaderService.versionedSdkLoader.flatMap { it.coreForSystemModulesProvider }),
      )
    }
    ...
    private fun createFakeDependencyConfig(project: Project): Configuration {
      val fakeJarService = getBuildService(project.gradle.sharedServices, FakeDependencyJarBuildService::class.java).get()
  ...
  override fun configureExtension(project: Project) {
    ...
    registerModels(project, registry, variantInputModel, extensionData, globalConfig)
  ...
  protected open fun registerModels(
    ...
    registry.register(
      com.android.build.gradle.internal.ide.v2.ModelBuilder(project, variantModel, extensionData.newExtension, extensionData.oldExtension)
    )

    // Register a builder for the native tooling model
    val nativeModelBuilderV2 =
  ...
  private fun createVariantModel(globalConfig: GlobalTaskCreationConfig): VariantModel {
    return VariantModelImpl(
      ...
      { @Suppress("DEPRECATION") extension.testBuildType },
  ...
  override fun createTasks(project: Project) {
    configuratorService.recordBlock(ExecutionType.TASK_MANAGER_CREATE_TASKS, project.path, null) {
      @Suppress("DEPRECATION")
    ...
    project.afterEvaluate(
      afterEvaluate {
        ...
        configuratorService.recordBlock(ExecutionType.BASE_PLUGIN_CREATE_ANDROID_TASKS, project.path, null) { createAndroidTasks(project) }
      }
    )
  }
  ...
  fun createAndroidTasks(project: Project) {
    ...
    (globalConfig.compileOptions as CompileOptions).finalizeSourceAndTargetCompatibility(project, globalConfig)

    if (extension.compileSdkVersion == null) {
      if (SyncOptions.getModelQueryMode(projectServices.projectOptions) == SyncOptions.EvaluationMode.IDE) {
        val newCompileSdkVersion: String = findHighestSdkInstalled() ?: ("android-" + SdkVersionInfo.HIGHEST_KNOWN_STABLE_API)
      ...
      dslServices.issueReporter.reportError(Type.COMPILE_SDK_VERSION_NOT_SET, message)
    }
    ...
    if (project.plugins.hasPlugin(JavaPlugin::class.java)) {
      throw BadPluginException("The 'java' plugin has been applied, but it is not compatible with the Android plugins.")
    }
...
"""
    ...
    if ((!project.state.executed || project.state.failure != null) && sdkTestDirectory == null) {
      return
    }
    variantInputModel.lock()
    extension.disableWrite()

    val projectBuilder = configuratorService.getProjectBuilder(project.path)
    if (projectBuilder != null) {
      ...
      getKotlinAndroidPluginVersion(project)?.let { projectBuilder.kotlinPluginVersion = it }
    }
    ...
    val buildFeatureValues = variantFactory.createBuildFeatureValues(extension.buildFeatures, projectServices)

    // create all registered custom source sets from the user on each AndroidSourceSet
    variantManager.variantApiOperationsRegistrar.onEachSourceSetExtensions { name: String ->
    ...
    projectBuilder?.let { builder ->
      variants.forEach { variant ->
        variant.experimentalProperties.get().keys.forEach { modulePropertyKey ->
          ModulePropertyKey.OptionalString[modulePropertyKey]
            ?.name
    ...
    @Suppress("DEPRECATION") val apiObjectFactory = ApiObjectFactory(extension, variantFactory, dslServices)
  ...
  private fun findHighestSdkInstalled(): String? {
    ...
    val folder =
      withProject("findHighestSdkInstalled") { project ->
        File(getSdkDir(project.rootDir, syncIssueReporter, project.providers), "platforms")
      }
    val listOfFiles = folder.listFiles()
    if (listOfFiles != null) {
      Arrays.sort(listOfFiles, Comparator.comparing { obj: File -> obj.name }.reversed())
  ...
  private fun checkSplitConfiguration() {
    ...
    @Suppress("DEPRECATION") val generatePureSplits = extension.generatePureSplits
  ...
  protected fun initExtensionFromSettings(extension: AndroidT) {
    settingsExtension?.let { extension.doInitExtensionFromSettings(it) }
  }
  ...
  protected open fun AndroidT.doInitExtensionFromSettings(settings: SettingsExtension) {
    ...
    settings.targetSdk targetSdkSettings@{
      val settingsTargetSdkVersion = this@targetSdkSettings.version ?: return@targetSdkSettings
      ...
      if (commonExtension is LibraryExtension) {
        ...
        (commonExtension.defaultConfig as DefaultConfig).targetSdk {
          settingsTargetSdkVersion.let { this.version = TargetSdkVersionImpl(apiLevel = it.apiLevel, codeName = it.codeName) }
        }
      }
    }

    settings.ndkVersion.let { ndkVersion -> this.ndkVersion = ndkVersion }
