...
/**
 * Arguments passed to data binding. This class mimics the [CompilerArguments] class except that it also implements
 * [CommandLineArgumentProvider] for input/output annotations.
 */
@Suppress("MemberVisibilityCanBePrivate")
class DataBindingCompilerArguments
constructor(
  @get:Input val artifactType: CompilerArguments.Type,
  @get:Input val packageName: Provider<String>,
  @get:Input val minApi: Int,
   ...
   *
   * Data binding will fall back to a fixed api versions file shipped as a java resource in the unusual case of compiling against an older
   * version.
   */
  @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) @get:Optional val apiVersionsFile: Provider<RegularFile>,
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) val dependencyArtifactsDir: Provider<Directory>,
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) val layoutInfoDir: Provider<Directory>,
  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) val classLogDir: Provider<Directory>,
  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) val baseFeatureInfoDir: Provider<Directory>,
  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) val featureInfoDir: Provider<Directory>,
  @get:OutputDirectory val aarOutDir: Provider<Directory>,
  @get:Optional @get:OutputFile val exportClassListOutFile: Provider<RegularFile>,
  @get:Input val enableDebugLogs: Boolean,
  ...
  @get:Internal val printEncodedErrorLogs: Boolean,
  @get:Input val isTestVariant: Boolean,
  @get:Input val isEnabledForTests: Boolean,
  @get:Input val isEnableV2: Boolean,
  @get:Input var isNonTransitiveR: Boolean,
  @get:Optional @get:InputFiles @get:Classpath val dependenciesLocalRFiles: FileCollection?,
  ...
  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.NONE) val localRFile: Provider<RegularFile>?,
  ...
  companion object {
    ...
    fun createArguments(
      ...
      val localRFile = if (isNonTransitiveR) artifacts.get(InternalArtifactType.LOCAL_ONLY_SYMBOL_LIST) else null
      ...
      return DataBindingCompilerArguments(
        ...
        baseFeatureInfoDir = artifacts.get(FEATURE_DATA_BINDING_BASE_FEATURE_INFO),
    ...
    /** Returns the module type of a variant. If it is a testing variant, return the module type of the tested variant. */
    @JvmStatic
    fun getModuleType(creationConfig: ComponentCreationConfig): CompilerArguments.Type {
      val component = (creationConfig as? TestComponentCreationConfig)?.mainVariant ?: creationConfig
    ...
    /**
     * Returns the appropriate artifact type of the layout info directory so that it does not trigger unnecessary computations (see bug
     * 133092984 and 110412851).
     */
    @JvmStatic
