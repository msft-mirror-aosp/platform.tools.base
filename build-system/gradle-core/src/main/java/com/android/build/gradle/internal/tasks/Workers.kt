...
/** Singleton object responsible for providing instances of [WorkerExecutorFacade] that relies on Gradle's [WorkerExecutor]. */
object Workers {

  /** Possibly, in the future, consider using a pool with a dedicated size using the gradle parallelism settings. */
   ...
   *
   * Callers cannot use a [WorkerExecutor] probably due to Serialization requirement of parameters being not possible.
   *
  ...
  /** Simple implementation of [WorkerExecutorFacade] that uses a Gradle [WorkerExecutor] to submit new work actions. */
  private class WorkerExecutorAdapter(
    ...
    val taskRecord by lazy { analyticsService.get().getTaskRecord(owner) }
    ...
    override fun submit(action: WorkerExecutorFacade.WorkAction) {
      ...
      workerExecutor.noIsolation().submit(RunnableWrapperWorkAction::class.java) { params: RunnableWrapperParams ->
     ...
     * 1) Gradle currently can only run a SINGLE @TaskAction for a given project (and this should be fixed!)
     * 2) WorkerExecutor passed to a task instance is tied to the task and Gradle is able to control which worker items are executed by
     *    which task
     *
     * Thus, if you put await() here, only a single task can run. If not (as it is), gradle will start another task right after it finishes
     * executing a
     *
     * @TaskAction (which ideally should be just some preparation + a number of submit() calls to a WorkerExecutorFacade. In case the task B
     *   depends on the task A and the work items of the task A hasn't finished yet, gradle will call await() on the dedicated
  ...
  class ProfileAwareExecutorServiceAdapter(
    ...
    /** [WorkerExecutorFacade] to delegate submissions that cannot be handled by this adapter or null if no delegation expected. */
    val delegate: WorkerExecutorFacade? = null,
    private val analyticsService: AnalyticsService,
  ) : ExecutorServiceAdapter(executor), WorkerExecutorFacade {

    private val taskRecord by lazy { analyticsService.getTaskRecord(owner) }
