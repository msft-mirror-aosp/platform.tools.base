 ...
 *
 * The high-level behavior of this function is: (1) Use android.ndkPath if possible (2) Use ndk.dir if possible (3) Otherwise, use
 * $SDK/ndk/$ndkVersion if possible (4) Otherwise, use $SDK/ndk-bundle if possible. (5) Otherwise, return null
 *
 ...
 * @param getNdkSourceProperties Given a folder to an NDK, this function returns source.properties content or null if that file doesn't
 *   exist.
 * @param sdkHandler If present, used to download and install NDK with [SdkHandler.installNdk]. If null, then don't download.
 * @return If the NDK was located, NdkLocatorRecord contains the path to it and the parsed [Revision] that the path refers to. If the NDK
 *   was not located then null is returned.
 */
@VisibleForTesting
fun findNdkPathImpl(
  ...
  fun getNdkFolderRevision(ndkDirFolder: File) = getNdkFolderParsedRevision(ndkDirFolder, getNdkSourceProperties)

  // Try to get the parsed revision for the requested version. If it's unparseable then
  // emit an error and return.
  val revisionFromNdkVersion = parseRevision(getNdkVersionOrDefault(ndkVersionFromDsl)) ?: return null
  ...
  if (!ndkPathFromDsl.isNullOrBlank()) {
    ...
    if (revisionFromNdkPath == null) {
      errorln(
        NDK_CORRUPTED,
        "Location specified by android.ndkPath ($ndkPathFromDsl) did not contain " + "a valid NDK and couldn't be used",
      )
      return null
    }
    if (!ndkDirProperty.isNullOrBlank()) {
      errorln(NDK_IS_AMBIGUOUS, "Both android.ndkPath and ndk.dir in local.properties are set")
      return null
    }
  ...
  if (!ndkDirProperty.isNullOrBlank()) {
    ...
    if (revision != null) {
      ...
      infoln("Checking whether deleting ndk.dir and setting " + "android.ndkVersion to [$revision] would result in the same NDK")
      val resolutionWithoutNdkDir =
        PassThroughPrefixingLoggingEnvironment(
          ...
          .use {
            ...
            if (resolutionWithoutNdkDir == resolutionWithNdkDir) {
              ...
              infoln("Deleting ndk.dir and setting android.ndkVersion to " + "[$revision] would *not* result in the same NDK.")
            }
    ...
    errorln(NDK_IS_INVALID, "Location specified by ndk.dir ($ndkDirProperty) did not contain a valid " + "NDK and couldn't be used")
    return null
  }
  ...
  throw InvalidUserDataException(
    "NDK not configured. Download " + "it with SDK manager. Preferred NDK version is " + "'$NDK_DEFAULT_VERSION'. "
  )
}

/**
 * Given a candidate NDK folder, get Pkg.Revision as parsed Revision. Return null if:
 ...
 */
fun getNdkFolderParsedRevision(ndkDirFolder: File, getNdkSourceProperties: (File) -> SdkSourceProperties?): Revision? {
  ...
  if (packageRevision == null) {
    errorln(NDK_CORRUPTED, "Folder $ndkDirFolder has no Pkg.Revision in source.properties. Ignoring.")
    return null
  }
...
private fun logUserInputs(ndkVersionFromDsl: String?, ndkPathFromDsl: String?, ndkDirProperty: String?, sdkFolder: File?) {
...
private fun parseRevision(version: String): Revision? {
  try {
    val revision = stripPreviewFromRevision(Revision.parseRevision(version))
    if (revision.toIntArray(true).size < 3) {
      errorln(NDK_IS_AMBIGUOUS, "Specified NDK version [$version] does not have " + "enough precision. Use major.minor.micro in version.")
      return null
    }
    return revision
...
/** Given a path to NDK, return the content of source.properties. Returns null if the file isn't found. */
@VisibleForTesting
...
data class NdkLocatorRecord(val ndk: File, val revision: Revision)
...
data class NdkLocator(
   ...
   *
   * (1) SDK unversioned: $(SDK)/ndk-bundle (2) SDK versioned: $(SDK)/ndk/18.1.2 (where 18.1.2 is an example) (3) Custom: Any location on
   * disk
   *
   * There are several ways the user can tell Android Gradle Plugin where to find the NDK
   *
   * (1) Set an explicit folder in local.settings for ndk.dir (2) Don't specify a folder which implies an NDK from the SDK folder should be
   * used
   *
   * If the user specifies android.ndkVersion in build.gradle then that version must be available or it is an error. If no such version is
   * specified then the default version is used.
   */
