...
abstract class MergeNativeLibsTask : NonIncrementalTask() {
  ...
  @get:Classpath @get:Optional @get:SkipWhenEmpty @get:IgnoreEmptyDirectories abstract val profilerNativeLibs: DirectoryProperty
  ...
  @get:Input abstract val testOnly: SetProperty<String>
  ...
  @get:OutputDirectory @get:Optional abstract val testOnlyDir: DirectoryProperty
  ...
  @get:Internal abstract val unfilteredProjectNativeLibs: ConfigurableFileCollection
  ...
  abstract class MergeNativeLibsTaskWorkAction : ProfileAwareWorkAction<MergeNativeLibsTaskWorkAction.Parameters>() {
  ...
  class CreationAction(creationConfig: ConsumableCreationConfig) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<MergeNativeLibsTask>) {
      ...
      if (creationConfig.writesTestOnlyDir()) {
        creationConfig.artifacts
          ...
          .withName("out")
          .on(MERGED_TEST_ONLY_NATIVE_LIBS)
      }
    }
    ...
    override fun configure(task: MergeNativeLibsTask) {
      ...
      task.projectNativeLibs
        .from(
          getProjectNativeLibs(creationConfig, task.buildTools, creationConfig.renderscriptCreationConfig?.renderscript)
            .asFileTree
            .matching(patternSet)
        )
        .disallowChanges()
      ...
      task.unfilteredProjectNativeLibs
        .from(getProjectNativeLibs(creationConfig, task.buildTools, creationConfig.renderscriptCreationConfig?.renderscript))
        .disallowChanges()
    }
  }
  ...
  companion object {
    ...
    val predicate =
      Predicate<String> { fileName ->
        fileName.endsWith(includedFileSuffix, ignoreCase = true) || includedFileNames.any { it.equals(fileName, ignoreCase = true) }
      }
    ...
    /**
     * [file] is one of these three kinds: (1) /path/to/{x86/lib.so} (2) /path/to/{lib/x86/lib.so} (3) /path/to/x86/{lib.so} Where the value
     * in {braces} is the [relativePath] from the file visitor. (1) and (2) are from tasks that process all ABIs in a single task. (3) is
     * from tasks the where each task processes one ABI.
     *
     * This function distinguishes the three cases and returns a relative path that always starts with an ABI. So, for example, all the
     * cases above would return:
     *
     * x86/lib.so
     */
    private fun toAbiRootedPath(file: File, relativePath: RelativePath): String {
      return when {
        ...
        relativePath.segments.size > 1 && abiTags.any { it == relativePath.segments[1] } -> {
...
fun mergeJavaNativeLibs(
  ...
  val excludesPathMatchers: List<PathMatcher> = exclude.map { compileGlob(it) }

  val testOnlyPathMatchers: List<PathMatcher> = testOnly.map { compileGlob(it) }

  // Keep track of the files matching each relative path so we can create a useful error
  // message if necessary
  val usedRelativePaths = mutableMapOf<String, MutableList<File>>().withDefault { mutableListOf() }

  for (inputFile in inputFiles) {
    val systemDependentPath = Paths.get("$separatorChar${inputFile.relativePath.replace('/', separatorChar)}")
    val pickFirstMatches = pickFirstsPathMatchers.filter { it.value.matches(systemDependentPath) }.keys
    val destinationDir =
      if (testOnlyDir != null && testOnlyPathMatchers.any { it.matches(systemDependentPath) }) {
        testOnlyDir
      } else {
        outputDir
      }
  ...
  for (entry in deduplicatedUsedRelativePaths) {
    if (entry.value.size > 1) {
      val projectFiles = entry.value.filter { projectNativeLibs.contains(it) }
      if (projectFiles.size == 1) {
        ...
        val logger = LoggerWrapper(Logging.getLogger(MergeNativeLibsTask::class.java))
        val message =
          StringBuilder()
            .apply {
        ...
        throw DuplicateRelativeFileException(entry.key, entry.value.size, entry.value.map { it.absolutePath }, null)
      }
    }
  }
}
...
private fun copyInputFileToOutput(inputFile: InputFile, outputDir: File, usedRelativePaths: MutableMap<String, MutableList<File>>) {
  usedRelativePaths[inputFile.relativePath] = usedRelativePaths.getValue(inputFile.relativePath).also { it.add(inputFile.file) }

  val outputFile = FileUtils.join(outputDir, inputFile.relativePath.replace('/', separatorChar))
...
fun getProjectNativeLibs(
  ...
  nativeLibs.from(artifacts.get(InternalArtifactType.MERGED_JNI_LIBS))

  // add content of the local external native build if there is one
  if (externalNativeBuildIsActive(creationConfig)) {
    nativeLibs.from(artifacts.getAll(InternalMultipleArtifactType.EXTERNAL_NATIVE_BUILD_LIBS))
  }

  // add renderscript compilation output if support mode is enabled.
  if (renderscript != null) {
    nativeLibs.from(
      renderscript.supportModeEnabled.map {
        if (it) {
          val rsFileCollection: ConfigurableFileCollection = creationConfig.services.fileCollection(artifacts.get(RENDERSCRIPT_LIB))
    ...
    nativeLibs.from(
      renderscript.supportModeBlasEnabled.map {
        if (it) {
          buildTools.supportBlasLibFolderProvider().map { rsBlasLib ->
            if (!rsBlasLib.isDirectory) {
              throw GradleException("Renderscript BLAS support mode is not supported in BuildTools $rsBlasLib")
            }
            rsBlasLib
          }
        } else {
...
fun getTestOnlyNativeLibs(creationConfig: ComponentCreationConfig): FileCollection {
  ...
  if (
    creationConfig is NestedComponentCreationConfig &&
      creationConfig.mainVariant.writesTestOnlyDir() &&
      creationConfig.componentType.isForTesting &&
      creationConfig.componentType.isApk
  ) {
    nativeLibs.from(creationConfig.mainVariant.artifacts.get(MERGED_TEST_ONLY_NATIVE_LIBS))
  }
  // Filter out directories without any file descendants so @SkipWhenEmpty works as desired.
 ...
 */
private fun ConsumableCreationConfig.writesTestOnlyDir() = this.packaging is TestedComponentPackaging && this !is KmpComponentCreationConfig
