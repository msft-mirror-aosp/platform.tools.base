...
open class OutOperationRequestImpl<TaskT : Task, FileTypeT : FileSystemLocation>(
  ...
  override fun <ArtifactTypeT : Single<FileTypeT>> toListenTo(type: ArtifactTypeT) {
    closeRequest()
    toListenTo(artifacts, taskProvider, with, type)
  }

  override val description: String
    get() = "Task ${taskProvider.name} was wired with an output but neither toAppend, toCreate, toListenTo " + "methods were invoked."
}
...
class MultipleArtifactTypeOutOperationRequestImpl<TaskT : Task, FileTypeT : FileSystemLocation>(
  ...
  override val description: String
    get() = "Task ${taskProvider.name} was wired with an output but neither toListenTo " + "methods were invoked."
}
...
class InAndOutFileOperationRequestImpl<TaskT : Task>(
  ...
  override fun <ArtifactTypeT> toTransform(type: ArtifactTypeT)
    where ArtifactTypeT : Single<RegularFile>, ArtifactTypeT : Artifact.Transformable {
    closeRequest()
    toTransform(artifacts, taskProvider, from, into, type, name)
  }

  override val description: String
    get() = "Task ${taskProvider.name} was wired with an Input and an Output but " + "toTransform method was never invoked"
}
...
class CombiningOperationRequestImpl<TaskT : Task, FileTypeT : FileSystemLocation>(
  ...
  override fun <ArtifactTypeT> toTransform(type: ArtifactTypeT)
    where ArtifactTypeT : Multiple<FileTypeT>, ArtifactTypeT : Artifact.Transformable {
    closeRequest()
  ...
  override val description: String
    get() = "Task ${taskProvider.name} was wired to combine multiple inputs into an output but " + "toTransform method was never invoked"
}
...
class InAndOutDirectoryOperationRequestImpl<TaskT : Task>(
  ...
  override fun <ArtifactTypeT> toTransform(type: ArtifactTypeT)
    where ArtifactTypeT : Single<Directory>, ArtifactTypeT : Artifact.Transformable {
    closeRequest()
  ...
  override fun <ArtifactTypeT> toTransformMany(type: ArtifactTypeT): ArtifactTransformationRequest<TaskT>
    where ArtifactTypeT : Single<Directory>, ArtifactTypeT : Artifact.ContainsMany, ArtifactTypeT : Artifact.Transformable {
    ...
    initializeInput(type, type, taskProvider, from, currentProvider, into, builtArtifactsReference)

    // set the output location, so public uses of the API do not have to do it.
    taskProvider.configure { task -> into(task).set(artifacts.calculateOutputPath(type, task)) }
    ...
    publicTypesToIdeModelTypeMap[type]?.let {
      ...
      ideModelContainer.replace(taskProvider, taskProvider.flatMap { task -> into(task).file(BuiltArtifactsImpl.METADATA_FILE_NAME) })
    }
  ...
  /**
   * Initiates a transform request of a single [Directory] artifact type that can contain more than one artifact. The transformed artifact
   * will be registered under a different type.
   *
   ...
   * @param atLocation relative location within the project's build folder to place the transformed [Directory] at.
   * @param builtArtifactsCustomizer optional lambda to post-process the [BuiltArtifactsImpl] before it is saved in the target directory
   *   along with the transformed [Artifact]s. This only runs during task execution, so it should only use task inputs
   * @return [ArtifactTransformationRequest] that will allow processing of individual artifacts located in the input directory.
   *
  ...
  override val description: String
    get() =
      "Task ${taskProvider.name} was wired with an Input and an Output but " + "toTransform or toTransformMany methods were never invoked"

  companion object {

    /**
     * Map of public [SingleArtifact] of [Directory] that have an associated listing file used by Android Studio and passed through the
     * model. The key is the artifact type and the value is the [InternalArtifactType] of [RegularFile] for the listing file artifact.
     */
    val publicTypesToIdeModelTypeMap: Map<Single<Directory>, InternalArtifactType<RegularFile>> =
      mapOf(SingleArtifact.APK to InternalArtifactType.APK_IDE_MODEL)

    /** Keep this method as a static to avoid all possible unwanted variable capturing from lambdas. */
      ...
      taskProvider.configure { task: T ->
        ...
        task.doLast {
          if (builtArtifactsReference.get() == null) {
            ...
            val builtArtifacts = builtArtifactsCustomizer?.let { it(task, builtArtifactsReference.get()) } ?: builtArtifactsReference.get()
...
) where ArtifactTypeT : Single<FileTypeT>, ArtifactTypeT : Artifact.Replaceable {

  checkWithName(type, outputFileSystemLocationName)
  val artifactContainer = artifacts.getArtifactContainer(type)
  taskProvider.configure { with(it).set(artifacts.getOutputPath(type, taskProvider.name, forceFilename = outputFileSystemLocationName)) }
...
private fun <TaskT : Task, FileTypeT : FileSystemLocation, ArtifactTypeT> toTransform(
  ...
  type: ArtifactTypeT,
  outputFileSystemLocationName: String? = null,
) where ArtifactTypeT : Single<FileTypeT>, ArtifactTypeT : Artifact.Transformable {
...
private fun <TaskT : Task, FileTypeT : FileSystemLocation, ArtifactTypeT> toListenTo(
  ...
  val artifactContainer = artifacts.getArtifactContainer(type)
  taskProvider.configure { with(it).set(artifactContainer.get()) }
  _toListenTo(artifacts, taskProvider, type, artifactContainer)
}
...
private fun <TaskT : Task, FileTypeT : FileSystemLocation, ArtifactTypeT> _toListenTo(
  ...
  artifacts.listenerManager.addAction {
    artifactContainer.getFinalProvider()?.configure { it.finalizedBy(taskProvider) }
      ?: artifacts.logger.warn(
        "${taskProvider.name} was registered to listen to the " +
...
private fun checkWithName(type: Artifact<*>, outputFileSystemLocationName: String?) {
  if (type.getFileSystemLocationName().isNotBlank() && !outputFileSystemLocationName.isNullOrBlank()) {
    throw RuntimeException(
      "You cannot use `withName(\"${outputFileSystemLocationName}\")` " +
