...
class ModelBuilder<ExtensionT : CommonExtension>(
  ...
  override fun buildAll(className: String, project: Project): Any =
    when (className) {
      ...
      ProjectGraph::class.java.name ->
        throw RuntimeException("Please use parameterized Tooling API to obtain ${className.split(".").last()} model.")
  ...
  override fun buildAll(className: String, parameter: ModelBuilderParameter, project: Project): Any? =
    when (className) {
      ...
      ProjectSyncIssues::class.java.name -> throw RuntimeException("Please use non-parameterized Tooling API to obtain $className model.")
      else -> throw RuntimeException("Does not support model '$className'")
    }
  ...
  private fun buildProjectGraphModel(project: Project, parameter: ModelBuilderParameter): ProjectGraph? {
    ...
    val globalLibraryBuildService = getBuildService(project.gradle.sharedServices, GlobalSyncService::class.java).get()

    val libraryService = LibraryServiceImpl(globalLibraryBuildService.libraryCache)

    return ProjectGraphImpl(
      resolvedVariantsWithProjectInfo =
        (variant.variantDependencies.getArtifactCollectionForToolingModel(
            RUNTIME_CLASSPATH,
  ...
  private fun buildModelVersions(): Versions {
    /**
     * This is currently unused, but could be used in the future to allow for softer breaking changes, where some models are unavailable,
     * but the project can still be imported.
     */
    val v2Version = VersionImpl(0, 1)
    /**
     * The version of the model-producing code (i.e. the model builders in AGP)
     *
     * The major version is increased every time an addition is made to the model interfaces, or other semantic change that the code
     * injected by studio should react to, such as instructing studio to stop calling a method that returns a collection that is no longer
     * populated which leaves the minor to be able to be incremented for partial backports.
     *
     * Changes made to the model must always be compatible with the MINIMUM_MODEL_CONSUMER version of Android Studio. To make a breaking
     ...
     *
     * The below Android Gradle plugin's minimumModelConsumerVersion will usually be increased after the next version of Studio becomes
     * stable, dropping support for previous Android Studio versions.
     */
    ...
    return VersionsImpl(
      ...
      versions =
        mutableMapOf<String, Versions.Version>(
  ...
  internal data class DimensionInformation(val buildTypes: Set<String>, val flavors: Set<Pair<String, String>>) {
  ...
  private fun buildBasicAndroidProjectModel(project: Project): BasicAndroidProject {
    ...
    val defaultConfig =
      if (variantDimensionInfo.isNotEmpty()) {
  ...
  private fun getTestSuiteSources(sourceSets: Collection<TestSuiteSourceSet>, isGenerated: Boolean): TestSuiteSources {
    // Separate by type and map to model in one go
    val assets = sourceSets.filterIsInstance<AssetsTestSuiteSourceSet>().map { it.toModel(isGenerated) }

    val hostJar = sourceSets.filterIsInstance<HostJarTestSuiteSourceSet>().map { it.toModel(isGenerated) }

    val testApk = sourceSets.filterIsInstance<TestApkTestSuiteSourceSet>().map { it.toModel(isGenerated) }

    return TestSuiteSources(assets, hostJar, testApk)
  }

  // Extension functions to translate variant instances into model ones.
  private fun AssetsTestSuiteSourceSet.toModel(isGenerated: Boolean) =
    AssetsTestSuiteSourceImpl(name = this.name, directories = variantSourcesForModel(this.get(), isGenerated))
  ...
  private fun TestApkTestSuiteSourceSet.toModel(isGenerated: Boolean) =
  ...
  private fun variantSourcesForModel(sourceDirectories: SourceDirectoriesImpl?, isGenerated: Boolean) =
    ...
    sourceDirectories?.variantSourcesForModel { it.shouldBeAddedToIdeModel && it.isGenerated == isGenerated } ?: emptyList()

  /**
   * Intermediary data structure to hold the suite and all its associated targets built from the variant objects. In many ways, it mirrors
   * how the DSL for test suite is organized but uses the variant objects to build it instead of the DSL.
   */
  class TestSuiteModelBuilder(val suite: TestSuiteCreationConfig, val variantsTargets: MutableList<TestSuiteTargetModelBuilder>) {
    class TestSuiteTargetModelBuilder(val targetedVariant: String, val targets: Map<String, TestSuiteTargetCreationConfig>)
  }

  /**
   * Gather all test suites for this project.
   *
   * This is a bit tricky. Test suites are defined at the project level and can have multiple targets, each targeting a particular variant.
   * However at the variant modeling level, each variant can have multiple test suites each targeting the variant in question. So we need to
   * translate this variant model where a variant can have multiple test suites each with targets all targeting the variant to a project
   * level model where each test suites is associated to all its targets, irrespective of the variants they target.
   *
  ...
  private fun gatherTestSuites(testSuites: Collection<TestSuiteCreationConfig>): Collection<TestSuiteModelBuilder> {
    ...
    for (testSuite in testSuites) {
      val testSuiteModelBuilder = suites.getOrPut(testSuite.name) { TestSuiteModelBuilder(testSuite, mutableListOf()) }
      testSuiteModelBuilder.variantsTargets.add(
        TestSuiteModelBuilder.TestSuiteTargetModelBuilder(testSuite.testedVariant.name, testSuite.targets)
      )
    }
  ...
  /**
   * Abstract class for building SourceSetContainer. It has logic to add sourceSets, deviceTestSource, hostTestSource and fixturesTesSources
   * based on variantModel. This will allow excluding from the model sourceSets that are not used by any of them.
   */
  abstract inner class BaseSourceSetContainerBuilder {
    ...
    internal val androidTests = DimensionInformation.createFrom(variantModel.testComponents.filterIsInstance<DeviceTestCreationConfig>())
    internal val unitTests =
      DimensionInformation.createFrom(variantModel.testComponents.filter { it.componentType == ComponentTypeImpl.UNIT_TEST })
    internal val testFixtures = DimensionInformation.createFrom(variantModel.variants.mapNotNull { (it as? HasTestFixtures)?.testFixtures })
    internal val screenshotTests =
      DimensionInformation.createFrom(variantModel.testComponents.filter { it.componentType == ComponentTypeImpl.SCREENSHOT_TEST })

    abstract val variantData: VariantDimensionData
    abstract val defaultSourceSet: DefaultAndroidSourceSet?

    private fun getAndroidTestSourceSet() = variantData.getSourceSetForModel(ComponentTypeImpl.ANDROID_TEST)

    private fun getUnitSourceSet() = variantData.getSourceSetForModel(ComponentTypeImpl.UNIT_TEST)

    private fun getScreenshotSourceSet() = variantData.getSourceSetForModel(ComponentTypeImpl.SCREENSHOT_TEST)

    private fun getFixtureSourceSet() = variantData.getSourceSetForModel(ComponentTypeImpl.TEST_FIXTURES)
    ...
    fun build() =
      SourceSetContainerImpl(
        ...
        deviceTestSourceProviders =
          mutableMapOf<String, SourceProvider>().apply {
        ...
        testFixturesSourceProvider =
          getFixtureSourceSet()?.takeIf { shouldTakeFixtureSourceSet() }?.convert(buildFeatures, additionalFixtureConfig()),
      )
  }
   ...
   *
   * It also adds user defined static folders (added with variant api) in case there are no flavours.
   */
  inner class BuildTypeSourceSetBuilder(val buildType: BuildTypeData<InternalBuildType>) : BaseSourceSetContainerBuilder() {

    private val buildTypeName = buildType.buildType.name
    override val defaultSourceSet = buildType.sourceSet

    override fun shouldTakeAndroidTestSourceSet() = androidTests.buildTypes.contains(buildTypeName)
    ...
    override fun shouldTakeScreenshotSourceSet() = screenshotTests.buildTypes.contains(buildTypeName)
    ...
    override fun additionalAndroidConfig(): List<DeviceTestCreationConfig> =
      additionalDefaultConfig().filterIsInstance<HasDeviceTestsCreationConfig>().flatMap {
        it.deviceTests.values.filterIsInstance<DeviceTestCreationConfig>()
      }
    ...
    override fun additionalFixtureConfig() = additionalDefaultConfig().filterIsInstance<HasTestFixtures>().mapNotNull { it.testFixtures }
  }
  ...
  inner class FlavorSourceSetContainerBuilder(flavor: ProductFlavorData<InternalFlavor>) : BaseSourceSetContainerBuilder() {

    private val flavorDimensionName = flavor.productFlavor.dimension to flavor.productFlavor.name

    override val defaultSourceSet = flavor.sourceSet

    override fun shouldTakeAndroidTestSourceSet() = androidTests.flavors.contains(flavorDimensionName)

    override fun shouldTakeUnitSourceSet() = unitTests.flavors.contains(flavorDimensionName)

    override fun shouldTakeFixtureSourceSet() = screenshotTests.flavors.contains(flavorDimensionName)

    override fun shouldTakeScreenshotSourceSet() = testFixtures.flavors.contains(flavorDimensionName)
  ...
  private fun buildAndroidProjectModel(project: Project): AndroidProject {
    ...
    return AndroidProjectImpl(
      ...
      viewBindingOptions = ViewBindingOptionsImpl(variantModel.variants.any { it.buildFeatures.viewBinding }),
      flags =
        getAgpFlags(
  ...
  private fun checkProguardFiles(component: VariantCreationConfig) {
    ...
    @Suppress("NoOp") component.optimizationCreationConfig.consumerProguardFiles
  }
  ...
  private fun buildAndroidDslModel(project: Project): AndroidDsl {
    ...
    val dependenciesInfo =
      if (extension is ApplicationExtension) {
        DependenciesInfoImpl(extension.dependenciesInfo.includeInApk, extension.dependenciesInfo.includeInBundle)
      } else null

    val extensionImpl = extension as? CommonExtensionImpl<*, *, *> ?: throw RuntimeException("Wrong extension provided to v2 ModelBuilder")
  ...
  private fun buildProjectSyncIssueModel(project: Project): ProjectSyncIssues {
    ...
    allIssues.addAll(getBuildService(project.gradle.sharedServices, GlobalSyncIssueService::class.java).get().getAllIssuesAndClear())
    ...
    val issues = allIssues.build().map { SyncIssueImpl(it.severity, it.type, it.data, it.message, it.multiLineMessage) }
    ...
    if (adjacencyList) {
      ...
      (variant as? HasTestSuitesCreationConfig)?.suites?.values?.forEach { testSuite ->
        testSuiteArtifacts[testSuite.name] =
          createDependenciesWithAdjacencyList(testSuite, libraryService, graphEdgeCache, parameter.dontBuildUnitTestRuntimeClasspath)
      }
      ...
      return VariantDependenciesAdjacencyListImpl(
        ...
        testFixturesArtifact =
          (variant as? HasTestFixtures)?.testFixtures?.let {
            createDependenciesWithAdjacencyList(
              it,
              libraryService,
              graphEdgeCache,
      ...
      (variant as? HasDeviceTestsCreationConfig)?.deviceTests?.values?.filterIsInstance<DeviceTestImpl>()?.forEach {
        deviceTestArtifacts[it.artifactName] =
          createDependencies(it, libraryService, parameter.dontBuildAndroidTestRuntimeClasspath, parameter.additionalArtifactsInModel)
      }
      ...
      (variant as? HasHostTestsCreationConfig)?.hostTests?.values?.forEach {
        hostTestArtifacts[it.componentType.artifactName] =
          createDependencies(
            it,
            libraryService,
      ...
      (variant as? HasTestSuitesCreationConfig)?.suites?.values?.forEach { testSuite ->
        testSuiteArtifacts[testSuite.name] = createDependencies(testSuite, libraryService)
      }

      return VariantDependenciesImpl(
        name = variantName,
        mainArtifact =
          createDependencies(variant, libraryService, parameter.dontBuildRuntimeClasspath, parameter.additionalArtifactsInModel),
        deviceTestArtifacts = deviceTestArtifacts,
        hostTestArtifacts = hostTestArtifacts,
        testSuiteArtifacts = testSuiteArtifacts,
        testFixturesArtifact =
          (variant as? HasTestFixtures)?.testFixtures?.let {
            createDependencies(it, libraryService, parameter.dontBuildTestFixtureRuntimeClasspath, parameter.additionalArtifactsInModel)
          },
        libraryService.getAllLibraries(),
      )
    }
  ...
  private fun createBasicVariant(variant: VariantCreationConfig, features: BuildFeatureValues): BasicVariantImpl {
    ...
    (variant as? HasHostTestsCreationConfig)?.hostTests?.values?.forEach { hostTest ->
      hostTestArtifacts[hostTest.componentType.artifactName] = createBasicArtifact(hostTest, features)
    }
    val testSuiteArtifacts = mutableMapOf<String, BasicTestSuiteArtifact>()
    (variant as? HasTestSuitesCreationConfig)?.suites?.values?.forEach { testSuite ->
      testSuiteArtifacts[testSuite.name] =
        // reconcile with reworked model.
        BasicTestSuiteArtifactImpl(testSuite.name)
    }
    return BasicVariantImpl(
      ...
      hostTestArtifacts = hostTestArtifacts,
      testSuiteArtifacts = testSuiteArtifacts,
      testFixturesArtifact = (variant as? HasTestFixtures)?.testFixtures?.let { createBasicArtifact(it, features) },
      buildType = variant.buildType,
      productFlavors = variant.productFlavorList.map { it.name },
    )
  }

  private fun createBasicArtifact(component: ComponentCreationConfig, features: BuildFeatureValues): BasicArtifact {
    return BasicArtifactImpl(
      variantSourceProvider = component.sources.variantSourceProvider?.convert(component.sources),
      multiFlavorSourceProvider = component.sources.multiFlavorSourceProvider?.convert(features),
    )
  }

  private fun createVariant(variant: VariantCreationConfig, instantAppResultMap: MutableMap<File, Boolean>): VariantImpl {
    ...
    (variant as? HasHostTestsCreationConfig)?.hostTests?.values?.forEach { hostTest ->
      hostTestArtifacts[hostTest.componentType.artifactName] = createJavaArtifact(hostTest)
    }
    ...
    return VariantImpl(
      ...
      hostTestArtifacts = hostTestArtifacts,
      testSuiteArtifacts = testSuiteArtifacts,
      testFixturesArtifact = (variant as? HasTestFixtures)?.testFixtures?.let { createAndroidArtifact(it) },
      testedTargetVariant = getTestTargetVariant(variant),
      runTestInSeparateProcess = ModulePropertyKey.BooleanWithDefault.SELF_INSTRUMENTING.getValue(variant.experimentalProperties.get()),
      isInstantAppCompatible = inspectManifestForInstantTag(variant, instantAppResultMap),
      desugaredMethods =
        getDesugaredMethods(variant.services, variant.isCoreLibraryDesugaringEnabledLintCheck, variant.minSdk, variant.global)
          .files
          .toList(),
      experimentalProperties =
        if (variant.experimentalProperties.isPresent) {
  ...
  private fun createTestSuiteArtifact(testSuiteModelBuilder: TestSuiteModelBuilder): TestSuiteArtifactImpl {
    val targets =
      testSuiteModelBuilder.suite.targets.mapValues { mapEntry ->
        val testSuiteTarget = mapEntry.value
        TestSuiteTargetImpl(testSuiteTarget.name, testSuiteTarget.testTaskName, testSuiteTarget.targetDevices)
      }
    return TestSuiteArtifactImpl(
      testInfo =
        TestSuiteTestInfoImpl(
          junitInfo = JUnitEngineInfoImpl(includedEngines = testSuiteModelBuilder.suite.junitEngineSpec.includeEngines),
          targets = targets,
        ),
  ...
  private fun createAndroidArtifact(component: ComponentCreationConfig): AndroidArtifactImpl {
    ...
    component.androidResourcesCreationConfig?.compiledRClassArtifact?.get()?.asFile?.let { classesFolders.add(it) }
    ...
    val testInfo: TestInfo? =
      when (component) {
        ...
        is DeviceTestCreationConfig -> {
          val runtimeApks: Collection<File> =
          ...
          val testTaskName =
            taskContainer.connectedTestTask?.name
              ?: ""
                .also {
    ...
    val signingConfig = if (component is ApkCreationConfig) component.signingConfig else null

    val minSdkVersion = ApiVersionImpl(component.minSdk.apiLevel, component.minSdk.codename)
    val targetSdkVersionOverride = (component as? TargetSdkAwareConfig)?.targetSdkOverride?.let { ApiVersionImpl(it.apiLevel, it.codename) }
    val maxSdkVersion = if (component is VariantCreationConfig) component.maxSdk else null

    val coreLibDesugaring = (component as? ConsumableCreationConfig)?.isCoreLibraryDesugaringEnabledLintCheck ?: false
    ...
    return AndroidArtifactImpl(
      ...
      codeShrinker =
        CodeShrinker.R8.takeIf { component is ConsumableCreationConfig && component.optimizationCreationConfig.minifiedEnabled },
      ...
      mappingR8PartitionFile = getR8PartitionFile(component),
    )
  }
  ...
  private fun getR8PartitionFile(component: ComponentCreationConfig): File? =
    component.artifacts.get(SingleArtifact.OBFUSCATION_MAPPING_PARTITION_FILE).orNull?.asFile
  ...
  private fun getApplicationId(component: ComponentCreationConfig): String? {
    ...
    return try {
      ...
      variantModel.syncIssueReporter.reportWarning(
        ...
        RuntimeException(
          "Failed to read applicationId for ${component.name}.\n" +
  ...
  private fun getBytecodeTransformations(component: ComponentCreationConfig): List<BytecodeTransformation> {
    ...
    val classesProject = component.artifacts.forScope(PROJECT).getScopedArtifactsContainer(ScopedArtifact.CLASSES).artifactsAltered.get()
    val classesAll = component.artifacts.forScope(ALL).getScopedArtifactsContainer(ScopedArtifact.CLASSES).artifactsAltered.get()

    val asmProject = component.instrumentationCreationConfig?.projectClassesAreInstrumented == true
    val asmAll = component.instrumentationCreationConfig?.dependenciesClassesAreInstrumented == true
  ...
  private fun createJavaArtifact(component: ComponentCreationConfig): JavaArtifact {
    ...
    if (extension.testOptions.unitTests.isIncludeAndroidResources || component.componentType.isForScreenshotPreview) {
  ...
  private fun createDependencies(testSuite: TestSuiteCreationConfig, libraryServices: LibraryService) =
  ...
  private fun createDependenciesWithAdjacencyList(
    ...
    dontBuildRuntimeClasspath: Boolean,
    additionalArtifactsInModel: Boolean,
  ): ArtifactDependenciesAdjacencyList =
    getGraphBuilder(dontBuildRuntimeClasspath, additionalArtifactsInModel, component, libraryService, graphEdgeCache)
    ...
    TestSuiteDependenciesAdjacencyListImpl(
      testSuite.sourceContainers.map { testSuiteSourceContainer ->
        TestSuiteSourceDependenciesAdjacencyListImpl(
          testSuiteSourceContainer.identifier,
          createTestSuiteType(testSuiteSourceContainer.source.type),
          getGraphBuilder(testSuiteSourceContainer, libraryService, graphEdgeCache).buildWithAdjacencyList(),
        )
      }
    )
  ...
  private fun getBundleInfo(component: ComponentCreationConfig): BundleInfo? {
  ...
  private fun inspectManifestForInstantTag(component: ComponentCreationConfig, instantAppResultMap: MutableMap<File, Boolean>): Boolean {
    ...
    manifests.addAll(
      manifestSources.allStatic
        .map {
          ...
          files ->
          files.dropLast(1).reversed().map { it.asFile }
        }
        .get()
        .filter { it.isFile }
    )
        ...
        FileInputStream(manifest).use { inputStream ->
          ...
          while (eventReader.hasNext() && !eventReader.peek().isEndDocument) {
            ...
            if (event.isStartElement) {
              ...
              if (
                startElement.name.namespaceURI == SdkConstants.DIST_URI && startElement.name.localPart.equals("module", ignoreCase = true)
              ) {
                val instant = startElement.getAttributeByName(QName(SdkConstants.DIST_URI, "instant"))
                if (instant != null && (instant.value == SdkConstants.VALUE_TRUE || instant.value == SdkConstants.VALUE_1)) {
                  eventReader.close()
                  instantAppResultMap[manifest] = true
                  return true
                }
              }
            } else if (event.isEndElement && (event as EndElement).name.localPart.equals("manifest", ignoreCase = true)) {
              break
            }
          }
          eventReader.close()
    ...
    if (extension is TestExtension) {
      ...
      if (apkArtifacts.artifacts.size == 1) {
        ...
        DependencyFailureHandler()
          .addErrors("${project.path}@${component.name}/testTarget", apkArtifacts.failures)
  ...
  private fun addGeneratedClassPaths(component: ComponentCreationConfig, classesFolders: MutableSet<File>): Map<String, File> {
    ...
    val preCompilationClasses = component.artifacts.getAll(MultipleArtifact.PRE_COMPILATION_CLASSES).get().map { it.asFile }
  ...
  companion object {
    internal fun getAgpFlags(
      ...
      val flags = ImmutableMap.builder<BooleanFlag, Boolean>()
      ...
      flags.put(BooleanFlag.JETPACK_COMPOSE, variants.any { it.buildFeatures.compose })
      flags.put(BooleanFlag.ML_MODEL_BINDING, variants.any { it.buildFeatures.mlModelBinding })
      flags.put(BooleanFlag.TRANSITIVE_R_CLASS, !projectOptions[BooleanOption.NON_TRANSITIVE_R_CLASS])
      flags.put(BooleanFlag.UNIFIED_TEST_PLATFORM, projectOptions[BooleanOption.ANDROID_TEST_USES_UNIFIED_TEST_PLATFORM])
      flags.put(BooleanFlag.USE_ANDROID_X, projectOptions[BooleanOption.USE_ANDROID_X])
      flags.put(BooleanFlag.BUILD_FEATURE_ANDROID_RESOURCES, variants.any { it.buildFeatures.androidResources })
      flags.put(
      ...
      flags.put(BooleanFlag.DATA_BINDING_ENABLED, variants.any { it.buildFeatures.dataBinding })
