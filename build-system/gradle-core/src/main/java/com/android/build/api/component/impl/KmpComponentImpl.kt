...
abstract class KmpComponentImpl<DslInfoT : KmpComponentDslInfo>(
  ...
  override val instrumentationCreationConfig by
    lazy(LazyThreadSafetyMode.NONE) { InstrumentationCreationConfigImpl(this, internalServices) }
  ...
  override val sources = KmpSourcesImpl(dslInfo, internalServices, manifestFile, androidKotlinCompilation, buildFeatures)
  ...
  override val compileClasspath: FileCollection by
  ...
  override val providedOnlyClasspath: FileCollection by
    lazy(LazyThreadSafetyMode.NONE) {
  ...
  override fun publishBuildArtifacts() {
    com.android.build.gradle.internal.scope.publishBuildArtifacts(
      creationConfig = this,
      publishInfo =
        VariantPublishingInfo(components = listOf(ComponentPublishingInfo(componentName = name, type = AbstractPublishing.Type.AAR))),
    )
  }
  ...
  override val androidResources: AndroidResourcesImpl? by
    lazy(LazyThreadSafetyMode.NONE) {
      if (buildFeatures.androidResources) {
  ...
  override val javaCompilation: JavaCompilation
    get() =
      if (dslInfo.withJava) {
        JavaCompilationImpl(MergedJavaCompileOptions(), buildFeatures.dataBinding, internalServices, variantDependencies)
      } else {
        throw IllegalAccessException(
          "The kotlin multiplatform android plugin doesn't" +
  ...
  class KmpSourcesImpl(
    ...
    override val resources =
      KotlinMultiplatformFlatSourceDirectoriesImpl(
        ...
        variantServices = variantServices,
        variantDslFilters = PatternSet().also { filter -> filter.exclude("**/*.java", "**/*.kt") },
      )

    override val res =
      if (buildFeatures.androidResources) {
        LayeredSourceDirectoriesImpl(_name = SourceType.RES.folder, variantServices = variantServices, variantDslFilters = PatternSet())
    ...
    override val assets =
      if (buildFeatures.androidResources) {
        LayeredSourceDirectoriesImpl(_name = SourceType.ASSETS.folder, variantServices = variantServices, variantDslFilters = PatternSet())
      } else null
    ...
    override val jniLibs =
      LayeredSourceDirectoriesImpl(_name = SourceType.JNI_LIBS.folder, variantServices = variantServices, variantDslFilters = PatternSet())
    ...
    internal val extras: NamedDomainObjectContainer<FlatSourceDirectoriesImpl> by
      lazy(LazyThreadSafetyMode.NONE) {
        variantServices.domainObjectContainer(FlatSourceDirectoriesImpl::class.java, SourcesImpl.SourceProviderFactory(variantServices))
      }
  ...
  open fun syncAndroidAndKmpClasspathAndSources() {
    ...
    androidKotlinCompilation.compileDependencyFiles =
      services.fileCollection(
        ...
        getJavaClasspath(AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH, AndroidArtifacts.ArtifactType.CLASSES_JAR),
      )
    ...
    sources.baselineProfiles.addStaticSources(
      services.provider {
        androidKotlinCompilation.allKotlinSourceSets.flatMap { sourceSet ->
          sourceSet.kotlin.srcDirs.map { srcDir ->
            FileBasedDirectoryEntryImpl(name = sourceSet.name, directory = File(srcDir.parentFile, SourceType.BASELINE_PROFILES.folder))
          }
        }
      }
    )
    ...
    sources.assets?.addStaticSources(
      androidKotlinCompilation.defaultSourceSet.let { sourceSet ->
        DirectoryEntries(
          ...
          sourceSet.resources.srcDirs
            .map { srcDir ->
              FileBasedDirectoryEntryImpl(name = sourceSet.name, directory = File(srcDir.parentFile, SourceType.ASSETS.folder))
            }
            .toMutableList(),
        )
      }
    ...
    sources.jniLibs.addStaticSources(
      androidKotlinCompilation.defaultSourceSet.let { sourceSet ->
        DirectoryEntries(
          ...
          sourceSet.resources.srcDirs
            .map { srcDir ->
              FileBasedDirectoryEntryImpl(name = sourceSet.name, directory = File(srcDir.parentFile, SourceType.JNI_LIBS.folder))
            }
            .toMutableList(),
        )
      }
  ...
  fun addKotlinSourcesToCompilation() {
    this.androidKotlinCompilation.compileTaskProvider.configure { kotlincTask ->
      this.sources.kotlin { (kotlincTask as AbstractKotlinCompile<*>).source(it.getAsFileTrees()) }
    }
  }
}
