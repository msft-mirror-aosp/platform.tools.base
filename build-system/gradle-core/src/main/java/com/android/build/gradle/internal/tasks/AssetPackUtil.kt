...
internal fun populateAssetPacksConfigurations(
  ...
  val notFound: MutableList<String> = ArrayList()
  for (assetPack in assetPacks) {
    if (project.findProject(assetPack) != null) {
      val filesDependency = ImmutableMap.of("path", assetPack, "configuration", "packElements")
      depHandler.add(assetPackFilesConfiguration.name, depHandler.project(filesDependency))
      val manifestDependency = ImmutableMap.of("path", assetPack, "configuration", "manifestElements")
      depHandler.add(assetPackManifestConfiguration.name, depHandler.project(manifestDependency))
    } else {
