 ...
 * configuration.
 */
internal class NdkBuildExternalNativeJsonGenerator(abi: CxxAbiModel, variantBuilder: GradleBuildVariant.Builder?) :
  ExternalNativeJsonGenerator(abi, variantBuilder) {

  /** Get the process builder with -n flag. This will tell ndk-build to emit the steps that it would do to execute the build. */
  ...
  private fun parseDryRunOutput(abi: CxxAbiModel, stdout: File) {
    ...
    val builder =
      NativeBuildConfigValueBuilder(makeFile, abi.variant.module.moduleRootFolder, abi.compileCommandsJsonBinFile)
        .setCommands(commandLine, commandLine.removeJobsFlagIfPresent() + listOf("clean"), abi.variant.variantName, buildOutput)
    ...
    val actualResult =
      GsonBuilder().registerTypeAdapter(File::class.java, PlainFileGsonTypeAdaptor()).setPrettyPrinting().create().toJson(buildConfig)

    // Write the captured ndk-build output to JSON file
    Files.write(abi.jsonFile.toPath(), actualResult.toByteArray(Charsets.UTF_8))
  }
  ...
  private fun List<String>.removeJobsFlagIfPresent() = toNdkBuildArguments().removeNdkBuildJobs().toStringList()
  ...
  init {
    ...
    if (abi.variant.module.makeFile.isDirectory) {
      errorln(
        ...
        "Gradle project ndkBuild.path %s is a folder. " + "Only files (like Android.mk) are allowed.",
