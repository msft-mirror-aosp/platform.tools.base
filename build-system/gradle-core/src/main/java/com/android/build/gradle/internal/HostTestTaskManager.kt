...
open class HostTestTaskManager(project: Project, globalConfig: GlobalTaskCreationConfig) : TaskManager(project, globalConfig) {
  ...
  fun createTopLevelTasksCore(taskName: String, description: String) {
    ...
    taskFactory.register(taskName) { hostTestTask: Task ->
  ...
  protected fun setupAndroidRequiredTasks(testedVariant: VariantCreationConfig, hostTestCreationConfig: HostTestCreationConfig) {
    if (testedVariant.componentType.isAar) {
      ...
      createApkProcessResTask(hostTestCreationConfig, InternalArtifactType.FEATURE_RESOURCE_PKG)
      ...
      hostTestCreationConfig.artifacts.copy(InternalArtifactType.LINKED_RESOURCES_BINARY_FORMAT, testedVariant.artifacts)
      hostTestCreationConfig.artifacts.copy(SingleArtifact.ASSETS, testedVariant.artifacts)
      taskFactory.register(PackageForHostTest.CreationAction(hostTestCreationConfig))
    } else {
  ...
  protected fun setupCompilationTaskDependencies(hostTestCreationConfig: HostTestCreationConfig, taskContainer: MutableTaskContainer) {
    val generateTestConfig = taskFactory.register(GenerateTestConfig.CreationAction(hostTestCreationConfig))
    val compileTask =
    ...
    compileTask.configure { task: Task ->
      ...
      taskInputs.files(testConfigInputs.resourceApk).withPropertyName("resourceApk").withPathSensitivity(PathSensitivity.RELATIVE)
      taskInputs.files(testConfigInputs.mergedAssets).withPropertyName("mergedAssets").withPathSensitivity(PathSensitivity.RELATIVE)
      taskInputs.files(testConfigInputs.mergedManifest).withPropertyName("mergedManifest").withPathSensitivity(PathSensitivity.RELATIVE)
      taskInputs.property("packageNameOfFinalRClassProvider", testConfigInputs.packageNameOfFinalRClass)
    }

    taskContainer.assembleTask.configure { task: Task ->
      task.dependsOn(hostTestCreationConfig.artifacts.get(InternalArtifactType.APK_FOR_LOCAL_TEST))
    }
  }
  ...
  protected fun setupAssembleTasks(
    ...
    taskContainer.assembleTask.configure { task: Task ->
      task.dependsOn(hostTestCreationConfig.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES))
    }
