...
/**
 * Common behaviors for a container of an artifact that gets published within the project scope.
 *
...
internal abstract class ArtifactContainer<FileTypeT, StoredT, AdapterT>(private val allocator: () -> AdapterT)
  ...
  /**
   * If another [org.gradle.api.Task] is replacing the initial providers through the Variant API, it is interesting to determine that
   * initial providers are useless since replaced and therefore should probably not even be configured.
   *
  ...
  fun getTaskProviders(): List<TaskProvider<*>> = (initialTaskProviders + transformationTaskProviders).toList()
   ...
   *
   * @return the current version of the artifact providers at the time this call is made. This is very useful for [org.gradle.api.Task] that
   *   want to transform an artifact and need the current version as its input while producing the final version.
   */
  @VisibleForTesting
   ...
   *
   * @param taskProvider the task provider for the task producing the [with]. Mainly provided for bookkeeping reasons, not strictly required
   *   for wiring.
...
internal class SingleArtifactContainer<FileTypeT : FileSystemLocation>(allocator: () -> SinglePropertyAdapter<FileTypeT>) :
  ...
  /**
   * Specific hook for AGP providers to register the initial producer of the artifact as well as folder/file naming.
   *
   * @param taskProvider the task provider for the task producing the [with]. Mainly provided for bookkeeping reasons, not strictly required
   *   for wiring.
   * @param with the provider that will be the transformed artifact.
   ...
   */
  fun initArtifactContainer(taskProvider: TaskProvider<*>, with: Provider<FileTypeT>, initialNamingContext: ArtifactNamingContext) {
