 ...
 *
 * Caching disabled by default for this task because the task does very little work. The task extracts some files, unchanged, from the
 * Inputs (all Jar files). Calculating cache hit/miss and fetching results is likely more expensive than simply executing the task.
 */
...
abstract class ExtractProfilerNativeDependenciesTask : NonIncrementalTask() {
  ...
  @get:Classpath abstract val inputJars: ConfigurableFileCollection
  ...
  class CreationAction(apkCreationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<ExtractProfilerNativeDependenciesTask, ApkCreationConfig>(apkCreationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ExtractProfilerNativeDependenciesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ExtractProfilerNativeDependenciesTask::outputDir)
        .withName("out")
        .on(InternalArtifactType.PROFILERS_NATIVE_LIBS)
    }

    override fun configure(task: ExtractProfilerNativeDependenciesTask) {
  ...
  abstract class ExtractProfilerNativeDepsWorkerAction : ProfileAwareWorkAction<ExtractProfilerNativeDepsWorkerAction.Parameters>() {
    ...
    override fun run() {
      ...
      parameters.inputJars.forEach { inputJar -> ZipInputStream(FileInputStream(inputJar)).use { extractNestedNativeLibs(it) } }
    }
    ...
    private fun extractNestedNativeLibs(zipInputStream: ZipInputStream) {
      ...
      actOnMatchingZipEntries(
        ...
        { _, dependencyJarBytes -> ZipInputStream(ByteArrayInputStream(dependencyJarBytes)).use { extractNativeLibs(it) } },
      )
    }
    ...
    private fun extractNativeLibs(zipInputStream: ZipInputStream) {
      ...
      actOnMatchingZipEntries(
        zipInputStream,
        { MergeNativeLibsTask.predicate.test(it.substringAfterLast('/')) && JAR_JNI_PATTERN.matcher(it).matches() },
    ...
    /**
     * Iterates through the zip entries and performs the specified action with each entry whose name matches the given predicate.
     *
     * This function does not open or close the [ZipInputStream]; it assumes the stream is already open.
     */
