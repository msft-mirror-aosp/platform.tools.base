 ...
 *
 * Caching disabled by default for this task because the task does very little work. The task moves files around on the disk, doing no
 * substantial computation. Calculating cache hit/miss and fetching results is likely more expensive than simply executing the task.
 */
@DisableCachingByDefault
@BuildAnalyzer(primaryTaskCategory = TaskCategory.NATIVE)
abstract class LibraryJniLibsTask : NonIncrementalTask() {

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val projectNativeLibs: DirectoryProperty

  @get:Classpath @get:Optional abstract val localJarsNativeLibs: ConfigurableFileCollection

  @get:OutputDirectory abstract val outputDirectory: DirectoryProperty

  override fun doTaskAction() {
    LibraryJniLibsDelegate(projectNativeLibs.get().asFile, localJarsNativeLibs.files, outputDirectory.get().asFile, workerExecutor, this)
      .copyFiles()
  }
    ...
    override fun run() {
      ...
      if (inputFile.isFile) {
        Preconditions.checkState(
          inputFile.name.endsWith(DOT_JAR) || inputFile.name.endsWith(DOT_AAR),
  ...
  abstract class CreationAction(creationConfig: VariantCreationConfig, val artifactType: InternalArtifactType<Directory>) :
    VariantTaskCreationAction<LibraryJniLibsTask, VariantCreationConfig>(creationConfig) {
    override val type = LibraryJniLibsTask::class.java

    override fun handleProvider(taskProvider: TaskProvider<LibraryJniLibsTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, LibraryJniLibsTask::outputDirectory)
        .withName(SdkConstants.FD_JNI)
        .on(artifactType)
    }
  }

  class ProjectOnlyCreationAction(creationConfig: VariantCreationConfig, artifactType: InternalArtifactType<Directory>) :
    ...
    override fun configure(task: LibraryJniLibsTask) {
      ...
      task.projectNativeLibs.setDisallowChanges(creationConfig.artifacts.get(MERGED_NATIVE_LIBS))
      task.localJarsNativeLibs.disallowChanges()
    }
  }

  class ProjectAndLocalJarsCreationAction(creationConfig: VariantCreationConfig, artifactType: InternalArtifactType<Directory>) :
    CreationAction(creationConfig, artifactType) {
    override val name: String = computeTaskName("copy", "JniLibsProjectAndLocalJars")

    override fun configure(task: LibraryJniLibsTask) {
      super.configure(task)
      task.projectNativeLibs.setDisallowChanges(creationConfig.artifacts.get(STRIPPED_NATIVE_LIBS))
...
private fun copyFromFolder(from: File, outputDirectory: File, pathSegments: MutableList<String>) {
  val children = from.listFiles { file, name -> file.isDirectory || name.endsWith(SdkConstants.DOT_NATIVE_LIBS) }
...
private fun copyFromJar(jarFile: File, outputDirectory: File) {
  ZipFile(jarFile).use { zipFile ->
    ...
    while (entries.hasMoreElements()) {
      ...
      // read the content.
      val byteBuffer = zipFile.getInputStream(entry).use { ByteBuffer.wrap(ByteStreams.toByteArray(it)) }
...
/**
 * computes a file path from a root folder and a zip archive path, removing the lib/ part of the path
 *
 * @param rootFolder the root folder
 * @param path the archive path
 * @return the File
 */
private fun computeFile(rootFolder: File, path: String) = File(rootFolder, FileUtils.toSystemDependentPath(path.substring(4)))
