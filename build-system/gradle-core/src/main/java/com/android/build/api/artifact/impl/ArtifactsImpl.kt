 ...
 *
 * Implementation will delegate most of its [FileSystemLocationProperty] handling to [ArtifactContainer] only handling the [TaskProvider]
 * relationships.
 *
 * This call will also contain some AGP private methods for added services to support AGP specific services like setting initial AGP
 * providers.
 */
class ArtifactsImpl(
  project: Project,
  ...
  override fun forScope(scope: ScopedArtifacts.Scope): ScopedArtifactsImpl =
    publicScopedArtifacts[mappingScopePolicy(scope)] ?: throw IllegalArgumentException("${scope.name} is not implemented yet !")
  ...
  fun forScope(scope: InternalScopedArtifacts.InternalScope): ScopedArtifactsImpl =
    internalScopedArtifacts[scope] ?: throw java.lang.IllegalArgumentException("${scope.name} not supported")

  override fun <FILE_TYPE : FileSystemLocation> get(type: SingleArtifact<FILE_TYPE>): Provider<FILE_TYPE> = getArtifactContainer(type).get()
  ...
  override fun <TaskT : Task> use(taskProvider: TaskProvider<TaskT>): TaskBasedOperationImpl<TaskT> {
    return TaskBasedOperationImpl(this, taskProvider).also { outstandingRequests.add(it) }
  }

  // End of public API implementation, start of private AGP services.

  /**
   * Returns a [File] representing the artifact type location (could be a directory or regular file). forceFilename is to overwrite default
   * fileName
   */
  ...
  private fun generateIntermediatePath(type: Single<*>, task: Task, ignorePredefinedLocation: Boolean, fileName: String?): File {
    ...
    return if (output == null || ignorePredefinedLocation) getIntermediateOutputPath(type, task.name, forceFilename = fileName)
    else FileUtils.join(output, fileName)
  }
  ...
  private fun calculateFileName(type: Single<*>): String {
    return when (type.kind) {
      is ArtifactKind.FILE ->
        type.getFileSystemLocationName().ifEmpty {
  ...
  /**
   * Returns the [ArtifactContainer] for the passed [type]. The instance may be allocated as part of the call if there is not
   * [ArtifactContainer] for this [type] registered yet.
   *
   * @param type requested artifact type
   * @return the [ArtifactContainer] for the passed type
   */
  internal fun <FILE_TYPE : FileSystemLocation> getArtifactContainer(type: Single<FILE_TYPE>): SingleArtifactContainer<FILE_TYPE> {
   ...
   * @return the [ArtifactContainer] for the passed type
   */
  internal fun <FILE_TYPE : FileSystemLocation> getArtifactContainer(type: Multiple<FILE_TYPE>): MultipleArtifactContainer<FILE_TYPE> {

    return storageProvider.getStorage(type.kind).getArtifact(objects, type)
  }

  fun <T : FileSystemLocation> republish(source: Single<T>, target: Single<T>) {
    storageProvider.getStorage(target.kind).copy(target, getArtifactContainer(source))
  }

  private fun <T : FileSystemLocation> republish(source: Multiple<T>, target: Multiple<T>) {
    storageProvider.getStorage(target.kind).copy(target, getArtifactContainer(source))
  }

  /**
   * Sets the Android Gradle Plugin producer. Although conceptually the AGP producers are first to process/consume artifacts, we want to
   * register them last after all custom cod`e has had opportunities to transform or replace it.
   *
   * Therefore, we cannot rely on the usual append/replace pattern but instead use this API to be artificially put first in the list of
   * producers.
   *
  ...
  ): SingleInitialProviderRequestImpl<TASK, FILE_TYPE> where FILE_TYPE : FileSystemLocation, TASK : Task =
   ...
   *
   * Although conceptually the AGP producers are first to produce artifacts, we want to register them last after all custom code had the
   * opportunity to transform or replace it.
   *
   * Therefore, we cannot rely on the usual append/replace pattern but instead use this API to be artificially put first in the list of
   * producers for the passed [type]
   *
   ...
   *
   * If there are more than one producer appending artifacts for the passed type, calling this method will generate an error.
   *
   ...
   */
  fun <T : FileSystemLocation> setTaskInputToFinalProduct(artifactType: Single<T>, taskInputProperty: Property<T>) {
    val finalProduct = get(artifactType)
    taskInputProperty.setDisallowChanges(finalProduct)
  }

  fun <FILE_TYPE : FileSystemLocation> copy(artifactType: Single<FILE_TYPE>, from: ArtifactsImpl) {
  ...
  fun <FILE_TYPE : FileSystemLocation> copy(artifactType: Multiple<FILE_TYPE>, from: ArtifactsImpl) {
    val artifactContainer = from.getArtifactContainer(artifactType)
    storageProvider.getStorage(artifactType.kind).copy(artifactType, artifactContainer)
  }
  ...
  /**
   * Appends a single [Provider] of [T] to a [ScopedArtifact] under the [ScopedArtifacts.Scope.PROJECT] scope.
   *
   * @param type the multiple type to append to.
   * @param from the element to add.
   */
  fun <T : FileSystemLocation> appendTo(type: ScopedArtifact, from: Single<T>) {
    forScope(ScopedArtifacts.Scope.PROJECT).setInitialContent(type, this, from)
  }
   ...
   *
   * Make sure all pending requests have been fully configured. Finalize all artifact type.
   *
   * TODO: lock the container so users cannot invoke any of the variant API methods on the artifacts.
   */
  internal fun finalizeAndLock() {
    if (outstandingRequests.isEmpty()) return
    throw RuntimeException(
      outstandingRequests.joinToString(
        separator = "\n\t",
   ...
   * @param item the static file/directory to add.
   */
  internal fun <T : FileSystemLocation> addStaticProvider(container: MultipleArtifactContainer<T>, type: Artifact<T>, item: Provider<T>) {
    // if no one is producing T yet, create an empty Task, register its output to be T
    // and register it to the providers for this artifact type.
    synchronized(staticProviders) {
      val taskProvider =
      ...
      val mappedValue = taskProvider.flatMap { _ -> item }
...
/**
 * Function fromProperty returns an output property, so we can set path to output file. Function fromProvider returns output provider that
 * gradle uses to connect tasks together. In most cases fromProperty is fromProvider, but sometimes (as for BundleAar/AbstractArchiveTask)
 * task has a provider declared as @Output. Gradle requires us to use the same provider that marked as @Output to connect tasks. In this
 * case we need separate mapped property to set output path for.
 */
internal class SingleInitialProviderRequestImpl<TASK : Task, FILE_TYPE : FileSystemLocation>(
  ...
  /**
   * Internal API to set the location of the directory where the produced [FILE_TYPE] should be located in.
   *
  ...
  fun atLocation(finalLocation: Provider<Directory>): SingleInitialProviderRequestImpl<TASK, FILE_TYPE> {
    buildOutputLocation.set(finalLocation)
    return this
  }

  fun atLocation(finalLocation: File): SingleInitialProviderRequestImpl<TASK, FILE_TYPE> {
    buildOutputLocation.set(finalLocation)
    return this
  }
  ...
  fun on(type: Single<FILE_TYPE>) {
    ...
    val context = ArtifactNamingContext(fileName, absoluteOutputLocation, buildOutputLocation)
    artifactContainer.initArtifactContainer(taskProvider, taskProvider.flatMap { fromProvider(it) }, context)
