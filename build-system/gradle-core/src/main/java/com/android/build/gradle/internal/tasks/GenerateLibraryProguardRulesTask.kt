...
/**
 * Generates the Proguard keep rules file `aapt_rules.txt`. In the past this was generated by AAPT, but since we don't call AAPT(2) for
 * packaging library resources we are generating the file ourselves.
 *
 * This task takes the merged manifest and the merged resources directory (local only, small merge), collects information from them and
 * generates the keep rules file.
 */
@CacheableTask
@BuildAnalyzer(primaryTaskCategory = TaskCategory.OPTIMIZATION)
abstract class GenerateLibraryProguardRulesTask : NewIncrementalTask() {

  @get:OutputFile abstract val proguardOutputFile: RegularFileProperty

  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val manifestFile: RegularFileProperty

  @get:Incremental @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputResourcesDir: DirectoryProperty
  ...
  override fun doTaskAction(inputChanges: InputChanges) {
    ...
    val changedResources =
      if (isIncremental) {
        inputChanges.getChangesInSerializableForm(inputResourcesDir).changes
      } else {
        emptyList()
      }
    workerExecutor.noIsolation().submit(GenerateProguardRulesWorkAction::class.java) {
  ...
  abstract class GenerateProguardRulesWorkAction : ProfileAwareWorkAction<GenerateProguardRulesWorkAction.Params>() {
    ...
    private fun canBeProcessedIncrementally(params: Params): Boolean =
      params.changedResources.get().all { canResourcesBeProcessedIncrementally(it) } && params.incremental.get()
  }

  class CreationAction(creationConfig: ComponentCreationConfig) :
    VariantTaskCreationAction<GenerateLibraryProguardRulesTask, ComponentCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<GenerateLibraryProguardRulesTask>) {
      super.handleProvider(taskProvider)
      creationConfig.artifacts
        .setInitialProvider(taskProvider, GenerateLibraryProguardRulesTask::proguardOutputFile)
        .withName(SdkConstants.FN_AAPT_RULES)
        .on(InternalArtifactType.AAPT_PROGUARD_FILE)
    }

    override fun configure(task: GenerateLibraryProguardRulesTask) {
      super.configure(task)

      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.PACKAGED_RES, task.inputResourcesDir)

      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.MERGED_MANIFEST, task.manifestFile)
    }
  }
}
...
internal fun runFullTask(params: GenerateLibraryProguardRulesTask.GenerateProguardRulesWorkAction.Params) {
  ...
  Files.write(
    ...
    generateMinifyKeepRules(parseManifest(params.manifestFile.get().asFile), params.inputResourcesDir.get().asFile),
  )
}

internal fun runIncrementalTask(params: GenerateLibraryProguardRulesTask.GenerateProguardRulesWorkAction.Params) {
  if (!params.proguardOutputFile.get().asFile.exists()) {
    Logging.getLogger(GenerateLibraryProguardRulesTask::class.java).warn("Cannot find file: ${params.proguardOutputFile.get().asFile.path}")
    runFullTask(params)
    return
  }
  val addedLayoutFiles = params.changedResources.get().filter { isLayoutFile(it.file) && it.fileStatus == FileStatus.NEW }.map { it.file }
  if (addedLayoutFiles.none()) {
    return
  }
  val currentKeepRules = parseMinifiedKeepRules(params.proguardOutputFile.get().asFile)
  val documentBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder()
  addedLayoutFiles.forEach { addedLayoutFile -> generateKeepRulesFromLayoutXmlFile(addedLayoutFile, documentBuilder, currentKeepRules) }
  val contentsToWrite = "# Generated by the gradle plugin\n${currentKeepRules.joinToString("\n")}".toByteArray()
  Files.write(params.proguardOutputFile.get().asFile.toPath(), contentsToWrite)
}

private fun isLayoutFile(file: File): Boolean = ResourceFolderType.getFolderType(file.parentFile.name) == ResourceFolderType.LAYOUT
