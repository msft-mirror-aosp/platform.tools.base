...
/**
 * For generating and collecting jacoco xml coverage reports for unit tests, instrumentation tests
 * and test suites for current module and dependent modules.
 */
...
abstract class CodeCoverageCollectionTask : NonIncrementalTask() {
    ...
    @get:Optional
    abstract val unitTestCoverageFile: ConfigurableFileCollection
    ...
    abstract class BaseCoverageCollectionCreationAction(val jacocoAntConfiguration: Configuration, creationConfig: CodeCoverageReportCreationConfig) :
        VariantTaskCreationAction<CodeCoverageCollectionTask, CodeCoverageReportCreationConfig>(
            creationConfig
        ) {
        ...
        override fun configure(
            task: CodeCoverageCollectionTask
        ) {
            ...
            creationConfig.unitTestCoverageFile?.let {
                task.unitTestCoverageFile.fromDisallowChanges(
                    it
                )
            }

            creationConfig.connectedTestCoverageDirectory?.let {
                task.connectedTestCoverageDirectory.fromDisallowChanges(
                    it
                )
            }

            creationConfig.java { javaSources ->
                task.sources.addAll(javaSources.getAsFileTrees())
            }
            creationConfig.kotlin { kotlinSources ->
    ...
    class CoverageCollectionCreationAction(jacocoAntConfiguration: Configuration, creationConfig: CodeCoverageReportCreationConfig) :
        BaseCoverageCollectionCreationAction(
        ...
        override fun handleProvider(
            taskProvider: TaskProvider<CodeCoverageCollectionTask>
        ) {
            super.handleProvider(taskProvider)

            creationConfig.artifacts.setInitialProvider(
                taskProvider,
                CodeCoverageCollectionTask::reportOutputDir
            ).on(InternalArtifactType.VARIANT_CODE_COVERAGE_DATA)

            creationConfig.globalArtifacts.use(taskProvider)
                .wiredWith(CodeCoverageCollectionTask::reportOutputDir)
                .toAppendTo(InternalMultipleArtifactType.CODE_COVERAGE_DATA)
        }
    }

    class AggregatedCoverageCollectionCreationAction(jacocoAntConfiguration: Configuration, creationConfig: CodeCoverageReportCreationConfig) :
        BaseCoverageCollectionCreationAction(
            jacocoAntConfiguration,
            creationConfig
        ) {
        ...
        override fun configure(task: CodeCoverageCollectionTask) {
            ...
            task.dependentModuleCoverageData.from(
                creationConfig.dependantModulesReports
            )
        }
    ...
    abstract class CodeCoverageCollectionWorkerAction : WorkAction<CodeCoverageWorkParameters> {
        ...
        companion object {
             ...
             *
             * This function adds custom properties and source folder information to the XML report
             * generated by Jacoco. The properties are added under a `<properties>` tag, and the
             * source folders are added under a `<sources>` tag.
             *
