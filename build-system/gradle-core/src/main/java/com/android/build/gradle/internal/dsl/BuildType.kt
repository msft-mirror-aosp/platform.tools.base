...
constructor(
  ...
  /**
   * Whether to enable the checks that the both the old and new way of configuring bytecode postProcessing are not used at the same time.
   *
  ...
  @Suppress("DEPRECATION") private val _isDefaultProperty: Property<Boolean> = dslServices.property(Boolean::class.java).convention(false)
  ...
  /** Initialize the DSL object with the debug signingConfig. Not meant to be used from the build scripts. */
  ...
  /** Initialize the DSL object without the signingConfig. Not meant to be used from the build scripts. */
  fun init() {
    if (BuilderConstants.DEBUG == name) {
      setDebuggable(true)
  ...
  override fun _initWith(that: BaseConfig) {
    ...
    val thatBuildType = that as BuildType
    ndkConfig._initWith(thatBuildType.ndkConfig)
    javaCompileOptions.annotationProcessorOptions._initWith(thatBuildType.javaCompileOptions.annotationProcessorOptions)
  ...
  override fun buildConfigField(type: String, name: String, value: String) {
    ...
    if (alreadyPresent != null) {
      val message = String.format("BuildType(%s): buildConfigField '%s' value is being replaced.", getName(), name)
   ...
   *
   * They are located in the SDK. Using `getDefaultProguardFile(String filename)` will return the full path to the files. They are identical
  ...
  fun setTestProguardFiles(files: Iterable<*>): BuildType {
    ...
    testProguardFiles(*Iterables.toArray(files, Any::class.java))
    return this
  }
  ...
  fun setConsumerProguardFiles(proguardFileIterable: Iterable<*>): BuildType {
    ...
    consumerProguardFiles(*Iterables.toArray(proguardFileIterable, Any::class.java))
    return this
  }
   ...
   * [`minifyEnabled`](com.android.build.gradle.internal.dsl.BuildType.html#com.android.build.gradle.internal.dsl.BuildType:minifyEnabled)
   * to `true`, the Android plugin uses R8.
   *
   * To learn more, read [Shrink, obfuscate, and optimize your app](https://developer.android.com/studio/build/shrink-code.html).
   */
  override val isUseProguard: Boolean?
    get() {
      dslServices.deprecationReporter.reportObsoleteUsage("useProguard", DeprecationReporter.DeprecationTarget.VERSION_8_0)
      return false
    }
