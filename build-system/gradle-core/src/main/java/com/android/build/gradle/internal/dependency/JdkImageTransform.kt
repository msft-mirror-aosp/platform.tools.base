...
abstract class JdkImageTransform : TransformAction<JdkImageTransform.Parameters> {
  ...
  interface Parameters : GenericTransformParameters {
    @get:Input val jdkId: Property<String>

    @get:Internal val javaHome: Property<File>
  }
  ...
  override fun transform(outputs: TransformOutputs) {
    ...
    JdkImageTransformDelegate(
        ...
        tempDir,
        jdkImageDir,
        JdkTools(parameters.javaHome.get(), GradleProcessExecutor(execOperations::exec), LoggerWrapper.getLogger(this::class.java)),
      )
      .run()
...
fun getJdkImageFromTransform(services: TaskCreationServices, javaCompiler: JavaCompiler?): FileCollection {
  ...
  return configuration.incoming
    .artifactView { config ->
      config.attributes {
        it.attribute(ARTIFACT_TYPE_ATTRIBUTE, ANDROID_JDK_IMAGE)
...
private fun registerJdkImageTransform(services: TaskCreationServices, javaCompiler: JavaCompiler?) {
  ...
  services.dependencies.registerTransform(JdkImageTransform::class.java) { spec: TransformSpec<JdkImageTransform.Parameters> ->
