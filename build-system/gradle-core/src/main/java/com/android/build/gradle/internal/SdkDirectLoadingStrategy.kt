...
/**
 * This loads the SDK and BuildTools specified inside the {@code options} directly, without performing a full SDK scan (see {@link
 * SdkFullLoadingStrategy}).
 *
 * <p>This is done by trying to predict the directory of the buildtool/SDK requested and parsing only those package.xml files (with some
 * basic validation).
 *
 * <p>Unlike the full SDK scan, it cannot load/find sdk components that are installed in a non-default directory structure.
 */
class SdkDirectLoadingStrategy(
  ...
  private val components: DirectLoadComponents? by lazy { init(providerFactory) }
  ...
  private fun init(providerFactory: ProviderFactory): DirectLoadComponents? {
    val targetHash = checkNotNull(platformTargetHashSupplier) { "Extension not initialized yet, couldn't access compileSdkVersion." }
    val buildToolRevision =
      checkBuildToolsRevision(
        checkNotNull(buildToolRevisionSupplier) { "Extension not initialized yet, couldn't access buildToolsVersion." }
      )

    return loadSdkComponents(targetHash, buildToolRevision, providerFactory)
  }

  private fun loadSdkComponents(targetHash: String, buildToolRevision: Revision, providerFactory: ProviderFactory): DirectLoadComponents? {
    ...
    val platformTools = PlatformToolsComponents.build(sdkDirectory, providerFactory)
    val supportTools = SupportToolsComponents.build(sdkDirectory, targetHash, providerFactory)

    val buildTools =
      buildToolsCache.getOrPut(buildToolRevision) { Optional.ofNullable(buildBuildTools(sdkDirectory, buildToolRevision)) }.orElse(null)
    val platform =
      platformCache
        .getOrPut(targetHash) {
          Optional.ofNullable(PlatformComponents.build(sdkDirectory, targetHash, ConsoleProgressIndicatorFactory(providerFactory)))
        }
        .orElse(null)
    ...
    return DirectLoadComponents(sdkDirectory, platformTools, supportTools, buildTools, platform, emulator)
  }
  ...
  private fun getFileFromBuildTool(component: BuildToolInfo.PathId) = getBuildToolsInfo()?.let { File(it.getPath(component)) }
  ...
  fun getRenderScriptSupportJar() = getBuildToolsInfo()?.let { RenderScriptProcessor.getSupportJar(it.location.toFile(), useAndroidX) }

  fun getSupportNativeLibFolder() = getBuildToolsInfo()?.let { RenderScriptProcessor.getSupportNativeLibFolder(it.location.toFile()) }

  fun getSupportBlasLibFolder() = getBuildToolsInfo()?.let { RenderScriptProcessor.getSupportBlasLibFolder(it.location.toFile()) }

  fun getSystemImageLibFolder(imageHash: String) =
    systemImageCache
      .getOrPut(imageHash) { Optional.ofNullable(SystemImageComponents.build(components?.sdkDirectory, imageHash)) }
      .orElse(null)
      ?.systemImageDir
...
private class PlatformToolsComponents(
  ...
  companion object {
    internal fun build(sdkDirectory: File, providerFactory: ProviderFactory): PlatformToolsComponents? {
      val fileExists =
        providerFactory.of(FileExistsValueSource::class.java) {
          it.parameters.file.set(sdkDirectory.resolve(SdkConstants.FD_PLATFORM_TOOLS).resolve("package.xml"))
        }
      if (!fileExists.get()) {
        return null
      }
      return PlatformToolsComponents(sdkDirectory.resolve(SdkConstants.FD_PLATFORM_TOOLS).resolve(SdkConstants.FN_ADB))
    }
  }
}
  ...
  companion object {
    internal fun build(sdkDirectory: File, targetHash: String, providerFactory: ProviderFactory): SupportToolsComponents? {
      ...
      return SupportToolsComponents(
        sdkDirectory.resolve(SdkConstants.FD_TOOLS).resolve(SdkConstants.FD_SUPPORT).resolve(SdkConstants.FN_ANNOTATIONS_JAR)
      )
    }
  }
}

/** This holds all components related to the platform targeted, usually $SDK/platforms/android-{api_level}/ */
private class PlatformComponents(
   ...
   *
   * This will not be present if the compile-sdk version is less than 26 (a fallback to platform-tools would not help for users that update
   * their SDK, as it is removed in recent platform-tools)
   */
  ...
  companion object {
    ...
    internal fun build(
      ...
      val platformVersionFromHash =
        AndroidTargetHash.getVersionFromHash(targetHash) ?: return null // We are not sure which version this hash maps to.
      ...
      val platformPackage = parsePackage(platformXml, consoleProgressIndicatorFactory)
      ...
      val platformVersion =
        if (platformVersionFromPlatformXml?.extensionLevel == null) {
      ...
      return PlatformComponents(
        ...
        platformBase.resolve(SdkConstants.FN_CORE_FOR_SYSTEM_MODULES).let { if (it.exists()) it else null },
      )
    }
  }
}
...
private class EmulatorComponents(internal val emulatorDir: File) {
  companion object {
    internal fun build(sdkDirectory: File, providerFactory: ProviderFactory): EmulatorComponents? {
      ...
      val fileExists = providerFactory.of(FileExistsValueSource::class.java) { it.parameters.file.set(emulatorBase.resolve("package.xml")) }
