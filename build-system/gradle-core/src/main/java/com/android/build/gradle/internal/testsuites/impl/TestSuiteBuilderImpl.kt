...
internal abstract class TestSuiteBuilderImpl
@Inject
internal constructor(
  ...
  companion object {
    ...
    /**
     * Create the list of test suites for this component, the list is driven by the passed [dslDefinedTestSuites] which is the list of test
     * suites targets implicitly or explicitly defined by the Component type and its DSL.
     */
    fun create(
      ...
      return dslDefinedTestSuites.associate { agpTestSuite ->
        agpTestSuite.testSuite.getJunitEngineIfUsed() ?: throw RuntimeException("Test suites must use junit engines for now")
  ...
  override val targets = testSuiteDslInfo.targets.associate { it.name to TestSuiteTargetBuilderImpl(it) }
  ...
  internal fun getSources(): Collection<TestSuiteSourceCreationConfig> {
    ...
    if (testSuite.getSourceContainers().isEmpty()) {
      return listOf(TestSuiteAssetsSpecImpl(objects, _name, services.projectInfo.projectDirectory, services.projectInfo.buildDirectory))
    }
    return testSuite.getSourceContainers()
  }
}

internal class JUnitEngineSpecForVariantBuilder(objects: ObjectFactory, dslDefinedJUnitEngineSpec: JUnitEngineSpecImpl) :
  JUnitEngineSpecBuilder {
  override val includeEngines: MutableSet<String> =
    mutableSetOf<String>().also { list -> list.addAll(dslDefinedJUnitEngineSpec.includeEngines) }
  override val inputs: MutableList<AgpTestSuiteInputParameters> =
    mutableListOf<AgpTestSuiteInputParameters>().also { list -> list.addAll(dslDefinedJUnitEngineSpec.inputs) }
  ...
  internal val inputProperties: MapProperty<String, String> =
    objects.mapProperty(String::class.java, String::class.java).also {
      ...
      dslDefinedJUnitEngineSpec.inputProperties.forEach { (t, u) -> it.put(t, u) }
    }

  override val enginesDependencies: DependencyCollector = dslDefinedJUnitEngineSpec.enginesDependencies
}
