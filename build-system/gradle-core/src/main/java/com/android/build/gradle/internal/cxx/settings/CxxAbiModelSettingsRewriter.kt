 ...
 * 1) Calculate the CMake or ndk-build command line in [CxxAbiModel::configurationArguments] without expanding any macros that refer to hash
 *    of this configuration. Also, the name of the ABI doesn't contribute to the hash so that the final folders end up with ABI as peers.
 * 2) Calculate a hash of CMake (or ndk-build) of [CxxAbiModel::configurationArguments] and store it in [CxxAbiModel::fullConfigurationHash]
 * 3) Substitute hash, variant, and ABI names into fields.
 *
 * The purpose of this separation is to allow file outputs to depend on configuration hash but also for the rest of the paths to contribute
 * to the hash itself.
 */
fun CxxAbiModel.calculateConfigurationArguments(providers: ProviderFactory, layout: ProjectLayout): CxxAbiModel {
  return calculateConfigurationArgumentsExceptHash(providers, layout).calculateConfigurationHash().expandConfigurationHashMacros()
}

/**
 * Calculates the configuration arguments for CMake and ndk-build. This incorporates the flags from the user's build.gradle file as well as
 * any values from CMakeSettings.json.
 *
 * Since some of those arguments come from the user, we then need to update the model based on the command-line. For example, if user
 * defines:
 *
 ...
 */
private fun CxxAbiModel.calculateConfigurationArgumentsExceptHash(providers: ProviderFactory, layout: ProjectLayout): CxxAbiModel {
  ...
  return argsRewritten.copy(
    configurationArguments =
      argsRewritten.configurationArguments
...
/**
 * Calculate hash and plug it into the model. Return the new model along with the text that was hashed.
 *
 * The user can pass arbitrary parameters to CMake (or ndk-build) so we can't guard against every case that could make the hash
 * local-machine-specific. This is best effort, works in the default case, and we provide the hash_key.txt file (written later) to be able
 * to diagnose problems. The consequence of a non-portable paths is that hypothetical caching tech (ccache, reclient, gomacc, etc) may not
 * be able to share results. However, build results will still be accurate.
 */
private fun CxxAbiModel.calculateConfigurationHash(): CxxAbiModel {
  ...
  return copy(fullConfigurationHash = sha256Of(hashKey, includeGradleVersionInHash = false), fullConfigurationHashKey = hashKey)
}
...
/** Create a [RewriteConfiguration] which has information for transforming this ABI by expanding macros in different fields. */
@VisibleForTesting
fun CxxAbiModel.getAbiRewriteConfiguration(providers: ProviderFactory, layout: ProjectLayout): RewriteConfiguration {
  val allSettings = gatherSettingsFromAllLocations(providers, layout)

  val configuration =
    allSettings
      .getConfiguration(TRADITIONAL_CONFIGURATION_NAME)!!
  ...
  val arguments = (builtInCommandLineArguments + buildGradleCommandLineArguments).removeSubsumedArguments()

  val environments =
    listOf(configuration.getSettingsFromConfiguration(), getSettingsFromCommandLine(arguments), allSettings).flatMap { it.environments }

  val reifier = getPreHashReifier(SettingsEnvironmentNameResolver(environments), configuration.inheritEnvironments)
...
/** This model contains the inner models of [CxxAbiModel] that are rewritten during clean/build for CMake builds. */
@VisibleForTesting
class RewriteConfiguration(val buildCommandArgs: String?, val reifier: (String) -> (String), val configurationArgs: List<String>)

/**
 * Get a reify function based on the given [SettingsEnvironmentNameResolver] that will expand all macros except for macros that should be
 * expanded *after* the unique configuration has been determined for the given ABI. The macros that are *not* reified are:
 * - [NDK_FULL_CONFIGURATION_HASH] -- so that the hash code can be used in the path to output files and folders.
 * - [NDK_CONFIGURATION_HASH] -- Just a short version of [NDK_FULL_CONFIGURATION_HASH].
 * - [NDK_ABI] -- This is so that that when folder paths are constructed containing [NDK_FULL_CONFIGURATION_HASH] each of the individual ABI
 *   folders are grouped together as peers.
 */
private fun getPreHashReifier(resolver: SettingsEnvironmentNameResolver, inheritEnvironments: List<String>): (String) -> (String) {
  return { value ->
    fun resolve(tokenMacro: String) = resolver.resolve(tokenMacro, inheritEnvironments) ?: error("Unrecognized macro $tokenMacro")
    reifyString(value) { tokenMacro ->
      when (tokenMacro) {
...
private fun SettingsConfiguration.getSettingsFromConfiguration(): Settings {
  return Settings(
    environments = NameTable(NDK_MODULE_CMAKE_EXECUTABLE to cmakeExecutable, NDK_CMAKE_TOOLCHAIN to cmakeToolchain).environments(),
    configurations = listOf(),
  )
}

/**
 * This is "our" version of the command-line arguments for CMake configuration. The user may override or enhance with arguments from
 * build.gradle or CMakeSettings.json.
 */
...
private fun CxxAbiModel.getNdkBuildCommandLineArguments() =
  getNdkBuildCommandLine().toNdkBuildArguments().removeSubsumedArguments().removeBlankProperties()

/** This is "our" version of the command-line arguments for ndk-build. The user may override or enhance with arguments from build.gradle. */
private fun CxxAbiModel.getNdkBuildCommandLine(): List<String> {
  ...
  applicationMk?.let { result.add("$NDK_APPLICATION_MK=" + it.absolutePath) }
  if (buildIsPrefabCapable()) {
    if (variant.module.ndkVersion.major < 21) {
      ...
      // warning message.
      warnln(NDK_FEATURE_NOT_SUPPORTED_FOR_VERSION, "Prefab packages cannot be automatically imported until NDK r21.")
    }
...
/**
 * General [CxxAbiModel] rewriter utility. It accepts a rewriter function [rewrite] that is called for each property of the model. The
 * implementation of [rewrite] receives the property definition and the current value of the property; it is expected to return a [String]
 * (or null) that can be converted back into the original type of the property.
 *
 * This function also rewrites the referenced [CxxVariantModel] which continues on to the rest of the model.
 */
fun CxxAbiModel.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(
...
private fun CxxProjectModel.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(
...
private fun CxxCmakeModuleModel.rewrite(rewrite: (property: ModelField, string: String) -> String) =
  copy(cmakeExe = rewrite.fileOrNull(CXX_CMAKE_MODULE_MODEL_CMAKE_EXE, cmakeExe))

// Rewriter for CxxModuleModel
private fun CxxModuleModel.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(
...
private fun CxxVariantModel.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(
...
private fun CxxCmakeAbiModel.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(buildCommandArgs = rewrite.stringOrNull(CXX_CMAKE_ABI_MODEL_BUILD_COMMAND_ARGS, buildCommandArgs))

// Rewriter for EnvironmentVariable
private fun EnvironmentVariable.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(name = rewrite(ENVIRONMENT_VARIABLE_NAME, name), value = rewrite.stringOrNull(ENVIRONMENT_VARIABLE_VALUE, value))

// Rewriter for BuildSettingsConfiguration
private fun BuildSettingsConfiguration.rewrite(rewrite: (property: ModelField, value: String) -> String) =
  copy(environmentVariables = environmentVariables.map { it.rewrite(rewrite) })
...
private fun ((ModelField, String) -> String).booleanOrNull(property: ModelField, flag: Boolean?): Boolean? {
  val value =
    when (flag) {
      null -> ""
      true -> "1"
