...
const val ANNOTATION_PROCESSORS_INDICATOR_FILE = "META-INF/services/javax.annotation.processing.Processor"
const val INCREMENTAL_ANNOTATION_PROCESSORS_INDICATOR_FILE = "META-INF/gradle/incremental.annotation.processors"
const val KSP_PROCESSORS_INDICATOR_FILE = "META-INF/services/com.google.devtools.ksp.processing.SymbolProcessorProvider"

/** Whether incremental compilation is enabled or disabled by default. */
const val DEFAULT_INCREMENTAL_COMPILATION = true

/**
 * Configures a [JavaCompile] task with necessary properties to perform compilation and/or annotation processing.
 *
...
fun JavaCompile.configureProperties(creationConfig: JavaCompileCreationConfig) {
  ...
  if (compileOptions.sourceCompatibility.isJava9Compatible) {
    ...
    val jdkImage = getJdkImageFromTransform(creationConfig.services, this.javaCompiler.orNull)
 ...
 */
fun JavaCompile.configurePropertiesForAnnotationProcessing(creationConfig: JavaCompileCreationConfig) {
  ...
  if (creationConfig.useBuiltInKaptSupport) {
    ...
    options.compilerArgumentProviders.add(
      CommandLineArgumentProviderAdapter(processorOptions.finalListOfClassNames, processorOptions.arguments)
    )
    options.compilerArgumentProviders.addAll(processorOptions.argumentProviders)
  }
}

data class SerializableArtifact(val displayName: String, val file: File) : Serializable {

  constructor(artifact: ResolvedArtifactResult) : this(artifact.id.displayName, artifact.file)
}

/**
 * Detects all the annotation processors that will be executed and finds out whether they are incremental or not.
 *
 * NOTE: The format of the annotation processor names is currently not consistent. If the processors are specified from the DSL's annotation
 * processor options, the format is "com.example.processor.SampleProcessor". If the processors are auto-detected on the annotation processor
 * classpath, the format is "processor.jar (com.example.processor:processor:1.0)".
 *
...
fun detectAnnotationAndKspProcessors(
  ...
  if (!apOptionClassNames.isEmpty()) {
    ...
    findAllAnnotationProcessors(annotationProcessorClasspath).forEach { processors[it.key.displayName] = it.value }
  }

  // KSP processors are always applied when there are in processor classpath.
  findAllKspProcessors(kspClasspath).forEach { processors[it.key.displayName] = it.value }

  return processors
}

/**
 * Detects all the annotation processors in the given [ArtifactCollection] and finds out whether they are incremental or not.
 *
 * @return the map from annotation processors to [ProcessorInfo]
 */
private fun findAllAnnotationProcessors(artifacts: Collection<SerializableArtifact>): Map<SerializableArtifact, ProcessorInfo> {
...
private fun findAllKspProcessors(artifacts: Collection<SerializableArtifact>): Map<SerializableArtifact, ProcessorInfo> {
  return detectProcessors(
    artifacts,
    dirFilter = { dir ->
...
private fun detectProcessors(
  ...
  for (artifact in artifacts) {
    ...
    if (artifactFile.isDirectory) {
      dirFilter(artifactFile)?.let { processors[artifact] = it }
    } else if (artifactFile.isFile) {
      try {
        JarFile(artifactFile).use { jarFile -> jarFilter(jarFile)?.let { processors[artifact] = it } }
      } catch (_: IOException) {
...
/**
 * Writes the map from annotation processors to ProcessorInfo indicating whether they are incremental or not, and whether they are KSP
 * processors or not, to the given file in Json format.
 */
fun writeAnnotationProcessorsToJsonFile(processors: Map<String, ProcessorInfo>, processorListFile: File) {
 ...
 *
 * NOTE: The format of the annotation processor names is currently not consistent. See [detectAnnotationAndKspProcessors] where the
 * processors are detected.
 */
fun readAnnotationProcessorsFromJsonFile(processorListFile: File): Map<String, ProcessorInfo> {
  val gson = GsonBuilder().create()
  try {
    FileReader(processorListFile).use { reader ->
      return gson.fromJson(reader, object : TypeToken<Map<String, ProcessorInfo>>() {}.type)
    }
  } catch (e: IOException) {
    throw UncheckedIOException(e)
  }
...
/**
 * Records names & incrementality of annotation processors, and whether all of them are incremental to see if the user can run annotation
...
fun recordAnnotationProcessorsForAnalytics(
  ...
  variant?.isAnnotationProcessingIncremental = !processors.values.contains(ProcessorInfo.NON_INCREMENTAL_AP)
}
...
fun recordCompileOptionsForAnalytics(
  ...
  getBuildService(buildServiceRegistry, AnalyticsConfiguratorService::class.java).get().getProjectBuilder(project.path)?.apply {
    compileOptions =
      GradleBuildProject.CompileOptions.newBuilder()
        .also {
          ...
          toolchainLanguageVersion?.let { version -> it.toolchainLanguageVersion = version.majorVersion.toInt() }
        }
        .build()
  }
}
...
private fun checkDeprecatedSourceAndTarget(
  ...
  val removedOrDeprecated =
    when (severity) {
      IssueReporter.Severity.ERROR -> "removed"
      IssueReporter.Severity.WARNING -> "deprecated"
    }
  val suppressWarningMessage =
    if (severity == IssueReporter.Severity.WARNING) {
...
/**
 * Determines the level of support of [javacVersion] when compiling with the given [sourceCompatibility]/[targetCompatibility] version.
 * - [IssueReporter.Severity.ERROR]: javac will produce an ERROR when compiling with the given source/target version
 * - [IssueReporter.Severity.WARNING]: javac will produce a WARNING when compiling with the given source/target version
...
class JdkImageInput(private val jdkImage: FileCollection) : CommandLineArgumentProvider {
  ...
  @get:Classpath
  val generatedModuleFile: Provider<File> =
    jdkImage.elements.map { it.single().asFile.resolve(JDK_IMAGE_OUTPUT_DIR).resolve("lib/modules") }
...
fun AnnotationProcessorInfo.toProcessorInfo(): ProcessorInfo =
  when {
