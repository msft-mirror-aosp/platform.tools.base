...
abstract class PackageAndroidArtifact : NewIncrementalTask() {
  ...
  @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:InputFiles abstract val baseModuleMetadata: ConfigurableFileCollection

  @get:Optional @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Incremental @get:InputFile abstract val appMetadata: RegularFileProperty
  ...
  @get:Incremental @get:Classpath abstract val javaResourceFiles: ConfigurableFileCollection

  @get:Incremental @get:Classpath abstract val featureJavaResourceFiles: ConfigurableFileCollection

  @get:Incremental @get:Classpath abstract val jniFolders: ConfigurableFileCollection

  @get:PathSensitive(PathSensitivity.RELATIVE) @get:Incremental @get:InputFiles abstract val dexFolders: ConfigurableFileCollection

  @get:PathSensitive(PathSensitivity.RELATIVE) @get:Incremental @get:InputFiles abstract val featureDexFolder: ConfigurableFileCollection

  @get:PathSensitive(PathSensitivity.RELATIVE) @get:Incremental @get:InputFiles abstract val assets: DirectoryProperty

  @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional @get:InputFile abstract val dependencyDataFile: RegularFileProperty

  @get:Input abstract val createdBy: Property<String>

  @get:Input var jniDebugBuild = false

  @get:Nested internal var signingConfigData: SigningConfigDataProvider? = null

  @get:Input abstract val aaptOptionsNoCompress: ListProperty<String>

  @get:Input abstract val jniLibsUseLegacyPackaging: Property<Boolean>

  @get:Input abstract val dexUseLegacyPackaging: Property<Boolean>
  ...
  @get:Input abstract val projectBaseName: Property<String>

  @get:Input abstract val debugBuild: Property<Boolean>

  @get:Input abstract val deterministicEntryOrder: Property<Boolean>
      ...
      manifests
        ...
        .forEach(
          Consumer { manifest: File ->
            if (manifest.isFile && (manifest.name == SdkConstants.ANDROID_MANIFEST_XML)) {
              val parser: ManifestAttributeSupplier = DefaultManifestParser(manifest, { true }, true, null)
              val nativeLibsPackagingMode = PackagingUtils.getNativeLibrariesLibrariesPackagingMode(parser.extractNativeLibs).toString()
              listBuilder.add(nativeLibsPackagingMode)
              val dexPackagingMode = PackagingUtils.getDexPackagingMode(parser.useEmbeddedDex, dexUseLegacyPackaging.get()).toString()
  ...
  @get:OutputFile abstract val ideModelOutputFile: RegularFileProperty

  @get:Nested abstract val outputsHandler: Property<MultiOutputHandler>

  @Input abstract fun getTransformationRequest(): ArtifactTransformationRequest<PackageAndroidArtifact>
  ...
  override fun doTaskAction(inputChanges: InputChanges) {
    ...
    getTransformationRequest()
      .submit(this, workerExecutor.noIsolation(), IncrementalSplitterRunnable::class.java, configure(changedResourceFiles, inputChanges))
  }

  private fun configure(changedResourceFiles: HashSet<File>, changes: InputChanges): (BuiltArtifact, Directory, SplitterParams) -> File {
    val outputsHandler = outputsHandler.get()
    return { builtArtifact: BuiltArtifact, _: Directory?, parameter: SplitterParams ->
      val variantOutput = outputsHandler.getOutput((builtArtifact as BuiltArtifactImpl).variantOutputConfiguration)
      parameter.variantOutput.set(variantOutput)
      parameter.outputHandler.set(outputsHandler.toSerializable())
      val outputFile = outputsHandler.computeBuildOutputFile(outputDirectory.get().asFile, variantOutput!!)
      parameter.incrementalDirForSplit.set(
        outputsHandler.computeUniqueDirForSplit(incrementalFolder.get().asFile, variantOutput, variantName)
      )
      parameter.androidResourcesFile.set(File(builtArtifact.outputFile))
      parameter.androidResourcesChanged.set(changedResourceFiles.contains(File(builtArtifact.outputFile)))
      ...
      if (featureDexFolder.isEmpty) {
        parameter.dexFiles.set(changes.getChangesInSerializableForm(dexFolders))
        parameter.javaResourceFiles.set(changes.getChangesInSerializableForm(javaResourceFiles))
      } else {
        ...
        parameter.dexFiles.set(changes.getChangesInSerializableForm(featureDexFolder))
        parameter.javaResourceFiles.set(changes.getChangesInSerializableForm(featureJavaResourceFiles))
      }
      parameter.assetsFiles.set(changes.getChangesInSerializableForm(assets))
      parameter.jniFiles.set(changes.getChangesInSerializableForm(jniFolders))
      if (appMetadata.isPresent) {
        parameter.appMetadataFiles.set(changes.getChangesInSerializableForm(appMetadata))
      } else {
        parameter.appMetadataFiles.set(SerializableInputChanges(listOf(), setOf()))
      }
      if (mergedArtProfile.isPresent && mergedArtProfile.get().asFile.exists()) {
        parameter.mergedArtProfile.set(changes.getChangesInSerializableForm(mergedArtProfile))
      } else {
        parameter.mergedArtProfile.set(SerializableInputChanges(listOf(), listOf()))
      }
      if (mergedArtProfileMetadata.isPresent && mergedArtProfileMetadata.get().asFile.exists()) {
        parameter.mergedArtProfileMetadata.set(changes.getChangesInSerializableForm(mergedArtProfileMetadata))
      } else {
        parameter.mergedArtProfileMetadata.set(SerializableInputChanges(listOf(), listOf()))
      }
      if (versionControlInfoFile.isPresent && versionControlInfoFile.get().asFile.exists()) {
        parameter.versionControlInfoFile.set(changes.getChangesInSerializableForm(versionControlInfoFile))
      } else {
        parameter.versionControlInfoFile.set(SerializableInputChanges(listOf(), listOf()))
      }
      parameter.getSigningConfigData().set(signingConfigData!!.convertToParams())
      parameter.signingConfigVersionsFile.set(signingConfigVersions.singleFile)
  ...
  abstract class SplitterParams : DecoratedWorkParameters {
    ...
    @Optional abstract fun getSigningConfigData(): Property<SigningConfigProviderParams>
    ...
    @get:Optional abstract val createdBy: Property<String>
    ...
    @get:Optional abstract val dependencyDataFile: RegularFileProperty

    @get:Optional abstract val mergedArtProfile: Property<SerializableInputChanges>

    @get:Optional abstract val mergedArtProfileMetadata: Property<SerializableInputChanges>

    @get:Optional abstract val versionControlInfoFile: Property<SerializableInputChanges>
  ...
  abstract class IncrementalSplitterRunnable : WorkActionAdapter<SplitterParams> {
    override fun doExecute() {
      ...
      try {
        ...
        val cache = ZipSnapshotRepository(cacheDir) { file: File -> Objects.requireNonNull(cacheKeyMap[file])!! }
        val cacheUpdates = mutableSetOf<Runnable>()
        val changedDexFiles = classpathToRelativeFileSet(params.dexFiles.get(), cache, cacheUpdates)
        val changedJavaResources = getChangedJavaResources(params, cacheKeyMap, cache, cacheUpdates)
        val changedAndroidResources: Map<RelativeFile, FileStatus> =
          if (params.androidResourcesChanged.get()) {
            IncrementalRelativeFileSets.fromZip(params.androidResourcesFile.get().asFile, cache, cacheUpdates)
          } else {
            mapOf()
          }
        val changedJniLibs = classpathToRelativeFileSet(params.jniFiles.get(), cache, cacheUpdates)
    ...
    companion object {
      private fun addCacheKeys(builder: MutableMap<File, String?>, prefix: String, changes: SerializableInputChanges) {
      ...
      /** An adapted version of [classpathToRelativeFileSet] that handles zip64 support within this task. */
        ...
        for (change in params!!.javaResourceFiles.get().changes) {
          if (change.normalizedPath.isEmpty()) {
            try {
              ...
              val nonZip64 = copyJavaResourcesOnly(params.incrementalFolder.get().asFile, change.file)
  ...
  // ----- CreationAction -----
  abstract class CreationAction<TaskT : PackageAndroidArtifact>(creationConfig: ApkCreationConfig) :
    ...
    override fun configure(packageAndroidArtifact: TaskT) {
      ...
      packageAndroidArtifact.minSdkVersion.set(packageAndroidArtifact.project.provider { creationConfig.minSdk.apiLevel })
      ...
      packageAndroidArtifact.outputsHandler.set(MultiOutputHandler.create(creationConfig))
      packageAndroidArtifact.outputsHandler.disallowChanges()
      packageAndroidArtifact.incrementalFolder.set(File(creationConfig.paths.getIncrementalDir(packageAndroidArtifact.name), "tmp"))
      if (creationConfig.androidResourcesCreationConfig != null) {
        packageAndroidArtifact.aaptOptionsNoCompress.set(creationConfig.androidResources.noCompress)
      } else {
      ...
      packageAndroidArtifact.jniLibsUseLegacyPackaging.set(creationConfig.packaging.jniLibs.useLegacyPackaging)
      packageAndroidArtifact.jniLibsUseLegacyPackaging.disallowChanges()
      packageAndroidArtifact.dexUseLegacyPackaging.set(creationConfig.packaging.dex.useLegacyPackaging)
      ...
      packageAndroidArtifact.javaResourceFiles.from(creationConfig.artifacts.get(InternalArtifactType.MERGED_JAVA_RES))
      ...
      packageAndroidArtifact.deterministicEntryOrder.set(isDeterministicEntryOrder(creationConfig))
      packageAndroidArtifact.deterministicEntryOrder.disallowChanges()
      if (creationConfig is ApplicationCreationConfig) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.APP_METADATA, packageAndroidArtifact.appMetadata)
        ...
        if (isDeterministic(creationConfig)) {
          packageAndroidArtifact.allInputFilesWithNameOnlyPathSensitivity.from(packageAndroidArtifact.appMetadata)
          if (vcsTaskRan) {
            packageAndroidArtifact.allInputFilesWithNameOnlyPathSensitivity.from(packageAndroidArtifact.versionControlInfoFile)
          }
        }
        if (!creationConfig.debuggable) {
          creationConfig.artifacts.setTaskInputToFinalProduct(BINARY_ART_PROFILE, packageAndroidArtifact.mergedArtProfile)
          if (isDeterministic(creationConfig)) {
            packageAndroidArtifact.allInputFilesWithRelativePathSensitivity.from(packageAndroidArtifact.mergedArtProfile)
          }
          creationConfig.artifacts.setTaskInputToFinalProduct(BINARY_ART_PROFILE_METADATA, packageAndroidArtifact.mergedArtProfileMetadata)
          if (isDeterministic(creationConfig)) {
            packageAndroidArtifact.allInputFilesWithRelativePathSensitivity.from(packageAndroidArtifact.mergedArtProfileMetadata)
          }
        }
      }
      packageAndroidArtifact.assets.set(creationConfig.artifacts.get(COMPRESSED_ASSETS))
      ...
      packageAndroidArtifact.projectBaseName.set(creationConfig.services.projectInfo.getProjectBaseName())
      ...
      if (creationConfig is KmpComponentCreationConfig) {
        ...
        packageAndroidArtifact.buildTargetAbi =
          if (creationConfig.global.splits.abi.isEnable) projectOptions[StringOption.IDE_BUILD_TARGET_ABI] else null
      }
      if (creationConfig.componentType.isDynamicFeature) {
        packageAndroidArtifact.baseModuleMetadata.from(
      ...
      if (userDefinedAbis.isNotEmpty()) {
        ...
        if (projectOptions[BooleanOption.BUILD_ONLY_TARGET_ABI]) {
          abiFilters = firstValidInjectedAbi(projectOptions[StringOption.IDE_BUILD_TARGET_ABI])
        }
      ...
      if (
        creationConfig.componentType.isBaseModule && creationConfig.services.projectOptions[BooleanOption.INCLUDE_DEPENDENCY_INFO_IN_APKS]
      ) {
        creationConfig.artifacts.setTaskInputToFinalProduct(SDK_DEPENDENCY_DATA, packageAndroidArtifact.dependencyDataFile)
      }
      ...
      if (
        creationConfig.componentType.isDynamicFeature ||
          (creationConfig is TestComponentCreationConfig &&
            (creationConfig as TestComponentCreationConfig).mainVariant.componentType.isDynamicFeature)
      ) {
        packageAndroidArtifact.signingConfigVersions.from(
          creationConfig.variantDependencies.getArtifactFileCollection(
            ConsumedConfigType.COMPILE_CLASSPATH,
            ArtifactScope.PROJECT,
            AndroidArtifacts.ArtifactType.FEATURE_SIGNING_CONFIG_VERSIONS,
          )
        )
      } else {
        packageAndroidArtifact.signingConfigVersions.from(creationConfig.artifacts.get(SIGNING_CONFIG_VERSIONS))
      }
      ...
      if (isDeterministic(creationConfig)) {
        packageAndroidArtifact.allInputFilesWithRelativePathSensitivity.from(
        ...
        packageAndroidArtifact.allClasspathInputFiles.from(
    ...
    companion object {
      ...
      fun getFeatureDexFolder(creationConfig: ApkCreationConfig, projectPath: String): FileCollection? {
        return if (!creationConfig.componentType.isDynamicFeature) {
          null
        } else
          creationConfig.variantDependencies.getArtifactFileCollection(
      ...
      private fun firstValidInjectedAbi(abis: String?): Set<String> {
        ...
        val firstValidAbi =
          Arrays.stream(abis.split(",".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
            .map { obj: String -> obj.trim { it <= ' ' } }
            .filter { o: String -> allowedAbis.contains(o) }
            .findFirst()
        return firstValidAbi.map<Set<String>> { element -> setOf(element) }.orElseGet { setOf() }
      }

      private fun getDesugarLibDexIfExists(creationConfig: ApkCreationConfig): FileCollection {
        return if (!creationConfig.dexing.shouldPackageDesugarLibDex) {
          creationConfig.services.fileCollection()
        } else creationConfig.services.fileCollection(creationConfig.artifacts.get(InternalArtifactType.DESUGAR_LIB_DEX))
      }

      private fun getGlobalSyntheticsDex(creationConfig: ApkCreationConfig): FileCollection {
        ...
        return if (
          !creationConfig.enableGlobalSynthetics ||
            (creationConfig.dexing.dexingType != DexingType.NATIVE_MULTIDEX) ||
            creationConfig.optimizationCreationConfig.minifiedEnabled
        ) {
          creationConfig.services.fileCollection()
        } else creationConfig.services.fileCollection(creationConfig.artifacts.get(InternalArtifactType.GLOBAL_SYNTHETICS_DEX))
      }
      ...
      private fun isDeterministicEntryOrder(creationConfig: ApkCreationConfig): Boolean {
        val projectOptions = creationConfig.services.projectOptions
        return (isDeterministic(creationConfig) || !projectOptions[BooleanOption.IDE_INVOKED_FROM_IDE])
      }
      ...
      private fun isDeterministic(creationConfig: ApkCreationConfig): Boolean {
        val projectOptions = creationConfig.services.projectOptions
        return (!creationConfig.debuggable || projectOptions[BooleanOption.FORCE_DETERMINISTIC_APK])
      }
    }
  }
  ...
  companion object {
    ...
    fun checkFileNameUniqueness(builtArtifacts: BuiltArtifactsImpl?) {
      ...
      val fileOutputs: Collection<File?> =
        builtArtifacts.elements.stream().map { (outputFile): BuiltArtifactImpl -> File(outputFile) }.collect(Collectors.toList())
      val repeatingFileNameOptional =
        fileOutputs
          .stream()
      ...
      if (repeatingFileNameOptional.isPresent) {
        ...
        val conflictingApks =
          builtArtifacts.elements
            .stream()
            .filter { (outputFile): BuiltArtifactImpl -> (File(outputFile).name == repeatingFileName) }
        ...
        throw RuntimeException(
          String.format(
            "Several variant outputs are configured to use " + "the same file name \"%1\$s\", filters : %2\$s",
            repeatingFileName,
            Joiner.on(":").join(conflictingApks),
          )
        )
      }
    }
    ...
    /**
     * Copy the input zip file (probably a Zip64) content into a new Zip in the destination folder stripping out all .class files.
     *
     * @param destinationFolder the destination folder to use, the output jar will have the same name as the input zip file.
    ...
    fun copyJavaResourcesOnly(destinationFolder: File?, zip64File: File): File {
      ...
      ZipFile(zip64File).use { inFile ->
        ZipOutputStream(BufferedOutputStream(FileOutputStream(copiedZip))).use { outFile ->
          val entries = inFile.entries()
          while (entries.hasMoreElements()) {
            val zipEntry = entries.nextElement()
            if (!zipEntry.name.endsWith(SdkConstants.DOT_CLASS) && isValidZipEntryName(zipEntry)) {
              outFile.putNextEntry(ZipEntry(zipEntry.name))
              try {
                ByteStreams.copy(BufferedInputStream(inFile.getInputStream(zipEntry)), outFile)
              } finally {
                outFile.closeEntry()
              }
            }
    ...
    private fun doTask(
      ...
      val (outputFile1) =
        params.outputHandler.get().extractArtifactForSplit(manifestOutputs, params.variantOutput.get().variantOutputConfiguration)
          ?: throw RuntimeException(
      ...
      val manifestData = parseManifest(File(outputFile1).readText(UTF_8), outputFile1, true, null, THROW_ON_ERROR_ISSUE_REPORTER)
      val nativeLibsPackagingMode = PackagingUtils.getNativeLibrariesLibrariesPackagingMode(manifestData.extractNativeLibs)
      ...
      val dexPackagingMode = PackagingUtils.getDexPackagingMode(useEmbeddedDex, params.dexUseLegacyPackaging.get())
      if (useEmbeddedDex != null) {
        ...
        if (params.dexUseLegacyPackaging.isPresent && params.dexUseLegacyPackaging.get() == useEmbeddedDex) {
          throw GradleException(
            "PackagingOptions.dex.useLegacyPackaging is set " +
          ...
          logger.warning(
            "$removalSuggestion. This should be replaced with " + "PackagingOptions.dex.useLegacyPackaging in the build script."
          )
        }
      }
      val dependencyData =
        if (params.dependencyDataFile.isPresent) Files.readAllBytes(params.dependencyDataFile.get().asFile.toPath()) else null
      IncrementalPackagerBuilder(params.packagerMode.get())
        .withOutputFile(outputFile)
        .withSigning(
          params.getSigningConfigData().get().resolve(),
          loadSigningConfigVersions(params.signingConfigVersionsFile.get().asFile),
        ...
        .withNoCompressPredicate(
          PackagingUtils.getNoCompressPredicate(params.aaptOptionsNoCompress.get(), nativeLibsPackagingMode, dexPackagingMode)
        )
        ...
        .build()
        .use { packager -> packager.updateFiles() }
      /*
       ...
       */ Stream.concat(
          ...
          Stream.concat(
            ...
            Stream.concat(changedAndroidResources.keys.stream(), changedNLibs.keys.stream()),
          ),
        )
    ...
    internal val THROW_ON_ERROR_ISSUE_REPORTER: IssueReporter =
    ...
    /**
     * Calculates the accepted ABIs based on the given [SplitterParams]. Also checks that the accepted ABIs are all available, and logs a
     * warning if not.
     *
        ...
        if (missingAbis.isNotEmpty()) {
          ...
          logger.warning(
            String.format(
              ...
              Joiner.on(", ").join(missingAbis.stream().sorted().collect(Collectors.toList())),
            )
          )
        }
      }
...
internal fun getPageSize(creationConfig: ApkCreationConfig): Provider<Long> {
  ...
  experimentalProperties =
    when (creationConfig) {
      ...
      is NestedComponentCreationConfig -> {
        (creationConfig as NestedComponentCreationConfig).mainVariant.experimentalProperties
      }
  ...
  return experimentalProperties.map {
    val pageSize = OptionalString.NATIVE_LIBRARY_PAGE_SIZE.getValue(it) ?: return@map defaultPageSize
    ...
    when (lowercasePageSize) {
      ...
      "64k" -> return@map ApkFlinger.PAGE_SIZE_64K
        ...
        throw java.lang.RuntimeException(
          "Invalid value for ${OptionalString.NATIVE_LIBRARY_PAGE_SIZE.key}. " + "Supported values are \"4k\", \"16k\", and \"64k\"."
        )
    }
  }
}
