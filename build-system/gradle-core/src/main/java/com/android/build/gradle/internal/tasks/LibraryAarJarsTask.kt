...
/**
 * A Task that takes the project/project local jars for CLASSES and RESOURCES, and processes and combines them, and put them in the bundle
 * folder.
 *
 * This creates a main jar with the classes from the main scope, and all the java resources, and a set of local jars (which are mostly the
 * same as before except without the resources). This is used to package the AAR.
 */
...
abstract class LibraryAarJarsTask : NonIncrementalTask() {
  @get:Nested @get:Optional abstract val dataBindingExcludeDelegate: Property<DataBindingExcludeDelegate>

  @get:Input abstract val namespace: Property<String>

  @get:Optional @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val typedefRecipe: RegularFileProperty

  @get:Classpath abstract val mainScopeClassFiles: ConfigurableFileCollection

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val javaResourcesJar: RegularFileProperty

  @get:Classpath abstract val localScopeInputFiles: ConfigurableFileCollection

  @get:OutputFile abstract val mainClassLocation: RegularFileProperty

  @get:OutputDirectory abstract val localJarsLocation: DirectoryProperty
  ...
  override fun doTaskAction() {
    ...
    mergeInputs(
      ...
      { archivePath: String -> excludePatterns.any { it.matcher(archivePath).matches() }.not() },
  ...
  private fun computeExcludeList(): List<Pattern> {
    ...
    dataBindingExcludeDelegate.orNull?.let { excludes.addAll(it.getExcludedClassList(namespaceValue)) }
  ...
  companion object {
    ...
    fun mergeInputs(
      ...
      // process main scope.
      mergeInputsToLocation(classFiles, resourceJar, toFile, filter, typedefRemover, compressionLevel)

      // process local scope
      processLocalJars(localJars, localJarsLocation, compressionLevel)
    }

    private fun processLocalJars(inputs: MutableSet<File>, localJarsLocation: File, compressionLevel: Int?) {
      ...
      if (dirInputs.isNotEmpty()) {
        JarFlinger(File(localJarsLocation, "otherclasses.jar").toPath(), JarFlinger.CLASSES_ONLY).use { jarCreator ->
      ...
      JarFlinger(toFile.toPath(), null).use { jarFlinger ->
        ...
        for (input in classFiles) {
          ...
          if (input.name.endsWith(SdkConstants.DOT_JAR)) {
            ...
            jarFlinger.addDirectory(input.toPath(), filterAndOnlyClasses, typedefRemover)
          }
        }
  ...
  class CreationAction(creationConfig: ComponentCreationConfig, private val minifyEnabled: Boolean) :
    VariantTaskCreationAction<LibraryAarJarsTask, ComponentCreationConfig>(creationConfig) {
    override val type = LibraryAarJarsTask::class.java
    override val name = computeTaskName("sync", "LibJars")

    override fun handleProvider(taskProvider: TaskProvider<LibraryAarJarsTask>) {
      super.handleProvider(taskProvider)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, LibraryAarJarsTask::mainClassLocation)
        .withName(SdkConstants.FN_CLASSES_JAR)
        .on(InternalArtifactType.AAR_MAIN_JAR)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, LibraryAarJarsTask::localJarsLocation)
    ...
    override fun configure(task: LibraryAarJarsTask) {
      ...
      artifacts.setTaskInputToFinalProduct(InternalArtifactType.ANNOTATIONS_TYPEDEF_FILE, task.typedefRecipe)
      ...
      task.javaResourcesJar.setDisallowChanges(creationConfig.artifacts.get(InternalArtifactType.MERGED_JAVA_RES))
      ...
      val disableMinifyLocalDepForLibraries =
      ...
      if (!minifyEnabled || disableMinifyLocalDepForLibraries) {
        task.localScopeInputFiles.from(
          creationConfig.artifacts.forScope(InternalScopedArtifacts.InternalScope.LOCAL_DEPS).getFinalArtifacts(ScopedArtifact.CLASSES)
        )
      }
