...
abstract class CheckAarMetadataTask : NonIncrementalTask() {
  ...
  @VisibleForTesting @get:Internal internal lateinit var aarMetadataArtifacts: ArtifactCollection
  ...
  @get:Input abstract val checkCoreLibraryDesugaring: Property<Boolean>
  ...
  @get:Input @get:Optional abstract val platformSdkExtension: Property<Int>
  ...
  @get:Input abstract val platformSdkApiLevel: Property<Int>
  ...
  override fun doTaskAction() {
    workerExecutor.noIsolation().submit(CheckAarMetadataWorkAction::class.java) { it ->
      it.aarMetadataArtifacts.addAll(
        aarMetadataArtifacts.artifacts.map { artifact -> AarMetadataArtifact(artifact.file, artifact.userFacingName) }
      )
      ...
      it.maxRecommendedStableCompileSdkVersionForThisAgp.set(maxRecommendedStableCompileSdkVersionForThisAgp)
...
private val ResolvedArtifactResult.userFacingName: String
  get() =
...
abstract class CheckAarMetadataWorkAction : WorkAction<CheckAarMetadataWorkParameters> {
  ...
  override fun execute() {
    ...
    parameters.aarMetadataArtifacts.get().forEach { checkAarMetadataArtifact(it, errorMessages) }
  ...
  private fun checkAarMetadataArtifact(aarMetadataArtifact: AarMetadataArtifact, errorMessages: MutableList<String>) {
    ...
    if (aarFormatVersion == null) {
      ...
      try {
        ...
        val maxMajorAarVersion = Revision.parseRevision(parameters.aarFormatVersion.get()).major
    ...
    if (aarMetadataVersion == null) {
      ...
      try {
        ...
        val maxMajorAarMetadataVersion = Revision.parseRevision(parameters.aarMetadataVersion.get()).major
    ...
    if (coreLibraryDesugaringEnabled != null && parameters.checkCoreLibraryDesugaring.get()) {
      if (coreLibraryDesugaringEnabled.toBoolean() && !parameters.coreLibraryDesugaringEnabled.get()) {
        errorMessages.add(
          """
      ...
      aarMetadataReader.desugarJdkLibId?.split(":")?.also {
        ...
        if (parameters.desugarJdkVariant.isPresent && parameters.desugarJdkVersion.isPresent) {
