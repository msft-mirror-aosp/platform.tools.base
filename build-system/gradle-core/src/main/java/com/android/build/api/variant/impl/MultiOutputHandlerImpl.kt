...
internal abstract class MultiOutputHandlerImplBase : MultiOutputHandler {
  ...
  override fun getOutputs(configFilter: (VariantOutputConfiguration) -> Boolean) = delegate.getOutputs(configFilter)

  override fun getMainSplitArtifact(artifactsDirectory: Provider<Directory>) = delegate.getMainSplitArtifact(artifactsDirectory)

  override fun computeBuildOutputFile(dir: File, output: VariantOutputImpl.SerializedForm) = delegate.computeBuildOutputFile(dir, output)

  override fun computeUniqueDirForSplit(dir: File, output: VariantOutputImpl.SerializedForm, variantName: String) =
...
internal class ApplicationMultiOutputHandler(creationConfig: ApplicationCreationConfig) : MultiOutputHandlerImplBase() {

  @get:Nested val variantOutputs = creationConfig.outputs.getEnabledVariantOutputs()

  override fun createSerializable(): MultiOutputHandler = SerializableApplicationMultiOutputHandler(this)
}

private class SerializableApplicationMultiOutputHandler(applicationOutputHandler: ApplicationMultiOutputHandler) :
  MultiOutputHandler, Serializable {

  @get:Nested val variantOutputs = applicationOutputHandler.variantOutputs.map { it.toSerializedForm() }

  private val mainSplit: VariantOutputImpl.SerializedForm
    get() =
      variantOutputs.find { it.variantOutputConfiguration.outputType == VariantOutputConfiguration.OutputType.SINGLE }
        ?: variantOutputs.find { it.variantOutputConfiguration.outputType == VariantOutputConfiguration.OutputType.UNIVERSAL }
        ?: variantOutputs.find { it.variantOutputConfiguration.outputType == VariantOutputConfiguration.OutputType.ONE_OF_MANY }
  ...
  override fun getOutputNameForSplit(
    ...
    val split =
      variantOutputs.find { it.variantOutputConfiguration.outputType == outputType && it.variantOutputConfiguration.filters == filters }
  ...
  override fun computeBuildOutputFile(dir: File, output: VariantOutputImpl.SerializedForm) = File(dir, output.outputFileName)
...
internal class SingleOutputHandler(creationConfig: ComponentCreationConfig) : MultiOutputHandlerImplBase() {
  ...
  override fun createSerializable(): MultiOutputHandler = SerializableSingleOutputHandler(this)
}

private class SerializableSingleOutputHandler(singleOutputHandler: SingleOutputHandler) : MultiOutputHandler, Serializable {

  @get:Input val singleOutputFileName: String = singleOutputHandler.singleOutputFileName.get()
  ...
  override fun extractArtifactForSplit(artifacts: BuiltArtifactsImpl, config: VariantOutputConfiguration): BuiltArtifactImpl? {
  ...
  private fun getMainSplitArtifact(artifacts: BuiltArtifactsImpl?) = artifacts?.getBuiltArtifact(dummyOutput.variantOutputConfiguration)
  ...
  override fun getOutput(config: VariantOutputConfiguration) = dummyOutput.takeIf { config == it.variantOutputConfiguration }
  ...
  override fun computeBuildOutputFile(dir: File, output: VariantOutputImpl.SerializedForm) = File(dir, singleOutputFileName)

  override fun computeUniqueDirForSplit(dir: File, output: VariantOutputImpl.SerializedForm, variantName: String) = File(dir, variantName)
