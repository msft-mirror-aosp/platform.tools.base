...
class LibraryTaskManager(
  ...
  override fun doCreateTasksForVariant(variantInfo: ComponentInfo<LibraryVariantBuilder, LibraryCreationConfig>) {
    ...
    taskFactory.register(ProcessLibraryManifest.CreationAction(libraryVariant, libraryVariant.targetSdk, libraryVariant.maxSdk))
    ...
    // b/37564303
    if (libraryVariant.services.projectOptions[BooleanOption.ENABLE_EXTRACT_ANNOTATIONS]) {
    ...
    taskFactory.register(BundleLibraryClassesJar.CreationAction(libraryVariant, PublishedConfigType.RUNTIME_ELEMENTS))
    ...
    // scope.
    taskFactory.register(ProjectOnlyCreationAction(libraryVariant, InternalArtifactType.LIBRARY_JNI))
    ...
    taskFactory.register(LibraryAarJarsTask.CreationAction(libraryVariant, libraryVariant.optimizationCreationConfig.minifiedEnabled))

    // now add a task that will take all the native libs and package
    // them into the libs folder of the bundle. This processes both the PROJECT
    // and the LOCAL_PROJECT scopes
    taskFactory.register(ProjectAndLocalJarsCreationAction(libraryVariant, InternalArtifactType.LIBRARY_AND_LOCAL_JARS_JNI))
    ...
    if (
      !libraryVariant.debuggable &&
        (ModulePropertyKey.OptionalBoolean.VERIFY_AAR_CLASSES.getValue(experimentalProperties.get())
  ...
  private fun createBundleTask(variant: LibraryCreationConfig) {
    ...
    if (publishInfo != null) {
      ...
      components.forEach { createComponent(variant, it.componentName, it.isClassifierRequired) }
    }
  }
  ...
  private fun createComponent(variant: LibraryCreationConfig, componentName: String, isClassifierRequired: Boolean) {
    ...
    val apiPub =
      variantDependencies.getElements(PublishedConfigSpec(PublishedConfigType.API_PUBLICATION, componentName, isClassifierRequired))
    val runtimePub =
      variantDependencies.getElements(PublishedConfigSpec(PublishedConfigType.RUNTIME_PUBLICATION, componentName, isClassifierRequired))
    val sourcePub =
    ...
    if (javaDocPub != null) {
      component.addVariantsFromConfiguration(javaDocPub, ConfigurationVariantMapping("runtime", true))
    }
  }

  private class MergeResourceCallback(private val variant: LibraryCreationConfig) : TaskProviderCallback<MergeResources> {
  ...
  private fun createMergeResourcesTasks(variant: LibraryCreationConfig) {
    val flags: ImmutableSet<MergeResources.Flag> = Sets.immutableEnumSet(MergeResources.Flag.PROCESS_VECTOR_DRAWABLES)
  ...
  override fun postJavacCreation(creationConfig: ComponentCreationConfig) {
    ...
    taskFactory.register(BundleLibraryClassesJar.CreationAction(creationConfig, PublishedConfigType.API_ELEMENTS))
  }
