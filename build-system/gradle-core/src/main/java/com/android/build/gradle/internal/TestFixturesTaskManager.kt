...
class TestFixturesTaskManager(project: Project, globalConfig: GlobalTaskCreationConfig, private val localConfig: TaskManagerConfig) :
  ...
  fun createTasks(testFixturesComponent: TestFixturesCreationConfig) {
    ...
    if (testFixturesComponent.buildFeatures.androidResources) {
      ...
      val flags: Set<MergeResources.Flag> = Sets.immutableEnumSet(MergeResources.Flag.PROCESS_VECTOR_DRAWABLES)
      ...
      createMergeResourcesTask(testFixturesComponent, processResources = false, flags = ImmutableSet.of())
      ...
      taskFactory.register(ProcessNavigationXmlTask.LibraryCreationAction(testFixturesComponent))
    ...
    taskFactory.register(ProcessLibraryManifest.CreationAction(testFixturesComponent, targetSdkVersion = null, maxSdkVersion = null))
    ...
    // b/37564303
    if (testFixturesComponent.services.projectOptions.get(BooleanOption.ENABLE_EXTRACT_ANNOTATIONS)) {
    ...
    taskFactory.register(BundleLibraryClassesJar.CreationAction(testFixturesComponent, AndroidArtifacts.PublishedConfigType.API_ELEMENTS))
    taskFactory.register(
      BundleLibraryClassesJar.CreationAction(testFixturesComponent, AndroidArtifacts.PublishedConfigType.RUNTIME_ELEMENTS)
    )
    ...
    taskFactory.register(LibraryAarJarsTask.CreationAction(testFixturesComponent, minifyEnabled = false))
    ...
    if (
      globalConfig.avoidTaskRegistration.not() &&
        ...
        testFixturesComponent.mainVariant.enableLint
    ) {
      taskFactory.register(AndroidLintAnalysisTask.PerComponentCreationAction(testFixturesComponent, fatalOnly = false))
      taskFactory.register(
    ...
    testFixturesComponent.taskContainer.assembleTask.configure { task: Task -> task.group = null }
  }
  ...
  private fun createBundleTaskForTestFixtures(testFixturesComponent: TestFixturesCreationConfig) {
    ...
    testFixturesComponent.taskContainer.assembleTask.configure { task: Task ->
      task.dependsOn(testFixturesComponent.artifacts.get(SingleArtifact.AAR))
    }
    ...
    testFixturesComponent.publishInfo?.components?.forEach {
      ...
      val component =
        project.components.findByName(componentName) as AdhocComponentWithVariants?
