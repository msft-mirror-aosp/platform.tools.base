 ...
 *
 * @param symbolTables a list of symbol tables for the current module and its' dependencies. The order matters, the closest modules are at
 *   the front, the furthest are at the end. The first symbol table should be for the module from which the transformed classes come from.
 * @param onlyRewriteReferencesInFirstSymbolTable When enabled. if the first symbol table does not contain a resource, then its reference
 *   will not be re-written to the tablePackage of the first symbol table. This is a requirement for Fused Library see b/426351548.
 */
class NamespaceRewriter(
  ...
  private val referenceRewriter = NamespaceReferenceRewriter(localPackage, this::findPackage)
  ...
  private fun rewriteClass(originalClass: ByteArray): ByteArray {
    ...
    val crw = ClassReWriter(ASM_API_VERSION, cw)
  ...
  /**
   * Rewrites all classes from the input JAR file to be fully resource namespace aware and places them in the output JAR; it will also
   * filter out all non .class files, so that the output JAR contains only the namespaced classes.
   *
  ...
  /** Rewrites the input file to be fully namespaced using the provided method. Writes fully namespaced document to the output. */
  private inline fun rewriteFile(input: InputStream, output: OutputStream, displayInput: Any, method: (node: Document) -> Unit) {
    // Read the file.
    val doc =
      try {
  ...
  /**
   * Rewrites the AndroidManifest.xml file to be fully resource namespace aware. Finds all resource references (e.g. '@string/app_name') and
   * makes them namespace aware (e.g. '@com.foo.bar:string/app_name'). This will also append the package to the references to resources from
   * this library - it is not necessary, but saves us from comparing the package names.
   */
  fun rewriteManifest(inputManifest: InputStream, outputManifest: OutputStream, displayInput: Any) {
    rewriteFile(inputManifest, outputManifest, displayInput, referenceRewriter::rewriteManifestNode)
  }

  /**
   * Rewrites a values file to be fully resource namespace aware. Finds all resource references and makes them namespace aware, for example:
   * - simple references, e.g. '@string/app_name' becomes '@com.foo.bar:string/app_name'
   * - styles' parents, e.g. 'parent="@style/Parent', 'parent="Parent"' both become 'parent="@com.foo.bar:style/Parent"'
   * - styles' item name references, e.g. 'item name="my_attr"' becomes 'item name="com.foo.bar:my_attr" (no "@" or "attr/" in this case)
   *   This will also append the package to the references to resources from this library - it is not necessary, but saves us from comparing
   *   the package names.
   */
  ...
  private fun rewriteValuesNode(node: Node) {
    ...
    node.attributes?.forEach { rewriteValuesNode(it) }

    // Now fix the children.
    node.childNodes?.forEach { rewriteValuesNode(it) }
  }
  ...
  private fun rewriteParent(element: Element, type: String) {
    ...
    if (originalParent == null) {
      ...
      if (!possibleParent.isEmpty()) {
        val possiblePackage = maybeFindPackage(type = type, name = possibleParent, reversed = false)
  ...
  private fun rewriteStyleableElement(element: Element) {
    ...
    element.childNodes.forEach { child ->
      if (child.nodeType == Node.ELEMENT_NODE && (child.nodeName == "attr" || child.nodeName == "item")) {
        child as Element
  ...
  /**
   * Rewrites an XML file (e.g. layout) to be fully resource namespace aware. Finds all resource references and makes them namespace aware,
   * for example:
   * - simple references, e.g. '@string/app_name' becomes '@com.foo.bar:string/app_name'
   ...
   * - updates XML namespaces from to the correct package, e.g app:layout_constraintLeft_toLeftOf becomes
   *   android_support_constraint:layout_constraintLeft_toLeftOf
   * - removes res-auto namespace to make sure we don't leave anything up to luck This will also append the package to the references to
   *   resources from this library - it is not necessary, but saves us from comparing the package names.
   */
  ...
  /**
   * Rewrites all the resources from an exploded-aar input directory as passed in input to the output directory.
  ...
  private fun rewriteXmlDoc(document: Document) {
    ...
    // Now fix the children.
    mainNode.childNodes?.forEach { rewriteXmlNode(it, document, namespacesToFix, usedNamespaces) }

    // Finally add the used namespaces.
    for ((pckg, namespace) in usedNamespaces.toSortedMap()) {
      mainNode.setAttribute("xmlns:${namespace.replace('.', '_')}", "http://schemas.android.com/apk/res/$pckg")
    }
  }

  /** Goes through all nodes of the document and removes "res-auto" namespaces, while collecting their XML names. */
  private fun collectAndRemoveXmlNamespaces(node: Element, namespacesToFix: HashSet<String>) {
    node.attributes?.forEach {
      if (it.nodeName.startsWith("xmlns:") && it.nodeValue == "http://schemas.android.com/apk/res-auto") {
    ...
    node.childNodes?.forEach { if (it is Element) collectAndRemoveXmlNamespaces(it, namespacesToFix) }
  }

  /** Resource XML files should have only one main element everything else should be whitespace and comments */
  ...
  private fun rewriteXmlNode(node: Node, document: Document, namespacesToFix: HashSet<String>, usedNamespaces: HashMap<String, String>) {
    if (node.nodeType == Node.TEXT_NODE) {
      // The content could be a resource reference. If it is not, do not update the content.
      val content = node.nodeValue
      val (namespacedContent, _) = referenceRewriter.rewritePossibleReference(content)
      ...
      if (namespacesToFix.any { node.nodeName.startsWith("$it:") }) {
        ...
        val (namespacedContent, foundPackage) = referenceRewriter.rewritePossibleReference(content, true)
        if (content != namespacedContent) {
          usedNamespaces.computeIfAbsent(foundPackage, { "ns${usedNamespaces.size}" })
          document.renameNode(node, "http://schemas.android/apk/res/$foundPackage", "${usedNamespaces[foundPackage]!!}:$name")
        }
      }
    }

    // First fix the attributes.
    node.attributes?.forEach { rewriteXmlNode(it, document, namespacesToFix, usedNamespaces) }

    // Now fix the children.
    node.childNodes?.forEach { rewriteXmlNode(it, document, namespacesToFix, usedNamespaces) }
  }

  /** Writes the bytecode of the R class to the specified output JAR file, making sure all resources are properly namespaced. */
  fun writeRClass(rClassPath: Path) {
    ...
    val fixedTable = SymbolTable.builder().tablePackage(currentTable.tablePackage)
    ...
    for (type in ResourceType.values()) {
      if (type != ResourceType.STYLEABLE) {
        ...
        namespaceStyleables(currentTable.getSymbolByResourceType(ResourceType.STYLEABLE), fixedTable)
      }
    }
  ...
  fun namespaceStyleables(styleables: List<Symbol>, fixedTable: SymbolTable.Builder) {
    ...
    styleables.forEach { symbol ->
      if (symbol.children.isEmpty()) {
        ...
        val newStyleable = Symbol.createStyleableSymbol(symbol.name, symbol.values, newChildren.build())
  ...
  /**
   * Rewrites a class to be namespaced. It removes all R inner classes references from read inner classes, rewrites and collects R
   * references using the [MethodReWriter]. After an [ClassReader.accept] method is called using this [ClassVisitor], the method
   * [ClassReWriter.writeInnerRClasses] needs to be called to correctly fill the InnerClasses attribute for the transformed class.
   */
  private inner class ClassReWriter internal constructor(api: Int, cv: ClassVisitor?) : ClassVisitor(api, cv) {
    ...
    override fun visitMethod(access: Int, name: String?, desc: String?, signature: String?, exceptions: Array<String?>?): MethodVisitor {
      return MethodReWriter(api, cv.visitMethod(access, name, desc, signature, exceptions), this)
    }

    /**
     * References all found inner R classes. According to the JVM specification the InnerClasses attribute must contain all inner classes
     * referenced in the transformed class even if they aren't member of this class.
     */
    fun writeInnerRClasses() {

      for (innerClass in innerClasses) {
        cv.visitInnerClass(innerClass, innerClass.substringBeforeLast('$'), innerClass.substringAfterLast('$'), ACC_PUBLIC + ACC_STATIC)
      }
    }

    /**
     * Finds the parent Symbol and its package for the given declare-styleable attribute name. This will search through all SymbolTables
     * trying to find a matching parent, and once the parent is found, it will return it and the package it was found in. If the parent is
     * not found, this method will raise an error.
     */
    internal fun findStyleableChildSymbol(name: String): Pair<Symbol.StyleableSymbol, String> {
      return findStyleableChild(name)
    }

    /** Finds the first package in which the R file contains a symbol with the given type and name. */
  ...
  private inner class MethodReWriter(api: Int, mv: MethodVisitor?, private val crw: ClassReWriter) : MethodVisitor(api, mv) {

    override fun visitFieldInsn(opcode: Int, owner: String, name: String, desc: String?) {
      val type = owner.substringAfterLast('$')
      val isRClassRef = owner.contains("/R$")
      val rewriteReference =
        isRClassRef && (!onlyRewriteReferencesInFirstSymbolTable || symbolTables.first().containsSymbol(getResourceType(type), name))
      if (rewriteReference) {
  ...
  fun findPackage(type: String, name: String): String {
    ...
    return maybeFindPackage(type = type, name = name, reversed = false)
      ?: error("In package $localPackage found unknown symbol of type " + "$type and name $name.")
  }

  /**
   * Finds the first package in which the attribute was defined. If none match (the attribute was not defined explicitly), we need to search
   * in reverse order to find the last/furthest "maybe" definition of the attribute (defined under a declare-styleable, kept in the symbol
   * table as the MAYBE_ATTR type).
   */
  private fun findPackageForAttr(name: String): String {
    ...
    return foundPackage ?: error("In package $localPackage found unknown attribute $name")
  }
  ...
  private fun packageContainsSymbol(table: SymbolTable, type: String, canonicalName: String): Boolean {
  ...
  /**
   * Generates a public.xml file containing public definitions for the resources from the current package.
   *
   ...
   */
  fun generatePublicFile(publicTxt: InputStream?, outputDirectory: Path): Path {
    ...
    if (Files.exists(publicXml)) {
      error("Internal error: Auto namespaced public XML file already exists: " + { publicXml.toAbsolutePath() })
    }
    ...
    val publicSymbols: SymbolTable = if (publicTxt != null) SymbolIo.readFromPublicTxtFile(publicTxt, "", symbols.tablePackage) else symbols
...
private fun getResourceType(typeString: String): ResourceType =
  if (typeString == "attr?") ResourceType.ATTR else ResourceType.fromClassName(typeString) ?: error("Unknown type '$typeString'")
