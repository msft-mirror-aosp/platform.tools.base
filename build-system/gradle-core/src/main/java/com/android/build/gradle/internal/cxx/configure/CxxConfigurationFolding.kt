 ...
 *
 * For CMake, this is the list of arguments passed to CMake.exe when the Ninja project is generated.
 *
 * For ndk-build, this is the list of arguments passed along with -nB to do a dry run of the build
 */
...
/**
 * Creates unique and human-readable Gradle task names for a group of ABIs where identical The names should be unique per C/C++
 * configuration and, in the case of build task, build targets.
 */
class CxxConfigurationFolding(abis: List<CxxAbiModel>) {
   ...
   *
   * Key: A [Configuration]. Value: The name of the task for this configuration.
   */
   ...
   *
   * Key: A [Configuration] annotation name combined with the targets built with this task
   *
  ...
  /**
   * Configuration task name to ABI. Key: The name of the configuration task (like 'configureCMakeDebug[x86]') Value: The ABI to configure
   * when this task is run along with the names of variants covered.
   */
  val configureAbis = mutableMapOf<TaskName, Pair<List<VariantName>, CxxAbiModel>>()

  /**
   * Configure group task name to individual per-ABI configure tasks. Key: The name of a configuration group task (like
   * 'configureCMakeDebug') Value: The names of the per-ABI configure tasks (like 'configureCMakeDebug[x86]')
   */
  val configureGroups = mutableMapOf<VariantName, MutableSet<TaskName>>()

  /**
   * Build task name to ABI. Key: The name of the build task (like 'buildCMakeDebug') Value: The ABI to build when this task is run along
   * with the names of variants covered.
   */
  val buildAbis = mutableMapOf<TaskName, Pair<List<VariantName>, CxxAbiModel>>()

  /**
   * Build group task name to individual per-ABI build tasks. Key: The name of a configuration group task (like 'buildCMakeDebug') Value:
   * The names of the per-ABI configure tasks (like 'buildCMakeDebug[x86]')
   */
  val buildGroups = mutableMapOf<VariantName, MutableSet<TaskName>>()

  /** The configuration tasks for each variant. Key: Name of the variant. Value: The names of configuration tasks for this variant. */
  val variantToConfiguration = mutableMapOf<VariantName, MutableSet<TaskName>>()

  /** The build tasks for each variant. Key: Name of the variant. Value: The names of build tasks for this variant. */
  val variantToBuild = mutableMapOf<VariantName, MutableSet<TaskName>>()

  /** Dependencies from build tasks to configure tasks. First: Name of a build task. Second: Name of a configure task. */
  ...
  private fun createConfigurationTask(abi: CxxAbiModel): TaskName {
    ...
    configureGroups.computeIfAbsent(groupTaskName) { mutableSetOf() }.add(configureTaskName)
    variantToConfiguration.computeIfAbsent(abi.variant.variantName) { mutableSetOf() }.add(configureTaskName)
  ...
  private fun createBuildTask(abi: CxxAbiModel, configureTaskName: TaskName) {
    val (groupTaskName, buildTaskName) = buildTaskNameOf(abi.configurationArguments, abi.variant.buildTargetSet)
    buildAbis.setRepresentativeAbiForTask(buildTaskName, abi)
    buildConfigureEdges += buildTaskName to configureTaskName
    buildGroups.computeIfAbsent(groupTaskName) { mutableSetOf() }.add(buildTaskName)
    variantToBuild.computeIfAbsent(abi.variant.variantName) { mutableSetOf() }.add(buildTaskName)
  }
  ...
  /**
   * Create some text that can be appended to a task name to distinguish which targets are built. When there are more than two targets use
   * 'etc' to avoid very long task names. Uniqueness of the overall task name is still guaranteed.
   */
  ...
  /**
   * Create a configure task name based on the content of the configuration. Returns a pair of [TaskName] where the first is a group task
   * name for all ABIs (like configureCMakeDebug) and the second is a task name for the specific ABI (like configureCMakeDebug[x86]).
   */
  private fun configureTaskNameOf(configuration: Configuration): Pair<TaskName, TaskName> {
    ...
    return groupTaskName to
      configurers.computeIfAbsent(configuration) {
  ...
  /**
   * Create a build task name based on the content of the configuration and list of targets. Returns a pair of [TaskName] where the first is
   * a group task name for all ABIs (like buildCMakeDebug[target1, target2]) and the second is a task name for the specific ABI (like
   * configureCMakeDebug[x86][target1, target2]).
   */
  private fun buildTaskNameOf(configuration: Configuration, targets: Targets): Pair<TaskName, TaskName> {
    val annotation = taskNameAnnotation(configuration)
    val groupTaskName = "build$annotation"
    return groupTaskName to
      builders.computeIfAbsent(configuration to targets) {
        val abi = abiOf(buildSystem, configuration)
  ...
  /**
   * Utility method that sets a representative ABI for the given task. If there is already an ABI then this function verifies that the new
   * ABI is accurately represented by the prior ABI.
   */
  private fun MutableMap<TaskName, Pair<List<VariantName>, CxxAbiModel>>.setRepresentativeAbiForTask(taskName: TaskName, abi: CxxAbiModel) {
    val map = this
...
fun abiOf(buildSystem: NativeBuildSystem, commands: Configuration): String {
  return when (buildSystem) {
    NINJA -> {
      ...
      arguments.getMSBuildProperty(Platform)
        ?: run {
          val asCMakeArguments = commands.toCmakeArguments()
          asCMakeArguments.getCmakeProperty(CMAKE_ANDROID_ARCH_ABI) ?: asCMakeArguments.getCmakeProperty(ANDROID_ABI)
        }
        ?: run {
          bugln(
...
fun buildTypeOf(buildSystem: NativeBuildSystem, commands: Configuration): String {
  when (buildSystem) {
    NINJA -> {
      ...
      val arguments = commands.toMSBuildArguments()
      var result = ""
      arguments.getMSBuildProperty(MSBuildProperty.Configuration)?.let { result = result.appendCapitalized(it) }
      arguments.getMSBuildProperty(MSBuildProperty.NinjaProject)?.let { result = result.appendCapitalized(it) }
      if (result != "") {
        return result
      }
      val asCMakeArguments = commands.toCmakeArguments()
      return asCMakeArguments.getCmakeProperty(CMAKE_BUILD_TYPE)
        ?: run {
          bugln(
            NINJA_BUILD_SCRIPT_AUTHOR_FEEDBACK,
            "Ninja-generating script must accept a parameter the defines the " +
