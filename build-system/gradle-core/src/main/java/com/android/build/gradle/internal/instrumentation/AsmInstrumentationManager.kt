...
/**
 * Instruments the given classes with the given list of [visitors]. Frames are skipped and not regenerated after visiting each class and
 * should be regenerated separately if needed.
 *
 * @param visitors the list of registered [AsmClassVisitorFactoryEntry].
 * @param apiVersion the asm api version.
 * @param classesHierarchyResolver used to derive information about classes hierarchy without having to load the actual classes.
 * @param framesComputationMode the frame computation mode that will be applied to the bytecode of the instrumented classes.
 * @param excludes the set of patterns to exclude from instrumentation
 * @param profilingTransforms list of paths to the profiler jars injected by the IDE to transform classes.
 */
class AsmInstrumentationManager(
  ...
  private fun getClassReaderFlags(containsJsrOrRetInstruction: Boolean): Int {
    ...
    return when (framesComputationMode) {
      FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_CLASSES,
      FramesComputationMode.COMPUTE_FRAMES_FOR_ALL_CLASSES -> ClassReader.SKIP_FRAMES
  ...
  private fun includeFileInInstrumentation(relativePath: String): Boolean {
    val pathWithoutExtension by lazy { Paths.get(FileUtils.toSystemIndependentPath(relativePath).removeSuffix(DOT_CLASS)) }
    return CLASS_MATCHER.test(relativePath) && excludesMatchers.none { it.matches(pathWithoutExtension) }
  }
        ...
        override fun visitFile(file: Path, attrs: BasicFileAttributes): FileVisitResult {
          ...
          if (includeFileInInstrumentation(relativePath.toString())) {
            instrumentClassToDir(
              packageName = relativePath.toString().removeSuffix(File.separatorChar + fileName).replace(File.separatorChar, '.'),
    ...
    ZipOutputStream(BufferedOutputStream(FileOutputStream(outputJarFile))).use { outputJar ->
      ...
      ZipFile(inputJarFile).use { inputJar ->
        ...
        while (entries.hasMoreElements()) {
          ...
          instrumentClassToJar(entry, outputJar) { inputJar.getInputStream(entry) }
        }
      }
    }
  }
  ...
  private fun performProfilingTransformations(inputStream: InputStream): ByteArray {
    ...
    ByteArrayOutputStream().use { outputStream ->
      profilingTransforms.forEach { transform ->
        ByteArrayInputStream(bytes).use { inputStream -> transform.accept(inputStream, outputStream) }
  ...
  private fun doInstrumentByteCode(
    ...
    val classWriter =
      FixFramesClassWriter(classReader, getClassWriterFlags(containsJsrOrRetInstruction), classesHierarchyResolver, issueHandler)
    ...
    visitors.forEach { entry -> nextVisitor = entry.createClassVisitor(classContext, nextVisitor) }
  ...
  private fun doInstrumentClass(packageName: String, className: String, classInputStream: () -> InputStream): ByteArray? {
    ...
    return when {
      filteredVisitors.isNotEmpty() -> {
        classInputStream.invoke().use {
          ...
          try {
            ...
            if (e.message == "JSR/RET are not supported with computeFrames option" && javaVersion == Opcodes.V1_6) {
              try {
      ...
      profilingTransforms.isNotEmpty() -> {
        classInputStream.invoke().use { performProfilingTransformations(it) }
      }
      else -> {
        null
      }
  ...
  private fun saveEntryToJar(entryName: String, byteArray: ByteArray, jarOutputStream: ZipOutputStream) {
  ...
  private fun instrumentClassToJar(entry: ZipEntry, jarOutputStream: ZipOutputStream, classInputStream: () -> InputStream) {
    val entryName = entry.name
    if (!includeFileInInstrumentation(entryName)) {
      classInputStream.invoke().use { saveEntryToJar(entryName, ByteStreams.toByteArray(it), jarOutputStream) }
      return
    }
    ...
    var instrumentedByteArray = doInstrumentClass(packageName = packageName, className = className, classInputStream = classInputStream)
    if (instrumentedByteArray == null) {
      classInputStream.invoke().use { instrumentedByteArray = ByteStreams.toByteArray(it) }
    }
    saveEntryToJar(entryName, instrumentedByteArray!!, jarOutputStream)
  }

  private fun instrumentClassToDir(packageName: String, className: String, classFile: File, outputFile: File) {
    val instrumentedByteArray = doInstrumentClass(packageName, className) { classFile.inputStream().buffered() }
    if (instrumentedByteArray != null) {
      FileOutputStream(outputFile).use { outputStream -> outputStream.write(instrumentedByteArray) }
    } else {
   ...
   *
   * Because the flag [ClassReader.SKIP_FRAMES] doesn't skip the maxs, [ClassWriter] will recalculate all frames from scratch, but will only
   * recalculate the maxs of the visited methods. This class visitor visits all methods in order to force [ClassWriter] to calculate the
   * maxs for all methods when the frame computation mode is [FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_CLASSES].
   */
  private class MaxsInvalidatingClassVisitor(apiVersion: Int, classVisitor: ClassVisitor) : ClassVisitor(apiVersion, classVisitor) {
  ...
  private class MethodVisitorDelegator(apiVersion: Int, methodVisitor: MethodVisitor) : MethodVisitor(apiVersion, methodVisitor)
}
