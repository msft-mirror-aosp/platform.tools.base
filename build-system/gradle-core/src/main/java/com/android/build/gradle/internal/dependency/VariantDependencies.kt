 ...
 *
 * The dependencies are expressed as composite Gradle configuration objects that extends all the configuration objects of the "configs".
 *
...
internal constructor(
  ...
  init {
    check(!componentType.isTestComponent || testedVariant != null) { "testedVariantDependencies null for test component" }
  }
  ...
  override fun getAdditionalArtifacts(configType: ConsumedConfigType, type: AdditionalArtifactType): ArtifactCollection {
    val configuration =
      when (configType) {
    ...
    return configuration.incoming
      .artifactView { view ->
        ...
        view.attributes.apply {
          buildType?.let { attribute(BuildTypeAttr.ATTRIBUTE, buildType) }
          flavorMap.entries.forEach { attribute(it.key, it.value) }
  ...
  ): FileCollection = getArtifactCollection(configType, scope, artifactType, attributes).artifactFiles
  ...
  fun getArtifactCollection(
    ...
    var artifacts = computeArtifactCollection(configType, scope, artifactType, attributes)

    if (configType == ConsumedConfigType.RUNTIME_CLASSPATH && isArtifactTypeExcluded(artifactType)) {
      val excludedDirectories = computePackagedDependencies()
      artifacts = FilteredArtifactCollection(FilteringSpec(artifacts, excludedDirectories, project.objects))
    }
    ...
    if (testedVariant.componentType.isDynamicFeature) {
      ...
      val excludedDirectories =
        testedVariant.variantDependencies
          .computeArtifactCollection(ConsumedConfigType.PROVIDED_CLASSPATH, ArtifactScope.PROJECT, PACKAGED_DEPENDENCIES, null)
          .artifactFiles
      artifacts = FilteredArtifactCollection(FilteringSpec(artifacts, excludedDirectories, project.objects))
    }

    val testedArtifactCollection = testedVariant.variantDependencies.getArtifactCollection(configType, scope, artifactType, attributes)
    artifacts = SubtractingArtifactCollection(artifacts, testedArtifactCollection, project.objects)
    return artifacts
  }
  ...
  private fun isArtifactTypeExcluded(artifactType: AndroidArtifacts.ArtifactType): Boolean {
    return when {
      ...
      componentType.isSeparateTestProject -> isArtifactTypeSubtractedForInstrumentationTests(artifactType)
      else -> false
    }
  }
  ...
  private fun getConfiguration(configType: ConsumedConfigType): Configuration {
    return when (configType) {
      ...
      ConsumedConfigType.REVERSE_METADATA_VALUES ->
        checkNotNull(reverseMetadataValuesConfiguration) { "reverseMetadataValuesConfiguration is null" }
    }
  }
  ...
  private fun computeArtifactCollection(
    ...
    val attributesAction = Action { container: AttributeContainer ->
      ...
      artifactType.getAttributes { type, name -> project.objects.named(type, name) }.addAttributesToContainer(container)
      attributes?.addAttributesToContainer(container)
    }
    val filter = getComponentFilter(scope, additionalFilter)
    val lenientMode = projectOptions[BooleanOption.IDE_BUILD_MODEL_ONLY] || projectOptions[BooleanOption.IDE_BUILD_MODEL_ONLY_V2]
  ...
  fun computeLocalFileDependencies(services: VariantServices, filePredicate: Predicate<File>): FileCollection {
    val excludeDirectories =
      if (componentType.isDynamicFeature) {
        computePackagedDependencies()
      } else {
        null
      }
    return computeLocalFileDependencies(runtimeClasspath, services::fileCollection, filePredicate, excludeDirectories)
  }

  private fun computePackagedDependencies(): FileCollection =
    computeArtifactCollection(ConsumedConfigType.PROVIDED_CLASSPATH, ArtifactScope.PROJECT, PACKAGED_DEPENDENCIES, null).artifactFiles
  ...
  companion object {
    ...
    @Deprecated("") const val CONFIG_NAME_FEATURE = "feature"
        ...
        fileCollectionBuilder(
            Callable {
              excludeDirectories.elements.map { excludedDirectoriesSet ->
                val excludedDirectoriesContent =
                  excludedDirectoriesSet
                    .asSequence()
                ...
                dependencies
                  .call()
                  .flatMap { it.files }
                  .filter { fileFilterPredicate.test(it) && !excludedDirectoriesContent.contains(it.absolutePath) }
              }
            }
          )
          .builtBy(dependencies)
          .builtBy(excludeDirectories.buildDependencies)
      } else {
        fileCollectionBuilder(Callable { dependencies.call().flatMap { it.files }.filter { fileFilterPredicate.test(it) } })
          .builtBy(dependencies)
      }
    }
    ...
    fun createForKotlinMultiplatform(
      ...
      project.objects.named(TargetJvmEnvironment::class.java, TargetJvmEnvironment.ANDROID).let { androidTarget ->
        (incomingConfigurations + outgoingConfigurations + publicationConfigurations).forEach {
          it.attributes.attribute(TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE, androidTarget)
        }
      }

      project.objects.named(AgpVersionAttr::class.java, Version.ANDROID_GRADLE_PLUGIN_VERSION).let { agpVersion ->
        // Add the agp version attribute to all configurations but the publication
        (incomingConfigurations + outgoingConfigurations).forEach { it.attributes.attribute(AgpVersionAttr.ATTRIBUTE, agpVersion) }
      }

      listOfNotNull(apiElements, apiPublication).forEach { it.extendsFrom(apiConfiguration) }
      ...
      apiPublication?.attributes?.apply {
        ...
        attribute(LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE, aarLibraryElements)
      }
      ...
      runtimeElements?.let { elements[PublishedConfigSpec(PublishedConfigType.RUNTIME_ELEMENTS)] = runtimeElements }
      apiElements?.let { elements[PublishedConfigSpec(PublishedConfigType.API_ELEMENTS)] = apiElements }

      apiPublication?.let {
        elements[PublishedConfigSpec(PublishedConfigType.API_PUBLICATION, dslInfo.componentIdentity.name)] = apiPublication
      }
      runtimePublication?.let {
        elements[PublishedConfigSpec(PublishedConfigType.RUNTIME_PUBLICATION, dslInfo.componentIdentity.name)] = runtimePublication
      }
    ...
    private fun isArtifactTypeSubtractedForInstrumentationTests(artifactType: AndroidArtifacts.ArtifactType): Boolean {
    ...
    private fun checkComputeArtifactCollectionArguments(
      ...
      when (artifactType) {
        PACKAGED_DEPENDENCIES ->
          check(configType == ConsumedConfigType.PROVIDED_CLASSPATH || configType == ConsumedConfigType.REVERSE_METADATA_VALUES) {
    ...
    private fun getComponentFilter(
      ...
      if (scope != ArtifactScope.ALL) {
        check(additionalFilter == null) { "Additional filter is only respected when scope is ALL" }
      }
      return when (scope) {
        ...
        ArtifactScope.FILE -> Spec { !(it is ProjectComponentIdentifier || it is ModuleComponentIdentifier) }
      }
    }
  }
}
