...
abstract class RenderscriptCompile : NdkTask() {
  ...
  @get:OutputDirectory abstract val resOutputDir: DirectoryProperty

  @get:OutputDirectory lateinit var objOutputDir: Provider<Directory>

  // ----- PRIVATE TASK API -----

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) lateinit var importDirs: FileCollection

  @get:Input abstract val targetApi: Property<Int>

  @get:Input abstract val supportMode: Property<Boolean>

  @get:Input
  var useAndroidX: Boolean = false
    private set

  @get:Input abstract val optimLevel: Property<Int>

  @get:Input abstract val ndkMode: Property<Boolean>

  @Input fun getBuildToolsVersion(): String = buildToolsRevision.get().toString()
  ...
  @get:OutputDirectory abstract val libOutputDir: DirectoryProperty
  ...
  override fun doTaskAction() {
    ...
    val buildToolsInfo =
      sdkBuildService
        .get()
        .sdkLoader(compileSdkVersion = compileSdkVersion, buildToolsRevision = buildToolsRevision)
        .buildToolInfoProvider
        .get()

    if (!buildToolsInfo.containsRenderscript()) {
      throw IllegalStateException(
        "Build tools Revision '${buildToolsInfo.revision}' does not support Renderscript. Select an earlier version of Build Tools if you need Renderscript."
      )
    }
  ...
  /**
   * Does a check on whether the given BuildToolInfo has renderscript binaries and folders. This is not as efficient as checking the removal
   * dates on the PathId enums but because we can't predict the future (ie exactly when things will be removed), this works well enough and
   * is more lenient.
   */
   ...
   *
   * Right now this is the only way to compile them as the renderscript compiler requires all renderscript files to be passed for all
   * compilation.
   *
  ...
  class CreationAction(creationConfig: ConsumableCreationConfig, private val ndkConfig: CoreNdkOptions) :
    VariantTaskCreationAction<RenderscriptCompile, ConsumableCreationConfig>(creationConfig),
    ...
    override fun handleProvider(taskProvider: TaskProvider<RenderscriptCompile>) {
      super.handleProvider(taskProvider)
      creationConfig.taskContainer.renderscriptCompileTask = taskProvider
      creationConfig.artifacts
        .setInitialProvider(taskProvider, RenderscriptCompile::sourceOutputDir)
        .withName("out")
        .on(RENDERSCRIPT_SOURCE_OUTPUT_DIR)

      creationConfig.artifacts.setInitialProvider(taskProvider, RenderscriptCompile::libOutputDir).withName("lib").on(RENDERSCRIPT_LIB)

      creationConfig.artifacts
        .setInitialProvider(taskProvider, RenderscriptCompile::resOutputDir)
    ...
    override fun configure(task: RenderscriptCompile) {
      ...
      task.supportMode.setDisallowChanges(renderscriptCreationConfig.renderscript.supportModeEnabled)
      ...
      task.sourceDirs =
        creationConfig.services.fileCollection().also { fileCollection ->
          creationConfig.sources.renderscript { fileCollection.from(it.all) }
        }

      task.importDirs = creationConfig.variantDependencies.getArtifactFileCollection(COMPILE_CLASSPATH, ALL, RENDERSCRIPT)
      ...
      task.sdkBuildService.setDisallowChanges(getBuildService(creationConfig.services.buildServiceRegistry))
