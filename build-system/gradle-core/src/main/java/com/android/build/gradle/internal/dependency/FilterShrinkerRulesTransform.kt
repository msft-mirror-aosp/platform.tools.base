...
abstract class FilterShrinkerRulesTransform : TransformAction<FilterShrinkerRulesTransform.Parameters> {
  interface Parameters : GenericTransformParameters {
    @get:Input val shrinkerVersion: Property<ShrinkerVersion>
  }

  @get:InputArtifact @get:PathSensitive(PathSensitivity.RELATIVE) abstract val inputArtifact: Provider<FileSystemLocation>
      ...
      input
        ...
        .flatMap { libDir ->
          ...
          if (toolsDir.isDirectory) {
            ...
            return@flatMap toolsDir
              .listFiles { file -> file.isDirectory && configDirMatchesVersion(file.name, parameters.shrinkerVersion.get()) }
            ...
            if (proguardTxtFile.isFile) {
              ...
              if (proguardConfigDir.isDirectory) {
                ...
                return@flatMap proguardConfigDir.listFiles { file -> file.isFile }.asIterable()
              }
            }
          }
          emptyList<File>()
        }
        .forEach { file -> transformOutputs.file(file.absolutePath) }
    }
  }
}
...
@VisibleForTesting
internal fun configDirMatchesVersion(dirName: String, version: ShrinkerVersion): Boolean {
