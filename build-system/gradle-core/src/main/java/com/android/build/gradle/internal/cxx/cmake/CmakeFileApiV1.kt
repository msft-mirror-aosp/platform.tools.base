 ...
 *
 * cmakeFiles-v1-{hash}.json -- List of CMakeLists.txt and other build files.
 *
 * cache-v2-{hash}.json -- CMake properties
 *
 * codemodel-v2-{hash}.json -- References to projects and targets. Contains filenames of target json files.
 *
 * target-hello-jni-Debug-{hash}.json -- Referenced by codemodel file. Contains source files and flags for one target.
 *
 * This is the V1 version of the CMake file API schema as described in https://cmake.org/cmake/help/latest/manual/cmake-file-api.7.html
 */

/** The index file is the entry-point for locating the real query results. It is defined as the lexically greatest index*.json file */
private fun findCmakeQueryApiIndexFile(replyFolder: File): File? {
  ...
  return files.toList().filter { it.name.startsWith("index") && it.name.endsWith(".json") }.maxByOrNull { it.name }
}
...
/**
 * This function makes a single pass through CMake file API reply file structure returning information needed to construct a native model.
 * The source files and flags may be large so they are streamed via a callback function [sourceFlagAction].
 */
fun readCmakeFileApiReply(
  ...
  val index = FileReader(indexFile).use { reader -> GSON.fromJson(reader, IndexData::class.java) }
      ...
      .filter { input ->
        ...
        input.name.equals("CMakeLists.txt", ignoreCase = true) ||
          (input.path.startsWith(rootSourceFolder.path, ignoreCase = true) &&
            ...
            (input.path.endsWith(".cmake.in", ignoreCase = true) || input.path.endsWith(".cmake", ignoreCase = true)))
      }
      .distinct()
      .sorted()
  ...
  val targetDataFiles =
    codeModel.configurations
      .flatMap { it.targets }
      .map { target ->
        targetIdToNativeLibraryValue.computeIfAbsent(target.id) { NativeLibraryValue() }.artifactName = target.name
        Pair(target.id, target.jsonFile)
      }

  targetDataFiles.forEach { (id, jsonFile) ->
    ...
    TargetDataStream(id, replyFolder.resolve(jsonFile)).use {
      it.stream().forEach { item ->
        when (item) {
          is Artifacts -> {
            targetIdToOutputs
              .computeIfAbsent(item.targetId) { mutableSetOf() }
              .addAll(item.artifacts.map { artifact -> File(cmakeFiles.paths.build).resolve(artifact).normalize().path })
          }
          ...
          is Source -> {
            ...
            if (compileGroup != null) {
              languageToExtensionMap.computeIfAbsent(compileGroup.language) { mutableSetOf() }.add(File(item.path).extension)
            }
  ...
  targetIdToOutputs
    ...
    .forEach { (id, outputs) ->
      if (outputs.size > 1) {
        errorln(EXTRA_OUTPUT, "Target $id produces multiple outputs ${outputs.joinToString(", ")}")
      }
      targetIdToNativeLibraryValue.computeIfAbsent(id) { NativeLibraryValue() }.output = outputs.map(::File).firstOrNull()
    }

  // Populate NativeLibraryValues#runtimeFiles

  targetIdToLink.forEach { (id, link) ->
    targetIdToNativeLibraryValue.computeIfAbsent(id) { NativeLibraryValue() }.runtimeFiles =
      link.compileCommandFragments
        ...
        .map { File(cmakeFiles.paths.build).resolve(it.fragment).normalize() }
        .toList()
  }
...
/**
 * Unfortunately, CMake file API doesn't provide a way to get the path to the compiler. This function infers a tool from an existing tool.
 * It uses the path and file extension.
 */
fun inferToolExeFromExistingTool(existingTool: String, newTool: String): File {
  ...
  return if (extension.isEmpty()) binFolder.resolve(newTool) else binFolder.resolve("$newTool.$extension")
}

private class TargetDataStream(val targetId: String, file: File) : Closeable {
      ...
      when (reader.peek()) {
        JsonToken.NAME -> {
          when (reader.nextName()) {
            "artifacts" -> yield(Artifacts(targetId, readSingleStringObjectList("path")))
            "compileGroups" -> yield(CompileGroups(readCompileGroups()))
            "link" -> yield(readLink())
            "dependencies" -> yield(Dependencies(targetId, readSingleStringObjectList("id")))
            "sourceGroups" -> yield(SourceGroups(readSingleStringObjectList("name")))
  ...
  private fun readCommandFragment(): CommandFragmentData {
    ...
    reader.endObject()
    return CommandFragmentData(role = role, fragment = fragment)
  }
...
/** Convert a CMake file API reply folder into the metadata that we need to provide Intellisense, caching, etc. */
fun parseCmakeFileApiReply(
  ...
  try {
    ...
    val config =
      readCmakeFileApiReply(replyFolder, createNinjaCommand) { source ->
    ...
    writeNativeBuildConfigValueToJsonFile(androidGradleBuildJsonFile, config)

    // Write compile_commands.json.bin
    convertCMakeToCompileCommandsBin(compileCommandsJsonFile, compileCommandsJsonBinFile)
  } catch (e: Exception) {
    // Give the user a way to open a constructive bug
    warnln(
      "There was an error parsing CMake File API result. Please open a bug with zip of ${compileCommandsJsonFile.parentFile.absoluteFile}",
      e,
    )
    throw e
  }
