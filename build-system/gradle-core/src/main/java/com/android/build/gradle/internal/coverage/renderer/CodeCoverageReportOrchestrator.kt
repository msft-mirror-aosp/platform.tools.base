...
/**
 * Orchestrates the entire process of generating the final HTML code coverage report.
 * This object acts as the main entry point for the report generation task. It coordinates
 * the parsing of multiple JaCoCo XML files, aggregates the data into a comprehensive model,
 * serializes the model to JavaScript files, and copies the static web assets (HTML, CSS, JS)
 * to create a self-contained, viewable report.
 */
object CodeCoverageReportOrchestrator {

    private val STATIC_HTML_RESOURCES = listOf(
        "css/style.css",
        "index.html",
        "javascript/codecoveragescript.js",
        "javascript/sourceviewscript.js"
    )

    /**
     * Executes the full report generation workflow.
     * The process involves:
     *  1. Finding and parsing all JaCoCo XML reports from the input directories. If any XML file
     *  file is malformed or cannot be parsed, the build will fail.
    ...
    fun orchestrate(
        ...
        SourceFileReportOrchestrator.orchestrate(
            sourceFileReportsBuilder,
            rootProjectDir,
            sourceFilesDir
        )
