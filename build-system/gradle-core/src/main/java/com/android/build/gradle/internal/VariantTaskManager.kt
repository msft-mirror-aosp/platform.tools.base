...
abstract class VariantTaskManager<VariantBuilderT : VariantBuilder, VariantT : VariantCreationConfig>(
  ...
  @JvmField protected val variantPropertiesList: List<VariantT> = variants.map(ComponentInfo<VariantBuilderT, VariantT>::variant)
  ...
  private val nestedComponents: List<NestedComponentCreationConfig> = testComponents + testFixturesComponents
  private val allPropertiesList: List<ComponentCreationConfig> = variantPropertiesList + nestedComponents
  ...
  fun createPostApiTasks() {
    val anyHostTestUsingAndroidResources = testComponents.filterIsInstance<HostTest>().any { it.androidResourcesIncluded }

    // must run this after scopes are created so that we can configure kotlin
    // kapt tasks
    addBindingDependenciesIfNecessary(globalConfig.dataBinding, anyHostTestUsingAndroidResources)

    // configure Kotlin compilation if needed.
    configureKotlinPluginTasksIfNecessary()

    createAnchorAssembleTasks(globalConfig.productFlavorCount, globalConfig.productFlavorDimensionCount)
  }
  ...
  private fun createTasksForVariant(componentInfo: ComponentInfo<VariantBuilderT, VariantT>) {
    ...
    if (variant is ApkCreationConfig && variant.dexing.dexingType.isLegacyMultiDex) {
      val multiDexDependency =
        if (variant.services.projectOptions[BooleanOption.USE_ANDROID_X]) ANDROIDX_MULTIDEX_MULTIDEX else COM_ANDROID_SUPPORT_MULTIDEX
      project.dependencies.add(variantDependencies.compileClasspath.name, multiDexDependency)
      project.dependencies.add(variantDependencies.runtimeClasspath.name, multiDexDependency)
    }
    if (variant.renderscriptCreationConfig?.renderscript?.supportModeEnabled?.get() == true) {
      val fileCollection = project.files(globalConfig.versionedSdkLoader.flatMap { it.renderScriptSupportJarProvider })
  ...
  private fun createReportTasks() {
    val signingReportComponents =
      allPropertiesList
        .stream()
        .filter { component: ComponentCreationConfig -> component is ApkCreationConfig }
        .map { component -> component as ApkCreationConfig }
        .collect(Collectors.toList())
    if (signingReportComponents.isNotEmpty()) {
      taskFactory.register("signingReport", SigningReportTask::class.java) { task: SigningReportTask ->
    ...
    val checkJetifierBuildService = CheckJetifierBuildService.RegistrationAction(project, globalConfig.services.projectOptions).execute()
    taskFactory.register(
      CheckJetifierTask.CreationAction(globalConfig, checkJetifierBuildService, variants, testComponents, testFixturesComponents)
    )
  }
  ...
  private fun createTasksForTest(testVariant: TestComponentCreationConfig) {
    ...
    if (testedVariant.renderscriptCreationConfig?.renderscript?.supportModeEnabled?.get() == true) {
      ...
      project.dependencies.add(
        ...
        project.files(globalConfig.versionedSdkLoader.flatMap { it.renderScriptSupportJarProvider }),
      )
    }
    if (testVariant.componentType.isApk) { // ANDROID_TEST
      if ((testVariant as ApkCreationConfig).dexing.dexingType.isLegacyMultiDex) {
        val variantDependencies = testVariant.variantDependencies
        val multiDexInstrumentationDep =
          if (testVariant.services.projectOptions[BooleanOption.USE_ANDROID_X]) ANDROIDX_MULTIDEX_MULTIDEX_INSTRUMENTATION
  ...
  private fun configureKotlinPluginTasksIfNecessary() {
    ...
    if (kotlinVersion != null && kotlinVersion.major >= 2 && !isComposeCompilerPluginApplied(project)) {
  ...
  private fun addBindingDependenciesIfNecessary(dataBindingOptions: DataBinding, anyHostTestUsingAndroidResources: Boolean) {
    val viewBindingEnabled =
      allPropertiesList.stream().anyMatch { componentProperties: ComponentCreationConfig -> componentProperties.buildFeatures.viewBinding }
    val dataBindingEnabled =
      allPropertiesList.stream().anyMatch { componentProperties: ComponentCreationConfig -> componentProperties.buildFeatures.dataBinding }
    ...
    if (viewBindingEnabled) {
      ...
      val groupAndArtifact = if (useAndroidX) SdkConstants.ANDROIDX_VIEW_BINDING_ARTIFACT else SdkConstants.VIEW_BINDING_ARTIFACT
      project.dependencies.add("api", "$groupAndArtifact:$version")
    }
    if (dataBindingEnabled) {
      val version = MoreObjects.firstNonNull(dataBindingOptions.version, dataBindingBuilder.compilerVersion)
      val baseLibArtifact =
        if (useAndroidX) SdkConstants.ANDROIDX_DATA_BINDING_BASELIB_ARTIFACT else SdkConstants.DATA_BINDING_BASELIB_ARTIFACT
      project.dependencies.add("api", baseLibArtifact + ":" + dataBindingBuilder.getBaseLibraryVersion(version))
      project.dependencies.add("annotationProcessor", SdkConstants.DATA_BINDING_ANNOTATION_PROCESSOR_ARTIFACT + ":" + version)
      // TODO load config name from source sets
      if (dataBindingOptions.enableForTests || this is LibraryTaskManager) {
        val dataBindingArtifact = SdkConstants.DATA_BINDING_ANNOTATION_PROCESSOR_ARTIFACT + ":" + version
        project.dependencies.add("androidTestAnnotationProcessor", dataBindingArtifact)
      ...
      if ((dataBindingOptions as DataBindingOptions).addDefaultAdapters) {
        val libArtifact = if (useAndroidX) SdkConstants.ANDROIDX_DATA_BINDING_LIB_ARTIFACT else SdkConstants.DATA_BINDING_LIB_ARTIFACT
        val adaptersArtifact =
          if (useAndroidX) SdkConstants.ANDROIDX_DATA_BINDING_ADAPTER_LIB_ARTIFACT else SdkConstants.DATA_BINDING_ADAPTER_LIB_ARTIFACT
        project.dependencies.add("api", libArtifact + ":" + dataBindingBuilder.getLibraryVersion(version))
        project.dependencies.add("api", adaptersArtifact + ":" + dataBindingBuilder.getBaseAdaptersVersion(version))
      }

      addDataBindingKtxIfNecessary(project, dataBindingOptions, dataBindingBuilder, version, useAndroidX)

      project.pluginManager.withPlugin(KOTLIN_KAPT_PLUGIN_ID) { configureKotlinKaptTasksForDataBinding(project, version) }
      project.pluginManager.withPlugin(ANDROID_BUILT_IN_KAPT_PLUGIN_ID) { configureKotlinKaptTasksForDataBinding(project, version) }
    }
  }
  ...
  private fun addDataBindingKtxIfNecessary(
    ...
    val ktxGradlePropertyValue = globalConfig.services.projectOptions.get(BooleanOption.ENABLE_DATABINDING_KTX)
    ...
    if (enableKtx) {
      ...
      if (useAndroidX && isKotlinConfigurationNecessary) {
        project.dependencies.add("api", DATA_BINDING_KTX_LIB_ARTIFACT + ":" + dataBindingBuilder.getLibraryVersion(version))
      } else {
  ...
  private fun configureKotlinKaptTasksForDataBinding(project: Project, version: String) {
    ...
    project.dependencies.add("kapt", SdkConstants.DATA_BINDING_ANNOTATION_PROCESSOR_ARTIFACT + ":" + version)
    var kaptTaskClass: Class<out Task>? = null
    try {
      kaptTaskClass = Class.forName("org.jetbrains.kotlin.gradle.internal.KaptTask") as Class<out Task>
    ...
    val kaptTaskNameOrPrefixToVariant = allPropertiesList.associateBy { it.computeTaskNameInternal("kapt", "Kotlin") }
    project.tasks.withType(kaptTaskClass) { kaptTask: Task ->
      val variant =
        kaptTaskNameOrPrefixToVariant.keys.firstOrNull { kaptTask.name.startsWith(it) }?.let { kaptTaskNameOrPrefixToVariant[it]!! }
      ...
      variant?.let { configureKaptTaskInScopeForDataBinding(variant, kaptTask) }
    }
  }

  private fun configureKaptTaskInScopeForDataBinding(creationConfig: ComponentCreationConfig, kaptTask: Task) {
    ...
    kaptTask.outputs.dir(dataBindingArtifactDir).withPropertyName("dataBindingArtifactDir")
    if (creationConfig.componentType.isExportDataBindingClassList) {
      kaptTask.outputs.file(exportClassListFile).withPropertyName("dataBindingExportClassListFile")
    }
  }
   ...
   *
   * This does not create the variant specific version of these tasks only the ones that are per build-type, per-flavor, per-flavor-combo
   * and the main 'assemble' and 'bundle' ones.
   *
   * @param flavorCount the number of flavors
   * @param flavorDimensionCount whether there are flavor dimensions at all.
   */
  private fun createAnchorAssembleTasks(flavorCount: Int, flavorDimensionCount: Int) {
    ...
    if (flavorCount > 0) {
      ...
      val assembleMap: ListMultimap<String, TaskProvider<out Task>> = ArrayListMultimap.create()
      val bundleMap: ListMultimap<String, TaskProvider<out Task>?> = ArrayListMultimap.create()
      ...
      for (dimensionKey in dimensionKeys) {
        ...
        subAssembleTasks.add(
          taskFactory.register("assemble$dimensionName") { task: Task ->
            task.description = ("Assembles main outputs for all " + dimensionName + " variants.")
        ...
        if (!subBundleMap.isEmpty()) {
          ...
          // create the task and add it to the list
          subBundleTasks.add(
            taskFactory.register("bundle$dimensionName") { task: Task ->
              task.description = ("Assembles bundles for all " + dimensionName + " variants.")
              task.group = BasePlugin.BUILD_GROUP
    ...
    if (subAssembleTasks.isNotEmpty()) {
      ...
      taskFactory.configure("assemble") { task: Task ->
    ...
    if (subBundleTasks.isNotEmpty()) {
      // root bundle task
      taskFactory.register("bundle") { task: Task ->
   ...
   */
  protected abstract fun doCreateTasksForVariant(variantInfo: ComponentInfo<VariantBuilderT, VariantT>)
  ...
  companion object {
    ...
    private const val COM_ANDROID_SUPPORT_MULTIDEX = "com.android.support:multidex:$MULTIDEX_VERSION"
    private val ANDROIDX_MULTIDEX_MULTIDEX = "androidx.multidex:multidex:$ANDROIDX_MULTIDEX_VERSION"
    private const val COM_ANDROID_SUPPORT_MULTIDEX_INSTRUMENTATION = "com.android.support:multidex-instrumentation:$MULTIDEX_VERSION"
    private val ANDROIDX_MULTIDEX_MULTIDEX_INSTRUMENTATION = "androidx.multidex:multidex-instrumentation:$ANDROIDX_MULTIDEX_VERSION"
  }
}
