...
abstract class AndroidLintTask : NonIncrementalTask() {
  ...
  @get:Nested abstract val lintTool: LintTool

  @get:Input abstract val textReportEnabled: Property<Boolean>

  @get:OutputFile @get:Optional abstract val textReportOutputFile: RegularFileProperty

  @get:OutputFile @get:Optional abstract val intermediateTextReport: RegularFileProperty

  @get:Input abstract val htmlReportEnabled: Property<Boolean>

  @get:OutputFile @get:Optional abstract val htmlReportOutputFile: RegularFileProperty

  @get:Input abstract val xmlReportEnabled: Property<Boolean>

  @get:OutputFile @get:Optional abstract val xmlReportOutputFile: RegularFileProperty
  ...
  @get:Input abstract val android: Property<Boolean>

  @get:Input abstract val fatalOnly: Property<Boolean>

  @get:Input abstract val autoFix: Property<Boolean>

  @get:Internal abstract val lintFixBuildService: Property<LintFixBuildService>

  @get:Input abstract val checkOnly: ListProperty<String>

  @get:Classpath abstract val lintRuleJars: ConfigurableFileCollection

  @get:Nested abstract val projectInputs: ProjectInputs

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val lintModels: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.ABSOLUTE) abstract val mainDependencyLintModels: ConfigurableFileCollection

  @get:InputFiles @get:PathSensitive(PathSensitivity.ABSOLUTE) abstract val dynamicFeatureLintModels: ConfigurableFileCollection
  ...
  @get:InputFiles @get:PathSensitive(PathSensitivity.ABSOLUTE) @get:Optional abstract val partialResults: ConfigurableFileCollection
  ...
  @get:Input abstract val printStackTrace: Property<Boolean>

  @get:Nested abstract val systemPropertyInputs: SystemPropertyInputs

  @get:Nested abstract val environmentVariableInputs: EnvironmentVariableInputs

  @get:OutputFile @get:Optional abstract val returnValueOutputFile: RegularFileProperty

  @get:Input abstract val lintMode: Property<LintMode>
  ...
  override fun doTaskAction() {
    ...
    if (
      lintMode.get() == LintMode.UPDATE_BASELINE &&
  ...
  abstract class AndroidLintLauncherWorkAction : WorkAction<AndroidLintLauncherWorkAction.LauncherParameters> {
    abstract class LauncherParameters : WorkParameters {
      @get:Nested abstract val lintTool: Property<LintTool>
  ...
  class SingleVariantCreationAction(variant: VariantWithTests) : VariantCreationAction(variant) {
    ...
    override val lintMode: LintMode
      get() = LintMode.REPORTING
  ...
  class LintVitalCreationAction(variant: VariantCreationConfig) :
    VariantCreationAction(VariantWithTests(variant, androidTest = null, unitTest = null, testFixtures = null)) {
  ...
  abstract class VariantCreationAction(val variant: VariantWithTests) :
    ...
    final override val type: Class<AndroidLintTask>
      get() = AndroidLintTask::class.java
    ...
    final override fun configure(task: AndroidLintTask) {
      ...
      val isLintAnalysisPerComponent =
        variant.main.services.projectOptions.get(LINT_ANALYSIS_PER_COMPONENT) || variant.main is KmpCreationConfig
      ...
      task.partialResults.from(
        if (fatalOnly) {
          creationConfig.artifacts.getAll(InternalMultipleArtifactType.LINT_VITAL_PARTIAL_RESULTS)
        } else {
          creationConfig.artifacts.getAll(InternalMultipleArtifactType.LINT_PARTIAL_RESULTS)
        }
      )
      ...
      task.mainDependencyLintModels.addRuntimeAndCompileArtifacts(creationConfig, ArtifactScope.PROJECT, lintModelArtifactType)
      task.dependencyPartialResults.addRuntimeAndCompileArtifacts(creationConfig, ArtifactScope.PROJECT, lintPartialResultsArtifactType)
      if (!fatalOnly && isLintAnalysisPerComponent) {
        val ignoreTestSources = task.projectInputs.lintOptions.ignoreTestSources.get()
        val ignoreTestFixturesSources = task.projectInputs.lintOptions.ignoreTestFixturesSources.get()
        // Add unit test lint models and partial results
        if ((creationConfig as? HasUnitTest)?.unitTest != null && ignoreTestSources.not()) {
          task.nestedComponentLintModels.from(creationConfig.artifacts.get(UNIT_TEST_LINT_MODEL))
          task.nestedComponentPartialResults.from(creationConfig.artifacts.get(UNIT_TEST_LINT_PARTIAL_RESULTS))
        }
        ...
        if ((creationConfig as? HasDeviceTests)?.deviceTests?.get(DeviceTestBuilder.ANDROID_TEST_TYPE) != null && ignoreTestSources.not()) {
          task.nestedComponentLintModels.from(creationConfig.artifacts.get(ANDROID_TEST_LINT_MODEL))
          task.nestedComponentPartialResults.from(creationConfig.artifacts.get(ANDROID_TEST_LINT_PARTIAL_RESULTS))
        }
        task.mainDependencyLintModels.addRuntimeAndCompileArtifacts(
          creationConfig,
          ArtifactScope.PROJECT,
        ...
        // Add test fixtures lint models and partial results
        if ((creationConfig as? HasTestFixtures)?.testFixtures != null && ignoreTestFixturesSources.not()) {
          task.nestedComponentLintModels.from(creationConfig.artifacts.get(TEST_FIXTURES_LINT_MODEL))
          task.nestedComponentPartialResults.from(creationConfig.artifacts.get(TEST_FIXTURES_LINT_PARTIAL_RESULTS))
        }
        task.mainDependencyLintModels.addRuntimeAndCompileArtifacts(
          creationConfig,
          ArtifactScope.PROJECT,
      ...
      task.missingBaselineIsEmptyBaseline.setDisallowChanges(
        creationConfig.services.projectOptions.getProvider(BooleanOption.MISSING_LINT_BASELINE_IS_EMPTY_BASELINE)
      )
      task.baselineOmitLineNumbers.setDisallowChanges(
        creationConfig.services.projectOptions.getProvider(BooleanOption.LINT_BASELINE_OMIT_LINE_NUMBERS)
      )
  ...
  private fun initializeGlobalInputs(services: TaskCreationServices, isAndroid: Boolean, lintMode: LintMode) {
    ...
    val globalLintJarsFromEnvVariable: Provider<List<String>> =
      project.providers.environmentVariable(ANDROID_LINT_JARS_ENVIRONMENT_VARIABLE).orElse("").map {
  ...
  fun configureForStandalone(
    ...
    this.missingBaselineIsEmptyBaseline.setDisallowChanges(
      taskCreationServices.projectOptions.getProvider(BooleanOption.MISSING_LINT_BASELINE_IS_EMPTY_BASELINE)
    )
    this.baselineOmitLineNumbers.setDisallowChanges(
      taskCreationServices.projectOptions.getProvider(BooleanOption.LINT_BASELINE_OMIT_LINE_NUMBERS)
    )
  ...
  companion object {
    ...
    /**
     * Return whether [newBaselineFile] has different content than [originalBaselineFileLines], ignoring changes to the attributes of the
     * <issues> element (b/248338457).
     */
    private fun isBaselineChanged(originalBaselineFileLines: List<String>, newBaselineFile: File): Boolean {
