...
class VariantServicesImpl(projectServices: ProjectServices, private val forUnitTesting: Boolean = false) :
  BaseServicesImpl(projectServices), VariantServices {
  ...
  override fun <T : Any> listPropertyOf(type: Class<T>, value: Collection<T>, disallowUnsafeRead: Boolean): ListProperty<T> {
  ...
  override fun <T : Any> listPropertyOf(type: Class<T>, fillAction: (ListProperty<T>) -> Unit): ListProperty<T> {
    return projectServices.objectFactory.listProperty(type).also {
      fillAction(it)
      it.finalizeValueOnRead()
      if (!forUnitTesting) {
  ...
  override fun <T : Any> setPropertyOf(type: Class<T>, value: Collection<T>, disallowUnsafeRead: Boolean): SetProperty<T> {
    return projectServices.objectFactory.setProperty(type).also {
      it.set(value)
      it.finalizeValueOnRead()
      if (disallowUnsafeRead && !forUnitTesting) {
  ...
  override fun <T : Any> providerOf(type: Class<T>, value: Provider<T>, id: String, disallowUnsafeRead: Boolean): Provider<T> {
    return initializeProperty(type).also {
      it.set(value)
      it.disallowChanges()
      it.finalizeValueOnRead()
  ...
  override fun regularFileProperty(): RegularFileProperty = projectServices.objectFactory.fileProperty().also { properties.add(it) }

  override fun directoryProperty(): DirectoryProperty = projectServices.objectFactory.directoryProperty().also { properties.add(it) }
  ...
  private fun <T : Any> initializeProperty(type: Class<T>): Property<T> = projectServices.objectFactory.property(type)

  private fun <T : Any> initializeListProperty(type: Class<T>): ListProperty<T> = projectServices.objectFactory.listProperty(type)

  private fun <T : Any> initializeNullableProperty(type: Class<T>): Property<T> = projectServices.objectFactory.property(type)
}
