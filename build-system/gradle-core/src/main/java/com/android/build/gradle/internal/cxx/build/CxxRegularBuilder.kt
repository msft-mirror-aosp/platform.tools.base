...
class CxxRegularBuilder(val abi: CxxAbiModel) : CxxBuilder {
  ...
  /**
   * Get native build config minis. Also gather stats if they haven't already been gathered for this variant
   *
  ...
  override fun build(providers: ProviderFactory) {
    ...
    if (config.buildTargetsCommandComponents?.isNotEmpty() == true) {
      ...
      val artifactNames = librariesToBuild.mapNotNull { library -> library.artifactName }.distinct().sorted()
      val buildTargetsCommand = substituteBuildTargetsCommand(config.buildTargetsCommandComponents!!, artifactNames)
      buildSteps.add(BuildStep(buildTargetsCommand, librariesToBuild))
      ...
      for (libraryValue in librariesToBuild) {
        buildSteps.add(BuildStep(libraryValue.buildCommandComponents!!, listOf(libraryValue)))
    ...
    for (library in config.libraries.values) {
      ...
      if (!output.exists()) {
        throw GradleException("Expected output file at $output for target ${library.artifactName} but there was none")
      }
      ...
      val expectedOutputFile = FileUtils.join(abi.soFolder, output.name)

      if (!FileUtils.isSameFile(output, expectedOutputFile)) {
        infoln("external build set its own library output location for " + "'${output.name}', hard link or copy to expected location")
        // Use synchronizeFile so that libs removed from the build will
        // also be deleted.
        synchronizeFile(output, expectedOutputFile)
      }

      for (runtimeFile in library.runtimeFiles) {
        val dest = FileUtils.join(abi.soFolder, runtimeFile.name)
        synchronizeFile(runtimeFile, dest)
      }
    }
  ...
  /** Verifies that all targets provided by the user will be built. Throws GradleException if it detects an unexpected target. */
  private fun verifyTargetExists(config: NativeBuildConfigValueMini) {
    ...
    for (target in targets) {
      if (!matchingTargets.contains(target)) {
        throw GradleException("Unexpected native build target $target. " + "Valid values are: ${Joiner.on(", ").join(unmatchedTargets)}")
      }
    }
  }

  /**
   * @return List of libraries defined in the input config file, filtered based on the targets field optionally provided by the user, and
   *   other criteria.
   */
  private fun findLibrariesToBuild(config: NativeBuildConfigValueMini): List<NativeLibraryValueMini> {
    ...
    loop@ for (libraryValue in config.libraries.values) {
      ...
      if (targets.isEmpty()) {
        ...
        if (output == null) {
          infoln(
            "not building target ${libraryValue.artifactName!!} because no targets " + "are specified and library build output file is null"
          )
          continue
        }
  ...
  private fun executeProcessBatch(providers: ProviderFactory, buildSteps: List<BuildStep>) {
    for (buildStep in buildSteps) {
      ...
      val command =
        createExecuteProcessCommand(tokens[0]).addArgs(tokens.drop(1)).addEnvironments(abi.buildSettings.getEnvironmentVariableMap())

      val logFileSuffix =
        if (buildStep.libraries.size > 1) {
          val targetNames = buildStep.libraries.stream().map { library -> library.artifactName + "_" + abi.name }.toList()
      ...
      if (variant.module.buildSystem == NativeBuildSystem.CMAKE || variant.module.buildSystem == NativeBuildSystem.NINJA) {
        ...
        val attributions by lazy {
          generateNinjaSourceFileAttribution(abi, linesToSkip, buildStartTime)
            .toBuilder()
        ...
        abi.variant.module.project.chromeTraceJsonFolder?.let { traceFolder ->
          generateChromeTrace(attributions, abi, buildStartTime, traceFolder)
        }
  ...
  companion object {
     ...
     * @return Replaces the placeholder in the input command with the given artifacts and returns a command that can be executed directly.
     */
    private fun substituteBuildTargetsCommand(buildTargetsCommandComponents: List<String>, artifactNames: List<String>): List<String> {
