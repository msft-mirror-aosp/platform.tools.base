...
/**
 * Build service that provides locations for C/C++ tools that aren't in the NDK (cmake and ninja). Results are cached within the
 * BuildService so that discovery of these tools is shared between different tasks.
 *
...
constructor(private val exec: ExecOperations, private val providerFactory: ProviderFactory) :
  ...
  private val androidLocationProvider
    get() = parameters.androidLocationsService.get()

  private val sdkComponents
    get() = parameters.sdkService.get()
  ...
  /** Locate CMake.exe and cache the result. If not found, try to install CMake from the SDK. */
  fun locateCMake(cmakeVersionFromDsl: String?, localPropertiesCMakeDir: File?): File? {
    ThreadLoggingEnvironment.requireExplicitLogger()
    synchronized(cmakeLocations) {
      val locatorKey = CMakeLocatorParameters(cmakeVersionFromDsl = cmakeVersionFromDsl, localPropertiesCMakeDir = localPropertiesCMakeDir)
  ...
  fun locateNinja(cmakeBinFolder: File?): File? {
    ...
    synchronized(ninjaLocations) {
      return ninjaLocations.computeIfAbsent(NinjaLocatorParameters(cmakeBinFolder = cmakeBinFolder)) {
        NinjaLocator().findNinjaPath(cmakePath = cmakeBinFolder, sdkFolder = sdkComponents.sdkDirectoryProvider.get().asFile)
      }
    }
  }
  ...
  private class RegistrationAction(project: Project) :
    ServiceRegistrationAction<NativeLocationsBuildService, ServiceParameters>(project, NativeLocationsBuildService::class.java) {
  ...
  private data class CMakeLocatorParameters(val cmakeVersionFromDsl: String?, val localPropertiesCMakeDir: File?)

  /** Parameters for caching location of ninja.exe. */
  private data class NinjaLocatorParameters(val cmakeBinFolder: File?)

  companion object {
    /** Register this service. It is safe to call multiple times and will only be registered once per build (like all build services). */
...
fun CxxAbiModel.rewriteWithLocations(locationService: NativeLocationsBuildService): CxxAbiModel {
  ...
  if (variant.module.buildSystem == NDK_BUILD) {
    ...
    return copy(variant = variant.copy(module = variant.module.copy(hasBuildTimeInformation = true)))
  }
  val cmake = variant.module.cmake
  val cmakeExe =
    locationService.locateCMake(
      cmakeVersionFromDsl = cmake?.cmakeVersionFromDsl,
      localPropertiesCMakeDir = cmake?.cmakeDirFromPropertiesFile,
    )
  val ninjaExe = locationService.locateNinja(cmakeBinFolder = cmakeExe?.parentFile)
  val cmakeExePath = cmakeExe?.path ?: ""
  val ninjaExePath = ninjaExe?.path ?: ""
  val buildTimeProperties =
    mapOf(
