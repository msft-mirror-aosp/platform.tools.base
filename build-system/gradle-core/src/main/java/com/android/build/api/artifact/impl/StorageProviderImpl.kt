...
class TypedStorageProvider<T : FileSystemLocation>(private val propertyAllocator: (ObjectFactory) -> FileSystemLocationProperty<T>) {
  ...
  @Synchronized
  internal fun getArtifact(objects: ObjectFactory, artifactType: Artifact.Single<T>): SingleArtifactContainer<T> {

    return singleStorage.getOrPut(artifactType) { SingleArtifactContainer { SinglePropertyAdapter(propertyAllocator(objects)) } }
  }

  internal fun getArtifact(objects: ObjectFactory, artifactType: Artifact.Multiple<T>): MultipleArtifactContainer<T> {
  ...
  internal fun copy(type: Artifact.Single<T>, container: SingleArtifactContainer<T>) {
    ...
    if (singleStorage[type] == null) singleStorage[type] = container else singleStorage[type]?.transferFrom(container)
  }

  internal fun copy(type: Artifact.Multiple<T>, container: MultipleArtifactContainer<T>) {
    // if the target container is null, we can just override with the source container
    // however, if it is not null, which mean that is has been queried, we cannot just
    // override. In that case, we need to just link the source to the target.
    if (multipleStorage[type] == null) multipleStorage[type] = container else multipleStorage[type]?.addInitialProvider(container)
  }
