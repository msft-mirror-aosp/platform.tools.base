...
/**
 * A derivative of the [SigningConfig] object, with input annotations on all of its properties to be used with `@Nested`, with an important
 * exception below.
 *
 * IMPORTANT: To support cache relocatability, we annotate storeFile with `PathSensitivity.NONE` to ignore the store file's path. This
 * requires that the tasks consuming this object do not take the store file's path as input (i.e., the store file's path does not affect the
 * output of those tasks). If the store file's path does affect the output of a task (e.g., as with `SigningConfigWriterTask`), the task
 * must explicitly declare the store file's path as an input.
 */
data class SigningConfigData(
  @get:Input val name: String,
  @get:Input @get:Optional val storeType: String?,
  @get:InputFile
  ...
  @get:Internal val storePassword: String?,
  @get:Input @get:Optional val keyAlias: String?,

  // Don't set the password as @Input as Gradle may store it to disk. Instead, we set the
  // password's hash as @Input (see getKeyPasswordHash()).
  @get:Internal val keyPassword: String?,
) : Serializable {

  @Input @Optional fun getStorePasswordHash(): String? = storePassword?.let { Hashing.sha256().hashUnencodedChars(it).toString() }

  @Input @Optional fun getKeyPasswordHash(): String? = keyPassword?.let { Hashing.sha256().hashUnencodedChars(it).toString() }
