 ...
 * unzipped AAR or they may be module references.
 *
 * In the case of module references a temporary Prefab package is generated in a temporary folder. However, the resulting CMake (or
...
fun createPrefabBuildSystemGlue(providers: ProviderFactory, abi: CxxAbiModel) {
  ...
  val prefabClassPath: File = abi.variant.prefabClassPath ?: error("CxxAbiModule.prefabClassPath cannot be null when Prefab is used")

  abi.createFolderLayout().use { layout ->
    val prefabPackages = getPrefabCliInputs(abi.name, layout.cliStagedInput, abi.variant.prefabPackageDirectoryList)

    // TODO: Get main class from manifest.
    val executeCommand =
      createJavaExecuteProcessCommand(classPath = prefabClassPath.path, main = "com.google.prefab.cli.AppKt")
...
val CxxAbiModel.prefabPublicationFiles
  get() =
    variant.prefabPackageDirectoryList
      .flatMap { realPackage -> listOf(Configuration.publicationFileFrom(realPackage), HeaderOnly.publicationFileFrom(realPackage)) }
      .filterNotNull()
...
/** Directory structure for staging and invoking Prefab CLI to generate CMake and ndk-build glue code. */
private data class FolderLayout(
  ...
  private val temporaryRootFolder: File,
) : AutoCloseable {
  override fun close() {
...
sealed class PrefabCliInput {
  ...
  data class AarPackage(override val packageFolder: File) : PrefabCliInput()

  /** A Prefab package coming from another module in this project. */
  data class ModulePackage(override val packageFolder: File, val payloadMappings: List<PayloadMapping>) : PrefabCliInput()
}
...
private fun getPrefabCliInputs(abiName: String, cliStagedInput: File, realPackages: List<File>): List<PrefabCliInput> {
  ...
  return realPackages.indices.map { i ->
    ...
    if (publication != null) {
      ...
      val payloadMappings =
        buildPrefabPackage(payloadIndirection = true, publication = patched.copy(installationFolder = temporaryPackageFolder))
      ModulePackage(temporaryPackageFolder, payloadMappings)
    } else AarPackage(realPackage)
  }
}

/**
 * Copy files from [cliStagedOutput] to [cliFinalOutput]. Along the way, translate lines that reference the configuration folder to
 * reference the final folder when needed
 */
private fun translateFromStagedToFinal(cliStagedOutput: File, cliFinalOutput: File, payloadMappings: List<PayloadMapping>) {
...
private data class ErrorMatchBehavior(val type: ErrorMatchType, val code: CxxDiagnosticCode? = null)
...
private val errorMatchers: List<Pair<Regex, ErrorMatchBehavior>> = run {
  ...
  listOf(
      "^.*NoMatchingLibraryException.*No compatible library found for (.*)\\." to ErrorMatchBehavior(RelevantLibraryDiscovery),
      "^.*(User requested .* but library requires .*)" to library(PREFAB_MISMATCHED_STL_TYPE),
      "^.*(User has minSdkVersion .* but library was built for .*)" to library(PREFAB_MISMATCHED_MIN_SDK_VERSION),
      "^.*(Library is a shared library with a statically linked STL and cannot be used with any library using the STL.*)" to
        library(PREFAB_SINGLE_STL_VIOLATION_LIBRARY_IS_SHARED_WITH_STATIC_STL),
      "^.*(User is using a static STL but library requires a shared STL.*)" to
        library(PREFAB_SINGLE_STL_VIOLATION_LIBRARY_REQUIRES_SHARED_STL),
      "^.*UnsupportedPlatformException: (.*)" to other(PREFAB_UNSUPPORTED_PLATFORM),
      "^.*DuplicateModuleNameException: (.*)" to other(PREFAB_DUPLICATE_MODULE_NAME),
      "^.*IllegalArgumentException: (Only schema_version 1 is supported\\. .* uses version .*)" to other(PREFAB_MISMATCHED_SCHEMA),
      "^.*IllegalArgumentException: (schema_version must be between .* and .*. Package uses version .*)" to other(PREFAB_MISMATCHED_SCHEMA),
      "^.*JsonDecodingException: (.*)" to other(PREFAB_JSON_FORMAT_PROBLEM),
      "^.*RuntimeException: (Prebuilt directory does not contain .*)" to other(PREFAB_PREBUILTS_MISSING),
      "^.*IllegalArgumentException: (Unknown ABI.*)" to fatal,
      "^.*IllegalArgumentException: (version must be compatible with CMake, if present.*)" to fatal,
      "^.*Error: (invalid value for --stl: invalid choice:.*)" to fatal,
      "^.*Error: (Invalid value for \".*\".*)" to fatal,
      "^.*(Unknown ABI: .*)" to fatal,
      "^.*(Unknown STL: .*)" to fatal,
      "^.*(User is targeting .* but library is for .*)" to informational,
      "^Usage: prefab.*" to informational,
      "Picked up _JAVA_OPTIONS: .*" to informational,
      "Picked up JAVA_TOOL_OPTIONS: .*" to informational,
