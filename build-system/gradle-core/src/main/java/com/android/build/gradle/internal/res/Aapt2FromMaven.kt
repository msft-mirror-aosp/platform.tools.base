 ...
 *
 * Contains a file collection, which will contain the directory with AAPT2 to be used, and a String identifying the version of AAPT2 being
 * used.
 */
class Aapt2FromMaven(val aapt2Directory: FileCollection, val version: String) {
  companion object {
    ...
    object DefaultAapt2Version {
      ...
      private val BUILD_NUMBER: String by
        lazy(LazyThreadSafetyMode.PUBLICATION) {
          DefaultAapt2Version::class.java.getResourceAsStream("aapt2_version.properties").buffered().use { stream ->
     ...
     *
     * If [StringOption.AAPT2_FROM_MAVEN_OVERRIDE] is set, the value of that flag will be returned as the version since the AAPT2 version is
     * not necessarily known in this case.
     *
     * This uses a detached configuration and is not idempotent, and should only be called when creating the project services.
     */
    @JvmStatic
    fun create(project: Project, stringOption: (option: StringOption) -> String?): Aapt2FromMaven {
      ...
      val overriddenVersion: String = stringOption(StringOption.AAPT2_FROM_MAVEN_VERSION_OVERRIDE) ?: ""

      if (customAapt2.any() && overriddenVersion.any()) {
        error(
          "You cannot specify both local and remote custom versions of AAPT2.\n" +
      ...
      val configuration =
        project.configurations.detachedConfiguration(
          project.dependencyFactory.create("com.android.tools.build", "aapt2", version, classifier, null)
        )
      ...
      val aapt2Directory =
        configuration.incoming
          .artifactView { config -> config.attributes { it.attribute(ARTIFACT_TYPE_ATTRIBUTE, TYPE_EXTRACTED_AAPT2_BINARY) } }
          .artifacts
          .artifactFiles
      ...
      override fun transform(transformOutputs: TransformOutputs) {
        ...
        ZipInputStream(input.inputStream().buffered()).use { zipInputStream ->
          while (true) {
            ...
            if (
              entry.name == SdkConstants.FN_AAPT2 &&
              ...
              Files.setPosixFilePermissions(destinationFile, Sets.union(permissions, setOf(PosixFilePermission.OWNER_EXECUTE)))
            }
          }
        }
      }
