...
fun createPublishingInfoForLibrary(
  ...
  val singleVariant = publishing.singleVariants.find { it.variantName == variantName }
  ...
  for (multipleVariant in publishing.multipleVariantsContainer) {
    ...
    val flavorDimensionAttributes = computeFlavorDimensionAttribute(multipleVariant, flavorList)

    val isClassifierRequired = buildTypeAttribute != null || flavorDimensionAttributes.isNotEmpty()

    val publishThisVariant = multipleVariant.allVariants || isVariantSelectedExplicitly(multipleVariant, buildType, flavorList)
    ...
    if (publishThisVariant) {
      components.add(
        ComponentPublishingInfo(
          ...
          AbstractPublishing.Type.AAR,
          ComponentPublishingInfo.AttributesConfig(buildTypeAttribute, flavorDimensionAttributes),
...
private fun ensureUsersInputCorrectness(
  ...
  fun computeErrorMessage(element: String) =
    "Using non-existing $element when selecting variants to be published in " + "multipleVariants publishing DSL."
...
private fun computeBuildTypeAttribute(multipleVariant: MultipleVariantsImpl, buildType: BuildType): String? {
  return if (multipleVariant.includedBuildTypes.size > 1 || multipleVariant.allVariants) buildType.name else null
}

private fun computeFlavorDimensionAttribute(multipleVariant: MultipleVariantsImpl, allFlavors: List<ProductFlavor>): MutableSet<String> {
  val flavorDimensionAttributes = mutableSetOf<String>()
  if (multipleVariant.allVariants) {
    allFlavors.map { flavorDimensionAttributes.add(it.dimension!!) }
  } else {
    for (entry in multipleVariant.includedFlavorDimensionAndValues.entries) {
...
private fun ensureComponentNameUniqueness(publishing: LibraryPublishingImpl, issueReporter: IssueReporter) {
...
fun createPublishingInfoForApp(
  ...
  val components = mutableListOf<ComponentPublishingInfo>()
  publishing.singleVariants
    .find { it.variantName == variantName }
    ?.let {
      if (it.publishVariantAsApk) {
        ...
        components.add(ComponentPublishingInfo(variantName, AbstractPublishing.Type.AAB))
      }
    }
  return VariantPublishingInfo(components)
}
