...
abstract class VariantBuilderImpl(
...
) : ComponentBuilderImpl(globalVariantBuilderConfig, componentIdentity, variantBuilderServices), VariantBuilder, InternalVariantBuilder {
  ...
  internal val targetSdkVersion: AndroidVersion
    get() {
      ...
      return if (variantBuilderServices.projectOptions.get(BooleanOption.DEFAULT_TARGET_SDK_TO_COMPILE_SDK_IF_UNSET)) {
        sanitizedMutableTargetSdk ?: globalVariantBuilderConfig.compileSdk ?: minSdkVersion
      } else {
        sanitizedMutableTargetSdk ?: minSdkVersion
      }
    }

  /** backing property for [targetSdk] and [targetSdkPreview] This could be null and will be instantiated on demand in the setter. */
  ...
  override var targetSdk: Int?
    ...
    set(value) {
      val target = mutableTargetSdk ?: MutableAndroidVersion(null, null).also { mutableTargetSdk = it }
  ...
  private val renderscriptTargetInDsl = variantDslInfo.renderscriptDslInfo?.renderscriptTarget ?: -1
  ...
  private val unitTest: HostTestBuilder
    get() = hostTests[HostTestBuilder.UNIT_TEST_TYPE] ?: throw RuntimeException("Invalid component, no unit test defined")
  ...
  fun getRegisteredExtensions(): Map<Class<out Any>, Any>? =
    if (registeredExtensionDelegate.isInitialized()) registeredExtensionDelegate.value else null
}
