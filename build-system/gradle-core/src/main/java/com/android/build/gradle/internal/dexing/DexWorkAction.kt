...
fun launchProcessing(dexWorkActionParams: DexWorkActionParams, receiver: MessageReceiver) {
  val dexArchiveBuilder = getDexArchiveBuilder(dexWorkActionParams, receiver)
...
private fun processIncrementally(dexArchiveBuilder: DexArchiveBuilder, dexWorkActionParams: DexWorkActionParams) {
  with(dexWorkActionParams.dexSpec.get()) {
    ...
    desugarGraphFile?.let { writeDesugarGraph(it, desugarGraph!!) }
  }
}

private fun processNonIncrementally(dexArchiveBuilder: DexArchiveBuilder, dexWorkActionParams: DexWorkActionParams) {
  // Dex outputs have been removed earlier before the workers are launched)

  with(dexWorkActionParams.dexSpec.get()) {
    val desugarGraph = desugarGraphFile?.let { MutableDependencyGraph<File>() }
  ...
  try {
    Closer.create().use { closer ->
      inputClassFiles.getClassFiles(filter = inputFilter, closer = closer).use {
        dexArchiveBuilder.convert(it, dexOutputPath.toPath(), globalSyntheticsOutput?.toPath(), desugarGraphUpdater)
      }
    }
  } catch (ex: DexArchiveBuilderException) {
    throw DexArchiveBuilderException("Failed to process: ${inputRoots.joinToString(", ") { it.path }}", ex)
  }
}

private fun getDexArchiveBuilder(dexWorkActionParams: DexWorkActionParams, messageReceiver: MessageReceiver): DexArchiveBuilder {
    ...
    dexArchiveBuilder =
      DexArchiveBuilder.createD8DexBuilder(
        com.android.builder.dexing.DexParameters(
          ...
          desugarBootclasspath = DexArchiveBuilderTaskDelegate.sharedState.getService(dexSpec.dexParams.desugarBootclasspath).service,
          desugarClasspath = DexArchiveBuilderTaskDelegate.sharedState.getService(dexSpec.dexParams.desugarClasspath).service,
...
fun writeDesugarGraph(desugarGraphFile: File, desugarGraph: MutableDependencyGraph<File>) {
  ObjectOutputStream(FileOutputStream(desugarGraphFile).buffered()).use { it.writeObject(desugarGraph) }
}
