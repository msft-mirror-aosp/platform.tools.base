...
/**
 * Classes and functions in this file are for dealing with CMake and ndk-build command-line parameters. Any unrecognized flags are
 * classified as UnknownArgument.
 */

/**
 * Interface that represents a single CMake or ndk-build command-line argument. Argument types that are specific to only one build system
 * are prefixed with Cmake (or in the future NdkBuild).
 */
sealed class CommandLineArgument {
   ...
   */
  data class CmakeListsPath(override val sourceArgument: String, val path: String) : CommandLineArgument() {
    companion object {
      /** Create a CmakeListsPath from a path. Don't use if you also have an an original sourceArgument. */
      @JvmStatic
   ...
   *
   * This is the build output folder. This is where the Ninja project is generated. For us, it usually has a value like
   * .cxx/cmake/debug/x86.
   */
  data class CmakeBinaryOutputPath(override val sourceArgument: String, val path: String) : CommandLineArgument() {
    companion object {
      /** Create a BinaryOutputPath from a path. Don't use if you also have an an original sourceArgument. */
      @JvmStatic
      fun from(path: String): CmakeListsPath {
   ...
   */
  data class CmakeGeneratorName(override val sourceArgument: String, val generator: String) : CommandLineArgument() {
    companion object {
      /** Create a GeneratorName. Don't use if you also have an an original sourceArgument. */
      @JvmStatic
   ...
   */
  data class NdkBuildJobs(override val sourceArgument: String, val jobs: String) : CommandLineArgument()
   ...
   */
  data class DefineProperty(override val sourceArgument: String, val propertyName: String, val propertyValue: String) :
    CommandLineArgument() {
    companion object {
      /** Create a DefineProperty from a name and value. Don't use if you also have an an original sourceArgument. */
      @JvmStatic
   ...
   */
  data class DefineMultiProperty(override val sourceArgument: String, val properties: Map<String, String>) : CommandLineArgument() {}
   ...
   */
  data class NdkBuildAppendProperty(override val sourceArgument: String, val listProperty: String, val flagValue: String) :
    CommandLineArgument()
}

/**
 * Check whether a CMake flag looks combinable with the argument that immediately follows it. See:
 * https://cmake.org/cmake/help/latest/manual/cmake.1.html
 *
 * There are some flags that are definitely known to be combinable (see [cmakeKnownCombinable]). There are some flags that are definitely
 * known to not be combinable (see [cmakeKnownNotCombinable]). For the remainder, a heuristic is used to decide whether it's combinable or
 * not.
 *
...
fun parseCmakeCommandLine(commandLine: String, host: OsBehavior = createOsBehavior()): List<CommandLineArgument> {
  val combinedTokens = host.tokenizeCommandLineToEscaped(commandLine) zip host.tokenizeCommandLineToRaw(commandLine)
  ...
  for (combinedToken in combinedTokens) {
    ...
    when {
      prior != null -> {
        result += "${prior.first} $escaped".toCmakeArgument("${prior.second} $raw")
        prior = null
      }
...
fun String.toNdkBuildArgument(sourceArgument: String = this): CommandLineArgument {
  return when {
    ...
    startsWith("--jobs=") -> NdkBuildJobs(sourceArgument, substringAfter("=").trim())
    startsWith("--jobs ") || startsWith("-j ") -> NdkBuildJobs(sourceArgument, substringAfterLast(" ").trim())
    startsWith("-j") -> NdkBuildJobs(sourceArgument, substringAfter("-j").trim())
    else ->
      // Didn't recognize the flag so return unknown argument
      UnknownArgument(sourceArgument)
  }
...
/**
 * Parse a single MSBuild command-line argument. See: https://docs.microsoft.com/en-us/visualstudio/msbuild/msbuild-command-line-reference
 */
fun String.toMSBuildArgument(sourceArgument: String = this): CommandLineArgument {
  fun propertyAfter(prefix: String) = run {
    DefineMultiProperty(
      sourceArgument,
      substringAfter(prefix).split(";").associate { body -> body.substringBefore("=").trim() to body.substringAfter("=").trim() },
    )
  }
  return when {
...
/**
 * Returns true when a set of args contains a property whose value would be considered true by CMake. If a property is set more than one
 * time then CMake behavior is to use the last. If the property isn't in the list then returns null.
 */
...
fun List<CommandLineArgument>.getCmakeProperty(property: CmakeProperty) = getProperty(property.name)

/** Returns the value of the property. Null if not present. If the value is present more than once then the last value is taken. */
fun List<CommandLineArgument>.getNdkBuildProperty(property: NdkBuildProperty) = getProperty(property.name)

/** Returns the value of the property. Empty string if not present. If the value is present more t han oncethen the last value is taken. */
fun List<CommandLineArgument>.getMSBuildProperty(property: MSBuildProperty) = getProperty(property.name)
...
fun List<CommandLineArgument>.removeCmakeProperty(property: CmakeProperty) = filter {
...
private inline fun <reified T : CommandLineArgument> List<CommandLineArgument>.filterType(predicate: (T) -> Boolean = { true }) =
  filterIsInstance<T>().lastOrNull { predicate(it) }
...
fun List<CommandLineArgument>.onlyKeepCmakeServerArguments() = filter { argument ->
  when (argument) {
...
/** Keep the [CommandLineArgument]s that should be passed to CMake Server. Remove the rest. */
fun List<CommandLineArgument>.onlyKeepProperties() = filterIsInstance(DefineProperty::class.java)

/**
 * Keep the [CommandLineArgument]s that are not explicitly recognized but that need to be forwarded to the CMake command-line invocation.
 */
fun List<CommandLineArgument>.onlyKeepUnknownArguments() = filterIsInstance(UnknownArgument::class.java)
...
fun List<CommandLineArgument>.removeBlankProperties() = filter { argument ->
  when (argument) {
