...
class KotlinMultiplatformAndroidPlugin
@Inject
constructor(listenerRegistry: BuildEventsListenerRegistry, private val buildFeatures: BuildFeatures) :
  ...
  private lateinit var kmpVariantApiOperationsRegistrar:
  ...
  private val dslServices by lazy {
    withProject("dslServices") { project ->
      ...
      DslServicesImpl(projectServices, sdkComponentsBuildService, ProjectType.LIBRARY)
    }
  }

  private val versionedSdkLoaderService: VersionedSdkLoaderService by lazy {
    withProject("versionedSdkLoaderService") { project ->
      VersionedSdkLoaderService(dslServices, project, ::getCompileSdkVersion, ::getBuildToolsVersion)
    }
  }

  private val kotlinMultiplatformHandler: KotlinMultiplatformAndroidHandler by
    lazy(LazyThreadSafetyMode.NONE) {
      withProject("kotlinMultiplatformHandler") { project ->
        KotlinMultiplatformAndroidHandlerImpl(project = project, dslServices = dslServices, objectFactory = projectServices.objectFactory)
      }
    }

  override fun getAnalyticsPluginType(): GradleBuildProject.PluginType? = GradleBuildProject.PluginType.KOTLIN_MULTIPLATFORM_ANDROID_LIBRARY
  ...
  override fun configureProject(project: Project) {
    ...
    project.extensions.extraProperties.set("kotlin.publishJvmEnvironmentAttribute", "true")
    ...
    AvdComponentsBuildService.RegistrationAction(
        ...
        getManagedDeviceAvdFolder(
          project.objects,
          project.providers,
          getBuildService(project.gradle.sharedServices, AndroidLocationsBuildService::class.java).get(),
        ),
        dslServices.sdkComponents,
        project.providers.provider { getCompileSdkVersion() },
        project.providers.provider { getBuildToolsVersion() },
      )
      .execute()
  }
  ...
  override fun configureExtension(project: Project) {
    ...
    kmpVariantApiOperationsRegistrar = VariantApiOperationsRegistrar(androidExtension)

    settingsExtension?.let { androidExtension.initExtensionFromSettings(it) }

    BasePlugin.createAndroidTestUtilConfiguration(project)

    val versionedSdkLoaderService =
      withProject("versionedSdkLoaderService") {
        VersionedSdkLoaderService(dslServices, project, ::getCompileSdkVersion, ::getBuildToolsVersion)
      }

    val bootClasspathConfig =
    ...
    global =
      KmpGlobalTaskCreationConfigImpl(
        project,
        androidExtension,
        settingsExtension,
        versionedSdkLoaderService,
    ...
    kotlinMultiplatformAndroidComponentsExtension =
      createComponentExtension(project, dslServices, kmpVariantApiOperationsRegistrar, bootClasspathConfig)
  }

  override fun createTasks(project: Project) {
    TaskManager.createTasksBeforeEvaluate(project, ComponentTypeImpl.KMP_ANDROID, emptySet(), global)

    project.afterEvaluate {
      if (!project.plugins.hasPlugin(KOTLIN_MPP_PLUGIN_ID)) {
        throw RuntimeException(
          "Kotlin multiplatform plugin was not found. This plugin needs" + " to be applied as part of the kotlin multiplatform plugin."
        )
      }
      afterEvaluate(it)
    }
  ...
  private fun KotlinMultiplatformAndroidLibraryExtension.initExtensionFromSettings(settings: SettingsExtension) {
    ...
    settings.minSdk minSdkSettings@{
      this@initExtensionFromSettings.minSdk {
        ...
        settingsMinSdkVersion?.let { this.version = MinSdkVersionImpl(apiLevel = it.apiLevel, codeName = it.codeName) }
      }
    }
  ...
  private fun getCompileSdkVersion(): String =
    androidExtension.compileSdkVersion.also { logger.verbose("compileSdkVersion=$it") }
      ?: throw RuntimeException(
  ...
  private fun getBuildToolsVersion(): Revision = Revision.parseRevision(androidExtension.buildToolsVersion, Revision.Precision.MICRO)

  private fun afterEvaluate(project: Project) {
    kmpVariantApiOperationsRegistrar.executeDslFinalizationBlocks()
    androidExtension.lock()

    val dependencyConfigurator =
      DependencyConfigurator(project = project, projectServices = projectServices)
        .configureDependencySubstitutions()
        .configureDependencyChecks()
        .configureGeneralTransforms(global.aarOrJarTypeToConsume)
    ...
    val taskManager = KmpTaskManager(project, global)
    ...
    val androidTest =
      createAndroidTestComponent(project, global, variantServices, taskServices, taskManager, kotlinMultiplatformHandler.getAndroidTarget())

    mainVariant.unitTest = unitTest
    androidTest?.let { mainVariant.addDeviceTest(DeviceTestBuilder.ANDROID_TEST_TYPE, it) }

    val stats = configuratorService.getVariantBuilder(project.path, mainVariant.name)

    kmpVariantApiOperationsRegistrar.variantOperations.executeOperations(
      stats?.let { variantServices.newInstance(AnalyticsEnabledKotlinMultiplatformAndroidVariant::class.java, mainVariant, stats) }
        ?: mainVariant
    )
    ...
    (global.compileOptions as CompileOptions).finalizeSourceAndTargetCompatibility(project, global)

    if (androidTest?.codeCoverageEnabled == true) {
      dependencyConfigurator.configureJacocoTransforms()
    }

    taskManager.createTasks(project, mainVariant, unitTest, androidTest)
  ...
  private fun updateTestComponentFriendPaths(components: List<KmpComponentImpl<out KmpComponentDslInfo>>) {
    ...
    components.forEach {
      it.androidKotlinCompilation.compileTaskProvider.configure { task ->
        (task as KotlinJvmCompile)
          .friendPaths
          .from(it.services.fileCollection(mainVariant.artifacts.get(InternalArtifactType.COMPILE_LIBRARY_CLASSES_JAR)))
      }
    }
  }

  private fun Configuration.forMainVariantConfiguration(dslInfo: KmpComponentDslInfo): Configuration? {
    return this.takeIf { !dslInfo.componentType.isTestComponent }
  }
  ...
  private fun createVariantDependencies(
    ...
    return VariantDependencies.createForKotlinMultiplatform(
      ...
      dslInfo = dslInfo,
      apiConfiguration = project.configurations.getByName(androidKotlinCompilation.apiConfigurationName),
      implementationConfiguration = project.configurations.getByName(androidKotlinCompilation.implementationConfigurationName),
      compileOnlyConfiguration = project.configurations.getByName(androidKotlinCompilation.compileOnlyConfigurationName),
      runtimeOnlyConfiguration = project.configurations.getByName(androidKotlinCompilation.runtimeOnlyConfigurationName),
      compileClasspath = project.configurations.getByName(androidKotlinCompilation.compileDependencyConfigurationName),
      runtimeClasspath = project.configurations.getByName(androidKotlinCompilation.runtimeDependencyConfigurationName!!),
      lintChecksClasspath =
      ...
      apiElements =
        (androidTarget as KotlinMultiplatformAndroidLibraryTargetImpl).apiElementsConfiguration.forMainVariantConfiguration(dslInfo),
      runtimeElements = androidTarget.runtimeElementsConfiguration.forMainVariantConfiguration(dslInfo),
      sourcesElements =
        project.configurations.findByName(androidTarget.sourcesElementsConfigurationName)?.forMainVariantConfiguration(dslInfo),
      ...
      sourcesPublication =
        androidTarget.sourcesElementsPublishedConfiguration.forMainVariantConfiguration(dslInfo).also {
  ...
  private fun getAndroidManifestDefaultLocation(compilation: KotlinMultiplatformAndroidCompilation) =
    FileUtils.join(compilation.project.projectDir, "src", compilation.defaultSourceSet.name, "AndroidManifest.xml")
  ...
  private fun createVariant(
    ...
    val kotlinCompilation =
      androidTarget.compilations.getByName(KmpAndroidCompilationType.MAIN.defaultCompilationName)
    ...
    val userVisibleVariantBuilder =
      (variantBuilder as InternalVariantBuilder).createUserVisibleVariantObject<KotlinMultiplatformAndroidVariantBuilder>(
        projectServices,
        profileEnabledVariantBuilder,
      )

    kmpVariantApiOperationsRegistrar.variantBuilderOperations.executeOperations(userVisibleVariantBuilder)

    if (!variantBuilder.enable) {
      throw RuntimeException(
        "Android Kotlin multiplatform plugin has a single variant (${variantBuilder.name}). " + "Disabling that variant is not permitted."
      )
    }
    ...
    return KmpVariantImpl(
        ...
        buildFeatures =
          KotlinMultiplatformBuildFeaturesValuesImpl(
            androidExtension.androidResources,
            ModulePropertyKey.BooleanWithDefault.KMP_ANDROID_RESOURCES_ENABLED.getValue(dslInfo.experimentalProperties),
          ),
            ...
            kotlinCompilation.defaultSourceSet.resources.srcDirs.map { srcDir ->
              sourcesImpl.extras.maybeCreate(sourceDirName).also {
                (it as FlatSourceDirectoriesImpl).addStaticSource(
                  FileBasedDirectoryEntryImpl(sourceDirName, File(srcDir.parentFile, sourceDirName))
                )
              }
            }
          }
  ...
  private fun createUnitTestComponent(
    ...
    val kotlinCompilation =
      androidTarget.compilations.getByName(androidExtension.androidTestOnJvmBuilder!!.compilationName)
        as KotlinMultiplatformAndroidCompilationImpl
    ...
    return KmpHostTestImpl(
      ...
      internalServices = variantServices,
      buildFeatures =
        KotlinMultiplatformHostTestBuildFeaturesValuesImpl(
          androidExtension.androidResources,
  ...
  private fun createAndroidTestComponent(
    ...
    val kotlinCompilation =
      androidTarget.compilations.getByName(androidExtension.androidTestOnDeviceBuilder!!.compilationName)
        as KotlinMultiplatformAndroidCompilationImpl

    val manifestLocation = getAndroidManifestDefaultLocation(kotlinCompilation)

    taskManager.canParseManifest.set(!dslServices.projectOptions[BooleanOption.DISABLE_EARLY_MANIFEST_PARSING])
    ...
    return KmpAndroidTestImpl(
      ...
      buildFeatures =
        KotlinMultiplatformBuildFeaturesValuesImpl(
          androidExtension.androidResources,
          ModulePropertyKey.BooleanWithDefault.KMP_ANDROID_RESOURCES_ENABLED.getValue(mainVariant.dslInfo.experimentalProperties),
        ),
      variantDependencies = createVariantDependencies(project, dslInfo, kotlinCompilation, androidTarget),
      paths = paths,
      artifacts = artifacts,
  ...
  private fun configureDisambiguationRules(project: Project) {
    project.dependencies.attributesSchema { schema ->
      ...
      (androidExtension.localDependencySelection as DependencySelectionImpl).getDimensions().forEach { (dimension, fallbacks) ->
        schema.attribute(ProductFlavorAttr.of(dimension)).disambiguationRules.add(SingleVariantProductFlavorRule::class.java) { config ->
  ...
  val managedDeviceRegistry: ManagedDeviceRegistry by
    lazy(LazyThreadSafetyMode.NONE) { ManagedDeviceRegistry(KmpDeviceTestOptionsDslInfoImpl(androidExtension)) }
  ...
  private fun createComponentExtension(
    ...
    val sdkComponents: SdkComponents =
      dslServices.newInstance(
        ...
        project.provider(bootClasspathConfig::bootClasspath),
      )
