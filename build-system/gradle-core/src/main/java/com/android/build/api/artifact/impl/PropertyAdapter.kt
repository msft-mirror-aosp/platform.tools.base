...
/**
 * Gradle's [Property] and [ListProperty] are used to store single and multiple files respectively. Unfortunately, they do not have a common
 * interface for services like [Property.set] and [ListProperty.set] so a small adapter is necessary to delegate to the right method and
 * provider a common set of APIs.
 *
...
interface PropertyAdapter<FileTypeT : Any, T : Any> {
  ...
  /** @return a [Provider] of a [FileSystemLocation] or a [Provider] or a [List] of [FileSystemLocation] */
  fun get(): Provider<T>
...
class MultiplePropertyAdapter<FileTypeT : FileSystemLocation>(
  ...
  override fun set(with: Provider<FileTypeT>) {
    property.empty()
    add(with)
    locationsOnly.add(propertyAllocator().also { it.set(with) }.locationOnly)
  }
  ...
  fun add(item: Provider<FileTypeT>) {
    property.add(item)
    locationsOnly.add(propertyAllocator().also { it.set(item) }.locationOnly)
  }

  /** Adds a [Provider] of a [List] of [FileSystemLocation] to the collection of elements. */
