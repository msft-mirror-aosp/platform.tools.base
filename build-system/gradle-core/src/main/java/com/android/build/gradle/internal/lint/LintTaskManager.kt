...
class LintTaskManager(
  ...
  fun createBeforeEvaluateLintTasks() {
    ...
    taskFactory.register(AndroidLintGlobalTask.UpdateBaselineCreationAction(globalTaskCreationConfig))
  }
  ...
  fun createLintTasks(
    ...
    val variantsWithTests =
      attachTestsToVariants(
    ...
    for (variantWithTests in variantsWithTests.values) {
      ...
      if (componentType.isAar || componentType.isDynamicFeature) {
        ...
        taskFactory.register(AndroidLintAnalysisTask.LintVitalCreationAction(mainVariant))
      ...
      if (isPerComponent) {
        ...
        taskFactory.register(AndroidLintAnalysisTask.PerComponentCreationAction(mainVariant, fatalOnly = false))
      } else {
        taskFactory.register(
          LintModelWriterTask.CreationAction(
            variantWithTests,
        ...
        taskFactory.register(AndroidLintAnalysisTask.SingleVariantCreationAction(variantWithTests))
      }
      ...
      if (needsCopyReportTask) {
        val copyLintReportTask = taskFactory.register(AndroidLintCopyReportTask.CreationAction(mainVariant))
        variantLintTextOutputTask?.configure { it.finalizedBy(copyLintReportTask) }
      }

      if (shouldRegisterLintVitalTasks(mainVariant)) {
        if (isPerComponent) {
          taskFactory.register(AndroidLintAnalysisTask.PerComponentCreationAction(mainVariant, fatalOnly = true))
          taskFactory.register(
            LintModelWriterTask.PerComponentCreationAction(
              mainVariant,
              useModuleDependencyLintModels = true,
        ...
        val lintVitalTask =
          taskFactory.register(AndroidLintTask.LintVitalCreationAction(mainVariant)).also {
            it.configure { task -> task.mustRunAfter(updateLintBaselineTask) }
          }
  ...
  private fun attachTestsToVariants(
    ...
    for (variant in variantPropertiesList) {
      variantsWithTests[variant.name] =
        VariantWithTests(
          variant,
          testFixtures =
    ...
    for (testComponent in testComponentPropertiesList) {
      ...
      when (testComponent) {
        is DeviceTestCreationConfig -> {
          ...
          variantsWithTests[key] = VariantWithTests(current.main, testComponent, current.unitTest, current.testFixtures)
        }
        is HostTestCreationConfig -> {
          when (testComponent.componentType) {
            ComponentTypeImpl.UNIT_TEST -> {
              ...
              variantsWithTests[key] = VariantWithTests(current.main, current.androidTest, testComponent, current.testFixtures)
            }
  ...
  private fun runConfigurationValidation(
    ...
    if (targetSdkVersion != null && !componentType.isAar) {
      ...
      for (variant in variantPropertiesList) {
        val variantTargetSdkVersion =
          when (variant) {
            is ApkCreationConfig -> variant.targetSdk
            is TestCreationConfig -> variant.targetSdkVersion
            else -> null
          }
        if (variantTargetSdkVersion != null && targetSdkVersion.apiLevel < variantTargetSdkVersion.apiLevel) {
