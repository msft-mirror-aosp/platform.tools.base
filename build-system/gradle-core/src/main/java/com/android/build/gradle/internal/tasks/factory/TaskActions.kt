...
@JvmOverloads
constructor(@JvmField protected val creationConfig: CreationConfigT, private val dependsOnPreBuildTask: Boolean = true) :
  TaskCreationAction<TaskT>() where TaskT : Task, TaskT : VariantTask {

  protected fun computeTaskName(prefix: String, suffix: String): String = creationConfig.computeTaskNameInternal(prefix, suffix)

  protected fun computeTaskName(prefix: String): String = creationConfig.computeTaskNameInternal(prefix)
 ...
 *
 * DISCOURAGED USAGE: The use of this class is not recommended because it does not provide a [variantName] by default. It is typically used
 * when a [ComponentCreationConfig] is not available. If a [ComponentCreationConfig] is available, use [VariantTaskCreationAction] instead.
 * If you still need to use this class, try to provide a [variantName] if possible.
 */
abstract class AndroidVariantTaskCreationAction<TaskT : AndroidVariantTask>(private val variantName: String = "") :
...
interface PreConfigAction {
   ...
   *
   * This is meant to handle configuration that must happen always, even when the task is configured lazily.
   *
