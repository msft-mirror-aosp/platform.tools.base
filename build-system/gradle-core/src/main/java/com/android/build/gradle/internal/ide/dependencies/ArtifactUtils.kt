...
interface ArtifactCollectionsInputs {
  ...
  @get:Internal val buildPath: Provider<String>
  @get:Input val variantName: String
  @get:Nested val compileClasspath: ArtifactCollections
  @get:Nested @get:Optional val runtimeClasspath: ArtifactCollections?

  @get:Internal val runtimeLintJars: ArtifactCollection
  @get:Internal val compileLintJars: ArtifactCollection

  @get:Nested val level1RuntimeArtifactCollections: Level1RuntimeArtifactCollections
...
class ArtifactCollectionsInputsImpl(
  ...
  @Suppress("DEPRECATION") override val buildPath: Provider<String> = getBuildPath(variantDependencies)

  override val compileClasspath: ArtifactCollections =
    ArtifactCollections(variantDependencies, AndroidArtifacts.ConsumedConfigType.COMPILE_CLASSPATH)

  /** The full runtime graphs for more complex dependency models */
  override val runtimeClasspath: ArtifactCollections? =
    if (runtimeType == RuntimeType.FULL) {
      ArtifactCollections(variantDependencies, AndroidArtifacts.ConsumedConfigType.RUNTIME_CLASSPATH)
    } else null
  ...
  override fun getAllArtifacts(
    ...
    return resolveArtifacts(collections.all, collections.aarOrAsar, collections.lintJar, ignoreUnexpectedArtifactTypes) {
      if (dependencyFailureHandler != null) {
        // compute the name of the configuration
        dependencyFailureHandler.addErrors(
          projectPath + "@" + variantName + "/" + collections.consumedConfigType.getName(),
...
class ArtifactCollections(
  variantDependencies: VariantDependencies,
  @get:Internal val consumedConfigType: AndroidArtifacts.ConsumedConfigType,
) {
  constructor(
    componentImpl: ComponentCreationConfig,
    consumedConfigType: AndroidArtifacts.ConsumedConfigType,
  ) : this(componentImpl.variantDependencies, consumedConfigType)

  /**
   * A collection containing 'all' artifacts, i.e. jar and AARs from subprojects, repositories and files.
   *
   * This will give the following mapping:
   * * Java library project → Untransformed jar output
   * * Android library project → *jar* output, aar is not published between projects. This could be a separate type in the future if it was
   *   desired not to publish the jar from android-library projects.
   ...
   *
   * Using an artifact view as that contains local dependencies, unlike `configuration.incoming.resolutionResult` which only contains
   * project and \[repository\] module dependencies.
   *
   * This captures dependencies without transforming them using `AttributeCompatibilityRule`s.
   */
  @get:Internal val all: ArtifactCollection = variantDependencies.aarOrJar(consumedConfigType)

  @get:Classpath
  val allFileCollection: FileCollection
    get() = all.artifactFiles

  @get:Internal val lintJar: ArtifactCollection = variantDependencies.lintJars(consumedConfigType, externalOnly = false)
  ...
  @get:Internal val aarOrAsar: ArtifactCollection = variantDependencies.aarsOrAsars(consumedConfigType)

  /**
   * For project jars, query for JAR instead of PROCESSED_JAR for two reasons:
   * - Performance: Project jars are currently considered already processed (unlike external jars).
   * - Workaround for a Gradle issue: Gradle may throw FileNotFoundException if a project jar has not been built yet; this issue does not
   *   affect external jars (see bug 110054209).
   */
  @get:Internal
  val projectJars: ArtifactCollection =
    variantDependencies.getArtifactCollectionForToolingModel(
      consumedConfigType,
      AndroidArtifacts.ArtifactScope.PROJECT,
      AndroidArtifacts.ArtifactType.JAR,
...
/** Model builder is decoupled from artifact collections / inputs to be able to optimize some of its behavior. */
...
/**
 * Returns a set of ResolvedArtifact where the [ResolvedArtifact.dependencyType] and [ResolvedArtifact.isWrappedModule] fields have been
 * setup properly.
 *
...
fun getArtifactsForComponent(
  ...
  return resolveArtifacts(collections.all, collections.aarOrAsar, collections.lintJar)
}
...
private fun resolveArtifacts(
  ...
  /** See [ArtifactCollections.projectJars]. */
  val projectJarsMap: ImmutableMultimap<VariantKey, ResolvedArtifactResult> by
    lazy(LazyThreadSafetyMode.NONE) {
      allArtifacts.artifacts
        .filter { it.variant.owner is ProjectComponentIdentifier && it.hasType(AndroidArtifacts.ArtifactType.JAR) }
        .asMultiMap()
    }
  ...
  val aarWrappedAsProjects = explodedAars.keys.filter { it.owner is ProjectComponentIdentifier }
  ...
  val artifacts = Sets.newLinkedHashSetWithExpectedSize<ResolvedArtifact>(resolvedArtifactResults.size)
  ...
  for (resolvedComponentResult in resolvedArtifactResults) {
    ...
    val artifactType = resolvedComponentResult.variant.attributes.getAttribute(AndroidArtifacts.ARTIFACT_TYPE)
    when (artifactType) {
      ...
      AndroidArtifacts.ArtifactType.JAR.type ->
        if (resolvedComponentResult.isAndroidProjectDependency()) {
          ...
          projectJars
            ...
            .forEach { addArtifact(dependencyType = ResolvedArtifact.DependencyType.JAVA, mainArtifact = it) }
        }
      else ->
        if (!ignoreUnexpectedArtifactTypes) {
          throw IllegalStateException("Internal error: Artifact type $artifactType not expected, only jar or aar are handled.")
        }
    }
  }
 ...
 *
 * This is used when we want to compare artifacts to see if they are coming from the same dependency as we cannot use [ComponentIdentifier]
 * (it does not handle multi-variant cases like test fixtures), and we cannot use [ResolvedVariantResult] directly because one of its
 * attributes is artifactType which is tied to the query that returned the artifact.
 *
 * This key only takes into account the [ComponentIdentifier], the list of [Capability] and [ResolvedVariantResult.getExternalVariant]
 */
data class VariantKey(val owner: ComponentIdentifier, val capabilities: List<Capability>, val externalVariant: VariantKey?)

@Suppress("UnstableApiUsage")
fun ResolvedVariantResult.toKey(): VariantKey = VariantKey(owner, capabilities, externalVariant.orElse(null)?.toKey())
...
fun ResolvedVariantResult.isAndroidProjectDependency(): Boolean {
  val variantAttribute = attributes.keySet().firstOrNull { it.name == AgpVersionAttr.ATTRIBUTE.name } ?: return false
...
private fun VariantDependencies.lintJars(configType: AndroidArtifacts.ConsumedConfigType, externalOnly: Boolean) =
  getArtifactCollectionForToolingModel(
    configType,
...
private fun VariantDependencies.aarsOrAsars(
  configType: AndroidArtifacts.ConsumedConfigType,
  additionalFilter: ((ComponentIdentifier) -> Boolean)? = null,
) =
  getArtifactCollectionForToolingModel(
