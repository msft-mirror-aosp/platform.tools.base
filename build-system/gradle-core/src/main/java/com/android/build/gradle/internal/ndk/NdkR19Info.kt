...
open class NdkR19Info(val root: File) : DefaultNdkInfo(root) {
  ...
  override fun getStlSharedObjectFile(stl: Stl, abi: String): File {
    checkArgument(stl == Stl.LIBCXX_SHARED, "Only c++_shared is valid for packaging as of NDK r19")
    ...
    val file = rootDirectory.resolve("toolchains/llvm/prebuilt/$hostTag/sysroot/usr/lib/$sysrootTriple/${stl.libraryName}")
  ...
  override fun getStripExecutable(abi: String): File {
    ...
    return rootDirectory.resolve("toolchains/llvm/prebuilt/$hostTag/bin/$triple-strip")
  }

  override fun getObjcopyExecutable(abi: String): File {
    val triple = abiInfoList.single { info -> info.name == abi }.triple
    return rootDirectory.resolve("toolchains/llvm/prebuilt/$hostTag/bin/$triple-objcopy")
  }
