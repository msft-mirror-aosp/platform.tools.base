...
abstract class ExternalNativeCleanTask @Inject constructor(private val providers: ProviderFactory) : NonIncrementalTask() {
  ...
  /**
   * See http://b/262059864 file not found exception in native tests after 8.0 upgrade This declares the folders that this task may destroy
   * so that Gradle can properly order tasks in dependency order.
   */
  @get:Destroys
  val destroys: List<File>
    get() = (configurationModel.activeAbis + configurationModel.unusedAbis).map { it.soFolder }

  override fun doTaskAction() {
    IssueReporterLoggingEnvironment(DefaultIssueReporter(LoggerWrapper(logger)), analyticsService.get(), configurationModel.variant).use {
        ...
        configValueList
          ...
          .map { config ->
            ...
            CleanProcessInfo(
              ...
              targets = config.libraries.values.joinToString { library -> "${library.artifactName}-${library.abi}" },
            )
          }
  ...
  /**
   * Information about a batch of clean commands to execute. There is one command for each element of [commands] outer list. The inner list
   * first contains the executable to run then each subsequent command-line parameter for it.
   */
  private data class CleanProcessInfo(val abi: CxxAbiModel, val commands: List<List<String>>, val targets: String)

  /** Given a list of clean commands, execute each. If there is a failure, processing is stopped at that point. */
...
fun createVariantCxxCleanTask(configurationModel: CxxConfigurationModel, creationConfig: VariantCreationConfig) =
