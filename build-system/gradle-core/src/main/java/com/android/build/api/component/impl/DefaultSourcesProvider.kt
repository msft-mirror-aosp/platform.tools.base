...
interface DefaultSourcesProvider {
   ...
   *
   * The [List] is ordered in ascending order of importance, meaning the first set is meant to be overridden by the 2nd one and so on.
   */
  ...
  /** the list of sources [DirectoryEntry] for R8 */
  fun getKeepRules(lateAdditionsDelegate: FlatSourceDirectoriesImpl): List<DirectoryEntry>
