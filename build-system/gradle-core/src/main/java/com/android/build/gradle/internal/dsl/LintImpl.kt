...
abstract class LintImpl @Inject constructor(private val dslServices: DslServices) : Lint {
  ...
  private val severities: Map<LintModelSeverity, MutableSet<String>> by
  ...
  internal val severityOverridesMap: Map<String, LintModelSeverity>
    get() =
      Collections.unmodifiableMap(
        mutableMapOf<String, LintModelSeverity>().apply {
          severities.forEach { (severity, issues) -> issues.forEach { put(it, severity) } }
        }
      )
  ...
  // file.
  private inner class SeverityOverrideSet(private val severity: LintModelSeverity) : AbstractMutableSet<String>() {

    private val backing: MutableSet<String>
      get() = severities[severity]!!

    override fun add(element: String): Boolean {
      return backing.add(element).also {
        severities.forEach { (severity, severitySet) -> if (severity != this.severity) severitySet.remove(element) }
      }
    }
   ...
   *
   * The implementation first copies its `items` set argument before clearing the underlying set. If it cleared the underlying set first,
   * and the build author writes disable = disable, that would have the surprising result of clearing the set. See the similar
   * implementation in [com.android.build.gradle.internal.dsl.decorator.DslDecorator.createGroovyMutatingSetter]
   */
  ...
  override val informational: MutableSet<String> = SeverityOverrideSet(LintModelSeverity.INFORMATIONAL)
  ...
  override fun targetSdk(action: TargetSdkSpec.() -> Unit) {
    createTargetSdkSpec().also {
      action.invoke(it)
      updateIfChanged(targetSdkApiVersion, it.version) { version -> targetSdkApiVersion = version }
    }
  }
  ...
  override var targetSdkPreview: String?
    ...
    set(value) {
      targetSdk { version = value?.let { preview(it) } }
    }
