...
abstract class AnalyticsEnabledComponent(
  ...
  override val artifacts: Artifacts
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.ARTIFACTS_VALUE
      return objectFactory.newInstance(AnalyticsEnabledArtifacts::class.java, delegate.artifacts, stats, objectFactory)
    }
  ...
  override val instrumentation: Instrumentation
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.INSTRUMENTATION_VALUE
      return objectFactory.newInstance(AnalyticsEnabledInstrumentation::class.java, delegate.instrumentation, stats, objectFactory)
    }

  override val compileClasspath: FileCollection
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.COMPILE_CLASSPATH_VALUE
      return delegate.compileClasspath
    }

  override val compileConfiguration: Configuration
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.COMPILE_CONFIGURATION_VALUE
      return delegate.compileConfiguration
    }

  override val runtimeConfiguration: Configuration
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.RUNTIME_CONFIGURATION_VALUE
  ...
  override fun <ParamT : InstrumentationParameters> transformClassesWith(
    ...
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.ASM_TRANSFORM_CLASSES_VALUE
    stats
      ...
      .build()
    delegate.instrumentation.transformClassesWith(classVisitorFactoryImplClass, scope, instrumentationParamsConfig)
  }

  @Deprecated("Will be removed in v9.0, use the instrumentation block.")
  override fun setAsmFramesComputationMode(mode: FramesComputationMode) {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.ASM_FRAMES_COMPUTATION_NODE_VALUE
  ...
  override val buildType: String?
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.BUILD_TYPE_VALUE
      return delegate.buildType
    }

  override val productFlavors: List<Pair<String, String>>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.PRODUCT_FLAVORS_VALUE
      return delegate.productFlavors
    }

  override val flavorName: String?
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.FLAVOR_NAME_VALUE
      return delegate.flavorName
    }

  override val namespace: Provider<String>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.NAMESPACE_VALUE
  ...
  override val lifecycleTasks: LifecycleTasks
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.LIFECYCLE_TASKS_VALUE
      return objectFactory.newInstance(AnalyticsEnabledLifecycleTasks::class.java, delegate.lifecycleTasks, stats)
    }

  override fun computeTaskName(action: String, subject: String): String {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.COMPUTE_TASK_NAME_VALUE
  ...
  override fun getResolvableConfiguration(sourceSetConfigurationsAffix: String): Configuration {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.GET_RESOLVABLE_CONFIGURATION_VALUE
  ...
  override fun configureJavaCompileTask(action: (JavaCompile) -> Unit) {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.CONFIGURE_JAVA_COMPILE_TASK_VALUE
