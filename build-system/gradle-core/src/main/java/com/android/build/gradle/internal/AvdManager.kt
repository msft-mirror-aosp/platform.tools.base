...
class AvdManager(
  ...
  private val deviceManager = DeviceManager.createInstance(androidLocationsProvider, sdkDirectory.toPath(), logger)

  private val avdManager =
    com.android.sdklib.internal.avd.AvdManager.createInstance(sdkHandler, sdkHandler.toCompatiblePath(avdFolder), deviceManager, logger)

  fun createOrRetrieveAvd(imageProvider: Provider<Directory>, imageHash: String, deviceName: String, hardwareProfile: String): File {
    ...
    return runWithMultiProcessLocking(deviceName) {
      ...
      info?.let {
        ...
        logger.warning("$deviceName needs to be recreated because it is invalid " + "(AvdStatus = ${info.status})")
        avdManager.deleteAvd(info)
      }

      val newInfo = createAvd(imageProvider, imageHash, deviceName, hardwareProfile)
      return@runWithMultiProcessLocking newInfo?.configFile?.toFile() ?: error("AVD could not be created.")
    }
  }

  private fun <V> runWithMultiProcessLocking(deviceName: String, runnable: () -> V): V {
    return SynchronizedFile.getInstanceWithMultiProcessLocking(avdFolder.resolve(deviceName)).write { runnable() }
  }

  private fun <V> runWithMultiProcessReadLocking(deviceName: String, runnable: () -> V): V {
    return SynchronizedFile.getInstanceWithMultiProcessLocking(avdFolder.resolve(deviceName)).read { runnable() }
  }
  ...
  internal fun createAvd(imageProvider: Provider<Directory>, imageHash: String, deviceName: String, hardwareProfile: String): AvdInfo? {
  ...
  /**
   * Returns a list of available hardware profile names. The returned list contains up to [maxSuggestions] hardware profile names that is
   * sorted by edit-distance. Candidates whose edit-distance is farther than [maxEditDistance] will be excluded.
   */
  ...
  fun loadSnapshotIfNeeded(deviceName: String) {
    ...
    runWithMultiProcessLocking(deviceName) {
      if (snapshotHandler.checkSnapshotLoadable(deviceName, avdFolder, logger)) {
        logger.verbose("Snapshot already exists for device $deviceName")
        return@runWithMultiProcessLocking
      }

      deviceLockManager.lockAndExecute(1) { snapshotHandler.generateSnapshot(deviceName, avdFolder, avdManager, logger) }
   ...
   *
   * This function safely launches multiple instances of a given AVD configuration. It blocks the calling thread until the AVDs have booted
   * and are online. Once ready, it executes the [onDevicesReady] lambda, providing a list of the online device serial numbers.
   *
   * After the [onDevicesReady] block completes, the AVDs are automatically shut down, and all underlying locks are released.
   *
      ...
      deviceLockManager.lockAndExecute(desiredDeviceCount) { deviceLock ->
        ...
        fun startEmulatorsThenStop(numberOfRemainingDevices: Int): T {
          return if (numberOfRemainingDevices == 0) {
            ...
            snapshotHandler.startEmulatorThenStop(false, deviceName, avdFolder, logger) { deviceSerial ->
  ...
  fun allAvds(): List<String> {
    avdManager.reloadAvds()
    return avdManager.allAvds.map { it.name }
  }
   ...
   *
   * This deletes the tracking file for all managed devices. It does not check if managed devices are currently running. Should be used if
   * the tracking file becomes stale.
   */
  ...
  fun closeOpenEmulators(idPrefix: String) {
    adbHelper.findAllDeviceSerialsWithIdPrefix(idPrefix).forEach { serial -> adbHelper.killDevice(serial) }
  }
  ...
  private fun defaultHardwareConfig(): MutableMap<String, String> {
    ...
    val emulatorLib =
      if (emulatorProvider.isPresent) {
        ...
        error("AVD Emulator package is not downloaded. Failed to retrieve hardware defaults" + " for virtual device.")
      }

    val libDirectory = File(emulatorLib, SdkConstants.FD_LIB)
    val hardwareDefs = File(libDirectory, SdkConstants.FN_HARDWARE_INI)
    val hwMap =
      HardwareProperties.parseHardwareDefinitions(PathFileWrapper(sdkHandler.toCompatiblePath(hardwareDefs)), logger)
   ...
   *
   * This attempts to retrieve the system image from the file system. The retries are required to mitigate an issue on Windows where the
   * download of the system image has occurred, but the file system has yet to be updated.
   */
  private fun retrieveSystemImage(sdkHandler: AndroidSdkHandler, imageLocation: Path): com.android.sdklib.ISystemImage? {
    var delay = BASE_RETRY_DELAY_SECONDS
    for (retry in 0..MAX_SYSTEM_IMAGE_RETRIES) {
      val systemImage = sdkHandler.getSystemImageManager(LoggerProgressIndicatorWrapper(logger)).getImageAt(imageLocation)
      ...
      if (retry != MAX_SYSTEM_IMAGE_RETRIES) {
        logger.warning("Failed to to retrieve system image at: $imageLocation " + "Retrying in $delay seconds")
  ...
  private val defaultEmulatorPropertiesMap: Map<String, String> =
    mapOf(
      ...
      EmulatedProperties.CPU_CORES_KEY to EmulatedProperties.RECOMMENDED_NUMBER_OF_CORES.toString(),
      ...
      EmulatedProperties.INTERNAL_STORAGE_KEY to EmulatedProperties.DEFAULT_INTERNAL_STORAGE.toString(),
