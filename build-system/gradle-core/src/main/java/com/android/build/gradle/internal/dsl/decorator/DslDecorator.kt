 ...
 *
 * Given an abstract class, calling [decorate] will return a generated subclass which has anything that was abstract and is included in the
 * [supportedPropertyTypes] implemented.
 */
class DslDecorator(supportedPropertyTypes: List<SupportedPropertyType>) {
  ...
  internal fun <T : Any> decorateDslClassImpl(dslClass: Class<T>): Class<out T> {
    ...
    val generatedClass = Type.getType(dslClassType.descriptor.removeSuffix(";") + "\$AgpDecorated;")
    ...
    val lazyInitializationMethods = dslClass.declaredMethods.filter { it.isAnnotationPresent(WithLazyInitialization::class.java) }
    ...
    for (constructor in constructors) {
      ...
      GeneratorAdapter(constructor.modifiers, methodToGenerate, null, null, classWriter).apply {
        ...
        for (property in abstractProperties) {
          when (val type = property.supportedPropertyType) {
            ...
            is SupportedPropertyType.Var -> {
              /* Defaults to field default value */
            }
            is SupportedPropertyType.Block -> {
              ...
              visitArrayOf { loadArg(dslServicesArgumentIndex) }
        ...
        lazyInitializationMethods.forEach {
          loadThis()
          invokeVirtual(generatedClass, Method(it.name, Type.VOID_TYPE, arrayOf()))
        }
  ...
  private val ManagedProperty.guardedBy: BooleanOption?
    get() {
      val optionsFromAnnotation =
        (gettersAnnotations + settersAnnotations).filterIsInstance<RuntimeGuardedExperimentalApi>().map { it.enableFlag }.toSet()
      return when {
        optionsFromAnnotation.isEmpty() -> null
        optionsFromAnnotation.size == 1 -> optionsFromAnnotation.single()
        else ->
          throw IllegalStateException(
            "Expected single enable flag in RuntimeGuardedExperimentalApi for property " +
              this.name +
              " but found multiple ${optionsFromAnnotation.map { it.name }}"
          )
      }
    }
  ...
  class CollectingInfoVisitor : ClassInfoVisitor {
    ...
    private fun MutableMap<String, MutableList<java.lang.reflect.Method>>.recordMethod(name: String, method: java.lang.reflect.Method) {
    ...
    override fun method(method: java.lang.reflect.Method) {
      ...
      when {
        ...
        method.name.startsWith("is") && method.name.length > 2 && method.name[2].isUpperCase() -> {
          if (method.parameterCount != 0) return
        ...
        method.returnType === Void.TYPE && method.parameterCount == 1 && Type.getType(method.parameterTypes[0]) == KOTLIN_FUNCTION1 -> {
    ...
    return visitor.getters.mapNotNull { (propertyName, getters) ->
      ...
      for (getter in getters) {
        ...
        if (getterSupportedPropertyType != supportedPropertyType) {
          throw IllegalStateException(
            "Invalid abstract property '$propertyName' - ambiguous property mapping to multiple supported property types - $getterSupportedPropertyType & $supportedPropertyType"
          )
        }
      }
      ...
      val blockToGenerate =
        visitor.blocks[propertyName]?.also {
  ...
  private fun createLockField(classWriter: ClassWriter) {
    classWriter.visitField(Opcodes.ACC_PRIVATE, LOCK_FIELD_NAME, Type.BOOLEAN_TYPE.descriptor, null, false).visitEnd()
  }

  private fun createLockMethod(classWriter: ClassWriter, generatedClass: Type, abstractProperties: List<ManagedProperty>) {
  ...
  private fun createExperimentalGuardField(classWriter: ClassWriter) {
    /**
     * Create __experimental_guard__ array which will contain the state of each BooleanOption used to require opt-in for each experimental
     * DSL element in the block currently being decorated.
     *
     * Indexes in the array are determined at generation time, from the order in experimentalGuards (List<BooleanOption>), used in both
     * [populateExperimentalGuardsBooleanArray] and [checkExperimentalGuard].
     */
    classWriter.visitField(Opcodes.ACC_PRIVATE, EXPERIMENTAL_GUARD_FIELD_NAME, "[Z", null, false).visitEnd()
  }

  private fun GeneratorAdapter.populateExperimentalGuardsBooleanArray(
    generatedClass: Type,
    dslServicesArgumentIndex: Int,
  ...
  private fun createField(classWriter: ClassWriter, managedProperty: ManagedProperty) {
    classWriter
      .visitField(
        Opcodes.ACC_PRIVATE,
      ...
      is SupportedPropertyType.Var -> {
        for (setter in property.settersToGenerate) {
          createFieldBackedSetter(
            ...
            property,
            setter,
            disallowNullableValue = property.settersAnnotations.any { it.annotationClass == NonNullableSetter::class },
            experimentalGuards,
          )
        }
      }
  ...
  private fun createFieldBackedSetter(
    ...
    check(type.implementationType == setter.argumentTypes[0]) { "Currently only setters that use the same type are supported." }
  ...
  private fun createGroovyMutatingSetter(
    ...
    val extraSetter = Method("set${property.name.usLocaleCapitalize()}", Type.VOID_TYPE, arrayOf(property.supportedPropertyType.type))
    if (property.supportedPropertyType == SupportedPropertyType.Collection.Map) {
      GeneratorAdapter(Opcodes.ACC_PUBLIC.or(Opcodes.ACC_SYNTHETIC), extraSetter, null, null, classWriter).apply {
        ...
        loadArg(0)
        checkCast(MAP)
        invokeConstructor(HASH_MAP, HASH_MAP_COPY_CONSTRUCTOR)
        loadThis()
        getField(generatedClass, property.backingFieldName, property.supportedPropertyType.implementationType)
        dup()
        invokeVirtual(property.supportedPropertyType.implementationType, CLEAR)
        swap()
        invokeVirtual(property.supportedPropertyType.implementationType, PUT_ALL)
        returnValue()
        endMethod()
      }
    } else {
      GeneratorAdapter(Opcodes.ACC_PUBLIC.or(Opcodes.ACC_SYNTHETIC), extraSetter, null, null, classWriter).apply {
        ...
        invokeConstructor(ARRAY_LIST, ARRAY_LIST_COPY_CONSTRUCTOR)
        loadThis()
        getField(generatedClass, property.backingFieldName, property.supportedPropertyType.implementationType)
        dup()
        invokeVirtual(property.supportedPropertyType.implementationType, CLEAR)
        swap()
        invokeVirtual(property.supportedPropertyType.implementationType, ADD_ALL)
        returnValue()
        endMethod()
      }
    }
  ...
  private fun GeneratorAdapter.checkExperimentalGuard(
    generatedClass: Type,
    property: ManagedProperty,
    experimentalGuards: List<BooleanOption>,
  ) {
  ...
  private fun createFieldBackedGetter(
    ...
    val access =
      when (getter.returnType) {
        type.type,
        type.implementationType -> property.access
  ...
  companion object {
    ...
    private val LOCKABLE_CONSTRUCTOR = Method("<init>", Type.VOID_TYPE, arrayOf(Type.getType(String::class.java)))
    ...
    private val DSL_SERVICES_NEW_DECORATED_INSTANCE = Method("newDecoratedInstance", OBJECT_TYPE, arrayOf(CLASS, ARRAY))
    private val PROJECT_OPTIONS_TYPE = Type.getType(ProjectOptions::class.java)
    private val DSL_SERVICES_PROJECT_OPTIONS = Method("getProjectOptions", PROJECT_OPTIONS_TYPE, arrayOf())
    private val BOOLEAN_OPTION_TYPE = Type.getType(BooleanOption::class.java)
    private val PROJECT_OPTIONS_GET_BOOLEAN_OPTION = Method("get", Type.BOOLEAN_TYPE, arrayOf(BOOLEAN_OPTION_TYPE))
    ...
    private val PRECONDITIONS_CHECK_NOT_NULL = Method("checkNotNull", OBJECT_TYPE, arrayOf(OBJECT_TYPE))
