...
/**
 * Returns `true` if any of the Kotlin plugins (e.g., [KOTLIN_ANDROID_PLUGIN_ID] or [KOTLIN_KAPT_PLUGIN_ID]) is applied *AND* it is loaded
 * in the same classloader as AGP.
 *
 * Here is a likely scenario that causes AGP and KGP to be loaded in different classloaders:
 * - The project uses buildSrc and the buildSrc project has an `implementation` dependency on AGP, but not KGP. Because AGP has a
 *   `compileOnly` dependency on KGP, at runtime the buildSrc classloader loads AGP only, not KGP.
 * - The main project applies KGP. Gradle loads KGP in a child classloader of the buildSrc classloader.
 * - Classes in the buildSrc classloader (AGP) can't access classes in the child classloader (KGP).
 *
 * If we want to check whether a specific Kotlin plugin is applied and check it across classloaders, use a different method instead (e.g.,
 * [isKotlinAndroidPluginApplied] or [isKotlinKaptPluginApplied]).
 */
...
/**
 * returns the kotlin plugin version, or null if plugin is not applied to this project or if plugin is applied but version can't be
 * determined.
 */
...
/**
 * returns the kotlin plugin version as string, or null if plugin is not applied to this project, or "unknown" if plugin is applied but
...
fun getKotlinAndroidPluginVersion(project: Project): String? {
  ...
  val plugin = project.plugins.findPlugin("kotlin-android") ?: kotlinBaseApiPlugin ?: return null
...
fun getKotlinPluginVersionFromPlugin(plugin: Plugin<*>): String? {
  return try {
    ...
    val method = plugin.javaClass.methods.first { it.name == "getKotlinPluginVersion" || it.name == "getPluginVersion" }
...
fun isKotlinAndroidPluginApplied(project: Project) = project.pluginManager.hasPlugin(KOTLIN_ANDROID_PLUGIN_ID)

fun isKotlinKaptPluginApplied(project: Project) = project.pluginManager.hasPlugin(KOTLIN_KAPT_PLUGIN_ID)

fun isComposeCompilerPluginApplied(project: Project) = project.pluginManager.hasPlugin(COMPOSE_COMPILER_PLUGIN_ID)

fun isKspPluginApplied(project: Project) = project.pluginManager.hasPlugin(KSP_PLUGIN_ID)
...
fun recordKotlinCompilePropertiesForAnalytics(kotlinCompile: KotlinCompile, creationConfig: ComponentCreationConfig) {
  getLanguageVersionUnsafe(kotlinCompile)?.let { languageVersion ->
    getBuildService(creationConfig.services.buildServiceRegistry, AnalyticsConfiguratorService::class.java)
      .get()
      .getVariantBuilder(creationConfig.services.projectInfo.path, creationConfig.name)
      ?.apply { setKotlinOptions(GradleBuildVariant.KotlinOptions.newBuilder().setLanguageVersion(languageVersion)) }
  }
}

/**
 * Attempts to return the language version of [kotlinCompile]. If we fail to access the property (e.g., because the compile and runtime
 * versions of KGP differ), this method will return `null`.
 */
...
fun addComposeArgsToKotlinCompile(task: KotlinCompile, compilerExtension: FileCollection) {
...
/**
 * Adds Kotlin compiler argument `-Xuse-inline-scopes-numbers` to improve the debugging experience in Android Studio (b/372264148).
 *
 * Note: When the Kotlin compiler enables this option by default (KT-79401), we can remove this method.
 */
fun maybeUseInlineScopesNumbers(task: KotlinCompile, creationConfig: ComponentCreationConfig, logger: Logger) {
...
private fun KotlinCompile.addPluginOption(pluginId: String, key: String, value: String) {
  ...
  pluginOptions.add(CompilerPluginConfig().apply { addPluginArgument(pluginId, SubpluginOption(key, value)) })
}
...
/**
 * Add the Compose Compiler Gradle Plugin's sourceInformation flag only if the kotlin version is below 2.1.20-Beta2. Starting at that
 * version, the Compose Compiler Gradle Plugin adds the flag itself (see https://youtrack.jetbrains.com/issue/KT-74415).
 */
private fun KotlinCompile.maybeAddSourceInformationOption(kotlinVersion: KotlinVersion?) {
  ...
  addPluginOption("androidx.compose.compiler.plugins.kotlin", "sourceInformation", "true")
}
...
fun handleKotlinSourceSets(
  ...
  fun reportErrorIfKotlinSourceSetsAreUsed(kotlinSourceSets: NamedDomainObjectContainer<KotlinSourceSet>) {
    androidSourceSets.forEach { androidSourceSet ->
      ...
      if (kotlinSourceSet.kotlin.srcDirs.isNotEmpty()) {
        ...
        val nonDefaultKotlinSrcDirs =
          kotlinSourceSet.kotlin.srcDirs.filterNot { it.invariantSeparatorsPath.endsWith("${kotlinSourceSet.name}/kotlin") }
  ...
  fun syncAndroidAndKotlinSourceSets() {
    ...
    androidSourceSets.forEach { androidSourceSet ->
      ...
      @Suppress("DEPRECATION") androidSourceSet.kotlin.setSrcDirs(kotlinSourceSet.kotlin.srcDirs)
    }
  }
...
/**
 * Handles the case where the user adds a Kotlin dependency without a version (b/443037365, b/471410336). In that case, we will set a
 * default version.
 *
...
internal fun handleKotlinDependenciesWithoutVersion(
  ...
  val defaultKotlinVersion = projectServices.builtInKotlinServices.kotlinAndroidProjectExtension.coreLibrariesVersion
...
/**
 * Adds kotlin-stdlib dependency to the `api` configurations if it's not yet added by the user, and they haven't set
 * `kotlin.stdlib.default.dependency=false` (b/452246814).
 *
 * This is similar to the `kotlin-android` plugin's behavior:
...
internal fun maybeAddKotlinStdlibDependency(
  ...
  if (mainSourceSet.getConfigurations(project).any { it.hasKotlinStdlib() } || kotlinStdlibDefaultDependencyProperty() == false) {
    return
  }
  ...
  val defaultKotlinVersion = projectServices.builtInKotlinServices.kotlinAndroidProjectExtension.coreLibrariesVersion
  apiConfiguration.dependencies.add(project.dependencies.create("$KOTLIN_GROUP:$KOTLIN_STDLIB:$defaultKotlinVersion"))
}

/** Returns all [Configuration]s associated with this [AndroidSourceSet] where the user can declare dependencies. */
...
/**
 * Attempts to find the corresponding `kapt` or `ksp` configurations for the source sets of the given variant. The returned list may be
 * incomplete or empty if unsuccessful.
 */
fun findKaptOrKspConfigurationsForVariant(creationConfig: ComponentCreationConfig, kaptOrKsp: String): List<Configuration> {
  return creationConfig.sources.sourceProviderNames.mapNotNull { sourceSetName ->
    val configurationName = if (sourceSetName != SourceSet.MAIN_SOURCE_SET_NAME) kaptOrKsp.appendCapitalized(sourceSetName) else kaptOrKsp
