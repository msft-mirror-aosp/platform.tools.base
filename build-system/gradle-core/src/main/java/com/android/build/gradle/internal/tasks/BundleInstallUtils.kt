...
internal fun getDeviceSpec(device: DeviceConfigProvider): Devices.DeviceSpec {
  return Devices.DeviceSpec.newBuilder()
    .also { spec ->
...
internal fun getApkFiles(apkBundle: Path, device: DeviceConfigProvider, moduleName: String? = null): List<Path> {
  return getApkFiles(setOf(apkBundle), device, moduleName)
}

internal fun getApkFiles(apkBundles: Collection<Path>, device: DeviceConfigProvider, moduleName: String? = null): List<Path> {
  val deviceSpec: Devices.DeviceSpec = getDeviceSpec(device)

  return getApkFiles(apkBundles, deviceSpec, moduleName)
}

internal fun getApkFiles(apkBundles: Collection<Path>, deviceSpec: Devices.DeviceSpec, moduleName: String? = null): List<Path> {
  ...
  val apkPaths = apkBundles.flatMap { apkBundle ->
    val command = ExtractApksCommand.builder().setApksArchivePath(apkBundle).setDeviceSpec(deviceSpec)
...
internal fun extractApkFilesBypassingBundleTool(apkBundle: Path): List<Path> {
  ...
  ZipArchive(apkBundle).use { it.getInputStream("standalones/standalone.apk").use { inputStream -> Files.copy(inputStream, outputFile) } }
