...
abstract class LintParallelBuildService : BuildService<BuildServiceParameters.None> {
  ...
  companion object {
    ...
    private fun calculateMaxParallelUsagesInProcess(projectOptions: ProjectOptions, maxRuntimeMemory: Long): Int {
      // We reserve 512 megabytes per lint task, unless the user specifies a different value.
      val memoryPerLintTask: Long =
        projectOptions.get(LINT_RESERVED_MEMORY_PER_TASK)?.let { parseMemorySize(it, LINT_RESERVED_MEMORY_PER_TASK.propertyName) }
          ?: (512 * 1024 * 1024)
    ...
    private fun calculateMaxParallelUsagesOutOfProcess(
      ...
      val maxParallelUsage =
        Math.floorDiv(
          totalPhysicalMemory,
          parseMemorySize(calculateLintHeapSize(projectOptions.get(LINT_HEAP_SIZE), maxRuntimeMemory), LINT_HEAP_SIZE.propertyName),
        )
    ...
    fun calculateLintHeapSize(userSpecifiedLintHeapSize: String?, maxRuntimeMemory: Long): String =
      userSpecifiedLintHeapSize ?: "${maxRuntimeMemory / 1024 / 1024}m"

    /** Parses a memory size (e.g., max heap) string (e.g., "512m" or "2g"), and returns the corresponding number of bytes as a Long. */
    private fun parseMemorySize(memorySize: String, propertyName: String): Long {
      ...
      val longOrNull =
        when {
          ...
          value.endsWith("g") -> {
            value.substring(0, value.length - 1).toLongOrNull()?.let { it * 1024 * 1024 * 1024 }
          }
          else -> {
            null
          }
        }
      return longOrNull ?: throw RuntimeException("Failed to parse $propertyName \"$memorySize\".")
    }
  }
}
...
private fun getTotalPhysicalMemory() =
  try {
    ...
    Logging.getLogger(LintParallelBuildService::class.java).info("Failed to read total available memory", t)
    null
  }
