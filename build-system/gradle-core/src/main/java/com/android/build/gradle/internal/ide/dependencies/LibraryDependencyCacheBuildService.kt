...
abstract class LibraryDependencyCacheBuildService : BuildService<LibraryDependencyCacheBuildService.Parameters>, AutoCloseable {
  ...
  val libraryCache =
    CreatingCache(
      CreatingCache.ValueFactory<ResolvedArtifact, Library> {
        artifactHandler
          .handleArtifact(
            artifact = it,
          ...
          .also { synchronized(globalLibrary) { globalLibrary[it.artifactAddress] = it } }
      }
    )
  ...
  private val artifactHandler = Level2ArtifactHandler(localJarCache, parameters.mavenCoordinatesCache.get())

  fun getGlobalLibMap(): Map<String, Library> {
    return ImmutableMap.copyOf(globalLibrary)
  }

  fun clone(dependencyGraphs: DependencyGraphs, modelLevel: Int, modelWithFullDependency: Boolean): DependencyGraphs {
    ...
    return if (modelWithFullDependency) {
      FullDependencyGraphsImpl(nodes, nodes, ImmutableList.of(), ImmutableList.of())
  ...
  class RegistrationAction(project: Project, private val mavenCoordinatesCache: Provider<MavenCoordinatesCacheBuildService>) :
    ServiceRegistrationAction<LibraryDependencyCacheBuildService, Parameters>(project, LibraryDependencyCacheBuildService::class.java) {
...
interface UsesLibraryDependencyCacheBuildService {
  ...
  @get:Internal val libraryDependencyCacheBuildService: Property<LibraryDependencyCacheBuildService>

  fun initializeLibraryDependencyCacheBuildService(task: Task) {
    getBuildService<LibraryDependencyCacheBuildService, LibraryDependencyCacheBuildService.Parameters>(task.project.gradle.sharedServices)
      .let {
