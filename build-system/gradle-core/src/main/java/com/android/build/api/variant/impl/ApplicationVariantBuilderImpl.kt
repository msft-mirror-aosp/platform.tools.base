...
open class ApplicationVariantBuilderImpl
@Inject
constructor(
...
) : VariantBuilderImpl(globalVariantBuilderConfig, dslInfo, componentIdentity, variantBuilderServices), ApplicationVariantBuilder {

  @Suppress("OVERRIDE_DEPRECATION") override var debuggable: Boolean = dslInfo.isDebuggable
  ...
  override fun <T : VariantBuilder> createUserVisibleVariantObject(
    ...
    if (stats == null) {
      ...
      projectServices.objectFactory.newInstance(AnalyticsEnabledApplicationVariantBuilder::class.java, this, stats) as T
    }

  override var isMinifyEnabled: Boolean = dslInfo.optimizationDslInfo.postProcessingOptions.codeShrinkerEnabled()

  override var shrinkResources: Boolean = dslInfo.optimizationDslInfo.postProcessingOptions.resourcesShrinkingEnabled()
  ...
  override val androidResources: ApplicationAndroidResourcesBuilder = ApplicationAndroidResourcesBuilderImpl(dslInfo.generateLocaleConfig)
