 ...
 *
 * This task is never up-to-date because AGP should print lint's text output (if there are issues and lintOptions is configured to print to
 * stdout or stderr) even if the other lint tasks are up-to-date.
 */
@DisableCachingByDefault
@BuildAnalyzer(primaryTaskCategory = TaskCategory.LINT)
abstract class AndroidLintTextOutputTask : NonIncrementalTask() {

  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val textReportInputFile: RegularFileProperty

  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val returnValueInputFile: RegularFileProperty

  @get:Optional @get:OutputFile abstract val lintVitalOutput: RegularFileProperty

  @get:Input abstract val outputStream: Property<OutputStream>
  ...
  @get:Input abstract val hasBaseline: Property<Boolean>
  ...
  class LintVitalCreationAction(variant: ComponentCreationConfig) : VariantCreationAction(variant) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<AndroidLintTextOutputTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, AndroidLintTextOutputTask::lintVitalOutput)
