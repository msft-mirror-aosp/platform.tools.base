...
/** Task installing an app variant. It looks at connected device and install the best matching variant output on each device. */
...
abstract class InstallVariantViaBundleTask : NonIncrementalTask() {
  ...
  abstract class InstallRunnable : ProfileAwareWorkAction<Params>(), BundleApkFetcher {
    override fun run() {
      ...
      val apkOutput =
        ViaBundleDeviceApkOutput(parameters.apkBundle, androidVersion, parameters.variantName.get(), parameters.projectPath.get(), this)
      ...
      deviceProvider.use {
        ...
        for (device in devices) {
          ...
          for (apkInstallGroup in apkInstallGroups) {
            val apkFiles = apkInstallGroup.apks.stream().map { obj: RegularFile -> obj.asFile }.collect(Collectors.toList())
        ...
        if (successfulInstallCount == 0) {
          ...
          logger.quiet("Installed on {} {}.", successfulInstallCount, if (successfulInstallCount == 1) "device" else "devices")
        }
      }
    }
  ...
  internal class CreationAction(creationConfig: ApkCreationConfig) :
    VariantTaskCreationAction<InstallVariantViaBundleTask, ApkCreationConfig>(creationConfig) {
    ...
    override fun configure(task: InstallVariantViaBundleTask) {
      ...
      creationConfig.global.installationOptions.installOptions.let { task.installOptions.addAll(it) }

      creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.APKS_FROM_BUNDLE, task.apkBundle)
    ...
    override fun handleProvider(taskProvider: TaskProvider<InstallVariantViaBundleTask>) {
