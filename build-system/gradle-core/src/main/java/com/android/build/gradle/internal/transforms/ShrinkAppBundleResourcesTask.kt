...
/**
 * Task to shrink resources inside Android app bundle. Consumes a bundle built by bundletool and replaces unused resources with dummy
 * content there and outputs bundle with replaced resources.
 *
...
abstract class ShrinkAppBundleResourcesTask : NonIncrementalTask() {
  ...
  @get:OutputFile abstract val shrunkBundle: RegularFileProperty

  @get:InputFile @get:PathSensitive(PathSensitivity.NAME_ONLY) abstract val originalBundle: RegularFileProperty

  @get:Input abstract val baseNamespace: Property<String>

  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val featureSetMetadata: RegularFileProperty

  @get:InputFiles @get:PathSensitive(PathSensitivity.NAME_ONLY) @get:Optional abstract val mappingFileSrc: RegularFileProperty

  override fun doTaskAction() {
    val modules = FeatureSetMetadata.load(featureSetMetadata.get().asFile).featureNameToNamespaceMap + ("base" to baseNamespace.get())
    ...
    workerExecutor.noIsolation().submit(ShrinkAppBundleResourcesAction::class.java) {
      ...
      if (mappingFileSrc.isPresent) {
        mappingFileSrc.get().asFile.parentFile?.let { logDir -> it.report.set(File(logDir, "resources.txt")) }
      }
    }
  }

  class CreationAction(creationConfig: ApplicationCreationConfig) :
    VariantTaskCreationAction<ShrinkAppBundleResourcesTask, ApplicationCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ShrinkAppBundleResourcesTask>) {
      creationConfig.artifacts
        .use(taskProvider)
        .wiredWithFiles(ShrinkAppBundleResourcesTask::originalBundle, ShrinkAppBundleResourcesTask::shrunkBundle)
    ...
    override fun configure(task: ShrinkAppBundleResourcesTask) {
      ...
      creationConfig.artifacts.setTaskInputToFinalProduct(SingleArtifact.OBFUSCATION_MAPPING_FILE, task.mappingFileSrc)
    }
  }
}
...
private abstract class ShrinkAppBundleResourcesAction @Inject constructor() : WorkAction<ResourceShrinkerParams> {
  ...
  override fun execute() {
    ...
    FileUtils.createZipFilesystem(originalBundleFile.toPath()).use { fs ->
      val proguardMappings = fs.getPath("BUNDLE-METADATA", "com.android.tools.build.obfuscation", "proguard.map")

      val resourcesGatherers = allModules.map { ProtoResourceTableGatherer(fs.getPath(it.key, "resources.pb")) }

      val obfuscationMappingsRecorder = proguardMappings.takeIf { Files.isRegularFile(it) }?.let { ProguardMappingsRecorder(it) }
      ...
      val usageRecorders =
        allModules
          .flatMap {
            ...
            listOf(
              DexUsageRecorder(dexPath).takeIf { Files.isDirectory(dexPath) },
              ToolsAttributeUsageRecorder(fs.getPath(it.key, "res", "raw")).takeIf { Files.isDirectory(rawResourcesPath) },
      ...
      ResourceShrinkerImpl(
        ...
        .use { shrinker ->
          ...
          shrinker.rewriteResourcesInBundleFormat(originalBundleFile, shrunkBundleFile, allModules)
          ...
          if (shrinker.unusedResourceCount > 0) {
            ...
            val stat =
              "Removed unused resources: Binary bundle size reduced from " +
