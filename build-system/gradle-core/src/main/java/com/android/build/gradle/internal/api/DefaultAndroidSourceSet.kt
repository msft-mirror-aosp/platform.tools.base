...
open class DefaultAndroidSourceSet
@Inject
constructor(
  name: String,
  ...
  @Deprecated("Unused") final override val jni: AndroidSourceDirectorySet
  ...
  init {
    ...
    kotlin = DefaultAndroidSourceDirectorySet(displayName, "Kotlin source", project, SourceArtifactType.KOTLIN_SOURCES)
    kotlin.filter.include("**/*.kt", "**/*.kts")

    resources = DefaultAndroidSourceDirectorySet(displayName, "Java resources", project, SourceArtifactType.JAVA_RESOURCES)
    resources.filter.exclude("**/*.java", "**/*.kt")

    manifest = DefaultAndroidSourceFile("$displayName manifest", project)

    assets = DefaultAndroidSourceDirectorySet(displayName, "assets", project, SourceArtifactType.ASSETS)

    res = DefaultAndroidSourceDirectorySet(displayName, "resources", project, SourceArtifactType.ANDROID_RESOURCES)

    aidl = DefaultAndroidSourceDirectorySet(displayName, "aidl", project, SourceArtifactType.AIDL)

    renderscript = DefaultAndroidSourceDirectorySet(displayName, "renderscript", project, SourceArtifactType.RENDERSCRIPT)

    baselineProfiles = DefaultAndroidSourceDirectorySet(displayName, "baselineProfiles", project, SourceArtifactType.BASELINE_PROFILES)
    ...
    jniLibs = DefaultAndroidSourceDirectorySet(displayName, "jniLibs", project, SourceArtifactType.JNI_LIBS)

    shaders = DefaultAndroidSourceDirectorySet(displayName, "shaders", project, SourceArtifactType.SHADERS)

    mlModels = DefaultAndroidSourceDirectorySet(displayName, "ML models", project, SourceArtifactType.ML_MODELS)
  ...
  @Suppress("OverridingDeprecatedMember") override val compileConfigurationName = sourceSetName.compileConfigurationName
  ...
  @Suppress("OverridingDeprecatedMember") override val providedConfigurationName = sourceSetName.providedConfigurationName

  override val annotationProcessorConfigurationName = sourceSetName.annotationProcessorConfigurationName
  ...
  override fun getCustomDirectories(): List<CustomSourceDirectory> {
    return extras.asMap.values.groupBy(DefaultAndroidSourceDirectorySet::getSourceSetName, DefaultAndroidSourceDirectorySet::srcDirs).map {
      ...
      CustomSourceDirectoryImpl(it.key, it.value.flatten().single())
    }
  }

  /**
   * Internal API to add customs folders to the DSL. Custom folders cannot be added through the public
   * [com.android.build.api.dsl.AndroidSourceSet] interface because it would allow users to add a custom directory for a specific source set
   * like 'main' or 'debug'.
   *
   * Since users cannot add have access to the created [DefaultAndroidSourceDirectorySet] for the source types, they cannot add to it.
   * Therefore there can only be one source directory per custom source type on an Android Source Set.
   *
   * Users need to the use the public [AndroidComponents.registerSourceSet] API so custom sources are added to all the source sets of the
  ...
  class AndroidSourceDirectorySetFactory(
    ...
    override fun create(name: String): DefaultAndroidSourceDirectorySet {
      return DefaultAndroidSourceDirectorySet(sourceSetDisplayName, name, project, SourceArtifactType.CUSTOMS).also {
