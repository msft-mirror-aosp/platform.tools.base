 ...
 * 1. When we want to put the smallest number of dex files in the APK (e.g., in release or legacy multidex builds). In this case, this task
 *    merges all the input dex files at once.
 * 2. When we can put a large number of dex files in the APK but have to keep them within a certain limit (e.g., in native multidex debug
 *    builds). In this case, for incrementality and parallelism, this task splits the input dex files into buckets, and merges each bucket
 *    in a Gradle work action. (The merged dex files of the buckets are then copied to the APK by another task without further merging). In
 *    an incremental build, this task re-merges only the impacted buckets (those containing changed input dex files).
 */
@CacheableTask
...
abstract class DexMergingTask : NewIncrementalTask() {
  ...
  /** Parameters shared between [DexMergingTask], [DexMergingTaskDelegate], and [DexMergingWorkAction]. */
  abstract class SharedParams {

    @get:Input abstract val dexingType: Property<DexingType>
    ...
    @get:Nested abstract val mainDexListConfig: MainDexListConfig

    @get:Input abstract val useThreadPool: Property<Boolean>
    ...
    abstract class MainDexListConfig {
      ...
      @get:Optional @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val aaptGeneratedRules: RegularFileProperty
      ...
      @get:Optional @get:Input abstract val platformMultidexProguardRules: ListProperty<String>

      @get:Optional @get:InputFile @get:PathSensitive(PathSensitivity.NONE) abstract val userMultidexKeepFile: RegularFileProperty

      @get:Optional @get:Classpath abstract val libraryClasses: ConfigurableFileCollection
    }
  }

  @get:Nested abstract val sharedParams: SharedParams

  @get:Input abstract val numberOfBuckets: Property<Int>
  ...
  @get:InputFiles @get:Optional @get:PathSensitive(PathSensitivity.RELATIVE) abstract val globalSynthetics: ConfigurableFileCollection

  // Fake folder, used as a way to set up dependency
  @get:Optional @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val duplicateClassesCheck: DirectoryProperty
  ...
  @get:Optional @get:OutputFile abstract val d8Metadata: RegularFileProperty
  ...
  class CreationAction @JvmOverloads constructor(creationConfig: DexMergingCreationConfig) :
    ...
    override fun handleProvider(taskProvider: TaskProvider<DexMergingTask>) {
      ...
      creationConfig.artifacts.use(taskProvider).wiredWith(DexMergingTask::outputDir).toAppendTo(creationConfig.outputType)
      ...
      if (creationConfig.action == MERGE_ALL) {
        creationConfig.artifacts.setInitialProvider(taskProvider, DexMergingTask::d8Metadata).on(InternalArtifactType.D8_METADATA)
      }
    }
    ...
    override fun configure(task: DexMergingTask) {
      ...
      task.sharedParams.minSdkVersion.setDisallowChanges(dexingCreationConfig.minSdkVersionForDexing)
      task.sharedParams.debuggable.setDisallowChanges(creationConfig.debuggable)
      task.sharedParams.errorFormatMode.setDisallowChanges(SyncOptions.getErrorFormatMode(projectOptions))
      if (dexingType === LEGACY_MULTIDEX) {
        ...
        task.sharedParams.mainDexListConfig.userMultidexKeepFile.setDisallowChanges(dexingCreationConfig.multiDexKeepFile)

        task.sharedParams.mainDexListConfig.platformMultidexProguardRules.setDisallowChanges(getPlatformRules())
      ...
      if (creationConfig.separateFileDependenciesDexingTask) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.EXTERNAL_FILE_LIB_DEX_ARCHIVES, task.fileDependencyDexDir)
      }
      if (creationConfig.enableGlobalSynthetics && dexingType != NATIVE_MULTIDEX) {
        task.globalSynthetics.from(getGlobalSyntheticsInput(creationConfig))
      }
      if (projectOptions[BooleanOption.ENABLE_DUPLICATE_CLASSES_CHECK]) {
        creationConfig.artifacts.setTaskInputToFinalProduct(InternalArtifactType.DUPLICATE_CLASSES_CHECK, task.duplicateClassesCheck)
      }

      if (!creationConfig.debuggable) {
        task.inputProfileForDexStartupOptimization.set(creationConfig.artifacts.get(InternalArtifactType.MERGED_STARTUP_PROFILE))
      }
    }

    private fun getDexDirs(creationConfig: DexMergingCreationConfig): FileCollection {
        ...
        when (action) {
          ...
          MERGE_LIBRARY_PROJECTS -> {
            return if (creationConfig.dexingUsingArtifactTransforms) {
              ...
              val classesLibraryElements = creationConfig.services.named(LibraryElements::class.java, LibraryElements.CLASSES)
              val updatedAttributes =
                attributes +
                  AndroidAttributes(namedAttributes = mapOf(LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE to classesLibraryElements))
              creationConfig.variantDependencies.getArtifactFileCollection(
                AndroidArtifacts.ConsumedConfigType.RUNTIME_CLASSPATH,
              ...
              creationConfig.services.fileCollection(creationConfig.artifacts.get(InternalArtifactType.SUB_PROJECT_DEX_ARCHIVE))
            }
          }
          MERGE_PROJECT -> {
          ...
          MERGE_TRANSFORMED_CLASSES -> {
            ...
            return creationConfig.services.fileCollection(creationConfig.artifacts.get(InternalArtifactType.PROJECT_DEX_ARCHIVE))
          }
        }
      }
    ...
    private fun getNumberOfBuckets(projectOptions: ProjectOptions): Int {
      return when (creationConfig.action) {
        MERGE_ALL,
        MERGE_EXTERNAL_LIBS,
        MERGE_TRANSFORMED_CLASSES -> 1 // No bucketing
        MERGE_PROJECT,
        MERGE_LIBRARY_PROJECTS -> {
          val customNumberOfBuckets = projectOptions.getProvider(IntegerOption.DEXING_NUMBER_OF_BUCKETS).orNull
          if (customNumberOfBuckets != null) {
            check(customNumberOfBuckets >= 1) {
              "The value of ${IntegerOption.DEXING_NUMBER_OF_BUCKETS.propertyName}" + " is invalid (must be >= 1): $customNumberOfBuckets"
            }
    ...
    /**
     * Returns the number of buckets based on a number of factors (e.g., dex file limit and number of workers).
     *
     * Note that while increasing this number gives more fine-grained incrementality and more parallelism, it could also have the following
     * negative effects:
     * 1. Increased risk of hitting the dex file limit.
     * 2. Overhead of launching work actions and D8 invocations to process the too many small/empty buckets.
     */
    private fun getNumberOfBuckets(dexingType: DexingType, minSdkVersion: Int): Int {
      return min(getMaxNumberOfBucketsBasedOnDexFileLimit(dexingType, minSdkVersion), getRecommendedNumberOfBucketsBasedOnWorkers())
    }

    /**
     * Returns the maximum number of buckets based on the dex file limit. (In this context, the dex file limit means the maximum number of
     * dex files that can be put in an APK, not the 64K method reference limit within a dex file.)
     *
     ...
     *
     * When changing this number, we'll need to consider the effects documented at [getNumberOfBuckets].
     */
    private fun getMaxNumberOfBucketsBasedOnDexFileLimit(dexingType: DexingType, minSdkVersion: Int): Int {
...
abstract class DexMergingTaskDelegate : ProfileAwareWorkAction<DexMergingTaskDelegate.Params>() {
  ...
  abstract class Params : Parameters() {
    ...
    fun initialize(
      ...
      inputProfileForDexStartupOptimization?.let { this.inputProfileForDexStartupOptimization.set(it) }
    }
  }
  ...
  override fun run() {
    with(parameters) {
      val buckets = getBucketsToMerge(dexDirsOrJars.get(), numberOfBuckets.get(), incremental.get(), fileChanges.orNull)
      ...
      for ((bucketNumber, bucket) in buckets) {
        ...
        // multidex case”).
        val outputDirForBucket =
          if (numberOfBuckets.get() == 1) {
        ...
        workQueue.submit(DexMergingWorkAction::class.java) {
          ...
          it.initialize(
            ...
            inputProfileForDexStartupOptimization = inputProfileForDexStartupOptimization.asFile.orNull,
          )
        }
      }
    }
  ...
  companion object {
    ...
    private val isDexFile: (relativePath: String) -> Boolean = { it.endsWith(SdkConstants.DOT_DEX, ignoreCase = true) }
     ...
     *
     * In an incremental build, this method returns only the buckets that are impacted by the file changes.
     *
    ...
    private fun getBucketsToMerge(
      ...
      for (dexDirOrJar in dexDirsOrJars) {
        val dexEntryRelativePaths =
          if (dexDirOrJar.isDirectory) {
            getSortedFilesInDir(dexDirOrJar.toPath(), isDexFile).map { dexDirOrJar.toPath().relativize(it).toString() }
          } else {
    ...
    private fun getImpactedBuckets(fileChanges: SerializableFileChanges, numberOfBuckets: Int): Set<Int> {
      val hasModifiedRemovedJars = (fileChanges.modifiedFiles + fileChanges.removedFiles).find { isJarFile(it.file) } != null
      ...
      val relativePathsOfDexFilesInAddedJars = addedJars.flatMap { getSortedRelativePathsInJar(it, isDexFile) }
      val relativePathsOfChangedDexFilesInDirs = fileChanges.fileChanges.map { it.normalizedPath }.filter { isDexFile(it) }
     ...
     *
     * Note that classes of the same package must be put in the same bucket, so that they can be put in the same merged dex file by D8 (the
     * merged dex files of the buckets are then copied to the APK by another task without further merging).
     * - This requirement is documented in a comment in the "Instant Run implementation" design doc (but not Instant Run specific): "there
     *   was a verifier error in preN where the virtual machine would fail to verify classes from the same package that are split in 2 dex
     *   files".
    ...
    fun getBucketNumber(relativePath: String, numberOfBuckets: Int): Int {
      check(!File(relativePath).isAbsolute) { "Expected relative path but found absolute path: $relativePath" }
      check(relativePath.endsWith(SdkConstants.DOT_DEX, ignoreCase = true)) { "Expected .dex file but found: $relativePath" }
...
abstract class DexMergingWorkAction : ProfileAwareWorkAction<DexMergingWorkAction.Params>() {
  ...
  override fun run() {
    ...
    val (d8ExecutorService, shouldCloseExecutorService) =
      if (useThreadPool) {
  ...
  private fun merge(
    ...
    val messageReceiver = MessageReceiverImpl(sharedParams.errorFormatMode.get(), Logging.getLogger(DexMergingTask::class.java))
    ...
    try {
      ...
      sharedParams.mainDexListConfig.aaptGeneratedRules.asFile.orNull?.let { proguardRules.add(it.toPath()) }
      sharedParams.mainDexListConfig.userMultidexProguardRules.get().asSequence().map(RegularFile::getAsFile).map(File::toPath).forEach {
        proguardRules.add(it)
      }
