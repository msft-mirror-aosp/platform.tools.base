...
abstract class BaseDexingTransform<T : BaseDexingTransform.Parameters> : TransformAction<T> {
  ...
  interface Parameters : GenericTransformParameters {
    ...
    @get:Input val debuggable: Property<Boolean>
    @get:Input val enableDesugaring: Property<Boolean>
    @get:Classpath val bootClasspath: ConfigurableFileCollection
  ...
  @get:Classpath @get:Incremental @get:InputArtifact abstract val inputArtifact: Provider<FileSystemLocation>
  ...
  override fun transform(outputs: TransformOutputs) {
    ...
    val inputDirOrJar =
      inputArtifact.get().asFile.also { check(it.isDirectory || isJarFile(it)) { "Expected directory or jar but found: ${it.path}" } }
  ...
  private fun processIncrementally(
    ...
    val desugarGraph =
      DesugarGraph.read(desugarGraphFile = desugarGraphOutputFile, rootDir = inputDir, filesToIgnore = parameters.bootClasspath.files)
    ...
    (removedFiles + modifiedFiles + unchangedButImpactedFiles)
      ...
      .forEach { classFile ->
        ...
        DexFilePerClassFile.getDexOutputRelativePath(classFileRelativePath).let { FileUtils.deleteIfExists(dexOutputDir.resolve(it)) }
        globalSyntheticsOutputDir?.let {
          DexFilePerClassFile.getGlobalSyntheticOutputRelativePath(classFileRelativePath).let {
            FileUtils.deleteIfExists(globalSyntheticsOutputDir.resolve(it))
          }
        }
    ...
    val modifiedImpactedOrAddedFiles = (modifiedFiles + unchangedButImpactedFiles + addedFiles).toSet()
  ...
  private fun processNonIncrementally(
    ...
    process(inputDirOrJar, { _, _ -> true }, classpath, dexOutputDir, globalSyntheticsOutputDir, provideIncrementalSupport, desugarGraph)
  ...
  private fun process(
    ...
    Closer.create().use { closer ->
      val d8DexBuilder =
        DexArchiveBuilder.createD8DexBuilder(
          DexParameters(
            ...
            desugarBootclasspath = ClassFileProviderFactory(parameters.bootClasspath.files.map(File::toPath)).also { closer.register(it) },
            desugarClasspath = ClassFileProviderFactory(classpath?.map(File::toPath) ?: emptyList()).also { closer.register(it) },
      ...
      ClassFileInputs.fromPath(inputDirOrJar.toPath()).use { classFileInput ->
        classFileInput
          .entries { rootPath, relativePath -> inputFilter(rootPath.toFile(), relativePath) }
          .use { classesInput ->
            d8DexBuilder.convert(classesInput, dexOutputDir.toPath(), globalSyntheticsOutputDir?.toPath(), desugarGraph)
          }
      }
    }
  }
 ...
 *
 * This graph handles files with absolute paths. To make it relocatable, this graph internally maintains a [relocatableDesugarGraph] which
 * contains Unix-style relative paths of the files (the paths need to be *relative* and in *Unix style* so that they can be used across
 * filesystems). When writing this graph to disk, we will write the [relocatableDesugarGraph].
 *
 * To make it easier to convert absolute paths to relative paths, we currently require that all files in the graph share one single root
 * directory ([rootDir]).
 */
@VisibleForTesting
internal class DesugarGraph(
   ...
   *
   * Context (b/362339872): In some cases, D8 may report dependencies between the project's classes and android.jar, which is likely located
   * outside [rootDir]. Instead of failing the build, we can ignore those dependencies. It is safe to ignore because:
   * - [BaseDexingTransform.Parameters.bootClasspath] is a non-incremental transform input, so any changes to it will cause the transform to
   *   run non-incrementally (i.e., we don't need to track changes to `bootClasspath` and files that are impacted by those changes).
   * - `bootClasspath` is self-contained, it doesn't depend on anything else, so we don't need to track its dependencies.
   */
  ...
  override fun addEdge(dependent: File, dependency: File) {
    ...
    relocatableDesugarGraph.addEdge(dependent.toUnixStyleRelativePath(), dependency.toUnixStyleRelativePath())
  }
  ...
  private fun File.toUnixStyleRelativePath(): String {
    ...
    check(!unixStyleRelativePath.startsWith("..")) { "The given file '$path' is located outside the root directory '${rootDir.path}'" }

    return unixStyleRelativePath
  }

  fun write(desugarGraphFile: File) {
    ObjectOutputStream(desugarGraphFile.outputStream().buffered()).use { it.writeObject(relocatableDesugarGraph) }
  }
 ...
 * [com.android.build.gradle.internal.publishing.AndroidArtifacts.ArtifactScope.ALL]. However, this would impact build performance,
 * especially for incremental builds where `PROJECT` classes often change, causing all dexing transforms to rerun.
 *
 * To mitigate this performance impact while keeping the build correct, the full classpath will consist of the following instead:
 * 1. All external artifacts ([Parameters.externalArtifacts])
 * 2. The input artifact's dependencies provided by Gradle ([inputArtifactDependencies]) -- This will ensure that if some of the input
 *    artifact's dependencies have `PROJECT` scope (i.e., they are not part of [Parameters.externalArtifacts]), they will still be included
 *    in the full classpath. (The downside is that if some of the input artifact's dependencies have `EXTERNAL` scope, they will overlap
 *    with [Parameters.externalArtifacts], but that is fine.)
 */
@CacheableTransform
abstract class DexingWithFullClasspathTransform : BaseDexingTransform<DexingWithFullClasspathTransform.Parameters>() {

  @get:CompileClasspath @get:InputArtifactDependencies abstract val inputArtifactDependencies: FileCollection

  interface Parameters : BaseDexingTransform.Parameters {

    @get:CompileClasspath val externalArtifacts: ConfigurableFileCollection
  }

  override fun computeClasspathFiles() = inputArtifactDependencies.files.toList() + parameters.externalArtifacts.files
}
...
object DexingRegistration {
   ...
   *
   * Note: This class is a data class so that we can identify equivalent instances of this class (see [registerTransforms]).
   *
   * IMPORTANT: The properties of this class must be of primitive types (e.g., [Boolean], [Int], [String]) because the [getAttributes]
   * method relies on [toString], and the implementation of [toString] on non-primitive types are not well-defined and subject to change
   * (i.e., it can't be used to uniquely represent an object).
   */
  data class ComponentSpecificParameters(
    ...
    companion object {
      ...
      private fun needsDesugaring(dexing: DexingCreationConfig): Boolean = dexing.java8LangSupportType == Java8LangSupport.D8

      private fun needsClasspath(dexing: DexingCreationConfig): Boolean = needsDesugaring(dexing) && dexing.minSdkVersionForDexing < 24

      private fun useFullClasspath(creationConfig: DexingCreationConfig, services: BaseServices): Boolean =
        needsClasspath(creationConfig) && services.projectOptions.get(BooleanOption.USE_FULL_CLASSPATH_FOR_DEXING_TRANSFORM)
    }

    /**
     * Returns [AndroidAttributes] that uniquely represent the contents of this object.
     *
     * These attributes will be used when registering the transforms and when consuming the artifacts to ensure correct artifact
     * production/consumption.
     */
    fun getAttributes(): AndroidAttributes =
      AndroidAttributes(Attribute.of("dexing-component-attributes", String::class.java) to this.toString()) +
  ...
  fun registerTransforms(components: List<ComponentCreationConfig>, parameters: ComponentAgnosticParameters) {
    ...
    components
      ...
      .forEach { registerTransform(parameters, it) }
  }

  fun registerTransform(allComponents: ComponentAgnosticParameters, component: ComponentSpecificParameters) {
    @Suppress("UNCHECKED_CAST")
    val transformClass =
      when {
    ...
    allComponents.dependencyHandler.registerTransform(transformClass) { spec ->
      ...
      val inputArtifactType: AndroidArtifacts.ArtifactType =
        if (component.useJacocoTransformInstrumentation) {
          when {
            component.dependenciesClassesAreInstrumented -> AndroidArtifacts.ArtifactType.JACOCO_ASM_INSTRUMENTED_JARS
            !component.needsClasspath && !allComponents.disableIncrementalDexing -> AndroidArtifacts.ArtifactType.JACOCO_CLASSES
            else -> AndroidArtifacts.ArtifactType.JACOCO_CLASSES_JAR
          }
        } else {
          when {
            component.dependenciesClassesAreInstrumented -> AndroidArtifacts.ArtifactType.ASM_INSTRUMENTED_JARS
            !component.needsClasspath && !allComponents.disableIncrementalDexing -> AndroidArtifacts.ArtifactType.CLASSES
      ...
      if (component.useFullClasspath) {
        ...
        (spec.parameters as DexingWithFullClasspathTransform.Parameters)
          .externalArtifacts
          .from(
            creationConfig.variantDependencies
              .getArtifactCollection(
                ...
                inputArtifactType,
                attributes =
                  AsmClassesTransform.getAttributesForConfig(creationConfig).takeIf { component.dependenciesClassesAreInstrumented },
              )
              .artifactFiles
          )
      }
