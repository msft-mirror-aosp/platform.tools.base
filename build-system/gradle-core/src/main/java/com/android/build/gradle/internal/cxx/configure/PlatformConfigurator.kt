...
class PlatformConfigurator(private val ndkRoot: File) {
  ...
  /**
   * This is a hard-coded version of meta/platforms.json aliases. It does not need to be updated in sync with the release of new platforms
   * in the NDK because more recent NDKs will get this information from the JSON file.
   */
  private val platformNameAliases: Map<String, Int> =
    mapOf(
      "20" to 19,
      "25" to 24,
      "J" to 16,
  ...
  fun findSuitablePlatformVersion(
    ...
    val ndkMetaPlatforms =
      if (ndkMetaPlatformsFile.isFile) {
        ndkMetaPlatformsFile.reader().use { reader -> NdkMetaPlatforms.fromReader(reader) }
      } else {
        null
      }
  ...
  fun findSuitablePlatformVersionLogged(
    ...
    val abi =
      allAbis.singleOrNull { it.name == abiName }
        ?: run {
    ...
    if (ignoreMinSdkVersionFromDsl != null && ignoreMinSdkVersionFromProperty != null) {
      infoln(
        "Both ${StringOption.NDK_SUPPRESS_MIN_SDK_VERSION_ERROR.propertyName} Gradle property and " +
    ...
    val ignoreMinSdkVersion =
      when (supersededIgnoreMinSdkVersion) {
        null -> null
    ...
    if (ndkMetaPlatforms == null) {
      ...
      if (!platformDir.isDirectory) {
        warnln(NDK_CORRUPTED, "NDK folder '$ndkRoot' specified does not contain 'platforms'.")
        // Fall back so that processing can continue after the error
        return sensibleDefaultPlatformApiVersionForErrorCase
      }

      val minSdkVersion = computeMinSdkVersion(abi, androidVersionOrNull, platformNameAliases)
      return findPlatformConfiguratorLegacy(abi, minSdkVersion, androidVersionOrNull, platformDir, ignoreMinSdkVersion)
    }

    // Algorithm use d when meta/platforms.json exists (likely r17+). See "b" comments at the
    // top of this class.
    val minSdkVersion = computeMinSdkVersion(abi, androidVersionOrNull, ndkMetaPlatforms.aliases)
    return clamp(minSdkVersion, androidVersionOrNull, ndkMetaPlatforms.min, ndkMetaPlatforms.max, ignoreMinSdkVersion)
  }

  /**
   * This function combines the minSdkVersion and codeName fields from build.gradle into a recognized minSdkVersion. Along the way it issues
   * errors, warnings, and infos about how the decisions were made and what problems were seen.
   */
  private fun computeMinSdkVersion(abi: AbiInfo, androidVersionOrNull: AndroidVersion?, platformNameAliases: Map<String, Int>): Int {
    ...
    val minSdkVersionFromCodeName =
      when {
        codeNameOrNull == null || codeNameOrNull.isEmpty() -> null
        else -> {
          ...
          when {
            lookup != null -> {
              ...
              errorln(NDK_DOES_NOT_SUPPORT_API_LEVEL, "API codeName '$codeNameOrNull' is not supported by NDK '$ndkRoot'.")
    ...
    return when {
      minSdkVersionIsDefault && minSdkVersionFromCodeName == null -> {
        infoln("Neither codeName nor minSdkVersion specified. Using minimum platform " + "version for '${abi.name}'.")
        0
      }
      !minSdkVersionIsDefault && minSdkVersionFromCodeName != null ->
        when (minSdkVersionOrNull) {
          minSdkVersionFromCodeName -> {
            warnln("Both codeName and minSdkVersion specified. " + "They agree but only one should be specified.")
            minSdkVersionOrNull
          }
          else -> {
            infoln(
  ...
  /**
   * Algorithm used in the pre-meta/platform ABIs era (likely < r17) to find an existing and appropriate platform. See the "c" steps in the
  ...
  private fun findPlatformConfiguratorLegacy(
    ...
    val linkerSysrootPath = getLinkerSysrootPath(ndkRoot, abi, "android-$minSdkVersion")
    ...
    if (!versions.contains(clamped)) {
      ...
      warnln("Expected platform folder platforms/android-$clamped, " + "using platform API $min instead.")
      return min
    }
    return clamped
  }
  ...
  private fun clamp(minSdkVersion: Int, displayVersion: AndroidVersion?, min: Int, max: Int, ignoreMinSdkVersion: Int?): Int {
    if (minSdkVersion > max) {
      warnln(
        "Platform version ${displayVersionString(minSdkVersion, displayVersion)} is beyond $max, the maximum API level supported by this NDK. Using $max instead."
      )
      return max
    }
  ...
  private fun getLinkerSysrootPath(ndkRoot: File, abi: AbiInfo, platformVersion: String): String {
    return FileUtils.join(ndkRoot.path, "platforms", platformVersion, "arch-" + abi.architecture)
  }
}
