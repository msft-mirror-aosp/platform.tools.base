...
object NamespaceRemover : ResourceCompilationService {
  ...
  fun rewrite(input: Path, output: Path) {
    BufferedInputStream(Files.newInputStream(input)).use { `is` -> Files.write(output, rewrite(`is`).toByteArray()) }
  }
  ...
  fun rewrite(input: InputStream, lineSeparator: String = "\n"): String {
    ...
    return XmlPrettyPrinter.prettyPrint(doc, XmlFormatPreferences.defaults(), XmlFormatStyle.get(doc), lineSeparator, false)
  }
  ...
  private fun removeNamespaces(node: Node) {
    if (node.nodeType == Node.TEXT_NODE) {
      ...
      if (prefix != null && prefix == "xmlns") {
        ...
        if (uri != SdkConstants.ANDROID_URI && uri != SdkConstants.TOOLS_URI && uri != SdkConstants.AAPT_URI) {
