...
abstract class ManagedDeviceInstrumentationTestTask : NonIncrementalTask(), AndroidTestTask {
  ...
  abstract class TestRunnerFactory {
    ...
    @get:Input abstract val executionEnum: Property<TestOptions.Execution>

    @get:Input abstract val forceCompilation: Property<Boolean>

    @get:Input abstract val emulatorControlConfig: Property<EmulatorControlConfig>

    @get:Input abstract val compileSdkVersion: Property<String>

    @get:Input abstract val buildToolsRevision: Property<Revision>

    @get:Input @get:Optional abstract val testShardsSize: Property<Int>

    @get:Internal abstract val sdkBuildService: Property<SdkComponentsBuildService>
    ...
    @get:Input abstract val enableEmulatorDisplay: Property<Boolean>

    @get:Input @get:Optional abstract val getTargetIsSplitApk: Property<Boolean>

    fun createTestRunner(workerExecutor: WorkerExecutor, objectFactory: ObjectFactory, numShards: Int?): ManagedDeviceTestRunner {

      val useOrchestrator =
        when (executionEnum.get()) {
  ...
  @get:Nested abstract val testRunnerFactory: TestRunnerFactory

  @get:Nested abstract val testData: Property<TestData>

  @get:Optional @get:Input abstract val installOptions: ListProperty<String>

  @get:InputFiles @get:PathSensitive(PathSensitivity.RELATIVE) abstract val buddyApks: ConfigurableFileCollection
  ...
  @get:Nested abstract val device: Property<ManagedVirtualDevice>

  @get:Classpath @get:Optional abstract val classes: ConfigurableFileCollection

  @get:Classpath @get:Optional abstract val buildConfigClasses: ConfigurableFileCollection

  @get:Classpath @get:Optional abstract val rClasses: ConfigurableFileCollection
  ...
  @OutputDirectory abstract override fun getResultsDir(): DirectoryProperty

  @OutputDirectory abstract fun getReportsDir(): DirectoryProperty

  @OutputDirectory abstract fun getCoverageDirectory(): DirectoryProperty

  @Input abstract fun getAdditionalTestOutputEnabled(): Property<Boolean>

  @Optional @OutputDirectory abstract fun getAdditionalTestOutputDir(): DirectoryProperty

  @get:Input abstract val enableEmulatorDisplay: Property<Boolean>

  @Option(
    option = "enable-display",
    description = "Adding this option will display the emulator while testing, instead" + " of running the tests on a headless emulator.",
  )
  ...
  public override fun doTaskAction() {
    ...
    val additionalTestOutputDir =
      if (getAdditionalTestOutputEnabled().get()) {
        getAdditionalTestOutputDir().get().getAsFile().also { FileUtils.cleanOutputDir(it) }
      } else {
        null
      }
    ...
    val success =
      if (!testsFound()) {
        ...
        val runner = testRunnerFactory.createTestRunner(workerExecutor, objectFactory, testRunnerFactory.testShardsSize.getOrNull())
        ...
        try {
          ...
          recordCrashedInstrumentedTestRun(dependencies, testRunnerFactory.executionEnum.get(), false, analyticsService.get())
          throw e
        }
      }
  ...
  private fun testsFound(): Boolean {
    return testData.get().hasTests(classes, rClasses, buildConfigClasses).get()
  }
  ...
  class CreationAction(
    ...
    nameSuffix: String = "",
  ) : VariantTaskCreationAction<ManagedDeviceInstrumentationTestTask, InstrumentedTestCreationConfig>(creationConfig) {
    ...
    override fun handleProvider(taskProvider: TaskProvider<ManagedDeviceInstrumentationTestTask>) {
      ...
      creationConfig.artifacts
        .setInitialProvider(taskProvider, ManagedDeviceInstrumentationTestTask::getCoverageDirectory)
      ...
      if (isAdditionalAndroidTestOutputEnabled) {
        creationConfig.artifacts
          .setInitialProvider(taskProvider, ManagedDeviceInstrumentationTestTask::getAdditionalTestOutputDir)
    ...
    override fun configure(task: ManagedDeviceInstrumentationTestTask) {
      ...
      task.testRunnerFactory.enableEmulatorDisplay.set(task.enableEmulatorDisplay)
      ...
      if (device.sdkVersion <= 26 && !projectOptions[BooleanOption.GRADLE_MANAGED_DEVICE_ALLOW_OLD_API_LEVEL_DEVICES]) {
        throw GradleException(
          """
                    API level 26 and lower is currently not supported for Gradle Managed devices.
                    Your current configuration requires API level ${device.sdkVersion}.
      ...
      task.testRunnerFactory.avdComponents.setDisallowChanges(getBuildService(creationConfig.services.buildServiceRegistry))
      task.group = JavaBasePlugin.VERIFICATION_GROUP

      task.testData.setDisallowChanges(testData)
      task.installOptions.set(globalConfig.installationOptions.installOptions)
      task.testRunnerFactory.compileSdkVersion.setDisallowChanges(globalConfig.compileSdkHashString)
      task.testRunnerFactory.buildToolsRevision.setDisallowChanges(globalConfig.buildToolsRevision)
      task.testRunnerFactory.sdkBuildService.setDisallowChanges(
        getBuildService(creationConfig.services.buildServiceRegistry, SdkComponentsBuildService::class.java)
      )
      task.testRunnerFactory.testShardsSize.setDisallowChanges(projectOptions.get(IntegerOption.MANAGED_DEVICE_SHARD_POOL_SIZE))
      ...
      task.testRunnerFactory.forceCompilation.setDisallowChanges(creationConfig.isForceAotCompilation)

      if (!projectOptions.get(BooleanOption.ANDROID_TEST_USES_UNIFIED_TEST_PLATFORM)) {
        LoggerWrapper.getLogger(CreationAction::class.java)
          .warning(
      ...
      task.testRunnerFactory.utpDependencies.resolveDependencies(task.project.configurations)
      task.testRunnerFactory.getTargetIsSplitApk.setDisallowChanges(testedConfig?.componentType?.isDynamicFeature ?: false)

      task.testRunnerFactory.emulatorControlConfig.setDisallowChanges(
        createEmulatorControlConfig(projectOptions, globalConfig.androidTestOptions.emulatorControl as EmulatorControl)
      )

      task.testRunnerFactory.installApkTimeout.setDisallowChanges(projectOptions[IntegerOption.INSTALL_APK_TIMEOUT])
      ...
      task.getAdditionalTestOutputEnabled().set(projectOptions[BooleanOption.ENABLE_ADDITIONAL_ANDROID_TEST_OUTPUT])

      task.resultsDir.set(testResultOutputDir)
      task.getReportsDir().set(testReportOutputDir)

      task.project.configurations.findByName(SdkConstants.GRADLE_ANDROID_TEST_UTIL_CONFIGURATION)?.let { task.buddyApks.from(it) }

      task.classes.from(creationConfig.artifacts.forScope(ScopedArtifacts.Scope.PROJECT).getFinalArtifacts(ScopedArtifact.CLASSES))
      task.classes.disallowChanges()
      creationConfig.buildConfigCreationConfig?.let { task.buildConfigClasses.from(it.compiledBuildConfig) }
