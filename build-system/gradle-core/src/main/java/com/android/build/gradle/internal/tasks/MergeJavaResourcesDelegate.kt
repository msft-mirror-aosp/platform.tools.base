...
internal class CompressedJavaResJar(val name: String, val file: File, val priority: JavaResMergingPriority)
...
/** A delegate which actually does the merging of java resources, for example for the [MergeJavaResourceTask] */
internal class MergeJavaResourcesDelegate(
  ...
  fun run() {
    ...
    saveMergeState(
      IncrementalFileMerger.merge(finalInputList, output, loadMergeState(), PackagingUtils.getNoCompressPredicateForJavaRes(noCompress))
    )
  }
}
