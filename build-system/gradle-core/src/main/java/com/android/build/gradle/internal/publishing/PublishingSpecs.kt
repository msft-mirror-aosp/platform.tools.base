...
import com.android.build.gradle.internal.scope.InternalArtifactType.ANDROID_RES_SOURCE_SET_PATH_MAP
...
import com.android.build.gradle.internal.scope.InternalArtifactType.APKS_FROM_BUNDLE
...
import com.android.build.gradle.internal.scope.InternalArtifactType.SOURCE_JAR
...
import com.android.build.gradle.internal.scope.InternalArtifactType.VARIANT_CODE_COVERAGE_DATA
import com.android.build.gradle.internal.scope.InternalArtifactType.VARIANT_TEST_RESULTS
 ...
 *
 * This builds a bi-directional mapping between task outputs and published artifacts (for project to project publication), as well as where
 * to publish the artifact (with [org.gradle.api.artifacts.Configuration] via the [PublishedConfigType] enum.)
 *
 * This mapping is per [ComponentType] to allow for different task outputs to be published under the same [ArtifactType].
 */
  ...
  companion object {
    ...
    init {
      variantSpec(ComponentTypeImpl.BASE_APK) {
        ...
        runtime(APKS_FROM_BUNDLE, ArtifactType.APKS_FROM_BUNDLE)
      ...
      variantSpec(ComponentTypeImpl.LIBRARY) {
        ...
        source(SOURCE_JAR, ArtifactType.SOURCES_JAR)
        javaDoc(JAVA_DOC_JAR, ArtifactType.JAVA_DOC_JAR)
        ...
        output(DATA_BINDING_BASE_CLASS_LOG_ARTIFACT, ArtifactType.DATA_BINDING_BASE_CLASS_LOG_ARTIFACT)
        ...
        runtime(LIBRARY_ART_PROFILE, ArtifactType.ART_PROFILE)
        runtime(SUPPORTED_LOCALE_LIST, ArtifactType.SUPPORTED_LOCALE_LIST)
        runtime(SOURCE_JAR, ArtifactType.SOURCES_JAR)
        runtime(VARIANT_CODE_COVERAGE_DATA, ArtifactType.CODE_COVERAGE_DATA)
        runtime(ANDROID_RES_SOURCE_SET_PATH_MAP, ArtifactType.ANDROID_RES_SOURCE_SET_MAPPING)
    ...
    private fun variantSpec(componentType: ComponentType, action: VariantSpecBuilder.() -> Unit) {
...
private val REVERSE_METADATA_ELEMENTS_ONLY: ImmutableList<PublishedConfigType> = ImmutableList.of(REVERSE_METADATA_ELEMENTS)
...
private val SOURCE_PUBLICATION: ImmutableList<PublishedConfigType> = ImmutableList.of(PublishedConfigType.SOURCE_PUBLICATION)
private val JAVA_DOC_PUBLICATION: ImmutableList<PublishedConfigType> = ImmutableList.of(PublishedConfigType.JAVA_DOC_PUBLICATION)
private val APK_PUBLICATION: ImmutableList<PublishedConfigType> = ImmutableList.of(PublishedConfigType.APK_PUBLICATION)
private val AAB_PUBLICATION: ImmutableList<PublishedConfigType> = ImmutableList.of(PublishedConfigType.AAB_PUBLICATION)
...
private class VariantPublishingSpecImpl(
  ...
  override fun getSpec(artifactType: ArtifactType, publishConfigType: PublishedConfigType?): PublishingSpecs.OutputSpec? {
    return artifactMap[artifactType]?.let { specs ->
      if (specs.size <= 1) {
        specs.singleOrNull()
      } else {
        val matchingSpecs =
          if (publishConfigType != null) {
...
private open class VariantSpecBuilderImpl(override val componentType: ComponentType) : PublishingSpecs.VariantSpecBuilder {
  ...
  fun toSpec(parentSpec: PublishingSpecs.VariantSpec? = null): PublishingSpecs.VariantSpec {
    return VariantPublishingSpecImpl(componentType, parentSpec, outputs.toImmutableSet())
  }
}

private class TestFixturesVariantSpecBuilder(componentType: ComponentType) : VariantSpecBuilderImpl(componentType) {
...
private class KotlinMultiplatformVariantSpecBuilder(componentType: ComponentType) : VariantSpecBuilderImpl(componentType) {
