...
abstract class AnalyticsEnabledVariant(override val delegate: Variant, stats: GradleBuildVariant.Builder, objectFactory: ObjectFactory) :
  ...
  private val userVisiblePackaging: Packaging by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) { objectFactory.newInstance(AnalyticsEnabledPackaging::class.java, delegate.packaging, stats) }
  ...
  private val userVisibleExternalNativeBuild: ExternalNativeBuild? by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      delegate.externalNativeBuild?.let {
        if (it is ExternalNdkBuildImpl) {
          objectFactory.newInstance(AnalyticsEnabledExternalNdkBuild::class.java, it, stats)
        } else {
          objectFactory.newInstance(AnalyticsEnabledExternalCmake::class.java, it, stats)
        }
      }
    }
  ...
  private val userVisibleUnitTest: AnalyticsEnabledUnitTest? by
    lazy(LazyThreadSafetyMode.SYNCHRONIZED) {
      (delegate as? HasUnitTest)?.unitTest?.let { objectFactory.newInstance(AnalyticsEnabledUnitTest::class.java, it, stats) }
    }
  ...
  override fun <T> getExtension(type: Class<T>): T? {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.GET_EXTENSION_VALUE
  ...
  override val minSdk: AndroidVersion
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.MIN_SDK_VERSION_VALUE
  ...
  override val maxSdk: Int?
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.MAX_SDK_VERSION_VALUE
  ...
  override val experimentalProperties: MapProperty<String, Any>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.VARIANT_PROPERTIES_VALUE
      return delegate.experimentalProperties
    }

  override val nestedComponents: List<Component>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.NESTED_COMPONENTS_VALUE
      return delegate.nestedComponents
    }

  override val components: List<Component>
    get() {
      stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.COMPONENTS_VALUE
      return delegate.components
    }

  override fun missingDimensionStrategy(dimension: String, vararg requestedValues: String) {
    stats.variantApiAccessBuilder.addVariantPropertiesAccessBuilder().type = VariantPropertiesMethodType.MISSING_DIMENSION_STRATEGY_VALUE
