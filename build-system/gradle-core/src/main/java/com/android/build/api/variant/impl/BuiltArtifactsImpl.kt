...
constructor(
  ...
  val initializedElementType: String? by lazy { elementType ?: initFileType(elements) }
  ...
  companion object {
    ...
    private fun initFileType(elements: Collection<BuiltArtifactImpl>): String? {
      val (files, directories) = elements.asSequence().map { File(it.outputFile) }.filter { it.exists() }.partition { it.isFile }
    ...
    private fun display(files: Collection<File>, singular: String, plural: String): String {
      return if (files.size > 1) "${files.joinToString(",") { it.name }} are $plural" else "${files.first().name} is a $singular"
    }
  ...
  fun addElement(element: BuiltArtifactImpl): BuiltArtifactsImpl {
    ...
    elements
      .find { it.variantOutputConfiguration == element.variantOutputConfiguration }
      ?.also { previous: BuiltArtifact -> elementsCopy.remove(previous) }
    elementsCopy.add(element)
    return copy(elements = elementsCopy)
  }

  /** Finds the main split in the current variant context or throws a [RuntimeException] if there are none. */
  fun getMainSplit(targetConfigurations: Collection<FilterConfiguration>?): BuiltArtifactImpl =
    getMainSplitOrNull(targetConfigurations) ?: throw RuntimeException("Cannot determine main split information, file a bug.")
  ...
  private fun getMainSplitOrNull(targetConfigurations: Collection<FilterConfiguration>?): BuiltArtifactImpl? =
    elements.find { builtArtifact -> builtArtifact.outputType == VariantOutputConfiguration.OutputType.SINGLE }
      ?: elements.find { it.outputType == VariantOutputConfiguration.OutputType.UNIVERSAL }
      ...
      ?: elements.find { it.outputType == VariantOutputConfiguration.OutputType.ONE_OF_MANY }
  ...
  fun saveToFile(out: File) = out.writeText(persist(out.parentFile.toPath()), Charsets.UTF_8)

  private fun persist(projectPath: Path): String {
    // flatten and relativize the file paths to be persisted.
    val withRelativePaths =
      copy(elements = elements.map { it.newOutput(projectPath.relativize(Paths.get(it.outputFile))) }, elementType = initializedElementType)
    return StringWriter()
      .also {
...
internal class BuiltArtifactsTypeAdapter(projectPath: Path) :
  CommonBuiltArtifactsTypeAdapter<BuiltArtifactsImpl, Artifact<*>, BuiltArtifactImpl>(projectPath) {

  override val artifactTypeTypeAdapter
    get() = ArtifactTypeTypeAdapter

  override val elementTypeAdapter
    get() = BuiltArtifactTypeAdapter
