...
abstract class AndroidPluginBaseServices(
  ...
  private val optionService: ProjectOptionService by lazy {
    withProject("optionService") { ProjectOptionService.RegistrationAction(it).execute().get() }
  }
  ...
  private val androidProblemReporterProviderService: Provider<AndroidProblemReporterProvider> by lazy {
    withProject("androidProblemReporterProviderService") {
      AndroidProblemReporterProvider.RegistrationAction(it, optionService.projectOptions).execute()
    }
  }
  ...
  @JvmField protected var project: Project? = null
  ...
  protected open fun applyBaseServices(project: Project, buildFeatures: BuildFeatures) {
    ...
    if (attributionFileLocation != null) {
      BuildAnalyzerService.RegistrationAction(
          ...
          listenerRegistry,
          BuildAnalyzerConfiguratorService.RegistrationAction(project).execute().get(),
          buildFeatures.configurationCacheActive(),
          buildFeatures.projectIsolationActive(),
        )
        .execute()
    ...
    configuratorService.getProjectBuilder(project.path)?.let {
      it
        ...
        .options = AnalyticsUtil.toProto(projectOptions)
    }
    configuratorService.recordBlock(ExecutionType.BASE_PLUGIN_PROJECT_CONFIGURE, project.path, null) { configureProject(project) }
    configuratorService.recordBlock(ExecutionType.BASE_PLUGIN_PROJECT_BASE_EXTENSION_CREATION, project.path, null) {
      configureExtension(project)
    }
    configuratorService.recordBlock(ExecutionType.BASE_PLUGIN_PROJECT_TASKS_CREATION, project.path, null) { createTasks(project) }
  }
  ...
  protected val settingsExtension: SettingsExtension? by
  ...
  protected fun createSettingsOptions(dslServices: DslServices): SettingsOptions {
    // resolve settings extension
    val actualSettingsExtension =
      settingsExtension
        ?: run {
    ...
    val buildProfileOptions = { profile: ExecutionProfile ->
      ExecutionProfileOptions(
        name = profile.name,
        r8Options = profile.r8.let { r8 -> ToolExecutionOptions(jvmArgs = r8.jvmOptions, runInSeparateProcess = r8.runInSeparateProcess) },
      )
    }

    // If the string option is set use that one instead
    val actualProfileName =
      dslServices.projectOptions[StringOption.EXECUTION_PROFILE_SELECTION] ?: actualSettingsExtension.execution.defaultProfile
  ...
  /**
   * Runs a lambda function if [project] has been initialized and return the function's result or generate an exception if [project] is
   * null.
   *
  ...
  protected fun <T> withProject(context: String, action: (project: Project) -> T): T =
    project?.let { action(it) } ?: throw IllegalStateException("Cannot obtain $context until Project is known")
}
