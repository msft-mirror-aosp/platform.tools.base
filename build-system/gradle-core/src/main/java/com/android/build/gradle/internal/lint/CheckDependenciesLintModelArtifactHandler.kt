 ...
 *
 * Note that project dependencies on java libraries that do not have the custom lint plugin applied will be treated as remote dependencies
 * with a warning emitted that those sources will not be analyzed. This will hopefully help smooth the transition to AGP 7.0 for build
 * authors who enable checkDependencies.
 */
internal class CheckDependenciesLintModelArtifactHandler(
...
) : ArtifactHandler<LintModelLibrary>(dependencyCaches.localJarCache, dependencyCaches.mavenCoordinatesCache) {

  private val projectDependencyLintModels =
    projectCompileLintModels.asProjectKeyedMap().keys + projectRuntimeLintModels.asProjectKeyedMap().keys
  ...
  private fun errorJarNotFound(key: ProjectSourceSetKey): Nothing {
    error(
      "Internal errorr: Could not find jar for project $key\n" +
