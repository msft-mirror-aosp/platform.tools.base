...
class OptimizationCreationConfigImpl(
  ...
  override val proguardFiles: ListProperty<RegularFile> by
    ...
    lazy(LazyThreadSafetyMode.NONE) {
      ...
      val consumerProguardFilesProperty: ListProperty<RegularFile> =
        internalServices.listPropertyOf(RegularFile::class.java) { list ->
          consumerProguardFilePaths.forEach { list.add(internalServices.toRegularFileProvider(it)) }
        }
      ...
      val result: Provider<List<RegularFile>> =
        if (consumerProguardFilePaths.any { it in defaultProguardFiles }) {
          component.global.globalArtifacts.get(InternalArtifactType.DEFAULT_PROGUARD_FILES).zip(consumerProguardFilesProperty) { _, right ->
            right
          }
        } else {
          consumerProguardFilesProperty
        }
      internalServices.listPropertyOf(RegularFile::class.java) { it.set(result) }
    }

  private val consumerProguardFilePaths: List<File> by
    lazy(LazyThreadSafetyMode.NONE) {
  ...
  override val ignoreFromInKeepRules: Provider<List<String>> =
    baseModuleMetadata?.map { it.ignoreFromInKeepRules }
      ?: internalServices.listPropertyOf(String::class.java, dslInfo.ignoreFromInKeepRules)

  override val ignoreFromAllExternalDependenciesInKeepRules: Boolean = dslInfo.ignoreFromAllExternalDependenciesInKeepRules

  override val ignoreFromInBaselineProfile: Provider<Set<String>> =
    internalServices.setPropertyOf(String::class.java, dslInfo.ignoreFromInBaselineProfile)

  override val ignoreFromAllExternalDependenciesInBaselineProfile: Boolean = dslInfo.ignoreFromAllExternalDependenciesInBaselineProfile
