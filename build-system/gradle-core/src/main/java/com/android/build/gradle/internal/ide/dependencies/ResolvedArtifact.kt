 ...
 *
 * This is generally computed from a [ResolvedArtifactResult] (which is not usable as a [Map] key) plus additional information.
 */
...
internal constructor(
  ...
  constructor(
    ...
    artifactFile,
    mainArtifactResult.hasProjectTestFixturesCapability() || mainArtifactResult.hasLibraryTestFixturesCapability(),
    extractedFolder,
    publishedLintJar,
    dependencyType,
    isWrappedModule,
  ...
  fun computeMavenCoordinates(stringCachingService: StringCachingService): MavenCoordinates {
    return when (componentIdentifier) {
      ...
      is OpaqueComponentIdentifier -> {
        MavenCoordinatesCacheBuildService.getMavenCoordForLocalFile(artifactFile!!, stringCachingService)
      }
  ...
  fun computeModelAddress(mavenCoordinatesCache: MavenCoordinatesCacheBuildService): String =
    when (componentIdentifier) {
      ...
      is OpaqueComponentIdentifier -> {
        ...
        throw RuntimeException(
          "Don't know how to handle ComponentIdentifier '" + componentIdentifier.displayName + "'of type " + componentIdentifier.javaClass
        )
      }
    }
}
