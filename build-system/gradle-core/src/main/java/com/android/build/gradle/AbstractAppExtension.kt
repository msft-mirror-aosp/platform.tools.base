...
) : TestedExtension(dslServices, bootClasspathConfig, buildOutputs, sourceSetManager, isBaseModule, stats) {

  /**
   * Returns a collection of [build variants](https://developer.android.com/studio/build/build-variants.html) that the app project includes.
   *
   * To process elements in this collection, you should use the
   * [`all`](https://docs.gradle.org/current/javadoc/org/gradle/api/DomainObjectCollection.html#all(org.gradle.api.Action)) iterator. That's
   * because the plugin populates this collection only after the project is evaluated. Unlike the `each` iterator, using `all` processes
   * future elements as the plugin creates them.
   *
