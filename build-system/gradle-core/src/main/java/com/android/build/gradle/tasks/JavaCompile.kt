 ...
 *
 * Note that when Kapt is not used (e.g., in Java-only projects), [JavaCompile] performs both annotation processing and compilation. When
 * Kapt is used (e.g., in most Kotlin-only or hybrid Kotlin-Java projects), [JavaCompile] performs compilation only, without annotation
 * processing.
 */
class JavaCompileCreationAction(val creationConfig: JavaCompileCreationConfig) : TaskCreationAction<JavaCompile>() {

  private val dataBindingArtifactDir = creationConfig.services.directoryProperty()
  private val dataBindingExportClassListFile = creationConfig.services.regularFileProperty()
  ...
  override fun handleProvider(taskProvider: TaskProvider<JavaCompile>) {
    ...
    artifacts.setInitialProvider(taskProvider) { it.destinationDirectory }.withName("classes").on(JAVAC)
    ...
    if (creationConfig.dataBinding) {
      ...
      registerDataBindingOutputs(
        ...
        creationConfig.isExportDataBindingClassList,
        taskProvider,
        artifacts,
        forJavaCompile = true,
      )
  ...
  override fun configure(task: JavaCompile) {
    ...
    task.project.extensions.getByType(JavaPluginExtension::class.java).let {
      if (it.toolchain.languageVersion.isPresent) {
        val toolchainService = task.project.extensions.getByType(JavaToolchainService::class.java)
    ...
    task.source = creationConfig.source
    ...
    task.options.isIncremental = creationConfig.isIncremental
    ...
    task.inputs.files(apList).withPathSensitivity(PathSensitivity.NONE).withPropertyName("annotationProcessorList")
    task.recordAnnotationProcessors(apList, creationConfig.name, getBuildService(creationConfig.services.buildServiceRegistry))
    ...
    if (creationConfig.dataBinding) {
      ...
      if (!creationConfig.usingKapt) {
        task.outputs.dir(dataBindingArtifactDir).withPropertyName("dataBindingArtifactDir")
        if (creationConfig.isExportDataBindingClassList) {
          task.outputs.file(dataBindingExportClassListFile).withPropertyName("dataBindingExportClassListFile")
        }
      }
    }
...
fun registerDataBindingOutputs(
  ...
  if (forJavaCompile) {
    artifacts.setInitialProvider(taskProvider) { dataBindingArtifactDir }.on(DATA_BINDING_ARTIFACT)
    if (isExportDataBindingClassList) {
      artifacts.setInitialProvider(taskProvider) { dataBindingExportClassListFile }.on(DATA_BINDING_EXPORT_CLASS_LIST)
    }
  } else {
    artifacts.use(taskProvider).wiredWith { dataBindingArtifactDir }.toCreate(DATA_BINDING_ARTIFACT)
    if (isExportDataBindingClassList) {
      artifacts.use(taskProvider).wiredWith { dataBindingExportClassListFile }.toCreate(DATA_BINDING_EXPORT_CLASS_LIST)
    }
  }
}

private class JavaCompileOptionsForRoom(
  @get:InputFile @get:PathSensitive(PathSensitivity.NONE) val annotationProcessorListFile: Provider<RegularFile>,
  @get:Input val isTargetJava8Compatible: Boolean,
) : CommandLineArgumentProvider {

  override fun asArguments(): Iterable<String> {
    val annotationProcessors = readAnnotationProcessorsFromJsonFile(annotationProcessorListFile.get().asFile).keys
...
private fun JavaCompile.recordAnnotationProcessors(
  ...
  doFirst {
    val annotationProcessors = readAnnotationProcessorsFromJsonFile(processorListFile.get().asFile)
    val nonIncrementalAPs = annotationProcessors.filter { it.value == ProcessorInfo.NON_INCREMENTAL_AP }
    ...
    recordAnnotationProcessorsForAnalytics(annotationProcessors, projectPath, variantName, analyticsService.get())
  }
}
 ...
 * @param includeKotlinSources whether or not the kotlin sources should be included. For compilation, include only java sources, otherwise
 *   we hit b/144249620.
 */
fun computeJavaSource(creationConfig: ComponentCreationConfig, includeKotlinSources: Boolean = false): FileTree {
  ...
  return creationConfig.services
    ...
    .also { fileCollection ->
      ...
      creationConfig.getBuiltInKaptArtifact(InternalArtifactType.BUILT_IN_KAPT_GENERATED_JAVA_SOURCES)?.let { fileCollection.from(it) }
    }
    .asFileTree
