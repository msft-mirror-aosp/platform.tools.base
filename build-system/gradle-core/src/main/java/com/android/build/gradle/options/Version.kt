...
/** Represents an Android Gradle plugin major and minor version (e.g., AGP 9.0), ignoring micro version. */
enum class Version(val major: Int, val minor: Int) {

  /**
   * A version before version 4.0, used when the exact version is not known, except that it's guaranteed to be before 4.0.
   *
   * Internally, we use "0.0" to represent this version (not the nicest solution, but probably works for now).
   */
