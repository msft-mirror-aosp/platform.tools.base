...
constructor(
  ...
  private val variantScope: FusedLibraryGlobalScope by
    lazy(LazyThreadSafetyMode.NONE) {
      withProject("variantScope") { project -> FusedLibraryGlobalScopeImpl(project, projectServices, { extension }) }
    }

  private val extension: InternalFusedLibraryExtension by
    lazy(LazyThreadSafetyMode.NONE) { withProject("extension") { project -> instantiateExtension(project) } }
  ...
  private fun instantiateExtension(project: Project): InternalFusedLibraryExtension {
    ...
    val fusedLibraryExtensionImpl = FusedLibraryExtensionImpl.getDecoratedInstance(dslServices, settingsExtension)

    open class Extension(val publicExtensionImpl: FusedLibraryExtensionImpl) : InternalFusedLibraryExtension by publicExtensionImpl
  ...
  private fun maybePublishToMaven(
    ...
    val bundleTaskProvider =
      variantScope.artifacts.getArtifactContainer(FusedLibraryInternalArtifactType.BUNDLED_LIBRARY).getFinalProvider()
    val sourcesJarProvider =
      variantScope.artifacts.getArtifactContainer(FusedLibraryInternalArtifactType.MERGED_SOURCES_JAR).getFinalProvider()

    val runtimePublication =
      if (bundleTaskProvider != null) {
        project.configurations.register("runtimePublication") {
          ...
          it.attributes.attribute(
            ...
            project.objects.named(LibraryElements::class.java, AndroidArtifacts.ArtifactType.AAR.type),
          )
    ...
    val runtimeSourcePublication =
      if (sourcesJarProvider != null) {
        project.configurations.register("runtimeSourcePublication") {
          ...
          it.attributes.attribute(DocsType.DOCS_TYPE_ATTRIBUTE, project.objects.named(DocsType::class.java, DocsType.SOURCES))
          it.attributes.attribute(
            LibraryElements.LIBRARY_ELEMENTS_ATTRIBUTE,
            project.objects.named(LibraryElements::class.java, AndroidArtifacts.ArtifactType.JAR.type),
          )
    ...
    val adhocComponent = softwareComponentFactory.adhoc(FusedLibraryConstants.FUSED_LIBRARY_PUBLICATION_COMPONENT_NAME)
    // add it to the list of components that this project declares
    project.components.add(adhocComponent)

    listOfNotNull(runtimePublication, runtimeSourcePublication).forEach { outgoingRuntimeConfigurationProvider ->
      adhocComponent.addVariantsFromConfiguration(outgoingRuntimeConfigurationProvider.get()) { it.mapToMavenScope("runtime") }
    }

    project.afterEvaluate {
      project.extensions.findByType(PublishingExtension::class.java)?.also {
        component(
          it.publications.create("maven", MavenPublication::class.java).also { mavenPublication -> mavenPublication.from(adhocComponent) },
        ...
        project.pluginManager.withPlugin(publishPlugin) {
          val publicationTasks = setOf("publish", "generatePomFileForMavenPublication", "generateMetadataFileForMavenPublication")
          project.tasks.named { it in publicationTasks }.forEach { it.dependsOn(FusedLibraryConstants.VALIDATE_DEPENDENCIES_TASK_NAME) }
        }
      }
    }
  }

  fun component(publication: MavenPublication, runtimeDependenciesProvider: Provider<Set<ModuleVersionIdentifier>>) {
    publication.pom { pom: MavenPom ->
      pom.withXml { xml ->
        val dependenciesNode =
          xml.asNode().let {
            it.children().removeIf { node -> ((node as Node).name() as QName).qualifiedName == "dependencies" }
  ...
  override fun getAnalyticsPluginType(): GradleBuildProject.PluginType = GradleBuildProject.PluginType.FUSED_LIBRARY
  ...
  override fun apply(project: Project) {
    ...
    // be fused.
    val include =
      project.configurations.register(FusedLibraryConstants.INCLUDE_CONFIGURATION_NAME) { include ->
        include.description = "Used for declaring dependencies that should be packaged in the fused artifact."
    ...
    fun getIncludeTransitiveResolved(name: String, usage: String): Configuration =
      project.configurations
        .register(name) { includePlatform ->
          includePlatform.description = "Used for resolving transitive dependency version constraints of include dependencies."
          ...
          includePlatform.attributes.attribute(Usage.USAGE_ATTRIBUTE, project.objects.named(Usage::class.java, usage))
    ...
    val fusedApi =
      project.configurations.register(FusedLibraryConstants.FUSED_API_CONFIGURATION_NAME) { apiClasspath ->
        ...
        apiClasspath.attributes.attribute(Usage.USAGE_ATTRIBUTE, project.objects.named(Usage::class.java, Usage.JAVA_API))
    ...
    val fusedRuntime =
      project.configurations.register(FusedLibraryConstants.FUSED_RUNTIME_CONFIGURATION_NAME) { runtimeClasspath ->
        ...
        runtimeClasspath.attributes.attribute(Usage.USAGE_ATTRIBUTE, project.objects.named(Usage::class.java, Usage.JAVA_RUNTIME))
    ...
    val sourcesConfiguration =
      project.configurations.register(FusedLibraryConstants.FUSED_SOURCES_CONFIGURATION_NAME) { sourcesConfig ->
        ...
        sourcesConfig.attributes.attribute(CATEGORY_ATTRIBUTE, project.objects.named(Category::class.java, Category.DOCUMENTATION))
        sourcesConfig.attributes.attribute(DocsType.DOCS_TYPE_ATTRIBUTE, project.objects.named(DocsType::class.java, DocsType.SOURCES))
    ...
    if (projectServices.projectOptions[BooleanOption.FUSED_LIBRARY_PUBLICATION_ONLY_MODE]) {
      ...
      project.configurations.register("runtimeElements") { runtimeElements ->
        configureElements(
          ...
          variantScope.artifacts,
          mapOf(
            FusedLibraryInternalArtifactType.SYMBOL_LIST_WITH_PACKAGE_NAME to AndroidArtifacts.ArtifactType.SYMBOL_LIST_WITH_PACKAGE_NAME
          ),
        )
    ...
    val dependenciesModuleVersionIds = getFusedLibraryDependencyModuleVersionIdentifiers(includeTransitiveRuntimeResolved)
    val dependenciesProvider = dependenciesModuleVersionIds.toDependenciesProvider(project)

    maybePublishToMaven(project, dependenciesModuleVersionIds, dependenciesProvider)
  ...
  private fun applyCommonConsumptionAttributes(project: Project, configurations: List<Configuration>) {
    val consumptionBuildType: BuildTypeAttr = project.objects.named(BuildTypeAttr::class.java, "release")
    val jvmEnvironment: TargetJvmEnvironment = project.objects.named(TargetJvmEnvironment::class.java, TargetJvmEnvironment.ANDROID)

    configurations.forEach {
      it.attributes.attribute(BuildTypeAttr.ATTRIBUTE, consumptionBuildType)
      it.attributes.attribute(TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE, jvmEnvironment)
    }
    project.dependencies.attributesSchema.attribute(TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE).also {
