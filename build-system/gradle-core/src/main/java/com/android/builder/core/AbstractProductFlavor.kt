 ...
 *
 * This is also used to describe the default configuration of all builds, even those that do not contain any flavors.
 */
@Deprecated("This is deprecated, use DSL objects directly.")
abstract class AbstractProductFlavor(private val name: String) : BaseConfigImpl(), ProductFlavor {
  ...
  /**
   * Implemented as a separate property to make versionCode read-only in MergedFlavor, but still allow [mergeWithHigherPriorityFlavor] to
   * set it.
   */
  ...
  /**
   * Implemented as a separate property to make versionName read-only in MergedFlavor, but still allow [mergeWithHigherPriorityFlavor] to
   * set it.
   */
  ...
  fun setTestInstrumentationRunnerArguments(testInstrumentationRunnerArguments: MutableMap<String, String>): ProductFlavor {
  ...
  fun setSigningConfig(signingConfig: SigningConfig?): ProductFlavor {
    this.signingConfig = signingConfig as com.android.build.gradle.internal.dsl.SigningConfig?
    return this
  }
  ...
  fun addResourceConfiguration(configuration: String) {
    resourceConfigurations.add(configuration.also { validateResourceConfiguration(it) })
  }

  /** Adds a res config filter (for instance 'hdpi') */
  fun addResourceConfigurations(vararg configurations: String) {
    resourceConfigurations.addAll(listOf(*configurations.onEach { validateResourceConfiguration(it) }))
  }

  /** Adds a res config filter (for instance 'hdpi') */
  fun addResourceConfigurations(configurations: Collection<String>) {
    resourceConfigurations.addAll(configurations.onEach { validateResourceConfiguration(it) })
  }
   ...
   *
   * Android plugin 3.0.0 and higher try to match each variant of your module with the same one from its dependencies. For example, consider
   * if both your app and its dependencies include a "tier" [flavor dimension](/studio/build/build-variants.html#flavor-dimensions), with
   * flavors "free" and "paid". When you build a "freeDebug" version of your app, the plugin tries to match it with "freeDebug" versions of
   * the local library modules the app depends on.
   *
   * However, there may be situations in which **a library dependency includes a flavor dimension that your app does not**. For example,
   * consider if a library dependency includes flavors for a "minApi" dimension, but your app includes flavors for only the "tier"
   * dimension. So, when you want to build the "freeDebug" version of your app, the plugin doesn't know whether to use the "minApi23Debug"
   * or "minApi18Debug" version of the dependency, and you'll see an error message similar to the following:
   * ```
   ...
   * [`defaultConfig`](com.android.build.gradle.internal.dsl.DefaultConfig.html) block to specify the default flavor the plugin should
   * select from each missing dimension, as shown in the sample below. You can also override your selection in the
   * [`productFlavors`](com.android.build.gradle.internal.dsl.ProductFlavor.html) block, so each flavor can specify a different matching
   * strategy for a missing dimension. (Tip: you can also use this property if you simply want to change the matching strategy for a
   ...
   *
   * The behavior is that if a value is present in the overlay, then it is used, otherwise we use the existing value.
   *
  ...
  protected fun mergeWithHigherPriorityFlavor(overlay: ProductFlavor) {
    ...
    renderscriptSupportModeEnabled = chooseNotNull(overlay.renderscriptSupportModeEnabled, renderscriptSupportModeEnabled)
    renderscriptSupportModeBlasEnabled = chooseNotNull(overlay.renderscriptSupportModeBlasEnabled, renderscriptSupportModeBlasEnabled)
    ...
    applicationIdSuffix = mergeApplicationIdSuffix(overlay.applicationIdSuffix, applicationIdSuffix)
    testApplicationId = chooseNotNull(overlay.testApplicationId, testApplicationId)
    testInstrumentationRunner = chooseNotNull(overlay.testInstrumentationRunner, testInstrumentationRunner)
    testInstrumentationRunnerArguments.putAll(overlay.testInstrumentationRunnerArguments)
    testHandleProfiling = chooseNotNull(overlay.testHandleProfiling, testHandleProfiling)
    testFunctionalTest = chooseNotNull(overlay.testFunctionalTest, testFunctionalTest)
    wearAppUnbundled = chooseNotNull(overlay.wearAppUnbundled, wearAppUnbundled)
    ...
    vectorDrawables.setGeneratedDensities(chooseNotNull(overlay.vectorDrawables.generatedDensities, vectorDrawables.generatedDensities))
    vectorDrawables.useSupportLibrary = chooseNotNull(overlay.vectorDrawables.useSupportLibrary, vectorDrawables.useSupportLibrary)
    if (overlay is AbstractProductFlavor) {
      if (missingDimensionSelections == null) {
        missingDimensionSelections = Maps.newHashMap()
      }
      missingDimensionSelections!!.putAll(overlay.missingDimensionStrategies)
  ...
  override fun toString(): String {
    return MoreObjects.toStringHelper(this)
      ...
      .add("testInstrumentationRunner", testInstrumentationRunner)
      .add("testInstrumentationRunnerArguments", testInstrumentationRunnerArguments)
  ...
  companion object {
    ...
    private fun joinWithSeparator(overlay: String?, base: String?, separator: Char?): String? {
