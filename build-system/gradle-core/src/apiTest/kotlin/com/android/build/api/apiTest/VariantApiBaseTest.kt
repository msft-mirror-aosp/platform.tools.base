...
/**
 * Base test class for Variant API related tasks. These tasks setup a Gradle project and execute some tasks on those and finally verifies
 * the build behavior or output.
 *
...
open class VariantApiBaseTest(
  ...
  companion object {
    ...
    val agpVersion = System.getenv("API_TESTS_VERSION") ?: com.android.Version.ANDROID_GRADLE_PLUGIN_VERSION

    val generalRepos = listOf("google()", "mavenCentral()")

    /** List of custom repositories where all projects dependencies can be satisfied. */
    val mavenRepos: List<String> by lazy { System.getenv("CUSTOM_REPO").split(File.pathSeparatorChar) }
  }
  ...
  enum class ScriptingLanguage(val buildFileName: String, val settingsFileName: String) {
    ...
    Kotlin("build.gradle.kts", "settings.gradle.kts") {
      ...
      override fun configureBuildFile(repositories: List<String>, additionalClasspath: List<String>) =
        """
buildscript {
...
"""
     ...
     *
     * By default the AGP plugin will be automatically added, depending on the scripting language, other plugins like the kotlin gradle
     * plugin when using kotlin for instance.
     *
     * @param repositories the list of repositories where project dependencies will be obtained from
     */
    abstract fun configureBuildFile(repositories: List<String>, additionalClasspath: List<String> = listOf()): String

    /**
     * Configure the buildSrc/ build file for the [scriptingLanguage]
     *
     * @param globalRepos the list of global repositories where project dependencies will be obtained from.
     * @param localRepos the list of repositories where project dependencies will be obtained from
     */
     ...
     *
     * @param repositories the list of local repositories where project dependencies will be obtained from
     */
  ...
  @JvmField
  val testProjectDir =
    if (System.getenv("API_TESTS_OUTDIR") != null) {
      ApiTestFolder(File(System.getenv("API_TESTS_OUTDIR")), testType.getDirName(this))
    } else {
      TemporaryFolder()
    }

  @Rule @JvmField val privateTestProjectDir = TemporaryFolder()

  @Rule @JvmField val testBuildDir = TemporaryFolder()
  ...
  open class ModuleGivenBuilder(val scriptingLanguage: ScriptingLanguage) {
    ...
    /** Build file for the module, must be provided by the time [AbstractBuildGivenBuildCheckTest.when] is called. */
     ...
     * @param action: the lambda to execute once the directory is created to configure its content.
     */
    ...
    internal open fun writeModule(folder: File) {
      ...
      if (manifest != null) {
        File(folder, "src/main").apply {
          mkdirs()
          File(this, SdkConstants.ANDROID_MANIFEST_XML).writeText(manifest!!.trimIndent())
        }
      }
  ...
  open class GivenBuilder(scriptingLanguage: ScriptingLanguage, val testName: TestName? = null) : ModuleGivenBuilder(scriptingLanguage) {

    private val modules = mutableListOf<Pair<String, GivenBuilder>>()

    /** the list of tasks' names to invoke on the project, by default, we only invoke the assembleDebug task. */
    ...
    fun addModule(path: String, action: GivenBuilder.() -> Unit) {
      ...
      object : GivenBuilder(scriptingLanguage) {
          override fun addBuildFile(folder: File) {
            if (buildFile != null) {
              File(folder, scriptingLanguage.buildFileName).apply { writeText(buildFile!!) }
            }
          }
        }
        .also {
    ...
    fun writeModule(folder: File, sdkLocation: String) {
      ...
      val includeList = modulesPath().joinToString(separator = "\")\ninclude(\"", prefix = "include(\"", postfix = "\")\n")
...
"""
  ...
  private fun withGradleWrapper(gradleVersion: String = SdkConstants.GRADLE_LATEST_VERSION) {
    ...
    TestResources.getFile("/gradlew").copyTo(File("${projectPath}/gradlew")).setExecutable(true)
    TestResources.getFile("/gradlew.bat").copyTo(File("${projectPath}/gradlew.bat"))
    TestResources.getFile("/gradle-wrapper.jar").copyTo(File("${projectPath}/gradle/wrapper/gradle-wrapper.jar"))
    File("${projectPath}/gradle/wrapper/gradle-wrapper.properties")
      .writeText(
...
zipStorePath=wrapper/dists
  ...
  override fun defaultWhen(given: GivenBuilder): BuildResult? {
    ...
    docs.index?.apply { File(projectDir, "readme.md").writeText(this) }
    ...
    val initScript =
      privateTestProjectDir.newFile(scriptingLanguage.makeScriptFileName("init.gradle")).also {
    ...
    Logger.getGlobal()
      .info(
        """Init script:
            ${scriptingLanguage.configureInitScript(mavenRepos)}"""
          .trim()
      )

    val runnerParameters = mutableListOf<String>()
    parameters.map { "-P${it.key.propertyName}=${it.value}" }.toCollection(runnerParameters)
    runnerParameters.addAll(
      listOf("-Dorg.gradle.jvmargs=-Xmx2G", "--init-script", initScript.absolutePath, *given.tasksToInvoke.toTypedArray(), "--stacktrace")
    )
    val gradleRunner =
      GradleRunner.create()
    ...
    if (System.getenv("SRC") != null) {
      ...
      FileReader(File("$srcDir/tools/gradle/wrapper/gradle-wrapper.properties")).apply {
        val gradleWrapperProperties = Properties().also { properties -> properties.load(this) }
        val relativePathDistribution = gradleWrapperProperties["distributionUrl"]
        gradleRunner.withGradleDistribution(File("$srcDir/tools/gradle/wrapper/$relativePathDistribution").toURI())
      }
    }
  ...
  protected fun onVariantStats(block: (GradleBuildVariant) -> Unit) {
    val androidProfile = File(testProjectDir.root, "${testName.methodName}/build/android-profile")
    ...
    val gradleBuildProfile = GradleBuildProfile.parseFrom(Files.readAllBytes(listFiles[0].toPath()))
    Truth.assertThat(gradleBuildProfile).isNotNull()
    Truth.assertThat(gradleBuildProfile.projectList).isNotEmpty()
    gradleBuildProfile.projectList.first { it.androidPluginVersion.isNotEmpty() }.variantList.forEach { variant -> block.invoke(variant) }
  }
}
