...
class AdaptNinjaToCxxBuildTest {
  ...
  fun `simplest viable build dot ninja`() {
    val (config, compileCommandsSummary) =
      adaptNinja(
        """
        rule CLANG
          command = /ndk/clang ${'$'}in -o ${'$'}out
  ...
  fun `simplest viable passthrough`() {
    ...
    config.libraries.getValue("lib").apply { assertThat(hasPassthrough).isTrue() }
  }
  ...
  /**
   * There is a passthrough [lib.passthrough] for a phony [lib] to the actual .so file. The output file should still be 'lib.so' and not
   * 'lib'.
   */
  @Test
  ...
  fun `check isPackageable`() {
    ...
    assertThat(
      ...
      .isTrue()
    assertThat(
        isPackageable("/path/to/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64/sysroot/usr/lib/aarch64-linux-android/21/libEGL.so")
      )
      .isFalse()
  }
  ...
  fun `check precompiled headers`() {
    ...
    val body = summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "/path/to/sources", ndk = "/path/to/ndk")
  ...
  fun `'all' target in sub folder`() {
    ...
    summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "/path/to/sources", ndk = "/path/to/ndk")
  }
  ...
  fun `wrapped clang bug`() {
    ...
    assertThat(config.libraries["hello-jni"]).isNotNull()
    val body = summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "../../../..", ndk = "/path/to/ndk")
  ...
  fun `executable target works like SO target`() {
    ...
    summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "../../../../src", ndk = "/path/to/ndk")
  }
  ...
  fun `sysroot files are not runtime files`() {
    ...
    summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "../../../..", ndk = "/path/to/ndk")
  }
  ...
  fun `dolphin via AGDE 21 2 canary`() {
    ...
    assertThat(body).doesNotContain("Source-File: [SOURCE ROOT]/Build/Android-x86_64/Debug/pch/pch.pch")
    // If [body] gets large it may be a sign that flags aren't being coalesced between different
    // compile commands.
  ...
  fun `orphan dot o should not crash`() {
    ...
    val body =
      summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "undefined", ndk = "undefined", compareSnapshot = false)
  ...
  private fun adaptNinja(ninja: String): Pair<NativeBuildConfigValueMini, String> {
    ...
    val compileCommandsSummary =
      summarizeCompileCommandsJsonBin(buildNinja = buildNinja, sourceRoot = "undefined", ndk = "undefined", compareSnapshot = false)
    return config to compileCommandsSummary
  }

  private fun summarizeCompileCommandsJsonBin(buildNinja: File, sourceRoot: String, ndk: String, compareSnapshot: Boolean = true): String {
    ...
    streamCompileCommands(compileCommandsJsonBin) { commands.add(this) }
    sb.appendLine("# There are ${commands.size} entries.")
    commands
      .sortedBy { it.outputFile.path.lowercase(Locale.getDefault()).replace("\\", "/") }
      .forEach { command ->
    ...
    val currentText = sb.toString().replace(sourceRoot, "[SOURCE ROOT]").replace(ndk, "[NDK]")
  ...
  private fun StringTable.text(value: File, tag: String): String {
    var created = false
    val id = getIdCreateIfAbsent(value.path) { created = true }
    return if (created) {
  ...
  private fun StringTable.text(value: List<String>): String {
    ...
    var created = false
    val render = value.joinToString("\n") { "              $it" }
    val id = getIdCreateIfAbsent(render) { created = true }
    return if (created) {
      "$render (flags$id)"
    } else {
      "             [flags$id]"
  ...
  private val NativeLibraryValueMini.hasPassthrough
    get() = buildCommandComponents?.any { it.contains(".passthrough") } ?: false
}
