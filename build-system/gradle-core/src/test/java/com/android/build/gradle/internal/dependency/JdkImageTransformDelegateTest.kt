...
class JdkImageTransformDelegateTest {
  ...
  fun copyCfsmJarToTempFolder(): File {
    val originalCfsmJar = TestUtils.resolvePlatformPath(SdkConstants.FN_CORE_FOR_SYSTEM_MODULES, TestUtils.TestType.AGP).toFile()
  ...
  fun testModuleDescriptorCompilation() {
    ...
    val delegate = JdkImageTransformDelegate(cfsmJar, tmp.newFolder(), outDir, JdkTools(javaHome, DefaultProcessExecutor(logger), logger))
    ...
    DefaultProcessExecutor(logger).execute(pib.createProcess(), processHandler).rethrowFailure().assertNormalExitValue()
  ...
  fun testDelegateRunMethod() {
    ...
    ZipFileSubject.assertThat(jrtFsJar) { it.doesNotContain("META-INF/MANIFEST.MF") }
    ...
    compileTestClass(
      outDir,
      logger,
      TestResources.getFile(JdkImageTransformDelegateTest::class.java, "Java11Class.java"),
      compiledJava11Dir,
    )
    compileTestClass(
      outDir,
      logger,
      TestResources.getFile(JdkImageTransformDelegateTest::class.java, "Java11ClassWithAndroidApis.java"),
      compiledJava11Dir,
    )
  }

  private fun compileTestClass(systemImageDir: File, logger: ILogger, java11SourceFile: File, compiledJava11Dir: File) {
    // compile a java 11 source file against the system image
    assertThat(java11SourceFile).exists()

    val androidJar = TestUtils.resolvePlatformPath(SdkConstants.FN_FRAMEWORK_LIBRARY, TestUtils.TestType.AGP)
    ...
    DefaultProcessExecutor(logger).execute(pib.createProcess(), LoggedProcessOutputHandler(logger)).rethrowFailure().assertNormalExitValue()

    val compiledJava11File = FileUtils.join(compiledJava11Dir, "${java11SourceFile.nameWithoutExtension}.class")
