...
class ConfigureInvalidationStateTest {
  ...
  private fun getState(forceConfigure: Boolean = false, configureInputFiles: List<File> = listOf()): ConfigureInvalidationState {
  ...
  fun `test initial state`() {
    ...
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- no fingerprint file, will remove stale configuration folder")
  }
  ...
  fun `should hard configure when forceConfigure true`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isFalse()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- force flag, will remove stale configuration folder")
  }
  ...
  fun `should hard configure due to command-file change`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isFalse()
    assertThat(state.shouldConfigureReasonMessages)
      .containsExactly(
  ...
  fun `should configure due to configure input files changed`() {
    ...
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $cmakeListsFile (LAST_MODIFIED_CHANGED)")
  }
  ...
  fun `should configure due to discovered input files changed`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isTrue()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $discoveredCmakeFile (LAST_MODIFIED_CHANGED)")
  }
  ...
  fun `should configure due to configure outputs files changed`() {
    ...
    val state = getState()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $expectedJson (LAST_MODIFIED_CHANGED)")
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isTrue()
  }
  ...
  fun `should configure on added expected output file`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isTrue()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $buildNinjaFile (CREATED)")
  }
  ...
  fun `should configure on deleted output file`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isTrue()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $expectedJson (DELETED)")
  }
  ...
  fun `should configure due to missing input file`() {
    ...
    assertThat(state.shouldConfigure).isTrue()
    assertThat(state.softConfigureOkay).isTrue()
    assertThat(state.shouldConfigureReasonMessages).containsExactly("- a file changed", "  - $cmakeListsFile (DELETED)")
  }
  ...
  private fun ConfigureInvalidationState.describe(): String {
    ...
    val sb = StringBuilder()
    sb.appendLine()
    shouldConfigureReasonMessages.forEach { message -> sb.appendLine(clean(message)) }
    sb.appendLine(stateText)

    return "$sb".trim('\n')
  }

  fun clean(value: CharSequence) = "$value".replace("\\\\", "/").replace(temporaryFolder.path.replace("\\", "/"), "TEST")
}
