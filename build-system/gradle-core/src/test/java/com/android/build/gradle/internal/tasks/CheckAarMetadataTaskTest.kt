...
class CheckAarMetadataTaskTest {
  ...
  abstract class CheckAarMetadataTaskForTest @Inject constructor(testWorkerExecutor: WorkerExecutor) : CheckAarMetadataTask() {
  ...
  fun testFailsOnAarFormatVersion() {
    ...
    task.aarMetadataArtifacts =
      FakeArtifactCollection(
        mutableSetOf(FakeResolvedArtifactResult(file = aarMetadataFile, identifier = FakeComponentIdentifier("displayName")))
      )
  ...
  fun testFailsOnDesugarJdkLibId() {
    ...
    task.desugarJdkLibDependencyGraph.set(
      FakeResolvedComponentResult(
        dependencies =
          mutableSetOf(
            FakeResolvedDependencyResult(requested = FakeModuleComponentSelector("com.android.tools", "desugar_jdk_libs", "1.1.5"))
          )
      )
    )
    try {
