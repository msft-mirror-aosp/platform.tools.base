...
class TaskBasedOperationsImplTest {
  ...
  fun setUp() {
    ...
    artifacts
      .getArtifactContainer(InternalArtifactType.COMPATIBLE_SCREEN_MANIFEST)
      .initArtifactContainer(mock<TaskProvider<*>>(), inputFolderProperty, mock<ArtifactNamingContext>())
  ...
  abstract class SynchronousTask @Inject constructor(val workers: WorkerExecutor) : VariantTask, DefaultTask() {
    ...
    @get:Internal lateinit var replacementRequest: ArtifactTransformationRequest<SynchronousTask>
    ...
    class CreationAction(val component: VariantCreationConfig) :
      VariantTaskCreationAction<SynchronousTask, VariantCreationConfig>(component) {
      ...
      override fun handleProvider(taskProvider: TaskProvider<SynchronousTask>) {
        ...
        replacementRequest =
          component.artifacts
            .use(taskProvider)
            .wiredWithDirectories(SynchronousTask::inputDir, SynchronousTask::outputDir)
            .toTransformMany(InternalArtifactType.COMPATIBLE_SCREEN_MANIFEST, InternalArtifactType.PACKAGED_MANIFESTS)
      }
  ...
  fun synchronousApiTest() {
    ...
    task.execute()
    @Suppress("UNCHECKED_CAST") (task.replacementRequest as ArtifactTransformationRequestImpl<SynchronousTask>).wrapUp(task)

    checkOutputFolderCorrectness(outputFolder)
  }

  abstract class InternalApiTask @Inject constructor(var workers: WorkerExecutor) : NonIncrementalTask() {
    ...
    @get:Internal lateinit var replacementRequest: ArtifactTransformationRequest<InternalApiTask>
    ...
    abstract class WorkItem @Inject constructor(private val workItemParameters: WorkItemParameters) :
      ...
      override fun doExecute() {
        ...
        workItemParameters.outputFile.get().writeText(workItemParameters.builtArtifact.get().filters.joinToString { it.identifier })
      }
    }
    ...
    class CreationAction(val component: VariantCreationConfig) :
      VariantTaskCreationAction<InternalApiTask, VariantCreationConfig>(component) {
      override val name: String
        get() = computeTaskName("foo", "bar")
      ...
      override fun handleProvider(taskProvider: TaskProvider<InternalApiTask>) {
        ...
        replacementRequest =
          component.artifacts
            .use(taskProvider)
            .wiredWithDirectories(InternalApiTask::inputDir, InternalApiTask::outputDir)
            .toTransformMany(InternalArtifactType.COMPATIBLE_SCREEN_MANIFEST, InternalArtifactType.PACKAGED_MANIFESTS)
      }
  ...
  private fun asynchronousTest(workers: WorkerExecutor) {
    ...
    val analyticsService = getBuildService(project.gradle.sharedServices, AnalyticsService::class.java)
    analyticsService.get().onFinish(finishEvent)
    // force wrap up
    @Suppress("UNCHECKED_CAST") (task.replacementRequest as ArtifactTransformationRequestImpl<InternalApiTask>).wrapUp(task)
  ...
  private fun internalAgpApiTestWithProfiler(waiting: Boolean) {
    asynchronousTest(workers = getFakeWorkerExecutor(waiting = waiting))
    val profile = loadProfile(profileDir)
    Truth.assertThat(profile.spanCount).isEqualTo(6)
    // assert task execution span is recorded
    Truth.assertThat(getRecordForId(profile.spanList, 2).type).isEqualTo(GradleBuildProfileSpan.ExecutionType.TASK_EXECUTION)
    // assert worker spans are recorded
    val workerSpans = profile.spanList.filter { it.hasId() && it.type == GradleBuildProfileSpan.ExecutionType.THREAD_EXECUTION }
  ...
  /**
   * Create fake [WorkerExecutor] that can only return a WorkQueue processing its job synchronously.
   *
  ...
  private fun checkOutputFolderCorrectness(outputFolder: File) {
    ...
    val updatedBuiltArtifacts = BuiltArtifactsLoaderImpl().load(project.layout.projectDirectory.dir(outputFolder.absolutePath))
    ...
    updatedBuiltArtifacts.elements.forEach { builtArtifactImpl ->
      Truth.assertThat(File(builtArtifactImpl.outputFile).absolutePath).startsWith(outputFolder.absolutePath)
  ...
  private fun createBuiltArtifact(outputFolder: File, fileName: String, densityValue: String) =
  ...
  private fun getRecordForId(records: List<GradleBuildProfileSpan>, recordId: Long): GradleBuildProfileSpan {
    ...
    throw AssertionError("No record with id $recordId  found in [${Joiner.on(", ").join(records)}]")
  }
}
