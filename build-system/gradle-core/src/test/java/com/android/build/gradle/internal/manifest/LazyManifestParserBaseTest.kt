...
internal abstract class LazyManifestParserBaseTest :
  AbstractBuildGivenBuildExpectTest<LazyManifestParserBaseTest.GivenBuilder, LazyManifestParserBaseTest.Result>() {
  ...
  override fun defaultWhen(given: GivenBuilder): Result? {
    ...
    val manifestFile =
      given.manifestFile
        ?: if (manifestValue != null) {
          temporaryFolder.newFile("AndroidManifest.xml").also { it.writeText(manifestValue) }
        } else {
    ...
    return Result(manifestParser.manifestData.get(), projectServices.issueReporter.syncIssues)
  }
  ...
  override fun compareResult(
    ...
    expected.packageName?.let { Truth.assertThat(actual.packageName).named("packageName").isEqualTo(it) }

    expected.split?.let { Truth.assertThat(actual.split).named("split").isEqualTo(it) }

    expected.minSdkVersion?.let { Truth.assertThat(actual.minSdkVersion).named("minSdkVersion").isEqualTo(it) }

    expected.targetSdkVersion?.let { Truth.assertThat(actual.targetSdkVersion).named("targetSdkVersion").isEqualTo(it) }

    expected.instrumentationRunner?.let { Truth.assertThat(actual.instrumentationRunner).named("instrumentationRunner").isEqualTo(it) }

    expected.testLabel?.let { Truth.assertThat(actual.testLabel).named("testLabel").isEqualTo(it) }

    expected.functionalTest?.let { Truth.assertThat(actual.functionalTest).named("functionalTest").isEqualTo(it) }

    expected.handleProfiling?.let { Truth.assertThat(actual.handleProfiling).named("handleProfiling").isEqualTo(it) }

    expected.extractNativeLibs?.let { Truth.assertThat(actual.extractNativeLibs).named("extractNativeLibs").isEqualTo(it) }

    expected.useEmbeddedDex?.let { Truth.assertThat(actual.useEmbeddedDex).named("useEmbeddedDex").isEqualTo(it) }

    expected.isolatedSplits?.let { Truth.assertThat(actual.isolatedSplits).named("isolatedSplits").isEqualTo(it) }

    // finally check for errors
    issueChecker?.invoke(actualMaybe.issues)
      ?: run {
  ...
  internal class Result(val data: ManifestData = ManifestData(), val issues: MutableList<SyncIssue> = mutableListOf()) {
    fun data(action: ManifestData.() -> Unit) {
      action(data)
    }

    fun issue(action: SyncIssueBuilder.() -> Unit) {
      val builder = SyncIssueBuilder().also { action(it) }
