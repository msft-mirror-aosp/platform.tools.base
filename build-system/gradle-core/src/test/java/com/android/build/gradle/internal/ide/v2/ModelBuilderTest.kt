...
class ModelBuilderTest {
  ...
  fun `test canBuild`() {
    fun testModel(name: String) {
      Truth.assertThat(createApplicationModelBuilder().canBuild(name)).named("can build '$name").isTrue()
    }
  ...
  fun `test wrong model`() {
    ...
    Truth.assertThat(modelBuilder.canBuild("com.FooModel")).named("can build com.FooModel").isFalse()
  ...
  private val dslServices = createDslServices(projectServices = projectServices, sdkComponents = sdkComponentProvider)
  ...
  data class FakeModelBuilderParameter(
    ...
    override var dontBuildHostTestRuntimeClasspath: Map<String, Boolean> = mapOf("UnitTest" to false, "ScreenshotTest" to false),
