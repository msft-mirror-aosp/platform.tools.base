...
class DesugarGraphTest {
  ...
  fun `test write and read`() {
    ...
    val desugarGraph = DesugarGraph(rootDir = rootDir, relocatableDesugarGraph = MutableDependencyGraph(), filesToIgnore = emptySet())
  ...
  fun `test files located outside rootDir`() {
    ...
    val desugarGraph = DesugarGraph(rootDir = rootDir, relocatableDesugarGraph = MutableDependencyGraph(), filesToIgnore = setOf(bKt))
    ...
    // expect failure
    val exception = assertFailsWith<IllegalStateException> { desugarGraph.addEdge(aKt, cKt) }
    assertThat(exception.message).isEqualTo("The given file '$cKt' is located outside the root directory '$rootDir'")
  }
}
