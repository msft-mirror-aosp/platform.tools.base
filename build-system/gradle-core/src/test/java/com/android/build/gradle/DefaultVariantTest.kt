...
class DefaultVariantTest : AbstractVariantInputModelTest<String>() {
  ...
  fun `test removed all variants returns null`() {
    ...
    withVariantFilter { variant -> variant.ignore = true }
  ...
  fun `test default variant build type explicit`() {
    ...
    given { android { buildTypes { create("dev") { isDefault = true } } } }
  ...
  fun `test alphabetical flavor choice`() {
    ...
    given {
      android {
        productFlavors {
          create("f1") { dimension = "dim" }
          create("f2") { dimension = "dim" }
          create("f3") { dimension = "dim" }
        }
      }
    }
  ...
  fun `test alphabetical default flavor with multiple dimensions`() {
    ...
    given {
      android {
        productFlavors {
          create("f1") { dimension = "1" }
          create("f2") { dimension = "1" }
          create("f3") { dimension = "2" }
          create("f4") { dimension = "2" }
        }
      }
    }
  ...
  fun `test removal default flavor with multiple dimensions`() {
    ...
    given {
      android {
        productFlavors {
          create("f1") { dimension = "number" }
          create("f2") { dimension = "number" }
          create("fa") { dimension = "letter" }
          create("fb") { dimension = "letter" }
        }
      }
    }
  ...
  fun `default respects tested build type`() {
    ...
    given { android { buildTypes { create("a") } } }
  ...
  fun `check task name generation`() {
    ...
    assertThat(assertFailsWith<IllegalArgumentException> { component.computeTaskName("", "ManifestReport") })
      .hasMessageThat()
      .isEqualTo("Action parameter must not be empty or blank")

    assertThat(assertFailsWith<IllegalArgumentException> { component.computeTaskName("produce", "") })
      .hasMessageThat()
      .isEqualTo("Subject parameter must not be empty or blank")
  }
  ...
  override fun defaultWhen(given: TestVariantInputModel): String? {
    ...
    val variantComputer = DimensionCombinator(given, dslServices.issueReporter, given.implicitFlavorDimensions)
    ...
    for (variant in variantComputer.computeVariants()) {
      ...
      val flavors = variant.productFlavors.map { (given.productFlavors[it.second] ?: error("Cant find flavor ${it.second}")).productFlavor }

      // run the filter
      var ignore = false
      if (variantFilter != null) {
        val variantInfo =
          VariantFilterImpl(name, given.defaultConfigData.defaultConfig, given.buildTypes[variant.buildType]!!.buildType, flavors)
      ...
      if (!ignore) {
        ...
        whenever(component.productFlavorList).thenReturn(flavors.map { com.android.build.gradle.internal.core.ProductFlavor(it) })
      }
    }
    ...
    return VariantModelImpl(
        ...
        { listOf() },
        { listOf() },
        { BuildFeatureValuesImpl(dslServices.newInstance(ApplicationBuildFeaturesImpl::class.java), mock<ProjectServices>()) },
