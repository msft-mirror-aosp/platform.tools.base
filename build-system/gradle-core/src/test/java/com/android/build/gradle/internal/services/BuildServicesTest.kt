...
class BuildServicesTest {
  ...
  fun `test global BuildService`() {
    ...
    val differentClassloader = CustomClassLoader { className: String -> className.startsWith(BuildServicesTest::class.java.packageName) }
    ...
    assertEquals("Returned value from $actualBuildService", buildService.methodCallWithBootstrapTypes(123))
    assertEquals("Returned value from $actualBuildService", buildService.methodCallWithInterfaceTypes(FooImpl("Some text")).data)
    assertEquals("Returned value from $actualBuildService", buildService.methodCallWithSerializableTypes(FooSerializable("Some text")).data)
  }
}

/**
 * Custom [ClassLoader] that loads a [Class] if the class's name satisfies the given [classNameFilter], otherwise it delegates loading the
 * class to the default (system) class loader.
 */
@NotThreadSafe
...
abstract class ExampleBuildServiceImpl : BuildService<BuildServiceParameters.None>, ExampleBuildService {
  ...
  class RegistrationAction(project: Project) :
    GlobalServiceRegistrationAction<ExampleBuildService, ExampleBuildServiceImpl, BuildServiceParameters.None>(
      project,
      ExampleBuildService::class.java,
      ExampleBuildServiceImpl::class.java,
    ) {
