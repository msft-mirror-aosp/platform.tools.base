...
class ExecuteProcessTest {
   ...
   * - 'my-script.bat' to execute on Windows It is meant to prove that these two scripts can coexist and the correct one is executed on the
   *   current platform.
   */
  @Test
  ...
  fun `check problematic strings in arguments can round trip`() {
    ...
    checkRoundtrip("'")
    checkRoundtrip("\u0008")
    checkRoundtrip("\u0000", posix = "argument had embedded 0x0000", windows = "argument had embedded 0x0000")
    checkRoundtrip("\r", windows = "argument had embedded line-feed (\\r)")
    checkRoundtrip("\n", windows = "argument had embedded carriage-return (\\n)")
    checkRoundtrip("`", windows = "argument had embedded tick-mark (`)")
  }

  /**
   * This is a continuation of `check problematic strings in arguments can round trip` test above. It also checks that script file paths
   * behave as expected when the script name has unusual characters embedded in it. See https://issuetracker.google.com/272534943 Gradle's
   * prefab build fails on path with parentheses
   */
  @Test
  ...
  private data class WorkingContext(val workingDir: File, val stdout: File, val stderr: File)
  ...
  private fun createWorkingContext(scriptsBaseName: String = "scripts") = run {
    ...
    // test code.
    val scriptsBaseNameCleaned = scriptsBaseName.replace("?", "_").replace("*", "_").replace("\u0000", "_").replace('/', '_')
    val workingDir = tempFolder.newFolder().resolve(scriptsBaseNameCleaned).also { it.mkdir() }
    WorkingContext(workingDir, workingDir.resolve("stdout.txt"), workingDir.resolve("stderr.txt"))
  }
   ...
   * - The Windows script will be written to the same file but with '.bat' extension. When the script is invoked by the shell then
   *   [callback] will be called with the command-line arguments.
   *
   * Usage,
   * ```
   ...
   * ```
   *
   * where, {classpath:of:this:test} -- is the JAVA class path of this test.
   * com.android.build.gradle.internal.cxx.process.ShellScriptCallback -- is the java entry point to execute.
   * tmp/path/to//write-main-callback-shell-script.bin -- is the serialization of the 'callback' passed to createCallbackShellScripts(...)
  ...
  private fun WorkingContext.createCallbackShellScripts(posixScriptBase: String, callback: (args: Array<String>) -> Unit): File {
    ...
    // with unsupported chars and check the exceptions from shipping code rather than this
    // test code.
    val posixScriptBaseCleaned = posixScriptBase.replace("?", "_").replace("*", "_").replace("\u0000", "_").replace('/', '_')
    val context = callback as Serializable
    val contextFile = File.createTempFile("write-main-callback-shell-script", "bin")
    ObjectOutputStream(FileOutputStream(contextFile)).use { objects -> objects.writeObject(context) }
    val runtime = ManagementFactory.getRuntimeMXBean()
    val sb = StringBuilder()
    val java = File(runtime.systemProperties["java.home"]!!).resolve("bin/java")
    val classPath =
      runtime.systemProperties["java.class.path"]!!
