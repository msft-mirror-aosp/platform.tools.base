...
/**
 * A test to assert compatibility and correct conversion between v2 models and their proto representation used in kotlin multiplatform
 * android import.
 */
class ConvertersTest {
  ...
  fun convertTestInfo() {
    ...
    val result =
      assertAllGettersMatch(obj = testInfo, objType = TestInfo::class.java, convertedObj = testInfo.convert(), callChain = "TestInfo")
  ...
  fun convertExecutionEnum() {
    TestInfo.Execution.values().forEach {
      ...
      Truth.assertWithMessage(enumMissingErrorMessage("TestInfo.Execution")).that(it.name).isEqualTo(protoValue.name)
    }
  }
  ...
  fun convertBooleanFlag() {
    ...
    AndroidGradlePluginProjectFlags.BooleanFlag.values().forEach {
      ...
      if (unrecognized.contains(it)) {
        Truth.assertWithMessage(enumMissingErrorMessage("AndroidGradlePluginProjectFlags.BooleanFlag")).that(protoValue).isNull()
      } else {
        Truth.assertWithMessage(enumMissingErrorMessage("AndroidGradlePluginProjectFlags.BooleanFlag"))
          .that(it.name)
          .isEqualTo(protoValue!!.name)
      }
    }
  }
  ...
  fun testAndroidLibraryConversion() {
    ...
    val result =
      assertAllGettersMatch(obj = library, convertedObj = library.convert().build(), objType = Library::class.java, callChain = "Library")
  ...
  private fun assertAllGettersMatch(obj: Any, objType: Class<*>, convertedObj: Any, callChain: String): VisitorResult {
    if (objType.isEnum) {
      Truth.assertWithMessage("Expected $callChain to be $obj, instead was $convertedObj")
        .that(obj.toString())
        .isEqualTo(convertedObj.toString())
      return VisitorResult(1)
    } else if (Collection::class.java.isAssignableFrom(objType)) {
      Truth.assertWithMessage("Expected $callChain to have the same list size")
        .that((obj as Collection<*>).size)
        .isEqualTo((convertedObj as List<*>).size)
      ...
      Truth.assertWithMessage("Expected $callChain to be the same").that(obj as Map<*, *>).isEqualTo(convertedObj as Map<*, *>)
      return VisitorResult(1)
    }

    when (objType) {
      String::class.java -> {
        Truth.assertWithMessage("Expected $callChain to be $obj, instead was $convertedObj").that(obj).isEqualTo(convertedObj)
        return VisitorResult(1)
      }
      File::class.java -> {
        Truth.assertWithMessage("Expected $callChain to be $obj, instead was $convertedObj")
          .that((obj as File).absolutePath)
          .isEqualTo((convertedObj as com.android.builder.model.proto.ide.File).absolutePath)
        return VisitorResult(1)
      }
    }

    fun getProtoEquivalent(convertedObj: Any, methodName: String): Any? {
    ...
    objType.methods.toList().forEach { method ->
      ...
      val converted = getProtoEquivalent(objectMapper(convertedObj, method), methodMapper(method))

      if (original == null) {
        Truth.assertWithMessage("Expected $currentCallChain to be null, instead was $converted").that(converted).isNull()
      } else {
        Truth.assertWithMessage("Expected $currentCallChain to be $original, instead was null").that(converted).isNotNull()

        val ret =
          assertAllGettersMatch(obj = original, objType = method.returnType, convertedObj = converted!!, callChain = currentCallChain)
