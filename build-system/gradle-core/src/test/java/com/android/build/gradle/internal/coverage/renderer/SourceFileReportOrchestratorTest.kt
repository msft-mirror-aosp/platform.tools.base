...
class SourceFileReportOrchestratorTest {
    ...
    fun `orchestrate creates correct file with aggregated content from multiple variants`() {
        ...
        val builder = SourceFileReportBuilder(packageFlattenedPath = sourceFileLogicalPath).apply {
            variantFileCoverageBuilders["debug"] = VariantFileCoverageBuilder().apply {
                testSuiteFileCoverageBuilders["test"] = TestSuiteFileCoverageBuilder(
                    fileCoverage = CoverageInfo(percent = 80, covered = 8, total = 10),
                    lineCoverageBuilders = mutableMapOf(
                        4 to LineCoverageBuilder(
                            CoverageInfo(percent = 100, covered = 4, total = 4),
                            CoverageInfo(0, 0, 0)
                        )
                    )
                )
                testSuiteFileCoverageBuilders["androidTest"] = TestSuiteFileCoverageBuilder(
            ...
            variantFileCoverageBuilders["release"] = VariantFileCoverageBuilder().apply {
                testSuiteFileCoverageBuilders["test"] = TestSuiteFileCoverageBuilder(
                    ...
                    lineCoverageBuilders = mutableMapOf(
                        4 to LineCoverageBuilder(
                            CoverageInfo(percent = 100, covered = 2, total = 2),
                            CoverageInfo(0, 0, 0)
                        )
                    )
                )
        ...
        val sourceFileReportsBuilder = SourceFileReportsBuilder().apply {
            sourceFileBuilders[sourceFilePhysicalPath] = builder
        }
    ...
    fun `orchestrate with missing source file throws exception`() {
        ...
        val builder = SourceFileReportBuilder(missingSourceLogicalPath).apply {
            variantFileCoverageBuilders["debug"] = VariantFileCoverageBuilder().apply {
                testSuiteFileCoverageBuilders["test"] = TestSuiteFileCoverageBuilder(
                    fileCoverage = CoverageInfo(10, 1, 10)
                )
            }
        }
        val sourceFileReportsBuilder = SourceFileReportsBuilder().apply {
            sourceFileBuilders[missingSourcePhysicalPath] = builder
        }
