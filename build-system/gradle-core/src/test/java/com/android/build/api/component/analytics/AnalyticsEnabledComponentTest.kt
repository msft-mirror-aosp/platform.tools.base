...
class AnalyticsEnabledComponentTest {
  ...
  private val proxy: AnalyticsEnabledComponent by lazy { object : AnalyticsEnabledComponent(delegate, stats, FakeObjectFactory.factory) {} }
  ...
  fun transformClasspathWith() {
    whenever(delegate.instrumentation).thenReturn(mock<Instrumentation>())
    val block = { _: InstrumentationParameters -> }
    proxy.instrumentation.transformClassesWith(MockedVisitor::class.java, InstrumentationScope.PROJECT, block)

    proxy.instrumentation.transformClassesWith(MockedVisitor::class.java, InstrumentationScope.ALL, block)

    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessCount).isEqualTo(4)
    Truth.assertThat(
        stats.variantApiAccess.variantPropertiesAccessList.count { it.type == VariantPropertiesMethodType.INSTRUMENTATION_VALUE }
      )
      .isEqualTo(2)
    Truth.assertThat(
    ...
    Truth.assertThat(stats.asmClassesTransformsList.first().scope).isEqualTo(AsmClassesTransformRegistration.Scope.PROJECT)
    Truth.assertThat(stats.asmClassesTransformsList.last().scope).isEqualTo(AsmClassesTransformRegistration.Scope.ALL)

    val instrumentationDelegate = (proxy.instrumentation as AnalyticsEnabledInstrumentation).delegate
    verify(instrumentationDelegate, times(1)).transformClassesWith(MockedVisitor::class.java, InstrumentationScope.PROJECT, block)
    verify(instrumentationDelegate, times(1)).transformClassesWith(MockedVisitor::class.java, InstrumentationScope.ALL, block)
  }
  ...
  fun setAsmFramesComputationNode() {
    ...
    proxy.instrumentation.setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_METHODS)
    proxy.instrumentation.setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_CLASSES)
    proxy.instrumentation.setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_ALL_CLASSES)
    ...
    Truth.assertThat(stats.framesComputationModeUpdatesList[0].mode).isEqualTo(AsmFramesComputationModeUpdate.Mode.COPY_FRAMES)
    Truth.assertThat(stats.framesComputationModeUpdatesList[1].mode)
      .isEqualTo(AsmFramesComputationModeUpdate.Mode.COMPUTE_FRAMES_FOR_INSTRUMENTED_METHODS)
    Truth.assertThat(stats.framesComputationModeUpdatesList[2].mode)
      .isEqualTo(AsmFramesComputationModeUpdate.Mode.COMPUTE_FRAMES_FOR_INSTRUMENTED_CLASSES)
    Truth.assertThat(stats.framesComputationModeUpdatesList[3].mode)
      .isEqualTo(AsmFramesComputationModeUpdate.Mode.COMPUTE_FRAMES_FOR_ALL_CLASSES)

    val instrumentationDelegate = (proxy.instrumentation as AnalyticsEnabledInstrumentation).delegate

    verify(instrumentationDelegate, times(1)).setAsmFramesComputationMode(FramesComputationMode.COPY_FRAMES)
    verify(instrumentationDelegate, times(1)).setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_METHODS)
    verify(instrumentationDelegate, times(1)).setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_INSTRUMENTED_CLASSES)
    verify(instrumentationDelegate, times(1)).setAsmFramesComputationMode(FramesComputationMode.COMPUTE_FRAMES_FOR_ALL_CLASSES)
  }
  ...
  fun instrumentationExcludes() {
    ...
    Truth.assertThat(
      ...
      .isEqualTo(2)
    Truth.assertThat(
        stats.variantApiAccess.variantPropertiesAccessList.count { it.type == VariantPropertiesMethodType.INSTRUMENTATION_EXCLUDES_VALUE }
      )
      .isEqualTo(2)
  }
  ...
  fun getBuildType() {
    ...
    verify(delegate, times(1)).buildType
  }
  ...
  fun getProductFlavors() {
    ...
    verify(delegate, times(1)).productFlavors
  }
  ...
  fun getFlavorName() {
    ...
    verify(delegate, times(1)).flavorName
  }
  ...
  fun getJavaCompilation() {
    ...
    verify(delegate, times(1)).javaCompilation
  }
  ...
  fun getAnnotationProcessorConfiguration() {
    ...
    whenever(delegate.annotationProcessorConfiguration).thenReturn(annotationProcessorConfiguration)
    Truth.assertThat(proxy.annotationProcessorConfiguration).isEqualTo(annotationProcessorConfiguration)
  ...
  fun getLifecycleTasks() {
    ...
    Truth.assertThat(lifecycleTasks).isEqualTo((anchorTasksProxy as AnalyticsEnabledLifecycleTasks).delegate)

    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessList.first().type)
      .isEqualTo(VariantPropertiesMethodType.LIFECYCLE_TASKS_VALUE)
    verify(delegate, times(1)).lifecycleTasks
  }
  ...
  fun configureJavaCompileTask() {
    ...
    verify(delegate, times(1)).configureJavaCompileTask(action)
  }
}
