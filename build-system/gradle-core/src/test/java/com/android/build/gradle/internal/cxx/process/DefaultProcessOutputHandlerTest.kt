...
class DefaultProcessOutputHandlerTest {
  ...
  private val stdoutFile: File by lazy { temporaryFolderRule.root.resolve("stdout") }
  private val stderrFile: File by lazy { temporaryFolderRule.root.resolve("stderr") }

  @Test
  fun `do not log anything`() =
    check(logStdout = false, logStderr = false, logFullStdout = false, stdout = "stdout", stderr = "stderr", lifecycle = "")

  @Test
  fun `only log stderr`() =
    check(logStdout = false, logStderr = true, logFullStdout = false, stdout = "stdout", stderr = "stderr", lifecycle = "C/C++: stderr")

  @Test
  fun `only log compiler output from stdout`() =
    check(
      logStdout = true,
      logStderr = false,
      logFullStdout = false,
  ...
  @Test
  fun `skip logging ninja directory line if no compiler output`() =
    check(
      logStdout = true,
      logStderr = false,
      logFullStdout = false,
  ...
  @Test
  fun `log ninja directory line if there are compiler outputs`() =
    check(
      logStdout = true,
      logStderr = false,
      logFullStdout = false,
  ...
  fun fuzz() {
    ...
    (0L until 100).forEach { n ->
      RandomInstanceGenerator(n).synthetics(StdOut::class.java, 1000).forEach { lines ->
        val stdout = lines.lines.joinToString("\n")
        try {
          check(logStdout = true, logStderr = false, logFullStdout = false, stdout = stdout, stderr = "stderr")
        } catch (e: Exception) {
