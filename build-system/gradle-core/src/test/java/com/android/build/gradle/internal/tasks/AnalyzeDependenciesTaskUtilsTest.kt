...
class AnalyzeDependenciesTaskUtilsTest {
  ...
  fun testClassHolder() {
    ...
    TestInputsGenerator.pathWithClasses(classesDir, ImmutableList.of<Class<*>>(Animal::class.java))

    val classesArtifacts =
      FakeArtifactCollection(mutableSetOf(FakeResolvedArtifactResult(classesDir.toFile(), FakeComponentIdentifier("foo"))))
    ...
    assertThat(publicClasses).containsExactly("com/android/build/gradle/internal/transforms/testdata/Animal.class")
  }
  ...
  fun testClassFinder() {
    ...
    TestInputsGenerator.pathWithClasses(fooJar, ImmutableList.of<Class<*>>(class1, class2))
    TestInputsGenerator.pathWithClasses(barJar, ImmutableList.of<Class<*>>(class3))
    ...
    val fooClasses = writeListToFile(File(fooDependencySources, "classes.txt"), getSortedRelativePathsInJar(fooJar.toFile()))
    val barClasses = writeListToFile(File(barDependencySources, "classes.txt"), getSortedRelativePathsInJar(barJar.toFile()))
    ...
    assertThat(finder.findClassesInDependency(fooPackage)).containsExactly(getClassName(class1), getClassName(class2))
    assertThat(finder.findClassesInDependency(barPackage)).containsExactly(getClassName(class3))
  }
  ...
  fun testResourceFinder() {
    ...
    val appManifest =
      ...
      """
        .trimIndent()
    val appManifestFile = File(appProject, SdkConstants.ANDROID_MANIFEST_XML).also { it.writeText(appManifest) }
    ...
    val fooResSymbols =
      writeListToFile(File(fooDependencySources, "resources_symbols${SdkConstants.DOT_TXT}"), getResourcesFromExplodedAarToFile(fooProject))
    val barResSymbols =
      writeListToFile(File(barDependencySources, "resources_symbols${SdkConstants.DOT_TXT}"), getResourcesFromExplodedAarToFile(barProject))
    val bazResSymbols =
      writeListToFile(File(bazDependencySources, "resources_symbols${SdkConstants.DOT_TXT}"), getResourcesFromExplodedAarToFile(bazProject))
    ...
    assertThat(finder.findUsedDependencies()).containsExactlyElementsIn(setOf(fooProject.name, barProject.name))
    assertThat(finder.findUnUsedDependencies()).containsExactlyElementsIn(setOf(bazProject.name))
  ...
  fun testResourceFinderIdentifiesManifestReferences() {
    ...
    val artifacts =
      FakeArtifactCollection(
        mutableSetOf(
          FakeResolvedArtifactResult(fooResSymbols, FakeComponentIdentifier(fooProject.name)),
          FakeResolvedArtifactResult(emptyResSymbols, FakeComponentIdentifier(emptyProject.name)),
        )
      )
  ...
  fun testDependencyUsage() {
    ...
    val dep1Classes = writeListToFile(File(dependency1Sources, "classes.txt"), getSortedRelativePathsInJar(jarA.toFile()))
    val dep2Classes = writeListToFile(File(dependency2Sources, "classes.txt"), getSortedRelativePathsInJar(jarB.toFile()))
    val dep3Classes = writeListToFile(File(dependency3Sources, "classes.txt"), getSortedRelativePathsInJar(jarC.toFile()))

    // Create an ArtifactCollection
    val externalArtifactCollection =
      FakeArtifactCollection(
        mutableSetOf(
    ...
    val projectClassesArtifact =
      FakeArtifactCollection(mutableSetOf(FakeResolvedArtifactResult(classesDir.toFile(), FakeComponentIdentifier("project"))))
    ...
    val dependencyUsageFinder = DependencyUsageFinder(classFinder, variantClasses, variantDependencies)

    assertThat(dependencyUsageFinder.usedDirectDependencies).containsExactly("com/example:AnotherDependency:2")
    assertThat(dependencyUsageFinder.requiredDependencies).containsExactly("com/example:AnotherDependency:2", "com/library:Dependency:1")
    assertThat(dependencyUsageFinder.unusedDirectDependencies).containsExactly("com/test:NewDependency:3")
  }
  ...
  fun testDependencyGraph() {
    ...
    */
    ...
    assertThat(dependencyUsageFinder.usedDirectDependencies).containsExactly("com/library:Dependency:1")
    assertThat(dependencyUsageFinder.requiredDependencies).containsExactly("com/library:Dependency:1", "com/example:AnotherDependency:2")
    assertThat(dependencyUsageFinder.unusedDirectDependencies).containsExactly("com/test:NewDependency:3")
  ...
  private fun getClassName(cls: Class<*>): String = cls.name.replace(".", "/").plus(SdkConstants.DOT_CLASS)

  private class FakeDependency(private val fakeGroup: String, private val fakeName: String, private val fakeVersion: String) : Dependency {
    ...
    @Incubating override fun because(var1: String?) {}
  }
}
