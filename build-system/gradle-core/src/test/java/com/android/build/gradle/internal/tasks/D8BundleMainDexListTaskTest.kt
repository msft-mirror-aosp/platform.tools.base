...
class D8BundleMainDexListTaskTest {
  ...
  @get:Rule val tmpDir: TemporaryFolder = TemporaryFolder()
  ...
  fun testProguardRules() {
    ...
    executeWorkerAction(output = output, programDexFiles = programDexFiles, proguardRulesFiles = listOf(proguardRules))
  ...
  fun testAllInputs() {
    ...
    executeWorkerAction(output = output, programDexFiles = programDexFiles, proguardRulesFiles = listOf(proguardRules, userProguardRules))
  ...
  fun testUserClassesKeptAndDeDuped() {
    ...
    userClasses.writeText(
      listOf("test/User1.class", "test/User2.class", "test/User2.class").joinToString(separator = System.lineSeparator())
    )
    executeWorkerAction(output = output, programDexFiles = programDexFiles, userMultidexKeepFile = userClasses)
  ...
  fun testNoneKept() {
    ...
    executeWorkerAction(output = output, programDexFiles = programDexFiles)
  ...
  fun testThrowsIfDuplicateClasses() {
    ...
    val programDexFiles = listOf(generateDexArchive(listOf("test/A")), generateDexArchive(listOf("test/A")))

    val exception =
      assertFailsWith(D8MainDexList.MainDexListException::class) { executeWorkerAction(output = output, programDexFiles = programDexFiles) }
        ...
        override fun getParameters() =
          object : Params() {
            ...
            override val bootClasspath: ConfigurableFileCollection
              get() = FakeConfigurableFileCollection(TestUtils.resolvePlatformPath("android.jar", TestUtils.TestType.AGP).toFile())
  ...
  private fun generateDexArchive(emptyClassNames: List<String>): File {
    ...
    val emptyClassesDir = tmpDir.newFolder().also { TestInputsGenerator.dirWithEmptyClasses(it.toPath(), emptyClassNames) }
  ...
  private fun generateDexArchive(classesDirOrJar: File, dexArchivePath: File, globalSyntheticsFile: File) {
    ...
    ClassFileInputs.fromPath(classesDirOrJar.toPath()).use { input ->
      builder.convert(input.entries { _, _ -> true }, dexArchivePath.toPath(), globalSyntheticsFile.toPath())
    }
  }
}
