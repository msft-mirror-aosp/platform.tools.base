...
class DexArchiveBuilderDelegateDesugaringTest {
  ...
  @Rule @JvmField var tmpDir = TemporaryFolder()
  ...
  fun testLambdas() {
    ...
    TestInputsGenerator.pathWithClasses(input, setOf(CarbonForm::class.java, Animal::class.java, Cat::class.java, Toy::class.java))

    getDelegate(projectClasses = setOf(input.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testDefaultMethods_minApiBelow24() {
    val input = tmpDir.root.toPath().resolve("input")
    TestInputsGenerator.pathWithClasses(input, setOf(WithDefault::class.java, WithStatic::class.java))

    getDelegate(minSdkVersion = 23, projectClasses = setOf(input.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testDefaultMethods_minApiAbove24() {
    ...
    getDelegate(minSdkVersion = 24, projectClasses = setOf(input.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testIncremental_lambdaClass() {
    ...
    getDelegate(projectClasses = setOf(input.toFile()), projectOutput = out.toFile(), inputJarHashes = inputJarHashes).doProcess()
  ...
  fun testIncremental_changeSuperTypes() {
    ...
    TestInputsGenerator.pathWithClasses(
      input,
      setOf(CarbonForm::class.java, Animal::class.java, Cat::class.java, Tiger::class.java, Toy::class.java),
    )

    val inputJarHashes = tmpDir.newFile()
    getDelegate(projectClasses = setOf(input.toFile()), projectOutput = out.toFile(), inputJarHashes = inputJarHashes).doProcess()

    setFilesToTrackChanges(getDexFiles(Tiger::class.java, Cat::class.java, Animal::class.java, CarbonForm::class.java, Toy::class.java))
    ...
    getDelegate(
      ...
      .doProcess()
    assertThat(getChangedFiles()).containsExactlyElementsIn(getDexFiles(Tiger::class.java, Cat::class.java))
    ...
    getDelegate(
      ...
      .doProcess()
    assertThat(getChangedFiles()).containsExactlyElementsIn(getDexFiles(Tiger::class.java, Cat::class.java, Animal::class.java))
    ...
    assertThat(getChangedFiles()).containsExactlyElementsIn(getDexFiles(Cat::class.java, Toy::class.java))
  }

  @Test
  fun test_incremental_full_incremental() {
    val input = tmpDir.root.toPath().resolve("input")
    TestInputsGenerator.pathWithClasses(input, setOf(CarbonForm::class.java, Animal::class.java))

    val inputJarHashes = tmpDir.newFile().also { writeEmptyInputJarHashes(it) }
    getDelegate(
        projectClasses = setOf(input.toFile()),
        projectChanges = getChanges(input, ChangeType.ADDED, CarbonForm::class.java, Animal::class.java),
        projectOutput = out.toFile(),
        isIncremental = true,
        inputJarHashes = inputJarHashes,
      )
  ...
  fun test_incremental_jarAndDir() {
    ...
    val input = tmpDir.root.toPath().resolve("input")
    TestInputsGenerator.pathWithClasses(jar, setOf(CarbonForm::class.java, Animal::class.java))
    TestInputsGenerator.pathWithClasses(input, setOf(Toy::class.java, Cat::class.java, Tiger::class.java))

    val inputJarHashes = tmpDir.newFile()
    getDelegate(projectClasses = setOf(input.toFile(), jar.toFile()), projectOutput = out.toFile(), inputJarHashes = inputJarHashes)
      .doProcess()
    ...
    getDelegate(
        ...
        projectChanges = setOf(FakeFileChange(file = jar.toFile(), changeType = ChangeType.MODIFIED)),
        projectOutput = out.toFile(),
        isIncremental = true,
        inputJarHashes = inputJarHashes,
      )
  ...
  fun test_duplicateClasspathEntries() {
    ...
    TestInputsGenerator.pathWithClasses(lib1, setOf(ImplementsWithDefault::class.java, WithDefault::class.java))
    val lib2 = tmpDir.root.toPath().resolve("lib2.jar")
    TestInputsGenerator.pathWithClasses(lib2, setOf(WithDefault::class.java))
    val app = tmpDir.root.toPath().resolve("app")
    TestInputsGenerator.pathWithClasses(app, setOf(InvokesDefault::class.java))
  ...
  fun test_incrementalDesugaring_removesImpactedJars() {
    val lib1 = tmpDir.root.toPath().resolve("lib1.jar")
    TestInputsGenerator.pathWithClasses(lib1, setOf(ImplementsWithDefault::class.java))
    val lib2 = tmpDir.root.toPath().resolve("lib2.jar")
    TestInputsGenerator.pathWithClasses(lib2, setOf(WithDefault::class.java))
    ...
    getDelegate(
        ...
        includeAndroidJar = true,
        projectClasses = setOf(lib1.toFile(), lib2.toFile()),
        projectChanges = setOf(FakeFileChange(file = lib2.toFile(), changeType = ChangeType.MODIFIED)),
        projectOutput = out.toFile(),
        inputJarHashes = inputJarHashes,
      )
      .doProcess()
  ...
  private fun getDelegate(
    ...
    return DexArchiveBuilderTaskDelegate(
      ...
      taskPath = "",
      analyticsService = FakeObjectFactory.factory.property(AnalyticsService::class.java).value(FakeNoOpAnalyticsService()),
    )
  }
  ...
  private fun getDex(clazz: Class<*>): File {
    return FileUtils.find(out.toFile(), Pattern.compile(".*" + clazz.simpleName + "\\.dex")).single()
  }

  private fun getChanges(root: Path, status: ChangeType, vararg classes: Class<*>): Set<FileChange> {
    return classes
      .map {
        val normalizedPath = TestInputsGenerator.getPath(it)
        FakeFileChange(file = root.resolve(normalizedPath).toFile(), changeType = status, normalizedPath = normalizedPath)
      }
      .toSet()
  }
}
