...
class MergedFlavorTest {
  ...
  private val custom: AbstractProductFlavor by lazy {
    productFlavor("custom").also { custom ->
      ...
      custom.addManifestPlaceholders(ImmutableMap.of<String, Any>("one", "oneValue", "two", "twoValue"))
  ...
  private val custom2: AbstractProductFlavor by lazy {
    productFlavor("custom2").also { custom2 ->
      ...
      custom2.addManifestPlaceholders(ImmutableMap.of<String, Any>("two", "twoValueBis", "three", "threeValue"))
  ...
  private fun initservices(enableLegacyApi: Boolean, throwOnError: Boolean = !enableLegacyApi) {
    ...
    properties.put(BooleanOption.ENABLE_LEGACY_API.propertyName, enableLegacyApi)

    dslServices =
      createDslServices(
        createProjectServices(
          projectOptions = ProjectOptions(FakeProviderFactory(FakeProviderFactory.factory, properties.build())),
  ...
  fun testClone() {
    ...
    assertThat(flavor.toString().substringAfter("{")).isEqualTo(custom.toString().substringAfter("{"))
    CopyOfTester.assertAllGettersCalled(MergedFlavor::class.java, flavor, listOf("getApplicationId")) {
  ...
  fun testMergeOnDefault() {
    initservices(true)
    val flavor = MergedFlavor.mergeFlavors(defaultFlavor, ImmutableList.of(custom), FakeGradleProperty("com.forty.two"), services)
  ...
  fun testMergeOnCustom() {
    ...
    val flavor = MergedFlavor.mergeFlavors(custom, ImmutableList.of(defaultFlavor), FakeGradleProperty("com.forty.two"), services)
  ...
  fun testMergeDefaultOnDefault() {
    ...
    val flavor = MergedFlavor.mergeFlavors(defaultFlavor2, ImmutableList.of(defaultFlavor), FakeGradleProperty("com.forty.two"), services)
  ...
  fun testManifestPlaceholdersMerge() {
    ...
    assertThat(manifestPlaceholders).containsExactly("one", "oneValue", "two", "twoValue", "three", "threeValue")
  }
  ...
  fun testMergeMultiple() {
    ...
    val flavor = MergedFlavor.mergeFlavors(custom, ImmutableList.of(custom3, custom2), FakeGradleProperty("com.forty.two"), services)
