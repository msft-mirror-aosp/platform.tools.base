...
class SigningConfigImplTest(
  ...
  fun signingConfig(name: String) =
    createDslServices().let { services -> services.newDecoratedInstance(SigningConfig::class.java, name, services) }
  ...
  fun testSignatureVersions() {
    ...
    val signingConfigImpl = SigningConfigImpl(dslSigningConfig, createVariantPropertiesApiServices(), minSdk, targetApi)
    ...
    if (!v1Signed && !v2Signed && !v3Signed) {
      ...
      assertThat(enableV2Signing == false || (targetApi != null && targetApi < MIN_V2_SDK)).isTrue()
      assertThat(enableV3Signing != true || (targetApi != null && targetApi < MIN_V3_SDK)).isTrue()
    }
    ...
    // Check logic for device API < MIN_V2_SDK
    if ((targetApi != null && targetApi < MIN_V2_SDK) || (targetApi == null && minSdk < MIN_V2_SDK)) {
      if (enableV1Signing != false) {
        assertThat(v1Signed).isTrue()
      }
    }
            ...
            listOf(true, false, null).forEach { enableV4Signing ->
              targetApis.forEach { targetApi ->
                minSdks.forEach { minSdk ->
                  list.add(arrayOf(enableV1Signing, enableV2Signing, enableV3Signing, enableV4Signing, minSdk, targetApi))
                }
              }
            }
          }
