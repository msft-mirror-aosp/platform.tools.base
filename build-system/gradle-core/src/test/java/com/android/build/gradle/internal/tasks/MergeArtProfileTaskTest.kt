...
internal class MergeArtProfileTaskTest {
  ...
  abstract class MergeArtProfileTaskForTest @Inject constructor(testWorkerExecutor: WorkerExecutor) : MergeArtProfileTask() {
  ...
  fun setUp() {
    ...
    artProfileSourceFile = project.objects.fileProperty().also { it.set(File("/does/not/exist")) }
  ...
  fun `test single file`() {
    val singleFile = temporaryFolder.newFile().also { it.writeText("file 1 - line 1") }
  ...
  fun `test single file with art profile source`() {
    val singleFile = temporaryFolder.newFile().also { it.writeText("file 1 - line 1") }
    inputFiles.from(singleFile)
    artProfileSourceFile.set(temporaryFolder.newFile().also { it.writeText("source-file 1 - line 1") })
    task.taskAction()
    assertThat(task.outputFile.get().asFile.readText())
      .isEqualTo("${singleFile.readText()}\n${artProfileSourceFile.asFile.get().readText()}")
  }

  @Test
  fun `test multiple files`() {
    inputFiles.from(mutableListOf<File>().apply { repeat(5) { this.add(createTestFile(it)) } })

    task.taskAction()

    val expectedResult = StringBuilder().apply { repeat(5) { this.append("file $it - line 1\n") } }.toString().trimEnd()
  ...
  fun `test with multiple baseline profiles`() {
    ...
    repeat(5) {
      val dir = project.objects.directoryProperty().fileValue(createTestFile(it))
      baselineProfileSourceDirs.add(dir)
    }

    artProfileSourceFile.set(temporaryFolder.newFile().also { it.writeText("art-profile-source-file 1 - line 1") })
    task.taskAction()
  ...
  private fun createTestFile(fileIndex: Int) = temporaryFolder.newFile().also { it.writeText("file $fileIndex - line 1") }
}
