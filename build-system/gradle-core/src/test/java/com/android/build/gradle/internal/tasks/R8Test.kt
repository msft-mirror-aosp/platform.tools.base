...
/** Testing the basic scenarios for R8 task processing class files. Both dex and class file backend are tested. */
...
class R8Test(private val r8OutputType: R8OutputType) {
  ...
  companion object {
    @Parameterized.Parameters @JvmStatic fun setups() = R8OutputType.values().map { arrayOf(it) }
  }
  ...
  fun testClassesProcessed() {
    ...
    runR8(classes = listOf(classes.toFile()), r8Keep = "class **")
  ...
  fun testOneClassIsKept_noExtractableRules() {
    ...
    runR8(classes = listOf(classes.toFile()), resourcesJar = classes, r8Keep = "class test.A")
  ...
  fun testOneClassIsKept_hasExtractableRulesInClasses() {
    ...
    runR8(classes = listOf(classes.toFile()), r8Keep = "class test.A")
  ...
  fun testLibraryClassesPassedToR8() {
    ...
    runR8(classes = listOf(classes.toFile()), referencedInputs = listOf(libraryClasses.toFile()), r8Keep = "class **")
  ...
  fun testDesugaring() {
    val classes = tmp.root.toPath().resolve("classes.jar")
    TestInputsGenerator.pathWithClasses(classes, listOf(Animal::class.java, CarbonForm::class.java, Cat::class.java, Toy::class.java))

    runR8(classes = listOf(classes.toFile()), java8Support = Java8LangSupport.R8, disableTreeShaking = true, r8Keep = "class ***")
  ...
  fun testProguardConfiguration() {
    ...
    runR8(classes = listOf(classes.toFile()), java8Support = Java8LangSupport.R8, r8Keep = "class " + CarbonForm::class.java.name)
  ...
  fun testPartialShrinkingConfiguration() {
    ...
    runR8(
      ...
      useFullR8 = true,
      minSdkVersion = 23,
      partialShrinkingIncludes = PartialShrinkingConfig(listOf(Cat::class.java.name, Animal::class.java.name)),
    )
  ...
  fun testProguardConfiguration_withDynamicFeatures() {
    Assume.assumeTrue(r8OutputType == R8OutputType.DEX)
    val classes = tmp.root.toPath().resolve("classes.jar")
    TestInputsGenerator.pathWithClasses(classes, listOf(Animal::class.java, CarbonForm::class.java))

    val featureClassesJar = tmp.root.toPath().resolve("feature.jar")
    TestInputsGenerator.pathWithClasses(featureClassesJar, listOf(Cat::class.java, Toy::class.java))
    ...
    checkBaseAndFeatureDex(
      baseClasses = listOf(Type.getDescriptor(CarbonForm::class.java)),
      featureClasses =
        listOf(
    ...
    // Check proguard compatibility mode
    assertThat(Dex(featureDexDir.resolve("feature/classes.dex"))).containsClass(Type.getDescriptor(Toy::class.java)).that().hasAnnotations()
    ...
    ZipArchive(featureJavaResOutput.toPath()).use { assertThat(it.listEntries()).containsExactly("foo.txt") }
    ...
    assertThat(baseDex).containsExactlyClassesIn(listOf(Type.getDescriptor(CarbonForm::class.java)))
  ...
  fun testNonAsciiClassName() {
    ...
    runR8(classes = listOf(classes.toFile()), r8Keep = "class " + nonAsciiName.replace("/", "."))
  ...
  fun testMappingProduced() {
    ...
    runR8(classes = listOf(classes.toFile()), disableMinification = false, outputProguardMapping = outputMapping, r8Keep = "class **")
  ...
  fun testJavaResourcesCopied() {
    ...
    runR8(classes = listOf(mixedResources.toFile()), resourcesJar = mixedResources, r8Keep = "class **")
  ...
  private fun assertClassExists(className: String) {
    if (r8OutputType == R8OutputType.DEX) {
      ...
      assertThat(outputDir.resolve("main.jar")) { it.contains("$className.class") }
    }
  }
  ...
  private fun assertClassDoesNotExist(className: String) {
    if (r8OutputType == R8OutputType.DEX) {
      ...
      assertThat(outputDir.resolve("main.jar")) { it.doesNotContain("$className.class") }
    }
  }

  private fun assertClassHasAnnotations(className: String) {
    if (r8OutputType == R8OutputType.DEX) {
      assertThat(getDex()).containsClass(Type.getDescriptor(Toy::class.java)).that().hasAnnotations()
    } else {
  ...
  private fun assertClassDoesNotHaveAnnotations(className: String) {
    if (r8OutputType == R8OutputType.DEX) {
      // Check proguard compatibility mode
      assertThat(getDex()).containsClass("L$className;").that().doesNotHaveAnnotations()
    } else {
  ...
  private fun hasAnnotations(className: String): Boolean {
    ...
    ZipFile(outputDir.resolve("main.jar").toFile()).use {
      val input = it.getInputStream(it.getEntry("$className.class"))
      ClassReader(input)
        .accept(
  ...
  fun testClassesIgnoredFromResources() {
    ...
    runR8(classes = listOf(), resourcesJar = resJar)
  ...
  fun testNonExistingFileAsInput() {
    ...
    runR8(classes = listOf(classes.toFile(), tmp.root.resolve("non_existing")), r8Keep = "class **")

    assertClassExists("test/A")
  }

  private fun getDex(): Dex {
    val dexFiles = Files.walk(outputDir).filter { it.toString().endsWith(".dex") }.collect(Collectors.toList())
    return Dex(dexFiles.single())
  }

  private fun checkBaseAndFeatureDex(baseClasses: List<String>, featureClasses: List<String>, featureName: String) {
    val baseDex = Dex(outputDir.resolve("main").resolve("classes.dex"))
  ...
  private fun runR8(
    ...
    val componentType = if (r8OutputType == R8OutputType.DEX) ComponentTypeImpl.BASE_APK else ComponentTypeImpl.LIBRARY
