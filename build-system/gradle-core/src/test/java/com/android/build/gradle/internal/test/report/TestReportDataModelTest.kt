...
class TestReportDataModelTest {
  ...
  fun `test Function serialization`() {
    val function = Function(name = "testSomething", results = mapOf("debug" to TestResults("pass"), "release" to TestResults("fail")))
  ...
  fun `test RootReport serialization`() {
    ...
    val classType = ClassType(name = "MyTest", functions = listOf(function))
