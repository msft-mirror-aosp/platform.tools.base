...
class AvdManagerTest {
  ...
  fun setup() {
    ...
    val userImg = systemImageFolder.resolve(com.android.sdklib.internal.avd.AvdManager.USERDATA_IMG)
  ...
  fun testSnapshotCreation() {
    whenever(snapshotHandler.checkSnapshotLoadable(any(), any<File>(), any<ILogger>(), any()))
    ...
    verify(snapshotHandler).generateSnapshot(any(), any<File>(), any<com.android.sdklib.internal.avd.AvdManager>(), any<ILogger>())
  }

  @Test
  fun testSnapshotSkippedIfValid() {
    whenever(snapshotHandler.checkSnapshotLoadable(any(), any<File>(), any<ILogger>(), any())).thenReturn(true)
    ...
    verify(snapshotHandler, times(0))
      .generateSnapshot(any(), any<File>(), any<com.android.sdklib.internal.avd.AvdManager>(), any<ILogger>())
  }
  ...
  private fun setupVersionedSdkLoader(): SdkComponentsBuildService.VersionedSdkLoader =
    mock<SdkComponentsBuildService.VersionedSdkLoader>().also {
      whenever(it.sdkDirectoryProvider).thenReturn(FakeGradleProvider(FakeGradleDirectory(FileOpUtils.toFile(sdkFolder))))
      ...
      whenever(it.emulatorDirectoryProvider).thenReturn(FakeGradleProvider(FakeGradleDirectory(FileOpUtils.toFile(emulatorFolder))))
      whenever(it.adbExecutableProvider).thenReturn(FakeGradleProvider(FakeGradleRegularFile(FileOpUtils.toFile(adbExecutable))))
    }

  private fun setupSdkHandler(): AndroidSdkHandler {
    emulatorFolder.resolve("snapshots.img").recordExistingFile()
    emulatorFolder.resolve(SdkConstants.FD_LIB).resolve(SdkConstants.FN_HARDWARE_INI).recordExistingFile()
    recordSysImg()
  ...
  private fun recordSysImg() {
    ...
    systemImageFolder.resolve(com.android.sdklib.internal.avd.AvdManager.USERDATA_IMG).recordExistingFile()
