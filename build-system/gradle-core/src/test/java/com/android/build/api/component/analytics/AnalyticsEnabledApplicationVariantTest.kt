...
class AnalyticsEnabledApplicationVariantTest {
  ...
  fun getOutputs() {
    ...
    verify(delegate, times(1)).outputs
  }
  ...
  fun getDependenciesInfo() {
    ...
    verify(delegate, times(1)).dependenciesInfo
  }
  ...
  fun getSigningConfig() {
    ...
    Truth.assertThat((proxy.signingConfig as AnalyticsEnabledSigningConfig).delegate).isEqualTo(signingConfig)

    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessList.first().type)
      .isEqualTo(VariantPropertiesMethodType.SIGNING_CONFIG_VALUE)
    verify(delegate, times(1)).signingConfig
  }
  ...
  fun testBundleConfig() {
    ...
    Truth.assertThat((proxy.bundleConfig as AnalyticsEnabledBundleConfig).delegate).isEqualTo(bundleConfig)

    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantPropertiesAccessList.first().type)
      .isEqualTo(VariantPropertiesMethodType.GET_BUNDLE_CONFIG_VALUE)
    verify(delegate, times(1)).bundleConfig
  }
}
