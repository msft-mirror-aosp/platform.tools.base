...
class TaskMethodModifiersAndAnnotationsTest {
  ...
  fun `check for non-public methods with gradle input or output annotations`() {
    ...
    val error = StringBuilder().append("The following gradle-annotated methods are not public:\n")
    for (nonPublicMethod in nonPublicMethods) {
      error.append(nonPublicMethod.declaringClass.toString().substringAfter(" ")).append("::${nonPublicMethod.name}\n")
    }
  ...
  fun `check for relative path sensitivity on files`() {
    val relativePathSensitivityOnFiles =
      taskInputOutputMethods
        .filter {
  ...
  fun `check for TaskAction use in tasks that extend tasks that already define it`() {
    ...
    val methodsThatUseTaskAction =
      taskActionMethods
        ...
        .map { methods -> methods.key.toString().substringAfter(" ") + " ${methods.value.map { it.name }}" }
  ...
  fun `check that tasks should extend a set of base tasks`() {
    ...
    val (tasksExtendingBaseTask, tasksExtendingDefaultTask) = tasks.partition { BaseTask::class.java.isAssignableFrom(it) }

    // For type 1, they need to extend one of the following subtypes
    val type1Violations =
      tasksExtendingBaseTask
        .filter {
    ...
    val type2Violations =
      tasksExtendingDefaultTask
        .filter { !VariantTask::class.java.isAssignableFrom(it) && !GlobalTask::class.java.isAssignableFrom(it) }
        .map { it.name }
        .sorted()
    ...
    assertWithMessage(
      ...
      .that(type1Violations + type2Violations)
      .containsExactly(
    ...
    val violations =
      taskCreationActions
        .filter { creationAction ->
          permittedCreationActionTypes.none { permittedCreationActionType -> permittedCreationActionType.isAssignableFrom(creationAction) }
        }
        .minus(TaskCreationAction::class.java)
        .map { it.name }
        .sorted()

    assertWithMessage("All AGP task creation actions should extend: ${permittedCreationActionTypes.map { it.simpleName }}")
      .that(violations)
      .containsExactly(
  ...
  fun `check for fields with gradle input or output annotations`() {
    ...
    for (annotatedField in annotatedFields) {
      error.append(annotatedField.declaringClass.toString().substringAfter(" ")).append(".${annotatedField.name}\n")
    }
    throw AssertionError(error.toString())
  }
  ...
  fun `check for public task setters`() {
    ...
    val publicSetters =
      classPath
        ...
        .filter { it.name.startsWith("set") && !it.name.contains('$') && Modifier.isPublic(it.modifiers) }

    val publicSettersAsStrings = publicSetters.map { "${it.declaringClass.toString().substringAfter(" ")}::${it.name}" }
  ...
  /**
   * Test that the workaround for https://github.com/gradle/gradle/issues/16976 in [PackageAndroidArtifact] is kept up to date.
   *
   * Adding new incremental inputs to [PackageAndroidArtifact] without updating this test will cause this test to fail. Please add any new
   * incremental inputs to the appropriate non-incremental input (e.g., [PackageAndroidArtifact.getAllClasspathInputFiles]) before updating
   * this test.
   */
  @Test
    ...
    val incrementalInputs =
      classPath
        ...
        .map {
          val annotation =
            when {
              it.getAnnotation(Classpath::class.java) != null -> it.getAnnotation(Classpath::class.java).toString()
              it.getAnnotation(PathSensitive::class.java) != null ->
                it.getAnnotation(PathSensitive::class.java).toString().replace("value=", "")
              else -> "OTHER"
            }
          return@map "${it.name}  $annotation"
        }
    assertThat(incrementalInputs)
      .containsExactly(
  ...
  fun `check BuildService properties are annotated with @ServiceReference`() {
    ...
    val nestedProperties = tasks.flatMap { task -> task.methods.filter { it.isNestedProperty() }.map { it.returnType } }.toSet()

    val violations =
      (tasks + nestedProperties)
  ...
  private fun getMatchingGetter(setter: Method): Method? {
    ...
    return setter.declaringClass.declaredMethods.firstOrNull { it.name == nameWithGet || it.name == nameWithIs }
  }
  ...
  companion object {
    ...
    private val classPath: ClassPath by lazy(LazyThreadSafetyMode.PUBLICATION) { ClassPath.from(this.javaClass.classLoader) }
    ...
    private val comAndroidBuildClasses by
      lazy(LazyThreadSafetyMode.PUBLICATION) {
        classPath.getTopLevelClassesRecursive("com.android.build").map { classInfo -> classInfo.load() as Class<*> }
      }

    private val taskInputOutputMethods: List<Method> by
      lazy(LazyThreadSafetyMode.PUBLICATION) {
        comAndroidBuildClasses.flatMap { it.declaredMethods.asIterable() }.filter { it.hasGradleInputOrOutputAnnotation() }
      }

    private val tasks: List<Class<out Task>> by
      lazy(LazyThreadSafetyMode.PUBLICATION) {
        comAndroidBuildClasses.filterIsInstance<Class<out Task>>().filter { Task::class.java.isAssignableFrom(it) }
      }

    private val taskCreationActions: List<Class<out TaskCreationAction<out Task>>> by
      lazy(LazyThreadSafetyMode.PUBLICATION) {
        classPath.allClasses
          .filter { it.name.startsWith("com.android.build") }
          .map { classInfo -> classInfo.load() as Class<*> }
