...
class NinjaBuildDepsCodecTest {
  ...
  private fun testFile(testFileName: String): File {
    ...
    return TestResources.getFile(NinjaBuildDepsCodecTest::class.java, resourceFolder + testFileName)
  }
  ...
  fun `check dolphin x86_64`() {
    ...
    val iterations = 1000
    repeat(iterations) { NinjaDepsInfo.readFile(file) }
  ...
  fun `write one target schema 3`() {
    ...
    createEmptyNinjaDepsFile(depsFile, schemaVersion = 3)
    NinjaDepsEncoder.open(depsFile).use { encoder -> encoder.writeTarget(target.path, 5L, listOf(dependency.path)) }
    sanityCheckNinjaDependencies(depsFile)
    ...
    streamNinjaDepsFile(input) { record ->
      when (record) {
        is Dependencies -> {
          ...
          encoder.writeTarget(target, record.timestamp, dependencies)
        }
