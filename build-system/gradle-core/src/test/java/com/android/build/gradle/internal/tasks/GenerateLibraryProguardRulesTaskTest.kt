...
class GenerateLibraryProguardRulesTaskTest {
  ...
  /** Test to verify only added layout resources (no changed or removed) can trigger incremental task run. */
  @Test
  fun testCanBeProcessedIncrementally_layoutFiles() {
    ...
    val newLayoutFile = SerializableChange(layoutFile, FileStatus.NEW, layoutFile.absolutePath)
    val changedLayoutFile = SerializableChange(layoutFile, FileStatus.CHANGED, layoutFile.absolutePath)
    val removedLayoutFile = SerializableChange(layoutFile, FileStatus.REMOVED, layoutFile.absolutePath)
  ...
  /** Test to verify that adding, changing or removing non-layout resources allows for incremental task run. */
  @Test
  fun testCanBeProcessedIncrementally_nonLayoutFiles() {
    ...
    val newStringsFile = SerializableChange(stringsFile, FileStatus.NEW, stringsFile.absolutePath)
    val changedStringsFile = SerializableChange(stringsFile, FileStatus.CHANGED, stringsFile.absolutePath)
    val removedStringsFile = SerializableChange(stringsFile, FileStatus.REMOVED, stringsFile.absolutePath)
  ...
  /**
   * Test to verify that adding a new layout file will only add the new unique keep classes to the Proguard keep classes file
   * ("aapt_rules.txt"). This assumes that the other conditions are set for an incremental run.
   */
  @Test
  fun testRunIncrementalTask_appendsNewLayoutClasspaths() {
    ...
    val params =
      object : GenerateLibraryProguardRulesTask.GenerateProguardRulesWorkAction.Params() {
        override val manifestFile = FakeObjectFactory.factory.fileProperty().fileValue(File(parentFolder, "AndroidManifest.xml"))
        override val proguardOutputFile = FakeObjectFactory.factory.fileProperty().fileValue(previousProguardOutput)
        override val inputResourcesDir = FakeObjectFactory.factory.directoryProperty().fileValue(resourcesFolder)
        override val changedResources = FakeObjectFactory.factory.listProperty(SerializableChange::class.java).value(changedResources)
    ...
    assertThat(params.proguardOutputFile.get().asFile.readLines())
      .containsExactly(
        "# Generated by the gradle plugin",
  ...
  fun testRunFullTask_createsExpectedKeepRules() {
    ...
    val params =
      object : GenerateLibraryProguardRulesTask.GenerateProguardRulesWorkAction.Params() {
        override val manifestFile = FakeObjectFactory.factory.fileProperty().fileValue(manifestFile)
        override val proguardOutputFile = FakeObjectFactory.factory.fileProperty().fileValue(previousProguardOutput)
        override val inputResourcesDir = FakeObjectFactory.factory.directoryProperty().fileValue(resourcesFolder)
        override val changedResources = FakeObjectFactory.factory.listProperty(SerializableChange::class.java)
