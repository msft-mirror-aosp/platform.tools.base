...
class ArtifactsImplTest {
  ...
  @Suppress("ClassName")
  sealed class TestSingleArtifactType<T : FileSystemLocation>(kind: ArtifactKind<T>, private val fileName: String? = null) :
    ...
    object TEST_TRANSFORMABLE_DIRECTORY : TestSingleArtifactType<Directory>(DIRECTORY), Transformable

    object TEST_REPLACABLE_FILE : TestSingleArtifactType<RegularFile>(FILE), Replaceable

    object TEST_REPLACABLE_DIRECTORY : TestSingleArtifactType<Directory>(DIRECTORY), Replaceable
  ...
  sealed class TestMultipleArtifactType<T : FileSystemLocation>(kind: ArtifactKind<T>) :
    ...
    object TEST_APPENDABLE_DIRECTORIES : TestMultipleArtifactType<Directory>(DIRECTORY), Appendable
  ...
  fun testEarlyFileLookup() {
    ...
    val agpTaskProvider = project.tasks.register(taskName, AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFile).on(TEST_FILE)

    // now get it.
    Truth.assertThat(finalVersion.get().asFile.relativeTo(project.layout.buildDirectory.asFile.get()).path)
      .isEqualTo(
        FileUtils.join(
  ...
  fun testAddDirectoryArtifacts() {
    ...
    Truth.assertThat(artifacts.getAll(TEST_DIRECTORIES).get())
      .containsExactly(project.layout.buildDirectory.get(), project.layout.projectDirectory)
  }
    ...
    try {
      repeat(6) {
        artifacts.addStaticProvider(
          ...
          TEST_DIRECTORIES,
          project.layout.buildDirectory.dir("file-${Random().nextInt()}").get(),
        )
      }
    } catch (e: RuntimeException) {
      Truth.assertThat(e.message)
        .isEqualTo("There are more than 5 static files registered for TEST_DIRECTORIES, " + "which is not supported.")
    }
  }
  ...
  fun testAddExternalFileArtifactsWithTask() {
    ...
    artifacts.addInitialProvider(MultipleArtifact.MULTIDEX_KEEP_PROGUARD, agpTaskProvider, AGPTask::outputFile)
    artifacts.add(MultipleArtifact.MULTIDEX_KEEP_PROGUARD, project.layout.buildDirectory.get().file("test.txt"))
  ...
  private fun <T : Artifact.Multiple<RegularFile>> verifyTestAddFileArtifactWithTask(type: T, expectedFiles: List<String>) {
    val actualFiles: List<String> = artifacts.getAll(type).get().map { it.asFile.relativeTo(project.buildDir).path }
  ...
  fun testEarlyDirectoryLookup() {
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTask", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFiles).on(TEST_DIRECTORY)

    // now get it.
    Truth.assertThat(finalVersion.get().asFile.relativeTo(project.buildDir).path)
      .isEqualTo(
        FileUtils.join(
          Artifact.Category.INTERMEDIATES.name.lowercase(Locale.getDefault()),
  ...
  fun testSingleFileAGPProvider() {
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFile).on(TEST_FILE)
  ...
  fun testMultipleAGPProvidersForMultipleFileArtifactType() {
    ...
    for (i in 0..2) {
      val agpTaskProvider = project.tasks.register("agpTaskProvider$i", AGPTask::class.java) { initializedTasks.incrementAndGet() }
  ...
  fun testFileTransform() {
    ...
    val transformerProvider = project.tasks.register("transformer", TransformTask::class.java) { transformerInitialized.set(true) }
    artifacts
      .use(transformerProvider)
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFile).on(TEST_TRANSFORMABLE_FILE)
  ...
  fun testDirectoryTransform() {
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFolder).on(TEST_TRANSFORMABLE_DIRECTORY)
  ...
  fun testOverlappingFileTransforms() {
    ...
    val transformerOneProvider = project.tasks.register("transformerOne", TransformTask::class.java) { transformerOneInitialized.set(true) }
    artifacts
      .use(transformerOneProvider)
      .wiredWithFiles(TransformTask::inputFile, TransformTask::outputFile)
      .toTransform(TEST_TRANSFORMABLE_FILE)
    ...
    val transformerTwoProvider = project.tasks.register("transformerTwo", TransformTask::class.java) { transformerTwoInitialized.set(true) }
    artifacts
      .use(transformerTwoProvider)
      .wiredWithFiles(TransformTask::inputFile, TransformTask::outputFile)
      .toTransform(TEST_TRANSFORMABLE_FILE)
    ...
    agpTaskProvider.configure { agpInitialized.set(true) }
  ...
  fun testFileReplace() {
    ...
    val replaceTaskProvider = project.tasks.register("replaceTask", ReplaceTask::class.java) { replaceTaskInitialized.set(true) }
    artifacts.use(replaceTaskProvider).wiredWith(ReplaceTask::outputFile).toCreate(TEST_REPLACABLE_FILE)
    ...
    Truth.assertThat(artifacts.getArtifactContainer(TEST_REPLACABLE_FILE).needInitialProducer().get()).isFalse()

    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFile).on(TEST_REPLACABLE_FILE)
  ...
  fun testDirectoryReplace() {
    ...
    val replaceTaskInitialized = AtomicBoolean(false)
    val replaceTaskProvider = project.tasks.register("replaceTask", ReplaceTask::class.java) { replaceTaskInitialized.set(true) }
    artifacts.use(replaceTaskProvider).wiredWith(ReplaceTask::outputFolder).toCreate(TEST_REPLACABLE_DIRECTORY)
    ...
    Truth.assertThat(artifacts.getArtifactContainer(TEST_REPLACABLE_DIRECTORY).needInitialProducer().get()).isFalse()

    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.setInitialProvider(agpTaskProvider, AGPTask::outputFolder).on(TEST_REPLACABLE_DIRECTORY)
  ...
  fun testOverlappingFileReplace() {
    ...
    val replaceTaskOneProvider = project.tasks.register("replaceTaskOne", ReplaceTask::class.java) { replaceTaskOneInitialized.set(true) }
    artifacts.use(replaceTaskOneProvider).wiredWith(ReplaceTask::outputFile).toCreate(TEST_REPLACABLE_FILE)

    Truth.assertThat(replaceTaskOneInitialized.get()).isFalse()

    val replaceTaskTwoInitialized = AtomicBoolean(false)
    val replaceTaskTwoProvider = project.tasks.register("replaceTaskTwo", ReplaceTask::class.java) { replaceTaskTwoInitialized.set(true) }
    artifacts.use(replaceTaskTwoProvider).wiredWith(ReplaceTask::outputFile).toCreate(TEST_REPLACABLE_FILE)
  ...
  fun testOverlappingDirectoryReplace() {
    ...
    val replaceTaskOneInitialized = AtomicBoolean(false)
    val replaceTaskOneProvider = project.tasks.register("replaceTaskOne", ReplaceTask::class.java) { replaceTaskOneInitialized.set(true) }
    artifacts.use(replaceTaskOneProvider).wiredWith(ReplaceTask::outputFolder).toCreate(TEST_REPLACABLE_DIRECTORY)

    Truth.assertThat(replaceTaskOneInitialized.get()).isFalse()

    val replaceTaskTwoInitialized = AtomicBoolean(false)
    val replaceTaskTwoProvider = project.tasks.register("replaceTaskTwo", ReplaceTask::class.java) { replaceTaskTwoInitialized.set(true) }
    artifacts.use(replaceTaskTwoProvider).wiredWith(ReplaceTask::outputFolder).toCreate(TEST_REPLACABLE_DIRECTORY)
  ...
  fun testSingleFileAppend() {
    ...
    val appendTaskProvider = project.tasks.register("appendTask", AppendTask::class.java) { appendTaskInitialized.set(true) }
    artifacts.use(appendTaskProvider).wiredWith(AppendTask::outputFile).toAppendTo(TEST_APPENDABLE_FILES)
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.addInitialProvider(TEST_APPENDABLE_FILES, agpTaskProvider, AGPTask::outputFile)
  ...
  fun testMultipleFilesAppend() {
    ...
    for (i in 0..2) {
      Truth.assertThat(artifactContainer.get().get()[i].asFile.absolutePath).contains("appendTask")
    }
    Truth.assertThat(artifactContainer.get().get()[3].asFile.absolutePath).contains("agpTaskProvider")
  }
  ...
  fun testSingleDirectoryAppend() {
    ...
    val appendTaskInitialized = AtomicBoolean(false)
    val appendTaskProvider = project.tasks.register("appendTask", AppendTask::class.java) { appendTaskInitialized.set(true) }
    artifacts.use(appendTaskProvider).wiredWith(AppendTask::outputDirectory).toAppendTo(TEST_APPENDABLE_DIRECTORIES)
    ...
    val agpInitialized = AtomicBoolean(false)
    val agpTaskProvider = project.tasks.register("agpTaskProvider", AGPTask::class.java) { agpInitialized.set(true) }
    artifacts.addInitialProvider(TEST_APPENDABLE_DIRECTORIES, agpTaskProvider, AGPTask::outputDirectory)
  ...
  fun testMultipleDirectoriesToListenTo() {
    ...
    artifacts.use(appendTaskProvider).wiredWith(ProducerTask::outputDirectory).toAppendTo(TEST_APPENDABLE_DIRECTORIES)
    ...
    artifacts.use(agpTaskProvider).wiredWithMultiple(ListenerTask::inputDirectories).toListenTo(TEST_APPENDABLE_DIRECTORIES)
  ...
  fun testMultipleAGPProvidersReplacementOnMultipleFileArtifactType() {
    ...
    for (i in 0..2) {
      val agpTaskProvider = project.tasks.register("agpTaskProvider$i", AGPTask::class.java) { initializedTasks.incrementAndGet() }
      agpTaskProviders.add(agpTaskProvider)
      artifacts.addInitialProvider(TEST_TRANSFORMABLE_FILES, agpTaskProvider, AGPTask::outputFile)
    }
    Truth.assertThat(initializedTasks.get()).isEqualTo(0)
    ...
    Truth.assertThat(artifactContainer.get().get()).hasSize(1)
    Truth.assertThat(artifactContainer.get().get()[0].asFile.absolutePath).contains("transformTask")
    Truth.assertThat(transformTask.get().inputFiles.get()).hasSize(3)
    for (i in 0..2) {
      Truth.assertThat(transformTask.get().inputFiles.get()[i].asFile.absolutePath).contains("agpTaskProvider$i")
    }
    Truth.assertThat(transformTask.get().outputFile.get().asFile).isEqualTo(artifactContainer.get().get()[0].asFile)
  }
  ...
  fun testSuccessiveMultipleAGPProvidersReplacementOnMultipleFileArtifactType() {
    ...
    artifacts
      .use(transformOneTask)
      .wiredWith(TransformMultipleTask::inputFiles, TransformMultipleTask::outputFile)
      .toTransform(TEST_TRANSFORMABLE_FILES)

    val transformTwoTask = project.tasks.register("transformTwoTask", TransformMultipleTask::class.java)
    artifacts
      .use(transformTwoTask)
      .wiredWith(TransformMultipleTask::inputFiles, TransformMultipleTask::outputFile)
      .toTransform(TEST_TRANSFORMABLE_FILES)

    val artifactContainer = artifacts.getArtifactContainer(TEST_TRANSFORMABLE_FILES)
    Truth.assertThat(artifactContainer.get().get()).hasSize(1)
    Truth.assertThat(artifactContainer.get().get()[0].asFile.absolutePath).contains("transformTwoTask")
    Truth.assertThat(transformOneTask.get().inputFiles.get()).hasSize(3)
    for (i in 0..2) {
      Truth.assertThat(transformOneTask.get().inputFiles.get()[i].asFile.absolutePath).contains("agpTaskProvider$i")
    }
    Truth.assertThat(transformOneTask.get().outputFile.get().asFile.absolutePath).contains("transformOneTask")
    Truth.assertThat(transformTwoTask.get().inputFiles.get()).hasSize(1)
    Truth.assertThat(transformTwoTask.get().inputFiles.get()[0].asFile.absolutePath).contains("transformOneTask")

    Truth.assertThat(transformTwoTask.get().outputFile.get().asFile).isEqualTo(artifactContainer.get().get()[0].asFile)
  }
  ...
  fun testMultipleAGPProvidersReplacementOnMultipleDirectoryArtifactType() {
    ...
    for (i in 0..2) {
      val agpTaskProvider = project.tasks.register("agpTaskProvider$i", AGPTask::class.java) { initializedTasks.incrementAndGet() }
      artifacts.addInitialProvider(TEST_TRANSFORMABLE_DIRECTORIES, agpTaskProvider, AGPTask::outputDirectory)
    }
    Truth.assertThat(initializedTasks.get()).isEqualTo(0)
    ...
    abstract class TransformMultipleTask : DefaultTask() {
      ...
      @get:OutputFile abstract val outputDirectory: DirectoryProperty
    }
    ...
    for (i in 0..2) {
      Truth.assertThat(transformTask.get().inputDirectories.get()[i].asFile.absolutePath).contains("agpTaskProvider$i")
    }
    Truth.assertThat(transformTask.get().outputDirectory.get().asFile).isEqualTo(artifactContainer.get().get()[0].asFile)
  }
  ...
  fun testSuccessiveMultipleAGPProvidersReplacementOnMultipleDirectoryArtifactType() {
    ...
    for (i in 0..2) {
      Truth.assertThat(transformOneTask.get().inputDirectories.get()[i].asFile.absolutePath).contains("agpTaskProvider$i")
    }
    Truth.assertThat(transformOneTask.get().outputDirectory.get().asFile.absolutePath).contains("transformOneTask")
    Truth.assertThat(transformTwoTask.get().inputDirectories.get()).hasSize(1)
    Truth.assertThat(transformTwoTask.get().inputDirectories.get()[0].asFile.absolutePath).contains("transformOneTask")

    Truth.assertThat(transformTwoTask.get().outputDirectory.get().asFile).isEqualTo(artifactContainer.get().get()[0].asFile)
  }
  ...
  fun testLateAppendReplacementOnMultipleFileArtifactType() {
    ...
    for (i in 0..2) {
      val agpTaskProvider = project.tasks.register("agpTaskProvider$i", ProducerTask::class.java) { initializedTasks.incrementAndGet() }
      agpTaskProviders.add(agpTaskProvider)
      artifacts.addInitialProvider(TEST_TRANSFORMABLE_FILES, agpTaskProvider, ProducerTask::outputFile)
    }
    Truth.assertThat(initializedTasks.get()).isEqualTo(0)
    ...
    artifacts.use(lateProducer).wiredWith(ProducerTask::outputFile).toAppendTo(TEST_TRANSFORMABLE_FILES)
    ...
    Truth.assertThat(transformTask.get().inputFiles.get()[3].asFile.absolutePath).contains("lateProducer")
    Truth.assertThat(transformTask.get().outputFile.get().asFile).isEqualTo(artifactContainer.get().get()[0].asFile)
  }
  ...
  fun `test regular-file artifacts have a default file name`() {
    ...
    Truth.assertThat(artifacts.getArtifactContainer(TEST_FILE).get().get().asFile.absolutePath)
      .endsWith(FileUtils.join("test_file", "debug", "agpTask", "test-file.dat"))
  }

  @Test
  fun testAddingSameStaticFileMultipleTimes() {
    val container = MultipleArtifactContainer { @Suppress("UNCHECKED_CAST") mock<MultiplePropertyAdapter<RegularFile>>() }

    artifacts.addStaticProvider(container, MultipleArtifact.MULTIDEX_KEEP_PROGUARD, FakeGradleRegularFile(File("/tmp/foo.txt")))
