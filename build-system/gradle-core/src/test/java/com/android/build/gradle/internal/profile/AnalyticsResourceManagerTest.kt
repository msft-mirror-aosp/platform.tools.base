...
class AnalyticsResourceManagerTest {
  ...
  fun setUp() {
    ...
    resourceManager =
      AnalyticsResourceManager(GradleBuildProfile.newBuilder(), getProjects(), true, profileDir, getTaskMetaData(), null, null)
  }
  ...
  fun testRecordBlockExecutionAtConfiguration() {
    ...
    Truth.assertThat(span.id).isEqualTo(2)
    Truth.assertThat(span.parentId).isEqualTo(0)
    Truth.assertThat(span.type).isEqualTo(GradleBuildProfileSpan.ExecutionType.SOME_RANDOM_PROCESSING)
  ...
  fun testRecordMixedExecutionSpans() {
    ...
    Truth.assertThat(blockSpan.type).isEqualTo(GradleBuildProfileSpan.ExecutionType.SOME_RANDOM_PROCESSING)
  ...
  fun testRecordNestedBlockExecutionSpan() {
    ...
    Truth.assertThat(records[1].durationInMs).isAtLeast(records[2].durationInMs + records[3].durationInMs + records[4].durationInMs)
  ...
  fun testRecordIdNumberingInMultiThreadInvocation() {
    ...
    val threads = Stream.generate { Thread(recordRunnable) }.limit(20).collect(Collectors.toList())
  ...
  private fun getRecordForId(records: Collection<GradleBuildProfileSpan>, recordId: Long): GradleBuildProfileSpan {
