...
class AbiConfiguratorTest {
  ...
  fun testNon64BitWarning() {
    configure(externalNativeBuildAbiFilters = setOf("x86"))
    assertThat(logger.errors).isEmpty()
    assertThat(logger.warnings.first().toString())
      .isEqualTo(
  ...
  fun testNoNon64BitWarningForInjectedAbi() {
    configure(externalNativeBuildAbiFilters = setOf("x86", "armeabi-v7a"), ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "x86")
    assertThat(logger.errors).isEmpty()
  ...
  fun testValidAbiInBuildGradleDsl() {
    val configurator = configure(externalNativeBuildAbiFilters = setOf("x86"))
    assertThat(logger.errors).isEmpty()
    // Should be no messages reported
  ...
  fun testInvalidAbiInBuildGradleDsl() {
    val configurator = configure(externalNativeBuildAbiFilters = setOf("x87"))
    assertThat(logger.errors.first().toString())
      .isEqualTo(
        "${ABI_IS_UNSUPPORTED.codeText} ABIs [x87] are not supported for platform. Supported ABIs " + "are [$ALL_ABI_COMMA_STRING]."
      )
  ...
  fun testSplitsEnabled() {
    val configurator = configure(splitsFilterAbis = setOf("x86"))
    assertThat(logger.errors).isEmpty()
    // Should be no messages reported
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactly("x86")
  ...
  fun testSplitsEnabledInvalidAbi() {
    val configurator = configure(splitsFilterAbis = setOf("x87"))
  ...
  fun testValidAbiThatIsNotInNdk() {
    val configurator =
      configure(ndkHandlerSupportedAbis = setOf("x86_64"), externalNativeBuildAbiFilters = setOf("x86"), splitsFilterAbis = setOf())
    assertThat(logger.errors)
      .containsExactly("${ABI_IS_UNSUPPORTED.codeText} ABIs [x86] are not supported for platform. " + "Supported ABIs are [x86_64].")
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactly("x86")
  }
  ...
  fun testExternalNativeBuildAbiFiltersAndNdkAbiFiltersAreTheSame() {
    val configurator = configure(externalNativeBuildAbiFilters = setOf("x86"), ndkConfigAbiFilters = setOf("x86"))
    assertThat(logger.errors).isEmpty()
    // Should be no messages reported
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactly("x86")
  ...
  fun testExternalNativeBuildAbiFiltersAndNdkAbiFiltersAreNonIntersecting() {
    val configurator = configure(externalNativeBuildAbiFilters = setOf("x86_64"), ndkConfigAbiFilters = setOf("x86"))
    assertThat(configurator.validAbis).isEmpty()
  ...
  fun testValidInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "x86")
    assertThat(logger.errors).isEmpty()
    // Should be no messages reported
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactlyElementsIn(ALL_ABI)
  ...
  fun testValidBogusAndValidInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "bogus,x86")

    assertThat(logger.warnings)
      .containsExactly(
        "[CXX5200] ABIs [bogus,x86] set by 'android.injected.build.abi' gradle flag contained " + "'bogus' which is invalid."
      )
    assertThat(logger.errors).isEmpty()
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactlyElementsIn(ALL_ABI)
  ...
  fun testToleratedAndValidInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "mips64,x86")
  ...
  fun testToleratedAndBogusInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "mips64,bogus,x86")
    assertThat(logger.infos)
      .containsExactly(
    ...
    assertThat(logger.warnings)
      .containsExactly(
        "[CXX5200] ABIs [mips64,bogus,x86] set by 'android.injected.build.abi' gradle flag " + "contained 'bogus' which is invalid."
      )
    assertThat(logger.errors).isEmpty()
    assertThat(configurator.validAbis).containsExactly("x86")
    assertThat(configurator.allAbis).containsExactlyElementsIn(ALL_ABI)
  ...
  fun testBogusInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "bogus")
    assertThat(logger.errors)
      .containsExactly(
  ...
  fun testValidEmptyInjectedAbi() {
    ...
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "")
    assertThat(configurator.validAbis).containsExactlyElementsIn(ALL_ABI)
    assertThat(configurator.allAbis).containsExactlyElementsIn(ALL_ABI)
  }
  ...
  fun testValidNullInjectedAbi() {
    // Empty list should not error
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = null)
    assertThat(configurator.validAbis).containsExactlyElementsIn(ALL_ABI)
    assertThat(configurator.allAbis).containsExactlyElementsIn(ALL_ABI)
  }
  ...
  fun testPeopleCanSpecifyMipsIfTheyReallyWantTo() {
    // Empty list should not error
    val configurator = configure(splitsFilterAbis = setOf("mips"), ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = null)
  ...
  fun testMisspelledMips() {
    // Empty list should not error
    val configurator = configure(splitsFilterAbis = setOf("misp"), ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = null)
    assertThat(logger.errors)
      .containsExactly(
  ...
  fun testIdeSelectedAbiDoesntIntersectWithNdkConfigAbiFilters() {
    val configurator =
      configure(ndkConfigAbiFilters = setOf("arm64-v8a", "x86_64"), ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "armeabi-v7a,armeabi")
    assertThat(logger.errors).isEmpty()
    assertThat(logger.infos)
      .containsExactly(
  ...
  fun testIdeSelectedAbiDoesntIntersectWithSplitsFilterAbis() {
    val configurator =
      configure(splitsFilterAbis = setOf("arm64-v8a", "x86_64"), ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = "armeabi-v7a,x86_64")
    assertThat(logger.errors).isEmpty()
    assertThat(logger.infos)
      .containsExactly(
  ...
  fun testAllowSpaceInInjectedAbi() {
    val configurator = configure(ideBuildOnlyTargetAbi = true, ideBuildTargetAbi = " x86, x86_64 ")
    assertThat(logger.errors).isEmpty()
    assertThat(logger.warnings).isEmpty()
    assertThat(configurator.validAbis).containsExactly("x86")
  }
