...
class MergeClassesTaskTest {
  ...
  fun `test basic`() {
    ...
    TestInputsGenerator.writeJarWithEmptyEntries(jarFile1.toPath(), listOf("Foo.class", "foo.txt", "META-INF/duplicate.kotlin_module"))
    assertThat(jarFile1).exists()
    val jarFile2 = temporaryFolder.newFile("bar.jar")
    TestInputsGenerator.writeJarWithEmptyEntries(jarFile2.toPath(), listOf("Bar.class", "bar.txt", "META-INF/duplicate.kotlin_module"))
