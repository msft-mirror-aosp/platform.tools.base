...
class BuiltArtifactsLoaderImplTest {
  ...
  @get:Rule val outFolder = TemporaryFolder()
  ...
  fun testSingleFileTransformation() {
    ...
    // load the new file
    val updatedBuiltArtifacts = BuiltArtifactsLoaderImpl().load(FakeGradleDirectory(outFolder.root))
  ...
  "elements": [
    ...
    val newBuiltArtifacts =
      BuiltArtifactsImpl(
        ...
        elements =
          builtArtifacts.elements.map {
            assertThat(File(it.outputFile).readText(Charsets.UTF_8)).isEqualTo(it.filters.joinToString())
            it.newOutput(
              outFolder
                .newFile("${File(it.outputFile).name}.new")
                .also { file -> file.writeText("updated APK : ${it.filters.joinToString()}") }
                .toPath()
            )
          },
      )
    ...
    updatedBuiltArtifacts.elements.forEach { builtArtifact ->
      ...
      assertThat(builtArtifact.filters.first().filterType).isEqualTo(FilterConfiguration.FilterType.DENSITY)
      val filterValue = builtArtifact.filters.first().identifier
      assertThat(File(builtArtifact.outputFile).readText(Charsets.UTF_8)).isEqualTo("updated APK : $filterValue")
    }
  }
  ...
  fun testSimpleLoading() {
    ...
    assertThat(builtArtifact.outputFile).isEqualTo(FileUtils.toSystemIndependentPath(File(tmpFolder.root, "file1.xml").absolutePath))
  ...
  fun testLoadingWithBaselineProfiles() {
    ...
    val baselineProfileFile1 = builtArtifacts!!.baselineProfiles?.first()?.getBaselineProfileFile("app-release-unsigned-1")
    assertThat(baselineProfileFile1).isEqualTo(profileOneDmFile1)
    val baselineProfileFile2 = builtArtifacts!!.baselineProfiles?.first()?.getBaselineProfileFile("app-release-unsigned-2")
    assertThat(baselineProfileFile2).isEqualTo(profileOneDmFile2)

    val baselineProfileFile3 = builtArtifacts!!.baselineProfiles?.get(1)?.getBaselineProfileFile("app-release-unsigned")
    assertThat(baselineProfileFile3).isEqualTo(profileZeroDmFile1)

    val baselineProfileFile4 = builtArtifacts!!.baselineProfiles?.get(2)?.getBaselineProfileFile("app-release-unsigned")
  ...
  fun testLoadingWithBaselineProfilesMaxApiNotPresent() {
    ...
    val failure = assertFailsWith<IOException> { BuiltArtifactsLoaderImpl().load(FakeGradleDirectory(tmpFolder.root)) }
