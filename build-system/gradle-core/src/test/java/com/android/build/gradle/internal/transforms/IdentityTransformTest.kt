...
class IdentityTransformTest {
  ...
  fun `input is a regular file`() {
    ...
    val transform = TestIdentityTransform(FakeGradleProvider(FakeGradleRegularFile(inputFile)))
  ...
  fun `input is a directory`() {
    ...
    val transform = TestIdentityTransform(FakeGradleProvider(FakeGradleRegularFile(inputDir)))
    transform.transform(output)
  ...
  fun `input does not exist, acceptNonExistentInputFile not set or set to false, expect failure`() {
    ...
    var transform = TestIdentityTransform(FakeGradleProvider(FakeGradleRegularFile(input)))
    assertFailsWith(IllegalArgumentException::class) { transform.transform(output) }

    transform = TestIdentityTransform(FakeGradleProvider(FakeGradleRegularFile(input)), acceptNonExistentInputFile = false)
    assertFailsWith(IllegalArgumentException::class) { transform.transform(output) }
  }
  ...
  fun `input does not exist, acceptNonExistentInputFile set to true, expect success`() {
    ...
    val transform = TestIdentityTransform(FakeGradleProvider(FakeGradleRegularFile(input)), acceptNonExistentInputFile = true)
    transform.transform(output)
...
private class TestIdentityTransform(
  ...
  override fun getParameters(): Parameters {
    return object : Parameters {
      override val acceptNonExistentInputFile: Property<Boolean> = FakeGradleProperty(this@TestIdentityTransform.acceptNonExistentInputFile)
