...
class BuildAnalyzerTest {
  ...
  private val expectedTasksWithoutAnnotations =
    listOf(
      BaseTask::class.java,
  ...
  fun `all tasks have build analyzer annotations unless in allow-list`() {
    ...
    val actualTasksWithoutAnnotations = allTasks.filter { !it.isAnnotationPresent(BuildAnalyzer::class.java) }
    assertWithMessage(
        "These tasks do not have BuildAnalyzer annotations, either add this annotation to the task class " +
          "or add to expectedTasksWithoutAnnotations in BuildAnalyzerTest.kt. Refer to TaskCategory.kt " +
          "on guidance on how to label task classes."
      )
      .that(actualTasksWithoutAnnotations)
      .containsExactlyElementsIn(expectedTasksWithoutAnnotations)
  }
  ...
  fun `BuildAnalyzer annotated tasks does not have unallowed categories`() {
    ...
    val tasksWithUnallowedCategories =
      allTasks
        .filter { it.isAnnotationPresent(BuildAnalyzer::class.java) }
        .map { it.getAnnotation(BuildAnalyzer::class.java) }
        .filter { annotation ->
          annotation.primaryTaskCategory in unallowedCategories || annotation.secondaryTaskCategories.any { it in unallowedCategories }
        }
    assertWithMessage(
        "These tasks have GRADLE or UNKNOWN task category, these categories " +
          "should not be added manually as it is handled on the IDE side."
      )
      .that(tasksWithUnallowedCategories)
      .isEmpty()
  }
  ...
  fun `primaryTaskCategory field only has primary task categories`() {
    val allTasks = getAllTasks()
    val tasksWithIncorrectPrimaryTaskCategories =
      allTasks
        .filter { it.isAnnotationPresent(BuildAnalyzer::class.java) }
        .filter { !it.getAnnotation(BuildAnalyzer::class.java).primaryTaskCategory.isPrimary }
    assertWithMessage(
      ...
      .that(tasksWithIncorrectPrimaryTaskCategories)
      .isEmpty()
  }
  ...
  fun `all categories in each BuildAnalyzer annotation are unique`() {
    ...
    allTasks
      .filter { it.isAnnotationPresent(BuildAnalyzer::class.java) }
      .map { clazz -> clazz.getAnnotation(BuildAnalyzer::class.java) }
      .forEach { assertThat(listOf(it.primaryTaskCategory) + it.secondaryTaskCategories).containsNoDuplicates() }
  }
  ...
  private fun getAllTasks(): List<Class<*>> {
    ...
    return classPath
      ...
      .filter { TypeToken.of(it).getTypes().contains(taskInterface) }
  }
}
