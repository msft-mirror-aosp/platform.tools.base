...
class AppMetadataTaskTest {
  ...
  abstract class AppMetadataForTest @Inject constructor(testWorkerExecutor: WorkerExecutor) : AppMetadataTask() {
  ...
  fun setUp() {
    ...
    task =
      project.tasks
        .register("appMetadataTask", AppMetadataForTest::class.java, FakeGradleWorkExecutor(project.objects, temporaryFolder.newFolder()))
        .get()
  ...
  private fun checkAppMetadataFile(file: File, appMetadataVersion: String, agpVersion: String, agdeVersion: String? = null) {
    assertThat(file).exists()
    val properties = Properties()
    file.inputStream().use { properties.load(it) }
    assertThat(properties.getProperty(APP_METADATA_VERSION_PROPERTY)).isEqualTo(appMetadataVersion)
    assertThat(properties.getProperty(ANDROID_GRADLE_PLUGIN_VERSION_PROPERTY)).isEqualTo(agpVersion)
    assertThat(properties.getProperty(ANDROID_GAME_DEVELOPMENT_EXTENSION_VERSION_PROPERTY)).isEqualTo(agdeVersion)
  }
}
