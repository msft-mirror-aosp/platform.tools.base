...
class NinjaLocatorKtTest {
  ...
  fun `find Ninja at found CMake location`() {
    val found = findNinjaPath(cmakePath = File("/path/to/cmake/bin")) { path -> path == "/path/to/cmake/bin" }
  ...
  fun `find Ninja on PATH`() {
    val found = findNinjaPath(environmentPaths = listOf("/path/to/ninja/bin")) { path -> path == "/path/to/ninja/bin" }
  ...
  fun `find Ninja ot other SDK CMake location`() {
    val found = findNinjaPath(sdkCMakeFolders = listOf("/path/to/cmake/bin")) { path -> path == "/path/to/cmake/bin" }
    assertThat(found.result).isEqualTo("/path/to/cmake/bin/ninja")
    assertThat(found.errors).isEmpty()
    assertThat(found.warnings).isEmpty()
  }
  ...
  fun `PATH order is honored `() {
    val found =
      findNinjaPath(
        ...
        path.startsWith("/path/to/environment")
      }
    assertThat(found.result).isEqualTo("/path/to/environment1/ninja/bin/ninja")
    assertThat(found.environmentPathsRetrieved).isTrue()
    assertThat(found.sdkCMakeFoldersRetrieved).isFalse()
  ...
  private fun findNinjaPath(
    ...
    PassThroughRecordingLoggingEnvironment().use { logger ->
      val fileResult =
        findNinjaPathLogic(
          ...
          getSdkCmakeFolders = {
            ...
            if (cmakePath != null) (sdkCMakeFolders.map { File(it) } + cmakePath).sorted() else sdkCMakeFolders.map { File(it) }
          },
          getEnvironmentPaths = {
      ...
      encounter.errors +=
        logger.errorMessages.map { message -> CxxDiagnosticCode.values().first { it.errorCode == message.diagnosticCode } }
      encounter.warnings +=
        logger.warningMessages.map { message -> CxxDiagnosticCode.values().first { it.warningCode == message.diagnosticCode } }
