...
class KotlinDslTest {
  ...
  private val ApplicationExtension.compileSdkHash: String?
    get() {
      var result: String? = null
      compileSdk { version?.let { result = (it as CompileSdkVersionImpl).toHash() } }
      return result
    }
  ...
  fun testCompileSdkPreview() {
    ...
    android.compileSdkAddon("Google Inc.", "Google APIs", TestConstants.COMPILE_SDK_VERSION_WITH_GOOGLE_APIS)
    assertThat(android.compileSdkPreview).isNull()
  ...
  fun testDslLocking() {
    ...
    val exception = assertFailsWith(AgpDslLockedException::class) { android.compileSdk = 28 }
  ...
  fun testNewDslLocking() {
    ...
    android.compileSdk { release(32) }
    android.defaultConfig {
      maxSdk { release(28) }
      targetSdk { preview("S") }
      minSdk {}
    }
    val exception = assertFailsWith(AgpDslLockedException::class) { android.defaultConfig.targetSdk { version = release(28) } }
    assertThat(exception)
      .hasMessageThat()
      .isEqualTo(
        """
  ...
  fun testDslLockingLists() {
    plugin.createAndroidTasks(project)
    val exception = assertFailsWith(AgpDslLockedException::class) { android.flavorDimensions += "test" }
    assertThat(exception)
      .hasMessageThat()
      .isEqualTo(
        """
  ...
  fun testDslLockingCollections() {
    plugin.createAndroidTasks(project)
    assertFailsWith(AgpDslLockedException::class) { android.buildTypes.create("customBuildType") }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
    ...
    assertFailsWith(AgpDslLockedException::class) { android.productFlavors.create("orange") }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
    ...
    assertFailsWith(AgpDslLockedException::class) { android.signingConfigs.create("qa") }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
  ...
  fun testDslLockingWithinCollections() {
    ...
    assertFailsWith(AgpDslLockedException::class) { debug.isCrunchPngs = true }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
    ...
    assertFailsWith(AgpDslLockedException::class) { android.defaultConfig.applicationId = "com.example.fruit" }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
    ...
    assertFailsWith(AgpDslLockedException::class) { orange.applicationId = "com.example.orange" }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
    ...
    assertFailsWith(AgpDslLockedException::class) { qa.enableV1Signing = true }
      .also { exception ->
        assertThat(exception)
          .hasMessageThat()
          .isEqualTo(
  ...
  fun `compile against variant specific external native build class`() {
    android.defaultConfig.apply {
      ...
      externalNativeBuild { ndkBuild { arguments("b") } }
      externalNativeBuild { cmake { arguments("y") } }

      assertThat(externalNativeBuild.ndkBuild.arguments).containsExactly("a", "b").inOrder()
      assertThat(externalNativeBuild.cmake.arguments).containsExactly("x", "y").inOrder()
    }
  }
  ...
  fun `extension aware build types`() {
    android.buildTypes.all { it.extensions.add("custom", CustomExtension::class.java) }
    android.buildTypes.getByName("release").apply { customExtension { customSetting = true } }
  ...
  fun `extension aware product flavors`() {
    android.productFlavors.all { it.extensions.add("custom", CustomExtension::class.java) }
    android.productFlavors.create("one")
    android.productFlavors.create("two").apply { customExtension { customSetting = true } }
  ...
  fun `compatibility for missingDimensionStrategy on productFlavor`() {
    android.productFlavors.create("example").apply { missingDimensionStrategy("minApi", "minApi18", "minApi23") }
  }

  @Test
  fun `compatibility for missingDimensionStrategy on defaultConfig`() {
    android.defaultConfig.apply { missingDimensionStrategy("minApi", "minApi18", "minApi23") }
  }

  @Test
  fun `whitespace handling in names`() {
    assertFailsWith(RuntimeException::class) { android.productFlavors.create("fla1 ") }
      .also { exception -> assertThat(exception).hasMessageThat().isEqualTo("ProductFlavor names cannot contain whitespace") }

    assertFailsWith(RuntimeException::class) { android.buildTypes.create("buildType ") }
      .also { exception -> assertThat(exception).hasMessageThat().isEqualTo("BuildType names cannot contain whitespace") }
  }
}
