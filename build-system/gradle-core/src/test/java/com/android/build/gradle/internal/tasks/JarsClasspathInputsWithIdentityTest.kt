...
class JarsClasspathInputsWithIdentityTest {
  ...
  fun testInitialIdentity() {
    ...
    inputs.forEachIndexed { index, file -> assertThat(jarsIdentity.jarsInfo[file]).isEqualTo(FileInfo(index.toString(), true)) }
  }

  @Test
  fun testIdentityLoadedOnce() {
    val mapping = FakeObjectFactory.factory.newInstance(JarsClasspathInputsWithIdentity::class.java)
    val changes = FakeInputChanges(false, emptyList())
    assertThat(mapping.getMappingState(changes)).isSameInstanceAs(mapping.getMappingState(changes))
  }
  ...
  fun testIdentityWithAdded() {
    ...
    val incrementalIdentity = getNewJarsIdentity(listOf(FakeFileChange(file = first, changeType = ChangeType.ADDED)), inputs)
  ...
  fun testIdentityWithRemoved() {
    ...
    val first = inputs.toList()[0]
    val incrementalIdentity = getNewJarsIdentity(listOf(FakeFileChange(file = first, changeType = ChangeType.REMOVED)), inputs)
  ...
  fun testIdentityWithModified() {
    ...
    val first = inputs.toList()[0]
    val incrementalIdentity = getNewJarsIdentity(listOf(FakeFileChange(file = first, changeType = ChangeType.MODIFIED)), inputs)
  ...
  fun testIdentityWithNonIncrementalChange() {
    ...
    val first = inputs.toList()[0]
    val nonIncrementalIdentity = getNewJarsIdentity(listOf(), inputs, false)
  ...
  private fun getInitialIdentity(vararg files: String): Pair<ConfigurableFileCollection, JarsIdentityMapping> {
    ...
    return Pair(
      inputs,
      mapping.getMappingState(FakeInputChanges(true, inputs.map { FakeFileChange(file = it, changeType = ChangeType.ADDED) })),
    )
  }
  ...
  private fun getNewJarsIdentity(
    ...
    return mapping.getMappingState(FakeInputChanges(incremental, changed))
  }
}
