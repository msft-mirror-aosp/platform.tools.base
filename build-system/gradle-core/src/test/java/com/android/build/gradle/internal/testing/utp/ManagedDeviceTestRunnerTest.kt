...
class ManagedDeviceTestRunnerTest {
  ...
  fun setupMocks() {
    ...
    whenever(mockObjectFactory.newInstance(eq(UtpRunConfig::class.java))).thenReturn(mock(defaultAnswer = RETURNS_DEEP_STUBS))
    ...
    whenever(mockAvdComponents.runWithAvds(any(), any(), any<(List<String>) -> Boolean>())).then {
  ...
  fun runUtpWithShardsAndPassed() {
    val result = runUtp(results = true, numShards = 2)
