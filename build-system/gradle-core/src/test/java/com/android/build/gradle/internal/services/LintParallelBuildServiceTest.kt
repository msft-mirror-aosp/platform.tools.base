...
class LintParallelBuildServiceTest {
  ...
  @Test
  fun testCalculateMaxParallelUsages() {
    // first test in process cases
    whenever(projectOptions.get(BooleanOption.RUN_LINT_IN_PROCESS)).thenReturn(true)

    // Check normal case
    Truth.assertThat(
        LintParallelBuildService.calculateMaxParallelUsages(projectOptions, maxRuntimeMemory = 20 * GB, totalPhysicalMemory = 40 * GB)
      )
      .isEqualTo(30)
    ...
    Truth.assertThat(
        LintParallelBuildService.calculateMaxParallelUsages(projectOptions, maxRuntimeMemory = 0, totalPhysicalMemory = 40 * GB)
      )
      .isEqualTo(1)
    ...
    try {
      LintParallelBuildService.calculateMaxParallelUsages(projectOptions, maxRuntimeMemory = 1 * GB, totalPhysicalMemory = 1 * GB)
      fail("expected RuntimeException")
    } catch (e: RuntimeException) {
      Truth.assertThat(e.message).isEqualTo("Failed to parse ${LINT_RESERVED_MEMORY_PER_TASK.propertyName} \"invalid\".")
    }
    ...
    Truth.assertThat(
        LintParallelBuildService.calculateMaxParallelUsages(projectOptions, maxRuntimeMemory = 10 * GB, totalPhysicalMemory = 40 * GB)
      )
      .isEqualTo(2)
    ...
    try {
      ...
      Truth.assertThat(e.message).isEqualTo("Failed to parse ${LINT_HEAP_SIZE.propertyName} \"invalid\".")
    }
  }
}
