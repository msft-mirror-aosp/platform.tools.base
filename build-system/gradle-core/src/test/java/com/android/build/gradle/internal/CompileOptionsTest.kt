...
class CompileOptionsTest {
  ...
  fun `test source and targetCompatibility, read before finalization`() {
    ...
    assertFailsWith<IllegalStateException> { options.sourceCompatibility }
      .also { assertThat(it.message).isEqualTo("sourceCompatibility is not yet finalized") }
    assertFailsWith<IllegalStateException> { options.targetCompatibility }
      .also { assertThat(it.message).isEqualTo("targetCompatibility is not yet finalized") }
  }
  ...
  fun `test source and targetCompatibility, write after finalization`() {
    ...
    assertFailsWith<IllegalStateException> { options.sourceCompatibility = JavaVersion.VERSION_1_8 }
      .also { assertThat(it.message).isEqualTo("sourceCompatibility has been finalized") }
    assertFailsWith<IllegalStateException> { options.targetCompatibility = JavaVersion.VERSION_1_8 }
      .also { assertThat(it.message).isEqualTo("targetCompatibility has been finalized") }
  }
