...
/** Give the shortest explanation for the difference between two multiline strings. If there are no differences then return "". */
...
private fun describeSpecial(value: String) =
  when (value) {
    "" -> "{NewLine}"
    else -> value
  }
...
private fun <Element> minimumEditDifference(from: List<Element>, to: List<Element>) =
  minimumEditDifferenceImpl(from = from.toIndexed(), to = to.toIndexed(), recorder = EditTrackingDifferenceRecorder())

/** Difference between two strings. */
private fun minimumEditDifference(from: CharSequence, to: CharSequence) =
  minimumEditDifferenceImpl(from = from.toIndexed(), to = to.toIndexed(), recorder = EditTrackingDifferenceRecorder())

/** Metric distance between two strings. */
@VisibleForTesting
fun minimumEditDistance(from: CharSequence, to: CharSequence) =
  minimumEditDifferenceImpl(from = from.toIndexed(), to = to.toIndexed(), recorder = CountingDifferenceRecorder())
...
private fun <T> List<T>.toIndexed() =
  object : Indexed<T> {
...
private fun CharSequence.toIndexed() =
  object : Indexed<Char> {
...
private class EditTrackingDifferenceRecorder : DifferenceRecorder<Record?> {
  ...
  override fun edit(prior: Record?, type: EditType, fromIndex: Int, toIndex: Int) =
    Record(prior = prior, count = (prior?.count ?: 0) + 1, type = type, fromIndex = fromIndex, toIndex = toIndex)
  ...
  override fun finalize(result: Record?): Record? {
    ...
    while (current != null) {
      ...
      previous = current.copy(prior = previous, count = count++)
      current = next
    }
    return previous
  }
...
private data class Record(val prior: Record?, val count: Int, val type: EditType, val fromIndex: Int, val toIndex: Int)

/**
 * Calculate the minimum edits (inserts, deletes, and replacements) needed to convert from [from] to [to]. This is an implementation of
 * Levenshtein distance (https://en.wikipedia.org/wiki/Levenshtein_distance) modified to return the steps needed to do the transformation
 * rather than just the count of differences.
 */
private fun <Element, Record> minimumEditDifferenceImpl(
  ...
  with(recorder) {
    ...
    repeat(m) { i ->
      repeat(n) { j ->
        val replace = if (to[j] == from[i]) edits[i][j] else edit(edits[i][j], Replace, i, j)
