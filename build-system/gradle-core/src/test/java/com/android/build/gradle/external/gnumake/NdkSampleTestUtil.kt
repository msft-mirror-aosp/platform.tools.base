...
/**
 * Compares all commands in the -nB output file to the commands that could be classified. They should be the same. If there's a test-only
 * case, then add a test-only classifier to [NdkSampleTest.extraTestClassifiers].
 */
internal fun checkAllCommandsRecognized(allCommands: List<CommandLine?>, classifiableCommands: List<BuildStepInfo>) {
  ...
  if (allCommands.size != classifiableCommands.size) {
    ...
    val classifiableCommandExecutables = classifiableCommands.map { it.command.executable }

    // Build a set of executable commands that were classified.
    val unclassifiedExecutables =
      allCommands.filterNotNull().map { it.executable }.filter { !classifiableCommandExecutables.contains(it) }.toSet()
...
/**
 * Check whether the ABIs found match recognized ABIs. Even very old ABIs like "mips" are recognized here because we have test samples that
 * reference them.
 */
