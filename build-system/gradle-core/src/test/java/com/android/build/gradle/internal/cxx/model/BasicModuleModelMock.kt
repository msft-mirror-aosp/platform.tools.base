...
/** Set up up a mock for constructing [CxxModuleModel]. It takes a lot of plumbing so this can be reused between tests that need this. */
open class BasicModuleModelMock {
  ...
  val ndkInstallStatus = NdkInstallStatus.Valid(mock<NdkPlatform>(defaultAnswer = throwUnmocked))
  ...
  val configurationParameters by lazy { tryCreateConfigurationParameters(projectOptions, variantImpl)!! }
  ...
  fun mockModule(appName: String): File {
    ...
    doReturn(prefabArtifactCollection)
      .whenever(variantDependencies)
  ...
  init {
    ...
    doReturn("").whenever(projectOptions).get(StringOption.NDK_SUPPRESS_MIN_SDK_VERSION_ERROR)
    doReturn(false).whenever(projectOptions).get(BooleanOption.ENABLE_PROFILE_JSON)
    doReturn(BooleanOption.ENABLE_CMAKE_BUILD_COHABITATION.defaultValue)
      .whenever(projectOptions)
      .get(BooleanOption.ENABLE_CMAKE_BUILD_COHABITATION)
    doReturn(true).whenever(projectOptions).get(BooleanOption.BUILD_ONLY_TARGET_ABI)
    doReturn(false).whenever(buildFeatures).prefab
    doReturn(true).whenever(projectOptions).get(BooleanOption.ENABLE_SIDE_BY_SIDE_CMAKE)
    doReturn(null).whenever(projectOptions).get(StringOption.IDE_BUILD_TARGET_ABI)
    doReturn("verbose").whenever(projectOptions).get(StringOption.NATIVE_BUILD_OUTPUT_LEVEL)

    doReturn(defaultCmakeVersion.toString()).whenever(cmake).version
    doReturn(listOf(Abi.X86.tag, Abi.X86_64.tag, Abi.ARMEABI_V7A.tag, Abi.ARM64_V8A.tag))
      .whenever(ndkInstallStatus.getOrThrow())
      .supportedAbis
    doReturn(listOf(Abi.X86.tag)).whenever(ndkInstallStatus.getOrThrow()).defaultAbis

    doReturn(ndkHandler).whenever(sdkComponents).versionedNdkHandler(any(), any())
    ...
    doReturn(cmakeDir.parentFile)
      .whenever(cmakeFinder)
      .findCmakePath(anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull(), anyOrNull())

    doReturn(cmakeDir.parentFile.resolve("ninja.exe")).whenever(ninjaFinder).findNinjaPath(anyOrNull(), anyOrNull())
