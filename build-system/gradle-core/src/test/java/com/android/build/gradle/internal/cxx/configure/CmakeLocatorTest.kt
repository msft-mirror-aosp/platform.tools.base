...
class CmakeLocatorTest {
  ...
  private fun fakeLocalCmakeOf(revision: CMakeVersion): FakePackage.FakeLocalPackage =
    fakeLocalCmakeOf(revision.sdkFolderName, revision.version)

  private fun fakeLocalCmakeOf(sdkFolderName: String, version: String): FakePackage.FakeLocalPackage {
    // path is like p;1.1
  ...
  /**
   * User request: "3.bob" or "+" Candidates from Local Repository: "3.6.4111459" Result: Invalid revision error is issued. Default version
   * "3.6.4111459" is selected.
   */
  private fun dslVersionInvalidTestCase(cmakeVersion: String) {
    val localCmake = fakeLocalCmakeOf(CMakeVersion.DEFAULT)
    val encounter = findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(localCmake) })
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/sdk/cmake/$defaultCmakeVersion") // This is a fallback.
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors.single())
      .isEqualTo("${CMAKE_VERSION_IS_INVALID.codeText} CMake version '$cmakeVersion' is not formatted correctly.")
  ...
  private fun dslVersionNumberTooLowTestCase(cmakeVersion: String) {
    ...
    val encounter = findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(defaultCmake, threeSix) })
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors.single())
      .isEqualTo(
  ...
  /** User request: "3.12.0" Candidates from Local Properties: "3.12.0" Result: "3.12.0" from local properties is selected. */
  private fun findByCmakeDirTestCase(cmakeVersionFromDsl: String) {
    ...
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/a/b/c/cmake")
    assertThat(encounter.warnings).hasSize(0)
  ...
  /** User request: "3.13.0" Candidates from Local Properties: "3.12.0" Result: Error. No matches found. */
  @Test
  ...
  /**
   * User request: default Candidates from Local Properties: "3.12" Candidates from Local Repository: "3.6.4111459" Result: The "3.12"
   * version from cmake.dir is prioritized over the default.
   */
  @Test
  ...
  /**
   * User request: default Candidates from Local Properties: "not a valid cmake directory" Candidates from Local Repository:
   * "$defaultCmakeVersion" Result: $defaultCmakeVersion is selected from local repository.
   *
   * In this scenario, user specified path in cmake.dir, but the directory does not contain a valid cmake version, so the cmake.dir input is
   * ignored.
   */
  @Test
  fun invalidCmakeDir() {
    ...
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!).isEqualTo("/sdk/cmake/$defaultCmakeVersion") // This is a fallback
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors)
      .containsExactly(
  ...
  /**
   * User request: null, "3.6.4111459" or "3.6.0-rc1" Candidates from Local Repository: "3.6.4111459", "3.10.2" Result: "3.6.4111459" from
   * local repository is selected. Higher version 3.10.2 is ignored.
   */
  private fun sdkCmakeExistsLocallyRequestLowerVersionTestCase(cmakeVersion: String?) {
    ...
    val encounter = findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(threeSix, threeTen) })
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/sdk/cmake/$defaultCmakeVersion")
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
  ...
  /**
   * User request: "3.10.2" or "3.10.2-rc1" Candidates from Local Repository: "3.6.4111459", "3.10.2" Result: "3.10.2" from local repository
   * is selected. Lower version 3.6.4111459 is ignored.
   */
  private fun sdkCmakeExistsLocallyRequestHigherVersionTestCase(cmakeVersion: String?) {
    ...
    val encounter = findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(threeSix, threeTen) })
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/sdk/cmake/3.10.2")
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
  ...
  /** User request: "3.10.2" Candidates from Local Repository: "3.8.0" Result: No matches. */
  private fun sdkCmakeNoExactMatchTestCase(cmakeVersion: String) {
    ...
    expectException(
      ...
      findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(localCmake) })
    }
  }
  ...
  /**
   * User request: default, "3.6.0", or "3.6.0-rc1" Candidates from Local Repository: "3.10.2" Downloader: Null downloader that does not
   * download anything. Result: Default version not found. Download attempt from SDK failed. Higher version 3.10.2 ignored.
   */
  ...
  /**
   * User request: default, "$defaultCmakeVersion", or "$defaultCmakeVersion-rc1" Candidates from Local Repository: "3.99.2" Candidates from
   * Path: "3.12.0", "3.13.0" Downloader: Successfully downloads "$defaultCmakeVersion" from SDK. Result: Default version not found.
   * Download attempt from SDK succeeded. Version "$defaultCmakeVersion" from local repository is selected. Higher version 3.99.2 is
   * ignored. Versions from path are ignored.
   */
  private fun sdkCmakeAutoInstallSuccessTestCase(cmakeVersion: String?) {
    ...
    val encounter =
      findCmakePath(
        ...
        environmentPaths = { listOf(File("/a/b/c/cmake/bin"), File("/d/e/f/cmake/bin")) },
        cmakeVersionGetter = { folder ->
          when (folder.toString().replace("\\", "/")) {
            "/a/b/c/cmake/bin" -> Revision.parseRevision("3.12.0")
            "/d/e/f/cmake/bin" -> Revision.parseRevision("3.13.0")
            else -> null
          }
        },
        downloader = { repositoryPackages.add(fakeLocalCmakeOf(CMakeVersion.DEFAULT)) },
      )
  ...
  /**
   * User request: "3.6.4111459" Candidates from Local Repository: "3.6.0" Result: An error is reported even though "3.6.4111459" is the SDK
   * version of 3.6.0-rc2
   */
  @Test
  fun sdkCmakeInternalCmakeVersionRejected() {
    ...
    expectException(
      ...
      findCmakePath(cmakeVersionFromDsl = "3.6.4111459", repositoryPackages = { listOf(threeSix) })
    }
  }

  /**
   * User request: "3.7.0+", "3.7.0-rc1+", "3.10.2+", "3.10.2-rc1+" Candidates from Local Repository: 3.6.411459, 3.10.2 Result: 3.10.2 is
   * selected, because it is higher and -rc1 is ignored.
   */
  private fun sdkCmakeWithPlusTestCase(cmakeVersion: String) {
    ...
    val encounter = findCmakePath(cmakeVersionFromDsl = cmakeVersion, repositoryPackages = { listOf(threeSix, threeTen) })
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!).isEqualTo("/sdk/cmake/3.10.2.4988404")
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
  ...
  fun sdkCmakeWithPlusNoMatch() {
    ...
    expectException(
      ...
      findCmakePath(cmakeVersionFromDsl = cmakeVersionWithPlus, repositoryPackages = { listOf(localCmake) })
    }
  }

  /**
   * User request: "3.12.0" or "3.12.0-rc1" Candidates from path: "3.12.0-a" and "3.12.0-b" Result: "3.12.0-a" from path is selected because
   * it is the first on path, and "-rc1" is ignored.
   */
  private fun findByPathTestCase(cmakeVersion: String) {
    val encounter =
      findCmakePath(
        cmakeVersionFromDsl = cmakeVersion,
        environmentPaths = { listOf(File("/a/b/c/cmake/3.12.0-a/bin"), File("/a/b/c/cmake/3.12.0-b/bin"), File("/d/e/f")) },
        cmakeVersionGetter = { folder ->
          when (folder.toString().replace("\\", "/")) {
    ...
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/a/b/c/cmake/3.12.0-a")
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
  ...
  /**
   * User request: "3.12.0" or "3.12.0-rc1" Candidates from SDK Folders: "3.12.0-a" and "3.12.0-b" Result: "3.12.0-a" from SDK is selected
   * because it is the first, and "-rc1" is ignored.
   */
  private fun findBySdkFolderTestCase(cmakeVersion: String) {
    val encounter =
      findCmakePath(
        cmakeVersionFromDsl = cmakeVersion,
        sdkFolders = { listOf(File("/a/b/c/cmake/3.12.0-a/bin"), File("/a/b/c/cmake/3.12.0-b/bin"), File("/d/e/f")) },
        cmakeVersionGetter = { folder ->
          when (folder.toString().replace("\\", "/")) {
            "/a/b/c/cmake/3.12.0-a/bin" -> Revision.parseRevision("3.12.0")
            "/a/b/c/cmake/3.12.0-b/bin" -> Revision.parseRevision("3.12.0")
  ...
  /**
   * User request: "3.12.0" Candidates from Path: "reading version throws IOException", "3.12.0" Result: The first candidate on path is
   * skipped. Selects "3.12.0" from path.
   */
  @Test
  fun findByPathCmakeExeThrowsException() {
    ...
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/d/e/f/cmake")
    assertThat(encounter.warnings)
      .contains("C/C++: Could not execute cmake at " + "'${slash}a${slash}b${slash}c${slash}cmake${slash}bin' to get version. Skipping.")
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
  ...
  fun `get specific version`() {
    assertThat(CmakeVersionRequirements("3.19.2").effectiveRequestVersion).isEqualTo(Revision.parseRevision("3.19.2"))
  }
  ...
  fun `special cases for fork CMake version`() {
    ...
    CmakeVersionRequirements(CMakeVersion.FORK.version).isSatisfiedBy(forkCmakeSdkVersionRevision)
    CmakeVersionRequirements(CMakeVersion.FORK.sdkFolderName).isSatisfiedBy(forkCmakeReportedVersion)
  }
  ...
  fun `bug 226225504 prefer SDK CMake over PATH when both satisfy`() {
    ...
    assertThat(encounter.result).isNotNull()
    assertThat(encounter.result!!.toString()).isEqualTo("/sdk/cmake/3.22.1")
    assertThat(encounter.warnings).hasSize(0)
    assertThat(encounter.errors).hasSize(0)
    assertThat(encounter.downloadAttempts).isEqualTo(0)
  }
