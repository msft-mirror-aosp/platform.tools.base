...
class MergeJavaResWorkActionTest {
  @get:Rule var tmpDir = TemporaryFolder()
  ...
  fun testMergeResources() {
    ...
    object : MergeJavaResWorkAction() {
        override fun getParameters(): Params {
          return object : Params() {
            override val projectJavaRes = FakeObjectFactory.factory.fileCollection().from(jarFile1)
            ...
            override val incrementalStateFile = FakeObjectFactory.factory.fileProperty().also { it.set(incrementalStateFile) }
            override val incremental = FakeGradleProperty(false)
            override val cacheDir = FakeObjectFactory.factory.directoryProperty().also { it.set(cacheDir) }
            override val changedInputs = FakeObjectFactory.factory.property(SerializableInputChanges::class.java)
    ...
    ZipFile(outputFile).use {
      val entry1Timestamp = it.getEntry("fileEndingWithDot.").lastModifiedTime.toInstant().toString()
      val entry2Timestamp = it.getEntry("fileNotEndingWithDot").lastModifiedTime.toInstant().toString()
  ...
  fun testMergeResourcesWithNoCompress() {
    ...
    object : MergeJavaResWorkAction() {
        override fun getParameters(): Params {
          return object : Params() {
            override val projectJavaRes = FakeObjectFactory.factory.fileCollection().from(dir)
            ...
            override val cacheDir = FakeObjectFactory.factory.directoryProperty().also { it.set(cacheDir) }
            override val changedInputs = FakeObjectFactory.factory.property(SerializableInputChanges::class.java)
            override val noCompress = FakeObjectFactory.factory.listProperty(String::class.java).also { it.add(".no_compress") }
            override val projectPath = FakeGradleProperty("projectName")
            override val taskOwner = FakeGradleProperty("taskOwner")
            override val workerKey = FakeGradleProperty("workerKey")
            override val analyticsService: Property<AnalyticsService> = FakeGradleProperty(FakeNoOpAnalyticsService())
  ...
  fun testIncrementalMergeResources() {
    ...
    val jarFile = File(tmpDir.root, "jarFile.jar")
    ZFile(jarFile).use { it.add("javaRes1", ByteArrayInputStream(ByteArray(0))) }
    ...
    object : MergeJavaResWorkAction() {
        override fun getParameters(): Params {
          return object : Params() {
            override val projectJavaRes = FakeObjectFactory.factory.fileCollection().from(jarFile)
    ...
    ZipArchive(jarFile.toPath()).use { it.add(BytesSource(ByteArray(0), "javaRes2", 0)) }
    ...
    object : MergeJavaResWorkAction() {
        override fun getParameters(): Params {
          return object : Params() {
            ...
            override val cacheDir = FakeObjectFactory.factory.directoryProperty().also { it.set(cacheDir) }
            val changes =
              SerializableInputChanges(roots = listOf(jarFile), changes = listOf(SerializableChange(jarFile, FileStatus.CHANGED, "")))
            override val changedInputs = FakeObjectFactory.factory.property(SerializableInputChanges::class.java).also { it.set(changes) }
            override val noCompress = FakeObjectFactory.factory.listProperty(String::class.java)
            override val projectPath = FakeGradleProperty("projectName")
            override val taskOwner = FakeGradleProperty("taskOwner")
            override val workerKey = FakeGradleProperty("workerKey")
