...
internal class ListingFileRedirectTaskTest {
  ...
  fun testTaskExecution() {
    ...
    task.listingFile.set(project.objects.fileProperty().also { it.set(File(folder1, "listingFile")) })
    val redirectFile = File(folder2, "redirectFile")
    task.redirectFile.set(project.objects.fileProperty().also { it.set(redirectFile) })
  ...
  fun testDifferentRootTestCase() {
    ...
    whenever(mockPath.relativize(listingFile.toPath())).thenThrow(IllegalArgumentException("Different roots"))

    // need to mock task.redirectFile.asFile.get()
    val mockFileProperty = mock<RegularFileProperty>()
    @Suppress("UNCHECKED_CAST") val mockProvider = mock<Provider<File>>()
    ...
    spyTask.listingFile.set(project.objects.fileProperty().also { it.set(listingFile) })
    ...
    Truth.assertThat(redirectFile.readLines()[1]).isEqualTo("listingFile=" + listingFile.canonicalPath.replace("\\", "/"))
  }
  ...
  fun testTaskConfiguration() {
    ...
    val artifactType = object : Artifact.Single<RegularFile>(ArtifactKind.FILE, Category.INTERMEDIATES) {}
    val creationAction =
    ...
    @Suppress("UNCHECKED_CAST") val request = mock<SingleInitialProviderRequestImpl<ListingFileRedirectTask, RegularFile>>()
    whenever(artifacts.setInitialProvider(taskProvider, ListingFileRedirectTask::redirectFile)).thenReturn(request)
    ...
    whenever(artifacts.get(InternalArtifactType.APK_IDE_MODEL)).thenReturn(project.objects.fileProperty().also { it.set(listingFile) })
