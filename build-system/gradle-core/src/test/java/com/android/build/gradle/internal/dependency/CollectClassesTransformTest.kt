...
internal class CollectClassesTransformTest {
  ...
  fun `extracts all class paths from jar file`() {
    ...
    val testJar = writeTestJar(jarDirectory, listOf(EnumClass::class.java, OuterClass::class.java, OuterClass.InnerClass::class.java))
    ...
    assertThat(transformOutputs.outputFile.readLines())
  ...
  private fun writeTestJar(outputFolder: File, classes: List<Class<*>>, name: String = "testJar"): File {
    val jarFile = File(outputFolder, "${name.removeSuffix(SdkConstants.DOT_JAR)}${SdkConstants.DOT_JAR}")
  ...
  private fun getTestTransform(jarFile: File): CollectClassesTransform {
    return object : CollectClassesTransform() {
      ...
      override fun getParameters(): GenericTransformParameters =
        object : GenericTransformParameters {
