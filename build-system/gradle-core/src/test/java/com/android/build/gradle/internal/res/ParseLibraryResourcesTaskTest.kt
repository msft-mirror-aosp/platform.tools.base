...
class ParseLibraryResourcesTaskTest(private val usePartialR: Boolean, private val useResourceValidation: Boolean) {
  ...
  companion object {
    ...
    fun data() = arrayOf(arrayOf(true, true), arrayOf(false, false), arrayOf(true, false), arrayOf(false, true))
  }
  ...
  fun testCanBeProcessedIncrementally() {
    ...
    val removedFile = SerializableChange(valuesFile, FileStatus.REMOVED, valuesFile.absolutePath)
    val addedFile = SerializableChange(valuesFile, FileStatus.NEW, valuesFile.absolutePath)
    val modifiedValuesFile = SerializableChange(valuesFile, FileStatus.CHANGED, valuesFile.absolutePath)
    val modifiedDrawableXml = SerializableChange(drawableXml, FileStatus.CHANGED, drawableXml.absolutePath)
    val modifiedDrawablePng = SerializableChange(drawablePng, FileStatus.CHANGED, drawablePng.absolutePath)

    if (!usePartialR) {
      assertThat(canBeProcessedIncrementallyWithRDef(removedFile)).isFalse()
      assertThat(canBeProcessedIncrementallyWithRDef(addedFile)).isTrue()
      assertThat(canBeProcessedIncrementallyWithRDef(modifiedValuesFile)).isFalse()
      assertThat(canBeProcessedIncrementallyWithRDef(modifiedDrawableXml)).isFalse()
    }
    assertThat(canBeProcessedIncrementallyWithRDef(addedFile)).isTrue()
    assertThat(canBeProcessedIncrementallyWithRDef(modifiedDrawablePng)).isTrue()
  }
  ...
  fun testDoFullTaskAction_producesExpectedSymbolTableFile() {
    ...
    val params =
      object : ParseLibraryResourcesTask.ParseResourcesParams() {
        ...
        override val enablePartialRIncrementalBuilds = FakeGradleProperty(this@ParseLibraryResourcesTaskTest.usePartialR)
    ...
    assertThat(librarySymbolsFile.readLines())
      .containsExactly(
  ...
  fun testDoFullTaskAction_producesExpectedPartialRFiles() {
    ...
    val createdPartialRFiles = partialRDirectory.walkTopDown().toList().filter { it.isFile }.sortedBy { it.name }
    if (usePartialR) {
      ...
      assertThat(createdPartialRFiles[3].readLines()).containsExactly("undefined int string farewell", "undefined int string greeting")
    } else {
  ...
  fun testDoIncrementalTaskAction_producesExpectedSymbolTableFileFromAddedResource() {
    ...
    val addedLayout = File(FileUtils.join(resourcesFolder.path, "layout"), "second_activity.xml")
    ...
    val changedResources = listOf(SerializableChange(addedLayout, FileStatus.NEW, addedLayout.absolutePath))
    val innerResDirs = objects.listProperty(Directory::class.java)
    innerResDirs.add(objects.directoryProperty().fileValue(resourcesFolder))
  ...
  fun testDoIncrementalTaskAction_producesExpectedSymbolTableFileFromModifiedResource() {
    ...
    val platformAttrsRTxtFile = File(parentFolder, "R-def.txt")
    val librarySymbolsFile = File(parentFolder, "Symbols.txt")
    val modifiedLayout = File(FileUtils.join(resourcesFolder.path, "layout"), "second_activity.xml")
    val modifiedLayoutPartialR = File(partialRFolder, "layout_second_activity.xml.flat-R.txt")
    ...
    FileUtils.createFile(modifiedLayoutPartialR, "undefined int layout second_activity\n" + "undefined int id chipOne\n")

    val changedResources = listOf(SerializableChange(modifiedLayout, FileStatus.CHANGED, modifiedLayout.absolutePath))
    val innerResDirs = objects.listProperty(Directory::class.java)
    innerResDirs.add(objects.directoryProperty().fileValue(resourcesFolder))
    val params =
      object : ParseLibraryResourcesTask.ParseResourcesParams() {
  ...
  fun testDoIncrementalTaskAction_producesExpectedSymbolTableFileFromRemovedResource() {
    ...
    val platformAttrsRTxtFile = File(parentFolder, "R-def.txt")
    val librarySymbolsFile = File(parentFolder, "Symbols.txt")
    val removedLayout = File(FileUtils.join(resourcesFolder.path, "layout"), "second_activity.xml")
    ...
    FileUtils.createFile(removedLayoutPartialR, "undefined int layout second_activity\n" + "undefined int id chipOne")
    FileUtils.delete(removedLayout)
    val changedResources = listOf(SerializableChange(removedLayout, FileStatus.REMOVED, removedLayout.absolutePath))
    val innerResDirs = objects.listProperty(Directory::class.java)
    innerResDirs.add(objects.directoryProperty().fileValue(resourcesFolder))
    val params =
      object : ParseLibraryResourcesTask.ParseResourcesParams() {
  ...
  fun testGenerateResourceSymbolTables_generatesExpectedSymbolTables() {
    ...
    val symbolTables = getResourceDirectorySymbolTables(fakeResourceDirectory, null, documentBuilder, true, logger).toTypedArray()

    assertThat(symbolTables.count()).isEqualTo(4)
    assertThat(symbolTables[0].symbolTable.symbols.values().toList().toString()).isEqualTo("[UNDEFINED drawable img = 0x0]")
    assertThat(symbolTables[1].symbolTable.symbols.values().toList().toString()).isEqualTo("[UNDEFINED layout content_layout = 0x0]")
    assertThat(symbolTables[3].symbolTable.symbols.values().toList().toString())
      .isEqualTo("[UNDEFINED string greeting = 0x0, UNDEFINED string farewell = 0x0]")
  }
  ...
  fun testSavePartialRFilesToDirectory_checkAllPartialFilesSaved() {
    ...
    val symbolTables = getResourceDirectorySymbolTables(fakeResourceDirectory, emptyPlatformAttrSymbolTable, documentBuilder, true, logger)
    ...
    assertThat(createdPartialRFiles.size).isEqualTo(4)
    assertThat(createdPartialRFiles.first { it.name == "values_strings.arsc.flat-R.txt" }.readLines())
      .containsExactly("undefined int string greeting", "undefined int string farewell")
    assertThat(createdPartialRFiles.first { it.name == "layout_main_activity.xml.flat-R.txt" }.readLines())
      .containsExactly("undefined int layout main_activity")
    assertThat(createdPartialRFiles.first { it.name == "drawable_img.png.flat-R.txt" }.readLines())
      .containsExactly("undefined int drawable img")
  }
  ...
  fun testResourceValidation_isDisabledByFlag() {
    ...
    File(FileUtils.join(fakeResourceDirectory.path, "layout"), "second_activity.png").also { FileUtils.createFile(it, "") }
    val innerResDirs = objects.listProperty(Directory::class.java)
    ...
    val params =
      object : ParseLibraryResourcesTask.ParseResourcesParams() {
        ...
        override val librarySymbolsFile = objects.fileProperty().fileValue(librarySymbolsFile)
        override val incremental = FakeGradleProperty(false)
        override val changedResources = objects.listProperty(SerializableChange::class.java).value(emptyList())
        override val partialRDir = objects.directoryProperty().fileValue(partialRFolder)
        override val enablePartialRIncrementalBuilds = FakeGradleProperty(true)
        override val projectPath = FakeGradleProperty("projectName")
        override val taskOwner = FakeGradleProperty("taskOwner")
  ...
  private fun createPartialRDirectory(partialRFolder: File): File {
    ...
    partialRMap.forEach { (fileName, fileContent) -> FileUtils.createFile(File(partialRFolder, fileName), fileContent) }
