...
class ManagedDeviceInstrumentationTestSetupTaskTest {
  ...
  fun setup() {
    ...
    whenever(globalConfig.services.buildServiceRegistry.registrations.getByName(any())).thenReturn(mockGeneralRegistration)
  }
  ...
  private fun basicTaskSetup(): ManagedDeviceInstrumentationTestSetupTask {
    ...
    doReturn(realPropertyFor(mock<AnalyticsService>())).whenever(task).analyticsService
    ...
    doReturn(realPropertyFor(mock<Revision>())).whenever(task).buildToolsRevision
    ...
    doReturn(FakeGradleWorkExecutor(project.objects, temporaryFolderRule.newFolder())).whenever(task).workerExecutor
    return task
  }
  ...
  fun taskAction_basicSetupPath() {
    ...
    whenever(mockVersionedSdkLoader.sdkImageDirectoryProvider(any())).thenReturn(FakeGradleProperty(imageDirectory))
    whenever(avdService.avdProvider(any(), any(), any(), any())).thenReturn(FakeGradleProperty(mock<Directory>()))

    task.taskAction()

    verify(mockVersionedSdkLoader).emulatorDirectoryProvider
    verify(mockVersionedSdkLoader).sdkImageDirectoryProvider("system-images;android-29;default;x86_64")
    verifyNoMoreInteractions(mockVersionedSdkLoader)
    verify(emulatorProvider).get()

    verify(avdService)
      .avdProvider(
        argThat { this is FakeGradleProperty && this.get() == imageDirectory },
        eq("system-images;android-29;default;x86_64"),
        eq("dev29_default_x86_64_Pixel_2"),
        eq("Pixel 2"),
      )
    verify(avdService).ensureLoadableSnapshot("dev29_default_x86_64_Pixel_2")
  ...
  fun testTaskAction_armTranslationUnavailableOnX86() {
    ...
    val error = assertThrows(IllegalStateException::class.java) { task.taskAction() }
  ...
  fun testTaskAction_armTranslationUnavailableOnIncompatibleSource() {
    ...
    val wrongSourceError = assertThrows(IllegalStateException::class.java) { task.taskAction() }

    assertThat(wrongSourceError.message)
      .isEqualTo(
        """
    ...
    val wrongSdkVersionError = assertThrows(IllegalStateException::class.java) { task.taskAction() }

    assertThat(wrongSdkVersionError.message)
      .isEqualTo(
        """
        ARM translation is only available for Google APIs or Play Store images
        with an API level of 30 or higher.
  ...
  fun testTaskAction_missingImage() {
    ...
    whenever(mockVersionedSdkLoader.sdkImageDirectoryProvider(any())).thenReturn(FakeGradleProperty(null))
    ...
    verify(mockVersionedSdkLoader).sdkImageDirectoryProvider("system-images;android-29;default;x86_64")
    verify(mockVersionedSdkLoader).offlineMode
    verifyNoMoreInteractions(mockVersionedSdkLoader)
  ...
  fun testTaskAction_selectsImageBasedOnMinorVersion() {
    ...
    verify(mockVersionedSdkLoader).sdkImageDirectoryProvider("system-images;android-36.1;default;x86_64")
    verify(mockVersionedSdkLoader).offlineMode
    verifyNoMoreInteractions(mockVersionedSdkLoader)
    verifyNoInteractions(avdService)
  }
  ...
  fun testTaskAction_errorOnAutoProfile() {
    val task = basicTaskSetup()
    doReturn(realPropertyFor("Automotive (1024p landscape)")).whenever(task).hardwareProfile
    ...
    try {
      ...
      SystemPropertyOverrides().use { systemPropertyOverrides ->
        ...
        Environment.instance =
          object : Environment() {
            override fun getVariable(name: EnvironmentVariable): String? = if (name.key == "HOSTTYPE") "x86_64" else null
          }
        ...
        whenever(globalConfig.services.projectOptions[StringOption.GRADLE_MANAGED_DEVICE_EMULATOR_GPU_MODE]).thenReturn(null)
  ...
  fun generateSystemErrorMessage_offlineMode() {
    ...
    val result =
      ManagedDeviceInstrumentationTestSetupTask.generateSystemImageErrorMessage(
        "test_device_name",
        28,
        0,
        null,
  ...
  fun generateSystemErrorMessage_onlineMode() {
    ...
    assertThat(result).isEqualTo("System Image specified by some_test_device does not exist.\n\n" + "Try one of the following fixes:")
  }
}
