...
internal class MergeGeneratedProguardFilesCreationActionTest {
  ...
  fun testGetSubFolder_does_not_exist() {
    ...
    Truth.assertThat(MergeGeneratedProguardFilesCreationAction.getSubFolder(topFolder1, "meta-info", "proguard")).isNull()
  }
  ...
  fun testGetSubFolder_exists() {
    temporaryFolder.newFolder().also {
      File(File(it, "meta-inf"), "proguard").also { directory ->
        ...
        val subFolder = MergeGeneratedProguardFilesCreationAction.getSubFolder(FakeGradleDirectory(it), "meta-inf", "proguard")
