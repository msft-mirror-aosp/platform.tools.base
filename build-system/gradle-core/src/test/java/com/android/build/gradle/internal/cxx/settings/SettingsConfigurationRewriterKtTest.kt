...
class SettingsConfigurationRewriterKtTest {
  ...
  fun `check rewrite with CMakeSettings json`() {
    CmakeSettingsMock().apply {
      val variant = variant.copy(cmakeSettingsConfiguration = DIFFERENT_MOCK_CMAKE_SETTINGS_CONFIGURATION)
      val abi = createCxxAbiModel(sdkComponents, configurationParameters, variant, "x86")
      val rewritten = abi.calculateConfigurationArguments(providers, layout)
      assertThat(rewritten.cxxBuildFolder.path).contains("some other build root folder")
      assertThat(rewritten.variant.module.cmake!!.cmakeExe!!.path.replace('\\', '/')).isEqualTo("my/path/to/cmake")
      assertThat(rewritten.variant.module.cmakeToolchainFile.path.replace('\\', '/')).isEqualTo("my/path/to/toolchain")
  ...
  fun `alternate check`() {
    CmakeSettingsMock().apply {
      val variant = variant.copy(cmakeSettingsConfiguration = DIFFERENT_MOCK_CMAKE_SETTINGS_CONFIGURATION)
      val abi = createCxxAbiModel(sdkComponents, configurationParameters, variant, "x86").calculateConfigurationArguments(providers, layout)
  ...
  fun `map CMAKE_BUILD_TYPE to MinSizeRel`() {
    CmakeSettingsMock().apply {
      val configurationParameters = configurationParameters.copy(variantName = "myMinSizeRel")
      val model = createInitialCxxModel(sdkComponents, listOf(configurationParameters), providers, layout)
    ...
    BasicCmakeMock().apply {
      val configurationParameters =
        configurationParameters.copy(
          ...
          nativeVariantConfig =
            configurationParameters.nativeVariantConfig.copy(arguments = listOf("NDK_MODULE_PATH+=./third_party/modules")),
        )
      val abis = createInitialCxxModel(sdkComponents, listOf(configurationParameters), providers, layout)
      abis.forEach { abi ->
  ...
  fun `bug 159434435--unknown user build args are forwarded`() {
    CmakeSettingsMock().apply {
      val variant =
        variant.copy(
          buildSystemArgumentList = listOf("-CD:\\Test\\TargetProperties.cmake", "--log-level=VERBOSE", "-X some-parameter-after-a-space")
        )
      val abi = createCxxAbiModel(sdkComponents, configurationParameters, variant, "x86").calculateConfigurationArguments(providers, layout)
  ...
  fun `make sure ANDROID_STL can be extracted`() {
    CmakeSettingsMock().apply {
      val variant = variant.copy(buildSystemArgumentList = listOf("-DANDROID_STL=c++_shared"))
      val abi = createCxxAbiModel(sdkComponents, configurationParameters, variant, "x86").calculateConfigurationArguments(providers, layout)
      println(abi.toJsonString())
  ...
  fun `check MinSizeRel affects soFolder`() {
    CmakeSettingsMock().apply {
      val variant = variant.copy(buildSystemArgumentList = listOf("-D$CMAKE_BUILD_TYPE=MinSizeRel"))
      val abi = createCxxAbiModel(sdkComponents, configurationParameters, variant, "x86").calculateConfigurationArguments(providers, layout)
      val variables = abi.configurationArguments.toCmakeArguments()
      println(variables.joinToString("\n") { it.sourceArgument })
  ...
  fun `build settings macros are expanded`() {
    CmakeSettingsMock().apply {
      ...
      assertThat(abi.buildSettings.environmentVariables)
        .isEqualTo(listOf(EnvironmentVariable("NDK_ABI", "\${ndk.abi}"), EnvironmentVariable("NDK_DIR", NDK_MODULE_NDK_DIR.ref)))

      assertThat(rewritten.buildSettings.environmentVariables)
        .isEqualTo(listOf(EnvironmentVariable("NDK_ABI", abi.name), EnvironmentVariable("NDK_DIR", abi.variant.module.ndkFolder.path)))
    }
  }
  ...
  private fun abisOf(
    ...
    CmakeSettingsMock().apply {
      ...
      val configurationModel1 = tryCreateConfigurationParameters(mock<ProjectOptions>(), variantImpl)!!
      val variant1 = createCxxVariantModel(configurationModel1, module)
      val result1 =
        createCxxAbiModel(sdkComponents, configurationModel1, variant1, abi1.tag).calculateConfigurationArguments(providers, layout)
      result1.toJsonString() // Force all lazy values

      setup(this, 2)
      val configurationModel2 = tryCreateConfigurationParameters(mock<ProjectOptions>(), variantImpl)!!
      val variant2 = createCxxVariantModel(configurationModel2, module)
      val result2 =
        createCxxAbiModel(sdkComponents, configurationModel2, variant2, abi2.tag).calculateConfigurationArguments(providers, layout)
