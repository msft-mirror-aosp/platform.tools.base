...
class DexArchiveBuilderDelegateTest {
  ...
  @JvmField @Rule var tmpDir = TemporaryFolder()
  ...
  fun testInitialBuild() {
    ...
    getDelegate(projectClasses = setOf(dirInput.toFile(), jarInput.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testEntryRemovedFromTheArchive() {
    ...
    val change = FakeFileChange(toDelete, ChangeType.REMOVED, FileType.FILE, "$PACKAGE/B.class")
    getDelegate(
        projectClasses = setOf(inputDir.toFile(), inputJar.toFile()),
        isIncremental = true,
  ...
  fun testNonIncremental() {
    ...
    getDelegate(projectClasses = setOf(inputDir.toFile(), inputJar.toFile()), projectOutput = out.toFile()).doProcess()

    val inputDir2 = tmpDir.root.toPath().resolve("dir_2_input")
    dirWithEmptyClasses(inputDir2, ImmutableList.of("$PACKAGE/C"))

    getDelegate(projectClasses = setOf(inputDir2.toFile(), inputJar.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testIncrementalUnchangedDirInput() {
    ...
    val inputJarHashesFile = tmpDir.newFile().also { writeEmptyInputJarHashes(it) }
    getDelegate(
        isIncremental = true,
  ...
  fun testDirectoryRemovedInIncrementalBuild() {
    ...
    getDelegate(
        ...
        projectClasses = setOf(input.toFile()),
        projectOutput = out.toFile(),
        projectChanges = setOf(FakeFileChange(nestedDir.toFile(), ChangeType.REMOVED, FileType.DIRECTORY, "nested_dir")),
        inputJarHashesFile = inputJarHashesFile,
      )
      .doProcess()
  ...
  fun testMultiReleaseDir() {
    val input =
      tmpDir.root.resolve("classes").also {
    ...
    getDelegate(projectClasses = setOf(input), projectOutput = out.toFile(), inputJarHashesFile = inputJarHashes).doProcess()
    ...
    getDelegate(
        ...
        projectOutput = out.toFile(),
        projectChanges = setOf(FakeFileChange(moduleInfoClass, ChangeType.REMOVED, FileType.FILE, "module-info.class")),
  ...
  fun testJarNameDoesNotImpactOutput() {
    ...
    jarWithEmptyClasses(inputJar1, ImmutableList.of("$PACKAGE/A", "$PACKAGE/B", "$PACKAGE/C"))

    getDelegate(projectClasses = setOf(inputJar1.toFile()), projectOutput = out.toFile()).doProcess()
    ...
    getDelegate(projectClasses = setOf(inputJar2.toFile()), projectOutput = out.toFile()).doProcess()
  ...
  fun testBucketingStrategy() {
    ...
    getDelegate(projectClasses = setOf(jarInput1.toFile()), projectOutput = out.toFile(), numberOfBuckets = numberOfBuckets).doProcess()
    ...
    getDelegate(projectClasses = setOf(jarInput2.toFile()), projectOutput = out.toFile(), numberOfBuckets = numberOfBuckets).doProcess()

    // Check that jar bucket location does not change as the package names of the input classes
    // are the same.
    assertThat(FileUtils.find(out.toFile(), Pattern.compile(".*_$jarBucket\\.jar"))).hasSize(1)
  }
  ...
  fun test_removingInputHashesRunsNonIncrementally() {
    ...
    val inputJarHashes = tmpDir.newFile()
    getDelegate(projectClasses = setOf(input.toFile()), projectOutput = out.toFile(), inputJarHashesFile = inputJarHashes).doProcess()
    ...
    assertThat(FileUtils.find(out.toFile(), "A.dex").get().lastModified()).isGreaterThan(initialTimestamp)
  }
  ...
  private fun findOutputBucketForDirInput(numberOfBuckets: Int): String =
    (0 until numberOfBuckets)
      .find { FileUtils.find(out.resolve(it.toString()).toFile(), Pattern.compile(".*\\.dex")).size == 1 }
      ?.toString() ?: fail("Failed to find a folder with dex output")

  private fun findOutputBucketForJarInput(numberOfBuckets: Int): String {
    val file = FileUtils.find(out.toFile(), Pattern.compile(".*\\.jar")).first() ?: fail("Failed to find a jar dex output")
    val name = file.name
  ...
  companion object {
    @JvmStatic @Parameterized.Parameters(name = "incrementalDexingTaskV2_{0}") fun parameters() = listOf(arrayOf(false), arrayOf(true))
  }
}

internal fun writeEmptyInputJarHashes(file: File) {
  ObjectOutputStream(file.outputStream().buffered()).use { it.writeObject(mutableMapOf<File, String>()) }
}
