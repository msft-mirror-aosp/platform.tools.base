...
internal class FullDependencyGraphBuilderTest {
  ...
  fun `basic dependency`() {
    ...
    Truth.assertThat(graphs.compileDependencies.map { it.key }).containsExactly("foo|bar|1.0||foo:bar:1.0")
    ...
    val (graphs, libraries) =
      buildModelGraph {
        module("foo", "bar", "1.0") {
          file = File("path/to/bar-1.0.jar")
          setupDefaultCapability()
          attribute(BuildTypeAttr.ATTRIBUTE, objectFactory.named(BuildTypeAttr::class.java, "debug"))
        }
      }

    Truth.assertThat(graphs.compileDependencies.map { it.key }).containsExactly("foo|bar|1.0|debug||foo:bar:1.0")
    val item = graphs.compileDependencies.first()
    Truth.assertThat(item.dependencies).isEmpty()
  }
  ...
  fun `graph with java module test fixtures`() {
    ...
    Truth.assertThat(compileDependencies.map { it.key })
      .containsExactly("foo|bar|1.0||foo:bar:1.0", "foo|bar|1.0||foo:bar-test-fixtures:1.0")

    // check that the dependency instance of the fixture is the same instance as the main
    val fixture = compileDependencies.single { it.key == "foo|bar|1.0||foo:bar-test-fixtures:1.0" }
  ...
  fun `graph with android project test fixtures`() {
    ...
    val compileDependencies = graphs.compileDependencies
    Truth.assertThat(compileDependencies.map { it.key })
      .containsExactly("defaultBuildPath|:foo||foo:bar:1.0", "defaultBuildPath|:foo||foo:bar-test-fixtures:1.0")

    // check that the dependency instance of the fixture is the same instance as the main
    val fixture = compileDependencies.single { it.key == "defaultBuildPath|:foo||foo:bar-test-fixtures:1.0" }
  ...
  fun `relocated artifact`() {
    ...
    val rootLibrary = libraries["foo|bar|1.0||foo:bar:1.0"] ?: throw RuntimeException("Failed to find root library")
    ...
    Truth.assertThat(children.map { it.key }).containsExactly("relocated-foo|bar|1.0||relocated-foo:bar:1.0")
  }
  ...
  fun testDependencyConstraint() {
    val (graphs, libraries) =
      buildModelGraph {
        val barParentLib = module("foo", "bar-parent", "1.0") { file = File("path/to/bar-parent-1.0.jar") }
    ...
    Truth.assertThat(graphs.compileDependencies.map { it.key }).containsExactly("foo|bar-parent|1.0||", "foo|bar|1.0||")
  }
    ...
    val (graphs, _) =
      buildModelGraph {
        module("foo", "bar", "1.0") {
          ...
          dependency(module("foo", "bar-parent", "1.0") { file = File("path/to/bar-parent-1.0.jar") })
        }
      }

    Truth.assertThat(graphs.compileDependencies.map { it.key }).containsExactly("foo|bar|1.0||")
    Truth.assertThat(graphs.compileDependencies.single().dependencies.map { it.key }).containsExactly("foo|bar-parent|1.0||")
  }

  /** Ensure model builder for variant dependencies respects the parameter to disable the resolving of the runtime classpath. */
  @Test
  fun testBuildCompileClasspathOnly() {
    val (graphs, libraries) =
      buildModelGraph(true) {
        module("foo", "bar", "1.0") {
          file = File("path/to/bar-1.0.jar")
          setupDefaultCapability()
          attribute(BuildTypeAttr.ATTRIBUTE, objectFactory.named(BuildTypeAttr::class.java, "debug"))
    ...
    Truth.assertThat(graphs.runtimeDependencies).isNull()
    Truth.assertThat(graphs.compileDependencies.map { it.key }).containsExactly("foo|bar|1.0|debug||foo:bar:1.0")
  }
}
...
private fun getResolutionResultProvider(compileResultsResults: Set<DependencyResult>): ResolutionResultProvider {
...
private class ResolutionResultProviderImpl(
  ...
  override fun getResolutionResult(configType: AndroidArtifacts.ConsumedConfigType): ResolutionResult =
    when (configType) {
