...
class MergeNativeLibsTaskTest {
  ...
  abstract class TestMergeNativeLibsTask @Inject constructor(testWorkerExecutor: WorkerExecutor) : MergeNativeLibsTask() {
  ...
  fun setUp() {
    ...
    projectNativeLibs = unfilteredProjectNativeLibs.flatMap { dir -> dir.walk().filter { it.extension == "so" } }
  ...
  fun testBasic() {
    ...
    abis.forEach { abi ->
      ...
      assertThat(outputDir.resolve("lib/$abi/projectNativeLib1.so")).hasContents("projectNativeLib1")
      assertThat(outputDir.resolve("lib/$abi/projectNativeLib2.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/projectNativeLib2.so")).hasContents("projectNativeLib2")
      assertThat(outputDir.resolve("lib/$abi/subProjectNativeLib1.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/subProjectNativeLib1.so")).hasContents("subProjectNativeLib1")
      assertThat(outputDir.resolve("lib/$abi/subProjectNativeLib2.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/subProjectNativeLib2.so")).hasContents("subProjectNativeLib2")
      assertThat(outputDir.resolve("lib/$abi/externalLibNativeLib1.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/externalLibNativeLib1.so")).hasContents("externalLibNativeLib1")
      assertThat(outputDir.resolve("lib/$abi/externalLibNativeLib2.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/externalLibNativeLib2.so")).hasContents("externalLibNativeLib2")
      assertThat(outputDir.resolve("lib/$abi/profilerNativeLib1.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/profilerNativeLib1.so")).hasContents("profilerNativeLib1")
      assertThat(outputDir.resolve("lib/$abi/profilerNativeLib2.so")).exists()
      assertThat(outputDir.resolve("lib/$abi/profilerNativeLib2.so")).hasContents("profilerNativeLib2")
      ...
      assertThat(outputDir.resolve("lib/$abi/pickFirst.so")).hasContents("subProjectPickFirst")

      assertThat(testOnlyDir.resolve("lib/$abi/projectTestOnly.so")).exists()
      assertThat(testOnlyDir.resolve("lib/$abi/projectTestOnly.so")).hasContents("projectTestOnly")
      assertThat(testOnlyDir.resolve("lib/$abi/subProjectTestOnly.so")).exists()
      assertThat(testOnlyDir.resolve("lib/$abi/subProjectTestOnly.so")).hasContents("subProjectTestOnly")
  ...
  fun testErrorWhenDuplicateNativeLibsInDependencies() {
    abis.forEach { abi ->
      subProjectNativeLibs[0].resolve("$abi/duplicate.so").writeText("subProjectDuplicate1")
      subProjectNativeLibs[1].resolve("$abi/duplicate.so").writeText("subProjectDuplicate2")
      externalLibNativeLibs[0].resolve("$abi/duplicate.so").writeText("externalLibDuplicate1")
      externalLibNativeLibs[1].resolve("$abi/duplicate.so").writeText("externalLibDuplicate2")
      profilerNativeLibs.resolve("$abi/duplicate.so").writeText("profilerDuplicate")
    }
  ...
  fun testDuplicateNativeLibInProject() {
    abis.forEach { abi ->
      val duplicateProjectNativeLib = unfilteredProjectNativeLibs[0].resolve("$abi/duplicate.so").also { it.writeText("projectDuplicate") }
