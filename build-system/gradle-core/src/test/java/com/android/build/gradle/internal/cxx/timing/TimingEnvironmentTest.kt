...
class TimingEnvironmentTest {
  ...
  private fun replaceTimes(value: String): String {
    return Regex("[0-9]+ms").replace(value, "{XX}ms").trim()
  }

  @Test
  fun simple() {
    val log = timeTest { time += 50 }

    assertThat(replaceTimes(log))
      .isEqualTo(
        """
  ...
  fun `one level nested`() {
    val log = timeTest { time("nest-level-1") { time += 50 } }
    assertThat(replaceTimes(log))
      .isEqualTo(
        """
        # C/C++ build system timings
  ...
  fun `two level nested`() {
    val log = timeTest { time("nest-level-1") { time("nest-level-2") { time += 50 } } }
    assertThat(replaceTimes(log))
      .isEqualTo(
        """
        # C/C++ build system timings
  ...
  fun `repro case where time inside nested scope was attributed to outer`() {
    val log = timeTest { time("scope-1") { time("scope-2") { time += 50 } } }
    assertThat(replaceTimes(log))
      .isEqualTo(
        """
        # C/C++ build system timings
