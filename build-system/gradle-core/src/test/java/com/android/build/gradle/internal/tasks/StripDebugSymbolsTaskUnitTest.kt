...
class StripDebugSymbolsTaskUnitTest {
  ...
  fun setUp() {
        ...
        listOf(
          FakeInjectableService(
            ...
            FakeGradleWorkExecutor(
              FakeObjectFactory.factory,
              temporaryFolder.newFolder(),
              listOf(FakeInjectableService(StripDebugSymbolsRunnable::execOperations.getter.javaMethod!!, execOperations)),
            ),
          )
        ),
      )
    ...
    stripToolFinderProvider =
      FakeProviderFactory.factory.provider {
        SymbolStripExecutableFinder(mapOf(Pair(Abi.X86.tag, fakeExe), Pair(Abi.ARMEABI.tag, fakeExe)))
      }
  }
  ...
  fun `test non-incremental`() {
    ...
    workers.noIsolation().submit(StripDebugSymbolsDelegate::class.java) {
    ...
    for (processInfo in execOperations.capturedExecutions) {
      ...
      if (processInfo.args.contains(x86Foo.toString())) {
        ...
        assertThat(processInfo.args).containsExactly("--strip-unneeded", "-o", outputFile.toString(), x86Foo.toString())
      } else {
        val path = armeabiFoo.toRelativeString(inputDir)
        val outputFile = File(outputDir, path)
        assertThat(processInfo.args).containsExactly("--strip-unneeded", "-o", outputFile.toString(), armeabiFoo.toString())
      }
    }
  ...
  fun `test incremental`() {
    ...
    assertThat(execSpec.args).containsExactly("--strip-unneeded", "-o", x86FooOutputFile.toString(), x86Foo.toString())
