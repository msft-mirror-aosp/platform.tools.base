...
internal class AnalyticsEnabledApplicationVariantBuilderTest {
  ...
  @Suppress("DEPRECATION") private val androidTest: com.android.build.api.variant.AndroidTestBuilder = mock()
  ...
  private val proxy: AnalyticsEnabledApplicationVariantBuilder by lazy { AnalyticsEnabledApplicationVariantBuilder(delegate, stats) }

  @Before
  fun setup() {
    @Suppress("DEPRECATION") whenever(delegate.androidTest).thenReturn(androidTest)
  }
  ...
  fun testFixtures() {
    ...
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.TEST_FIXTURES_ENABLED_VALUE)
  }
  ...
  fun testProfileableWriteOnly() {
    ...
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.PROFILEABLE_ENABLED_VALUE)
    val exception =
      Assert.assertThrows(PropertyAccessNotAllowedException::class.java) {
  ...
  fun testAndroidResources() {
    ...
    Truth.assertThat(proxiedAndroidResources).isInstanceOf(AnalyticsEnabledApplicationAndroidResourcesBuilder::class.java)
    Truth.assertThat((proxiedAndroidResources as AnalyticsEnabledApplicationAndroidResourcesBuilder).delegate).isEqualTo(androidResources)

    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.ANDROID_RESOURCES_BUILDER_VALUE)
  }
  ...
  fun getDeviceTests() {
    ...
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.DEVICE_TESTS_BUILDER_VALUE)
  ...
  fun getDeviceTests_for_android_test() {
    ...
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.DEVICE_TESTS_BUILDER_VALUE)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.last().type).isEqualTo(VariantMethodType.ANDROID_TEST_BUILDER_VALUE)
    verify(delegate, times(1)).deviceTests
  ...
  @Test
  fun testAndroidTest() {
    @Suppress("DEPRECATION") Truth.assertThat(proxy.androidTest).isInstanceOf(AndroidTestBuilder::class.java)
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.ANDROID_TEST_BUILDER_VALUE)
  }
  ...
  fun testHostTests() {
    ...
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.HOST_TESTS_BUILDER_VALUE)
  }
}
