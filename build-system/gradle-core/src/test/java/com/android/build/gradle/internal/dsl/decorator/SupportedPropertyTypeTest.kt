...
class SupportedPropertyTypeTest {
  ...
  fun checkValTypes() {
    ...
    objects.forEach { propertyType ->
      ...
      assertWithMessage("Bridge types should be distinct %s", propertyType).that(propertyType.bridgeTypes).containsNoDuplicates()
      ...
      propertyType.bridgeTypes.forEach { bridgeType ->
        assertWithMessage("Bridge types should be distinct from property type %s", propertyType)
          .that(bridgeType)
          .isNotEqualTo(propertyType.type)
      ...
      val constructor = implementationType.getDeclaredConstructor(String::class.java)
      assertWithMessage("Constructor is annotated with @Inject").that(constructor.getAnnotation(Inject::class.java)).isNotNull()
  ...
  fun checkAgpBlockTypes() {
    AGP_SUPPORTED_PROPERTY_TYPES.filterIsInstance<SupportedPropertyType.Block>().forEach { propertyType ->
      val type = load(propertyType.type)
      val implementationType = load(propertyType.implementationType)
      assertWithMessage("Implementation type must be a subtype of property type %s", propertyType)
        .that(implementationType)
