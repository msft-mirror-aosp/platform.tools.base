...
abstract class AbstractSingleArtifactTest<T : FileSystemLocation>(
  ...
  private fun allocateNamingContext() = ArtifactNamingContext(project.objects.property(String::class.java), null, null)
  ...
  fun testChangesDisallowed() {
    ...
    finalProducer.configure { it.getOutputFile().set(outputFile) }
    ...
    otherProducer.configure { it.getOutputFile().set(outputFile2) }
  ...
  fun testAgpProducer() {
    ...
    val consumerTaskProvider = project.tasks.register("consumerTask", ConsumerTask::class.java) as TaskProvider<ConsumerTask<T>>

    consumerTaskProvider.configure { it.getInputFile().set(artifact.get()) }
    ...
    otherProducer.configure { it.getOutputFile().set(outputFile2) }
    artifact.initArtifactContainer(otherProducer, otherProducer.flatMap { it.getOutputFile() }, allocateNamingContext())
  ...
  abstract class ProducerTask<T : FileSystemLocation> : DefaultTask() {
    @OutputFile abstract fun getOutputFile(): Property<T>
  }
}
