...
class OsBehaviorTests {
  ...
  @Test
  fun `which on Linux`() {
    val setup = setupFolders(platform = PLATFORM_LINUX, "folder1/my-exe", "folder2/my-exe")
    assertThat(setup.which("my-exe")).isEqualTo("folder1/my-exe")
  }

  @Test
  fun `which on Linux finds on second path`() {
    val setup = setupFolders(platform = PLATFORM_LINUX, "folder1/my-other-exe", "folder2/my-exe")
    assertThat(setup.which("my-exe")).isEqualTo("folder2/my-exe")
  }

  @Test
  fun `which on Windows with no extension in paths`() {
    val setup = setupFolders(platform = PLATFORM_WINDOWS, "folder1/my-other-exe", "folder2/my-exe")
    assertThat(setup.which("my-exe")).isNull()
  }

  @Test
  fun `which on Windows finds BAT`() {
    val setup = setupFolders(platform = PLATFORM_WINDOWS, "folder1/my-exe.BAT", "folder2/my-exe.BAT")
    assertThat(setup.which("my-exe")).isEqualTo("folder1/my-exe.BAT")
  }

  @Test
  fun `which on Windows finds on second path`() {
    val setup = setupFolders(platform = PLATFORM_WINDOWS, "folder1/my-exe.EXE", "folder2/my-exe.EXE")
  ...
  fun `which on Windows finds BAT in second path`() {
    val setup = setupFolders(platform = PLATFORM_WINDOWS, "folder1/my-other-exe.BAT", "folder2/my-exe.BAT")
    assertThat(setup.which("my-exe.BAT")).isEqualTo("folder2/my-exe.BAT")
  }

  private data class TestFolders(val os: OsBehavior, val base: File) {
  ...
  private fun setupFolders(platform: Int, vararg files: String): TestFolders {
    ...
    val os = createOsBehavior(platform = platform, environmentPaths = paths)
    return TestFolders(os = os, base = base)
  }
}
