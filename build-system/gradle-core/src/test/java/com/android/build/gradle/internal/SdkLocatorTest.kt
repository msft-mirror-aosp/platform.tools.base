...
class SdkLocatorTest {
  ...
  fun propertiesSetToEmptyString() {
    val sourceSet =
      getLocationSourceSet(localSdkDir = "", localAndroidDir = "", envAndroidHome = "", envSdkRoot = "", systemAndroidHome = "")
    val sdkLocation = SdkLocator.getSdkLocation(sourceSet, issueReporter)
    ...
    assertThat(issueReporter.messages[0])
      .contains("Where: Fallback android.dir property in local.properties file. Problem: Set with empty value")
  ...
  fun selectionPreference_sdkDirOverAndroidDir() {
    val sourceSet =
      getLocationSourceSet(localSdkDir = validSdkDirectory.absolutePath, localAndroidDir = anotherValidSdkDirectory.absolutePath)
    val sdkLocation = SdkLocator.getSdkLocation(sourceSet, issueReporter)
    assertThat(issueReporter.messages).isEmpty()
    assertThat(sdkLocation.directory).isEqualTo(validSdkDirectory)
    assertThat(sdkLocation.type).isEquivalentAccordingToCompareTo(SdkType.REGULAR)
  ...
  fun selectionPreference_androidDirOverBadSdkDir() {
    val sourceSet = getLocationSourceSet(localSdkDir = missingSdkDirectory.absolutePath, localAndroidDir = validSdkDirectory.absolutePath)
    val sdkLocation = SdkLocator.getSdkLocation(sourceSet, issueReporter)
    assertThat(issueReporter.messages).hasSize(1)
    assertThat(issueReporter.messages[0]).contains("Where: sdk.dir property in local.properties file. Problem: Directory does not exist")
    assertThat(sdkLocation.directory).isEqualTo(validSdkDirectory)
    assertThat(sdkLocation.type).isEquivalentAccordingToCompareTo(SdkType.PLATFORM)
  }
  ...
  fun selectionPreference_sdkRootOverBadAndroidHome() {
    val sourceSet = getLocationSourceSet(envAndroidHome = missingSdkDirectory.absolutePath, envSdkRoot = validSdkDirectory.absolutePath)
    val sdkLocation = SdkLocator.getSdkLocation(sourceSet, issueReporter)
  ...
  fun conflict_androidHomeAndSdkRoot() {
    val sourceSet =
      getLocationSourceSet(envAndroidHome = validSdkDirectory.absolutePath, envSdkRoot = anotherValidSdkDirectory.absolutePath)
  ...
  private fun writeLocalPropertiesFile(properties: Properties) {
    ...
    OutputStreamWriter(FileOutputStream(localProperties, false), Charsets.UTF_8).use { properties.store(it, "Generated by SdkLocatorTest") }
  }
}
