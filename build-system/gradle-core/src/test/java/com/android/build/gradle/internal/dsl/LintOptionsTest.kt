...
/**
 * Test for the semantic of the legacy [com.android.build.api.dsl.LintOptions] block, which is now backed by the new
 * [com.android.build.api.dsl.Lint].
 *
 * This looks similar to `LintImplTest`, but as they have some differences and can evolve separately they are tested separately
 */
class LintOptionsTest {
  ...
  private val severityOverrides: Map<String, LintModelSeverity>
    get() = (lintWrapper.lintOptions as LintOptions).severityOverridesMap

  private val dslServices: DslServices = createDslServices()

  @Suppress("DEPRECATION") private fun lintOptions(action: com.android.build.api.dsl.LintOptions.() -> Unit) = action.invoke(lintOptions)

  private val lintOptions
    get() = lintWrapper.lintOptions
  ...
  fun testSeverityOverridesMutuallyExclusive() {
    ...
    assertThat(severityOverrides).named("severityOverrides").containsExactly("MyCheck", DEFAULT_ENABLED)

    lintOptions { disable += "MyCheck" }
    ...
    assertThat(severityOverrides).named("severityOverrides").containsExactly("MyCheck", IGNORE)
  }
  ...
  fun testLocking() {
    ...
    val failure = assertFailsWith<RuntimeException> { lintOptions { disable += "MyCheck" } }
  ...
  fun testGroovyHelpers() {
    ...
    assertThat(lintOptions.disable).named("lint.disable").containsExactly("Disable", "Disable2", "Disable3", "Ignore", "Ignore2", "Ignore3")
    assertThat(lintOptions.enable).named("lint.enable").containsExactly("Enable", "Enable2", "Enable3")
    assertThat(lintOptions.informational).named("lint.informational").containsExactly("Informational", "Informational2", "Informational3")
    assertThat(lintOptions.warning).named("lint.warning").containsExactly("Warning", "Warning2", "Warning3")
  ...
  fun testGroovyAppend() {
    ...
    assertThat(severityOverrides)
      ...
      .containsExactlyEntriesIn(mapOf("Ignore" to IGNORE, "Disable" to IGNORE, "Enable" to DEFAULT_ENABLED))
  }
  ...
  @Test
  fun testHtmlOutput() {
    lintOptions { htmlOutput = File("stdout") }
  ...
  @Test
  fun testSarifOutput() {
    lintOptions { sarifOutput = File("stdout") }
  ...
  @Test
  fun testXmlOutput() {
    lintOptions { xmlOutput = File("stdout") }
  ...
  fun testBaselineFileSetter() {
    ...
    assertThat(lintOptions.baselineFile?.path).named("lint.baselineFile").isEqualTo("lint_baseline.xml")
  }
  ...
  fun testBaselineFileHelper() {
    ...
    assertThat(lintOptions.baselineFile?.path).named("lint.baselineFile").isEqualTo("lint_baseline2.xml")
    Eval.me(
      "android",
      lintWrapper,
      """
    ...
    assertThat(lintOptions.baselineFile?.path).named("lint.baselineFile").isEqualTo("lint_baseline3.xml")
  }

  @Test
  fun testLintConfig() {
    lintOptions { lintConfig = File("lint_config.xml") }
    assertThat(lintOptions.lintConfig?.path).named("lint.lintConfig").isEqualTo("lint_config.xml")
  }
  ...
  companion object {
    ...
    val com.android.build.api.dsl.LintOptions.warning
      get() = (this as LintOptions).severityOverridesMap.filterValues { it == WARNING }.keys

    val com.android.build.api.dsl.LintOptions.error
      get() = (this as LintOptions).severityOverridesMap.filterValues { it == ERROR }.keys

    val com.android.build.api.dsl.LintOptions.fatal
      get() = (this as LintOptions).severityOverridesMap.filterValues { it == FATAL }.keys
  }
}
