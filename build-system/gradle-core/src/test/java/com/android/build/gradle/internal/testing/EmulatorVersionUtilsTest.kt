...
class EmulatorVersionUtilsTest {
  ...
  fun testEmulatorVersionMetadata() {
    ...
    assertThrows(RuntimeException::class.java) { metadata = getEmulatorMetadata(emulatorDir) }
      .also { assertThat(it).hasMessageThat().contains("Emulator needs to be updated in order to use managed devices.") }

    packageFile.writeText("")
    assertThrows(Exception::class.java) { metadata = getEmulatorMetadata(emulatorDir) }
      .also { assertThat(it).hasMessageThat().contains("Could not determine version of Emulator") }
  }
}
