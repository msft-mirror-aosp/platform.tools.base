...
: NamedDomainObjectContainer<T> {
  ...
  override fun <S : T> named(p0: String, p1: Class<S>, p2: Action<in S>): NamedDomainObjectProvider<S> {
    TODO("not implemented") // To change body of created functions use File | Settings | File Templates.
  }
  ...
  private fun createAndAddItem(name: String): T {
    ...
    whenAddedActions.forEach { it.execute(item) }
  ...
  private fun removeItemFromMap(item: T) {
    ...
    whenRemovedActions.forEach { it.execute(item) }
  }
