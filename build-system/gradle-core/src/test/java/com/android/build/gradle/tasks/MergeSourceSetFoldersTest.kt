...
class MergeSourceSetFoldersTest {
  ...
  fun testMergingAssetSets() {
    ...
    assertThat(result[0].sourceFiles).containsExactly(file1, file2, file3)
    assertThat(result[1].configName).isEqualTo(debugSet.configName)
  ...
  fun everything() {
    ...
    // the opposite order.
    val librarySets = setupLibraryDependencies(libFile, "foo:bar:1.0", libFile2, "foo:bar:2.0")
  ...
  private fun createAssetSet(name: String, vararg files: File): AssetSet {
