...
internal class AbstractSourceDirectoriesImplTest {
  ...
  fun testAddSrcDir() {
    ...
    testTarget.addStaticSourceDirectory(addedSource.absolutePath)

    Truth.assertThat(listOfSources.size).isEqualTo(1)
    val listProperty = project.objects.listProperty(Directory::class.java)
    listOfSources.single().addTo(project.layout.projectDirectory, listProperty)
    Truth.assertThat(listProperty.get().single().asFile.absolutePath).isEqualTo(addedSource.absolutePath)

    Truth.assertThat(listOfStaticSources.size).isEqualTo(1)
    listProperty.empty()
    listOfStaticSources.single().addTo(project.layout.projectDirectory, listProperty)
    Truth.assertThat(listProperty.get().single().asFile.absolutePath).isEqualTo(addedSource.absolutePath)
  }
  ...
  private fun createTestTarget(patternFilterable: PatternFilterable? = null): SourceDirectoriesImpl {
    ...
    return object : SourceDirectoriesImpl("_for_test", variantServices, patternFilterable) {
      ...
      override fun variantSourcesForModel(filter: (DirectoryEntry) -> Boolean): List<File> = emptyList()
