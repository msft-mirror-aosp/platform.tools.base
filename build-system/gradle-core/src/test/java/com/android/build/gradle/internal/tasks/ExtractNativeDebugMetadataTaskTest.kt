...
class ExtractNativeDebugMetadataTaskTest {
  ...
  fun setUp() {
    ...
    workers =
      FakeGradleWorkExecutor(
        ...
        listOf(FakeInjectableService(ExtractNativeDebugMetadataRunnable::execOperations.getter.javaMethod!!, fakeExecOperations)),
      )
  }
    ...
    object : ExtractNativeDebugMetadataWorkAction() {
        ...
        override fun getParameters(): Parameters {
          return object : Parameters() {
            override val inputDir = objectFactory.directoryProperty().fileValue(this@ExtractNativeDebugMetadataTaskTest.inputDir)
            override val strippedNativeLibs =
              objectFactory.directoryProperty().fileValue(this@ExtractNativeDebugMetadataTaskTest.strippedNativelibs)
            override val outputDir = objectFactory.directoryProperty().fileValue(this@ExtractNativeDebugMetadataTaskTest.outputDir)
            override val objcopyExecutableMap =
              objectFactory
                .mapProperty(String::class.java, File::class.java)
            ...
            override val analyticsService: Property<AnalyticsService>
              get() = project.objects.property(AnalyticsService::class.java).also { it.set(FakeNoOpAnalyticsService()) }
          }
        }
      }
      .execute()

    assertThat(fakeExecOperations.capturedExecutions).named("number of process invocations").hasSize(2)
    for (processInfo in fakeExecOperations.capturedExecutions) {
      assertThat(processInfo.executable).isEqualTo(fakeExe.absolutePath)
      val nativeLib =
        if (processInfo.args.contains(x86NativeLib.toString())) {
          x86NativeLib
        } else {
          armeabiNativeLib
        }
      ...
      assertThat(processInfo.args).containsExactly("--only-keep-debug", nativeLib.toString(), outputFile.toString())
  ...
  fun symbolTableTest() {
          ...
          return object : Parameters() {
            ...
            override val objcopyExecutableMap =
              objectFactory
                .mapProperty(String::class.java, File::class.java)
                .value(this@ExtractNativeDebugMetadataTaskTest.objcopyExecutableMap)
            override val debugSymbolLevel = FakeGradleProperty(DebugSymbolLevel.SYMBOL_TABLE)
            override val maxWorkerCount = FakeGradleProperty(2)
            override val projectPath = factory.property(String::class.java).value("projectName")
            override val taskOwner = FakeGradleProperty("taskOwner")
            override val workerKey = FakeGradleProperty("workerKey")
    ...
    for (processInfo in fakeExecOperations.capturedExecutions) {
      ...
      assertThat(processInfo.args).containsExactly("--strip-debug", nativeLib.toString(), outputFile.toString())
