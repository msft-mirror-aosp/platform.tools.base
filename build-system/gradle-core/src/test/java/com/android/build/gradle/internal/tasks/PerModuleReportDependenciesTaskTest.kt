...
class PerModuleReportDependenciesTaskTest {
  ...
  fun testReportGenerationWithRepositoriesIncluded() {
    ...
    PerModuleReportDependenciesTask.CreationAction.run { task.projectRepositories.set(project.repositories.toInternalRepoMetadataList()) }
  ...
  fun generateReportWithLargeFileInvolved() {
    val largeFile =
      File(temporaryFolder.newFolder("demo"), "large.file").also {
        it.bufferedWriter().use { out ->
          val kilobyte = "0".repeat(1000)
          repeat(2_200_000) { out.write(kilobyte) }
        }
      }
    assertThat(largeFile.length()).isGreaterThan(Int.MAX_VALUE.toLong())
    task.runtimeClasspathArtifacts.set(
      FakeArtifactCollection(mutableSetOf(FakeResolvedArtifactResult(largeFile, FakeComponentIdentifier("Large file"))))
    )
    val rootComponent = createProjectComponent("root_module")
    val stubbedGraph = FakeResolutionResult(rootComponent)
    task.getRootComponent().set(stubbedGraph.root)
...
private class FakeResolvedComponentResultInternal(thingy: ResolvedComponentResult, private val repositoryName: String) :
  ...
  @Deprecated("This property is deprecated starting with Gradle 8.2") override fun getRepositoryName(): String = repositoryName
