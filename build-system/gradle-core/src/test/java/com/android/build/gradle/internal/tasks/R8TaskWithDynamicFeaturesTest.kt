...
/** Testing legacy multidex and monodex for [R8Task] with dynamic features. Similar to [R8MainDexRulesTest], but with dynamic features. */
class R8TaskWithDynamicFeaturesTest {
  ...
  fun setUp() {
    ...
    TestInputsGenerator.pathWithClasses(classesJar, listOf(Animal::class.java, CarbonForm::class.java, Toy::class.java))
    feature1ClassesJar = tmp.root.toPath().resolve("feature1.jar")
    TestInputsGenerator.pathWithClasses(feature1ClassesJar, listOf(Cat::class.java))
    ...
    TestInputsGenerator.pathWithClasses(feature2ClassesJar, listOf(Dog::class.java))
  ...
  fun testMainDexRules() {
    ...
    runR8(
      ...
      featureJavaResourceJars = listOf(feature1JavaResJar.toFile(), feature2JavaResJar.toFile()),
    ...
    assertThat(feature1Dex).containsExactlyClassesIn(listOf(Type.getDescriptor(Cat::class.java)))

    val feature2Dex = Dex(featureDexDir.resolve("feature2").resolve("classes.dex"))
    assertThat(feature2Dex).containsExactlyClassesIn(listOf(Type.getDescriptor(Dog::class.java)))
    ...
    ZipArchive(feature1JavaResOutput.toPath()).use { Truth.assertThat(it.listEntries()).containsExactly("foo.txt") }
    val feature2JavaResOutput = featureJavaResOutputDir.resolve("feature2.jar")
    assertThat(feature2JavaResOutput).exists()
    ZipArchive(feature2JavaResOutput.toPath()).use { Truth.assertThat(it.listEntries()).isEmpty() }
  }
