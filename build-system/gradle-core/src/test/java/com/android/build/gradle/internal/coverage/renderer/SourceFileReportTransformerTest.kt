...
class SourceFileReportTransformerTest {
  ...
  fun `createSourceFileReport with non-existent file throws exception`() {
    ...
    val builder =
      SourceFileReportBuilder("non/existent/File.kt").apply {
        variantFileCoverageBuilders["debug"] =
          VariantFileCoverageBuilder().apply {
            testSuiteFileCoverageBuilders["unitTest"] = TestSuiteFileCoverageBuilder(fileCoverage = CoverageInfo(50, 5, 10))
          }
      }
  ...
  fun `createSourceFileReport aggregates full report correctly`() {
    ...
    val builder =
      SourceFileReportBuilder(packageFlattenedPath).apply {
        variantFileCoverageBuilders["debug"] =
          VariantFileCoverageBuilder().apply {
            testSuiteFileCoverageBuilders["unitTest"] =
              TestSuiteFileCoverageBuilder(
                fileCoverage = CoverageInfo(66, 10, 15),
                lineCoverageBuilders =
                  mutableMapOf(
                    4 to
                      LineCoverageBuilder(instructionCoverageInfo = CoverageInfo(100, 10, 10), branchCoverageInfo = CoverageInfo(0, 0, 0))
                  ),
              )
            testSuiteFileCoverageBuilders["connectedTest"] =
              TestSuiteFileCoverageBuilder(
                fileCoverage = CoverageInfo(65, 13, 20),
                lineCoverageBuilders =
                  mutableMapOf(
                    4 to LineCoverageBuilder(instructionCoverageInfo = CoverageInfo(100, 8, 8), branchCoverageInfo = CoverageInfo(0, 0, 0)),
                    5 to LineCoverageBuilder(instructionCoverageInfo = CoverageInfo(100, 5, 5), branchCoverageInfo = CoverageInfo(0, 0, 0)),
                  ),
              )
          }
            ...
            testSuiteFileCoverageBuilders["unitTest"] =
              TestSuiteFileCoverageBuilder(
                ...
                lineCoverageBuilders =
                  mutableMapOf(
                    4 to
                      LineCoverageBuilder(instructionCoverageInfo = CoverageInfo(100, 12, 12), branchCoverageInfo = CoverageInfo(0, 0, 0))
                  ),
              )
          }
      }
    ...
    assertThat(debugSummary.testSuiteCoverages.find { it.testSuiteName == "connectedTest" }!!.variantCoverage.instruction.covered)
      .isEqualTo(13)
    ...
    assertThat(line4DebugGroup.testSuiteCoverages.find { it.testSuiteName == "unitTest" }!!.variantCoverage.instruction.covered)
      .isEqualTo(10)
    assertThat(line4DebugGroup.testSuiteCoverages.find { it.testSuiteName == "connectedTest" }!!.variantCoverage.instruction.covered)
      .isEqualTo(8)
