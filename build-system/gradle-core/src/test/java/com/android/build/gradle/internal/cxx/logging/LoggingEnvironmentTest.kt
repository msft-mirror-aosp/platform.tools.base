...
class LoggingEnvironmentTest {
  ...
  fun testEnvironmentIsPerThread() {
    ...
    thread1.join()
    thread2.join()
    assertThat(thread1Messages)
      .containsExactly(
        "error thread 1: ${RESERVED_FOR_TESTS.codeText} error",
        "warn thread 1: C/C++: warn",
        "lifecycle thread 1: C/C++: lifecycle",
        "info thread 1: C/C++: info",
      )
    assertThat(thread2Messages)
      .containsExactly(
        "error thread 2: ${RESERVED_FOR_TESTS.codeText} error",
        "warn thread 2: C/C++: warn",
        "lifecycle thread 2: C/C++: lifecycle",
        "info thread 2: C/C++: info",
      )
  }
  ...
  fun testEnvironmentsNest() {
    TestLoggingEnvironment("nest 1").use { outer ->
      ...
      errorln(RESERVED_FOR_TESTS, "error")
      assertThat(outer.messages)
        .containsExactly("error nest 1: ${RESERVED_FOR_TESTS.codeText} error", "error nest 1: ${RESERVED_FOR_TESTS.codeText} error")
    }
  }

  @Test
  fun `131271062 percent in format with no args`() {
    TestLoggingEnvironment("nest 1").use { errorln(RESERVED_FOR_TESTS, "error %F") }
  }

  @Test
  fun `131271062 percent in format with args`() {
    TestLoggingEnvironment("nest 1").use { errorln(RESERVED_FOR_TESTS, "error %F", "arg") }
  }
}
