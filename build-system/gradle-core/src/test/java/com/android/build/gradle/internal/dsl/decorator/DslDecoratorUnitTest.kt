...
class DslDecoratorUnitTest {
  ...
  fun `check constructors with arguments work correctly`() {
    ...
    assertWithMessage("Constructor values should be passed through").that(o.integer).isEqualTo(1)
    assertWithMessage("Constructor values should be passed through").that(o.boxed).isEqualTo(2)
    assertWithMessage("Constructor values should be passed through").that(o.dslServices).isSameInstanceAs(dslServices)
  }
  ...
  fun `check only annotated and zero-arg constructors are generated`() {
    ...
    assertFailsWith(NoSuchMethodException::class) { decorated.getIntConstructor() }
  }
  ...
  fun `check locking works for managed var properties`() {
    ...
    o.managedString = "a"
    (o as Lockable).lock()
    val failure = assertFailsWith<AgpDslLockedException> { o.managedString = "b" }
  ...
  fun `check managed integer properties`() {
    ...
    (o as Lockable).lock()
    val exception = assertFailsWith(AgpDslLockedException::class) { o.managedInteger = 6 }
  ...
  fun `check managed nullable integer properties`() {
    ...
    (o as Lockable).lock()
    val exception = assertFailsWith(AgpDslLockedException::class) { o.managedNullableInteger = 6 }
    assertThat(exception)
      .hasMessageThat()
      .isEqualTo(
        """
  ...
  abstract class LeaveAbstract {
    @Suppress("unused") abstract val forGradle: Property<String>
  }
  ...
  fun `check protected field is implemented`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Var.String)).decorate(WithProtectedField::class)
    val o = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    assertThat(o).isNotNull()
  ...
  fun `check protected field in superclass is implemented`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Var.String)).decorate(Subclass::class)
    val o = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    assertThat(o).isNotNull()
    assertThat(o.compileSdk).isNull()
    o.compileSdk = "a"
  ...
  fun `check handling of public override of a protected field`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Var.String)).decorate(PublicFieldOverridesProtectedField::class)
    val o = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    assertThat(o).isNotNull()
  ...
  fun `check locking propagation`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Collection.List)).decorate(LockableWithList::class)
    val o = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    o.foo += "one"
    o.lock()
    assertThat(o.foo).containsExactly("one")
    val exception = assertFailsWith(AgpDslLockedException::class) { o.foo += "two" }
    assertThat(exception)
      .hasMessageThat()
      .isEqualTo(
        """
  ...
  fun `check doesn't override concrete implementations`() {
    ...
    val decoratedInterface =
      dslDecorator.decorate(WithoutConcreteImplementation::class).getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    assertThat(decoratedInterface.foo).isNull()
    val decorated =
      dslDecorator.decorate(ConcreteImplementation::class).getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
  ...
  fun `check groovy setter generation`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Collection.List)).decorate(WithList::class)
  ...
  fun `check groovy setter generation for set`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Collection.Set)).decorate(WithSet::class)
  ...
  fun `check groovy setter generation for map`() {
    val decorated = DslDecorator(listOf(SupportedPropertyType.Collection.Map)).decorate(WithMap::class)
  ...
  fun `check green-field sub-block instantiation`() {
    ...
    withSubBlock.subBlock { string = "two" }
    ...
    decorated.getDeclaredMethod("subBlock", Action::class.java).invoke(withSubBlock, action)
  ...
  fun `check existing sub-block instantiation`() {
    val subBlockPropertyType = SupportedPropertyType.Block(type = SubBlock::class.java, implementationType = SubBlockImpl::class.java)
    val decorator = DslDecorator(listOf(subBlockPropertyType, SupportedPropertyType.Var.String))
    registerTestDecorator(decorator)
  ...
  fun `check locking works for subBlocks`() {
    ...
    val failure = assertFailsWith<AgpDslLockedException> { withSubBlock.subBlock.string = "b" }
  ...
  abstract class WithNonNullableValue {
    @set:NonNullableSetter abstract var value: String?
  ...
  fun `check non nullable setter annotation`() {
    val withNonNullableValue =
      DslDecorator(listOf(SupportedPropertyType.Var.String))
  ...
  private fun registerTestDecorator(decorator: DslDecorator) {
    whenever(dslServices.newDecoratedInstance(any<Class<*>>(), any<DslServices>())).then { invocation ->
  ...
  abstract class WithCustomImpl() {
    ...
    var string: String?
      get() = _string
      set(value) {
        _string = value?.removeSurrounding("\"")
      }
  }
  ...
  fun `check error message for underlying managed properties`() {
    ...
    (o as Lockable).lock()
    val failure = assertFailsWith<AgpDslLockedException> { o.string = "b" }
    assertThat(failure).hasMessageThat().contains("It is too late to set string")
    assertThat(o.string).isEqualTo("a")

    val failure2 = assertFailsWith<AgpDslLockedException> { o.list += "b" }
  ...
  abstract class WithRuntimeGuardedExperimentalApi() {
    ...
    @get:RuntimeGuardedExperimentalApi(BooleanOption.ENABLE_IMPROVED_DEPENDENCY_RESOLUTION) abstract val experimentalBlock: SubBlock

    abstract fun experimentalBlock(action: SubBlock.() -> Unit)

    @get:RuntimeGuardedExperimentalApi(BooleanOption.ENABLE_IMPROVED_DEPENDENCY_RESOLUTION) abstract var experimentalValue: String
  ...
  fun checkWithRuntimeGuardedExperimentalApi() {
    val subBlockPropertyType = SupportedPropertyType.Block(type = SubBlock::class.java, implementationType = SubBlockImpl::class.java)
    val decorator = DslDecorator(listOf(subBlockPropertyType, SupportedPropertyType.Var.String, SupportedPropertyType.Collection.List))
    registerTestDecorator(decorator)
    val decorated = decorator.decorate(WithRuntimeGuardedExperimentalApi::class)

    val gradleProperties = mutableMapOf<BooleanOption, Boolean>(BooleanOption.ENABLE_IMPROVED_DEPENDENCY_RESOLUTION to true)
    whenever(dslServices.projectOptions).thenAnswer { invocation ->
      ProjectOptions(FakeProviderFactory(FakeProviderFactory.factory, gradleProperties.mapKeys { it.key.propertyName }))
    }

    val o1 = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
    ...
    o1.experimentalBlock { string = "example3" }
    ...
    val o2 = decorated.getDeclaredConstructor(DslServices::class.java).newInstance(dslServices)
  ...
  abstract class WithDifferentExperimentalGuards() {
    ...
    @get:RuntimeGuardedExperimentalApi(BooleanOption.ENABLE_NEW_RESOURCE_PROCESSING) abstract var experimentalNewResourceProcessing: String
  ...
  fun checkDifferentGuards() {
    ...
    val gradleProperties =
      mutableMapOf<BooleanOption, Boolean>(
    ...
    assertThat(failure)
      .hasMessageThat()
      .isEqualTo(
        "API experimentalImprovedResolution is experimental, and requires opt in, as it may change or break in future versions without notice.\n" +
