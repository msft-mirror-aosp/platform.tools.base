...
class ManagedDeviceInstrumentationTestTaskTest {
  ...
  private val creationConfig: TestVariantImpl =
    mock(defaultAnswer = RETURNS_DEEP_STUBS, extraInterfaces = arrayOf(DeviceTestCreationConfig::class))
  ...
  fun setup() {
    ...
    whenever(creationConfig.services.buildServiceRegistry.registrations.getByName(any())).thenReturn(mockGeneralRegistration)
  ...
  private fun mockFileProperty(file: RegularFile): RegularFileProperty =
    mock<RegularFileProperty>().apply { whenever(get()).thenReturn(file) }
  ...
  private fun basicTaskSetup(): ManagedDeviceInstrumentationTestTask {
    ...
    doReturn(FakeGradleProperty(mock<AnalyticsService>())).whenever(task).analyticsService
    ...
    doReturn(FakeGradleProperty(true)).whenever(testData).hasTests(any(), any(), any())
    doReturn(FakeGradleProperty("flavor_name")).whenever(testData).flavorName
  ...
  fun testRunnerFactory_testCreateTestRunner() {
    val factory = mock<ManagedDeviceInstrumentationTestTask.TestRunnerFactory>(defaultAnswer = CALLS_REAL_METHODS)

    whenever(factory.executionEnum).thenReturn(FakeGradleProperty(TestOptions.Execution.ANDROIDX_TEST_ORCHESTRATOR))
    whenever(factory.forceCompilation).thenReturn(FakeGradleProperty(false))
    whenever(factory.emulatorControlConfig).thenReturn(FakeGradleProperty(mock<EmulatorControlConfig>()))
    whenever(factory.compileSdkVersion).thenReturn(FakeGradleProperty("sdkVersion"))
    whenever(factory.buildToolsRevision).thenReturn(FakeGradleProperty(mock<Revision>()))
  ...
  fun creationConfig_testTaskConfiguration() {
    ...
    val managedDevice =
      ManagedVirtualDevice("someNameHere").also {
        it.device = "Pixel 2"
        it.sdkVersion = 27
