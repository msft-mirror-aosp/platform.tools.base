...
class NdkSymlinkerKtTest {
  ...
  private fun testSymLinkFolder(originalNdkFolder: File, cxxVariantFolder: File, ndkSymlinkFolder: File?): File {
    val targetNdkSymLinkFolder = computeNdkSymLinkFolder(originalNdkFolder, cxxVariantFolder, ndkSymlinkFolder) ?: return originalNdkFolder
  ...
  fun keepOriginalNdkIfNdkDoesntExist() {
    ...
    val result = testSymLinkFolder(originalNdk, cxxVariantFolder, ndkSymlinkDirInLocalProp)
  ...
  fun workingAndIdempotent() {
    PassThroughRecordingLoggingEnvironment().use { log ->
      ...
      val ndkSymlinkDirInLocalProp = tmpFolder.root.toPath().resolve("symlinkto").toFile()
      val result = testSymLinkFolder(originalNdk, cxxVariantFolder, ndkSymlinkDirInLocalProp)
      val result2 = testSymLinkFolder(originalNdk, cxxVariantFolder, ndkSymlinkDirInLocalProp)
      val message = "${log.errors}\n${log.warnings}\n${log.infos}"
      assertThat(result.path).named(message).endsWith("17.2.4988734")
      assertThat(result2.path).named(message).endsWith("17.2.4988734")
      assertThat(result.toPath()).named(message).isEqualTo(result2.toPath())
    }
  }
