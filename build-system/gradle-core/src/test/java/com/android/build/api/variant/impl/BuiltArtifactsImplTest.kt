...
class BuiltArtifactsImplTest {
  ...
  fun simpleDirectoryWriting() {
    ...
    BuiltArtifactsImpl(
        ...
        applicationId = "com.android.test",
        variantName = "debug",
        elements = listOf(BuiltArtifactImpl.make(outputFile = innerFolder.absolutePath, versionCode = 123, versionName = "version_name")),
      )
      .save(FakeGradleDirectory(outputFolder))
            ...
            BuiltArtifactImpl.make(
              ...
              variantOutputConfiguration =
                VariantOutputConfigurationImpl(
                  isUniversal = false,
                  filters = listOf(FilterConfigurationImpl(FilterConfiguration.FilterType.DENSITY, "xhdpi")),
                ),
            ),
            BuiltArtifactImpl.make(
              ...
              variantOutputConfiguration =
                VariantOutputConfigurationImpl(
                  isUniversal = false,
                  filters = listOf(FilterConfigurationImpl(FilterConfiguration.FilterType.DENSITY, "xxhdpi")),
                ),
            ),
            BuiltArtifactImpl.make(
              ...
              variantOutputConfiguration =
                VariantOutputConfigurationImpl(
                  isUniversal = false,
                  filters = listOf(FilterConfigurationImpl(FilterConfiguration.FilterType.DENSITY, "xxxhdpi")),
                ),
            ),
          ),
      )
  ...
  fun testWithProperties() {
    val outputFolder = tmpFolder.newFolder("some_folder")
    createBuiltArtifacts(createBuiltArtifact(outputFolder = outputFolder, fileName = "file1", versionCode = 123, densityValue = "xhdpi"))
      .save(FakeGradleDirectory(outputFolder))
  ...
  fun testStudioLoading() {
    ...
    createBuiltArtifacts(
        createBuiltArtifact(
          ...
          versionCode = 123,
          densityValue = "xhdpi",
          attributes = mapOf("deliveryType" to "install-time", "a" to "b"),
        ),
        createBuiltArtifact(
          outputFolder = outputFolder,
          fileName = "file2",
    ...
    val builtArtifacts = GenericBuiltArtifactsLoader.loadFromFile(File(outputFolder, BuiltArtifactsImpl.METADATA_FILE_NAME), NullLogger())
    Truth.assertThat(builtArtifacts).isNotNull()
    Truth.assertThat(builtArtifacts!!.version).isEqualTo(BuiltArtifacts.METADATA_FILE_VERSION)
    Truth.assertThat(builtArtifacts.artifactType.type).isEqualTo(SingleArtifact.APK.name())
    Truth.assertThat(builtArtifacts.artifactType.kind).isEqualTo(SingleArtifact.APK.kind.dataType().simpleName)
    ...
    builtArtifactsList.forEach { builtArtifact ->
      Truth.assertThat(File(builtArtifact.outputFile).name).isAnyOf("file1.apk", "file2.apk")
  ...
  "elements": [
    ...
    val builtArtifacts = GenericBuiltArtifactsLoader.loadFromFile(listingFile, NullLogger())
    Truth.assertThat(builtArtifacts).isNotNull()
  ...
  fun testSerialization() {
    val artifacts =
      createBuiltArtifacts(
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file1", versionCode = 123, densityValue = "xhdpi"),
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file2", versionCode = 123, densityValue = "xxhdpi"),
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file3", versionCode = 123, densityValue = "xxxhdpi"),
      )
  ...
  fun testMixedFileTypes() {
    ...
    try {
      createBuiltArtifacts(BuiltArtifactImpl.make(folder.absolutePath), BuiltArtifactImpl.make(file.absolutePath))
  ...
  fun testMixedFileTypesWithMultipleFiles() {
    ...
    try {
      ...
      Truth.assertThat(e.message).contains("${fileOne.name},${fileTwo.name} are files")
      Truth.assertThat(e.message).contains("${folder.name} is a directory")
      return
    }
    fail("exception not thrown")
  ...
  fun testMixedFileTypesWithMultipleDirectories() {
    ...
    try {
      ...
      Truth.assertThat(e.message).contains("${folderOne.name},${folderTwo.name} are directories")
      Truth.assertThat(e.message).contains("${file.name} is a file")
      return
    }
    fail("exception not thrown")
  ...
  fun testIncrementalBuildWithFilters() {
    ...
    val updatedArtifacts =
      artifacts.addElement(
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file2b", versionCode = 123, densityValue = "xxhdpi")
      )

    Truth.assertThat(updatedArtifacts.elements).hasSize(3)
    val updatedArtifact =
      updatedArtifacts.getBuiltArtifact(
        VariantOutputConfigurationImpl(
          isUniversal = false,
          filters = listOf(FilterConfigurationImpl(FilterConfiguration.FilterType.DENSITY, "xxhdpi")),
        )
  ...
  fun testIncrementalBuildWithoutFilters() {
    val artifacts =
      createBuiltArtifacts(
        BuiltArtifactImpl.make(
          ...
          versionCode = 123,
          versionName = "123",
          variantOutputConfiguration = VariantOutputConfigurationImpl(isUniversal = true, filters = listOf()),
        ),
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file2", versionCode = 123, densityValue = "xxhdpi"),
        createBuiltArtifact(outputFolder = tmpFolder.root, fileName = "file3", versionCode = 123, densityValue = "xxxhdpi"),
      )
    ...
    Truth.assertThat(updatedArtifacts.elements).hasSize(3)
    val updatedArtifact = updatedArtifacts.getBuiltArtifact(VariantOutputConfigurationImpl(isUniversal = true))
  ...
  private fun createBuiltArtifact(
    ...
    BuiltArtifactImpl.make(
      ...
      variantOutputConfiguration =
        VariantOutputConfigurationImpl(
          isUniversal = false,
          filters = listOf(FilterConfigurationImpl(FilterConfiguration.FilterType.DENSITY, densityValue)),
        ),
  ...
  private fun createOutputFile(outputFolder: File, name: String): File = File(outputFolder, name).also { it.writeText("SomeFile") }
}
