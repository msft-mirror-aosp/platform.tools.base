...
class DecoratedApiChangesTest {
  ...
  fun undecoratedClassesTest() {
    ...
    getApiTester().checkApiElements { getApiElements(it, dslInterfaces) }
  }
}

val API_LIST_URL: URL = Resources.getResource(DecoratedApiChangesTest::class.java, "undecorated-dsl-api.txt")
...
fun getApiElements(klass: Class<*>, dslInterfaces: List<Class<*>>): List<String> {
  ...
  val superInterfaces = dslInterfaces.filter { it.isAssignableFrom(klass) }

  if (superInterfaces.isEmpty()) {
    return emptyList()
  }

  val interfaceMethods = superInterfaces.flatMap { it.methods.map(::getSignature) }.toSet()
  ...
  val innerClassesElements = klass.declaredClasses.flatMap { getApiElements(it, dslInterfaces) }
...
fun getDslInterfaces(): List<ClassPath.ClassInfo> {
  return ClassPath.from(Bundle::class.java.classLoader).getTopLevelClassesRecursive("com.android.build.api.dsl").filter {
...
private fun getApiClasses(): Set<ClassPath.ClassInfo> {
  ...
  builder.addAll(
    classPath
      .getTopLevelClasses("com.android.builder.signing")
      .stream()
      .filter { classInfo: ClassPath.ClassInfo -> classInfo.simpleName == "DefaultSigningConfig" }
      .collect(Collectors.toList())
  )

  val allClasses = builder.build().filter { (!it.simpleName.endsWith("Test") && it.simpleName != "StableApiUpdater") }
  ...
  return dslInterfacesImplClasses
    .filter {
      val clazz = it.load()
...
private fun getImplClasses(allClasses: List<ClassPath.ClassInfo>, interfaces: List<ClassPath.ClassInfo>): List<ClassPath.ClassInfo> {
  val interfaceClasses = interfaces.map { it.load() }

  return allClasses.filter {
    val clazz = it.load()
    interfaceClasses.any { iface -> iface.isAssignableFrom(clazz) }
  }
}
