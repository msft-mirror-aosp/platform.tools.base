...
class NdkLocatorKtTest {
  ...
  fun getVersionedFolderNames() {
    ...
    v2.mkdirs()
    f1.writeText("touch")
    assertThat(getNdkVersionedFolders(versionRoot)).containsExactly("17.1.2", "18.1.2")
  }
      ...
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            ...
            "/my/ndk/environment-folder".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to NDK_DEFAULT_VERSION))
  ...
  fun `non-existing ndkPath without NDK version in DSL and with side-by-side versions available (bug 129789776)`() {
    val path =
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            "/my/sdk/folder/ndk/18.1.23456".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.23456"))

            "/my/sdk/folder/ndk/18.1.00000".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.00000"))
      ...
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            ...
            "/my/sdk/folder/ndk-bundle".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.23456"))
  ...
  fun ndkPathPropertyLocationExists() {
    val path =
      findNdkPathImpl(
          ...
          getNdkSourceProperties = { path: File ->
            when (path.path) {
              "/my/ndk/folder".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to NDK_DEFAULT_VERSION))
  ...
  fun `ndkPath properties has -rc2 in version and ndkVersion exists`() {
    val ndk =
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            "/my/ndk/folder".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "21.0.6011959-rc2"))
  ...
  fun nonExistingNdkPathWithNdkVersionInDsl() {
    findNdkPathImpl(
      ...
      getNdkSourceProperties = { path: File ->
        when (path.path) {
          "/my/ndk/folder".toSlash() -> null
          "/my/ndk/environment-folder".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.23456"))
  ...
  fun sdkFolderNdkBundleExists() {
    val path =
      findNdkPathImpl(
          ...
          getNdkSourceProperties = { path: File ->
            when (path.path) {
              "/my/sdk/folder/ndk-bundle".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to NDK_DEFAULT_VERSION))
  ...
  fun `download fails with LicenceNotAcceptedException thrown`() {
    ...
    val sourceProperties =
      mapOf("/my/sdk/folder/ndk/$existingNdk".toSlash() to SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to existingNdk)))
    val sdkHandler = mock<SdkHandler>()
    val pkg = mock<RemotePackage>()
    doThrow(RuntimeException(LicenceNotAcceptedException(sdkFolder.toSlashFile()!!.toPath(), listOf(pkg))))
      .whenever(sdkHandler)
    ...
    try {
      ...
      // RuntimeException
      assertThat(e.cause is LicenceNotAcceptedException).named("${e.cause} is not of the expected type").isTrue()
      return
    }
  ...
  fun `download fails with InstallFailedException thrown`() {
    ...
    val sdkHandler = mock<SdkHandler>()
    val pkg = mock<RemotePackage>()
    doThrow(RuntimeException(InstallFailedException(sdkFolder.toSlashFile()!!.toPath(), listOf(pkg))))
      .whenever(sdkHandler)
      .installNdk(Revision.parseRevision(NDK_DEFAULT_VERSION))
    ...
    try {
      ...
      // RuntimeException
      assertThat(e.cause is InstallFailedException).named("${e.cause} is not of the expected type").isTrue()
      return
    }
    fail("Expected exception")
  }
  ...
  fun `download fails with null returned`() {
    ...
    val sourceProperties =
      mapOf("/my/sdk/folder/ndk/$existingNdk".toSlash() to SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to existingNdk)))
    val sdkHandler = mock<SdkHandler>()
    doReturn(null).whenever(sdkHandler).installNdk(Revision.parseRevision(NDK_DEFAULT_VERSION))
  ...
  fun `no version specified by user and only old ndk available download succeeds`() {
    val existingNdk = "18.1.23456"
    val sourceProperties =
      mutableMapOf("/my/sdk/folder/ndk/$existingNdk".toSlash() to SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to existingNdk)))
    val sdkHandler = mock<SdkHandler>()
    doReturn("/my/sdk/folder/ndk/$NDK_DEFAULT_VERSION".toSlashFile())
      .whenever(sdkHandler)
      .installNdk(Revision.parseRevision(NDK_DEFAULT_VERSION))
    ...
    assertThat(ndk.ndk).isEqualTo("/my/sdk/folder/ndk/$NDK_DEFAULT_VERSION".toSlashFile())
    assertThat(log.errors()).hasSize(0)
    assertThat(log.warnings()).hasSize(0)
  }
  ...
  fun `ndk rc configured with space-rc1 version in DSL`() {
    val path =
      findNdkPathImpl(
          ...
          getNdkSourceProperties = { path: File ->
            when (path.path) {
              "/my/sdk/folder/ndk/18.1.23456".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.23456 rc1"))
  ...
  fun ndkPathPropertyLocationExistsWithDslVersion() {
    val ndk =
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            "/my/ndk/folder".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.23456"))
  ...
  fun sdkFolderNdkBundleExistsWithVersionedNdk() {
    val path =
      findNdkPathImpl(
          ...
          getNdkSourceProperties = { path: File ->
            when (path.path) {
              "/my/sdk/folder/ndk/$NDK_DEFAULT_VERSION".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to NDK_DEFAULT_VERSION))
      ...
      findNdkPathImpl(
        ...
        getNdkSourceProperties = { path: File ->
          when (path.path) {
            ...
            "/my/sdk/folder/ndk/18.1.99999".toSlash() -> SdkSourceProperties(mapOf(SDK_PKG_REVISION.key to "18.1.99999"))
  ...
  fun unparseableNdkVersionFromDsl() {
    ...
    assertThat(log.errors().single().message).isEqualTo("Requested NDK version '17.1.unparseable' could not be parsed")
  }
      ...
      PassThroughRecordingLoggingEnvironment().use {
        for (i in 0..10000) {
          ...
          findNdkPathImpl(
            ...
            getNdkSourceProperties = { path: File ->
              when (path.path) {
                pathToNdk() -> SdkSourceProperties(mapOf((sourcePropertyVersionKey() ?: "") to (ndkVersion() ?: "")))
