...
class CompileCommandsCodecTest {
  ...
  private fun testFile(testFileName: String): File {
    if (TestUtils.runningFromBazel()) {
      ...
      return TestResources.getFile(CompileCommandsCodecTest::class.java, "$resourceFolder$testFileName")
    }
  ...
  /**
   * The purpose of this test is to detect incompatible changes that don't have a corresponding change to [COMPILE_COMMANDS_CODEC_VERSION].
   * - If you get a failure like "Expected path/to/version_$XYZ_compile_commands.json.bin to exist. Did you forget to check it in?" then it
   *   means that you increased [COMPILE_COMMANDS_CODEC_VERSION] in this CL. The expected file has been created and should be added to the
   *   CL. The test will pass when you run it again.
   * - If one of the asserts inside the "Make sure we can stream all 2+ versions" block of code fires, then there is a behavior change. If
   *   it's expected (for example, a bug fix) then the test should be modified to check the version number and assert based on specific
   *   version.
   * - If you get an error like "Content of path/to/version_$XYZ_compile_commands.json.bin is not the same when recreated" then something
   *   changed in the binary output even though it seems compatible according to the current decoding logic. The safest course is to
   *   increase [COMPILE_COMMANDS_CODEC_VERSION]. This check is not done one Windows because the binary can be different due to path
   *   handling.
   */
  @Test
  fun `confirm current version binary compatibility`() {
    fun versionFile(version: Int) = testFile("version_${version}_compile_commands.json.bin")
    fun createJsonBinForCurrentVersion() =
      convertCMakeToCompileCommandsBin(testFile("dolphin_compile_commands.json"), versionFile(COMPILE_COMMANDS_CODEC_VERSION))
    ...
    val recreationOfVersionRecord = tempFolder.newFolder().resolve("recreation_compile_commands.json.bin")
    recreationOfVersionRecord.parentFile.mkdirs()
    convertCMakeToCompileCommandsBin(testFile("dolphin_compile_commands.json"), recreationOfVersionRecord)
    val comparison = compareFileContents(versionRecord, recreationOfVersionRecord)
    assertThat(comparison).named("Content of $versionRecord is not the same when recreated").isEqualTo(SAME_CONTENT)
  }

  @Test
  fun `confirm prior AGP versions can be read`() {
    fun versionFile(version: String) = testFile("agp_${version}_compile_commands.json.bin")
    ...
    val binVersionToAgpVersionsMap =
      listOf(
    ...
    for ((binVersion, agpVersions) in binVersionToAgpVersionsMap) {
      for (agpVersion in agpVersions) {
        ...
        assertThat(hasBug201754404(versionRecord)).named(agpVersion).isEqualTo(hasBug201754404.contains(agpVersion))
        val readVersion = readCompileCommandsVersionNumber(versionRecord)
        ...
        if (readVersion === 1) {
          streamCompileCommandsV1(versionRecord) { sourceFile: File, compiler: File, flags: List<String>, workingDirectory: File ->
            distinctSourceFiles += sourceFile.path
            distinctCompilers += compiler.path
            distinctFlags += flags
  ...
  fun `check whether compile_commands json bin supports output file for current version`() {
    ...
    CompileCommandsEncoder(out).use { encoder ->
      encoder.writeCompileCommand(sourceFile, compiler, flags, workingDirectory, output, target)
    }
  ...
  fun `check whether compile_commands json bin supports output file for version 1 file`() {
    ...
    streamCompileCommandsV1(version1) { _, _, _, _ -> }
  }
  ...
  fun checkInterning() {
    ...
    assertThat(count).named("Did not process two files").isEqualTo(2)
  }
  ...
  fun testStripArgsForIde() {
    assertThat(stripArgsForIde("", listOf("-abc", "-def", "foo", "bar"))).containsExactly("-abc", "-def", "foo", "bar").inOrder()

    assertThat(stripArgsForIde("path/to/source", listOf("-abc", "-def", "foo", "bar", "path/to/source")))
      .containsExactly("-abc", "-def", "foo", "bar")
      .inOrder()
    ...
    assertThat(stripArgsForIde("", listOf("-abc", "-def", "foo", "bar", "--output=blah.o")))
      .containsExactly("-abc", "-def", "foo", "bar")
      .inOrder()
    assertThat(stripArgsForIde("", listOf("-abc", "-def", "foo", "bar", "-MFblah"))).containsExactly("-abc", "-def", "foo", "bar").inOrder()
    assertThat(stripArgsForIde("", listOf("-abc", "-def", "foo", "bar", "-MF", "blah")))
      .containsExactly("-abc", "-def", "foo", "bar")
      .inOrder()
    assertThat(stripArgsForIde("", listOf("-abc", "-def", "foo", "bar", "-M", "-MM", "-MD", "-MG", "-MP", "-MMD", "-c")))
      .containsExactly("-abc", "-def", "foo", "bar")
      .inOrder()
  }
  ...
  fun `extract output file`() {
    assertThat(extractFlagArgument("-o", "--output", listOf("-o", "app/src/main/cpp/native-lib.o")))
