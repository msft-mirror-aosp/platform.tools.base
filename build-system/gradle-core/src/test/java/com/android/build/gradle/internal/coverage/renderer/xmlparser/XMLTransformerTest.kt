...
class XMLTransformerTest {
    ...
    fun `transform parses and aggregates multiple reports correctly`() {
        val unitTestXml = createXmlFile(
            "unit_test_report.xml",
            getXmlContent("my-module", "debug", "UnitTest", sourceRoot.absolutePath)
        )
        val aggregatedXml = createXmlFile(
            "aggregated_report.xml",
            getAggregatedXmlContent("my-module", "debug")
        )
    ...
    fun `transform with default package and aggregated packages`() {
        ...
        val defaultPackageXml = createXmlFile(
            "default_package_report.xml",
            getDefaultPackageXmlContent("my-module", "debug", "DefaultPackageTest", sourceRoot.absolutePath)
        )
        val aggregatedWithPackagesXml = createXmlFile(
            "aggregated_with_packages.xml",
            getAggregatedWithPackagesXmlContent("my-module", "debug")
        )
        val coverageBuilder = CoverageReportBuilder("Test Project", "now")
        ...
        val testSuiteFileBuilder = sourceFileBuilder.variantFileCoverageBuilders["debug"]!!.testSuiteFileCoverageBuilders["DefaultPackageTest"]!!
