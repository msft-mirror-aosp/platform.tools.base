...
/**
 * Tool used to originally generate cxx/model/MetaModel.kt. That file is now hand maintained but this tool is left here in case we want to
 * use it to make a more sweeping change.
 */
...
data class FieldInfo(
  ...
  private val typeToStringSuffix =
    when {
      !isString && isNullable -> "?.toString()"
      !isString && !isNullable -> ".toString()"
      else -> ""
    }
  val lookupFromAbi: String =
    when (modelMember) {
