...
class LintKtTest {
  ...
  fun `no error case`() {
    NativeBuildConfigValueMini()
      .apply { buildFiles = listOf(tempFolder.newFile("CMakeLists.txt")) }
  ...
  fun `non-existent buildFile`() {
    NativeBuildConfigValueMini().apply { buildFiles = listOf(File("CMakeLists.txt")) }.checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(BUILD_FILE_DID_NOT_EXIST, "expected buildFiles file 'CMakeLists.txt' to exist")
  }

  @Test
  fun `non-existent buildTargetsCommandComponents command`() {
    NativeBuildConfigValueMini()
      .apply { buildTargetsCommandComponents = listOf("path/to/cmake.exe") }
  ...
  fun `non-existent library buildCommandComponents command`() {
    NativeBuildConfigValueMini()
      .apply {
        libraries = mutableMapOf()
        libraries["lib1"] = NativeLibraryValueMini().apply { buildCommandComponents = listOf("path/to/cmake.exe") }
      }
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(
  ...
  fun `non-existent library artifact name`() {
    NativeBuildConfigValueMini()
      ...
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(LIBRARY_ARTIFACT_NAME_DID_NOT_EXIST, "expected lib1.artifactName to exist")
  }
  ...
  fun `non-canonicalizable runtime library name`() {
    NativeBuildConfigValueMini()
      .apply {
        libraries = mutableMapOf()
        libraries["lib1"] = NativeLibraryValueMini().apply { runtimeFiles = listOf(filePathThatIsNotCanonicalizableOnAllPlatforms) }
      }
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(COULD_NOT_CANONICALIZE_PATH, "Could not canonicalize '\u0000' in lib1.runtimeFiles due to IOException")
  }
  ...
  fun `missing ABI`() {
    NativeBuildConfigValueMini()
      ...
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(LIBRARY_ABI_NAME_DID_NOT_EXIST, "expected lib1.abi to exist")
  }
  ...
  fun `misspelled ABI name`() {
    ...
    NativeBuildConfigValueMini()
      ...
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(LIBRARY_ABI_NAME_IS_INVALID, "lib1.abi 'x86_65' is invalid. Valid values are 'arm64-v8a, armeabi-v7a, x86, x86_64'")
  }
  ...
  fun `mismatched ABI names`() {
    ...
    NativeBuildConfigValueMini()
      ...
      .checkLint(fileBeingLinted, abis, supportedAbis)
    log.assertError(LIBRARY_HAD_MULTIPLE_ABIS, "unexpected mismatched library ABIs: x86, x86_64")
  }
