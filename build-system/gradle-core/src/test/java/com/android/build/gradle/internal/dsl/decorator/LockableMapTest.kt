...
internal class LockableMapTest {
  ...
  fun `check map modification after locking`() {
    ...
    fun checkModificationFails(action: () -> Unit) {
      val failure = assertFailsWith<AgpDslLockedException> { action.invoke() }
  ...
  fun `check map reading after locking`() {
    ...
    doAndCheckContents { assertThat(lockableMap.getOrDefault("one", "0")).isEqualTo("1") }
    doAndCheckContents { assertThat(lockableMap.getOrDefault("zero", "0")).isEqualTo("0") }
    doAndCheckContents { assertThat(lockableMap.getOrElse("zero") { "0" }).isEqualTo("0") }
    doAndCheckContents { assertThat(lockableMap.filterKeys { it != "one" }.keys).containsExactly("three", "four") }
    doAndCheckContents {
  ...
  fun `check map entrySet reading after locking`() {
    ...
    doAndCheckContents {
      assertThat(lockableMap.entries.map { it.toPair() }).containsExactly(Pair("one", "1"), Pair("three", "3"), Pair("four", "4"))
    }

    doAndCheckContents { assertThat(lockableMap.entries.iterator().next().toPair()).isEqualTo(Pair("one", "1")) }
  }
}
