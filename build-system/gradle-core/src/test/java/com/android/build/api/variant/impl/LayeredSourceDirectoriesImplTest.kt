...
internal class LayeredSourceDirectoriesImplTest {
  ...
  fun setup() {
    ...
    whenever(variantServices.newListPropertyForInternalUse(Directory::class.java)).thenAnswer {
  ...
  fun testGetAllStaticSplit() {
    ...
    testTarget.addStaticSources(
      DirectoryEntries(
        "lowestStatic",
        mutableListOf(FileBasedDirectoryEntryImpl("lowestStatic", temporaryFolder.newFolder("lowestStatic"))),
      )
    )

    testTarget.addSource(FileBasedDirectoryEntryImpl("lowerGenerated", temporaryFolder.newFolder("lowerGenerated")))

    testTarget.addStaticSources(
      DirectoryEntries(
        "higherStatic",
        mutableListOf(FileBasedDirectoryEntryImpl("higherStatic", temporaryFolder.newFolder("higherStatic"))),
      )
    )

    testTarget.addSource(FileBasedDirectoryEntryImpl("highestGenerated", temporaryFolder.newFolder("highestGenerated")))
  ...
  private fun createTestTarget(): LayeredSourceDirectoriesImpl {
    ...
    testTarget.addSource(FileBasedDirectoryEntryImpl("highest1", temporaryFolder.newFolder("highest1")))

    testTarget.addStaticSourceDirectory(temporaryFolder.newFolder("somewhere/safe").absolutePath)
