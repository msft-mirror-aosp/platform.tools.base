 ...
 *
 * The `getReport()` method converts this internal map structure into the
 * final list-based [RootReport] data model for serialization.
 */
class XMLReportAggregator(private val files: List<File>) {

    @VisibleForTesting
    fun getInputFiles(): List<File> = files
    ...
    /**
     * Generates the RootReport and writes it to the specified output directory
     * along with the necessary JSON, JS, and HTML resources.
     */
    fun writeReport(outputDir: File) {
        ...
        resources.forEach { fileName ->
            XMLReportAggregator::class.java.getResourceAsStream("/com/android/build/gradle/internal/test/report/renderer/$fileName")?.use { inputStream ->
            ...
            ?.forEach { xmlFile ->
                ...
                try {
                    xmlFile.inputStream().use { inputStream ->
                        processXmlStream(inputStream, xmlFile.name)
                    }
                } catch (e: IOException) {
    ...
    /**
     * Converts the internal aggregated structure into the final [RootReport]
     * data model, ready for JSON serialization.
     */
    ...
    private class ModuleBuilder(val name: String) {
        ...
        fun build() = Module(
            name = name,
            testSuites = testSuites.values.map { it.build() }.sortedBy { it.name }
        )
    }
    ...
    private class TestSuiteBuilder(val name: String) {
        ...
        fun build() = TestSuite(
            name = name,
            packages = packages.values.map { it.build() }.sortedBy { it.name }
        )
    }
    ...
    private class PackageBuilder(val name: String) {
        ...
        fun build() = Package(
            name = name,
            classes = classes.values.map { it.build() }.sortedBy { it.name }
        )
    }
    ...
    private class ClassBuilder(val name: String) {
        ...
        fun build() = ClassType(
            name = name,
            functions = functions.values.map { it.build() }.sortedBy { it.name }
        )
    }
    ...
    private fun addTestResult(
        ...
        try {
            ...
            rootReportBuilder.getOrAddModule(modulePath)
