...
class DexMergingTaskTest {
  ...
  fun testMonoDex() {
    ...
    assertThatDex(outputDir.resolve("classes.dex")).containsExactlyClassesIn(listOf("Ltest/A;", "Ltest/B;", "Ltest/C;"))
  }

  @Test
  fun testLegacyMultiDex() {
    val inputDirs = listOf(generateDexArchive(listOf("test/A", "test/B", "test/C")))
    val multidexProguardRulesFile = tmp.newFile().apply { writeText(" -keep class test.A") }
    val userMultidexKeepFile = tmp.newFile().apply { writeText("test/B.class") }
    val outputDir = tmp.newFolder()
    runDexMerging(
      inputDirs,
      outputDir,
      ...
      libraryClasses = listOf(TestUtils.resolvePlatformPath("android.jar", TestUtils.TestType.AGP).toFile()),
      userMultidexKeepFile = userMultidexKeepFile,
    )
    assertThatDex(outputDir.resolve("classes.dex")).containsExactlyClassesIn(listOf("Ltest/A;", "Ltest/B;"))
    assertThatDex(outputDir.resolve("classes2.dex")).containsExactlyClassesIn(listOf("Ltest/C;"))
  }

  @Test
  fun testNativeMultiDex() {
    val numInputDirs = 5
    val inputDirs = (0 until numInputDirs).map { generateDexArchive(listOf("test/A$it")) }
    val outputDir = tmp.newFolder()

    runDexMerging(inputDirs, outputDir, dexingType = DexingType.NATIVE_MULTIDEX)

    assertThatDex(outputDir.resolve("classes.dex")).containsExactlyClassesIn((0 until numInputDirs).map { "Ltest/A$it;" })
  }

  @Test
  fun `test non-incremental build with buckets`() {
    val inputDirs = (0 until 2).map { generateDexArchive(listOf("package$it/Class0", "package$it/Class1"), outputToDir = true) }
    val inputJars = (2 until 4).map { generateDexArchive(listOf("package$it/Class0", "package$it/Class1"), outputToDir = false) }
    val outputDir = tmp.newFolder()
    ...
    assertThatDex(bucket1)
      .containsExactlyClassesIn(listOf("Lpackage0/Class0;", "Lpackage0/Class1;", "Lpackage2/Class0;", "Lpackage2/Class1;"))
    assertThatDex(bucket2)
      .containsExactlyClassesIn(listOf("Lpackage1/Class0;", "Lpackage1/Class1;", "Lpackage3/Class0;", "Lpackage3/Class1;"))
  }

  @Test
  fun `test incremental build with changed dex file`() {
    val inputDirs = (0 until 4).map { generateDexArchive(listOf("package$it/Class$it")) }
    val outputDir = tmp.newFolder()
    ...
    val bucket2 = outputDir.resolve("2/classes.dex")
    assertThat(bucket0).doesNotExist()
    assertThatDex(bucket1).containsExactlyClassesIn(listOf("Lpackage0/Class0;", "Lpackage2/Class2;"))
    assertThatDex(bucket2).containsExactlyClassesIn(listOf("Lpackage1/Class1;", "Lpackage3/Class3;"))
  ...
  fun `test incremental build with added jar`() {
    val inputJars = (0 until 4).map { generateDexArchive(listOf("package$it/Class$it"), outputToDir = false) }
    val outputDir = tmp.newFolder()
    ...
    val inputChanges =
      FakeInputChanges(
        incremental = true,
        inputChanges = listOf(FakeFileChange(file = addedInputJar, normalizedPath = addedInputJar.name, changeType = ChangeType.ADDED)),
      )
    ...
    assertThat(bucket0).doesNotExist()
    assertThatDex(bucket1).containsExactlyClassesIn(listOf("Lpackage0/Class0;", "Lpackage2/Class2;", "Lpackage4/Class4;"))
    assertThatDex(bucket2).containsExactlyClassesIn(listOf("Lpackage1/Class1;", "Lpackage3/Class3;"))
  ...
  fun `test incremental build with modified jar`() {
    ...
    val inputChanges =
      FakeInputChanges(
        incremental = true,
        inputChanges = listOf(FakeFileChange(file = inputJars[0], normalizedPath = inputJars[0].name, changeType = ChangeType.MODIFIED)),
      )
  ...
  fun `test incremental build with removed jar`() {
    ...
    val inputChanges =
      FakeInputChanges(
        incremental = true,
        inputChanges = listOf(FakeFileChange(file = inputJars[0], normalizedPath = inputJars[0].name, changeType = ChangeType.REMOVED)),
      )
    ...
    assertThat(bucket0).doesNotExist()
    assertThatDex(bucket1).containsExactlyClassesIn(listOf("Lpackage2/Class2;"))
    assertThatDex(bucket2).containsExactlyClassesIn(listOf("Lpackage1/Class1;", "Lpackage3/Class3;"))
          ...
          return object : Params() {
            override val sharedParams =
              project.objects.property(DexMergingTask.SharedParams::class.java).also {
                it.set(
                  object : DexMergingTask.SharedParams() {
                    ...
                    override val errorFormatMode = FakeGradleProperty(SyncOptions.ErrorFormatMode.HUMAN_READABLE)
                    override val mainDexListConfig: MainDexListConfig
                      get() =
                        object : MainDexListConfig() {
                          override val userMultidexKeepFile = FakeObjectFactory.factory.fileProperty().fileValue(userMultidexKeepFile)
                          override val aaptGeneratedRules = FakeObjectFactory.factory.fileProperty()
                          override val userMultidexProguardRules =
                            FakeObjectFactory.factory.listProperty(RegularFile::class.java).also { listProperty ->
                              multidexProguardRulesFile?.let { file ->
                                listProperty.add(FakeObjectFactory.factory.fileProperty().fileValue(file))
                              }
                            }
                          override val libraryClasses = FakeConfigurableFileCollection(libraryClasses)
                          override val platformMultidexProguardRules = FakeObjectFactory.factory.listProperty(String::class.java)
                        }
            ...
            override val dexDirsOrJars = FakeObjectFactory.factory.listProperty(File::class.java).value(inputDirsOrJars)
            ...
            override val mainDexListOutput = FakeObjectFactory.factory.fileProperty().fileValue(mainDexListOutput)
            override val d8Metadata = FakeObjectFactory.factory.fileProperty().fileValue(d8Metadata)
            override val incremental = FakeGradleProperty(inputChanges.isIncremental)
            override val fileChanges =
              FakeGradleProperty(inputChanges.getFileChanges(FakeFileCollection(dexDirsOrJars.get())).toSerializable())
            ...
            override val analyticsService =
              project.objects.property(AnalyticsService::class.java).also { it.set(FakeNoOpAnalyticsService()) }
  ...
  private fun generateDexArchive(emptyClassNames: List<String>, outputToDir: Boolean = true): File {
    val dexArchivePath =
      if (outputToDir) {
        tmp.newFolder()
      } else {
  ...
  private fun generateDexArchive(emptyClassNames: List<String>, dexArchivePath: File, globalSyntheticsOutput: Path) {
    val emptyClassesDir = tmp.newFolder().also { TestInputsGenerator.dirWithEmptyClasses(it.toPath(), emptyClassNames) }
    generateDexArchive(emptyClassesDir, dexArchivePath, globalSyntheticsOutput)
  }

  private fun generateDexArchive(classesDirOrJar: File, dexArchivePath: File, globalSyntheticsOutput: Path) {
    val builder =
    ...
    ClassFileInputs.fromPath(classesDirOrJar.toPath()).use { input ->
      builder.convert(input.entries { _, _ -> true }, dexArchivePath.toPath(), globalSyntheticsOutput)
    }
  }
}
