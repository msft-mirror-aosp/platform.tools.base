...
class ExtractJniTransformTest {
  ...
  fun testMultipleNativeLibs() {
    ...
    assertThat(File(transformOutputs.outputDirectory, "x86_64/foo.so")).hasContents("x86_64 foo")
    assertThat(File(transformOutputs.outputDirectory, "x86_64/bar.so")).hasContents("x86_64 bar")
    assertThat(File(transformOutputs.outputDirectory, "future_abi/foo.so")).hasContents("future_abi foo")
    assertThat(File(transformOutputs.outputDirectory, "future_abi/bar.so")).hasContents("future_abi bar")
    assertThat(File(transformOutputs.outputDirectory, "x86/$FN_GDBSERVER")).hasContents("x86 gdbserver")
  ...
  private fun createTransform(primaryInput: File): ExtractJniTransform {
    return object : ExtractJniTransform() {
      override val inputJar: Provider<FileSystemLocation> = FakeGradleProvider(FakeGradleRegularFile(primaryInput))
