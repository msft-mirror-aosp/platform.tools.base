...
class ExtractProfilerNativeDependenciesTaskTest {
  ...
  fun setUp() {
    ...
    val fooJar = tempFolder.resolve("foo.jar").also { TestInputsGenerator.writeJarWithEmptyEntries(it.toPath(), listOf("foo")) }

    val nativeLibJar1 =
      tempFolder.resolve("nativeLibJar1.jar").also { TestInputsGenerator.writeJarWithEmptyEntries(it.toPath(), listOf("lib/x86/foo.so")) }

    val nativeLibJar2 =
      tempFolder.resolve("nativeLibJar2.jar").also { TestInputsGenerator.writeJarWithEmptyEntries(it.toPath(), listOf("lib/x86/bar.so")) }

    val nativeLibJar3 =
      tempFolder.resolve("nativeLibJar3.jar").also { TestInputsGenerator.writeJarWithEmptyEntries(it.toPath(), listOf("lib/x86/baz.so")) }

    jarWithoutDeps =
      tempFolder.resolve("jarWithoutDeps.jar").also { jarJar ->
        ZipOutputStream(FileOutputStream(jarJar)).use { zip ->
          zip.putNextEntry(ZipEntry("foo.jar"))
          BufferedInputStream(FileInputStream(fooJar)).use { ByteStreams.copy(it, zip) }
        }
      }

    jarWithoutNativeDeps =
      tempFolder.resolve("jarWithoutNativeDeps.jar").also { jarJar ->
        ZipOutputStream(FileOutputStream(jarJar)).use { zip ->
          zip.putNextEntry(ZipEntry("foo.jar"))
          BufferedInputStream(FileInputStream(fooJar)).use { ByteStreams.copy(it, zip) }
    ...
    jarWithNativeDep =
      tempFolder.resolve("jarWithNativeDep.jar").also { jarJar ->
        ZipOutputStream(FileOutputStream(jarJar)).use { zip ->
          zip.putNextEntry(ZipEntry("dependencies/nativeLibJar1.jar"))
          BufferedInputStream(FileInputStream(nativeLibJar1)).use { ByteStreams.copy(it, zip) }
        }
      }
    ...
    jarWithMultipleNativeDeps =
      tempFolder.resolve("jarWithMultipleNativeDeps.jar").also { jarJar ->
        ZipOutputStream(FileOutputStream(jarJar)).use { zip ->
          ...
          BufferedInputStream(FileInputStream(nativeLibJar2)).use { ByteStreams.copy(it, zip) }
          zip.putNextEntry(ZipEntry("dependencies/nativeLibJar3.jar"))
          BufferedInputStream(FileInputStream(nativeLibJar3)).use { ByteStreams.copy(it, zip) }
        }
      }
  }
  ...
  fun testNativeLibExtractionFromMultiple() {
    ...
    executeWorkerAction(listOf(jarWithoutDeps, jarWithoutNativeDeps, jarWithNativeDep, jarWithMultipleNativeDeps), outputDir)

    assertThat(outputDir.list()!!.asList()).containsExactly("x86")
    assertThat(outputDir.resolve("x86").list()!!.asList()).containsExactly("foo.so", "bar.so", "baz.so")
  }
  ...
  fun testErrorFromDuplicateNativeLibs() {
    ...
    try {
      ...
      assertThat(e.message).contains("Unexpected duplicate profiler native dependency: x86/foo.so")
    }
  }
