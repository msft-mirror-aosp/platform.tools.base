...
class AndroidLintTextOutputTaskTest {
  ...
  abstract class TaskForTest @Inject constructor(testWorkerExecutor: WorkerExecutor, val testLogger: FakeLogger) :
  ...
  fun testNoIssuesFound() {
    task.textReportInputFile.set(temporaryFolder.newFile().also { it.writeText("No issues found.") })
    task.returnValueInputFile.set(temporaryFolder.newFile().also { it.writeText("0") })
    task.outputStream.set(AndroidLintTextOutputTask.OutputStream.STDOUT)
    task.fatalOnly.set(false)
    task.android.set(true)
  ...
  fun testNoErrorsOrWarnings() {
    task.textReportInputFile.set(temporaryFolder.newFile().also { it.writeText("Foo bar 0 errors, 0 warnings.") })
    task.returnValueInputFile.set(temporaryFolder.newFile().also { it.writeText("0") })
    task.outputStream.set(AndroidLintTextOutputTask.OutputStream.STDOUT)
    task.fatalOnly.set(false)
    task.android.set(true)
  ...
  fun testFatalOnlyFailure() {
    ...
    try {
      ...
      assertThat(e.message).contains("Lint found fatal errors while assembling a release target.")
      assertThat(e.message).contains("android {")
      assertThat(e.message).contains("Foo")
    }
    assertThat((task as TaskForTest).testLogger.lifeCycles).contains("Foo")
  ...
  fun testNewBaselineFileCreated() {
    task.textReportInputFile.set(temporaryFolder.newFile().also { it.writeText("Foo") })
    task.returnValueInputFile.set(temporaryFolder.newFile().also { it.writeText("$ERRNO_CREATED_BASELINE") })
    task.outputStream.set(AndroidLintTextOutputTask.OutputStream.STDOUT)
    task.fatalOnly.set(false)
    task.android.set(true)
    task.abortOnError.set(false)
  ...
  fun testHasBaselineFailure() {
    ...
    try {
      ...
      assertThat(e.message)
        .contains("Fix the issues identified by lint, or add the issues to the lint baseline via `gradlew updateLintBaseline`.")
      assertThat(e.message).doesNotContain("android {")
      assertThat(e.message).contains("Foo")
    }
    assertThat((task as TaskForTest).testLogger.lifeCycles).contains("Foo")
