...
class BooleanOptionTest {
  ...
  fun `check experimental features have a FutureStage`() {
    ...
    checkViolatingProjectOptions(
      violatingOptions =
        BooleanOption.entries.filter {
          (it.stage is FeatureStage.Experimental || it.stage is ApiStage.Experimental) && it.futureStage == null
        },
  ...
  fun `check supported features have a FutureStage`() {
    ...
    checkViolatingProjectOptions(
      violatingOptions = BooleanOption.entries.filter { it.stage is FeatureStage.Supported && it.futureStage == null },
      ignoreList = ignoreList,
  ...
  fun `check softly-enforced and enforced features have default value 'true'`() {
    checkViolatingProjectOptions(
      violatingOptions =
        BooleanOption.entries.filter { (it.stage is FeatureStage.SoftlyEnforced || it.stage is FeatureStage.Enforced) && !it.defaultValue },
  ...
  fun `check deprecated and removed features have default value 'false'`() {
    val ignoreList = listOf(BooleanOption.ENABLE_LEGACY_API)
    checkViolatingProjectOptions(
      violatingOptions =
        BooleanOption.entries.filter { (it.stage is FeatureStage.Deprecated || it.stage is FeatureStage.Removed) && it.defaultValue },
      ignoreList = ignoreList,
  ...
  fun `check that FutureStage is different from the current stage`() {
    checkViolatingProjectOptions(
      violatingOptions =
        BooleanOption.entries.filter {
          it.futureStage != null && it.futureStage.stage::class == it.stage::class && it.futureStage.defaultValue == it.defaultValue
        },
