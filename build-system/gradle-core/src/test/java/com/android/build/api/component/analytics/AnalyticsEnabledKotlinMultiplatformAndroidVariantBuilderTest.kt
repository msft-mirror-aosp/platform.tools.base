...
class AnalyticsEnabledKotlinMultiplatformAndroidVariantBuilderTest {
  @get:Rule val rule: MockitoRule = MockitoJUnit.rule().strictness(Strictness.STRICT_STUBS)

  private val delegate: KotlinMultiplatformAndroidVariantBuilder = mock()

  private val stats = GradleBuildVariant.newBuilder()
  private val proxy: AnalyticsEnabledKotlinMultiplatformAndroidVariantBuilder by lazy {
    AnalyticsEnabledKotlinMultiplatformAndroidVariantBuilder(delegate, stats)
  }
  ...
  fun getDeviceTests_for_android_test() {
    ...
    Truth.assertThat((androidTestProxy as AnalyticsEnabledAndroidTestBuilder).delegate).isEqualTo(deviceTest)
  ...
  @Test
  fun testIsMinifyEnabled() {
    proxy.isMinifyEnabled = true

    Truth.assertThat(proxy.isMinifyEnabled).isInstanceOf(java.lang.Boolean::class.java)
    Truth.assertThat(stats.variantApiAccess.variantAccessCount).isEqualTo(1)
    Truth.assertThat(stats.variantApiAccess.variantAccessList.first().type).isEqualTo(VariantMethodType.CODE_MINIFICATION_VALUE_VALUE)
    verify(delegate, times(1)).isMinifyEnabled
  }
}
