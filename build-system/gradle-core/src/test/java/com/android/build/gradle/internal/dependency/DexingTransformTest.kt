...
class DexingTransformTest {
  ...
  fun testDexingJar() {
    ...
    val dexingTransform =
      TestDexingTransform(FakeGradleProvider(FakeGradleRegularFile(input)), parameters = TestDexingTransform.TestParameters())
    ...
    assertThatDex(outputs.outputDirectory.resolve("${computeDexDirName(outputs.outputDirectory)}/classes.dex"))
  ...
  fun testDexingDir() {
    ...
    val dexingTransform =
      TestDexingTransform(FakeGradleProvider(FakeGradleDirectory(input)), parameters = TestDexingTransform.TestParameters())
    val outputs = FakeTransformOutputs(tmp)
    ...
    assertThat(dexFiles).containsExactly(outputs.outputDirectory.resolve("${computeDexDirName(outputs.outputDirectory)}/test/A.dex"))
  ...
  fun testDexingBigJar() {
    ...
    ZipOutputStream(input.outputStream()).use {
      for (i in 0 until (totalMethods / methodsPerClass)) {
        ...
        val classContent = TestClassesGenerator.classWithEmptyMethods("test/A$i", *methodNames.toTypedArray())
    ...
    val transform = TestDexingTransform(FakeGradleProvider(FakeGradleRegularFile(input)), parameters = TestDexingTransform.TestParameters())
    val outputs = FakeTransformOutputs(tmp)
    transform.transform(outputs)

    assertThat(
        Dex(outputs.outputDirectory.resolve("${computeDexDirName(outputs.outputDirectory)}/classes.dex")).classes.size +
          Dex(outputs.outputDirectory.resolve("${computeDexDirName(outputs.outputDirectory)}/classes2.dex")).classes.size
      )
  ...
  fun testDexingWithDesugaring() {
    val input = tmp.newFolder("classes")
    val classes = listOf(Animal::class.java, CarbonForm::class.java, Toy::class.java, Cat::class.java)
    TestInputsGenerator.pathWithClasses(input.toPath(), classes)
    val dexingTransform =
      TestDexingTransform(
        FakeGradleProvider(FakeGradleDirectory(input)),
        parameters = TestDexingTransform.TestParameters(desugaring = true),
      )
    val outputs = FakeTransformOutputs(tmp)
    dexingTransform.transform(outputs)
  ...
  fun testDexingWithDesugaringBootclasspath() {
    ...
    TestInputsGenerator.pathWithClasses(bootclasspath.toPath(), listOf(Animal::class.java, CarbonForm::class.java, Toy::class.java))
    ...
    val dex = Dex(outputs.outputDirectory.resolve("${computeDexDirName(outputs.outputDirectory)}/classes.dex"))
    assertThat(dex).containsClassesIn(classes.map { Type.getDescriptor(it) })
    assertThat(dex.classes).hasSize(classes.size + 1)

    val synthesizedLambdas = dex.classes.keys.filter { it.contains("\$\$ExternalSyntheticLambda") }
    assertThat(synthesizedLambdas).hasSize(1)
  }
  ...
  fun testIncrementalDexingWithDesugaring() {
    ...
    val classes =
      listOf(InterfaceWithDefaultMethod::class.java, ClassUsingInterfaceWithDefaultMethod::class.java, StandAloneClass::class.java)
    TestInputsGenerator.pathWithClasses(input.toPath(), classes)
    var dexingTransform =
      TestDexingTransform(
        FakeGradleProvider(FakeGradleDirectory(input)),
    ...
    val synthesizedDefaultMethodClass =
      Type.getDescriptor(InterfaceWithDefaultMethod::class.java).replace("InterfaceWithDefaultMethod", "InterfaceWithDefaultMethod\$-CC")
    assertThat(dexClasses).containsExactlyElementsIn(classes.map { Type.getDescriptor(it) } + synthesizedDefaultMethodClass)

    val interfaceWithDefaultMethodClass = TestInputsGenerator.getPath(InterfaceWithDefaultMethod::class.java)
    val classUsingInterfaceWithDefaultMethodClass = TestInputsGenerator.getPath(ClassUsingInterfaceWithDefaultMethod::class.java)
    ...
    val interfaceWithDefaultMethodTimestampBefore =
      Files.getLastModifiedTime(outputs.outputDirectory.resolve(interfaceWithDefaultMethodDex).toPath())
    val classUsingInterfaceWithDefaultMethodTimestampBefore =
      Files.getLastModifiedTime(outputs.outputDirectory.resolve(classUsingInterfaceWithDefaultMethodDex).toPath())
    val standAloneClassTimestampBefore = Files.getLastModifiedTime(outputs.outputDirectory.resolve(standAloneClassDex).toPath())
    ...
    val interfaceWithDefaultMethodTimestampAfter =
      Files.getLastModifiedTime(outputs.outputDirectory.resolve(interfaceWithDefaultMethodDex).toPath())
    val classUsingInterfaceWithDefaultMethodTimestampAfter =
      Files.getLastModifiedTime(outputs.outputDirectory.resolve(classUsingInterfaceWithDefaultMethodDex).toPath())
    val standAloneClassTimestampAfter = Files.getLastModifiedTime(outputs.outputDirectory.resolve(standAloneClassDex).toPath())

    assertNotEquals(interfaceWithDefaultMethodTimestampBefore, interfaceWithDefaultMethodTimestampAfter)
    assertNotEquals(classUsingInterfaceWithDefaultMethodTimestampBefore, classUsingInterfaceWithDefaultMethodTimestampAfter)
