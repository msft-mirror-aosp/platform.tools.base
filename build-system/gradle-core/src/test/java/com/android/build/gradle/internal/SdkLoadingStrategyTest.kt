...
class SdkLoadingStrategyTest {
  private data class SystemImageInfo(val api: Int, val vendor: String, val abi: String, val directory: String) {
  ...
  private fun getSystemImageXml(systemImageInfo: SystemImageInfo) =
    """
        <?xml version="1.0" encoding="UTF-8" standalone="yes"?>
        <ns2:repository
            xmlns:ns2="http://schemas.android.com/repository/android/common/01"
  ...
  fun directLoad_PlatformWithNonBaseExtension() {
    ...
    configureSdkDirectory(platformDirectory = "android-28-ext2", extensionLevel = 2, isBaseExtension = "false")
  ...
  fun directLoad_PlatformWithBaseExtension() {
    ...
    configureSdkDirectory(platformDirectory = "android-28", extensionLevel = 2, isBaseExtension = "true")

    val directLoader = getDirectLoader(platformHash = "android-28")
    assertThat(directLoader.loadedSuccessfully()).isTrue()
    assertAllComponentsArePresent(directLoader, platformHash = "android-28", expectedAndroidVersion = AndroidVersion(28, null, 2, true))
  }
  ...
  fun directLoad_missingSupportTools_apiGreaterThan16() {
    configureSdkDirectory(platformDirectory = "android-16", platformApiLevel = 16, configureSupportTools = false)
  ...
  fun directLoad_missingSupportTools_apiLessThan16() {
    configureSdkDirectory(platformDirectory = "android-15", platformApiLevel = 15, configureSupportTools = false)
  ...
  fun directLoad_missingSystemImage() {
    ...
    assertThat(directLoader.getSystemImageLibFolder("system-images;android-31;google-apis-playstore;x86")).isNull()
  ...
  fun directLoad_systemImage() {
    configureSdkDirectory(
      systemImageInfos =
        listOf(
          SystemImageInfo(30, "google-apis-playstore", "x86", "system-images/android-31/google-apis-playstore/x86"),
          SystemImageInfo(30, "default", "x86", "system-images/android-31/default/x86"),
        )
    )

    val directLoader = getDirectLoader()

    assertThat(directLoader.getSystemImageLibFolder("system-images;android-31;google-apis-playstore;x86")).isNotNull()
    assertThat(directLoader.getSystemImageLibFolder("system-images;android-31;default;x86")).isNotNull()
    assertThat(directLoader.getSystemImageLibFolder("system-images;android-28;default;x86")).isNull()
  }

  @Test
  fun directLoad_cacheHitSystemImage() {
    configureSdkDirectory(
      systemImageInfos = listOf(SystemImageInfo(30, "google-apis-playstore", "x86", "system-images/android-31/google-apis-playstore/x86"))
    )
  ...
  fun fullLoad_getTargetPlatformVersion_noExtension() {
    configureSdkDirectory()
    val fullLoader =
      SdkFullLoadingStrategy(sdkHandler, "android-28", Revision.parseRevision(SdkConstants.CURRENT_BUILD_TOOLS_VERSION), useAndroidX = true)
  ...
  private fun getDirectLoader(
    ...
    buildTools: String = SdkConstants.CURRENT_BUILD_TOOLS_VERSION,
  ...
  private fun configureSdkDirectory(
    ...
    if (configurePlatform) {
      ...
      val platformText =
        if (extensionLevel == null) getPlatformXml(platformApiLevel)
        else getPlatformWithExtensionsXml(apiLevel = platformApiLevel, extensionLevel = extensionLevel, isBaseExtension = isBaseExtension)
    ...
    if (configureBuildTools) {
      ...
      val buildToolInfo =
        BuildToolInfo.fromStandardDirectoryLayout(ToolsRevisionUtils.DEFAULT_BUILD_TOOLS_REVISION, buildToolsRoot.toPath())
    ...
    if (configureSystemImages) {
      for (info in systemImageInfos) {
        val systemImageRoot = sdkDir.resolve(info.directory)
  ...
  private fun assertAllComponentsArePresent(
    ...
    assertThat(sdkDirectLoadingStrategy.getAdbExecutable()).isEqualTo(sdkRoot.resolve("platform-tools/${SdkConstants.FN_ADB}"))

    assertThat(sdkDirectLoadingStrategy.getAnnotationsJar()).isEqualTo(sdkRoot.resolve("tools/support/${SdkConstants.FN_ANNOTATIONS_JAR}"))

    assertThat(sdkDirectLoadingStrategy.getAidlFramework())
      .isEqualTo(sdkRoot.resolve("platforms/$platformHash/${SdkConstants.FN_FRAMEWORK_AIDL}"))
    assertThat(sdkDirectLoadingStrategy.getAndroidJar())
      .isEqualTo(sdkRoot.resolve("platforms/$platformHash/${SdkConstants.FN_FRAMEWORK_LIBRARY}"))
    ...
    assertThat(sdkDirectLoadingStrategy.getTargetBootClasspath())
      .containsExactly(sdkRoot.resolve("platforms/$platformHash/${SdkConstants.FN_FRAMEWORK_LIBRARY}"))
    if (AndroidTargetHash.getVersionFromHash(platformHash).isAtLeast(30)) {
      assertThat(sdkDirectLoadingStrategy.getCoreForSystemModulesJar())
        .isEqualTo(sdkRoot.resolve("platforms/$platformHash/$FN_CORE_FOR_SYSTEM_MODULES"))
    }
    if (AndroidTargetHash.getVersionFromHash(platformHash).isAtLeast(26)) {
      assertThat(sdkDirectLoadingStrategy.getApiVersionsFile()).isEqualTo(sdkRoot.resolve("platforms/$platformHash/data/api-versions.xml"))
    } else {
    ...
    assertThat(sdkDirectLoadingStrategy.getBuildToolsRevision()).isEqualTo(ToolsRevisionUtils.MIN_BUILD_TOOLS_REV)
    assertThat(sdkDirectLoadingStrategy.getAidlExecutable()).isEqualTo(buildToolDirectory.resolve(SdkConstants.FN_AIDL))
    assertThat(sdkDirectLoadingStrategy.getCoreLambaStubs()).isEqualTo(buildToolDirectory.resolve(SdkConstants.FN_CORE_LAMBDA_STUBS))

    assertThat(sdkDirectLoadingStrategy.getRenderScriptSupportJar())
      .isEqualTo(RenderScriptProcessor.getSupportJar(buildToolDirectory, true))
    assertThat(sdkDirectLoadingStrategy.getSupportNativeLibFolder())
      .isEqualTo(RenderScriptProcessor.getSupportNativeLibFolder(buildToolDirectory))
    assertThat(sdkDirectLoadingStrategy.getSupportBlasLibFolder())
      .isEqualTo(RenderScriptProcessor.getSupportBlasLibFolder(buildToolDirectory))
  }
  ...
  private fun getExpectedOptionalJars(platformHash: String): List<Path> {
    ...
    return listOf("org.apache.http.legacy.jar", "android.test.mock.jar", "android.test.base.jar", "android.test.runner.jar").map {
      optionalDir.resolve(it)
    }
  }
}
