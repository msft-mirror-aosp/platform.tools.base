...
class FeatureSetMetadataWriterTaskTest(val minSdkVersion: Int) {
  ...
  companion object {
    ...
    fun getParameters(): Collection<Array<Any>> {
      return listOf(arrayOf<Any>(AndroidVersion.VersionCodes.LOLLIPOP), arrayOf<Any>(AndroidVersion.VersionCodes.O))
    }
  }
  ...
  fun testTask() {
    ...
    object : FeatureSetMetadataWriterTask.FeatureSetRunnable() {
        override fun getParameters(): FeatureSetMetadataWriterTask.Params {
          return object : FeatureSetMetadataWriterTask.Params() {
            override val featureFiles = FakeObjectFactory.factory.fileCollection().from(inputFiles)
            override val minSdkVersion = FakeGradleProperty(this@FeatureSetMetadataWriterTaskTest.minSdkVersion)
            override val maxNumberOfFeaturesBeforeOreo = FakeGradleProperty(FeatureSetMetadata.MAX_NUMBER_OF_SPLITS_BEFORE_O)
            override val outputFile = FakeObjectFactory.factory.fileProperty().fileValue(outputLocation)
    ...
    for (i in 0..4) {
      assertThat(loaded.getResOffsetFor("id_$i"))
        .isEqualTo(
          if (minSdkVersion < AndroidVersion.VersionCodes.O) FeatureSetMetadata.BASE_ID - i - 1 else FeatureSetMetadata.BASE_ID + i + 1
        )
    }
  }
