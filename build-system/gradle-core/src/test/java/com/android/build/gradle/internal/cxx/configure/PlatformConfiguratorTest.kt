...
class PlatformConfiguratorTest {
  ...
  private fun findSuitablePlatformVersion(
    ...
    val androidVersion =
      if (minSdkVersion == null && codeName == null) {
        null
      } else {
  ...
  fun testPlatformJustFoundNdk16() {
    ...
    val platform = findSuitablePlatformVersion(configurator, "x86", 27, null)
  ...
  fun testNdkPlatformFallbackNdk16() {
    val configurator = platformConfiguratorNdk16()
    val platform = findSuitablePlatformVersion(configurator, "x86", 28, null)
    assertThat(platform).isEqualTo(27)
    assertThat(logger.warnings)
      .containsExactly("C/C++: Platform version " + "28 is beyond 27, the maximum API level supported by this NDK. Using 27 instead.")
  }

  @Test
  fun testPlatformTooLowClimbToMinimumNdk16() {
    val configurator = platformConfiguratorNdk16()
    val platform = findSuitablePlatformVersion(configurator, "x86", 13, null)
    assertThat(platform).isEqualTo(14)
    assertThat(logger.messageCount).isEqualTo(2)
    assertThat(logger.errors.single())
      .isEqualTo(
  ...
  fun testNdkPlatformFallbackNdk17() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 29, null)
    assertThat(platform).isEqualTo(28)
    assertThat(logger.warnings)
      .containsExactly("C/C++: Platform version 29 " + "is beyond 28, the maximum API level supported by this NDK. Using 28 instead.")
  }
  ...
  fun testPlatformFoundByCodeNdk17() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, "P")
    assertThat(platform).isEqualTo(28)
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='P' is mapped to '28'.")
  }

  @Test
  fun testAliasInMinSdkVersionPositionNdk17() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 20, null)
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='20' " + "is mapped to '19'.")
  ...
  fun testPlatformUnknownMrCodeNdk17() {
    ...
    assertThat(logger.errors)
      .containsExactly("${NDK_DOES_NOT_SUPPORT_API_LEVEL.codeText} API codeName 'O-MR2' is not supported by NDK '$ndk17'.")
  }

  @Test
  fun testNoVersionSpecifiedNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, null, expectedNdkR17MetaPlatforms())
    assertThat(platform).isEqualTo(16)
    assertThat(logger.infos)
      .containsExactly("C/C++: Neither codeName nor " + "minSdkVersion specified. Using minimum platform version for 'x86'.")
  }

  @Test
  fun testPlatformJustFoundNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 28, null, expectedNdkR17MetaPlatforms())
    assertThat(platform).isEqualTo(28)
    assertThat(logger.messageCount).isEqualTo(0)
  }
  ...
  fun testNdkPlatformFallbackNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 29, null, expectedNdkR17MetaPlatforms())
    assertThat(platform).isEqualTo(28)
    assertThat(logger.warnings)
      .containsExactly("C/C++: Platform version 29 " + "is beyond 28, the maximum API level supported by this NDK. Using 28 instead.")
  }
  ...
  fun testPlatformTooLowClimbToMinimumNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform =
      findSuitablePlatformVersion(configurator, abiName = "x86", minSdkVersion = 13, codeName = null, expectedNdkR17MetaPlatforms())
  ...
  fun `Bug 310718265 platform too low becomes error after min SDK version 19`() {
    ...
    val platform =
      findSuitablePlatformVersion(configurator, abiName = "x86", minSdkVersion = 19, codeName = null, expectedNdkR26MetaPlatforms())
    assertThat(platform).isEqualTo(19)
    assertThat(logger.messageCount).isEqualTo(1)
    assertThat(logger.errors.single())
      .isEqualTo(
  ...
  fun `Bug 310718265 ignoreMinSdkVersionFromDsl supersedes ignoreMinSdkVersionFromProperty`() {
    ...
    assertThat(platform).isEqualTo(19)
    assertThat(logger.messageCount).isEqualTo(1)
    assertThat(logger.infos.single())
      .isEqualTo(
  ...
  fun `Bug 310718265 experimental property not convertible to int`() {
    ...
    assertThat(logger.errors.single())
      .isEqualTo(
        "[CXX1111] Value \"Not an Int\" of android.ndk.suppressMinSdkVersionError was " + "not convertible to integer and will be ignored."
      )
  }
  ...
  fun testPlatformFoundByCodeNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, "P", expectedNdkR17MetaPlatforms())
    assertThat(platform).isEqualTo(28)
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='P' is mapped to '28'.")
  }
  ...
  fun testAliasInMinSdkVersionPositionNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 20, null, expectedNdkR17MetaPlatforms())
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='20' " + "is mapped to '19'.")
    assertThat(platform).isEqualTo(19)
  }
  ...
  fun testPlatformMrCodeNdk17PlatformsMeta() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, "O-MR1", expectedNdkR17MetaPlatforms())
    assertThat(platform).isEqualTo(27)
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='O-MR1' is mapped to '27'.")
  }
  ...
  fun testWeirdABI() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "bob", 13, null)
    assertThat(platform).isEqualTo(AndroidVersion.MIN_RECOMMENDED_API)
    assertThat(logger.errors).containsExactly("${ABI_IS_INVALID.codeText} Specified abi='bob' " + "is not recognized.")
  }

  @Test
  fun testBothMinSdkAndCodeNameAgree() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 28, "P")
    assertThat(platform).isEqualTo(28)
    assertThat(logger.infos).containsExactly("C/C++: Version minSdkVersion='P' is mapped to '28'.")
    assertThat(logger.warnings)
      .containsExactly("C/C++: Both codeName and minSdkVersion specified. They agree but only " + "one should be specified.")
  }

  @Test
  fun testBothMinSdkAndCodeNameDisagree() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", 27, "P")
    assertThat(logger.infos)
      .containsExactly(
        "C/C++: Version minSdkVersion='P' is mapped to '28'.",
        "C/C++: Disagreement between codeName='P' and minSdkVersion='27'. Probably a preview " + "release. Using 28 to match code name.",
      )
    assertThat(platform).isEqualTo(28)
  }
  ...
  fun testVeryOldCodenameGetsPromoted() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, "J")
    assertThat(platform).isEqualTo(16)
    assertThat(logger.infos).containsExactly("C/C++: Version " + "minSdkVersion='J' is mapped to '16'.")
  }

  @Test
  fun testNotYetKnownCodenameFallsBackToMaximumForNdk() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", defaultApiLevelFromDsl, "Z")
    assertThat(platform).isEqualTo(28)
    assertThat(logger.errors)
      .containsExactly("${NDK_DOES_NOT_SUPPORT_API_LEVEL.codeText} API codeName 'Z' is not supported by NDK '$ndk17'.")
  }

  @Test
  fun testEmptyVersionInfoInBuildGradle() {
    val configurator = platformConfiguratorNdk17()
    val platform = findSuitablePlatformVersion(configurator, "x86", null, null)
  ...
  fun testAgainstIncompletePlatformsFolder() {
    ...
    val platform = findSuitablePlatformVersion(configurator, "x86", null, null)
    assertThat(platform).isEqualTo(19)
    assertThat(logger.warnings).containsExactly("C/C++: Expected platform " + "folder platforms/android-22, using platform API 19 instead.")
  }
}
