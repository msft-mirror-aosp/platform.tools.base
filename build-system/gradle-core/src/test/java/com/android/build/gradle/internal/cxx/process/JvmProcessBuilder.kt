...
/** Test utility class that starts Java.executableExtension and calls back into the main function of the given [KClass]. */
class JvmProcessBuilder(clazz: KClass<*>) {
  ...
  companion object {
    private enum class Os(val executableExtension: String, val classPathSeparator: String) {
      WINDOWS(".exe", ";"),
      LINUX("", ":"),
      DARWIN("", ":"),
    }
    ...
    private fun jarUrlToFile(url: String): String {
      val result = tryJarUrlToFile(url) ?: throw RuntimeException("Invalid URL $url")
      val full = result.absolutePath
      require(File(full).exists())
      return full
    }

    private fun javaExeFolder() = File(System.getProperties().getProperty("java.home"), "bin")

    private fun javaExeBase() = "java" + os.executableExtension

    private fun javaExe(): File {
      val java = File(javaExeFolder(), javaExeBase())
      if (!java.isFile) {
