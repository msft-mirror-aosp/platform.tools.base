 ...
 * - It is not suitable for integration test because Gradle expect all implementation of FileCollection to also implement
...
open class FakeFileCollection(vararg collection: Any?) : FileCollection {
      ...
      null -> throw NullPointerException("Null object found in FakeFileCollection")
        ...
        throw IllegalStateException(
          """FakeFileCollection can only resolve object of type:
                         ...
                         |    Iterable<File>
                         |    Provider
                         |It cannot resolve '$obj' of type ${obj.javaClass}"""
            .trimMargin()
        )
    }
  }
  ...
  override fun getAsFileTree(): FileTree {
    ...
    files.forEach { dir -> dir.walk().filter { it.isFile }.forEach { allFiles.add(it) } }
  ...
  override fun getSingleFile(): File {
    return if (files.size == 1) first()
    else throw IllegalStateException("Failed to get single file.  File collection contains ${files.size} " + "files: $files")
  }
