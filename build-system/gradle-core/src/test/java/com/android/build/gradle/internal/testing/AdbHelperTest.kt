...
class AdbHelperTest {
  ...
  fun setup() {
    ...
    adbFilePath =
      FileOpUtils.toFile(adbExecutable)
        .also {
          it.writeText("")
  ...
  fun isBootCompleted_checksBothDevAndSys() {
    ...
    assertThat(processCalls[0]).isEqualTo(listOf(adbFilePath, "-s", "deviceSerial", "shell", "getprop", "sys.boot_completed"))
    assertThat(processCalls[1]).isEqualTo(listOf(adbFilePath, "-s", "deviceSerial", "shell", "getprop", "dev.bootcomplete"))
  }
  ...
  fun isPackageManagerStarted_pmReady() {
    ...
    assertThat(processCalls).hasSize(1)
    assertThat(processCalls[0]).isEqualTo(listOf(adbFilePath, "-s", "deviceSerial", "shell", "/system/bin/pm", "path", "android"))
  }
  ...
  fun findDeviceSerialWithID_findsSerial() {
    ...
    assertThat(adbHelper.findDeviceSerialWithId("SomeDeviceAndroidDebugTest")).isEqualTo("testDevice3")

    assertThat(processCalls).hasSize(3)
    assertThat(processCalls[0]).isEqualTo(listOf(adbFilePath, "devices"))
    assertThat(processCalls[1]).isEqualTo(listOf(adbFilePath, "-s", "testDevice1", "emu", "avd", "id"))
    // testDevice2 is skipped as it is offline
    assertThat(processCalls[2]).isEqualTo(listOf(adbFilePath, "-s", "testDevice3", "emu", "avd", "id"))
  ...
  fun killDevice() {
    ...
    assertThat(processCalls).hasSize(1)
    assertThat(processCalls[0]).isEqualTo(listOf(adbFilePath, "-s", "testSerial", "emu", "kill"))
  }
        ...
        mock<ProcessBuilder>().apply {
          whenever(start()).then {
            ...
            mock<Process>(defaultAnswer = Answers.RETURNS_DEEP_STUBS).apply {
              ...
              whenever(inputStream).thenReturn(ByteArrayInputStream(processOutput.toByteArray()))
              whenever(errorStream).thenReturn(ByteArrayInputStream("".toByteArray()))
            }
          }
        }
      }
