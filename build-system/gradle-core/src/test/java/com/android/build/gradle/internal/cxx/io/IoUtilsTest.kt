...
class IoUtilsTest {
  ...
  fun `two files that both don't exist are the same`() {
    ...
    file2.delete()
    assertThat(compareFileContents(file1, file2)).isEqualTo(SAME_SOURCE_AND_DESTINATION_DID_NOT_EXIST)
  }
  ...
  fun `when files have equivalent paths according to canonical path they are the same`() {
    ...
    assertThat(compareFileContents(file1, file2)).isEqualTo(SAME_PATH_ACCORDING_TO_CANONICAL_PATH)
  }
  ...
  fun `one file hard linked to another is the same without comparing contents`() {
    ...
    assertThat(compareFileContents(source, destination)).isEqualTo(SAME_PATH_ACCORDING_TO_FILE_SYSTEM_PROVIDER)
  }
  ...
  fun `two files with the same content are the same`() {
    ...
    synchronizeFile(source, destination)
    assertThat(compareFileContents(source, destination)).isEqualTo(SAME_CONTENT)
  }
  ...
  fun `changing source file changes linked file`() {
    ...
    // Expect a hard link
    assertThat(compareFileContents(file1, file2)).isEqualTo(SAME_PATH_ACCORDING_TO_FILE_SYSTEM_PROVIDER)
  ...
  fun `check removeDuplicateFiles`() {
    ...
    assertThat(removeDuplicateFiles(listOf(file1, file2, sameContentAsFile1))).isEqualTo(listOf(file1, file2))
    assertThat(removeDuplicateFiles(listOf(file1, sameContentAsFile1, file2))).isEqualTo(listOf(file1, file2))
  }
}
