...
class AidlCompileTest {
  ...
  fun testAidlCompileRunnable() {
    ...
    aidlCompileDelegate(workers, fakeExe, fakeFramework, outputDir, null, null, listOf(sourceFolder), listOf(), instantiatorTask)
    ...
    for (processInfo in execOperations.capturedExecutions) {
      ...
      Truth.assertThat(processInfo.args).containsAtLeast("-p" + fakeFramework.canonicalPath, "-o" + outputDir.absolutePath)

      Truth.assertThat(processInfo.args).containsAnyOf(file1.absolutePath, file2.absolutePath, file3.absolutePath)
