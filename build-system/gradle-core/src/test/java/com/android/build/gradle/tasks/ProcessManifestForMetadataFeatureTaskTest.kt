...
class ProcessManifestForMetadataFeatureTaskTest {
  ...
  abstract class ProcessManifestForMetadataFeatureTaskForTest @Inject constructor(testWorkerExecutor: WorkerExecutor) :
  ...
  fun setUp() {
    ...
    whenever(variantOutputConfiguration.outputType).thenReturn(VariantOutputConfiguration.OutputType.SINGLE)
  ...
  fun testNonDynamicFeatureModule() {
    ...
    Truth.assertThat(task.metadataFeatureManifest.get().asFile.readText(Charsets.UTF_8)).isEqualTo("Some content")
  }
  ...
  fun testFeatureMetadataManifestForDynamicModule() {
    val sourceManifest =
      File(sourceManifestFolder, "AndroidManifest.xml").also {
        it.writeText(
          """
          <manifest
    ...
    val xmlDocument = PositionXmlParser.parse(task.metadataFeatureManifest.get().asFile.readText(Charsets.UTF_8))

    Truth.assertThat(xmlDocument.documentElement.getAttribute(SdkConstants.ATTR_FEATURE_SPLIT)).isEqualTo("feature")
    ...
    Truth.assertThat(
        xmlDocument
          ...
          .item(0)
          .attributes
          .getNamedItemNS(SdkConstants.ANDROID_URI, SdkConstants.ATTR_TARGET_SDK_VERSION)
          .nodeValue
      )
      .isEqualTo("27")
    ...
    Truth.assertThat(xmlDocument.getElementsByTagName(SdkConstants.TAG_USES_SPLIT).length).isEqualTo(0)

    Truth.assertThat(xmlDocument.documentElement.getAttribute(ATTR_PACKAGE)).isEqualTo(expectedNamespace)
  }
