...
class BuildSystemCommandLineKtTest {
  ...
  fun `MSBuild check define property -p`() {
    ...
    assertThat(parsed).containsExactly(DefineMultiProperty("-p:Platform=Android-x86", mapOf("Platform" to "Android-x86")))
  ...
  fun `MSBuild check define property -property`() {
    ...
    assertThat(parsed).containsExactly(DefineMultiProperty("-property:Platform=Android-x86", mapOf("Platform" to "Android-x86")))
    assertThat(parsed.getMSBuildProperty(MSBuildProperty.Platform)).isEqualTo("Android-x86")
  }
  ...
  fun `ndk-build check subsumed doesn't removed additive`() {
    ...
    assertThat(parsed).containsExactly(NdkBuildAppendProperty("X+=Y", "X", "Y"), NdkBuildAppendProperty("X+=Z", "X", "Z"))
  ...
  fun cmakeCheckDefine() {
    ...
    assertThat(parsed).containsExactly(DefineProperty("-DX=Y", "X", "Y"))
  ...
  fun cmakeCheckCmakeListsPath() {
    ...
    assertThat(parsed).containsExactly(CmakeListsPath("-H<path-to-cmakelists>", "<path-to-cmakelists>"))
  ...
  fun cmakeCheckBinaryOutputPath() {
    ...
    assertThat(parsed).containsExactly(CmakeBinaryOutputPath("-B<path-to-binary>", "<path-to-binary>"))
  ...
  fun cmakeCheckGeneratorName() {
    ...
    assertThat(parsed).containsExactly(CmakeGeneratorName("-GAndroid Gradle - Ninja", "Android Gradle - Ninja"))
  ...
  fun cmakeCheckUnknownArgument() {
    ...
    assertThat(parsed).containsExactly(UnknownArgument("-X"))
  ...
  fun convertCmakeCommandLineArgumentsToStringList() {
    val arguments = listOf(DefineProperty.from(CMAKE_ANDROID_NDK, "ndk"))
    assertThat(arguments.toStringList()).isEqualTo(listOf("-D$CMAKE_ANDROID_NDK=ndk"))
  }
  ...
  fun `CMake keep CMake Server arguments`() {
    val arguments = parseCmakeCommandLine("-H/usr/path -G Ninja -DX=Y").onlyKeepCmakeServerArguments()
  ...
  fun `CMake remove subsumed properties`() {
    val arguments = parseCmakeCommandLine("-D X=1 -DX=2").removeSubsumedArguments()
  ...
  fun `CMake remove subsumed generator`() {
    val arguments = parseCmakeCommandLine("-GA -GB").removeSubsumedArguments()
  ...
  fun `CMake remove blank properties`() {
    val arguments = parseCmakeCommandLine("-DX=\"\"").removeBlankProperties()
    assertThat(arguments).hasSize(0)
  }

  @Test
  fun `CMake property with quotes gets unquoted`() {
    val arguments = parseCmakeCommandLine("-DX=\"1\" -D Y=\"2\"")
