...
class ImagePanelRenderer(private val isRecordGolden: Boolean) : ReportRenderer<ScreenshotTestImages, SimpleHtmlWriter>() {
  ...
  private fun renderImage(htmlWriter: SimpleHtmlWriter, imagePath: String, text: String) {
    ...
    else htmlWriter.startElement(("td")).characters(text).endElement()
  }
  ...
  private fun getTexts(ssImages: ScreenshotTestImages): ImageTexts {
    ...
    if (ssImages.golden.path.isEmpty()) goldenText = ssImages.golden.message
    if (ssImages.actual != null && ssImages.actual.path.isEmpty()) actualText = ssImages.actual.message
    if (ssImages.diff != null && ssImages.diff.path.isEmpty()) diffText = ssImages.diff.message
