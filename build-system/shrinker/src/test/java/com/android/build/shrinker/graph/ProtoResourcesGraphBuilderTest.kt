...
import com.android.build.shrinker.util.addAttributeWithRefNameOnly
...
class ProtoResourcesGraphBuilderTest {
  ...
  companion object {
    ...
    fun ResourceShrinkerModel.referencesWithPackage(resourceId: Int) =
      resourceStore.getResource(resourceId)?.references?.map { "${it.packageName}:${it.name}" } ?: emptyList()
  }
  ...
  fun `find simple references between resources`() {
    ...
    (1..4).forEach { i -> model.addResource(STRING, PACKAGE_NAME, "string$i", i) }
    ...
    ProtoResourcesGraphBuilder(temporaryFolder.root.toPath(), table.toPath()).buildGraph(model)
  ...
  fun `find simple references between resources in multi-modules project`() {
    ...
    assertThat(model.referencesWithPackage(2)).containsExactly("com.test:string1", "com.test.feature:string1")
  ...
  fun `find references from array`() {
    ...
    val table = createResourceTable {
      ...
      it.addType(1, "array", arrayEntry(0, "string-array", listOf("inlined"), listOf(3, 1)))
    }
  ...
  fun `find references from plurals`() {
    ...
    val table = createResourceTable {
      it.addType(0, "string", stringEntry(1, "string1", "Hello world"), stringEntry(2, "string2", "Bye-bye"))
      it.addType(1, "plurals", pluralsEntry(0, "my_plurals", "Zero", 2, "Two"))
    }
  ...
  fun `find references from external file in compiled XML format, skip ID resources`() {
    ...
    val table = createResourceTable {
      it.addType(0, "string", stringEntry(1, "string1", "Hello world"), stringEntry(2, "string.bye", "Bye-bye"))
      it.addType(2, "menu", xmlFile(1, "my_menu", "res/menu/my_menu.xml"))
    }
    xmlElement("menu")
      .addNamespace("android", ANDROID_NS)
      .addChild(
        xmlElement("item").addAttribute("id", ANDROID_NS, "@id/menu_item", 0x10001).addAttribute("title", ANDROID_NS, "@string/string1", 1)
      )
      .addChild(xmlElement("item").addAttributeWithRefNameOnly("title", refName = "string/string.bye"))
      .writeToFile(temporaryFolder.root.toPath().resolve("res/menu/my_menu.xml"))

    ProtoResourcesGraphBuilder(temporaryFolder.root.toPath().resolve("res"), table.toPath()).buildGraph(model)
  ...
  fun `find inlined android_res references inside XML resources`() {
    ...
    val table = createResourceTable {
      it.addType(1, "xml", xmlFile(1, "some_xml", "res/xml/some_xml.xml"))
      it.addType(2, "menu", xmlFile(1, "menu_xml", "res/xml/menu_xml.xml"))
    }

    ProtoResourcesGraphBuilder(rootPath.resolve("res"), table.toPath()).buildGraph(model)

    assertThat(model.referencesFor(0x10001)).containsExactly("drawable_1", "drawable_2", "drawable_3")
  ...
  fun `skip external files which are not found in res directory`() {
    ...
    assertThat(messages).containsExactly("File 'res/raw/html.html' can not be processed. Skipping.")
  }
  ...
  fun test_constraint_referenced_ids() {
    ...
    val table = createResourceTable {
      it.addType(0, "id", idEntry(1, "constraint_referenced_id"), idEntry(2, "android_id_only"))
      it.addType(1, "menu", xmlFile(1, "my_menu", "res/menu/my_menu.xml"))
    }
    xmlElement("menu")
      .addNamespace("android", ANDROID_NS)
      .addChild(xmlElement("item1").addAttribute("constraint_referenced_ids", value = "constraint_referenced_id1"))
      .addChild(
        xmlElement("item1").addAttribute("constraint_referenced_ids", value = "constraint_referenced_id2,constraint_referenced_id3")
      )
      .addChild(
        xmlElement("item1")
          // Ensure that we support spaces in the list
          .addAttribute("constraint_referenced_ids", value = "constraint_referenced_id4 ,  constraint_referenced_id5")
      )
      .writeToFile(temporaryFolder.root.toPath().resolve("res/menu/my_menu.xml"))
  ...
  fun createResourceTable(fn: (Package.Builder) -> Package.Builder): File {
    ...
    val resourceTable =
      fn(Package.newBuilder().setPackageId(PackageId.newBuilder().setId(0)).setPackageName(PACKAGE_NAME)).buildResourceTable()
