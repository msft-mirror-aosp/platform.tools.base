...
class Aapt2DaemonImplInternalErrorHandlingTest {
  ...
  private fun createCommand(implementationClass: KClass<*>) =
    listOf<String>(
  ...
  fun testBinaryThatExitsBeforeReady() {
    val daemon =
      Aapt2DaemonImpl(
        ...
        daemonTimeouts = Aapt2DaemonTimeouts(start = 10, startUnit = TimeUnit.SECONDS, stop = 10, stopUnit = TimeUnit.SECONDS),
        logger = logger,
      )
    ...
    assertThat(exception.message)
      .isEqualTo(
        "AAPT2 fake_version Daemon 'Aapt2DaemonImplTest.testBinaryThatExitsBeforeReady': $message\n" +
    ...
    assertThat(exception.message)
      .isEqualTo(
        "AAPT2 fake_version Daemon " +
          ...
          "Unexpected error during compile 'values" +
          File.separatorChar +
          "does_not_matter.xml', " +
  ...
  fun testBinaryThatHangsBeforeReady() {
    val daemon =
      Aapt2DaemonImpl(
        ...
        versionString = "fake_version",
        daemonTimeouts = Aapt2DaemonTimeouts(start = 1, startUnit = TimeUnit.NANOSECONDS, stop = 1, stopUnit = TimeUnit.NANOSECONDS),
        logger = logger,
      )
  ...
  private fun testCrashesDuringInteraction(action: (Aapt2Daemon) -> Unit) {
    ...
    val exception = assertFailsWith(Aapt2InternalException::class) { action(daemon) }
