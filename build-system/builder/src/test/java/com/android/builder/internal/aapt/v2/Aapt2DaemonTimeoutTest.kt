...
class Aapt2DaemonTimeoutTest {
  ...
  fun testLinkTimeout() {
    ...
    val daemon = CompileLinkTimeoutAapt2Daemon(name = testName.methodName)
    val exception = assertFailsWith(Aapt2InternalException::class) { daemon.link(request, logger) }
  ...
  fun testConvertTimeout() {
    ...
    val daemon = CompileLinkTimeoutAapt2Daemon(name = testName.methodName)
    val exception = assertFailsWith(Aapt2InternalException::class) { daemon.convert(request, logger) }
  ...
  fun testShutdownTimeout() {
    ...
    daemon.shutDown()
    assertThat(daemon.state).isEqualTo(Aapt2Daemon.State.SHUTDOWN)
    assertThat(logger.messages).contains("E testShutdownTimeout Shutdown Timeout AAPT Daemon " + "Failed to shutdown within timeout")
  }
