...
class ZipEntryListTest {
  ...
  fun `deserialization of an empty file should throw exception`() {
    assertFailsWith<EOFException> {
      val file = File(tempRoot, "test").apply { createNewFile() }
  ...
  fun `zip entry list creation from a zip file matches expected content`() {
    val zip = ZipTestTool.createZipFile(tempRoot.absolutePath + "/a.zip", "item.txt", "content")
  ...
  private fun ZipEntryList.dump(): String {
    return entries.entries.toList().flatMap { (path, entry) -> listOf(path, entry.name, entry.size, entry.crc) }.joinToString()
  }

  private fun File.dump(): String {
    return ZipMap.from(toPath())
      .entries
      .entries
      .toList()
      .flatMap { (path, entry) -> listOf(path, entry.name, entry.uncompressedSize, entry.crc) }
      .joinToString()
  }
  ...
  companion object {
    ...
    val TEST_ENTRY_LIST =
      ZipEntryList(mapOf("a" to ZipEntry("a.txt", 1L, 1L), "b" to ZipEntry("b.txt", 1000L, 10L), "c" to ZipEntry("c.txt", 2000L, 20L)))
  }
}
