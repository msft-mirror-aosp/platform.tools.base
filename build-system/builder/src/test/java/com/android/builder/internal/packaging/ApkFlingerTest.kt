...
class ApkFlingerTest {
  ...
  @Mock lateinit var creationData: ApkCreatorFactory.CreationData
  ...
  fun deleteFile() {
    ...
    assertThat(apkFile) {
      ...
      it.containsFileWithContent("foo/bar.txt", "foobar")
    }
  }
  ...
  fun writeUncompressedFile() {
    ...
    assertThat(apkFile) { it.exists() }
    val compressedSize = apkFile.length()

    // then write uncompressed file
    Mockito.`when`(creationData.noCompressPredicate).thenReturn(Predicate { it == "bigFile.txt" })
  ...
  fun writeLargeFile() {
    ...
    ApkFlinger(creationData, Deflater.BEST_SPEED).use { it.writeFile(largeFile, "largeFile.txt") }
    assertThat(ZipArchive.listEntries(apkFile.toPath()).keys).containsExactly("largeFile.txt")
  }

  @Test
  fun testV1Signing() {
    testSigning(enableV1 = true, enableV2 = false, enableV3 = false, enableV4 = false, minSdk = 1)
  }

  @Test
  fun testV2Signing() {
    // API 24 is the first to support V2 signing
    testSigning(enableV1 = false, enableV2 = true, enableV3 = false, enableV4 = false, minSdk = 24)
  }

  @Test
  fun testV3Signing() {
    // API 28 is the first to support V3 signing
    testSigning(enableV1 = false, enableV2 = false, enableV3 = true, enableV4 = false, minSdk = 28)
  }

  @Test
  fun testV4AndV2Signing() {
    // API 30 is the first to support V4 signing
    testSigning(enableV1 = false, enableV2 = true, enableV3 = false, enableV4 = true, minSdk = 30)
  }

  @Test
  fun testV4AndV3Signing() {
    // API 30 is the first to support V4 signing
    testSigning(enableV1 = false, enableV2 = false, enableV3 = true, enableV4 = true, minSdk = 30)
  }

  @Test
  fun testSigningWithAllVersions() {
    testSigning(enableV1 = true, enableV2 = true, enableV3 = true, enableV4 = true, minSdk = 1)
  }

  private fun testSigning(enableV1: Boolean, enableV2: Boolean, enableV3: Boolean, enableV4: Boolean, minSdk: Int) {
    ...
    assertThat(apkFile) {
      ...
      it.doesNotContain("META-INF/CERT.RSA")
      it.doesNotContain("META-INF/CERT.SF")
    }
    ...
    ApkFlinger(creationData, Deflater.BEST_SPEED, enableV3Signing = enableV3, enableV4Signing = enableV4).use {}
    if (enableV1) {
      assertThat(apkFile) {
        it.contains("META-INF/MANIFEST.MF")
        it.contains("META-INF/CERT.RSA")
        it.contains("META-INF/CERT.SF")
      }
    }
