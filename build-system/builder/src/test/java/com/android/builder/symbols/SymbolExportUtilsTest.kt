...
class SymbolExportUtilsTest {
  ...
  fun sanityCheckNamespacedRClass() {
    ...
    val depSymbolTable =
      SymbolTable.builder().tablePackage("com.example.lib").add(Symbol.createSymbol(ResourceType.STRING, "libstring", 0)).build()
    val librarySymbols =
      SymbolTable.builder().tablePackage("com.example.mylib").add(Symbol.createSymbol(ResourceType.STRING, "mystring", 0)).build()
    ...
    assertThat(symbolFileOut).hasContents("int string libstring 0x7f140001", "int string mystring 0x7f140002")
    ...
    assertThat(myProcessedSymbols.symbols.columnKeySet()).containsExactly("libstring", "mystring")
  ...
  fun testWriteSymbolListWithPackageName() {
    val librarySymbols =
      SymbolTable.builder()
        ...
        .add(Symbol.createStyleableSymbol("my_styleable", ImmutableList.of(), ImmutableList.of("child1", "child2")))
        .build()
    val os = ByteArrayOutputStream()
    os.writer().use { writer -> writeSymbolListWithPackageName(librarySymbols, writer) }
    assertThat(os.toString(Charsets.UTF_8.name()))
      .isEqualTo(
        """
        com.example.mylib
