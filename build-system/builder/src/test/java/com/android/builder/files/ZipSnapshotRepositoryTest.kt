...
class ZipSnapshotRepositoryTest {
    ...
    fun cannotRemoveWhatHasNotBeenAdded() {
        val exception = assertFailsWith<IllegalArgumentException> {
            repository.removeSnapshotOfZip(createZipA())
        }
    ...
    private fun createZip(name: String, fileCount: Int) = ZipTestTool.createZipFile(
    ...
    private fun ZipSnapshot.dump(): String {
        return entryList.entries.toList().joinToString { (name, entry) ->
            "$name, ${entry.name}, ${entry.crc}, ${entry.size}"
        }
    }
