...
class IssueReporterTest {
  ...
  fun `check that sync issues have valid type names and type values`() {
    ...
    for ((index, syncIssueField) in IssueReporter.Type::class.java.fields.withIndex()) {
      val syncIssue = syncIssueField.get(IssueReporter.Type::class.java) as IssueReporter.Type
      ...
      assert(expectedTypeName == actualTypeName) {
        "Expected type name $expectedTypeName" + " but found $actualTypeName for ${syncIssue.name}"
      }
      assert(expectedTypeValue == actualTypeValue) {
        "Expected type value $expectedTypeValue" + " but found $actualTypeValue for ${syncIssue.name}"
      }
    }
  }
}
