...
class Aapt2DaemonManagerTest {
  ...
  /** These tests are doing manual concurrency control. Add a timeout so they don't prevent other tests from running if they deadlock */
  @Rule @JvmField val timeout: Timeout = Timeout.seconds(120)
  ...
  fun testAbnormalShutdown() {
    ...
    daemonReady.acquire()
    val exception = assertFailsWith(IllegalStateException::class) { manager.shutdown() }
    daemonWait.release()
    thread.join()
    assertThat(exception).hasMessageThat().isEqualTo("AAPT Process manager cannot be shut down while daemons are in use")
    manager.shutdown()
  }
  ...
  fun testTimeoutHandling() {
    ...
    manager.leaseDaemon().use { daemon ->
      val exception =
        assertFailsWith(Aapt2InternalException::class) {
          daemon.compile(
            CompileResourceRequest(
              inputFile = File("in1"),
              outputDirectory = File("out1"),
  ...
  class TestAapt2Daemon(displayId: Int) : Aapt2Daemon("Test AAPT Daemon #$displayId", NoErrorsOrWarningsLogger()) {
  ...
  private object Listener : Aapt2DaemonManager.Listener {
    ...
    fun assertStartCountEquals(expected: Int) {
      assertThat(startCount.get()).named("Number of calls to firstDaemonStarted()").isEqualTo(expected)
    }

    fun assertStopCountEquals(expected: Int) {
      assertThat(stopCount.get()).named("Number of calls to lastDaemonStopped()").isEqualTo(expected)
    }
  }
}
