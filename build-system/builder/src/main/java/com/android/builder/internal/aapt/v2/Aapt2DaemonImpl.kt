...
class Aapt2DaemonImpl(
  ...
  logger: ILogger,
) : Aapt2Daemon(displayName = "AAPT2 $versionString Daemon $displayId", logger = logger) {
  ...
  constructor(
    ...
    aaptCommand = listOf(aaptExecutable.toFile().absolutePath, Aapt2DaemonUtil.DAEMON_MODE_COMMAND),
  ...
  private val processOutput =
    object : GrabProcessOutput.IProcessOutput {
      @Volatile var delegate: GrabProcessOutput.IProcessOutput = noOutputExpected
  ...
  override fun startProcess() {
    ...
    writer =
      try {
        GrabProcessOutput.grabProcessOutput(process, GrabProcessOutput.Wait.ASYNC, processOutput, null, null)
      ...
      try {
        ...
        stopQuietly(
          "Failed to start AAPT2 process $displayName. " +
            ...
            "${daemonTimeouts.startUnit.name.lowercase(Locale.US)}.",
          e,
        )
      } catch (e: Exception) {
    ...
    if (!ready) {
      stopQuietly("Failed to start AAPT2 process.", IOException("Process unexpectedly exit."))
    }
    // Process is ready
    ...
    try {
      ...
      when (val result = waitForTask.future.get(daemonTimeouts.link, daemonTimeouts.linkUnit)) {
        ...
        is WaitForTaskCompletion.Result.Failed -> {
          ...
          val args = makeLinkCommand(configWithResourcesListed).joinToString("\\\n        ")
  ...
  class NoOutputExpected(private val displayName: String, val logger: ILogger) : GrabProcessOutput.IProcessOutput {
  ...
  class WaitForReadyOnStdOut(private val displayName: String, val logger: ILogger) : GrabProcessOutput.IProcessOutput {
    ...
    override fun err(line: String?) {
      line?.let { logger.error(null, "$displayName: Unexpected error output: $it") }
    }
  }

  class WaitForTaskCompletion(private val displayName: String, val logger: ILogger) : GrabProcessOutput.IProcessOutput {
    ...
    override fun err(line: String?) {
      when (line) {
        null -> {
          ...
          future.set(Result.InternalAapt2Error(IOException("AAPT2 process unexpectedly exit. Error output:\n" + errors.toString())))
        }
        "Done" -> {
          when {
  ...
  class AllowShutdown(private val displayName: String, val logger: ILogger) : GrabProcessOutput.IProcessOutput {

    override fun out(line: String?) {
      when (line) {
        null,
        "",
        "Exiting daemon" -> return
        else -> logger.error(null, "$displayName: Unexpected standard output: $line")
      }
    }

    override fun err(line: String?) {
      line?.let { logger.error(null, "$displayName: Unexpected error output: $line") }
    }
  }
}
