...
class RenderScriptProcessor(
  ...
  class Abi constructor(val device: String, val toolchain: String, private val linkerArgs: Array<String>) {
  ...
  init {
    ...
    if (targetApi < 21 && is64Bit && ndkMode) {
      throw RuntimeException("Api version $targetApi does not support 64 bit ndk compilation")
    }
  }
  ...
  private fun doMainCompilation(
    ...
    if (actualTargetApi >= 21 && ndkMode) {
      ...
      for (arch in architectures) {
        compileBCFiles(inputFiles, processExecutor, processOutputHandler, env, getArchSpecificRawFolder(arch), arch)
      }
    } else {
      compileBCFiles(inputFiles, processExecutor, processOutputHandler, env, genericRawFolder)
    }
  }
  ...
  private fun compileBCFiles(
    ...
    val rsClangPath = buildToolInfo.getPath(PathId.ANDROID_RS_CLANG)
  ...
  companion object {
    ...
    fun getSupportJar(buildToolsFolder: File, useAndroidX: Boolean): File {
      return File(getBaseRenderscriptLibFolder(buildToolsFolder), if (useAndroidX) FN_ANDROIDX_RS_JAR else FN_RENDERSCRIPT_V8_JAR)
    }
        ...
        AbiType.BIT_32 ->
          arrayOf(
            Abi(
              ...
              arrayOf("-dynamic-linker", "/system/bin/linker", "-X", "-m", "armelf_linux_eabi"),
            ),
            Abi("mips", "mipsel-unknown-linux", arrayOf("-EL")),
            Abi("x86", "i686-unknown-linux", arrayOf("-m", "elf_i386")),
          )
        AbiType.BIT_64 ->
          arrayOf(
            Abi("arm64-v8a", "aarch64-linux-android", arrayOf("-X", "--fix-cortex-a53-843419")),
            Abi("x86_64", "x86_64-unknown-linux", arrayOf("-m", "elf_x86_64")),
          )
      }
    }
