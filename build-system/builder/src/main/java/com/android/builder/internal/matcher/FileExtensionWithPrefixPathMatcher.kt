...
/**
 * Implementation of [PathMatcher] that can handle file path wih a static prefix and file name with an extension :
 *
...
open class FileExtensionWithPrefixPathMatcher(val matcher: Matcher) : PathMatcher {
  ...
  override fun matches(p0: Path?): Boolean {
    val pathAsString = p0?.toString()?.replace(File.separatorChar, '/') ?: return false
    return pathAsString.startsWith(prefix) && pathAsString.indexOf('/', prefix.length) == -1 && pathAsString.endsWith(extension)
  }
}
