...
/**
 * This class extends the information in the packaging options for resources or native libs by compiling patterns and providing matching
 * data.
 */
 ...
 */
constructor(excludePatterns: Collection<String>, pickFirstPatterns: Collection<String>, mergePatterns: Collection<String>) {
  ...
  companion object {
    ...
    fun compileGlob(pattern: String): PathMatcher {
      ...
      return GlobPathMatcherFactory.create(if (!pattern.startsWith("/") && !pattern.startsWith("*")) "/$pattern" else pattern)
    }
  }
}
