  ...
  companion object {
    ...
    fun create(
      ...
      return MavenCoordinatesImpl(
        ...
        stringCachingService.cacheString(computeVersionLessId(groupId, artifactId, classifier)),
        stringCachingService.cacheString(computeToString(groupId, artifactId, version, packagingStr, classifier)),
      )
    }

    private fun computeVersionLessId(groupId: String, artifactId: String, classifier: String?): String {
      val sb = StringBuilder(groupId.length + groupId.length + artifactId.length + 2 + (classifier?.let { it.length + 1 } ?: 0))
    ...
    private fun computeToString(groupId: String, artifactId: String, version: String, packaging: String, classifier: String?): String {
      val sb =
        StringBuilder(
          groupId.length +
            artifactId.length +
  ...
  fun compareWithoutVersion(coordinates: MavenCoordinates): Boolean {
    return this === coordinates ||
      Objects.equal(groupId, coordinates.groupId) &&
        Objects.equal(artifactId, coordinates.artifactId) &&
        Objects.equal(packaging, coordinates.packaging) &&
        Objects.equal(classifier, coordinates.classifier)
  }
