 ...
 *
 * This class is only for use in the Gradle tooling model. On the IDE side use `IdeAndroidGradlePluginProjectFlags` which provides an
 * interpreted view of the flags.
 *
 * @since 4.2
 */
interface AndroidGradlePluginProjectFlags {
   ...
   *
   * Flags **must** never be removed from here. This is to avoid issues when the current version of studio fetches models from a project
   * that has a legacy flag set. They can be marked as `@Deprecated` and the getter removed from `IdeAndroidGradlePluginProjectFlags`
   */
  enum class BooleanFlag(
    /**
     * The apparent value of this flag from Studio if it is not explicitly set in the AGP model.
     *
     * As Studio can open projects from older Android Gradle Plugins this is used to supply a value if it was not supplied by the build
     * system.
     *
     ...
     *
     * If the value is not supplied by the build system, return [fallbackValue] (e.g., [fallbackValue] could be read through a
     * `ToolingModelBuilder` during sync). If [fallbackValue] is also not provided, return [legacyDefault].
     */
   ...
   *
   * Uses the string of the enum constant rather than the enum itself to avoid the Gradle tooling api proxy throwing if the enum constant is
   * not present on the AGP side. Returns null if the flag is unknown, or if the value is not set.
   */
