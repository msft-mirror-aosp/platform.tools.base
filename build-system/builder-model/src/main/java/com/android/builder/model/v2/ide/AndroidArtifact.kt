 ...
 * An Android Artifact.
 *
 * This is the entry point for the output of a [Variant]. This can be more than one output in the case of multi-apk where more than one APKs
 * are generated from the same set of sources.
 */
interface AndroidArtifact : AbstractArtifact, AndroidModel {
  ...
  /**
   * The target SDK version override. If null, there is no override and the value may be coming from the manifest(s) if present there. If
   * not null, this is the final resolved value.
   */
  ...
  /**
   * Returns the name of the [SigningConfig] used for the signing. If none are setup or if this is not the main artifact of an application
   * project, then this is null.
   *
  ...
  /**
   * The name of the task used to generate the resources. The actual value might depend on the build system front end.
   *
   ...
   *
   * This is only applicable to the main artifact of the APPLICATION modules. All other cases this should be null.
   */
