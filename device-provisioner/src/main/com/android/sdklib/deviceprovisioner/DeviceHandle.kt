 ...
 *
 * This is stateful and has identity: if a device corresponding to this DeviceHandle is connected, disconnected, and reconnected, this
 * DeviceHandle should remain linked to it.
 */
interface DeviceHandle : Extensible {
  val id: DeviceId

  /**
   * A [CoroutineScope] tied to the lifecycle of this [DeviceHandle]: when this [DeviceHandle] goes away, the scope will be cancelled by
   * DeviceProvisioner. (Note that DeviceHandles may or may not continue to exist while disconnected: this scope may be same as the
   * ConnectedDevice's scope, or may outlive it. However, if a plugin removes a [DeviceHandle] from its devices, it should never add the
   * same object back later, since its scope will have been cancelled already.)
   */
  ...
  /** An action that allows acquiring or extending a reservation for the device, or null if the device doesn't use reservations. */
  ...
  /**
   * Shows the device in its source location; depending on the device, this could launch a file system browser to show the location of the
   * device on disk, or it could launch a web browser to show a device definition.
   */
