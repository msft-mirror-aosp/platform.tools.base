...
/**
 * This function builds on and is one level higher than [streamNinjaStatements]. It adds variable namespace scoping and evaluation. The
 * scope precedence order is global variables then 'build' statement properties.
 *
 * The output is a stream of [NinjaBuildUnexpandedCommand], one for each 'build' statement. It has the input files, the output files, the
 * 'rule' properties, and an [expand] function that can be used to look up properties values in the current scope according Ninja property
 * scoping rules.
 *
 * Each of the values in [NinjaBuildUnexpandedCommand] have properties unexpanded. So, for example, an unexpanded compile command might look
 * like this:
 *
 * /path/to/clang++ --target=aarch64-none-linux-android21 --gcc-toolchain=/path/to/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64
 * --sysroot=/path/to/ndk/21.4.7075529/toolchains/llvm/prebuilt/darwin-x86_64/sysroot $DEFINES $INCLUDES $FLAGS -MD -MT $out -MF $DEP_FILE
 * -o $out -c $in
 *
 * Notice the $ variables.
 *
 * The caller of [streamNinjaBuildCommands] can replace $ variables by looking them up with the supplied [expand] function.
 *
 * The variables $in and $out are special in Ninja. They don't literally exist in 'build.ninja'. Instead, they are constructed from the
 * current 'build' statements inputs and outputs.
 */
fun streamNinjaBuildCommands(file: File, progress: ProgressCallback? = null, action: NinjaBuildUnexpandedCommand.() -> Unit) {
  ...
  streamNinjaStatements(file, progress) { node ->
    ...
    when (node) {
      ...
      is NinjaStatement.BuildDef -> {
        ...
        val unexpanded = NinjaBuildUnexpandedCommand(buildDef = node, ruleDef = rule, globalProperties = state.variables)
        action(unexpanded)
      }
...
/**
 * Represents a Ninja 'build' statement along with the 'rule' that applies to it and an [expand] function that can be used to expand
 * property values.
 */
data class NinjaBuildUnexpandedCommand(
  ...
  private val ins
    get() = _ins ?: (buildDef.explicitInputs + buildDef.implicitInputs).joinToString(" ").also { _ins = it }

  // All output files separated by spaces
  private var _outs: String? = null
  private val outs
    get() = _outs ?: (buildDef.explicitOutputs + buildDef.implicitOutputs).joinToString(" ").also { _outs = it }

  // Explicit inputs to a build step
  private var _explicitInputs: List<String>? = null
  val explicitInputs
    get() = _explicitInputs ?: buildDef.explicitInputs.map { expand(it) }.also { _explicitInputs = it }

  // Implicit inputs of a build step
  private var _implicitInputs: List<String>? = null
  val implicitInputs
    get() = _implicitInputs ?: buildDef.implicitInputs.map { expand(it) }.also { _implicitInputs = it }

  // Inputs used only to determine build order
  private var _orderOnlyInputs: List<String>? = null
  val orderOnlyInputs
    get() = _orderOnlyInputs ?: buildDef.orderOnlyInputs.map { expand(it) }.also { _orderOnlyInputs = it }

  // Explicit outputs of a build step
  private var _explicitOutputs: List<String>? = null
  val explicitOutputs
    get() = _explicitOutputs ?: buildDef.explicitOutputs.map { expand(it) }.also { _explicitOutputs = it }

  // Implicit outputs of a build step
  private var _implicitOutputs: List<String>? = null
  val implicitOutputs
    get() = _implicitOutputs ?: buildDef.implicitOutputs.map { expand(it) }.also { _implicitOutputs = it }

  // Rule properties before ${macros} are expanded
  private var _unexpandedRuleProperties: Map<String, String>? = null
  private val unexpandedRuleProperties
    get() =
      _unexpandedRuleProperties
        ?: ruleDef.properties.map { it.key.lowercase(Locale.getDefault()) to it.value }.toMap().also { _unexpandedRuleProperties = it }

  private fun getProperty(name: String): String {
    return when (name) {
      "in" -> ins
      "out" -> outs
      else -> buildDef.properties[name] ?: globalProperties[name] ?: ""
    }
  }
  ...
  /**
   * The 'command' property is the only property on 'rule' that is required. If the property doesn't exist then "" is returned and the
   * caller is expected to handle it as an error.
   */
  val command: String
    get() = unexpandedRuleProperties["command"] ?: ""

  /** A generator rule is one that's meant to regenerate the 'build.ninja' file. */
  val generator: String?
    get() = unexpandedRuleProperties["generator"]

  /** Rule's response file. */
  val rspfile: String?
    get() = unexpandedRuleProperties["rspfile"]

  /** Rule's response file content. */
  val rspfileContent: String?
    get() = unexpandedRuleProperties["rspfile_content"]
}

private data class EvaluationState(
  /** Key is the name of the property. Value is the property value for that name at the current scope level. */
  var variables: MutableMap<String, String> = mutableMapOf(),
  /** Key is the name of the rule. Value is the definition of the rule at the current scope level. */
