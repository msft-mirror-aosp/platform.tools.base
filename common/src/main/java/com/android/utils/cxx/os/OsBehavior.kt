...
/**
 * Convenience methods like [exe], [bat], and [which] should be preferred over [os.exe] etc except in cases where the behavior should be
 * overridden/mocked for testing.
 */

/** File extension to use for executable files for the current host OS. Either ".exe" or "". */
val exe
  get() = os.exe

/** A shell script file extension. Either ".bat" or "". */
val bat
  get() = os.bat

/**
 * Given a literal string [argument] quote it such that if [argument] is passed to a program it is treated as a single argument with all
 * text, including special characters, kept unchanged.
 */
fun quoteCommandLineArgument(argument: String) = os.quoteCommandLineArgument(argument)

/**
 * Quote [executable] that will be executed as a process. In particular, Windows needs paths with special characters like '(', ')', and ' '
 * quoted.
 */
fun quoteExecutablePath(executable: String) = os.quoteExecutablePath(executable)

/**
 * Given a name like 'executable' find the full path by searching on PATH. On Windows, look for .exe, .bat, and .cmd. Return null if not
 * found.
 */
...
interface OsBehavior {
  ...
  /**
   * Convert a single command into a List<String> following host-specific rules. Does not convert any escaped characters to their evaluated
   * equivalent.
   */
  fun tokenizeCommandLineToRaw(command: String): List<String>

  /**
   * Convert a single command into a List<String> following host-specific rules. Convert any escaped characters to their evaluated
   * equivalent.
   */
  fun tokenizeCommandLineToEscaped(command: String): List<String>

  /**
   * Given a literal string [argument] quote it such that if [argument] is passed to a program it is treated as a single argument all text,
   * including special characters, kept intact.
   */
...
fun createOsBehavior(
  ...
  return if (platform == PLATFORM_WINDOWS)
    object : OsBehavior {
      ...
      override fun quoteCommandLineArgument(argument: String): String {
        ...
        return if (
          listOf(
              ...
              "(",
              ")",
              // Colon (:) is a special case. A normal windows.bat script will accept this
              // as
              // a single parameter. However, if that script then invokes PowerShell.exe
              // with
      ...
      override fun quoteExecutablePath(executable: String): String {
        ...
        return if (listOf('&', '(', ')', '{', '}', '^', '=', ';', '!', '\'', '+', ',', '[', ']', ' ').any { executable.contains(it) })
          "\"$executable\""
        else executable
      }
      ...
      override fun which(executable: File): File? {
        ...
        if (!executable.isRooted) {
          for (path in environmentPaths) {
            find(path.resolve(executable))?.let {
              return@which it
            }
          }
        }
...
private fun getExecutableExtensions(platform: Int) =
  when (platform) {
    PLATFORM_WINDOWS ->
...
/** @return list of folders (as Files) retrieved from PATH environment variable and from Sdk cmake folder. */
fun getEnvironmentPaths(): List<File> {
  ...
  return envPath.split(pathSeparator).asSequence().filter { it.isNotEmpty() }.map { File(it) }.toList()
}
