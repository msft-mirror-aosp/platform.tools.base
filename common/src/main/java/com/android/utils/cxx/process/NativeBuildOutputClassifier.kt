...
/**
 * Groups stdout from native tools (mainly clang) in to separate logical chunks so that they can eventually be presented to the user in
 * Android Studio. Android Studio presents a tree of errors and warning and each node must know which lines from STDOUT should be attributed
 * to each.
 *
 * ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~[AGP Process ↓] +--------------------------+
 * +------------------------+ | AGP Executes Native Tool | ---stdout--->| NativeOutputClassifier | -----------+ +--------------------------+
 * | running in AGP | | +------------------------+ NativeOutputClassifier Message(s) +-----------------------------+ subset of stdout | AGP
 * elevates lines to | | +---------------| 'lifecycle' level in | <-----------------------+ | | DefaultProcessOutputHandler | |
 * +-----------------------------+ Gradle ~~~~~ lifecycle ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~[AGP/AS Process Boundary ↕]
 * lines | +---------------------------+ +--------------->| NativeOutputClassifier |--------------------------+ | running in Android Studio
 * | | +---------------------------+ NativeOutputClassifier Diagnostic and +-------------------------------------+ Generic Messages(s) |
 * ClangOutputParser uses | | | NativeOutputClassifier messages |<--------------------+ | to generate build output tree nodes |
 * +-------------------------------------+ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~[AS Process ↑]
 *
 * NativeOutputClassifier runs twice for each native process executed. The first time is in Android Gradle Plugin (AGP) to elevate the
 * relevant lines to 'lifecycle' so that they are visible by default when the user executes 'gradle assemble' without the --info flag. The
...
class NativeBuildOutputClassifier(val send: (Message) -> Unit) : LineOutputStream {
  ...
  /**
   * Define message to be received by caller. When the caller is AGP, just 'lines' are used. These are elevated ti 'lifecycle'.
   *
   * When the caller is Android Studio, the 'Diagnostic' Messages are turned in to build tree nodes.
   */
  sealed interface Message {
    ...
    /**
     * Non-diagnostic lines that should still be sent to lifecycle in AGP so that Android Studio has ir available for parsing purposes.
     *
     * Examples,
     *
     * ninja: Entering directory `/path/to/app/.cxx/Debug/4c2h4t3a/arm64-v8a' ^ Used for ABI in Diagnostic ---+
     *
     * 1 warnings and 2 errors. ^ +--- Used to terminate the final error or warning.
     */
    class Generic(line: String, override val classification: NativeToolLineClassification) : Message {
  ...
  /** Record of messages to send if a diagnostic appears later. If no diagnostic appears then these messages are not sent. */
  ...
  /**
   * Processes the given line, and calls send() for any current/past line(s) that are terminated (e.g., finished diagnostic statement or
   * block).
   */
  override fun consume(line: String) {
    ...
    if (
      ...
      (state == IN_ERROR_OR_WARNING && classification.mayBeMainDiagnostic && !classification.mayComeAfterMainDiagnostic)
      // If we're not currently in an error but this line can be a main error then record this
      // as the new main error.
      || (state == NOT_IN_ERROR_OR_WARNING && classification.mayBeMainDiagnostic)
    ) {
      match ?: error("Expected non-null match")
      details =
        DiagnosticDetails(
          classification = classification,
          file = match.regexFieldOrNull("file")?.let { makePathRelativeToWorkingDirectory(it, match) },
    ...
    state =
      if (classification.mayBeMainDiagnostic) {
        IN_ERROR_OR_WARNING
  ...
  /** When possible, convert relative paths in diagnostics into absolute paths that are easier for the user to reason about. */
  ...
  companion object {
    ...
    private fun DiagnosticDetails.applyLockedOutputFileMessage(lines: MutableList<String>) {
      if (classification == CLANG_FATAL_LINKER_ERROR_BUG_124104842 && !lines.any { it.contains(MESSAGE_BUG_124104842) }) {
...
enum class NativeToolLineClassification(
  ...
  /**
   * True if this line is a candidate to be a main error or warning. The main diagnostic is the error or warning that should be shown as the
   * node title text of the build output tree. It is typically contains ": error:" or ": warning:".
   *
   * For example, /path/to/app.cpp:5:3: error: no matching function for call to 'bar'
   */
  val mayBeMainDiagnostic: Boolean,
  /**
   * True if this line is a candidate to be appended to a main diagnostic.
   *
   * For example, /path/to/app.cpp:1:13: note: candidate not viable: no known conversion from...
   */
  val mayComeAfterMainDiagnostic: Boolean,
  /**
   * Whether this is a line that can come before the main diagnostic. For example, the first two lines of: In file included from
   * /path/to/app.cpp:3: In file included from /path/tp/include1.h:7: /path/to/include2.h:7:1: error: unknown type name 'snake'
   */
...
private fun classifyLine(message: String): Pair<NativeToolLineClassification, MatchResult?> {
  ...
  if (!message.contains(':') && !message.contains("error") && !message.contains("warning")) return NONE to null
