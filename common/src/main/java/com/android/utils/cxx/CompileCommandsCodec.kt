...
/**
 * Encodes and decodes a file format that represents the same information as in compile_commands.json.
 *
 * When decoding, strings are naturally interned. It means that a string doesn't need to be allocated to determine whether it exists already
 * in an intern table.
 *
 * For this reason, deserialization is as much as 200x faster than GSON for highly redundant cases (which is the usual). Also, the on-disk
 * representation is up to 190x smaller (138MB vs 720KB in HugeProject*3 without distinct per-file flag).
 *
 * File format
 * -----------
 * header:
...
class CompileCommandsEncoder(val file: File, initialBufferSize: Int = BYTEBUFFER_WINDOW_SIZE) : AutoCloseable {
  ...
  init {
    ...
    /**
     * Write the header and section index. Offsets into the file are not known yet, so they are left as zero for now
     *
     * header:
  ...
  private fun encodeUTF8(string: String) {
    ...
    val map = ensureAtLeast(Int.SIZE_BYTES + bytes.size)
  ...
  /** Intern a string and return it's unique index number. Index 0 is reserved for null. */
  private fun intern(string: String?) =
    if (string == null) 0
    else
      stringTable.computeIfAbsent(string) {
        stringTable.size + 1 // +1 to reserve 0 for 'null'
      }

  /** Intern a string list and return it's unique index. */
  private fun intern(strings: List<Int>) = stringListTable.computeIfAbsent(strings) { stringListTable.size }
   ...
   *
   * First, if compiler, flags, target, or working directory has changed since last time then write a [COMPILE_COMMAND_CONTEXT_MESSAGE] with
   * that information.
   *
   ...
   *         - source-file-path: int32 (index into string-table-section)
   */
  fun writeCompileCommand(sourceFile: File, compiler: File, flags: List<String>, workingDirectory: File, outputFile: File, target: String) {
    ...
    if (
      compilerIndex != lastCompilerWritten ||
  ...
  override fun close() {
    ...
    stringTable.toList().sortedBy { (_, index) -> index }.forEach { (string, _) -> encodeUTF8(string) }
...
/**
 * Read [MAGIC] and [COMPILE_COMMANDS_CODEC_VERSION] and return the position immediately after. Returns Pair(0, 0) if this isn't a
 * compile_commands.json.bin file.
 */
...
private fun ByteBuffer.readStringTable(start: Int): Array<String?> {
  ...
  val count = int
  return (listOf(null) + (0 until count).map { readUTF8() }).toTypedArray()
}

/** Read the string lists table. */
private fun ByteBuffer.readStringListsTable(start: Int, strings: Array<String?>): Array<List<String>> {
 ...
 */
fun streamCompileCommandsV1(file: File, action: (sourceFile: File, compiler: File, flags: List<String>, workingDirectory: File) -> Unit) {
  streamCompileCommands(file) { action(sourceFile, compiler, flags, workingDirectory) }
}
...
/** Methods for streaming over compile commands messages and converting ordinals to Strings and String lists. */
private class CompileCommandsInputStream(private val file: File) : AutoCloseable {
  ...
  fun file(): File {
    ...
    return internedFiles.computeIfAbsent(index) { File(strings[index]!!) }
  }

  /** Read an Int from the stream and look it up in the string table. */
  fun string(): String = strings[map.int]!!

  /** Read an Int from the stream and look it up in the string table. Return null if the int is out-of-range of the string table. */
  ...
  /**
   * Read an Int from the stream and look it up in the string list table. Return null if the int is out-of-range of the string list table.
   */
...
/**
 * Open and use a [CompileCommandsInputStream] to execute action. Inline and return T so that 'return' can be called inside the [action]
 * block.
 */
...
/**
 * Check for the presence of b/201754404. This is a variant that version 2 the mistakenly did not have the version number bumped up.
 *
 * Differences between V2 and V2' (the one with the bug): In the header, V2 has no source file count but V2' does In the message
 * COMPILE_COMMAND_CONTEXT_MESSAGE, V2 had 'outputFile'. In V2' it was replaced with 'target' name. In the message
 * COMPILE_COMMAND_FILE_MESSAGE, V2' added 'outputFile'.
 *
 * V3 and V2' are identical aside from version number written into the file.
 *
 * This function checks for the presence of V2' by trying to step through the file's messages as if the file was V2. Once an invalid
 * message, string, or string list is encountered the function returns 'true' indicating the bug is present.
 *
 * Note that it isn't possible to construct a file that can be parsed as both V2 and V2' because there is an exact number of messages to be
 * read. The COMPILE_COMMAND_FILE_MESSAGE has changed size from 4 to 8 (because output file was added).
 */
...
/**
 * Implementation class that streams all information for the current version. When earlier-than-current files are read in, fallback nonsense
 * values are used.
 */
...
/** Return true iff the given compile_commands.json.bin is the latest version supported by the code in this class. */
...
/**
 * Strips off arguments that don't affect IDE functionalities from the given list of arguments passed to clang.
 *
 * @param sourceFile the source file to compile
 * @param args the raw args passed to the compiler
 * @param scratchSpace the list used to build the final result. This is useful if caller would like to save some object allocations when
 *   stripping a large amount of commands. Caller don't need to clear the scratch space.
 */
fun stripArgsForIde(sourceFile: String, args: List<String>, scratchSpace: MutableList<String> = ArrayList()): List<String> {
  scratchSpace.clear()
  var i = 0
  while (i < args.size) {
    when (val arg = args[i]) {
      ...
      in STRIP_FLAGS_WITH_ARG_INCLUDING_OUTPUT_FILE -> i++
        ...
        if (STRIP_FLAGS_WITH_IMMEDIATE_ARG_INCLUDING_OUTPUT_FILE.none { arg.startsWith(it) } && arg != sourceFile) {
...
val STRIP_FLAGS_WITH_ARG = listOf("-MF", "-MT", "-MQ")

val STRIP_FLAGS_WITH_ARG_INCLUDING_OUTPUT_FILE = listOf("-o", "--output") + STRIP_FLAGS_WITH_ARG

/** These are flags that have arguments immediate following them. */
val STRIP_FLAGS_WITH_IMMEDIATE_ARG = listOf("-MF", "-MT", "-MQ")

val STRIP_FLAGS_WITH_IMMEDIATE_ARG_INCLUDING_OUTPUT_FILE = listOf("--output=") + STRIP_FLAGS_WITH_IMMEDIATE_ARG
