 ...
 * listener.
 *
 * If calling code is asynchronous, it should add a listener to the result of [get]. Note that the returned future may already be done, so
 * the value may be ready to consume immediately.
 *
...
interface AsyncSupplier<V> : Supplier<ListenableFuture<V>> {
   ...
   *
   * [AsyncSupplier] implementations don't need to hold on to old values, so this is not guaranteed to ever be non-null. Even if the
   * [AsyncSupplier] caches the last known value, it may have not been computed yet.
   */
  val now: V?
   ...
   * [ListenableFuture] will be returned from multiple calls to [AsyncSupplier.get]. Because of that, cancelling this future will have no
   * effect.
   *
   * If the [AsyncSupplier] keeps the value up-to-date through a listener or a similar mechanism, the future is returned in done state.
   */
 ...
 *
 * @param compute The expensive computation that returns the value. This computation will be called on the given [executor]. The calls to
...
constructor(private val compute: () -> V, private val isUpToDate: (value: V) -> Boolean = { _ -> true }, executor: ExecutorService) :
  ...
  private var lastComputedValue: V? = null
    get() = lastComputedValueLock.read { field }
    set(newValue) {
      lastComputedValueLock.write { field = newValue }
    }
...
/**
 * An [AsyncSupplier] value with a method to calculate the value. This class also provides a convenience timestamp of when the value was
 * originally calculated in case the value does not have an obvious way to determine if it's expired.
 *
 * @param compute The expensive computation that returns the value. This computation will be called on the given executor.
 * @param isUpToDate Function that receives a value and the creation timestamp of that value and returns whether is up to date.
 ...
 */
class CachedAsyncSupplierWithTimestamp<V>
@JvmOverloads
constructor(
  compute: () -> V,
  isUpToDate: (timestampMs: Long, value: V) -> Boolean = { _, _ -> true },
  private val executor: ExecutorService,
  timestampSource: () -> Long = { System.currentTimeMillis() },
) : AsyncSupplier<V> {
  private val delegate =
    CachedAsyncSupplier(
      compute = { ValueWithTimestamp(value = compute(), timestampMs = timestampSource()) },
  ...
  override fun get(): ListenableFuture<V> =
    Futures.transform(delegate.get(), com.google.common.base.Function<ValueWithTimestamp<V>, V> { computed -> computed!!.value }, executor)
}

/**
 * Utility class to auto-refresh [AsyncSupplier] values. This will only refresh the value as long as there are references to it and it will
 * stop if the value is garbage collected.
 *
 ...
 */
class AsyncSupplierRefresher<V : AsyncSupplier<*>>(
  asyncSupplier: V,
  ...
  @GuardedBy("scheduledFutureLock") private var scheduledFuture: ScheduledFuture<*>? = null
  ...
  fun cancel() {
    scheduledFutureLock.withLock { scheduledFuture?.cancel(true) }
  }
}
