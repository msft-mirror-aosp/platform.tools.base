...
/**
 * [ExecutorService] that only allows [maxQueueingTasks] tasks waiting to be executed. If the limit is reached, the next task to arrive will
 * preempt the first one in the queue. The tasks are executed on the given [delegateExecutor].
 *
 * This class is thread-safe
 */
class EvictingExecutor(private val delegateExecutor: ExecutorService, maxQueueingTasks: Int = 1) : AbstractExecutorService() {
  ...
  override fun shutdownNow(): MutableList<Runnable> {
    ...
    evictingQueueLock.withLock { evictingQueue.removeAll { pendingRunnables.add(it) } }
    return pendingRunnables.apply { addAll(delegateExecutor.shutdownNow()) }
  }

  override fun isShutdown(): Boolean = delegateExecutor.isShutdown

  override fun awaitTermination(timeout: Long, unit: TimeUnit): Boolean = delegateExecutor.awaitTermination(timeout, unit)
}
