 ...
 */
abstract class AbstractAndroidLocations
protected constructor(
  ...
  companion object {
    ...
    @JvmField val FOLDER_DOT_ANDROID = ".android"

    /** Virtual Device folder inside the path returned by [avdLocation] */
    @JvmField val FOLDER_AVD = "avd"

    /** Virtual Device folder for devices managed by the Android plugin for gradle inside the path returned by [gradleAvdLocation]. */
    @JvmField val FOLDER_GRADLE_AVD = "gradle-managed"

    @JvmField @Deprecated("Use ANDROID_USER_HOME") val ANDROID_PREFS_ROOT = "ANDROID_PREFS_ROOT"

    @JvmStatic val ANDROID_USER_HOME = "ANDROID_USER_HOME"
  }
...
This is the path of preference folder expected by the Android tools."""
  ...
  override val avdLocation: Path
    get() =
      internalAvdLocation
        ...
        (PathLocator(environmentProvider).singlePathOf(Global.ANDROID_AVD_HOME) ?: prefsLocation.resolve(FOLDER_AVD)).also {
          internalAvdLocation = it
        }
  ...
  override val gradleAvdLocation: Path
    get() = internalGradleAvdLocation ?: avdLocation.resolve(FOLDER_GRADLE_AVD).also { internalGradleAvdLocation = it }
  ...
  /**
   * Computes, and returns the root folder where the android folder will be located.
   *
   * This is using the old, deprecated ANDROID_SDK_HOME or ANDROID_PREFS_ROOT as a backup in case ANDROID_USER_HOME does not exist
   */
  fun computeAndroidFolder(): Path {
    val locator = AndroidPathLocator(environmentProvider, if (!silent) logger else NullLogger())

    val folder = locator.singlePathOf(Global.ANDROID_USER_HOME, Global.ANDROID_PREFS_ROOT, Global.ANDROID_SDK_HOME)
  ...
  /** Reset the cached paths so that they can be reinitialized as needed, e.g. if system properties are changed by a test. */
  @VisibleForTesting
...
private data class QueryResult(val global: Global, val type: VariableType, val path: Path) : LocationValue {
...
private data class VariableValue(override val propertyName: String, val type: VariableType, val path: Path, val correctPath: Path) :
  LocationValue {
...
private open class PathLocator(private val environmentProvider: EnvironmentProvider) {
   ...
   *
   * This does not detect conflicts between variables (or conflicts within a single variable used by both env var and sys prop)
   */
  fun firstPathOf(vararg globalVars: Global): Path? {
   ...
   *
   * This method will check all variables and only succeed if a single variable contains a valid value. This includes checking for both env
   * var and sys prop where applicable.
   */
  fun singlePathOf(vararg globalVars: Global): Path? {
    val values = globalVars.asSequence().map { it.gatherPaths() }.flatMap { it.asSequence() }.toList()
   ...
   */
  private fun Global.gatherPaths(): List<VariableValue> {
   ...
   */
  private fun queryPath(globalVar: Global, queryType: VariableType): VariableValue? {
    val location =
      when (queryType) {
    ...
    return VariableValue(globalVar.propName, queryType, path, correctPath)
  }
}

private class AndroidPathLocator(environmentProvider: EnvironmentProvider, private val logger: ILogger? = null) :
  PathLocator(environmentProvider) {
  ...
  private fun isSdkRootWithoutDotAndroid(folder: Path): Boolean {
    return (folder.hasSubFolder("platforms") && folder.hasSubFolder("platform-tools") && !folder.hasSubFolder(FOLDER_DOT_ANDROID))
  }
...
/**
 * Enum describing which variables to check and whether they should be checked via [EnvironmentProvider.getSystemProperty] or
 * [EnvironmentProvider.getEnvVariable] or both.
 */
private enum class Global(
  ...
  ANDROID_AVD_HOME(propName = "ANDROID_AVD_HOME", isSysProp = true, isEnvVar = true, androidLeaf = null),
  ANDROID_SDK_HOME(propName = "ANDROID_SDK_HOME", isSysProp = true, isEnvVar = true),
  @Suppress("DEPRECATION") // Support for deprecated ANDROID_PREFS_ROOT
  ANDROID_PREFS_ROOT(propName = AbstractAndroidLocations.ANDROID_PREFS_ROOT, isSysProp = true, isEnvVar = true),
  TEST_TMPDIR( // Bazel kludge
    propName = "TEST_TMPDIR",
    isSysProp = false,
    isEnvVar = true,
  ),
  USER_HOME(propName = "user.home", isSysProp = true, isEnvVar = false),
  HOME(propName = "HOME", isSysProp = false, isEnvVar = true),
  XDG_CONFIG_HOME(propName = "XDG_CONFIG_HOME", isSysProp = true, isEnvVar = true),
}
