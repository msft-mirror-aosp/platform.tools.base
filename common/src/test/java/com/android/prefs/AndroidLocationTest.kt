...
class AndroidLocationTest {
  ...
  fun `ANDROID_SDK_HOME usage warning`() {
    ...
    val result = AndroidLocation.getFolder(provider, logger)

    Truth.assertWithMessage("Test Location").that(File(result)).isEqualTo(File(testLocation, ".android"))
  }
  ...
  fun `ANDROID_PREFS_ROOT and ANDROID_SDK_HOME with different values`() {
    ...
    AndroidLocation.getFolder(provider, logger)
  }
  ...
  fun `ANDROID_SDK_HOME points to SDK`() {
    ...
    Truth.assertWithMessage("Emitted Warnings")
