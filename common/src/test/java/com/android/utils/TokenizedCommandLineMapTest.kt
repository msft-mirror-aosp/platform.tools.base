...
internal class TokenizedCommandLineMapTest {
  @Suppress("unused") val preload = SdkConstants.PLATFORM_LINUX // Preload some classes to remove that overhead

  @Test
  fun computeIfAbsent() {
    val map =
      TokenizedCommandLineMap<String>(raw = false) { tokens, sourceFile ->
        tokens.removeNth(0)
        tokens.removeTokenGroup(sourceFile, 0)
        tokens.removeTokenGroup("-o", 1)
      }
    fun check(commandLine: String, expected: String) {
      val result = map.computeIfAbsent(commandLine, "blah") { tokens -> tokens.toString() }
  ...
  fun reproCaseFromCompilationDatabaseIndexer() {
    val map =
      TokenizedCommandLineMap<String>(raw = true) { tokens, sourceFile ->
    ...
    val actual =
      map.computeIfAbsent(original, "/usr/local/google/home/jomof/projects/MyApplication22/app/src/main/cpp/native-lib.cpp") {
        it.toString()
      }
  ...
  /**
   * Test highly redundant mapping where nearly all of the command-lines map to the same key value in the map.
   *
   * When I ran this on my macbook, the rates were: Small command-line rate is 115384 chars/ms Large command-line rate is 78892 chars/ms
   */
  @Test
  fun highlyRedundantMapThroughput() {
    val map =
      TokenizedCommandLineMap<List<String>>(raw = false) { tokens, sourceFile ->
        tokens.removeNth(0)
        tokens.removeTokenGroup(sourceFile, 0)
        tokens.removeTokenGroup("-o", 1)
      }
    val smallCommandLine = "clang.exe -c my-file.cpp -o -my-file.o -DFLAG"
    val largeCommandLine = "clang.exe -c my-file.cpp -o -my-file.o -DFLAG " + "${repeat(1000) {'x'}}"
