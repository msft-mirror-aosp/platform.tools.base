...
class StreamNinjaStatementsTest {
  ...
  fun rules() {
    ...
    assertThat((ninja.tops[1] as RuleDef).properties["command"]).isEqualTo("date > \$out")
  }
  ...
  fun assignment() {
    ...
    assertThat(ninja).isEqualTo(NinjaFileDef(listOf(Assignment("a", "b"))))
  }
  ...
  fun twoAssign() {
    ...
    assertThat(ninja.tops[1]).isEqualTo(Assignment("x", "y"))
  }
  ...
  fun build() {
    ...
    assertThat(writeNinjaToString(ninja)).isEqualTo("build output.txt : RULE input.txt")
  }
  ...
  fun buildTwoOutputs() {
    ...
    assertThat(writeNinjaToString(ninja)).isEqualTo("build output1.txt output2.txt : RULE input1.txt")
  }

  @Test
  fun colonInBuildOutput() {
    val ninja = parseNinja("build build.ninja: RERUN_CMAKE C\$:/abc")
    assertThat(writeNinjaToString(ninja)).isEqualTo("build build.ninja : RERUN_CMAKE C\$:/abc")
  }
  ...
  fun emptyRule() {
    ...
    assertThat(writeNinjaToString(ninja)).isEqualTo("rule my_rule")
  }
  ...
  private fun parseNinja(reader: Reader): NinjaFileDef {
    ...
    reader.streamNinjaStatements { expression -> expressions += expression }
