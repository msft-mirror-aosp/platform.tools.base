...
class StreamNinjaBuildCommandsTest {
  ...
  fun `dolphin via CMake 3 18 1`() {
    val originalWorkingFolder =
      File(
        "/Users/jomof/projects/studio-main/out/build/base/build-system/integration-test/native/build/tmp/junit2760794059956044528/junit4505799956879287491/src/Source/Android/app/.cxx/RelWithDebInfo/4z4p6154/arm64-v8a"
      )
    ...
    streamNinjaBuildCommands(buildNinja) {
      ...
      if (out.contains(".o")) {
        ...
        val current = parseCommand(expand(command, sb), expand("\$in", sb), originalWorkingFolder.path)
  ...
  private val tokenMap =
    TokenizedCommandLineMap<Command>(raw = false, platform = SdkConstants.PLATFORM_DARWIN) { tokens, sourceFile ->
      tokens.removeTokenGroup(sourceFile, 0, filePathSlashAgnostic = true)
  ...
  private fun parseCommand(command: String, sourceFile: String, workingDirectory: String): Command {
