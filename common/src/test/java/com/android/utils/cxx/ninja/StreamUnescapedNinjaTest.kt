...
class StreamUnescapedNinjaTest {
  ...
  /** https://ninja-build.org/manual.html "Whitespace at the beginning of a line after a line continuation is also stripped." */
  @Test
  ...
  fun `build with spaces in file names`() {
    check(
      "build a$ b|c$ d:ru$ le e$ f|g$ h||i$ j",
      "build a[esc:space]b|c[esc:space]d:ru[esc:space]le e[esc:space]f|g[esc:space]h||i[esc:space]j",
    )
  }
}
