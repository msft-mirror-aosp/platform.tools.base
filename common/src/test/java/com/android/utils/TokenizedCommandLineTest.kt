...
class TokenizedCommandLineTest {
  ...
  /**
   * This is a very basic test just to confirm toTokenList can be called. The extensive platform-specific tests are in
   * [TokenizedCommandLineWindowsTest] and [TokenizedCommandLinePOSIXTest].
   */
  @Test
  fun simpleTest() {
  ...
  fun emptyTest() {
    fun check(raw: Boolean, platform: Int) {
      val empty = TokenizedCommandLine("", raw, platform).toString()
      assertThat("[$empty]").named("raw = $raw platform = $platform").isEqualTo("[]")
    }
    check(false, 1)
  ...
  @Test
  fun removeNth() {
    fun check(commandLine: String, n: Int, expectedToken: String?, expectedCommandLine: String, raw: Boolean, platform: Int) {
  ...
  @Test
  fun tokenMatches() {
    fun checkMatches(commandLine: String, matchString: String, offset: Int, matchPrefix: Boolean) {
      val tokens = TokenizedCommandLine(commandLine, true, SdkConstants.PLATFORM_WINDOWS)
      assertThat(tokens.tokenMatches(matchString, offset, matchPrefix)).named("'$matchString' in '$commandLine' at $offset").isTrue()
    }

    fun checkDoesntMatch(commandLine: String, matchString: String, offset: Int, matchPrefix: Boolean) {
      val tokens = TokenizedCommandLine(commandLine, true, SdkConstants.PLATFORM_WINDOWS)
      assertThat(tokens.tokenMatches(matchString, offset, matchPrefix)).named("'$matchString' in '$commandLine' at $offset").isFalse()
    }
  ...
  fun removeTokenGroup() {
    fun checkRemove(commandLine: String, token: String, extra: Int, expect: String) {
      ...
      assertThat(tokens.toString("|")).named("Removing ${extra + 1} tokens starting with '$token' in '$commandLine'").isEqualTo(expect)
    }
  ...
  fun testIdentity() {
    fun checkSame(c1: String, c2: String) {
      ...
      assertThat(t1.computeNormalizedCommandLineHashCode()).isEqualTo(t2.computeNormalizedCommandLineHashCode())
    ...
    fun checkNotSame(c1: String, c2: String) {
      ...
      t1.removeTokenGroup("-c", 1)
      t2.removeTokenGroup("-c", 1)
      assertThat(t1.computeNormalizedCommandLineHashCode()).isNotEqualTo(t2.computeNormalizedCommandLineHashCode())
  ...
  /**
   * No real test here. It just prints tokenization rate for Windows tokenizer. On my Macbook this printed:
   *
   * Windows: zero allocation rate is 186 parses/ms Windows: string list allocation rate is 44 parses/ms
   */
  @Test
  fun printWindowsTokenizeRates() {
    ...
    time("Windows: zero allocation") {
      TokenizedCommandLine(commandLine = commandLine, raw = true, platform = SdkConstants.PLATFORM_WINDOWS, indexes = indexes)
    }
    time("Windows: string list allocation") {
      TokenizedCommandLine(commandLine = commandLine, raw = true, platform = SdkConstants.PLATFORM_WINDOWS).toTokenList()
    }
  }

  /**
   * No real test here. It just prints tokenization rate for POSIX tokenizer. On my Macbook this printed:
   *
   * POSIX: zero allocation rate is 240 parses/ms POSIX: string list allocation rate is 85 parses/ms
   */
  @Test
  fun printPOSIXTokenizeRates() {
    ...
    time("POSIX: zero allocation") {
      TokenizedCommandLine(commandLine = commandLine, raw = true, platform = SdkConstants.PLATFORM_LINUX, indexes = indexes)
    }
    time("POSIX: string list allocation") {
      TokenizedCommandLine(commandLine = commandLine, raw = true, platform = SdkConstants.PLATFORM_LINUX).toTokenList()
    }
  }
  ...
  fun fuzzFoundSingleDot() {
    TokenizedCommandLine(".", false, SdkConstants.PLATFORM_WINDOWS).computeNormalizedCommandLineHashCode()
  }

  @Test
  fun checkLeadingWhitespaceDoesntCreateToken() {
    val check2 = TokenizedCommandLine(" ls", raw = false, platform = SdkConstants.PLATFORM_WINDOWS).toTokenList()
  ...
  fun fuzz() {
    ...
    fun ci(tokens: TokenizedCommandLine) = "TokenizedCommandLine(\"${tokens}\", raw = ${tokens.raw}, platform = ${tokens.platform})"
    ...
    fun check(tokens1: TokenizedCommandLine, tokens2: TokenizedCommandLine) {
      ...
      if (equalityTruth) {
        ...
        if (tokens1.computeNormalizedCommandLineHashCode() != tokens2.computeNormalizedCommandLineHashCode())
          fault("expect hashCode equals: $named")
