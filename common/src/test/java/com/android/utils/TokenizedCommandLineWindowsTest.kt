...
class TokenizedCommandLineWindowsTest {
  ...
  @Throws(Exception::class)
  fun checkOneCommand() {
    checkCommandLineSplitting("foo bar", listOf("foo bar"))
  }

  @Test
  @Throws(Exception::class)
  fun checkTwoCommandsWithSingleAmpersand() {
    checkCommandLineSplitting("foo bar& baz qux", listOf("foo bar", " baz qux"))
  }
  ...
  fun checkMultipleTokens() {
    checkTokenization("a b\tc", listOf("a", "b", "c"))
  }

  @Test
  @Throws(Exception::class)
  fun checkDoubleQuote() {
    checkTokenizationToCompilerFlags("a \"b  c\" d", listOf("a", "b  c", "d"), listOf("a", "\"b  c\"", "d"))
  }
  ...
  fun checkOddSlashesBeforeQuote() {
    checkTokenizationToCompilerFlags("a\\\\\\\"b c", listOf("a\\\"b", "c"), listOf("a\\\\\\\"b", "c"))
  }
  ...
  fun checkSingleQuoteWithinDoubleQuotes() {
    checkTokenizationToCompilerFlags("a \"b's  c\"", listOf("a", "b's  c"), listOf("a", "\"b's  c\""))
  }

  @Test
  @Throws(Exception::class)
  fun checkDoubleQuoteWithinSingleQuotes() {
    checkTokenizationToCompilerFlags("a 'b\"s\"  c'", listOf("a", "'bs", "c'"), listOf("a", "'b\"s\"", "c'"))
  }

  @Test
  @Throws(Exception::class)
  fun checkEscapedSpace() {
    checkTokenization("a b\\ c d", listOf("a", "b\\", "c", "d"))
  }
  ...
  fun caseFromCommandLineParserTest() {
    checkTokenizationToCompilerFlags("a\\\\\\b d\"e f\"g h", listOf("a\\\\\\b", "de fg", "h"), listOf("a\\\\\\b", "d\"e f\"g", "h"))
  }

  @Test
  @Throws(Exception::class)
  fun singleSlash() {
    checkTokenizationToCompilerFlags("\\", listOf("\\"), listOf("\\"))
  }
  ...
  fun checkDoubleQuotedNewline() {
    checkTokenizationToCompilerFlags("\"a\nb\"", listOf("a\nb"), listOf("\"a\nb\""))
  }
  ...
  fun checkCaretEscapedCaret() {
    checkTokenizationToCompilerFlags("a^^b", listOf("a^b"), listOf("a^^b"))
  }
  ...
  companion object {
    ...
    @Throws(Exception::class)
    private fun checkCommandLineSplitting(originalCommand: String, splitCommands: List<String>) {
      Truth.assertThat(splitCommandLine(originalCommand)).isEqualTo(splitCommands)
    }

    private fun tokenizeCommandLineToEscaped(commandLine: String) =
      TokenizedCommandLine(commandLine = commandLine, raw = false, platform = SdkConstants.PLATFORM_WINDOWS).toTokenList()
    ...
    // Tokenization tests.
    @Throws(Exception::class)
    private fun checkTokenizationToCompilerFlags(originalString: String, escapedExpected: List<String?>, rawExpected: List<String?>) {
      val tokenizedEscaped = tokenizeCommandLineToEscaped(originalString)
      val tokenizedRaw = tokenizeCommandLineToRaw(originalString)
      Truth.assertThat(tokenizedEscaped).named("escaped tokens (1st)").containsExactlyElementsIn(escapedExpected)
      Truth.assertThat(tokenizedRaw).named("raw tokens (2nd)").containsExactlyElementsIn(rawExpected)
    }

    @Throws(Exception::class)
    private fun checkTokenization(originalString: String, expected: List<String?>) {
