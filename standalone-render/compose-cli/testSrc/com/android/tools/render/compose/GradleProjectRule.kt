...
/**
 * This is the rule to conveniently set up an android gradle project to be able to run gradle tasks in that project. There is a number of
 * assumption about the project:
 * * Repositories are located only in settings.gradle.kts, google() is present there.
 * * gradle/wrapper/gradle-wrapper.properties has distributionUrl that will be replaced with [relativeGradleDistributionPath] distribution
 *   version.
 * * local.properties is either absent or has no useful settings, it will be replaced with the one containing sdk path.
 *
 ...
 * * For sdk, include corresponding sdk e.g. "//prebuilts/studio/sdk:platforms/android-34", "//prebuilts/studio/sdk:build-tools/34.0.0".
 */
class GradleProjectRule(
  ...
  fun createSettingsFile(outputFolder: File, resultsFile: File, metaDataFolder: File, screenshots: List<ComposeScreenshot>): File {
    executeGradleTask(":app:assembleDebug")
    executeGradleTask(":app:bundleDebugClassesToCompileJar")
    executeGradleTask(":app:debugExtractClasspath", "--init-script", "initscript.gradle")
  ...
  private fun injectGradleDistribution() {
    ...
    val replacedDistributionUrl = gradleDistrib.toUri().toString().replace("file:", "file\\:")
    content = content.replace("distributionUrl=.*".toRegex(), "distributionUrl=$replacedDistributionUrl")
