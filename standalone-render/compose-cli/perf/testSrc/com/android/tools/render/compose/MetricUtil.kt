...
private val timeBenchmark =
  Benchmark.Builder("Compose CLI rendering Time Benchmark")
    .setDescription("Baseline for Compose CLI rendering time (mean) after $NUMBER_OF_SAMPLES samples.")
    .build()
private val memoryBenchmark =
  Benchmark.Builder("Compose CLI rendering Memory Usage Benchmark")
    .setDescription("Baseline for Compose CLI rendering memory usage (mean) after $NUMBER_OF_SAMPLES samples.")
    .build()

internal fun computeAndRecordMetric(timeMetricName: String, memoryMetricName: String, computable: () -> ComposeRenderingMetric) {
  System.gc()
  // Classloading and thread pool require warm up
  repeat(NUMBER_OF_WARM_UP) { computable() }
...
/**
 * Remove the largest time measurements as they are outliers. One cannot measure the time that is less than it actually took to run, however
 * larger measurements are possible. Remove them, assuming they do not happen too often.
 */
