...
data class WearTileScreenshot(
  ...
  override fun toPreviewElement(module: StandaloneRenderModelModule): ScreenshotPreviewElement {
    val previewAnnotationClass =
      module.environment.moduleClassLoaderManager.getPrivate(WearTilePreviewElement::class.java.classLoader).useWithClassLoader {
        classLoader ->
        classLoader.loadClass("androidx.wear.tiles.tooling.preview.Preview")
      }
    val attributesProvider =
      DeserializedAnnotationAttributesProvider(this.previewParams)
    ...
    return WearTileScreenshotPreviewElement(
...
private data class WearTileScreenshotPreviewElement(private val wearTilePreviewElement: WearTilePreviewElement<Unit>) :
  ScreenshotPreviewElement, ConfigurablePreviewElement<Unit> by wearTilePreviewElement {

  override fun resolveXmlLayouts() = sequenceOf(wearTilePreviewElement.toPreviewXml().buildString())
}
