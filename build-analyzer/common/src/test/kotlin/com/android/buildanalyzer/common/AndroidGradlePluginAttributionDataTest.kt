...
class AndroidGradlePluginAttributionDataTest {
  ...
  private val data =
    AndroidGradlePluginAttributionData(
      ...
      buildscriptDependenciesInfo = setOf("a.a:a:1.0", "b.b:b:1.0", "c.c:c:1.0"),
      buildInfo = BuildInfo(agpVersion = "8.1.0", configurationCacheIsOn = true, gradleVersion = "8.1.0"),
      ...
      taskCategoryIssues = listOf(TaskCategoryIssue.MINIFICATION_ENABLED_IN_DEBUG_BUILD, TaskCategoryIssue.NON_TRANSITIVE_R_CLASS_DISABLED),
    )
  ...
  fun testDataSerialization() {
    ...
    val file = FileUtils.join(outputDir, SdkConstants.FD_BUILD_ATTRIBUTION, SdkConstants.FN_AGP_ATTRIBUTION_DATA)
    assertThat(file.readLines()[0])
      .isEqualTo(
        """
        |{
  ...
  fun testDeserializationOfOldAgpData() {
    ...
    assertThat(deserializedData.taskNameToTaskInfoMap).containsExactlyEntriesIn(data.taskNameToTaskInfoMap.normalizeForOlderAgpVersions())
