...
class OverviewPageRenderer : PageRenderer<AllTestResults>() {
  ...
  private fun renderPackages(htmlWriter: SimpleHtmlWriter) {
    ...
    for (testPackage in results.getPackages()) {
      ...
      htmlWriter.startElement("td").characters(testPackage.errorCount.toString()).endElement()
      htmlWriter.startElement("td").characters(testPackage.failureCount.toString()).endElement()
      htmlWriter.startElement("td").characters(testPackage.skipCount.toString()).endElement()
      htmlWriter.startElement("td").characters(testPackage.getFormattedDuration()).endElement()
      htmlWriter.startElement("td").attribute("class", testPackage.statusClass).characters(testPackage.formattedSuccessRate).endElement()
      htmlWriter.endElement()
    }
    htmlWriter.endElement()
    htmlWriter.endElement()
  ...
  private fun renderClasses(htmlWriter: SimpleHtmlWriter) {
    ...
    for (testPackage in results.getPackages()) {
      for (testClass in testPackage.getClasses()) {
        htmlWriter.startElement("tr")
        htmlWriter.startElement("td").attribute("class", testClass!!.statusClass).endElement()
        htmlWriter
          .startElement("a")
        ...
        htmlWriter.startElement("td").characters(testClass.skipCount.toString()).endElement()
        htmlWriter.startElement("td").characters(testClass.getFormattedDuration()).endElement()
        htmlWriter.startElement("td").attribute("class", testClass.statusClass).characters(testClass.formattedSuccessRate).endElement()
        htmlWriter.endElement()
      }
    }
    htmlWriter.endElement()
