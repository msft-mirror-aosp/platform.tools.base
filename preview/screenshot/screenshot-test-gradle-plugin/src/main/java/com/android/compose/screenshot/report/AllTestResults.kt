...
class AllTestResults : CompositeTestResults(null) {
  ...
  private val packages: MutableMap<String, PackageTestResults> = TreeMap<String, PackageTestResults>()
  ...
  fun addTest(
    ...
    val testResult: TestResult = addTest(packageResults.addTest(className, testName, duration, project, flavor, ssImages))
  ...
  private fun addPackage(packageName: String): PackageTestResults {
    var packageResults: PackageTestResults? = packages[packageName]
