...
abstract class PageRenderer<T : CompositeTestResults> : TabbedPageRenderer<T>() {
  ...
  protected open fun renderErrors(htmlWriter: SimpleHtmlWriter) {
    ...
    for (test: TestResult in results.errors) {
      ...
      htmlWriter
        ...
        .startElement("a")
        .attribute("href", String.format("%s.html?title=Errors", test.classResults.getFilename()))
  ...
  protected fun renderCompositeResults(htmlWriter: SimpleHtmlWriter, map: Map<String, CompositeTestResults>, name: String) {
    ...
    for (results: CompositeTestResults in map.values) {
      ...
      htmlWriter.startElement("td").characters(results.skipCount.toString()).endElement()
    ...
    get() =
      object : ReportRenderer<T, SimpleHtmlWriter>() {
        ...
        override fun render(model: T, htmlWriter: SimpleHtmlWriter) {
          ...
          htmlWriter.startElement("div").attribute("class", "counter").characters(results.testCount.toString()).endElement()
          htmlWriter.startElement("p").characters("tests").endElement()
          htmlWriter.endElement()
          htmlWriter.endElement()
          htmlWriter.startElement("td")
          htmlWriter.startElement("div").attribute("class", "infoBox").attribute("id", "errors")
          htmlWriter.startElement("div").attribute("class", "counter").characters(results.errorCount.toString()).endElement()
          ...
          htmlWriter.startElement("div").attribute("class", "counter").characters(results.failureCount.toString()).endElement()
          htmlWriter.startElement("p").characters("failures").endElement()
          htmlWriter.endElement()
          htmlWriter.endElement()
          htmlWriter.startElement("td")
          htmlWriter.startElement("div").attribute("class", "infoBox").attribute("id", "skipped")
          htmlWriter.startElement("div").attribute("class", "counter").characters(results.skipCount.toString()).endElement()
          ...
          htmlWriter.endElement()
          htmlWriter.startElement("td")
          htmlWriter.startElement("div").attribute("class", String.format("infoBox %s", results.statusClass)).attribute("id", "successRate")
          htmlWriter.startElement("div").attribute("class", "percent").characters(results.formattedSuccessRate).endElement()
          htmlWriter.startElement("p").characters(if (results.successRate != null) "successful" else "N/A").endElement()
          htmlWriter.endElement()
          htmlWriter.endElement()
          htmlWriter.endElement()
          htmlWriter.endElement()
