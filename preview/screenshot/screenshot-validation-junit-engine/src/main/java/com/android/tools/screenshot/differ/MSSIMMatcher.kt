...
/**
 * Image comparison using Structural Similarity Index, developed by Wang, Bovik, Sheikh, and Simoncelli. Details can be read in their paper:
...
class MSSIMMatcher(private var imageDiffThreshold: Float = 0f) : ImageDiffer {
  ...
  override fun diff(a: BufferedImage, b: BufferedImage): DiffResult {
    ...
    val stats =
      "[MSSIM] Required SSIM: ${String.format(Locale.ROOT, "%.3f", SSIMThreshold)}, Actual SSIM: ${String.format(Locale.ROOT, "%.3f", SSIMTotal)}"
  ...
  private fun calculateSSIM(ideal: IntArray, given: IntArray, width: Int, height: Int): Double {
  ...
  private fun calculateSSIM(ideal: IntArray, given: IntArray, offset: Int, stride: Int, width: Int, height: Int): Double {
    ...
    while (currentWindowY < height) {
      ...
      while (currentWindowX < width) {
        ...
        if (
          isWindowWhite(ideal, start, stride, windowWidth, windowHeight) && isWindowWhite(given, start, stride, windowWidth, windowHeight)
        ) {
        ...
        val meanX = means[0]
        val meanY = means[1]
        val variances = getVariances(ideal, given, meanX, meanY, start, stride, windowWidth, windowHeight)
  ...
  /** Compute the size of the window. The window defaults to WINDOW_SIZE, but must be contained within dimension. */
  ...
  /**
   * Finds the variance of the two sets of pixels, as well as the covariance of the windows. The return value is an array of doubles, the
   * first is the variance of the first set of pixels, the second is the variance of the second set of pixels, and the third is the
   * covariance.
   */
