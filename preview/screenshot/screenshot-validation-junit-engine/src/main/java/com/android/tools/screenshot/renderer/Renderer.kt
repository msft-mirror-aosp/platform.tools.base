 ...
 *
 * LayoutLib performs critical bytecode transformations on Android framework classes at load time.
 * However, the JUnit test engine may load these same classes prematurely, bypassing the necessary
 * transformations and causing rendering failures.
 *
 * This class mitigates this issue by loading LayoutLib and its dependencies into a separate
 * [URLClassLoader]. This ensures that LayoutLib's custom class loader can operate without
 * interference.
 *
 * Communication with the isolated renderer is handled by serializing and deserializing data objects
 * (like [PreviewScreenshot]) across the class loader boundary.
 */
class Renderer : Closeable {
    ...
    init {
        ...
        val classPath =
            (RendererInput.mainAllClassPath + RendererInput.screenshotAllClassPath)
        val projectClassPath =
            (RendererInput.mainProjectClassPath + RendererInput.screenshotProjectClassPath)
        ...
        isolatedClassLoaderForRendering = URLClassLoader(
            layoutLibClassPath.map { it.toURI().toURL() }.toTypedArray(),
            platformClassLoader)

        val rendererClass =
            isolatedClassLoaderForRendering.loadClass(
     ...
     *
     * This method orchestrates the rendering process across the class loader boundary. It first
     * serializes the [screenshot] object, transfers it to the isolated class loader, and invokes
     * the underlying renderer. The resulting [PreviewScreenshotResult] is then serialized back
     ...
     * @return A list of [PreviewScreenshotResult] objects, each detailing the outcome for a
     * rendered preview.
     */
    fun render(screenshot: PreviewScreenshot, outputFolderPath: String): List<PreviewScreenshotResult> {
        ...
        val previewScreenshotClass = isolatedClassLoaderForRendering.loadClass(
            PreviewScreenshot::class.java.name
        )
        val renderMethod = rendererInstance.javaClass.getMethod(
            "render",
        ...
        val resultFromIsolatedLoader = renderMethod.invoke(
            rendererInstance,
            copiedScreenshot,
            outputFolderPath
        ) as Serializable
        ...
        val resultInCurrentLoader = copyObject(
            resultFromIsolatedLoader,
     ...
     *
     * This is achieved by serializing the object into a byte array and then deserializing it
     * using a custom [ObjectInputStream] that resolves classes using the [targetClassLoader].
     *
