...
class DdmlibAndroidDeviceController(
  ...
  companion object {
    ...
    private val INCOMPATIBLE_APK_INSTALLATION_ERROR_NAMES: Set<String> =
      setOf(
  ...
  /** Error code to be reported in result proto as a platform error when unexpected error happens in this device controller. */
  ...
  override fun install(artifact: Artifact): CommandResult {
    require(ANDROID_APK == artifact.type) { "Artifact needs to be of type: $ANDROID_APK, but was ${artifact.type}" }
  ...
  override fun execute(args: List<String>, timeout: Duration?): CommandResult {
    ...
    try {
      ...
      logger.warning { "adb command [${args.joinToString(" ")}] failed due to timeout.\n${e.message}" }
      handler.stop()
    }
  ...
  private fun getInstallOptionsIndex(args: List<String>): Int {
    var index = 0
    for (arg in args) {
      if (arg.endsWith(".apk")) return index
      index++
    }
    return index
  }
          ...
          when (command) {
                  ...
                  override fun processNewLines(lines: Array<out String>) {
                    lines.forEach { line ->
                      if (line.startsWith(SHELL_EXIT_CODE_TAG)) {
                        commandExitCode = line.removePrefix(SHELL_EXIT_CODE_TAG).trim().toIntOrNull() ?: 0
                      } else {
                        processor(line)
                      }
                    }
            ...
            "install" -> {
              ...
              while (retryInstall) {
                ...
                try {
                  ...
                  val errorSummary =
                    object : ErrorSummary {
                      override val errorCode: Int = DdmlibAndroidDeviceControllerErrorCode.ERROR_APK_INSTALL.errorCode
                  ...
                  if (uninstallIncompatibleApks && INCOMPATIBLE_APK_INSTALLATION_ERROR_NAMES.contains(errorSummary.errorName)) {
                  ...
                  if (!retryInstall) {
                    throw UtpException(errorSummary, "Failed to install APK(s): $apkPath", e)
                  }
                }
              }
              receiver
            ...
            "install-multiple" -> {
              ...
              while (retryInstall) {
                ...
                try {
                  controlledDevice.installPackages(apkPaths.map { File(it) }, /* reinstall= */ true, additionalInstallOptions)
                } catch (e: InstallException) {
                  val errorSummary =
                    object : ErrorSummary {
                      override val errorCode: Int = DdmlibAndroidDeviceControllerErrorCode.ERROR_APK_INSTALL.errorCode
                  ...
                  if (uninstallIncompatibleApks && INCOMPATIBLE_APK_INSTALLATION_ERROR_NAMES.contains(errorSummary.errorName)) {
                    ...
                    apkPackageNameResolver.getPackageNameFromApk(apkPaths[0])?.let { uninstallPackageName ->
                      logger.warning("Uninstalling package: $uninstallPackageName")
                      controlledDevice.uninstallPackage(uninstallPackageName)
                      // Only retry installation after initial failure, otherwise
                      // it potentially goes into an infinite loop.
                  ...
                  if (!retryInstall) {
                    throw UtpException(errorSummary, "Failed to install split APK(s): $apkPaths", e)
                  }
                }
              }
              receiver
  ...
  override fun push(artifact: Artifact): Int {
    require(artifact.hasSourcePath() && artifact.hasDestinationPath()) { "Artifact source and destination paths need to be set!" }
