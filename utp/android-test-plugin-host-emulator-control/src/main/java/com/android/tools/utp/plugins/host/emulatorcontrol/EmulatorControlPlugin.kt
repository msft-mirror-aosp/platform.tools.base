...
/**
 * The EmulatorAccessPlugin configures the gRPC access point such that it can become accessible from the running instrumentation tests.
 *
 * This is done by providing a set of well known parameters that can be used by the instrumentation tests to safely access the gRPC endpoint
 * of the emulator.
 *
...
class EmulatorControlPlugin : HostPlugin {
  ...
  @VisibleForTesting lateinit var emulatorControlPluginConfig: EmulatorControlPluginConfig
  ...
  override fun configure(context: Context) {
    ...
    emulatorControlPluginConfig = EmulatorControlPluginConfig.parseFrom(config.configProto!!.value)
    jwtConfig = JwtConfig(emulatorControlPluginConfig.token, emulatorControlPluginConfig.jwkFile)
  }

  private fun pushSourceIfExists(source: String, dest: String, deviceController: DeviceController): Unit {
  ...
  override fun beforeEach(testCase: TestCase?, deviceController: DeviceController) {
    ...
    if (!File(jwtConfig.jwkPath).setLastModified(System.currentTimeMillis())) {
      logger.warning("Unable to update timestamp of ${jwtConfig.jwkPath}" + ", the emulator might delete the key!")
    }
  }
      ...
      if (jwtConfig == INVALID_JWT_CONFIG) {
        ...
        this.context.events.send(
          instrumentationTestOptionsProvided {
            testOptions.putAll(mapOf("grpc.port" to grpcInfo.port.toString(), "grpc.token" to jwtConfig.token))
          }
        )
      }
    }
  ...
  override fun afterAll(testSuiteResult: TestSuiteResult, deviceController: DeviceController, cancelled: Boolean /* = compiled code */) {
   ...
   *
   * First attempts to find the gRPC info from the provided configuration proto. If not specified, attempts to determine the gRPC info from
   * the device serial.
   *
  ...
  private fun getGrpcInfo(deviceController: DeviceController): EmulatorGrpcInfo {
    if (emulatorControlPluginConfig.emulatorGrpcPort != 0) {
      return EmulatorGrpcInfo(emulatorControlPluginConfig.emulatorGrpcPort, emulatorControlPluginConfig.token, "", "", "")
    }
