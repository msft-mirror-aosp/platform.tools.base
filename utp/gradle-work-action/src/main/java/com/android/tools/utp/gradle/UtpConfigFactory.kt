...
/**
 * Creates a runner config proto which you can pass into the Unified Test Platform's test executor.
 *
 * @param uninstallIncompatibleApks uninstalls APKs on the device when an installation failure occurs due to incompatible APKs such as
 *   INSTALL_FAILED_UPDATE_INCOMPATIBLE, INCONSISTENT_CERTIFICATES, etc.
 * @param additionalTestOutputDir an additional test output directory on host machine, or null when disabled.
 */
fun createRunnerConfigProtoForLocalDevice(
  ...
  return RunnerConfigProto.RunnerConfig.newBuilder()
    .apply {
...
private fun createLocalDevice(
  ...
  return DeviceProto.Device.newBuilder()
    .apply {
      deviceIdBuilder.apply { id = deviceSerialNumber }
...
private fun createLocalDeviceProvider(
  ...
  val localConfig = LocalAndroidDeviceProvider.newBuilder().apply { serial = deviceSerialNumber }.build()
  return UtpDependency.ANDROID_DEVICE_PROVIDER_DDMLIB.toExtensionProto(utpDependencies, DdmlibAndroidDeviceProviderConfig::newBuilder) {
...
private fun createTestFixture(
  ...
  return FixtureProto.TestFixture.newBuilder()
    .apply {
      val additionalTestParams: MutableMap<String, String> = mutableMapOf()
      testFixtureIdBuilder.apply { id = UTP_TEST_FIXTURE_ID }
      environment = createEnvironment(outputDir, tmpDir, androidSdkPath, adbExecutablePath, aaptExecutablePath, dexdumpExecutablePath)
      ...
      testDriver =
        createTestDriver(testData, utpDependencies, useOrchestrator, additionalTestOutputOnDeviceDir, shardConfig, additionalTestParams)
      ...
      addHostPlugin(createAndroidTestLogcatPlugin(testData.instrumentationTargetPackageId, utpDependencies))
      if (testData.isTestCoverageEnabled) {
        addHostPlugin(createAndroidTestCoveragePlugin(coverageOutputDir, useOrchestrator, testData, utpDependencies))
      }
      if (additionalTestOutputDir != null) {
        addHostPlugin(createAdditionalTestOutputPlugin(additionalTestOutputDir, additionalTestOutputOnDeviceDir, utpDependencies))
      }
    }
    .build()
}
...
private fun createEmulatorControlPlugin(
  ...
  return UtpDependency.ANDROID_TEST_PLUGIN_HOST_EMULATOR_CONTROL.toExtensionProto(utpDependencies, EmulatorControlPlugin::newBuilder) {
...
private fun createAndroidTestPlugin(utpDependencies: UtpDependencies): Extension {
  return UtpDependency.ANDROID_TEST_PLUGIN.toExtensionProto(utpDependencies, AndroidDevicePlugin::newBuilder) {}
}
...
private fun createEnvironment(
  ...
  return EnvironmentProto.Environment.newBuilder()
    .apply {
      outputDirBuilder.apply { path = outputDir.absolutePath }
      tmpDirBuilder.apply { path = tmpDir.absolutePath }
      androidEnvironmentBuilder.apply {
        androidSdkBuilder.apply {
          sdkPathBuilder.apply { path = androidSdkPath }
          adbPathBuilder.apply { path = adbExecutablePath }
          aaptPathBuilder.apply { path = aaptExecutablePath }
          dexdumpPathBuilder.apply { path = dexdumpExecutablePath }
          testLogDirBuilder.apply {
            path = TEST_LOG_DIR // Must be relative path to outputDir
          }
          testRunLogBuilder.apply { path = TEST_RUNNER_LOG_FILE_NAME }
        }
      }
    }
    .build()
...
private fun createTestDriver(
  ...
  return UtpDependency.ANDROID_DRIVER_INSTRUMENTATION.toExtensionProto(utpDependencies, AndroidInstrumentationDriver::newBuilder) {
 ...
 *
 * It specifies two paths, a directory or file path to writes test coverage files on device and a destination directory on a host. This
 * logic used to be implemented in SimpleTestRunnable and this new implementation is compatible with it (a drop-in replacement).
 */
...
private fun TestData.getTestCoverageFilePath(useOrchestrator: Boolean): String {
  val customCoveragePath = instrumentationRunnerArguments.getOrDefault("coverageFilePath", "")
  return when {
...
private fun createAdditionalTestOutputPlugin(
  ...
  return UtpDependency.ANDROID_TEST_ADDITIONAL_TEST_OUTPUT_PLUGIN.toExtensionProto(
    utpDependencies,
    AndroidAdditionalTestOutputConfig::newBuilder,
  ) {
    additionalOutputDirectoryOnHost = additionalTestOutputDir.absolutePath + File.separator
    additionalTestOutputOnDeviceDir?.let { additionalOutputDirectoryOnDevice = it }
  }
}

private fun createAndroidTestLogcatPlugin(testPackageName: String, utpDependencies: UtpDependencies): Extension {
  return UtpDependency.ANDROID_TEST_LOGCAT_PLUGIN.toExtensionProto(utpDependencies, AndroidTestLogcatConfig::newBuilder) {
...
private fun createApkInstallerPlugin(
  ...
  return UtpDependency.ANDROID_TEST_PLUGIN_APK_INSTALLER.toExtensionProto(utpDependencies, AndroidApkInstallerConfig::newBuilder) {
...
private fun createSingleDeviceExecutor(identifier: String, shardConfig: ShardConfig?): ExecutorProto.SingleDeviceExecutor {
  return ExecutorProto.SingleDeviceExecutor.newBuilder()
    .apply {
      deviceExecutionBuilder.apply {
        deviceIdBuilder.apply { id = identifier }
        testFixtureIdBuilder.apply { id = UTP_TEST_FIXTURE_ID }
      }
      shardConfig?.let {
...
private fun UtpDependency.toExtensionProto(utpDependencies: UtpDependencies, config: Any? = null): Extension {
  val builder =
    Extension.newBuilder().apply {
      label = LabelProto.Label.newBuilder().apply { label = name }.build()
      className = mainClass
      addAllJar(mapperFunc(utpDependencies).files.map { PathProto.Path.newBuilder().apply { path = it.absolutePath }.build() })
...
private fun <T : Message.Builder> UtpDependency.toExtensionProto(
  ...
  val config = newBuilder().apply { configFunc(this) }.build()
