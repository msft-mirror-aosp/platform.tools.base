...
class EmulatorControlUtilsTest {
  ...
  @get:Rule val testDir = TemporaryFolder()
  ...
  fun audiencesInTheToken() {
    ...
    val publicKeysetHandle = JwkSetConverter.toPublicKeysetHandle(File(jwtConfig.jwkPath).readText(Charsets.UTF_8))
  ...
  fun noFallbackIfJwksAndTokenIsPresent() {
    ...
    val aud = setOf("a", "b")
    val info = EmulatorGrpcInfo(123, "studio-token", null, testDir.root.absolutePath, "some_fake_allowlist_path")
  ...
  fun noFallbackIfJwksAndNoTokenIsPresent() {
    ...
    val aud = setOf("a", "b")
    val info = EmulatorGrpcInfo(123, null, null, testDir.root.absolutePath, "some-fake-access-list-path")
    val jwtConfig = createTokenConfig(aud, 20, iss, info)
    assertThat(jwtConfig.token).isNotEmpty()
