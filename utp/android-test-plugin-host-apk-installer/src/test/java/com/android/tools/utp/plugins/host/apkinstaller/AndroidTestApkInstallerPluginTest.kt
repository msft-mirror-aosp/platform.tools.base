...
class AndroidTestApkInstallerPluginTest {
  ...
  @Mock(answer = Answers.RETURNS_DEEP_STUBS) private lateinit var mockDeviceController: DeviceController

  @Mock(answer = Answers.RETURNS_DEEP_STUBS) private lateinit var mockSubprocessComponent: SubprocessComponent
  ...
  companion object {
    ...
    private val createTestFixture: (TestFixtureId, List<String>?) -> TestFixture = { fixtureId, installablePath ->
      TestFixture.newBuilder()
        .apply {
          testFixtureId = fixtureId
          setup = createTestSetup(installablePath)
          environmentBuilder.apply { androidEnvironmentBuilder.apply { androidSdk = AndroidSdkProto.AndroidSdk.newBuilder().build() } }
        }
        .build()
    }

    private val createTestSetup: (List<String>?) -> SetupProto.TestSetup = { installablePath ->
      SetupProto.TestSetup.newBuilder().apply { installablePath?.forEach { addInstallableBuilder().sourcePathBuilder.path = it } }.build()
    }
    ...
    private val createPluginConfig: (AndroidApkInstallerConfig?, List<String>?) -> ConfigBase = { pluginConfig, installablePath ->
    ...
    private val installErrorSummary =
      object : ErrorSummary {
  ...
  private fun createPlugin(config: AndroidApkInstallerConfig, installablePath: List<String>? = null): AndroidTestApkInstallerPlugin {
    return AndroidTestApkInstallerPlugin(mockLogger) { mockSubprocessComponent }
      .apply {
        configure(
          object : Context {
  ...
  fun setup() {
    ...
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to mockDeviceApiLevel))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
  }

  @Test
  fun emptyAPKInstallListTest() {
    createPlugin(AndroidApkInstallerConfig.newBuilder().apply { clearApksToInstall() }.build(), null).apply {
  ...
  fun splitAPKWithAdditionalInstallOptionTestWithInstallable() {
    ...
    createPlugin(
        AndroidApkInstallerConfig.newBuilder()
          .apply {
            addApksToInstallBuilder()
              .apply {
      ...
      .apply { beforeAll(mockDeviceController) }
    ...
    inOrder.verify(mockDeviceController).execute(eq(listOf(INSTALL_MULTIPLE_CMD, "-t") + additionalInstallOptions + testApkPaths), eq(null))
  ...
  fun splitAPKInstallFail() {
    ...
    val exception =
      assertThrows(UtpException::class.java) {
        createPlugin(
            AndroidApkInstallerConfig.newBuilder()
              .apply {
                addApksToInstallBuilder()
    ...
    inOrder.verify(mockDeviceController).execute(eq(listOf(INSTALL_MULTIPLE_CMD, "-t") + additionalInstallOptions + testApkPaths), eq(null))
    assertEquals(
      UtpException(installErrorSummary, "Failed to install APK: $testApkPaths on device $mockDeviceSerial.\n${adbInstallFailOutput}")
        .message,
      exception.message,
    )
  }
  ...
  fun installTestServiceApi23() {
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    `when`(mockDeviceController.execute(anyList(), eq(null))).thenReturn(CommandResult(0, listOf()))
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "23"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    createPlugin(
        AndroidApkInstallerConfig.newBuilder()
          .apply {
    ...
    testApkPaths.forEach {
      verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t", "-g") + additionalInstallOptions + it, null)
    }
  }
  ...
  fun installTestServiceApi24() {
    ...
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "24"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    createPlugin(
        AndroidApkInstallerConfig.newBuilder()
          .apply {
    ...
    testApkPaths.forEach {
      verify(mockDeviceController)
        .execute(listOf(INSTALL_MULTIPLE_CMD, "-t", "-g", "--user", mockUserId) + additionalInstallOptions + it, null)
    }
  }
  ...
  fun installTestServiceApi30() {
    ...
    `when`(mockDeviceController.execute(eq(listOf("shell", "am", "get-current-user")), eq(null)))
      .thenReturn(CommandResult(0, listOf(mockUserId)))
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "30"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    createPlugin(
        AndroidApkInstallerConfig.newBuilder()
          .apply {
    ...
    testApkPaths.forEach {
      verify(mockDeviceController)
        .execute(listOf(INSTALL_MULTIPLE_CMD, "-t", "-g", "--force-queryable", "--user", mockUserId) + additionalInstallOptions + it, null)
    }
  }
  ...
  fun invalidUserIdFormat() {
    ...
    `when`(mockDeviceController.execute(listOf("shell", "am", "get-current-user"))).thenReturn(CommandResult(0, listOf(invalidUserId)))
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "24"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    createPlugin(AndroidApkInstallerConfig.getDefaultInstance(), testArtifactsPath).apply { beforeAll(mockDeviceController) }
    verify(mockDeviceController, times(4)).getDevice()
    verify(mockLogger)
      .warning("Unexpected output of command get-current-user, " + "expected a user id found the following output: $invalidUserId")
    testArtifactsPath.forEach { verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t") + it, null) }
  }

  @Test
  fun getUserIdExecutionFailed() {
    `when`(mockDeviceController.execute(listOf("shell", "am", "get-current-user"))).thenReturn(CommandResult(1, listOf()))
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "24"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)

    createPlugin(AndroidApkInstallerConfig.getDefaultInstance()).apply { beforeAll(mockDeviceController) }
    verify(mockDeviceController, times(5)).getDevice()
    verifyNoMoreInteractions(mockDeviceController)
    verify(mockLogger).warning("Failed to execute command to obtain user ID from " + "device $mockDeviceSerial")
  }
  ...
  fun failToInstallInstallable() {
    ...
    assertEquals(
      UtpException(installErrorSummary, "Failed to install APK: $testArtifactPath1 on device " + "$mockDeviceSerial.").message,
      exception.message,
    )
  }
  ...
  fun nonSplitAPKTest() {
        ...
        AndroidApkInstallerConfig.newBuilder()
          .apply {
            addApksToInstallBuilder()
              .apply {
                addAllApkPaths(testApkPaths)
                installOptionsBuilder.apply { addAllCommandLineParameter(additionalInstallOptions) }.build()
              }
              .build()
          }
          .build()
    ...
    verify(mockLogger).info("Installing $testApkPaths on device $mockDeviceSerial.")
    verify(mockDeviceController, times(4)).getDevice()
    testApkPaths.forEach { verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t") + additionalInstallOptions + it, null) }
  }
  ...
  fun nonSplitAPKWithForceReinstallTest() {
    ...
    verify(mockDeviceController, times(4)).getDevice()
    testApkPaths.forEach {
      verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t", "-r", "-d") + additionalInstallOptions + it, null)
    }
  }
  ...
  fun nonSplitAPKFailTest() {
    ...
    verify(mockLogger).info("Installing $testApkPaths on device $mockDeviceSerial.")
    verify(mockDeviceController, times(4)).getDevice()
    verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t") + additionalInstallOptions + testApkPaths.first(), null)
    assertEquals(
      UtpException(
          installErrorSummary,
          "Failed to install APK: ${testApkPaths.first()} on " + "device $mockDeviceSerial.\n${adbInstallFailOutput}",
        )
        .message,
      exception.message,
    )
  ...
  fun installableWithEmptyPath() {
    ...
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    `when`(mockDeviceController.execute(anyList(), eq(null))).thenReturn(CommandResult(0, listOf()))
    createPlugin(AndroidApkInstallerConfig.getDefaultInstance(), listOf(apkPath, "")).apply { beforeAll(mockDeviceController) }
    verify(mockDeviceController, times(4)).getDevice()
    verify(mockLogger).info("Installing installable artifact: $apkPath on device $mockDeviceSerial.")
    verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t") + apkPath, null)
  ...
  fun emptyTestApkPath() {
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    createPlugin(AndroidApkInstallerConfig.newBuilder().apply { addApksToInstallBuilder().defaultInstanceForType }.build()).apply {
      beforeAll(mockDeviceController)
    }
    verify(mockDeviceController, times(4)).getDevice()
    verifyNoMoreInteractions(mockDeviceController)
  ...
  fun nonSplitAPKTestWithTimeout() {
    ...
    testApkPaths.forEach {
      verify(mockDeviceController)
        .execute(listOf(INSTALL_MULTIPLE_CMD, "-t") + additionalInstallOptions + it, Duration.ofSeconds(installTimeout.toLong()))
    }
  }
  ...
  fun splitInstallOnDeviceApiLowerThanMinApiRequiredForSplitInstallTest() {
    ...
    assertEquals(
      UtpException(
          installErrorSummary,
          "Minimum API level for installing SPLIT_APK " + "feature is 21 but device $mockDeviceSerial is API level 20.",
        )
        .message,
      exception.message,
    )
  ...
  fun nonSplitInstallOnDeviceApiLowerThanMinApiRequiredForSplitInstallTest() {
    ...
    verify(mockLogger).info("Installing $testApkPaths on device $mockDeviceSerial.")
    verify(mockDeviceController, times(5)).getDevice()
    testApkPaths.forEach { verify(mockDeviceController).execute(listOf(BASE_INSTALL_CMD, "-t") + additionalInstallOptions + it, null) }
  }
  ...
  fun uninstallSuccessTest() {
    `when`(mockDeviceController.getDevice().serial).thenReturn(mockDeviceSerial)
    `when`(
        mockSubprocessComponent
          .subprocess()
          .executeAsync(eq(listOf("/Android/Sdk/aapt", "dump", "badging", "base.apk")), anyMap(), any(), eq(null))
      )
      .then {
    ...
    plugin.afterAll(TestSuiteResultProto.TestSuiteResult.getDefaultInstance(), mockDeviceController)
  ...
  fun installWithForceCompilationOption() {
    ...
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "30"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    `when`(mockSubprocessComponent.subprocess().executeAsync(anyList(), anyMap(), anyOrNull(), anyOrNull())).then {
    ...
    verify(mockDeviceController, times(5)).getDevice()
    testApkPaths.forEach {
      verify(mockDeviceController).execute(listOf(INSTALL_MULTIPLE_CMD, "-t", "--user", mockUserId) + additionalInstallOptions + it, null)
    }

    `verify`(mockSubprocessComponent.subprocess())
      .executeAsync(eq(listOf("/Android/Sdk/aapt", "dump", "badging", "base.apk")), anyMap(), any(), eq(null))

    verify(mockDeviceController)
      .execute(listOf("shell", "cmd", "package", "compile", "-m", "speed", "-f", "com.example.myapplication"), null)
  }
  ...
  fun disableAnrInDebugRunApi35() {
    ...
    // given
    mockDeviceProperties = AndroidDeviceProperties(mapOf(DEVICE_API_LEVEL to "35"))
    `when`(mockDeviceController.getDevice().properties).thenReturn(mockDeviceProperties)
    ...
    // then
    verify(mockDeviceController).execute(listOf("shell", "am", "set-debug-app", packageName))

    // when
    plugin.afterAll(TestSuiteResultProto.TestSuiteResult.getDefaultInstance(), mockDeviceController)

    // then
    verify(mockDeviceController).execute(listOf("shell", "am", "clear-debug-app"))
  }
}
