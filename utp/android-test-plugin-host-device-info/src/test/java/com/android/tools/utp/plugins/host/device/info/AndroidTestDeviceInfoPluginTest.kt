...
class AndroidTestDeviceInfoPluginTest {
  ...
  private val androidDeviceProperty =
    AndroidDeviceProperties(
      map = mapOf("ro.product.manufacturer" to deviceManufacturer, "ro.product.cpu.abilist" to deviceAbis.joinToString(",")),
      deviceApiLevel = "28",
    )
  ...
  fun setup() {
    ...
    `when`(mockDeviceController.deviceShell(anyList(), nullable(Long::class.java))).thenAnswer {
      @Suppress("UNCHECKED_CAST") val command = (it.arguments[0] as List<String>).joinToString(separator = " ")
  ...
  @Test
  fun parseProcessors() {
    val cpuInfo = listOf("header", "model name : cpu_type_a", "model name : cpu_type_b", "model name : cpu_type_a", "footer")
  ...
  fun parseEmptyProcessors() {
    val cpuInfo = listOf("header", "footer")
    val processors = cpuInfo.getDeviceProcessors()
