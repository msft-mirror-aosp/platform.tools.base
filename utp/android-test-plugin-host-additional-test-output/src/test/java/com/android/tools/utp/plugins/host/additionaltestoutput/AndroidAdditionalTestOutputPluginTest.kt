...
class AndroidAdditionalTestOutputPluginTest {
  ...
  private fun createTestArtifact(filePathOnDevice: String, filePathOnHost: String): TestArtifactProto.Artifact {
    return TestArtifactProto.Artifact.newBuilder()
      .apply {
  ...
  private fun createPlugin(config: AndroidAdditionalTestOutputConfig): AndroidAdditionalTestOutputPlugin {
    ...
    return AndroidAdditionalTestOutputPlugin(mockLogger).apply { configure(mockContext) }
  }
  ...
  private fun runPlugin(
    ...
    val config = AndroidAdditionalTestOutputConfig.newBuilder().apply { configFunc(this) }.build()
  ...
  fun runPluginAndSuccess() {
    ...
    `when`(mockDeviceController.execute(eq(listOf("shell", "[[ -d \"${deviceDir}/output1.txt\" ]]")), nullable(Duration::class.java)))
      .thenReturn(CommandResult(1, listOf()))

    `when`(mockDeviceController.execute(eq(listOf("shell", "[[ -d \"${deviceDir}/output2.txt\" ]]")), nullable(Duration::class.java)))
      .thenReturn(CommandResult(1, listOf()))

    `when`(mockDeviceController.execute(eq(listOf("shell", "ls \"${deviceDir}/subdir\" | cat")), nullable(Duration::class.java)))
      .thenReturn(CommandResult(0, listOf("output3.txt")))

    `when`(
        mockDeviceController.execute(eq(listOf("shell", "[[ -d \"${deviceDir}/subdir/output3.txt\" ]]")), nullable(Duration::class.java))
      )
      .thenReturn(CommandResult(1, listOf()))
    ...
    inOrder(mockDeviceController).apply {
      ...
      verify(mockDeviceController)
        .pull(createTestArtifact("${deviceDir}/subdir/output3.txt", "${hostDir}${File.separator}subdir${File.separator}output3.txt"))
    }
  }
  ...
  fun runPluginAndFailsOnFileCopy() {
    ...
    `when`(mockDeviceController.pull(eq(createTestArtifact("${deviceDir}/output1.txt", "${hostDir}${File.separator}output1.txt"))))
      .thenThrow(exception)
  ...
  fun benchmarkResultFilesShouldBeCopiedAfterEachTest() {
    ...
    val benchmarkMessage =
      ...
      """
        .trimIndent()
    val benchmarkOutputDir = "/sdcard/Android/media/com.example.macrobenchmark/additional_test_output"

    val traceFiles =
      listOf("FrameTimingBenchmark_start_iter000_2021-07-15-21-32-39.trace", "FrameTimingBenchmark_start_iter001_2021-07-15-21-33-09.trace")
    `when`(mockDeviceController.execute(eq(listOf("shell", "ls \"${benchmarkOutputDir}\" | cat")), nullable(Duration::class.java)))
      .thenReturn(CommandResult(0, traceFiles))
    traceFiles.forEach {
      `when`(mockDeviceController.execute(eq(listOf("shell", "[[ -d \"${benchmarkOutputDir}/$it\" ]]")), nullable(Duration::class.java)))
        .thenReturn(CommandResult(1, listOf()))
    }

    val testResultWithBenchmark =
    ...
    assertThat(testResultAfterEach.getOutputArtifact(0).label.label).isEqualTo("additionaltestoutput.benchmark.message")
    ...
    assertThat(testResultAfterEach.getOutputArtifact(1).label.label).isEqualTo("additionaltestoutput.benchmark.trace")
    ...
    assertThat(testResultAfterEach.getOutputArtifact(2).label.label).isEqualTo("additionaltestoutput.benchmark.trace")
  ...
  fun runPluginWithTestStorageService() {
    ...
    runPlugin { additionalOutputDirectoryOnHost = hostDir }
