...
class AndroidAdditionalTestOutputPlugin(private val logger: Logger = getLogger()) : HostPluginAdapter() {
  ...
  private fun getApiLevel(deviceController: DeviceController): Int? {
    return (deviceController.getDevice().properties as? AndroidDeviceProperties)?.deviceApiLevel?.toIntOrNull()
  }
  ...
  /** Creates an empty directory on test device. If a directory exists at the given path, it removes all contents in the directory. */
  private fun createEmptyDirectoryOnDevice(deviceController: DeviceController) {
    ...
    if (deviceController.isTestServiceInstalled()) {
      deviceController.deviceShellAndCheckSuccess("rm -rf \"${TEST_STORAGE_SERVICE_OUTPUT_DIR}\"")
      deviceController.deviceShellAndCheckSuccess("mkdir -p \"${TEST_STORAGE_SERVICE_OUTPUT_DIR}\"")
    }
  }
  ...
  override fun afterEachWithReturn(testResult: TestResult, deviceController: DeviceController, cancelled: Boolean): TestResult {
    ...
    return builder.build().also { context.events.sendTestResultUpdate(it) }
  }

  /** Retrieves benchmark output text from a given [testResult] and appends them into the [builder]. */
  private fun addBenchmarkOutput(testResult: TestResult, deviceController: DeviceController, builder: TestResult.Builder) {
    val keyForMessage =
      benchmarkKey(
        testResult,
        orderedKeys = listOf(BENCHMARK_V3_TEST_METRICS_KEY, BENCHMARK_V2_TEST_METRICS_KEY, BENCHMARK_TEST_METRICS_KEY),
      )
    val keyForCopy = benchmarkKey(testResult, orderedKeys = listOf(BENCHMARK_V2_TEST_METRICS_KEY, BENCHMARK_TEST_METRICS_KEY))
    ...
    val benchmarkOutputDir = benchmarkTestDetail(testResult, BENCHMARK_PATH_TEST_METRICS_KEY)?.value ?: ""
    addBenchmarkMessage(benchmarkMessageWithoutPrefix, builder, testResult)
    addBenchmarkFiles(copyMessageWithoutPrefix, benchmarkOutputDir, deviceController, builder)
  }

  /**
   * Gets the benchmark output from the [testResult] map. The order of the keys is important here, given we look at the first key (in that
   * specific order) that exists in [testResult].
   *
   * We do this because, benchmark output is versioned, and we typically want outputs in the latest version while gracefully falling back to
   * a prior version if that version of the output is not a part of [testResult]. That might happen when an older version of the
  ...
  private fun benchmarkKey(testResult: TestResult, orderedKeys: List<String>): String? {
    return orderedKeys.firstOrNull { key -> testResult.detailsList.firstOrNull { entry -> entry.key == key } != null }
  }
  ...
  /**
   * Finds a file path to a profiler output file from [benchmarkMessage], pulls files from the test device to the host machine and adds them
   * to the [builder].
   */
  private fun addBenchmarkFiles(
    ...
    val benchmarkFileRelativePaths =
      benchmarkMessage
        ...
        .filter { matchValue -> matchValue.startsWith(BENCHMARK_TRACE_FILE_PREFIX) }
        .map { matchValue -> matchValue.replace(BENCHMARK_TRACE_FILE_PREFIX, "") }
        .toSet()

    val hostOutputDir = File(config.additionalOutputDirectoryOnHost).absolutePath
    copyFilesFromDeviceToHost(deviceController, benchmarkOutputDir, hostOutputDir, benchmarkFileRelativePaths::contains)
    ...
    benchmarkFileRelativePaths.forEach { relativeFilePath ->
      builder.addOutputArtifactBuilder().apply {
        ...
        sourcePathBuilder.apply { path = "$hostOutputDir${File.separator}${relativeFilePath}" }
      }
    }
  }
  ...
  private fun addBenchmarkMessage(benchmarkMessage: String, builder: TestResult.Builder, testResult: TestResult) {
    ...
    val benchmarkMessageOutputFile =
      File(config.additionalOutputDirectoryOnHost, "additionaltestoutput.benchmark.message_${fileNameSuffix}.txt")
    ...
    builder.addOutputArtifactBuilder().apply {
      ...
      sourcePathBuilder.apply { path = benchmarkMessageOutputFile.absolutePath }
    }
  }
  ...
  override fun afterAllWithReturn(
    ...
    try {
      ...
      logger.log(Level.WARNING, e) { "Failed to retrieve additional test outputs from device." }
    }
    try {
      if (deviceController.isTestServiceInstalled()) {
      ...
      logger.log(Level.WARNING, e) { "Failed to retrieve test storage service outputs from device." }
    }
    ...
    deviceController
      .deviceShellAndCheckSuccess(
        "content query --uri content://media/external/file --user $androidUser --projection _id:_data " +
          "--where \"mime_type IS NOT NULL AND _data LIKE '$normalizedDeviceDir%'\""
      )
      .output
      .forEach {
        ...
        if (filter(relativeFilePath)) {
          ...
          val result =
            deviceController.deviceShellAndCheckSuccess("content read --user $androidUser --uri content://media/external/file/$id")
          hostFile.outputStream().use { fileOutputStream -> result.byteOutputStream.copyTo(fileOutputStream) }
        }
      }
  }
  ...
  private fun getCurrentAndroidUser(deviceController: DeviceController): String {
    return deviceController
      .deviceShellAndCheckSuccess("am get-current-user")
      .output
      .asSequence()
      .map(String::trim)
  ...
  private fun DeviceController.deviceShellAndCheckSuccess(vararg commands: String): CommandResult {
    ...
    if (result.statusCode != 0) {
      logger.warning {
        "Shell command failed (${result.statusCode}): " + "${commands.joinToString(" ")}\n" + result.output.joinToString("\n")
      }
    }
    return result
  }
