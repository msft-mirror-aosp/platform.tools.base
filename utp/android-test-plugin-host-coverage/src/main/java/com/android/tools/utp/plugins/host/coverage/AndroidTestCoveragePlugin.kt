...
/** A host plugin that retrieves test coverage data files (.ec) from a device to a host machine. */
class AndroidTestCoveragePlugin(
  private val logger: Logger = getLogger(),
  private val createRandomId: () -> String = { UUID.randomUUID().toString() },
) : HostPlugin {
  ...
  override fun configure(context: Context) {
    ...
    testCoverageConfig = AndroidTestCoverageConfig.parseFrom(config.configProto!!.value)
  }
  ...
  override fun afterAll(testSuiteResult: TestSuiteResult, deviceController: DeviceController, cancelled: Boolean) {
    try {
      ...
      logger.log(Level.WARNING, e) { "Failed to retrieve code coverage data from device." }
    }
  }
  ...
  private fun retrieveCoverageFiles(deviceController: DeviceController) {
    ...
    try {
      when (testCoverageConfig.testCoveragePathOnDeviceCase) {
        ...
        TestCoveragePathOnDeviceCase.MULTIPLE_COVERAGE_FILES_IN_DIRECTORY -> this::copyCoverageFilesInDirectoryToHost
        else -> throw UnsupportedOperationException("test_coverage_path_on_device must be specified.")
  ...
  /**
   * Executes adb shell command wrapped with run-as command if runAsPackageName is not blank in [testCoverageConfig] and
   * [useTestStorageService] is false, otherwise it executes the command as-is.
   *
   * When [useTestStorageService] is true, test coverage file is always written under non-package private directory
   * ([TEST_STORAGE_SERVICE_OUTPUT_DIR]), so run-as command is not needed (actually, you cannot use it as tested package may not have access
   * to sdcard).
   */
