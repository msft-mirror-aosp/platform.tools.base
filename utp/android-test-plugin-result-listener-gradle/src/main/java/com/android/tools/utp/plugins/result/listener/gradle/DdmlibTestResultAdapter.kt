 ...
 *
 * This adapter allows consumers that rely on the Ddmlib listener interface to process results generated by UTP.
 *
 ...
 */
class DdmlibTestResultAdapter(private val ddmlibTestRunName: String, private val ddmlibTestResultListener: XmlTestRunListener) {
  ...
  fun onTestResultEvent(testResultEvent: GradleAndroidTestResultListenerProto.TestResultEvent) {
    when (testResultEvent.stateCase) {
      StateCase.TEST_SUITE_STARTED -> {
        ...
        val testSuite: TestSuiteResultProto.TestSuiteMetaData = testResultEvent.testSuiteStarted.testSuiteMetadata.unpack()
        ddmlibTestResultListener.testRunStarted(ddmlibTestRunName, testSuite.scheduledTestCaseCount)
      }

      StateCase.TEST_CASE_STARTED -> {
        val testCase: TestCaseProto.TestCase = testResultEvent.testCaseStarted.testCase.unpack()
        val testId = TestIdentifier("${testCase.testPackage}.${testCase.testClass}", testCase.testMethod)
        ddmlibTestResultListener.testStarted(testId)
      }

      StateCase.TEST_CASE_FINISHED -> {
        val testResult: TestResultProto.TestResult = testResultEvent.testCaseFinished.testCaseResult.unpack()
        val testId = TestIdentifier("${testResult.testCase.testPackage}.${testResult.testCase.testClass}", testResult.testCase.testMethod)
      ...
      StateCase.TEST_SUITE_FINISHED -> {
        ...
        val testSuiteResult: TestSuiteResultProto.TestSuiteResult = testResultEvent.testSuiteFinished.testSuiteResult.unpack()
        if (testSuiteResult.platformError.errorsCount > 0) {
          ddmlibTestResultListener.addSystemError(getPlatformErrorMessage(testSuiteResult) + "\n")
        }
        testSuiteResult.issueList.forEach { issue -> ddmlibTestResultListener.addSystemError(issue.message + "\n") }
        ddmlibTestResultListener.testRunEnded(System.currentTimeMillis() - startTimestamp, mapOf())
      }
