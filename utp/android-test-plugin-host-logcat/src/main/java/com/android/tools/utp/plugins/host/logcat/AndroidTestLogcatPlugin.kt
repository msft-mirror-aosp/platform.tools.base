...
class AndroidTestLogcatPlugin(private val logger: Logger = getLogger(), private val logcatTimeoutSeconds: Long = LOGCAT_TIMEOUT_SECONDS) :
  HostPluginAdapter() {
  ...
  private val crashLogcatFinishMatcher: Regex by lazy { Regex(".*I\\sProcess.*Sending\\ssignal.*PID:.*${testPid}.*SIG:\\s9") }
  private val crashLogcatProgressMatcher: Regex by lazy { Regex(".*${testPid}.*${TEST_CRASH_INDICATOR}.*") }

  override fun configure(context: Context) {
    val config = context[Context.CONFIG_KEY] as ProtoConfig
    outputDir = config.environment.outputDirectory
    this.targetTestProcessName = AndroidTestLogcatConfig.parseFrom(config.configProto!!.value).targetTestProcessName
    crashLogcatStartMatcher = Regex(".*E\\sAndroidRuntime:\\sProcess:\\s${targetTestProcessName}.*")
  ...
  private fun generateLogcatFileName(testPackageAndClass: String, testMethod: String): String {
    return File(outputDir, "logcat-$testPackageAndClass-$testMethod.txt".replace(INVALID_FILE_NAME_CHARS, "_")).absolutePath
  }
  ...
  /**
   * Parse test package, class, and method info from logcat Assumes that the line is of the form "**TestRunner: started:
