...
class DeviceFileSystem {
  ...
  /**
   * Map of "normalized" path to [DeviceFileState]. Note that [DeviceFileState.path] may be different from the map key due to path
   * normalization.
   */
  ...
  internal fun copyFrom(source: DeviceFileSystem) {
    entries.clear()
    source.entries.forEach { entry -> entries.put(entry.key, entry.value) }
  }
  ...
  /** Returns the [DeviceFileState] for the given [filePath], or `null` if the file entry does not exist. */
  ...
  fun getDirectoryFiles(directoryPath: String): List<DeviceFileState> {
    ...
    ) + entries.filter { it.key.parentPath == npath }.map { it.value }
  }
  ...
  private fun getOrCreateDirectory(path: NormalizedPath): DeviceFileState {
    // Create parent dir
    path.parentPath?.also { getOrCreateDirectory(it) }
    ...
    return entries[path]?.also {
      ...
      ?: run {
        DeviceFileState(path = path.path, permission = 0, modifiedDate = 0, bytes = ByteArray(0), kind = DeviceFileState.Kind.Directory)
          .also { entries[path] = it }
      }
  }
