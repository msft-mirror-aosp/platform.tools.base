...
class HeloHandler : DdmPacketHandler {
  ...
  override fun handlePacket(
    ...
    val instructionSetDescription =
      if (device.cpuAbi.contains("64")) {
        "64-bit"
      } else {
        "32-bit"
      }
    val abi =
      if (device.cpuAbi.isEmpty()) {
    ...
    try {
      val apnmPayloadLength =
        (4 + appName.length * 2 + (if (writeUserId) 4 else 0) + if (writePackageName) 4 + packageName.length * 2 else 0)
      ...
      val apnmPacket = DdmPacket.createCommand(client.nextDdmsCommandId(), DdmPacket.encodeChunkType("APNM"), apmnPayload.array())
