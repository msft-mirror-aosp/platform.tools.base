...
class RootCommandHandler : DeviceCommandHandler("root") {
  ...
  companion object {
    ...
    internal fun restartDeviceAsRoot(fakeAdbServer: FakeAdbServer, device: DeviceState, isRoot: Boolean) {
      fakeAdbServer.coroutineScope.launch {
        val newDeviceState = fakeAdbServer.restartDeviceAsync(device) { it.copy(isRoot = isRoot) }.await()
