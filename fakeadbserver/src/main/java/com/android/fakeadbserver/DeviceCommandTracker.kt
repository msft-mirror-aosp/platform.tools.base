...
/**
 * Tracks currently running commands (see [DeviceCommandHandler]) on the given [device][deviceSerial] so that they can be cancelled if the
...
internal class DeviceCommandTracker(private val deviceSerial: String) : AutoCloseable {
  ...
  inline fun <R> trackCommand(command: String, scope: CoroutineScope, socket: Socket, block: () -> R): R {
    throwIfClosed()
  ...
  override fun close() {
    if (closed) return
    closed = true
    activeCommands.values.forEach { it.close() }
