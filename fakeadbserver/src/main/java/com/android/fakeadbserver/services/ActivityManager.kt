    ...
    return when (val cmd = args[0]) {
      ...
      "capabilities" -> {
        ...
        val deviceCapabilities =
          deviceState.deviceCapabilities
            ?: run {
        ...
        if (protobufOutput) {
          val proto =
            AmCapabilitiesProto.Capabilities.newBuilder()
              .also { capabilitiesProto ->
                capabilitiesProto.addAllValues(
                  deviceCapabilities.capabilities.map { AmCapabilitiesProto.Capability.newBuilder().setName(it).build() }
                )
                capabilitiesProto.addAllVmCapabilities(
                  deviceCapabilities.vmCapabilities.map { AmCapabilitiesProto.VMCapability.newBuilder().setName(it).build() }
                )
                capabilitiesProto.addAllFrameworkCapabilities(
                  deviceCapabilities.frameworkCapabilities.map { AmCapabilitiesProto.FrameworkCapability.newBuilder().setName(it).build() }
                )
                capabilitiesProto.setVmInfo(
                  AmCapabilitiesProto.VMInfo.newBuilder().also { vmInfoProto ->
                    deviceCapabilities.vmInfo?.name?.also { vmInfoProto.setName(it) }
                    deviceCapabilities.vmInfo?.version?.also { vmInfoProto.setVersion(it) }
                  }
                )
              }
              .build()
          ...
          deviceCapabilities.capabilities.forEach { shellCommandOutput.writeStdout(it) }
          deviceCapabilities.vmCapabilities.forEach { shellCommandOutput.writeStdout("vm:$it") }
          deviceCapabilities.frameworkCapabilities.forEach { shellCommandOutput.writeStdout("framework:$it") }
