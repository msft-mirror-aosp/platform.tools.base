...
class MdnsStateChangeHub : StateChangeHub<MdnsStateChangeHandlerFactory>() {
  ...
  fun mdnsServiceListChanged(serviceList: Collection<MdnsService>) {
    synchronized(mHandlers) {
      mHandlers.forEach { (stateChangeQueue, handlerFactory) ->
        stateChangeQueue.add(handlerFactory.createMdnsServiceListChangedHandler(serviceList))
      }
    }
  }
}
