...
/**
 * Handle a connection according to adb SERVICE.TXT and protocol.txt. In a nutshell, a smart connection is able to request HOST (adb server)
 * services or LOCAL (adbd running on a device) services. The smart part is the "transport" part which allows to "route" service request to
 * a device after the connection has been established with the HOST.
 */
// TODO: Rename this class SmartConnection (not doing it now to help review process but should
// be in next CL).
internal class ConnectionHandler(private val mServer: FakeAdbServer, socket: SocketChannel) : Runnable {

  private val mSmartSocket: SmartSocket

  /**
   * On demand access to the [DeviceState] associated to the current request being processed. Most [HostCommandHandler] and
   * [DeviceCommandHandler] implementation require either a single device or no device to be specified in the request device "header". There
   * are a few exceptions where the device is optional, so we need to account for that.
   */
  ...
  /**
   * Repeatedly read host service requests and serve them until a local request is received. After service the local request, the connection
   * is closed. Note that a host request can also request the connection to end.
   */
  override fun run() {
  ...
  private fun handleHostService(request: ServiceRequest, socketScope: CoroutineScope) {
    when (request.nextToken()) {
      "host" -> {
        ...
        mTargetDeviceSelector = DeviceStateSelector.forCollection(deviceListProvider = { findAnyDevice() }, errorReporter = {})
      }
      ...
      "host-serial" -> {
        ...
        mTargetDeviceSelector =
          DeviceStateSelector.forOptional(
            ...
            errorReporter = { reportErrorAndStop("No device with serial: '$serial' is connected.") },
          )
      }
      ...
      "host-transport-id" -> {
        ...
        mTargetDeviceSelector =
          DeviceStateSelector.forOptional(
            ...
            errorReporter = { reportErrorAndStop("No device with transport id: '$transportId' is connected.") },
          )
      }
        ...
        mTargetDeviceSelector =
          DeviceStateSelector.forError(
            errorReporter = {
              val err = String.format(Locale.US, "Command not handled '%s' '%s'", request.currToken(), request.original())
  ...
  private fun reportDevicesErrorAndStop(numDevices: Int) {
    val msg: String =
      if (numDevices == 0) {
        ...
        String.format(Locale.US, "More than one device found (%d). Please specify which.", numDevices)
      }
      ...
      is DeviceStateSelector.DeviceResult.One -> {
        targetDevice.deviceState.trackCommand(request.original(), socketScope, mSmartSocket.socket) {
          ...
          for (handler in mServer.handlers) {
            val accepted = handler.accept(mServer, socketScope, mSmartSocket.socket, targetDevice.deviceState, serviceName, command)
            if (accepted) {
  ...
  private fun dispatchToHostHandlers(request: ServiceRequest, socketScope: CoroutineScope) {
    ...
    if (handler == null) {
      val err = String.format(Locale.US, "Unimplemented host command received: '%s'", hostCommand)
      mSmartSocket.sendFailWithReason(err)
      return
    }
    mKeepRunning = handler.invoke(mServer, socketScope, mSmartSocket.socket, mTargetDeviceSelector, hostCommand, request.remaining())
  }
  ...
  private fun findDevicesWithProtocol(type: HostConnectionType): List<DeviceState> {
    return findDevicesWithProtocols(listOf(type))
  }

  private fun findDevicesWithProtocols(types: List<HostConnectionType>): List<DeviceState> {
    val candidates: List<DeviceState>
    val devices: MutableList<DeviceState> = ArrayList()
    try {
      candidates = mServer.deviceListCopy.get()
    } catch (ignored: ExecutionException) {
      mSmartSocket.sendFailWithReason("Internal server failure while retrieving device list.")
      mKeepRunning = false
      return ArrayList()
  ...
  private fun findDeviceWithSerial(serial: String): Optional<DeviceState> {
    return try {
      ...
      devices.stream().filter { streamDevice: DeviceState -> serial == streamDevice.deviceId }.findAny()
  ...
  private fun findDeviceWithTransportId(transportId: String): Optional<DeviceState> {
    return try {
      val devices = mServer.deviceListCopy.get()
      devices.stream().filter { streamDevice: DeviceState -> transportId == streamDevice.transportId.toString() }.findAny()
    } catch (e: InterruptedException) {
      throw IllegalStateException(e)
    } catch (e: ExecutionException) {
      throw IllegalStateException(e)
...
/**
 * "Graceful"" shutdown of a socket: shutdown the output then read the input until EOF. Closing a socket this way ensures the socket peer
 * has received all the data sent from this side of the socket.
 */
