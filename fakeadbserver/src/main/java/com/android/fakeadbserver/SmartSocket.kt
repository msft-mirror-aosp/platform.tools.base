...
class SmartSocket internal constructor(private val mSocket: SocketChannel) : AutoCloseable {
  ...
  private fun readFully(buffer: ByteArray) {
    ...
    while (bytesRead < buffer.size) {
      bytesRead += mSocket.socket().getInputStream().read(buffer, bytesRead, buffer.size - bytesRead)
    }
  }
  ...
  fun sendFailWithReason(reason: String) {
    try {
      ...
      stream.write(String.format("%04x", reason.length).toByteArray(StandardCharsets.US_ASCII))
